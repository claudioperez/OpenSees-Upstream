	...
	OPS_ADD_RUNTIME_VPV(OPS_TDConcreteEXP) {
		...
		// Parse the input line for the material parameters:
			...
			if (numArgs == 14) {
				...
				theMaterial = new TDConcreteEXP(iData,dData[0],dData[1],dData[2],dData[3],dData[4],dData[5],dData[6],dData[7],dData[8],dData[9],dData[10],dData[11],dData[12]);
                if (theMaterial == 0) {
					opserr << "WARNING: could not create uniaxialMaterial of type TDConcreteEXP \n";
					return 0;
				}
...
TDConcreteEXP::TDConcreteEXP(int tag, double _fc, double _ft, double _Ec, double _beta, double _age, double _epsshu, double _epssha, double _tcr, double _epscru, double _sigCr, double _epscra, double _epscrd, double _tcast): 
  UniaxialMaterial(tag, MAT_TAG_TDConcreteEXP),
...
TDConcreteEXP::TDConcreteEXP(void):
  UniaxialMaterial(0, MAT_TAG_TDConcreteEXP)
{
...
{
  TDConcreteEXP *theCopy = new TDConcreteEXP(this->getTag(), fc, ft, Ec, beta, age, epsshu, epssha, tcr, epscru,sigCr, epscra, epscrd, tcast); 
...
TDConcreteEXP::setTrialStrain(double trialStrain, double strainRate)
{
    ...
    */
    ...
    //If t = t(i-1), use creep/shrinkage from last calculated time step
    	...
    	} else { //Static Analysis using previously converged TD strains
        	    eps_cr = epsP_cr;
            	eps_sh = epsP_sh;
            	eps_m = eps_total-eps_cr-eps_sh;
    	        sig = setStress(eps_m, e);
...
{
  s << "TDConcreteEXP:(strain, stress, tangent) " << eps << " " << sig << " " << e << endln;
}
