...
void * OPS_ADD_RUNTIME_VPV(OPS_SFI_MVLEM_3D)
{
	...
	if (numArgs < 14) {
		opserr << "Want: element SFI_MVLEM_3D eleTag iNode jNode kNode lNode m -thick {Thicknesses} -width {Widths} -mat {Material_tags} <-CoR c> <-ThickMod tMod> <-Poisson Nu> <-Density Dens>\n";
		return 0;
	}
	...
	theElement = new SFI_MVLEM_3D(iData[0], dData[3],
	...
	nd1Crds(3), nd2Crds(3), nd3Crds(3), nd4Crds(3), modifiedT(0), t(0)
{
	...
	if (externalNodes.Size() != 4 + m)
		opserr << "FATAL SFI_MVLEM_3D::SFI_MVLEM_3D() - out of memory, could not create an ID of size 2+m\n";
	...
	// Panel strains
	SFI_MVLEM_3DStrainX = new double[m];
	SFI_MVLEM_3DStrainY = new double[m];
	SFI_MVLEM_3DStrainXY = new double[m];
	SFI_MVLEM_3DStrain = new double[3 * m];
	...
	for (int i = 0; i < m; i++) {
		...
		SFI_MVLEM_3DStrainX[i] = 0.0;
		SFI_MVLEM_3DStrainY[i] = 0.0;
		SFI_MVLEM_3DStrainXY[i] = 0.0;

		SFI_MVLEM_3DStrain[i] = 0.0;
		SFI_MVLEM_3DStrain[i + m] = 0.0;
		SFI_MVLEM_3DStrain[i + 2 * m] = 0.0;
	}
...
SFI_MVLEM_3D::SFI_MVLEM_3D()
	:Element(0, ELE_TAG_SFI_MVLEM_3D),
	externalNodes(4 + m),
	theNodesX(0),
	theNodesALL(0),
	theMaterial(0), theLoad(0),
...
SFI_MVLEM_3D::~SFI_MVLEM_3D()
{
	...
	// memory the SFI_MVLEM_3D objects allocates and memory allocated 
	// by other objects that the SFI_MVLEM_3D object is responsible for 
...
int SFI_MVLEM_3D::getNumExternalNodes(void) const
{
	return 4 + m;
}

// Get node tags
const ID & SFI_MVLEM_3D::getExternalNodes(void)
{
	return externalNodes;
}

// Get node pointers
Node ** SFI_MVLEM_3D::getNodePtrs(void)
{
...
// Get number of DOFs 
int SFI_MVLEM_3D::getNumDOF(void) {
...
// Set Domain
void SFI_MVLEM_3D::setDomain(Domain *theDomain)
{
	...
	for (int i = 0; i < m; i++) {
		...
		Vector Dummy_GLcs(3); Dummy_GLcs.Zero();
	...
	if ((dofNd1 != 6) || (dofNd2 != 6) || (dofNd3 != 6) || (dofNd4 != 6)) {
		opserr << "SFI_MVLEM_3D::setDomain(): 6 dof required at all nodes. " << dofNd1 << " provided at node 1, " << dofNd2 << " provided at node 2, "
	...
	for (int i = 0; i < m; i++) {
		...
		{
			opserr << "SFI_MVLEM_3D::setDomain(): 1 dof required at internal nodes, " << dofNdXi << " provided\n";
		}
	}
	...
	if (theLoad == 0) {
		opserr << "SFI_MVLEM_3D::setDomain() - element: " << this->getTag()
...
// Revert to start
int SFI_MVLEM_3D::revertToStart()
{
	int errCode = 0;
...
// Update state
int SFI_MVLEM_3D::update()
{
	// Get the current strain given trial displacements at nodes 
	//SFI_MVLEM_3DStrain = this->computeCurrentStrain();
	...
	for (int i = 0; i < m; i++) {
		...
		strain(0) = SFI_MVLEM_3DStrain[i];
		strain(1) = SFI_MVLEM_3DStrain[i + m];
		strain(2) = SFI_MVLEM_3DStrain[i + 2 * m];
...
double *SFI_MVLEM_3D::computeCurrentStrain(void)
{
	...
	for (int i = 0; i < m; i++) {
		SFI_MVLEM_3DStrainX[i] = Dx[i] / b[i];
		SFI_MVLEM_3DStrainY[i] = Dy[i] / h;
		SFI_MVLEM_3DStrainXY[i] = -Dxy[i] / h;
	}

	// Store strains into a single vector
	for (int i = 0; i < m; i++) {
		SFI_MVLEM_3DStrain[i] = SFI_MVLEM_3DStrainX[i];
		SFI_MVLEM_3DStrain[i + m] = SFI_MVLEM_3DStrainY[i];
		SFI_MVLEM_3DStrain[i + 2 * m] = SFI_MVLEM_3DStrainXY[i];
	}

	// Return strain vector
	return SFI_MVLEM_3DStrain;
...
const Matrix & SFI_MVLEM_3D::getInitialStiff(void)
{
	...
	SFI_MVLEM_3DK.Zero();		// Global stiffness matrix
	...
	SFI_MVLEM_3DKlocal(7, 3) = SFI_MVLEM_3DKlocal(3, 7);
	...
	SFI_MVLEM_3DKlocal(8, 3) = SFI_MVLEM_3DKlocal(3, 8);
	SFI_MVLEM_3DKlocal(8, 4) = SFI_MVLEM_3DKlocal(4, 8);
	SFI_MVLEM_3DKlocal(8, 5) = SFI_MVLEM_3DKlocal(5, 8);
	SFI_MVLEM_3DKlocal(8, 6) = SFI_MVLEM_3DKlocal(6, 8);
	SFI_MVLEM_3DKlocal(8, 7) = SFI_MVLEM_3DKlocal(7, 8);
	...
	SFI_MVLEM_3DKlocal(9, 4) = SFI_MVLEM_3DKlocal(4, 9);
	SFI_MVLEM_3DKlocal(9, 5) = SFI_MVLEM_3DKlocal(5, 9);
	SFI_MVLEM_3DKlocal(9, 6) = SFI_MVLEM_3DKlocal(6, 9);
	SFI_MVLEM_3DKlocal(9, 7) = SFI_MVLEM_3DKlocal(7, 9);
	...
	SFI_MVLEM_3DKlocal(10, 3) = SFI_MVLEM_3DKlocal(3, 10);
	SFI_MVLEM_3DKlocal(10, 4) = SFI_MVLEM_3DKlocal(4, 10);
	SFI_MVLEM_3DKlocal(10, 5) = SFI_MVLEM_3DKlocal(5, 10);
	SFI_MVLEM_3DKlocal(10, 6) = SFI_MVLEM_3DKlocal(6, 10);
	SFI_MVLEM_3DKlocal(10, 7) = SFI_MVLEM_3DKlocal(7, 10);
	...
	SFI_MVLEM_3DKlocal(11, 3) = SFI_MVLEM_3DKlocal(3, 11);
	SFI_MVLEM_3DKlocal(11, 4) = SFI_MVLEM_3DKlocal(4, 11);
	SFI_MVLEM_3DKlocal(11, 5) = SFI_MVLEM_3DKlocal(5, 11);
	SFI_MVLEM_3DKlocal(11, 6) = SFI_MVLEM_3DKlocal(6, 11);
	SFI_MVLEM_3DKlocal(11, 7) = SFI_MVLEM_3DKlocal(7, 11);
	...
	SFI_MVLEM_3DKlocal(12, 3) = SFI_MVLEM_3DKlocal(3, 12);
	SFI_MVLEM_3DKlocal(12, 4) = SFI_MVLEM_3DKlocal(4, 12);
	SFI_MVLEM_3DKlocal(12, 5) = SFI_MVLEM_3DKlocal(5, 12);
	SFI_MVLEM_3DKlocal(12, 6) = SFI_MVLEM_3DKlocal(6, 12);
	SFI_MVLEM_3DKlocal(12, 7) = SFI_MVLEM_3DKlocal(7, 12);
	SFI_MVLEM_3DKlocal(12, 8) = SFI_MVLEM_3DKlocal(8, 12);
	...
	SFI_MVLEM_3DKlocal(13, 3) = SFI_MVLEM_3DKlocal(3, 13);
	SFI_MVLEM_3DKlocal(13, 4) = SFI_MVLEM_3DKlocal(4, 13);
	SFI_MVLEM_3DKlocal(13, 5) = SFI_MVLEM_3DKlocal(5, 13);
	SFI_MVLEM_3DKlocal(13, 6) = SFI_MVLEM_3DKlocal(6, 13);
	SFI_MVLEM_3DKlocal(13, 7) = SFI_MVLEM_3DKlocal(7, 13);
	SFI_MVLEM_3DKlocal(13, 8) = SFI_MVLEM_3DKlocal(8, 13);
	SFI_MVLEM_3DKlocal(13, 9) = SFI_MVLEM_3DKlocal(9, 13);
	...
	SFI_MVLEM_3DKlocal(14, 3) = SFI_MVLEM_3DKlocal(3, 14);
	SFI_MVLEM_3DKlocal(14, 4) = SFI_MVLEM_3DKlocal(4, 14);
	SFI_MVLEM_3DKlocal(14, 5) = SFI_MVLEM_3DKlocal(5, 14);
	SFI_MVLEM_3DKlocal(14, 6) = SFI_MVLEM_3DKlocal(6, 14);
	SFI_MVLEM_3DKlocal(14, 7) = SFI_MVLEM_3DKlocal(7, 14);
	SFI_MVLEM_3DKlocal(14, 8) = SFI_MVLEM_3DKlocal(8, 14);
	SFI_MVLEM_3DKlocal(14, 9) = SFI_MVLEM_3DKlocal(9, 14);
	SFI_MVLEM_3DKlocal(14, 10) = SFI_MVLEM_3DKlocal(10, 14);
	SFI_MVLEM_3DKlocal(14, 11) = SFI_MVLEM_3DKlocal(11, 14);
	SFI_MVLEM_3DKlocal(14, 12) = SFI_MVLEM_3DKlocal(12, 14);
	SFI_MVLEM_3DKlocal(14, 13) = SFI_MVLEM_3DKlocal(13, 14);
	...
	SFI_MVLEM_3DKlocal(15, 3) = SFI_MVLEM_3DKlocal(3, 15);
	SFI_MVLEM_3DKlocal(15, 4) = SFI_MVLEM_3DKlocal(4, 15);
	SFI_MVLEM_3DKlocal(15, 5) = SFI_MVLEM_3DKlocal(5, 15);
	SFI_MVLEM_3DKlocal(15, 6) = SFI_MVLEM_3DKlocal(6, 15);
	SFI_MVLEM_3DKlocal(15, 7) = SFI_MVLEM_3DKlocal(7, 15);
	SFI_MVLEM_3DKlocal(15, 8) = SFI_MVLEM_3DKlocal(8, 15);
	SFI_MVLEM_3DKlocal(15, 9) = SFI_MVLEM_3DKlocal(9, 15);
	SFI_MVLEM_3DKlocal(15, 10) = SFI_MVLEM_3DKlocal(10, 15);
	SFI_MVLEM_3DKlocal(15, 11) = SFI_MVLEM_3DKlocal(11, 15);
	SFI_MVLEM_3DKlocal(15, 12) = SFI_MVLEM_3DKlocal(12, 15);
	SFI_MVLEM_3DKlocal(15, 13) = SFI_MVLEM_3DKlocal(13, 15);
	...
	SFI_MVLEM_3DKlocal(16, 3) = SFI_MVLEM_3DKlocal(3, 16);
	SFI_MVLEM_3DKlocal(16, 4) = SFI_MVLEM_3DKlocal(4, 16);
	SFI_MVLEM_3DKlocal(16, 5) = SFI_MVLEM_3DKlocal(5, 16);
	SFI_MVLEM_3DKlocal(16, 6) = SFI_MVLEM_3DKlocal(6, 16);
	SFI_MVLEM_3DKlocal(16, 7) = SFI_MVLEM_3DKlocal(7, 16);
	SFI_MVLEM_3DKlocal(16, 8) = SFI_MVLEM_3DKlocal(8, 16);
	SFI_MVLEM_3DKlocal(16, 9) = SFI_MVLEM_3DKlocal(9, 16);
	SFI_MVLEM_3DKlocal(16, 10) = SFI_MVLEM_3DKlocal(10, 16);
	SFI_MVLEM_3DKlocal(16, 11) = SFI_MVLEM_3DKlocal(11, 16);
	SFI_MVLEM_3DKlocal(16, 12) = SFI_MVLEM_3DKlocal(12, 16);
	SFI_MVLEM_3DKlocal(16, 13) = SFI_MVLEM_3DKlocal(13, 16);
	...
	SFI_MVLEM_3DKlocal(17, 3) = SFI_MVLEM_3DKlocal(3, 17);
	SFI_MVLEM_3DKlocal(17, 4) = SFI_MVLEM_3DKlocal(4, 17);
	SFI_MVLEM_3DKlocal(17, 5) = SFI_MVLEM_3DKlocal(5, 17);
	SFI_MVLEM_3DKlocal(17, 6) = SFI_MVLEM_3DKlocal(6, 17);
	SFI_MVLEM_3DKlocal(17, 7) = SFI_MVLEM_3DKlocal(7, 17);
	SFI_MVLEM_3DKlocal(17, 8) = SFI_MVLEM_3DKlocal(8, 17);
	SFI_MVLEM_3DKlocal(17, 9) = SFI_MVLEM_3DKlocal(9, 17);
	SFI_MVLEM_3DKlocal(17, 10) = SFI_MVLEM_3DKlocal(10, 17);
	SFI_MVLEM_3DKlocal(17, 11) = SFI_MVLEM_3DKlocal(11, 17);
	SFI_MVLEM_3DKlocal(17, 12) = SFI_MVLEM_3DKlocal(12, 17);
	SFI_MVLEM_3DKlocal(17, 13) = SFI_MVLEM_3DKlocal(13, 17);
	...
	SFI_MVLEM_3DKlocal(18, 3) = SFI_MVLEM_3DKlocal(3, 18);
	SFI_MVLEM_3DKlocal(18, 4) = SFI_MVLEM_3DKlocal(4, 18);
	SFI_MVLEM_3DKlocal(18, 5) = SFI_MVLEM_3DKlocal(5, 18);
	SFI_MVLEM_3DKlocal(18, 6) = SFI_MVLEM_3DKlocal(6, 18);
	SFI_MVLEM_3DKlocal(18, 7) = SFI_MVLEM_3DKlocal(7, 18);
	SFI_MVLEM_3DKlocal(18, 8) = SFI_MVLEM_3DKlocal(8, 18);
	SFI_MVLEM_3DKlocal(18, 9) = SFI_MVLEM_3DKlocal(9, 18);
	SFI_MVLEM_3DKlocal(18, 10) = SFI_MVLEM_3DKlocal(10, 18);
	SFI_MVLEM_3DKlocal(18, 11) = SFI_MVLEM_3DKlocal(11, 18);
	SFI_MVLEM_3DKlocal(18, 12) = SFI_MVLEM_3DKlocal(12, 18);
	SFI_MVLEM_3DKlocal(18, 13) = SFI_MVLEM_3DKlocal(13, 18);
	SFI_MVLEM_3DKlocal(18, 14) = SFI_MVLEM_3DKlocal(14, 18);
	...
	SFI_MVLEM_3DKlocal(19, 3) = SFI_MVLEM_3DKlocal(3, 19);
	SFI_MVLEM_3DKlocal(19, 4) = SFI_MVLEM_3DKlocal(4, 19);
	SFI_MVLEM_3DKlocal(19, 5) = SFI_MVLEM_3DKlocal(5, 19);
	SFI_MVLEM_3DKlocal(19, 6) = SFI_MVLEM_3DKlocal(6, 19);
	SFI_MVLEM_3DKlocal(19, 7) = SFI_MVLEM_3DKlocal(7, 19);
	SFI_MVLEM_3DKlocal(19, 8) = SFI_MVLEM_3DKlocal(8, 19);
	SFI_MVLEM_3DKlocal(19, 9) = SFI_MVLEM_3DKlocal(9, 19);
	SFI_MVLEM_3DKlocal(19, 10) = SFI_MVLEM_3DKlocal(10, 19);
	SFI_MVLEM_3DKlocal(19, 11) = SFI_MVLEM_3DKlocal(11, 19);
	SFI_MVLEM_3DKlocal(19, 12) = SFI_MVLEM_3DKlocal(12, 19);
	SFI_MVLEM_3DKlocal(19, 13) = SFI_MVLEM_3DKlocal(13, 19);
	SFI_MVLEM_3DKlocal(19, 14) = SFI_MVLEM_3DKlocal(14, 19);
	SFI_MVLEM_3DKlocal(19, 15) = SFI_MVLEM_3DKlocal(15, 19);
	...
	SFI_MVLEM_3DKlocal(20, 3) = SFI_MVLEM_3DKlocal(3, 20);
	SFI_MVLEM_3DKlocal(20, 4) = SFI_MVLEM_3DKlocal(4, 20);
	SFI_MVLEM_3DKlocal(20, 5) = SFI_MVLEM_3DKlocal(5, 20);
	SFI_MVLEM_3DKlocal(20, 6) = SFI_MVLEM_3DKlocal(6, 20);
	SFI_MVLEM_3DKlocal(20, 7) = SFI_MVLEM_3DKlocal(7, 20);
	SFI_MVLEM_3DKlocal(20, 8) = SFI_MVLEM_3DKlocal(8, 20);
	SFI_MVLEM_3DKlocal(20, 9) = SFI_MVLEM_3DKlocal(9, 20);
	SFI_MVLEM_3DKlocal(20, 10) = SFI_MVLEM_3DKlocal(10, 20);
	SFI_MVLEM_3DKlocal(20, 11) = SFI_MVLEM_3DKlocal(11, 20);
	SFI_MVLEM_3DKlocal(20, 12) = SFI_MVLEM_3DKlocal(12, 20);
	SFI_MVLEM_3DKlocal(20, 13) = SFI_MVLEM_3DKlocal(13, 20);
	SFI_MVLEM_3DKlocal(20, 14) = SFI_MVLEM_3DKlocal(14, 20);
	SFI_MVLEM_3DKlocal(20, 15) = SFI_MVLEM_3DKlocal(15, 20);
	SFI_MVLEM_3DKlocal(20, 16) = SFI_MVLEM_3DKlocal(16, 20);
	SFI_MVLEM_3DKlocal(20, 17) = SFI_MVLEM_3DKlocal(17, 20);
	SFI_MVLEM_3DKlocal(20, 18) = SFI_MVLEM_3DKlocal(18, 20);
	SFI_MVLEM_3DKlocal(20, 19) = SFI_MVLEM_3DKlocal(19, 20);
	...
	SFI_MVLEM_3DKlocal(21, 3) = SFI_MVLEM_3DKlocal(3, 21);
	SFI_MVLEM_3DKlocal(21, 4) = SFI_MVLEM_3DKlocal(4, 21);
	SFI_MVLEM_3DKlocal(21, 5) = SFI_MVLEM_3DKlocal(5, 21);
	SFI_MVLEM_3DKlocal(21, 6) = SFI_MVLEM_3DKlocal(6, 21);
	SFI_MVLEM_3DKlocal(21, 7) = SFI_MVLEM_3DKlocal(7, 21);
	SFI_MVLEM_3DKlocal(21, 8) = SFI_MVLEM_3DKlocal(8, 21);
	SFI_MVLEM_3DKlocal(21, 9) = SFI_MVLEM_3DKlocal(9, 21);
	SFI_MVLEM_3DKlocal(21, 10) = SFI_MVLEM_3DKlocal(10, 21);
	SFI_MVLEM_3DKlocal(21, 11) = SFI_MVLEM_3DKlocal(11, 21);
	SFI_MVLEM_3DKlocal(21, 12) = SFI_MVLEM_3DKlocal(12, 21);
	SFI_MVLEM_3DKlocal(21, 13) = SFI_MVLEM_3DKlocal(13, 21);
	SFI_MVLEM_3DKlocal(21, 14) = SFI_MVLEM_3DKlocal(14, 21);
	SFI_MVLEM_3DKlocal(21, 15) = SFI_MVLEM_3DKlocal(15, 21);
	SFI_MVLEM_3DKlocal(21, 16) = SFI_MVLEM_3DKlocal(16, 21);
	SFI_MVLEM_3DKlocal(21, 17) = SFI_MVLEM_3DKlocal(17, 21);
	SFI_MVLEM_3DKlocal(21, 18) = SFI_MVLEM_3DKlocal(18, 21);
	SFI_MVLEM_3DKlocal(21, 19) = SFI_MVLEM_3DKlocal(19, 21);
	...
	SFI_MVLEM_3DKlocal(22, 3) = SFI_MVLEM_3DKlocal(3, 22);
	SFI_MVLEM_3DKlocal(22, 4) = SFI_MVLEM_3DKlocal(4, 22);
	SFI_MVLEM_3DKlocal(22, 5) = SFI_MVLEM_3DKlocal(5, 22);
	SFI_MVLEM_3DKlocal(22, 6) = SFI_MVLEM_3DKlocal(6, 22);
	SFI_MVLEM_3DKlocal(22, 7) = SFI_MVLEM_3DKlocal(7, 22);
	SFI_MVLEM_3DKlocal(22, 8) = SFI_MVLEM_3DKlocal(8, 22);
	SFI_MVLEM_3DKlocal(22, 9) = SFI_MVLEM_3DKlocal(9, 22);
	SFI_MVLEM_3DKlocal(22, 10) = SFI_MVLEM_3DKlocal(10, 22);
	SFI_MVLEM_3DKlocal(22, 11) = SFI_MVLEM_3DKlocal(11, 22);
	SFI_MVLEM_3DKlocal(22, 12) = SFI_MVLEM_3DKlocal(12, 22);
	SFI_MVLEM_3DKlocal(22, 13) = SFI_MVLEM_3DKlocal(13, 22);
	SFI_MVLEM_3DKlocal(22, 14) = SFI_MVLEM_3DKlocal(14, 22);
	SFI_MVLEM_3DKlocal(22, 15) = SFI_MVLEM_3DKlocal(15, 22);
	SFI_MVLEM_3DKlocal(22, 16) = SFI_MVLEM_3DKlocal(16, 22);
	SFI_MVLEM_3DKlocal(22, 17) = SFI_MVLEM_3DKlocal(17, 22);
	SFI_MVLEM_3DKlocal(22, 18) = SFI_MVLEM_3DKlocal(18, 22);
	SFI_MVLEM_3DKlocal(22, 19) = SFI_MVLEM_3DKlocal(19, 22);
	...
	SFI_MVLEM_3DKlocal(23, 3) = SFI_MVLEM_3DKlocal(3, 23);
	SFI_MVLEM_3DKlocal(23, 4) = SFI_MVLEM_3DKlocal(4, 23);
	SFI_MVLEM_3DKlocal(23, 5) = SFI_MVLEM_3DKlocal(5, 23);
	SFI_MVLEM_3DKlocal(23, 6) = SFI_MVLEM_3DKlocal(6, 23);
	SFI_MVLEM_3DKlocal(23, 7) = SFI_MVLEM_3DKlocal(7, 23);
	SFI_MVLEM_3DKlocal(23, 8) = SFI_MVLEM_3DKlocal(8, 23);
	SFI_MVLEM_3DKlocal(23, 9) = SFI_MVLEM_3DKlocal(9, 23);
	SFI_MVLEM_3DKlocal(23, 10) = SFI_MVLEM_3DKlocal(10, 23);
	SFI_MVLEM_3DKlocal(23, 11) = SFI_MVLEM_3DKlocal(11, 23);
	SFI_MVLEM_3DKlocal(23, 12) = SFI_MVLEM_3DKlocal(12, 23);
	SFI_MVLEM_3DKlocal(23, 13) = SFI_MVLEM_3DKlocal(13, 23);
	SFI_MVLEM_3DKlocal(23, 14) = SFI_MVLEM_3DKlocal(14, 23);
	SFI_MVLEM_3DKlocal(23, 15) = SFI_MVLEM_3DKlocal(15, 23);
	SFI_MVLEM_3DKlocal(23, 16) = SFI_MVLEM_3DKlocal(16, 23);
	SFI_MVLEM_3DKlocal(23, 17) = SFI_MVLEM_3DKlocal(17, 23);
	SFI_MVLEM_3DKlocal(23, 18) = SFI_MVLEM_3DKlocal(18, 23);
	SFI_MVLEM_3DKlocal(23, 19) = SFI_MVLEM_3DKlocal(19, 23);
...
const Matrix & SFI_MVLEM_3D::getMass(void)
{

	SFI_MVLEM_3DM.Zero();
	SFI_MVLEM_3DMlocal.Zero();

	// No rotational mass
	SFI_MVLEM_3DMlocal(0, 0) = NodeMass;
	SFI_MVLEM_3DMlocal(1, 1) = NodeMass;
	SFI_MVLEM_3DMlocal(2, 2) = NodeMass;

	SFI_MVLEM_3DMlocal(6, 6) = NodeMass;
	SFI_MVLEM_3DMlocal(7, 7) = NodeMass;
	SFI_MVLEM_3DMlocal(8, 8) = NodeMass;

	SFI_MVLEM_3DMlocal(12, 12) = NodeMass;
	SFI_MVLEM_3DMlocal(13, 13) = NodeMass;
	SFI_MVLEM_3DMlocal(14, 14) = NodeMass;

	SFI_MVLEM_3DMlocal(18, 18) = NodeMass;
	SFI_MVLEM_3DMlocal(19, 19) = NodeMass;
	SFI_MVLEM_3DMlocal(20, 20) = NodeMass;

	// Convert matrix from local to global cs
	SFI_MVLEM_3DM.addMatrixTripleProduct(0.0, T, SFI_MVLEM_3DMlocal, 1.0); 

	// Return element mass matrix
	return SFI_MVLEM_3DM;
}

// Get element damping matrix
const Matrix & SFI_MVLEM_3D::getDamp(void)
{
	SFI_MVLEM_3DD.Zero();

	SFI_MVLEM_3DD = this->Element::getDamp();

	// Return element damping matrix
	return SFI_MVLEM_3DD;
}

// N/A to this model - no element loads
void SFI_MVLEM_3D::zeroLoad(void)
{
	// does nothing - no elemental loads
}

// N/A to this model - no element loads
int SFI_MVLEM_3D::addLoad(ElementalLoad *theLoad, double loadFactor)
{
	return 0;
}


int SFI_MVLEM_3D::addInertiaLoadToUnbalance(const Vector &accel)
{
	if (density == 0.0)
		return 0;
	...
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 3; j++) {
			SFI_MVLEM_3DRlocal(6 * i + j) += - SFI_MVLEM_3DMlocal(6 * i + j, 6 * i + j) * RaccelL(6 * i + j);
		}
	}

	// Transform forces from local to global cs
	SFI_MVLEM_3DR.addMatrixTransposeVector(1.0, T, SFI_MVLEM_3DRlocal, 1.0);

	return 0;
}

// Get element force vector
const Vector & SFI_MVLEM_3D::getResistingForce()
{

	SFI_MVLEM_3DR.Zero();
	SFI_MVLEM_3DRlocal.Zero();
	...
	for (int i = 0; i < m; i++)
	{
		Fh += -1.0*Fxy[i];
		Fysum += Fy[i];
		SFI_MVLEM_3DRlocal[24 + i] = Fx[i]; // Force on internal (dummy) DOFs
	}
...
const Vector & SFI_MVLEM_3D::getResistingForceIncInertia()
{
	...
	if (density == 0.0) {
		...
		if (alphaM != 0.0 || betaK != 0.0 || betaK0 != 0.0 || betaKc != 0.0)
			SFI_MVLEM_3DR += this->getRayleighDampingForces();
	...
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 3; j++) {
			SFI_MVLEM_3DRlocal(6 * i + j) += SFI_MVLEM_3DMlocal(6 * i + j, 6 * i + j) * accelL(6 * i + j);
		}
	}
...
// Send Self
int SFI_MVLEM_3D::sendSelf(int commitTag, Channel &theChannel)
{
	int res;
	...
	if (res < 0) {
		opserr << "WARNING SFI_MVLEM_3D::sendSelf() - failed to send ID\n";
		return -2;
	}
...
// Receive Self
int SFI_MVLEM_3D::recvSelf(int commitTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{
	int res;
	int dataTag = this->getDbTag();

	// SFI_MVLEM_3D creates a Vector, receives the Vector and then sets the 
	...
	if (res < 0) {
		opserr << "WARNING SFI_MVLEM_3D::recvSelf() - failed to receive ID\n";
		return -2;
	}
...
// Display model
int SFI_MVLEM_3D::displaySelf(Renderer& theViewer, int displayMode, float fact, const char** modes, int numMode)
{
...
void SFI_MVLEM_3D::Print(OPS_Stream &s, int flag)
{
	if (flag == 0) {
		s << "SFI_MVLEM_3D Element tag: " << this->getTag() << endln;
...
double SFI_MVLEM_3D::getShearDef(void)
{
	return Dsh;
}

// Get curvature (from vertical strains)
double SFI_MVLEM_3D::getCurvature(void)
{
	double Curv;

	Curv = (SFI_MVLEM_3DStrainY[0] - SFI_MVLEM_3DStrainY[m - 1]) / (x[0] - x[m - 1]);

	return Curv;
}

// Get global forces at 24 DOFs (top and bottom node)
Vector SFI_MVLEM_3D::getResistingForce_24DOF(void)
{

	for (int i = 0; i < 24; i++) {
		P_24DOF(i) = SFI_MVLEM_3DR(i);
	}
...
int SFI_MVLEM_3D::getResponse(int responseID, Information &eleInfo)
{
...
Vector SFI_MVLEM_3D::getResistingForce_24DOF_local(void)
{
	for (int i = 0; i < 24; i++) {
		P_24DOF_local(i) = SFI_MVLEM_3DRlocal(i);
	}
...
void  SFI_MVLEM_3D::setTransformationMatrix(void) {
