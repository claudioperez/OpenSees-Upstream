...
void *
OPS_ShellDKGQ(const ID& info)
{

    if (info.Size() == 0) {
	opserr << "WARNING: info is empty -- ShellDKGQ\n";
	return 0;
    }
	...
	if (theSection == 0) {
	    opserr << "ERROR:  element ShellDKGQ " << info(2) << "section " << secTag << " not found\n";
	    return 0;
	}
	...
	return new ShellDKGQ(info(2), info(3), info(4), info(5),
			     info(6), *theSection);
    }
