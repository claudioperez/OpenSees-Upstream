...
myIter (*this)
{
    ...
    if (theComponents == 0)
      {
          opserr <<
              "ArrayOfTaggedObjects::ArrayOfTaggedObjects - failed to allocate an array of size "
              << size << endln;
      }
    else
...
ArrayOfTaggedObjects::setSize (int newSize)
{
    ...
    if (newSize < 0 && newSize > sizeComponentArray)
      {
          opserr << "ArrayOfTaggedObjects::setSize - invalid size " << newSize
              << endln;
          return -1;
      }
    ...
    if (newArray == 0)
      {
          ...
          opserr <<
              "ArrayOfTaggedObjects::setSize - failed to allocate an array of size "
              << newSize << endln;
          return -2;
      }
                  ...
                  if (this->addComponent (oldArray[j]) == false)
                    {
                        ...
                        opserr <<
                            "SERIOUS ERROR: ArrayOfTaggedObjects::setSize() - we have lost a component with tag: "
...
//    }
    ...
    if (numComponents == sizeComponentArray)
        if (this->setSize (2 * numComponents) < 0)
          {
              opserr <<
                  "ArrayOfTaggedObjects::addComponent()- failed to enlarge the array with size"
    ...
    if (positionLastNoFitEntry == sizeComponentArray)
      {
          opserr <<
              "ArrayOfTaggedObjects::addComponent() - could not - find a vacant spot after enlarging!!\n";
          return false;
      }
...
TaggedObjectIter & ArrayOfTaggedObjects::getComponents ()
{
...
{
    ArrayOfTaggedObjects *theCopy =
        new ArrayOfTaggedObjects (sizeComponentArray);
