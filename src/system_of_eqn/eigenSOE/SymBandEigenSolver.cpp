...
SymBandEigenSolver::SymBandEigenSolver ():EigenSolver (EigenSOLVER_TAGS_SymBandEigenSolver),
theSOE (0), numModes (0), eigenvalue (0), eigenvector (0),
eigenV (0)
{
...
SymBandEigenSolver::solve (int nModes, bool generalized, bool findSmallest)
{
    ...
    if (generalized == true)
      {
          opserr <<
              "SymBandEigenSolver::solve() - only does standard problem\n";
          return -1;
      }
    ...
    if (theSOE == 0)
      {
          opserr <<
              "SymBandEigenSolver::solve() -- no EigenSOE has been set yet\n";
          return -1;
      }
                ...
                if (M[k] == 0.0)
                  {
                      ...
                      opserr <<
                          "SymBandEigenSolver::solve() - M matrix singular\n";
                      return -1;
                  }
                else
                  {
    ...
    if (info < 0)
      {
          opserr << "SymBandEigenSolver::solve() -- invalid argument number "
              << -info << " passed to LAPACK dsbevx\n";
          return info;
      }
    ...
    if (info > 0)
      {
          opserr <<
              "SymBandEigenSolver::solve() -- LAPACK dsbevx returned error code "
              << info << endln;
          return -info;
      }
    ...
    if (m < numModes)
      {
          opserr <<
              "SymBandEigenSolver::solve() -- LAPACK dsbevx only computed " <<
              m << " eigenvalues, " << numModes << "were requested\n";
...
SymBandEigenSolver::getEigenvector (int mode)
{
    if (mode < 1 || mode > numModes)
      {
          opserr << "SymBandEigenSolver::getEigenVector() -- mode " << mode <<
    ...
    if (eigenvector != 0)
      ...
      {
          opserr <<
              "SymBandEigenSolver::getEigenVector() -- eigenvectors not yet computed\n";
          eigenV->Zero ();
      }
...
SymBandEigenSolver::getEigenvalue (int mode)
{
    if (mode < 1 || mode > numModes)
      {
          opserr << "SymBandEigenSolver::getEigenvalue() -- mode " << mode <<
              " is out of range (1 - " << numModes << ")\n";
    ...
    if (eigenvalue != 0)
      ...
      {
          opserr <<
              "SymBandEigenSolver::getEigenvalue() -- eigenvalues not yet computed\n";
          return 0.0;
      }
}
    ...
    if (eigenV == 0 || eigenV->Size () != size)
      {
          ...
          if (eigenV == 0 || eigenV->Size () != size)
            {
                opserr <<
                    "SymBandEigenSolver::ssetSize() -- ran out of memory for eigenvector of size "
                    << size << endln;
                return -2;
            }
      }
