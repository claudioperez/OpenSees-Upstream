...
extern "C" int dseupd_ (bool *rvec, char *howmny, logical * select, double *d,
...
BandArpackSolver::solve (int numModes, bool generalized, bool findSmallest)
{
    if (generalized == false)
      {
          opserr <<
              "BandArpackSolver::solve(int numMode, bool generalized) - only solves generalized problem\n";
          return -1;
      }
          ...
          if (ido == -1)
            {
                ...
                if (ierr != 0)
                  {
                      opserr << "BandArpackSolver::Error with dgbtrs_ 1" <<
                          endln;
                      exit (0);
                  }
                continue;
          ...
          else if (ido == 1)
            {
                ...
                if (ierr != 0)
                  {
                      opserr << "BandArpackSolver::Error with dgbtrs_ 2" <<
                          endln;
                      exit (0);
                  }
                continue;
    ...
    if (info < 0)
      {
          opserr << "BandArpackSolver::Error with _saupd info = " << info <<
              endln;
          switch (info)
            {
    ...
    else
      {
          if (info == 1)
            ...
            {
                opserr <<
                    "BandArpackSolver::No Shifts could be applied during implicit,";
          ...
          if (iparam[4] > 0)
            {
                ...
                /*
                   ...
                   workl, &lworkl, &info);
                 */
                DSEUPD (&rvec, &howmy, select, d, z, &ldv, &sigma, &bmat, &n,
                        which, &nev, &tol, resid, &ncv, v, &ldv, iparam,
                        ipntr, workd, workl, &lworkl, &info);
#else
                dseupd_ (&rvec, &howmy, select, d, z, &ldv, &sigma, &bmat, &n,
                         which, &nev, &tol, resid, &ncv, v, &ldv, iparam,
                         ipntr, workd, workl, &lworkl, &info);
#endif
                if (info != 0)
                  {
                      opserr << "BandArpackSolver::Error with dseupd_" <<
                          info;
                      switch (info)
                        {
                        ...
                        case -11:
                            opserr <<
                                " IPARAM(7) = 1 and BMAT = 'G' are incompatibl\n";
                            break;
                        case -12:
                            opserr <<
...
BandArpackSolver::getEigenvector (int mode)
{
    if (mode <= 0 || mode > theNev)
      {
          opserr <<
              "BandArpackSOE::getEigenvector() - mode is out of range(1 - nev)";
          eigenV->Zero ();
          return *eigenV;
      }
    ...
    if (eigenvector != 0)
      ...
      {
          opserr <<
              "BandArpackSOE::getEigenvalue() - eigenvectors not yet determined";
          eigenV->Zero ();
      }
...
BandArpackSolver::getEigenvalue (int mode)
{
    if (mode <= 0 || mode > theNev)
      {
          opserr <<
              "BandArpackSOE::getEigenvalue() - mode is out of range(1 - nev)";
          return -1;
      }
    ...
    if (value != 0)
      ...
      {
          opserr <<
              "BandArpackSOE::getEigenvalue() - eigenvalues not yet determined";
          return -2;
      }
}
