...
FullGenEigenSOE::FullGenEigenSOE (FullGenEigenSolver & theSolver, AnalysisModel & aModel):EigenSOE (theSolver, EigenSOE_TAGS_FullGenEigenSOE),
...
int
FullGenEigenSOE::getNumEqn (void) const
{
    return size;
}
...
int
FullGenEigenSOE::setSize (Graph & theGraph)
{
    int result = 0;
...
FullGenEigenSOE::addA (const Matrix & m, const ID & id, double fact)
{
    ...
    if (idSize != m.noRows () && idSize != m.noCols ())
      {
          opserr <<
              "FullGenEigenSOE::addA() - Matrix and ID not of similar sizes\n";
          return -1;
      }
...
int
FullGenEigenSOE::addM (const Matrix & m, const ID & id, double fact)
{
    // check for quick return 
    if (fact == 0.0)
        return 0;
    ...
    if (idSize != m.noRows () && idSize != m.noCols ())
      {
          opserr <<
              "FullGenEigenSOE::addM() - Matrix and ID not of similar sizes\n";
          return -1;
      }
