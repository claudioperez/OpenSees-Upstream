...
ArpackSOE::setSize (Graph & theGraph)
{
    ...
    if (size != Msize && size > 0)
      {
          ...
          if (M == 0)
            {
                opserr <<
                    "WARNING ArpackSOE::ArpackSOE : - out of memory creating memory for M\n";
                Msize = 0;
            }
          else
              Msize = size;
    ...
    if (theSolvr == 0)
      {
          opserr <<
              "ArpackSOE::setSize(Graph &theGraph) - no EigenSolver set\n";
          return -1;
      }
    ...
    if (solverOK < 0)
      {
          opserr <<
              "WARNING:ArpackSOE::setSize() -  solver failed setSize()\n";
          return solverOK;
      }
          ...
          if (found == false)
            {
                ...
                if (nextNumChannels == 0)
                  {
                      opserr <<
                          "ArpackSOE::sendSelf() - failed to allocate channel array of size: "
                ...
                if (localCol == 0)
                  {
                      opserr <<
                          "ArpackSOE::sendSelf() - failed to allocate id array of size: "
                          << nextNumChannels << endln;
                      return -1;
                  }
                for (int i = 0; i < numChannels; i++)
