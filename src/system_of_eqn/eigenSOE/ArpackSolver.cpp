...
ArpackSolver::ArpackSolver ():EigenSolver (EigenSOLVER_TAGS_ArpackSolver),
...
ArpackSolver::solve (int numModes, bool generalized, bool findSmallest)
{
    if (generalized == false)
      {
          opserr <<
              "ArpackSolver::solve() - at moment only solves generalized problem\n";
          return -1;
      }
    ...
    while (1)
      {
          ...
          if (theArpackSOE->checkSameInt (ido) != 1)
            {
                opserr <<
                    "ArpackSolver::solve - ido values not the same .. ido, processID: "
                    << ido << " " << processID << endln;
                return -1;
            }
    ...
    if (info < 0)
      {
          opserr << "ArpackSolver::Error with _saupd info = " << info <<
              endln;
          switch (info)
            {
            ...
            case -9999:
                ...
                opserr <<
                    "IPARAM(5) the size of the current Arnoldi factorization: is ";
                opserr << iparam[4];
    ...
    else
      {
          if (info == 1)
            ...
            {
                opserr <<
                    "ArpackSolver::No Shifts could be applied during implicit,";
          ...
          if (iparam[4] > 0)
            {
                ...
                DSEUPD (&rvec, &howmy, select, eigenvalues, eigenvectors,
                        &ldv, &sigma, &bmat, &n, which, &nev, &tol, resid,
                        &ncv, v, &ldv, iparam, ipntr, workd, workl, &lworkl,
                        &info);
#else
                dseupd_ (&rvec, &howmy, select, eigenvalues, eigenvectors,
                         &ldv, &sigma, &bmat, &n, which, &nev, &tol, resid,
                         &ncv, v, &ldv, iparam, ipntr, workd, workl, &lworkl,
                         &info);
#endif
...
ArpackSolver::getEigenvalue (int mode)
{
    if (mode <= 0 || mode > numMode)
      {
          opserr <<
              "ArpackSOE::getEigenvalue() - mode is out of range(1 - nev)";
          return -1;
      }
    ...
    if (eigenvalues != 0)
      ...
      {
          opserr <<
              "ArpackSOE::getEigenvalue() - eigenvalues not yet determined";
          return -2;
      }
}
