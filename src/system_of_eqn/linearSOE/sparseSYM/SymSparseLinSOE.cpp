 ...
 */
extern "C" int symFactorization (int *fxadj, int *adjncy, int neq,
                                 int LSPARSE, int **xblkMY, int **invpMY,
                                 int **rowblksMY, OFFDBLK *** begblkMY,
                                 OFFDBLK ** firstMY, double ***penvMY,
                                 double **diagMY);
...
SymSparseLinSOE::setSize (Graph & theGraph)
{
    ...
    nblks = symFactorization (rowStartA, colA, size, this->LSPARSE,
                              &xblk, &invp, &rowblks, &begblk, &first, &penv,
                              &diag);
