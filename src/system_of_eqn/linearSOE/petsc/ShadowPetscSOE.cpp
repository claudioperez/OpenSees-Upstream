...
ShadowPetscSOE::getX (void)
{
    ...
    CHKERRA (ierr);
    opserr << "ShadowPetscSOE::getX - low:high " << low << " " << high <<
        endln;
    ...
    for (i = 1; i < numProcessors; i++)
      {
          MPI_Status status;
          MPI_Recv (sendBuffer, 3, MPI_INT, i, tag, PETSC_COMM_WORLD,
                    &status);
          low = sendData[0];
          high = sendData[1];
          opserr << "ShadowPetscSOE::getX - low:high " << low << " " << high
          ...
          MPI_Recv (buffer, high - low, MPI_DOUBLE, i, tag, PETSC_COMM_WORLD,
                    &status);
      }
...
{
    opserr <<
        "WARNING ShadowPetscSOE::sendSelf - does not receive itself YET\n";
    return 0;
}
