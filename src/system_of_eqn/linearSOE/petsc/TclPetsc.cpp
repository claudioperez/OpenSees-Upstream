...
TclCommand_Petsc (ClientData clientData,
                  ...
                  int argc,
                  TCL_Char ** argv,
                  FEM_ObjectBroker * theBroker, LinearSOE ** theSOE)
{
    ...
    if (matType == 0)
      {
          PetscSolver *theSolver =
              new PetscSolver (method, preconditioner, rTol, aTol, dTol,
                               maxIts);
          (*theSOE) = new PetscSOE (*theSolver);
      }
    else
      {
          PetscSparseSeqSolver *theSolver =
              new PetscSparseSeqSolver (method, preconditioner, rTol, aTol,
                                        dTol, maxIts);
