...
PetscSparseSeqSolver::PetscSparseSeqSolver (KSPType meth, PCType pre):SparseGenRowLinSolver (SOLVER_TAGS_PetscSparseSeqSolver),
...
PetscSparseSeqSolver::PetscSparseSeqSolver (KSPType meth, PCType pre,
                                            ...
                                            double divTol, int maxIterations):
...
PetscSparseSeqSolver::setSize ()
{
    ...
    PetscBool flg;              // PetsTruth --> PetscBool
    PetscErrorCode ierr =
        PetscOptionsGetInt (PETSC_NULL, NULL, "-n", &n, &flg);
    CHKERRQ (ierr);
    ierr = VecCreateSeqWithArray (PETSC_COMM_WORLD, 1, n, Xdata, &x);
    CHKERRQ (ierr);
    ierr = VecCreateSeqWithArray (PETSC_COMM_WORLD, 1, n, Bdata, &b);
    CHKERRQ (ierr);
    ierr =
        MatCreateSeqAIJWithArrays (PETSC_COMM_WORLD, n, n, rowStartA, colA,
                                   Adata, &A);
    ...
    KSPGetPC (ksp, &pc);
    ierr = PCSetType (pc, preconditioner);
    CHKERRQ (ierr);
