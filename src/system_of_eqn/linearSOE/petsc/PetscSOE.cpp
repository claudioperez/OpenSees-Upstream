...
PetscSOE::setSize (Graph & theGraph)
{
    ...
    if (!init_done)
      {
          ...
          if (FILE * file = fopen (PETSCSOE_PETSCOPTIONS_FILENAME, "r"))
            {
                fclose (file);
                cout << "        + PetscSOE::setSize() [" << processID <<
                    "] : Found petsc_options.txt file. \n";
                PetscInitialize (0, PETSC_NULL,
                                 PETSCSOE_PETSCOPTIONS_FILENAME,
                                 "PetscSOE::setSize - Initialize with file");
                PetscPopSignalHandler ();
            }
          else
            {
                PetscInitialize (0, PETSC_NULL, (char *) 0,
                                 "PetscSOE::setSize - Initialize");
                PetscPopSignalHandler ();
            }
    ...
    PETSCSOE_DEBUGOUT << "        + PetscSOE::setSize() [" << processID <<
        "] : Getting local max dof tag\n";
...
#undef max                      // Angrily, J. Abell
    ...
    PETSCSOE_DEBUGOUT << "        + PetscSOE::setSize() [" << processID <<
        "] : Determining global maximum dof tag\n";
    isFactored = 0;
    static ID data (1);
    if (processID != 0)
      {
    ...
    if (vectX == NULL)
      {
          std::
              cerr <<
              "PetscSOE::PetscSOE : can not allocate memory for vectX \n";
      }
    ...
    if (vectB == NULL)
      {
          std::
              cerr <<
              "PetscSOE::PetscSOE : can not allocate memory for vectB \n";
      }
    ...
    PETSCSOE_DEBUGOUT << "        + PetscSOE::setSize() [" << processID <<
        "] : Distributing SOE rows amongst processors\n";
    ...
    if (processID >= 0)
      {
          ...
          PETSCSOE_DEBUGOUT << "        + PetscSOE::setSize() - [" <<
              processID << "] : Initializing PETSc\n";
          ...
          PETSCSOE_DEBUGOUT << "        + PetscSOE::setSize() - Process " <<
              processID << " owns rows from " << startRow << " to " << endRow
              << endl;
                      ...
                      for (int proc = 0; proc < numProcesses; proc++)
                        {
                            ...
                            if (startRow_vec[proc] <= dof
                                && dof <= endRow_vec[proc])
                              {
                                  int row = dof - startRow_vec[proc];
                                  if (startRow_vec[proc] <= col
                                      && col <= endRow_vec[proc])
                                    {
                                        d_nnz[proc][row] += 1;
                                    }
          ...
          for (int proc = 0; proc < numProcesses; proc++)
            {
                MPI_Reduce (d_nnz[proc], d_nnz_global, nlocaldofs[proc], MPI_INT, MPI_SUM, proc,        // Because this is the process number in the petsc_comm communicator
                            PETSC_COMM_WORLD);
                MPI_Reduce (o_nnz[proc], o_nnz_global, nlocaldofs[proc], MPI_INT, MPI_SUM, proc,        // Because this is the process number in the petsc_comm communicator
                            PETSC_COMM_WORLD);
            }
          ...
          ierr =
              VecCreateMPIWithArray (PETSC_COMM_WORLD, blockSize, ndofs, size,
                                     &X[startRow], &x);
          CHKERRQ (ierr);
          ierr =
              VecCreateMPIWithArray (PETSC_COMM_WORLD, blockSize, ndofs, size,
    ...
    PETSCSOE_DEBUGOUT << "Process " << processID << " owns rows from " <<
        startRow << " to " << endRow << endl;
...
PetscSOE::setSize (int MaxDOFtag)
{
    int result = -1;
    cerr <<
        "PetscSOE::setSize(int MaxDOFtag) -- DEPRECATED! Should not get called\n";
    ...
    if (processID >= 0)
      {
          ...
          if (idSize != m.noRows () && idSize != m.noCols ())
            {
                cerr <<
                    "PetscSOE::addA() - Matrix and ID not of similar sizes\n";
                return -1;
            }
          ...
          int ierr =
              MatSetValues (A, nsmall, indx_ij, nsmall, indx_ij, values,
                            ADD_VALUES);
    ...
    if (processID >= 0)
      {
          ...
          if (idSize != v.Size ())
            {
                cerr <<
                    "addB::addB()	- Vector and ID not of similar sizes\n";
                return -1;
            }
                      ...
                      if (pos < size && pos >= 0)
                        {
                            ...
                            if (B[pos] != B[pos])
                              {
                                  cerr << "PetscSOE::addB() - rank = " <<
                                      processID << "- NaN found! At pos=" <<
                                      pos << endl;
                                  return -1;
                              }
                        }
          ...
          if (v.Size () != size)
            {
                ...
                cerr << " incomptable sizes " << size << " and " << v.
                    Size () << endln;
                return -1;
            }
          ...
          if (solverOK < 0)
            {
                ...
                cerr <<
                    "the new solver could not setSeize() - staying with old\n";
                return solverOK;
            }
      }
    ...
    if (found == false)
      {
          ...
          if (localCol == 0)
            {
                cerr <<
                    "DistributedBandGenLinSOE::sendSelf() - failed to allocate id array of size: "
                    << nextNumChannels << endln;
                return -1;
            }
    ...
    if (idSize != m.noRows () && idSize != m.noCols ())
      {
          std::
              cerr <<
              "PetscSOE::addM() - Matrix and ID not of similar sizes\n";
          return -1;
      }
                      ...
                      if (col >= 0)
                        {
                            value = m (i, j) * fact;
                            int ierr =
                                MatSetValues (M, 1, &row, 1, &col, &value,
                                              ADD_VALUES);
                      ...
                      if (col >= 0)
                        {
                            value = m (i, j) * fact;
                            int ierr =
                                MatSetValues (K, 1, &row, 1, &col, &value,
                                              ADD_VALUES);
