...
PetscSOE::PetscSOE (PetscSolver & theSOESolver):LinearSOE (theSOESolver, LinSOE_TAGS_PetscSOE),
...
PetscSOE::setSize (Graph & theGraph)
{
    ...
    int ierr = OptionsGetInt (PETSC_NULL, "-n", &size, &flg);
    CHKERRA (ierr);
    ierr = MatCreate (PETSC_COMM_WORLD, size, size, &A);
    CHKERRA (ierr);
    ierr = VecCreate (PETSC_COMM_WORLD, PETSC_DECIDE, size, &x);
    CHKERRA (ierr);
...
PetscSOE::setSizeParallel (int newSize, int N,
                           int d_nz, int *d_nnz, int o_nz, int *o_nnz)
{
    int result = 0;
    int oldSize = size;
    size = newSize;
                      ...
                      if (col >= 0)
                        {
                            value = m (i, j) * fact;
                            int ierr =
                                MatSetValues (A, 1, &row, 1, &col, &value,
                                              ADD_VALUES);
                            CHKERRA (ierr);
                        }
                  }
    ...
    for (int i = 0; i < n; i++)
      {
          ...
          if (row >= 0)
            {
                value = v (i) * fact;
                int ierr = VecSetValues (b, 1, &row, &value, ADD_VALUES);
                CHKERRA (ierr);
            }
      }
...
PetscSOE::setB (const Vector & v, double fact)
{
    ...
    for (int i = 0; i < n; i++)
      {
          value = v (i) * fact;
          int ierr = VecSetValues (b, 1, &i, &value, INSERT_VALUES);
          CHKERRA (ierr);
      }
    int ierr = VecAssemblyBegin (b);
    CHKERRA (ierr);
...
PetscSOE::getX (void)
{
    ...
    double *theData = 0;
    int ierr = VecGetArray (x, &theData);
    CHKERRA (ierr);
    for (int i = 0; i < size; i++)
        X[i] = theData[i];
    ierr = VecRestoreArray (x, &theData);
    CHKERRA (ierr);
...
PetscSOE::getB (void)
{
    ...
    double *theData = 0;
    int ierr = VecGetArray (b, &theData);
    CHKERRA (ierr);
    for (int i = 0; i < size; i++)
        B[i] = theData[i];
    ierr = VecRestoreArray (b, &theData);
    CHKERRA (ierr);
...
{
    int ierr = VecSetValues (x, 1, &loc, &value, INSERT_VALUES);
    CHKERRA (ierr);
}
