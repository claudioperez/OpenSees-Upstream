...
FullGenLinSOE::FullGenLinSOE (FullGenLinSolver & theSolvr):LinearSOE (theSolvr, LinSOE_TAGS_FullGenLinSOE),
size (0), A (0), B (0), X (0),
vectX (0), vectB (0), matA (0), Asize (0), Bsize (0), factored (false)
{
...
FullGenLinSOE::addColA (const Vector & colData, int col, double fact)
{
    ...
    if (colData.Size () != size)
      {
          opserr <<
              "FullGenLinSOE::addColA() - colData size not equal to n\n";
          return -1;
      }

    if (col > size && col < 0)
      {
          opserr << "FullGenLinSOE::addColA() - col " << col <<
...
FullGenLinSOE::addB (const Vector & v, const ID & id, double fact)
{
    ...
    if (idSize != v.Size ())
      {
          opserr <<
              "FullGenLinSOE::addB()	- Vector and ID not of similar sizes\n";
          return -1;
      }
...
int
FullGenLinSOE::recvSelf (int commitTag, Channel & theChannel,
