...
ItpackLinSOE::ItpackLinSOE (ItpackLinSolver & the_Solver):LinearSOE (the_Solver, LinSOE_TAGS_ItpackLinSOE),
                            ...
                            if (col < size && col >= 0)
                              {
                                  ...
                                  for (int k = startRowLoc; k < endRowLoc;
                                       k++)
                                      if (colA[k] == col)
                                        {
                                            A[k] += m (i, j);
    ...
    if (x.Size () != size)
      {
          ...
          opserr << " incompatible sizes " << size << " and " << x.
              Size () << endln;
      }
    else
      {
