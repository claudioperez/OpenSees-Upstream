          ...
          for (int i = 0; i < maxNeighbors; i++)
            {
                ...
                MPI_Bcast (maxDOFsSharedArray, myNeighborsSizes[i], MPI_INT,
                           i, MPI_COMM_WORLD);
                MPI_Bcast (maxSharedB, myNeighborsSizes[i], MPI_DOUBLE, i,
                           MPI_COMM_WORLD);
                MPI_Bcast (maxSharedA, myNeighborsSizes[i], MPI_DOUBLE, i,
                           MPI_COMM_WORLD);
                ...
                if ((i != processID) && (myNeighbors[i] == 1))
                  {
                      // cached neighbors
                      int *temp =
                          ((theSOE->myActualNeighborsSharedDOFs).find (i)->
                           second);
                      ...
                      theSOE->myActualNeighborsBsToSend[i] =
                          new double[myNeighborsSizes[i]];
                      intersectionsAB (myDOFs,
                                       (theSOE->myActualNeighborsSharedDOFs).
                                       find (i)->second, size,
                                       myNeighborsSizes[i], A, maxSharedA, B,
                                       maxSharedB, data, i);
                  }
            }
          //
          ...
          for (int i = 0; i < maxNeighbors; i++)
            {
                if ((i != processID) && (myNeighbors[i] == 1))
                  {
                      MPI_Irecv (((theSOE->myActualNeighborsSharedBs).
                                  find (i)->second), maxShared, MPI_DOUBLE, i,
                ...
                if ((i != processID) && (myNeighbors[i] == 1))
                  {
                      ...
                      double *tmpptr =
                          (theSOE->myActualNeighborsBsToSend).find (i)->
                          second;
                      int *tmpptr2 =
                          (theSOE->myActualNeighborsSharedDOFs).find (i)->
                          second;
                ...
                if ((i != processID) && (myNeighbors[i] == 1))
                  {
                      ...
                      double *tmpptr =
                          (theSOE->myActualNeighborsBsToSend).find (i)->
                          second;
                      MPI_Isend (tmpptr, upto, MPI_DOUBLE, i, i,
                                 MPI_COMM_WORLD, &theRequests[ct++]);
                  }
            }
                ...
                if ((i != processID) && (myNeighbors[i] == 1))
                  {
                      int *posloc =
                          ...
                          second;
                      double *dat =
                          (theSOE->myActualNeighborsSharedBs).find (i)->
                          second;
                      // update RHS
...
int
MPIDiagonalSolver::sendSelf (int cTag, Channel & theChannel)
{
    return 0;
}
...
void
MPIDiagonalSolver::intersectionsAB (ID & arrayA, int *arrayB, int sizeA,
                                    int sizeB, double *A, double *sharedA,
                                    double *B, double *sharedB, int *storage,
                                    int neighbor_pid)
{
    ...
    (theSOE->myActualNeighborsSharedDOFs).find (neighbor_pid)->second =
        storage;
