...
MPIDiagonalSOE::MPIDiagonalSOE (MPIDiagonalSolver & the_Solver):LinearSOE (the_Solver, LinSOE_TAGS_MPIDiagonalSOE),
size (0), A (0), B (0), X (0), sharedA (0), sharedB (0), maxSharedA (0),
maxSharedB (0), isAfactored (false), updateA (true), vectX (0), vectB (0),
dataShared (0), actualNeighbors (0), maxNeighbors (0), myNeighbors (0),
myNeighborsSizes (0), myDOFsArray (0), myDOFsSharedArray (0),
maxDOFsSharedArray (0), posLocKey (0), processID (0), numProcesses (0),
numChannels (0), theChannels (0), localCol (0), myDOFs (0, 32),
myDOFsShared (0, 16), numShared (0), theModel (0)
{
...
MPIDiagonalSOE::~MPIDiagonalSOE ()
{
    ...
    if (myNeighbors != 0)
        delete[]myNeighbors;
    if (myNeighborsSizes != 0)
        delete[]myNeighborsSizes;
    if (dataShared != 0)
        delete[]dataShared;
    if (myDOFsSharedArray != 0)
        delete[]myDOFsSharedArray;
    if (myDOFsArray != 0)
        delete[]myDOFsArray;
    if (maxDOFsSharedArray != 0)
        delete[]maxDOFsSharedArray;
    if (posLocKey != 0)
        delete[]posLocKey;
...
MPIDiagonalSOE::setSize (Graph & theGraph)
{
    ...
    *tmpsendSize = size;
    ...
    for (int i = 0; i < numProcesses; i++)
        allSizes[i] = 0;
    MPI_Allgather (tmpsendSize, 1, MPI_INT, allSizes, 1, MPI_INT,
                   MPI_COMM_WORLD);
    int *max = new int[1];
    max[0] = 0;
    for (int i = 0; i < numProcesses; i++)
    ...
    if (max[0] < size)
      {
          opserr <<
              " MPIDiagonalSOE::setSize() L170 : SEVERE ERROR : ABORTING TASK \n";
          exit (-1);
      }
          ...
          if (i != processID)
            {
                ...
                intersections (myDOFs, *piDOFs, size, allSizes[i], piShared,
                               sharedDOFs);
    ...
    *tmpsendSize = numShared;
    ...
    MPI_Allgather (tmpsendSize, 1, MPI_INT, myNeighborsSizes, 1, MPI_INT,
                   MPI_COMM_WORLD);
    ...
    if (maxShared < numShared)
      {
          opserr <<
              " MPIDiagonalSOE::setSize() L247 : SEVERE ERROR : ABORTING TASK \n";
          exit (-1);
      }
    ...
    if (A == 0 || B == 0 || X == 0 || sharedA == 0 || sharedB == 0
        || maxSharedA == 0 || maxSharedB == 0 || dataShared == 0 || vectX == 0
        || vectB == 0 || posLocKey == 0)
      {
...
MPIDiagonalSOE::addA (const Matrix & m, const ID & id, double fact)
{
    ...
    if (idSize != m.noRows () && idSize != m.noCols ())
      {
          opserr <<
              "MPIDiagonalSOE::addA()	- Matrix and ID not of similar sizes\n";
          return -1;
      }
#endif
...
void
MPIDiagonalSOE::intersections (ID & arrayA, ID & arrayB, int sizeA, int sizeB,
                               int *shared, int *sharedDOFs)
{
