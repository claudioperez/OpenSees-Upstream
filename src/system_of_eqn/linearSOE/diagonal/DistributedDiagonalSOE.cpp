...
DistributedDiagonalSOE::DistributedDiagonalSOE (DistributedDiagonalSolver & the_Solver):LinearSOE (the_Solver, LinSOE_TAGS_DistributedDiagonalSOE),
size (0), A (0), B (0), X (0), vectX (0), vectB (0), isAfactored (false),
processID (0), numProcesses (0),
numChannels (0), theChannels (0), localCol (0),
myDOFs (0, 32), myDOFsShared (0, 16), numShared (0), dataShared (0),
vectShared (0), theModel (0)
{
    the_Solver.setLinearSOE (*this);
}

DistributedDiagonalSOE::DistributedDiagonalSOE ():LinearSOE (LinSOE_TAGS_DistributedDiagonalSOE),
size (0), A (0), B (0), X (0), vectX (0), vectB (0), isAfactored (false),
processID (0), numProcesses (0),
numChannels (0), theChannels (0), localCol (0),
myDOFs (0, 32), myDOFsShared (0, 16), numShared (0), dataShared (0),
...
DistributedDiagonalSOE::setSize (Graph & theGraph)
{
    ...
    if (processID != 0)
                      ...
                      for (int j = 0; j < numOther; j++)
                        {
                            ...
                            if (myDOFs.getLocation (otherTag) != -1
                                && myDOFsShared.getLocation (otherTag) == -1)
      ...
      {                         // in absence of a broadcast we need the following
    ...
    if (A == 0 || B == 0 || X == 0 || vectX == 0 || vectB == 0
        || dataShared == 0 || vectShared == 0)
      {
    ...
    if (theModel == 0)
      {
          opserr <<
              "WARNING DistributedDiagonalSOE::setSize - no AnalysisModel\n";
      }
    else
      {
    ...
    DistributedDiagonalSolver *the_Solver =
        (DistributedDiagonalSolver *) this->getSolver ();
    the_Solver->setLinearSOE (*this);
...
DistributedDiagonalSOE::addA (const Matrix & m, const ID & id, double fact)
{
    ...
    if (idSize != m.noRows () && idSize != m.noCols ())
      {
          opserr <<
              "FullGenLinSOE::addA()	- Matrix and ID not of similar sizes\n";
          return -1;
      }
#endif
    ...
    if (v.Size () != size)
      {
          ...
          opserr << " incompatible sizes " << size << " and " << v.
              Size () << endln;
          return -1;
      }
...
setDiagonalSolver (DistributedDiagonalSolver & newSolver)
{
...
DistributedDiagonalSOE::sendSelf (int cTag, Channel & theChannel)
{
    ...
    if (res < 0)
      {
          opserr <<
              "WARNING DistributedDiagonalSOE::sendSelf() - failed to send data\n";
          return -1;
      }
    ...
    if (theSoeSolver != 0)
      {
          if (theSoeSolver->sendSelf (cTag, theChannel) < 0)
            {
                opserr <<
                    "WARNING DistributedDiagonalSOE::sendSelf() - failed to send solver\n";
                return -1;
            }
      }
    else
      {
          opserr <<
              "WARNING DistributedDiagonalSOE::sendSelf() - no solver to send!\n";
          return -1;
      }
                                  ...
                                  FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0)
      {
          opserr <<
              "WARNING DistributedProfileSPDLinSOE::recvSelf() - failed to send data\n";
          return -1;
      }
    processID = idData (0);
    ...
    DistributedDiagonalSolver *theDistributedDiagonalSolver =
        new DistributedDiagonalSolver ();
    if (theDistributedDiagonalSolver->recvSelf (cTag, theChannel, theBroker) <
        0)
      {
          opserr <<
              "WARNING DistributedBandgenLinSOE::sendSelf() - failed to recv solver\n";
          return -1;
      }
