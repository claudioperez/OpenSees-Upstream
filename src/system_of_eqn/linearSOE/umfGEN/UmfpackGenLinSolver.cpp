...
UmfpackGenLinSolver::UmfpackGenLinSolver ():LinearSOESolver (SOLVER_TAGS_UmfpackGenLinSolver), Symbolic (0),
theSOE (0)
{
}
...
UmfpackGenLinSolver::solve (void)
{
    ...
    if (Symbolic == 0)
      {
          opserr <<
              "WARNING: setSize has not been called -- Umfpackgenlinsolver::solve\n";
          return -1;
      }
    ...
    // solve
    status =
        umfpack_di_solve (UMFPACK_A, Ap, Ai, Ax, X, B, Numeric, Control,
                          Info);
