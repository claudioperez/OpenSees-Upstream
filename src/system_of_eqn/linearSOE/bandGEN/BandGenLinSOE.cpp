...
BandGenLinSOE::BandGenLinSOE (BandGenLinSolver & theSolvr):LinearSOE (theSolvr, LinSOE_TAGS_BandGenLinSOE),
...
BandGenLinSOE::BandGenLinSOE ():LinearSOE (LinSOE_TAGS_BandGenLinSOE),
size (0), numSuperD (0), numSubD (0), A (0), B (0), X (0),
vectX (0), vectB (0), Asize (0), Bsize (0), factored (false)
{
...
BandGenLinSOE::addColA (const Vector & colData, int col, double fact)
{
    ...
    if (colData.Size () != size)
      {
          opserr <<
              "BandGenLinSOE::addColA() - colData size not equal to n\n";
          return -1;
      }

    if (col > size && col < 0)
      {
          opserr << "BandGenLinSOE::addColA() - col " << col <<
              "outside range 0 to " << size << endln;
          return -1;
      }
...
BandGenLinSOE::addB (const Vector & v, const ID & id, double fact)
{
    ...
    if (idSize != v.Size ())
      {
          opserr <<
              "BandGenLinSOE::addB()	- Vector and ID not of similar sizes\n";
          return -1;
      }
...
int
BandGenLinSOE::recvSelf (int commitTag, Channel & theChannel,
