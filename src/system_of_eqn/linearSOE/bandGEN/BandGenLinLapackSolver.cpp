...
BandGenLinLapackSolver::BandGenLinLapackSolver ():BandGenLinSolver (SOLVER_TAGS_BandGenLinLapackSolver),
iPiv (0), iPivSize (0)
{
...
#ifdef _WIN32
    {
        if (theSOE->factored == false)
        ...
        else
          {
              ...
              char type[] = "N";
              DGBTRS (type, &n, &kl, &ku, &nrhs, Aptr, &ldA, iPIV, Xptr, &ldB,
                      &info);
    }}
#else
    {
        if (theSOE->factored == false)
            ...
            // solve only using factored matrix     
            dgbtrs_ ("N", &n, &kl, &ku, &nrhs, Aptr, &ldA, iPIV, Xptr, &ldB,
                     &info);
    }
#endif
    // check if successful
    if (info != 0)
      {
          if (info > 0)
            ...
            {
                opserr <<
                    "WARNING BandGenLinLapackSolver::solve() - OpenSees code error\n";
                return info;
            }
      }
