...
DistributedBandGenLinSOE::DistributedBandGenLinSOE (BandGenLinSolver & theSolvr):BandGenLinSOE (LinSOE_TAGS_DistributedBandGenLinSOE),
...
DistributedBandGenLinSOE::DistributedBandGenLinSOE ():BandGenLinSOE (LinSOE_TAGS_DistributedBandGenLinSOE),
processID (0), numChannels (0), theChannels (0), localCol (0), workArea (0),
sizeWork (0), myB (0), myVectB (0)
{
    ...
    if (newSize != Asize)
      {                         // we have to get another space for A
          ...
          if (A == 0)
            {
                opserr <<
                    "WARNING DistributedBandGenLinSOE::DistributedBandGenLinSOE :";
...
DistributedBandGenLinSOE::addA (const Matrix & m, const ID & id, double fact)
{
    ...
    if (idSize != m.noRows () && idSize != m.noCols ())
      {
          opserr <<
              "BandGenLinSOE::addA()	- Matrix and ID not of similar sizes\n";
          return -1;
      }
                ...
                if (col < size && col >= 0)
                  {
                      ...
                      if (processID == 0)
                          ...
                          coliiPtr =
                              A + (*theMap) (col) * ldA + numSubD + numSuperD;
...
DistributedBandGenLinSOE::addB (const Vector & v, const ID & id, double fact)
{
    ...
    if (idSize != v.Size ())
      {
          opserr <<
              "BandSPDLinSOE::addB() - Vector and ID not of similar sizes\n";
          return -1;
      }
...
DistributedBandGenLinSOE::sendSelf (int commitTag, Channel & theChannel)
{
    ...
    if (res < 0)
      {
          opserr <<
              "WARNING DistributedBandGenLinSOE::sendSelf() - failed to send data\n";
          return -1;
      }
    ...
    if (theSoeSolver != 0)
      {
          if (theSoeSolver->sendSelf (commitTag, theChannel) < 0)
            {
                opserr <<
                    "WARNING DistributedBandGenLinSOE::sendSelf() - failed to send solver\n";
                return -1;
            }
      }
    else
      {
          opserr <<
              "WARNING DistributedBandGenLinSOE::sendSelf() - no solver to send!\n";
          return -1;
      }
...
int
DistributedBandGenLinSOE::recvSelf (int commitTag, Channel & theChannel,
                                    FEM_ObjectBroker & theBroker)
{
    ID idData (1);
    ...
    if (res < 0)
      {
          opserr <<
              "WARNING DistributedBandGenLinSOE::recvSelf() - failed to send data\n";
          return -1;
      }
    processID = idData (0);
