...
profilematrix::profilematrix (int matrix_order,
                              ...
                              char flag_init,
                              double *au_init_val,
                              double *al_init_val, double *ad_init_val)
{
...
// put all jp_init's in the jp
    ...
    pc_profilematrix_rep->total_numb =
        pc_profilematrix_rep->jp[matrix_order - 1];
...
// allocate memory for the actual profilematrix as profilematrix
    pc_profilematrix_rep->au =
        new double[(size_t) pc_profilematrix_rep->total_numb];
    ...
    if (pc_profilematrix_rep->flag == 'N')
      ...
      {
          ::printf
              ("flags not set properly, flag=='S' symmetric; flag=='N' NON(un)symmetric\n");
      }
...
//##########################################################################
int
profilematrix::dimension_of_profile_M (void) const      // dimension of profile matrix
{
...
//##########################################################################
int *
profilematrix::get_jp (void) const      // get pointer to array of
...
void
profilematrix::set_penalty_element (int *eqn_no_shake, int no_of_shake,
                                    double max_element)
{
...
{
    long int
        this_total_numb = this->pc_profilematrix_rep->total_numb;
    long int
        rval_total_numb = rval.pc_profilematrix_rep->total_numb;
//    this_total_numb = 696;
//    rval_total_numb = 696;
...
// create the structure:
          profilematrix_rep *
              New_pc_profilematrix_rep = new profilematrix_rep; // this 'new' is overloaded

          New_pc_profilematrix_rep->neq = this->pc_profilematrix_rep->neq;

// get space for the JP vector
          New_pc_profilematrix_rep->jp =
              new int[New_pc_profilematrix_rep->neq];
// put all jp_init's in the jp
          for (int j = 0; j < this->pc_profilematrix_rep->neq; j++)
              New_pc_profilematrix_rep->jp[j] =
                  this->pc_profilematrix_rep->jp[j];

          New_pc_profilematrix_rep->total_numb =
...
// allocate memory for the actual profilematrix as profilematrix
          New_pc_profilematrix_rep->au =
              new double[(size_t) New_pc_profilematrix_rep->total_numb];
          ...
          New_pc_profilematrix_rep->ad =
              new double[(size_t) New_pc_profilematrix_rep->neq];
...
//        for ( int iau=0 ; iau<New_pc_profilematrix_rep->total_numb ; iau++ )
          for (int iau = 0; iau < this_total_numb; iau++)
              New_pc_profilematrix_rep->au[iau] =
                  this->pc_profilematrix_rep->au[iau];

          for (int iad = 0; iad < New_pc_profilematrix_rep->neq; iad++)
              New_pc_profilematrix_rep->ad[iad] =
                  this->pc_profilematrix_rep->ad[iad];

          if (New_pc_profilematrix_rep->flag == 'N')
            {
                for (int ial = 0; ial < this_total_numb; ial++)
                    New_pc_profilematrix_rep->al[ial] =
                        this->pc_profilematrix_rep->al[ial];
            }
          else if (New_pc_profilematrix_rep->flag == 'S')
            {
                New_pc_profilematrix_rep->al = New_pc_profilematrix_rep->au;
...
//    for (int kau=0 ; kau<this->pc_profilematrix_rep->total_numb ; kau++)
    for (int kau = 0; kau < this_total_numb; kau++)
        this->pc_profilematrix_rep->au[kau] +=
            rval.pc_profilematrix_rep->au[kau];
    for (int kad = 0; kad < this->pc_profilematrix_rep->neq; kad++)
        this->pc_profilematrix_rep->ad[kad] +=
            rval.pc_profilematrix_rep->ad[kad];
    if (this->pc_profilematrix_rep->flag == 'N')
      {
          for (int kal = 0; kal < this_total_numb; kal++)
              this->pc_profilematrix_rep->al[kal] +=
                  rval.pc_profilematrix_rep->al[kal];
      }
//..// this is OK
...
// profilematrix addition
profilematrix
operator+ (const profilematrix & lval, const profilematrix & rval)
{
    profilematrix
    result (lval);
    result += rval;
...
// I can add this two profilematrices just as a simple vectors:
    ...
    if (this->pc_profilematrix_rep->flag == 'N')
      {
          for (int kal = 0; kal < this->pc_profilematrix_rep->total_numb;
               kal++)
...
// class stiffness matrix those values are to be altered!!!!!!!!!!!!!!!!!!!!!!!!
          ...
          for (int iad = 0; iad < New_pc_profilematrix_rep->neq; iad++)
              New_pc_profilematrix_rep->au[iad] =
                  this->pc_profilematrix_rep->ad[iad];

          if (New_pc_profilematrix_rep->flag == 'N')
            {
                for (int ial = 0; ial < New_pc_profilematrix_rep->total_numb;
                     ial++)
                    New_pc_profilematrix_rep->al[ial] =
                        this->pc_profilematrix_rep->al[ial];
            }
...
double *
profilematrix::operator* (BJvector & arg)
{
...
//       ::printf(" \n\nI%d %+6.2e%+6.2e%+6.2e\n\n",i,this->pc_profilematrix_rep->ad[i],arg.val(i+1),result[i+1] );
          ...
          if (i == 0)
            ...
            {
                colht =
                    this->pc_profilematrix_rep->jp[i] -
                    this->pc_profilematrix_rep->jp[i - 1];
            }
          ...
          for (int j = 1; j <= colht; j++)
            {
                ...
                irow = i - j;
                result[irow] +=
                    this->pc_profilematrix_rep->au[iau] * arg.val (i + 1);
                result[i] +=
                    this->pc_profilematrix_rep->al[iau] * arg.val (irow + 1);
...
double &
profilematrix::operator () (int row, int col)
     const                      // |c
     {                          // |o
         if (row == col)        // |l
           {                    // |u
...
//##########################################################################
double &
profilematrix::mval (int row, int col) const    // only for the members inside
{                               // the profile !!!!!!!!!!!
...
double
profilematrix::full_val (int row, int col) const
{
    if (row == col)
      {
          return this->pc_profilematrix_rep->ad[row - 1];
    ...
    else if (row < col)         // upper part AU
      {
          ...
          int zeros_above_profile = total_column_depth - real_column_depth;
    ...
    else if (col < row)         // lower part AL
      {
          ...
          int zeros_left_of_profile = total_row_width - real_row_width;
...
//##########################################################################
double &
profilematrix::val (int row, int col)
{
//    double zero=0.0;
    if (row <= 0 && row >= dimension_of_profile_M ()
        && col >= dimension_of_profile_M ())
      {
...
//##########################################################################
double
profilematrix::cval (int row, int col) const
{
    double zero = 0.0;
    if (row <= 0 && row >= dimension_of_profile_M ()
        && col >= dimension_of_profile_M ())
...
void
profilematrix::profile_jp_print (void)
{
    ::printf ("\n jp \n\n ");
...
void
profilematrix::profile_al_print (void)
{
    ::printf ("\n al \n\n ");
    for (int count05 = 0;
         count05 < pc_profilematrix_rep->jp[pc_profilematrix_rep->neq - 1];
         count05++)
      {
...
void
profilematrix::profile_ad_print (void)
{
    ::printf ("\n al \n\n ");
    for (int count05 = 0; count05 < pc_profilematrix_rep->neq; count05++)
      {
...
//##########################################################################
void
profilematrix::full_print (char *msg)
{
    if (*msg)
        printf ("%s\n", msg);
    for (int row = 1; row <= dimension_of_profile_M (); row++)
...
//##########################################################################
void
profilematrix::error (char *msg1, char *msg2) const
{
...
//
long int
profilematrix::total_number (void) const
{
    return this->pc_profilematrix_rep->total_numb;
}

void
profilematrix::total_number (int number)
{
...
//
int
profilematrix::reference_count (int up_down)
{
...
void
profilematrix::set_reference_count (int ref_count)
{
...
//            if(abs(ad(j)).lt.tol*abs(dd))  write(iow,2000) j
                if ((fabs (pc_profilematrix_rep->ad[j - 1]) <
                     (tol * fabs (dd))))
                    ::printf
...
//            ad(j) = one/ad(j)
                pc_profilematrix_rep->ad[j - 1] =
                    one / pc_profilematrix_rep->ad[j - 1];
//          endif
            }
//  200   continue
      }
...
//      end
double
profilematrix::datest (double *au, int jh)
{
    double daval = 0.0;
    for (int j = 1; j <= jh; j++)
        daval += fabs (au[j - 1]);
...
double
profilematrix::dot (double *a, double *b, int n)
{
    double dot = 0.0;
    for (int i = 1; i <= n; i++)
        dot += a[i - 1] * b[i - 1];
...
//..      end
void
profilematrix::dredu (double *al,
                      double *au, double *ad, int jh, char flag, double *dj)
{
    double ud = 0.0;
    if (flag == 'N')
      {
...
//      end
void
profilematrix::saxpb (double *a, double *b, double x, int jh, double *c)
{
    for (int k = 1; k <= jh; k++)
        c[k - 1] = a[k - 1] * x + b[k - 1];
}
