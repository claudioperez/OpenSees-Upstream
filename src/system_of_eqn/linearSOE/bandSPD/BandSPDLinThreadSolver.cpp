...
BandSPDLinThreadSolver::BandSPDLinThreadSolver ():BandSPDLinSolver (SOLVER_TAGS_BandSPDLinThreadSolver), NP (1),
running (false),
blockSize (1)
{
...
BandSPDLinThreadSolver::BandSPDLinThreadSolver (int numProcessors,
                                                int blckSize):
BandSPDLinSolver (SOLVER_TAGS_BandSPDLinThreadSolver),
NP (numProcessors),
running (false),
blockSize (blckSize)
{
    ...
    if (theSOE->factored == false)
      {
          ...
          if (running == false)
            {
                mutex_init (&TCB_BandSPDLinThreadSolver.start_mutex,
                            USYNC_THREAD, 0);
                mutex_init (&TCB_BandSPDLinThreadSolver.end_mutex,
                            USYNC_THREAD, 0);
                mutex_init (&TCB_BandSPDLinThreadSolver.startBlock_mutex,
                            USYNC_THREAD, 0);
                mutex_init (&TCB_BandSPDLinThreadSolver.endBlock_mutex,
                            USYNC_THREAD, 0);
                cond_init (&TCB_BandSPDLinThreadSolver.start_cond,
                           USYNC_THREAD, 0);
                cond_init (&TCB_BandSPDLinThreadSolver.end_cond, USYNC_THREAD,
                           0);
                cond_init (&TCB_BandSPDLinThreadSolver.startBlock_cond,
                           USYNC_THREAD, 0);
                cond_init (&TCB_BandSPDLinThreadSolver.endBlock_cond,
                           USYNC_THREAD, 0);
          ...
          while (TCB_BandSPDLinThreadSolver.threadsDone < NP)
              cond_wait (&TCB_BandSPDLinThreadSolver.end_cond,
                         &TCB_BandSPDLinThreadSolver.end_mutex);
...
void *
BandSPDLinThreadSolver_Worker (void *arg)
{
    ...
    while (true)
      {
          ...
          while (TCB_BandSPDLinThreadSolver.workToDo == 0)
              cond_wait (&TCB_BandSPDLinThreadSolver.start_cond,
                         &TCB_BandSPDLinThreadSolver.start_mutex);
                ...
                if (myID == currentDiagThread)
                  {
                      ...
                      while (TCB_BandSPDLinThreadSolver.threadsDoneBlock <
                             numThreads)
                          cond_wait (&TCB_BandSPDLinThreadSolver.
                                     endBlock_cond,
                                     &TCB_BandSPDLinThreadSolver.
                                     endBlock_mutex);

                      TCB_BandSPDLinThreadSolver.threadsDoneBlock = 0;
                      mutex_unlock (&TCB_BandSPDLinThreadSolver.
                                    endBlock_mutex);

                      // FACTOR DIAG BLOCK

                      // allow other threads to now proceed
                      mutex_lock (&TCB_BandSPDLinThreadSolver.
                                  startBlock_mutex);
                      TCB_BandSPDLinThreadSolver.currentBlock = i;

                      cond_broadcast (&TCB_BandSPDLinThreadSolver.
                                      startBlock_cond);
                      mutex_unlock (&TCB_BandSPDLinThreadSolver.
                                    startBlock_mutex);
