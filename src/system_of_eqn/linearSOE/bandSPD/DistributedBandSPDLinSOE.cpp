...
DistributedBandSPDLinSOE::DistributedBandSPDLinSOE (BandSPDLinSolver & theSolvr):BandSPDLinSOE (theSolvr, LinSOE_TAGS_DistributedBandSPDLinSOE),
processID (0), numChannels (0), theChannels (0), localCol (0), workArea (0),
sizeWork (0), myVectB (0), myB (0)
{
    theSolvr.setLinearSOE (*this);
}

DistributedBandSPDLinSOE::DistributedBandSPDLinSOE ():BandSPDLinSOE (LinSOE_TAGS_DistributedBandSPDLinSOE),
processID (0), numChannels (0), theChannels (0), localCol (0), workArea (0),
sizeWork (0), myVectB (0), myB (0)
{
    ...
    if (newSize != Asize)
      {                         // we have to get another space for A
          ...
          if (A == 0)
            {
                opserr <<
                    "WARNING DistributedBandSPDLinSOE::DistributedBandSPDLinSOE :";
...
DistributedBandSPDLinSOE::addA (const Matrix & m, const ID & id, double fact)
{
    ...
    if (idSize != m.noRows () && idSize != m.noCols ())
      {
          opserr <<
              "BandSPDLinSOE::addA()	- Matrix and ID not of similar sizes\n";
          return -1;
      }
...
DistributedBandSPDLinSOE::sendSelf (int commitTag, Channel & theChannel)
{
          ...
          if (found == false)
            {
                ...
                if (nextNumChannels == 0)
                  {
                      opserr <<
                          "DistributedBandSPDLinSOE::sendSelf() - failed to allocate channel array of size: "
                ...
                if (localCol == 0)
                  {
                      opserr <<
                          "DistributedBandSPDLinSOE::sendSelf() - failed to allocate id array of size: "
                          << nextNumChannels << endln;
                      return -1;
                  }
                for (int i = 0; i < numChannels; i++)
    ...
    if (res < 0)
      {
          opserr <<
              "WARNING DistributedBandSPDLinSOE::sendSelf() - failed to send data\n";
          return -1;
      }
...
int
DistributedBandSPDLinSOE::recvSelf (int commitTag, Channel & theChannel,
                                    FEM_ObjectBroker & theBroker)
{
    ID idData (1);
    ...
    if (res < 0)
      {
          opserr <<
              "WARNING DistributedBandSPDLinSOE::recvSelf() - failed to send data\n";
          return -1;
      }
    processID = idData (0);
