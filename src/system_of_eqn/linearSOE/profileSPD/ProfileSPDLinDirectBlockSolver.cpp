...
ProfileSPDLinDirectBlockSolver::ProfileSPDLinDirectBlockSolver (double tol,
                                                                int blckSize):
    ...
    if (size != theSOE->size)
      {
          ...
          if (RowTop == 0 || topRowPtr == 0 || invD == 0)
            {
                opserr <<
                    "Warning :ProfileSPDLinDirectBlockSolver::ProfileSPDLinDirectBlockSolver :";
                      ...
                      if (currentRow < n)
                        {       // this is for case when size%blockSize != 0
                            ...
                            if (ajj <= 0.0)
                              {
                                  opserr <<
                                      "ProfileSPDLinDirectBlockSolver::solve() - ";
                                  opserr << " aii < 0 (i, aii): (" <<
                                      currentRow << ", " << ajj << ")\n";
                                  return (-2);
                              }
...
ProfileSPDLinDirectBlockSolver::sendSelf (int cTag, Channel & theChannel)
{
    if (size != 0)
        opserr <<
            "ProfileSPDLinDirectBlockSolver::sendSelf - does not send itself YET\n";
    return 0;
}
