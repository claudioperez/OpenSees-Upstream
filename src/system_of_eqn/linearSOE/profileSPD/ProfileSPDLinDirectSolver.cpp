...
ProfileSPDLinDirectSolver::setSize (void)
{
    ...
    if (RowTop == 0 || topRowPtr == 0 || invD == 0)
      {
          opserr <<
              "Warning :ProfileSPDLinDirectSolver::ProfileSPDLinDirectSolver :";
...
ProfileSPDLinDirectSolver::factor (int n)
{
    ...
    if (n > theSize)
      {
          opserr << "ProfileSPDLinDirectSolver::factor: ";
          opserr << " - n " << n << " greater than size of system" << theSize
              << endln;
          return -1;
      }
...
int
ProfileSPDLinDirectSolver::sendSelf (int cTag, Channel & theChannel)
{
    return 0;
}
