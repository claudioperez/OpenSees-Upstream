...
ProfileSPDLinSOE::ProfileSPDLinSOE (ProfileSPDLinSolver & the_Solver):LinearSOE (the_Solver, LinSOE_TAGS_ProfileSPDLinSOE),
...
ProfileSPDLinSOE::addA (const Matrix & m, const ID & id, double fact)
{
    ...
    if (idSize != m.noRows () && idSize != m.noCols ())
      {
          opserr <<
              "ProfileSPDLinSOE::addA()	- Matrix and ID not of similar sizes\n";
          return -1;
      }
                ...
                if (col < size && col >= 0)
                  {
                      ...
                      if (col == 0)
                          ...
                          minColRow =
                              col - (iDiagLoc[col] - iDiagLoc[col - 1]) + 1;
...
ProfileSPDLinSOE::addColA (const Vector & colData, int col, double fact)
{
    ...
    if (colData.Size () != size)
      {
          opserr <<
              "ProfileSPDLinSOE::addColA() - colData size not equal to n\n";
          return -1;
      }

    if (col > size && col < 0)
      {
          opserr << "ProfileSPDLinSOE::addColA() - col " << col <<
...
int
ProfileSPDLinSOE::recvSelf (int cTag, Channel & theChannel,
