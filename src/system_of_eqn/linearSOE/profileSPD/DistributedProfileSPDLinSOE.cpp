...
DistributedProfileSPDLinSOE::DistributedProfileSPDLinSOE (ProfileSPDLinSolver & theSolvr):ProfileSPDLinSOE (theSolvr, LinSOE_TAGS_DistributedProfileSPDLinSOE),
...
DistributedProfileSPDLinSOE::DistributedProfileSPDLinSOE ():ProfileSPDLinSOE (LinSOE_TAGS_DistributedProfileSPDLinSOE),
processID (0), numChannels (0), theChannels (0),
localCol (0), sizeLocal (0), workArea (0), sizeWork (0), myVectB (0), myB (0)
{
          ...
          for (int j = 0; j < numChannels; j++)
            {
                ...
                if (theSubGraph.recvSelf (0, *theChannel, theBroker) < 0)
                  {
                      opserr <<
                          "WARNING DistributedProfileSPDLinSOE::setSize() : ";
...
int
DistributedProfileSPDLinSOE::addA (const Matrix & m, const ID & id,
                                   double fact)
{
    ...
    if (idSize != m.noRows () && idSize != m.noCols ())
      {
          opserr <<
              "DistributedProfileSPDLinSOE::addA()	- Matrix and ID not of similar sizes\n";
          return -1;
      }
...
int
DistributedProfileSPDLinSOE::addB (const Vector & v, const ID & id,
                                   double fact)
{
...
DistributedProfileSPDLinSOE::sendSelf (int commitTag, Channel & theChannel)
{
          ...
          if (found == false)
            {
                ...
                if (nextNumChannels == 0)
                  {
                      opserr <<
                          "DistributedProfileSPDLinSOE::sendSelf() - failed to allocate channel array of size: "
                ...
                if (localCol == 0)
                  {
                      opserr <<
                          "DistributedProfileSPDLinSOE::sendSelf() - failed to allocate id array of size: "
                          << nextNumChannels << endln;
                      return -1;
                  }
                for (int i = 0; i < numChannels; i++)
    ...
    if (res < 0)
      {
          opserr <<
              "WARNING DistributedProfileSPDLinSOE::sendSelf() - failed to send data\n";
          return -1;
      }
...
int
DistributedProfileSPDLinSOE::recvSelf (int commitTag, Channel & theChannel,
                                       FEM_ObjectBroker & theBroker)
{
