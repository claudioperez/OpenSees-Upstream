...
SProfileSPDLinSOE::SProfileSPDLinSOE (SProfileSPDLinSolver & the_Solver):LinearSOE (the_Solver, LinSOE_TAGS_SProfileSPDLinSOE),
...
numInt (0)
{
    ...
    if (A == 0)
    ...
    else
      {
          ...
          if (B == 0 || X == 0 || iDiagLoc == 0 || doubleX == 0
              || doubleB == 0)
            {
...
SProfileSPDLinSOE::addA (const Matrix & m, const ID & id, double fact)
{
    ...
    if (idSize != m.noRows () && idSize != m.noCols ())
      {
          opserr <<
              "SProfileSPDLinSOE::addA()	- Matrix and ID not of similar sizes\n";
          return -1;
      }
...
int
SProfileSPDLinSOE::recvSelf (int cTag, Channel & theChannel,
