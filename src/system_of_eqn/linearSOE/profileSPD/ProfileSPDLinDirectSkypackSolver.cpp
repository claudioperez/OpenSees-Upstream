...
ProfileSPDLinDirectSkypackSolver::ProfileSPDLinDirectSkypackSolver ():ProfileSPDLinSolver (SOLVER_TAGS_ProfileSPDLinDirectSkypackSolver),
...
ProfileSPDLinDirectSkypackSolver::ProfileSPDLinDirectSkypackSolver (int Mcols,
                                                                    int
                                                                    Mrows):
...
invD (0)
{
    ...
    if (rw == 0 || tw == 0 || index == 0)
      {
          opserr <<
              "WARNING ProfileSPDLinDirectSkypackSolver::ProfileSPDLinDirectSkypack";
          opserr <<
              "Solver() - ran out of memory for work areas, setting mCols and mRows = 0\n";
          mCols = 0;
          mRows = 0;
      }
}
...
extern "C" int skysf2_ (double *A, double *D, int *DGPNT, int *JMIN,
                        int *JMAX);

extern "C" int skypf2_ (int *index, int *JMAX, int *JMIN, int *MCOLS,
                        int *MROWS, int *DGPNT, double *A, double *RW,
                        double *TW);
    ...
    if (theSOE->isAfactored == false)
      {
          ...
          if (mRows == 0 || mCols == 0)
            ...
            {                   // factor using skypf2_
                ...
                double *TW = tw;
                int *INDEX = index;
                skypf2_ (INDEX, &JMAX, &JMIN, &MCOLS, &MROWS, iDiagLoc, A, RW,
                         TW);
...
int
ProfileSPDLinDirectSkypackSolver::sendSelf (int cTag, Channel & theChannel)
{
    if (size != 0)
        opserr <<
            "ProfileSPDLinDirectSkypackSolver::sendSelf - does not send itself YET\n";
    return 0;
}
