...
ProfileSPDLinDirectThreadSolver::ProfileSPDLinDirectThreadSolver ():ProfileSPDLinSolver (SOLVER_TAGS_ProfileSPDLinDirectThreadSolver),
NP (2), running (false),
    ...
    if (size != theSOE->size)
      {
          ...
          if (RowTop == 0 || topRowPtr == 0 || invD == 0)
            {
                opserr <<
                    "Warning :ProfileSPDLinDirectThreadSolver::ProfileSPDLinDirectThreadSolver :";
    ...
    if (theSOE->isAfactored == false)
      {
          ...
          if (running == false)
            {
                mutex_init (&TCB_ProfileSPDDirectThreadSolver.start_mutex,
                            USYNC_THREAD, 0);
                mutex_init (&TCB_ProfileSPDDirectThreadSolver.end_mutex,
                            USYNC_THREAD, 0);
                mutex_init (&TCB_ProfileSPDDirectThreadSolver.
                            startBlock_mutex, USYNC_THREAD, 0);
                mutex_init (&TCB_ProfileSPDDirectThreadSolver.endBlock_mutex,
                            USYNC_THREAD, 0);
                cond_init (&TCB_ProfileSPDDirectThreadSolver.start_cond,
                           USYNC_THREAD, 0);
                cond_init (&TCB_ProfileSPDDirectThreadSolver.end_cond,
                           USYNC_THREAD, 0);
                cond_init (&TCB_ProfileSPDDirectThreadSolver.startBlock_cond,
                           USYNC_THREAD, 0);
                cond_init (&TCB_ProfileSPDDirectThreadSolver.endBlock_cond,
                           USYNC_THREAD, 0);
                ...
                for (int j = 0; j < NP; j++)
                    thr_create (NULL, 0,
                                ProfileSPDLinDirectThreadSolver_Worker, NULL,
          ...
          while (TCB_ProfileSPDDirectThreadSolver.threadsDone < NP)
              cond_wait (&TCB_ProfileSPDDirectThreadSolver.end_cond,
                         &TCB_ProfileSPDDirectThreadSolver.end_mutex);
...
int
ProfileSPDLinDirectThreadSolver::sendSelf (int cTag, Channel & theChannel)
{
    if (size != 0)
        opserr <<
            "ProfileSPDLinDirectThreadSolver::sendSelf - does not send itself YET\n";
    return 0;
}
    ...
    while (true)
      {
          ...
          while (TCB_ProfileSPDDirectThreadSolver.workToDo == 0)
              cond_wait (&TCB_ProfileSPDDirectThreadSolver.start_cond,
                         &TCB_ProfileSPDDirectThreadSolver.start_mutex);
                ...
                if (myID == currentDiagThread)
                  {
                      ...
                      mutex_lock (&TCB_ProfileSPDDirectThreadSolver.
                                  startBlock_mutex);
                      TCB_ProfileSPDDirectThreadSolver.currentBlock = i;

                      cond_broadcast (&TCB_ProfileSPDDirectThreadSolver.
                                      startBlock_cond);
                      mutex_unlock (&TCB_ProfileSPDDirectThreadSolver.
                                    startBlock_mutex);
                ...
                else
                  {
                      ...
                      mutex_lock (&TCB_ProfileSPDDirectThreadSolver.
                                  startBlock_mutex);
                      while (TCB_ProfileSPDDirectThreadSolver.currentBlock <
                             i)
                          cond_wait (&TCB_ProfileSPDDirectThreadSolver.
                                     startBlock_cond,
                                     &TCB_ProfileSPDDirectThreadSolver.
                                     startBlock_mutex);
                      mutex_unlock (&TCB_ProfileSPDDirectThreadSolver.
