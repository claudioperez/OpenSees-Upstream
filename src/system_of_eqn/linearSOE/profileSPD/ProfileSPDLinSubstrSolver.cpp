...
ProfileSPDLinSubstrSolver::condenseA (int numInt)
{
    ...
    opserr << "ProfileSPDLinSubstrSolver::condenseA  numDOF: " << size <<
        "  numInt: " << numInt << "  numExt: " << size - numInt << endln;
...
ProfileSPDLinSubstrSolver::condenseRHS (int numInt, Vector * v)
{
    ...
    if (theSOE->numInt != numInt)
      {
          opserr << "ProfileSPDLinSubstrSolver::condenseRHS()";
          opserr << " - numInt " << numInt <<
              "does not agree with condensedA";
...
int
ProfileSPDLinSubstrSolver::computeCondensedMatVect (int numInt,
                                                    const Vector & theVect)
{
...
int
ProfileSPDLinSubstrSolver::sendSelf (int cTag, Channel & theChannel)
{
    if (size != 0)
        opserr <<
            "ProfileSPDLinSubstrSolver::sendSelf - does not send itself YET\n";
    return 0;
}
