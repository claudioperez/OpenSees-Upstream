...
DistributedSparseGenRowLinSOE::DistributedSparseGenRowLinSOE (DistributedSparseGenRowLinSolver & the_Solver):LinearSOE (the_Solver, LinSOE_TAGS_DistributedSparseGenRowLinSOE),
...
int
DistributedSparseGenRowLinSOE::addA (const Matrix & m, const ID & id,
                                     double fact)
{
          ...
          if (row < n && row >= 0)
            {
                ...
                if (localRow == -1)
                    opserr <<
                        "DistributedSparseGenRowLinSOE::addA() - you goofed\n";
...
int
DistributedSparseGenRowLinSOE::addB (const Vector & v, const ID & id,
                                     double fact)
{
    // check for a quick return 
    if (fact == 0.0)
...
setDistributedSparseGenRowSolver (DistributedSparseGenRowLinSolver &
                                  newSolver)
{
    ...
    if (n != 0)
      {
          ...
          if (solverOK < 0)
            {
                opserr <<
                    "WARNING:DistributedSparseGenRowLinSOE::setSolver :";
                opserr <<
