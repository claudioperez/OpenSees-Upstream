...
DistributedSuperLU::DistributedSuperLU ():SparseGenColLinSolver (SOLVER_TAGS_DistributedSuperLU),
    ...
    if (iam < (npRow * npCol))
      {
          ...
          pdgssvx_ABglobal (&options, &A, &ScalePermstruct, Xptr, ldb, nrhs,
...
DistributedSuperLU::setChannels (int nChannels, Channel ** theC)
{
    ...
    if (theChannels == 0)
      {
          opserr <<
              "DistributedSuperLU::sendSelf() - failed to allocate channel array of size: "
              << numChannels << endln;
          return -1;
      }
...
DistributedSuperLU::sendSelf (int cTag, Channel & theChannel)
{
    ...
    if (res < 0)
      {
          opserr <<
              "WARNING DistributedSuperLU::sendSelf() - failed to send data\n";
          return -1;
      }
                              ...
                              FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0)
      {
          opserr <<
              "WARNING DistributedSuperLU::recvSelf() - failed to receive data\n";
          return -1;
      }
