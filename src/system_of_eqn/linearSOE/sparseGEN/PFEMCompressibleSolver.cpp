...
PFEMCompressibleSolver::PFEMCompressibleSolver ():LinearSOESolver (SOLVER_TAGS_PFEMCompressibleSolver),
theSOE (0)
{
...
PFEMCompressibleSolver::solve ()
{
    ...
    void *Symbolic = 0;
    int status =
        umfpack_di_symbolic (Vsize, Vsize, Ap, Ai, Ax, &Symbolic, Control,
                             Info);
    ...
    double *x_ptr = &x (0);
    status =
        umfpack_di_solve (UMFPACK_A, Ap, Ai, Ax, x_ptr, dV_ptr, Numeric,
                          Control, Info);
    dV = x;
...
int
PFEMCompressibleSolver::setSize ()
{
    return 0;
}
