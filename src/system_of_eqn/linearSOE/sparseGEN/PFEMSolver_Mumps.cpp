...
OPS_PFEMSolver_Mumps ()
{
    ...
    PFEMSolver_Mumps *theSolver =
        new PFEMSolver_Mumps (relax, err, add, sym, print,
                              ptol, maxiter, Bitol);
    ...
    if (Msize > 0 && stage != 2)
      {
          ...
          if (sid.info[0] != 0)
            {
                opserr <<
                    "WARNING: failed to factorize -- PFEMSolver_Mumps::solve\n";
                return -1;
            }
      }
    ...
    if (Msize > 0 && stage != 2)
      {
          ...
          if (sid.info[0] != 0)
            {
                opserr <<
                    "WARNING: failed to solve predictor -- PFEMSolver_Mumps::solve\n";
                return -1;
            }
      }
          ...
          if (Isize > 0 && stage == 0)
            {
                ...
                if (sid.info[0] != 0)
                  {
                      opserr << "info[1] = " << sid.
                          info[0] << ", info[2] =" << sid.info[1] << "\n";
                      opserr <<
                          "WARNING: failed to solve Bi -- PFEMSolver_Mumps::solve\n";
                      return -1;
                  }
                ICNTL (sid, 20, 0);
                ICNTL (sid, 30, 0);
          ...
          if (S->nzmax > 0)
            {
                ...
                Solver solve (amgcl::adapter::
                              zero_copy (Psize, &ptr[0], &num[0], val), prm);
                prof.toc ("setup");
    ...
    if (Isize > 0 && stage == 0)
      {
          ...
          if (sid.info[0] != 0)
            {
                opserr <<
                    "WARNING: failed to solve corrector -- PFEMSolver_Mumps::solve\n";
                return -1;
            }
      }
...
PFEMSolver_Mumps::setSize ()
{
    ...
    if (sid.info[0] != 0)
      {
          opserr <<
              "WARNING: failed to analyze -- PFEMSolver_Mumps::setSize\n";
          return -1;
      }
#endif
    return 0;
