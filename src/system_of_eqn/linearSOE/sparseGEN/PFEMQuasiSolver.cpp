...
PFEMQuasiSolver::PFEMQuasiSolver ():LinearSOESolver (SOLVER_TAGS_PFEMQuasiSolver),
theSOE (0)
{
...
PFEMQuasiSolver::solve ()
{
    ...
    void *Numeric = 0;
    status =
        umfpack_di_numeric (M->p, M->i, M->x, Symbolic, &Numeric, Control,
                            Info);
    if (status != UMFPACK_OK)
      {
