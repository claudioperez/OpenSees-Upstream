...
PFEMGeneralLinSOE::PFEMGeneralLinSOE ():PFEMLinSOE (LinSOE_TAGS_PFEMGeneralLinSOE), N (0),
rowInd (), colPtr (), A ()
{
}

PFEMGeneralLinSOE::PFEMGeneralLinSOE (PFEMUnifiedSolver_Hybrid & theSolver):PFEMLinSOE (LinSOE_TAGS_PFEMGeneralLinSOE), N (0),
rowInd (), colPtr (),
A ()
{
    ...
    for (int j = 0; j < dofType.Size (); j++)
      {
          ...
          if (theVertex == 0)
            {
                opserr << "WARNING: vertex " << j <<
                    " not in graph -- PFEMGeneralLinSOE::setSize\n";
                return -1;
            }
