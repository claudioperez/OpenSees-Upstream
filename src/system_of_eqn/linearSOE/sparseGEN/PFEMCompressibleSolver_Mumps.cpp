...
PFEMCompressibleSolver_Mumps::PFEMCompressibleSolver_Mumps (int r, int e,
                                                            int s):
    ...
    if (myid == 0)
      {
          if (sid.info[0] != 0)
            {
                opserr << "WARNING: failed to solve velocity with error code "
                    << sid.info[0];
...
int
PFEMCompressibleSolver_Mumps::setSize ()
{
    return 0;
}
