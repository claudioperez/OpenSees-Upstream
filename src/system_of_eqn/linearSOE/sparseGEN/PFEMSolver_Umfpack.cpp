...
OPS_PFEMSolver_Umfpack ()
{
                ...
                if (OPS_GetNumRemainingInputArgs () > 0)
                  {
                      if (OPS_GetIntInput (&numdata, &maxiter) < 0)
                        {
                            opserr <<
                                "WARNING: failed to get max iteration for pressure\n";
                            return 0;
                        }
                  }
            }
    ...
    PFEMSolver_Umfpack *theSolver =
        new PFEMSolver_Umfpack (ptol, maxiter, print);
    ...
    if (Msize > 0)
      {
          ...
          double *Ax = M->x;
          int status =
              umfpack_di_numeric (Ap, Ai, Ax, Symbolic, &Numeric, Control,
                                  Info);
    ...
    if (Msize > 0)
      {
          ...
          double *Ax = M->x;
          int status =
              umfpack_di_solve (UMFPACK_A, Ap, Ai, Ax, x_ptr, deltaV1_ptr,
          ...
          for (int j = 0; j < Isize; j++)
            {
                ...
                double *Ax = M->x;
                int status =
                    umfpack_di_solve (UMFPACK_A, Ap, Ai, Ax, x_ptr, eyes_ptr,
                                      Numeric, Control, Info);
                eyes = x;
                ...
                if (status != UMFPACK_OK)
                  {
                      opserr << "WARNING: solving interface returns " <<
                          status << " -- PFEMSolver_Umfpack::solve\n";
                      if (Numeric != 0)
                        {
                            umfpack_di_free_numeric (&Numeric);
                        }
          ...
          if (S->nzmax > 0)
            {
                ...
                try
                {
                    ...
                    typedef
                        ...
                        >,
                        amgcl::solver::lgmres < amgcl::backend::builtin <
                        double >>>Solver;
                    ...
                    // setup
                    prof.tic ("setup");
                    std::vector < std::ptrdiff_t >ptr (Psize + 1),
                        num (S->nzmax);
                    ...
                    double *val = &(S->x[0]);
                    Solver solve (amgcl::adapter::
                                  zero_copy (Psize, &ptr[0], &num[0], val),
                                  prm);
                    prof.toc ("setup");
                ...
                {
                    opserr <<
                        "Pressure: AMGCL solver failed -- Fall back to Umfpack solver\n";
#endif
                    int *Sp = S->p;
                    int *Si = S->i;
                    double *Sx = S->x;
                    ...
                    void *Ssymbolic = 0;
                    int status =
                        umfpack_di_symbolic (Psize, Psize, Sp, Si, Sx,
                                             &Ssymbolic, Control, Info);
                    // check error
                    if (status != UMFPACK_OK)
                      {
                          opserr <<
                              "WARNING: pressure symbolic analysis returns "
                              << status <<
                              " -- PFEMSolver_Umfpack::setsize\n";
                          return -1;
                      }
                    ...
                    void *SNumeric = 0;
                    status =
                        umfpack_di_numeric (Sp, Si, Sx, Ssymbolic, &SNumeric,
                                            Control, Info);
                    ...
                    if (status != UMFPACK_OK)
                      {
                          opserr <<
                              "WARNING: pressure numeric analysis returns " <<
                              status << " -- PFEMSolver_Umfpack::solve\n";
                          return -1;
                      }
                    ...
                    status =
                        umfpack_di_solve (UMFPACK_A, Sp, Si, Sx, soln_ptr,
                                          rhsP_ptr, SNumeric, Control, Info);
                    ...
                    if (status != UMFPACK_OK)
                      {
                          opserr << "WARNING: pressure solving returns " <<
                              status << " -- PFEMSolver_Umfpack::solve\n";
                          return -1;
                      }
