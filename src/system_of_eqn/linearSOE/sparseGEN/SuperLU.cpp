...
OPS_SuperLUSolver ()
{
    ...
    while (OPS_GetNumRemainingInputArgs () > 1)
      {
          ...
          if (type == "p" || type == "piv" || type == "-piv")
            ...
            {
                if (OPS_GetIntInput (&numData, &np) < 0)
                    return 0;
            }
          else if (type == "npRow" || type == "-npRow")
            {
                if (OPS_GetIntInput (&numData, &npRow) < 0)
                    return 0;
            }
          else if (type == "npCol" || type == "-npCol")
            {
                if (OPS_GetIntInput (&numData, &npCol) < 0)
                    return 0;
            }
    ...
    SparseGenColLinSolver *
        theSolver =
        new SuperLU (permSpec, drop_tol, panelSize, relax, symmetric);
    if (theSolver == 0)
      {
...
SuperLU::SuperLU (int perm,
                  double drop_tolerance, int panel, int relx, char symm):
    ...
    if (n > 0)
      {
          ...
          dCreate_Dense_Matrix (&B, n, 1, theSOE->X, n, SLU_DN, SLU_D,
                                SLU_GE);
