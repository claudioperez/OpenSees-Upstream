...
PFEMUnifiedSolver_Hybrid::PFEMUnifiedSolver_Hybrid (int r, int e, int h,
                                                    int s):
LinearSOESolver (SOLVER_TAGS_PFEMUnifiedSolver_Hybrid),
theSOE (0),
id (),
myid (0),
handle (),
...
sym (s)
{
    ...
    if (cusolverDnCreate (&handle) != CUSOLVER_STATUS_SUCCESS)
      {
          opserr <<
              "WARNING: failed to create cusolver handle--PFEMUnifiedSolver_Hybrid\n";
          exit (-1);
      }
}
...
PFEMUnifiedSolver_Hybrid::solve ()
{
    ...
    if (id.info[0] != 0)
      {
          opserr <<
              "WARNING: failed to solve predictor -- PFEMUnifiedSolver_Hybrid::solve\n";
          return -1;
      }
    ...
    if (id.info[0] != 0)
      {
          opserr <<
              "WARNING: failed to get pressure rhs -- PFEMUnifiedSolver_Hybrid::solve\n";
          return -1;
      }
    ...
    if (myid == 0)
      {
          ...
          double *d_S, *d_dP;
          cudaMalloc ((void **) &d_S,
                      id.size_schur * id.size_schur * sizeof (double));
          ...
          cudaMemcpy (d_dP, h_dP, id.size_schur * sizeof (double),
                      cudaMemcpyHostToDevice);

          // get workspace size
          int lda = id.size_schur, Lwork = 0;
          status =
              cusolverDnDgetrf_bufferSize (handle, id.size_schur,
                                           id.size_schur, d_S, lda, &Lwork);
          if (status != CUSOLVER_STATUS_SUCCESS)
            {
                opserr <<
                    "WARNING: failed to get GPU workspace size--PFEMUnifiedSolver_Hybrid::solve\n";
                return -1;
            }
          ...
          if (status != CUSOLVER_STATUS_SUCCESS)
            {
                opserr <<
                    "WARNING: failed to do LU in GPU for pressure--PFEMUnifiedSolver_Hybrid::solve\n";
                return -1;
            }
          ...
          status =
              cusolverDnDgetrs (handle, trans, lda, nrhs, d_S, lda, d_ipiv,
                                d_dP, ldb, d_info);
          if (status != CUSOLVER_STATUS_SUCCESS)
            {
                opserr <<
                    "WARNING: failed to solve pressure in GPU--PFEMUnifiedSolver_Hybrid::solve\n";
                return -1;
            }

          // check results
          cudaMemcpy (h_dP, d_dP, id.size_schur * sizeof (double),
    ...
    if (id.info[0] != 0)
      {
          opserr <<
              "WARNING: failed to solve corrector -- PFEMUnifiedSolver_Hybrid::solve\n";
          return -1;
      }
...
int
PFEMUnifiedSolver_Hybrid::setSize ()
{
    Timer timer;
    timer.start ();
    ...
    if (id.info[0] != 0)
      {
          opserr <<
              "WARNING: failed to analyze -- PFEMUnifiedSolver_Hybrid::setSize\n";
          return -1;
      }
