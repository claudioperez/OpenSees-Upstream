...
DistributedSparseGenColLinSOE::DistributedSparseGenColLinSOE (SparseGenColLinSolver & theSolvr):SparseGenColLinSOE (theSolvr, LinSOE_TAGS_DistributedSparseGenColLinSOE),
...
DistributedSparseGenColLinSOE::DistributedSparseGenColLinSOE ():SparseGenColLinSOE (LinSOE_TAGS_DistributedSparseGenColLinSOE),
processID (0), numChannels (0), theChannels (0), localCol (0), workArea (0),
sizeWork (0), myB (0), myVectB (0)
{
          ...
          if (nnz > Asize)
            {                   // we have to get more space for A and rowA
                ...
                if (rowA == 0)
                  {
                      opserr <<
                          "WARNING SparseGenColLinSOE::SparseGenColLinSOE :";
                      opserr <<
                          " ran out of memory for A and rowA with nnz = ";
                      opserr << nnz << " \n";
                      size = 0;
                      Asize = 0;
                      nnz = 0;
...
int
DistributedSparseGenColLinSOE::addA (const Matrix & m, const ID & id,
                                     double fact)
{
                            ...
                            if (row < size && row >= 0)
                              {
                                  ...
                                  for (int k = startColLoc; k < endColLoc;
                                       k++)
                                      if (rowA[k] == row)
                                        {
                                            A[k] += m (j, i);
...
int
DistributedSparseGenColLinSOE::addB (const Vector & v, const ID & id,
                                     double fact)
{
    // check for a quick return 
    if (fact == 0.0)
          ...
          if (found == false)
            {
                ...
                if (nextNumChannels == 0)
                  {
                      opserr <<
                          "DistributedSparseGenColLinSOE::sendSelf() - failed to allocate channel array of size: "
                ...
                if (localCol == 0)
                  {
                      opserr <<
                          "DistributedSparseGenColLinSOE::sendSelf() - failed to allocate id array of size: "
                          << nextNumChannels << endln;
                      return -1;
                  }
                for (int i = 0; i < numChannels; i++)
...
int
DistributedSparseGenColLinSOE::recvSelf (int commitTag, Channel & theChannel,
                                         FEM_ObjectBroker & theBroker)
{
    ID idData (1);
    ...
    if (res < 0)
      {
          opserr <<
              "WARNING DistributedSparseGenColLinSOE::recvSelf() - failed to send data\n";
          return -1;
      }
    processID = idData (0);
