...
CulaSparseSolverS5::CulaSparseSolverS5 (double relTol,
                                        int maxInteration,
                                        int preCond,
                                        int solver, int single, int host):
SparseGenRowLinSolver (SOLVER_TAGS_CulaSparseS5)
{
    //config.debug = 1;
    ...
    switch (preCond)
      {
      case 0:
          status = culaSparseSetNoPreconditioner (handle, plan, 0);
          break;
      case 1:
          status = culaSparseSetJacobiPreconditioner (handle, plan, 0);
          break;
      case 2:
          status = culaSparseSetBlockJacobiPreconditioner (handle, plan, 0);
          break;
      case 3:
          status = culaSparseSetIlu0Preconditioner (handle, plan, 0);
          break;
      case 4:
          status = culaSparseSetAinvPreconditioner (handle, plan, 0);
          break;
      case 5:
          culaSparseFainvOptions fainv;
    ...
    switch (solver)
      {
      case 0:
          status = culaSparseSetCgSolver (handle, plan, 0);
          break;
      case 1:
          status = culaSparseSetBicgSolver (handle, plan, 0);
          break;
      case 2:
          status = culaSparseSetBicgstabSolver (handle, plan, 0);
          break;
      case 3:
          status = culaSparseSetBicgstablSolver (handle, plan, 0);
          break;
      case 4:
          status = culaSparseSetGmresSolver (handle, plan, 0);
          break;
      case 5:
          status = culaSparseSetMinresSolver (handle, plan, 0);
          break;
      default:
          status = culaSparseArgumentError;
          return;
...
CulaSparseSolverS5::solve (void)
{
    ...
    if (single == 0)
      {
          culaSparseSetDcsrData (handle, plan, &CsrOpt, n, nnz, Aptr, rowPtr,
                                 colInd, Xptr, Bptr);
      }
    else if (single == 1)
      {
          ...
          culaSparseSetScsrData (handle, plan, &CsrOpt, n, nnz, Asingle,
                                 rowPtr, colInd, Xsingle, Bsingle);
