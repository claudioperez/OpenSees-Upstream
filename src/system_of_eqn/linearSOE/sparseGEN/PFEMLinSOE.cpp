...
PFEMLinSOE::PFEMLinSOE (PFEMSolver & the_Solver):LinearSOE (the_Solver, LinSOE_TAGS_PFEMLinSOE),
M (0), Gft (0), Git (0), L (0), Qt (0),
X (), B (), Mhat (), Mf (), dofType (), dofID (), assemblyFlag (0), stage (0)
{
    the_Solver.setLinearSOE (*this);
}


PFEMLinSOE::PFEMLinSOE ():LinearSOE (LinSOE_TAGS_PFEMLinSOE),
M (0), Gft (0), Git (0), L (0), Qt (0),
X (), B (), Mhat (), Mf (), dofType (), dofID (), assemblyFlag (0), stage (0)
{
                ...
                for (int j = 0; j < idSize; j++)
                  {
                      ...
                      else if (rowtype == 2 && coltype == 2)
                        {       // Ms
                            mat = M;
                            rowid += Ssize;
                            cid += Ssize;
                        }
                      else if (rowtype == 0 && coltype == 2)
                        {       // Msi
                            mat = M;
                            cid += Ssize;
                        }
                      else if (rowtype == 2 && coltype == 0)
                        {       // Mis
                            mat = M;
                            rowid += Ssize;
                        }
                      else if (rowtype == 3 && coltype == 1)
                        {       // Gft
                            if (hasFluid)
                                mat = Gft;
                        }
                      else if (rowtype == 3 && coltype == 2)
                        {       // Git
                            mat = Git;
                        }
                      else if (rowtype == 3 && coltype == 3)
                        {       // L
                            if (hasFluid)
                                mat = L;
                        }
                      else if (rowtype == 4 && coltype == 3)
                        {       // Qt
                            if (hasFluid)
                                mat = Qt;
                        }
    ...
    if (v.Size () != B.Size ())
      {
          ...
          opserr << " incompatible sizes " << B.Size () << " and " << v.
              Size () << endln;
          return -1;
      }
...
int
PFEMLinSOE::setDofIDs (int size, int &Ssize, int &Fsize, int &Isize,
                       int &Psize, int &Pisize)
{
    if (theModel == 0)
      {
          opserr <<
              "Analysis model has not been linked - PFEMLinSOE::setDofIDs()\n";
          return -1;
      }
    ...
    while ((thePC = thePCs ()) != 0)
      {
          ...
          if (nnode == 0)
            {
                opserr << "WARNING: pressure node " << ntag <<
                    " does not exists ";
                opserr << " -- PFEMLinSOE::setDofIDs\n";
                return -1;
            }
          ...
          if (pnode != 0)
            {
                ...
                if (pDOF == 0)
                  {
                      opserr << "WARNING: no DOF_Group is found with node " <<
                          ptag;
                      opserr << " -- PFEMLinSOE::setDofIDs\n";
                      return -1;
                  }
...
int
PFEMLinSOE::setMatIDs (Graph & theGraph, int Ssize, int Fsize, int Isize,
                       int Psize, int Pisize)
{
    ...
    for (int a = 0; a < size; a++)
      {                         // columns
          ...
          int colid = dofID (col);      // column id
          if (coltype == 4 || coltype < 0)
              continue;         // don't need this column
...
{
    BackgroundMesh & bgmesh = OPS_getBgMesh ();
    return assemblyFlag == 1 && bgmesh.isDispOn () == false
        && bgmesh.isFastAssembly ();
}

void
PFEMLinSOE::saveK (OPS_Stream & output)
{
    if (M == 0)
        return;
