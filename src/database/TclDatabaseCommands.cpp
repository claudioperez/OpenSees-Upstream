...
typedef struct databasePackageCommand {
    ...
    int (*funcPtr)(ClientData clientData,
                   ...
                   int argc,
                   TCL_Char ** argv,
                   Domain *, FEM_ObjectBroker *, FE_Datastore **);
               ...
               FEM_ObjectBroker & theBroker)
{
    ...
    if (argc < 2) {
        opserr <<
            "WARNING need to specify a Database type; valid type File, MySQL, BerkeleyDB \n";
        return TCL_ERROR;
    }
    //
    ...
    if (strcmp(argv[1], "File") == 0) {
        ...
        if (theDatabase == 0) {
            opserr << "WARNING ran out of memory - database File " <<
                argv[2] << endln;
            return TCL_ERROR;
        }
        ...
        while (dataCommands != NULL && found == false) {
            if (strcmp(argv[1], dataCommands->funcName) == 0) {
                int result =
                    (*(dataCommands->funcPtr)) (clientData, interp,
                                                argc, argv, &theDomain,
                                                &theBroker,
                                                &theDatabase);
                return result;
        ...
        if (res == 0) {
            ...
            DatabasePackageCommand *theDataCommand =
    ...
    opserr << "for database of type:" << argv[1] <<
        ...
        TCL_Char ** argv)
{
    ...
    if (argc < 2) {
        opserr <<
            "WARNING restore no commit tag - want restore commitTag?";
        return TCL_OK;
    }
    // check argv[1] for commitTag
    int commitTag;
