                       ...
                       const ID & theID, ChannelAddress * theAddress)
{
    ...
    if (idSize >= maxIDsize) {
        opserr <<
            "FileDatastore::sendID() - the database does not deal with IDs of this size ";
        opserr << idSize << endln;
    }
                          ...
                          ID & theID, ChannelAddress * theAddress)
{
    ...
    if (idSize >= maxIDsize) {
        opserr <<
            "FileDatastore::recvID() - the database does not deal with IDs";
    ...
    if (found == false) {
        opserr <<
            "FileDatastore::recvID() - failed to find data for ID of size ";
        opserr << idSize << endln;
        return -1;
    }
