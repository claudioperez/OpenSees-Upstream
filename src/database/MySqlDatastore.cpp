...
MySqlDatastore::MySqlDatastore(const char *projectName,
                               ...
                               int run):FE_Datastore(theDomain,
                                                     theObjectBroker),
dbTag(0), dbRun(run), connection(true), query(0), sizeQuery(0),
sizeColumnString(0)
{
    ...
    if (mysql_real_connect
        (&mysql, NULL, NULL, NULL, NULL, 0, "/tmp/mysql.sock", 0) == NULL)
    {

        opserr <<
            "MySqlDatastore::MySqlDatastore() - could not connect to server\n";
        ...
        if (mysql_select_db(&mysql, projectName) != 0) {
            ...
            if (this->createOpenSeesDatabase(projectName) != 0) {
                ...
                mysql_close(&mysql);
                opserr <<
                    "MySqlDatastore::MySqlDatastore() - could not open the database\n";
                opserr << mysql_error(&mysql) << endln;
            }
        }
    }
...
int MySqlDatastore::recvMsg(int dataTag, int commitTag,
...
int MySqlDatastore::sendMatrix(int dbTag, int commitTag,
                               ...
                               ChannelAddress * theAddress)
{
    ...
    if (sizeData > MAX_BLOB_SIZE) {
        opserr <<
            "MySqlDatastore::sendMatrix - vector too big to send to MySQL databse, enlarge BLOBS!";
        return -2;
    }
    ...
    if ((query == 0) || ((sizeData * 2 + 254) > sizeQuery)) {   // *2 for extra space MySQL may need
        ...
        if (query == 0) {
            opserr <<
                "MySqlDatastore::sendMatrix - out of memory creating query of size";
    ...
    sprintf(query, "INSERT INTO Matrices VALUES (%d, %d, %d,'", dbTag,
            commitTag, theMatrix.dataSize);
    ...
    if (mysql_query(&mysql, query) != 0) {
        ...
        sprintf(p, "' WHERE dbTag=%d AND commitTag=%d AND size=%d", dbTag,
                commitTag, theMatrix.dataSize);

        // invoke the query on the database
        if (mysql_query(&mysql, query) != 0) {
            opserr <<
                "MySqlDatastore::sendMatrix() - failed to send the Matrix to MySQL database";
...
int MySqlDatastore::recvMatrix(int dbTag, int commitTag,
                               Matrix & theMatrix,
                               ChannelAddress * theAddress)
{
    // check that we have a connection
    ...
    if ((query == 0) || ((sizeData * 2 + 256) > sizeQuery)) {   // *2 for extra space mysql needs
        ...
        if (query == 0) {
            opserr <<
                "MySqlDatastore::recvMatrix - out of memory creating query of size";
            opserr << sizeQuery << endln;
            return -2;
        }
    }
    ...
    sprintf(query,
            "Select data FROM Matrices WHERE dbTag=%d AND commitTag=%d AND size=%d",
            dbTag, commitTag, theMatrix.dataSize);

    // execute the SELECT query
    if (mysql_query(&mysql, query) != 0) {
        opserr <<
            "MySqlDatastore::recvMatrix() - failed to receive vector from MySQL database";
        opserr << endln << mysql_error(&mysql) << endln;
        return -3;
    }
    ...
    if (results == NULL) {
        ...
        opserr <<
            "MySqlDatastore::recvMatrix - no data in database for Matrix with dbTag, cTag: ";
...
int MySqlDatastore::sendVector(int dbTag, int commitTag,
                               ...
                               ChannelAddress * theAddress)
{
    ...
    if (sizeData > MAX_BLOB_SIZE) {
        opserr <<
            "MySqlDatastore::sendVector - vector too big to send to MySQL databse, enlarge BLOBS!";
        return -2;
    }
    // check that query string is big enough to hold the data, if not enlarge
    if ((query == 0) || ((sizeData * 2 + 254) > sizeQuery)) {   // *2 for extra space MySQL may need
        ...
        if (query == 0) {
            opserr <<
                "MySqlDatastore::sendVector - out of memory creating query of size";
            opserr << sizeQuery << endln;
            return -2;
        }
    }
    ...
    // form the INSERT query
    sprintf(query, "INSERT INTO Vectors VALUES (%d, %d, %d,'", dbTag,
            commitTag, theVector.sz);
    char *p = query + strlen(query);
    ...
    if (mysql_query(&mysql, query) != 0) {
        ...
        if (mysql_query(&mysql, query) != 0) {
            opserr <<
                "MySqlDatastore::sendVector() - failed to send the Vector to MySQL database";
            opserr << endln << mysql_error(&mysql) << endln;
            return -3;
        }
    }
...
int MySqlDatastore::recvVector(int dbTag, int commitTag,
                               Vector & theVector,
                               ChannelAddress * theAddress)
{
    // check that we have a connection
    ...
    if ((query == 0) || ((sizeData * 2 + 256) > sizeQuery)) {   // *2 for extra space mysql needs
        ...
        if (query == 0) {
            opserr <<
                "MySqlDatastore::recvVector - out of memory creating query of size";
            opserr << sizeQuery << endln;
            return -2;
        }
    }
    ...
    // form the SELECT query
    sprintf(query,
            "Select data FROM Vectors WHERE dbTag=%d AND commitTag=%d AND size=%d",
            dbTag, commitTag, theVector.sz);

    // execute the SELECT query
    if (mysql_query(&mysql, query) != 0) {
        opserr <<
            "MySqlDatastore::recvVector() - failed to receive vector from MySQL database";
        opserr << endln << mysql_error(&mysql) << endln;
        return -3;
    }
    // fetch the results from the database
    ...
    if (results == NULL) {
        // no vector stored in db with these keys
        opserr <<
            "MySqlDatastore::recvVector - no data in database for Vector with dbTag, cTag: ";
        opserr << dbTag << ", " << commitTag << endln;
        return -4;
    }
    row = mysql_fetch_row(results);
...
int MySqlDatastore::sendID(int dbTag, int commitTag,
                           const ID & theID, ChannelAddress * theAddress)
{
    ...
    if (sizeData > MAX_BLOB_SIZE) {
        opserr <<
            "MySqlDatastore::sendID - vector too big to send to MySQL databse, enlarge BLOBS!";
        return -2;
    }
    // check that query string is big enough to hold the data, if not enlarge
    if ((query == 0) || ((sizeData * 2 + 254) > sizeQuery)) {   // *2 for extra space MySQL may need
        ...
        if (query == 0) {
            opserr <<
                "MySqlDatastore::sendID - out of memory creating query of size";
            opserr << sizeQuery << endln;
            return -2;
        }
    }
    ...
    // form the INSERT query
    sprintf(query, "INSERT INTO IDs VALUES (%d, %d, %d,'", dbTag,
            commitTag, theID.sz);
    char *p = query + strlen(query);
    ...
    if (mysql_query(&mysql, query) != 0) {
        ...
        if (mysql_query(&mysql, query) != 0) {
            opserr <<
                "MySqlDatastore::sendID() - failed to send the ID to MySQL database";
            opserr << endln << mysql_error(&mysql) << endln;
            return -3;
        }
    }
...
int MySqlDatastore::recvID(int dbTag, int commitTag,
                           ID & theID, ChannelAddress * theAddress)
{
    ...
    if ((query == 0) || ((sizeData * 2 + 256) > sizeQuery)) {   // *2 for extra space mysql needs
        ...
        if (query == 0) {
            opserr <<
                "MySqlDatastore::recvID - out of memory creating query of size";
            opserr << sizeQuery << endln;
            return -2;
        }
    }
    ...
    // form the SELECT query
    sprintf(query,
            "Select data FROM IDs WHERE dbTag=%d AND commitTag=%d AND size=%d",
            dbTag, commitTag, theID.sz);

    // execute the SELECT query
    if (mysql_query(&mysql, query) != 0) {
        opserr <<
            "MySqlDatastore::recvID() - failed to receive vector from MySQL database";
        opserr << endln << mysql_error(&mysql) << endln;
        return -3;
    }
    // fetch the results from the database
    ...
    if (results == NULL) {
        // no vector stored in db with these keys
        opserr <<
            "MySqlDatastore::recvID - no data in database for ID with dbTag, cTag: ";
        opserr << dbTag << ", " << commitTag << endln;
        return -4;
    }
    row = mysql_fetch_row(results);
...
int MySqlDatastore::createTable(const char *tableName, int numColumns,
                                char *columns[])
{
    // check that we have a connection
    if (connection == false)
    ...
    char *p = query;
    int numChar = sprintf(query,
                          "CREATE TABLE %s (dbRun INT NOT NULL, commitTag INT NOT NULL, ",
                          tableName);
    p += numChar;
    ...
    if (mysql_query(&mysql, query) != 0) {
        if (mysql_errno(&mysql) != ER_TABLE_EXISTS_ERROR) {
            opserr <<
                "MySqlDatastore::createTable() - failed to create the table in the database";
            opserr << endln << mysql_error(&mysql) << endln;
...
int MySqlDatastore::insertData(const char *tableName, char *columns[],
                               int commitTag, const Vector & data)
{
    ...
    if ((query == 0) || (sizeData > sizeQuery)) {       // *2 for extra space mysql needs
        ...
        if (query == 0) {
            opserr <<
                "MySqlDatastore::getData - out of memory creating query of size";
            opserr << sizeQuery << endln;
            return -2;
        }
    }
    // form the insert query
    sprintf(query, "INSERT INTO %s VALUES (%d, %d ", tableName, dbRun,
            commitTag);
    char *p = query + strlen(query);
    ...
    if (mysql_query(&mysql, query) != 0) {
        ...
        // formulate the query
        sprintf(query, "UPDATE %s SET %s=%f", tableName, columns[0],
                data(0));
        char *p = query + strlen(query);
        ...
        if (mysql_query(&mysql, query) != 0) {
            opserr <<
                "MySqlDatastore::insertData() - failed to send the data to MySQL database";
            opserr << p;
            opserr << endln << mysql_error(&mysql) << endln;
            return -3;
        }
...
int MySqlDatastore::getData(const char *tableName, char *columns[],
                            int commitTag, Vector & data)
{
    ...
    if (mysql_query(&mysql, query) != 0) {
        opserr <<
            "MySqlDatastore::getData() - failed to receive vector from MySQL database";
        opserr << endln << mysql_error(&mysql) << endln;
        return -3;
    }
    // fetch the results from the database
    ...
    if (results == NULL) {
        // no vector stored in db with these keys
        opserr <<
            "MySqlDatastore::getData - no data in database for Vector with dbTag, cTag: ";
        opserr << dbTag << ", " << commitTag << endln;
        return -4;
    }
    row = mysql_fetch_row(results);
...
int MySqlDatastore::createOpenSeesDatabase(const char *projectName)
{
    if (query == 0) {
        query = new char[512];
        sizeQuery = 512;
    ...
    if (this->execute(query) != 0) {
        opserr <<
            "MySqlDatastore::createOpenSeesDatabase() - could not create the database\n";
        return -1;
    }
    // link to the database, 
    if (mysql_select_db(&mysql, projectName) != 0) {
        opserr <<
            "MySqlDatastore::createOpenSeesDatabase() - could not set the database\n";
        return -2;
    }
    // now create the tables in the database
    sprintf(query,
            "CREATE TABLE Messages ( dbTag INT NOT NULL, commitTag INT NOT NULL, size INT NOT NULL,\
                                          data MEDIUMBLOB, PRIMARY KEY (dbTag, commitTag, size) )");

    if (this->execute(query) != 0) {
        opserr <<
            "MySqlDatastore::createOpenSeesDatabase() - could not create the Messagess table\n";
        return -3;
    }

    sprintf(query,
            "CREATE TABLE Matrices ( dbTag INT NOT NULL, commitTag INT NOT NULL, size INT NOT NULL,\
                                         data MEDIUMBLOB, PRIMARY KEY (dbTag, commitTag, size) )");

    if (this->execute(query) != 0) {
        opserr <<
            "MySqlDatastore::createOpenSeesDatabase() - could not create the Matricess table\n";
        return -3;
    }

    sprintf(query,
            "CREATE TABLE Vectors ( dbTag INT NOT NULL, commitTag INT NOT NULL, size INT NOT NULL,\
                                         data MEDIUMBLOB, PRIMARY KEY (dbTag, commitTag, size) )");

    if (this->execute(query) != 0) {
        opserr <<
            "MySqlDatastore::createOpenSeesDatabase() - could not create the Vectors table\n";
        return -3;
    }

    sprintf(query,
            "CREATE TABLE IDs ( dbTag INT NOT NULL, commitTag INT NOT NULL, size INT NOT NULL,\
                                     data MEDIUMBLOB, PRIMARY KEY (dbTag, commitTag, size) )");

    if (this->execute(query) != 0) {
        opserr <<
            "MySqlDatastore::createOpenSeesDatabase() - could not create the ID's table\n";
        return -3;
    }
...
int MySqlDatastore::execute(const char *query)
{

    if (mysql_query(&mysql, query) != 0) {
        opserr << "MySqlDatastore::execute() - could not execute command: "
            << query;
        opserr << endln << mysql_error(&mysql) << endln;
        return -1;
    } else
