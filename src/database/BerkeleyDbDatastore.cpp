...
BerkeleyDbDatastore::BerkeleyDbDatastore(const char *projectName,
                                         ...
                                         theObjectBroker,
                                         char
                                         *dbType):FE_Datastore(theDomain,
                                                               theObjectBroker),
dbTag(0), connection(true)
{
    ...
    if (stat(project, &sb) != 0) {
        ...
        if (mkdir(project, S_IRWXU) != 0) {
            opserr <<
                "BerkeleyDbDatastore::BerkeleyDbDatastore - failed mkdir: ";
    ...
    if ((ret = db_env_create(&dbenv, 0)) != 0) {
        opserr <<
            "BerkeleyDbDatastore::BerkeleyDbDatastore - failed db_env_create: ";
    ...
    if ((ret = dbenv->open(dbenv, project,
                           DB_CREATE | DB_INIT_LOG | DB_PRIVATE |
                           DB_INIT_MPOOL, S_IRUSR | S_IWUSR)) != 0) {
    ...
    if ((result = db_create(&dbMatrix, dbenv, 0)) != 0) {
        opserr <<
            "BerkeleyDbDatastore::BerkeleyDbDatastore - failed to init dbMatrix";
    ...
    if ((result = db_create(&dbVector, dbenv, 0)) != 0) {
        opserr <<
            "BerkeleyDbDatastore::BerkeleyDbDatastore - failed to init dbVector";
        opserr << db_strerror(result) << endln;
        connection = false;
        return;
    }
    if ((result = db_create(&dbID, dbenv, 0)) != 0) {
        opserr <<
            "BerkeleyDbDatastore::BerkeleyDbDatastore - failed to init dbID";
        opserr << db_strerror(result) << endln;
        connection = false;
        return;
    }
    ...
    DBTYPE type = DB_HASH;
    if (dbType != NULL) {
        if ((strcmp(dbType, "hash") == 0)
            || (strcmp(dbType, "HASH") == 0))
            type = DB_HASH;
        else if ((strcmp(dbType, "queue") == 0)
                 || (strcmp(dbType, "QUEUE") == 0))
            type = DB_QUEUE;
        else if ((strcmp(dbType, "btree") == 0)
                 || (strcmp(dbType, "BTREE") == 0))
            type = DB_BTREE;
        else if ((strcmp(dbType, "recno") == 0)
                 || (strcmp(dbType, "RECNO") == 0))
            type = DB_RECNO;
    }
    ...
    if ((result = dbMatrix->open(dbMatrix, NULL, "Matrices.db", NULL, type,
        ...
        //type = DB_UNKNOWN;
        if ((result =
             dbMatrix->open(dbMatrix, NULL, "Matrices.db", NULL, type,
                            DB_CREATE, 0664)) != 0) {
#else
    if ((result = dbMatrix->open(dbMatrix, "Matrices.db", NULL, type,
        ...
        if ((result = dbMatrix->open(dbMatrix, "Matrices.db", NULL, type,
            ...
            opserr <<
                "BerkeleyDbDatastore::BerkeleyDbDatastore - failed to open dbMatrix\n";
            opserr << db_strerror(result) << endln;
            connection = false;
            return;
        }
    ...
    if ((result = dbVector->open(dbVector, NULL, "Vectors.db", NULL, type,
        ...
        //type = DB_UNKNOWN;
        if ((result =
             dbVector->open(dbVector, NULL, "Vectors.db", NULL, type,
                            DB_CREATE, 0664)) != 0) {
...
#endif
            opserr <<
                "BerkeleyDbDatastore::BerkeleyDbDatastore - failed to open dbVector\n";
            opserr << db_strerror(result) << endln;
            connection = false;
            return;
        }
    ...
    if ((result = dbID->open(dbID, "IDs.db", NULL, type,
        ...
        if ((result = dbID->open(dbID, "IDs.db", NULL, type,
            ...
            opserr <<
                "BerkeleyDbDatastore::BerkeleyDbDatastore - failed to open dbID\n";
            opserr << db_strerror(result) << endln;
            connection = false;
            return;
        } else
...
int BerkeleyDbDatastore::sendMsg(int dataTag, int commitTag,
                                 const Message &,
...
int BerkeleyDbDatastore::recvMsg(int dataTag, int commitTag,
...
int BerkeleyDbDatastore::sendMatrix(int dbTag, int commitTag,
                                    ...
                                    ChannelAddress * theAddress)
{
    ...
    if ((ret = dbMatrix->put(dbMatrix, NULL, &key, &data, 0)) != 0) {
        opserr <<
            "BerkeleyDbDatastore::sendMatrix() - failed to send the Matrix to database\n";
...
int BerkeleyDbDatastore::recvMatrix(int dbTag, int commitTag,
                                    Matrix & theMatrix,
                                    ChannelAddress * theAddress)
{
    // check that we have a connection
    ...
    if ((ret = dbMatrix->get(dbMatrix, NULL, &key, &data, 0)) != 0) {
        opserr <<
            "BerkeleyDbDatastore::recvMatrix() - failed to get the Matrix from database\n";
...
int BerkeleyDbDatastore::sendVector(int dbTag, int commitTag,
                                    ...
                                    ChannelAddress * theAddress)
{
    ...
    if ((ret = dbVector->put(dbVector, NULL, &key, &data, 0)) != 0) {
        opserr <<
            "BerkeleyDbDatastore::sendVector() - failed to send the Vector to database\n";
...
int BerkeleyDbDatastore::recvVector(int dbTag, int commitTag,
                                    Vector & theVector,
                                    ChannelAddress * theAddress)
{
    // check that we have a connection
    ...
    if ((ret = dbVector->get(dbVector, NULL, &key, &data, 0)) != 0) {
        opserr <<
            "BerkeleyDbDatastore::recvVector() - failed to get the Vector from database\n";
...
int BerkeleyDbDatastore::sendID(int dbTag, int commitTag,
                                const ID & theID,
                                ChannelAddress * theAddress)
{
    int ret = 0;
    ...
    if ((ret = dbID->put(dbID, 0, &key, &data, 0)) != 0) {
        opserr <<
            "BerkeleyDbDatastore::sendID() - failed to send the ID to database\n";
        dbID->err(dbID, ret, "DB->put");
        return -2;

    }
    opserr << dbTag << " " << commitTag << " " << theID.Size() << " " <<
        query << endln;
...
int BerkeleyDbDatastore::recvID(int dbTag, int commitTag,
                                ID & theID, ChannelAddress * theAddress)
{
    ...
    if ((ret = dbID->get(dbID, NULL, &key, &data, 0)) != 0) {
        opserr <<
            "BerkeleyDbDatastore::recvID() - failed to get the ID from database\n";
