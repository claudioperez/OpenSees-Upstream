...
CompositeResponse::addResponse (Response * nextResponse)
{
    ...
    if (otherType.theType == DoubleType || otherType.theType == VectorType)
      {
          ...
          if (myInfo.theType != VectorType)
            {
                opserr <<
                    "WARNING: CompositeResponse::addResponse() - mismatching type, no responses will be addeed\n";
                return -1;
            }
    ...
    if (theNextResponses == 0)
      {
          opserr <<
              "WARNING: CompositeResponse::addResponse() - out of memory, no responses will be added\n";
          return -1;
      }
                ...
                if (otherType.theType == DoubleType)
                ...
                else
                  {
                      ...
                      for (int i = 0; i < otherSize; i++, currentLoc++)
                          (*myInfo.theVector) (currentLoc) =
                              (*otherType.theVector) (i);
                  }
            }
