...
DriftRecorder::DriftRecorder ():Recorder (RECORDER_TAGS_DriftRecorder),
...
DriftRecorder::record (int commitTag, double timeStamp)
{
    ...
    if (theOutputHandler == 0)
      {
          opserr <<
              "DriftRecorder::record() - no DataOutputHandler has been set\n";
          return -1;
      }
...
DriftRecorder::sendSelf (int commitTag, Channel & theChannel)
{
    ...
    if (ndI != 0)
        if (theChannel.sendID (0, commitTag, *ndI) < 0)
          {
              opserr <<
                  "DriftRecorder::sendSelf() - failed to send dof id's\n";
              return -1;
          }
    ...
    if (theOutputHandler->sendSelf (commitTag, theChannel) < 0)
      {
          opserr <<
              "DriftRecorder::sendSelf() - failed to send the DataOutputHandler\n";
          return -1;
      }
                         ...
                         FEM_ObjectBroker & theBroker)
{
    ...
    if (idData (0) != 0)
      {
          ...
          if (theChannel.recvID (0, commitTag, *ndI) < 0)
            {
                opserr <<
                    "DriftRecorder::sendSelf() - failed to recv dof id's\n";
                return -1;
            }
      }
    ...
    if (theOutputHandler == 0)
      {
          opserr <<
              "DriftRecorder::sendSelf() - failed to get a data output handler\n";
          return -1;
      }
...
DriftRecorder::initialize (void)
{
    ...
    if (ndIsize != ndJsize)
      {
          opserr <<
              "DriftRecorder::initialize() - error node arrays differ in size\n";
          return -2;
      }
...
//added by SAJalali
double
DriftRecorder::getRecordedValue (int clmnId, int rowOffset, bool reset)
{
    double res = 0;
