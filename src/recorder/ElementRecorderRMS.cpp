...
ElementRecorderRMS::ElementRecorderRMS ():Recorder (RECORDER_TAGS_ElementRecorderRMS),
...
ElementRecorderRMS::ElementRecorderRMS (const ID * ele,
                                        ...
                                        double dT, const ID * indexValues):
...
ElementRecorderRMS::sendSelf (int commitTag, Channel & theChannel)
{
    ...
    if (theChannel.isDatastore () == 1)
      {
          opserr <<
              "ElementRecorderRMS::sendSelf() - does not send data to a datastore\n";
          return -1;
      }
    initializationDone = false;
    //
    ...
    if (theChannel.sendID (0, commitTag, idData) < 0)
      {
          opserr <<
              "ElementRecorderRMS::sendSelf() - failed to send idData\n";
          return -1;
      }
    ...
    if (theChannel.sendMsg (0, commitTag, theMessage) < 0)
      {
          opserr <<
              "ElementRecorderRMS::sendSelf() - failed to send message\n";
          return -1;
      }
    ...
    if (theHandler == 0 || theHandler->sendSelf (commitTag, theChannel) < 0)
      {
          opserr <<
              "ElementRecorderRMS::sendSelf() - failed to send the DataOutputHandler\n";
          return -1;
      }
                              ...
                              FEM_ObjectBroker & theBroker)
{
    ...
    if (theChannel.isDatastore () == 1)
      {
          opserr <<
              "ElementRecorderRMS::recvSelf() - does not recv data to a datastore\n";
          return -1;
      }
    ...
    if (theChannel.recvID (0, commitTag, idData) < 0)
      {
          opserr <<
              "ElementRecorderRMS::recvSelf() - failed to recv idData\n";
          return -1;
      }
    ...
    if (msgLength == 0)
      {
          opserr <<
              "ElementRecorderRMS::recvSelf() - 0 sized string for responses\n";
          return -1;
      }
    ...
    if (theChannel.recvMsg (0, commitTag, theMessage) < 0)
      {
          opserr <<
              "ElementRecorderRMS::recvSelf() - failed to recv message\n";
          return -1;
      }
...
ElementRecorderRMS::initialize (void)
{
    ...
    if (runningTotal == 0 || currentData == 0)
      {
          opserr <<
              "ElementRecorderRMS::ElementRecorderRMS() - out of memory\n";
          exit (-1);
      }
    runningTotal->Zero ();
    currentData->Zero ();
...
//added by SAJalali
double
ElementRecorderRMS::getRecordedValue (int clmnId, int rowOffset, bool reset)
{
    double res = 0;
