...
DamageRecorder::DamageRecorder (int elemid, ID & secIDs, int dofid,
                                DamageModel * dmgPtr, Domain & theDomainPtr,
...
numSec (secIDs.Size ()),
dofID (dofid),
responseID (secIDs.Size ()),
sectionTags (secIDs.Size ()),
...
data (0)
{
    ...
    if (dmgPtr == NULL)
      {
          opserr <<
              "DamageRecorder::DamageRecorder - no damage pointer associated with the damge recorder"
              << endln;
          exit (-1);
      }
    ...
    for (j = 0; j < numSec; j++)
      {
          ...
          if (theDamageModels[j] == NULL)
            {
                opserr <<
                    "DamageRecorder::DamageRecorder - out of memory copying damage models ";
                exit (-1);
            }
    ...
    if (theEle == NULL)
      {
          opserr <<
              "WARNING DamageRecorder::DamageRecorder() - no element with tag: "
    ...
    for (j = 0; j < numSec; j++)
      {
          ...
          theResponses[j] =
              theEle->setResponse ((const char **) argv, argc, *theOutput);
          if (theResponses[j] == 0)
            {
                opserr <<
    ...
    for (j = 0; j < numSec; j++)
      {
          ...
          theResponses[j + numSec] =
              theEle->setResponse ((const char **) argv, argc, *theOutput);
          if (theResponses[j + numSec] == 0)
            {
                opserr <<
                    "DamageRecorder::DamageRecorder - out of memory creating force response ";
                exit (-1);
            }
      }
    ...
    for (j = 0; j < numSec; j++)
      {
          ...
          sprintf (argv[1], "%d", sectionTags (j));
          // _itoa( sectionTags(j) , argv[1] , 10);
          theResponses[j + 2 * numSec] =
              theEle->setResponse ((const char **) argv, argc, *theOutput);
          if (theResponses[j + 2 * numSec] == 0)
            {
                opserr <<
                    "DamageRecorder::DamageRecorder - out of memory creating tanegnt response ";
                exit (-1);
            }
      }
                ...
                for (int j = 0; j < 2; j++)
                  {
                      if (theResponses[i + numSec * j] == 0)
                        ...
                        {
                            if (theResponses[i + numSec * j]->getResponse () <
                                0)
                              {
                                  DamageInformation (j) = 0.0;
                              }
