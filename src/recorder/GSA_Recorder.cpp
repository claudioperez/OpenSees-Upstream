...
GSA_Recorder::GSA_Recorder (Domain & theDom,
                            ...
                            const char *currency,
                            const char *length,
                            const char *force, const char *temp, double dT):
    ...
    while ((theNode = theNodes ()) != 0)
      {
          ...
          if (crds.Size () != ndm)
            {
                opserr << "WARNING - GSA_Recorder::GSA_Recorder() - node: " <<
                    nodeTag;
                opserr << " has invalid number of coordinates, expecting: " <<
                    ndm << " got: " << crds.Size () << endln;
                exit (-1);
            }
          ...
          if (disp.Size () != ndf)
            {
                opserr << "WARNING - GSA_Recorder::GSA_Recorder() - node: " <<
                    nodeTag;
                opserr << " has invalid number of dof, expecting: " << ndf <<
                    " got: " << disp.Size () << endln;
                exit (-1);
            }
          for (int i = 0; i < ndm; i++)
    ...
    if (deltaT == 0.0 || timeStamp >= nextTimeStampToRecord)
      {
          ...
          theFile << "ANAL_CASE\t" << counter << "\tStep" << counter << "\tL"
              << counter << "\tGSS\tSTATIC\tPOST\t" << counter <<
              "\topensees\t" << "20030204165318	0" << endln;
