...
OPS_EnvelopeElementRecorder ()
{
                ...
                if (OPS_GetNumRemainingInputArgs () > 0)
                  {
                      ...
                      if (OPS_GetIntInput (&num, &start) < 0)
                        {
                            opserr <<
                                "WARNING: failed to read start element\n";
                            return 0;
                        }
                  }
                if (OPS_GetNumRemainingInputArgs () > 0)
    ...
    if (domain == 0)
        return 0;
    EnvelopeElementRecorder *recorder =
        new EnvelopeElementRecorder (&elements,
                                     data, nargrem, *domain, *theOutputStream,
                                     dT, echoTimeFlag, &dofs);
...
EnvelopeElementRecorder::EnvelopeElementRecorder ():Recorder (RECORDER_TAGS_EnvelopeElementRecorder),
...
echoTimeFlag (false), addColumnInfo (0)
{
    ...
    if (ele != 0)
      {
          ...
          if (eleID == 0 || eleID->Size () != numEle)
              opserr <<
                  "ElementRecorder::ElementRecorder() - out of memory\n";
      }
    ...
    if (initializationDone == false)
      {
          if (this->initialize () != 0)
            {
                opserr <<
                    "ElementRecorder::record() - failed to initialize\n";
                return -1;
            }
      }
...
EnvelopeElementRecorder::sendSelf (int commitTag, Channel & theChannel)
{
    ...
    if (theChannel.isDatastore () == 1)
      {
          opserr <<
              "EnvelopeElementRecorder::sendSelf() - does not send data to a datastore\n";
          return -1;
      }
    initializationDone = false;
    //
    ...
    if (theChannel.sendID (0, commitTag, idData) < 0)
      {
          opserr <<
              "EnvelopeElementRecorder::sendSelf() - failed to send idData\n";
          return -1;
      }
    ...
    if (theChannel.sendVector (0, commitTag, dData) < 0)
      {
          opserr <<
              "EnvelopeElementRecorder::sendSelf() - failed to send dData\n";
          return -1;
      }
    ...
    if (msgLength == 0)
      {
          opserr <<
              "EnvelopeElementRecorder::sendSelf() - no data to send!!\n";
          return -1;
      }
    ...
    if (theChannel.sendMsg (0, commitTag, theMessage) < 0)
      {
          opserr <<
              "EnvelopeElementRecorder::sendSelf() - failed to send message\n";
          return -1;
      }
    ...
    if (theHandler == 0 || theHandler->sendSelf (commitTag, theChannel) < 0)
      {
          opserr <<
              "EnvelopeElementRecorder::sendSelf() - failed to send the DataOutputHandler\n";
          return -1;
      }
                                   ...
                                   FEM_ObjectBroker & theBroker)
{
    ...
    if (theChannel.isDatastore () == 1)
      {
          opserr <<
              "EnvelopeElementRecorder::recvSelf() - does not recv data to a datastore\n";
          return -1;
      }
    ...
    if (theChannel.recvID (0, commitTag, idData) < 0)
      {
          opserr <<
              "EnvelopeElementRecorder::recvSelf() - failed to recv idData\n";
          return -1;
      }
    ...
    if (theChannel.recvVector (0, commitTag, dData) < 0)
      {
          opserr <<
              "EnvelopeElementRecorder::recvSelf() - failed to recv dData\n";
          return -1;
      }
    deltaT = dData (1);
    ...
    if (eleSize != 0)
      {
          ...
          if (eleID == 0)
            {
                opserr <<
                    "ElementRecorder::recvSelf() - failed to recv idData\n";
                return -1;
            }
    ...
    if (numDOF != 0)
      {
          ...
          if (dof == 0)
            {
                opserr <<
                    "ElementRecorder::recvSelf() - failed to create dof\n";
                return -1;
            }
          if (theChannel.recvID (0, commitTag, *dof) < 0)
            {
                opserr <<
                    "ElementRecorder::recvSelf() - failed to recv dof\n";
                return -1;
            }
      }
    ...
    if (msgLength == 0)
      {
          opserr <<
              "EnvelopeElementRecorder::recvSelf() - 0 sized string for responses\n";
          return -1;
      }
    ...
    if (theChannel.recvMsg (0, commitTag, theMessage) < 0)
      {
          opserr <<
              "EnvelopeElementRecorder::recvSelf() - failed to recv message\n";
          return -1;
      }
    ...
    for (int j = 0; j < numArgs; j++)
      {
          ...
          if (responseArgs[j] == 0)
            {
                opserr <<
                    "EnvelopeElementRecorder::recvSelf() - out of memory\n";
                return -1;
            }
    ...
    if (theHandler == 0)
      {
          opserr <<
              "NodeRecorder::sendSelf() - failed to get a data output handler\n";
          return -1;
      }
    ...
    if (theHandler->recvSelf (commitTag, theChannel, theBroker) < 0)
      {
          opserr <<
              "NodeRecorder::sendSelf() - failed to send the DataOutputHandler\n";
          return -1;
      }
...
EnvelopeElementRecorder::initialize (void)
{
          ...
          for (int ii = 0; ii < numEle; ii++)
            {
                ...
                if (theEle == 0)
                ...
                else
                  {
                      ...
                      theResponses[ii] =
                          theEle->setResponse ((const char **) responseArgs,
                                               numArgs, *theHandler);
                      if (theResponses[ii] != 0)
                        {
                            // from the response type determine no of cols for each      
                            Information & eleInfo =
                                theResponses[ii]->getInformation ();
                            const Vector & eleData = eleInfo.getData ();
                            int dataSize = eleData.Size ();
                            ...
                            if (addColumnInfo == 1)
                              {
                                  if (echoTimeFlag == true)
                                    {
                                        if (numDOF == 0)
                                            for (int j = 0; j < 2 * dataSize;
                                                 j++)
                                                responseOrder[responseCount++]
                                                    = i + 1;
                                        else
                                            for (int j = 0; j < 2 * numDOF;
                                                 j++)
                                                responseOrder[responseCount++]
                                                    = i + 1;
                                    }
                ...
                if (theResponse != 0)
                  {
                      if (numResponse == numEle)
                        {
                            Response **theNextResponses =
                                new Response *[numEle * 2];
                      ...
                      // from the response type determine no of cols for each
                      Information & eleInfo =
                          theResponses[numResponse]->getInformation ();
                      const Vector & eleData = eleInfo.getData ();
                      if (numDOF == 0)
                        {
    ...
    if (data == 0 || currentData == 0)
      {
          opserr <<
              "EnvelopeElementRecorder::EnvelopeElementRecorder() - out of memory\n";
          exit (-1);
      }
...
//added by SAJalali
double
EnvelopeElementRecorder::getRecordedValue (int clmnId, int rowOffset,
                                           bool reset)
{
    double res = 0;
