...
std::map < int,
    GmshRecorder::GmshType >
    GmshRecorder::gmshtypes;
...
OPS_GmshRecorder ()
{
    ...
    // create recorder
    return new GmshRecorder (name, nodedata, eledata, indent, precision,
                             write_graph_mesh, write_update_time,
                             write_ele_updatetime);
}
#endif

GmshRecorder::GmshRecorder (const char *name, const NodeData & ndata,
                            const std::vector < EleData > &edata, int ind,
                            int pre, int write_graph_mesh_,
                            int write_update_time_,
                            int write_ele_updatetime_):
Recorder (RECORDER_TAGS_GmshRecorder),
precision (pre),
write_header_now (true),
write_mesh_now (true),
write_binary_mode (false),
write_ele_updatetime (write_ele_updatetime_),
filename (name),
timestep (),
timeparts (),
theFile (),
...
theDomain (NULL),
current_step (0),
write_graph_mesh (write_graph_mesh_),
write_update_time (write_update_time_)
{
    DEBUGSTREAM << "edata.size() = " << (int) edata.size () << endln;
    DEBUGSTREAM << "eledata.size() = " << (int) eledata.size () << endln;
}

GmshRecorder::GmshRecorder ():Recorder (RECORDER_TAGS_GmshRecorder),
precision (0),
write_header_now (true), write_mesh_now (true), write_binary_mode (false),
write_ele_updatetime (false), filename (), timestep (), timeparts (),
theFile (), theDomain (NULL), current_step (0), write_graph_mesh (0)
{
    GmshRecorder::setGMSHType ();
}
    ...
    if (false)                  // Disabled for now
    ...
    else
      {
          ...
          if (nodedata.disp ||
              ...
              nodedata.reaction ||
              nodedata.rayleigh ||
              nodedata.pressure || nodedata.unbalanced || nodedata.mass)
              write_node_data ();
...
GmshRecorder::write_mesh ()
{
    ...
    if (theDomain == 0)
      {
          opserr <<
              "WARNING: setDomain has not been called -- GmshRecorder\n";
          return -1;
      }
    ...
    if (theFile.fail ())
      {
          opserr << "WARNING: Failed to open file " << mshname.
              c_str () << "\n";
          return -1;
      }
...
GmshRecorder::write_node_data ()
{
    ...
    DEBUGSTREAM << "GmshRecorder -- Saving mesh for rank ---> " << rank <<
        endln;

    if (theDomain == 0)
      {
          opserr <<
              "GmshRecorder::write_data() - WARNING: setDomain has not been called -- GmshRecorder\n";
          return -1;
      }

    std::string viewname (" ");
    if (nodedata.disp)
        viewname = std::string ("\"Displacment\"");
    if (nodedata.vel)
        viewname = std::string ("\"Velocity\"");
    if (nodedata.accel)
        viewname = std::string ("\"Acceleration\"");
    if (nodedata.incrdisp)
        viewname = std::string ("\"Incremental Displacement\"");
    if (nodedata.reaction)
        viewname = std::string ("\"Reaction Forces\"");
    if (nodedata.rayleigh)
        viewname = std::string ("\"Rayleigh Damping Forces\"");
    if (nodedata.pressure)
        viewname = std::string ("\"Pressure\"");
    if (nodedata.unbalanced)
        viewname = std::string ("\"Unbalanced Forces\"");
    if (nodedata.mass)
        viewname = std::string ("\"Mass\"");
    ...
    if (!theFile.is_open ())
      {
          ...
          if (write_binary_mode)
            {
                theFile.open (mshname.c_str (),
                              std::ios::trunc | std::ios::out | std::ios::
                              binary);
            }
          else
            {
    ...
    while ((theNode = theNodes ()) != 0)
      {
          theFile << theNode->getTag () << ' ';
          if (nodedata.disp)
              write_data_line (theFile, theNode->getDisp ());
          if (nodedata.vel)
              write_data_line (theFile, theNode->getVel ());
          if (nodedata.accel)
              write_data_line (theFile, theNode->getAccel ());
          if (nodedata.incrdisp)
              write_data_line (theFile, theNode->getIncrDisp ());
          if (nodedata.reaction)
              write_data_line (theFile, theNode->getReaction ());
          if (nodedata.unbalanced)
              write_data_line (theFile, theNode->getUnbalancedLoad ());
      }
    theFile << "$EndNodeData\n";
...
int
GmshRecorder::write_data_line (std::ofstream & s, const Vector & data,
                               const int truncatesize)
{
...
GmshRecorder::write_eleupdatetime_now ()
{
    ...
    DEBUGSTREAM << "GmshRecorder -- write_eleupdatetime_now---> " << rank <<
        endln;
    ...
    for (int i = 0; i < 1; i++)
      {
          ...
          const Vector *data =
              theDomain->getElementResponse (theElement->getTag (),
                                             &(argv[0]), argc);
          ...
          while (theElement != 0)
            {
                int tag = theElement->getTag ();
                const Vector *data =
                    theDomain->getElementResponse (tag, &(argv[0]), argc);
                theFile << tag << ' ';
                if (data != NULL)
    ...
    if (do_only_once)
      {
          ...
          while (v != 0)
            {
                ...
                if (eletag > maxvertextag)
                    maxvertextag = eletag;
                theFile << eletag << " " << GMSH_VERTEX << " " << "2 " << rank
                    + 1 << " " << rank + 1 << " " << eletag << endln;
                v = vertices ();
            }
          ...
          MPI_Allreduce (&maxvertextag,
                         &maxvertextag_all,
                         1, MPI_INT, MPI_MAX, MPI_COMM_WORLD);

          MPI_Allgather (&numvertex,
                         1,
                         MPI_INT, numvertex_proc, 1, MPI_INT, MPI_COMM_WORLD);
          ...
          DEBUGSTREAM << "Rank " << rank << " gets tags from " << start_tag <<
              " to " << start_tag + numvertex << endln;
          ...
          while (v != 0)
            {
                ...
                for (int i = 0; i <= adjacent_vertices.Size (); i++)
                  {
                      theFile << neweletag << " " << GMSH_LINE << " " << "2 "
                          << rank + 1 << " " << rank +
                          1 << " " << eletag << " " << adjacent_vertices (i)
                          << endln;
                      neweletag++;
                  }
                v = vertices ();
    ...
    if (do_only_once)
      {
          ...
          // theFile.close();
          std::stringstream ssmshname;
          ssmshname << filename << ".updatetime." << rank << "." << nproc <<
              ".out";
          std::string mshname = ssmshname.str ();
          ...
          theUpdateTimeFilePtr->open (mshname.c_str (),
                                      std::ios::trunc | std::ios::out);
          ...
          for (int j = 0; j < send_how_many; j++)
            {
                ...
                if (theChannel.
                    sendID (this->getDbTag (), commitTag,
                            string_length_vec) < 0)
                  {
                      opserr <<
                          "GmshRecorder::sendSelf - Problem sending string_length_vec "
                          << eledatastring.c_str () << endln;
                  }

                Message msg_string (charbuffer, string_length);
                if (theChannel.
                    sendMsg (this->getDbTag (), commitTag, msg_string) < 0)
                  {
                      opserr <<
                          "GmshRecorder::sendSelf - Problem sending eledata "
                          << eledatastring.c_str () << endln;
                  }

                DEBUGSTREAM << "Sent: eledatastring = " << eledatastring.
                    c_str () << endln;
...
int
GmshRecorder::recvSelf (int commitTag, Channel & theChannel,
                        FEM_ObjectBroker & theBroker)
{
    ...
    if (theChannel.recvID (this->getDbTag (), commitTag, data) < 0)
      {
          opserr <<
              "GmshRecorder::recvSelf - Problem sending filename string length\n";
      }
    length = data (0);
    precision = data (1);
    ...
    if (theChannel.recvMsg (this->getDbTag (), commitTag, msg) < 0)
      {
          opserr <<
              "GmshRecorder::recvSelf - Problem receiving the filename\n";
      }
          ...
          for (int j = 0; j < recv_how_many; j++)
            {
                ...
                if (theChannel.
                    recvID (this->getDbTag (), commitTag,
                            string_length_vec) < 0)
                  {
                      opserr <<
                          "GmshRecorder::recvSelf - Problem receiving string_length_vec "
                          << endln;
                  }


                int string_length = string_length_vec (0);
                DEBUGSTREAM << "Received: string_length = " << string_length
                    << endln;

                char *charbuffer = new char[string_length];
                Message msg_string (charbuffer, string_length);
                if (theChannel.
                    recvMsg (this->getDbTag (), commitTag, msg_string) < 0)
                  {
                      opserr <<
                          "GmshRecorder::recvSelf - Problem receiving eledata "
                          << endln;
                  }

                std::string eledatastring (charbuffer, string_length);

                DEBUGSTREAM << "Received: eledatastring = " << eledatastring.
                    c_str () << endln;
...
GmshRecorder::setGMSHType ()
{
    ...
    gmshtypes[ELE_TAG_TotalLagrangianFD20NodeBrick] =
        GMSH_QUADRATIC_HEXAHEDRON;
