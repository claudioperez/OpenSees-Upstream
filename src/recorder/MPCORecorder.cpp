...
namespace libload
{
    ...
    int loadLib (const std::string & lib_name, void **libHandle)
    {
        ...
        *libHandle = NULL;
        ...
        if (hLib != NULL)
          ...
          {
              opserr << "cannot load library \"" << lib_name.
                  c_str () << "\"\n";
              result = -1;
          }
    ...
    int loadSym (void *libHandle, const std::string & sym_name,
                 void **funcHandle)
    {
        ...
        *funcHandle = NULL;
        ...
        if (hLib != NULL)
          {
              (*funcHandle) =
                  (void *) GetProcAddress ((HMODULE) hLib, sym_name.c_str ());
              if (*funcHandle == NULL)
                {
                    std::stringstream ss;
                    ss << sym_name << "_";
                    ...
                    (*funcHandle) =
                        (void *) GetProcAddress ((HMODULE) hLib,
                                                 sym_name_.c_str ());
                }
              if (*funcHandle == NULL)
                {
                    opserr << "cannot load symbol \"" << sym_name.
                        c_str () << "\"\n";
                    result = -2;
                }
          }
...
{
  private:
    LibraryLoader ():loaded (false), lib_handle (NULL)
    {
        std::string lib_name = libload::appendSharedLibPostfix ("hdf5");
        MPCO_LIBLOADER_COUT ("loading \"" << lib_name.
                             c_str () << "\" ..." << "\n");

        if (libload::loadLib (lib_name, &lib_handle) != 0)
          {
              MPCO_LIBLOADER_COUT ("\"" << lib_name.
                                   c_str () <<
                                   "\" not found, trying with \"lib" <<
                                   lib_name.c_str () << "\"\n");
              std::stringstream ss_for_prefix;
              ss_for_prefix << "lib" << lib_name;
              std::string lib_name_2 = ss_for_prefix.str ();
              MPCO_LIBLOADER_COUT ("loading \"" << lib_name_2.
                                   c_str () << "\" ..." << "\n");
              if (libload::loadLib (lib_name_2, &lib_handle) != 0)
                {
                    opserr << "MPCORecorder error:\n"
                        "Cannot load HDF5 (tried both \"" << lib_name.
                        c_str () << "\" and \"" << lib_name_2.
                        c_str () << "\" names for the shared library).\n"
                        "HDF5 version " << H5_VERS_MAJOR << "." <<
                        H5_VERS_MINOR <<
  ...
  public:
    ...
    herr_t (*ptr_H5check_version) (unsigned majnum, unsigned minnum,
                                   unsigned relnum);
    herr_t (*ptr_H5open) (void);
    hid_t (*ptr_H5Screate_simple) (int rank, const hsize_t dims[],
                                   const hsize_t maxdims[]);
    herr_t (*ptr_H5Sclose) (hid_t space_id);
    hid_t (*ptr_H5Acreate2) (hid_t loc_id, const char *attr_name,
    ...
    hid_t (*ptr_H5Gcreate2) (hid_t loc_id, const char *name, hid_t lcpl_id,
                             hid_t gcpl_id, hid_t gapl_id);
    herr_t (*ptr_H5Gclose) (hid_t group_id);
    hid_t (*ptr_H5Dcreate2) (hid_t loc_id, const char *name, hid_t type_id,
                             hid_t space_id, hid_t lcpl_id, hid_t dcpl_id,
                             hid_t dapl_id);
    herr_t (*ptr_H5Dclose) (hid_t dset_id);
    herr_t (*ptr_H5Dwrite) (hid_t dset_id, hid_t mem_type_id,
                            hid_t mem_space_id, hid_t file_space_id,
                            hid_t plist_id, const void *buf);
    hid_t (*ptr_H5Pcreate) (hid_t cls_id);
    herr_t (*ptr_H5Pclose) (hid_t plist_id);
    herr_t (*ptr_H5Pset_link_creation_order) (hid_t plist_id,
                                              unsigned crt_order_flags);
    herr_t (*ptr_H5Pset_libver_bounds) (hid_t plist_id, H5F_libver_t low,
                                        H5F_libver_t high);
    hid_t (*ptr_H5Fcreate) (const char *filename, unsigned flags,
...
namespace utils
{
    ...
    namespace strings
    {
        ...
        template < class T > inline std::string to_string (T x)
        {
            std::stringstream ss;
            ss << x;
            return ss.str ();
        ...
        inline void split (const std::string & text, char sep,
    ...
    namespace shell
    {
        ...
        inline bool isShellElementTag (int ele_tag)
        {
			...
			*/
            return (ele_tag == ELE_TAG_ShellMITC4 ||
    ...
    namespace locax
    {
        ...
        inline quaternion quatFromMat (const Vector & vx, const Vector & vy,
                                       const Vector & vz)
        {
    ...
    namespace misc
    {
        ...
        template <>
            inline bool areVectorsEqual < std::string > (const std::vector <
                                                         std::string > &a,
                                                         const std::vector <
                                                         std::string > &b)
        {
        ...
        void bufferNodeResponseVec3u (size_t node_counter, int ndim,
                                      const Vector & iresponse,
        ...
        void bufferNodeResponseVec3r (size_t node_counter, int ndim,
                                      const Vector & iresponse,
                                      std::vector < double >&buffer)
        {
            if (ndim == 2)
...
namespace mpco
{
    ...
    namespace element
    {
        ...
        struct FiberData
        {
            FiberData ():y (0.0), z (0.0), a (0.0)
            {
            }
            FiberData (double _y, double _z, double _a):y (_y), z (_z), a (_a)
            {
            }
            inline bool operator < (const FiberData & other) const
            {
                ...
                double tol =
                    std::max (std::abs (y), std::abs (other.y)) * rel_tol;
                if (utils::misc::lessThanWithTol (y, other.y, tol))
                      return true;
                if (utils::misc::greaterThanWithTol (y, other.y, tol))
                      return false;
                  tol = std::max (std::abs (z), std::abs (other.z)) * rel_tol;
                if (utils::misc::lessThanWithTol (z, other.z, tol))
                      return true;
                if (utils::misc::greaterThanWithTol (z, other.z, tol))
                      return false;
                  tol = std::max (std::abs (a), std::abs (other.a)) * rel_tol;
                if (utils::misc::lessThanWithTol (a, other.a, tol))
                      return true;
                if (utils::misc::greaterThanWithTol (a, other.a, tol))
                      return false;
                  return false; // equal
            }
            inline bool operator > (const FiberData & other) const
        ...
        struct FiberSectionData
        {
            ...
            {
                if (fibers.size () < other.fibers.size ())
                    return true;
                if (fibers.size () > other.fibers.size ())
                    return false;
        ...
        {
            ElemGaussPair ():elem_id (0), gauss_id (0)
            {
            }
            ElemGaussPair (int _elem_id, int _gauss_id):elem_id (_elem_id),
                gauss_id (_gauss_id)
            {
            }
        ...
        {
            SectionAssignment ():is_new (true), name ("UnkownClassType"),
                fiber_section_data (), assignments ()
            {
            }
            bool is_new;
            std::string name;
...
namespace h5
{
    ...
    namespace attribute
    {
        ...
        herr_t writei (hid_t obj, const char *attr_name, const int *attr_data,
                       hsize_t data_size)
        {
            herr_t status;
            ...
            hid_t attr =
                H5Acreate (obj, attr_name, H5T_STD_I32LE, space, H5P_DEFAULT,
                           H5P_DEFAULT);
        ...
        herr_t writed (hid_t obj, const char *attr_name,
                       const double *attr_data, hsize_t data_size)
        {
            herr_t status;
            hsize_t dim[1] = { data_size };
            hid_t space = H5Screate_simple (1, dim, NULL);
            hid_t attr =
                H5Acreate (obj, attr_name, H5T_IEEE_F64LE, space, H5P_DEFAULT,
                           H5P_DEFAULT);
        ...
        herr_t writes (hid_t obj, const char *attr_name,
                       const char *attr_data, hsize_t data_size)
        {
            herr_t status;
            hsize_t dim[1] = { 1 };
            hid_t space = H5Screate_simple (1, dim, NULL);
            ...
            hid_t attr =
                H5Acreate (obj, attr_name, atype, space, H5P_DEFAULT,
                           H5P_DEFAULT);
        ...
        herr_t write (hid_t obj, const char *attr_name,
                      const std::vector < int >&attr_data)
        {
            if (attr_data.size () > 0)
              {
                  return writei (obj, attr_name, &attr_data[0],
                                 attr_data.size ());
              }
            return 0;
        }
                      ...
                      std::vector < double >&attr_data)
        {
            if (attr_data.size () > 0)
              {
                  return writed (obj, attr_name, &attr_data[0],
                                 attr_data.size ());
              }
            return 0;
        }
                      ...
                      const std::string & attr_data)
        {
            if (!attr_data.empty ())
              {
                  return writes (obj, attr_name, attr_data.c_str (),
                                 attr_data.size ());
              }
            return 0;
        }
    ...
    namespace group
    {
        ...
        hid_t create (hid_t loc_id, const char *name, hid_t lcpl_id,
                      hid_t gcpl_id, hid_t gapl_id)
        {
        ...
        hid_t createResultGroup (hid_t obj, hid_t gplist,
                                 ...
                                 int result_type, int data_type)
        {
            herr_t status;
            hid_t h_gp_result =
                h5::group::create (obj, result_name.c_str (), H5P_DEFAULT,
                                   gplist, H5P_DEFAULT);
            status =
                h5::attribute::write (h_gp_result, "DISPLAY_NAME", disp_name);
            status =
                h5::attribute::write (h_gp_result, "COMPONENTS", components);
            status =
                h5::attribute::write (h_gp_result, "DIMENSION", dimension);
            status =
                h5::attribute::write (h_gp_result, "DESCRIPTION",
                                      description);
            status = h5::attribute::write (h_gp_result, "TYPE", result_type);
            status =
                h5::attribute::write (h_gp_result, "DATA_TYPE", data_type);
            return h_gp_result;
        }
    ...
    namespace dataset
    {
        ...
        {
            if (dataset_id == HID_INVALID)
                return -1;
            return H5Dclose (dataset_id);
        }
        hid_t createAndWrited1 (hid_t obj, const char *name,
                                const double *data, hsize_t data_size)
        {
            // error flags
            herr_t status;
            ...
            hid_t dset =
                H5Dcreate (obj, name, H5T_IEEE_F64LE, space, H5P_DEFAULT,
        ...
        hid_t createAndWrited2 (hid_t obj, const char *name,
                                const double *data, hsize_t rows,
                                hsize_t cols)
        {
            // error flags
            herr_t status;
        ...
        hid_t createAndWritei1 (hid_t obj, const char *name, const int *data,
                                hsize_t data_size)
        {
            // error flags
            herr_t status;
            ...
            // create the dataset and write data to it.
            hid_t dset =
                H5Dcreate (obj, name, H5T_STD_I32LE, space, H5P_DEFAULT,
                           H5P_DEFAULT, H5P_DEFAULT);
            status =
        ...
        hid_t createAndWritei2 (hid_t obj, const char *name, const int *data,
        ...
        hid_t createAndWrites (hid_t obj, const char *name, const char *data,
                               hsize_t data_size)
        {
            // error flags
            herr_t status;
            ...
            // create the dataset and write data to it.
            hid_t dset =
                H5Dcreate (obj, name, atype, space, H5P_DEFAULT, H5P_DEFAULT,
                           H5P_DEFAULT);
            status =
                H5Dwrite (dset, atype, H5S_ALL, H5S_ALL, H5P_DEFAULT, data);
            // close and release resources
            status = H5Sclose (space);
            return dset;
        }
        ...
        hid_t createAndWrite (hid_t obj, const char *name,
        ...
        hid_t createAndWrite (hid_t obj, const char *name,
                              const std::vector < double >&data, size_t rows,
                              size_t cols)
        {
        ...
        hid_t createAndWrite (hid_t obj, const char *name,
                              const std::vector < int >&data, size_t rows,
                              size_t cols)
        {
            if (data.size () > 0 && data.size () == rows * cols)
              {
                              ...
                              const std::string & data)
        {
            if (!data.empty ())
              {
                  return createAndWrites (obj, name, data.c_str (),
                                          data.size ());
              }
            return HID_INVALID;
        }
        hid_t createAndWrite (hid_t obj, const char *name,
                              const std::vector < utils::locax::quaternion >
                              &data)
        {
            if (data.size () > 0)
              {
                  return createAndWrited2 (obj, name, data[0].data (),
                                           data.size (), 4);
              }
            return HID_INVALID;
        }
        hid_t createAndWrite (hid_t obj, const char *name,
                              const std::vector < mpco::element::FiberData >
                              &data)
        {
            if (data.size () > 0)
              {
        ...
        hid_t createAndWrite (hid_t obj, const char *name,
                              const std::vector <
                              mpco::element::ElemGaussPair > &data)
        {
            if (data.size () > 0)
              {
                  return createAndWritei2 (obj, name, data[0].data (),
                                           data.size (), 2);
              }
            return HID_INVALID;
        }
    ...
    namespace file
    {
        ...
        hid_t create (const char *filename, hid_t create_plist,
                      hid_t acc_plist)
        {
            return H5Fcreate (filename, H5F_ACC_TRUNC, create_plist,
                              acc_plist);
        }
#ifdef MPCO_USE_SWMR
    ...
    namespace plist
    {
        ...
        herr_t setLinkCreationOrder (hid_t plist_id,
                                     unsigned int crt_order_flags)
        {
            return H5Pset_link_creation_order (plist_id, crt_order_flags);
        }
        herr_t setLibVerBounds (hid_t plist_id, unsigned int minor,
                                unsigned int major)
        {
            return H5Pset_libver_bounds (plist_id, (H5F_libver_t) minor,
                                         (H5F_libver_t) major);
        }
    }
...
namespace mpco
{
    ...
    struct ElementOutputDescriptorType
    {
		...
		*/
        ...
        static const char *toString (mpco::ElementOutputDescriptorType::
                                     Enum _type)
        {
            switch (_type)
              {
              case mpco::ElementOutputDescriptorType::Element:
                  return "ELEMENT";
                  case mpco::ElementOutputDescriptorType::
                      Gauss:return "GAUSS";
                  case mpco::ElementOutputDescriptorType::
...
namespace mpco
{
    ...
    struct OutputFrequency
    {
        ...
        int last_step;
          ...
          OutputFrequency ():type (DeltaTime), dt (0.0), nsteps (1),
            last_time (0.0), last_step (0)
        {
        }
        void reset ()
        {
    ...
    {
      public:
        Timer (const std::string & task_name):m_task_name (task_name),
            m_t0 (0), m_t1 (0)
        {
        }
        inline void start ()
        ...
        inline void stop ()
        {
            m_t1 = clock ();
            std::
                cout << "          MPCORecorder. Task = \"" << m_task_name <<
                "\". Elapsed time: " << double (m_t1 -
                                                m_t0) /
    ...
    class ProcessInfo
    {
      public:
        ...
        :domain (0), num_dimensions (0), current_model_stage_id (-1)
            // some handles in the hdf5 file
        , h_file_id (HID_INVALID), h_file_proplist (HID_INVALID)
#ifdef MPCO_USE_SWMR
        , h_file_acc_proplist (HID_INVALID)
#endif                          // MPCO_USE_SWMR
        , h_group_proplist (HID_INVALID)
            // time step info
        , current_time_step_id (0), current_time_step (0.0)
            // misc
        , eigen_first_initialization_done (false),
            record_eigen_on_this_step (false), eigen_last_time_set (0.0),
            eigen_last_values ()
        {
        }
      public:
...
namespace mpco
{
    ...
    namespace node
    {
        ...
        {
          public:
            ResultRecorder (const mpco::
                            ProcessInfo & info):m_initialized (false),
                m_ndim (info.num_dimensions), m_result_name (""),
                m_result_display_name (""), m_components_name (""),
                m_num_components (0), m_dimension (""), m_description (""),
            ...
            virtual int getReactionFlag () const
            {
                return -1;
            }
            virtual int record (mpco::ProcessInfo & info,
                                std::vector < Node * >&nodes)
            {
                ...
                if (!m_initialized)
                  {
                       ...
                       */
                      hid_t h_gp_result =
                          h5::group::createResultGroup (info.h_file_id,
                                                        info.h_group_proplist,
                                                        m_result_name,
                      ...
                      hid_t h_dset_id =
                          h5::dataset::createAndWrite (h_gp_result, "ID",
                                                       buffer_id,
                                                       buffer_id.size (), 1);
                      /*
                       ...
                       */
                      hid_t h_gp_data =
                          h5::group::create (h_gp_result, "DATA", H5P_DEFAULT,
                                             info.h_group_proplist,
                                             H5P_DEFAULT);
                      /*
                         done
                 ...
                 */
                std::vector <
                    double >buffer_data (nodes.size () * m_num_components);
                bufferResponse (info, nodes, buffer_data);
                std::stringstream ss_dset_name;
                ss_dset_name << m_result_name << "/DATA/STEP_" << info.
                    current_time_step_id;
                std::string dset_name = ss_dset_name.str ();
                hid_t h_dset_data =
                    h5::dataset::createAndWrite (info.h_file_id,
                                                 dset_name.c_str (),
                                                 buffer_data, nodes.size (),
                                                 m_num_components);
                status =
                    h5::attribute::write (h_dset_data, "STEP",
                                          info.current_time_step_id);
                status =
                    h5::attribute::write (h_dset_data, "TIME",
          ...
          protected:
            virtual void bufferResponse (mpco::ProcessInfo & info,
                                         std::vector < Node * >&nodes,
                                         std::vector <
        ...
        {
          public:
            ResultRecorderDisplacement (const mpco::
                                        ProcessInfo &
                                        info):ResultRecorder (info)
            {
                std::stringstream ss_buffer;
                ss_buffer << "MODEL_STAGE[" << info.
                    current_model_stage_id <<
                    "]/RESULTS/ON_NODES/DISPLACEMENT";
          ...
          protected:
            ...
            {
                for (size_t i = 0; i < nodes.size (); i++)
                    utils::misc::bufferNodeResponseVec3u (i, m_ndim,
                                                          nodes[i]->
                                                          getTrialDisp (),
                                                          buffer);
            }
        ...
        {
          public:
            ResultRecorderRotation (const mpco::
                                    ProcessInfo & info):ResultRecorder (info)
            {
                std::stringstream ss_buffer;
                ss_buffer << "MODEL_STAGE[" << info.
                    current_model_stage_id << "]/RESULTS/ON_NODES/ROTATION";
                m_result_name = ss_buffer.str ();
          ...
          protected:
            ...
            {
                for (size_t i = 0; i < nodes.size (); i++)
                    utils::misc::bufferNodeResponseVec3r (i, m_ndim,
                                                          nodes[i]->
                                                          getTrialDisp (),
                                                          buffer);
            }
        ...
        {
          public:
            ResultRecorderPressure (const mpco::
                                    ProcessInfo & info):ResultRecorder (info)
            {
                std::stringstream ss_buffer;
                ss_buffer << "MODEL_STAGE[" << info.
                    current_model_stage_id << "]/RESULTS/ON_NODES/PRESSURE";
                m_result_name = ss_buffer.str ();
        ...
        {
          public:
            ResultRecorderReactionForce (const mpco::
                                         ProcessInfo &
                                         info):ResultRecorder (info)
            {
                std::stringstream ss_buffer;
                ss_buffer << "MODEL_STAGE[" << info.
                    current_model_stage_id <<
                    "]/RESULTS/ON_NODES/REACTION_FORCE";
                m_result_name = ss_buffer.str ();
        ...
        {
          public:
            ResultRecorderReactionMoment (const mpco::
                                          ProcessInfo &
                                          info):ResultRecorder (info)
            {
                std::stringstream ss_buffer;
                ss_buffer << "MODEL_STAGE[" << info.
                    current_model_stage_id <<
                    "]/RESULTS/ON_NODES/REACTION_MOMENT";
                m_result_name = ss_buffer.str ();
        ...
        class ResultRecorderReactionForceIncIntertia:public
            ResultRecorderReactionForce
        {
          public:
            ResultRecorderReactionForceIncIntertia (const mpco::
                                                    ProcessInfo &
                                                    info):ResultRecorderReactionForce
                (info)
            {
                std::stringstream ss_buffer;
                ss_buffer << "MODEL_STAGE[" << info.
                    current_model_stage_id <<
                    "]/RESULTS/ON_NODES/REACTION_FORCE_INCLUDING_INERTIA";
                m_result_name = ss_buffer.str ();
        ...
        class ResultRecorderReactionMomentIncIntertia:public
            ResultRecorderReactionMoment
        {
          public:
            ResultRecorderReactionMomentIncIntertia (const mpco::
                                                     ProcessInfo &
                                                     info):ResultRecorderReactionMoment
                (info)
            {
                std::stringstream ss_buffer;
                ss_buffer << "MODEL_STAGE[" << info.
                    current_model_stage_id <<
                    "]/RESULTS/ON_NODES/REACTION_MOMENT_INCLUDING_INERTIA";
                m_result_name = ss_buffer.str ();
        ...
        class ResultRecorderReactionForceRayleigh:public
            ResultRecorderReactionForce
        {
          public:
            ResultRecorderReactionForceRayleigh (const mpco::
                                                 ProcessInfo &
                                                 info):ResultRecorderReactionForce
                (info)
            {
                ...
                ss_buffer << "MODEL_STAGE[" << info.
                    current_model_stage_id <<
                    "]/RESULTS/ON_NODES/RAYLEIGH_FORCE";
                m_result_name = ss_buffer.str ();
        ...
        class ResultRecorderReactionMomentRayleigh:public
            ResultRecorderReactionMoment
        {
          public:
            ResultRecorderReactionMomentRayleigh (const mpco::
                                                  ProcessInfo &
                                                  info):ResultRecorderReactionMoment
                (info)
            {
                ...
                ss_buffer << "MODEL_STAGE[" << info.
                    current_model_stage_id <<
                    "]/RESULTS/ON_NODES/RAYLEIGH_MOMENT";
                m_result_name = ss_buffer.str ();
        ...
        {
          public:
            ResultRecorderVelocity (const mpco::
                                    ProcessInfo & info):ResultRecorder (info)
            {
                std::stringstream ss_buffer;
                ss_buffer << "MODEL_STAGE[" << info.
                    current_model_stage_id << "]/RESULTS/ON_NODES/VELOCITY";
                m_result_name = ss_buffer.str ();
        ...
        {
          public:
            ResultRecorderAngularVelocity (const mpco::
                                           ProcessInfo &
                                           info):ResultRecorder (info)
            {
                std::stringstream ss_buffer;
                ss_buffer << "MODEL_STAGE[" << info.
                    current_model_stage_id <<
                    "]/RESULTS/ON_NODES/ANGULAR_VELOCITY";
                m_result_name = ss_buffer.str ();
        ...
        {
          public:
            ResultRecorderAcceleration (const mpco::
                                        ProcessInfo &
                                        info):ResultRecorder (info)
            {
                std::stringstream ss_buffer;
        ...
        {
          public:
            ResultRecorderAngularAcceleration (const mpco::
                                               ProcessInfo &
                                               info):ResultRecorder (info)
            {
                std::stringstream ss_buffer;
                ss_buffer << "MODEL_STAGE[" << info.
                    current_model_stage_id <<
                    "]/RESULTS/ON_NODES/ANGULAR_ACCELERATION";
                m_result_name = ss_buffer.str ();
        ...
        {
          public:
            ResultRecorderModesOfVibration (const mpco::
                                            ProcessInfo &
                                            info):ResultRecorder (info),
                m_current_mode_to_buffer (0)
            {
                ...
                ss_buffer << "MODEL_STAGE[" << info.
                    current_model_stage_id <<
                    "]/RESULTS/ON_NODES/MODES_OF_VIBRATION(U)";
                m_result_name = ss_buffer.str ();
                                ...
                                std::vector < Node * >&nodes)
            {
                ...
                ss_gp_step_name << m_result_name << "/DATA/STEP_" << info.
                    current_time_step_id;
                std::string gp_step_name = ss_gp_step_name.str ();
                hid_t h_gp_step =
                    h5::group::create (info.h_file_id, gp_step_name.c_str (),
                                       H5P_DEFAULT, info.h_group_proplist,
                                       H5P_DEFAULT);
                status =
                    h5::attribute::write (h_gp_step, "STEP",
                                          info.current_time_step_id);
                status =
                    h5::attribute::write (h_gp_step, "TIME",
                                          info.current_time_step);
                /*
                   allocate buffer response
                 */
                std::vector <
                    double >buffer (nodes.size () * m_num_components);
                /*
                   for each mode
                 */
                ...
                for (int k = 0; k < num_eigen; k++)
                  {
                      ...
                      double lambda =
                          k <
                          info.eigen_last_values.Size ()? info.
                          eigen_last_values[k] : 0.0;
                      ...
                      std::string dset_name = ss_dset_name.str ();
                      hid_t h_dset_data =
                          h5::dataset::createAndWrite (h_gp_step,
                                                       dset_name.c_str (),
                                                       buffer, nodes.size (),
                                                       m_num_components);
                      status = h5::attribute::write (h_dset_data, "MODE", k);
                      status =
                          h5::attribute::write (h_dset_data, "LAMBDA",
                                                lambda);
                      status =
                          h5::attribute::write (h_dset_data, "OMEGA", omega);
                      status =
                          h5::attribute::write (h_dset_data, "FREQUENCY",
                                                freq);
                      status =
                          h5::attribute::write (h_dset_data, "PERIOD",
                                                period);
                      status = h5::dataset::close (h_dset_data);
                  }
                /*
          ...
          protected:
                                         ...
                                         std::vector < double >&buffer) const
            {
                for (size_t i = 0; i < nodes.size (); i++)
                  {
                      size_t j = i * m_ndim;
                      const Matrix & inode_eigenvec =
                          nodes[i]->getEigenvectors ();
                        buffer[j] =
                          inode_eigenvec (0, m_current_mode_to_buffer);
                      if (m_ndim > 1)
                        {
                            buffer[j + 1] =
                                inode_eigenvec (1, m_current_mode_to_buffer);
                            if (m_ndim > 2)
                              {
                                  buffer[j + 2] =
                                      inode_eigenvec (2,
                                                      m_current_mode_to_buffer);
                              }
                        }
                  }
            }
        ...
        class ResultRecorderModesOfVibrationRotational:public
            ResultRecorderModesOfVibration
        {
          public:
            ResultRecorderModesOfVibrationRotational (const mpco::
                                                      ProcessInfo &
                                                      info):ResultRecorderModesOfVibration
                (info)
            {
                std::stringstream ss_buffer;
                ss_buffer << "MODEL_STAGE[" << info.
                    current_model_stage_id <<
                    "]/RESULTS/ON_NODES/MODES_OF_VIBRATION(R)";
                m_result_name = ss_buffer.str ();
                      ...
                      if (m_ndim == 2 && inode_eigenvec.noRows () > 2)
                        {
                            size_t j = i;
                              buffer[j] =
                                inode_eigenvec (2, m_current_mode_to_buffer);
                        }
                      else if (m_ndim == 3 && inode_eigenvec.noRows () > 5)
                        {
                            size_t j = i * 3;
                            buffer[j] =
                                inode_eigenvec (3, m_current_mode_to_buffer);
                            buffer[j + 1] =
                                inode_eigenvec (4, m_current_mode_to_buffer);
                            buffer[j + 2] =
                                inode_eigenvec (5, m_current_mode_to_buffer);
                        }
                  }
            }
        };
        ...
        {
          public:
            ResultRecorderDisplacementSensitivity (const mpco::
                                                   ProcessInfo & info,
                                                   int
                                                   grad):ResultRecorder
                (info), m_grad (grad)
            {
                ...
                ss_buffer << "MODEL_STAGE[" << info.
                    current_model_stage_id <<
                    "]/RESULTS/ON_NODES/DISPLACEMENT_SENSITIVITY_" << m_grad;
                m_result_name = ss_buffer.str ();
                m_result_display_name =
                    MPCO_MAKE_STRING ("Displacement Sensitivity d_U/d_q" <<
                                      m_grad);
                m_num_components = 0;
                if (m_ndim == 1)
                  {
                      m_components_name =
                          MPCO_MAKE_STRING ("d_Ux/d_q" << m_grad);
                      m_num_components = 1;
                      m_result_data_type = mpco::ResultDataType::Scalar;
                  }
                else if (m_ndim == 2)
                ...
                m_description =
                    MPCO_MAKE_STRING
                    ("Nodal displacement sensitivity field d_U/d_q" <<
                     m_grad);
                m_result_type = mpco::ResultType::Generic;
            }
          protected:
                                         ...
                                         std::vector < double >&buffer) const
            {
                if (m_grad < 1 || m_grad > info.domain->getNumParameters ())
                  {
                      opserr <<
                          "MPCORecorder sensitivity parameter is out of range: grad index = "
                          << m_grad << ", domain->getNumParameters() = " <<
                          info.domain->getNumParameters () << "\n";
                      return;
                  }
                for (size_t i = 0; i < nodes.size (); i++)
                  {
                      ...
                      if (m_ndim > 1)
                        {
                            buffer[j + 1] =
                                inode->getDispSensitivity (2, m_grad - 1);
                            if (m_ndim > 2)
                              {
                                  buffer[j + 2] =
                                      inode->getDispSensitivity (3,
                                                                 m_grad - 1);
                              }
                        }
                  }
        ...
        {
          public:
            ResultRecorderRotationSensitivity (const mpco::ProcessInfo & info,
                                               int
                                               grad):ResultRecorder (info),
                m_grad (grad)
            {
                std::stringstream ss_buffer;
                ss_buffer << "MODEL_STAGE[" << info.
                    current_model_stage_id <<
                    "]/RESULTS/ON_NODES/ROTATION_SENSITIVITY_" << m_grad;;
                m_result_name = ss_buffer.str ();
                m_result_display_name =
                    MPCO_MAKE_STRING ("Rotation Sensitivity d_R/d_q" <<
                                      m_grad);
                m_num_components = 0;
                if (m_ndim == 2)
                  {
                      m_components_name =
                          MPCO_MAKE_STRING ("d_Rz/d_q" << m_grad);
                      m_num_components = 1;
                      m_result_data_type = mpco::ResultDataType::Scalar;
                  }
                else
                  {
                      m_components_name =
                          MPCO_MAKE_STRING ("d_Rx/d_q" << m_grad <<
                                            ",d_Ry/d_q" << m_grad <<
                                            ",d_Rz/d_q" << m_grad);
                      m_num_components = 3;
                      m_result_data_type = mpco::ResultDataType::Vectorial;
                ...
                m_description =
                    MPCO_MAKE_STRING
                    ("Nodal rotation sensitivity field d_R/d_q" << m_grad);
                m_result_type = mpco::ResultType::Generic;
            }
          protected:
            virtual void bufferResponse (mpco::ProcessInfo & info,
                      ...
                      if (m_ndim == 2)
                        {
                            ...
                            if (iresponse.Size () > 2)
                                buffer[j] =
                                    inode->getDispSensitivity (3, m_grad - 1);
                            else
                                buffer[j] = 0.0;
                        }
                      else if (m_ndim == 3)
                        {
                            ...
                            if (iresponse.Size () > 5)
                              {
                                  buffer[j] =
                                      inode->getDispSensitivity (4,
                                                                 m_grad - 1);
                                  buffer[j + 1] =
                                      inode->getDispSensitivity (5,
                                                                 m_grad - 1);
                                  buffer[j + 2] =
                                      inode->getDispSensitivity (6,
                                                                 m_grad - 1);
                              }
                            else
                              {
        ...
        {
          public:
            ResultRecorderVelocitySensitivity (const mpco::ProcessInfo & info,
                                               int
                                               grad):ResultRecorder (info),
                m_grad (grad)
            {
                ...
                ss_buffer << "MODEL_STAGE[" << info.
                    current_model_stage_id <<
                    "]/RESULTS/ON_NODES/VELOCITY_SENSITIVITY_" << m_grad;
                m_result_name = ss_buffer.str ();
                m_result_display_name =
                    MPCO_MAKE_STRING ("Velocity Sensitivity d_V/d_q" <<
                                      m_grad);
                m_num_components = 0;
                if (m_ndim == 1)
                  {
                      m_components_name =
                          MPCO_MAKE_STRING ("d_Vx/d_q" << m_grad);
                      m_num_components = 1;
                      m_result_data_type = mpco::ResultDataType::Scalar;
                  }
                else if (m_ndim == 2)
                ...
                m_description =
                    MPCO_MAKE_STRING
                    ("Nodal velocity sensitivity field d_V/d_q" << m_grad);
                m_result_type = mpco::ResultType::Generic;
            }
          protected:
            virtual void bufferResponse (mpco::ProcessInfo & info,
                ...
                for (size_t i = 0; i < nodes.size (); i++)
                  {
                      ...
                      if (m_ndim > 1)
                        {
                            buffer[j + 1] =
                                inode->getVelSensitivity (2, m_grad - 1);
                            if (m_ndim > 2)
                              {
                                  buffer[j + 2] =
                                      inode->getVelSensitivity (3,
                                                                m_grad - 1);
                              }
                        }
                  }
        ...
        {
          public:
            ResultRecorderAngularVelocitySensitivity (const mpco::
                                                      ProcessInfo & info,
                                                      int
                                                      grad):ResultRecorder
                (info), m_grad (grad)
            {
                ...
                ss_buffer << "MODEL_STAGE[" << info.
                    current_model_stage_id <<
                    "]/RESULTS/ON_NODES/ANGULAR_VELOCITY_SENSITIVITY_" <<
                    m_grad;;
                m_result_name = ss_buffer.str ();
                m_result_display_name =
                    MPCO_MAKE_STRING ("Angular Velocity Sensitivity d_RV/d_q"
                                      << m_grad);
                m_num_components = 0;
                if (m_ndim == 2)
                  {
                      m_components_name =
                          MPCO_MAKE_STRING ("d_RVz/d_q" << m_grad);
                      m_num_components = 1;
                      m_result_data_type = mpco::ResultDataType::Scalar;
                  }
                else
                  {
                      m_components_name =
                          MPCO_MAKE_STRING ("d_RVx/d_q" << m_grad <<
                                            ",d_RVy/d_q" << m_grad <<
                                            ",d_RVz/d_q" << m_grad);
                      m_num_components = 3;
                      m_result_data_type = mpco::ResultDataType::Vectorial;
                ...
                m_description =
                    MPCO_MAKE_STRING
                    ("Nodal angular velocity sensitivity field d_RV/d_q" <<
                     m_grad);
                m_result_type = mpco::ResultType::Generic;
            }
          protected:
                      ...
                      if (m_ndim == 2)
                        {
                            ...
                            if (iresponse.Size () > 2)
                                buffer[j] =
                                    inode->getVelSensitivity (3, m_grad - 1);
                            else
                                buffer[j] = 0.0;
                        }
                      else if (m_ndim == 3)
                        {
                            ...
                            if (iresponse.Size () > 5)
                              {
                                  buffer[j] =
                                      inode->getVelSensitivity (4,
                                                                m_grad - 1);
                                  buffer[j + 1] =
                                      inode->getVelSensitivity (5,
                                                                m_grad - 1);
                                  buffer[j + 2] =
                                      inode->getVelSensitivity (6,
                                                                m_grad - 1);
                              }
                            else
                              {
        ...
        {
          public:
            ResultRecorderAccelerationSensitivity (const mpco::
                                                   ProcessInfo & info,
                                                   int
                                                   grad):ResultRecorder
                (info), m_grad (grad)
            {
                ...
                ss_buffer << "MODEL_STAGE[" << info.
                    current_model_stage_id <<
                    "]/RESULTS/ON_NODES/ACCELERATION_SENSITIVITY_" << m_grad;
                m_result_name = ss_buffer.str ();
                m_result_display_name =
                    MPCO_MAKE_STRING ("Acceleration Sensitivity d_A/d_q" <<
                                      m_grad);
                m_num_components = 0;
                if (m_ndim == 1)
                  {
                      m_components_name =
                          MPCO_MAKE_STRING ("d_Ax/d_q" << m_grad);
                      m_num_components = 1;
                      m_result_data_type = mpco::ResultDataType::Scalar;
                  }
                else if (m_ndim == 2)
                ...
                m_description =
                    MPCO_MAKE_STRING
                    ("Nodal acceleration sensitivity field d_A/d_q" <<
                     m_grad);
                m_result_type = mpco::ResultType::Generic;
            }
          protected:
        ...
        class ResultRecorderAngularAccelerationSensitivity:public
            ResultRecorder
        {
          public:
            ResultRecorderAngularAccelerationSensitivity (const mpco::
                                                          ProcessInfo & info,
                                                          int
                                                          grad):ResultRecorder
                (info), m_grad (grad)
            {
                ...
                ss_buffer << "MODEL_STAGE[" << info.
                    current_model_stage_id <<
                    "]/RESULTS/ON_NODES/ANGULAR_ACCELERATION_SENSITIVITY_" <<
                    m_grad;;
                m_result_name = ss_buffer.str ();
                m_result_display_name =
                    MPCO_MAKE_STRING
                    ("Angular Acceleration Sensitivity d_RA/d_q" << m_grad);
                m_num_components = 0;
                if (m_ndim == 2)
                  {
                      m_components_name =
                          MPCO_MAKE_STRING ("d_RAz/d_q" << m_grad);
                      m_num_components = 1;
                      m_result_data_type = mpco::ResultDataType::Scalar;
                  }
                else
                  {
                      m_components_name =
                          MPCO_MAKE_STRING ("d_RAx/d_q" << m_grad <<
                                            ",d_RAy/d_q" << m_grad <<
                                            ",d_RAz/d_q" << m_grad);
                      m_num_components = 3;
                      m_result_data_type = mpco::ResultDataType::Vectorial;
                ...
                m_description =
                    MPCO_MAKE_STRING
                    ("Nodal angular acceleration sensitivity field dRA/d_q" <<
                     m_grad);
                m_result_type = mpco::ResultType::Generic;
            }
          protected:
        ...
        typedef std::map < mpco::NodalResultType::Enum,
            ResultRecorder * >ResultRecorderMap;
...
namespace mpco
{
    ...
    namespace element
    {
        ...
        {
            OutputDescriptorHeader ():num_columns (0), num_components (),
                gauss_id (), components_path (), components (),
                multiplicity ()
            {
            }
            ...
            inline bool operator < (const OutputDescriptorHeader & other) const
            {
                ...
                if (num_columns > other.num_columns)
                    return false;
                if (multiplicity.size () < other.multiplicity.size ())
                    return true;
                if (multiplicity.size () > other.multiplicity.size ())
                    return false;
                for (size_t i = 0; i < multiplicity.size (); i++)
                  {
                      if (multiplicity[i] < other.multiplicity[i])
                          return true;
                      if (multiplicity[i] > other.multiplicity[i])
                          return false;
                  }
                if (num_components.size () < other.num_components.size ())
                      return true;
                if (num_components.size () > other.num_components.size ())
                      return false;
                for (size_t i = 0; i < num_components.size (); i++)
                  {
                      if (num_components[i] < other.num_components[i])
                          return true;
                      if (num_components[i] > other.num_components[i])
                          return false;
                  }
                if (gauss_id.size () < other.gauss_id.size ())
                      return true;
                if (gauss_id.size () > other.gauss_id.size ())
                    return false;
                for (size_t i = 0; i < gauss_id.size (); i++)
                  {
                      if (gauss_id[i] < other.gauss_id[i])
                          return true;
                      if (gauss_id[i] > other.gauss_id[i])
                          return false;
                  }
                if (components_path.size () < other.components_path.size ())
                    return true;
                if (components_path.size () > other.components_path.size ())
                    return false;
                for (size_t i = 0; i < components_path.size (); i++)
                  {
                      if (components_path[i] < other.components_path[i])
                          return true;
                      if (components_path[i] > other.components_path[i])
                          return false;
                  }
            ...
            inline std::string toString () const
            {
                ...
                for (std::vector < std::vector < int > >::const_iterator
                     it1 = components_path.begin ();
                     it1 != components_path.end (); ++it1)
                  {
                      for (std::vector < int >::const_iterator
                           it2 = it1->begin (); it2 != it1->end (); ++it2)
                        {
                            ss << mpco::ElementOutputDescriptorType::
                                toString ((mpco::ElementOutputDescriptorType::
                                           Enum) * it2) << ".";
                        }
                      ss << "\n";
                  }
                ...
                for (std::vector < std::vector < std::string >
                     >::const_iterator it1 = components.begin ();
                ...
                if (all_zero && data_size > 0)
                  {
                      if (num_components.size () == 0)
                        {       // empty
                            ...
                            components_path[0].
                                push_back (mpco::ElementOutputDescriptorType::
                                           Element);
                            components.resize (1);
                            ...
                            if (data_size % num_items == 0)
                              {
                                  ...
                                  for (size_t i = 0;
                                       i < num_components.size (); i++)
                                    {
                                        num_components[i] = n_comp;
                                        multiplicity[i] = 1;
                                        components[i].
                                            resize ((size_t) n_comp);
                                        for (int j = 0; j < n_comp; j++)
                                          {
                                              std::stringstream aux;
                                              aux << "C" << j + 1;
                                              components[i][j] = aux.str ();
                                          }
                                    }
            ...
            void workaroundForDuplicatedComponents ()
            {
                for (size_t i = 0; i < components.size (); i++)
                  {
                      std::vector < std::string > &i_components =
                          components[i];
        ...
        {
          public:
            OutputDescriptor ():type (mpco::ElementOutputDescriptorType::
                                      Element), tag (0),
                dummy_section_flag (false), gp_number (0), gp_eta (0.0),
                fib_y (0.0), fib_z (0.0), fib_a (0.0), components (), items ()
            {
            }

            OutputDescriptor (const OutputDescriptor & other):type (other.
                                                                    type),
                tag (other.tag),
                dummy_section_flag (other.dummy_section_flag),
                gp_number (other.gp_number), gp_eta (other.gp_eta),
                fib_y (other.fib_y), fib_z (other.fib_z), fib_a (other.fib_a),
                components (other.components), items ()
            {
                               ...
                               std::vector < bool >&dummy_sec_flags) const
            {
                ...
                bool *temp_dummy = 0;
                  appendFiberData (data, data_mat_id,
                                   sec_id, gp_id, dummy_sec_flags, temp_sec,
                                   temp_gp, temp_dummy);
                if (temp_gp)
                    delete temp_gp;
                if (temp_sec)
          ...
          private:
            void printInfo (int level, std::stringstream & ss) const
            {
                ...
                ss << indent << "<" << mpco::ElementOutputDescriptorType::
                    toString (this->type);
                if (this->type == mpco::ElementOutputDescriptorType::Gauss)
                  {
                      ss << " number=\"" << this->
                          gp_number << "\" eta=\"" << this->gp_eta << "\"";
                  }
                else if (this->type ==
                         mpco::ElementOutputDescriptorType::Section)
                  {
                      ss << " tag=\"" << this->tag << "\"";
                  }
                else if (this->type ==
                         mpco::ElementOutputDescriptorType::Material)
                  {
                      ss << " tag=\"" << this->tag << "\"";
                  }
                ss << ">\n";
            ...
            void makeHeaderInternal (mpco::element::
                                     OutputDescriptorHeader & header,
                                     std::list < int >&temp_path,
                                     int &temp_gp_id) const
            {
                ...
                if (components.size () > 0 || items.size () == 0)
                  {
                      ...
                      int next_gauss_id = temp_gp_id;
                        std::vector <
                          int >next_components_path (temp_path.begin (),
                                                     temp_path.end ());
                      if (header.multiplicity.size () == 0)
                        {
                            header.num_components.
                                push_back (next_num_components);
                            header.gauss_id.push_back (next_gauss_id);
                            header.components_path.
                                push_back (next_components_path);
                      ...
                      else
                        {
                            ...
                            size_t last_index =
                                header.multiplicity.size () - 1;
                            if (header.num_components[last_index] ==
                                next_num_components)
                              {
                                  if (header.gauss_id[last_index] ==
                                      next_gauss_id)
                                    {
                                        if (utils::misc::
                                            areVectorsEqual (header.
                                                             components_path
                                                             [last_index],
                                                             next_components_path))
                                          {
                                              if (utils::misc::
                                                  areVectorsEqual (header.
                                                                   components
                ...
                for (size_t i = 0; i < items.size (); i++)
                    items[i]->makeHeaderInternal (header, temp_path,
                                                  temp_gp_id);
                temp_path.pop_back ();
            }
            ...
            void appendFiberData (std::vector < mpco::element::FiberData >
                                  &data, std::vector < int >&data_mat_id,
                                  std::vector < int >&sec_id,
                                  std::vector < int >&gp_id,
                                  std::vector < bool >&dummy_sec_flags,
                                  int *&temp_sec, int *&temp_gp,
                                  bool *&temp_dummy) const
            {
                if (type == mpco::ElementOutputDescriptorType::Gauss)
                  {
                      if (temp_gp == 0)
                  ...
                  {
                      data.
                          push_back (mpco::element::
                                     FiberData (fib_y, fib_z, fib_a));
                      int fiber_mat_tag = -1;
                      if (items.size () == 1)
                        {
                            if (items[0]->type ==
                ...
                if (type == mpco::ElementOutputDescriptorType::Fiber
                    || items.size () == 0)
                  {
                      if (temp_sec)
                ...
                if (items.size () > 0)
                  {
                      if (type != mpco::ElementOutputDescriptorType::Section)
                        {
                            if (items[0]->type ==
                                mpco::ElementOutputDescriptorType::Fiber)
                              {
                                  /* check only the first one. items are of the same type... */
                                  OutputDescriptor *dummy_section_level =
                                      new OutputDescriptor ();
                                  dummy_section_level->type =
                                      mpco::ElementOutputDescriptorType::
                                      Section;
                                  dummy_section_level->tag = -123456;
                                  dummy_section_level->dummy_section_flag =
                                      true;
            ...
            void mergeGaussInternal ()
            {
                ...
                if (items.size () > 0)
                  {
                      if (items[0]->type ==
                          mpco::ElementOutputDescriptorType::Gauss)
                        {
                            ...
                            for (size_t i = 0; i < items.size (); i++)
                              {
                                  ...
                                  std::map < int,
                                      OutputDescriptor * >::iterator it =
                                      aux.find (curr_item->gp_number);
                                  if (it == aux.end ())
                                    {   // non existing
                                        aux[curr_item->gp_number] = curr_item;
                                    }
                                  else
                                    {   // existing, move curr_item->items into existing_item->items
                                        OutputDescriptor *existing_item =
                                            it->second;
                                        for (size_t j = 0;
                                             j < curr_item->items.size ();
                                             j++)
                                            existing_item->items.
                                                push_back (curr_item->
                                                           items[j]);
        ...
        {
          public:
            OutputDescriptorStream (mpco::element::OutputDescriptor *
                                    _d):OPS_Stream
                (OPS_STREAM_TAGS_MPCORecorder_ElementOutputDescriptorStream),
                descr (_d), current_level (0), pending_close_tag (false)
            {
            }
                ...
                for (int i = 1; i <= current_level; i++)
                  {
                      if (eo_curr_lev->items.size () == 0)
                        {
                            opserr <<
                                "MPCORecorder Error: cannot set attribute(name, int), empty item list.\n";
                            exit (-1);
                        }
                      eo_curr_lev =
                          eo_curr_lev->items[eo_curr_lev->items.size () - 1];
                  }
                if (current_level == 0)
                  {
                      ...
                      else if (strcmp (name, "GaussPoint") == 0
                               || strcmp (name, "GaussPointOutput") == 0)
                        {
                            mpco::element::OutputDescriptor *
                                eo_new_curr_lev =
                                new mpco::element::OutputDescriptor ();
                            eo_new_curr_lev->type =
                                mpco::ElementOutputDescriptorType::Gauss;
                            ensureItemsOfUniformType (eo_curr_lev,
                                                      eo_new_curr_lev);
                  ...
                  {
                      if (strcmp (name, "NdMaterialOutput") == 0
                          || strcmp (name, "UniaxialMaterialOutput") == 0)
                        {
                      ...
                      else if (strcmp (name, "SectionOutput") == 0
                               || strcmp (name,
                                          "SectionForceDeformation") == 0)
                        {
                ...
                if (current_level > 0)
                  {
                      ...
                      for (int i = 1; i <= current_level; i++)
                        {
                            ...
                            eo_curr_lev =
                                eo_curr_lev->items[eo_curr_lev->items.
                                                   size () - 1];
                        }
                      if (eo_curr_lev->type ==
                          mpco::ElementOutputDescriptorType::Gauss)
                      ...
                      else if (eo_curr_lev->type ==
                               mpco::ElementOutputDescriptorType::Material)
                        {
                            if (strcmp (name, "tag") == 0
                                || strcmp (name, "matTag") == 0)
          ...
          private:
            void ensureItemsOfUniformType (mpco::element::OutputDescriptor *
                                           parent,
                                           mpco::element::OutputDescriptor *
                                           child)
            {
                if (parent->items.size () > 0)
                  {
                      if (child->type != parent->items.back ()->type)
                        {
                            opserr <<
                                "MPCORecorder Error: (mpco::element::OutputDescriptor) "
                                "Responses at the same level of the response tree must be of the same type.\n"
                                "Expected: " << mpco::
                                ElementOutputDescriptorType::
                                toString (parent->items.back ()->
                                          type) << ", given: " << mpco::
                                ElementOutputDescriptorType::toString (child->
                                                                       type)
                                << "\n";
                            exit (-1);
                        }
        ...
        {
          public:
            OutputResponse ():response (0), element (0)
            {
            }
            OutputResponse (Element * _elem,
        ...
        {
            OutputResponseCollection ():is_new (true), dir_name (""),
        ...
        {
            typedef std::map < OutputDescriptorHeader,
                OutputResponseCollection > collection_type;
        ...
        {
            typedef std::map < int,
                OutputWithSameCustomIntRuleCollection > collection_type;
            collection_type items;
        };

        struct OutputWithSameClassTagCollection
        {
            typedef std::map < ElementIntegrationRuleType::Enum,
                OutputWithSameIntRuleCollection > collection_type;
            collection_type items;
        };
        ...
        {
            typedef std::map < int,
                OutputWithSameClassTagCollection > collection_type;
              ResultRecorder ():initialized (false), result_request (),
                response_map ()
            {
            }
            bool initialized;
		...
		**************************************************************************************/
        ...
        {
            ElementIntegrationRule ():int_rule_type
                (ElementIntegrationRuleType::CustomIntegrationRule), x ()
            {
            }
            ElementIntegrationRule (ElementIntegrationRuleType::
                                    Enum
                                    _int_rule_type):int_rule_type
                (_int_rule_type), x ()
            {
            }
            ...
            {
                const double rel_tol = 1.0e-5;
                if (int_rule_type < other.int_rule_type)
                      return true;
                if (int_rule_type > other.int_rule_type)
                      return false;
                ...
                for (size_t i = 0; i < x.size (); i++)
                  {
                      double tol =
                          std::max (std::abs (x[i]),
                                    std::abs (other.x[i])) * rel_tol;
                      if (utils::misc::
                          lessThanWithTol (x[i], other.x[i], rel_tol))
                            return true;
                      if (utils::misc::
                          greaterThanWithTol (x[i], other.x[i], rel_tol))
                            return false;
        ...
        struct ElementWithSameCustomIntRuleCollection
        {
            typedef std::vector < Element * >collection_type;
              ...
              ElementWithSameCustomIntRuleCollection ():is_new (true),
                custom_int_rule_index (0), name (""), items ()
            {
            }
        ...
        {
            typedef std::map < int,
                ElementWithSameCustomIntRuleCollection > submap_type;

              ElementWithSameIntRuleCollection ():is_new (true),
                int_rule_type (ElementIntegrationRuleType::
                               CustomIntegrationRule), items ()
            {
            }
        ...
        {
            typedef std::map < ElementIntegrationRuleType::Enum,
                ElementWithSameIntRuleCollection > submap_type;

              ElementWithSameClassTagCollection ():is_new (true),
                class_tag (0), class_name ("unknown"), num_nodes (0),
                geom_type (ElementGeometryType::Custom), items ()
            {
            }
            ...
            std::map < mpco::ElementIntegrationRuleType::Enum,
                ElementWithSameIntRuleCollection > items;
        };

        struct ElementCollection
        {
            typedef std::map < int,
                ElementWithSameClassTagCollection > submap_type;

              ElementCollection ():registered_custom_rules (), items ()
            {
            }

            void mapElements (Domain * d, bool has_region,
                ...
                while (true)
                  {
                      ...
                      if (has_region)
                        {
                            if (subset_elem_counter == subset.size ())
                                break;
                            current_element =
                                d->getElement (subset[subset_elem_counter++]);
                      ...
                      ElementGeometryType::Enum geom_type;
                      ElementIntegrationRuleType::Enum int_rule_type;
                      getGeometryAndIntRuleByClassTag (elem_type, geom_type,
                                                       int_rule_type);
                      /*
                         map by class tag
                       */
                      ElementWithSameClassTagCollection & elem_coll_by_tag =
                          items[elem_type];
                      if (elem_coll_by_tag.is_new)
                        {
                            elem_coll_by_tag.class_tag = elem_type;
                            elem_coll_by_tag.class_name =
                                current_element->getClassType ();
                            elem_coll_by_tag.num_nodes =
                                current_element->getNumExternalNodes ();
                       ...
                       */
                      if (current_element->getNumExternalNodes () !=
                          elem_coll_by_tag.num_nodes)
                        {
                            opserr <<
                                "MPCORecorder Error while mapping elements: elements with different number of nodes "
                      ...
                      if (int_rule_type ==
                          ElementIntegrationRuleType::CustomIntegrationRule)
                          getCustomGaussPointLocations (current_element,
                                                        int_rule);
                      /*
                      ...
                      if (int_rule_type ==
                          ElementIntegrationRuleType::CustomIntegrationRule)
                        {
                            std::map < ElementIntegrationRule,
                                int >::iterator reg_int_rule_iter =
                                aux_map_custom_rules.find (int_rule);
                            if (reg_int_rule_iter ==
                                aux_map_custom_rules.end ())
                              {
                                  // new rule, define a new index starting from 1
                                  custom_int_rule_index =
                                      (int) aux_map_custom_rules.size () + 1;
                                  aux_map_custom_rules[int_rule] =
                                      custom_int_rule_index;
                              }
                            else
                              {
                                  custom_int_rule_index =
                                      reg_int_rule_iter->second;
                              }
                        }
                      /*
                         map by integration rule
                       */
                      ElementWithSameIntRuleCollection & elem_coll_by_rule =
                          elem_coll_by_tag.items[int_rule_type];
                      ...
                      ElementWithSameCustomIntRuleCollection &
                          elem_coll_by_custom_rule =
                          elem_coll_by_rule.items[custom_int_rule_index];
                      if (elem_coll_by_custom_rule.is_new)
                        {
                            elem_coll_by_custom_rule.custom_int_rule_index =
                                custom_int_rule_index;
                       ...
                       */
                      elem_coll_by_custom_rule.items.
                          push_back (current_element);
                  }
                /*
                   now fill the custom integration rule map
                 */
                for (std::map < ElementIntegrationRule, int >::iterator it =
                     aux_map_custom_rules.begin ();
            ...
            void getGeometryAndIntRuleByClassTag (int elem_class_tag,
                                                  ElementGeometryType::
                                                  Enum & geom_type,
                                                  ...
                                                  Enum & int_type)
            {
                ...
                if (
                       ...
                       || elem_class_tag ==
                       ELE_TAG_ElastomericBearingBoucWen3d
                       || elem_class_tag ==
                       ELE_TAG_ElastomericBearingBoucWenMod3d
                       || elem_class_tag ==
                       ELE_TAG_ElastomericBearingPlasticity2d
                       || elem_class_tag ==
                       ELE_TAG_ElastomericBearingPlasticity3d
                    ...
                    )
                  {
                      ...
                      int_type =
                          ElementIntegrationRuleType::Line_GaussLegendre_1;
                  }
                /*
                 ...
                 */
                            ...
                            || elem_class_tag ==
                            ELE_TAG_FourNodeQuadWithSensitivity ||
                            // ./shell
                 ...
                 */
                            ...
                            // ./up
                            elem_class_tag ==
                            ELE_TAG_Twenty_Eight_Node_BrickUP)
                  {
            ...
            void getCustomGaussPointLocations (Element * elem,
                                               ElementIntegrationRule & rule)
            {
                /*
                    ...
                    if (eo_response)
                      {
                          ...
                          const Vector & data =
                              eo_response->getInformation ().getData ();
                          if (data.Size () > 0)
                            {
                                if (data.Size () == 1)
                                  {
                    ...
                    while (true)
                      {
                          ...
                          std::stringstream ss_trial_num;
                          ss_trial_num << trial_num;
                                ...
                                if (trial_x.size () > 1)
                                  {
                                      ...
                                      opserr <<
                                          "MPCORecorder warning: iterative guess of ngp: expected 1 trial gauss location, given = "
                                          << (int) trial_x.
                                          size () <<
                 ...
                 */
                opserr <<
                    "MPCORecorder warning: cannot get custom integration rule from element "
                    << elem->getTag () << "[class = " << elem->
                    getClassType () << "]\n";
            }
...
#define SerializerFormatDouble std::setprecision(std::numeric_limits<double>::digits10 + 1)
        ...
        class Serializer
        {
          ...
          public:
            ...
            inline Serializer & operator << (mpco::OutputFrequency::
                                             IncrementType x)
            {
            ...
            inline Serializer & operator << (const std::vector <
                                             mpco::NodalResultType::Enum > &x)
            {
                ss << x.size () << '\n';
              for (auto i:x)
                    ss << i << '\n';
          ...
          public:
                                             ...
                                             IncrementType & x)
            {
                int temp;
                ss >> temp;
                x = static_cast < mpco::OutputFrequency::IncrementType >
                    (temp);
                return *this;
            }
            inline Serializer & operator >> (std::size_t &x)
            ...
            inline Serializer & operator >> (std::vector <
                                             mpco::NodalResultType::Enum > &x)
            {
                std::size_t n;
                if (!(ss >> n))
                ...
                for (std::size_t i = 0; i < n; ++i)
                  {
                      ...
                      ss >> temp;
                      x[i] =
                          static_cast < mpco::NodalResultType::Enum > (temp);
                  }
                return *this;
            }
        };
...
MPCORecorder::MPCORecorder ():Recorder (RECORDER_TAGS_MPCORecorder),
    ...
    if (m_data->initialized)
      {
          ...
          if (status < 0)
            {
                opserr <<
                    "MPCORecorder Error: cannot close file on destructor\n";
            }
          /*
...
int
MPCORecorder::record (int commitTag, double timeStamp)
{
    /*
       error flags
     */
    ...
    if (!m_data->initialized)
      ...
      {
          if (m_data->output_freq.type ==
              mpco::OutputFrequency::NumberOfSteps)
            {
                if ((m_data->info.current_time_step_id -
                     m_data->output_freq.last_step) >=
                    m_data->output_freq.nsteps)
                  {
                      do_record = true;
                  }
            }
          else if (m_data->output_freq.type ==
                   mpco::OutputFrequency::DeltaTime)
            {
                if (std::
                    abs (m_data->info.current_time_step -
                         m_data->output_freq.last_time) >=
    ...
    if (!m_data->initialized)
      {
          ...
          if (retval)
            {
                opserr <<
                    "MPCRecorder Error: cannot initialize the recorder\n";
                return retval;
            }
      }
    /*
    ...
    if (!m_data->first_domain_changed_done)
      {
          m_data->info.current_model_stage_id =
              m_data->info.domain->hasDomainChanged ();
...
int
MPCORecorder::restart (void)
{
    return 0;
}

int
MPCORecorder::domainChanged (void)
{
    return 0;
}

int
MPCORecorder::setDomain (Domain & theDomain)
{
    if (m_data->info.domain)
      {
          if (m_data->info.domain != &theDomain)
            {
                opserr <<
                    "MPCORecorder Error: setDomain with an existing domain is not implemented yet...\n";
                exit (-1);
            }
      }
...
int
MPCORecorder::sendSelf (int commitTag, Channel & theChannel)
{
    if (theChannel.isDatastore () == 1)
      {
          opserr <<
              "MPCORecorder::sendSelf() - does not send data to a datastore\n";
          return -1;
      }
    ...
    {
        std::stringstream ss;
        for (size_t i = 0; i < m_data->elemental_results_requests.size ();
             i++)
          {
              if (i > 0)
                  ss << ':';    // char separator for results
              const std::vector < std::string > &i_request =
                  m_data->elemental_results_requests[i];
    ...
    if (!(ser
          // misc info
          << getTag () << m_data->send_self_count
          // filename
          << m_data->filename
          // output frequency
          << m_data->output_freq.type
          << m_data->output_freq.dt << m_data->output_freq.nsteps
          ...
          // region
          << m_data->has_region << m_data->node_set << m_data->elem_set))
      {
    ...
    if (theChannel.sendID (0, commitTag, idata) < 0)
      {
          opserr <<
              "MPCORecorder::sendSelf() - failed to send message size\n";
          return -1;
      }
...
int
MPCORecorder::recvSelf (int commitTag, Channel & theChannel,
                        FEM_ObjectBroker & theBroker)
{
    if (theChannel.isDatastore () == 1)
      {
          opserr <<
              "MPCORecorder::recvSelf() - does not recv data to a datastore\n";
          return -1;
      }
    ...
    if (theChannel.recvID (0, commitTag, idata) < 0)
      {
          opserr <<
              "MPCORecorder::recvSelf() - failed to recv message size\n";
          return -1;
      }
    ...
    if (!(ser
          // misc info
          >> my_tag >> m_data->p_id     // use the send self counter as p_id in the receiver
          // filename
          >> m_data->filename
          // output frequency
          >> m_data->output_freq.type
      ...
      {
          opserr <<
              "MPCORecorder::recvSelf() - failed to de-serialzie data\n";
          return -1;
      }
    ...
    if (elem_res_merged_string.size () > 0)
      {
          ...
          utils::strings::split (std::
                                 string (elem_res_merged_string.begin (),
                                         elem_res_merged_string.end ()), ':',
                                 aux_1, true);
...
MPCORecorder::initialize ()
{
                ...
                if (m_data->info.num_dimensions < 1)
                ...
                else
                  {
                      if (m_data->info.num_dimensions != trial_dim)
                        {
                            opserr <<
                                "MPCORecorder error: nodes with different dimensions, this is not supported\n";
                            exit (-1);
                        }
                  }
            }
          if (m_data->info.num_dimensions < 1
              || m_data->info.num_dimensions > 3)
            {
                opserr <<
                    "MPCORecorder error: invalid spatial dimension. expected 1 2 or 3, given: "
                    << m_data->info.num_dimensions << "\n";
                exit (-1);
            }
      }
    /*
    ...
    status =
        h5::plist::setLinkCreationOrder (m_data->info.h_file_proplist,
                                         H5P_CRT_ORDER_TRACKED |
                                         H5P_CRT_ORDER_INDEXED);
    m_data->info.h_group_proplist = h5::plist::crate (h5::plist::GroupCreate);
    status =
        h5::plist::setLinkCreationOrder (m_data->info.h_group_proplist,
                                         H5P_CRT_ORDER_TRACKED |
                                         H5P_CRT_ORDER_INDEXED);
    /*
    ...
    std::string the_filename;
    {
        ...
        if (m_data->send_self_count != 0)
          {                     // > 0 -> we are in p0, < 0 -> we are in secondary procs, = 0 -> not in parallel
...
#ifdef MPCO_USE_SWMR
    m_data->info.h_file_acc_proplist =
        h5::plist::crate (h5::plist::FileAccess);
    status =
        h5::plist::setLibVerBounds (m_data->info.h_file_acc_proplist,
                                    H5F_LIBVER_LATEST, H5F_LIBVER_LATEST);
    m_data->info.h_file_id =
        h5::file::create (the_filename.c_str (), m_data->info.h_file_proplist,
                          m_data->info.h_file_acc_proplist);
    ...
    if (m_data->info.h_file_id == HID_INVALID)
      {
          opserr << "MPCORecorder Error: cannot create or open file: \"" <<
              the_filename.c_str () << "\"";
          exit (-1);
      }
    /*
     ...
     */
    hid_t h_gp_info =
        h5::group::create (m_data->info.h_file_id, "INFO", H5P_DEFAULT,
                           m_data->info.h_group_proplist, H5P_DEFAULT);
    hid_t h_dset_dim =
        h5::dataset::createAndWrite (h_gp_info, "SPATIAL_DIM",
                                     m_data->info.num_dimensions);
...
MPCORecorder::writeModel ()
{
    ...
    ss_current_model_stage_name << "MODEL_STAGE[" << m_data->info.
        current_model_stage_id << "]";
    std::string model_stage_dir = ss_current_model_stage_name.str ();
    hid_t h_gp_model_stage =
        h5::group::create (m_data->info.h_file_id, model_stage_dir.c_str (),
                           H5P_DEFAULT, m_data->info.h_group_proplist,
                           H5P_DEFAULT);
    hid_t h_gp_model =
        h5::group::create (h_gp_model_stage, "MODEL", H5P_DEFAULT,
                           m_data->info.h_group_proplist, H5P_DEFAULT);
    hid_t h_gp_results =
        h5::group::create (h_gp_model_stage, "RESULTS", H5P_DEFAULT,
                           m_data->info.h_group_proplist, H5P_DEFAULT);
    hid_t h_gp_results_on_nodes =
        h5::group::create (h_gp_results, "ON_NODES", H5P_DEFAULT,
                           m_data->info.h_group_proplist, H5P_DEFAULT);
    hid_t h_gp_results_on_elements =
        h5::group::create (h_gp_results, "ON_ELEMENTS", H5P_DEFAULT,
                           m_data->info.h_group_proplist, H5P_DEFAULT);
    /*
       attributes
     */
    status =
        h5::attribute::write (h_gp_model_stage, "STEP",
                              m_data->info.current_time_step_id);
    status =
        h5::attribute::write (h_gp_model_stage, "TIME",
                              m_data->info.current_time_step);
    /*
       close groups
     */
...
int
MPCORecorder::writeModelNodes ()
{
#ifdef MPCO_TIMING
    mpco::Timer timer ("writeModelNodes");
    timer.start ();
#endif // MPCO_TIMING
    /*
       error flags
    ...
    Pressure_ConstraintIter & pressure_constraint_iter =
        m_data->info.domain->getPCs ();
    ...
    if (m_data->has_region)
      {                         // only nodes in the requested regions
          for (std::vector < int >::const_iterator id_iter =
               m_data->node_set.begin (); id_iter != m_data->node_set.end ();
               ++id_iter)
            {
                int node_id = *id_iter;
                if (pressure_node_tags.getLocationOrdered (node_id) < 0)
                  {
                      Node *current_node =
                          m_data->info.domain->getNode (node_id);
                      if (current_node != 0)
                        {
    ...
    std::vector <
        double >buffer_coord (num_nodes * m_data->info.num_dimensions);
    ...
    ss_gp_nodes_dir << "MODEL_STAGE[" << m_data->info.
        current_model_stage_id << "]/MODEL/NODES";
    std::string gp_nodes_dir = ss_gp_nodes_dir.str ();
    hid_t h_gp_nodes =
        h5::group::create (m_data->info.h_file_id, gp_nodes_dir.c_str (),
                           H5P_DEFAULT, m_data->info.h_group_proplist,
                           H5P_DEFAULT);
    /*
       create the ID dataset
     ...
     */
    hid_t dset_coord =
        h5::dataset::createAndWrite (h_gp_nodes, "COORDINATES", buffer_coord,
                                     num_nodes,
                                     (hsize_t) m_data->info.num_dimensions);
...
int
MPCORecorder::writeModelElements ()
{
#ifdef MPCO_TIMING
    mpco::Timer timer ("writeModelElements");
    timer.start ();
#endif // MPCO_TIMING
    /*
       error flags
    ...
    ss_gp_elements_dir << "MODEL_STAGE[" << m_data->info.
        current_model_stage_id << "]/MODEL/ELEMENTS";
    std::string gp_elements_dir = ss_gp_elements_dir.str ();
    hid_t h_gp_elements =
        h5::group::create (m_data->info.h_file_id, gp_elements_dir.c_str (),
                           H5P_DEFAULT, m_data->info.h_group_proplist,
                           H5P_DEFAULT);
    /*
       map all elements
     */
    m_data->elements.mapElements (m_data->info.domain, m_data->has_region,
                                  m_data->elem_set);
    /*
       loop over all element group-by-tag
     */
    for (mpco::element::ElementCollection::submap_type::iterator it1 =
         m_data->elements.items.begin ();
         it1 != m_data->elements.items.end (); ++it1)
      {
          mpco::element::ElementWithSameClassTagCollection & elem_by_tag =
              it1->second;
          /*
             loop over all element group-by-integration rule
           */
          for (mpco::element::ElementWithSameClassTagCollection::submap_type::
               iterator it2 = elem_by_tag.items.begin ();
               it2 != elem_by_tag.items.end (); ++it2)
            {
                mpco::element::
                    ElementWithSameIntRuleCollection & elem_by_rule =
                    it2->second;
                /*
                   loop over all element group-by-custom(or default) integration rule
                 */
                for (mpco::element::ElementWithSameIntRuleCollection::
                     submap_type::iterator it3 = elem_by_rule.items.begin ();
                     it3 != elem_by_rule.items.end (); ++it3)
                  {
                      mpco::element::
                          ElementWithSameCustomIntRuleCollection &
                          elem_by_custom_rule = it3->second;
                      /*
                      ...
                      if (elem_by_custom_rule.items.size () > 0)
                        {
                             ...
                             */
                            std::stringstream ss_dset_name;
                            ss_dset_name << elem_by_tag.
                                class_tag << "-" << elem_by_tag.
                                class_name << "[" << elem_by_rule.
                                int_rule_type << ":" << elem_by_custom_rule.
                                custom_int_rule_index << "]";
                             ...
                             */
                            std::vector <
                                int >buffer (elem_by_custom_rule.items.
                                             size () * (1 +
                                                        elem_by_tag.
                                                        num_nodes));
                            size_t offset = 0;
                            for (mpco::element::
                                 ElementWithSameCustomIntRuleCollection::
                                 collection_type::iterator it4 =
                                 elem_by_custom_rule.items.begin ();
                                 it4 != elem_by_custom_rule.items.end ();
                                 ++it4)
                              {
                                  Element *elem = *it4;
                                  buffer[offset] = elem->getTag ();
                                  for (size_t j = 0;
                                       j < elem_by_tag.num_nodes; j++)
                                      buffer[j + 1 + offset] =
                                          elem->getExternalNodes ()((int) j);
                                  offset += (1 + elem_by_tag.num_nodes);
                              }
                            hid_t dset_id =
                                h5::dataset::createAndWrite (h_gp_elements,
                                                             elem_by_custom_rule.
                                                             name.c_str (),
                                                             buffer,
                                                             elem_by_custom_rule.
                             ...
                             */
                            status =
                                h5::attribute::write (dset_id, "GEOMETRY",
                                                      (int) elem_by_tag.
                                                      geom_type);
                            status =
                                h5::attribute::write (dset_id,
                                                      "INTEGRATION_RULE",
                                                      (int) elem_by_rule.
                                                      int_rule_type);
                            if (elem_by_rule.int_rule_type ==
                                mpco::ElementIntegrationRuleType::
                                CustomIntegrationRule)
                              {
                                  status =
                                      h5::attribute::write (dset_id,
                                                            "CUSTOM_INTEGRATION_RULE",
                                                            elem_by_custom_rule.
                                                            custom_int_rule_index);
                                  if (elem_by_custom_rule.
                                      custom_int_rule_index != 0)
                                    {
                                        mpco::element::
                                            ...
                                            [elem_by_custom_rule.
                                             custom_int_rule_index];
                                        h5::attribute::write (dset_id, "GP_X",
                                                              custom_rule.x);
                                    }
                              }
                            /*
...
int
MPCORecorder::writeModelLocalAxes ()
{
#ifdef MPCO_TIMING
    mpco::Timer timer ("writeModelLocalAxes");
    timer.start ();
#endif // MPCO_TIMING
               ...
               it2 != elem_by_tag.items.end (); ++it2)
            {
                ...
                for (mpco::element::ElementWithSameIntRuleCollection::
                     ...
                     it3 != elem_by_rule.items.end (); ++it3)
                  {
                       ...
                       */
                      for (std::vector < Element * >::iterator it4 =
                           elem_by_custom_rule.items.begin ();
                           it4 != elem_by_custom_rule.items.end (); ++it4)
                        {
                            ...
                            mpco::element::
                                OutputDescriptorStream
                                eo_stream (&eo_descriptor);
                            Response *eo_response =
                                elem->setResponse (argv, argc, eo_stream);
                            if (eo_response)
                              {
                                  eo_response->getResponse ();
                                  const Vector & local_axes_packed_9 =
                                      eo_response->getInformation ().
                                      getData ();
                                  if (local_axes_packed_9.Size () != 9)
                                    {
                                        opserr <<
                                            "MPCORecorder Error: invalid response data size for local axes\n";
                                        continue;
                                    }
                                  for (int i = 0; i < 3; i++)
                                    {
                                  ...
                                  buffer_el_data.
                                      push_back (utils::locax::
                                                 quatFromMat (vx, vy, vz));
#ifdef MPCO_WRITE_LOC_AX_IS_VERBOSE
                                  const utils::locax::quaternion & qq =
                                      buffer_el_data.back ();
                                  ss << "E: " << elem->
                                      getTag () << " " << "[" << vx (0) <<
                                      ", " << vx (1) << ", " << vx (2) << "]"
    ...
    if (buffer_el_id.size () != buffer_el_data.size ())
      {
          opserr <<
              "MPCORecorder Error: buffer_el_id.size() != buffer_el_data.size()\n";
          return -1;
      }
    ...
    ss_gp_local_axes_dir << "MODEL_STAGE[" << m_data->info.
        current_model_stage_id << "]/MODEL/LOCAL_AXES";
    std::string gp_local_axes_dir = ss_gp_local_axes_dir.str ();
    hid_t h_gp_local_axes =
        h5::group::create (m_data->info.h_file_id, gp_local_axes_dir.c_str (),
                           H5P_DEFAULT, m_data->info.h_group_proplist,
                           H5P_DEFAULT);
    /*
       ID dataset
     */
    hid_t dset_id =
        h5::dataset::createAndWrite (h_gp_local_axes, "ID", buffer_el_id);
    status = h5::dataset::close (dset_id);
    /*
       local axes dataset
     */
    hid_t dset_axes =
        h5::dataset::createAndWrite (h_gp_local_axes, "LOCAL_AXES",
                                     buffer_el_data);
...
int
MPCORecorder::writeSections ()
{
#ifdef MPCO_TIMING
    mpco::Timer timer ("writeSections");
    timer.start ();
#endif // MPCO_TIMING
    /*
       error flags
    ...
    std::map < mpco::element::FiberSectionData,
        mpco::element::SectionAssignment > aux_sec_assignments;
    ...
    ss_gp_section_assignments_dir << "MODEL_STAGE[" << m_data->info.
        current_model_stage_id << "]/MODEL/SECTION_ASSIGNMENTS";
    std::string gp_section_assignments_dir =
        ss_gp_section_assignments_dir.str ();
    hid_t h_gp_section_assignments =
        h5::group::create (m_data->info.h_file_id,
                           gp_section_assignments_dir.c_str (), H5P_DEFAULT,
                           m_data->info.h_group_proplist, H5P_DEFAULT);
    /*
       loop over all element group-by-tag
     */
                     ...
                     it3 != elem_by_rule.items.end (); ++it3)
                  {
                      ...
                      for (std::vector < Element * >::iterator it4 =
                           ...
                           it4 != elem_by_custom_rule.items.end (); ++it4)
                        {
                             ...
                             */
                            std::string request1 =
                                utils::shell::isShellElementTag (elem->
                                                                 getClassTag
                                                                 ())?
                                "material" : "section";
                            ...
                            std::vector < mpco::element::FiberSectionData >
                                elem_sections;
                            ...
                            while (true)
                              {
                                  ...
                                  std::stringstream ss_trial_num_sec;
                                  ss_trial_num_sec << trial_num_sec;
                                  std::string s_trial_num_sec =
                                      ss_trial_num_sec.str ();
                                  ...
                                  while (true)
                                    {
                                        ...
                                        std::stringstream ss_trial_num_fib;
                                        ss_trial_num_fib << trial_num_fib;
                                        std::string s_trial_num_fib =
                                            ss_trial_num_fib.str ();
                                        ...
                                        if (trial_gp_id.size () == 0)
                                          {
                                              ...
                                              std::
                                                  cout <<
                                                  "MPCORecorder: exiting fiber loop (no gp). iter = "
                                                  << trial_num_fib << "\n";
#endif // MPCO_WRITE_SECTION_IS_VERBOSE
                                              break_sec_loop = true;
                                              break;
                                        ...
                                        if (trial_sec_id.size () == 0)
                                          {
                                              ...
                                              std::
                                                  cout <<
                                                  "MPCORecorder: exiting fiber loop (no sec). iter = "
                                                  << trial_num_fib << "\n";
#endif // MPCO_WRITE_SECTION_IS_VERBOSE
                                              break;
                                          }
                                        ...
                                        if (trial_fiber_material_id.size () !=
                                            trial_fiberdata.size ())
                                          {
                                              // this should never happen!
                                              opserr <<
                                                  "MPCORecorder FATAL Error: trial_fiber_material_id.size() != trial_fiberdata.size()\n";
                                              exit (-1);
                                          }
                                        if ((trial_sec_id.size () > 0)
                                            && (trial_gp_id.size () !=
                                                trial_sec_id.size ()))
                                          {
                                              // this should never happen!
                                              opserr <<
                                                  "MPCORecorder FATAL Error: trial_gp_id.size() != trial_sec_id.size()\n";
                                              exit (-1);
                                          }
                                        if (trial_sec_id.size () !=
                                            trial_dummy_flag.size ())
                                          {
                                              // this should never happen!
                                              opserr <<
                                                  "MPCORecorder FATAL Error: trial_sec_id.size() != trial_dummy_flag.size()\n";
                                              exit (-1);
                                          }
                                        if ((trial_fiberdata.size () > 1)
                                            || (trial_sec_id.size () > 1)
                                            || (trial_gp_id.size () > 1)
                                            || (trial_dummy_flag.size () > 1))
                                          {
                                              // this should never happen!
                                              opserr <<
                                                  "MPCORecorder FATAL Error: iterative guess of num fibers: expected 1 trial output, given = \n"
                                                  << "# fiber data: " << (int)
                                                  trial_fiberdata.
                                                  size () << "\n" <<
                                                  "# sec id: " << (int)
							...
							*/
                                        ...
                                        if (!first_fiber_done)
                                          {
                                              elem_gauss_id.
                                                  push_back (trial_gp_id[0]);
                                              elem_sec_id.
                                                  push_back (trial_sec_id[0]);
                                              elem_dummy_sec_flags.
                                                  push_back (trial_dummy_flag
                                                             [0]);
                                              elem_sections.
                                                  push_back (mpco::element::
                                                             FiberSectionData
                                        ...
                                        if (trial_fiberdata.size () == 0)
                                          {
                                              ...
                                              std::
                                                  cout <<
                                                  "MPCORecorder: exiting fiber loop. iter = "
                                                  << trial_num_fib << "\n";
#endif // MPCO_WRITE_SECTION_IS_VERBOSE
                                              break;
                                          }
                                        ...
                                        bool curr_dummy_flag =
                                            trial_dummy_flag[0];
                                        if (curr_gauss_id != trial_gp_id[0])
                                          {
                                              opserr <<
                                                  "MPCORecorder Error: curr gauss id = "
                                                  << curr_gauss_id <<
                                                  ", trial gauss id = " <<
                                                  trial_gp_id[0] << "\n";
                                              exit (-1);
                                        ...
                                        if (curr_sec_id != trial_sec_id[0])
                                          {
                                              opserr <<
                                                  "MPCORecorder Error: curr sec id = "
                                                  << curr_sec_id <<
                                                  ", trial sec id = " <<
                                                  trial_sec_id[0] << "\n";
                                              exit (-1);
                                        ...
                                        if (curr_dummy_flag !=
                                          ...
                                          {
                                              opserr <<
                                                  "MPCORecorder Error: curr dummy_flag = "
                                                  << std::
                                                  boolalpha << curr_dummy_flag
                                                  << ", trial dummy flag = "
                                                  << std::
                                                  boolalpha <<
                                                  trial_dummy_flag[0] << "\n";
                                        ...
                                        if (trial_fiberdata.size () == 1)
                                          {
                                              mpco::element::
                                                  ...
                                                  curr_sec_fiberdata =
                                                  elem_sections.back ();
                                              curr_sec_fiberdata.fibers.
                                                  push_back (trial_fiberdata
                                                             [0]);
                                              curr_sec_fiberdata.materials.
                                                  push_back
                                                  (trial_fiber_material_id
                                                   [0]);
                                          }
                                  ...
                                  if (break_sec_loop)
                                    {
                                        ...
                                        std::
                                            cout <<
                                            "MPCORecorder: exiting section loop. iter = "
                                            << trial_num_sec << "\n";
#endif // MPCO_WRITE_SECTION_IS_VERBOSE
                                        break;
                                    }
                            ...
                            buff << "\nsection dummy flags:\n";
                            for (size_t i = 0;
                                 i < elem_dummy_sec_flags.size (); i++)
                                buff << std::
                                    boolalpha << elem_dummy_sec_flags[i] <<
                                    "  ";
                            buff << "\nsection fiber data:\n";
                            for (size_t i = 0; i < elem_sections.size (); i++)
                              {
                                  mpco::element::FiberSectionData & idata =
                                      elem_sections[i];
                                  ...
                                  buff << "    " << std::
                                      ...
                                      setw (10) << "Z" << std::
                                      setw (10) << "A" << "\n";
                                  for (size_t j = 0; j < idata.fibers.size ();
                                       j++)
                                    {
                                        mpco::element::FiberData & jdata =
                                            idata.fibers[j];
                                        buff << "    " << std::
                                            setw (10) << jdata.
                                            y << std::setw (10) << jdata.
                            ...
                            std::vector < std::pair < int,
                                int >>&nfibers_per_gauss_point =
                                m_data->elem_ngauss_nfiber_info[elem->
                                                                getTag ()];
                            nfibers_per_gauss_point.resize (elem_gauss_id.
                                                            size ());
                            for (size_t igp = 0; igp < elem_gauss_id.size ();
                                 igp++)
                              {
                                  mpco::element::
                                      FiberSectionData & igp_fibersec_data =
                                      elem_sections[igp];
                                  nfibers_per_gauss_point[igp].first = elem_fiber_base_index[igp];      // start witn standard 0-based
                                  nfibers_per_gauss_point[igp].second =
                                      static_cast <
                                      int >(igp_fibersec_data.fibers.size ());
                              }
                            /*
                            ...
                            if (ngauss != elem_sec_id.size ()
                                || ngauss != elem_dummy_sec_flags.size ()
                                || ngauss != elem_sections.size ())
                              {
                                  opserr <<
                                      "MPCORecorder FATAL Error: inconsistent dimensions in writeFiberSections():\n"
                                      << "# gauss = " << (int) ngauss <<
                                      "# sec id = " << (int) elem_sec_id.
                                      size () << "# sec dummy flags = " <<
                                      (int) elem_dummy_sec_flags.
                                      size () << "# sections = " << (int)
                                      elem_sections.size () << "\n";
                                  exit (-1);
                              }
                            for (size_t igp = 0; igp < ngauss; igp++)
                              {
                                  ...
                                  bool igp_dummy_sec_flag =
                                      elem_dummy_sec_flags[igp];
                                  mpco::element::
                                      FiberSectionData & igp_fibersec_data =
                                      elem_sections[igp];
    ...
    for (std::map < int, mpco::element::SectionAssignment >::iterator
      ...
      {
          SectionForceDeformation *sfd =
              OPS_getSectionForceDeformation (it->first);
          if (sfd)
            {
     ...
     */
    int max_sec_tag =
        sec_assignments.size () > 0 ? (--sec_assignments.end ())->first : 0;
    for (std::map < mpco::element::FiberSectionData,
         mpco::element::SectionAssignment >::iterator it =
         aux_sec_assignments.begin (); it != aux_sec_assignments.end (); ++it)
      {
          ...
          if (sec_asn.assignments.size () > 0)
            {
                 ...
                 */
                hid_t h_gp_isec_asn =
                    h5::group::create (h_gp_section_assignments,
                                       gp_name_buffer.c_str (), H5P_DEFAULT,
                                       m_data->info.h_group_proplist,
                                       H5P_DEFAULT);
                /*
                   write attributes
                 */
                status =
                    h5::attribute::write (h_gp_isec_asn, "ID", it->first);
                status =
                    h5::attribute::write (h_gp_isec_asn, "NAME",
                                          sec_asn.name);
                /*
                ...
                {
                    hid_t dset =
                        h5::dataset::createAndWrite (h_gp_isec_asn,
                                                     "ASSIGNMENT",
                                                     sec_asn.assignments);
...
int
MPCORecorder::writeSets ()
{
#ifdef MPCO_TIMING
    mpco::Timer timer ("writeSets");
    timer.start ();
#endif // MPCO_TIMING
    /*
       error flags
    ...
    if (tags.Size () > 0)
      {
          ...
          ss_gp_sets << "MODEL_STAGE[" << m_data->info.
              current_model_stage_id << "]/MODEL/SETS";
          std::string gp_sets = ss_gp_sets.str ();
          hid_t h_gp_sets =
              h5::group::create (m_data->info.h_file_id, gp_sets.c_str (),
                                 H5P_DEFAULT, m_data->info.h_group_proplist,
                                 H5P_DEFAULT);
          /*
             for each region
          ...
          for (int i = 0; i < tags.Size (); i++)
            {
                 ...
                 */
                MeshRegion *region =
                    m_data->info.domain->getRegion (tags (i));
                if (region == NULL)
                    continue;
                /*
                ...
                hid_t h_gp_curr_set =
                    h5::group::create (h_gp_sets, gp_curr_set.c_str (),
                                       H5P_DEFAULT,
                                       m_data->info.h_group_proplist,
                                       H5P_DEFAULT);
                /*
                          ...
                          if (aux.size () > 0)
                            {
                                hid_t dset_id =
                                    h5::dataset::
                                    createAndWrite (h_gp_curr_set, "NODES",
                                                    aux);
                                status = h5::dataset::close (dset_id);
                            }
                      }
                          ...
                          if (aux.size () > 0)
                            {
                                hid_t dset_id =
                                    h5::dataset::
                                    createAndWrite (h_gp_curr_set, "ELEMENTS",
                                                    aux);
                                status = h5::dataset::close (dset_id);
                            }
                      }
...
int
MPCORecorder::initNodeRecorders ()
{
#ifdef MPCO_TIMING
    mpco::Timer timer ("initNodeRecorders");
    timer.start ();
#endif // MPCO_TIMING
    /*
    ...
    for (size_t i = 0; i < m_data->nodal_results_requests.size (); i++)
      {
          mpco::NodalResultType::Enum rtype =
              m_data->nodal_results_requests[i];
          switch (rtype)
            {
            case mpco::NodalResultType::Displacement:
                m_data->nodal_recorders[rtype] =
                    new mpco::node::ResultRecorderDisplacement (m_data->info);
                break;
            case mpco::NodalResultType::Rotation:
                m_data->nodal_recorders[rtype] =
                    new mpco::node::ResultRecorderRotation (m_data->info);
                break;
            case mpco::NodalResultType::Pressure:
                m_data->nodal_recorders[rtype] =
                    new mpco::node::ResultRecorderPressure (m_data->info);
                break;
            case mpco::NodalResultType::ReactionForce:
                m_data->nodal_recorders[rtype] =
                    new mpco::node::ResultRecorderReactionForce (m_data->
                                                                 info);
                break;
            case mpco::NodalResultType::ReactionMoment:
                m_data->nodal_recorders[rtype] =
                    new mpco::node::ResultRecorderReactionMoment (m_data->
                                                                  info);
                break;
            case mpco::NodalResultType::ReactionForceIncludingInertia:
                m_data->nodal_recorders[rtype] =
                    new mpco::node::
                    ResultRecorderReactionForceIncIntertia (m_data->info);
                break;
            case mpco::NodalResultType::ReactionMomentIncludingInertia:
                m_data->nodal_recorders[rtype] =
                    new mpco::node::
                    ResultRecorderReactionMomentIncIntertia (m_data->info);
                break;
            case mpco::NodalResultType::RayleighForce:
                m_data->nodal_recorders[rtype] =
                    new mpco::node::
                    ResultRecorderReactionForceRayleigh (m_data->info);
                break;
            case mpco::NodalResultType::RayleighMoment:
                m_data->nodal_recorders[rtype] =
                    new mpco::node::
                    ResultRecorderReactionMomentRayleigh (m_data->info);
                break;
            case mpco::NodalResultType::Velocity:
                m_data->nodal_recorders[rtype] =
                    new mpco::node::ResultRecorderVelocity (m_data->info);
                break;
            case mpco::NodalResultType::AngularVelocity:
                m_data->nodal_recorders[rtype] =
                    new mpco::node::ResultRecorderAngularVelocity (m_data->
                                                                   info);
                break;
            case mpco::NodalResultType::Acceleration:
                m_data->nodal_recorders[rtype] =
                    new mpco::node::ResultRecorderAcceleration (m_data->info);
                break;
            case mpco::NodalResultType::AngularAcceleration:
                m_data->nodal_recorders[rtype] =
                    new mpco::node::
                    ResultRecorderAngularAcceleration (m_data->info);
                break;
            case mpco::NodalResultType::ModesOfVibration:
                m_data->nodal_recorders[rtype] =
                    new mpco::node::ResultRecorderModesOfVibration (m_data->
                                                                    info);
                break;
            case mpco::NodalResultType::ModesOfVibrationRotational:
                m_data->nodal_recorders[rtype] =
                    new mpco::node::
                    ResultRecorderModesOfVibrationRotational (m_data->info);
                break;
            case mpco::NodalResultType::DisplacementSensitivity:
                m_data->nodal_recorders[rtype] =
                    new mpco::node::
                    ResultRecorderDisplacementSensitivity (m_data->info,
                                                           m_data->
                                                           sens_grad_indices
                                                           [i]);
                break;
            case mpco::NodalResultType::RotationSensitivity:
                m_data->nodal_recorders[rtype] =
                    new mpco::node::
                    ResultRecorderRotationSensitivity (m_data->info,
                                                       m_data->
                                                       sens_grad_indices[i]);
                break;
            case mpco::NodalResultType::VelocitySensitivity:
                m_data->nodal_recorders[rtype] =
                    new mpco::node::
                    ResultRecorderVelocitySensitivity (m_data->info,
                                                       m_data->
                                                       sens_grad_indices[i]);
                break;
            case mpco::NodalResultType::AngularVelocitySensitivity:
                m_data->nodal_recorders[rtype] =
                    new mpco::node::
                    ResultRecorderAngularVelocitySensitivity (m_data->info,
                                                              m_data->
                                                              sens_grad_indices
                                                              [i]);
                break;
            case mpco::NodalResultType::AccelerationSensitivity:
                m_data->nodal_recorders[rtype] =
                    new mpco::node::
                    ResultRecorderAccelerationSensitivity (m_data->info,
                                                           m_data->
                                                           sens_grad_indices
                                                           [i]);
                break;
            case mpco::NodalResultType::AngularAccelerationSensitivity:
                m_data->nodal_recorders[rtype] =
                    new mpco::node::
                    ResultRecorderAngularAccelerationSensitivity (m_data->
                                                                  info,
                                                                  m_data->
                                                                  sens_grad_indices
                                                                  [i]);
...
int
MPCORecorder::clearNodeRecorders ()
{
    for (mpco::node::ResultRecorderMap::iterator it =
         m_data->nodal_recorders.begin ();
...
int
MPCORecorder::initElementRecorders ()
{
#ifdef MPCO_TIMING
    mpco::Timer timer ("initElementRecorders");
    timer.start ();
#endif // MPCO_TIMING
    /*
       clear previous recorders
     ...
     */
    m_data->elemental_recorders.resize (m_data->elemental_results_requests.
                                        size ());
    for (size_t cnt_request = 0;
         cnt_request < m_data->elemental_results_requests.size ();
         cnt_request++)
      {
          const std::vector < std::string > &request =
              m_data->elemental_results_requests[cnt_request];
          /*
          ...
          for (size_t i = 0; i < request.size (); i++)
            {
                ...
                if (i == 0)
                  {             // note: all sections or all materials available only as first options
                      if (current_request == "section")
                        {
                            request_mod.push_back (""); // placeholder for section id
                            section_id_placeholder_index =
                                request_mod.size () - 1;
		...
		*/
           ...
           */
          mpco::element::ResultRecorder & recorder =
              m_data->elemental_recorders[cnt_request];
          ...
          for (mpco::element::ElementCollection::submap_type::iterator it1 =
            ...
            {
                mpco::element::
                    ElementWithSameClassTagCollection & elem_by_tag =
                    it1->second;
                mpco::element::OutputWithSameClassTagCollection & eo_by_tag =
                    recorder.response_map[it1->first];
                        /**
			$WO:SHELL_SEC_KEYWORD
			...
			*/
                ...
                if (do_all_sections
                    && utils::shell::isShellElementTag (it1->first))
                  {
                ...
                /*
                   loop over all element group-by-integration rule
                 */
                for (mpco::element::ElementWithSameClassTagCollection::
                     submap_type::iterator it2 = elem_by_tag.items.begin ();
                     it2 != elem_by_tag.items.end (); ++it2)
                  {
                      mpco::element::
                          ElementWithSameIntRuleCollection & elem_by_rule =
                          it2->second;
                      mpco::element::
                          OutputWithSameIntRuleCollection & eo_by_rule =
                          eo_by_tag.items[it2->first];
                      /*
                         loop over all element group-by-custom(or default) integration rule
                       */
                      for (mpco::element::ElementWithSameIntRuleCollection::
                           submap_type::iterator it3 =
                           elem_by_rule.items.begin ();
                           it3 != elem_by_rule.items.end (); ++it3)
                        {
                            mpco::element::
                                ElementWithSameCustomIntRuleCollection &
                                elem_by_custom_rule = it3->second;
                            mpco::element::
                                OutputWithSameCustomIntRuleCollection &
                                eo_by_custom_rule =
                                eo_by_rule.items[it3->first];
                                  ...
                                  if (do_all_sections)
                                    {
                                        ...
                                        std::map < int,
                                            ...
                                            int >>>::
                                            const_iterator
                                            it_elem_ngauss_nfiber =
                                            m_data->elem_ngauss_nfiber_info.
                                            find (elem->getTag ());
                                        if (it_elem_ngauss_nfiber !=
                                            m_data->elem_ngauss_nfiber_info.
                                            end ())
                                          {
                                              CompositeResponse
                                              ...
                                              for (size_t section_id = 0;
                                                   ...
                                                   section_id++)
                                                {
                                                    ...
                                                    std::
                                                        stringstream
                                                        ss_section_id;
                                                    ss_section_id << section_id + 1;    /* note: in setResponse gp number is 1-based */
                                                    std::string s_section_id =
                                                        ss_section_id.str ();
                                                    ...
                                                    if (do_all_fibers)
                                                      {
                                                          ...
                                                          if (num_fib_responses == 0)
                                                            {   // no valid fiber responses found
                                              ...
                                              if (num_sec_responses == 0)
                                                {       // no valid section responses found
                                  ...
                                  else if (do_all_materials)
                                    {
                                         ...
                                         */
                                        CompositeResponse *mat_comp_response =
                                            new CompositeResponse ();
                                        ...
                                        while (true)
                                          {
                                              ...
                                              std::
                                                  stringstream ss_material_id;
                                              ss_material_id << material_id;
                                              std::string s_material_id =
                                                  ss_material_id.str ();
                                              argv[material_id_placeholder_index] = s_material_id.c_str ();
                                              // set response
                                              Response *mat_response =
                                                  elem->setResponse (argv,
                                                                     argc,
                                                                     eo_stream);
                                              if (mat_response)
                                                {
                                        ...
                                        if (num_mat_responses == 0)
                                          {     // no valid material responses found
                                  ...
                                  if (eo_response)
                                    {
                                        ...
                                        mpco::element::
                                            ...
                                            header (eo_descriptor.
                                                    makeHeader ());
                                        header.
                                            workaroundForSizeInconsistency
                                            (eo_response->getInformation ().
                                             getData ().Size ());
                                        header.
                                            workaroundForDuplicatedComponents
                                            ();
                                        mpco::element::
                                            OutputResponseCollection &
                                            eo_by_header =
                                            eo_by_custom_rule.items[header];
                                         ...
                                         */
                                        m_data->elemental_responses.
                                            push_back (eo_response);
                                    }
                              }
                        }
                  }
...
int
MPCORecorder::clearElementRecorders ()
{
    /*
...
{
#ifdef MPCO_TIMING
    mpco::Timer timer ("recordResultsOnNodes");
    timer.start ();
#endif // MPCO_TIMING
    /*
       error flags
    ...
    if (num_eigen > 0)
      {
          ...
          if (m_data->nodal_recorders.
              find (mpco::NodalResultType::ModesOfVibration) !=
              m_data->nodal_recorders.end ())
              eigen_requested = true;
          else if (m_data->nodal_recorders.
                   find (mpco::NodalResultType::ModesOfVibrationRotational) !=
                   m_data->nodal_recorders.end ())
              eigen_requested = true;
          if (eigen_requested)
            {
                 ...
                 */
                double eigen_set_time =
                    m_data->info.domain->getTimeEigenvaluesSet ();
                const Vector & new_eigenvalues =
                    m_data->info.domain->getEigenvalues ();
                if (m_data->info.eigen_first_initialization_done)
                  {
                      ...
                      if (std::
                          abs (eigen_set_time -
                               m_data->info.eigen_last_time_set) > 1.0e-10)
                        {
                            m_data->info.record_eigen_on_this_step = true;
                        }
                      else if (num_eigen !=
                               m_data->info.eigen_last_values.Size ())
                        {
                            m_data->info.record_eigen_on_this_step = true;
                        }
                      else
                        {
                            ...
                            for (int i = 0; i < num_eigen; i++)
                              {
                                  if (std::
                                      abs (new_eigenvalues[i] -
                                           m_data->info.
                                           eigen_last_values[i]) > 1.0e-10)
                                    {
                                        m_data->info.
                                            record_eigen_on_this_step = true;
                                        break;
                                    }
                              }
                        }
                ...
                if (curr_reac_type != previous_reac_type)
                  {
                      if (curr_reac_type > -1 && curr_reac_type < 3)
                        {
                            m_data->info.domain->
                                calculateNodalReactions (curr_reac_type);
                        }
...
int
MPCORecorder::recordResultsOnElements ()
{
#ifdef MPCO_TIMING
    mpco::Timer timer ("recordResultsOnElements");
    timer.start ();
#endif // MPCO_TIMING
    /*
       error flags
     ...
     */
    for (mpco::element::ResultRecorderCollection::iterator it_recorder =
         m_data->elemental_recorders.begin ();
         it_recorder != m_data->elemental_recorders.end (); ++it_recorder)
      {
          ...
          {
              std::stringstream ss_buffer;
              ss_buffer << "MODEL_STAGE[" << m_data->info.
                  current_model_stage_id << "]/RESULTS/ON_ELEMENTS/" <<
                  result_display_name;
          ...
          if (!recorder.initialized)
            {
                 ...
                 */
                hid_t h_gp_result =
                    h5::group::createResultGroup (m_data->info.h_file_id,
                                                  m_data->info.
                                                  h_group_proplist,
                                                  result_name,
                                                  result_display_name, "", 0,
                                                  "", "",
                                                  (int) mpco::ResultType::
                                                  Generic,
                                                  (int) mpco::ResultDataType::
                                                  Scalar);
                status = h5::group::close (h_gp_result);
           ...
           */
          for (mpco::element::ResultRecorder::collection_type::iterator it1 =
               recorder.response_map.begin ();
               it1 != recorder.response_map.end (); ++it1)
            {
                 ...
                 */
                for (mpco::element::OutputWithSameClassTagCollection::
                     collection_type::iterator it2 = eo_by_tag.items.begin ();
                     it2 != eo_by_tag.items.end (); ++it2)
                  {
                       ...
                       */
                      for (mpco::element::OutputWithSameIntRuleCollection::
                           collection_type::iterator it3 =
                           eo_by_rule.items.begin ();
                           it3 != eo_by_rule.items.end (); ++it3)
                        {
                            mpco::element::
                                OutputWithSameCustomIntRuleCollection &
                                eo_by_custom_rule = it3->second;
                            /*
                            ...
                            for (mpco::element::
                                 ...
                                 it4 != eo_by_custom_rule.items.end (); ++it4)
                              {
                                  const mpco::element::
                                      ...
                                      it4->first;
                                  mpco::element::
                                      OutputResponseCollection & eo_by_header
                                      = it4->second;
                                  size_t num_rows =
                                      eo_by_header.items.size ();
                                  /*
                                  ...
                                  if (!eo_by_header.initialized)
                                    {
                                        ...
                                        hid_t h_gp_header =
                                            h5::group::create (m_data->info.
                                                               ...
                                                               H5P_DEFAULT);
                                        status =
                                            h5::attribute::write (h_gp_header,
                                                                  "NUM_COLUMNS",
                                                                  header.
                                                                  num_columns);
                                        /*
                                        ...
                                        hid_t h_dset_ncomp =
                                            ...
                                            createAndWrite (h_gp_meta,
                                                            ...
                                                            size (), 1);
                                        status =
                                            h5::dataset::close (h_dset_ncomp);
                                        std::
                                            stringstream ss_buffer_components;
                                        for (size_t i = 0;
                                             i <
                                             header.components_path.size ();
                                             i++)
                                          {
                                              if (i > 0)
                                                  ss_buffer_components << ";";
                                              const std::vector <
                                                  int >&ipath =
                                                  header.components_path[i];
                                              const std::vector <
                                                  std::string > &icmps =
                                                  header.components[i];
                                              for (size_t j = 0;
                                        ...
                                        std::string buffer_components =
                                            ss_buffer_components.str ();
                                        hid_t h_dset_comp =
                                            h5::dataset::
                                            createAndWrite (h_gp_meta,
                                                            "COMPONENTS",
                                                            buffer_components);
                                        status =
                                            h5::dataset::close (h_dset_comp);
                                        status = h5::group::close (h_gp_meta);
                                        /*
                                           create the id dataset
                                         */
                                        std::vector <
                                            int >buffer_id (num_rows);
                                        for (size_t i = 0; i < num_rows; i++)
                                            buffer_id[i] =
                                                eo_by_header.items[i].
                                                element->getTag ();
                                        hid_t h_dset_id =
                                            h5::dataset::
                                            createAndWrite (h_gp_header, "ID",
                                                            buffer_id,
                                                            num_rows, 1);
                                        /*
                                           create the data group
                                   ...
                                   */
                                  std::stringstream ss_dset_name;
                                  ss_dset_name << header_dir_name <<
                                      "/DATA/STEP_" << m_data->info.
                                      current_time_step_id;
                                  std::string dset_name = ss_dset_name.str ();
                                  std::vector <
                                      double >buffer_data (num_rows *
                                                           header.
                                                           num_columns);
                                  for (size_t i = 0; i < num_rows; i++)
                                    {
                                        mpco::element::
                                            OutputResponse & current_response
                                            = eo_by_header.items[i];
                                        current_response.response->
                                            getResponse ();
                                        const Vector & current_data =
                                            current_response.response->
                                        ...
                                        size_t offset =
                                            i * header.num_columns;
                                        for (size_t j = 0;
                                             j < header.num_columns; j++)
                                            buffer_data[offset + j] =
                                                current_data[(int) j];
                                    }
                                  hid_t h_dset_data =
                                      h5::dataset::createAndWrite (m_data->
                                                                   info.
                                                                   h_file_id,
                                                                   dset_name.
                                                                   c_str (),
                                                                   ...
                                                                   num_columns);
                                  status =
                                      h5::attribute::write (h_dset_data,
                                                            "STEP",
                                                            m_data->info.
                                                            current_time_step_id);
                                  status =
...
OPS_MPCORecorder ()
{
                ...
                if (numdata > 0)
                  {
                      ...
                      if (OPS_GetInt (&one_item, &region_tag) != 0)
                        {
                            opserr <<
                                "MPCORecorder error: option -R (region) requires an extra parameter (int) for the region tag. (cannot get int value)\n";
                            return 0;
                        }
                      MeshRegion *region = domain->getRegion (region_tag);
                      if (region == 0)
                        {
                            opserr << "MPCORecorder error: region " <<
                                region_tag << " is null\n";
                            return 0;
                        }
                  ...
                  {
                      opserr <<
                          "MPCORecorder error: option -R (region) requires an extra parameter (int) for the region tag\n";
                      return 0;
                  }
                has_region = true;
            }
          else
            {
                switch (curr_opt)
                  ...
                  case utils::parsing::opt_result_on_nodes:
                      {
                          if (strcmp (data, "displacement") == 0)
                              nodal_results_requests.
                                  push_back (mpco::NodalResultType::
                                             Displacement);
                          else if (strcmp (data, "rotation") == 0)
                              nodal_results_requests.
                                  push_back (mpco::NodalResultType::Rotation);
                          else if (strcmp (data, "velocity") == 0)
                              nodal_results_requests.
                                  push_back (mpco::NodalResultType::Velocity);
                              ...
                              nodal_results_requests.
                                  push_back (mpco::NodalResultType::
                                   ...
                                   (data,
                                    "reactionForceIncludingInertia") == 0)
                              nodal_results_requests.
                                  push_back (mpco::NodalResultType::
                                             ReactionForceIncludingInertia);
                          else if (strcmp
                                   (data,
                                    "reactionMomentIncludingInertia") == 0)
                              nodal_results_requests.
                                  push_back (mpco::NodalResultType::
                                             ReactionMomentIncludingInertia);
                              ...
                              nodal_results_requests.
                                  push_back (mpco::NodalResultType::Pressure);
                          else if (strcmp (data, "modesOfVibration") == 0)
                              nodal_results_requests.
                                  push_back (mpco::NodalResultType::
                                             ModesOfVibration);
                          else if (strcmp (data, "modesOfVibrationRotational")
                                   == 0)
                              nodal_results_requests.
                                  push_back (mpco::NodalResultType::
                                             ModesOfVibrationRotational);
                            ...
                            {
                                opserr <<
                                    "MPCORecorder error: option -N with unknown result type ("
                                    << data << ")\n";
                                return 0;
                            }
                  ...
                  case utils::parsing::opt_result_on_nodes_sens:
                      {
                          if (strcmp (data, "displacementSensitivity") == 0)
                              ...
                              nodal_results_requests.
                                  push_back (mpco::NodalResultType::
                                             VelocitySensitivity);
                          else if (strcmp (data, "angularVelocitySensitivity")
                                   == 0)
                              nodal_results_requests.
                                  push_back (mpco::NodalResultType::
                                             AngularVelocitySensitivity);
                          else if (strcmp (data, "accelerationSensitivity") ==
                                   0)
                              nodal_results_requests.
                                  push_back (mpco::NodalResultType::
                                             AccelerationSensitivity);
                                   ...
                                   (data,
                                    "angularAccelerationSensitivity") == 0)
                              nodal_results_requests.
                                  push_back (mpco::NodalResultType::
                                             AngularAccelerationSensitivity);
                          else
                          ...
                          if (numdata > 0)
                            {
                                ...
                                if (OPS_GetInt (&one_item, &grad_index) != 0)
                                  {
                                      opserr <<
                                          "MPCORecorder error: option -NS requires an extra parameter (int) for the sensitivity parameter index. (cannot get int value)\n";
                                      return 0;
                                  }
                                numdata--;
                            ...
                            {
                                opserr <<
                                    "MPCORecorder error: option -NS requires an extra argument for the sensitivity parameter index\n";
                                return 0;
                            }
                          break;
                      }
                          ...
                          if (strcmp (data, "dt") == 0)
                            {
                                ...
                                if (numdata > 0)
                                  {
                                      if (OPS_GetDouble
                                          (&one_item, &output_freq.dt) != 0)
                                        {
                                            opserr <<
                                                "MPCORecorder error: invalid double argument for the delta time\n";
                                            return 0;
                                        }
                                      if (output_freq.dt < 0.0)
                                          output_freq.dt = 0.0;
                                      numdata--;
                                  }
                                else
                                  {
                                      opserr <<
                                          "MPCORecorder error: option -T with type dt requires an extra argument for the delta time\n";
                                      return 0;
                                  }
                            }
                                ...
                                if (numdata > 0)
                                  {
                                      if (OPS_GetInt
                                          (&one_item,
                                           &output_freq.nsteps) != 0)
                                        {
                                            opserr <<
                                                "MPCORecorder error: invalid int argument for the number of steps\n";
                                            return 0;
                                        }
                                      if (output_freq.nsteps < 1)
                                          output_freq.nsteps = 1;       // make sure it's positive
                                      numdata--;
                                  }
                                else
                                  {
                            ...
                            {
                                opserr <<
                                    "MPCORecorder error: option -T with unknown frquency type ("
                                    << data << ")\n";
                                return 0;
                            }
                          break;
                  ...
                  default:
                      {
                          opserr <<
                              "MPCORecorder error: unknown arg with option none "
                              << data << "\n";
                          return 0;
                      }
                  }
    ...
    new_recorder->m_data->nodal_results_requests.
        swap (nodal_results_requests);
    new_recorder->m_data->sens_grad_indices.swap (sens_grad_indices);
    new_recorder->m_data->elemental_results_requests.
        swap (elemental_results_requests);
    new_recorder->m_data->has_region = has_region;
    for (std::set < int >::const_iterator it = node_set.begin ();
         it != node_set.end (); ++it)
        new_recorder->m_data->node_set.push_back (*it);
    for (std::set < int >::const_iterator it = elem_set.begin ();
         it != elem_set.end (); ++it)
