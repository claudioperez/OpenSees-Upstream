...
ElementRecorder::ElementRecorder ():Recorder (RECORDER_TAGS_ElementRecorder),
...
ElementRecorder::sendSelf (int commitTag, Channel & theChannel)
{
    ...
    if (theChannel.isDatastore () == 1)
      {
          opserr <<
              "ElementRecorder::sendSelf() - does not send data to a datastore\n";
          return -1;
      }
    ...
    if (eleID != 0)
        if (theChannel.sendID (0, commitTag, *eleID) < 0)
          {
              opserr <<
                  "ElementRecorder::sendSelf() - failed to send eleID\n";
              return -1;
          }
    ...
    if (theOutputHandler == 0
      ...
      {
          opserr <<
              "ElementRecorder::sendSelf() - failed to send the DataOutputHandler\n";
          return -1;
      }
                           ...
                           FEM_ObjectBroker & theBroker)
{
    ...
    if (eleSize != 0)
      {
          ...
          if (eleID == 0)
            {
                opserr <<
                    "ElementRecorder::recvSelf() - failed to create eleID\n";
                return -1;
            }
          if (theChannel.recvID (0, commitTag, *eleID) < 0)
            {
                opserr <<
                    "ElementRecorder::recvSelf() - failed to recv eleOD\n";
                return -1;
            }
      }
    ...
    if (msgLength == 0)
      {
          opserr <<
              "ElementRecorder::recvSelf() - 0 sized string for responses\n";
          return -1;
      }
