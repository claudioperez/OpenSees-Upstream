...
data2b (0)
{
...
#ifdef _WGL
    theRenderer =
        new OpenGLRenderer (windowTitle, xLoc, yLoc, width, height, *theMap);
#elif _GLX
    theRenderer =
        new OpenGLRenderer (windowTitle, xLoc, yLoc, width, height, *theMap);
#elif _AGL
...
#else
    theRenderer =
        new X11Renderer (windowTitle, xLoc, yLoc, width, height, *theMap);
#endif
    ...
    if (fileName1 == 0)
      {
          opserr <<
              "FilePlotter::FilePlotter -- out of memory copying fileName1 "
              << endln;
      }
...
FilePlotter::plotFile (void)
{
          ...
          if (!isspace (c))
            {
                ...
                for (int i = 0; i < cols->Size (); i++)
                  {
                      if (i % 2 == 0)
                        {       // an xValue if numLineEntries == colX
                        ...
                        {       // a y value if (numLineEntries == colY) {
                            if (numLineEntries == (*cols) (i))
                              {
                                  if (xValue < yMin)
                                      yMin = xValue;
    ...
    if (data1a == 0 || data1b == 0 ||
        data1a->Size () != numLineEntries
        || data1b->Size () != numLineEntries)
      {
          if (data1a != 0)
              delete data1a;
          if (data1b != 0)
    ...
    if (numLines > 0)
      {
          if (cols == 0)
            {
                opserr <<
                    "FilePLotter::plotFile() - no valid columns have been set\n";
          ...
          else
            {
                ...
                theRenderer->setViewWindow (xMin - xBnd, xMax + xBnd,
                                            yMin - yBnd, yMax + yBnd);
            }
      }
          ...
          if (xMin != 0.0 && -100 * xMin > xMax)
            {
                ...
                theRenderer->drawText (pt1, theText, strlen (theText), 'l',
                                       'b');
            }
          if (xMax != 0.0)
            {
                sprintf (theText, "%.2e", xMax);
                theRenderer->drawText (pt2, theText, strlen (theText), 'r',
                                       'b');
            }
          ...
          if (yMin != 0.0 && -100 * yMin > yMax)
            {
                ...
                theRenderer->drawText (pt1, theText, strlen (theText), 'c',
                                       't');
            }
          if (yMax != 0.0)
            {
                sprintf (theText, "%.2e", yMax);
                theRenderer->drawText (pt2, theText, strlen (theText), 'c',
                                       'b');
            }
...
FilePlotter::plotFiles (void)
{
    ...
    if (theXfile.bad () || theYfile.bad ())
      {
          opserr << "WARNING - FilePlotter::FilePlotter()";
          opserr << " - could not open files " << fileName1 << " " <<
              fileName2 << endln;
          return -1;
      }
    ...
    if (data1a == 0 || data1b == 0 ||
        data1a->Size () != numLineEntriesX
        || data1b->Size () != numLineEntriesX || data2a == 0 || data2b == 0
        || data2a->Size () != numLineEntriesY
        || data2b->Size () != numLineEntriesY)
      {
    ...
    if (numLinesX != numLinesY)
      {
          opserr <<
              "FilePLotter::plotFile() - two files have different line counts\n";
          return -1;
      }
...
FilePlotter::setCol (const ID & theCols)
{
    if (theCols.Size () % 2 != 0)
      {
          opserr << "FilePlotter::setCol() - the size of the cols ID " <<
              theCols.Size () << " is not a multiple of 2\n";
          return -1;
      }
    ...
    for (int i = 0; i < theCols.Size (); i++)
      {
          if (theCols (i) < 1)
            {
                opserr << "FilePlotter::FilePlotter() - a value of the cols "
                    << theCols (i) << " is < 1\n";
                return -2;
            }
      }
