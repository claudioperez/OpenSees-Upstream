...
NodeRecorderRMS::NodeRecorderRMS ():Recorder (RECORDER_TAGS_NodeRecorderRMS),
...
NodeRecorderRMS::NodeRecorderRMS (const ID & dofs,
                                  ...
                                  double dT, TimeSeries ** theSeries):
...
timeSeriesValues (0)
{
    ...
    for (i = 0; i < numDOF; i++)
      {
          ...
          if (dof >= 0)
            ...
            {
                opserr << "NodeRecorderRMS::NodeRecorderRMS - invalid dof  "
                    << dof;
    ...
    if (dataToStore == 0 || (strcmp (dataToStore, "disp") == 0))
      ...
      {
          dataFlag = 10;
          opserr << "NodeRecorderRMS::NodeRecorder - dataToStore " <<
              dataToStore;
          opserr <<
...
NodeRecorderRMS::record (int commitTag, double timeStamp)
{
    ...
    if (theHandler == 0)
      {
          opserr <<
              "NodeRecorderRMS::record() - no DataOutputHandler has been set\n";
          return -1;
      }
    ...
    if (initializationDone != true)
        if (this->initialize () != 0)
          {
              opserr <<
                  "NodeRecorderRMS::record() - failed in initialize()\n";
              return -1;
          }
...
NodeRecorderRMS::sendSelf (int commitTag, Channel & theChannel)
{
    ...
    if (theChannel.isDatastore () == 1)
      {
          opserr <<
              "NodeRecorderRMS::sendSelf() - does not send data to a datastore\n";
          return -1;
      }
    ...
    if (theDofs != 0)
        if (theChannel.sendID (0, commitTag, *theDofs) < 0)
          {
              opserr <<
                  "NodeRecorderRMS::sendSelf() - failed to send dof id's\n";
              return -1;
          }
    ...
    if (theNodalTags != 0)
        if (theChannel.sendID (0, commitTag, *theNodalTags) < 0)
          {
              opserr <<
                  "NodeRecorderRMS::sendSelf() - failed to send nodal tags\n";
              return -1;
          }
    ...
    if (theHandler->sendSelf (commitTag, theChannel) < 0)
      {
          opserr <<
              "NodeRecorderRMS::sendSelf() - failed to send the DataOutputHandler\n";
          return -1;
      }
    ...
    if (theTimeSeries != 0)
      {
          ...
          if (theChannel.sendID (0, commitTag, timeSeriesTags) < 0)
            {
                opserr <<
                    "NodeRecorderRMS::sendSelf() - failed to send time series tags\n";
                return -1;
            }
          for (int i = 0; i < numDOF; i++)
            {
                if (theTimeSeries[i] != 0)
                  {
                      if (theTimeSeries[i]->sendSelf (commitTag, theChannel) <
                        ...
                        {
                            opserr <<
                                "NodeRecorderRMS::sendSelf() - time series failed in send\n";
                            return -1;
                           ...
                           FEM_ObjectBroker & theBroker)
{
          ...
          if (numDOFs != 0)
            {
                ...
                if (theDofs == 0 || theDofs->Size () != numDOFs)
                  {
                      opserr <<
                          "NodeRecorderRMS::recvSelf() - out of memory\n";
                      return -1;
                  }
            }
      }
    if (theDofs != 0)
        if (theChannel.recvID (0, commitTag, *theDofs) < 0)
          {
              opserr <<
                  "NodeRecorderRMS::recvSelf() - failed to recv dof data\n";
              return -1;
          }
    ...
    if (theHandler == 0)
      {
          opserr <<
              "NodeRecorderRMS::sendSelf() - failed to get a data output handler\n";
          return -1;
      }
    ...
    if (idData[6] == 1)
      {
          ...
          if (theChannel.recvID (0, commitTag, timeSeriesTags) < 0)
            {
                opserr <<
                    "NodeRecorderRMS::recvSelf() - failed to recv time series tags\n";
                return -1;
            }
                ...
                if (timeSeriesTags[i] == -1)
                ...
                else
                  {
                      ...
                      if (theTimeSeries[i]->
                        ...
                        {
                            opserr <<
                                "NodeRecorderRMS::recvSelf() - time series failed in recv\n";
                            return -1;
                        }
                  }
            }
...
NodeRecorderRMS::initialize (void)
{
    if (theDofs == 0 || theDomain == 0)
      {
          opserr <<
              "NodeRecorderRMS::initialize() - either nodes, dofs or domain has not been set\n";
          return -1;
      }
...
//added by SAJalali
double
NodeRecorderRMS::getRecordedValue (int clmnId, int rowOffset, bool reset)
{
    double res = 0;
    ...
    if (count != 0)
        res =
            sqrt (((*runningTotal) (clmnId) * (*runningTotal) (clmnId)) /
                  count);
    if (reset)
        count = 0;
    return res;
