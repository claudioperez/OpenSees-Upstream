...
OPS_RemoveRecorder ()
{
          ...
          if (strcmp (opt, "-node") == 0)
            {
                ...
                Node *
                    theNode = domain->getNode (nodeTag);
                if (theNode == 0)
                  {
                      opserr
                          <<
          ...
          else if (strcmp (opt, "-global_gravaxis") == 0)
            {
                ...
                if (OPS_GetIntInput (&num, &globgrav) < 0)
                  {
                      opserr <<
                          "WARNING recorder Collapse -global_gravaxis - "
                   ...
                   (strcmp (opt, "-element") == 0))
            {
                ...
                if (OPS_GetNumRemainingInputArgs () < 1)
                  {
                      opserr <<
                          "WARNING recorder Collapse .. -ele tag1? .. - "
          ...
          else if (strcmp (opt, "-eleRange") == 0)
            {
                ...
                if (OPS_GetNumRemainingInputArgs () < 2)
                  {
                      opserr <<
                          "WARNING recorder Element .. -eleRange start? "
                ...
                if (OPS_GetIntInput (&num, &startend[0]) < 0)
                  {
                      opserr <<
                          "WARNING recorder Element -eleRange start? end? "
          ...
          else if (strcmp (opt, "-region") == 0)
            {
                ...
                if (OPS_GetNumRemainingInputArgs () < 1)
                  {
                      opserr <<
                          "WARNING recorder Element .. -region tag?  .. - "
                          "no region specified\n";
                      return 0;
                  }
                int
                ...
                MeshRegion *
                    theRegion = domain->getRegion (tag);
                if (theRegion == 0)
                  {
                      opserr << "WARNING recorder Element -region " << tag
                ...
                const
                    ID &
                    eleRegion = theRegion->getElements ();
                ...
                if (critTag != 7)
                  {
                      ...
                      if (OPS_GetDoubleInput (&num, &critValue) < 0)
                        {
                            opserr <<
                                "WARNING recorder Remove -crit critTag? "
    ...
    return new RemoveRecorder (nodeTag, eleIDs, secIDs, secondaryEleIDs,
                               remCriteria, *domain, *theOutputStream,
                               echoTime, dT, fileName, eleMass, gAcc, gDir,
                               gPat, nTagbotn, nTagmidn, nTagtopn, globgrav,
                               fileNameinf);
}
#endif
...
int
    RemoveRecorder::numRecs = 0;
ID
RemoveRecorder::remEleList (0);
ID
RemoveRecorder::remNodeList (0);
int
    RemoveRecorder::numRemEles = 0;
int
    RemoveRecorder::numRemNodes = 0;

Element **
    RemoveRecorder::remEles = 0;
Node **
    RemoveRecorder::remNodes = 0;
...
RemoveRecorder::RemoveRecorder (int nodeID,
                                ...
                                int nTagmidn,
                                int nTagtopn,
                                int globgrav, const char *thefileNameinf):
...
#ifdef MMTDEBUG
                opserr << "storing secondaryEleID = " << secondaryTags[l] <<
                    endln;
#endif
            }
      }
    ...
    for (int j = 0; j < numEles; j++)
      {
          ...
          if (theEle == NULL)
            {
                opserr <<
                    "WARNING RemoveRecorder::RemoveRecorder() - no element with tag: "
          ...
          else
            {
                ...
                if (eleResponses[j] == 0)
                  {
                      opserr <<
                          "WARNING :: getRemCriteria - not a response quantity of element\n";
                  }
                else
                  {
                      if (fileNameinf != 0)
    ...
    if (nodeTag != 0)
      {
          ...
          if (theNode == NULL)
            {
                opserr <<
                    "WARNING RemoveRecorder::RemoveRecorder() - no node with tag: "
    ...
    if (theFileName != 0 && fileName == 0)
      {
          ...
          if (fileName == 0)
            {
                opserr <<
                    "RemoveRecorder::RemoveRecorder - out of memory creating string "
                    << fileNameLength << " long\n";
                exit (-1);
            }
...
#ifdef MMTDEBUGIO
          opserr << "filename " << fileName << " len " << strlen (fileName) <<
              endln;
#endif
          //  delete [] temp;
                ...
                if (remFlag == 0)
                  {
                      ...
                      for (int j = 0; j < numEles; j++)
                        {
                            for (int m = 0; m < RemoveRecorder::numRemEles;
                                 m++)
                              {
                                  if (eleTags[j] ==
                                      RemoveRecorder::remEleList[m])
                                    {
                                        numConEles--;
#ifdef MMTDEBUG
                                        opserr << " element " << eleTags[j] <<
                                            " already removed from node " <<
                                            nodeTag << " " << numConEles <<
                                            " elements left" << endln;
#endif
                                    }
                      ...
                      if (numConEles == 0)
                        {
                            ...
                            //                                    numRemNodes ++;
#ifdef MMTDEBUG
                            opserr << "Node removal # " << numRemNodes <<
                                " of node # " << remNodeList[numRemNodes -
                                                             1] << endln;
                      ...
                      if (remFlag == 0)
                        {
                            ...
                            Element *theEle =
                                theDomain->getElement (eleTags[j]);
                            if (theEle != 0)
                              {
                                  int eleFlag = 0;
                                  ...
                                  for (int i = 0; i < numSecs; i++)
                                    {
#ifdef MMTDEBUG
                                        opserr << " section to be checked " <<
                                            secTags[i] << endln;
#endif
                                        // changed
                                        int secFlag =
                                            this->checkEleRemoval (theEle,
                                                                   eleResponses
                                                                   [j],
                                                                   secTags[i],
                                                                   criteria);
...
#ifdef MMTDEBUG
                                  opserr <<
                                      " finished checking element, flag = " <<
                                      eleFlag << endln;
#endif
                                  if (eleFlag > 0)
                                    {
#ifdef MMTDEBUG
                                        opserr <<
                                            " about to eliminate element " <<
                                            eleTags[j] << endln;
#endif
                                        this->updateNodalMasses (eleTags[j],
                                                                 eleMasses
                                                                 [j]);
                                        this->elimElem (eleTags[j],
                                                        timeStamp);
#ifdef MMTDEBUG
                                        opserr << " eliminated element " <<
                                            eleTags[j] << endln;
#endif
...
#ifdef MMTDEBUG
                                        opserr << "Element removal # " <<
                                            numRemEles << " of element # " <<
                                            remEleList[numRemEles -
                                                       1] << endln;
#endif
                                        remFlag = 1;
                                    }
...
#ifdef MMTDEBUG
                opserr << "eleCount = " << eleCount << " numEles = " <<
                    numEles << " secondaryFlag = " << int (secondaryFlag) <<
                    endln;
#endif
                if (eleCount == numEles && secondaryFlag == true)
                  {
                      if (this->elimSecondaries (timeStamp) != 0)
                        {
                            opserr <<
                                "Error: Collapse Recorder - failed to remove secondary components to element "
                                << eleTags[0] << endln;
                            return -1;
                        }
                      secondaryFlag = false;
...
RemoveRecorder::restart (void)
{
    ...
    if (!(&theFile))
      {
          opserr <<
              "WARNING - RemoveRecorder::restart() - could not open file ";
...
int
// changed
RemoveRecorder::checkEleRemoval (Element * theEle, Response * eleResponse,
                                 int &theComponent, const Vector & criteria)
{
    if (eleResponse == 0)
        return 0;
    ...
    for (int k = 0; k < numRules; k++)
      {
          ...
          switch (int (criteria[2 * k]))
            ...
            default:
                ...
                opserr <<
                    "RemoveRecorder::checkRemoval() - Failed to identify removal criterion "
                    << criteria[2 * k] << endln;
                return -1;
                break;
...
#ifdef MMTDEBUG
    opserr << "RemoveRecorder::elimElem() remving ele: " << theEleTag <<
        " at timeStamp: " << timeStamp << endln;;
#endif
    ...
    if (theEle != 0)
      {
          ...
          while ((thePattern = theLoadPatterns ()) != 0)
            {
                ...
                ElementalLoadIter theEleLoads =
                    thePattern->getElementalLoads ();
                ...
                while ((theLoad = theEleLoads ()) != 0)
                  {
                      ...
                      if (loadEleTag == theEleTag)
                        {
                            opserr <<
                                "RemoveRecorder::elimElem() -3 removing  eleLoad\n";

                            ElementalLoad *theElementalLoad =
                                thePattern->removeElementalLoad (theLoad->
                                                                 getTag ());
          ...
          RemoveRecorder::remEleList[RemoveRecorder::numRemEles] =
              theEle->getTag ();
...
#ifdef MMTDEBUG
              opserr << " element " << theEle->
                  getTag () << " removed automatically" << endln;
#endif
          ;
      }
    return 0;
                ...
                if (theNode == theNodeTag)
                  {
                      ...
                      //                              cnt++;
#ifdef MMTDEBUG
                      opserr << "identified load pattern " << theLoad->
                          getTag () << " acting on node " << theNode << endln;
#endif
                      ...
                      if (theNodalLoad != 0)
                        {
#ifdef MMTDEBUG
                            opserr << "deleting nodal load pattern " <<
                                theLoad->getTag () << endln;
#endif
                            delete theNodalLoad;
                        }
                  }
...
//                              cnt++;
#ifdef MMTDEBUG
                      opserr << "identified SP_Constraint " << theSP->
                          getTag () << " acting on node " << spNode << endln;
#endif
                      ...
                      if (theSPConstraint != 0)
                        {
#ifdef MMTDEBUG
                            opserr << "deleting SP_Constraint " << theSP->
                                getTag () << endln;
#endif
...
#endif
                SP_Constraint *theSPConstraint =
                    theDomain->removeSP_Constraint (theSP->getTag ());
                if (theSPConstraint != 0)
                  {
#ifdef MMTDEBUG
                      opserr << "deleting SP_Constraint " << theSP->
    ...
    if (echoTimeFlag == true)
        opserr << "Node " << theNode->
            getTag () << " removed, Time/Load Factor = " << timeStamp <<
            endln;
                      ...
                      for (int m = 0; m < RemoveRecorder::numRemNodes; m++)
                        {
                            ...
                            if (secondaryNodes[k] ==
                                RemoveRecorder::remNodeList[m])
                              {
                                  nodeRemFlag = 1;
#ifdef MMTDEBUG
                                  opserr << " node " << secondaryNodes[k] <<
                                      " already removed " << endln;
#endif
                              }
                        }
...
#ifdef MMTDEBUG
                            opserr << " eliminated node " << secondaryNodes[k]
                                << endln;
                            opserr << "Node removal # " << numRemNodes <<
                                " of node # " << remNodeList[numRemNodes -
                                                             1] << endln;
...
#ifdef MMTDEBUG
                opserr << " eliminated element " << secondaryEleTags[i] <<
                    endln;
                opserr << "Element removal # " << numRemEles <<
                    " of element # " << remEleList[numRemEles - 1] << endln;
#endif
            }
          ...
          for (int k = 0; k < theEle->getNumExternalNodes (); k++)
            {
                ...
                if (theDomain->setMass (theNodalMass, theNode->getTag ()) !=
                    0)
                  {
                      opserr <<
                          "Remove Recorder::WARNING failed to set mass at node "
                          << theNode->getTag () << endln;
                  }
                ...
                if (this->gAcc != 0)
                  {
                      ...
                      opserr << "eleWeight " << theEleWeight <<
                          " NodalForces " << theNodalForces (0) << " " <<
                          theNodalForces.Size () << endln;
                      int nodeLoadTag = 987654 + theEleTag * 30 + k;    // big number + many element nodes + current node
                      NodalLoad *theNodalLoad =
                          new NodalLoad (nodeLoadTag, theNode->getTag (),
                                         theNodalForces, isLoadConst);
                      if (theNodalLoad == 0)
                        {
                            opserr <<
                                "RemoveRecorder::WARNING ran out of memory while updating loads on node  "
                                << theNode->getTag ();
                        }
                      if (theDomain->addNodalLoad (theNodalLoad, gPat) ==
                          false)
                        {
                            opserr <<
                                "RemoveRecorder::WARNING could not add updated nodal load to domain\n";
                            delete theNodalLoad;
                        }
                  }
            }
