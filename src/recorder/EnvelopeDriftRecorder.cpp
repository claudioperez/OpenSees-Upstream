...
EnvelopeDriftRecorder::EnvelopeDriftRecorder ():Recorder (RECORDER_TAGS_EnvelopeDriftRecorder),
...
currentData (0),
theDomain (&theDom),
theOutputHandler (&theCurrentDataOutputHandler),
...
currentData (0),
theDomain (&theDom),
theOutputHandler (&theDataOutputHandler),
initializationDone (false),
numNodes (0),
echoTimeFlag (timeFlag)
{
...
EnvelopeDriftRecorder::record (int commitTag, double timeStamp)
{
    ...
    if (theOutputHandler == 0)
      {
          opserr <<
              "EnvelopeDriftRecorder::record() - no DataOutputHandler has been set\n";
          return -1;
      }
    ...
    if (initializationDone != true)
        if (this->initialize () != 0)
          {
              opserr <<
                  "EnvelopeDriftRecorder::record() - failed in initialize()\n";
              return -1;
          }
...
EnvelopeDriftRecorder::sendSelf (int commitTag, Channel & theChannel)
{
    ...
    if (theChannel.sendID (0, commitTag, idData) < 0)
      {
          opserr <<
              "EnvelopeDriftRecorder::sendSelf() - failed to send idData\n";
          return -1;
      }
    ...
    if (ndI != 0)
        if (theChannel.sendID (0, commitTag, *ndI) < 0)
          {
              opserr <<
                  "EnvelopeDriftRecorder::sendSelf() - failed to send dof id's\n";
              return -1;
          }
    ...
    if (theOutputHandler->sendSelf (commitTag, theChannel) < 0)
      {
          opserr <<
              "EnvelopeDriftRecorder::sendSelf() - failed to send the DataOutputHandler\n";
          return -1;
      }
                                 ...
                                 FEM_ObjectBroker & theBroker)
{
    ...
    if (idData (0) != 0)
      {
          ...
          if (ndI == 0)
            {
                opserr <<
                    "EnvelopeDriftRecorder::sendSelf() - out of memory\n";
                return -1;
            }
          if (theChannel.recvID (0, commitTag, *ndI) < 0)
            {
                opserr <<
                    "EnvelopeDriftRecorder::sendSelf() - failed to recv dof id's\n";
                return -1;
            }
      }
    ...
    if (theOutputHandler == 0)
      {
          opserr <<
              "EnvelopeDriftRecorder::sendSelf() - failed to get a data output handler\n";
          return -1;
      }
...
EnvelopeDriftRecorder::initialize (void)
{
    ...
    if (ndI == 0 || ndJ == 0)
      {
          opserr <<
              "EnvelopeDriftRecorder::initialize() - no nodal id's set\n";
          return -1;
      }
    ...
    if (ndIsize != ndJsize)
      {
          opserr <<
              "EnvelopeDriftRecorder::initialize() - error node arrays differ in size\n";
          return -2;
      }
    ...
    if (numNodes == 0)
      {
          opserr <<
              "EnvelopeDriftRecorder::initialize() - no valid nodes or perpendicular direction\n";
          return 0;
      }
                    ...
                    if (crdI (perpDirn) != crdJ (perpDirn))
                      {
                          ...
                          theOutputHandler->attr ("lengthPerpDirn",
                                                  fabs (crdJ (perpDirn) -
                                                        crdI (perpDirn)));
                          ...
                          if (echoTimeFlag == true)
                            {
                                ...
                                theOutputHandler->tag ("ResponseType",
                                                       "time");
...
//added by SAJalali
double
EnvelopeDriftRecorder::getRecordedValue (int clmnId, int rowOffset,
                                         bool reset)
{
    double res = 0;
