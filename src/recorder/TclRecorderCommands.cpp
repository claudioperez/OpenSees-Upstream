...
extern TimeSeries *TclSeriesCommand (ClientData clientData,
                                     Tcl_Interp * interp, TCL_Char * arg);
...
enum outputMode
{ STANDARD_STREAM, DATA_STREAM, XML_STREAM, DATABASE_STREAM, BINARY_STREAM,
        DATA_STREAM_CSV, TCP_STREAM, DATA_STREAM_ADD };
                   ...
                   Recorder ** theRecorder)
{
    ...
    if ((strcmp (argv[1], "Element") == 0)
        ...
        || (strcmp (argv[1], "NormEnvelopeElement") == 0))
      {
                ...
                if ((strcmp (argv[loc], "-ele") == 0) ||
                    ...
                    (strcmp (argv[loc], "-element") == 0))
                  {
                      ...
                      if (argc < loc + 2)
                        {
                            opserr <<
                                "WARNING recorder Element .. -ele tag1? .. - no ele tags specified\n";
                            return TCL_ERROR;
                        }
                      ...
                      if (loc == argc)
                        {
                            opserr <<
                                "ERROR: No response type specified for element recorder. "
                                << endln;
                            delete eleIDs;
                            return TCL_ERROR;
                        }
                ...
                else if (strcmp (argv[loc], "-eleRange") == 0)
                  {
                      ...
                      if (argc < loc + 3)
                        {
                            opserr <<
                                "WARNING recorder Element .. -eleRange start? end?  .. - no ele tags specified\n";
                            return TCL_ERROR;
                        }
                      ...
                      int start, end;
                      if (Tcl_GetInt (interp, argv[loc + 1], &start) !=
                          TCL_OK)
                        {
                            opserr <<
                      ...
                      if (eleIDs == 0)
                        {
                            opserr <<
                                "WARNING recorder Element -eleRange start? end? - out of memory\n";
                            return TCL_ERROR;
                        }
                ...
                else if (strcmp (argv[loc], "-region") == 0)
                  {
                      ...
                      if (argc < loc + 2)
                        {
                            opserr <<
                                "WARNING recorder Element .. -region tag?  .. - no region specified\n";
                            return TCL_ERROR;
                        }
                      int tag;
                      if (Tcl_GetInt (interp, argv[loc + 1], &tag) != TCL_OK)
                        {
                            opserr <<
                                "WARNING recorder Element -region tag? - invalid tag "
                                << argv[loc + 1] << endln;
                            return TCL_ERROR;
                        }
                      ...
                      if (theRegion == 0)
                        {
                            opserr << "WARNING recorder Element -region " <<
                                tag << " - region does not exist" << endln;
                            return TCL_OK;
                        }
                  ...
                  {
                      loc++;
                      if (Tcl_GetInt (interp, argv[loc], &precision) !=
                          TCL_OK)
                          return TCL_ERROR;
                      loc++;
                  }
                  ...
                  {
                      loc++;
                      if (Tcl_GetInt (interp, argv[loc], &writeBufferSize) !=
                          TCL_OK)
                          return TCL_ERROR;
                      loc++;
                  }
                ...
                else if (strcmp (argv[loc], "-database") == 0)
                  {
                      ...
                      if (theRecorderDatabase != 0)
                        ...
                        {
                            opserr << "WARNING recorder Node .. -database " <<
                                fileName <<
                  ...
                  {
                      inetAddr = argv[loc + 1];
                      if (Tcl_GetInt (interp, argv[loc + 2], &inetPort) !=
                          TCL_OK)
                        {
                            ;
                        }
          ...
          if (eMode == DATA_STREAM && fileName != 0)
            {
                theOutputStream =
                    new DataFileStream (fileName, OVERWRITE, 2, 0,
                                        closeOnWrite, precision,
                                        doScientific);
            }
          else if (eMode == DATA_STREAM_ADD && fileName != 0)
            {
                theOutputStream =
                    new DataFileStreamAdd (fileName, OVERWRITE, 2, 0,
                                           closeOnWrite, precision,
                                           doScientific);
            }
            ...
            {
                theOutputStream =
                    new DataFileStream (fileName, OVERWRITE, 2, 1,
                                        closeOnWrite, precision,
                                        doScientific);
            }
    ...
    ///////////////////////////////By Arash Altoontash////////////////////////////
             ...
             || (strcmp (argv[1], "elementDamage") == 0))
      {
          ...
          if (Tcl_GetInt (interp, argv[loc], &eleID) != TCL_OK)
            {
                opserr <<
                    "WARNING recorder ElementDamage: No element tag specified ";
                return TCL_ERROR;
            }
          loc++;
          ...
          int dofID = 0;
          if (strcmp (argv[loc], "-dof") == 0
              || strcmp (argv[loc], "dof") == 0
              || strcmp (argv[loc], "-DOF") == 0
              || strcmp (argv[loc], "DOF") == 0)
            {
                loc++;
                if (Tcl_GetInt (interp, argv[loc], &dofID) != TCL_OK)
                  {
                      opserr <<
                          "WARNING recorder ElementDamage: No dof tag specified ";
                      return TCL_ERROR;
                  }
                loc++;
            }

          if (strcmp (argv[loc], "-damage") != 0
              && strcmp (argv[loc], "damage") != 0)
            {
                opserr <<
                    "WARNING recorder ElementDamage: No damege tag specified ";
                return TCL_ERROR;
            }
          loc++;
          ...
          if (dmgPTR == NULL)
            {
                opserr <<
                    "WARNING recorder ElementDamage: specified damage model not found\n";
                return TCL_ERROR;
            }
          ...
          (*theRecorder) =
              new DamageRecorder (eleID, secIDs, dofID, dmgPTR, theDomain,
    ...
    //////////////////////End of ElementDamage recorder////////////////////////////
             ...
             || (strcmp (argv[1], "Collapse") == 0))
      {
          ...
          if (argc < 4)
            {
                opserr <<
                    "WARNING recorder Collapse -ele eleID <eleID2? ...>  -node nodeID <-time> <-file fileName?> ? "
                    <<
                    "\n or recorder Collapse -ele eleID1 <eleID2? ...>? <-sec secID1? secID2? ...> -crit crit1? value1?"
                    <<
                    " <-crit crit2? value2?> <-time> <-file fileName?> <-mass mass1? mass2? ...> <-g gAcc gDir? gPat?>?"
                    << endln;
                return TCL_ERROR;
            }
          ...
          while (flags == 0 && loc < argc)
            {
                ...
                if (strcmp (argv[loc], "-node") == 0)
                  {
                      ...
                      if (Tcl_GetInt (interp, argv[loc + 1], &nodeTag) !=
                          TCL_OK)
                        {
                            opserr <<
                                "WARNING recorder Collapse -node - invalid node tag "
                                << argv[loc + 1] << endln;
                            return TCL_ERROR;
                        }
                ...
                // new
                  ...
                  {
                      if (Tcl_GetInt (interp, argv[loc + 1], &nTagbotn) !=
                          TCL_OK)
                        {
                            opserr <<
                                "WARNING recorder Collapse -node - invalid node tag "
                      ...
                      if (Tcl_GetInt (interp, argv[loc + 2], &nTagmidn) !=
                          TCL_OK)
                        {
                            opserr <<
                                "WARNING recorder Collapse -node - invalid node tag "
                      ...
                      if (Tcl_GetInt (interp, argv[loc + 3], &nTagtopn) !=
                          TCL_OK)
                        {
                            opserr <<
                                "WARNING recorder Collapse -node - invalid node tag "
                  ...
                  {
                      if (Tcl_GetInt (interp, argv[loc + 1], &globgrav) !=
                          TCL_OK)
                        {
                            opserr <<
                                "WARNING recorder Collapse -global_gravaxis - invalid global axis for gravity "
                                << argv[loc + 1] << endln;
                            return TCL_ERROR;
                        }
                      loc += 2;
                ...
                //    end of new
                         ...
                         (strcmp (argv[loc], "-element") == 0))
                  {
                      ...
                      if (argc < loc + 2)
                        {
                            opserr <<
                                "WARNING recorder Collapse .. -ele tag1? .. - no ele tags specified\n";
                            return TCL_ERROR;
                        }
                      ...
                      if (strcmp (argv[loc], "all") == 0)
                        {
                            ElementIter & theEleIter =
                                theDomain.getElements ();
                            Element *theEle;
                ...
                else if (strcmp (argv[loc], "-region") == 0)
                  {
                      ...
                      for (int i = 0; i < eleRegion.Size (); i++)
                          if (secondaryFlag == false)
                              ...
                              secondaryEleIDs[numSecondaryEle++] =
                                  eleRegion (i);
                ...
                else if ((strcmp (argv[loc], "-mass") == 0))
                  {
                      ...
                      if (loc < argc
                          && Tcl_GetDouble (interp, argv[loc + 1],
                                            &eleM) != TCL_OK)
                        {
                ...
                else if ((strcmp (argv[loc], "-g") == 0))
                  {
                      ...
                      if (Tcl_GetDouble (interp, argv[loc], &gAcc) != TCL_OK)
                        {
                            opserr <<
                                "WARNING recorder Remove -g gValue? gDir? gPat?... invalid gValue ";
                      ...
                      if (Tcl_GetInt (interp, argv[loc + 1], &gDir) != TCL_OK)
                        {
                            opserr <<
                                "WARNING recorder Remove -g gValue? gDir? gPat?... invalid gDir ";
                      ...
                      if (Tcl_GetInt (interp, argv[loc + 2], &gPat) != TCL_OK)
                        {
                            opserr <<
                                "WARNING recorder Remove -g gValue? gDir? gPat?... invalid gPat ";
                            opserr << argv[loc + 1] << endln;
                            return TCL_ERROR;
                        }
                      loc += 3;
                ...
                else if (strcmp (argv[loc], "-database") == 0)
                  {
                      ...
                      if (theRecorderDatabase != 0)
                        ...
                        {
                            opserr <<
                                "WARNING recorder Node .. -database &lt;fileName&gt; - NO CURRENT DATABASE, results to File instead\n";
                            fileName = argv[loc + 1];
                        }
                         ...
                         || (strcmp (argv[loc], "-comp") == 0))
                  {
                      ...
                      int endSecIDs = loc;
                      int numSec = 0;
                      while (Tcl_GetInt (interp, argv[endSecIDs], &secID) ==
                             TCL_OK)
                        {
                            endSecIDs++;
                        }
                      ...
                      for (int i = loc; i < endSecIDs; i++)
                        {
                            if (Tcl_GetInt (interp, argv[i], &secID) !=
                                TCL_OK)
                                return TCL_ERROR;
                            secIDs[i - loc] = secID;
                        }
                         ...
                         || strcmp (argv[loc], "-crit") == 0)
                  {
                      ...
                      //
                        ...
                        {
                            opserr <<
                                "Error: RemoveRecorder - Removal Criteria " <<
                                argv[loc + 1] << " not recognized" << endln;
                            return TCL_ERROR;
                        }
                      //     new
                      if (critTag != 7)
                        {
                            if (Tcl_GetDouble
                                (interp, argv[loc + 2], &critValue) != TCL_OK)
                              {
                                  opserr <<
                                      "WARNING recorder Remove -crit critTag? critValue?... invalid critValue ";
                                  opserr << argv[loc + 1] << endln;
                                  return TCL_ERROR;
                              }
                        }
    ...
    // create a recorder to write nodal displacement quantities to a file
             ...
             || (strcmp (argv[1], "NodeEnvelope") == 0))
      {
          ...
          if (argc < 7)
            {
                ...
                opserr <<
                    "-node <list nodes> -dof <doflist> -file <fileName> -dT <dT> reponse";
                return TCL_ERROR;
            }
          ...
          while (flags == 0 && pos < argc)
            {
                ...
                if (strcmp (argv[pos], "-time") == 0)
                  ...
                  {
                      pos++;
                      if (Tcl_GetInt (interp, argv[pos], &writeBufferSize) !=
                          TCL_OK)
                          return TCL_ERROR;
                      pos++;
                  }
                  ...
                  {
                      inetAddr = argv[pos + 1];
                      if (Tcl_GetInt (interp, argv[pos + 2], &inetPort) !=
                          TCL_OK)
                        {
                            return TCL_ERROR;
                        }
                ...
                else if (strcmp (argv[pos], "-timeSeries") == 0)
                  {
                      ...
                      for (int j = pos; j < argc; j++)
                        {
                            if (Tcl_GetInt (interp, argv[pos], &dof) !=
                                TCL_OK)
                              {
                                  j = argc;
                                  Tcl_ResetResult (interp);
                      ...
                      for (int j = 0; j < numTimeSeries; j++)
                        {
                            ...
                            if (timeSeriesTag != 0 && timeSeriesTag != -1)
                              {
                                  theTimeSeries[j] =
                                      OPS_getTimeSeries (theTimeSeriesID (j));
                              }
                            else
                              {
                                  theTimeSeries[j] = 0;
                  ...
                  {
                      pos++;
                      if (Tcl_GetInt (interp, argv[pos], &precision) !=
                          TCL_OK)
                          return TCL_ERROR;
                      pos++;
                  }
                         ...
                         (strcmp (argv[pos], "-nodes") == 0))
                  {
                      ...
                      if (strcmp (argv[pos], "all") == 0)
                        {
                            opserr <<
                                "recoder Node - error -all option has been removed, use -nodeRange instaed\n";
                            return TCL_ERROR;
                        }
                      else
                        {
                            ...
                            int node;
                            for (int j = pos; j < argc; j++)
                                if (Tcl_GetInt (interp, argv[pos], &node) !=
                                    TCL_OK)
                                  {
                                      j = argc;
                                      Tcl_ResetResult (interp);
                ...
                else if (strcmp (argv[pos], "-nodeRange") == 0)
                  {
                      ...
                      if (argc < pos + 3)
                        {
                            opserr <<
                                "WARNING recorder Node .. -nodeRange start? end?  .. - no ele tags specified\n";
                            return TCL_ERROR;
                        }
                      ...
                      int start, end;
                      if (Tcl_GetInt (interp, argv[pos + 1], &start) !=
                          TCL_OK)
                        {
                            opserr <<
                ...
                else if (strcmp (argv[pos], "-region") == 0)
                  {
                      ...
                      if (argc < pos + 2)
                        {
                            opserr <<
                                "WARNING recorder Node .. -region tag?  .. - no region specified\n";
                            return TCL_ERROR;
                        }
                      int tag;
                      if (Tcl_GetInt (interp, argv[pos + 1], &tag) != TCL_OK)
                        {
                            opserr <<
                                "WARNING recorder Node -region tag? - invalid tag "
                                << argv[pos + 1] << endln;
                            return TCL_ERROR;
                        }
                      MeshRegion *theRegion = theDomain.getRegion (tag);
                ...
                // AddingSensitivity:BEGIN //////////////////////////////////////
                else if (strcmp (argv[pos], "-sensitivity") == 0)
                  {
                      ...
                      if (Tcl_GetInt (interp, argv[pos], &paramTag) != TCL_OK)
                        {
                            opserr <<
                                "ERROR: Invalid parameter tag to node recorder."
                                << endln;
                            return TCL_ERROR;
                        }
                      pos++;
                      ...
                      Parameter *theParameter =
                          theDomain.getParameter (paramTag);
                      if (theParameter == 0)
                        {
                            opserr << "NodeRecorder: parameter " << paramTag
                                << " not found" << endln;
                            return TCL_ERROR;
                        }
          ...
          if (pos >= argc)
            {
                opserr <<
                    "WARNING: No response type specified for node recorder, will assume you meant -disp\n"
                    << endln;
            }
          ...
          if (theTimeSeries != 0 && theTimeSeriesID.Size () < theDofs.Size ())
            {
                opserr <<
                    "ERROR: recorder Node/EnvelopNode # TimeSeries must equal # dof - IGNORING TimeSeries OPTION\n";
    ...
    else if (strcmp (argv[1], "Pattern") == 0)
      {
          if (argc < 4)
            {
                opserr <<
                    "WARNING recorder Pattern filename? <startFlag> patternTag?";
                return TCL_ERROR;
            }
    ...
    // Create a recorder to write nodal drifts to a file
             ...
             || (strcmp (argv[1], "EnvelopeDrift") == 0))
      {
          ...
          while (pos < argc)
            {
                ...
                if (strcmp (argv[pos], "-file") == 0)
                  ...
                  {
                      if (Tcl_GetInt (interp, argv[pos + 1], &perpDirn) !=
                          TCL_OK)
                        {
                            pos = argc;
                        }
          ...
          if (iNodes.Size () != jNodes.Size ())
            {
                opserr <<
                    "WARNING recorder Drift - the number of iNodes and jNodes must be the same "
                    << iNodes << " " << jNodes << endln;
                return TCL_ERROR;
            }
          ...
          if (strcmp (argv[1], "Drift") == 0)
              (*theRecorder) =
                  new DriftRecorder (iNodes, jNodes, dof - 1, perpDirn - 1,
                                     theDomain, *theOutputStream,
                                     echoTimeFlag, dT);
          else
              (*theRecorder) =
                  new EnvelopeDriftRecorder (iNodes, jNodes, dof - 1,
                                             perpDirn - 1, theDomain,
    ...
    // a recorder for the graphical display of the domain
    else if (strcmp (argv[1], "display") == 0)
      {
          ...
          if (argc < 7)
            {
                opserr <<
                    "WARNING recorder display title xLoc yLoc pixelsX pixelsY <-wipe> <-dT deltaT?> <-file fileName?>";
                return TCL_ERROR;
            }
          ...
          while (pos < argc)
            {
                ...
                // allow user to specify time step size for recording
                  ...
                  {
                      if (Tcl_GetDouble (interp, argv[pos + 1], &dT) !=
                          TCL_OK)
                          return TCL_ERROR;
                      pos += 2;
                  }
          ...
          if (!saveToFile)
              (*theRecorder) =
                  new TclFeViewer (argv[2], xLoc, yLoc, width, height,
    ...
    else if (strcmp (argv[1], "plot") == 0)
      {
          ...
          if (argc < 9)
            {
                opserr <<
                    "WARNING recorder plot fileName? windowTitle? xLoc yLoc xPixels yPixels -columns colX1 colY1 -columns colX2 ...";
                return TCL_ERROR;
            }
          ...
          while (loc < argc)
            {
                if ((strcmp (argv[loc], "-columns") == 0) ||
                ...
                else if (strcmp (argv[loc], "-dT") == 0)
                  {
                      ...
                      if (Tcl_GetDouble (interp, argv[loc + 1], &dT) !=
                          TCL_OK)
                          return TCL_ERROR;
                      loc += 2;
                  }
          ...
          FilePlotter *thePlotter =
              new FilePlotter (argv[2], argv[3], xLoc, yLoc, width, height,
                               dT);
    ...
    else if (strcmp (argv[1], "plotDifferent") == 0)
      {
          ...
          if (argc < 10)
            {
                opserr <<
                    "WARNING recorder display fileName? windowTitle? xLoc yLoc pixelsX pixelsY -columns colX1 colY1 -columns colX2 ...";
                return TCL_ERROR;
            }
...
#else
          FilePlotter *thePlotter =
              new FilePlotter (argv[2], argv[3], argv[4], xLoc, yLoc, width,
                               height, dT);
          (*theRecorder) = thePlotter;
          thePlotter->setCol (cols);
#endif
    ...
    else if (strcmp (argv[1], "increments") == 0)
      {
          ...
          if (theAlgorithm == 0)
            {
                opserr <<
                    "WARNING recorder increments - only allowed as algorithmRecorder";
                return TCL_ERROR;
            }
          if (argc < 7)
            {
          ...
          AlgorithmIncrements *thePlotter =
              new AlgorithmIncrements (theAlgorithm,
      ...
      {
          OPS_ResetInputNoBuilder (clientData, interp, 2, argc, argv,
                                   &theDomain);
    ...
    /* *****************************************
    ...
    else
      {
          ...
          ExternalRecorderCommand *recorderCommands =
              theExternalRecorderCommands;
          bool found = false;
                ...
                if (strcmp (argv[1], recorderCommands->funcName) == 0)
                  {
                      ...
                      OPS_ResetInputNoBuilder (clientData, interp, 2, argc,
                                               argv, &theDomain);
    ...
    if ((*theRecorder) == 0)
      {
          opserr << "WARNING ran out of memory - recorder " << argv[1] <<
              endln;
          return TCL_ERROR;
      }
                ...
                TCL_Char ** argv, Domain & theDomain)
{
    ...
    TclCreateRecorder (clientData, interp, argc, argv, theDomain,
                       &theRecorder);
    ...
    if (theAlgorithm != 0)
      {
          if ((theAlgorithm->addRecorder (*theRecorder)) < 0)
            {
                opserr << "WARNING could not add to domain - recorder " <<
                    argv[1] << endln;
                delete theRecorder;
                return TCL_ERROR;
            }
