...
std::map < int,
    VTK_Recorder::VtkType >
    VTK_Recorder::vtktypes;
...
OPS_VTK_Recorder ()
{
    ...
    // create recorder
    return new VTK_Recorder (name, outputData, eledata, indent, precision,
                             dT);
}
#endif
...
VTK_Recorder::VTK_Recorder ():Recorder (RECORDER_TAGS_VTK_Recorder),
indentsize (0),
precision (0),
indentlevel (0),
quota ('\"'),
theDomain (0),
nextTimeStampToRecord (0),
deltaT (0), counter (0), initializationDone (false), sendSelfCount (0)
{
    name = NULL;
          ...
          if (sendSelfCount >= 0)
            {
                for (int i = 0; i <= sendSelfCount; i++)
                  {
                      sprintf (filename, "%s/%s%d%020d.vtu", name, name, i,
                               counter);
                      thePVDFile << "<DataSet timestep=\"" << counter <<
                          "\" group=\"\" part=\"" << i << "\"" << " file=\""
                          << filename << "\"/>\n";
                  }
            }
...
VTK_Recorder::vtu ()
{
    ...
    if (sendSelfCount < 0)
      {
          sprintf (filename, "%s/%s%d%020d.vtu", name, name, -sendSelfCount,
                   counter);
      }
    else
      {
    ...
    this->incrLevel ();
    this->indent ();
    theFileVTU << "<Piece NumberOfPoints=\"" << numNode <<
        "\" NumberOfCells=\"" << numElement << "\">\n";
    ...
    // node tags
    theFileVTU <<
        "<DataArray type=\"Int32\" Name=\"Node Tag\" format=\"ascii\">\n";
    this->incrLevel ();
  for (auto i:theNodeTags)
    ...
    if (outputData.disp == true)
      {
          theFileVTU <<
              "<DataArray type=\"Float32\" Name=\"Disp\" NumberOfComponents=\""
              << maxNDF << "\" format=\"ascii\">\n";
        for (auto i:theNodeTags)
            {
    ...
    if (outputData.disp2 == true)
      {
          theFileVTU <<
              "<DataArray type=\"Float32\" Name=\"Disp2\" NumberOfComponents=\""
              << 2 << "\" format=\"ascii\">\n";
        for (auto i:theNodeTags)
            {
                Node *theNode = theDomain->getNode (i);
    ...
    if (outputData.disp3 == true)
      {
          theFileVTU <<
              "<DataArray type=\"Float32\" Name=\"Disp3\" NumberOfComponents=\""
              << 3 << "\" format=\"ascii\">\n";
        for (auto i:theNodeTags)
            {
                Node *theNode = theDomain->getNode (i);
    ...
    if (outputData.vel == true)
      {
          theFileVTU <<
              "<DataArray type=\"Float32\" Name=\"Vel\" NumberOfComponents=\""
              << maxNDF << "\" format=\"ascii\">\n";
        for (auto i:theNodeTags)
            {
                Node *theNode = theDomain->getNode (i);
    ...
    if (outputData.accel == true)
      {
          theFileVTU <<
              "<DataArray type=\"Float32\" Name=\"Accel\" NumberOfComponents=\""
              << maxNDF << "\" format=\"ascii\">\n";
        for (auto i:theNodeTags)
            {
                Node *theNode = theDomain->getNode (i);
    ...
    // ele tags
    theFileVTU <<
        "<DataArray type=\"Int32\" Name=\"Element Tag\" format=\"ascii\">\n";
    this->incrLevel ();
  for (auto i:theEleTags)
    ...
    // ele class tags
    theFileVTU <<
        "<DataArray type=\"Int32\" Name=\"Element Class\" format=\"ascii\">\n";
    this->incrLevel ();
...
int
VTK_Recorder::recvSelf (int commitTag, Channel & theChannel,
          ...
          if (it != vtktypes.end ())
          ...
          else
            {
                if (classTag != ELE_TAG_Subdomain)
                    opserr << "VTK_Recorder::init Element: " << eleTag <<
                        " of unknown vtk type\n";
            }
      }
