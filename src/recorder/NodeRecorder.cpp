...
NodeRecorder::NodeRecorder ():Recorder (RECORDER_TAGS_NodeRecorder),
...
NodeRecorder::NodeRecorder (const ID & dofs,
                            ...
                            double dT,
                            bool timeFlag, TimeSeries ** theSeries):
...
response (0),
theDomain (&theDom),
theOutputHandler (&theOutput),
echoTimeFlag (timeFlag),
dataFlag (0),
deltaT (dT),
...
timeSeriesValues (0)
{
          ...
          for (i = 0; i < numDOF; i++)
            {
                ...
                if (dof >= 0)
                  ...
                  {
                      opserr << "NodeRecorder::NodeRecorder - invalid dof  "
                          << dof;
    ...
    if (dataToStore == 0 || (strcmp (dataToStore, "disp") == 0))
      ...
      {
          dataFlag = 10;
          opserr << "NodeRecorder::NodeRecorder - dataToStore " <<
              dataToStore;
          opserr <<
...
NodeRecorder::record (int commitTag, double timeStamp)
{
    ...
    if (theOutputHandler == 0)
      {
          opserr <<
              "NodeRecorder::record() - no DataOutputHandler has been set\n";
          return -1;
      }
                      ...
                      if (dataFlag == 0)
                                  ...
                                  for (int j = 0; j < numDOF; j++)
                                    {
                                        ...
                                        response (cnt) =
                                            theNode->getDispSensitivity (dof +
                                                                         1,
                                                                         gradIndex);
                                        cnt++;
                                    }
                      ...
                      else if (dataFlag == 10002)
                        {
                            ...
                            Pressure_Constraint *pc =
                                theDomain->getPressure_Constraint (theNode->
                                                                   getTag ());
                            if (pc != 0)
                              {
                        ...
                        {
                            const Vector *theResponse =
                                theNode->getResponse (RayleighForces);
                            for (int j = 0; j < numDOF; j++)
                              {
                                  int dof = (*theDofs) (j);
                                  if (theResponse->Size () > dof)
                            ...
                            for (int j = 0; j < numDOF; j++)
                              {
                                  ...
                                  response (cnt) =
                                      theNode->getDispSensitivity (dof, grad);
                                  cnt++;
                              }
                            ...
                            for (int j = 0; j < numDOF; j++)
                              {
                                  ...
                                  dof += 1;     // Terje uses 1 through DOF for the dof indexing
                                  response (cnt) =
                                      theNode->getVelSensitivity (dof, grad);
                                  cnt++;
                              }
                            ...
                            for (int j = 0; j < numDOF; j++)
                              {
                                  ...
                                  dof += 1;     // Terje uses 1 through DOF for the dof indexing
                                  response (cnt) =
                                      theNode->getAccSensitivity (dof, grad);
                                  cnt++;
                              }
                        }
...
NodeRecorder::sendSelf (int commitTag, Channel & theChannel)
{
    ...
    if (theChannel.isDatastore () == 1)
      {
          opserr <<
              "NodeRecorder::sendSelf() - does not send data to a datastore\n";
          return -1;
      }
    ...
    if (theDofs != 0)
        if (theChannel.sendID (0, commitTag, *theDofs) < 0)
          {
              opserr <<
                  "NodeRecorder::sendSelf() - failed to send dof id's\n";
              return -1;
          }
    ...
    if (theNodalTags != 0)
        if (theChannel.sendID (0, commitTag, *theNodalTags) < 0)
          {
              opserr <<
                  "NodeRecorder::sendSelf() - failed to send nodal tags\n";
              return -1;
          }
                        ...
                        FEM_ObjectBroker & theBroker)
{
    ...
    if (theDofs != 0)
        if (theChannel.recvID (0, commitTag, *theDofs) < 0)
          {
              opserr <<
                  "NodeRecorder::recvSelf() - failed to recv dof data\n";
              return -1;
          }
...
NodeRecorder::initialize (void)
{
    if (theDofs == 0 || theDomain == 0)
      {
          opserr <<
              "NodeRecorder::initialize() - either nodes, dofs or domain has not been set\n";
          return -1;
      }
...
//added by SAJalali
double
NodeRecorder::getRecordedValue (int clmnId, int rowOffset, bool reset)
{
    double res = 0;
