...
// constructor:
LinearCrdTransf2d::LinearCrdTransf2d(int tag):CrdTransf(tag,
                                                        CRDTR_TAG_LinearCrdTransf2d),
nodeIPtr(0), nodeJPtr(0), nodeIOffset(0), nodeJOffset(0), cosTheta(0),
sinTheta(0), L(0), nodeIInitialDisp(0), nodeJInitialDisp(0),
...
initialDispChecked(false)
{
    ...
    if (&rigJntOffset1 == 0 || rigJntOffset1.Size() != 2) {
        opserr <<
            "LinearCrdTransf2d::LinearCrdTransf2d:  Invalid rigid joint offset vector for node I\n";
    ...
    if (&rigJntOffset2 == 0 || rigJntOffset2.Size() != 2) {
        opserr <<
            "LinearCrdTransf2d::LinearCrdTransf2d:  Invalid rigid joint offset vector for node J\n";
        opserr << "Size must be 2\n";
...
int LinearCrdTransf2d::revertToLastCommit(void)
{
    return 0;
}


int LinearCrdTransf2d::revertToStart(void)
{
    return 0;
}


int LinearCrdTransf2d::initialize(Node * nodeIPointer, Node * nodeJPointer)
{
    int error;
...
int LinearCrdTransf2d::update(void)
{
    return 0;
}


int LinearCrdTransf2d::computeElemtLengthAndOrient()
{
    // element projection
    static Vector dx(2);
...
void LinearCrdTransf2d::compTransfMatrixLocalGlobal(Matrix & Tlg)
{
...
double LinearCrdTransf2d::getInitialLength(void)
{
    return L;
}


double LinearCrdTransf2d::getDeformedLength(void)
{
    return L;
}


const Vector & LinearCrdTransf2d::getBasicTrialDisp(void)
{
    ...
    if (nodeIOffset != 0) {
        double t02 =
            -cosTheta * nodeIOffset[1] + sinTheta * nodeIOffset[0];
    ...
    if (nodeJOffset != 0) {
        double t35 =
            -cosTheta * nodeJOffset[1] + sinTheta * nodeJOffset[0];
...
const Vector & LinearCrdTransf2d::getBasicIncrDisp(void)
{
    // determine global displacements
    ...
    dub(1) =
        -sl * dug[0] + cl * dug[1] + dug[2] + sl * dug[3] - cl * dug[4];
...
const Vector & LinearCrdTransf2d::getBasicIncrDeltaDisp(void)
{
    // determine global displacements
...
const Vector & LinearCrdTransf2d::getBasicTrialVel(void)
{
...
const Vector & LinearCrdTransf2d::getBasicTrialAccel(void)
{
...
const Vector &
    LinearCrdTransf2d::getGlobalResistingForce(const Vector & pb,
                                               const Vector & p0)
{
...
const Vector &
    LinearCrdTransf2d::
getGlobalResistingForceShapeSensitivity(const Vector & pb,
                                        const Vector & p0)
{
    // transform resisting forces from the basic system to local coordinates
    static double pl[6];
    ...
    if (nodeParameterID(0) != 0 || nodeParameterID(1) != 0) {
        ...
        if (nodeIOffset != 0 || nodeJOffset != 0) {
            opserr << "ERROR: Currently a node offset cannot be used in "
                << endln << " conjunction with random nodal coordinates."
                << endln;
        }
        ...
        pg(0) =
            dcosdh * pl[0] - dsindh * pl[1] - sinTheta * d1oLdh * (q1 +
                                                                   q2);
        pg(1) =
            dsindh * pl[0] + dcosdh * pl[1] + cosTheta * d1oLdh * (q1 +
                                                                   q2);

        pg(3) =
            dcosdh * pl[3] - dsindh * pl[4] + sinTheta * d1oLdh * (q1 +
                                                                   q2);
        pg(4) =
            dsindh * pl[3] + dcosdh * pl[4] - cosTheta * d1oLdh * (q1 +
                                                                   q2);
...
const Matrix &
    LinearCrdTransf2d::getGlobalStiffMatrix(const Matrix & kb,
                                            const Vector & pb)
{
    ...
    if (nodeIOffset != 0) {
        ...
        t12 =
            oneOverL * (sinTheta * nodeIOffset[1] +
                        cosTheta * nodeIOffset[0]) + 1.0;
        t22 =
            oneOverL * (sinTheta * nodeIOffset[1] +
                        cosTheta * nodeIOffset[0]);
    }
    ...
    if (nodeJOffset != 0) {
        ...
        t15 =
            -oneOverL * (sinTheta * nodeJOffset[1] +
                         cosTheta * nodeJOffset[0]);
        t25 =
            -oneOverL * (sinTheta * nodeJOffset[1] +
                         cosTheta * nodeJOffset[0]) + 1.0;
    }
    ...
    tmp[0][2] =
        (nodeIOffset) ? t02 * kb00 + t12 * kb01 + t22 * kb02 : kb01;
    tmp[0][3] = -tmp[0][0];
    tmp[0][4] = -tmp[0][1];
    tmp[0][5] =
        (nodeJOffset) ? t05 * kb00 + t15 * kb01 + t25 * kb02 : kb02;

    tmp[1][0] = -cosTheta * kb10 - sl * (kb11 + kb12);
    tmp[1][1] = -sinTheta * kb10 + cl * (kb11 + kb12);
    tmp[1][2] =
        (nodeIOffset) ? t02 * kb10 + t12 * kb11 + t22 * kb12 : kb11;
    tmp[1][3] = -tmp[1][0];
    tmp[1][4] = -tmp[1][1];
    tmp[1][5] =
        (nodeJOffset) ? t05 * kb10 + t15 * kb11 + t25 * kb12 : kb12;

    tmp[2][0] = -cosTheta * kb20 - sl * (kb21 + kb22);
    tmp[2][1] = -sinTheta * kb20 + cl * (kb21 + kb22);
    tmp[2][2] =
        (nodeIOffset) ? t02 * kb20 + t12 * kb21 + t22 * kb22 : kb21;
    tmp[2][3] = -tmp[2][0];
    tmp[2][4] = -tmp[2][1];
    tmp[2][5] =
        (nodeJOffset) ? t05 * kb20 + t15 * kb21 + t25 * kb22 : kb22;
...
CrdTransf *LinearCrdTransf2d::getCopy2d(void)
{
...
int LinearCrdTransf2d::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
...
int LinearCrdTransf2d::recvSelf(int cTag, Channel & theChannel,
                                FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0) {
        opserr <<
            "LinearCrdTransf2d::recvSelf - failed to receive Vector\n";
        return res;
    }
...
const Vector &
    LinearCrdTransf2d::getPointGlobalDisplFromBasic(double xi,
                                                    const Vector & uxb)
{
    // determine global displacements
    const Vector & disp1 = nodeIPtr->getTrialDisp();
...
const Vector &
    LinearCrdTransf2d::getPointLocalDisplFromBasic(double xi,
                                                   const Vector & uxb)
{
    // determine global displacements
    const Vector & disp1 = nodeIPtr->getTrialDisp();
 ...
 LinearCrdTransf2d::Print(OPS_Stream & s, int flag)
{
    if (flag == OPS_PRINT_CURRENTSTATE) {
        s << "\nCrdTransf: " << this->getTag() <<
            " Type: LinearCrdTransf2d";
        if (nodeIOffset != 0)
            s << "\tnodeI Offset: " << nodeIOffset[0] << ' ' <<
                nodeIOffset[1] << endln;
        if (nodeJOffset != 0)
            s << "\tnodeJ Offset: " << nodeJOffset[0] << ' ' <<
                nodeJOffset[1] << endln;
    }
    ...
    if (flag == OPS_PRINT_PRINTMODEL_JSON) {
        s << "\t\t\t{\"name\": \"" << this->getTag() <<
            "\", \"type\": \"LinearCrdTransf2d\"";
        if (nodeIOffset != 0)
            s << ", \"iOffset\": [" << nodeIOffset[0] << ", " <<
                nodeIOffset[1] << "]";
        if (nodeJOffset != 0)
            s << ", \"jOffset\": [" << nodeJOffset[0] << ", " <<
                nodeJOffset[1] << "]";
        s << "}";
    }
}
...
const Vector &
    LinearCrdTransf2d::
getGlobalResistingForceShapeSensitivity(const Vector & pb,
                                        const Vector & p0, int gradNumber)
{
    // transform resisting forces from the basic system to local coordinates
    static double pl[6];
...
const Vector & LinearCrdTransf2d::getBasicDisplSensitivity(int gradNumber)
{
    static Vector U(6);
    ...
    //dudh = A*dUdh + dAdh*U;
    dudh(0) =
        cosTheta * dUdh(0) + sinTheta * dUdh(1) + dcosThetadh * U(0) +
        dsinThetadh * U(1);
    dudh(1) =
        -sinTheta * dUdh(0) + cosTheta * dUdh(1) - dsinThetadh * U(0) +
        dcosThetadh * U(1);
    dudh(2) = dUdh(2);
    dudh(3) =
        cosTheta * dUdh(3) + sinTheta * dUdh(4) + dcosThetadh * U(3) +
        dsinThetadh * U(4);
    dudh(4) =
        -sinTheta * dUdh(3) + cosTheta * dUdh(4) - dsinThetadh * U(3) +
        dcosThetadh * U(4);
    dudh(5) = dUdh(5);
    ...
    dvdh(1) =
        dudh(2) + (dudh(1) - dudh(4)) / L + (u(1) - u(4)) * doneOverLdh;
    dvdh(2) =
        dudh(5) + (dudh(1) - dudh(4)) / L + (u(1) - u(4)) * doneOverLdh;

    return dvdh;
}


bool LinearCrdTransf2d::isShapeSensitivity(void)
{
...
double LinearCrdTransf2d::getdLdh(void)
{
    int nodeParameterI, nodeParameterJ;
    nodeParameterI = nodeIPtr->getCrdsSensitivity();
    nodeParameterJ = nodeJPtr->getCrdsSensitivity();
...
double LinearCrdTransf2d::getd1overLdh(void)
{
    int nodeParameterI, nodeParameterJ;
    nodeParameterI = nodeIPtr->getCrdsSensitivity();
    nodeParameterJ = nodeJPtr->getCrdsSensitivity();
...
const Vector & LinearCrdTransf2d::getBasicTrialDispShapeSensitivity(void)
{
    ...
    if (nodeParameterID(0) != 0 || nodeParameterID(1) != 0) {
        ...
        ub(0) =
            -dcosdh * ug[0] - dsindh * ug[1] + dcosdh * ug[3] +
            dsindh * ug[4];

        ub(1) =
            -dsldh * ug[0] + dcldh * ug[1] + dsldh * ug[3] - dcldh * ug[4];
...
int
 LinearCrdTransf2d::getLocalAxes(Vector & xAxis, Vector & yAxis,
                                 Vector & zAxis)
{
    xAxis(0) = cosTheta;
    xAxis(1) = sinTheta;
