...
CorotCrdTransfWarping2d::CorotCrdTransfWarping2d(int tag,
                                                 ...
                                                 rigJntOffsetI,
                                                 const Vector &
                                                 rigJntOffsetJ):CrdTransf
    (tag, CRDTR_TAG_CorotCrdTransfWarping2d), nodeIOffset(2),
...
nodeIInitialDisp(0), nodeJInitialDisp(0), initialDispChecked(false)
{
    ...
    if (&rigJntOffsetI == 0 || rigJntOffsetI.Size() != 2) {
        opserr <<
            "CorotCrdTransfWarping2d::CorotCrdTransfWarping2d:  Invalid rigid joint offset vector for node I\n";
    ...
    if (&rigJntOffsetJ == 0 || rigJntOffsetJ.Size() != 2) {
        opserr <<
            "CorotCrdTransfWarping2d::CorotCrdTransfWarping2d:  Invalid rigid joint offset vector for node J\n";
        opserr << "Size must be 2\n";
        nodeJOffset.Zero();
    } else
...
int CorotCrdTransfWarping2d::revertToLastCommit(void)
{
    ub = ubcommit;
...
int CorotCrdTransfWarping2d::revertToStart(void)
{
    ub.Zero();
    this->update();
    return 0;
...
int CorotCrdTransfWarping2d::initialize(Node * nodeIPointer,
                                        Node * nodeJPointer)
{
    int error;
...
int CorotCrdTransfWarping2d::update(void)
{
...
int CorotCrdTransfWarping2d::compElemtLengthAndOrient(void)
{
    // element projection
    static Vector dx(2);
...
void CorotCrdTransfWarping2d::getTransfMatrixLocalGlobal(Matrix & Tlg)
{
...
void CorotCrdTransfWarping2d::getTransfMatrixBasicLocal(Matrix & Tbl)
{
...
const Vector & CorotCrdTransfWarping2d::getBasicTrialDisp(void)
{
    return ub;
}


const Vector & CorotCrdTransfWarping2d::getBasicIncrDeltaDisp(void)
{
    // dub = ub - ubpr;
    dub = ub;
...
const Vector & CorotCrdTransfWarping2d::getBasicIncrDisp(void)
{
    // Dub = ub - ubcommit;
    Dub = ub;
...
const Vector & CorotCrdTransfWarping2d::getBasicTrialVel(void)
{
...
const Vector & CorotCrdTransfWarping2d::getBasicTrialAccel(void)
{
    // determine global velocities
    const Vector & vel1 = nodeIPtr->getTrialVel();
    const Vector & vel2 = nodeJPtr->getTrialVel();
...
const Vector &
    CorotCrdTransfWarping2d::getGlobalResistingForce(const Vector & pb,
                                                     const Vector & p0)
{
...
const Matrix &
    CorotCrdTransfWarping2d::getGlobalStiffMatrix(const Matrix & kb,
                                                  const Vector & pb)
{
...
double CorotCrdTransfWarping2d::getDeformedLength(void)
{
    return Ln;
}


CrdTransf *CorotCrdTransfWarping2d::getCopy2d(void)
{
    // create a new instance of CorotCrdTransfWarping2d 
    CorotCrdTransfWarping2d *theCopy =
        new CorotCrdTransfWarping2d(this->getTag(), nodeIOffset,
                                    nodeJOffset);

    if (!theCopy) {
        opserr <<
            "CorotCrdTransfWarping2d::getCopy() - out of memory creating copy\n";
        return 0;
    }
...
int CorotCrdTransfWarping2d::sendSelf(int cTag, Channel & theChannel)
{
    Vector data(15);
    data(0) = ubcommit(0);
    data(1) = ubcommit(1);
    ...
    if (theChannel.sendVector(this->getTag(), cTag, data) < 0) {
        opserr <<
            " CorotCrdTransfWarping2d::sendSelf() - data could not be sent\n";
        return -1;
    }
    return 0;
}
...
int CorotCrdTransfWarping2d::recvSelf(int cTag, Channel & theChannel,
                                      FEM_ObjectBroker & theBroker)
{
    Vector data(15);
    if (theChannel.recvVector(this->getTag(), cTag, data) < 0) {
        opserr <<
            " CorotCrdTransfWarping2d::recvSelf() - data could not be received\n";
        return -1;
    }
...
getPointGlobalCoordFromLocal(const Vector & xl)
{
    static Vector xg(5);
    opserr <<
        " CorotCrdTransfWarping2d::getPointGlobalCoordFromLocal: not implemented yet";
...
const Vector &
    CorotCrdTransfWarping2d::getPointGlobalDisplFromBasic(double xi,
                                                          const Vector &
                                                          uxb)
{
    opserr <<
        " CorotCrdTransfWarping2d::getPointGlobalDisplFromBasic: not implemented yet";
...
const Vector &
    CorotCrdTransfWarping2d::getPointLocalDisplFromBasic(double xi,
                                                         const Vector &
                                                         uxb)
{
    opserr <<
        " CorotCrdTransfWarping2d::getPointLocalDisplFromBasic: not implemented yet";
                                        ...
                                        const Vector & p0, int gradNumber)
{
    ...
    dpldh(0) =
        (-dcosAlphadh * q0 -
         dsinAlphaOverLndh * (q1 + q2 + q3 + q4)) * dLdh;
    dpldh(1) =
        (-dsinAlphadh * q0 +
         dcosAlphaOverLndh * (q1 + q2 + q3 + q4)) * dLdh;
    dpldh(2) = 0.0;
    dpldh(3) = 0.0;
    dpldh(4) =
        (dcosAlphadh * q0 +
...
double CorotCrdTransfWarping2d::getdLdh(void)
{
    int nodeIid = nodeIPtr->getCrdsSensitivity();
    int nodeJid = nodeJPtr->getCrdsSensitivity();
...
double CorotCrdTransfWarping2d::getd1overLdh(void)
{
    int nodeIid = nodeIPtr->getCrdsSensitivity();
    int nodeJid = nodeJPtr->getCrdsSensitivity();
