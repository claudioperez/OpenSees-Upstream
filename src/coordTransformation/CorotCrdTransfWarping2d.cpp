...
Matrix
CorotCrdTransfWarping2d::Tlg (8, 8);
Matrix
CorotCrdTransfWarping2d::Tbl (5, 8);
Vector
CorotCrdTransfWarping2d::uxg (5);
Vector
CorotCrdTransfWarping2d::pg (8);
Vector
CorotCrdTransfWarping2d::dub (5);
Vector
CorotCrdTransfWarping2d::Dub (5);
Matrix
CorotCrdTransfWarping2d::kg (8, 8);
...
void
CorotCrdTransfWarping2d::transfLocalDisplsToBasic (const Vector & ul)
{
...
CorotCrdTransfWarping2d::getBasicTrialAccel (void)
{
    ...
    ab (0) =
        (Lxdot * Lxdot + Lx * Lxdotdot + Ly * Lydotdot + Lydot * Lydot) / Ln -
...
int
CorotCrdTransfWarping2d::recvSelf (int cTag, Channel & theChannel,
...
CorotCrdTransfWarping2d::Print (OPS_Stream & s, int flag)
{
    if (flag == OPS_PRINT_CURRENTSTATE)
      {
          s << "\nCrdTransf: " << this->
              getTag () << " Type: CorotCrdTransfWarping2d";
...
const Vector &
CorotCrdTransfWarping2d::
getGlobalResistingForceShapeSensitivity (const Vector & q, const Vector & p0,
                                         int gradNumber)
{
    ...
    if (nodeIOffset.Norm () != 0.0 || nodeJOffset.Norm () != 0.0)
      {
          opserr << "ERROR: Currently a node offset cannot be used in " <<
              endln << " conjunction with random nodal coordinates." << endln;
      }
    ...
    dpldh (0) =
        (-dcosAlphadh * q0 - dsinAlphaOverLndh * (q1 + q2 + q3 + q4)) * dLdh;
    dpldh (1) =
        (-dsinAlphadh * q0 + dcosAlphaOverLndh * (q1 + q2 + q3 + q4)) * dLdh;
    dpldh (2) = 0.0;
    dpldh (3) = 0.0;
    dpldh (4) =
        (dcosAlphadh * q0 + dsinAlphaOverLndh * (q1 + q2 + q3 + q4)) * dLdh;
