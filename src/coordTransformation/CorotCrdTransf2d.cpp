...
CorotCrdTransf2d::CorotCrdTransf2d(int tag,
                                   ...
                                   rigJntOffsetJ):CrdTransf(tag,
                                                            CRDTR_TAG_CorotCrdTransf2d),
nodeIOffset(2), nodeJOffset(2), cosTheta(0), sinTheta(0), cosAlpha(0),
sinAlpha(0), nodeIPtr(0), nodeJPtr(0), L(0), Ln(0), ub(3), ubcommit(3),
ubpr(3), nodeIInitialDisp(0), nodeJInitialDisp(0),
initialDispChecked(false)
{
    // check rigid joint offset for node I
    if (&rigJntOffsetI == 0 || rigJntOffsetI.Size() != 2) {
        opserr <<
            "CorotCrdTransf2d::CorotCrdTransf2d:  Invalid rigid joint offset vector for node I\n";
    ...
    if (&rigJntOffsetJ == 0 || rigJntOffsetJ.Size() != 2) {
        opserr <<
            "CorotCrdTransf2d::CorotCrdTransf2d:  Invalid rigid joint offset vector for node J\n";
        opserr << "Size must be 2\n";
        nodeJOffset.Zero();
    } else
...
int CorotCrdTransf2d::revertToLastCommit(void)
{
    ub = ubcommit;
...
int CorotCrdTransf2d::revertToStart(void)
{
    ub.Zero();
    this->update();
    return 0;
...
int CorotCrdTransf2d::initialize(Node * nodeIPointer, Node * nodeJPointer)
{
    int error;
...
int CorotCrdTransf2d::update(void)
{
...
int CorotCrdTransf2d::compElemtLengthAndOrient(void)
{
    // element projection
    static Vector dx(2);
...
int CorotCrdTransf2d::
compElemtLengthAndOrientWRTLocalSystem(const Vector & ul)
{
...
void CorotCrdTransf2d::compTransfMatrixLocalGlobal(Matrix & Tlg)
{
...
void CorotCrdTransf2d::compTransfMatrixBasicLocal(Matrix & Tbl)
{
...
const Vector & CorotCrdTransf2d::getBasicTrialDisp(void)
{
    return ub;
}


const Vector & CorotCrdTransf2d::getBasicIncrDeltaDisp(void)
{
    // dub = ub - ubpr;
    dub = ub;
...
const Vector & CorotCrdTransf2d::getBasicIncrDisp(void)
{
    // Dub = ub - ubcommit;
    Dub = ub;
...
const Vector & CorotCrdTransf2d::getBasicTrialVel(void)
{
...
const Vector & CorotCrdTransf2d::getBasicTrialAccel(void)
{
    // determine global velocities
    const Vector & vel1 = nodeIPtr->getTrialVel();
    const Vector & vel2 = nodeJPtr->getTrialVel();
    ...
    ab(0) =
        (Lxdot * Lxdot + Lx * Lxdotdot + Ly * Lydotdot +
         Lydot * Lydot) / Ln - pow(Lx * Lxdot + Ly * Lydot, 2) / pow(Ln,
                                                                     3);
    ab(1) =
        al(2) - (Lx * Lydotdot - Ly * Lxdotdot) / pow(Ln,
                                                      2) +
        2 * (Lx * Lydot - Ly * Lxdot) * (Lx * Lxdot + Ly * Lydot) / pow(Ln,
                                                                        4);
    ab(2) = al(5) + ab(1) - al(2);
...
const Vector &
    CorotCrdTransf2d::getGlobalResistingForce(const Vector & pb,
                                              const Vector & p0)
{
...
const Matrix &
    CorotCrdTransf2d::getGlobalStiffMatrix(const Matrix & kb,
                                           const Vector & pb)
{
...
const Matrix & CorotCrdTransf2d::getGeomStiffMatrix(const Vector & pb) const
{
...
double CorotCrdTransf2d::getInitialLength(void)
{
    return L;
}


double CorotCrdTransf2d::getDeformedLength(void)
{
    return Ln;
}


CrdTransf *CorotCrdTransf2d::getCopy2d(void)
{
    ...
    if (!theCopy) {
        opserr <<
            "CorotCrdTransf2d::getCopy() - out of memory creating copy\n";
        return 0;
    }
...
int CorotCrdTransf2d::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (theChannel.sendVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            " CorotCrdTransf2d::sendSelf() - data could not be sent\n";
        return -1;
    }
    return 0;
}
...
int CorotCrdTransf2d::recvSelf(int cTag, Channel & theChannel,
                               FEM_ObjectBroker & theBroker)
{
    static Vector data(14);
    if (theChannel.recvVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            " CorotCrdTransf2d::recvSelf() - data could not be received\n";
        return -1;
    }
    ...
    CorotCrdTransf2d::getPointGlobalCoordFromLocal(const Vector & xl)
{
    static Vector xg(3);
    opserr <<
        " CorotCrdTransf2d::getPointGlobalCoordFromLocal: not implemented yet";
...
const Vector &
    CorotCrdTransf2d::getPointGlobalDisplFromBasic(double xi,
                                                   const Vector & uxb)
{
    opserr <<
        " CorotCrdTransf2d::getPointGlobalDisplFromBasic: not implemented yet";

    return uxg;
}

const Vector &
    CorotCrdTransf2d::getPointLocalDisplFromBasic(double xi,
                                                  const Vector & uxb)
{
    opserr <<
        " CorotCrdTransf2d::getPointLocalDisplFromBasic: not implemented yet";
...
const Vector &
    CorotCrdTransf2d::
getGlobalResistingForceShapeSensitivity(const Vector & q,
...
const Vector & CorotCrdTransf2d::getBasicDisplSensitivity(int gradNumber)
{
...
const Vector & CorotCrdTransf2d::getBasicTrialDispShapeSensitivity(void)
{
    static Vector dvdh(3);
    dvdh.Zero();
...
bool CorotCrdTransf2d::isShapeSensitivity(void)
{
    int nodeIid = nodeIPtr->getCrdsSensitivity();
    int nodeJid = nodeJPtr->getCrdsSensitivity();
...
double CorotCrdTransf2d::getdLdh(void)
{
    int nodeIid = nodeIPtr->getCrdsSensitivity();
    int nodeJid = nodeJPtr->getCrdsSensitivity();
...
double CorotCrdTransf2d::getd1overLdh(void)
{
    int nodeIid = nodeIPtr->getCrdsSensitivity();
    int nodeJid = nodeJPtr->getCrdsSensitivity();
