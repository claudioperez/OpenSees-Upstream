                         ...
                         Domain * theDomain, TclModelBuilder * theBuilder)
{
    ...
    if (argc < 2) {
        ...
        opserr << "Want: geomTransf type? tag? <specific transf args>" <<
            endln;
        return TCL_ERROR;
    }
    ...
    if ((NDM == 2 && NDF == 3) || (NDM == 2 && NDF == 4)) {
        ...
        if (argc < 3) {
            opserr <<
                "WARNING insufficient arguments - want: geomTransf type? tag? <-jntOffset dXi? dYi? dXj? dYj?>\n";
            return TCL_ERROR;
        }
        ...
        if (Tcl_GetInt(interp, argv[argi++], &crdTransfTag) != TCL_OK) {
            opserr <<
                "WARNING invalid tag - want: geomTransf type? tag? <-jntOffset dXi? dYi? dXj? dYj?>\n";
            return TCL_ERROR;
        }
        ...
        while (argi != argc) {
            if (strcmp(argv[argi], "-jntOffset") == 0) {
                ...
                for (i = 0; i < 2; i++) {
                    if (argi == argc
                        || Tcl_GetDouble(interp, argv[argi++],
                                         &jntOffsetI(i)) != TCL_OK) {
                        opserr <<
                            "WARNING invalid jntOffset value - want: geomTransf type? tag? <-jntOffset dXi? dYi? dXj? dYj?>\n";
                        return TCL_ERROR;
                    }
                }
                ...
                opserr <<
                    "WARNING bad command - want: geomTransf type? tag? <-jntOffset dXi? dYi? dXj? dYj?>\n";
        ...
        if (strcmp(argv[1], "Linear") == 0)
            crdTransf2d =
                new LinearCrdTransf2d(crdTransfTag, jntOffsetI,
                                      jntOffsetJ);

        else if (strcmp(argv[1], "LinearInt") == 0)
            //      crdTransf2d = new LinearCrdTransf2dInt(crdTransfTag, jntOffsetI, jntOffsetJ);
            crdTransf2d =
                new LinearCrdTransf2dInt(crdTransfTag, jntOffsetI,
                                         jntOffsetJ);

        else if (strcmp(argv[1], "PDelta") == 0
                 || strcmp(argv[1], "LinearWithPDelta") == 0)
            crdTransf2d =
                new PDeltaCrdTransf2d(crdTransfTag, jntOffsetI,
                                      jntOffsetJ);

        else if (strcmp(argv[1], "Corotational") == 0 && NDF == 3)
            crdTransf2d =
                new CorotCrdTransf2d(crdTransfTag, jntOffsetI, jntOffsetJ);

        else if (strcmp(argv[1], "Corotational") == 0 && NDF == 4)
            crdTransf2d =
                new CorotCrdTransfWarping2d(crdTransfTag, jntOffsetI,
                                            jntOffsetJ);

        else {
            opserr <<
                "WARNING TclElmtBuilder - addGeomTransf - invalid Type\n";
            opserr << argv[1] << endln;
            return TCL_ERROR;
        }
        ...
        if (crdTransf2d == 0) {
            opserr <<
                "WARNING TclElmtBuilder - addGeomTransf - ran out of memory to create geometric transformation object\n";
            return TCL_ERROR;
        }
        // add the transformation to the modelBuilder
        if (OPS_addCrdTransf(crdTransf2d) != true) {
            opserr <<
                "WARNING TclElmtBuilder - addGeomTransf  - could not add geometric transformation to model Builder\n";
            return TCL_ERROR;
        }
    }
        ...
        if (Tcl_GetInt(interp, argv[argi++], &crdTransfTag) != TCL_OK) {
            opserr <<
                "WARNING invalid tag - want: geomTransf type? tag? vecxzPlaneX? vecxzPlaneY? vecxzPlaneZ?  <-jntOffset dXi? dYi? dZi? dXj? dYj? dZj? >\n";
            return TCL_ERROR;
        }

        if (Tcl_GetDouble(interp, argv[argi++], &vecxzPlane(0)) != TCL_OK) {
            opserr <<
                "WARNING invalid vecxzPlaneX - want: geomTransf type? tag? vecxzPlaneX? vecxzPlaneY? vecxzPlaneZ?  <-jntOffset dXi? dYi? dZi? dXj? dYj? dZj? >\n";
            return TCL_ERROR;
        }

        if (Tcl_GetDouble(interp, argv[argi++], &vecxzPlane(1)) != TCL_OK) {
            opserr <<
                "WARNING invalid vecxzPlaneY - want: geomTransf type? tag? vecxzPlaneX? vecxzPlaneY? vecxzPlaneZ?  <-jntOffset dXi? dYi? dZi? dXj? dYj? dZj? >\n";
            return TCL_ERROR;
        }

        if (Tcl_GetDouble(interp, argv[argi++], &vecxzPlane(2)) != TCL_OK) {
            opserr <<
                "WARNING invalid vecxzPlaneZ - want: geomTransf type? tag? vecxzPlaneX? vecxzPlaneY? vecxzPlaneZ?  <-jntOffset dXi? dYi? dZi? dXj? dYj? dZj? >\n";
            return TCL_ERROR;
        }
        // allow additional options at end of command
        int i;
        ...
        while (argi != argc) {
            if (strcmp(argv[argi], "-jntOffset") == 0) {
                ...
                for (i = 0; i < 3; i++) {
                    if (argi == argc
                        || Tcl_GetDouble(interp, argv[argi++],
                                         &jntOffsetI(i)) != TCL_OK) {
                        opserr <<
                            "WARNING invalid jntOffset value - want: geomTransf type? tag? vecxzPlaneX? vecxzPlaneY? vecxzPlaneZ?  <-jntOffset dXi? dYi? dZi? dXj? dYj? dZj? >\n";
                        return TCL_ERROR;
                    }
                }
                ...
                opserr <<
                    "WARNING bad command - want: geomTransf type? tag? vecxzPlaneX? vecxzPlaneY? vecxzPlaneZ?  <-jntOffset dXi? dYi? dZi? dXj? dYj? dZj? > ";
                opserr << "invalid: " << argv[argi] << endln;
                return TCL_ERROR;
            }
        }
        ...
        if (strcmp(argv[1], "Linear") == 0)
            crdTransf3d =
                new LinearCrdTransf3d(crdTransfTag, vecxzPlane, jntOffsetI,
                                      jntOffsetJ);

        else if (strcmp(argv[1], "PDelta") == 0
                 || strcmp(argv[1], "LinearWithPDelta") == 0)
            crdTransf3d =
                new PDeltaCrdTransf3d(crdTransfTag, vecxzPlane, jntOffsetI,
                                      jntOffsetJ);

        else if (strcmp(argv[1], "Corotational") == 0)
            crdTransf3d =
                new CorotCrdTransf3d(crdTransfTag, vecxzPlane, jntOffsetI,
                                     jntOffsetJ);
