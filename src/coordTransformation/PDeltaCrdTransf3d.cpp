...
PDeltaCrdTransf3d::PDeltaCrdTransf3d(int tag,
                                     const Vector &
                                     vecInLocXZPlane):CrdTransf(tag,
                                                                CRDTR_TAG_PDeltaCrdTransf3d),
nodeIPtr(0), nodeJPtr(0), nodeIOffset(0), nodeJOffset(0), L(0), ul17(0),
ul28(0), nodeIInitialDisp(0), nodeJInitialDisp(0),
...
initialDispChecked(false)
{
    ...
    if (&rigJntOffset1 == 0 || rigJntOffset1.Size() != 3) {
        opserr <<
            "PDeltaCrdTransf3d::PDeltaCrdTransf3d:  Invalid rigid joint offset vector for node I\n";
    ...
    if (&rigJntOffset2 == 0 || rigJntOffset2.Size() != 3) {
        opserr <<
            "PDeltaCrdTransf3d::PDeltaCrdTransf3d:  Invalid rigid joint offset vector for node J\n";
        opserr << "Size must be 3\n";
...
int PDeltaCrdTransf3d::revertToLastCommit(void)
{
    return 0;
}


int PDeltaCrdTransf3d::revertToStart(void)
{
    return 0;
}


int PDeltaCrdTransf3d::initialize(Node * nodeIPointer, Node * nodeJPointer)
{
    int error;
...
int PDeltaCrdTransf3d::update(void)
{
...
int PDeltaCrdTransf3d::computeElemtLengthAndOrient()
{
    // element projection
    static Vector dx(3);
...
void PDeltaCrdTransf3d::compTransfMatrixLocalGlobal(Matrix & Tlg)
{
...
int PDeltaCrdTransf3d::getLocalAxes(Vector & XAxis, Vector & YAxis,
                                    Vector & ZAxis)
{
    // Compute y = v cross x
...
double PDeltaCrdTransf3d::getInitialLength(void)
{
    return L;
}


double PDeltaCrdTransf3d::getDeformedLength(void)
{
    return L;
}


const Vector & PDeltaCrdTransf3d::getBasicTrialDisp(void)
{
...
const Vector & PDeltaCrdTransf3d::getBasicIncrDisp(void)
{
    // determine global displacements
...
const Vector & PDeltaCrdTransf3d::getBasicIncrDeltaDisp(void)
{
    // determine global displacements
...
const Vector & PDeltaCrdTransf3d::getBasicTrialVel(void)
{
...
const Vector & PDeltaCrdTransf3d::getBasicTrialAccel(void)
{
...
const Vector &
    PDeltaCrdTransf3d::getGlobalResistingForce(const Vector & pb,
                                               const Vector & p0)
{
...
const Matrix &
    PDeltaCrdTransf3d::getGlobalStiffMatrix(const Matrix & KB,
                                            const Vector & pb)
{
    ...
    for (m = 0; m < 12; m++) {
        tmp[m][0] =
            kl[m][0] * R[0][0] + kl[m][1] * R[1][0] + kl[m][2] * R[2][0];
        tmp[m][1] =
            kl[m][0] * R[0][1] + kl[m][1] * R[1][1] + kl[m][2] * R[2][1];
        tmp[m][2] =
            kl[m][0] * R[0][2] + kl[m][1] * R[1][2] + kl[m][2] * R[2][2];

        tmp[m][3] =
            kl[m][3] * R[0][0] + kl[m][4] * R[1][0] + kl[m][5] * R[2][0];
        tmp[m][4] =
            kl[m][3] * R[0][1] + kl[m][4] * R[1][1] + kl[m][5] * R[2][1];
        tmp[m][5] =
            kl[m][3] * R[0][2] + kl[m][4] * R[1][2] + kl[m][5] * R[2][2];

        if (nodeIOffset) {
            tmp[m][3] +=
                kl[m][0] * RWI[0][0] + kl[m][1] * RWI[1][0] +
                kl[m][2] * RWI[2][0];
            tmp[m][4] +=
                kl[m][0] * RWI[0][1] + kl[m][1] * RWI[1][1] +
                kl[m][2] * RWI[2][1];
            tmp[m][5] +=
                kl[m][0] * RWI[0][2] + kl[m][1] * RWI[1][2] +
                kl[m][2] * RWI[2][2];
        }

        tmp[m][6] =
            kl[m][6] * R[0][0] + kl[m][7] * R[1][0] + kl[m][8] * R[2][0];
        tmp[m][7] =
            kl[m][6] * R[0][1] + kl[m][7] * R[1][1] + kl[m][8] * R[2][1];
        tmp[m][8] =
            kl[m][6] * R[0][2] + kl[m][7] * R[1][2] + kl[m][8] * R[2][2];

        tmp[m][9] =
            kl[m][9] * R[0][0] + kl[m][10] * R[1][0] + kl[m][11] * R[2][0];
        tmp[m][10] =
            kl[m][9] * R[0][1] + kl[m][10] * R[1][1] + kl[m][11] * R[2][1];
        tmp[m][11] =
            kl[m][9] * R[0][2] + kl[m][10] * R[1][2] + kl[m][11] * R[2][2];

        if (nodeJOffset) {
            tmp[m][9] +=
                kl[m][6] * RWJ[0][0] + kl[m][7] * RWJ[1][0] +
                kl[m][8] * RWJ[2][0];
            tmp[m][10] +=
                kl[m][6] * RWJ[0][1] + kl[m][7] * RWJ[1][1] +
                kl[m][8] * RWJ[2][1];
            tmp[m][11] +=
                kl[m][6] * RWJ[0][2] + kl[m][7] * RWJ[1][2] +
                kl[m][8] * RWJ[2][2];
        }
    ...
    for (m = 0; m < 12; m++) {
        kg(0, m) =
            R[0][0] * tmp[0][m] + R[1][0] * tmp[1][m] +
            R[2][0] * tmp[2][m];
        kg(1, m) =
            R[0][1] * tmp[0][m] + R[1][1] * tmp[1][m] +
            R[2][1] * tmp[2][m];
        kg(2, m) =
            R[0][2] * tmp[0][m] + R[1][2] * tmp[1][m] +
            R[2][2] * tmp[2][m];

        kg(3, m) =
            R[0][0] * tmp[3][m] + R[1][0] * tmp[4][m] +
            R[2][0] * tmp[5][m];
        kg(4, m) =
            R[0][1] * tmp[3][m] + R[1][1] * tmp[4][m] +
            R[2][1] * tmp[5][m];
        kg(5, m) =
            R[0][2] * tmp[3][m] + R[1][2] * tmp[4][m] +
            R[2][2] * tmp[5][m];

        if (nodeIOffset) {
            kg(3, m) +=
                RWI[0][0] * tmp[0][m] + RWI[1][0] * tmp[1][m] +
                RWI[2][0] * tmp[2][m];
            kg(4, m) +=
                RWI[0][1] * tmp[0][m] + RWI[1][1] * tmp[1][m] +
                RWI[2][1] * tmp[2][m];
            kg(5, m) +=
                RWI[0][2] * tmp[0][m] + RWI[1][2] * tmp[1][m] +
                RWI[2][2] * tmp[2][m];
        }

        kg(6, m) =
            R[0][0] * tmp[6][m] + R[1][0] * tmp[7][m] +
            R[2][0] * tmp[8][m];
        kg(7, m) =
            R[0][1] * tmp[6][m] + R[1][1] * tmp[7][m] +
            R[2][1] * tmp[8][m];
        kg(8, m) =
            R[0][2] * tmp[6][m] + R[1][2] * tmp[7][m] +
            R[2][2] * tmp[8][m];

        kg(9, m) =
            R[0][0] * tmp[9][m] + R[1][0] * tmp[10][m] +
            R[2][0] * tmp[11][m];
        kg(10, m) =
            R[0][1] * tmp[9][m] + R[1][1] * tmp[10][m] +
            R[2][1] * tmp[11][m];
        kg(11, m) =
            R[0][2] * tmp[9][m] + R[1][2] * tmp[10][m] +
            R[2][2] * tmp[11][m];

        if (nodeJOffset) {
            kg(9, m) +=
                RWJ[0][0] * tmp[6][m] + RWJ[1][0] * tmp[7][m] +
                RWJ[2][0] * tmp[8][m];
            kg(10, m) +=
                RWJ[0][1] * tmp[6][m] + RWJ[1][1] * tmp[7][m] +
                RWJ[2][1] * tmp[8][m];
            kg(11, m) +=
                RWJ[0][2] * tmp[6][m] + RWJ[1][2] * tmp[7][m] +
                RWJ[2][2] * tmp[8][m];
        }
    }
...
CrdTransf *PDeltaCrdTransf3d::getCopy3d(void)
{
...
int PDeltaCrdTransf3d::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
...
int PDeltaCrdTransf3d::recvSelf(int cTag, Channel & theChannel,
                                FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0) {
        opserr <<
            "PDeltaCrdTransf3d::recvSelf - failed to receive Vector\n";
        return res;
    }
...
const Vector &
    PDeltaCrdTransf3d::getPointGlobalDisplFromBasic(double xi,
                                                    const Vector & uxb)
{
    // determine global displacements
    const Vector & disp1 = nodeIPtr->getTrialDisp();
...
const Vector &
    PDeltaCrdTransf3d::getPointLocalDisplFromBasic(double xi,
                                                   const Vector & uxb)
{
    // determine global displacements
    const Vector & disp1 = nodeIPtr->getTrialDisp();
 ...
 PDeltaCrdTransf3d::Print(OPS_Stream & s, int flag)
{
    if (flag == OPS_PRINT_CURRENTSTATE) {
        s << "\nCrdTransf: " << this->getTag() <<
            " Type: PDeltaCrdTransf3d" << endln;
        if (nodeIOffset)
            s << "\tNode I offset: " << nodeIOffset[0] << " " <<
                nodeIOffset[1] << " " << nodeIOffset[2] << endln;
        if (nodeJOffset)
            s << "\tNode J offset: " << nodeJOffset[0] << " " <<
                nodeJOffset[1] << " " << nodeJOffset[2] << endln;
    }

    if (flag == OPS_PRINT_PRINTMODEL_JSON) {
        s << "\t\t\t{\"name\": \"" << this->getTag() <<
            "\", \"type\": \"PDeltaCrdTransf3d\"";
        s << ", \"vecInLocXZPlane\": [" << R[2][0] << ", " << R[2][1] <<
            ", " << R[2][2] << "]";
        if (nodeIOffset != 0)
            s << ", \"iOffset\": [" << nodeIOffset[0] << ", " <<
                nodeIOffset[1] << ", " << nodeIOffset[2] << "]";
        if (nodeJOffset != 0)
            s << ", \"jOffset\": [" << nodeJOffset[0] << ", " <<
                nodeJOffset[1] << ", " << nodeJOffset[2] << "]";
        s << "}";
    }
}
