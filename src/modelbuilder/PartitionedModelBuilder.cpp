...
PartitionedModelBuilder (PartitionedDomain & aPartitionedDomain,
                         int theClassTag):
ModelBuilder (aPartitionedDomain),
    ...
    while ((theSubdomain = theSubs ()) != 0)
      {
          ...
          if (result != 0)
            {
                opserr << "PartitionedModelBuilder::buildFE_Model(void) -";
                opserr << "buildSubdomain failed for Subdomain " <<
                    theSubdomain->getTag ();
                opserr << endln;
                return result;
            }
