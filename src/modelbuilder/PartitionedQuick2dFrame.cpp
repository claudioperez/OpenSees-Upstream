...
PartitionedQuick2dFrame (PartitionedDomain & theDomain, int numXX, int numYY,
                         int eleTType):
...
PartitionedQuick2dFrame::PartitionedQuick2dFrame (Subdomain & theDomain):PartitionedModelBuilder (theDomain,
                         PartitionedModelBuilder_TAGS_PartitionedQuick2dFrameModel),
                ...
                if (nodePtr != 0)
                  {
                      ...
                      if (ok < 0)
                        {
                            opserr <<
                                "PartitionedQuick2dFrame::PartitionedQuick2dFrame::Input ";
                            opserr << " could not add Node " << nodeNum <<
                                endln;
                            return -1;
                        }
                      nodeNum++;
                ...
                else
                  {
                      opserr <<
                          "PartitionedQuick2dFrame::PartitionedQuick2dFrame::Input ";
                      opserr << " ran out of memory creating Node " << nodeNum
                          << endln;
                      return -1;
                  }
    ...
    for (i = 0; i < numLoads; i++)
      {
          ...
          if (nodeLoadPtr != 0)
                ...
                if (ok < 0)
                  {
                      opserr <<
                          "PartitionedQuick2dFrame::PartitionedQuick2dFrame::Input ";
                      opserr << " could not add nodal load " << nodeNum <<
                          endln;
                      return -1;
                  }
            }
            ...
            {
                opserr <<
                    "PartitionedQuick2dFrame::PartitionedQuick2dFrame::Input ran out of ";
                                         ...
                                         Subdomain & theSubdomain)
{
    ...
    for (i = 0; i < numSect; i++)
      {
          Sections (i * SectDat) = 1;
          Sections (i * SectDat + 1) = 1;
      }
                ...
                if (nodePtr != 0)
                  {
                      ...
                      else if (i == numSubdomainStories
                               && subdomain != (numSubdomains - 1))
                          ok = theSubdomain.addExternalNode (nodePtr);
                      else
    ...
    int eleNum =
        0 + NO_x * numSubdomainStories * subdomain + (NO_x -
                                                      1) *
        numSubdomainStories * subdomain;
    if (subdomain == (numSubdomains - 1))
        numSubdomainStories += numY - numSubdomainStories * numSubdomains;
          ...
          for (j = 0; j < NO_x; j++)
            {
                ...
                if (elePtr != 0)
                      ...
                      if (ok < 0)
                        {
                            opserr <<
                                "PartitionedQuick2dFrame::PartitionedQuick2dFrame::Input ";
                            opserr << " could not add Element " << eleNum <<
                                endln;
                            return -1;
                        }
                  }
                else
                  {
                      opserr <<
                          "PartitionedQuick2dFrame::PartitionedQuick2dFrame::Input ";
                      opserr << " ran out of memory creating Element " <<
                          eleNum << endln;
                      return -1;
                  }
                inode++;
                ...
                if (x_rst == 1)
                  {
                      ...
                      if (spPtr != 0)
                            ...
                            if (ok < 0)
                              {
                                  opserr <<
                                      "PartitionedQuick2dFrame::PartitionedQuick2dFrame::Input ";
                                  opserr << " could not add SP_Constraint " <<
                                      nodeNum << endln;
                                  return -1;
                              }
                        }
                      else
                        {
                            opserr <<
                                "PartitionedQuick2dFrame::PartitionedQuick2dFrame::Input ran out of ";
                            opserr << "memory creating SP_Constraint " <<
                                nodeNum << endln;
                            return -1;
                        }
                  }
    ...
    forces (1) = y_ld;
    forces (2) = theta_ld;
    for (i = 1; i < numSubdomainStories; i++)
      {                         // don't add loads at interface nodes
