...
int TclUniaxialMaterialTester_setStrainUniaxialMaterial (ClientData
                                                         clientData,
                                                         Tcl_Interp * interp,
                                                         int argc,
                                                         TCL_Char ** argv);

int TclUniaxialMaterialTester_getStressUniaxialMaterial (ClientData
                                                         clientData,
                                                         Tcl_Interp * interp,
                                                         int argc,
                                                         TCL_Char ** argv);
                                                     ...
                                                     TCL_Char ** argv)
{
    ...
    if (Tcl_GetDouble (interp, argv[1], &strain) != TCL_OK)
      {
          opserr <<
              "WARNING could not read strain: strainUniaxialTest strain? <temp?>\n";
          return TCL_ERROR;
      }
                                                     ...
                                                     TCL_Char ** argv)
{
    ...
    if (theTestingUniaxialMaterial != 0)
      ...
      {
          opserr <<
              "WARNING no active UniaxialMaterial - use uniaxialTest command\n";
          return TCL_ERROR;
      }
}
