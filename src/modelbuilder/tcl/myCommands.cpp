    ...
    if ((strcmp (argv[1], "basic") == 0)
        ...
        || (strcmp (argv[1], "basicBuilder") == 0))
      {
          ...
          while (argPos < argc)
            {
                if (strcmp (argv[argPos], "-ndm") == 0 ||
                    strcmp (argv[argPos], "-NDM") == 0)
                  {
                      argPos++;
                      if (argPos < argc)
                          if (Tcl_GetInt (interp, argv[argPos], &ndm) !=
                              TCL_OK)
                            {
                                opserr << "WARNING error reading ndm: " <<
                                    argv[argPos];
                                opserr <<
                         ...
                         strcmp (argv[argPos], "-NDF") == 0)
                  {
                      argPos++;
                      if (argPos < argc)
                          if (Tcl_GetInt (interp, argv[argPos], &ndf) !=
                              TCL_OK)
                            {
                                opserr << "WARNING error reading ndf: " <<
                                    argv[argPos];
                                opserr <<
                                    "\nmodel modelBuilderType -ndm ndm? <-ndf ndf?>\n";
                                return TCL_ERROR;
          ...
          if (ndf == 0)
            {
                if (ndm == 1)
                  ...
                  {
                      opserr << "WARNING specified ndm, " << ndm <<
                          ", will not work\n";
          ...
          TclModelBuilder *theTclBuilder =
              new TclModelBuilder (theDomain, interp, ndm, ndf);

          if (theTclBuilder != 0)
            {
                OPS_ResetInput (clientData, interp, 0, argc, argv, &theDomain,
                                theTclBuilder);
          ...
          if (theBuilder == 0)
            {
                opserr <<
                    "WARNING ran out of memory in creating BasicBuilder model\n";
                return TCL_ERROR;
            }
      }
             ...
             || (strcmp (argv[1], "UniaxialMaterialTest") == 0))
      {
          ...
          theBuilder =
              new TclUniaxialMaterialTester (theDomain, interp, count);
          if (theBuilder == 0)
            {
                opserr <<
                    "WARNING ran out of memory in creating TclUniaxialMAterialTester model\n";
                return TCL_ERROR;
            }
      }
             ...
             (strcmp (argv[1], "PlaneStressMaterialTest") == 0))
      {
          ...
          theBuilder =
              new TclPlaneStressMaterialTester (theDomain, interp, count);
          if (theBuilder == 0)
            {
                opserr <<
                    "WARNING ran out of memory in creating TclUniaxialMAterialTester model\n";
