...
Block2D::Block2D (int numx, int numy,
                  const ID & nodeID, const Matrix & coorArray, int numNode):
coor (3),
element (numNode),
...
errorFlag (0)
{
    ...
    if (numNodesElement == 9)
      {
          if (((numx % 2) != 0) || ((numy % 2) != 0))
            {
                opserr <<
                    "ERROR: Block2D::Block2D - numX & numY for nine noded elements must be even\n";
                errorFlag = 1;
            }
      }
    ...
    if (numNodesElement != 9 && numNodesElement != 4)
      {
          opserr <<
              "ERROR: Block2D::Block2D - numNode must be either 4 or 9\n";
          errorFlag = 1;
      }
...
void
Block2D::transformNodalCoordinates ()
{
...
//shape functions
void
Block2D::shape2d (double x, double y, double shape[9])
{
    static double Nx[3];
    static double Ny[3];
