...
int TclSectionTester_setStrainSection (ClientData clientData,
                                       Tcl_Interp * interp, int argc,
                                       TCL_Char ** argv);

int TclSectionTester_getStressSection (ClientData clientData,
                                       Tcl_Interp * interp, int argc,
                                       TCL_Char ** argv);

int TclSectionTester_getTangSection (ClientData clientData,
                                     Tcl_Interp * interp, int argc,
                                     TCL_Char ** argv);
...
TclSectionTester::TclSectionTester (Domain & theDomain, Tcl_Interp * interp,
                                    int cTC):
TclModelBuilder (theDomain, interp, 3, 6),
theInterp (interp)
{
    ...
    Tcl_CreateCommand (interp, "strainSectionTest",
                       TclSectionTester_setStrainSection, (ClientData) NULL,
                       NULL);

    Tcl_CreateCommand (interp, "stressSectionTest",
                       TclSectionTester_getStressSection, (ClientData) NULL,
                       NULL);

    Tcl_CreateCommand (interp, "tangSectionTest",
                       TclSectionTester_getTangSection, (ClientData) NULL,
                       NULL);
...
int
TclSectionTester_setSection (ClientData clientData, Tcl_Interp * interp,
                             int argc, TCL_Char ** argv)
{
    count = 1;
    ...
    if (Tcl_GetInt (interp, argv[1], &sectionID) != TCL_OK)
      {
          opserr <<
              "WARNING could not read sectionID: sectionTest sectionID?\n";
          return TCL_ERROR;
      }
    ...
    SectionForceDeformation *theOrigMaterial =
        theTclBuilder->getSection (sectionID);
    ...
    for (int i = 1; i < argc; i++)
      {
          if (Tcl_GetDouble (interp, argv[i], &strain) != TCL_OK)
            {
                opserr <<
                    "WARNING could not read strain: strainSectionTest strain1? strain2? ... strainN?\n";
                return TCL_ERROR;
            }
          data (i - 1) = strain;
      }
...
int
TclSectionTester_getResponseSection (ClientData clientData,
                                     Tcl_Interp * interp, int argc,
                                     TCL_Char ** argv)
{
    // if the section exists, otherwise throw an error
    if (theTestingSection != 0)
      {
          ...
          Response *theResponse =
              theTestingSection->setResponse (argv + 1, argc - 1, dummy);
          if (theResponse == 0)
            {
                return TCL_ERROR;
            }
