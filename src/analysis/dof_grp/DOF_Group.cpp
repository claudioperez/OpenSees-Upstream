...
DOF_Group::DOF_Group(int tag, Node * node):TaggedObject(tag),
unbalance(0),
tangent(0),
myNode(node), myID(node->getNumberDOF()), numDOF(node->getNumberDOF())
{
...
DOF_Group::DOF_Group(int tag, int ndof):TaggedObject(tag),
unbalance(0), tangent(0), myNode(0), myID(ndof), numDOF(ndof)
{
    // get number of DOF & verify valid
    int numDOF = ndof;
    if (numDOF <= 0) {
    ...
    if (myID.Size() != numDOF) {
        opserr << "DOF_Group::DOF_Group(int, int ndof) ";
        opserr << " ran out of memory creating ID of size " << ndof <<
            endln;
        exit(-1);
    }
    // initially set all the IDs to be -2
...
void DOF_Group::setID(const ID & copy)
{
    myID = copy;
}
...
const ID & DOF_Group::getID(void) const
{
    return myID;
}
...
int DOF_Group::getNumDOF(void) const
{
    return numDOF;
}

int DOF_Group::getNodeTag(void) const
{
    if (myNode != 0)
...
int DOF_Group::getNumFreeDOF(void) const
{
...
int DOF_Group::getNumConstrainedDOF(void) const
{
    int numConstr = 0;
    for (int i = 0; i < numDOF; i++)
        if (myID(i) < 0)
...
const Matrix & DOF_Group::getTangent(Integrator * theIntegrator)
{
    if (theIntegrator != 0)
...
void DOF_Group::zeroTangent(void)
{
    tangent->Zero();
}


void DOF_Group::addMtoTang(double fact)
{
    if (myNode != 0) {
...
void DOF_Group::addCtoTang(double fact)
{
    if (myNode != 0) {
...
void DOF_Group::zeroUnbalance(void)
{
    unbalance->Zero();
}


const Vector & DOF_Group::getUnbalance(Integrator * theIntegrator)
{
    if (theIntegrator != 0)
...
void DOF_Group::addPtoUnbalance(double fact)
{
    if (myNode != 0) {
        if (unbalance->addVector(1.0, myNode->getUnbalancedLoad(), fact) <
            0) {
...
void DOF_Group::addPIncInertiaToUnbalance(double fact)
{
    if (myNode != 0) {
        if (unbalance->addVector
            (1.0, myNode->getUnbalancedLoadIncInertia(), fact) < 0) {
        ...
        opserr <<
            "DOF_Group::addPIncInertiaToUnbalance() - no Node associated";
        opserr << " subclass should provide the method \n";
    }
}


void DOF_Group::addM_Force(const Vector & Udotdot, double fact)
{
    if (myNode == 0) {
    ...
    if (unbalance->addMatrixVector(1.0, myNode->getMass(), accel, fact) <
        0) {
...
const Vector & DOF_Group::getTangForce(const Vector & Udotdot, double fact)
{
...
const Vector & DOF_Group::getM_Force(const Vector & Udotdot, double fact)
{
    if (myNode == 0) {
    ...
    if (unbalance->addMatrixVector(0.0, myNode->getMass(), accel, fact) <
        0) {
...
const Vector & DOF_Group::getC_Force(const Vector & Udotdot, double fact)
{
    if (myNode == 0) {
    ...
    if (unbalance->addMatrixVector(0.0, myNode->getDamp(), accel, fact) <
        0) {
...
const Vector & DOF_Group::getCommittedDisp(void)
{
    if (myNode == 0) {
...
const Vector & DOF_Group::getCommittedVel(void)
{
    if (myNode == 0) {
...
const Vector & DOF_Group::getCommittedAccel(void)
{
    if (myNode == 0) {
...
void DOF_Group::setNodeDisp(const Vector & u)
{
    if (myNode == 0) {
...
void DOF_Group::setNodeVel(const Vector & udot)
{
...
void DOF_Group::setNodeAccel(const Vector & udotdot)
{
...
void DOF_Group::incrNodeDisp(const Vector & u)
{
    if (myNode == 0) {
...
void DOF_Group::incrNodeVel(const Vector & udot)
{
...
void DOF_Group::incrNodeAccel(const Vector & udotdot)
{
...
const Vector & DOF_Group::getTrialDisp()
{
    if (myNode == 0) {
...
const Vector & DOF_Group::getTrialVel()
{
    if (myNode == 0) {
...
const Vector & DOF_Group::getTrialAccel()
{
    if (myNode == 0) {
...
void DOF_Group::setEigenvector(int mode, const Vector & theVector)
{
...
const Matrix & DOF_Group::getEigenvectors(void)
{
    if (myNode == 0) {
        opserr << "DOF_Group::setNodeAccel: 0 Node Pointer\n";
        exit(-1);
...
void DOF_Group::addLocalM_Force(const Vector & accel, double fact)
{
    if (myNode != 0) {
        if (unbalance->addMatrixVector(1.0, myNode->getMass(), accel, fact)
            < 0) {
...
const Vector & DOF_Group::getDispSensitivity(int gradNumber)
{
...
const Vector & DOF_Group::getVelSensitivity(int gradNumber)
{
    Vector & result = *unbalance;
    for (int i = 0; i < numDOF; i++) {
...
const Vector & DOF_Group::getAccSensitivity(int gradNumber)
{
    Vector & result = *unbalance;
    for (int i = 0; i < numDOF; i++) {
...
int DOF_Group::saveDispSensitivity(const Vector & v, int gradNum,
                                   int numGrads)
{
...
int DOF_Group::saveVelSensitivity(const Vector & v, int gradNum,
                                  int numGrads)
{
    Vector & dudh = *unbalance;
...
int DOF_Group::saveAccSensitivity(const Vector & v, int gradNum,
                                  int numGrads)
{
    Vector & dudh = *unbalance;
...
int DOF_Group::saveSensitivity(const Vector & v, const Vector & vdot,
                               const Vector & vdotdot, int gradNum,
                               int numGrads)
{
    int ok = 0;
...
void DOF_Group::addM_ForceSensitivity(const Vector & Udotdot, double fact)
{
    if (myNode == 0) {
        opserr << "DOF_Group::addM_Force() - no Node associated";
        opserr << " subclass should not call this method \n";
    ...
    if (unbalance->addMatrixVector
        (1.0, myNode->getMassSensitivity(), accel, fact) < 0) {
        opserr << "DOF_Group::addM_Force() ";
        opserr << " invoking addMatrixVector() on the unbalance failed\n";
    } else {
...
void DOF_Group::addD_Force(const Vector & Udot, double fact)
{
    if (myNode == 0) {
...
void DOF_Group::addD_ForceSensitivity(const Vector & Udot, double fact)
{
    if (myNode == 0) {
        opserr <<
            "DOF_Group::addD_ForceSensitivity() - no Node associated";
        opserr << " subclass should not call this method \n";
        return;
    }
    ...
    if (unbalance->addMatrixVector
        (1.0, myNode->getDampSensitivity(), vel, fact) < 0) {
...
void DOF_Group::resetNodePtr(void)
{
    myNode = 0;
}


double DOF_Group::getDampingBetaFactor(int mode, double ratio, double wn)
{
...
const Vector & DOF_Group::getDampingBetaForce(int mode, double beta)
{
