...
int
 TransformationDOF_Group::numTransDOFs(0);      // number of objects
...
TransformationDOF_Group::TransformationDOF_Group(int tag, Node * node,
                                                 ...
                                                 *
                                                 theTHandler):DOF_Group
    (tag, node), theMP(mp), Trans(0), modTangent(0), modUnbalance(0),
modID(0), theSPs(0)
{
    ...
    *******************/
    ...
    if (modID == 0 || modID->Size() == 0 ||
        ...
        opserr <<
            "FATAL TransformationDOF_Group::TransformationDOF_Group() -";
    ...
    if (numTransDOFs == 0) {
        ...
        if (modMatrices == 0 || modVectors == 0) {
            opserr <<
                "TransformationDOF_Group::TransformationDOF_Group(Node *) ";
...
TransformationDOF_Group::TransformationDOF_Group(int tag,
                                                 ...
                                                 *
                                                 theTHandler):DOF_Group
    (tag, node), theMP(0), Trans(0), modTangent(0), modUnbalance(0),
modID(0), theSPs(0)
{
...
const ID & TransformationDOF_Group::getID(void) const
{
    if (modID != 0)
        return *modID;
    else
...
int TransformationDOF_Group::getNumDOF(void) const
{
    return modNumDOF;
}


int TransformationDOF_Group::getNumFreeDOF(void) const
{
    if (modID != 0) {
...
int TransformationDOF_Group::getNumConstrainedDOF(void) const
{
    if (modID != 0) {
        int numConstr = 0;
        for (int i = 0; i < modNumDOF; i++)
    ...
    TransformationDOF_Group::getTangent(Integrator * theIntegrator)
{
    ...
    const Matrix & unmodTangent =
        this->DOF_Group::getTangent(theIntegrator);
    Matrix *T = this->getT();
    if (T != 0) {
...
const Matrix & TransformationDOF_Group::getEigenvectors(void)
{
    ...
    TransformationDOF_Group::getUnbalance(Integrator * theIntegrator)
{
    ...
    if (T != 0) {
        ...
        modUnbalance->addMatrixTransposeVector(0.0, *T, unmodUnbalance,
                                               1.0);
        return *modUnbalance;
    } else
...
const Vector & TransformationDOF_Group::getCommittedDisp(void)
{
...
const Vector & TransformationDOF_Group::getCommittedVel(void)
{
...
const Vector & TransformationDOF_Group::getCommittedAccel(void)
{
...
void TransformationDOF_Group::setNodeDisp(const Vector & u)
{
    ...
    if (needRetainedData == 0) {
        ...
        for (int i = numConstrainedNodeRetainedDOF, j = 0; i < modNumDOF;
             i++, j++) {
            int loc = theID(i);
            if (loc < 0)
...
void TransformationDOF_Group::setNodeVel(const Vector & u)
{
    // call base class method and return if no MP_Constraint
    if (theMP == 0) {
...
void TransformationDOF_Group::setNodeAccel(const Vector & u)
{
    // call base class method and return if no MP_Constraint
    if (theMP == 0) {
...
void TransformationDOF_Group::incrNodeDisp(const Vector & u)
{
    // call base class method and return if no MP_Constraint
    if (theMP == 0) {
...
void TransformationDOF_Group::incrNodeVel(const Vector & u)
{
    // call base class method and return if no MP_Constraint
    if (theMP == 0) {
...
void TransformationDOF_Group::incrNodeAccel(const Vector & u)
{
    // call base class method and return if no MP_Constraint
    if (theMP == 0) {
...
const Vector & TransformationDOF_Group::getTrialDisp(void)
{
...
const Vector & TransformationDOF_Group::getTrialVel(void)
{
...
const Vector & TransformationDOF_Group::getTrialAccel(void)
{
...
void TransformationDOF_Group::setEigenvector(int mode, const Vector & u)
{
    // call base class method and return if no MP_Constraint
    if (theMP == 0) {
...
Matrix *TransformationDOF_Group::getT(void)
{
    if (theMP == 0)
        return 0;
...
int TransformationDOF_Group::enforceSPs(int doMP)
{
    int numDof = myNode->getNumberDOF();
    ...
    if (doMP == 1) {
        ...
        if (needRetainedData == 0) {
            ...
            if (theMP != 0) {
                ...
                modUnbalance->Zero();
                for (int i = numConstrainedNodeRetainedDOF, j = 0;
                     i < modNumDOF; i++, j++) {
                    int loc = theID(i);
                    if (loc < 0)
                        (*modUnbalance) (i) = responseR(retainedDOF(j));
                ...
                if (T != 0) {
                    ...
                    // *unbalance = (*T) * (*modUnbalance);
                    unbalance->addMatrixVector(0.0, *T,
                                               *modUnbalance, 1.0);

                    const ID & constrainedDOF =
                        theMP->getConstrainedDOFs();
...
const Vector &
    TransformationDOF_Group::getM_Force(const Vector & Udotdot,
                                        double fact)
{
    // call base class method and return if no MP_Constraint
    if (theMP == 0 || modID == 0) {
...
const Vector &
    TransformationDOF_Group::getC_Force(const Vector & Udotdot,
                                        double fact)
{
    opserr <<
        "TransformationDOF_Group::getC_Force() - not yet implemented\n";
    return *modUnbalance;
}

const Vector &
    TransformationDOF_Group::getTangForce(const Vector & Udotdot,
                                          double fact)
{
    opserr <<
        "TransformationDOF_Group::getTangForce() - not yet implemented\n";
    return *modUnbalance;
}
...
const Vector & TransformationDOF_Group::getDispSensitivity(int gradNumber)
{
    const Vector & result =
        this->DOF_Group::getDispSensitivity(gradNumber);
...
const Vector & TransformationDOF_Group::getVelSensitivity(int gradNumber)
{
...
const Vector & TransformationDOF_Group::getAccSensitivity(int gradNumber)
{
...
int TransformationDOF_Group::saveDispSensitivity(const Vector & u,
...
int TransformationDOF_Group::saveVelSensitivity(const Vector & u,
                                                int gradNum, int numGrads)
{
    // call base class method and return if no MP_Constraint
    if (theMP == 0) {
...
int TransformationDOF_Group::saveAccSensitivity(const Vector & u,
                                                int gradNum, int numGrads)
{
    // call base class method and return if no MP_Constraint
    if (theMP == 0) {
...
int TransformationDOF_Group::saveSensitivity(const Vector & u,
                                             const Vector & udot,
                                             ...
                                             int gradNum, int numGrads)
{
    // call base class method and return if no MP_Constraint
    if (theMP == 0) {
        return this->DOF_Group::saveSensitivity(u, udot, udotdot, gradNum,
                                                numGrads);
    }
...
void TransformationDOF_Group::addD_Force(const Vector & Udot, double fact)
{
    // call base class method and return if no MP_Constraint
    if (theMP == 0 || modID == 0) {
...
void TransformationDOF_Group::addD_ForceSensitivity(const Vector & Udot,
                                                    double fact)
{
    // call base class method and return if no MP_Constraint
    if (theMP == 0 || modID == 0) {
