...
Matrix **
    TransformationDOF_Group::modMatrices;
Vector **
    TransformationDOF_Group::modVectors;
int
TransformationDOF_Group::numTransDOFs (0);      // number of objects
TransformationConstraintHandler *
    TransformationDOF_Group::theHandler = 0;    // number of objects
                ...
                if (theMP != 0)
                  {
                      ...
                      int retainedNode = theMP->getNodeRetained ();
                      Domain *theDomain = myNode->getDomain ();
                      Node *retainedNodePtr =
                          theDomain->getNode (retainedNode);
                      const Vector & responseR =
                          retainedNodePtr->getTrialDisp ();
                      const ID & retainedDOF = theMP->getRetainedDOFs ();
                            ...
                            for (int i = 0; i < constrainedDOF.Size (); i++)
                              {
                                  ...
                                  myNode->setTrialDisp ((*unbalance) (cDOF),
                                                        cDOF);
                              }
                        }
                  }
...
void
TransformationDOF_Group::addM_ForceSensitivity (const Vector & Udotdot,
                                                double fact)
{
    // call base class method and return if no MP_Constraint
    if (theMP == 0 || modID == 0)
...
void
TransformationDOF_Group::addD_ForceSensitivity (const Vector & Udot,
                                                double fact)
{
    // call base class method and return if no MP_Constraint
    if (theMP == 0 || modID == 0)
