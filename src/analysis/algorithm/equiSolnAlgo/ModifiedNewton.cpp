...
// Constructor
ModifiedNewton::ModifiedNewton(int theTangentToUse, double iFact,
                               double
                               cFact):EquiSolnAlgo
    (EquiALGORITHM_TAGS_ModifiedNewton), tangent(theTangentToUse),
 ...
 ModifiedNewton::solveCurrentStep(void)
{
    ...
    if ((theAnalysisModel == 0) || (theIncIntegratorr == 0)
        ...
        || (theTest == 0)) {
        opserr <<
            "WARNING ModifiedNewton::solveCurrentStep() - setLinks() has";
        opserr <<
...
int ModifiedNewton::sendSelf(int cTag, Channel & theChannel)
{
...
int ModifiedNewton::recvSelf(int cTag,
                             Channel & theChannel,
...
void ModifiedNewton::Print(OPS_Stream & s, int flag)
{
    if (flag == 0) {
        s << "ModifiedNewton";
    }
...
int ModifiedNewton::getNumIterations(void)
{
