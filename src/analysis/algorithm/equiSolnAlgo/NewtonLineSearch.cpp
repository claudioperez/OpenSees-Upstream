...
//Null Constructor
NewtonLineSearch::NewtonLineSearch():EquiSolnAlgo(EquiALGORITHM_TAGS_NewtonLineSearch),
theTest(0), theOtherTest(0),
theLineSearch(0)
{
}
...
//Constructor 
NewtonLineSearch::NewtonLineSearch(ConvergenceTest & theT, LineSearch * theSearch):EquiSolnAlgo(EquiALGORITHM_TAGS_NewtonLineSearch),
theTest(&theT),
...
int NewtonLineSearch::solveCurrentStep(void)
{
    ...
    if ((theAnaModel == 0) || (theIntegrator == 0) || (theSOE == 0)
        || (theTest == 0)) {
        opserr <<
            "WARNING NewtonLineSearch::solveCurrentStep() - setLinks() has";
        opserr <<
...
ConvergenceTest *NewtonLineSearch::getConvergenceTest(void)
{
    return theTest;
}

int NewtonLineSearch::sendSelf(int cTag, Channel & theChannel)
{
    static ID data(1);
    data(0) = theLineSearch->getClassTag();
    if (theChannel.sendID(0, cTag, data) < 0) {
        opserr <<
            "NewtonLineSearch::sendSelf(int cTag, Channel &theChannel)   - failed to send date\n";
        return -1;
    }

    if (theLineSearch->sendSelf(cTag, theChannel) < 0) {
        opserr <<
            "NewtonLineSearch::sendSelf(int cTag, Channel &theChannel)   - failed to send line search\n";
        return -1;
    }
...
int NewtonLineSearch::recvSelf(int cTag,
                               Channel & theChannel,
                               FEM_ObjectBroker & theBroker)
{
    ...
    if (theChannel.recvID(0, cTag, data) < 0) {
        opserr <<
            "NewtonLineSearch::recvSelf(int cTag, Channel &theChannel) - failed to recv data\n";
        return -1;
    }
    ...
    if (theLineSearch == 0
        || theLineSearch->getClassTag() != lineSearchClassTag) {
        if (theLineSearch != 0)
        ...
        if (theLineSearch == 0) {
            opserr <<
                "NewtonLineSearch::recvSelf(int cTag, Channel &theChannel) - failed to obtain a LineSerach object\n";
            return -1;
        }
    }
    ...
    if (theLineSearch->recvSelf(cTag, theChannel, theBroker) < 0) {
        opserr <<
            "NewtonLineSearch::recvSelf(int cTag, Channel &theChannel) - failed to recv the LineSerach object\n";
        return -1;
    }
...
void NewtonLineSearch::Print(OPS_Stream & s, int flag)
{
    if (flag == 0)
        s << "NewtonLineSearch\n";
