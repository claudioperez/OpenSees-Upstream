...
// Constructor
PeriodicNewton::PeriodicNewton(int theTangentToUse,
                               int
                               mc):EquiSolnAlgo
    (EquiALGORITHM_TAGS_PeriodicNewton), tangent(theTangentToUse),
maxCount(mc)
{
...
PeriodicNewton::PeriodicNewton(ConvergenceTest & theT, int theTangentToUse,
                               int
                               mc):EquiSolnAlgo
    (EquiALGORITHM_TAGS_PeriodicNewton), tangent(theTangentToUse),
maxCount(mc)
 ...
 PeriodicNewton::solveCurrentStep(void)
{
    ...
    if ((theAnalysisModel == 0) || (theIncIntegratorr == 0)
        || (theSOE == 0)
        || (theTest == 0)) {
        opserr <<
            "WARNING PeriodicNewton::solveCurrentStep() - setLinks() has";
        opserr <<
...
int PeriodicNewton::sendSelf(int cTag, Channel & theChannel)
{
    int result = 0;
    ...
    if (result != 0) {
        opserr <<
            "PeriodicNewton::sendSelf() - failed to send CTest object\n";
        return result;
    }
...
int PeriodicNewton::recvSelf(int cTag,
                             Channel & theChannel,
                             FEM_ObjectBroker & theBroker)
{
    ...
    if (result != 0) {
        opserr <<
            "PeriodicNewton::recvSelf() - failed to recv CTest object\n";
        return result;
    }
...
void PeriodicNewton::Print(OPS_Stream & s, int flag)
{
    if (flag == 0) {
