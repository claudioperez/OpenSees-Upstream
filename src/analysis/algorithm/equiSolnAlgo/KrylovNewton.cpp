...
KrylovNewton::KrylovNewton(int theTangentToUse,
                           int
                           maxDim):EquiSolnAlgo
    (EquiALGORITHM_TAGS_KrylovNewton), tangent(theTangentToUse), v(0),
Av(0), AvData(0), rData(0), work(0), lwork(0), numEqns(0),
maxDimension(maxDim)
{
    if (maxDimension < 0)
        maxDimension = 0;
 ...
 KrylovNewton::solveCurrentStep(void)
{
    ...
    if ((theAnaModel == 0) || (theIntegrator == 0) || (theSOE == 0)
        || (theTest == 0)) {
        opserr <<
            "WARNING KrylovNewton::solveCurrentStep() - setLinks() has";
        opserr <<
    ...
    do {
        ...
        if (dim > maxDimension) {
            ...
            if (theIntegrator->formTangent(tangent) < 0) {
                opserr << "WARNING KrylovNewton::solveCurrentStep() -";
                opserr <<
                    "the Integrator failed to produce new formTangent()\n";
                return -1;
            }
        }
...
int KrylovNewton::sendSelf(int cTag, Channel & theChannel)
{
    static ID data(2);
    data(0) = tangent;
    data(1) = maxDimension;
...
int KrylovNewton::recvSelf(int cTag, Channel & theChannel,
...
void KrylovNewton::Print(OPS_Stream & s, int flag)
{
    s << "KrylovNewton";
...
int KrylovNewton::leastSquares(int k)
{
    LinearSOE *theSOE = this->getLinearSOEptr();
...
#ifdef _WIN32
    DGELS(trans, &numEqns, &k, &nrhs, AvData, &numEqns, rData, &ldb, work,
          &lwork, &info);
#else
    dgels_(trans, &numEqns, &k, &nrhs, AvData, &numEqns, rData, &ldb, work,
           &lwork, &info);
#endif
