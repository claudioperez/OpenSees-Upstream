...
// Constructor
NewtonRaphson::NewtonRaphson(int theTangentToUse, double iFact,
                             double
                             cFact):EquiSolnAlgo
    (EquiALGORITHM_TAGS_NewtonRaphson), tangent(theTangentToUse),
iFactor(iFact), cFactor(cFact)
{

}

NewtonRaphson::NewtonRaphson():EquiSolnAlgo(EquiALGORITHM_TAGS_NewtonRaphson),
tangent(CURRENT_TANGENT), iFactor(0.),
cFactor(1.)
{
 ...
 NewtonRaphson::solveCurrentStep(void)
{
    ...
    if ((theAnaModel == 0) || (theIntegrator == 0) || (theSOE == 0)
        || (theTest == 0)) {
        opserr <<
            "WARNING NewtonRaphson::solveCurrentStep() - setLinks() has";
        opserr <<
        ...
        if (tangent == INITIAL_THEN_CURRENT_TANGENT) {
            if (numIterations == 0) {
                ...
                if (theIntegrator->formTangent(INITIAL_TANGENT) < 0) {
                    opserr <<
                        "WARNING NewtonRaphson::solveCurrentStep() -";
...
int NewtonRaphson::sendSelf(int cTag, Channel & theChannel)
{
...
int NewtonRaphson::recvSelf(int cTag,
                            Channel & theChannel,
...
void NewtonRaphson::Print(OPS_Stream & s, int flag)
{
    if (flag == 0) {
...
int NewtonRaphson::getNumIterations(void)
{
