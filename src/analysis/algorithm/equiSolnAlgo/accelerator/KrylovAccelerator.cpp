...
KrylovAccelerator::KrylovAccelerator(int max,
                                     int
                                     tangent):Accelerator
...
KrylovAccelerator::~KrylovAccelerator()
{
    if (v != 0) {
...
int KrylovAccelerator::accelerate(Vector & vStar, LinearSOE & theSOE,
...
int KrylovAccelerator::updateTangent(IncrementalIntegrator & theIntegrator)
{
...
bool KrylovAccelerator::updateTangent(void)
{
    if (dimension > maxDimension) {
        dimension = 0;
        return true;
...
void KrylovAccelerator::Print(OPS_Stream & s, int flag)
{
    s << "KrylovAccelerator" << endln;
    s << "\tMax subspace dimension: " << maxDimension << endln;
}

int KrylovAccelerator::sendSelf(int commitTag, Channel & theChannel)
{
    static ID data(2);
    data(0) = theTangent;
    data(1) = maxDimension;
...
int KrylovAccelerator::recvSelf(int commitTag, Channel & theChannel,
