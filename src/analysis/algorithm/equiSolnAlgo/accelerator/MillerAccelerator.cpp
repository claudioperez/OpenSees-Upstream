...
MillerAccelerator::MillerAccelerator(int max, double tol,
                                     int
                                     tangent):Accelerator
    (ACCELERATOR_TAGS_Miller), iteration(0), numEqns(0), dimension(0),
maxDimension(max), tolerance(tol), work(0), fData(0), theTangent(tangent)
{
    if (maxDimension < 1)
        maxDimension = 1;
...
int MillerAccelerator::accelerate(Vector & vStar, LinearSOE & theSOE,
...
int MillerAccelerator::updateTangent(IncrementalIntegrator & theIntegrator)
{
    /*
...
bool MillerAccelerator::updateTangent(void)
{
    if (dimension >= maxDimension) {
        dimension = 0;
        iteration = 1;          // reset Newton iteration if tangent is formed
...
void MillerAccelerator::Print(OPS_Stream & s, int flag)
{
...
int MillerAccelerator::sendSelf(int commitTag, Channel & theChannel)
{
    return -1;
}

int MillerAccelerator::recvSelf(int commitTag, Channel & theChannel,
