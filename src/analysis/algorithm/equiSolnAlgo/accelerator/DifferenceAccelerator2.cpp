...
DifferenceAccelerator2::DifferenceAccelerator2(int max,
                                               int
                                               tangent):Accelerator
    (ACCELERATOR_TAGS_Difference), dimension(0), numEqns(0),
maxDimension(max), v(0), Av(0), AvData(0), rData(0), work(0), lwork(0),
theTangent(tangent)
{
    if (maxDimension < 0)
        maxDimension = 0;
...
int DifferenceAccelerator2::accelerate(Vector & vStar, LinearSOE & theSOE,
...
void DifferenceAccelerator2::Print(OPS_Stream & s, int flag)
{
    s << "DifferenceAccelerator2" << endln;
    s << "\tMax subspace dimension: " << maxDimension << endln;
}

int DifferenceAccelerator2::sendSelf(int commitTag, Channel & theChannel)
{
    return -1;
}

int DifferenceAccelerator2::recvSelf(int commitTag, Channel & theChannel,
