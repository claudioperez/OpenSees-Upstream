...
// Constructor
AcceleratedNewton::
AcceleratedNewton(int
                  theTangentToUse):EquiSolnAlgo
(EquiALGORITHM_TAGS_AcceleratedNewton), theTest(0), tangent(theTangentToUse),
theAccelerator(0), vAccel(0), numFactorizations(0), numIterations(0)
...
AcceleratedNewton::AcceleratedNewton(ConvergenceTest & theT,
                                     ...
                                     int
                                     theTangentToUse):EquiSolnAlgo
    (EquiALGORITHM_TAGS_AcceleratedNewton), theTest(&theT),
tangent(theTangentToUse), theAccelerator(theAccel), vAccel(0),
...
int AcceleratedNewton::solveCurrentStep(void)
{
    ...
    if ((theAnaModel == 0) || (theIntegrator == 0) || (theSOE == 0)
        || (theTest == 0)) {
        opserr <<
            "WARNING AcceleratedNewton::solveCurrentStep() - setLinks() has";
        opserr <<
...
//      accelTimer.start();
            if (theAccelerator->accelerate
                (*vAccel, *theSOE, *theIntegrator) < 0) {
                opserr <<
                    "WARNING AcceleratedNewton::solveCurrentStep() -";
            ...
            if (theAccelerator != 0) {
                ...
                if (ret < 0) {
                    opserr <<
                        "WARNING AcceleratedNewton::solveCurrentStep() -";
                    opserr <<
                        "the Accelerator failed in updateTangent()\n";
                    return -1;
                }
                if (ret > 0)
                    numFactorizations++;
...
ConvergenceTest *AcceleratedNewton::getTest(void)
{
    return theTest;
}

int AcceleratedNewton::sendSelf(int cTag, Channel & theChannel)
{
    static ID data(2);
    data(0) = tangent;
    if (theAccelerator != 0)
    ...
    if (theAccelerator != 0) {
        ...
        if (res < 0) {
            opserr <<
                "AcceleratedNewton::recvSelf() - accelerator to send\n";
            return -1;
        }
    }
...
int AcceleratedNewton::recvSelf(int cTag, Channel & theChannel,
                                FEM_ObjectBroker & theBroker)
{
    ...
    if (data(1) != -1) {
        ...
        if (theAccelerator == 0) {
            opserr <<
                "AcceleratedNewton::recvSelf() - no acccelerator of classTag "
                << data(1) << " exists\n";
            return -1;
        }
        ...
        if (res == 0) {
            ...
            if (res < 0) {
                opserr <<
                    "AcceleratedNewton::recvSelf() - accelerator failed to recvSelf\n";
                return -1;
            }
        }
    }
...
void AcceleratedNewton::Print(OPS_Stream & s, int flag)
{
