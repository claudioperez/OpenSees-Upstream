...
StaticDomainDecompositionAnalysis::StaticDomainDecompositionAnalysis(Subdomain & the_Domain):DomainDecompositionAnalysis
...
theSOE(&theLinSOE), theEigenSOE(0), theIntegrator(&theStaticIntegrator),
...
bool StaticDomainDecompositionAnalysis::doesIndependentAnalysis(void)
{
    return true;
}


int StaticDomainDecompositionAnalysis::analyze(double dT)
{
    int result = 0;
    ...
    if (stamp != domainStamp) {
        ...
        if (result < 0) {
            opserr <<
                "StaticDomainDecompositionAnalysis::analyze() - domainChanged failed";
            return -1;
        }
    }
    ...
    if (result < 0) {
        opserr <<
            "StaticDomainDecompositionAnalysis::analyze() - the AnalysisModel failed";
...
//  result = this->checkAllResult(result);
    ...
    if (result < 0) {
        opserr <<
            "StaticDomainDecompositionAnalysis::analyze() - the Integrator failed";
        opserr << " with domain at load factor ";
        opserr << the_Domain->getCurrentTime() << endln;
        the_Domain->revertToLastCommit();
    ...
    if (result < 0) {
        opserr <<
            "StaticDomainDecompositionAnalysis::analyze() - the Algorithm failed";
        opserr << " with domain at load factor ";
        opserr << the_Domain->getCurrentTime() << endln;
        the_Domain->revertToLastCommit();
        theIntegrator->revertToLastStep();
...
int StaticDomainDecompositionAnalysis::eigen(int numMode, bool generalized,
                                             bool findSmallest)
{
    int result = 0;
    Domain *the_Domain = this->getDomainPtr();
    ...
    if (theEigenSOE == 0) {
        opserr <<
            "StaticDomainDecompositionAnalysis::eigen() - no eigen solver has been set\n";
        return -1;
    }
    // check for change in Domain since last step. As a change can
    // occur in a commit() in a domaindecomp with load balancing
    ...
    if (stamp != domainStamp) {
        ...
        if (result < 0) {
            opserr <<
                "StaticDomainDecompositionAnalysis::eigen() - domainChanged failed";
            return -1;
        }
    }
    //
...
int StaticDomainDecompositionAnalysis::initialize(void)
{
    Domain *the_Domain = this->getDomainPtr();
...
int StaticDomainDecompositionAnalysis::domainChanged(void)
{
    Domain *the_Domain = this->getDomainPtr();
    int stamp = the_Domain->hasDomainChanged();
    domainStamp = stamp;
...
int StaticDomainDecompositionAnalysis::getNumExternalEqn(void)
{
    opserr <<
        "StaticDomainDecompositionAnalysis::getNumExternalEqn() - should never be called\n";
    return 0;
}

int StaticDomainDecompositionAnalysis::getNumInternalEqn(void)
{
    opserr <<
        "StaticDomainDecompositionAnalysis::getNumInternalEqn() - should never be called\n";
    return 0;
}

int StaticDomainDecompositionAnalysis::analysisStep(double dT)
{
    this->analyze(dT);
    return 0;
}

int StaticDomainDecompositionAnalysis::eigenAnalysis(int numMode,
                                                     bool generalized,
                                                     bool findSmallest)
{
...
int StaticDomainDecompositionAnalysis::computeInternalResponse(void)
{
    opserr <<
        "StaticDomainDecompositionAnalysis::computeInternalResponse() - should never be called\n";
    return 0;
}

int StaticDomainDecompositionAnalysis::formTangent(void)
{
    opserr <<
        "StaticDomainDecompositionAnalysis::formTangent() - should never be called\n";
    return 0;
}

int StaticDomainDecompositionAnalysis::formResidual(void)
{
    opserr <<
        "StaticDomainDecompositionAnalysis::formResidual() - should never be called\n";
    return 0;
}

int StaticDomainDecompositionAnalysis::formTangVectProduct(Vector & force)
{
    opserr <<
        "StaticDomainDecompositionAnalysis::formTangVectProduct() - should never be called\n";
    return 0;
}

const Matrix & StaticDomainDecompositionAnalysis::getTangent(void)
{
    static Matrix errMatrix;
    opserr <<
        "StaticDomainDecompositionAnalysis::getTangent() - should never be called\n";
    return errMatrix;
}

const Vector & StaticDomainDecompositionAnalysis::getResidual(void)
{
    static Vector errVector;
    opserr <<
        "StaticDomainDecompositionAnalysis::getResidual() - should never be called\n";
    return errVector;
}

const Vector & StaticDomainDecompositionAnalysis::getTangVectProduct(void)
{
    static Vector errVector;
    opserr <<
        "StaticDomainDecompositionAnalysis::getTangVectProduct() - should never be called\n";
    return errVector;
}

int StaticDomainDecompositionAnalysis::sendSelf(int commitTag,
                                                Channel & theChannel)
{
    ...
    if (theAlgorithm == 0) {
        opserr <<
            "StaticDomainDecompositionAnalysis::sendSelf() - no objects exist!\n";
        return -1;
    }
    ...
    if (theConstraintHandler->sendSelf(commitTag, theChannel) != 0) {
        opserr <<
            "StaticDomainDecompositionAnalysis::sendSelf() - failed to send handler\n";
        return -1;
    }

    if (theDOF_Numberer->sendSelf(commitTag, theChannel) != 0) {
        opserr <<
            "StaticDomainDecompositionAnalysis::sendSelf() - failed to send numberer\n";
        return -1;
    }

    if (theAnalysisModel->sendSelf(commitTag, theChannel) != 0) {
        opserr <<
            "StaticDomainDecompositionAnalysis::sendSelf() - failed to send model\n";
        return -1;
    }

    if (theAlgorithm->sendSelf(commitTag, theChannel) != 0) {
        opserr <<
            "StaticDomainDecompositionAnalysis::sendSelf() - failed to send algorithm\n";
        return -1;
    }

    if (theSOE->sendSelf(commitTag, theChannel) != 0) {
        opserr <<
            "StaticDomainDecompositionAnalysis::sendSelf() - failed to send SOE\n";
        return -1;
    } else;
    ...
    if (theSolver->sendSelf(commitTag, theChannel) != 0) {
        opserr <<
            "StaticDomainDecompositionAnalysis::sendSelf() - failed to send Solver\n";
        return -1;
    }

    if (theIntegrator->sendSelf(commitTag, theChannel) != 0) {
        opserr <<
            "StaticDomainDecompositionAnalysis::sendSelf() - failed to send integrator\n";
        return -1;
    }
...
int StaticDomainDecompositionAnalysis::recvSelf(int commitTag,
                                                Channel & theChannel,
                                                FEM_ObjectBroker &
                                                theBroker)
{
...
int StaticDomainDecompositionAnalysis::
...
int StaticDomainDecompositionAnalysis::setLinearSOE(LinearSOE & theNewSOE)
{
    // invoke the destructor on the old one
    if (theSOE != 0)
        delete theSOE;
...
int StaticDomainDecompositionAnalysis::setEigenSOE(EigenSOE & theNewSOE)
{
