...
EigenAnalysis::EigenAnalysis(Domain & the_Domain, ConstraintHandler & theHandler, DOF_Numberer & theNumberer, AnalysisModel & theModel, EigenAlgorithm & theAlgo, EigenSOE & theEigenSOE, EigenIntegrator & theEigenIntegrator):Analysis(the_Domain), theConstraintHandler(&theHandler),
...
int EigenAnalysis::analyze(int numModes)
{
    int result = 0;
...
int EigenAnalysis::domainChanged()
{
...
int EigenAnalysis::setAlgorithm(EigenAlgorithm & theAlgo)
{
    opserr << "EigenAnalysis::setAlgorithm() - does nothing yet\n";
    return 0;
}

int EigenAnalysis::setIntegrator(EigenIntegrator & theIntegrator)
{
    opserr << "EigenAnalysis::setIntegrator() - does nothing yet\n";
    return 0;
}

int EigenAnalysis::setEigenSOE(EigenSOE & theSOE)
{
    opserr << "EigenAnalysis::setEigenSOE() - does nothing yet\n";
    return 0;
}

ConstraintHandler *EigenAnalysis::getConstraintHandlerPtr() const
{
    return theConstraintHandler;
}

DOF_Numberer *EigenAnalysis::getDOF_NumbererPtr() const
{
    return theDOF_Numberer;
}

AnalysisModel *EigenAnalysis::getAnalysisModelPtr() const
{
    return theAnalysisModel;
}

EigenAlgorithm *EigenAnalysis::getEigenAlgorithm() const
{
    return theAlgorithm;
}

EigenSOE *EigenAnalysis::getEigenSOE() const
{
    return theSOE;
}

EigenIntegrator *EigenAnalysis::getEigenIntegrator() const
{
