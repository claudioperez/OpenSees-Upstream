...
TransientDomainDecompositionAnalysis (Subdomain & the_Domain,
    ...
    while ((elePtr = theEles ()) != 0)
      {
          ...
          if (theEigenSOE->addA (elePtr->getTangent (0), elePtr->getID ()) <
              0)
            {
          ...
          while ((elePtr = theEles2 ()) != 0)
            {
                ...
                if (theEigenSOE->
                    addM (elePtr->getTangent (0), elePtr->getID ()) < 0)
                  {
                      opserr <<
          ...
          while ((dofPtr = theDofs ()) != 0)
            {
                ...
                if (theEigenSOE->
                    addM (dofPtr->getTangent (0), dofPtr->getID ()) < 0)
                  {
                      opserr <<
                          "WARNING TransientDomainDecomposition::eigen() -";
...
int
TransientDomainDecompositionAnalysis::eigenAnalysis (int numMode,
                                                     bool generalized,
                                                     bool findSmallest)
{
...
int
TransientDomainDecompositionAnalysis::recvSelf (int commitTag,
                                                Channel & theChannel,
          ...
          if (data (7) != -1)
            {
                ...
                if (theTest == 0)
                  {
                      opserr <<
                          "TransientDomainDecompositionAnalysis::recvSelf";
...
setIntegrator (IncrementalIntegrator & theNewIntegrator)
{
    ...
    if (theIntegrator != 0 && theConstraintHandler != 0 && theAlgorithm != 0
        && theAnalysisModel != 0 && theSOE != 0)
      {
          theIntegrator->setLinks (*theAnalysisModel, *theSOE, theTest);
          theConstraintHandler->setLinks (*the_Domain, *theAnalysisModel,
                                          *theIntegrator);
