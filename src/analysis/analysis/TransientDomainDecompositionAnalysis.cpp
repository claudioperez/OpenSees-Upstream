...
TransientDomainDecompositionAnalysis::TransientDomainDecompositionAnalysis(Subdomain & the_Domain):DomainDecompositionAnalysis
    (ANALYSIS_TAGS_TransientDomainDecompositionAnalysis, the_Domain),
theConstraintHandler(0), theDOF_Numberer(0), theAnalysisModel(0),
theAlgorithm(0), theSOE(0), theEigenSOE(0), theIntegrator(0), theTest(0),
domainStamp(0)
{
...
TransientDomainDecompositionAnalysis::TransientDomainDecompositionAnalysis
    (Subdomain & the_Domain, ConstraintHandler & theHandler,
     DOF_Numberer & theNumberer, AnalysisModel & theModel,
     EquiSolnAlgo & theSolnAlgo, LinearSOE & theLinSOE,
     TransientIntegrator & theTransientIntegrator,
     ConvergenceTest * theConvergenceTest,
     bool
     setLinks):DomainDecompositionAnalysis
    (ANALYSIS_TAGS_TransientDomainDecompositionAnalysis, the_Domain),
theConstraintHandler(&theHandler), theDOF_Numberer(&theNumberer),
theAnalysisModel(&theModel), theAlgorithm(&theSolnAlgo),
theSOE(&theLinSOE), theEigenSOE(0), theIntegrator(&theTransientIntegrator),
theTest(theConvergenceTest), domainStamp(0)
{
...
bool TransientDomainDecompositionAnalysis::doesIndependentAnalysis(void)
{
    return true;
}


int TransientDomainDecompositionAnalysis::analyze(double dT)
{
    int result = 0;
    ...
    if (stamp != domainStamp) {
        ...
        if (result < 0) {
            opserr <<
                "TransientDomainDecompositionAnalysis::analyze() - domainChanged failed";
            return -1;
        }
    }
    ...
    if (result < 0) {
        opserr <<
            "TransientDomainDecompositionAnalysis::analyze() - the AnalysisModel failed";
    ...
    if (result < 0) {
        opserr <<
            "TransientDomainDecompositionAnalysis::analyze() - the Integrator failed";
        opserr << " with domain at load factor ";
        opserr << the_Domain->getCurrentTime() << endln;
        the_Domain->revertToLastCommit();
    ...
    if (result < 0) {
        opserr <<
            "TransientDomainDecompositionAnalysis::analyze() - the Algorithm failed";
        opserr << " with domain at load factor ";
        opserr << the_Domain->getCurrentTime() << endln;
        the_Domain->revertToLastCommit();
        theIntegrator->revertToLastStep();
...
int TransientDomainDecompositionAnalysis::eigen(int numMode,
                                                bool generalized,
                                                bool findSmallest)
{
    int result = 0;
    ...
    if (theEigenSOE == 0) {
        opserr <<
            "TransientDomainDecompositionAnalysis::eigen() - no eigen solver has been set\n";
        return -1;
    }
    // check for change in Domain since last step. As a change can
    ...
    if (stamp != domainStamp) {
        ...
        if (result < 0) {
            opserr <<
                "TransientDomainDecompositionAnalysis::eigen() - domainChanged failed";
            return -1;
        }
    }
    //
    ...
    if (generalized == true) {
        ...
        while ((elePtr = theEles2()) != 0) {
            ...
            if (theEigenSOE->addM(elePtr->getTangent(0), elePtr->getID()) <
                0) {
                opserr <<
                    "WARNING TransientDomainDecomposition::eigen() -";
                opserr << " failed in addA for ID " << elePtr->getID();
                result = -2;
            }
        }
        ...
        while ((dofPtr = theDofs()) != 0) {
            ...
            if (theEigenSOE->addM(dofPtr->getTangent(0), dofPtr->getID()) <
                0) {
                opserr <<
                    "WARNING TransientDomainDecomposition::eigen() -";
    ...
    if (theEigenSOE->solve(numMode, generalized, findSmallest) < 0) {
        opserr <<
            "WARNING TransientDomainDecomposition::eigen() - EigenSOE failed in solve()\n";
        return -4;
    }
    //
    ...
    for (int i = 1; i <= numMode; i++) {
        ...
        theAnalysisModel->setEigenvector(i,
                                         theEigenSOE->getEigenvector(i));
    }
...
int TransientDomainDecompositionAnalysis::initialize(void)
{
    Domain *the_Domain = this->getDomainPtr();
    ...
    if (stamp != domainStamp) {
        ...
        if (this->domainChanged() < 0) {
            opserr <<
                "DirectIntegrationAnalysis::initialize() - domainChanged() failed\n";
            return -1;
        }
    }
    if (theIntegrator->initialize() < 0) {
        opserr <<
            "DirectIntegrationAnalysis::initialize() - integrator initialize() failed\n";
        return -2;
    } else
...
int TransientDomainDecompositionAnalysis::domainChanged(void)
{
    int result = 0;
    ...
    if (result < 0) {
        opserr <<
            "TransientDomainDecompositionAnalysis::setAlgorithm() - ";
...
int TransientDomainDecompositionAnalysis::getNumExternalEqn(void)
{
    opserr <<
        "TransientDomainDecompositionAnalysis::getNumExternalEqn() - should never be called\n";
    return 0;
}

int TransientDomainDecompositionAnalysis::getNumInternalEqn(void)
{
    opserr <<
        "TransientDomainDecompositionAnalysis::getNumInternalEqn() - should never be called\n";
    return 0;
}

int TransientDomainDecompositionAnalysis::analysisStep(double dT)
{
    this->analyze(dT);
    return 0;
}

int TransientDomainDecompositionAnalysis::eigenAnalysis(int numMode,
                                                        bool generalized,
                                                        bool findSmallest)
{
...
int TransientDomainDecompositionAnalysis::computeInternalResponse(void)
{
    opserr <<
        "TransientDomainDecompositionAnalysis::computeInternalResponse() - should never be called\n";
    return 0;
}

int TransientDomainDecompositionAnalysis::formTangent(void)
{
    opserr <<
        "TransientDomainDecompositionAnalysis::formTangent() - should never be called\n";
    return 0;
}

int TransientDomainDecompositionAnalysis::formResidual(void)
{
    opserr <<
        "TransientDomainDecompositionAnalysis::formResidual() - should never be called\n";
    return 0;
}

int TransientDomainDecompositionAnalysis::
formTangVectProduct(Vector & force)
{
    opserr <<
        "TransientDomainDecompositionAnalysis::formTangVectProduct() - should never be called\n";
    return 0;
}

const Matrix & TransientDomainDecompositionAnalysis::getTangent(void)
{
    static Matrix errMatrix;
    opserr <<
        "TransientDomainDecompositionAnalysis::getTangent() - should never be called\n";
    return errMatrix;
}

const Vector & TransientDomainDecompositionAnalysis::getResidual(void)
{
    static Vector errVector;
    opserr <<
        "TransientDomainDecompositionAnalysis::getResidual() - should never be called\n";
    return errVector;
}
    ...
    TransientDomainDecompositionAnalysis::getTangVectProduct(void)
{
    static Vector errVector;
    opserr <<
        "TransientDomainDecompositionAnalysis::getTangVectProduct() - should never be called\n";
    return errVector;
}

int
 TransientDomainDecompositionAnalysis::sendSelf(int commitTag,
                                                Channel & theChannel)
{
    ...
    if (theAlgorithm == 0) {
        opserr <<
            "TransientDomainDecompositionAnalysis::sendSelf() - no objects exist!\n";
        return -1;
    }
    ...
    if (theConstraintHandler->sendSelf(commitTag, theChannel) != 0) {
        opserr <<
            "TransientDomainDecompositionAnalysis::sendSelf() - failed to send handler\n";
        return -1;
    }

    if (theDOF_Numberer->sendSelf(commitTag, theChannel) != 0) {
        opserr <<
            "TransientDomainDecompositionAnalysis::sendSelf() - failed to send numberer\n";
        return -1;
    }

    if (theAnalysisModel->sendSelf(commitTag, theChannel) != 0) {
        opserr <<
            "TransientDomainDecompositionAnalysis::sendSelf() - failed to send model\n";
        return -1;
    }

    if (theAlgorithm->sendSelf(commitTag, theChannel) != 0) {
        opserr <<
            "TransientDomainDecompositionAnalysis::sendSelf() - failed to send algorithm\n";
        return -1;
    }

    if (theSOE->sendSelf(commitTag, theChannel) != 0) {
        opserr <<
            "TransientDomainDecompositionAnalysis::sendSelf() - failed to send SOE\n";
        return -1;
    } else;
    ...
    if (theSolver->sendSelf(commitTag, theChannel) != 0) {
        opserr <<
            "TransientDomainDecompositionAnalysis::sendSelf() - failed to send Solver\n";
        return -1;
    }

    if (theIntegrator->sendSelf(commitTag, theChannel) != 0) {
        opserr <<
            "TransientDomainDecompositionAnalysis::sendSelf() - failed to send integrator\n";
        return -1;
    }
...
int TransientDomainDecompositionAnalysis::recvSelf(int commitTag,
                                                   Channel & theChannel,
                                                   FEM_ObjectBroker &
                                                   theBroker)
{
    ...
    if (theConstraintHandler == 0
        || theConstraintHandler->getClassTag() != data(0)) {
        if (theConstraintHandler != 0)
            delete theConstraintHandler;
    ...
    if (theAnalysisModel == 0
        || theAnalysisModel->getClassTag() != data(2)) {
        if (theAnalysisModel != 0)
            delete theAnalysisModel;
...
int TransientDomainDecompositionAnalysis::
setAlgorithm(EquiSolnAlgo & theNewAlgorithm)
{
...
int TransientDomainDecompositionAnalysis::
setIntegrator(IncrementalIntegrator & theNewIntegrator)
{
    // invoke the destructor on the old one
    if (theIntegrator != 0) {
        delete theIntegrator;
    ...
    if (theIntegrator != 0 && theConstraintHandler != 0
        && theAlgorithm != 0 && theAnalysisModel != 0 && theSOE != 0) {
        theIntegrator->setLinks(*theAnalysisModel, *theSOE, theTest);
        theConstraintHandler->setLinks(*the_Domain, *theAnalysisModel,
                                       *theIntegrator);
        theAlgorithm->setLinks(*theAnalysisModel, *theIntegrator, *theSOE,
...
int TransientDomainDecompositionAnalysis::
setLinearSOE(LinearSOE & theNewSOE)
{
    // invoke the destructor on the old one
    if (theSOE != 0)
        delete theSOE;
    ...
    // set the links needed by the other objects in the aggregation
    theSOE = &theNewSOE;
    if (theIntegrator != 0 && theAlgorithm != 0 && theAnalysisModel != 0
        && theSOE != 0) {
        theIntegrator->setLinks(*theAnalysisModel, *theSOE, theTest);
        theAlgorithm->setLinks(*theAnalysisModel, *theIntegrator, *theSOE,
                               theTest);
...
int TransientDomainDecompositionAnalysis::setEigenSOE(EigenSOE & theNewSOE)
{
...
int TransientDomainDecompositionAnalysis::
setConvergenceTest(ConvergenceTest & theConvergenceTest)
{
    // invoke the destructor on the old one
    if (theTest != 0) {
        delete theTest;
