...
PFEMAnalysis::PFEMAnalysis(Domain & theDomain,
                           ...
                           double max, double min, double g,
                           double r):DirectIntegrationAnalysis(theDomain,
                                                               theHandler,
                                                               theNumberer,
                                                               theModel,
                                                               theSolnAlgo,
                                                               ...
                                                               theIntegrator,
                                                               theTest),
dtmax(max), dtmin(min), gravity(g), ratio(r), dt(max), next(max),
newstep(true)
{
 ...
 PFEMAnalysis::analyze()
{
    ...
    while (true) {
        ...
        if (this->identify() < 0) {
            opserr <<
                "WARNING: failed to identify domain -- PFEMAnalysis\n";
            return -1;
        }
        // analyze
#ifdef _PARALLEL_INTERPRETERS
...
int PFEMAnalysis::identify()
{
    Domain *theDomain = this->getDomainPtr();

    if (theDomain->getNumPCs() == 0)
        return 0;
    ...
    while ((elePtr = theElemIter()) != 0) {
        ...
        for (int i = 0; i < nodeids.Size(); i++) {
            Pressure_Constraint *thePC =
                theDomain->getPressure_Constraint(nodeids(i));
            if (thePC != 0) {
