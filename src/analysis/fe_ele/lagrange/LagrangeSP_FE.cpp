...
LagrangeSP_FE::LagrangeSP_FE(int tag, Domain & theDomain,
                             SP_Constraint & TheSP, DOF_Group & theGroup,
                             double Alpha):FE_Element(tag, 2, 2),
...
const Matrix & LagrangeSP_FE::getTangent(Integrator * theIntegrator)
{
    return *tang;
}

const Vector & LagrangeSP_FE::getResidual(Integrator * theNewIntegrator)
{
...
const Vector & LagrangeSP_FE::getK_Force(const Vector & disp, double fact)
{
    opserr << "WARNING PenaltySP_FE::getK_Force() - not yet implemented\n";
    return *resid;
}

const Vector & LagrangeSP_FE::getKi_Force(const Vector & disp, double fact)
{
    opserr <<
...
const Vector & LagrangeSP_FE::getC_Force(const Vector & disp, double fact)
{
    opserr << "WARNING PenaltySP_FE::getC_Force() - not yet implemented\n";
    return *resid;
}

const Vector & LagrangeSP_FE::getM_Force(const Vector & disp, double fact)
{
