...
LagrangeMP_FE::LagrangeMP_FE(int tag, Domain & theDomain,
                             MP_Constraint & TheMP, DOF_Group & theGroup,
                             double Alpha):FE_Element(tag, 3,
                                                      (tag,
                                                       TheMP.
                                                       getConstrainedDOFs
                                                       ()).Size() +
                                                      ...
                                                      (TheMP.
                                                       getRetainedDOFs()).
                                                      Size()),
alpha(Alpha), theMP(&TheMP), theConstrainedNode(0), theRetainedNode(0),
theDofGroup(&theGroup), tang(0), resid(0)
{
    ...
    if (tang == 0 || resid == 0 || tang->noCols() == 0
        || resid->Size() == 0) {
    ...
    // DOF_Group objects
    DOF_Group *theConstrainedNodesDOFs =
        theConstrainedNode->getDOF_GroupPtr();
...
const Matrix & LagrangeMP_FE::getTangent(Integrator * theNewIntegrator)
{
...
const Vector & LagrangeMP_FE::getResidual(Integrator * theNewIntegrator)
{
    return *resid;
}
...
{
    opserr <<
        "WARNING lagrangeMP_FE::getTangForce() - not yet implemented\n";
    return *resid;
}


const Vector & LagrangeMP_FE::getK_Force(const Vector & disp, double fact)
{
    opserr <<
        "WARNING lagrangeMP_FE::getK_Force() - not yet implemented\n";
    return *resid;
}

const Vector & LagrangeMP_FE::getKi_Force(const Vector & disp, double fact)
{
    opserr <<
        "WARNING LagrangeMP_FE::getKi_Force() - not yet implemented\n";
    return *resid;
}

const Vector & LagrangeMP_FE::getC_Force(const Vector & disp, double fact)
{
    opserr <<
        "WARNING lagrangeMP_FE::getC_Force() - not yet implemented\n";
    return *resid;
}

const Vector & LagrangeMP_FE::getM_Force(const Vector & disp, double fact)
{
    opserr <<
        "WARNING lagrangeMP_FE::getM_Force() - not yet implemented\n";
    return *resid;
}

void LagrangeMP_FE::determineTangent(void)
{
    const Matrix & constraint = theMP->getConstraint();
    int noRows = constraint.noRows();
    int noCols = constraint.noCols();
