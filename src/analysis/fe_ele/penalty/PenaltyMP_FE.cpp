...
PenaltyMP_FE::PenaltyMP_FE(int tag, Domain & theDomain,
                           ...
                           double Alpha):FE_Element(tag, 2,
                                                    ...
                                                    (TheMP.
                                                     getRetainedDOFs()).
                                                    Size()), theMP(&TheMP),
theConstrainedNode(0), theRetainedNode(0), tang(0), resid(0), C(0),
alpha(Alpha)
{
    ...
    if (theRetainedNode == 0 || theConstrainedNode == 0) {
        opserr <<
            "FATAL PenaltyMP_FE::PenaltyMP_FE() - Constrained or Retained";
        opserr << " Node does not exist in Domain\n";
        opserr << theMP->
            getNodeRetained() << " " << theMP->getNodeConstrained() <<
            endln;
        exit(-1);
    }
    ...
    if (dofGrpPtr != 0)
        ...
        opserr <<
            "WARNING PenaltyMP_FE::PenaltyMP_FE() - node no Group yet?\n";
...
const Matrix & PenaltyMP_FE::getTangent(Integrator * theNewIntegrator)
{
...
const Vector & PenaltyMP_FE::getResidual(Integrator * theNewIntegrator)
{
    // zero residual, CD = 0
    return *resid;
}
...
const Vector & PenaltyMP_FE::getTangForce(const Vector & disp, double fact)
{
    opserr <<
        "WARNING PenaltyMP_FE::getTangForce() - not yet implemented\n";
    return *resid;
}

const Vector & PenaltyMP_FE::getK_Force(const Vector & disp, double fact)
{
    opserr << "WARNING PenaltyMP_FE::getK_Force() - not yet implemented\n";
    return *resid;
}

const Vector & PenaltyMP_FE::getKi_Force(const Vector & disp, double fact)
{
    opserr << "WARNING PenaltyMP_FE::getK_Force() - not yet implemented\n";
    return *resid;
}

const Vector & PenaltyMP_FE::getC_Force(const Vector & disp, double fact)
{
    opserr << "WARNING PenaltyMP_FE::getC_Force() - not yet implemented\n";
    return *resid;
}

const Vector & PenaltyMP_FE::getM_Force(const Vector & disp, double fact)
{
    // opserr << "WARNING PenaltyMP_FE::getM_Force() - not yet implemented\n";
    return *resid;
}

void PenaltyMP_FE::determineTangent(void)
{
