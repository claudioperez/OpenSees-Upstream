...
PenaltySP_FE::PenaltySP_FE(int tag, Domain & theDomain,
                           SP_Constraint & TheSP,
                           double Alpha):FE_Element(tag, 1, 1),
alpha(Alpha), theSP(&TheSP), theNode(0)
{
 ...
 PenaltySP_FE::setID(void)
{
    ...
    if (theNodesDOFs == 0) {
        opserr <<
            "WARNING PenaltySP_FE::setID(void) - no DOF_Group with Node\n";
        return -2;
    }
...
const Matrix & PenaltySP_FE::getTangent(Integrator * theNewIntegrator)
{
    tang(0, 0) = alpha;
    return tang;
}
...
const Vector & PenaltySP_FE::getResidual(Integrator * theNewIntegrator)
{
    ...
    if (constrainedDOF < 0 || constrainedDOF >= nodeDisp.Size()) {
        ...
        opserr << " constrained DOF " << constrainedDOF <<
            " outside disp\n";
        resid(0) = 0;
    }
...
const Vector & PenaltySP_FE::getTangForce(const Vector & disp, double fact)
{
    double constraint = theSP->getValue();
    int constrainedID = myID(0);
    if (constrainedID < 0 || constrainedID >= disp.Size()) {
        opserr << "WARNING PenaltySP_FE::getTangForce() - ";
        opserr << " constrained DOF " << constrainedID <<
            " outside disp\n";
        resid(0) = 0.0;
        return resid;
    }
...
const Vector & PenaltySP_FE::getK_Force(const Vector & disp, double fact)
{
...
const Vector & PenaltySP_FE::getKi_Force(const Vector & disp, double fact)
{
    opserr <<
        "WARNING PenaltySP_FE::getKi_Force() - not yet implemented\n";
    resid(0) = 0.0;
    return resid;
}


const Vector & PenaltySP_FE::getC_Force(const Vector & disp, double fact)
{
...
const Vector & PenaltySP_FE::getM_Force(const Vector & disp, double fact)
{
