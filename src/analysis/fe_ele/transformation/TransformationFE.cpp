...
Matrix **
    TransformationFE::modMatrices;
Vector **
    TransformationFE::modVectors;
Matrix **
    TransformationFE::theTransformations;
int
TransformationFE::numTransFE (0);
int
TransformationFE::transCounter (0);
int
TransformationFE::sizeTransformations (0);
double *
    TransformationFE::dataBuffer = 0;
double *
    TransformationFE::localKbuffer = 0;
int *
    TransformationFE::dofData = 0;;
int
TransformationFE::sizeBuffer (0);
    ...
    for (int i = 0; i < numGroups; i++)
      {
          ...
          for (int j = 0; j < theDOFid.Size (); j++)
              if (current < numTransformedDOF)
                ...
                {
                    opserr <<
                        "WARNING TransformationFE::setID() - numDOF and";
    ...
    if (numTransformedDOF <= MAX_NUM_DOF)
      {
          ...
          if (modVectors[numTransformedDOF] == 0)
            {
                modVectors[numTransformedDOF] =
                    new Vector (numTransformedDOF);
                ...
                if (Ti != 0 && Tj != 0)
                ...
                else if (Ti != 0 && Tj == 0)
                  {
                      ...
                      localTtKT.setData (dataBuffer, noRowsTransformed,
                                         noColsTransformed);
                      //localTtKT = (*Ti) ^ localK;
                      localTtKT.addMatrixTransposeProduct (0.0, *Ti, localK,
                                                           1.0);
                  }
                else
                  {
                ...
                for (int j = 0; j < noRows; j++)
                  {
                      ...
                      for (int k = 0; k < noCols; k++)
                          sum +=
                              (*Ti) (k,
                                     j) * theResidual (startRowOriginal + k);
                ...
                for (int j = 0; j < noRows; j++)
                  {
                      ...
                      for (int k = 0; k < noCols; k++)
                          sum +=
                              (*Ti) (j,
                                     k) * modResp (startRowTransformed + k);
...
void
TransformationFE::addD_ForceSensitivity (int gradNumber, const Vector & disp,
                                         double fact)
{
    if (fact == 0.0)
        return;
...
void
TransformationFE::addM_ForceSensitivity (int gradNumber, const Vector & disp,
                                         double fact)
{
    if (fact == 0.0)
        return;
