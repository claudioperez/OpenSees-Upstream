...
TransformationFE::TransformationFE(int tag, Element * ele):FE_Element(tag,
                                                                      ele),
theDOFs(0), numSPs(0), theSPs(0), modID(0), modTangent(0), modResidual(0),
numGroups(0), numTransformedDOF(0)
{
    ...
    if (theDOFs == 0) {
        opserr <<
            "FATAL TransformationFE::TransformationFE() - out of memory craeting ";
        opserr << "array of size : " << numNodes <<
            " for storage of DOF_Group\n";
        exit(-1);
    }
    ...
    for (int i = 0; i < numNodes; i++) {
        ...
        if (theNode == 0) {
            opserr <<
                "FATAL TransformationFE::TransformationFE() - no Node with tag: ";
        ...
        if (theDofGroup == 0) {
            opserr <<
                "FATAL TransformationFE::TransformationFE() - no DOF_Group : ";
            opserr << " associated with node: " << nodes(i) <<
                " in the domain\n";;
            exit(-1);
        }
    ...
    if (numNodes > sizeTransformations) {
        ...
        if (theTransformations == 0) {
            opserr <<
                "FATAL TransformationFE::TransformationFE() - out of memory ";
            opserr <<
                "for array of pointers for Transformation matrices of size ";
            opserr << numNodes;
            exit(-1);
        }
...
const ID & TransformationFE::getDOFtags(void) const
{
    return this->FE_Element::getDOFtags();
}


const ID & TransformationFE::getID(void) const
{
    // make sure that it exists
    if (modID == 0) {
        opserr <<
            "FATAL TransformationFE::getID() called before setID()\n";
        exit(-1);
    }
    return *modID;
}
...
int TransformationFE::setID(void)
{
    ...
    if (numTransformedDOF <= MAX_NUM_DOF) {
        ...
        if (modVectors[numTransformedDOF] == 0) {
            ...
            modMatrices[numTransformedDOF] =
                new Matrix(numTransformedDOF, numTransformedDOF);
            modResidual = modVectors[numTransformedDOF];
            modTangent = modMatrices[numTransformedDOF];
            if (modResidual == 0
                || modResidual->Size() != numTransformedDOF
                || modTangent == 0
                || modTangent->noCols() != numTransformedDOF) {
                opserr << "TransformationFE::setID() ";
...
const Matrix & TransformationFE::getTangent(Integrator * theNewIntegrator)
{
    const Matrix & theTangent =
        this->FE_Element::getTangent(theNewIntegrator);
        ...
        for (int j = 0; j < numNode; j++) {
            ...
            for (int a = 0; a < numDOFi; a++)
                for (int b = 0; b < numDOFj; b++)
                    localK(a, b) =
                        theTangent(noRowsOriginal + a, noColsOriginal + b);
            ...
            if (Ti != 0 && Tj != 0) {
                ...
                // CHECK SIZE OF BUFFER
                localTtKT.setData(dataBuffer, noRowsTransformed,
                                  noColsTransformed);
                //localTtKT = (*Ti) ^ localK * (*Tj);
                localTtKT.addMatrixTripleProduct(0.0, *Ti, localK, *Tj,
                                                 1.0);
            } else if (Ti == 0 && Tj != 0) {
            ...
            for (int c = 0; c < noRowsTransformed; c++)
                for (int d = 0; d < noColsTransformed; d++)
                    (*modTangent) (startRow + c, startCol + d) =
                        localTtKT(c, d);
...
const Vector & TransformationFE::getResidual(Integrator * theNewIntegrator)
{
    const Vector & theResidual =
        this->FE_Element::getResidual(theNewIntegrator);
    // DO THE SP STUFF TO THE TANGENT
        ...
        if (Ti != 0) {
            ...
            for (int j = 0; j < noRows; j++) {
                ...
                for (int k = 0; k < noCols; k++)
                    sum +=
                        (*Ti) (k, j) * theResidual(startRowOriginal + k);
            ...
            noRows = noCols;
            for (int j = 0; j < noRows; j++)
                (*modResidual) (startRowTransformed + j) =
                    theResidual(startRowOriginal + j);
        }
...
void TransformationFE::addM_Force(const Vector & disp, double fact)
{
    if (fact == 0.0)
        return;
...
// CHANGE THE ID SENT
const Vector & TransformationFE::getLastResponse(void)
{
...
int TransformationFE::transformResponse(const Vector & modResp,
                                        Vector & unmodResp)
{
    ...
    for (int i = 0; i < numNode; i++) {
        ...
        if (Ti != 0) {
            ...
            for (int j = 0; j < noCols; j++)
                unmodResp(startRowOriginal + j) =
                    modResp(startRowTransformed + j);
        }
