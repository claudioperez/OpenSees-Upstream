...
FE_Element::FE_Element(int tag, Element * ele):TaggedObject(tag),
myDOF_Groups((ele->getExternalNodes()).Size()),
myID(ele->getNumDOF()),
numDOF(ele->getNumDOF()),
theModel(0), myEle(ele), theResidual(0), theTangent(0), theIntegrator(0)
{
    if (numDOF <= 0) {
    ...
    if (theDomain == 0) {
        opserr << "FATAL FE_Element::FE_Element() - element has no domain "
            << *ele;
        exit(-1);
    }
...
FE_Element::FE_Element(int tag, int numDOF_Group,
                       int ndof):TaggedObject(tag),
...
const ID & FE_Element::getID(void) const
{
    return myID;
}

void FE_Element::setAnalysisModel(AnalysisModel & theAnalysisModel)
{
...
const Vector & FE_Element::getResidual(Integrator * theNewIntegrator)
{
    theIntegrator = theNewIntegrator;
...
void FE_Element::addKtToTang(double fact)
{
...
void FE_Element::addCtoTang(double fact)
{
    if (myEle != 0 && myEle->isActive()) {
...
void FE_Element::addMtoTang(double fact)
{
    if (myEle != 0 && myEle->isActive()) {
...
void FE_Element::addKiToTang(double fact)
{
    if (myEle != 0 && myEle->isActive()) {
        // check for a quick return 
        if (fact == 0.0)
...
void FE_Element::addKgToTang(double fact)
{
    if (myEle != 0 && myEle->isActive()) {
        // check for a quick return 
        if (fact == 0.0)
            ...
            theTangent->addMatrix(1.0, myEle->getGeometricTangentStiff(),
                                  fact);
        else {
...
void FE_Element::addKpToTang(double fact, int numP)
{
    if (myEle != 0 && myEle->isActive()) {
        // check for a quick return 
        if (fact == 0.0)
...
int FE_Element::storePreviousK(int numP)
{
    int res = 0;
    if (myEle != 0) {
...
void FE_Element::zeroResidual(void)
{
    if (myEle != 0) {
        ...
        opserr <<
            "WARNING FE_Element::zeroResidual() - no Element *given ";
        opserr << "- subclasses must provide implementation\n";
    }
}

void FE_Element::addRtoResidual(double fact)
{
    if (myEle != 0) {
        // check for a quick return 
        ...
        opserr <<
            "WARNING FE_Element::addRtoResidual() - no Element *given ";
        opserr << "- subclasses must provide implementation\n";
    }
}


void FE_Element::addRIncInertiaToResidual(double fact)
{
    if (myEle != 0) {
        // check for a quick return     
        if (fact == 0.0 || !myEle->isActive())
...
const Vector & FE_Element::getTangForce(const Vector & disp, double fact)
{
    if (myEle != 0) {
        ...
        if (myEle->isSubdomain() == false) {
            ...
            if (theResidual->addMatrixVector(1.0, *theTangent, tmp, fact) <
                0) {
            ...
            Subdomain *theSub = (Subdomain *) myEle;
            if (theResidual->addMatrixVector
                (1.0, theSub->getTang(), tmp, fact) < 0) {
                opserr << "WARNING FE_Element::getTangForce() - ";
                opserr << "- addMatrixVector returned error\n";
            }
        }
        ...
        opserr <<
            "WARNING FE_Element::addTangForce() - no Element *given ";
        opserr << "- subclasses must provide implementation\n";
        return errVector;
    }
}
...
const Vector & FE_Element::getK_Force(const Vector & disp, double fact)
{
    if (myEle != 0) {
        ...
        if (theResidual->addMatrixVector
            (1.0, myEle->getTangentStiff(), tmp, fact) < 0) {
...
const Vector & FE_Element::getKi_Force(const Vector & disp, double fact)
{
    if (myEle != 0) {
        ...
        if (theResidual->addMatrixVector
            (1.0, myEle->getInitialStiff(), tmp, fact) < 0) {
            opserr << "WARNING FE_Element::getKForce() - ";
            opserr << "- addMatrixVector returned error\n";
        }
...
const Vector & FE_Element::getM_Force(const Vector & disp, double fact)
{
    ...
    if (myEle != 0) {
        ...
        if (theResidual->addMatrixVector(1.0, myEle->getMass(), tmp, fact)
            < 0) {
...
const Vector & FE_Element::getC_Force(const Vector & disp, double fact)
{
    if (myEle != 0) {
        ...
        if (theResidual->addMatrixVector(1.0, myEle->getDamp(), tmp, fact)
            < 0) {
...
Integrator *FE_Element::getLastIntegrator(void)
{
    return theIntegrator;
}

const Vector & FE_Element::getLastResponse(void)
{
    if (myEle != 0) {
        if (theIntegrator != 0) {
            if (theIntegrator->getLastResponse(*theResidual, myID) < 0) {
                ...
                opserr <<
                    " - the Integrator had problems with getLastResponse()\n";
            }
        } else {
            theResidual->Zero();
...
void FE_Element::addM_Force(const Vector & accel, double fact)
{
    if (myEle != 0) {
...
void FE_Element::addD_Force(const Vector & accel, double fact)
{
    if (myEle != 0) {
...
void FE_Element::addK_Force(const Vector & disp, double fact)
{
    if (myEle != 0) {
...
void FE_Element::addKg_Force(const Vector & disp, double fact)
{
    if (myEle != 0) {
        ...
        if (myEle->isSubdomain() == false) {
            ...
            if (theResidual->addMatrixVector
                (1.0, myEle->getGeometricTangentStiff(), tmp, fact) < 0) {
...
void FE_Element::addLocalM_Force(const Vector & accel, double fact)
{
    if (myEle != 0) {
        ...
        opserr <<
            "WARNING FE_Element::addLocalM_Force() - no Element *given ";
        opserr << "- subclasses must provide implementation\n";
    }
}

void FE_Element::addLocalD_Force(const Vector & accel, double fact)
{
    if (myEle != 0) {
        ...
        opserr <<
            "WARNING FE_Element::addLocalD_Force() - no Element *given ";
        opserr << "- subclasses must provide implementation\n";
    }
}
...
void FE_Element::addResistingForceSensitivity(int gradNumber, double fact)
{
...
void FE_Element::addM_ForceSensitivity(int gradNumber, const Vector & vect,
                                       double fact)
{
    // Get the components we need out of the vector
    // and place in a temporary vector
    ...
    if (theResidual->addMatrixVector
        (1.0, myEle->getMassSensitivity(gradNumber), tmp, fact) < 0) {
        opserr << "WARNING FE_Element::addM_ForceSensitivity() - ";
        opserr << "- addMatrixVector returned error\n";
    }
}

void FE_Element::addD_ForceSensitivity(int gradNumber, const Vector & vect,
                                       double fact)
{
    if (myEle != 0) {
        ...
        if (myEle->isSubdomain() == false) {
            ...
            if (theResidual->addMatrixVector(1.0,
                                             myEle->
                                             getDampSensitivity
                                             (gradNumber), tmp,
                                             fact) < 0) {
        ...
        opserr <<
            "WARNING FE_Element::addD_ForceSensitivity() - no Element *given ";
        opserr << "- subclasses must provide implementation\n";
    }
}
    ...
    if (myEle != 0) {
        ...
        if (myEle->isSubdomain() == false) {
            if (theResidual->addMatrixVector(1.0,
                ...
                opserr <<
                    "WARNING FE_Element::addLocalD_ForceSensitivity() - ";
                opserr << "- addMatrixVector returned error\n";
            }
        } else {
            opserr <<
        ...
        opserr <<
            "WARNING FE_Element::addLocalD_ForceSensitivity() - no Element *given ";
        opserr << "- subclasses must provide implementation\n";
    }
}
...
int FE_Element::commitSensitivity(int gradNum, int numGrads)
{
...
int FE_Element::updateElement(void)
{
    if (myEle != 0 && myEle->isActive()) {
        return myEle->update();
        opserr << "FE_Element::update()";
