...
LoadControl::LoadControl(double dLambda, int numIncr, double min,
                         double max,
                         int classtag):StaticIntegrator(classtag),
deltaLambda(dLambda), specNumIncrStep(numIncr), numIncrLastStep(numIncr),
...
int LoadControl::update(const Vector & deltaU)
{
...
int LoadControl::setDeltaLambda(double newValue)
{
...
int LoadControl::sendSelf(int cTag, Channel & theChannel)
{
    Vector data(5);
    data(0) = deltaLambda;
...
int LoadControl::recvSelf(int cTag,
                          Channel & theChannel,
...
void LoadControl::Print(OPS_Stream & s, int flag)
{
    AnalysisModel *theModel = this->getAnalysisModel();
    if (theModel != 0) {
        double currentLambda = theModel->getCurrentDomainTime();
...
int LoadControl::formEleResidual(FE_Element * theEle)
{
...
int LoadControl::formIndependentSensitivityRHS()
{
    return 0;
}

int LoadControl::formSensitivityRHS(int passedGradNumber)
{
    sensitivityFlag = 1;
...
int LoadControl::saveSensitivity(const Vector & v, int gradNum,
                                 int numGrads)
{
    // get model
    AnalysisModel *theAnalysisModel = this->getAnalysisModel();
...
int LoadControl::commitSensitivity(int gradNum, int numGrads)
{
    // get model
    AnalysisModel *theAnalysisModel = this->getAnalysisModel();
...
bool LoadControl::computeSensitivityAtEachIteration()
{
...
int LoadControl::computeSensitivities(void)
{
