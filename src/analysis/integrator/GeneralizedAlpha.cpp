...
void *OPS_GeneralizedAlpha(void)
{
    ...
    if (argc != 2 && argc != 4) {
        opserr <<
            "WARNING - incorrect number of args want GeneralizedAlpha $alphaM $alphaF <$gamma $beta>\n";
        return 0;
    }
    ...
    if (OPS_GetDouble(&argc, dData) != 0) {
        opserr <<
            "WARNING - invalid args want GeneralizedAlpha $alphaM $alphaF <$gamma $beta>\n";
        return 0;
    }
    ...
    if (theIntegrator == 0)
        opserr <<
            "WARNING - out of memory creating GeneralizedAlpha integrator\n";
...
GeneralizedAlpha::GeneralizedAlpha():TransientIntegrator(INTEGRATOR_TAGS_GeneralizedAlpha),
...
GeneralizedAlpha::GeneralizedAlpha(double _alphaM,
                                   double
                                   _alphaF):TransientIntegrator
    (INTEGRATOR_TAGS_GeneralizedAlpha), alphaM(_alphaM), alphaF(_alphaF),
beta((1 + _alphaM - _alphaF) * (1 + _alphaM - _alphaF) * 0.25),
gamma(0.5 + _alphaM - _alphaF), deltaT(0.0), c1(0.0), c2(0.0), c3(0.0),
Ut(0), Utdot(0), Utdotdot(0), U(0), Udot(0), Udotdot(0), Ualpha(0),
Ualphadot(0), Ualphadotdot(0)
{
...
GeneralizedAlpha::GeneralizedAlpha(double _alphaM, double _alphaF,
    ...
    (INTEGRATOR_TAGS_GeneralizedAlpha), alphaM(_alphaM), alphaF(_alphaF),
beta(_beta), gamma(_gamma), deltaT(0.0), c1(0.0), c2(0.0), c3(0.0), Ut(0),
...
int
 GeneralizedAlpha::newStep(double _deltaT)
{
    deltaT = _deltaT;
    if (beta == 0 || gamma == 0) {
    ...
    if (U == 0) {
        opserr <<
            "GeneralizedAlpha::newStep() - domainChange() failed or hasn't been called\n";
        return -3;
    }
    ...
    if (theModel->updateDomain(time, deltaT) < 0) {
        opserr <<
            "GeneralizedAlpha::newStep() - failed to update the domain\n";
        return -4;
    }
...
int GeneralizedAlpha::domainChanged()
{
    ...
    if (Ut == 0 || Ut->Size() != size) {
        ...
        if (Ut == 0 || Ut->Size() != size ||
            ...
            opserr <<
                "GeneralizedAlpha::domainChanged - ran out of memory\n";
...
int GeneralizedAlpha::update(const Vector & deltaU)
{
    ...
    if (theModel == 0) {
        opserr <<
            "WARNING GeneralizedAlpha::update() - no AnalysisModel set\n";
        return -1;
    }
    // check domainChanged() has been called, i.e. Ut will not be zero
    if (Ut == 0) {
        opserr <<
            "WARNING GeneralizedAlpha::update() - domainChange() failed or not called\n";
        return -2;
    }
    // check deltaU is of correct size
    if (deltaU.Size() != U->Size()) {
        opserr <<
            "WARNING GeneralizedAlpha::update() - Vectors of incompatible size ";
        opserr << " expecting " << U->
    ...
    if (theModel->updateDomain() < 0) {
        opserr <<
            "GeneralizedAlpha::update() - failed to update the domain\n";
        return -4;
    }
...
int GeneralizedAlpha::commit(void)
{
    ...
    if (theModel == 0) {
        opserr <<
            "WARNING GeneralizedAlpha::commit() - no AnalysisModel set\n";
        return -1;
    }
    // update the response at the DOFs
    ...
    if (theModel->updateDomain() < 0) {
        opserr <<
            "GeneralizedAlpha::commit() - failed to update the domain\n";
        return -4;
    }
...
const Vector & GeneralizedAlpha::getVel()
{
    return *Udot;
}
...
int GeneralizedAlpha::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (theChannel.sendVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING GeneralizedAlpha::sendSelf() - could not send data\n";
        return -1;
    }
...
int GeneralizedAlpha::recvSelf(int cTag, Channel & theChannel,
                               FEM_ObjectBroker & theBroker)
{
    ...
    if (theChannel.recvVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING GeneralizedAlpha::recvSelf() - could not receive data\n";
        return -1;
    }
...
void GeneralizedAlpha::Print(OPS_Stream & s, int flag)
{
    ...
    if (theModel != 0) {
        ...
        s << "  alphaF: " << alphaF << "  alphaM: " << alphaM << "  beta: "
