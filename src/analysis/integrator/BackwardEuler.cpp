...
BackwardEuler::
BackwardEuler(int
              eulerOption):TransientIntegrator
(INTEGRATOR_TAGS_BackwardEuler), step(0), dt(0.0), c1(0.0), c2(0.0), c3(0.0), Utm1(0),
Utm1dot(0), Ut(0), Utdot(0), Utdotdot(0), U(0), Udot(0), Udotdot(0)
{
    if (eulerOption == 0) {
...
const Vector & BackwardEuler::getVel()
{
    return *Udot;
}
...
int BackwardEuler::update(const Vector & deltaU)
{
    ...
    if (theModel->updateDomain() < 0) {
        opserr <<
            "BackwardEuler::update() - failed to update the domain\n";
        return -4;
    }
...
int BackwardEuler::recvSelf(int cTag, Channel & theChannel,
