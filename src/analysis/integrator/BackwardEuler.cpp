...
Udotdot (0)
{
    if (eulerOption == 0)
      ...
      {
          opserr <<
              "Unknown option specified in BackwardEuler, assuming option = 0\n";
          optn = 0;
      }
...
int
BackwardEuler::newStep (double deltaT)
{
    // check the vectors have been created
    if (U == 0)
      {
          opserr <<
              "BackwardEuler::newStep() - domainChange() failed or hasn't been called\n";
          return -3;
      }
          ...
          if (optn == 0)
            {
                ...
                (*Udotdot) = *Utm1;
                Udotdot->addVector (3.0 / (4.0 * deltaT * deltaT), *Ut,
                                    -3.0 / (4.0 * deltaT * deltaT));
    ...
    if (theModel->updateDomain (time, deltaT) < 0)
      {
          opserr <<
              "BackwardEuler::newStep() - failed to update the domain\n";
          return -4;
      }
...
int
BackwardEuler::revertToLastStep ()
{
...
int
BackwardEuler::formEleTangent (FE_Element * theEle)
{
    theEle->zeroTangent ();
...
int
BackwardEuler::formNodTangent (DOF_Group * theDof)
{
    theDof->zeroTangent ();
...
int
BackwardEuler::update (const Vector & deltaU)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel == 0)
      {
          opserr <<
              "WARNING BackwardEuler::update() - no AnalysisModel set\n";
          return -1;
      }
    ...
    if (Ut == 0)
      {
          opserr <<
              "WARNING BackwardEuler::update() - domainChange() failed or not called\n";
          return -2;
      }
    ...
    if (deltaU.Size () != U->Size ())
      {
          opserr <<
              "WARNING BackwardEuler::update() - Vectors of incompatible size ";
...
int
BackwardEuler::sendSelf (int cTag, Channel & theChannel)
{
    Vector data (1);
    data (0) = optn;
    ...
    if (theChannel.sendVector (this->getDbTag (), cTag, data) < 0)
      {
          opserr <<
              "WARNING BackwardEuler::sendSelf() - could not send data\n";
          return -1;
      }
...
int
BackwardEuler::recvSelf (int cTag, Channel & theChannel,
                         FEM_ObjectBroker & theBroker)
{
    ...
    if (theChannel.recvVector (this->getDbTag (), cTag, data) < 0)
      {
          opserr <<
              "WARNING BackwardEuler::recvSelf() - could not receive data\n";
          optn = 0;
          return -1;
      }
...
void
BackwardEuler::Print (OPS_Stream & s, int flag)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel != 0)
      {
