...
int
GimmeMCK::update (const Vector & aiPlusOne)
{
    updateCount++;
    if (updateCount > 1)
      {
...
int
GimmeMCK::sendSelf (int cTag, Channel & theChannel)
{
    Vector data (4);
    data (0) = m;
    data (1) = c;
...
int
GimmeMCK::recvSelf (int cTag, Channel & theChannel,
...
void
GimmeMCK::Print (OPS_Stream & s, int flag)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel != 0)
      {
