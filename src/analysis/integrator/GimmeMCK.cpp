...
void *OPS_GimmeMCK(void)
{
    ...
    if (argc > 3) {
        if (OPS_GetDouble(&numdata, &ki) != 0) {
            opserr <<
                "WARNING - invalid args want GimmeMCK $m $c $k <$ki>\n";
            return 0;
        }
    }
...
GimmeMCK::GimmeMCK():TransientIntegrator(INTEGRATOR_TAGS_GimmeMCK),
...
GimmeMCK::GimmeMCK(double mm, double cc, double kk,
                   double
                   kki):TransientIntegrator(INTEGRATOR_TAGS_GimmeMCK),
m(mm), c(cc), k(kk), ki(kki), updateCount(0), Ut(0), Utdot(0), Utdotdot(0),
...
int GimmeMCK::update(const Vector & aiPlusOne)
{
    ...
    if (updateCount > 1) {
        ...
        opserr <<
            " GimmeMCK integration scheme requires a LINEAR solution algorithm\n";
        return -1;
    }
    ...
    if (Ut == 0) {
        opserr <<
            "WARNING GimmeMCK::update() - domainChange() failed or not called\n";
        return -3;
    }
    // check aiPlusOne is of correct size
    if (aiPlusOne.Size() != U->Size()) {
        opserr <<
            "WARNING GimmeMCK::update() - Vectors of incompatible size ";
        opserr << " expecting " << U->
...
int GimmeMCK::recvSelf(int cTag, Channel & theChannel,
                       FEM_ObjectBroker & theBroker)
{
    Vector data(4);
    if (theChannel.recvVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING GimmeMCK::recvSelf() - could not receive data\n";
        return -1;
    }
