...
int
HHTHSIncrLimit::formNodTangent (DOF_Group * theDof)
{
    theDof->zeroTangent ();
...
int
HHTHSIncrLimit::update (const Vector & deltaU)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel == 0)
      {
...
int
HHTHSIncrLimit::sendSelf (int cTag, Channel & theChannel)
{
    Vector data (6);
    data (0) = alphaI;
    data (1) = alphaF;
...
int
HHTHSIncrLimit::recvSelf (int cTag, Channel & theChannel,
