...
void *OPS_HHTHSIncrLimit(void)
{
    ...
    if (argc != 2 && argc != 4 && argc != 5 && argc != 7) {
        opserr <<
            "WARNING - incorrect number of args want HHTHSIncrLimit $rhoInf $limit <-normType $T>\n";
        opserr <<
            "          or HHTHSIncrLimit $alphaI $alphaF $beta $gamma $limit <-normType $T>\n";
        return 0;
    }
    ...
    if (OPS_GetDouble(&numData, dData) != 0) {
        opserr <<
            "WARNING - invalid args want HHTHSIncrLimit $rhoInf $limit <-normType $T>\n";
        opserr <<
            "          or HHTHSIncrLimit $alphaI $alphaF $beta $gamma $limit <-normType $T>\n";
        return 0;
    }
    ...
    if (argc == 4 || argc == 7) {
        ...
        if (strcmp(argvLoc, "-normType") == 0) {
            ...
            if (OPS_GetInt(&numData, &normType) != 0) {
                opserr <<
                    "WARNING - invalid normType want HHTHSIncrLimit $rhoInf $limit <-normType $T>\n";
                opserr <<
                    "          or HHTHSIncrLimit $alphaI $alphaF $beta $gamma $limit <-normType $T>\n";
            }
        }
    ...
    if (argc < 5)
        ...
        theIntegrator =
            new HHTHSIncrLimit(dData[0], dData[1], dData[2], dData[3],
                               dData[4], normType);

    if (theIntegrator == 0)
        opserr <<
            "WARNING - out of memory creating HHTHSIncrLimit integrator\n";

    return theIntegrator;
}


HHTHSIncrLimit::HHTHSIncrLimit():TransientIntegrator(INTEGRATOR_TAGS_HHTHSIncrLimit),
alphaI(0.5), alphaF(0.5), beta(0.25), gamma(0.5), limit(0.1),
normType(2), deltaT(0.0), c1(0.0), c2(0.0), c3(0.0), Ut(0), Utdot(0),
Utdotdot(0), U(0), Udot(0), Udotdot(0), Ualpha(0), Ualphadot(0),
Ualphadotdot(0), scaledDeltaU(0)
{

}


HHTHSIncrLimit::HHTHSIncrLimit(double _rhoInf, double _limit,
                               int
                               normtype):TransientIntegrator
    (INTEGRATOR_TAGS_HHTHSIncrLimit),
...
normType(normtype), deltaT(0.0), c1(0.0), c2(0.0), c3(0.0), Ut(0),
Utdot(0), Utdotdot(0), U(0), Udot(0), Udotdot(0), Ualpha(0), Ualphadot(0),
Ualphadotdot(0), scaledDeltaU(0)
{
...
HHTHSIncrLimit::HHTHSIncrLimit(double _alphaI, double _alphaF,
                               double _beta, double _gamma, double _limit,
                               int
                               normtype):TransientIntegrator
    (INTEGRATOR_TAGS_HHTHSIncrLimit), alphaI(_alphaI), alphaF(_alphaF),
beta(_beta), gamma(_gamma), limit(_limit), normType(normtype), deltaT(0.0),
c1(0.0), c2(0.0), c3(0.0), Ut(0), Utdot(0), Utdotdot(0), U(0), Udot(0),
Udotdot(0), Ualpha(0), Ualphadot(0), Ualphadotdot(0), scaledDeltaU(0)
{
...
int
 HHTHSIncrLimit::newStep(double _deltaT)
{
    if (beta == 0 || gamma == 0) {
    ...
    if (U == 0) {
        opserr <<
            "HHTHSIncrLimit::newStep() - domainChange() failed or hasn't been called\n";
        return -3;
    }
    ...
    if (theModel->updateDomain(time, deltaT) < 0) {
        opserr <<
            "HHTHSIncrLimit::newStep() - failed to update the domain\n";
        return -4;
    }
...
int HHTHSIncrLimit::domainChanged()
{
    ...
    if (Ut == 0 || Ut->Size() != size) {
        ...
        if (Ut == 0 || Ut->Size() != size ||
            ...
            opserr <<
                "HHTHSIncrLimit::domainChanged() - ran out of memory\n";
...
int HHTHSIncrLimit::update(const Vector & deltaU)
{
    ...
    if (theModel == 0) {
        opserr <<
            "WARNING HHTHSIncrLimit::update() - no AnalysisModel set\n";
        return -1;
    }
    // check domainChanged() has been called, i.e. Ut will not be zero
    if (Ut == 0) {
        opserr <<
            "WARNING HHTHSIncrLimit::update() - domainChange() failed or not called\n";
        return -2;
    }
    // check deltaU is of correct size
    if (deltaU.Size() != U->Size()) {
        opserr <<
            "WARNING HHTHSIncrLimit::update() - Vectors of incompatible size ";
        opserr << " expecting " << U->
    ...
    if (theModel->updateDomain() < 0) {
        opserr <<
            "HHTHSIncrLimit::update() - failed to update the domain\n";
        return -4;
    }
...
int HHTHSIncrLimit::commit(void)
{
    ...
    if (theModel == 0) {
        opserr <<
            "WARNING HHTHSIncrLimit::commit() - no AnalysisModel set\n";
        return -1;
    }
    // update the response at the DOFs
    ...
    if (theModel->updateDomain() < 0) {
        opserr <<
            "HHTHSIncrLimit::commit() - failed to update the domain\n";
        return -2;
    }
...
const Vector & HHTHSIncrLimit::getVel()
{
    return *Udot;
}
...
int HHTHSIncrLimit::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (theChannel.sendVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING HHTHSIncrLimit::sendSelf() - could not send data\n";
        return -1;
    }
...
int HHTHSIncrLimit::recvSelf(int cTag, Channel & theChannel,
                             FEM_ObjectBroker & theBroker)
{
    Vector data(6);
    if (theChannel.recvVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING HHTHSIncrLimit::recvSelf() - could not receive data\n";
        return -1;
    }
