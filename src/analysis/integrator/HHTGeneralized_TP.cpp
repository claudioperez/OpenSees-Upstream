...
void *OPS_HHTGeneralized_TP(void)
{
    ...
    if (argc != 1 && argc != 4) {
        opserr <<
            "WARNING - incorrect number of args want HHTGeneralized_TP $rhoInf\n";
        opserr <<
            "          or HHTGeneralized_TP $alphaI $alphaF $beta $gamma\n";
        return 0;
    }
    ...
    if (OPS_GetDouble(&argc, dData) != 0) {
        opserr <<
            "WARNING - invalid args want HHTGeneralized_TP $rhoInf\n";
        opserr <<
            "          or HHTGeneralized_TP $alphaI $alphaF $beta $gamma\n";
        return 0;
    }
    ...
    if (theIntegrator == 0)
        opserr <<
            "WARNING - out of memory creating HHTGeneralized_TP integrator\n";

    return theIntegrator;
}


HHTGeneralized_TP::HHTGeneralized_TP():TransientIntegrator(INTEGRATOR_TAGS_HHTGeneralized_TP),
...
HHTGeneralized_TP::
HHTGeneralized_TP(double
                  _rhoInf):TransientIntegrator
(INTEGRATOR_TAGS_HHTGeneralized_TP), alphaI((2.0 - _rhoInf) / (1.0 + _rhoInf)),
alphaF(1.0 / (1.0 + _rhoInf)), beta(1.0 / (1.0 + _rhoInf) / (1.0 + _rhoInf)), gamma(0.5 * (3.0 - _rhoInf) / (1.0 + _rhoInf)),
deltaT(0.0), c1(0.0), c2(0.0), c3(0.0), alphaM(alphaI), alphaD(alphaF), alphaR(alphaF), alphaP(alphaF), Ut(0), Utdot(0), Utdotdot(0),
U(0), Udot(0), Udotdot(0), Put(0)
{
...
HHTGeneralized_TP::HHTGeneralized_TP(double _alphaI, double _alphaF,
    ...
    (INTEGRATOR_TAGS_HHTGeneralized_TP), alphaI(_alphaI), alphaF(_alphaF),
beta(_beta), gamma(_gamma), deltaT(0.0), c1(0.0), c2(0.0), c3(0.0),
alphaM(alphaI), alphaD(alphaF), alphaR(alphaF), alphaP(alphaF), Ut(0),
Utdot(0), Utdotdot(0), U(0), Udot(0), Udotdot(0), Put(0)
{
...
int
 HHTGeneralized_TP::newStep(double _deltaT)
{
    if (beta == 0 || gamma == 0) {
    ...
    if (U == 0) {
        opserr <<
            "HHTGeneralized_TP::newStep() - domainChange() failed or hasn't been called\n";
        return -4;
    }
    ...
    if (theModel->updateDomain(time, deltaT) < 0) {
        opserr <<
            "HHTGeneralized_TP::newStep() - failed to update the domain\n";
        return -5;
    }
...
int HHTGeneralized_TP::domainChanged()
{
    ...
    if (Ut == 0 || Ut->Size() != size) {
        ...
        if (Ut == 0 || Ut->Size() != size ||
            ...
            opserr <<
                "HHTGeneralized_TP::domainChanged() - ran out of memory\n";
...
int HHTGeneralized_TP::update(const Vector & deltaU)
{
    ...
    if (theModel == 0) {
        opserr <<
            "WARNING HHTGeneralized_TP::update() - no AnalysisModel set\n";
        return -1;
    }
    // check domainChanged() has been called, i.e. Ut will not be zero
    if (Ut == 0) {
        opserr <<
            "WARNING HHTGeneralized_TP::update() - domainChange() failed or not called\n";
        return -2;
    }
    // check deltaU is of correct size
    if (deltaU.Size() != U->Size()) {
        opserr <<
            "WARNING HHTGeneralized_TP::update() - Vectors of incompatible size ";
        opserr << " expecting " << U->
    ...
    if (theModel->updateDomain() < 0) {
        opserr <<
            "HHTGeneralized_TP::update() - failed to update the domain\n";
        return -4;
    }
...
const Vector & HHTGeneralized_TP::getVel()
{
    return *Udot;
}
...
int HHTGeneralized_TP::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (theChannel.sendVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING HHTGeneralized_TP::sendSelf() - could not send data\n";
        return -1;
    }
...
int HHTGeneralized_TP::recvSelf(int cTag, Channel & theChannel,
                                FEM_ObjectBroker & theBroker)
{
    Vector data(4);
    if (theChannel.recvVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING HHTGeneralized_TP::recvSelf() - could not receive data\n";
        return -1;
    }
