...
void *OPS_HHTHSIncrLimit_TP(void)
{
    ...
    if (argc != 2 && argc != 4 && argc != 5 && argc != 7) {
        opserr <<
            "WARNING - incorrect number of args want HHTHSIncrLimit_TP $rhoInf $limit <-normType $T>\n";
        opserr <<
            "          or HHTHSIncrLimit_TP $alphaI $alphaF $beta $gamma $limit <-normType $T>\n";
        return 0;
    }
    ...
    if (OPS_GetDouble(&numData, dData) != 0) {
        opserr <<
            "WARNING - invalid args want HHTHSIncrLimit_TP $rhoInf $limit <-normType $T>\n";
        opserr <<
            "          or HHTHSIncrLimit_TP $alphaI $alphaF $beta $gamma $limit <-normType $T>\n";
        return 0;
    }
    ...
    if (argc == 4 || argc == 7) {
        ...
        if (strcmp(argvLoc, "-normType") == 0) {
            ...
            if (OPS_GetInt(&numData, &normType) != 0) {
                opserr <<
                    "WARNING - invalid normType want HHTHSIncrLimit_TP $rhoInf $limit <-normType $T>\n";
                opserr <<
                    "          or HHTHSIncrLimit_TP $alphaI $alphaF $beta $gamma $limit <-normType $T>\n";
            }
        }
    ...
    if (argc < 5)
        theIntegrator =
            new HHTHSIncrLimit_TP(dData[0], dData[1], normType);
    else
        theIntegrator =
            new HHTHSIncrLimit_TP(dData[0], dData[1], dData[2], dData[3],
                                  dData[4], normType);

    if (theIntegrator == 0)
        opserr <<
            "WARNING - out of memory creating HHTHSIncrLimit_TP integrator\n";

    return theIntegrator;
}


HHTHSIncrLimit_TP::HHTHSIncrLimit_TP():TransientIntegrator(INTEGRATOR_TAGS_HHTHSIncrLimit_TP),
alphaI(0.5), alphaF(0.5), beta(0.25), gamma(0.5), limit(0.1),
normType(2), deltaT(0.0), c1(0.0), c2(0.0), c3(0.0), alphaM(0.5),
alphaD(0.5), alphaR(0.5), alphaP(0.5), Ut(0), Utdot(0), Utdotdot(0),
U(0), Udot(0), Udotdot(0), scaledDeltaU(0), Put(0)
{

}


HHTHSIncrLimit_TP::HHTHSIncrLimit_TP(double _rhoInf, double _limit,
                                     int
                                     normtype):TransientIntegrator
    (INTEGRATOR_TAGS_HHTHSIncrLimit_TP),
alphaI((2.0 - _rhoInf) / (1.0 + _rhoInf)), alphaF(1.0 / (1.0 + _rhoInf)),
beta(1.0 / (1.0 + _rhoInf) / (1.0 + _rhoInf)),
gamma(0.5 * (3.0 - _rhoInf) / (1.0 + _rhoInf)), limit(_limit),
normType(normtype), deltaT(0.0), c1(0.0), c2(0.0), c3(0.0), alphaM(alphaI),
alphaD(alphaF), alphaR(alphaF), alphaP(alphaF), Ut(0), Utdot(0),
Utdotdot(0), U(0), Udot(0), Udotdot(0), scaledDeltaU(0), Put(0)
{
...
HHTHSIncrLimit_TP::HHTHSIncrLimit_TP(double _alphaI, double _alphaF,
                                     ...
                                     int
                                     normtype):TransientIntegrator
    (INTEGRATOR_TAGS_HHTHSIncrLimit_TP), alphaI(_alphaI), alphaF(_alphaF),
beta(_beta), gamma(_gamma), limit(_limit), normType(normtype), deltaT(0.0),
c1(0.0), c2(0.0), c3(0.0), alphaM(alphaI), alphaD(alphaF), alphaR(alphaF),
alphaP(alphaF), Ut(0), Utdot(0), Utdotdot(0), U(0), Udot(0), Udotdot(0),
...
int
 HHTHSIncrLimit_TP::newStep(double _deltaT)
{
    if (beta == 0 || gamma == 0) {
    ...
    if (U == 0) {
        opserr <<
            "HHTHSIncrLimit_TP::newStep() - domainChange() failed or hasn't been called\n";
        return -4;
    }
    ...
    if (theModel->updateDomain(time, deltaT) < 0) {
        opserr <<
            "HHTHSIncrLimit_TP::newStep() - failed to update the domain\n";
        return -5;
    }
...
int HHTHSIncrLimit_TP::domainChanged()
{
    ...
    if (Ut == 0 || Ut->Size() != size) {
        ...
        if (Ut == 0 || Ut->Size() != size ||
            ...
            opserr <<
                "HHTHSIncrLimit_TP::domainChanged() - ran out of memory\n";
...
int HHTHSIncrLimit_TP::update(const Vector & deltaU)
{
    ...
    if (theModel == 0) {
        opserr <<
            "WARNING HHTHSIncrLimit_TP::update() - no AnalysisModel set\n";
        return -1;
    }
    // check domainChanged() has been called, i.e. Ut will not be zero
    if (Ut == 0) {
        opserr <<
            "WARNING HHTHSIncrLimit_TP::update() - domainChange() failed or not called\n";
        return -2;
    }
    // check deltaU is of correct size
    if (deltaU.Size() != U->Size()) {
        opserr <<
            "WARNING HHTHSIncrLimit_TP::update() - Vectors of incompatible size ";
        opserr << " expecting " << U->
    ...
    if (theModel->updateDomain() < 0) {
        opserr <<
            "HHTHSIncrLimit_TP::update() - failed to update the domain\n";
        return -4;
    }
...
const Vector & HHTHSIncrLimit_TP::getVel()
{
    return *Udot;
}
...
int HHTHSIncrLimit_TP::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (theChannel.sendVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING HHTHSIncrLimit_TP::sendSelf() - could not send data\n";
        return -1;
    }
...
int HHTHSIncrLimit_TP::recvSelf(int cTag, Channel & theChannel,
                                FEM_ObjectBroker & theBroker)
{
    Vector data(6);
    if (theChannel.recvVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING HHTHSIncrLimit_TP::recvSelf() - could not receive data\n";
        return -1;
    }
