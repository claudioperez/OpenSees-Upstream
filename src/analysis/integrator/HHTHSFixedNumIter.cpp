...
void *OPS_HHTHSFixedNumIter(void)
{
    ...
    if (argc != 1 && argc != 3 && argc != 4 && argc != 6) {
        opserr <<
            "WARNING - incorrect number of args want HHTHSFixedNumIter $rhoInf <-polyOrder $O>\n";
        opserr <<
            "          or HHTHSFixedNumIter $alphaI $alphaF $beta $gamma <-polyOrder $O>\n";
        return 0;
    }
    ...
    if (OPS_GetDouble(&numData, dData) != 0) {
        opserr <<
            "WARNING - invalid args want HHTHSFixedNumIter $rhoInf <-polyOrder $O>\n";
        opserr <<
            "          or HHTHSFixedNumIter $alphaI $alphaF $beta $gamma <-polyOrder $O>\n";
        return 0;
    }
    ...
    if (argc == 3 || argc == 6) {
        ...
        if (strcmp(argvLoc, "-polyOrder") == 0) {
            ...
            if (OPS_GetInt(&numData, &polyOrder) != 0) {
                opserr <<
                    "WARNING - invalid polyOrder want HHTHSFixedNumIter $rhoInf <-polyOrder $O>\n";
                opserr <<
                    "          or HHTHSFixedNumIter $alphaI $alphaF $beta $gamma <-polyOrder $O>\n";
            }
        }
    ...
    if (argc < 4)
        theIntegrator =
            new HHTHSFixedNumIter(dData[0], polyOrder, updDomFlag);
    else
        theIntegrator =
            new HHTHSFixedNumIter(dData[0], dData[1], dData[2], dData[3],
                                  polyOrder, updDomFlag);

    if (theIntegrator == 0)
        opserr <<
            "WARNING - out of memory creating HHTHSFixedNumIter integrator\n";

    return theIntegrator;
}


HHTHSFixedNumIter::HHTHSFixedNumIter():TransientIntegrator(INTEGRATOR_TAGS_HHTHSFixedNumIter),
alphaI(0.5), alphaF(0.5), beta(0.25), gamma(0.5),
polyOrder(2), updDomFlag(true),
deltaT(0.0), c1(0.0), c2(0.0), c3(0.0), x(1.0),
Ut(0), Utdot(0), Utdotdot(0), U(0), Udot(0), Udotdot(0),
Ualpha(0), Ualphadot(0), Ualphadotdot(0), Utm1(0), Utm2(0), scaledDeltaU(0)
{
...
HHTHSFixedNumIter::HHTHSFixedNumIter(double _rhoInf,
                                     int polyorder,
                                     bool
                                     upddomflag):TransientIntegrator
    (INTEGRATOR_TAGS_HHTHSFixedNumIter),
alphaI((2.0 - _rhoInf) / (1.0 + _rhoInf)), alphaF(1.0 / (1.0 + _rhoInf)),
beta(1.0 / (1.0 + _rhoInf) / (1.0 + _rhoInf)),
gamma(0.5 * (3.0 - _rhoInf) / (1.0 + _rhoInf)), polyOrder(polyorder),
updDomFlag(upddomflag), deltaT(0.0), c1(0.0), c2(0.0), c3(0.0), x(1.0),
Ut(0), Utdot(0), Utdotdot(0), U(0), Udot(0), Udotdot(0), Ualpha(0),
Ualphadot(0), Ualphadotdot(0), Utm1(0), Utm2(0), scaledDeltaU(0)
{
...
HHTHSFixedNumIter::HHTHSFixedNumIter(double _alphaI, double _alphaF,
                                     ...
                                     bool
                                     upddomflag):TransientIntegrator
    (INTEGRATOR_TAGS_HHTHSFixedNumIter), alphaI(_alphaI), alphaF(_alphaF),
beta(_beta), gamma(_gamma), polyOrder(polyorder), updDomFlag(upddomflag),
deltaT(0.0), c1(0.0), c2(0.0), c3(0.0), x(1.0), Ut(0), Utdot(0),
Utdotdot(0), U(0), Udot(0), Udotdot(0), Ualpha(0), Ualphadot(0),
Ualphadotdot(0), Utm1(0), Utm2(0), scaledDeltaU(0)
{
 ...
 HHTHSFixedNumIter::newStep(double _deltaT)
{
    ...
    if (U == 0) {
        opserr <<
            "HHTHSFixedNumIter::newStep() - domainChange() failed or hasn't been called\n";
        return -3;
    }
...
int HHTHSFixedNumIter::domainChanged()
{
    ...
    if (Ut == 0 || Ut->Size() != size) {
        ...
        if (Ut == 0 || Ut->Size() != size ||
            ...
            opserr <<
                "HHTHSFixedNumIter::domainChanged() - ran out of memory\n";
...
int HHTHSFixedNumIter::update(const Vector & deltaU)
{
    ...
    if (theModel == 0) {
        opserr <<
            "WARNING HHTHSFixedNumIter::update() - no AnalysisModel set\n";
        return -1;
    }
    ConvergenceTest *theTest = this->getConvergenceTest();
    if (theTest == 0) {
        opserr <<
            "WARNING HHTHSFixedNumIter::update() - no ConvergenceTest set\n";
        return -2;
    }
    // check domainChanged() has been called, i.e. Ut will not be zero
    if (Ut == 0) {
        opserr <<
            "WARNING HHTHSFixedNumIter::update() - domainChange() failed or not called\n";
        return -3;
    }
    // check deltaU is of correct size
    if (deltaU.Size() != U->Size()) {
        opserr <<
            "WARNING HHTHSFixedNumIter::update() - Vectors of incompatible size ";
        opserr << " expecting " << U->
    ...
    if (polyOrder == 1) {
        ...
        opserr <<
            "WARNING HHTHSFixedNumIter::update() - polyOrder > 3 not supported\n";
        return -5;
    }
    ...
    if (theModel->updateDomain() < 0) {
        opserr <<
            "HHTHSFixedNumIter::update() - failed to update the domain\n";
        return -6;
    }
...
int HHTHSFixedNumIter::commit(void)
{
    ...
    if (theModel == 0) {
        opserr <<
            "WARNING HHTHSFixedNumIter::commit() - no AnalysisModel set\n";
        return -1;
    }
    ...
    if (updDomFlag == true) {
        ...
        if (theSOE == 0) {
            opserr <<
                "WARNING HHTHSFixedNumIter::commit() - no LinearSOE set\n";
            return -2;
        }
...
const Vector & HHTHSFixedNumIter::getVel()
{
    return *Udot;
}
...
int HHTHSFixedNumIter::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (theChannel.sendVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING HHTHSFixedNumIter::sendSelf() - could not send data\n";
        return -1;
    }
...
int HHTHSFixedNumIter::recvSelf(int cTag, Channel & theChannel,
                                FEM_ObjectBroker & theBroker)
{
    Vector data(6);
    if (theChannel.recvVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING HHTHSFixedNumIter::recvSelf() - could not receive data\n";
        return -1;
    }
