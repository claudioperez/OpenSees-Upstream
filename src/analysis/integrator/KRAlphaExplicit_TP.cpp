...
int
KRAlphaExplicit_TP::revertToLastStep ()
{
...
int
KRAlphaExplicit_TP::formUnbalance ()
{
    // get a pointer to the LinearSOE and the AnalysisModel
    LinearSOE *theLinSOE = this->getLinearSOE ();
    AnalysisModel *theModel = this->getAnalysisModel ();
...
int
KRAlphaExplicit_TP::formNodTangent (DOF_Group * theDof)
{
    theDof->zeroTangent ();
...
int
KRAlphaExplicit_TP::formEleResidual (FE_Element * theEle)
{
    theEle->zeroResidual ();
...
int
KRAlphaExplicit_TP::formNodUnbalance (DOF_Group * theDof)
{
...
int
KRAlphaExplicit_TP::domainChanged ()
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    LinearSOE *theLinSOE = this->getLinearSOE ();
...
int
KRAlphaExplicit_TP::update (const Vector & aiPlusOne)
{
    updateCount++;
    if (updateCount > 1)
      {
...
int
KRAlphaExplicit_TP::commit (void)
{
    // get a pointer to the LinearSOE and the AnalysisModel
    LinearSOE *theLinSOE = this->getLinearSOE ();
    AnalysisModel *theModel = this->getAnalysisModel ();
...
int
KRAlphaExplicit_TP::sendSelf (int cTag, Channel & theChannel)
{
    Vector data (4);
    data (0) = alphaI;
    data (1) = alphaF;
...
int
KRAlphaExplicit_TP::recvSelf (int cTag, Channel & theChannel,
...
void
KRAlphaExplicit_TP::Print (OPS_Stream & s, int flag)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel != 0)
      {
