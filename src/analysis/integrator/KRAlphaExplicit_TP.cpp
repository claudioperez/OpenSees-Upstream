...
void *OPS_KRAlphaExplicit_TP(void)
{
    ...
    if (argc != 1) {
        opserr <<
            "WARNING - incorrect number of args want KRAlphaExplicit_TP $rhoInf\n";
        return 0;
    }
    ...
    if (OPS_GetDouble(&argc, &rhoInf) != 0) {
        opserr <<
            "WARNING - invalid args want KRAlphaExplicit_TP $rhoInf\n";
        return 0;
    }
...
KRAlphaExplicit_TP::KRAlphaExplicit_TP():TransientIntegrator(INTEGRATOR_TAGS_KRAlphaExplicit_TP),
...
U(0), Udot(0), Udotdot(0), Utdothat(0), Put(0)
{
...
KRAlphaExplicit_TP::
KRAlphaExplicit_TP(double
                   _rhoInf):TransientIntegrator
(INTEGRATOR_TAGS_KRAlphaExplicit_TP), alphaI((2.0 - _rhoInf) / (1.0 + _rhoInf)),
alphaF(1.0 / (1.0 + _rhoInf)), beta(1.0 / (1.0 + _rhoInf) / (1.0 + _rhoInf)), gamma(0.5 * (3.0 - _rhoInf) / (1.0 + _rhoInf)), deltaT(0.0),
alpha1(0), alpha3(0), Mhat(0), updateCount(0), initAlphaMatrices(1), c1(0.0), c2(0.0), c3(0.0), alphaM(0.0), alphaD(alphaF),
alphaR(alphaF), alphaP(alphaF), Ut(0), Utdot(0), Utdotdot(0), U(0), Udot(0), Udotdot(0), Utdothat(0), Put(0)
{
 ...
 KRAlphaExplicit_TP::newStep(double _deltaT)
{
    ...
    if (beta == 0 || gamma == 0) {
        opserr <<
            "WARNING KRAlphaExplicit_TP::newStep() - error in variable\n";
    ...
    if (initAlphaMatrices || _deltaT != deltaT) {
        ...
        if (theFullLinSOE == 0) {
            opserr <<
                "WARNING KRAlphaExplicit_TP::newStep() - failed to create FullLinearSOE\n";
            return -4;
        }
    ...
    if (U == 0) {
        opserr <<
            "WARNING KRAlphaExplicit_TP::newStep() - domainChange() failed or hasn't been called\n";
        return -6;
    }
    ...
    if (theModel->updateDomain(time, deltaT) < 0) {
        opserr <<
            "WARNING KRAlphaExplicit_TP::newStep() - failed to update the domain\n";
        return -7;
    }
...
int KRAlphaExplicit_TP::domainChanged()
{
    ...
    if (Ut == 0 || Ut->Size() != size) {
        ...
        if (alpha1 == 0 || alpha1->noRows() != size
            ...
            || Udotdot->Size() != size || Utdothat == 0
            || Utdothat->Size() != size || Put == 0
            || Put->Size() != size) {
...
int KRAlphaExplicit_TP::update(const Vector & aiPlusOne)
{
    ...
    if (updateCount > 1) {
        opserr <<
            "WARNING KRAlphaExplicit_TP::update() - called more than once -";
        opserr <<
            " KRAlphaExplicit_TP integration scheme requires a LINEAR solution algorithm\n";
        return -1;
    }
    ...
    if (theModel == 0) {
        opserr <<
            "WARNING KRAlphaExplicit_TP::update() - no AnalysisModel set\n";
        return -2;
    }
    // check domainChanged() has been called, i.e. Ut will not be zero
    if (Ut == 0) {
        opserr <<
            "WARNING KRAlphaExplicit_TP::update() - domainChange() failed or not called\n";
        return -3;
    }
    // check aiPlusOne is of correct size
    if (aiPlusOne.Size() != U->Size()) {
        opserr <<
            "WARNING KRAlphaExplicit_TP::update() - Vectors of incompatible size ";
        opserr << " expecting " << U->
    ...
    if (theModel->updateDomain() < 0) {
        opserr <<
            "WARNING KRAlphaExplicit_TP::update() - failed to update the domain\n";
        return -5;
    }
...
const Vector & KRAlphaExplicit_TP::getVel()
{
    return *Udot;
}
...
int KRAlphaExplicit_TP::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (theChannel.sendVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING KRAlphaExplicit_TP::sendSelf() - could not send data\n";
        return -1;
    }
...
int KRAlphaExplicit_TP::recvSelf(int cTag, Channel & theChannel,
                                 FEM_ObjectBroker & theBroker)
{
    Vector data(4);
    if (theChannel.recvVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING KRAlphaExplicit_TP::recvSelf() - could not receive data\n";
        return -1;
    }
...
void KRAlphaExplicit_TP::Print(OPS_Stream & s, int flag)
{
    ...
    if (theModel != 0) {
        ...
        s << "  alphaI: " << alphaI << "  alphaF: " << alphaF << "  beta: "
