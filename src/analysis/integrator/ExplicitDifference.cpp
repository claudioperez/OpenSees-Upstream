...
Explicitdifference::Explicitdifference(double _alphaM, double _betaK,
    ...
    (INTEGRATOR_TAGS_ExplicitDifference), deltaT(0.0), alphaM(_alphaM),
betaK(_betaK), betaKi(_betaKi), betaKc(_betaKc), updateCount(0), c2(0.0),
c3(0.0), Ut(0), Utdot(0), Utdotdot(0), Udot(0), Utdotdot1(0), U(0),
Utdot1(0)
{
...
int Explicitdifference::update(const Vector & Udotdot)
{
    ...
    if (Udotdot.Size() != Utdotdot->Size()) {
        ...
        opserr << " expecting " << Utdotdot->Size() << " obtained " <<
            Udotdot.Size() << endln;
        return -4;
    }
...
int Explicitdifference::recvSelf(int cTag, Channel & theChannel,
...
const Vector & Explicitdifference::getVel()
{
    return *Utdot;
}
