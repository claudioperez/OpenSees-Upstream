...
Explicitdifference::Explicitdifference ():TransientIntegrator (INTEGRATOR_TAGS_ExplicitDifference),
deltaT (0.0),
alphaM (0.0), betaK (0.0), betaKi (0.0), betaKc (0.0),
updateCount (0), c2 (0.0), c3 (0.0),
Ut (0), Utdot (0), Utdotdot (0), Udot (0), Utdotdot1 (0), U (0), Utdot1 (0)
{

}


Explicitdifference::Explicitdifference (double _alphaM, double _betaK,
...
Explicitdifference::newStep (double _deltaT)
{
    ...
    if (Ut == 0)
      {
          opserr <<
              "Explicitdifference::newStep() - domainChange() failed or hasn't been called\n";
          return -2;
      }
    ...
    if (theModel->updateDomain (time, deltaT) < 0)
      {
          opserr <<
              "Explicitdifference::newStep() - failed to update the domain\n";
          return -3;
      }
...
Explicitdifference::domainChanged ()
{
    ...
    if (Ut == 0 || Ut->Size () != size)
      {
          ...
          if (Ut == 0 || Ut->Size () != size ||
              ...
              Utdot1 == 0 || Utdot1->Size () != size)
            {
                ...
                opserr <<
                    "Explicitdifference::domainChanged - ran out of memory\n";
    ...
    opserr <<
        "WARNING: Explicitdifference::domainChanged() - assuming Ut-1 = Ut\n";
...
int
Explicitdifference::update (const Vector & Udotdot)
{
    updateCount++;
    if (updateCount > 2)
      {
          opserr <<
              "WARNING Explicitdifference::update() - called more than once -";
          opserr <<
              " Explicitdifference integration scheme requires a LINEAR solution algorithm\n";
          return -1;
      }
    ...
    if (theModel == 0)
      {
          opserr <<
              "WARNING Explicitdifference::update() - no souAnalysisModel set\n";
          return -2;
      }
    ...
    if (Ut == 0)
      {
          opserr <<
              "WARNING Explicitdifference::update() - domainChange() failed or not called\n";
          return -3;
      }
    ...
    if (Udotdot.Size () != Utdotdot->Size ())
      {
          opserr <<
              "WARNING Explicitdifference::update() - Vectors of incompatible size ";
          opserr << " expecting " << Utdotdot->
              Size () << " obtained " << Udotdot.Size () << endln;
          return -4;
      }
    ...
    if (theModel->updateDomain () < 0)
      {
          opserr <<
              "Explicitdifference::update() - failed to update the domain\n";
          return -5;
      }
...
int
Explicitdifference::commit (void)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel == 0)
      {
          opserr <<
              "WARNING Explicitdifference::commit() - no AnalysisModel set\n";
          return -1;
      }
...
int
Explicitdifference::sendSelf (int cTag, Channel & theChannel)
{
    Vector data (4);
    data (0) = alphaM;
    data (1) = betaK;
    ...
    if (theChannel.sendVector (this->getDbTag (), cTag, data) < 0)
      {
          opserr <<
              "WARNING Explicitdifference::sendSelf() - could not send data\n";
          return -1;
      }
...
int
Explicitdifference::recvSelf (int cTag, Channel & theChannel,
                              FEM_ObjectBroker & theBroker)
{
    ...
    if (theChannel.recvVector (this->getDbTag (), cTag, data) < 0)
      {
          opserr <<
              "WARNING Explicitdifference::recvSelf() - could not receive data\n";
          return -1;
      }
...
void
Explicitdifference::Print (OPS_Stream & s, int flag)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel != 0)
      {
