...
EQPath::EQPath(double arcLen,
               int method):StaticIntegrator(INTEGRATOR_TAGS_EQPath),
arclen(arcLen), uq(0), uq0(0), ur(0), du(0), du0(0), uqn(0), q(0),
type(method), m(1), dl(0), changed(0), nitr(0)
{
...
int EQPath::update(const Vector & dU)
{
    AnalysisModel *theModel = this->getAnalysisModel();
    LinearSOE *theLinSOE = this->getLinearSOE();
    if (theModel == 0 || theLinSOE == 0) {
...
int EQPath::domainChanged(void)
{
    ...
    if (uq == 0 || uq->Size() != size) {        // create new Vector
        ...
        if (uq == 0 || uq->Size() != size) {    // check got it
            opserr <<
                "FATAL EQPath::domainChanged() - ran out of memory for";
...
int EQPath::sendSelf(int cTag, Channel & theChannel)
{
    Vector data(3);
    data(0) = arclen;
    data(1) = dl;
...
void EQPath::Print(OPS_Stream & s, int flag)
{
    AnalysisModel *theModel = this->getAnalysisModel();
    if (theModel != 0) {
