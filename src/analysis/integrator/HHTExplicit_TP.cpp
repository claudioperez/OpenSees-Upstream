...
void *OPS_HHTExplicit_TP(void)
{
    ...
    if (argc < 1 || argc > 2) {
        opserr <<
            "WARNING - incorrect number of args want HHTExplicit_TP $alpha\n";
    ...
    if (theIntegrator == 0)
        opserr <<
            "WARNING - out of memory creating HHTExplicit_TP integrator\n";

    return theIntegrator;
}


HHTExplicit_TP::HHTExplicit_TP():TransientIntegrator(INTEGRATOR_TAGS_HHTExplicit_TP),
alpha(1.0), gamma(0.5),
...
HHTExplicit_TP::
HHTExplicit_TP(double
               _alpha):TransientIntegrator(INTEGRATOR_TAGS_HHTExplicit_TP),
alpha(_alpha), gamma(0.5), deltaT(0.0), updateCount(0), c2(0.0), c3(0.0),
alphaD(alpha), alphaR(alpha), alphaP(alpha), Ut(0), Utdot(0), Utdotdot(0),
...
HHTExplicit_TP::HHTExplicit_TP(double _alpha,
    ...
    (INTEGRATOR_TAGS_HHTExplicit_TP), alpha(_alpha), gamma(_gamma),
deltaT(0.0), updateCount(0), c2(0.0), c3(0.0), alphaD(alpha),
alphaR(alpha), alphaP(alpha), Ut(0), Utdot(0), Utdotdot(0), U(0), Udot(0),
Udotdot(0), Put(0)
{
...
int
 HHTExplicit_TP::newStep(double _deltaT)
{
    updateCount = 0;
    ...
    if (U == 0) {
        opserr <<
            "HHTExplicit_TP::newStep() - domainChange() failed or hasn't been called\n";
        return -4;
    }
    ...
    if (theModel->updateDomain(time, deltaT) < 0) {
        opserr <<
            "HHTExplicit_TP::newStep() - failed to update the domain\n";
        return -5;
    }
...
int HHTExplicit_TP::domainChanged()
{
    ...
    if (Ut == 0 || Ut->Size() != size) {
        ...
        if (Ut == 0 || Ut->Size() != size ||
            ...
            opserr <<
                "HHTExplicit_TP::domainChanged() - ran out of memory\n";
...
int HHTExplicit_TP::update(const Vector & aiPlusOne)
{
    ...
    if (updateCount > 1) {
        opserr <<
            "WARNING HHTExplicit_TP::update() - called more than once -";
        opserr <<
            " HHTExplicit_TP integration scheme requires a LINEAR solution algorithm\n";
        return -1;
    }
    ...
    if (theModel == 0) {
        opserr <<
            "WARNING HHTExplicit_TP::update() - no AnalysisModel set\n";
        return -2;
    }
    // check domainChanged() has been called, i.e. Ut will not be zero
    if (Ut == 0) {
        opserr <<
            "WARNING HHTExplicit_TP::update() - domainChange() failed or not called\n";
        return -3;
    }
    // check aiPlusOne is of correct size
    if (aiPlusOne.Size() != U->Size()) {
        opserr <<
            "WARNING HHTExplicit_TP::update() - Vectors of incompatible size ";
        opserr << " expecting " << U->
    ...
    if (theModel->updateDomain() < 0) {
        opserr <<
            "HHTExplicit_TP::update() - failed to update the domain\n";
        return -5;
    }
...
const Vector & HHTExplicit_TP::getVel()
{
    return *Udot;
}
...
int HHTExplicit_TP::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (theChannel.sendVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING HHTExplicit_TP::sendSelf() - could not send data\n";
        return -1;
    }
...
int HHTExplicit_TP::recvSelf(int cTag, Channel & theChannel,
                             FEM_ObjectBroker & theBroker)
{
    Vector data(2);
    if (theChannel.recvVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING HHTExplicit_TP::recvSelf() - could not receive data\n";
        return -1;
    }
