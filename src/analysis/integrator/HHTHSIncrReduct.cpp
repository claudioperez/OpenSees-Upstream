...
void *OPS_HHTHSIncrReduct(void)
{
    ...
    if (argc != 2 && argc != 5) {
        opserr <<
            "WARNING - incorrect number of args want HHTHSIncrReduct $rhoInf $reduct\n";
        opserr <<
            "          or HHTHSIncrReduct $alphaI $alphaF $beta $gamma $reduct\n";
        return 0;
    }
    ...
    if (OPS_GetDouble(&argc, dData) != 0) {
        opserr <<
            "WARNING - invalid args want HHTHSIncrReduct $rhoInf $reduct\n";
        opserr <<
            "          or HHTHSIncrReduct $alphaI $alphaF $beta $gamma $reduct\n";
        return 0;
    }
    ...
    if (argc == 2)
        ...
        theIntegrator =
            new HHTHSIncrReduct(dData[0], dData[1], dData[2], dData[3],
                                dData[4]);

    if (theIntegrator == 0)
        opserr <<
            "WARNING - out of memory creating HHTHSIncrReduct integrator\n";

    return theIntegrator;
}


HHTHSIncrReduct::HHTHSIncrReduct():TransientIntegrator(INTEGRATOR_TAGS_HHTHSIncrReduct),
alphaI(0.5), alphaF(0.5), beta(0.25), gamma(0.5), reduct(1.0),
deltaT(0.0), c1(0.0), c2(0.0), c3(0.0),
Ut(0), Utdot(0), Utdotdot(0), U(0), Udot(0), Udotdot(0),
Ualpha(0), Ualphadot(0), Ualphadotdot(0), scaledDeltaU(0)
{

}


HHTHSIncrReduct::HHTHSIncrReduct(double _rhoInf,
                                 double
                                 _reduct):TransientIntegrator
    (INTEGRATOR_TAGS_HHTHSIncrReduct),
...
deltaT(0.0), c1(0.0), c2(0.0), c3(0.0), Ut(0), Utdot(0), Utdotdot(0), U(0),
Udot(0), Udotdot(0), Ualpha(0), Ualphadot(0), Ualphadotdot(0),
scaledDeltaU(0)
{
...
HHTHSIncrReduct::HHTHSIncrReduct(double _alphaI, double _alphaF,
                                 ...
                                 double
                                 _reduct):TransientIntegrator
    (INTEGRATOR_TAGS_HHTHSIncrReduct), alphaI(_alphaI), alphaF(_alphaF),
beta(_beta), gamma(_gamma), reduct(_reduct), deltaT(0.0), c1(0.0), c2(0.0),
c3(0.0), Ut(0), Utdot(0), Utdotdot(0), U(0), Udot(0), Udotdot(0),
Ualpha(0), Ualphadot(0), Ualphadotdot(0), scaledDeltaU(0)
{
...
int
 HHTHSIncrReduct::newStep(double _deltaT)
{
    if (beta == 0 || gamma == 0) {
    ...
    if (U == 0) {
        opserr <<
            "HHTHSIncrReduct::newStep() - domainChange() failed or hasn't been called\n";
        return -3;
    }
    ...
    if (theModel->updateDomain(time, deltaT) < 0) {
        opserr <<
            "HHTHSIncrReduct::newStep() - failed to update the domain\n";
        return -4;
    }
...
int HHTHSIncrReduct::domainChanged()
{
    ...
    if (Ut == 0 || Ut->Size() != size) {
        ...
        if (Ut == 0 || Ut->Size() != size ||
            ...
            opserr <<
                "HHTHSIncrReduct::domainChanged() - ran out of memory\n";
...
int HHTHSIncrReduct::update(const Vector & deltaU)
{
    ...
    if (theModel == 0) {
        opserr <<
            "WARNING HHTHSIncrReduct::update() - no AnalysisModel set\n";
        return -1;
    }
    // check domainChanged() has been called, i.e. Ut will not be zero
    if (Ut == 0) {
        opserr <<
            "WARNING HHTHSIncrReduct::update() - domainChange() failed or not called\n";
        return -2;
    }
    // check deltaU is of correct size
    if (deltaU.Size() != U->Size()) {
        opserr <<
            "WARNING HHTHSIncrReduct::update() - Vectors of incompatible size ";
        opserr << " expecting " << U->
    ...
    if (theModel->updateDomain() < 0) {
        opserr <<
            "HHTHSIncrReduct::update() - failed to update the domain\n";
        return -4;
    }
...
int HHTHSIncrReduct::commit(void)
{
    ...
    if (theModel == 0) {
        opserr <<
            "WARNING HHTHSIncrReduct::commit() - no AnalysisModel set\n";
        return -1;
    }
    // update the response at the DOFs
    ...
    if (theModel->updateDomain() < 0) {
        opserr <<
            "HHTHSIncrReduct::commit() - failed to update the domain\n";
        return -2;
    }
...
const Vector & HHTHSIncrReduct::getVel()
{
    return *Udot;
}
...
int HHTHSIncrReduct::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (theChannel.sendVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING HHTHSIncrReduct::sendSelf() - could not send data\n";
        return -1;
    }
...
int HHTHSIncrReduct::recvSelf(int cTag, Channel & theChannel,
                              FEM_ObjectBroker & theBroker)
{
    Vector data(5);
    if (theChannel.recvVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING HHTHSIncrReduct::recvSelf() - could not receive data\n";
        return -1;
    }
