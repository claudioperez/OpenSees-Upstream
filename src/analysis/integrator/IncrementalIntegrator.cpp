...
IncrementalIntegrator(int clasTag):Integrator(clasTag),
statusFlag(CURRENT_TANGENT), theEigenSOE(0), eigenVectors(0),
eigenValues(0), dampingForces(0), isDiagonal(false), diagMass(0), mV(0),
tmpV1(0), tmpV2(0), theSOE(0), theAnalysisModel(0), theTest(0)
{
...
void
 IncrementalIntegrator::setLinks(AnalysisModel & theModel,
                                 LinearSOE & theLinSOE,
...
void IncrementalIntegrator::setEigenSOE(EigenSOE * theEigSOE)
{
    theEigenSOE = theEigSOE;
}

int IncrementalIntegrator::formTangent(int statFlag)
{
    int result = 0;
    statusFlag = statFlag;
...
int IncrementalIntegrator::formTangent(int statFlag, double iFact,
                                       double cFact)
{
    iFactor = iFact;
    cFactor = cFact;
...
int IncrementalIntegrator::formIndependentSensitivityLHS(int statFlag)
{
    return this->formTangent(statFlag);
}

int IncrementalIntegrator::formUnbalance(void)
{
    if (theAnalysisModel == 0 || theSOE == 0) {
...
int IncrementalIntegrator::getLastResponse(Vector & result, const ID & id)
{
...
int IncrementalIntegrator::newStep(double deltaT)
{
    return 0;
}


int IncrementalIntegrator::initialize(void)
{
    return 0;
}

int IncrementalIntegrator::commit(void)
{
...
int IncrementalIntegrator::revertToLastStep(void)
{
    return 0;
}
...
LinearSOE *IncrementalIntegrator::getLinearSOE(void) const
{
    return theSOE;
}

ConvergenceTest *IncrementalIntegrator::getConvergenceTest(void) const
{
    return theTest;
}

AnalysisModel *IncrementalIntegrator::getAnalysisModel(void) const
{
    return theAnalysisModel;
}

int IncrementalIntegrator::formNodalUnbalance(void)
{
    ...
    while ((dofPtr = theDOFs()) != 0) {
        ...
        if (theSOE->addB(dofPtr->getUnbalance(this), dofPtr->getID()) < 0) {
            opserr <<
                "WARNING IncrementalIntegrator::formNodalUnbalance -";
...
int IncrementalIntegrator::formElementResidual(void)
{
    ...
    while ((elePtr = theEles2()) != 0) {
        ...
        if (theSOE->addB(elePtr->getResidual(this), elePtr->getID()) < 0) {
            opserr <<
                "WARNING IncrementalIntegrator::formElementResidual -";
...
int IncrementalIntegrator::addModalDampingForce(const Vector *
                                                modalDampingValues)
{
    int res = 0;
...
int IncrementalIntegrator::addModalDampingMatrix(const Vector *
                                                 modalDampingValues)
{
    int res = 0;
    //    return 0;
...
const Vector & IncrementalIntegrator::getVel(void)
{
    opserr <<
...
int IncrementalIntegrator::doMv(const Vector & v, Vector & res)
{
