    ...
    for (int i = 0; i < id.Size (); i++)
      {
          ...
          if (loc < 0)
            ...
            {
                opserr <<
                    "WARNING IncrementalIntegrator::getLastResponse() -";
    ...
    while ((dofPtr = theDOFs ()) != 0)
      {
          ...
          if (theSOE->addB (dofPtr->getUnbalance (this), dofPtr->getID ()) <
              0)
            {
                opserr <<
...
int
IncrementalIntegrator::addModalDampingForce (const Vector *
                                             modalDampingValues)
{
    int res = 0;
...
int
IncrementalIntegrator::addModalDampingMatrix (const Vector *
                                              modalDampingValues)
{
    int res = 0;
    //    return 0;
...
double
IncrementalIntegrator::getCFactor (void)
{
    return 0;
}
