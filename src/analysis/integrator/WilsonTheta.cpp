...
void *OPS_WilsonTheta(void)
{
    ...
    if (argc != 1) {
        opserr <<
            "WARNING - incorrect number of args want WilsonTheta $theta\n";
        return 0;
    }
    ...
    if (theIntegrator == 0)
        opserr <<
            "WARNING - out of memory creating WilsonTheta integrator\n";

    return theIntegrator;
}


WilsonTheta::WilsonTheta():TransientIntegrator(INTEGRATOR_TAGS_WilsonTheta),
theta(0), deltaT(0),
...
WilsonTheta::
WilsonTheta(double
            _theta):TransientIntegrator(INTEGRATOR_TAGS_WilsonTheta),
theta(_theta), deltaT(0.0), c1(0.0), c2(0.0), c3(0.0), Ut(0), Utdot(0),
 ...
 WilsonTheta::newStep(double _deltaT)
{
    ...
    if (U == 0) {
        opserr <<
            "WilsonTheta::newStep() - domainChange() failed or hasn't been called\n";
        return -3;
    }
...
int WilsonTheta::update(const Vector & deltaU)
{
    ...
    if (Ut == 0) {
        opserr <<
            "WARNING WilsonTheta::update() - domainChange() failed or not called\n";
        return -2;
    }
    // check deltaU is of correct size
    if (deltaU.Size() != U->Size()) {
        opserr <<
            "WARNING WilsonTheta::update() - Vectors of incompatible size ";
        opserr << " expecting " << U->
...
const Vector & WilsonTheta::getVel()
{
    return *Udot;
}
...
int WilsonTheta::recvSelf(int cTag, Channel & theChannel,
                          FEM_ObjectBroker & theBroker)
{
    Vector data(1);
    if (theChannel.recvVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING WilsonTheta::recvSelf() - could not receive data\n";
        return -1;
    }
