...
void *OPS_HSConstraint()
{
    ...
    if (numdata < 1) {
        opserr <<
            "WARNING integrator HSConstraint <arcLength> <psi_u> <psi_f> <u_ref> \n";
        return 0;
    }
    if (numdata > 4)
        numdata = 4;
    ...
    if (OPS_GetDoubleInput(&numdata, data) < 0) {
        opserr <<
            "WARNING integrator HSConstraint invalid double inputs\n";
        return 0;
    }
...
HSConstraint::HSConstraint(double arcLength, double psi_u, double psi_f, double u_ref):StaticIntegrator(INTEGRATOR_TAGS_HSConstraint), arcLength2(arcLength * arcLength),       /*alpha2(alpha*alpha), */
psi_u2(psi_u * psi_u), psi_f2(psi_f * psi_f), u_ref2(u_ref * u_ref),    //new added
deltaUhat(0),
deltaUbar(0),
deltaU(0),
deltaUstep(0),
...
// old version with fext
    double dLambda =
        sqrt(arcLength2 /
             ((psi_u2 / u_ref2 * fabs(dUhat ^ dUhat)) +
              psi_f2 * (f_ext ^ f_ext)));
...
int HSConstraint::update(const Vector & dU)
{
    ...
    double a1 = psi_u2 / u_ref2 * ((*deltaUhat) ^ (*deltaUhat))
        + psi_f2 * (f_ext ^ f_ext);

    double a2 =
        2.0 * (psi_u2 / u_ref2 *
               (((*deltaUhat) ^ (*deltaUbar)) +
                ((*deltaUhat) ^ (*deltaUstep))) +
               psi_f2 * deltaLambdaStep * (f_ext ^ f_ext));

    double a3 =
        psi_u2 / u_ref2 * ((*deltaUstep) + (*deltaUbar)) ^ ((*deltaUstep) +
                                                            (*deltaUbar)) -
        arcLength2 +
    ...
    if (b24ac < 0) {
        opserr <<
            "HSConstraint::update() - imaginary roots due to multiple instability";
        opserr << " directions - initial load increment was too large\n";
        opserr << "a1: " << a1 << " a2: " << a2 << " a3: " << a3 <<
            " b24ac: " << b24ac << endln;
        return -1;
    }
    double dLambda;
...
int HSConstraint::domainChanged(void)
{
    ...
    if (deltaUhat == 0 || deltaUhat->Size() != size) {  // create new Vector
        ...
        if (deltaUhat == 0 || deltaUhat->Size() != size) {      // check got it
            opserr <<
                "FATAL HSConstraint::domainChanged() - ran out of memory for";
    ...
    if (haveLoad == 0) {
        opserr <<
            "WARNING HSConstraint::domainChanged() - zero reference load";
        return -1;
    }
...
int HSConstraint::sendSelf(int cTag, Channel & theChannel)
{
    Vector data(4);
    data(0) = arcLength2;
    //data(1) = alpha2;
...
int HSConstraint::recvSelf(int cTag,
                           Channel & theChannel,
                           FEM_ObjectBroker & theBroker)
{
    ...
    if (theChannel.recvVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "HSConstraint::recvSelf() - failed to receive the data\n";
        return -1;
    }
    // set the data
    arcLength2 = data(0);
...
void HSConstraint::Print(OPS_Stream & s, int flag)
{
    AnalysisModel *theModel = this->getAnalysisModel();
    if (theModel != 0) {
