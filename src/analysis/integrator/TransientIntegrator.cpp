...
TransientIntegrator::
TransientIntegrator(int clasTag):IncrementalIntegrator(clasTag)
{
...
int TransientIntegrator::formTangent(int statFlag)
{
    int result = 0;
    statusFlag = statFlag;
...
int TransientIntegrator::formUnbalance(void)
{
    LinearSOE *theLinSOE = this->getLinearSOE();
    AnalysisModel *theModel = this->getAnalysisModel();
...
int TransientIntegrator::formEleResidual(FE_Element * theEle)
{
    theEle->zeroResidual();
...
int TransientIntegrator::formNodUnbalance(DOF_Group * theDof)
{
