...
DistributedDisplacementControl::DistributedDisplacementControl(int node,
                                                               int dof,
                                                               double
                                                               increment,
                                                               int numIncr,
                                                               double min,
                                                               double
                                                               max):StaticIntegrator
    (INTEGRATOR_TAGS_DistributedDisplacementControl), processID(0),
theChannels(0), numChannels(0), theNode(node), theDof(dof),
theIncrement(increment), theDofID(0), deltaUhat(0), deltaUbar(0),
deltaU(0), deltaUstep(0), phat(0), deltaLambdaStep(0.0),
currentLambda(0.0), specNumIncrStep(numIncr), numIncrLastStep(numIncr),
minIncrement(min), maxIncrement(max)
{
    // to avoid divide-by-zero error on first update() ensure numIncr != 0
    if (numIncr == 0) {
        opserr <<
            "WARNING DistributedDisplacementControl::DistributedDisplacementControl() -";
...
DistributedDisplacementControl::DistributedDisplacementControl():StaticIntegrator(INTEGRATOR_TAGS_DistributedDisplacementControl),
...
specNumIncrStep(0), numIncrLastStep(0), minIncrement(0), maxIncrement(0)
{
 ...
 DistributedDisplacementControl::newStep(void)
{
    ...
    if (theModel->updateDomain() < 0) {
        opserr <<
            "DistributedDisplacementControl::newStep - model failed to update for new dU\n";
        return -1;
    }
...
int DistributedDisplacementControl::update(const Vector & dU)
{
    ...
    if (theModel->updateDomain() < 0) {
        opserr <<
            "DistributedDisplacementControl::update - model failed to update for new dU\n";
        return -1;
    }
...
int DistributedDisplacementControl::domainChanged(void)
{
    ...
    if (theDomain == 0) {
        opserr <<
            "BUG WARNING DistributedDisplacementControl::domainChanged() - no Domain associated!!";
        return -1;
    }
    ...
    if (theNodePtr != 0) {
        ...
        if (theGroup == 0) {
            opserr <<
                "BUG DistributedDisplacementControl::domainChanged() - no DOF_Group associated with the node!!\n";
            return -1;
        }
        const ID & theID = theGroup->getID();
        if (theDof < 0 || theDof >= theID.Size()) {
            opserr <<
                "DistributedDisplacementControl::domainChanged() - not a valid dof "
                << theDof << endln;
            return -1;
        }
        ...
        if (theDofID < 0) {
            opserr <<
                "DistributedDisplacementControl::domainChanged() - constrained dof not a valid a dof\n";;
            return -1;
        }
    }
    ...
    if (deltaUhat == 0 || deltaUhat->Size() != size) {  // create new Vector
        ...
        if (deltaUhat == 0 || deltaUhat->Size() != size) {      // check got it
            opserr <<
                "FATAL DistributedDisplacementControl::domainChanged() - ran out of memory for";
    ...
    if (haveLoad == 0) {
        opserr <<
            "WARNING DistributedDisplacementControl::domainChanged() - zero reference load";
        return -1;
    }
    ...
    if (theDofID == -1) {
        opserr <<
            "DistributedDisplacementControl::setSize() - failed to find valid dof - are the node tag and dof values correct?\n";
        return -1;
    }
...
int DistributedDisplacementControl::sendSelf(int cTag,
                                             Channel & theChannel)
{
    int sendID = 0;
    ...
    if (processID == 0) {
        ...
        if (found == false) {
            ...
            if (nextChannels == 0) {
                opserr <<
                    "DistributedDisplacementControl::sendSelf() - failed to allocate channel array of size: "
    ...
    if (res < 0) {
        opserr <<
            "WARNING DistributedDisplacementControl::sendSelf() - failed to send data\n";
        return -1;
    }
    ...
    if (res < 0) {
        opserr <<
            "WARNING DistributedDisplacementControl::recvSelf() - failed to recv vector data\n";
        return -1;
    }
...
int DistributedDisplacementControl::recvSelf(int cTag,
                                             Channel & theChannel,
                                             FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0) {
        opserr <<
            "WARNING DistributedDisplacementControl::recvSelf() - failed to recv id data\n";
        return -1;
    }
    processID = idData(0);
    theNode = idData(1);
...
void DistributedDisplacementControl::Print(OPS_Stream & s, int flag)
{
    // TO FINISH    
}

int DistributedDisplacementControl::setProcessID(int dTag)
{
    processID = dTag;
    return 0;
}

int DistributedDisplacementControl::setChannels(int nChannels,
                                                Channel ** theC)
{
