...
int
Collocation::formEleTangent (FE_Element * theEle)
{
    theEle->zeroTangent ();
...
int
Collocation::formNodTangent (DOF_Group * theDof)
{
    theDof->zeroTangent ();
    ...
    if (Ut == 0 || Ut->Size () != size)
      {
          ...
          if (Ut == 0 || Ut->Size () != size ||
              ...
              Udotdot == 0 || Udotdot->Size () != size)
            {
                ...
                opserr <<
                    "Collocation::domainChanged() - ran out of memory\n";
...
int
Collocation::update (const Vector & deltaU)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel == 0)
      {
...
int
Collocation::sendSelf (int cTag, Channel & theChannel)
{
    Vector data (3);
    data (0) = theta;
    data (1) = beta;
...
int
Collocation::recvSelf (int cTag, Channel & theChannel,
...
void
Collocation::Print (OPS_Stream & s, int flag)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel != 0)
      {
