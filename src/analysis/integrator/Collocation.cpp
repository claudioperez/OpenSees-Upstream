...
void *OPS_Collocation(void)
{
    ...
    if (argc != 1 && argc != 3) {
        opserr <<
            "WARNING - incorrect number of args want Collocation $theta\n";
    ...
    if (theIntegrator == 0)
        opserr <<
            "WARNING - out of memory creating Collocation integrator\n";

    return theIntegrator;
}


Collocation::Collocation():TransientIntegrator(INTEGRATOR_TAGS_Collocation),
...
Collocation::
Collocation(double
            _theta):TransientIntegrator(INTEGRATOR_TAGS_Collocation),
theta(_theta), beta(0.0), gamma(0.5), deltaT(0.0), c1(0.0), c2(0.0),
...
Collocation::Collocation(double _theta, double _beta,
                         double
                         _gamma):TransientIntegrator
    (INTEGRATOR_TAGS_Collocation), theta(_theta), beta(_beta),
gamma(_gamma), deltaT(0.0), c1(0.0), c2(0.0), c3(0.0), Ut(0), Utdot(0),
Utdotdot(0), U(0), Udot(0), Udotdot(0)
{
...
int
 Collocation::newStep(double _deltaT)
{
    if (theta <= 0.0) {
    ...
    if (U == 0) {
        opserr <<
            "Collocation::newStep() - domainChange() failed or hasn't been called\n";
        return -3;
    }
...
int Collocation::update(const Vector & deltaU)
{
    ...
    if (Ut == 0) {
        opserr <<
            "WARNING Collocation::update() - domainChange() failed or not called\n";
        return -2;
    }
    // check deltaU is of correct size
    if (deltaU.Size() != U->Size()) {
        opserr <<
            "WARNING Collocation::update() - Vectors of incompatible size ";
        opserr << " expecting " << U->
...
const Vector & Collocation::getVel()
{
    return *Udot;
}
...
int Collocation::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (theChannel.sendVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING Collocation::sendSelf() - failed to send the data\n";
        return -1;
    }
...
int Collocation::recvSelf(int cTag, Channel & theChannel,
                          FEM_ObjectBroker & theBroker)
{
    Vector data(3);
    if (theChannel.recvVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING Collocation::recvSelf() - could not receive data\n";
        return -1;
    }
