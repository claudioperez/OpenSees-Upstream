...
ArcLength1::ArcLength1(double arcLength,
                       double
                       alpha):StaticIntegrator(INTEGRATOR_TAGS_ArcLength1),
arcLength2(arcLength * arcLength), alpha2(alpha * alpha), deltaUhat(0),
deltaUbar(0), deltaU(0), deltaUstep(0), phat(0), deltaLambdaStep(0.0),
currentLambda(0.0), signLastDeltaLambdaStep(1)
{
...
int ArcLength1::update(const Vector & dU)
{
    AnalysisModel *theModel = this->getAnalysisModel();
    LinearSOE *theLinSOE = this->getLinearSOE();
    if (theModel == 0 || theLinSOE == 0) {
...
int ArcLength1::domainChanged(void)
{
    ...
    if (deltaUhat == 0 || deltaUhat->Size() != size) {  // create new Vector
        ...
        if (deltaUhat == 0 || deltaUhat->Size() != size) {      // check got it
            opserr <<
                "FATAL ArcLength1::domainChanged() - ran out of memory for";
...
int ArcLength1::sendSelf(int cTag, Channel & theChannel)
{
    Vector data(5);
    data(0) = arcLength2;
    data(1) = alpha2;
...
int ArcLength1::recvSelf(int cTag,
                         Channel & theChannel,
...
void ArcLength1::Print(OPS_Stream & s, int flag)
{
    AnalysisModel *theModel = this->getAnalysisModel();
    if (theModel != 0) {
