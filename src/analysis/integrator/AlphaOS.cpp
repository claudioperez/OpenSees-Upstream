...
AlphaOS::AlphaOS (double _alpha, bool updelemdisp):
...
int
AlphaOS::formEleTangent (FE_Element * theEle)
{
    theEle->zeroTangent ();
...
int
AlphaOS::formNodTangent (DOF_Group * theDof)
{
    theDof->zeroTangent ();
...
int
AlphaOS::update (const Vector & deltaU)
{
    updateCount++;
    if (updateCount > 1)
      {
...
int
AlphaOS::sendSelf (int cTag, Channel & theChannel)
{
    Vector data (4);
    data (0) = alpha;
    data (1) = beta;
...
int
AlphaOS::recvSelf (int cTag, Channel & theChannel,
...
int
AlphaOS::formElementResidual (void)
{
                ...
                if (statusFlag == CURRENT_TANGENT)
                  {
                      if (theSOE->
                        ...
                        {
                            opserr <<
                                "WARNING AlphaOS::formElementResidual() -";
