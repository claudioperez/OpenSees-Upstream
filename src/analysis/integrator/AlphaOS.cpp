...
AlphaOS::AlphaOS():TransientIntegrator(INTEGRATOR_TAGS_AlphaOS),
...
AlphaOS::AlphaOS(double _alpha,
                 bool
                 updelemdisp):TransientIntegrator(INTEGRATOR_TAGS_AlphaOS),
alpha(_alpha), beta((2 - _alpha) * (2 - _alpha) * 0.25),
gamma(1.5 - _alpha), updElemDisp(updelemdisp), deltaT(0.0), updateCount(0),
c1(0.0), c2(0.0), c3(0.0), Ut(0), Utdot(0), Utdotdot(0), U(0), Udot(0),
Udotdot(0), Ualpha(0), Ualphadot(0), Upt(0)
{
...
AlphaOS::AlphaOS(double _alpha, double _beta, double _gamma,
                 bool
                 updelemdisp):TransientIntegrator(INTEGRATOR_TAGS_AlphaOS),
alpha(_alpha), beta(_beta), gamma(_gamma), updElemDisp(updelemdisp),
deltaT(0.0), updateCount(0), c1(0.0), c2(0.0), c3(0.0), Ut(0), Utdot(0),
Utdotdot(0), U(0), Udot(0), Udotdot(0), Ualpha(0), Ualphadot(0), Upt(0)
{
...
int
 AlphaOS::newStep(double _deltaT)
{
    updateCount = 0;
    ...
    if (U == 0) {
        opserr <<
            "AlphaOS::newStep() - domainChange() failed or hasn't been called\n";
        return -3;
    }
...
int AlphaOS::update(const Vector & deltaU)
{
    ...
    if (updateCount > 1) {
        ...
        opserr <<
            " AlphaOS integration scheme requires a LINEAR solution algorithm\n";
        return -1;
    }
    ...
    if (Ut == 0) {
        opserr <<
            "WARNING AlphaOS::update() - domainChange() failed or not called\n";
        return -3;
    }
    // check deltaU is of correct size
    if (deltaU.Size() != U->Size()) {
        opserr <<
            "WARNING AlphaOS::update() - Vectors of incompatible size ";
        opserr << " expecting " << U->
...
int AlphaOS::recvSelf(int cTag, Channel & theChannel,
