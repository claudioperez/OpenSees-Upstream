...
AlphaOSGeneralized::AlphaOSGeneralized():TransientIntegrator(INTEGRATOR_TAGS_AlphaOSGeneralized),
...
AlphaOSGeneralized::AlphaOSGeneralized(double _alphaI, double _alphaF,
                                       ...
                                       bool
                                       updelemdisp):TransientIntegrator
    (INTEGRATOR_TAGS_AlphaOSGeneralized), alphaI(_alphaI), alphaF(_alphaF),
beta(_beta), gamma(_gamma), updElemDisp(updelemdisp), deltaT(0.0),
updateCount(0), c1(0.0), c2(0.0), c3(0.0), Ut(0), Utdot(0), Utdotdot(0),
U(0), Udot(0), Udotdot(0), Ualpha(0), Ualphadot(0), Ualphadotdot(0), Upt(0)
{
...
int
 AlphaOSGeneralized::newStep(double _deltaT)
{
    updateCount = 0;
    ...
    if (U == 0) {
        opserr <<
            "AlphaOSGeneralized::newStep() - domainChange() failed or hasn't been called\n";
        return -3;
    }
    ...
    if (theModel->updateDomain(time, deltaT) < 0) {
        opserr <<
            "AlphaOSGeneralized::newStep() - failed to update the domain\n";
        return -4;
    }
...
const Vector & AlphaOSGeneralized::getVel()
{
    return *Udot;
}
...
int AlphaOSGeneralized::domainChanged()
{
    ...
    if (Ut == 0 || Ut->Size() != size) {
        ...
        if (Ut == 0 || Ut->Size() != size ||
            ...
            opserr <<
                "AlphaOSGeneralized::domainChanged() - ran out of memory\n";
...
int AlphaOSGeneralized::update(const Vector & deltaU)
{
    ...
    if (updateCount > 1) {
        ...
        opserr <<
            " AlphaOSGeneralized integration scheme requires a LINEAR solution algorithm\n";
        return -1;
    }
    ...
    if (theModel == 0) {
        opserr <<
            "WARNING AlphaOSGeneralized::update() - no AnalysisModel set\n";
        return -2;
    }
    // check domainChanged() has been called, i.e. Ut will not be zero
    if (Ut == 0) {
        opserr <<
            "WARNING AlphaOSGeneralized::update() - domainChange() failed or not called\n";
        return -3;
    }
    // check deltaU is of correct size
    if (deltaU.Size() != U->Size()) {
        opserr <<
            "WARNING AlphaOSGeneralized::update() - Vectors of incompatible size ";
        opserr << " expecting " << U->
    ...
    if (theModel->updateDomain() < 0) {
        opserr <<
            "AlphaOSGeneralized::update() - failed to update the domain\n";
        return -5;
    }
...
int AlphaOSGeneralized::commit(void)
{
    ...
    if (theModel == 0) {
        opserr <<
            "WARNING AlphaOSGeneralized::commit() - no AnalysisModel set\n";
        return -1;
    }
...
int AlphaOSGeneralized::recvSelf(int cTag, Channel & theChannel,
                                 FEM_ObjectBroker & theBroker)
{
    Vector data(5);
    if (theChannel.recvVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING AlphaOSGeneralized::recvSelf() - could not receive data\n";
        return -1;
    }
...
int AlphaOSGeneralized::formElementResidual(void)
{
    ...
    while ((elePtr = theEles()) != 0) {
        if (theSOE->addB(elePtr->getResidual(this), elePtr->getID()) < 0) {
            opserr <<
                "WARNING AlphaOSGeneralized::formElementResidual() -";
