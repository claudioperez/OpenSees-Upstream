...
AlphaOSGeneralized::AlphaOSGeneralized (double _rhoInf, bool updelemdisp):
...
int
AlphaOSGeneralized::revertToLastStep ()
{
...
int
AlphaOSGeneralized::formEleTangent (FE_Element * theEle)
{
    theEle->zeroTangent ();
...
int
AlphaOSGeneralized::formNodTangent (DOF_Group * theDof)
{
    theDof->zeroTangent ();
...
int
AlphaOSGeneralized::domainChanged ()
{
    AnalysisModel *theModel = this->getAnalysisModel ();
...
int
AlphaOSGeneralized::update (const Vector & deltaU)
{
    updateCount++;
    if (updateCount > 1)
      {
...
int
AlphaOSGeneralized::commit (void)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel == 0)
      {
...
int
AlphaOSGeneralized::sendSelf (int cTag, Channel & theChannel)
{
    Vector data (5);
    data (0) = alphaI;
    data (1) = alphaF;
...
int
AlphaOSGeneralized::recvSelf (int cTag, Channel & theChannel,
...
void
AlphaOSGeneralized::Print (OPS_Stream & s, int flag)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel != 0)
      {
...
int
AlphaOSGeneralized::formElementResidual (void)
{
