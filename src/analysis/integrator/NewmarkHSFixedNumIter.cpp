...
OPS_NewmarkHSFixedNumIter (void)
{
    ...
    if (argc != 2 && argc != 4)
      {
          opserr <<
              "WARNING - incorrect number of args want NewmarkHSFixedNumIter $gamma $beta <-polyOrder $O>\n";
          return 0;
      }
    ...
    if (OPS_GetDouble (&numData, dData) != 0)
      {
          opserr <<
              "WARNING - invalid args want NewmarkHSFixedNumIter $gamma $beta <-polyOrder $O>\n";
          return 0;
      }
          ...
          if (strcmp (argvLoc, "-polyOrder") == 0)
            {
                ...
                if (OPS_GetInt (&numData, &polyOrder) != 0)
                  {
                      opserr <<
                          "WARNING - invalid polyOrder want NewmarkHSFixedNumIter $gamma $beta <-polyOrder $O>\n";
                  }
            }
      }
    ...
    if (theIntegrator == 0)
        opserr <<
            "WARNING - out of memory creating NewmarkHSFixedNumIter integrator\n";

    return theIntegrator;
}


NewmarkHSFixedNumIter::NewmarkHSFixedNumIter ():TransientIntegrator (INTEGRATOR_TAGS_NewmarkHSFixedNumIter),
...
int
NewmarkHSFixedNumIter::newStep (double deltaT)
{
    if (beta == 0 || gamma == 0)
      {
    ...
    if (U == 0)
      {
          opserr <<
              "NewmarkHSFixedNumIter::newStep() - domainChange() failed or hasn't been called\n";
          return -3;
      }
...
int
NewmarkHSFixedNumIter::revertToLastStep ()
{
...
int
NewmarkHSFixedNumIter::formEleTangent (FE_Element * theEle)
{
    theEle->zeroTangent ();
...
int
NewmarkHSFixedNumIter::formNodTangent (DOF_Group * theDof)
{
    theDof->zeroTangent ();
...
int
NewmarkHSFixedNumIter::domainChanged ()
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    ...
    if (Ut == 0 || Ut->Size () != size)
      {
          ...
          if (Ut == 0 || Ut->Size () != size ||
              ...
              scaledDeltaU == 0 || scaledDeltaU->Size () != size)
            {
                ...
                opserr <<
                    "NewmarkHSFixedNumIter::domainChanged() - ran out of memory\n";
...
int
NewmarkHSFixedNumIter::update (const Vector & deltaU)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel == 0)
      {
          opserr <<
              "WARNING NewmarkHSFixedNumIter::update() - no AnalysisModel set\n";
          return -1;
      }
    ...
    if (theTest == 0)
      {
          opserr <<
              "WARNING NewmarkHSFixedNumIter::update() - no ConvergenceTest set\n";
          return -2;
      }
    ...
    if (Ut == 0)
      {
          opserr <<
              "WARNING NewmarkHSFixedNumIter::update() - domainChange() failed or not called\n";
          return -3;
      }
    ...
    if (deltaU.Size () != U->Size ())
      {
          opserr <<
              "WARNING NewmarkHSFixedNumIter::update() - Vectors of incompatible size";
    ...
    if (polyOrder == 1)
      ...
      {
          opserr <<
              "WARNING NewmarkHSFixedNumIter::update() - polyOrder > 3 not supported\n";
          return -5;
      }
    ...
    if (theModel->updateDomain () < 0)
      {
          opserr <<
              "NewmarkHSFixedNumIter::update() - failed to update the domain\n";
          return -6;
      }
...
int
NewmarkHSFixedNumIter::commit (void)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel == 0)
      {
          opserr <<
              "WARNING NewmarkHSFixedNumIter::commit() - no AnalysisModel set\n";
          return -1;
      }
    ...
    if (updDomFlag == true)
      {
          ...
          if (theSOE == 0)
            {
                opserr <<
                    "WARNING NewmarkHSFixedNumIter::commit() - no LinearSOE set\n";
                return -2;
            }
...
int
NewmarkHSFixedNumIter::sendSelf (int cTag, Channel & theChannel)
{
    Vector data (4);
    data (0) = gamma;
    data (1) = beta;
    ...
    if (theChannel.sendVector (this->getDbTag (), cTag, data) < 0)
      {
          opserr <<
              "WARNING NewmarkHSFixedNumIter::sendSelf() - could not send data\n";
          return -1;
      }
...
int
NewmarkHSFixedNumIter::recvSelf (int cTag, Channel & theChannel,
                                 FEM_ObjectBroker & theBroker)
{
    ...
    if (theChannel.recvVector (this->getDbTag (), cTag, data) < 0)
      {
          opserr <<
              "WARNING NewmarkHSFixedNumIter::recvSelf() - could not receive data\n";
          return -1;
      }
...
void
NewmarkHSFixedNumIter::Print (OPS_Stream & s, int flag)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel != 0)
      {
