...
void *OPS_NewmarkHSFixedNumIter(void)
{
    ...
    theIntegrator =
        new NewmarkHSFixedNumIter(dData[0], dData[1], polyOrder,
                                  updDomFlag);
...
NewmarkHSFixedNumIter::NewmarkHSFixedNumIter(double _gamma,
    ...
    (INTEGRATOR_TAGS_NewmarkHSFixedNumIter), gamma(_gamma), beta(_beta),
polyOrder(polyorder), updDomFlag(upddomflag), c1(0.0), c2(0.0), c3(0.0),
...
const Vector & NewmarkHSFixedNumIter::getVel()
{
    return *Udot;
}
...
int NewmarkHSFixedNumIter::recvSelf(int cTag, Channel & theChannel,
