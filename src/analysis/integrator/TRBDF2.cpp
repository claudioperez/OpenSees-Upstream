...
TRBDF2::TRBDF2 ():TransientIntegrator (INTEGRATOR_TAGS_TRBDF2),
step (0), dt (0.0),
...
int
TRBDF2::newStep (double deltaT)
{
    // check the vectors have been created
    if (U == 0)
      {
          opserr <<
              "TRBDF2::newStep() - domainChange() failed or hasn't been called\n";
          return -3;
      }
...
int
TRBDF2::formEleTangent (FE_Element * theEle)
{
    theEle->zeroTangent ();
...
int
TRBDF2::formNodTangent (DOF_Group * theDof)
{
    theDof->zeroTangent ();
...
int
TRBDF2::update (const Vector & deltaU)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel == 0)
      {
    ...
    if (Ut == 0)
      {
          opserr <<
              "WARNING TRBDF2::update() - domainChange() failed or not called\n";
          return -2;
      }
    ...
    if (deltaU.Size () != U->Size ())
      {
          opserr <<
              "WARNING TRBDF2::update() - Vectors of incompatible size ";
...
int
TRBDF2::sendSelf (int cTag, Channel & theChannel)
{
    return 0;
}
...
int
TRBDF2::recvSelf (int cTag, Channel & theChannel,
...
void
TRBDF2::Print (OPS_Stream & s, int flag)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel != 0)
      {
