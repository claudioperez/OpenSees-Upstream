...
int
AlphaOSGeneralized_TP::formUnbalance ()
{
    // get a pointer to the LinearSOE and the AnalysisModel
    LinearSOE *theLinSOE = this->getLinearSOE ();
    AnalysisModel *theModel = this->getAnalysisModel ();
...
int
AlphaOSGeneralized_TP::formEleTangent (FE_Element * theEle)
{
    theEle->zeroTangent ();
...
int
AlphaOSGeneralized_TP::formNodTangent (DOF_Group * theDof)
{
    theDof->zeroTangent ();
...
int
AlphaOSGeneralized_TP::formEleResidual (FE_Element * theEle)
{
    theEle->zeroResidual ();
...
int
AlphaOSGeneralized_TP::commit (void)
{
    // get a pointer to the LinearSOE and the AnalysisModel
    LinearSOE *theLinSOE = this->getLinearSOE ();
    AnalysisModel *theModel = this->getAnalysisModel ();
...
int
AlphaOSGeneralized_TP::sendSelf (int cTag, Channel & theChannel)
{
    Vector data (5);
    data (0) = alphaI;
    data (1) = alphaF;
...
int
AlphaOSGeneralized_TP::recvSelf (int cTag, Channel & theChannel,
...
void
AlphaOSGeneralized_TP::Print (OPS_Stream & s, int flag)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel != 0)
      {
...
int
AlphaOSGeneralized_TP::formElementResidual (void)
{
          ...
          if (alphaKU > 0.0)
            {
                if (statusFlag == CURRENT_TANGENT)
                  {
                      if (theSOE->
                          addB (elePtr->getK_Force (*Ut - *Upt),
                                elePtr->getID (), -alphaKU) < 0)
                        {
                            opserr <<
                                "WARNING AlphaOSGeneralized_TP::formElementResidual() -";
                            opserr << " failed in addB for ID " << elePtr->
                                getID ();
                            return -2;
                        }
                  }
                  ...
                  {
                      if (theSOE->
                          addB (elePtr->getKi_Force (*Ut - *Upt),
                                elePtr->getID (), -alphaKU) < 0)
                        {
                            opserr <<
                                "WARNING AlphaOSGeneralized_TP::formElementResidual() -";
