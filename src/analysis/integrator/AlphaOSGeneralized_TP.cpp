...
AlphaOSGeneralized_TP::AlphaOSGeneralized_TP():TransientIntegrator(INTEGRATOR_TAGS_AlphaOSGeneralized_TP),
...
Ut(0), Utdot(0), Utdotdot(0), U(0), Udot(0), Udotdot(0), Upt(0), Put(0)
{
...
AlphaOSGeneralized_TP::AlphaOSGeneralized_TP(double _rhoInf,
    ...
    (INTEGRATOR_TAGS_AlphaOSGeneralized_TP),
...
deltaT(0.0), updateCount(0), c1(0.0), c2(0.0), c3(0.0), alphaM(alphaI),
alphaD(alphaF), alphaR(alphaF), alphaKU(0.0), alphaP(alphaF), Ut(0),
Utdot(0), Utdotdot(0), U(0), Udot(0), Udotdot(0), Upt(0), Put(0)
{

}


AlphaOSGeneralized_TP::AlphaOSGeneralized_TP(double _alphaI,
                                             double _alphaF, double _beta,
                                             double _gamma,
                                             bool
                                             updelemdisp):TransientIntegrator
    (INTEGRATOR_TAGS_AlphaOSGeneralized_TP), alphaI(_alphaI),
...
int
 AlphaOSGeneralized_TP::newStep(double _deltaT)
{
    updateCount = 0;
    ...
    if (U == 0) {
        opserr <<
            "AlphaOSGeneralized_TP::newStep() - domainChange() failed or hasn't been called\n";
        return -4;
    }
    ...
    if (theModel->updateDomain(time, deltaT) < 0) {
        opserr <<
            "AlphaOSGeneralized_TP::newStep() - failed to update the domain\n";
        return -5;
    }
...
const Vector & AlphaOSGeneralized_TP::getVel()
{
    return *Udot;
}
...
int AlphaOSGeneralized_TP::domainChanged()
{
    ...
    if (Ut == 0 || Ut->Size() != size) {
        ...
        if (Ut == 0 || Ut->Size() != size ||
            ...
            opserr <<
                "AlphaOSGeneralized_TP::domainChanged() - ran out of memory\n";
...
int AlphaOSGeneralized_TP::update(const Vector & deltaU)
{
    ...
    if (updateCount > 1) {
        opserr <<
            "WARNING AlphaOSGeneralized_TP::update() - called more than once -";
        opserr <<
            " AlphaOSGeneralized_TP integration scheme requires a LINEAR solution algorithm\n";
        return -1;
    }
    ...
    if (theModel == 0) {
        opserr <<
            "WARNING AlphaOSGeneralized_TP::update() - no AnalysisModel set\n";
        return -2;
    }
    // check domainChanged() has been called, i.e. Ut will not be zero
    if (Ut == 0) {
        opserr <<
            "WARNING AlphaOSGeneralized_TP::update() - domainChange() failed or not called\n";
        return -3;
    }
    // check deltaU is of correct size
    if (deltaU.Size() != U->Size()) {
        opserr <<
            "WARNING AlphaOSGeneralized_TP::update() - Vectors of incompatible size ";
        opserr << " expecting " << U->
            Size() << " obtained " << deltaU.Size() << "\n";
        return -4;
    }
    ...
    if (theModel->updateDomain() < 0) {
        opserr <<
            "AlphaOSGeneralized_TP::update() - failed to update the domain\n";
        return -5;
    }
...
int AlphaOSGeneralized_TP::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (theChannel.sendVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING AlphaOSGeneralized_TP::sendSelf() - could not send data\n";
        return -1;
    }
...
int AlphaOSGeneralized_TP::recvSelf(int cTag, Channel & theChannel,
                                    FEM_ObjectBroker & theBroker)
{
    Vector data(5);
    if (theChannel.recvVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING AlphaOSGeneralized_TP::recvSelf() - could not receive data\n";
        return -1;
    }
...
void AlphaOSGeneralized_TP::Print(OPS_Stream & s, int flag)
{
    ...
    if (theModel != 0) {
        ...
        s << "AlphaOSGeneralized_TP - currentTime: " << currentTime <<
            endln;
...
int AlphaOSGeneralized_TP::formElementResidual(void)
{
    ...
    while ((elePtr = theEles()) != 0) {
        if (theSOE->addB(elePtr->getResidual(this), elePtr->getID()) < 0) {
            opserr <<
                "WARNING AlphaOSGeneralized_TP::formElementResidual() -";
            opserr << " failed in addB for ID " << elePtr->getID();
            return -1;
        }
        if (alphaKU > 0.0) {
            if (statusFlag == CURRENT_TANGENT) {
                if (theSOE->addB(elePtr->getK_Force(*Ut - *Upt),
                                 elePtr->getID(), -alphaKU) < 0) {
                    opserr <<
                        "WARNING AlphaOSGeneralized_TP::formElementResidual() -";
                    opserr << " failed in addB for ID " << elePtr->getID();
                    return -2;
                ...
                if (theSOE->addB(elePtr->getKi_Force(*Ut - *Upt),
                                 elePtr->getID(), -alphaKU) < 0) {
                    opserr <<
                        "WARNING AlphaOSGeneralized_TP::formElementResidual() -";
                    opserr << " failed in addB for ID " << elePtr->getID();
