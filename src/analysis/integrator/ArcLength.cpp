...
ArcLength::ArcLength(double arcLength,
                     double
                     alpha):StaticIntegrator(INTEGRATOR_TAGS_ArcLength),
arcLength2(arcLength * arcLength), alpha2(alpha * alpha), deltaUhat(0),
deltaUbar(0), deltaU(0), deltaUstep(0), deltaUstep2(0), dDeltaUstepdh(0),
phat(0), deltaLambdaStep(0.0), dDeltaLambdaStepdh(0.0), currentLambda(0.0),
dLAMBDA(0.0), dLAMBDA2(0.0), dlambda1dh(0.0), dLAMBDAdh(0), Residual(0),
sensU(0), sensitivityFlag(0), signLastDeltaLambdaStep(1), dUhatdh(0),
dphatdh(0), dUIJdh(0), dlambdaJdh(0.0), gradNumber(0), a(0.0), b(0.0),
c(0.0), b24ac(0.0)
{
...
int ArcLength::update(const Vector & dU)
{
...
//opserr<<"Update:   phat is = "<<*phat<<"////////////////////////////"<<endln;
    ...
    b = alpha2 * deltaLambdaStep
        + ((*deltaUhat) ^ (*deltaUbar)) + ((*deltaUstep) ^ (*deltaUhat));
    b *= 2.0;
    ...
    if (b24ac < 0) {
        opserr <<
            "ArcLength::update() - imaginary roots due to multiple instability";
        opserr << " directions - initial load increment was too large\n";
        opserr << "a: " << a << " b: " << b << " c: " << c << " b24ac: " <<
            b24ac << endln;
        return -1;
    }
    double a2 = 2.0 * a;
...
//opserr<<"squareRoot of b24ac= "<<sqrtb24ac<<endln;
    ...
    double theta1 =
        ((*deltaUstep) ^ (*deltaUstep)) + ((*deltaUbar) ^ (*deltaUstep));
...
int ArcLength::domainChanged(void)
{
    ...
    if (deltaUhat == 0 || deltaUhat->Size() != size) {  // create new Vector
        ...
        if (deltaUhat == 0 || deltaUhat->Size() != size) {      // check got it
            opserr <<
                "FATAL ArcLength::domainChanged() - ran out of memory for";
    ...
    if (dphatdh == 0 || dphatdh->Size() != size) {
        ...
        if (dphatdh == 0 || dphatdh->Size() != size) {
            opserr <<
                "FATAL DisplacementControl::domainChanged() - ran out of memory for";
    ...
    if (haveLoad == 0) {
        opserr <<
            "WARNING ArcLength::domainChanged() - zero reference load";
        return -1;
    }
...
int ArcLength::sendSelf(int cTag, Channel & theChannel)
{
...
int ArcLength::recvSelf(int cTag,
...
void ArcLength::Print(OPS_Stream & s, int flag)
{
    AnalysisModel *theModel = this->getAnalysisModel();
    if (theModel != 0) {
...
void ArcLength::formTangDispSensitivity(int gradNumber)
{
    AnalysisModel *theModel = this->getAnalysisModel();
...
*/
    ...
    int nodeNumber, dofNumber, relevantID, i, sizeRandomLoads,
        numRandomLoads;
    ...
    while ((loadPatternPtr = thePatterns()) != 0) {
        const Vector & randomLoads =
            loadPatternPtr->getExternalForceSensitivity(gradNumber);
...
double ArcLength::formdLambdaDh(int gradNumber)
{
...
//opserr<<"ArcLength: dLAMBDA=0 !!!!!!!!!!!!!!!!!!!"<<endln;
        ...
        dlambda1dh =
            signLastDeltaLambdaStep * 1.0 / (dLAMBDA) * (-arcLength2 *
                                                         dUhatTUhatdh /
                                                         (denomerator));
...
double ArcLength::getLambdaSensitivity(int gradNumber)
{
    ...
    dBdh =
        2.0 * (((*dUIJdh) ^ (*deltaUhat)) + ((*deltaUbar) ^ (*dUhatdh)) +
               ((*deltaUstep2) ^ (*dUhatdh)) +
               ((*dDeltaUstepdh) ^ (*deltaUhat)) +
               alpha2 * dDeltaLambdaStepdh);
...
// opserr<<":a is "<<a<<endln;
    double sqrtb24ac = sqrt(b24ac);
    double dSqrtb24acdh =
        (2.0 * b * dBdh - 4.0 * (a * dCdh + dAdh * c)) / (2.0 * sqrtb24ac);
    double dlambda1 = (-b + sqrtb24ac) / a2;
    double dlambdaj1dh =
        (a2 * (-dBdh + dSqrtb24acdh) -
         ((-b + sqrtb24ac) * 2.0 * dAdh)) / (4.0 * a * a);
    ...
    double theta1 =
        ((*deltaUstep2) ^ (*deltaUstep2)) +
        ((*deltaUbar) ^ (*deltaUstep2));
    theta1 += dlambda1 * val;

    double dTheta1dh =
        2 * ((*deltaUstep2) ^ (*dDeltaUstepdh)) +
        ((*deltaUbar) ^ (*dDeltaUstepdh)) + ((*dUIJdh) ^ (*deltaUstep2));
    //    double theta2 = theta1 + dlambda2*val;
    double dvaldh =
        ((*deltaUhat) ^ (*dDeltaUstepdh)) + ((*dUhatdh) ^ (*deltaUstep2));
...
void ArcLength::formResidualDispSensitivity(int gradNumber)
{
    AnalysisModel *theModel = this->getAnalysisModel();
    int size = theModel->getNumEqn();
//static Matrix dKdh(size,size);
...
int ArcLength::formEleResidual(FE_Element * theEle)
{
...
int ArcLength::formIndependentSensitivityRHS()
{
    return 0;
}

int ArcLength::formSensitivityRHS(int passedGradNumber)
{
    sensitivityFlag = 1;
...
int ArcLength::saveSensitivity(const Vector & v, int gradNum, int numGrads)
{
    AnalysisModel *theAnalysisModel = this->getAnalysisModel();
...
int ArcLength::saveLambdaSensitivity(double dlambdadh, int gradNum,
                                     int numGrads)
{
    AnalysisModel *theAnalysisModel = this->getAnalysisModel();
    Domain *theDomain = theAnalysisModel->getDomainPtr();
...
int ArcLength::commitSensitivity(int gradNum, int numGrads)
{
...
bool ArcLength::computeSensitivityAtEachIteration()
{
...
int ArcLength::computeSensitivities(void)
{
    LinearSOE *theSOE = this->getLinearSOE();
