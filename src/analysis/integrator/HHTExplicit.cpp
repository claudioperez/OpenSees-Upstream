...
OPS_HHTExplicit (void)
{
    ...
    if (argc < 1 || argc > 3)
      {
          opserr <<
              "WARNING - incorrect number of args want HHTExplicit $alpha <-updateElemDisp>\n";
          opserr <<
              "          or HHTExplicit $alpha $gamma <-updateElemDisp>\n";
          return 0;
      }
...
HHTExplicit::HHTExplicit ():TransientIntegrator (INTEGRATOR_TAGS_HHTExplicit),
...
HHTExplicit::HHTExplicit (double _alpha, bool updelemdisp):
...
HHTExplicit::HHTExplicit (double _alpha, double _gamma, bool updelemdisp):
TransientIntegrator (INTEGRATOR_TAGS_HHTExplicit),
alpha (_alpha),
gamma (_gamma),
updElemDisp (updelemdisp),
...
int
HHTExplicit::newStep (double _deltaT)
{
    updateCount = 0;
    ...
    if (U == 0)
      {
          opserr <<
              "HHTExplicit::newStep() - domainChange() failed or hasn't been called\n";
          return -3;
      }
...
int
HHTExplicit::formEleTangent (FE_Element * theEle)
{
    theEle->zeroTangent ();
...
int
HHTExplicit::formNodTangent (DOF_Group * theDof)
{
    theDof->zeroTangent ();
    ...
    if (Ut == 0 || Ut->Size () != size)
      {
          ...
          if (Ut == 0 || Ut->Size () != size ||
              ...
              Ualphadot == 0 || Ualphadot->Size () != size)
            {
                ...
                opserr <<
                    "HHTExplicit::domainChanged() - ran out of memory\n";
...
int
HHTExplicit::update (const Vector & aiPlusOne)
{
    updateCount++;
    if (updateCount > 1)
      {
          ...
          opserr <<
              " HHTExplicit integration scheme requires a LINEAR solution algorithm\n";
          return -1;
      }
    ...
    if (Ut == 0)
      {
          opserr <<
              "WARNING HHTExplicit::update() - domainChange() failed or not called\n";
          return -3;
      }
    ...
    if (aiPlusOne.Size () != U->Size ())
      {
          opserr <<
              "WARNING HHTExplicit::update() - Vectors of incompatible size ";
...
int
HHTExplicit::sendSelf (int cTag, Channel & theChannel)
{
    Vector data (3);
    data (0) = alpha;
    data (1) = gamma;
...
int
HHTExplicit::recvSelf (int cTag, Channel & theChannel,
                       FEM_ObjectBroker & theBroker)
{
    ...
    if (theChannel.recvVector (this->getDbTag (), cTag, data) < 0)
      {
          opserr <<
              "WARNING HHTExplicit::recvSelf() - could not receive data\n";
          return -1;
      }
...
void
HHTExplicit::Print (OPS_Stream & s, int flag)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel != 0)
      {
