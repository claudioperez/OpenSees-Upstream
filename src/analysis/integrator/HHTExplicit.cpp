...
void *OPS_HHTExplicit(void)
{
    ...
    if (theIntegrator == 0)
        opserr <<
            "WARNING - out of memory creating HHTExplicit integrator\n";
...
HHTExplicit::HHTExplicit(double _alpha,
    ...
    (INTEGRATOR_TAGS_HHTExplicit), alpha(_alpha), gamma(0.5),
updElemDisp(updelemdisp), deltaT(0.0), updateCount(0), c2(0.0), c3(0.0),
...
HHTExplicit::HHTExplicit(double _alpha, double _gamma,
                         bool
                         updelemdisp):TransientIntegrator
    (INTEGRATOR_TAGS_HHTExplicit), alpha(_alpha), gamma(_gamma),
updElemDisp(updelemdisp), deltaT(0.0), updateCount(0), c2(0.0), c3(0.0),
Ut(0), Utdot(0), Utdotdot(0), U(0), Udot(0), Udotdot(0), Ualpha(0),
Ualphadot(0)
{
...
int HHTExplicit::update(const Vector & aiPlusOne)
{
    ...
    if (updateCount > 1) {
        opserr <<
            "WARNING HHTExplicit::update() - called more than once -";
        opserr <<
...
const Vector & HHTExplicit::getVel()
{
    return *Udot;
}
...
int HHTExplicit::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (theChannel.sendVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING HHTExplicit::sendSelf() - could not send data\n";
        return -1;
    }
...
int HHTExplicit::recvSelf(int cTag, Channel & theChannel,
