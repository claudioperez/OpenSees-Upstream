...
MinUnbalDispNorm::MinUnbalDispNorm(double lambda1, int specNumIter,
                                   ...
                                   int
                                   signFirstStep):StaticIntegrator
    (INTEGRATOR_TAGS_MinUnbalDispNorm), dLambda1LastStep(lambda1),
specNumIncrStep(specNumIter), numIncrLastStep(specNumIter), deltaUhat(0),
deltaUbar(0), deltaU(0), deltaUstep(0), dUhatdh(0), dLambdaj(0.0), phat(0),
deltaLambdaStep(0.0), currentLambda(0.0), dLambdaStepDh(0.0), dUIJdh(0),
Dlambdadh(0.0), dphatdh(0), Residual2(0), signLastDeltaLambdaStep(1),
sensitivityFlag(0), Residual(0), dlambdadh(0.0), dLambda(0.0), sensU(0),
d_deltaU_dh(0), gradNumber(0), dLAMBDAdh(0), dLambda1min(min),
dLambda1max(max), signLastDeterminant(1),
signFirstStepMethod(signFirstStep)
{
    // to avoid divide-by-zero error on first update() ensure numIncr != 0
    if (specNumIncrStep == 0) {
        opserr <<
            "WARNING LoadControl::LoadControl() - numIncr set to 0, 1 assumed\n";
        specNumIncrStep = 1.0;
        numIncrLastStep = 1.0;
    }
}
...
//opserr<<" NewStep=      "<<*phat<<endln;
    ...
    if (theLinSOE->solve() < 0) {
        opserr <<
            "MinUnbalanceDispNorm::newStep(void) - failed in solver\n";
        return -1;
    }
...
/////////////////    
    ...
    if (theModel->updateDomain() < 0) {
        opserr <<
            "MinUnbalDispNorm::newStep - model failed to update for new dU\n";
        return -1;
    }
...
int MinUnbalDispNorm::update(const Vector & dU)
{
...
//opserr<<"CURRENTLAMBDA= ..................................................../////////......."<<currentLambda<<endln;
    ...
    if (theModel->updateDomain() < 0) {
        opserr <<
            "MinUnbalDispNorm::update - model failed to update for new dU\n";
        return -1;
    }
...
int MinUnbalDispNorm::domainChanged(void)
{
    ...
    if (deltaUhat == 0 || deltaUhat->Size() != size) {  // create new Vector
        ...
        if (deltaUhat == 0 || deltaUhat->Size() != size) {      // check got it
            opserr <<
                "FATAL MinUnbalDispNorm::domainChanged() - ran out of memory for";
    ...
    if (dUhatdh == 0 || dUhatdh->Size() != size) {
        ...
        if (dUhatdh == 0 || dUhatdh->Size() != size) {
            opserr <<
                "FATAL MinUnbalDisporm::domainChanged() - ran out of memory for";
...
int MinUnbalDispNorm::sendSelf(int cTag, Channel & theChannel)
{
    Vector data(8);
    ...
    if (theChannel.sendVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "MinUnbalDispNorm::sendSelf() - failed to send the data\n";
        return -1;
    }
    return 0;
}
...
int MinUnbalDispNorm::recvSelf(int cTag,
                               Channel & theChannel,
...
void MinUnbalDispNorm::Print(OPS_Stream & s, int flag)
{
    AnalysisModel *theModel = this->getAnalysisModel();
    if (theModel != 0) {
...
Vector *MinUnbalDispNorm::formTangDispSensitivity(Vector * dUhatdh,
                                                  int gradNumber)
{
    LinearSOE *theLinSOE = this->getLinearSOE();
    dUhatdh->Zero();
...
double MinUnbalDispNorm::formdLambdaDh(int gradNumber)
{
...
double MinUnbalDispNorm::getLambdaSensitivity(int gradNumber)
{
...
int MinUnbalDispNorm::formEleResidual(FE_Element * theEle)
{
...
int MinUnbalDispNorm::formIndependentSensitivityRHS()
{
    return 0;
}
...
int MinUnbalDispNorm::formSensitivityRHS(int passedGradNumber)
{
...
int MinUnbalDispNorm::saveSensitivity(const Vector & v, int gradNum,
                                      int numGrads)
{
    AnalysisModel *theAnalysisModel = this->getAnalysisModel();
...
int MinUnbalDispNorm::saveLambdaSensitivity(double dlambdadh, int gradNum,
                                            int numGrads)
{
    AnalysisModel *theAnalysisModel = this->getAnalysisModel();
    Domain *theDomain = theAnalysisModel->getDomainPtr();
...
int MinUnbalDispNorm::commitSensitivity(int gradNum, int numGrads)
{
...
bool MinUnbalDispNorm::computeSensitivityAtEachIteration()
{
...
int MinUnbalDispNorm::computeSensitivities(void)
{
