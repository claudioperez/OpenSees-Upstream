...
void *OPS_CollocationHSFixedNumIter(void)
{
    ...
    if (argc != 1 && argc != 3 && argc != 5) {
        opserr <<
            "WARNING - incorrect number of args want CollocationHSFixedNumIter $theta <-polyOrder $O>\n";
        opserr <<
            "          or CollocationHSFixedNumIter $theta $beta $gamma <-polyOrder $O>\n";
        return 0;
    }
    ...
    if (OPS_GetDouble(&numData, dData) != 0) {
        opserr <<
            "WARNING - invalid args want CollocationHSFixedNumIter $theta <-polyOrder $O>\n";
        opserr <<
            "          or CollocationHSFixedNumIter $theta $beta $gamma <-polyOrder $O>\n";
        return 0;
    }
    ...
    if (numData + 2 == argc) {
        ...
        if (strcmp(argvLoc, "-polyOrder") == 0) {
            ...
            if (OPS_GetInt(&numData2, &polyOrder) != 0) {
                opserr <<
                    "WARNING - invalid polyOrder want CollocationHSFixedNumIter $rhoInf <-polyOrder $O>\n";
                opserr <<
                    "          or CollocationHSFixedNumIter $alphaI $alphaF $beta $gamma <-polyOrder $O>\n";
            }
        }
    }
    ...
    if (numData == 1)
        ...
        theIntegrator =
            new CollocationHSFixedNumIter(dData[0], dData[1], dData[2],
                                          polyOrder);

    if (theIntegrator == 0)
        opserr <<
            "WARNING - out of memory creating CollocationHSFixedNumIter integrator\n";

    return theIntegrator;
}


CollocationHSFixedNumIter::CollocationHSFixedNumIter():TransientIntegrator(INTEGRATOR_TAGS_CollocationHSFixedNumIter),
...
CollocationHSFixedNumIter::CollocationHSFixedNumIter(double _theta,
                                                     int
                                                     polyorder):TransientIntegrator
    (INTEGRATOR_TAGS_CollocationHSFixedNumIter), theta(_theta), beta(0.0),
gamma(0.5), polyOrder(polyorder), deltaT(0.0), c1(0.0), c2(0.0), c3(0.0),
x(1.0), Ut(0), Utdot(0), Utdotdot(0), U(0), Udot(0), Udotdot(0), Utm1(0),
Utm2(0), scaledDeltaU(0)
{
    beta = -6.018722044382699e+002 * pow(theta, 9) +
        ...
        -2.638652989051994e+004 * theta + 3.572862280471971e+003;
}
...
CollocationHSFixedNumIter::CollocationHSFixedNumIter(double _theta,
                                                     ...
                                                     int
                                                     polyorder):TransientIntegrator
    (INTEGRATOR_TAGS_CollocationHSFixedNumIter), theta(_theta),
beta(_beta), gamma(_gamma), polyOrder(polyorder), deltaT(0.0), c1(0.0),
c2(0.0), c3(0.0), x(1.0), Ut(0), Utdot(0), Utdotdot(0), U(0), Udot(0),
Udotdot(0), Utm1(0), Utm2(0), scaledDeltaU(0)
{
...
int
 CollocationHSFixedNumIter::newStep(double _deltaT)
{
    if (theta <= 0.0) {
        opserr <<
            "CollocationHSFixedNumIter::newStep() - error in variable\n";
    ...
    if (U == 0) {
        opserr <<
            "CollocationHSFixedNumIter::newStep() - domainChange() failed or hasn't been called\n";
        return -3;
    }
...
int CollocationHSFixedNumIter::domainChanged()
{
    ...
    if (Ut == 0 || Ut->Size() != size) {
        ...
        if (Ut == 0 || Ut->Size() != size ||
            ...
            opserr <<
                "CollocationHSFixedNumIter::domainChanged() - ran out of memory\n";
...
int CollocationHSFixedNumIter::update(const Vector & deltaU)
{
    ...
    if (theModel == 0) {
        opserr <<
            "WARNING CollocationHSFixedNumIter::update() - no AnalysisModel set\n";
        return -1;
    }
    ConvergenceTest *theTest = this->getConvergenceTest();
    if (theTest == 0) {
        opserr <<
            "WARNING CollocationHSFixedNumIter::update() - no ConvergenceTest set\n";
        return -2;
    }
    // check domainChanged() has been called, i.e. Ut will not be zero
    if (Ut == 0) {
        opserr <<
            "WARNING CollocationHSFixedNumIter::update() - domainChange() failed or not called\n";
        return -3;
    }
    // check deltaU is of correct size
    if (deltaU.Size() != U->Size()) {
        opserr <<
            "WARNING CollocationHSFixedNumIter::update() - Vectors of incompatible size ";
        opserr << " expecting " << U->
    ...
    if (polyOrder == 1) {
        ...
        (*scaledDeltaU) =
            x * (x + 1.0) / 2.0 * ((*U) + deltaU) - (x - 1.0) * (x +
                                                                 1.0) *
            (*Ut) + (x - 1.0) * x / 2.0 * (*Utm1) - (*U);
    } else if (polyOrder == 3) {
        (*scaledDeltaU) =
            x * (x + 1.0) * (x + 2.0) / 6.0 * ((*U) + deltaU) - (x -
                                                                 1.0) *
            (x + 1.0)
            * (x + 2.0) / 2.0 * (*Ut) + (x - 1.0) * x * (x +
                                                         2.0) / 2.0 *
            (*Utm1) - (x - 1.0) * x * (x + 1.0) / 6.0 * (*Utm2) - (*U);
    } else {
        opserr <<
            "WARNING CollocationHSFixedNumIter::update() - polyOrder > 3 not supported\n";
        return -5;
    }
    ...
    if (theModel->updateDomain() < 0) {
        opserr <<
            "CollocationHSFixedNumIter::update() - failed to update the domain\n";
        return -5;
    }
...
int CollocationHSFixedNumIter::commit(void)
{
    ...
    if (theModel == 0) {
        opserr <<
            "WARNING CollocationHSFixedNumIter::commit() - no AnalysisModel set\n";
        return -1;
    }
    ...
    if (theSOE == 0) {
        opserr <<
            "WARNING CollocationHSFixedNumIter::commit() - no LinearSOE set\n";
        return -2;
    }
...
const Vector & CollocationHSFixedNumIter::getVel()
{
    return *Udot;
}
...
int CollocationHSFixedNumIter::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (theChannel.sendVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING CollocationHSFixedNumIter::sendSelf() - failed to send the data\n";
        return -1;
    }
...
int CollocationHSFixedNumIter::recvSelf(int cTag, Channel & theChannel,
                                        FEM_ObjectBroker & theBroker)
{
    Vector data(4);
    if (theChannel.recvVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING CollocationHSFixedNumIter::recvSelf() - could not receive data\n";
        return -1;
    }
...
void CollocationHSFixedNumIter::Print(OPS_Stream & s, int flag)
{
    ...
    if (theModel != 0) {
        ...
        s << "CollocationHSFixedNumIter - currentTime: " << currentTime <<
            endln;
        s << "  theta: " << theta << endln;
