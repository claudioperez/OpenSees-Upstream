...
Newmark::Newmark(int classTag):TransientIntegrator(classTag),
displ(true),
gamma(0),
beta(0),
c1(0.0),
...
assemblyFlag(0), independentRHS(), dUn(), dVn(), dAn()
{
...
Newmark::Newmark(double _gamma, double _beta, bool dispFlag, bool aflag,
                 int classTag_):TransientIntegrator(classTag_),
displ(dispFlag),
gamma(_gamma),
beta(_beta),
c1(0.0),
...
massMatrixMultiplicator(0),
dampingMatrixMultiplicator(0),
assemblyFlag(aflag), independentRHS(), dUn(), dVn(), dAn()
{
 ...
 Newmark::newStep(double deltaT)
{
    ...
    if (U == 0) {
        opserr <<
            "Newmark::newStep() - domainChange() failed or hasn't been called\n";
        return -3;
    }
...
const Vector & Newmark::getVel(void)
{
    return *Udot;
}
 ...
 Newmark::update(const Vector & deltaU)
{
    ...
    if (Ut == 0) {
        opserr <<
            "WARNING Newmark::update() - domainChange() failed or not called\n";
        return -2;
    }
    ...
    if (deltaU.Size() != U->Size()) {
        opserr <<
            "WARNING Newmark::update() - Vectors of incompatible size ";
        opserr << " expecting " << U->
...
int
 Newmark::recvSelf(int cTag, Channel & theChannel,
...
int Newmark::formEleResidual(FE_Element * theEle)
{
    if (sensitivityFlag == 0) { // no sensitivity
        ...
        if (displ != 1) {
            opserr <<
                "ERROR: Newmark::formEleResidual() -- the implemented" <<
                " scheme only works if the displ variable is set to true."
                << endln;
        }
        double a2 = -c3;
        double a3 = -c2 / gamma;
        ...
        while ((dofPtr = theDOFs()) != 0) {
            ...
            const ID & id = dofPtr->getID();
            int idSize = id.Size();
            const Vector & dispSens =
                dofPtr->getDispSensitivity(gradNumber);
            for (i = 0; i < idSize; i++) {
                loc = id(i);
                if (loc >= 0) {
                    dUn(loc) = dispSens(i);
            ...
            const Vector & accelSens =
                dofPtr->getAccSensitivity(gradNumber);
            for (i = 0; i < idSize; i++) {
                loc = id(i);
                if (loc >= 0) {
                    dAn(loc) = accelSens(i);
...
int Newmark::formNodUnbalance(DOF_Group * theDof)
{
...
int Newmark::formSensitivityRHS(int passedGradNumber)
{
    sensitivityFlag = 1;
...
int Newmark::saveSensitivity(const Vector & vNew, int gradNum,
                             int numGrads)
{
    ...
    while ((dofPtr1 = theDOFGrps()) != 0) {
        dofPtr1->saveSensitivity(vNew, vdotNew, vdotdotNew, gradNum,
                                 numGrads);
    }
...
int Newmark::commitSensitivity(int gradNum, int numGrads)
{
...
double Newmark::getCFactor(void)
{
    return c2;
}
...
int Newmark::computeSensitivities(void)
{
