...
OPS_HHTGeneralizedExplicit_TP (void)
{
    ...
    if (argc != 2 && argc != 4)
      {
          opserr <<
              "WARNING - incorrect number of args want HHTGeneralizedExplicit_TP $rhoB $alphaF\n";
          opserr <<
              "          or HHTGeneralizedExplicit_TP $alphaI $alphaF $beta $gamma\n";
          return 0;
      }
    ...
    if (OPS_GetDouble (&argc, dData) != 0)
      {
          opserr <<
              "WARNING - invalid args want HHTGeneralizedExplicit_TP $rhoB $alphaF\n";
          opserr <<
              "          or HHTGeneralizedExplicit_TP $alphaI $alphaF $beta $gamma\n";
          return 0;
      }
    ...
    if (argc == 2)
        ...
        theIntegrator =
            new HHTGeneralizedExplicit_TP (dData[0], dData[1], dData[2],
                                           dData[3]);

    if (theIntegrator == 0)
        opserr <<
            "WARNING - out of memory creating HHTGeneralizedExplicit_TP integrator\n";

    return theIntegrator;
}


HHTGeneralizedExplicit_TP::HHTGeneralizedExplicit_TP ():TransientIntegrator (INTEGRATOR_TAGS_HHTGeneralizedExplicit_TP),
...
Ut (0), Utdot (0), Utdotdot (0), U (0), Udot (0), Udotdot (0), Put (0)
{

}


HHTGeneralizedExplicit_TP::HHTGeneralizedExplicit_TP (double _rhoB,
                                                      double _alphaF):
TransientIntegrator (INTEGRATOR_TAGS_HHTGeneralizedExplicit_TP),
alphaI ((2.0 - _rhoB) / (1.0 + _rhoB)),
alphaF (_alphaF),
...
HHTGeneralizedExplicit_TP::HHTGeneralizedExplicit_TP (double _alphaI,
                                                      double _alphaF,
                                                      double _beta,
                                                      double _gamma):
TransientIntegrator (INTEGRATOR_TAGS_HHTGeneralizedExplicit_TP),
...
int
HHTGeneralizedExplicit_TP::newStep (double _deltaT)
{
    updateCount = 0;
    ...
    if (deltaT <= 0.0)
      {
          opserr <<
              "HHTGeneralizedExplicit_TP::newStep() - error in variable\n";
    ...
    if (U == 0)
      {
          opserr <<
              "HHTGeneralizedExplicit_TP::newStep() - domainChange() failed or hasn't been called\n";
          return -4;
      }
    ...
    if (theModel->updateDomain (time, deltaT) < 0)
      {
          opserr <<
              "HHTGeneralizedExplicit_TP::newStep() - failed to update the domain\n";
          return -5;
      }
...
int
HHTGeneralizedExplicit_TP::revertToLastStep ()
{
...
int
HHTGeneralizedExplicit_TP::formEleTangent (FE_Element * theEle)
{
    theEle->zeroTangent ();
...
int
HHTGeneralizedExplicit_TP::formNodTangent (DOF_Group * theDof)
{
    theDof->zeroTangent ();
...
int
HHTGeneralizedExplicit_TP::formEleResidual (FE_Element * theEle)
{
    theEle->zeroResidual ();
...
int
HHTGeneralizedExplicit_TP::formNodUnbalance (DOF_Group * theDof)
{
...
int
HHTGeneralizedExplicit_TP::domainChanged ()
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    LinearSOE *theLinSOE = this->getLinearSOE ();
    ...
    if (Ut == 0 || Ut->Size () != size)
      {
          ...
          if (Ut == 0 || Ut->Size () != size ||
              ...
              Put == 0 || Put->Size () != size)
            {
                ...
                opserr <<
                    "HHTGeneralizedExplicit_TP::domainChanged() - ran out of memory\n";
...
HHTGeneralizedExplicit_TP::update (const Vector & aiPlusOne)
{
    ...
    if (updateCount > 1)
      {
          opserr <<
              "WARNING HHTGeneralizedExplicit_TP::update() - called more than once -";
          opserr <<
              " HHTGeneralizedExplicit_TP integration scheme requires a LINEAR solution algorithm\n";
          return -1;
      }
    ...
    if (theModel == 0)
      {
          opserr <<
              "WARNING HHTGeneralizedExplicit_TP::update() - no AnalysisModel set\n";
          return -2;
      }
    ...
    if (Ut == 0)
      {
          opserr <<
              "WARNING HHTGeneralizedExplicit_TP::update() - domainChange() failed or not called\n";
          return -3;
      }
    ...
    if (aiPlusOne.Size () != U->Size ())
      {
          opserr <<
              "WARNING HHTGeneralizedExplicit_TP::update() - Vectors of incompatible size ";
    ...
    if (theModel->updateDomain () < 0)
      {
          opserr <<
              "HHTGeneralizedExplicit_TP::update() - failed to update the domain\n";
          return -5;
      }
...
int
HHTGeneralizedExplicit_TP::commit (void)
{
    // get a pointer to the LinearSOE and the AnalysisModel
    LinearSOE *theLinSOE = this->getLinearSOE ();
    AnalysisModel *theModel = this->getAnalysisModel ();
...
int
HHTGeneralizedExplicit_TP::sendSelf (int cTag, Channel & theChannel)
{
    Vector data (4);
    data (0) = alphaI;
    data (1) = alphaF;
    ...
    if (theChannel.sendVector (this->getDbTag (), cTag, data) < 0)
      {
          opserr <<
              "WARNING HHTGeneralizedExplicit_TP::sendSelf() - could not send data\n";
          return -1;
      }
...
int
HHTGeneralizedExplicit_TP::recvSelf (int cTag, Channel & theChannel,
                                     FEM_ObjectBroker & theBroker)
{
    ...
    if (theChannel.recvVector (this->getDbTag (), cTag, data) < 0)
      {
          opserr <<
              "WARNING HHTGeneralizedExplicit_TP::recvSelf() - could not receive data\n";
          return -1;
      }
...
void
HHTGeneralizedExplicit_TP::Print (OPS_Stream & s, int flag)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel != 0)
      {
          double currentTime = theModel->getCurrentDomainTime ();
          s << "HHTGeneralizedExplicit_TP - currentTime: " << currentTime <<
              endln;
