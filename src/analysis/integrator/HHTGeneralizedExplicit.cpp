...
void *
OPS_HHTGeneralizedExplicit (void)
{
    ...
    if (argc < 2 || argc > 5)
      {
          opserr <<
              "WARNING - incorrect number of args want HHTGeneralizedExplicit $rhoB $alphaF <-updateElemDisp>\n";
          opserr <<
              "          or HHTGeneralizedExplicit $alphaI $alphaF $beta $gamma <-updateElemDisp>\n";
          return 0;
      }
    ...
    if (OPS_GetDouble (&numData, dData) != 0)
      {
          opserr <<
              "WARNING - invalid args want HHTGeneralizedExplicit $rhoB $alphaF <-updateElemDisp>\n";
          opserr <<
              "          or HHTGeneralizedExplicit $alphaI $alphaF $beta $gamma <-updateElemDisp>\n";
          return 0;
      }
    ...
    if (argc < 4)
        theIntegrator =
            new HHTGeneralizedExplicit (dData[0], dData[1], updElemDisp);
    else
        theIntegrator =
            new HHTGeneralizedExplicit (dData[0], dData[1], dData[2],
                                        dData[3], updElemDisp);

    if (theIntegrator == 0)
        opserr <<
            "WARNING - out of memory creating HHTGeneralizedExplicit integrator\n";

    return theIntegrator;
}


HHTGeneralizedExplicit::HHTGeneralizedExplicit ():TransientIntegrator (INTEGRATOR_TAGS_HHTGeneralizedExplicit),
...
HHTGeneralizedExplicit::HHTGeneralizedExplicit (double _alphaI,
                                                double _alphaF, double _beta,
                                                double _gamma,
                                                bool updelemdisp):
TransientIntegrator (INTEGRATOR_TAGS_HHTGeneralizedExplicit),
alphaI (_alphaI),
...
int
HHTGeneralizedExplicit::newStep (double _deltaT)
{
    updateCount = 0;
    ...
    if (U == 0)
      {
          opserr <<
              "HHTGeneralizedExplicit::newStep() - domainChange() failed or hasn't been called\n";
          return -3;
      }
    ...
    if (theModel->updateDomain (time, deltaT) < 0)
      {
          opserr <<
              "HHTGeneralizedExplicit::newStep() - failed to update the domain\n";
          return -4;
      }
...
int
HHTGeneralizedExplicit::revertToLastStep ()
{
...
int
HHTGeneralizedExplicit::formEleTangent (FE_Element * theEle)
{
    theEle->zeroTangent ();
...
int
HHTGeneralizedExplicit::formNodTangent (DOF_Group * theDof)
{
    theDof->zeroTangent ();
...
int
HHTGeneralizedExplicit::domainChanged ()
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    ...
    if (Ut == 0 || Ut->Size () != size)
      {
          ...
          if (Ut == 0 || Ut->Size () != size ||
              ...
              Ualphadotdot == 0 || Ualphadotdot->Size () != size)
            {
                ...
                opserr <<
                    "HHTGeneralizedExplicit::domainChanged() - ran out of memory\n";
...
int
HHTGeneralizedExplicit::update (const Vector & aiPlusOne)
{
    updateCount++;
    if (updateCount > 1)
      {
          opserr <<
              "WARNING HHTGeneralizedExplicit::update() - called more than once -";
          opserr <<
              " HHTGeneralizedExplicit integration scheme requires a LINEAR solution algorithm\n";
          return -1;
      }
    ...
    if (theModel == 0)
      {
          opserr <<
              "WARNING HHTGeneralizedExplicit::update() - no AnalysisModel set\n";
          return -2;
      }
    ...
    if (Ut == 0)
      {
          opserr <<
              "WARNING HHTGeneralizedExplicit::update() - domainChange() failed or not called\n";
          return -3;
      }
    ...
    if (aiPlusOne.Size () != U->Size ())
      {
          opserr <<
              "WARNING HHTGeneralizedExplicit::update() - Vectors of incompatible size ";
    ...
    if (theModel->updateDomain () < 0)
      {
          opserr <<
              "HHTGeneralizedExplicit::update() - failed to update the domain\n";
          return -5;
      }
...
int
HHTGeneralizedExplicit::commit (void)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel == 0)
      {
          opserr <<
              "WARNING HHTGeneralizedExplicit::commit() - no AnalysisModel set\n";
          return -1;
      }
...
int
HHTGeneralizedExplicit::sendSelf (int cTag, Channel & theChannel)
{
    Vector data (5);
    data (0) = alphaI;
    data (1) = alphaF;
    ...
    if (theChannel.sendVector (this->getDbTag (), cTag, data) < 0)
      {
          opserr <<
              "WARNING HHTGeneralizedExplicit::sendSelf() - could not send data\n";
          return -1;
      }
...
int
HHTGeneralizedExplicit::recvSelf (int cTag, Channel & theChannel,
                                  FEM_ObjectBroker & theBroker)
{
    ...
    if (theChannel.recvVector (this->getDbTag (), cTag, data) < 0)
      {
          opserr <<
              "WARNING HHTGeneralizedExplicit::recvSelf() - could not receive data\n";
          return -1;
      }
...
void
HHTGeneralizedExplicit::Print (OPS_Stream & s, int flag)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel != 0)
      {
          double currentTime = theModel->getCurrentDomainTime ();
          s << "HHTGeneralizedExplicit - currentTime: " << currentTime <<
              endln;
