...
void *OPS_StagedNewmark(void)
{
    ...
    if (argc != 2 && argc != 4) {
        opserr <<
            "WARNING - incorrect number of args want StagedNewmark $gamma $beta <-form $typeUnknown>\n";
        return 0;
    }
    ...
    if (OPS_GetDouble(&numData, dData) != 0) {
        opserr <<
            "WARNING - invalid args want StagedNewmark $gamma $beta <-form $typeUnknown>\n";
        return 0;
    }
    ...
    if (theIntegrator == 0)
        opserr <<
            "WARNING - out of memory creating StagedNewmark integrator\n";
...
StagedNewmark::StagedNewmark():Newmark(0, 0, 0, 0,
...
StagedNewmark::StagedNewmark(double _gamma, double _beta, bool dispFlag,
                             bool aflag):Newmark(_gamma, _beta, dispFlag,
                                                 aflag,
                                                 INTEGRATOR_TAGS_StagedNewmark)
{
...
int
 StagedNewmark::formTangent(int statFlag)
{
    int rank = 0;
    int nproc = 1;
