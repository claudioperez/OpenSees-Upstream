...
AlphaOS_TP::AlphaOS_TP (double _alpha, bool updelemdisp):
TransientIntegrator (INTEGRATOR_TAGS_AlphaOS_TP),
alpha (_alpha),
beta ((2 - _alpha) * (2 - _alpha) * 0.25),
gamma (1.5 - _alpha),
...
int
AlphaOS_TP::formEleTangent (FE_Element * theEle)
{
    theEle->zeroTangent ();
...
int
AlphaOS_TP::formNodTangent (DOF_Group * theDof)
{
    theDof->zeroTangent ();
...
int
AlphaOS_TP::formEleResidual (FE_Element * theEle)
{
    theEle->zeroResidual ();
...
int
AlphaOS_TP::formNodUnbalance (DOF_Group * theDof)
{
...
int
AlphaOS_TP::update (const Vector & deltaU)
{
    updateCount++;
    if (updateCount > 1)
      {
...
int
AlphaOS_TP::sendSelf (int cTag, Channel & theChannel)
{
    Vector data (4);
    data (0) = alpha;
    data (1) = beta;
...
int
AlphaOS_TP::recvSelf (int cTag, Channel & theChannel,
...
void
AlphaOS_TP::Print (OPS_Stream & s, int flag)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel != 0)
      {
...
int
AlphaOS_TP::formElementResidual (void)
{
