...
AlphaOS_TP::AlphaOS_TP():TransientIntegrator(INTEGRATOR_TAGS_AlphaOS_TP),
...
AlphaOS_TP::AlphaOS_TP(double _alpha,
    ...
    (INTEGRATOR_TAGS_AlphaOS_TP), alpha(_alpha),
beta((2 - _alpha) * (2 - _alpha) * 0.25), gamma(1.5 - _alpha),
updElemDisp(updelemdisp), deltaT(0.0), updateCount(0), c1(0.0), c2(0.0),
c3(0.0), alphaD(alpha), alphaR(alpha), alphaKU(0.0), alphaP(alpha), Ut(0),
...
AlphaOS_TP::AlphaOS_TP(double _alpha, double _beta, double _gamma,
                       bool
                       updelemdisp):TransientIntegrator
    (INTEGRATOR_TAGS_AlphaOS_TP), alpha(_alpha), beta(_beta),
gamma(_gamma), updElemDisp(updelemdisp), deltaT(0.0), updateCount(0),
c1(0.0), c2(0.0), c3(0.0), alphaD(alpha), alphaR(alpha), alphaKU(0.0),
alphaP(alpha), Ut(0), Utdot(0), Utdotdot(0), U(0), Udot(0), Udotdot(0),
Upt(0), Put(0)
{
...
int
 AlphaOS_TP::newStep(double _deltaT)
{
    updateCount = 0;
    ...
    if (U == 0) {
        opserr <<
            "AlphaOS_TP::newStep() - domainChange() failed or hasn't been called\n";
        return -4;
    }
...
const Vector & AlphaOS_TP::getVel()
{
    return *Udot;
}
...
int AlphaOS_TP::update(const Vector & deltaU)
{
    ...
    if (updateCount > 1) {
        ...
        opserr <<
            " AlphaOS_TP integration scheme requires a LINEAR solution algorithm\n";
        return -1;
    }
    ...
    if (Ut == 0) {
        opserr <<
            "WARNING AlphaOS_TP::update() - domainChange() failed or not called\n";
        return -3;
    }
    // check deltaU is of correct size
    if (deltaU.Size() != U->Size()) {
        opserr <<
            "WARNING AlphaOS_TP::update() - Vectors of incompatible size ";
        opserr << " expecting " << U->
...
int AlphaOS_TP::recvSelf(int cTag, Channel & theChannel,
                         FEM_ObjectBroker & theBroker)
{
    Vector data(4);
    if (theChannel.recvVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING AlphaOS_TP::recvSelf() - could not receive data\n";
        return -1;
    }
        ...
        if (alphaKU > 0.0) {
            if (statusFlag == CURRENT_TANGENT) {
                if (theSOE->addB(elePtr->getK_Force(*Ut - *Upt),
                    ...
                    opserr <<
                        "WARNING AlphaOS_TP::formElementResidual() -";
                    opserr << " failed in addB for ID " << elePtr->getID();
                    return -2;
                }
            } else if (statusFlag == INITIAL_TANGENT) {
