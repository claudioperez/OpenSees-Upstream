...
PFEMIntegrator::PFEMIntegrator():TransientIntegrator(INTEGRATOR_TAGS_PFEMIntegrator),
...
PFEMIntegrator::PFEMIntegrator(double _gamma, double _beta, int _disp,
                               int
                               _init):TransientIntegrator
    (INTEGRATOR_TAGS_PFEMIntegrator), displ(_disp), init(_init),
gamma(_gamma), beta(_beta), c1(0.0), c2(0.0), c3(0.0), Ut(0), Utdot(0),
Utdotdot(0), U(0), Udot(0), Udotdot(0), determiningMass(false),
sensitivityFlag(0), gradNumber(0), massMatrixMultiplicator(0),
dampingMatrixMultiplicator(0), assemblyFlag(false), independentRHS(),
dUn(), dVn(), dAn()
{
...
int
 PFEMIntegrator::newStep(double deltaT)
{
    if (beta == 0 || gamma == 0) {
    ...
    if (theModel == 0) {
        opserr <<
            "Analysis model has not been linked - PFEMIntegrator::newStep()\n";
        return -1;
    }
    ...
    if (U == 0) {
        opserr <<
            "PFEMIntegrator::newStep() - domainChange() failed or hasn't been called\n";
        return -3;
    }
    ...
    if (init == 1) {
        ...
        *Udotdot = *Utdotdot;
        if (gamma > 0 && beta > 0) {
            Udot->addVector(1.0 - gamma / beta, *Utdotdot,
                            deltaT * (1.0 - 0.5 * gamma / beta));
            Udot->addVector(1.0, *U, gamma / (deltaT * beta));
            Udot->addVector(1.0, *Ut, -gamma / (deltaT * beta));
            Udotdot->addVector(1.0 - 0.5 / beta, *Utdot,
                               -1.0 / (beta * deltaT));
        ...
        *Udotdot = *Utdotdot;
        if (gamma > 0 && beta > 0) {
            ...
            U->addVector(1.0, *Utdotdot,
                         deltaT * deltaT * (0.5 - beta / gamma));
            Udotdot->addVector((1 - 1.0 / gamma), *Udot,
                               1.0 / (gamma * deltaT));
    ...
    if (theModel->updateDomain(time, deltaT) < 0) {
        opserr <<
            "PFEMIntegrator::newStep() - failed to update the domain\n";
        return -4;
    }
...
int PFEMIntegrator::formTangent(int statFlag)
{
    int result = 0;
    statusFlag = statFlag;
...
int PFEMIntegrator::formEleResidual(FE_Element * theEle)
{
...
int PFEMIntegrator::formNodUnbalance(DOF_Group * theDof)
{
    if (sensitivityFlag == 0) { // NO SENSITIVITY ANALYSIS
...
int PFEMIntegrator::domainChanged()
{
    ...
    if (Ut == 0 || Ut->Size() != size) {
        ...
        if (Ut == 0 || Ut->Size() != size ||
            ...
            opserr <<
                "PFEMIntegrator::domainChanged - ran out of memory\n";
...
int PFEMIntegrator::update(const Vector & delta)
{
    ...
    if (theModel == 0) {
        opserr <<
            "WARNING PFEMIntegrator::update() - no AnalysisModel set\n";
        return -1;
    }
    // check domainChanged() has been called, i.e. Ut will not be zero
    if (Ut == 0) {
        opserr <<
            "WARNING PFEMIntegrator::update() - domainChange() failed or not called\n";
        return -2;
    }
    // check deltaU is of correct size
    if (delta.Size() != U->Size()) {
        opserr <<
            "WARNING PFEMIntegrator::update() - Vectors of incompatible size ";
        opserr << " expecting " << U->
            Size() << " obtained " << delta.Size() << endln;
        return -3;
    }
    ...
    if (theModel->updateDomain() < 0) {
        opserr <<
            "PFEMIntegrator::update() - failed to update the domain\n";
        return -4;
    }
...
const Vector & PFEMIntegrator::getVel()
{
    return *Udot;
}
...
int PFEMIntegrator::recvSelf(int cTag, Channel & theChannel,
...
int PFEMIntegrator::formIndependentSensitivityRHS()
{
    return 0;
}

int PFEMIntegrator::formIndependentSensitivityLHS(int statFlag)
{
...
int PFEMIntegrator::commitSensitivity(int gradNum, int numGrads)
{
