          ...
          if (gamma > 0 && beta > 0)
            {
                ...
                Udotdot->addVector (1.0, *Ut,
                                    -1.0 / (deltaT * deltaT * beta));
            }
          else
            {
...
int
PFEMIntegrator::revertToLastStep ()
{
...
int
PFEMIntegrator::formEleTangent (FE_Element * theEle)
{
...
int
PFEMIntegrator::formNodTangent (DOF_Group * theDof)
{
    if (determiningMass == true)
        return 0;
...
int
PFEMIntegrator::update (const Vector & delta)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel == 0)
      {
    ...
    if (delta.Size () != U->Size ())
      {
          ...
          opserr << " expecting " << U->Size () << " obtained " << delta.
              Size () << endln;
          return -3;
      }
...
int
PFEMIntegrator::sendSelf (int cTag, Channel & theChannel)
{
    Vector data (4);
    data (0) = gamma;
    data (1) = beta;
...
int
PFEMIntegrator::recvSelf (int cTag, Channel & theChannel,
...
void
PFEMIntegrator::Print (OPS_Stream & s, int flag)
{
    if (flag != 0)
        return;
    AnalysisModel *theModel = this->getAnalysisModel ();
...
int
PFEMIntegrator::saveSensitivity (const Vector & dVNew, int gradNum,
                                 int numGrads)
{
