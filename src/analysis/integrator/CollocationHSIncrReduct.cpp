...
OPS_CollocationHSIncrReduct (void)
{
    ...
    if (argc != 2 && argc != 4)
      {
          opserr <<
              "WARNING - incorrect number of args want CollocationHSIncrReduct $theta $reduct\n";
          opserr <<
    ...
    if (OPS_GetDouble (&argc, dData) != 0)
      {
          opserr <<
              "WARNING - invalid args want CollocationHSIncrReduct $theta $reduct\n";
          opserr <<
              "          or CollocationHSIncrReduct $theta $beta $gamma $reduct\n";
          return 0;
      }
    ...
    if (argc == 2)
        ...
        theIntegrator =
            new CollocationHSIncrReduct (dData[0], dData[1], dData[2],
                                         dData[3]);

    if (theIntegrator == 0)
        opserr <<
            "WARNING - out of memory creating CollocationHSIncrReduct integrator\n";

    return theIntegrator;
}


CollocationHSIncrReduct::CollocationHSIncrReduct ():TransientIntegrator (INTEGRATOR_TAGS_CollocationHSIncrReduct),
...
CollocationHSIncrReduct::CollocationHSIncrReduct (double _theta,
                                                  double _reduct):
...
CollocationHSIncrReduct::CollocationHSIncrReduct (double _theta, double _beta,
                                                  double _gamma,
                                                  double _reduct):
TransientIntegrator (INTEGRATOR_TAGS_CollocationHSIncrReduct),
theta (_theta),
...
int
CollocationHSIncrReduct::newStep (double _deltaT)
{
    if (theta <= 0.0)
      {
          opserr <<
              "CollocationHSIncrReduct::newStep() - error in variable\n";
    ...
    if (U == 0)
      {
          opserr <<
              "CollocationHSIncrReduct::newStep() - domainChange() failed or hasn't been called\n";
          return -3;
      }
...
int
CollocationHSIncrReduct::revertToLastStep ()
{
...
int
CollocationHSIncrReduct::formEleTangent (FE_Element * theEle)
{
    theEle->zeroTangent ();
...
int
CollocationHSIncrReduct::formNodTangent (DOF_Group * theDof)
{
    theDof->zeroTangent ();
...
int
CollocationHSIncrReduct::domainChanged ()
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    ...
    if (Ut == 0 || Ut->Size () != size)
      {
          ...
          if (Ut == 0 || Ut->Size () != size ||
              ...
              scaledDeltaU == 0 || scaledDeltaU->Size () != size)
            {
                ...
                opserr <<
                    "CollocationHSIncrReduct::domainChanged() - ran out of memory\n";
...
int
CollocationHSIncrReduct::update (const Vector & deltaU)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel == 0)
      {
          opserr <<
              "WARNING CollocationHSIncrReduct::update() - no AnalysisModel set\n";
          return -1;
      }
    ...
    if (Ut == 0)
      {
          opserr <<
              "WARNING CollocationHSIncrReduct::update() - domainChange() failed or not called\n";
          return -2;
      }
    ...
    if (deltaU.Size () != U->Size ())
      {
          opserr <<
              "WARNING CollocationHSIncrReduct::update() - Vectors of incompatible size ";
    ...
    if (theModel->updateDomain () < 0)
      {
          opserr <<
              "CollocationHSIncrReduct::update() - failed to update the domain\n";
          return -4;
      }
...
int
CollocationHSIncrReduct::commit (void)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel == 0)
      {
          opserr <<
              "WARNING CollocationHSIncrReduct::commit() - no AnalysisModel set\n";
          return -1;
      }
...
int
CollocationHSIncrReduct::sendSelf (int cTag, Channel & theChannel)
{
    ...
    if (theChannel.sendVector (this->getDbTag (), cTag, data) < 0)
      {
          opserr <<
              "WARNING CollocationHSIncrReduct::sendSelf() - failed to send the data\n";
          return -1;
      }
...
int
CollocationHSIncrReduct::recvSelf (int cTag, Channel & theChannel,
                                   FEM_ObjectBroker & theBroker)
{
    ...
    if (theChannel.recvVector (this->getDbTag (), cTag, data) < 0)
      {
          opserr <<
              "WARNING CollocationHSIncrReduct::recvSelf() - could not receive data\n";
          return -1;
      }
...
void
CollocationHSIncrReduct::Print (OPS_Stream & s, int flag)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel != 0)
      {
          double currentTime = theModel->getCurrentDomainTime ();
          s << "CollocationHSIncrReduct - currentTime: " << currentTime <<
              endln;
          s << "  theta: " << theta << endln;
