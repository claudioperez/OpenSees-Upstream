...
OPS_KRAlphaExplicit (void)
{
    ...
    if (argc != 1 && argc != 2)
      {
          opserr <<
              "WARNING - incorrect number of args want KRAlphaExplicit $rhoInf <-updateElemDisp>\n";
          return 0;
      }
    ...
    if (OPS_GetDouble (&numData, &rhoInf) != 0)
      {
          opserr <<
              "WARNING - invalid args want KRAlphaExplicit $rhoInf <-updateElemDisp>\n";
          return 0;
      }
    ...
    if (theIntegrator == 0)
        opserr <<
            "WARNING - out of memory creating KRAlphaExplicit integrator\n";

    return theIntegrator;
}


KRAlphaExplicit::KRAlphaExplicit ():TransientIntegrator (INTEGRATOR_TAGS_KRAlphaExplicit),
...
Ualpha (0), Ualphadot (0), Ualphadotdot (0), Utdothat (0)
{

}


KRAlphaExplicit::KRAlphaExplicit (double _rhoInf, bool updelemdisp):
TransientIntegrator (INTEGRATOR_TAGS_KRAlphaExplicit),
alphaM ((2.0 - _rhoInf) / (1.0 + _rhoInf)),
alphaF (1.0 / (1.0 + _rhoInf)),
beta (1.0 / (1.0 + _rhoInf) / (1.0 + _rhoInf)),
gamma (0.5 * (3.0 - _rhoInf) / (1.0 + _rhoInf)),
updElemDisp (updelemdisp),
deltaT (0.0),
alpha1 (0),
alpha3 (0),
Mhat (0),
...
int
KRAlphaExplicit::newStep (double _deltaT)
{
    updateCount = 0;
    ...
    if (beta == 0 || gamma == 0)
      {
          opserr <<
              "WARNING KRAlphaExplicit::newStep() - error in variable\n";
    ...
    if (theModel == 0)
      {
          opserr <<
              "WARNING KRAlphaExplicit::newStep() - no AnalysisModel set\n";
          return -2;
      }
    ...
    if (initAlphaMatrices || _deltaT != deltaT)
      {
          ...
          LinearSOE *theFullLinSOE =
              new FullGenLinSOE (size, *theFullLinSolver);
          if (theFullLinSOE == 0)
            {
                opserr <<
                    "WARNING KRAlphaExplicit::newStep() - failed to create FullLinearSOE\n";
                return -4;
            }
          theFullLinSOE->setLinks (*theModel);

          // now switch the SOE to the FullLinearSOE
          this->IncrementalIntegrator::setLinks (*theModel, *theFullLinSOE,
                                                 theTest);
          ...
          this->IncrementalIntegrator::setLinks (*theModel, *theLinSOE,
                                                 theTest);
    ...
    if (U == 0)
      {
          opserr <<
              "WARNING KRAlphaExplicit::newStep() - domainChange() failed or hasn't been called\n";
          return -6;
      }
    ...
    if (theModel->updateDomain (time, deltaT) < 0)
      {
          opserr <<
              "WARNING KRAlphaExplicit::newStep() - failed to update the domain\n";
          return -7;
      }
...
int
KRAlphaExplicit::revertToLastStep ()
{
...
int
KRAlphaExplicit::formTangent (int statFlag)
{
    statusFlag = statFlag;
...
int
KRAlphaExplicit::formEleTangent (FE_Element * theEle)
{
    theEle->zeroTangent ();
...
int
KRAlphaExplicit::formNodTangent (DOF_Group * theDof)
{
    theDof->zeroTangent ();
    ...
    if (Ut == 0 || Ut->Size () != size)
      {
          ...
          if (alpha1 == 0 || alpha1->noRows () != size
              || alpha1->noCols () != size || alpha3 == 0
              || alpha3->noRows () != size || alpha3->noCols () != size
              || Mhat == 0 || Mhat->noRows () != size
              || Mhat->noCols () != size || Ut == 0 || Ut->Size () != size
              || Utdot == 0 || Utdot->Size () != size || Utdotdot == 0
              || Utdotdot->Size () != size || U == 0 || U->Size () != size
              || Udot == 0 || Udot->Size () != size || Udotdot == 0
              || Udotdot->Size () != size || Ualpha == 0
              || Ualpha->Size () != size || Ualphadot == 0
              || Ualphadot->Size () != size || Ualphadotdot == 0
              || Ualphadotdot->Size () != size || Utdothat == 0
              || Utdothat->Size () != size)
            {
...
int
KRAlphaExplicit::update (const Vector & aiPlusOne)
{
    updateCount++;
    if (updateCount > 1)
      {
          opserr <<
              "WARNING KRAlphaExplicit::update() - called more than once -";
          opserr <<
              " KRAlphaExplicit integration scheme requires a LINEAR solution algorithm\n";
          return -1;
      }
    ...
    if (theModel == 0)
      {
          opserr <<
              "WARNING KRAlphaExplicit::update() - no AnalysisModel set\n";
          return -2;
      }
    ...
    if (Ut == 0)
      {
          opserr <<
              "WARNING KRAlphaExplicit::update() - domainChange() failed or not called\n";
          return -3;
      }
    ...
    if (aiPlusOne.Size () != U->Size ())
      {
          opserr <<
              "WARNING KRAlphaExplicit::update() - Vectors of incompatible size ";
          opserr << " expecting " << U->Size () << " obtained " << aiPlusOne.
              Size () << endln;
          return -4;
      }
    ...
    if (theModel->updateDomain () < 0)
      {
          opserr <<
              "KRAlphaExplicit::update() - failed to update the domain\n";
          return -5;
      }
...
int
KRAlphaExplicit::commit (void)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel == 0)
      {
          opserr <<
              "WARNING KRAlphaExplicit::commit() - no AnalysisModel set\n";
          return -1;
      }
...
int
KRAlphaExplicit::sendSelf (int cTag, Channel & theChannel)
{
    Vector data (5);
    data (0) = alphaM;
    data (1) = alphaF;
    ...
    if (theChannel.sendVector (this->getDbTag (), cTag, data) < 0)
      {
          opserr <<
              "WARNING KRAlphaExplicit::sendSelf() - could not send data\n";
          return -1;
      }
...
int
KRAlphaExplicit::recvSelf (int cTag, Channel & theChannel,
                           FEM_ObjectBroker & theBroker)
{
    ...
    if (theChannel.recvVector (this->getDbTag (), cTag, data) < 0)
      {
          opserr <<
              "WARNING KRAlphaExplicit::recvSelf() - could not receive data\n";
          return -1;
      }
...
void
KRAlphaExplicit::Print (OPS_Stream & s, int flag)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel != 0)
      {
          ...
          s << "  alphaM: " << alphaM << "  alphaF: " << alphaF << "  beta: "
              << beta << "  gamma: " << gamma << endln;
