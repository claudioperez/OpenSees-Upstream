...
KRAlphaExplicit::KRAlphaExplicit(double _rhoInf,
                                 bool
                                 updelemdisp):TransientIntegrator
    (INTEGRATOR_TAGS_KRAlphaExplicit),
alphaM((2.0 - _rhoInf) / (1.0 + _rhoInf)), alphaF(1.0 / (1.0 + _rhoInf)),
beta(1.0 / (1.0 + _rhoInf) / (1.0 + _rhoInf)),
gamma(0.5 * (3.0 - _rhoInf) / (1.0 + _rhoInf)), updElemDisp(updelemdisp),
deltaT(0.0), alpha1(0), alpha3(0), Mhat(0), updateCount(0),
initAlphaMatrices(1), c1(0.0), c2(0.0), c3(0.0), Ut(0), Utdot(0),
Utdotdot(0), U(0), Udot(0), Udotdot(0), Ualpha(0), Ualphadot(0),
Ualphadotdot(0), Utdothat(0)
{
...
int KRAlphaExplicit::update(const Vector & aiPlusOne)
{
    ...
    if (aiPlusOne.Size() != U->Size()) {
        ...
        opserr << " expecting " << U->
            Size() << " obtained " << aiPlusOne.Size() << endln;
        return -4;
    }
...
const Vector & KRAlphaExplicit::getVel()
{
    return *Udot;
}
...
int KRAlphaExplicit::recvSelf(int cTag, Channel & theChannel,
