...
void *OPS_HHTGeneralized(void)
{
    ...
    if (argc != 1 && argc != 4) {
        ...
        opserr <<
            "          or HHTGeneralized $alphaI $alphaF $beta $gamma\n";
        return 0;
    }
    ...
    if (argc == 1)
        ...
        theIntegrator =
            new HHTGeneralized(dData[0], dData[1], dData[2], dData[3]);

    if (theIntegrator == 0)
        opserr <<
            "WARNING - out of memory creating HHTGeneralized integrator\n";

    return theIntegrator;
}


HHTGeneralized::HHTGeneralized():TransientIntegrator(INTEGRATOR_TAGS_HHTGeneralized),
...
HHTGeneralized::
HHTGeneralized(double
               _rhoInf):TransientIntegrator
(INTEGRATOR_TAGS_HHTGeneralized), alphaI((2.0 - _rhoInf) / (1.0 + _rhoInf)),
alphaF(1.0 / (1.0 + _rhoInf)), beta(1.0 / (1.0 + _rhoInf) / (1.0 + _rhoInf)), gamma(0.5 * (3.0 - _rhoInf) / (1.0 + _rhoInf)),
deltaT(0.0), c1(0.0), c2(0.0), c3(0.0), Ut(0), Utdot(0), Utdotdot(0), U(0), Udot(0), Udotdot(0), Ualpha(0), Ualphadot(0),
Ualphadotdot(0)
{
...
HHTGeneralized::HHTGeneralized(double _alphaI, double _alphaF,
    ...
    (INTEGRATOR_TAGS_HHTGeneralized), alphaI(_alphaI), alphaF(_alphaF),
...
int
 HHTGeneralized::newStep(double _deltaT)
{
    if (beta == 0 || gamma == 0) {
    ...
    if (U == 0) {
        opserr <<
            "HHTGeneralized::newStep() - domainChange() failed or hasn't been called\n";
        return -3;
    }
    ...
    if (theModel->updateDomain(time, deltaT) < 0) {
        opserr <<
            "HHTGeneralized::newStep() - failed to update the domain\n";
        return -4;
    }
...
int HHTGeneralized::domainChanged()
{
    ...
    if (Ut == 0 || Ut->Size() != size) {
        ...
        if (Ut == 0 || Ut->Size() != size ||
            ...
            opserr <<
                "HHTGeneralized::domainChanged() - ran out of memory\n";
...
int HHTGeneralized::update(const Vector & deltaU)
{
    ...
    if (theModel == 0) {
        opserr <<
            "WARNING HHTGeneralized::update() - no AnalysisModel set\n";
        return -1;
    }
    // check domainChanged() has been called, i.e. Ut will not be zero
    if (Ut == 0) {
        opserr <<
            "WARNING HHTGeneralized::update() - domainChange() failed or not called\n";
        return -2;
    }
    // check deltaU is of correct size
    if (deltaU.Size() != U->Size()) {
        opserr <<
            "WARNING HHTGeneralized::update() - Vectors of incompatible size ";
        opserr << " expecting " << U->
    ...
    if (theModel->updateDomain() < 0) {
        opserr <<
            "HHTGeneralized::update() - failed to update the domain\n";
        return -4;
    }
...
int HHTGeneralized::commit(void)
{
    ...
    if (theModel == 0) {
        opserr <<
            "WARNING HHTGeneralized::commit() - no AnalysisModel set\n";
        return -1;
    }
    // update the response at the DOFs
    ...
    if (theModel->updateDomain() < 0) {
        opserr <<
            "HHTGeneralized::commit() - failed to update the domain\n";
        return -2;
    }
...
const Vector & HHTGeneralized::getVel()
{
    return *Udot;
}
...
int HHTGeneralized::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (theChannel.sendVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING HHTGeneralized::sendSelf() - could not send data\n";
        return -1;
    }
...
int HHTGeneralized::recvSelf(int cTag, Channel & theChannel,
                             FEM_ObjectBroker & theBroker)
{
    Vector data(4);
    if (theChannel.recvVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING HHTGeneralized::recvSelf() - could not receive data\n";
        return -1;
    }
