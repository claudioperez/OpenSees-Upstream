...
int
HHT_TP::formNodTangent (DOF_Group * theDof)
{
    theDof->zeroTangent ();
...
int
HHT_TP::formEleResidual (FE_Element * theEle)
{
    theEle->zeroResidual ();
...
int
HHT_TP::formNodUnbalance (DOF_Group * theDof)
{
...
int
HHT_TP::update (const Vector & deltaU)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel == 0)
      {
...
int
HHT_TP::sendSelf (int cTag, Channel & theChannel)
{
    Vector data (3);
    data (0) = alpha;
    data (1) = beta;
...
int
HHT_TP::recvSelf (int cTag, Channel & theChannel,
...
void
HHT_TP::Print (OPS_Stream & s, int flag)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel != 0)
      {
