...
void *OPS_HHT_TP(void)
{
    ...
    if (argc != 1 && argc != 3) {
        opserr <<
            "WARNING - incorrect number of args want HHT_TP $alpha <$gamma $beta>\n";
        return 0;
    }
    ...
    if (OPS_GetDouble(&argc, dData) != 0) {
        opserr <<
            "WARNING - invalid args want HHT_TP $alpha <$gamma $beta>\n";
        return 0;
    }
...
HHT_TP::HHT_TP():TransientIntegrator(INTEGRATOR_TAGS_HHT_TP),
...
HHT_TP::HHT_TP(double _alpha):TransientIntegrator(INTEGRATOR_TAGS_HHT_TP),
alpha(_alpha),
...
HHT_TP::HHT_TP(double _alpha, double _beta,
               double _gamma):TransientIntegrator(INTEGRATOR_TAGS_HHT_TP),
alpha(_alpha), beta(_beta), gamma(_gamma), deltaT(0.0), c1(0.0), c2(0.0),
c3(0.0), alphaM(1.0), alphaD(alpha), alphaR(alpha), alphaP(alpha), Ut(0),
 ...
 HHT_TP::newStep(double _deltaT)
{
    ...
    if (U == 0) {
        opserr <<
            "HHT_TP::newStep() - domainChange() failed or hasn't been called\n";
        return -4;
    }
...
int HHT_TP::update(const Vector & deltaU)
{
    ...
    if (Ut == 0) {
        opserr <<
            "WARNING HHT_TP::update() - domainChange() failed or not called\n";
        return -2;
    }
    // check deltaU is of correct size
    if (deltaU.Size() != U->Size()) {
        opserr <<
            "WARNING HHT_TP::update() - Vectors of incompatible size ";
        opserr << " expecting " << U->
...
int HHT_TP::recvSelf(int cTag, Channel & theChannel,
