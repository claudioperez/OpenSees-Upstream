...
Houbolt::Houbolt ():TransientIntegrator (INTEGRATOR_TAGS_Houbolt),
step (0), dt (0.0),
...
int
Houbolt::newStep (double deltaT)
{
    // check the vectors have been created
    if (U == 0)
      {
          opserr <<
              "Houbolt::newStep() - domainChange() failed or hasn't been called\n";
          return -3;
      }
    ...
    if (step < 2)
      ...
      {                         // Houbolt
          ...
          (*Udot) = *Utm2;
          Udot->addVector (-1.0 / (3.0 * deltaT), *Utm1,
                           3.0 / (2.0 * deltaT));
          Udot->addVector (1.0, *Ut, -7.0 / (6.0 * deltaT));

          (*Udotdot) = *Utm2;
          Udotdot->addVector (-1.0 / (deltaT * deltaT), *Utm1,
                              4.0 / (deltaT * deltaT));
...
int
Houbolt::formEleTangent (FE_Element * theEle)
{
    theEle->zeroTangent ();
...
int
Houbolt::formNodTangent (DOF_Group * theDof)
{
    theDof->zeroTangent ();
...
int
Houbolt::update (const Vector & deltaU)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel == 0)
      {
    ...
    if (Ut == 0)
      {
          opserr <<
              "WARNING Houbolt::update() - domainChange() failed or not called\n";
          return -2;
      }
    ...
    if (deltaU.Size () != U->Size ())
      {
          opserr <<
              "WARNING Houbolt::update() - Vectors of incompatible size ";
...
int
Houbolt::sendSelf (int cTag, Channel & theChannel)
{
    // nothing to send
    return 0;
}
...
int
Houbolt::recvSelf (int cTag, Channel & theChannel,
...
void
Houbolt::Print (OPS_Stream & s, int flag)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel != 0)
      {
