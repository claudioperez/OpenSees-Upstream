...
void *OPS_HHTHSFixedNumIter_TP(void)
{
    ...
    if (argc != 1 && argc != 3 && argc != 4 && argc != 6) {
        opserr <<
            "WARNING - incorrect number of args want HHTHSFixedNumIter_TP $rhoInf <-polyOrder $O>\n";
        opserr <<
            "          or HHTHSFixedNumIter_TP $alphaI $alphaF $beta $gamma <-polyOrder $O>\n";
        return 0;
    }
    ...
    if (OPS_GetDouble(&numData, dData) != 0) {
        opserr <<
            "WARNING - invalid args want HHTHSFixedNumIter_TP $rhoInf <-polyOrder $O>\n";
        opserr <<
            "          or HHTHSFixedNumIter_TP $alphaI $alphaF $beta $gamma <-polyOrder $O>\n";
        return 0;
    }
    ...
    if (argc == 3 || argc == 6) {
        ...
        if (strcmp(argvLoc, "-polyOrder") == 0) {
            ...
            if (OPS_GetInt(&numData, &polyOrder) != 0) {
                opserr <<
                    "WARNING - invalid polyOrder want HHTHSFixedNumIter_TP $rhoInf <-polyOrder $O>\n";
                opserr <<
                    "          or HHTHSFixedNumIter_TP $alphaI $alphaF $beta $gamma <-polyOrder $O>\n";
            }
        }
    ...
    if (argc < 4)
        theIntegrator =
            new HHTHSFixedNumIter_TP(dData[0], polyOrder, updDomFlag);
    else
        theIntegrator =
            new HHTHSFixedNumIter_TP(dData[0], dData[1], dData[2],
...
HHTHSFixedNumIter_TP::HHTHSFixedNumIter_TP():TransientIntegrator(INTEGRATOR_TAGS_HHTHSFixedNumIter_TP),
...
HHTHSFixedNumIter_TP::HHTHSFixedNumIter_TP(double _rhoInf,
    ...
    (INTEGRATOR_TAGS_HHTHSFixedNumIter_TP),
...
alphaM(alphaI), alphaD(alphaF), alphaR(alphaF), alphaP(alphaF), Ut(0),
Utdot(0), Utdotdot(0), U(0), Udot(0), Udotdot(0), Utm1(0), Utm2(0),
...
HHTHSFixedNumIter_TP::HHTHSFixedNumIter_TP(double _alphaI, double _alphaF,
                                           ...
                                           bool
                                           upddomflag):TransientIntegrator
    (INTEGRATOR_TAGS_HHTHSFixedNumIter_TP), alphaI(_alphaI),
alphaF(_alphaF), beta(_beta), gamma(_gamma), polyOrder(polyorder),
updDomFlag(upddomflag), deltaT(0.0), c1(0.0), c2(0.0), c3(0.0), x(1.0),
alphaM(alphaI), alphaD(alphaF), alphaR(alphaF), alphaP(alphaF), Ut(0),
Utdot(0), Utdotdot(0), U(0), Udot(0), Udotdot(0), Utm1(0), Utm2(0),
scaledDeltaU(0), Put(0)
...
int
 HHTHSFixedNumIter_TP::newStep(double _deltaT)
{
    if (beta == 0 || gamma == 0) {
    ...
    if (U == 0) {
        opserr <<
            "HHTHSFixedNumIter_TP::newStep() - domainChange() failed or hasn't been called\n";
        return -4;
    }
...
int HHTHSFixedNumIter_TP::domainChanged()
{
    ...
    if (Ut == 0 || Ut->Size() != size) {
        ...
        if (Ut == 0 || Ut->Size() != size ||
            ...
            opserr <<
                "HHTHSFixedNumIter_TP::domainChanged() - ran out of memory\n";
...
int HHTHSFixedNumIter_TP::update(const Vector & deltaU)
{
    ...
    if (theModel == 0) {
        opserr <<
            "WARNING HHTHSFixedNumIter_TP::update() - no AnalysisModel set\n";
        return -1;
    }
    ConvergenceTest *theTest = this->getConvergenceTest();
    if (theTest == 0) {
        opserr <<
            "WARNING HHTHSFixedNumIter_TP::update() - no ConvergenceTest set\n";
        return -2;
    }
    // check domainChanged() has been called, i.e. Ut will not be zero
    if (Ut == 0) {
        opserr <<
            "WARNING HHTHSFixedNumIter_TP::update() - domainChange() failed or not called\n";
        return -3;
    }
    // check deltaU is of correct size
    if (deltaU.Size() != U->Size()) {
        opserr <<
            "WARNING HHTHSFixedNumIter_TP::update() - Vectors of incompatible size ";
        opserr << " expecting " << U->
    ...
    if (polyOrder == 1) {
        ...
        opserr <<
            "WARNING HHTHSFixedNumIter_TP::update() - polyOrder > 3 not supported\n";
        return -5;
    }
    ...
    if (theModel->updateDomain() < 0) {
        opserr <<
            "HHTHSFixedNumIter_TP::update() - failed to update the domain\n";
        return -6;
    }
...
const Vector & HHTHSFixedNumIter_TP::getVel()
{
    return *Udot;
}
...
int HHTHSFixedNumIter_TP::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (theChannel.sendVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING HHTHSFixedNumIter_TP::sendSelf() - could not send data\n";
        return -1;
    }
...
int HHTHSFixedNumIter_TP::recvSelf(int cTag, Channel & theChannel,
                                   FEM_ObjectBroker & theBroker)
{
    Vector data(6);
    if (theChannel.recvVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING HHTHSFixedNumIter_TP::recvSelf() - could not receive data\n";
        return -1;
    }
...
void HHTHSFixedNumIter_TP::Print(OPS_Stream & s, int flag)
{
    ...
    if (theModel != 0) {
        ...
        s << "HHTHSFixedNumIter_TP - currentTime: " << currentTime <<
            endln;
