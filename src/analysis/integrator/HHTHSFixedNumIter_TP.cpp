...
OPS_HHTHSFixedNumIter_TP (void)
{
    ...
    if (argc < 4)
        ...
        theIntegrator =
            new HHTHSFixedNumIter_TP (dData[0], dData[1], dData[2], dData[3],
...
int
HHTHSFixedNumIter_TP::revertToLastStep ()
{
...
int
HHTHSFixedNumIter_TP::formEleTangent (FE_Element * theEle)
{
    theEle->zeroTangent ();
...
int
HHTHSFixedNumIter_TP::formNodTangent (DOF_Group * theDof)
{
    theDof->zeroTangent ();
...
int
HHTHSFixedNumIter_TP::formEleResidual (FE_Element * theEle)
{
    theEle->zeroResidual ();
...
int
HHTHSFixedNumIter_TP::commit (void)
{
    // get a pointer to the LinearSOE and the AnalysisModel
    LinearSOE *theLinSOE = this->getLinearSOE ();
    AnalysisModel *theModel = this->getAnalysisModel ();
...
int
HHTHSFixedNumIter_TP::sendSelf (int cTag, Channel & theChannel)
{
    Vector data (6);
    data (0) = alphaI;
    data (1) = alphaF;
...
int
HHTHSFixedNumIter_TP::recvSelf (int cTag, Channel & theChannel,
...
void
HHTHSFixedNumIter_TP::Print (OPS_Stream & s, int flag)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel != 0)
      {
