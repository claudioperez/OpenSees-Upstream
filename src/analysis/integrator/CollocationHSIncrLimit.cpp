...
void *OPS_CollocationHSIncrLimit(void)
{
    ...
    if (argc != 2 && argc != 4 && argc != 6) {
        opserr <<
            "WARNING - incorrect number of args want CollocationHSIncrLimit $theta $limit <-normType $T>\n";
        opserr <<
            "          or CollocationHSIncrLimit $theta $beta $gamma $limit <-normType $T>\n";
        return 0;
    }
    ...
    if (OPS_GetDouble(&numData, dData) != 0) {
        opserr <<
            "WARNING - invalid args want CollocationHSIncrLimit $theta $limit <-normType $T>\n";
        opserr <<
            "          or CollocationHSIncrLimit $theta $beta $gamma $limit <-normType $T>\n";
        return 0;
    }
    ...
    if (numData + 2 == argc) {
        ...
        if (strcmp(argvLoc, "-normType") == 0) {
            ...
            if (OPS_GetInt(&numData2, &normType) != 0) {
                opserr <<
                    "WARNING - invalid normType want CollocationHSIncrLimit $theta $limit <-normType $T>\n";
                opserr <<
                    "          or CollocationHSIncrLimit $theta $beta $gamma $limit <-normType $T>\n";
            }
        }
    ...
    if (numData == 2)
        theIntegrator =
            new CollocationHSIncrLimit(dData[0], dData[1], normType);
    else if (numData == 4)
        theIntegrator =
            new CollocationHSIncrLimit(dData[0], dData[1], dData[2],
                                       dData[3], normType);

    if (theIntegrator == 0)
        opserr <<
            "WARNING - out of memory creating CollocationHSIncrLimit integrator\n";

    return theIntegrator;
}


CollocationHSIncrLimit::CollocationHSIncrLimit():TransientIntegrator(INTEGRATOR_TAGS_CollocationHSIncrLimit),
...
CollocationHSIncrLimit::CollocationHSIncrLimit(double _theta,
    ...
    (INTEGRATOR_TAGS_CollocationHSIncrLimit), theta(_theta), beta(0.0),
gamma(0.5), limit(_limit), normType(normtype), deltaT(0.0), c1(0.0),
c2(0.0), c3(0.0), Ut(0), Utdot(0), Utdotdot(0), U(0), Udot(0), Udotdot(0),
scaledDeltaU(0)
{
...
CollocationHSIncrLimit::CollocationHSIncrLimit(double _theta,
                                               ...
                                               int
                                               normtype):TransientIntegrator
    (INTEGRATOR_TAGS_CollocationHSIncrLimit), theta(_theta), beta(_beta),
gamma(_gamma), limit(_limit), normType(normtype), deltaT(0.0), c1(0.0),
c2(0.0), c3(0.0), Ut(0), Utdot(0), Utdotdot(0), U(0), Udot(0), Udotdot(0),
scaledDeltaU(0)
{
...
int
 CollocationHSIncrLimit::newStep(double _deltaT)
{
    if (theta <= 0.0) {
        opserr <<
            "CollocationHSIncrLimit::newStep() - error in variable\n";
    ...
    if (U == 0) {
        opserr <<
            "CollocationHSIncrLimit::newStep() - domainChange() failed or hasn't been called\n";
        return -3;
    }
...
int CollocationHSIncrLimit::domainChanged()
{
    ...
    if (Ut == 0 || Ut->Size() != size) {
        ...
        if (Ut == 0 || Ut->Size() != size ||
            ...
            opserr <<
                "CollocationHSIncrLimit::domainChanged() - ran out of memory\n";
...
int CollocationHSIncrLimit::update(const Vector & deltaU)
{
    ...
    if (theModel == 0) {
        opserr <<
            "WARNING CollocationHSIncrLimit::update() - no AnalysisModel set\n";
        return -1;
    }
    // check domainChanged() has been called, i.e. Ut will not be zero
    if (Ut == 0) {
        opserr <<
            "WARNING CollocationHSIncrLimit::update() - domainChange() failed or not called\n";
        return -2;
    }
    // check deltaU is of correct size
    if (deltaU.Size() != U->Size()) {
        opserr <<
            "WARNING CollocationHSIncrLimit::update() - Vectors of incompatible size ";
        opserr << " expecting " << U->
    ...
    if (theModel->updateDomain() < 0) {
        opserr <<
            "CollocationHSIncrLimit::update() - failed to update the domain\n";
        return -4;
    }
...
int CollocationHSIncrLimit::commit(void)
{
    ...
    if (theModel == 0) {
        opserr <<
            "WARNING CollocationHSIncrLimit::commit() - no AnalysisModel set\n";
        return -1;
    }
...
const Vector & CollocationHSIncrLimit::getVel()
{
    return *Udot;
}
...
int CollocationHSIncrLimit::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (theChannel.sendVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING CollocationHSIncrLimit::sendSelf() - failed to send the data\n";
        return -1;
    }
...
int CollocationHSIncrLimit::recvSelf(int cTag, Channel & theChannel,
                                     FEM_ObjectBroker & theBroker)
{
    Vector data(5);
    if (theChannel.recvVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING CollocationHSIncrLimit::recvSelf() - could not receive data\n";
        return -1;
    }
...
void CollocationHSIncrLimit::Print(OPS_Stream & s, int flag)
{
    ...
    if (theModel != 0) {
        ...
        s << "CollocationHSIncrLimit - currentTime: " << currentTime <<
            endln;
        s << "  theta: " << theta << endln;
