...
void *OPS_HHT(void)
{
    ...
    if (argc != 1 && argc != 3) {
        opserr <<
            "WARNING - incorrect number of args want HHT $alpha <$gamma $beta>\n";
        return 0;
    }
    ...
    if (OPS_GetDouble(&argc, dData) != 0) {
        opserr <<
            "WARNING - invalid args want HHT $alpha <$gamma $beta>\n";
        return 0;
    }
...
HHT::HHT():TransientIntegrator(INTEGRATOR_TAGS_HHT),
...
HHT::HHT(double _alpha):TransientIntegrator(INTEGRATOR_TAGS_HHT),
alpha(_alpha),
...
HHT::HHT(double _alpha, double _beta,
         double _gamma):TransientIntegrator(INTEGRATOR_TAGS_HHT),
 ...
 HHT::newStep(double _deltaT)
{
    ...
    if (U == 0) {
        opserr <<
            "HHT::newStep() - domainChange() failed or hasn't been called\n";
        return -3;
    }
...
int HHT::update(const Vector & deltaU)
{
    ...
    if (Ut == 0) {
        opserr <<
            "WARNING HHT::update() - domainChange() failed or not called\n";
        return -2;
    }
...
int HHT::recvSelf(int cTag, Channel & theChannel,
