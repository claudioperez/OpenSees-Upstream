...
Newmark1::Newmark1():TransientIntegrator(INTEGRATOR_TAGS_Newmark1),
gamma(0), beta(0),
...
Newmark1::Newmark1(double theGamma, double theBeta,
                   bool
                   dispFlag):TransientIntegrator(INTEGRATOR_TAGS_Newmark1),
gamma(theGamma), beta(theBeta), alphaM(0.0), betaK(0.0), betaKi(0.0),
betaKc(0.0), c1(0.0), c2(0.0), c3(0.0), c4(0.0), Up(0), Updot(0), U(0),
Udot(0), Udotdot(0)
{
...
Newmark1::Newmark1(double theGamma, double theBeta,
                   ...
                   double
                   betakc):TransientIntegrator(INTEGRATOR_TAGS_Newmark1),
gamma(theGamma), beta(theBeta), alphaM(alpham), betaK(betak),
betaKi(betaki), betaKc(betakc), c1(0.0), c2(0.0), c3(0.0), c4(0.0), Up(0),
Updot(0), U(0), Udot(0), Udotdot(0)
{
...
int Newmark1::newStep(double deltaT)
{
    ...
    if (U == 0) {
        opserr <<
            "Newton::newStep() - domainChange() failed or hasn't been called\n";
        return -3;
    }
...
int Newmark1::revertToLastStep()
{
    return this->domainChanged();
}

int Newmark1::formEleTangent(FE_Element * theEle)
{
    theEle->zeroTangent();
...
int Newmark1::formNodTangent(DOF_Group * theDof)
{
    theDof->zeroTangent();
...
int Newmark1::update(const Vector & deltaU)
{
    AnalysisModel *theModel = this->getAnalysisModel();
    if (theModel == 0) {
    ...
    if (U == 0) {
        opserr <<
            "WARNING Newmark1::update() - domainChange() failed or not called\n";
        return -2;
    }
    // check deltaU is of correct size
    if (deltaU.Size() != U->Size()) {
        opserr <<
            "WARNING Newmark1::update() - Vectors of incompatible size ";
        opserr << " expecting " << U->
...
const Vector & Newmark1::getVel()
{
    return *Udot;
}

int Newmark1::sendSelf(int cTag, Channel & theChannel)
{
    Vector data(7);
    data(0) = gamma;
    data(1) = beta;
...
int Newmark1::recvSelf(int cTag, Channel & theChannel,
                       FEM_ObjectBroker & theBroker)
{
    Vector data(7);
    if (theChannel.recvVector(this->getDbTag(), cTag, data) < 0) {
        opserr <<
            "WARNING Newmark1::recvSelf() - could not receive data\n";
        gamma = 0.5;
        beta = 0.25;
        return -1;
    }
...
void Newmark1::Print(OPS_Stream & s, int flag)
{
    AnalysisModel *theModel = this->getAnalysisModel();
    if (theModel != 0) {
