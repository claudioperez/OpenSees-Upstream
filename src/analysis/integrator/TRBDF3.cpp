...
TRBDF3::TRBDF3 ():TransientIntegrator (INTEGRATOR_TAGS_TRBDF3),
step (0), dt (0.0),
...
int
TRBDF3::newStep (double deltaT)
{
    // check the vectors have been created
    if (U == 0)
      {
          opserr <<
              "TRBDF3::newStep() - domainChange() failed or hasn't been called\n";
          return -3;
      }
    ...
    if (step == 0)
      ...
      {                         // Houbolt
          ...
          (*Udotdot) = *Utm2;
          Udotdot->addVector (-1 / (deltaT * deltaT), *Utm1,
                              4 / (deltaT * deltaT));
...
int
TRBDF3::formNodTangent (DOF_Group * theDof)
{
    theDof->zeroTangent ();
...
int
TRBDF3::update (const Vector & deltaU)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel == 0)
      {
    ...
    if (Ut == 0)
      {
          opserr <<
              "WARNING TRBDF3::update() - domainChange() failed or not called\n";
          return -2;
      }
    ...
    if (deltaU.Size () != U->Size ())
      {
          opserr <<
              "WARNING TRBDF3::update() - Vectors of incompatible size ";
...
int
TRBDF3::sendSelf (int cTag, Channel & theChannel)
{
    // nothing to send
    return 0;
}
...
int
TRBDF3::recvSelf (int cTag, Channel & theChannel,
...
void
TRBDF3::Print (OPS_Stream & s, int flag)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel != 0)
      {
