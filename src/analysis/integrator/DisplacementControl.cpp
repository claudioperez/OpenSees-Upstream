...
void *OPS_DisplacementControlIntegrator()
{
    ...
    if (theNode == 0) {
        opserr <<
            "WARNING integrator DisplacementControl node dof dU : Node does not exist\n";
        return 0;
    }
    ...
    if (iData[1] <= 0 || iData[1] > numDOF) {
        opserr <<
            "WARNING integrator DisplacementControl node dof dU : invalid dof given\n";
        return 0;
    }
...
DisplacementControl::DisplacementControl(int node, int dof,
                                         ...
                                         int
                                         tang):StaticIntegrator
    (INTEGRATOR_TAGS_DisplacementControl), theNode(node), theDof(dof),
theIncrement(increment), theDomain(domain), theDofID(-1), deltaUhat(0),
deltaUbar(0), deltaU(0), deltaUstep(0), dUhatdh(0), phat(0),
deltaLambdaStep(0.0), currentLambda(0.0), dLambdaStepDh(0.0), dUIJdh(0),
Dlambdadh(0.0), specNumIncrStep(numIncr), numIncrLastStep(numIncr),
minIncrement(min), maxIncrement(max), sensitivityFlag(0), Residual(0),
dlambdadh(0.0), dLambda(0.0), sensU(0), d_deltaU_dh(0), Residual2(0),
gradNumber(0), dLAMBDAdh(0), dphatdh(0)
{
    tangFlag = tang;
...
int DisplacementControl::domainChanged(void)
{
    ...
    if (haveLoad == 0) {
        opserr <<
            "WARNING DisplacementControl::domainChanged() - zero reference load";
        return -1;
    }
...
int DisplacementControl::sendSelf(int cTag, Channel & theChannel)
{
    // TO FINISH
    return 0;
}
...
int DisplacementControl::recvSelf(int cTag,
                                  Channel & theChannel,
...
void DisplacementControl::Print(OPS_Stream & s, int flag)
{
    // TO FINISH    
}
...
Vector *DisplacementControl::formTangDispSensitivity(Vector * dUhatdh,
                                                     int gradNumber)
{
...
double DisplacementControl::formdLambdaDh(int gradNumber)
{
...
double DisplacementControl::getLambdaSensitivity(int gradNumber)
{
...
int DisplacementControl::formEleResidual(FE_Element * theEle)
{
...
int DisplacementControl::formIndependentSensitivityRHS()
{
    return 0;
}
...
int DisplacementControl::formSensitivityRHS(int passedGradNumber)
{
    sensitivityFlag = 1;
...
int DisplacementControl::saveSensitivity(const Vector & v, int gradNum,
                                         int numGrads)
{
    AnalysisModel *theAnalysisModel = this->getAnalysisModel();
...
int DisplacementControl::saveLambdaSensitivity(double dlambdadh,
...
int DisplacementControl::commitSensitivity(int gradNum, int numGrads)
{
...
bool DisplacementControl::computeSensitivityAtEachIteration()
{
...
int DisplacementControl::computeSensitivities(void)
{
    LinearSOE *theSOE = this->getLinearSOE();
