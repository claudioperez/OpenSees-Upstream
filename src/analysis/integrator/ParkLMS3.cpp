...
ParkLMS3::ParkLMS3 ():TransientIntegrator (INTEGRATOR_TAGS_ParkLMS3),
step (0), dt (0.0),
...
int
ParkLMS3::newStep (double deltaT)
{
    // check the vectors have been created
    if (U == 0)
      {
          opserr <<
              "ParkLMS3::newStep() - domainChange() failed or hasn't been called\n";
          return -3;
      }
    ...
    if (step < 2)
      ...
      {                         // ParkLMS3
          ...
          (*Udotdot) = *Utm2;
          Udotdot->addVector (-5.0 / (18.0 * deltaT * deltaT), *Utm1,
                              5.0 / (3.0 * deltaT * deltaT));
...
int
ParkLMS3::formEleTangent (FE_Element * theEle)
{
    theEle->zeroTangent ();
...
int
ParkLMS3::formNodTangent (DOF_Group * theDof)
{
    theDof->zeroTangent ();
...
int
ParkLMS3::update (const Vector & deltaU)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel == 0)
      {
    ...
    if (Ut == 0)
      {
          opserr <<
              "WARNING ParkLMS3::update() - domainChange() failed or not called\n";
          return -2;
      }
    ...
    if (deltaU.Size () != U->Size ())
      {
          opserr <<
              "WARNING ParkLMS3::update() - Vectors of incompatible size ";
...
int
ParkLMS3::sendSelf (int cTag, Channel & theChannel)
{
    // nothing to send
    return 0;
}
...
int
ParkLMS3::recvSelf (int cTag, Channel & theChannel,
...
void
ParkLMS3::Print (OPS_Stream & s, int flag)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel != 0)
      {
