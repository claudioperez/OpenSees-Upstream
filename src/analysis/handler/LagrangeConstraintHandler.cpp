...
LagrangeConstraintHandler::LagrangeConstraintHandler(double sp,
                                                     double
                                                     mp):ConstraintHandler
    (HANDLER_TAG_LagrangeConstraintHandler), alphaSP(sp), alphaMP(mp)
{
            ...
            if (nodPtr != 0) {
                ...
                for (int j = 0; j < id.Size(); j++)
                    if (id(j) == -2) {
                        ...
                        opserr <<
                            "WARNING LagrangeConstraintHandler::handle() ";
...
void LagrangeConstraintHandler::clearAll(void)
{
...
int LagrangeConstraintHandler::sendSelf(int cTag, Channel & theChannel)
{
    Vector data(2);
    int result = 0;
    data(0) = alphaSP;
    ...
    if (result != 0)
        opserr <<
            "LagrangeConstraintHandler::sendSelf() - error sending Vector\n";
    return result;
}

int LagrangeConstraintHandler::recvSelf(int cTag,
                                        Channel & theChannel,
