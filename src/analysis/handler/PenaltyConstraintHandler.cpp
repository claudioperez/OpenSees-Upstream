...
PenaltyConstraintHandler::PenaltyConstraintHandler(double sp,
                                                   double
                                                   mp):ConstraintHandler
    (HANDLER_TAG_PenaltyConstraintHandler), alphaSP(sp), alphaMP(mp)
{
 ...
 PenaltyConstraintHandler::handle(const ID * nodesLast)
{
            ...
            if (nodPtr != 0) {
                ...
                for (int j = 0; j < id.Size(); j++)
                    if (id(j) == -2) {
                        ...
                        opserr <<
                            "WARNING PenaltyConstraintHandler::handle() ";
    ...
    while ((spPtr = theSPss()) != 0) {
        if ((fePtr =
             new PenaltySP_FE(numFeEle, *theDomain, *spPtr, alphaSP)) == 0)
        {
            opserr << "WARNING PenaltyConstraintHandler::handle()";
            opserr << " - ran out of memory";
    ...
    while ((mpPtr = theMPs()) != 0) {
        if ((fePtr =
             new PenaltyMP_FE(numFeEle, *theDomain, *mpPtr, alphaMP)) == 0)
        {
            opserr << "WARNING PenaltyConstraintHandler::handle()";
            opserr << " - ran out of memory";
...
void PenaltyConstraintHandler::clearAll(void)
{
...
int PenaltyConstraintHandler::sendSelf(int cTag, Channel & theChannel)
{
    Vector data(2);
    int result = 0;
    data(0) = alphaSP;
    ...
    if (result != 0)
        opserr <<
            "PenaltyConstraintHandler::sendSelf() - error sending Vector\n";
    return result;
}

int PenaltyConstraintHandler::recvSelf(int cTag,
                                       Channel & theChannel,
                                       FEM_ObjectBroker & theBroker)
{
    ...
    if (result != 0)
        opserr <<
            "PenaltyConstraintHandler::recvSelf() - error receiving Vector\n";
    return result;
}
