...
PlainHandler::PlainHandler():ConstraintHandler(HANDLER_TAG_PlainHandler)
{
 ...
 PlainHandler::handle(const ID * nodesLast)
{
    ...
    while ((nodPtr = theNod()) != 0) {
        ...
        std::multimap < int, SP_Constraint * >::iterator first =
            allSPs.lower_bound(nodeID);
        std::multimap < int, SP_Constraint * >::iterator last =
            allSPs.upper_bound(nodeID);
        for (std::multimap < int, SP_Constraint * >::iterator it = first;
             it != last; it++) {
            spPtr = it->second;
            const ID & id = dofPtr->getID();
            ...
            if (id(dof) == -2) {
                ...
                opserr << "WARNING PlainHandler::handle() - ";
                opserr << " multiple single pointconstraints at DOF " <<
                    dof;
        ...
        while ((mpPtr = theMPs()) != 0)
            if (mpPtr->getNodeConstrained() == nodeID) {
                ...
                if (numRows != numCols) {
                    opserr << "WARNING PlainHandler::handle() - ";
                    opserr <<
                        " constraint matrix not diagonal, ignoring constraint";
                    ...
                    if (ok != 0) {
                        opserr << "WARNING PlainHandler::handle() - ";
                        opserr <<
                            " constraint matrix not identity, ignoring constraint";
                        opserr << " for node " << nodeID << endln;
                        opserr << " non-varyng assumed\n";
                    } else {
                        ...
                        for (int i = 0; i < dofs.Size(); i++) {
                            ...
                            if (id(dof) == -2) {
                                ...
                                opserr <<
                                    "WARNING PlainHandler::handle() - ";
                                opserr << " constraint at dof " <<
                                    dof <<
                                    " already specified for constrained node";
                                opserr <<
                                    " in MP_Constraint at node "
                                    << nodeID << endln;
                            }
        ...
        if (elePtr->isSubdomain() == true) {
            ...
            if (theSub->doesIndependentAnalysis() == false) {
                ...
                if ((fePtr = new FE_Element(numFe++, elePtr)) == 0) {
                    opserr <<
                        "WARNING PlainHandler::handle() - ran out of memory";
...
void PlainHandler::clearAll(void)
{
...
int PlainHandler::sendSelf(int cTag, Channel & theChannel)
{
    return 0;
}

int PlainHandler::recvSelf(int cTag,
                           Channel & theChannel,
