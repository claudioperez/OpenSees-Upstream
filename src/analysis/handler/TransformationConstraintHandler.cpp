...
TransformationConstraintHandler::TransformationConstraintHandler():ConstraintHandler(HANDLER_TAG_TransformationConstraintHandler),
 ...
 TransformationConstraintHandler::handle(const ID * nodesLast)
{
    ...
    if (numMPConstraints != 0) {
        ...
        if (mps == 0) {
            opserr <<
                "WARNING TransformationConstraintHandler::handle() - ";
    ...
    if (numSPConstraints != 0) {
        ...
        SP_ConstraintIter & theSPs =
            theDomain->getDomainAndLoadPatternSPs();
        SP_Constraint *theSP;
        int index = 0;
    ...
    while ((nodPtr = theNod()) != 0) {
        ...
        if (dofPtr == 0)
            opserr <<
                "TransformationConstraintHandler::handle() - error in logic\n";
        ...
        if (elePtr->isSubdomain() == true) {
            ...
            if (theSub->doesIndependentAnalysis() == false) {
                ...
                if (transformedEle.getLocation(tag) < 0) {
                    ...
                    if ((fePtr =
                         new TransformationFE(numFeEle, elePtr)) == 0) {
                        opserr <<
                            "WARNING TransformationConstraintHandler::handle()";
                        opserr << " - ran out of memory";
                        opserr << " creating TransformationFE " <<
                            elePtr->getTag() << endln;
                        if (mps != 0)
                            delete[]mps;
                        if (sps != 0)
            ...
            if (transformedEle.getLocation(tag) < 0) {
                if ((fePtr = new FE_Element(numFeEle, elePtr)) == 0) {
                    opserr <<
                        "WARNING TransformationConstraintHandler::handle()";
                    opserr << " - ran out of memory";
                    opserr << " creating FE_Element " << elePtr->getTag()
                        << endln;
                    if (mps != 0)
                        delete[]mps;
                    if (sps != 0)
...
void TransformationConstraintHandler::clearAll(void)
{
    // delete the arrays
    if (theFEs != 0)
        delete[]theFEs;
...
int TransformationConstraintHandler::sendSelf(int cTag,
                                              Channel & theChannel)
{
    return 0;
}

int TransformationConstraintHandler::recvSelf(int cTag,
                                              Channel & theChannel,
...
int TransformationConstraintHandler::applyLoad(void)
{
    return this->enforceSPs();
}

int TransformationConstraintHandler::enforceSPs(void)
{
...
int TransformationConstraintHandler::doneNumberingDOF(void)
{
