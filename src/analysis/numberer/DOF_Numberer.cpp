...
DOF_Numberer::DOF_Numberer(int clsTag):MovableObject(clsTag),
theAnalysisModel(0), theGraphNumberer(0)
{

}

DOF_Numberer::DOF_Numberer(GraphNumberer & aGraphNumberer):MovableObject(NUMBERER_TAG_DOF_Numberer),
theAnalysisModel(0),
...
DOF_Numberer::DOF_Numberer():MovableObject(NUMBERER_TAG_DOF_Numberer),
theAnalysisModel(0), theGraphNumberer(0)
{
...
int DOF_Numberer::numberDOF(int lastDOF_Group)
{
    ...
    const ID & orderedRefs =
        theGraphNumberer->number(theAnalysisModel->getDOFGroupGraph(),
                                 lastDOF_Group);
...
int DOF_Numberer::numberDOF(ID & lastDOFs)
{
    // check we have a model and a numberer
    Domain *theDomain = 0;
    if (theAnalysisModel != 0)
...
int DOF_Numberer::sendSelf(int cTag, Channel & theChannel)
{
    ID data(2);
...
int DOF_Numberer::recvSelf(int cTag, Channel & theChannel,
                           FEM_ObjectBroker & theBroker)
{
    ...
    if (data(0) != -1) {
        ...
        if (theGraphNumberer != 0) {
            ...
            opserr <<
                "DOF_Numberer::recvSelf() - failed to get GraphNumberer\n";
            return -1;
        }
    }
...
AnalysisModel *DOF_Numberer::getAnalysisModelPtr(void) const
{
    return theAnalysisModel;
}


GraphNumberer *DOF_Numberer::getGraphNumbererPtr(void) const
{
