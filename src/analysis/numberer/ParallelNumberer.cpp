...
ParallelNumberer::ParallelNumberer(int dTag, int numSub,
                                   Channel **
                                   theC):DOF_Numberer
    (NUMBERER_TAG_ParallelNumberer), processID(dTag), numChannels(numSub)
{
...
ParallelNumberer::ParallelNumberer():DOF_Numberer(NUMBERER_TAG_ParallelNumberer), theNumberer(0),
processID(0), numChannels(0),
theChannels(0)
{
...
ParallelNumberer::ParallelNumberer(GraphNumberer & theGraphNumberer):DOF_Numberer(NUMBERER_TAG_ParallelNumberer),
theNumberer(&theGraphNumberer), processID(0), numChannels(0),
theChannels(0)
{
...
int ParallelNumberer::setChannels(int nChannels, Channel ** theC)
{
...
int ParallelNumberer::numberDOF(int lastDOF)
{
    int result = 0;
    ...
    if (processID != 0) {
        ...
        for (int i = 0; i < numVertex; i++) {
            ...
            if (dofPtr == 0) {
                ...
                opserr << "DOF_Group " << dofTag <<
                    "not in AnalysisModel!\n";
                result = -4;
            } else {
    ...
    // id containing dof tags & start id's.
        ...
        for (int j = 0; j < numChannels; j++) {
            ...
            this->mergeSubGraph(theGraph, *theSubGraph, vertexTags,
                                vertexRefs, *theSubdomainIDs[j]);
            ...
            *theOrderedRefs = theNumberer->number(theGraph, lastDOF);
            ...
            for (int l = 0; l < numChannels; l++) {
                ...
                for (int i = 0; i < numVertexSubdomain; i++) {
                    int vertexTagMerged =
                        theSubdomain(i + numVertexSubdomain);
...
int ParallelNumberer::mergeSubGraph(Graph & theGraph, Graph & theSubGraph,
                                    ID & vertexTags, ID & vertexRefs,
                                    ID & theSubdomainMap)
{
    ...
    while ((subVertexPtr = theSubGraphIter2()) != 0) {
        ...
        for (int i = 0; i < adjacency.Size(); i++) {
            ...
            int vertexTagMergedAdjacent =
                theSubdomainMap[loc + numVertexSub];
...
int ParallelNumberer::sendSelf(int cTag, Channel & theChannel)
{
    int sendID = 0;
    ...
    if (processID == 0) {
        ...
        if (found == false) {
            ...
            if (nextNumChannels == 0) {
                opserr <<
                    "ParalellNumberer::sendSelf() - failed to allocate channel array of size: "
    ...
    if (res < 0) {
        opserr <<
            "WARNING DistributedSparseGenColLinSOE::sendSelf() - failed to send data\n";
        return -1;
    }
...
int ParallelNumberer::recvSelf(int cTag,
                               Channel & theChannel,
                               FEM_ObjectBroker & theBroker)
{
    ID idData(1);
...
int ParallelNumberer::numberDOF(ID & lastDOFs)
{
    int result = 0;
