...
ParallelNumberer::ParallelNumberer (GraphNumberer & theGraphNumberer):DOF_Numberer (NUMBERER_TAG_ParallelNumberer), theNumberer (&theGraphNumberer),
processID (0), numChannels (0),
theChannels (0)
{
          ...
          for (int j = 0; j < numChannels; j++)
            {
                ...
                theSubdomainIDs[j] =
                    new ID (theSubGraph->getNumVertex () * 2);
          ...
          for (int k = 0; k < numChannels; k++)
            {
                ...
                for (int i = 0; i < numVertexSubdomain; i++)
                  {
                      int vertexTagMerged =
                          theSubdomain[numVertexSubdomain + i];
                      Vertex *vertexPtr =
                          theGraph.getVertexPtr (vertexTagMerged);
...
int
ParallelNumberer::mergeSubGraph (Graph & theGraph, Graph & theSubGraph,
          ...
          if (loc < 0)
            {
                ...
                Vertex *newVertex =
                    new Vertex (vertexTagMerged, vertexTagRef,
