...
Graph & AnalysisModel::getDOFGraph (void)
{
    if (myDOFGraph == 0)
      {
          int
              numVertex = this->getNumDOF_Groups ();

          //    myDOFGraph = new Graph(numVertex);
          MapOfTaggedObjects *
              graphStorage = new MapOfTaggedObjects ();
                ...
                for (int i = 0; i < size; i++)
                  {
                      ...
                      if (dofTag >= START_EQN_NUM)
                        {
                            Vertex *
                                vertexPtr = myDOFGraph->getVertexPtr (dofTag);
                            if (vertexPtr == 0)
                              {
                                  Vertex *
                                      vertexPtr = new Vertex (dofTag, dofTag);
                                  if (vertexPtr == 0)
                                    {
                                        opserr <<
                                            "WARNING AnalysisModel::getDOFGraph";
                                        opserr <<
                                  ...
                                  if (myDOFGraph->
                                      addVertex (vertexPtr, false) == false)
                                    {
                                        opserr <<
    ...
    if (myGroupGraph == 0)
      {
          ...
          while ((dofPtr = dofIter2 ()) != 0)
            {
                int
                    DOF_GroupTag = dofPtr->getTag ();
                int
                    DOF_GroupNodeTag = dofPtr->getNodeTag ();
                int
                    numDOF = dofPtr->getNumFreeDOF ();
                Vertex *
                    vertexPtr =
...
void
AnalysisModel::setRayleighDampingFactors (double alphaM, double betaK,
