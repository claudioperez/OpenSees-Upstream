...
AnalysisModel::AnalysisModel(int theClassTag):MovableObject(theClassTag),
myDomain(0),
myHandler(0),
myDOFGraph(0), myGroupGraph(0), numFE_Ele(0), numDOF_Grp(0), numEqn(0)
{
...
AnalysisModel::AnalysisModel():MovableObject(AnaMODEL_TAGS_AnalysisModel),
...
AnalysisModel::AnalysisModel(TaggedObjectStorage & theFes, TaggedObjectStorage & theDofs):MovableObject(AnaMODEL_TAGS_AnalysisModel),
myDomain(0), myHandler(0),
myDOFGraph(0), myGroupGraph(0), numFE_Ele(0), numDOF_Grp(0), numEqn(0)
{
    theFEs = &theFes;
...
bool AnalysisModel::addFE_Element(FE_Element * theElement)
{
    ...
    if (other != 0) {
        opserr << "AnalysisModel::addFE_Element - element with tag " << tag
...
bool AnalysisModel::addDOF_Group(DOF_Group * theGroup)
{
    ...
    if (other != 0) {
        opserr << "AnalysisModel::addDOF_Group - group with tag " << tag <<
            "already exists in model\n";
        return false;
    }
    // add the element to the container object for the elements
...
void AnalysisModel::clearAll(void)
{
...
void AnalysisModel::clearDOFGraph(void)
{
    if (myDOFGraph != 0)
        delete myDOFGraph;
...
void AnalysisModel::clearDOFGroupGraph(void)
{
    if (myGroupGraph != 0)
        delete myGroupGraph;
...
int AnalysisModel::getNumDOF_Groups(void) const
{
    return numDOF_Grp;
}


DOF_Group *AnalysisModel::getDOF_GroupPtr(int tag)
{
    TaggedObject *other = theDOFs->getComponentPtr(tag);
    if (other == 0) {
        return 0;
...
void AnalysisModel::setNumEqn(int theNumEqn)
{
    numEqn = theNumEqn;
}

int AnalysisModel::getNumEqn(void) const
{
    return numEqn;
}


Graph & AnalysisModel::getDOFGraph(void)
{
    if (myDOFGraph == 0) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (dofTag >= START_EQN_NUM) {
                    ...
                    if (vertexPtr == 0) {
                        ...
                        if (myDOFGraph->addVertex(vertexPtr, false) ==
                            false) {
                            opserr <<
                                "WARNING AnalysisModel::getDOFGraph - error adding vertex\n";
                            return *myDOFGraph;
                        }
                    }
                }
...
Graph & AnalysisModel::getDOFGroupGraph(void)
{
    if (myGroupGraph == 0) {
        int numVertex = this->getNumDOF_Groups();
        ...
        while ((dofPtr = dofIter2()) != 0) {
            ...
            Vertex *vertexPtr =
                new Vertex(DOF_GroupTag, DOF_GroupNodeTag, 0, numDOF);
...
void AnalysisModel::setDisp(const Vector & disp)
{
    DOF_GrpIter & theDOFGrps = this->getDOFs();
    DOF_Group *dofPtr;
...
void AnalysisModel::setVel(const Vector & vel)
{
    DOF_GrpIter & theDOFGrps = this->getDOFs();
    DOF_Group *dofPtr;
...
void AnalysisModel::setAccel(const Vector & accel)
{
    DOF_GrpIter & theDOFGrps = this->getDOFs();
    DOF_Group *dofPtr;
...
void AnalysisModel::incrDisp(const Vector & disp)
{
    DOF_GrpIter & theDOFGrps = this->getDOFs();
    DOF_Group *dofPtr;
...
void AnalysisModel::incrVel(const Vector & vel)
{
    DOF_GrpIter & theDOFGrps = this->getDOFs();
    DOF_Group *dofPtr;
...
void AnalysisModel::incrAccel(const Vector & accel)
{
    DOF_GrpIter & theDOFGrps = this->getDOFs();
    DOF_Group *dofPtr;
...
void AnalysisModel::setNumEigenvectors(int numEigenvectors)
{
    Node *theNode;
...
void AnalysisModel::setEigenvalues(const Vector & eigenvalues)
{
    myDomain->setEigenvalues(eigenvalues);
}

const Vector & AnalysisModel::getEigenvalues(void)
{
...
const Vector *AnalysisModel::getModalDampingFactors(void)
{
    return myDomain->getModalDampingFactors();
}

bool AnalysisModel::inclModalDampingMatrix(void)
{
    return myDomain->inclModalDampingMatrix();
}

void AnalysisModel::setEigenvector(int mode, const Vector & eigenvalue)
{
    DOF_GrpIter & theDOFGrps = this->getDOFs();
    DOF_Group *dofPtr;
...
void AnalysisModel::applyLoadDomain(double pseudoTime)
{
    // check to see there is a Domain linked to the Model

    if (myDomain == 0) {
        opserr <<
            "WARNING: AnalysisModel::applyLoadDomain. No Domain linked.\n";
        return;
    }
    // invoke the method
...
int AnalysisModel::updateDomain(void)
{
    // check to see there is a Domain linked to the Model

    if (myDomain == 0) {
        opserr <<
            "WARNING: AnalysisModel::updateDomain. No Domain linked.\n";
        return -1;
    }
    // invoke the method
...
int AnalysisModel::updateDomain(double newTime, double dT)
{
...
int AnalysisModel::analysisStep(double dT)
{
    // check to see there is a Domain linked to the Model
...
int AnalysisModel::eigenAnalysis(int numMode, bool generalized,
                                 bool findSmallest)
{
    // check to see there is a Domain linked to the Model
...
int AnalysisModel::commitDomain(void)
{
    // check to see there is a Domain linked to the Model
    if (myDomain == 0) {
        opserr <<
            "WARNING: AnalysisModel::commitDomain. No Domain linked.\n";
        return -1;
    }
    // invoke the method
...
int AnalysisModel::revertDomainToLastCommit(void)
{
    // check to see there is a Domain linked to the Model
...
double AnalysisModel::getCurrentDomainTime(void)
{
    // check to see there is a Domain linked to the Model
...
void AnalysisModel::setCurrentDomainTime(double newTime)
{
    if (myDomain == 0) {
        opserr << "WARNING: AnalysisModel::getCurrentDomainTime.";
        opserr << " No Domain linked.\n";
...
void AnalysisModel::setRayleighDampingFactors(double alphaM, double betaK,
...
Domain *AnalysisModel::getDomainPtr(void) const
{
    return myDomain;
}


int AnalysisModel::sendSelf(int cTag, Channel & theChannel)
{
    return 0;
}


int AnalysisModel::recvSelf(int cTag, Channel & theChannel,
