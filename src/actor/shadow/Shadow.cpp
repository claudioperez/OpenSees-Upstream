...
Shadow::Shadow(Channel & theChan, FEM_ObjectBroker & myBroker):theChannel(&theChan), theObjectBroker(&myBroker), theMachineBroker(0),
theRemoteActorsAddress(0),
commitTag(0)
{
    if (theChannel->setUpConnection() != 0) {
        opserr << "Shadow::Shadow() " << "- failed to setup connection\n";
        exit(-1);
    }
}


Shadow::Shadow(Channel & theChan, FEM_ObjectBroker & myBroker, ChannelAddress & theAddress):theChannel(&theChan), theObjectBroker(&myBroker), theMachineBroker(0),
theRemoteActorsAddress(&theAddress),
commitTag(0)
{
    if (theChannel->setUpConnection() != 0) {
        opserr << "Shadow::Shadow() " << "- failed to setup connection\n";
...
Shadow::Shadow(int actorType,
               ...
               int compDemand):theObjectBroker(&myBroker),
theMachineBroker(&theMachineBrokr), theRemoteActorsAddress(0), commitTag(0)
{
 ...
 Shadow::sendObject(MovableObject & theObject)
{
    return theChannel->sendObj(commitTag, theObject,
                               theRemoteActorsAddress);
}

int Shadow::recvObject(MovableObject & theObject)
{
    return theChannel->recvObj(commitTag, theObject, *theObjectBroker,
                               theRemoteActorsAddress);
}


int Shadow::recvMessage(Message & theMessage)
{
    return theChannel->recvMsg(0, commitTag, theMessage,
                               theRemoteActorsAddress);
}

int Shadow::sendMessage(const Message & theMessage)
{
    return theChannel->sendMsg(0, commitTag, theMessage,
                               theRemoteActorsAddress);
}

int Shadow::sendMatrix(const Matrix & theMatrix)
{
    return theChannel->sendMatrix(0, commitTag, theMatrix,
                                  theRemoteActorsAddress);
}

int Shadow::recvMatrix(Matrix & theMatrix)
{
    return theChannel->recvMatrix(0, commitTag, theMatrix,
                                  theRemoteActorsAddress);
}

int Shadow::sendVector(const Vector & theVector)
{
    return theChannel->sendVector(0, commitTag, theVector,
                                  theRemoteActorsAddress);
}

int Shadow::recvVector(Vector & theVector)
{
    return theChannel->recvVector(0, commitTag, theVector,
                                  theRemoteActorsAddress);
}

int Shadow::sendID(const ID & theID)
{
    return theChannel->sendID(0, commitTag, theID, theRemoteActorsAddress);
}

int Shadow::recvID(ID & theID)
{
    return theChannel->recvID(0, commitTag, theID, theRemoteActorsAddress);
}


void Shadow::setCommitTag(int tag)
{
    commitTag = tag;
}
