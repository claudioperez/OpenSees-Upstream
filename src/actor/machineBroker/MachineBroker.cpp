...
MachineBroker::MachineBroker(FEM_ObjectBroker * theBroker):theObjectBroker(theBroker), actorChannels(0), numActorChannels(0),
numActiveChannels(0),
activeChannels(0)
{
    ...
    if (actorChannels != 0) {
        ...
        for (int i = 0; i < numActorChannels; i++) {
            ...
            if (theChannel->sendID(0, 0, idData) < 0) {
                opserr <<
                    "MachineBroker::shutdown(void) - failed to send ID\n";
            }

            if (theChannel->recvID(0, 0, idData) < 0) {
                opserr <<
                    "MachineBroker::shutdown(void) - failed to recv ID\n";
            }
...
int MachineBroker::runActors(void)
{
    Channel *theChannel = this->getMyChannel();

    if (theChannel == 0) {
        opserr <<
            "MachineBroker::runActors(void) - failed to get a free Channel\n";
        return -1;
    }
    ...
    while (done == 0) {
        ...
        if (theChannel->recvID(0, 0, idData) < 0) {
            opserr <<
                "MachineBroker::runActors(void) - failed to recv ID\n";
            return -1;
        }
        ...
        if (idData(0) == 0) {
            ...
            Actor *theActor =
                theObjectBroker->getNewActor(actorType, theChannel);
            if (theActor == 0) {
                opserr <<
            ...
            if (theActor->run() != 0) {
                opserr <<
                    "MachineBroker::run(void) - actor failed while running\n";
            }
            // destroying theActor
            delete theActor;
        }
...
Channel *MachineBroker::startActor(int actorType, int compDemand)
{
    if (compDemand != 0)
        opserr <<
            "MachineBroker::startActor() - does not take computational demand variable into account\n";
    ...
    if (theChannel == 0) {
        ...
        if (theChannel == 0) {
            opserr <<
                "MachineBroker::startActor() - no available channel available\n";
            return 0;
        }
    ...
    if (theChannel->sendID(0, 0, idData) != 0) {
        opserr <<
            "MachineBroker::startActor() - failed to send actorType\n";
        this->freeProcess(theChannel);
        return 0;
    }
    ...
    if (idData(0) != 0) {
        opserr <<
            "MachineBroker::startActor() - could not start actorType: " <<
            actorType << endln;
        this->freeProcess(theChannel);
        return 0;
    }
...
int MachineBroker::finishedWithActor(Channel * theChannel)
{
...
void MachineBroker::setObjectBroker(FEM_ObjectBroker * theBroker)
{
