...
//      given by the OS. 
TCP_Socket::TCP_Socket():myPort(0), connectType(0),
...
noDelay(0)
{
    ...
    if (bind(sockfd, &my_Addr.addr, sizeof(my_Addr.addr)) < 0) {
        opserr <<
            "TCP_Socket::TCP_Socket() - could not bind local address\n";
    }
    // get my_address info
...
TCP_Socket::TCP_Socket(unsigned int port, bool checkendianness,
                       int nodelay):myPort(0), connectType(0),
 ...
 TCP_Socket::setUpConnection()
{
    if (connectType == 1) {
        ...
        if (connect(sockfd, &other_Addr.addr, sizeof(other_Addr.addr)) < 0) {
            opserr <<
                "TCP_Socket::setUpConnection() - could not connect\n";
            return -1;
        }
        // get my_address info
        getsockname(sockfd, &my_Addr.addr, &addrLength);
        ...
        if (newsockfd < 0) {
            opserr <<
                "TCP_Socket::setUpConnection() - could not accept connection\n";
            return -1;
        }
...
int TCP_Socket::setNextAddress(const ChannelAddress & theAddress)
{
...
int TCP_Socket::recvMsg(int dbTag, int commitTag,
...
// void Recv(Message &):
//      Method to receive a message, also sets other_Addr.addr_in to that of sender
int TCP_Socket::recvMsgUnknownSize(int dbTag, int commitTag,
                                   Message & msg,
                                   ChannelAddress * theAddress)
{
    // first check address is the only address a TCP_socket can send to
...
int TCP_Socket::sendMsg(int dbTag, int commitTag,
...
int TCP_Socket::recvMatrix(int dbTag, int commitTag,
...
int TCP_Socket::sendMatrix(int dbTag, int commitTag,
...
int TCP_Socket::recvVector(int dbTag, int commitTag,
...
int TCP_Socket::sendVector(int dbTag, int commitTag,
...
int TCP_Socket::recvID(int dbTag, int commitTag,
...
int TCP_Socket::sendID(int dbTag, int commitTag,
...
unsigned int TCP_Socket::getPortNumber() const
{
    return myPort;
}


unsigned int TCP_Socket::getBytesAvailable()
{
