...
TCP_SocketNoDelay::TCP_SocketNoDelay():myPort(0)
{
    // set up my_Addr 
    ...
    if ((sockfd = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
        opserr <<
            "TCP_SocketNoDelay::TCP_SocketNoDelay - could not open socket\n";
    }
    // bind local address to it
    if (bind(sockfd, (struct sockaddr *) &my_Addr, sizeof(my_Addr)) < 0) {
        opserr <<
            "TCP_SocketNoDelay::TCP_SocketNoDelay - could not bind local address\n";
    }
    // get my_address info
...
TCP_SocketNoDelay::TCP_SocketNoDelay(unsigned int port):myPort(0)
{
 ...
 TCP_SocketNoDelay::setUpActor(void)
{
    ...
    if (connect(sockfd, (struct sockaddr *) &other_Addr,
        ...
        opserr <<
            "TCP_SocketNoDelay::TCP_SocketNoDelay - could not connect\n";
        return -1;
    }
    // get my_address info
    INET_getsockname(sockfd, &my_Addr, &addrLength);
    ...
    if ((setsockopt(sockfd, IPPROTO_TCP, TCP_NODELAY,
        ...
        opserr <<
            "TCP_SocketNoDelay::TCP_SocketNoDelay - could not set TCP_NODELAY\n";
    }
...
int TCP_SocketNoDelay::setUpShadow(void)
{
    // wait for other process to contact me & set up connection
    int newsockfd;
    listen(sockfd, 1);
    newsockfd =
        accept(sockfd, (struct sockaddr *) &other_Addr, &addrLength);

    if (newsockfd < 0) {
        opserr <<
            "TCP_SocketNoDelay::TCP_SocketNoDelay - could not accept connection\n";
        return -1;
    }
...
int TCP_SocketNoDelay::setNextAddress(const ChannelAddress & theAddress)
{
    ...
    if (theAddress.getType() == SOCKET_TYPE) {
        ...
        if (memcmp((char *) &other_Addr, (char *) &theSocketAddress->addr,
            ...
            opserr <<
                "TCP_SocketNoDelay::recvMsg() - a TCP_SocketNoDelay ";
            opserr <<
                "can only communicate with one other TCP_SocketNoDelay\n";
            return -1;
        }
    } else {
        opserr <<
            "TCP_SocketNoDelay::setNextAddress() - a TCP_SocketNoDelay ";
                               ...
                               ChannelAddress * theAddress)
{
    ...
    if (theAddress != 0) {
        if (theAddress->getType() == SOCKET_TYPE)
            ...
            opserr <<
                "TCP_SocketNoDelay::sendObj() - a TCP_SocketNoDelay ";
            opserr << "can only communicate with a TCP_SocketNoDelay";
            opserr << " address given is not of type SocketAddress\n";
            return -1;
        }
...
int TCP_SocketNoDelay::recvMsg(Message & msg, ChannelAddress * theAddress)
{
    // first check address is the only address a TCP_SocketNoDelay can send to
    SocketAddress *theSocketAddress = 0;
    if (theAddress != 0) {
...
int TCP_SocketNoDelay::sendMsg(const Message & msg,
                               ChannelAddress * theAddress)
{
    // first check address is the only address a TCP_SocketNoDelay can send to
    SocketAddress *theSocketAddress = 0;
                                  ...
                                  ChannelAddress * theAddress)
{
    ...
    if (theAddress != 0) {
        ...
        if (memcmp((char *) &other_Addr, (char *) &theSocketAddress->addr,
            ...
            opserr <<
                "TCP_SocketNoDelay::recvMatrix() - a TCP_SocketNoDelay ";
            opserr <<
                "can only communicate with one other TCP_SocketNoDelay\n";
            return -1;
        }
                                  ...
                                  ChannelAddress * theAddress)
{
    ...
    if (theAddress != 0) {
        ...
        if (memcmp((char *) &other_Addr, (char *) &theSocketAddress->addr,
            ...
            opserr <<
                "TCP_SocketNoDelay::recvVector() - a TCP_SocketNoDelay ";
            opserr <<
                "can only communicate with one other TCP_SocketNoDelay\n";
            return -1;
        }
...
int TCP_SocketNoDelay::recvID(ID & theID, ChannelAddress * theAddress)
{
    // first check address is the only address a TCP_SocketNoDelay can send to
    SocketAddress *theSocketAddress = 0;
    if (theAddress != 0) {
...
char *TCP_SocketNoDelay::addToProgram(void)
{
    char *tcp = " 3 ";
