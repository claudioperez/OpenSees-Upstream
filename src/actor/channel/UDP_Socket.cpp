...
UDP_Socket::UDP_Socket():myPort(0), connectType(0),
checkEndianness(false), endiannessProblem(false)
{
    ...
    if (bind(sockfd, &my_Addr.addr, sizeof(my_Addr.addr)) < 0) {
        opserr <<
            "UDP_Socket::UDP_Socket() - could not bind local address\n";
    }
    // get my_address info
...
UDP_Socket::UDP_Socket(unsigned int port, bool checkendianness):myPort(0),
connectType(0), checkEndianness(checkendianness), endiannessProblem(false)
{
    // initialize sockets
    startup_sockets();
...
UDP_Socket::UDP_Socket(unsigned int other_Port,
                       char *other_InetAddr,
                       bool checkendianness):myPort(0), connectType(1),
...
int UDP_Socket::setNextAddress(const ChannelAddress & theAddress)
{
    ...
    if (theAddress.getType() == SOCKET_TYPE) {
        ...
        bcopy((char *) &theSocketAddress->address.addr,
              (char *) &other_Addr, theSocketAddress->addrLength);
                        ...
                        ChannelAddress * theAddress)
{
    ...
    if (theAddress != 0) {
        ...
        if (theAddress->getType() == SOCKET_TYPE) {
            ...
            bcopy((char *) &theSocketAddress->address.addr,
                  (char *) &other_Addr.addr, theSocketAddress->addrLength);
            addrLength = theSocketAddress->addrLength;
        } else {
...
int UDP_Socket::recvMsg(int dbTag, int commitTag,
                        Message & msg, ChannelAddress * theAddress)
{
    ...
    while (size > 0) {
        if (size <= MAX_UDP_DATAGRAM) {
            ...
            recvfrom(sockfd, gMsg, MAX_UDP_DATAGRAM, 0, &other_Addr.addr,
                     &addrLength);
...
int UDP_Socket::recvMsgUnknownSize(int dbTag, int commitTag,
                                   Message & msg,
                                   ChannelAddress * theAddress)
{
...
int UDP_Socket::sendMsg(int dbTag, int commitTag,
                        const Message & msg, ChannelAddress * theAddress)
{
    ...
    while (size > 0) {
        if (size <= MAX_UDP_DATAGRAM) {
            ...
            sendto(sockfd, gMsg, MAX_UDP_DATAGRAM, 0, &other_Addr.addr,
                   addrLength);
            gMsg += MAX_UDP_DATAGRAM;
            size -= MAX_UDP_DATAGRAM;
        }
...
int UDP_Socket::recvMatrix(int dbTag, int commitTag,
...
int UDP_Socket::sendMatrix(int dbTag, int commitTag,
...
int UDP_Socket::recvVector(int dbTag, int commitTag,
...
int UDP_Socket::sendVector(int dbTag, int commitTag,
...
int UDP_Socket::recvID(int dbTag, int commitTag,
...
int UDP_Socket::sendID(int dbTag, int commitTag,
...
unsigned int UDP_Socket::getPortNumber() const
{
    return myPort;
}


unsigned int UDP_Socket::getBytesAvailable()
{
