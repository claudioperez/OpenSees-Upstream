...
TCP_SocketSSL::TCP_SocketSSL (unsigned int other_Port,
                              ...
                              bool checkendianness, int nodelay):
myPort (0),
connectType (1),
checkEndianness (checkendianness),
endiannessProblem (false),
    ...
    if (connectType == 1)
      {
          ...
          if (connect (sockfd, &other_Addr.addr, sizeof (other_Addr.addr)) <
              0)
            {
                opserr <<
    ...
    if (theAddress.getType () == SOCKET_TYPE)
      {
          ...
          if (memcmp
               ...
               theSocketAddress->addrLength) != 0)
            {
                ...
                opserr <<
                    "TCP_SocketSSL::setNextAddress() - a TCP_SocketSSL ";
                opserr <<
