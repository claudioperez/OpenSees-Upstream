...
//      given by the OS.
TCP_SocketSSL::TCP_SocketSSL():myPort(0), connectType(0),
...
noDelay(0)
{
    ...
    if (SSL_CTX_use_certificate_file
        (ctx, RSA_SERVER_CERT, SSL_FILETYPE_PEM)
        <= 0) {
        opserr <<
            "TCP_SocketSSL::TCP_SocketSSL() - could not load server certificate\n";
        exit(-1);
    }
    ...
    if (SSL_CTX_use_PrivateKey_file(ctx, RSA_SERVER_KEY, SSL_FILETYPE_PEM)
        <= 0) {
        opserr <<
            "TCP_SocketSSL::TCP_SocketSSL() - could not load server key\n";
        exit(-1);
    }
    // check if server certificate and private key match
    if (!SSL_CTX_check_private_key(ctx)) {
        opserr <<
            "TCP_SocketSSL::TCP_SocketSSL() - server private key does not match the certificate public key\n";
        exit(-1);
    }
    // client verification
    if (VERIFY_CLIENT) {
        ...
        if (!SSL_CTX_load_verify_locations(ctx, RSA_SERVER_CA_CERT, NULL)) {
            opserr <<
                "TCP_SocketSSL::TCP_SocketSSL() - could not load server CA certificate\n";
            exit(-1);
        }
    ...
    if ((sockfd = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
        opserr <<
            "TCP_SocketSSL::TCP_SocketSSL() - could not open socket\n";
    }
    // bind local address to it
    if (bind(sockfd, &my_Addr.addr, sizeof(my_Addr.addr)) < 0) {
        opserr <<
            "TCP_SocketSSL::TCP_SocketSSL() - could not bind local address\n";
    }
    // get my_address info
...
TCP_SocketSSL::TCP_SocketSSL(unsigned int port, bool checkendianness,
                             int nodelay):myPort(0),
connectType(0),
checkEndianness(checkendianness),
endiannessProblem(false), noDelay(nodelay)
{
    // initialize SSL library and context object
    SSL_library_init();
    SSL_load_error_strings();
...
TCP_SocketSSL::TCP_SocketSSL(unsigned int other_Port,
                             ...
                             bool checkendianness, int nodelay):myPort(0),
connectType(1),
checkEndianness(checkendianness),
endiannessProblem(false), noDelay(nodelay)
{
    ...
    if (VERIFY_CLIENT) {
        ...
        if (SSL_CTX_use_certificate_file
            (ctx, RSA_CLIENT_CERT, SSL_FILETYPE_PEM) <= 0) {
            opserr <<
                "TCP_SocketSSL::TCP_SocketSSL() - could not load client certificate\n";
            exit(-1);
        }
        // load private key corresponding to client certificate
        if (SSL_CTX_use_PrivateKey_file
            (ctx, RSA_CLIENT_KEY, SSL_FILETYPE_PEM) <= 0) {
            opserr <<
                "TCP_SocketSSL::TCP_SocketSSL() - could not load client key\n";
            exit(-1);
        }
        // check if client certificate and private key match
        if (!SSL_CTX_check_private_key(ctx)) {
            opserr <<
                "TCP_SocketSSL::TCP_SocketSSL() - client private key does not match the certificate public key\n";
            exit(-1);
        }
    }
    // server verification
    ...
    if (!SSL_CTX_load_verify_locations(ctx, RSA_CLIENT_CA_CERT, NULL)) {
        opserr <<
            "TCP_SocketSSL::TCP_SocketSSL() - could not load client CA certificate\n";
        exit(-1);
    }
    SSL_CTX_set_verify(ctx, SSL_VERIFY_PEER, NULL);
    SSL_CTX_set_verify_depth(ctx, 1);
 ...
 TCP_SocketSSL::setUpConnection()
{
    if (connectType == 1) {
        ...
        if (connect(sockfd, &other_Addr.addr, sizeof(other_Addr.addr)) < 0) {
            opserr <<
                "TCP_SocketSSL::setUpConnection() - could not connect\n";
            return -1;
        }
        // get my_address info
        getsockname(sockfd, &my_Addr.addr, &addrLength);
        ...
        if (SSL_connect(ssl) <= 0) {
            opserr <<
                "TCP_SocketSSL::setUpConnection() - could not initiate SSL handshake with server\n";
            SSL_free(ssl);
            SSL_CTX_free(ctx);
            cleanup_sockets();
            return -2;
        ...
        if (newsockfd < 0) {
            opserr <<
                "TCP_SocketSSL::setUpConnection() - could not accept connection\n";
            return -1;
        }
        ...
        if (SSL_accept(ssl) <= 0) {
            opserr <<
                "TCP_SocketSSL::setUpConnection() - could not initiate SSL handshake with client\n";
            SSL_free(ssl);
            SSL_CTX_free(ctx);
            cleanup_sockets();
            return -2;
            ...
            if (cert != NULL) {
                opserr << "\nSSL Client certificate:\n"
                    ...
                    << X509_NAME_oneline(X509_get_subject_name(cert),
                                         0,
                                         0) << "\nIssuer name: " <<
                    X509_NAME_oneline(X509_get_issuer_name(cert), 0,
                                      0) << endln;
                X509_free(cert);
            } else {
...
int TCP_SocketSSL::setNextAddress(const ChannelAddress & theAddress)
{
    ...
    if (theAddress.getType() == SOCKET_TYPE) {
        ...
        if (memcmp
            ((char *) &other_Addr.addr_in,
             (char *) &theSocketAddress->address.addr,
             theSocketAddress->addrLength) != 0) {

            opserr << "TCP_SocketSSL::setNextAddress() - a TCP_SocketSSL ";
            opserr <<
                "can only communicate with one other TCP_SocketSSL\n";
            return -1;
        }
    } else {
        opserr << "TCP_SocketSSL::setNextAddress() - a TCP_SocketSSL ";
...
int TCP_SocketSSL::sendObj(int commitTag,
...
int TCP_SocketSSL::recvObj(int commitTag,
                           MovableObject & theObject,
...
int TCP_SocketSSL::recvMsg(int dbTag, int commitTag,
                           Message & msg, ChannelAddress * theAddress)
{
    ...
    if (theAddress != 0) {
        ...
        if (memcmp
            ((char *) &other_Addr.addr_in,
             (char *) &theSocketAddress->address.addr_in,
             theSocketAddress->addrLength) != 0) {
...
// void Recv(Message &):
//      Method to receive a message, also sets other_Addr.addr_in to that of sender
int TCP_SocketSSL::recvMsgUnknownSize(int dbTag, int commitTag,
                                      Message & msg,
                                      ChannelAddress * theAddress)
{
    // first check address is the only address a TCP_socket can send to
    ...
    if (theAddress != 0) {
        if (theAddress->getType() == SOCKET_TYPE)
            ...
            opserr <<
                "TCP_SocketSSL::recvMsgUnknownSize() - a TCP_SocketSSL ";
            opserr << "can only communicate with a TCP_SocketSSL";
            opserr << " address given is not of type SocketAddress\n";
            return -1;
        }
    ...
    while (!eol) {
        ...
        while (nleft > 0) {
            ...
            if (nread < 0) {
                opserr <<
                    "TCP_SocketSSL::recvMsgUnknownSize() - could not read data\n";
                SSL_shutdown(ssl);
                SSL_free(ssl);
                SSL_CTX_free(ctx);
                cleanup_sockets();
...
int TCP_SocketSSL::sendMsg(int dbTag, int commitTag,
                           const Message & msg,
                           ChannelAddress * theAddress)
{
    // first check address is the only address a TCP_socket can send to
...
int TCP_SocketSSL::recvMatrix(int dbTag, int commitTag,
                              Matrix & theMatrix,
                              ChannelAddress * theAddress)
{
    // first check address is the only address a TCP_socket can send to
    ...
    while (nleft > 0) {
        ...
        if (nread < 0) {
            opserr <<
                "TCP_SocketSSL::recvMatrix() - could not read data\n";
            SSL_shutdown(ssl);
            SSL_free(ssl);
            SSL_CTX_free(ctx);
            cleanup_sockets();
...
int TCP_SocketSSL::sendMatrix(int dbTag, int commitTag,
...
int TCP_SocketSSL::recvVector(int dbTag, int commitTag,
                              Vector & theVector,
                              ChannelAddress * theAddress)
{
    // first check address is the only address a TCP_socket can send to
    ...
    while (nleft > 0) {
        ...
        if (nread < 0) {
            opserr <<
                "TCP_SocketSSL::recvVector() - could not read data\n";
            SSL_shutdown(ssl);
            SSL_free(ssl);
            SSL_CTX_free(ctx);
            cleanup_sockets();
...
int TCP_SocketSSL::sendVector(int dbTag, int commitTag,
...
int TCP_SocketSSL::recvID(int dbTag, int commitTag,
...
int TCP_SocketSSL::sendID(int dbTag, int commitTag,
...
unsigned int TCP_SocketSSL::getPortNumber() const
{
    return myPort;
}


unsigned int TCP_SocketSSL::getBytesAvailable()
{
...
char *TCP_SocketSSL::addToProgram()
{
    char *tcp = " 1 ";
