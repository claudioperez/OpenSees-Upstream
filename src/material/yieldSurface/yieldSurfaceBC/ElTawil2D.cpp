...
ElTawil2D::ElTawil2D (int tag, double xbal, double ybal, double ypos,
                      double yneg, YS_Evolution & model, double cz_,
                      double ty_):
...
void
ElTawil2D::getGradient (double &gx, double &gy, double x, double y)
{
...
double
ElTawil2D::getSurfaceDrift (double x, double y)
{
    double phi;
...
void
ElTawil2D::customizeInterpolate (double &xi, double &yi, double &xj,
                                 double &yj)
{
...
YieldSurface_BC *
ElTawil2D::getCopy (void)
{
    ElTawil2D *theCopy =
        new ElTawil2D (this->getTag (), xBal, yBal, yPosCap_orig,
                       yNegCap_orig, *hModel,
                       cz, ty);
...
int
ElTawil2D::displaySelf (Renderer & theViewer, int displayMode, float fact)
{
    ...
    double err = 1e-4;
    for (double yc = yNegCap / capY; yc <= yPosCap / capY + err;
         yc = yc + incr)
      {
          double y = yc;
          double yVal = y * capY;
...
void
ElTawil2D::Print (OPS_Stream & s, int flag)
{
