...
void
Orbison2D::getGradient (double &gx, double &gy, double x, double y)
{
// check if the point is on the surface
    ...
    if (forceLocation (drift) != 0)
      {
          opserr <<
              "ERROR - Orbison2D::getGradient(double &gx, double &gy, double x, double y)\n";
    ...
    else
      {
          ...
          gy = 2.3 * y / (capy) - 0.9 * pow (y, 5) / (capy) + 7.34 * pow (x,
                                                                          2) *
              (y / (capy));
...
double
Orbison2D::getSurfaceDrift (double x, double y)
{
    double phi =
        1.15 * y * y - 0.15 * pow (y, 6) + x * x + 3.67 * y * y * x * x;
    double drift = phi - 1;
    return drift;
}

YieldSurface_BC *
Orbison2D::getCopy (void)
{
...
int
Orbison2D::displaySelf (Renderer & theViewer, int displayMode, float fact)
{
    ...
    for (double y = 0; y <= 1 + err; y = y + incr)
      {
          if (y > 1)
              y = int (1);
          double x =
              (1 - 1.15 * y * y + 0.15 * pow (y, 6)) / (1 + 3.67 * y * y);
          if (x > 0)
              x = sqrt (x);
...
void
Orbison2D::Print (OPS_Stream & s, int flag)
{
