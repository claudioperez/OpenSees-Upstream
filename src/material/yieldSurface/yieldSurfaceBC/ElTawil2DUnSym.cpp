...
ElTawil2DUnSym::ElTawil2DUnSym
    (int tag, double xPosbal, double yPosbal,
     ...
     YS_Evolution & model,
     double cz_pos, double ty_pos, double cz_neg, double ty_neg):
...
czPos (cz_pos), tyPos (ty_pos), czNeg (cz_neg), tyNeg (ty_neg),
qy (0.005)
{
...
//      xNeg = -1;
    ...
    xPos = min_ (xPosBal / capX,
                 (xPosBal *
                  (1 -
                   pow (fabs (yPosBal / (yNegCap - yPosBal)),
                        tyPos))) / capX);
    // xNeg = xNegBal/capX;
    xNeg = max_ (xNegBal / capX,
                 (xNegBal *
                  (1 -
                   pow (fabs (yNegBal / (yNegCap - yNegBal)),
                        tyNeg))) / capX);
...
*/
    ...
    double xt4 =
        xPosBal * (1 - pow (fabs (yVal4 / (yNegCap - yPosBal)), tyPos));
    double xt2 = xNegBal * (1 - pow ((yVal2 / (yPosCap - yNegBal)), czNeg));
    double xt3 =
        xNegBal * (1 - pow (fabs (yVal3 / (yNegCap - yNegBal)), tyNeg));
...
void
ElTawil2DUnSym::getGradient (double &gx, double &gy, double x, double y)
{
// check if the point is on the surface
    ...
    if (loc != 0)
      {
          opserr <<
              "ERROR - ElTawil2D::getGradient(double &gx, double &gy, double x, double y)\n";
          opserr << "Force point not on yield surface, drift = " << drift <<
              " loc = " << loc << "\n";
          opserr << "\a";
      }
    else
          ...
          if (y > ytPos)
          ...
          else
            {
                ...
                if (xVal >= 0 && yVal >= yPosBal)       // quadrant 1
                  {
                      gx = 1 / xPosBal;
                      gy = (1 / pow ((yPosCap - yPosBal), czPos)) * czPos *
                          (pow (yVal - yPosBal, czPos - 1));
                  }
                else if (xVal >= 0 && yVal < yPosBal)   // quadrant 1 or 4
                  {
                      gx = 1 / xPosBal;
                      gy = -1 * (1 / pow (fabs (yNegCap - yPosBal), tyPos)) *
                          tyPos * (pow (fabs (yVal - yPosBal), tyPos - 1));
                  }
                else if (xVal < 0 && yVal >= yNegBal)   // quadrant 2
                  {
                      gx = 1 / xNegBal; // xNegBal should be < 0
                      gy = (1 / pow ((yPosCap - yNegBal), czNeg)) * czNeg *
                          (pow (yVal - yNegBal, czNeg - 1));
                  }
                else if (xVal < 0 && yVal < yNegBal)    // quadrant 2 or 3
                  {
                      gx = 1 / xNegBal;
                      gy = -1 * (1 / pow (fabs (yNegCap - yNegBal), tyNeg)) *
                          tyNeg * (pow (fabs (yVal - yNegBal), tyNeg - 1));
                  }
                else
                  {
                      opserr << "Eltawil2DUnsym - condition not possible" <<
                          endln;
                      opserr << "\a";
                  }
          ...
          if (xVal >= 0 && yVal >= yPosBal)     // quad 1
            {
                phi =
                    fabs (xVal / xPosBal) +
                    pow ((yVal - yPosBal) / (yPosCap - yPosBal), czPos);
            }
          else if (xVal >= 0 && yVal < yPosBal) // quad 1 or 4
            {
                phi =
                    fabs (xVal / xPosBal) +
                    pow (fabs ((yVal - yPosBal) / (yNegCap - yPosBal)),
                         tyPos);
            }
          else if (xVal < 0 && yVal >= yNegBal) // quad 2
            {
                phi =
                    fabs (xVal / xNegBal) +
                    pow ((yVal - yNegBal) / (yPosCap - yNegBal), czNeg);
            }
          else if (xVal < 0 && yVal < yNegBal)  // quad 2 or 3
            {
                ...
                phi =
                    fabs (xVal / xNegBal) +
                    pow (fabs ((yVal - yNegBal) / (yNegCap - yNegBal)),
                         tyNeg);
            }
          else
            {
                opserr <<
                    "ElTawil2DUnSym::getSurfaceDrift(..) - cond not possible\n";
                opserr << "x=" << x << ", y=" << y << ", capx=" << capx <<
                    ", capy=" << capy << endln;
...
void
ElTawil2DUnSym::customizeInterpolate (double &xi, double &yi, double &xj,
                                      double &yj)
{
...
ElTawil2DUnSym::getCopy (void)
{
    ...
    ElTawil2DUnSym *theCopy =
        new ElTawil2DUnSym (this->getTag (), xPosBal, yPosBal + yBal,
                            xNegBal, yNegBal + yBal, yPosCap + yBal,
                            yNegCap + yBal,
...
int
ElTawil2DUnSym::displaySelf (Renderer & theViewer, int displayMode,
                             float fact)
{
        ...
        //while(1)
      {
          ...
          if (yVal < yPosBal)
          ...
          else
            {
                xVal =
                    xPosBal * (1 -
                               pow (((yVal - yPosBal) / (yPosCap - yPosBal)),
                                    czPos));
            }
        ...
        //while(1)
      {
          ...
          if (yVal < yNegBal)
          ...
          else
            {
                xVal =
                    xNegBal * (1 -
                               pow (((yVal - yNegBal) / (yPosCap - yNegBal)),
                                    czNeg));
            }
...
void
ElTawil2DUnSym::Print (OPS_Stream & s, int flag)
{
    s << "\nYield Surface No: " << this->
        getTag () << " type: ElTawil2DUnSym\n";
}
