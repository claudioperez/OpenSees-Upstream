          ...
          if (argc < 6)
            {
                ...
                opserr <<
                    "Want: yieldSurfaceBC Orbison2D tag? xCap? yCap? ys_model_tag?"
                    << endln;
                return TCL_ERROR;
            }
...
//              double isoRatio;
          ...
          if (Tcl_GetInt (interp, argv[5], &modelID) != TCL_OK)
            {
                opserr << "WARNING invalid yieldSurfaceBC Orbison2D matID1" <<
                    modelID << endln;
                return TCL_ERROR;
            }
          ...
          if (theModel == 0)
            {
                opserr <<
                    "WARNING yieldSurfaceBC Orbison2D no ys_model exixts with tag: "
                    << modelID << endln;
                return TCL_ERROR;
            }
    ...
    else if (strcmp (argv[1], "ElTawil2D") == 0)
      {
          if (argc < 7)
            {
                ...
                // Orbison2D(int tag, double xmax, double ymax, YS_HardeningModel &model);
                opserr <<
                    "Want: yieldSurfaceBC ElTawil2D tag? xCap? yCap? ys_model_tag?"
                    << endln;
                return TCL_ERROR;
            }
          ...
          if (Tcl_GetInt (interp, argv[7], &modelID) != TCL_OK)
            {
                opserr << "WARNING invalid yieldSurfaceBC ElTawil2D matID1" <<
                    modelID << endln;
                return TCL_ERROR;
            }
          ...
          if (theModel == 0)
            {
                opserr <<
                    "WARNING yieldSurfaceBC ElTawil2D no ys_model exixts with tag: "
                    << modelID << endln;
                return TCL_ERROR;
            }
    ...
    else if (strcmp (argv[1], "ElTawil2DUnSym") == 0)
      {
          if (argc < 9)
            {
                ...
                // Orbison2D(int tag, double xmax, double ymax, YS_HardeningModel &model);
                opserr <<
                    "Want: yieldSurfaceBC ElTawil2DUnSym tag? xPosBal? yPosBal? "
          ...
          if (Tcl_GetInt (interp, argv[9], &modelID) != TCL_OK)
            {
                opserr <<
                    "WARNING invalid yieldSurfaceBC ElTawil2DUnSym matID1" <<
                    modelID << endln;
                return TCL_ERROR;
            }
          ...
          // Parsing was successful, allocate the material
          theYS =
              new ElTawil2DUnSym (tag, xPosBal, yPosBal, xNegBal, yNegBal,
                                  yPos, yNeg, *theModel);
      }
...
//                                      double a04=-1.64, double a05=2.21, double a06=2.10);
          ...
          if (argc < 6 || argc > 14)
            {
                ...
                printCommand (argc, argv);
                opserr <<
                    "Want: yieldSurfaceBC Attalla2D tag? xCap? yCap? matXTag? maxYTag? isoRatio? <..>"
                    << endln;
                return TCL_ERROR;
            }
          ...
          if (Tcl_GetInt (interp, argv[5], &modelID) != TCL_OK)
            {
                opserr << "WARNING invalid yieldSurfaceBC Attalla2D modelID"
                    << modelID << endln;
                return TCL_ERROR;
            }
          ...
          if (argc > 6)
            {
                ...
                for (int i = 0; i < 6; i++)
                  {
                      if (Tcl_GetDouble (interp, argv[count], &temp) !=
                          TCL_OK)
                        {
                            opserr << "WARNING invalid parameter " << i +
                                1 << "\n";
...
//              double isoRatio;
          ...
          if (Tcl_GetInt (interp, argv[3], &modelID) != TCL_OK)
            {
                opserr << "WARNING invalid yieldSurfaceBC Hajjar2D  matID1" <<
                    modelID << endln;
                return TCL_ERROR;
            }
