...
void
Attalla2D::getGradient (double &gx, double &gy, double x, double y)
{
// check if the point is on the surface
    ...
    if (loc != 0)
      {
          opserr <<
              "ERROR - Attalla2D::getGradient(double &gx, double &gy, double x, double y)\n";
    ...
    else
      {
          ...
          if (y > yt)
            ...
            {
                gx = 6 * a2 * pow (x, 5) / capx + 4 * a4 * pow (x,
                                                                3) / capx +
                    2 * a6 * x / capx;
                gy = 6 * a1 * pow (y, 5) / capy + 4 * a3 * pow (y,
                                                                3) / capy +
                    2 * a5 * y / capy;
            }
      }
...
double
Attalla2D::getSurfaceDrift (double x, double y)
{
    double phi;
    ...
    if (y > yt && fabs (x) < fabs (y) * xt / yt)
      ...
      {
          phi =
              a1 * pow (y, 6) + a2 * pow (x, 6) + a3 * pow (y,
                                                            4) + a4 * pow (x,
                                                                           4)
              + a5 * y * y + a6 * x * x;
      }
...
void
Attalla2D::customizeInterpolate (double &xi, double &yi, double &xj,
                                 double &yj)
{
...
int
Attalla2D::displaySelf (Renderer & theViewer, int displayMode, float fact)
{
...
void
Attalla2D::Print (OPS_Stream & s, int flag)
{
