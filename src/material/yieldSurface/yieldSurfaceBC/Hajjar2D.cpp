...
fy (fy_)
{
    ...
    capX = fc * (0.5 * (b - 2 * t) * (xn - t) * (xn - t))
        ...
        + fy * ((2 * t) * (D * D / 2 + xn * xn + t * t - D * t - D * xn) +
                (b * t) * (D - t));
...
void
Hajjar2D::getGradient (double &gx, double &gy, double xi, double yi)
{
// check if the point is on the surface
    ...
    if (forceLocation (drift) != 0)
      {
          opserr <<
              "ERROR - Hajjar2D::getGradient(double &gx, double &gy, double x, double y)\n";
...
double
Hajjar2D::getSurfaceDrift (double xi, double yi)
{
    double x = xi;
    double y = yi /* - centroidY */ ;
...
int
Hajjar2D::displaySelf (Renderer & theViewer, int displayMode, float fact)
{
...
//      return 0;
    ...
    opserr << " xmax = " << xmax << ", ymax = " << yOld << "( " << sqrt (1 /
                                                                         c2)
        << ")\n";
    if (fact < 1)
        incr = fact;
...
void
Hajjar2D::Print (OPS_Stream & s, int flag)
{
