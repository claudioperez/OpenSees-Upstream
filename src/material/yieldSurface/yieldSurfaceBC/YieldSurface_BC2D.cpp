...
double
YieldSurface_BC2D::error (1.0e-6);
Vector
YieldSurface_BC2D::v6 (6);
Vector
YieldSurface_BC2D::T2 (2);
Vector
YieldSurface_BC2D::F2 (2);
Vector
YieldSurface_BC2D::g2 (2);
Vector
YieldSurface_BC2D::v2 (2);
Vector
YieldSurface_BC2D::v4 (4);
...
YieldSurface_BC2D::YieldSurface_BC2D (int tag, int classTag, double xmax,
                                      double ymax, YS_Evolution & model):
...
const Vector &
YieldSurface_BC2D::getExtent (void)
{
    v4 (0) = xPos;
    v4 (1) = xNeg;
    v4 (2) = yPos;
...
void
YieldSurface_BC2D::setTransformation (int xDof, int yDof, int xFact,
                                      int yFact)
{
    ...
    if (xPos == 0 && yPos == 0 && xNeg == 0 && yNeg == 0)
      {
          opserr <<
              "WARNING - YieldSurface_BC2D - surface extent not set correctly\n";
      }
...
int
YieldSurface_BC2D::getState (int stateInfo)
{
...
int
YieldSurface_BC2D::commitState (Vector & force)
{
...
//      opserr << " FORCE = " << force;
    if (status_hist > 0)
      {
          opserr << "WARNING - YieldSurface_BC2D::commitState(..) [" <<
              getTag () << "]\n";
...
int
YieldSurface_BC2D::update (int flag)
{
...
int
YieldSurface_BC2D::revertToLastCommit (void)
{
...
int
YieldSurface_BC2D::modifySurface (double magPlasticDefo, Vector & Fsurface,
                                  Matrix & G, int flag)
{
...
// check 1
    if (this->getTrialForceLocation (Fsurface) != 0)
      {
          opserr << "Can't modify surface with Force Location = " <<
              getTrialForceLocation (Fsurface) << endln;
          return 0;
      }
...
void
YieldSurface_BC2D::getCommitGradient (Matrix & G)
{
    double gx = gx_hist;
    double gy = gy_hist;
...
void
YieldSurface_BC2D::getTrialGradient (Matrix & G, Vector & force)
{
    double gx, gy, fx, fy;
...
double
YieldSurface_BC2D::getTrialDrift (Vector & force)
{
    double fx, fy;
    toLocalSystem (force, fx, fy, true);
    hModel->toOriginalCoord (fx, fy);
...
// used by the element
int
YieldSurface_BC2D::getTrialForceLocation (Vector & force)
{
...
int
YieldSurface_BC2D::getCommitForceLocation ()
{
    return status_hist;
}
...
// +1 -> outside
int
YieldSurface_BC2D::forceLocation (double drift)
{
    double tolNeg = 0.00;
    double tolPos = 1e-5;
    ...
    if (drift < -tolNeg)
      ...
      {
          opserr <<
              "YieldSurface_BC2D::forceLocation(double drift) - this condition not possible\n";
          opserr << "\a";
      }
...
void
YieldSurface_BC2D::addPlasticStiffness (Matrix & K)
{
...
double
YieldSurface_BC2D::getDrift (double x, double y)
{
...
// intersects the internal quad -> x_in, y_in (R_in)
    ...
    if (x != 0)
      {
          ...
          if (x > 0 && y >= 0)
          ...
          else if (x > 0 && y <= 0)
            {
              ...
              opserr <<
                  "YieldSurface_BC2D::getDrift(..) - condition not possible, x = "
                  << x << ", y = " << y << endln;

          if (driftDebug)
            {
                opserr << "Equation Internal: a = " << a_int << ", b = " <<
                    b_int << "\n";
                opserr << "Equation External: a = " << a_ext << ", b = " <<
                    b_ext << "\n";
            }
    ...
    if (R0 == 0)
        opserr << "ERROR: YieldSurface_BC2D::getDrift(..) - R0 = 0 (yPos=" <<
            yPos << ", yNeg=" << yNeg << "\n";
    ...
    while (1)
      {
          ...
          if (getSurfaceDrift (xNew, yNew) > 0) // we just crossed the surface
            {
                ...
                if (driftDebug)
                  {
                      opserr << " Set to surface at xi = " << xi << ", yi = "
                          << yi << ", Ri = " << Ri << "\n";
          ...
          if (count > 100)
            {
                opserr <<
                    "ERROR: YieldSurface_BC2D::getDrift(..) - not converging\n";
                opserr << "\a";
            }
...
void
YieldSurface_BC2D::customizeInterpolate (double &xi, double &yi, double &xj,
                                         double &yj)
{
    double yCheck = yNeg;
...
double
YieldSurface_BC2D::interpolate (double xi, double yi, double xj, double yj)
{
...
//check
    ...
    if (di > 0)
      {
          opserr <<
              "ERROR - YieldSurface_BC2D::interpolate(xi, yi, xj, yj)\n";
...
double
YieldSurface_BC2D::interpolateClose (double xi, double yi, double xj,
                                     double yj)
{
...
//check
    ...
    if (di > 0)
      {
          opserr <<
              "ERROR - YieldSurface_BC2D::interpolateClose(xi, yi, xj, yj)\n";
          opserr << "point 1 is outside\n";
          opserr << xi << "," << yi << "  " << xj << "," << yj << " : " << di
              << "\n";
          opserr << "\a";
...
double
YieldSurface_BC2D::setToSurface (Vector & force, int algoType, int color)
{
    double x2, y2;
    double xi, yi, xj, yj;
    double dx, dy, t;
    ...
    if (returnDebug)
      {
          opserr << "Local system force - " << "fx = " << x2 << ",\tfy = " <<
              y2 << "\n";
          opserr << "toOriginalCoord    - " << "fx = " << xj << ",\tfy = " <<
              yj << "\n";
      }
...
int
YieldSurface_BC2D::displaySelf (Renderer & theViewer, int displayMode,
                                float fact)
{
...
int
YieldSurface_BC2D::displayCommitForcePoint (Renderer & theViewer,
...
int
YieldSurface_BC2D::displayForcePoint (Vector & force, int color)
{
    if (!theView)
        return -1;
...
int
YieldSurface_BC2D::displayForcePoint (bool toDeformed, double f_x, double f_y,
                                      int color)
{
    Vector p1 (3), p2 (3);
    Vector rgb (3);
