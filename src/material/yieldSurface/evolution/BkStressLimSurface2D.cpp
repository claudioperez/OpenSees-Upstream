...
BkStressLimSurface2D::BkStressLimSurface2D (int tag, int classTag,
                                            ...
                                            PlasticHardeningMaterial &
                                            isoYNeg, int restype,
                                            double res_Fact, double app_Fact,
                                            double dir):
...
direction_orig (dir)
{
    if (dir < -1.0)
      {
          opserr <<
              "WARNING: BkStressLimSurface2D() - Dir should be between -1 and +1\n";
...
void
BkStressLimSurface2D::setResidual (double res)
{
...
int
BkStressLimSurface2D::commitState ()
{
...
int
BkStressLimSurface2D::revertToLastCommit (void)
{
...
void
BkStressLimSurface2D::setTrialPlasticStrains (double lamda, const Vector & f,
                                              const Vector & g)
{
                    ...
                    if (sign (g (0)) != sign (translate_hist (0)))
                      {
                          ...
                          dR = fabs (limSurface->getDrift (0.0, y0)) * 2 -
                              fabs (drift);
//          not required
//                      else
//                              dR = fabs(drift);
...
const Vector &
BkStressLimSurface2D::getEquiPlasticStiffness ()
{
...
double
BkStressLimSurface2D::getTrialPlasticStrains (int dir)
{
    if (dir == 0 && defPosX)
        ...
        opserr <<
            "BkStressLimSurface2D::getTrialPlasticStrains(double dir) - incorrect dir||condition \n";
    return 0;
}

double
BkStressLimSurface2D::getCommitPlasticStrains (int dir)
{
...
double
BkStressLimSurface2D::getIsoPlasticStiffness (int dir)
{
    if (dir == 0 && defPosX)
        ...
        opserr <<
            "BkStressLimSurface2D::getIsoPlasticStiffness(double dir) - incorrect dir/condition \n";
    return 0;
}

double
BkStressLimSurface2D::getKinPlasticStiffness (int dir)
{
    if (dir == 0)
        ...
        opserr <<
            "BkStressLimSurface2D::getKinPlasticStiffness(double dir) - incorrect dir\n";
    return 0;
...
int
BkStressLimSurface2D::displaySelf (Renderer & theViewer, int displayMode,
                                   float fact)
{
