...
int
YS_Evolution2D::update (int flag)
{
...
int
YS_Evolution2D::revertToLastCommit (void)
{
...
int
YS_Evolution2D::evolveSurface (YieldSurface_BC * ys, double lamda,
    ...
    if (!ys_harden && (kinematicRatio != kinematicRatio_shrink)
        && (isotropicRatio != isotropicRatio_shrink))
      {
          ...
          if (ys->getDrift (fx_new, fy_new) > 0)
            {
                opserr <<
                    "oops: YS_Evolution2D::evolveSurface() - softens->hardens\n";
                ys_harden = true;
            }
      }
    ...
    if (evolDebug)
      {
          ...
          opserr << "Gradient: " << gl;
          opserr << "KpxIso = " << getIsoPlasticStiffness (0) << ", KpyIso = "
              << getIsoPlasticStiffness (1) << endln;
          opserr << "F_surf(1) = " << f_sur (1) << ", dfy_iso = " << dfy_iso
              << endln;
...
int
YS_Evolution2D::displaySelf (Renderer & theViewer, int displayMode,
                             float fact)
{
    return -1;
}
...
Response *
YS_Evolution2D::setResponse (char **argv, int argc, Information & matInfo)
{
    return 0;
/*
...
int
YS_Evolution2D::getResponse (int responseID, Information & matInfo)
{
    return 0;
    /*
