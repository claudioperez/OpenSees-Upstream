...
PlasticHardening2D::PlasticHardening2D (int tag, int classTag,
...
defPosY (true)
{
    if (dir < -1.0 || dir > 1.0)
      {
          opserr <<
              "WARNING: PlasticHardening2D() - Dir should be between -1 and +1\n";
          opserr << "Set to 0 \n";
          direction = 0.0;
      }
...
int
PlasticHardening2D::commitState ()
{
...
int
PlasticHardening2D::revertToLastCommit (void)
{
...
void
PlasticHardening2D::setTrialPlasticStrains (double lamda, const Vector & f,
                                            const Vector & g)
{
    ...
    if (strnDebug)
      {
          ...
          opserr << "bool defPosX = " << defPosX << ", bool defPosY = " <<
              defPosY << endln;
      }
...
const Vector &
PlasticHardening2D::getEquiPlasticStiffness ()
{
    if (freezeEvolution)
      {
          v2 (0) = 0.0;
...
double
PlasticHardening2D::getTrialPlasticStrains (int dir)
{
    if (dir == 0 && defPosX)
        ...
        opserr <<
            "PlasticHardening2D::getTrialPlasticStrains(double dir) - incorrect dir||condition \n";
    return 0;
}

double
PlasticHardening2D::getCommitPlasticStrains (int dir)
{
    opserr <<
        "PlasticHardening2D::getCommitPlasticStrains(double dir) - not yet implemented \n";
    this->getTrialPlasticStrains (dir);
    return 0;
}

double
PlasticHardening2D::getIsoPlasticStiffness (int dir)
{
    double kp = 0;
...
//                      kp = 0.123*kp;
    ...
    else if (dir == 1)
      {
        ...
        opserr <<
            "WARNING: PlasticHardening2D::getPlasticStiffness(int dir) - incorrect dir\n";
    return kp;
}

double
PlasticHardening2D::getKinPlasticStiffness (int dir)
{
...
void
PlasticHardening2D::Print (OPS_Stream & s, int flag)
{
