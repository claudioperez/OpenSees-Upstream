...
YS_Evolution::YS_Evolution (int tag, int classtag,
                            ...
                            int _dimension, double shr_iso, double shr_kin):
TaggedObject (tag),
MovableObject (classtag),
isotropicRatio (iso_ratio),
kinematicRatio (kin_ratio),
isotropicRatio_orig (iso_ratio),
kinematicRatio_orig (kin_ratio),
dimension (_dimension),
deformable (false),
translate_hist (_dimension),
translate (_dimension),
translate_init (_dimension),
isotropicFactor (_dimension),
isotropicFactor_hist (_dimension),
isotropicRatio_shrink (shr_iso),
kinematicRatio_shrink (shr_kin),
...
void
YS_Evolution::setInitTranslation (Vector & initTranslate)
{
    if (initTranslate.Size () > dimension)
      {
          opserr << "WARNING -  newTranslate" << initTranslate <<
...
const Vector &
YS_Evolution::getInitTranslation (void)
{
...
void
YS_Evolution::setResidual (double res)
{
    // does nothing
}
...
int
YS_Evolution::revertToLastCommit (void)
{
...
void
YS_Evolution::toDeformedCoord (Vector & coord)
{
//      if(getTag() == 10)
//      {
//              opserr << "DIM: " << dimension << endln;
...
void
YS_Evolution::toOriginalCoord (Vector & coord)
{
    for (int i = 0; i < coord.Size (); i++)
      {
...
void
YS_Evolution::toDeformedCoord (double &x)
{
...
void
YS_Evolution::toDeformedCoord (double &x, double &y)
{
//  opserr << "WARNING YS_Evolution::toDeformedCoord(..) - This method should not be called\n";
    crd2 (0) = x;
    crd2 (1) = y;
...
void
YS_Evolution::toDeformedCoord (double &x, double &y, double &z)
{
//      opserr << "WARNING YS_Evolution::toDeformedCoord(...) - This method should not be called\n";
    crd3 (0) = x;
    crd3 (1) = y;
...
void
YS_Evolution::toOriginalCoord (double &x)
{
...
void
YS_Evolution::toOriginalCoord (double &x, double &y)
{
//      opserr << "WARNING YS_Evolution::toOriginalCoord(..) - This method should not be called\n";
    crd2 (0) = x;
    crd2 (1) = y;
...
void
YS_Evolution::toOriginalCoord (double &x, double &y, double &z)
{
//      opserr << "WARNING YS_Evolution::toOriginalCoord(...) - This method should not be called\n";
    crd3 (0) = x;
    crd3 (1) = y;
...
double
YS_Evolution::getCommitTranslation (int dir)
{
#ifdef _G3DEBUG
    checkDimension (dir);
#endif
...
double
YS_Evolution::getTrialTranslation (int dir)
{
#ifdef _G3DEBUG
    checkDimension (dir);
#endif
...
double
YS_Evolution::getCommitIsotropicFactor (int dir)
{
    return isotropicFactor_hist (dir);
}

double
YS_Evolution::getTrialIsotropicFactor (int dir)
{
    return isotropicFactor (dir);
}

void
YS_Evolution::checkDimension (int dir)
{
    if (dir < 0 || dir >= dimension)
      {
          opserr << "WARNING - Direction " << dir <<
              " outside the dimensions\n";
      }
...
void
YS_Evolution::setDeformable (bool defo)
{
    this->deformable = defo;
}

void
YS_Evolution::Print (OPS_Stream & s, int flag)
{
