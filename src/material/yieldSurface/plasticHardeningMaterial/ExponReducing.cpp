...
double
ExponReducing::getTrialPlasticStiffness ()
{
...
//      opserr << "K = " << K << ", sFactor = " << sFactor << endln;
    ...
    if (K < 0.0)
      {
          opserr << "Ri = " << val_trial << ", Factor = " << K /
              Kp0 << ", res_fact = " << resFactor << endln;
          opserr << "\a";
      }
...
void
ExponReducing::Print (OPS_Stream & s, int flag)
{
...
ExponReducing::getCopy (void)
{
    PlasticHardeningMaterial *theMat =
        new ExponReducing (getTag (), Kp0, alpha, resFactor);
    return theMat;
}
