    ...
    if (strcmp (argv[1], "multiLinearKp") == 0)
      {
          ...
          if (numPoints < 2)
            {
                opserr <<
                    "WARNING invalid uniaxialMaterial MultilinearUniaxial tag"
                    << endln;
    ...
    if (argc == 6)
      {
          if (Tcl_GetDouble (interp, argv[5], &arg3) != TCL_OK)
            {
                opserr <<
                    "WARNING invalid double PlaticHardening exponReducing" <<
                    endln;
                return TCL_ERROR;
            }
...
int
TclModelBuilderPlasticMaterialCommand (ClientData clientData,
                                       ...
                                       TclModelBuilder * theTclBuilder)
{
    ...
    /*else if(strcmp(argv[1],"quadrReducing") == 0)
      ...
      {
          return TclNullPlasticMaterialCommand (clientData, interp, argc,
                                                argv, theTclBuilder);
      }
    else
      {
          opserr <<
              "Unknown PlasticMaterial: \nValid types: null, multiLinearKp, "
