...
void *OPS_Steel02()
{
    ...
    if (numData != 3 && numData != 6 && numData != 10 && numData != 11) {
        opserr << "Invalid #args, want: uniaxialMaterial Steel02 " <<
    ...
    if (numData == 3) {
        if (OPS_GetDoubleInput(&numData, dData) != 0) {
            opserr << "Invalid double: uniaxialMaterial Steel02 " <<
                iData[0] << " fy? E? b? <R0? cR1? cR2? <a1? a2? a3? a4?>>"
                << endln;
            return 0;
        }
        ...
        // Parsing was successful, allocate the material
        theMaterial =
            new Steel02(iData[0], dData[0], dData[1], dData[2], dData[3],
                        dData[4], dData[5]);

    } else if (numData == 10) {
        if (OPS_GetDoubleInput(&numData, dData) != 0) {
            opserr << "Invalid arggs: uniaxialMaterial Steel02 " <<
                iData[0] << " fy? E? b? <R0? cR1? cR2? <a1? a2? a3? a4?>>"
                << endln;
            return 0;
        }
    ...
    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type Steel02 Material\n";
        return 0;
    }
...
Steel02::Steel02(int tag,
                 ...
                 double sigInit):UniaxialMaterial(tag, MAT_TAG_Steel02),
Fy(_Fy),
E0(_E0),
b(_b),
R0(_R0),
cR1(_cR1), cR2(_cR2), a1(_a1), a2(_a2), a3(_a3), a4(_a4), sigini(sigInit)
{
...
Steel02::Steel02(int tag,
                 ...
                 double _cR2):UniaxialMaterial(tag, MAT_TAG_Steel02),
...
Steel02::Steel02(int tag, double _Fy, double _E0,
                 double _b):UniaxialMaterial(tag, MAT_TAG_Steel02),
...
Steel02::Steel02(void):UniaxialMaterial(0, MAT_TAG_Steel02)
{
    EnergyP = 0;                //by SAJalali
    konP = 0;
}
...
UniaxialMaterial *Steel02::getCopy(void)
{
    Steel02 *theCopy =
        new Steel02(this->getTag(), Fy, E0, b, R0, cR1, cR2, a1, a2, a3,
                    a4,
                    sigini);
...
double Steel02::getInitialTangent(void)
{
    return E0;
}

int Steel02::setTrialStrain(double trialStrain, double strainRate)
{
    double Esh = b * E0;
    double epsy = Fy / E0;
...
double Steel02::getStrain(void)
{
    return eps;
}

double Steel02::getStress(void)
{
    return sig;
}

double Steel02::getTangent(void)
{
    return e;
}

int Steel02::commitState(void)
{
    epsminP = epsmin;
    epsmaxP = epsmax;
    epsplP = epspl;
...
int Steel02::revertToLastCommit(void)
{
    epsmin = epsminP;
    epsmax = epsmaxP;
    epspl = epsplP;
...
int Steel02::revertToStart(void)
{
    EnergyP = 0;                //by SAJalali
    eP = E0;
    epsP = 0.0;
...
int Steel02::sendSelf(int commitTag, Channel & theChannel)
{
...
int Steel02::recvSelf(int commitTag, Channel & theChannel,
...
void Steel02::Print(OPS_Stream & s, int flag)
{
...
int Steel02::setParameter(const char **argv, int argc, Parameter & param)
{
...
int Steel02::updateParameter(int parameterID, Information & info)
{
    switch (parameterID) {
    case -1:
        return -1;
