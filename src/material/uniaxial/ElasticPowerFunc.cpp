          ...
          if (OPS_GetDoubleInput (&numData, &eta) != 0)
            {
                ...
                opserr << "uniaxialMaterial ElasticPowerFunc: " << tag[0] <<
                    endln;
                return 0;
            }
          argc--;
...
ElasticPowerFunc::ElasticPowerFunc (int tag,
                                    const Vector & coeff, const Vector & exp,
                                    double et):
...
ElasticPowerFunc::ElasticPowerFunc ():UniaxialMaterial (0, MAT_TAG_ElasticPowerFunc),
...
int
ElasticPowerFunc::setTrialStrain (double strain, double strainRate)
{
    trialStrain = strain;
    ...
    for (int i = 0; i < numTerms; i++)
      {
          if (exponents (i) >= 0.0 || trialStrain != 0.0)
              trialStress +=
                  coefficients (i) * sgn (trialStrain) *
                  pow (fabs (trialStrain), exponents (i));
          else
              trialStress += 0.0;

          if (exponents (i) >= 1.0 || trialStrain != 0.0)
              trialTangent +=
                  coefficients (i) * exponents (i) * pow (fabs (trialStrain),
                                                          exponents (i) -
                                                          1.0);
          else
              trialTangent +=
                  coefficients (i) * pow (DBL_EPSILON, exponents (i) - 1.0);
      }
...
int
ElasticPowerFunc::revertToLastCommit ()
{
    return 0;
}
...
int
ElasticPowerFunc::revertToStart ()
{
    trialStrain = 0.0;
    trialStrainRate = 0.0;
    trialStress = 0.0;
    ...
    for (int i = 0; i < numTerms; i++)
      {
          if (exponents (i) >= 1.0)
              trialTangent +=
                  coefficients (i) * exponents (i) * pow (0.0,
                                                          exponents (i) -
                                                          1.0);
          else
              trialTangent +=
...
int
ElasticPowerFunc::sendSelf (int cTag, Channel & theChannel)
{
    int res = 0;
...
int
ElasticPowerFunc::recvSelf (int cTag, Channel & theChannel,
                            FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0)
    ...
    else
      {
          ...
          res +=
              theChannel.recvVector (this->getDbTag (), cTag, coefficients);
          res += theChannel.recvVector (this->getDbTag (), cTag, exponents);
          if (res < 0)
              opserr <<
                  "ElasticPowerFunc::recvSelf() - failed to recv arrays.\n";
      }
...
void
ElasticPowerFunc::Print (OPS_Stream & s, int flag)
{
