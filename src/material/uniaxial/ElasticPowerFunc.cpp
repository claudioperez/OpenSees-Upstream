...
void *OPS_ElasticPowerFunc()
{
    ...
    if (OPS_GetIntInput(&numData, tag) != 0) {
        opserr <<
            "WARNING invalid uniaxialMaterial ElasticPowerFunc tag\n";
        return 0;
    }
    ...
    theMaterial =
        new ElasticPowerFunc(tag[0], coefficients, exponents, eta);
...
ElasticPowerFunc::ElasticPowerFunc(int tag,
                                   ...
                                   double et):UniaxialMaterial(tag,
                                                               MAT_TAG_ElasticPowerFunc),
coefficients(coeff), exponents(exp), eta(et), numTerms(1),
initTangent(0.0), trialStrain(0.0), trialStrainRate(0.0), trialStress(0.0),
trialTangent(0.0)
{
