...
OPS_TakedaUnloadingRule (void)
{
    ...
    if (OPS_GetNumRemainingInputArgs () < 3)
      {
          opserr <<
              "Invalid number of args, want: unloadingRule Takeda(Ductility) tag? alpha? beta?"
              << endln;
          return 0;
      }
...
TakedaUnloadingRule::TakedaUnloadingRule (int tag, double c, double b):
UnloadingRule (tag, DEG_TAG_UNLOAD_Takeda), isNegative (false), C (c),
beta (b)
{
    this->revertToStart ();
                               ...
                               FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0)
      {
          opserr <<
              "TakedaUnloadingRule::recvSelf() - failed to receive data\n";
          this->setTag (0);
      }
    else
      {
