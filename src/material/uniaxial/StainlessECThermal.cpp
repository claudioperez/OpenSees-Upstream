...
OPS_StainlessECThermal(void)
{
    ...
    if (OPS_GetIntInput(&numData, iData) != 0) {
        opserr <<
            "WARNING invalid uniaxialMaterial StainlessECThermal tag?" <<
            endln;
        return 0;
    }
    //
...
StainlessECThermal::StainlessECThermal(int tag, int grade, double Fy,
                                       double E, double Fu,
                                       double
                                       sigInit):UniaxialMaterial(tag,
                                                                 MAT_TAG_StainlessECThermal),
gradeTag(grade)
{
...
int StainlessECThermal::setTrialStrain(double strain,
                                       double FiberTemperature,
                                       double strainRate)
{
...
int StainlessECThermal::setTrial(double strain, double &stress,
...
double StainlessECThermal::getThermalElongation(void)
{
...
double
    StainlessECThermal::getElongTangent(double TempT, double &ET,
                                        double &Elong, double TempTmax)
{
...
0.15,
    ...
    // grade 1.4462
        ...
        double EpsiUR[12] =
            { 0.2, 0.2, 0.2, 0.20, 0.20, 0.20, 0.15, 0.15, 0.15, 0.15,
0.15,
            0.15
        };
        for (int i = 0; i < 11; i++) {
...
//
#ifdef _dDEBUG
    opserr << "  TempT:  " << TempT << "  Ttangent: " << Ttangent <<
        "  E0:  " << E0 << endln;
#endif
    return 0;
}
...
// AddingSensitivity:BEGIN ///////////////////////////////////
int StainlessECThermal::setParameter(const char **argv, int argc,
                                     Parameter & param)
{
/*
...
int StainlessECThermal::updateParameter(int parameterID,
                                        Information & info)
{
/*	
	switch (parameterID) {
...
int StainlessECThermal::activateParameter(int passedParameterID)
{
...
double
    StainlessECThermal::getStressSensitivity(int gradIndex,
                                             bool conditional)
{
    ...
    if ((sigmaMax < sigmaElastic)
        && (fabs(sigmaMax - sigmaElastic) > 1e-5)) {
        Tstress = sigmaMax;
...
int StainlessECThermal::commitSensitivity(double TstrainSensitivity,
                                          int gradIndex, int numGrads)
{
    ...
    if ((sigmaMax < sigmaElastic)
        ...
        Tstress = sigmaElastic;
        gradient = CstressSensitivity + E0Sensitivity * (Tstrain - Cstrain)
...
int StainlessECThermal::getVariable(const char *variable, Information & info)   // is there need to get Ect and EpsiU?? MZ 07/16
{
