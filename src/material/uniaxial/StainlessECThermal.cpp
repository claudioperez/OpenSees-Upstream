...
OPS_StainlessECThermal (void)
{
    ...
    if (strcmp (gradeChar, "Grade14301") == 0)
      ...
      {
          opserr <<
              "WARNING invalid material grade for uniaxialMaterial StainlessECThermal "
              << iData[0] << endln;
          return 0;
      }
    numData = OPS_GetNumRemainingInputArgs ();
    if (numData != 3 && numData != 4)
      {
          opserr <<
              "Invalid #args, want: uniaxialMaterial StainlessECThermal " <<
...
//
    ...
    theMaterial =
        new StainlessECThermal (iData[0], iData[1], dData[0], dData[1],
                                dData[2], dData[3]);

    if (theMaterial == 0)
      {
          opserr <<
              "WARNING could not create uniaxialMaterial of type StainlessECThermal Material\n";
          return 0;
      }
...
StainlessECThermal::StainlessECThermal (int tag, int grade, double Fy,
                                        double E, double Fu, double sigInit):
UniaxialMaterial (tag, MAT_TAG_StainlessECThermal),
gradeTag (grade)
{
...
#endif
// default constructor
StainlessECThermal::StainlessECThermal ():UniaxialMaterial (0, MAT_TAG_StainlessECThermal), gradeTag (0), fyT (0.0),
E0T (0.0), fuT (0.0), sigini (0.0)
{
...
//
double
StainlessECThermal::determineYieldSurface (double sigini)       // Added by Mian Zhou 10/16
{
    ...
    if (fabsSigini < fyT)
    ...
    else if (fabsSigini = fyT)
      {
          if (sigini > 0)       // positive/tensile initial stress
            ...
            {                   //negative/compressive initial stress
                epsini = -0.02;
            }
      }
    else
      {
          opserr << "WARNING: Initial Stress Exceeds Plastic Yield strength "
              << endln;
      }
    return epsini;
}
...
int
StainlessECThermal::setTrialStrain (double strain, double FiberTemperature,
                                    double strainRate)
{
...
int
StainlessECThermal::setTrial (double strain, double &stress, double &tangent,
                              double strainRate)
{
    // Reset history variables to last converged state
    TminStrain = CminStrain;
...
void
StainlessECThermal::determineTrialState (double dStrain)
{
...
// calculating the Positive stress
    ...
    double BT =
        (1 - EpsiC * Ect / fy) * E0 * EpsiC / ((E0 * EpsiC / fy - 1) * fy);
    ...
    if (fabsTstrain <= EpsiC)
      {
          ...
          Ttangent =
              E0 * (1 + AT * Epsi_pow_b -
                    AT * BT * Epsi_pow_b) / ((1 + AT * Epsi_pow_b) * (1 +
                                                                      AT *
                                                                      Epsi_pow_b));
      }
    else if (fabsTstrain <= EpsiU)
      {
          ...
          Tstress =
              fy - ET + (DT / CT) * pow (CT * CT -
                                         (EpsiU_Epsi) * (EpsiU_Epsi), 0.5);
          Ttangent =
              DT * (EpsiU_Epsi) / (CT *
                                   pow (CT * CT - (EpsiU_Epsi) * (EpsiU_Epsi),
                                        0.5));
...
#ifdef _BDEBUG
    opserr << "Trial strain: " << Tstrain << "   tangent: " << Ttangent <<
        "   Tstress: " << Tstress << endln;
#endif
    Ttangent = 1e11;
    Ctemp = Ttemp;
...
*/
double
StainlessECThermal::getStrain ()
{
    return Tstrain;
}

double
StainlessECThermal::getStress ()
{
    return Tstress;
}

double
StainlessECThermal::getTangent ()
{
    return Ttangent;
}
...
double
StainlessECThermal::getElongTangent (double TempT, double &ET, double &Elong,
                                     double TempTmax)
{
...
// Grade 1.4301
    if (gradeTag == 1)
      {
          double FyR[12] =
              { 0.82, 0.68, 0.64, 0.60, 0.54, 0.49, 0.40, 0.27, 0.14, 0.06,
     0.03, 0.0 };
          double FuR[12] =
              { 0.87, 0.77, 0.73, 0.72, 0.67, 0.58, 0.43, 0.27, 0.15, 0.07,
     0.03, 0.0 };
          double E0R[12] =
              { 0.96, 0.92, 0.88, 0.84, 0.80, 0.76, 0.71, 0.63, 0.45, 0.20,
     0.10, 0.0 };
          double EctR[12] =
              { 0.05, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02,
    0.02, 0.02 };
          double EpsiUR[12] =
              { 0.4, 0.4, 0.4, 0.4, 0.4, 0.35, 0.3, 0.20, 0.2, 0.20, 0.20,
  0.20 };
          for (int i = 0; i < 11; i++)
            {
                FyRfactors[i] = FyR[i];
    ...
    // Grade 1.4401/1.4404
      ...
      {
          double FyR[12] =
              { 0.88, 0.76, 0.71, 0.66, 0.63, 0.61, 0.51, 0.40, 0.19, 0.10,
     0.005, 0.0 };
          double FuR[12] =
              { 0.93, 0.87, 0.84, 0.83, 0.79, 0.72, 0.55, 0.34, 0.18, 0.09,
     0.04, 0.0 };
          double E0R[12] =
              { 0.96, 0.92, 0.88, 0.84, 0.80, 0.76, 0.71, 0.63, 0.45, 0.20,
     0.10, 0.0 };
          double EctR[12] =
              { 0.049, 0.047, 0.045, 0.030, 0.025, 0.02, 0.02, 0.02, 0.02,
    0.02, 0.02, 0.02 };
          double EpsiUR[12] =
              { 0.4, 0.4, 0.4, 0.4, 0.4, 0.40, 0.30, 0.20, 0.2, 0.20, 0.20,
  0.20 };
          for (int i = 0; i < 11; i++)
            {
                FyRfactors[i] = FyR[i];
    ...
    // Grade 1.4571
      ...
      {
          double FyR[12] =
              { 0.89, 0.83, 0.77, 0.72, 0.69, 0.66, 0.59, 0.50, 0.28, 0.15,
     0.075, 0.0 };
          double FuR[12] =
              { 0.88, 0.81, 0.80, 0.80, 0.77, 0.71, 0.57, 0.38, 0.22, 0.11,
     0.055, 0.0 };
          double E0R[12] =
              { 0.96, 0.92, 0.88, 0.84, 0.80, 0.76, 0.71, 0.63, 0.45, 0.20,
     0.10, 0.0 };
          double EctR[12] =
              { 0.06, 0.05, 0.04, 0.030, 0.025, 0.02, 0.02, 0.02, 0.02, 0.02,
    0.02, 0.02 };
          double EpsiUR[12] =
              { 0.4, 0.4, 0.4, 0.4, 0.4, 0.35, 0.30, 0.20, 0.2, 0.20, 0.20,
  0.20 };
          for (int i = 0; i < 11; i++)
            {
                FyRfactors[i] = FyR[i];
    ...
    // grade 1.4003
      ...
      {
          double FyR[12] =
              { 1.0, 1.0, 0.98, 0.91, 0.80, 0.45, 0.19, 0.13, 0.10, 0.07,
     0.035, 0.0 };
          double FuR[12] =
              { 0.94, 0.88, 0.86, 0.83, 0.81, 0.42, 0.21, 0.12, 0.11, 0.09,
     0.045, 0.0 };
          double E0R[12] =
              { 0.96, 0.92, 0.88, 0.84, 0.80, 0.76, 0.71, 0.63, 0.45, 0.20,
     0.10, 0.0 };
          double EctR[12] =
              { 0.03, 0.03, 0.03, 0.030, 0.03, 0.03, 0.03, 0.03, 0.03, 0.03,
    0.03, 0.03 };
          double EpsiUR[12] =
              { 0.2, 0.2, 0.2, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15, 0.15,
  0.15 };
          for (int i = 0; i < 11; i++)
            {
                FyRfactors[i] = FyR[i];
    ...
    // grade 1.4462
      ...
      {
          double FyR[12] =
              { 0.91, 0.80, 0.75, 0.72, 0.65, 0.56, 0.37, 0.26, 0.10, 0.03,
     0.015, 0.0 };
          double FuR[12] =
              { 0.93, 0.85, 0.83, 0.82, 0.71, 0.57, 0.38, 0.29, 0.12, 0.04,
     0.02, 0.0 };
          double E0R[12] =
              { 0.96, 0.92, 0.88, 0.84, 0.80, 0.76, 0.71, 0.63, 0.45, 0.20,
     0.10, 0.0 };
          double EctR[12] =
              { 0.07, 0.037, 0.035, 0.030, 0.03, 0.025, 0.025, 0.025, 0.025,
    0.025, 0.025, 0.025 };
          double EpsiUR[12] =
              { 0.2, 0.2, 0.2, 0.20, 0.20, 0.20, 0.15, 0.15, 0.15, 0.15, 0.15,
  0.15 };
          for (int i = 0; i < 11; i++)
            {
                FyRfactors[i] = FyR[i];
      ...
      {
          opserr <<
              "WARNING StainlessECThermal received an invalid gradeTag: " <<
              gradeTag << endln;
      }
//
                ...
                if (i == 0)
                  {
                      ...
                      Ect =
                          E0T * (0.11 - TempT * (0.11 - EctRfactors[0]) / 80);
                      EpsiU =
                          EpsiUT - (EpsiUT - EpsiURfactors[0]) * TempT / 80;
                  }
                else if (i == 12)
                  {
                      opserr << "Warning:The temperature " << TempT <<
                          " for StainlessECThermal is out of range\n";
                      return -1;
                  }
                else
                  {
                      fy = fyT * (FyRfactors[i - 1] -
                                  (TempT + 20 -
                                   100 * i) * (FyRfactors[i - 1] -
                                               FyRfactors[i]) / 100);
                      fu = fuT * (FuRfactors[i - 1] -
                                  (TempT + 20 -
                                   100 * i) * (FuRfactors[i - 1] -
                                               FuRfactors[i]) / 100);
                      E0 = E0T * (E0Rfactors[i - 1] -
                                  (TempT + 20 -
                                   100 * i) * (E0Rfactors[i - 1] -
                                               E0Rfactors[i]) / 100);
                      Ect =
                          E0T * (EctRfactors[i - 1] -
                                 (TempT + 20 -
                                  100 * i) * (EctRfactors[i - 1] -
                                              EctRfactors[i]) / 100);
                      EpsiU =
                          EpsiURfactors[i - 1] - (TempT + 20 -
                                                  100 * i) *
                          (EpsiURfactors[i - 1] - EpsiURfactors[i]) / 100;
                  }
                break;
            }
      }
...
///*
    if (TempT <= 1)
    ...
    else if (TempT <= 1200)
      {
          ...
          ThermalElongation =
              (16 + 4.79e-3 * RealTemp -
               1.243e-6 * RealTemp * RealTemp) * TempT * 1e-6;
      }
    else
      {
...
//
#ifdef _dDEBUG
    opserr << "  TempT:  " << TempT << "  Ttangent: " << Ttangent << "  E0:  "
        << E0 << endln;
#endif
    return 0;
}
...
int
StainlessECThermal::commitState ()
{
    // History variables
...
int
StainlessECThermal::revertToLastCommit ()
{
...
int
StainlessECThermal::revertToStart ()
{
    // History variables
    CminStrain = 0.0;
    CmaxStrain = 0.0;
...
UniaxialMaterial *
StainlessECThermal::getCopy ()
{
    StainlessECThermal *theCopy =
        new StainlessECThermal (this->getTag (), gradeTag, fy, E0, fu,
                                sigini);
...
int
StainlessECThermal::sendSelf (int commitTag, Channel & theChannel)
{
    int res = 0;
...
int
StainlessECThermal::recvSelf (int commitTag, Channel & theChannel,
                              FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0)
      {
          opserr <<
              "StainlessECThermal::recvSelf() - failed to receive data\n";
          this->setTag (0);
      }
    else
      {
...
void
StainlessECThermal::Print (OPS_Stream & s, int flag)
{
...
// AddingSensitivity:BEGIN ///////////////////////////////////
int
StainlessECThermal::setParameter (const char **argv, int argc,
                                  Parameter & param)
{
/*
    ...
    if ((sigmaMax < sigmaElastic) && (fabs (sigmaMax - sigmaElastic) > 1e-5))
    ...
    else
      {
          ...
          gradient = CstressSensitivity
              + E0Sensitivity * (Tstrain - Cstrain) - E0 * CstrainSensitivity;
      }
    if (sigmaMin > Tstress)
      {
          gradient = E0Sensitivity * b * Tstrain
...
int
StainlessECThermal::commitSensitivity (double TstrainSensitivity,
...
int
StainlessECThermal::setTrialStrain (double strain, double strainRate)
{
    opserr <<
        "StainlessECThermal::setTrialStrain (double strain, double strainRate) - should never be called\n";
    return 0;
}
