...
void *OPS_IMKBilin(void)
{
    if (numIMKBilinMaterials == 0) {
        ...
        OPS_Error
            ("Mod. IMK Model with Bilinear Hysteretic Response - Code by A. ELKADY\n",
             1);
    }
    ...
    if (OPS_GetDoubleInput(&numData, dData) != 0) {
        ...
        opserr <<
            "Theta_p_pos? Theta_pc_pos? Theta_u_pos? Mpe_pos? MmaxMpe_pos? ResM_pos? ";
        opserr <<
            "Theta_p_neg? Theta_pc_neg? Theta_u_neg? Mpe_neg? MmaxMpe_neg? ResM_neg? ";
    ...
    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type IMKBilin Material\n";
        return 0;
    }
...
IMKBilin::IMKBilin(int tag, double p_Ke,
                   double p_Theta_p_pos0, double p_Theta_pc_pos0,
                   double p_Theta_u_pos0, double p_Mpe_pos0,
                   double p_MmaxMpe_pos0, double p_ResM_pos0,
                   double p_Theta_p_neg0, double p_Theta_pc_neg0,
                   double p_Theta_u_neg0, double p_Mpe_neg0,
                   double p_MmaxMpe_neg0, double p_ResM_neg0,
                   double p_LAMBDA_S, double p_LAMBDA_C, double p_LAMBDA_K,
                   double p_c_S, double p_c_C, double p_c_K,
                   double p_D_pos, double p_D_neg):UniaxialMaterial(tag,
                                                                    0),
Ke(p_Ke), Theta_p_pos0(p_Theta_p_pos0), Theta_pc_pos0(p_Theta_pc_pos0),
Theta_u_pos0(p_Theta_u_pos0), Mpe_pos0(p_Mpe_pos0),
MmaxMpe_pos0(p_MmaxMpe_pos0), ResM_pos0(p_ResM_pos0),
Theta_p_neg0(p_Theta_p_neg0), Theta_pc_neg0(p_Theta_pc_neg0),
Theta_u_neg0(p_Theta_u_neg0), Mpe_neg0(p_Mpe_neg0),
MmaxMpe_neg0(p_MmaxMpe_neg0), ResM_neg0(p_ResM_neg0), LAMBDA_S(p_LAMBDA_S),
LAMBDA_C(p_LAMBDA_C), LAMBDA_K(p_LAMBDA_K), c_S(p_c_S), c_C(p_c_C),
c_K(p_c_K), D_pos(p_D_pos), D_neg(p_D_neg)
{
    this->revertToStart();
}

IMKBilin::IMKBilin():UniaxialMaterial(0, 0), Ke(0),
Theta_p_pos0(0), Theta_pc_pos0(0), Theta_u_pos0(0), Mpe_pos0(0),
MmaxMpe_pos0(0), ResM_pos0(0), Theta_p_neg0(0), Theta_pc_neg0(0),
Theta_u_neg0(0), Mpe_neg0(0), MmaxMpe_neg0(0), ResM_neg0(0), LAMBDA_S(0),
LAMBDA_C(0), LAMBDA_K(0), c_S(0), c_C(0), c_K(0), D_pos(0), D_neg(0)
{
    this->revertToStart();
}
...
int
 IMKBilin::setTrialStrain(double strain, double strainRate)
{
    ...
    double K_j;
    double Ki, slope_pi, slope_pci, Theta_yi, Theta_maxi, Mmaxi,
        MpeProjecti, MmaxProjecti, Theta_pci;
    ...
    if (Reversal_Flag == 1) {
        ...
        DISP_Rev =
            Energy_total - Energy_Excrsni_1 +
            0.5 * Mreversal * (Rintrsct_K - Rreversal);
        // Update Loading/Unloading Stiffness
        beta_K_j =
            pow((DISP_Rev /
                 (2 * Ref_Energy_K - Energy_total +
                  0.5 * Mreversal * (Rintrsct_K - Rreversal))), c_K);
...
/* 		// Update Paramters for Smooth Transition Option
    ...
    if (Excursion_Flag == 1.0) {
        beta_S_j =
            pow((Energy_Excrsn / (Ref_Energy_S - Energy_total)), c_S);
        beta_C_j =
            pow((Energy_Excrsn / (Ref_Energy_C - Energy_total)), c_C);

        if (Ri - Ri_1 >= 0.0) {
            //  Update Mpe, Mmax Projection, slope_p, and slope_pc for Current Step
            Mpe_pos_j = Mpe_pos_j_1 * (1.0 - beta_S_j * D_pos);
            MmaxProject_pos_j =
                MmaxProject_pos_j_1 * (1.0 - beta_C_j * D_pos);
            slope_p_pos_j = slope_p_pos_j_1 * (1.0 - beta_S_j * D_pos);
            if (Mr_pos0 == 0.0) {
                slope_pc_pos_j =
                    slope_pc_pos0 * (MmaxProject_pos_j -
                                     Mr_pos0) / MmaxProject_pos_j;
            } else {
                slope_pc_pos_j =
                    slope_pc_pos0 * (Mpe_pos_j - Mr_pos0) / (Mpe_pos0 -
                                                             Mr_pos0);
            }
            ...
            Theta_max_pos_j =
                fabs((MmaxProject_pos_j -
                      MpeProject_pos_j) / (slope_pc_pos_j +
                                           slope_p_pos_j));
            Mmax_pos_j =
                MpeProject_pos_j + Theta_max_pos_j * slope_p_pos_j;
        } else {
            //  Update Mpe, Mmax Projection, slope_p, and slope_pc for Current Step
            Mpe_neg_j = Mpe_neg_j_1 * (1.0 - beta_S_j * D_neg);
            MmaxProject_neg_j =
                MmaxProject_neg_j_1 * (1.0 - beta_C_j * D_neg);
            slope_p_neg_j = slope_p_neg_j_1 * (1.0 - beta_S_j * D_neg);
            if (Mr_neg0 == 0.0) {
                slope_pc_neg_j =
                    slope_pc_neg0 * (MmaxProject_neg_j -
                                     Mr_neg0) / MmaxProject_neg_j;
            } else {
                slope_pc_neg_j =
                    slope_pc_neg0 * (Mpe_neg_j - Mr_neg0) / (Mpe_neg0 -
                                                             Mr_neg0);
            }
            ...
            Theta_max_neg_j =
                fabs((MmaxProject_neg_j -
                      MpeProject_neg_j) / (slope_pc_neg_j +
                                           slope_p_neg_j));
            Mmax_neg_j =
                MpeProject_neg_j + Theta_max_neg_j * slope_p_neg_j;
        }
...
//      ///////////////////////////////////////////////////////////////////////
    ...
    if (Excursion_Flag == 1) {
        if ((Energy_total >= Ref_Energy_S)
            || (Energy_total >= Ref_Energy_C)) {
            Energy_Flag = 1;
        }
...
int IMKBilin::revertToStart(void)
{
    ...
    MpeProject_pos_j_1 =
        Mmax_pos_j_1 - slope_p_pos_j_1 * Theta_max_pos_j_1;
    MmaxProject_pos_j_1 =
        Mmax_pos_j_1 + slope_pc_pos_j_1 * Theta_max_pos_j_1;
    ...
    MpeProject_neg_j_1 =
        Mmax_neg_j_1 - slope_p_neg_j_1 * Theta_max_neg_j_1;
    MmaxProject_neg_j_1 =
        Mmax_neg_j_1 + slope_pc_neg_j_1 * Theta_max_neg_j_1;
...
UniaxialMaterial *IMKBilin::getCopy(void)
{
    IMKBilin *theCopy = new IMKBilin(this->getTag(), Ke,
                                     Theta_p_pos0, Theta_pc_pos0,
                                     Theta_u_pos0, Mpe_pos0, MmaxMpe_pos0,
                                     ResM_pos0,
                                     Theta_p_neg0, Theta_pc_neg0,
                                     Theta_u_neg0, Mpe_neg0, MmaxMpe_neg0,
                                     ResM_neg0,
                                     LAMBDA_S, LAMBDA_C, LAMBDA_K, c_S,
                                     c_C,
                                     c_K, D_pos, D_neg);
...
int IMKBilin::recvSelf(int cTag, Channel & theChannel,
