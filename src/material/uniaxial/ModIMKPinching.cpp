...
OPS_ModIMKPinching()
{
    ...
    if (OPS_GetIntInput(&numData, iData) != 0) {
        opserr << "WARNING invalid uniaxialMaterial  ModIMKPinching tag" <<
            endln;
        return 0;
    }
    ...
    if (numData != 27 && numData != 26) {
        opserr <<
            "Invalid Args want: uniaxialMaterial ModIMKPinching tag? Ke?, alfaPos?, alfaNeg?, My_pos?, My_neg?";
        opserr <<
...
ModIMKPinching::ModIMKPinching(int tag, double ke0, double alfanPos, double alfanNeg, double my_pos, double my_neg,     // Updated: Filipe Ribeiro and Andre Barbosa
                               double fprPos, double fprNeg, double a_pinch, double ls, double ld, double la, double lk, double cs, double cd, double ca, double ck, double thetaPpos, double thetaPneg, double thetaPCpos, double thetaPCneg, double resfacPos, double resfacNeg, double fracDispPos, double fracDispNeg, double dPos, double dNeg):UniaxialMaterial(tag, MAT_TAG_ModIMKPinching), Ke0(ke0), AlfanPos(alfanPos), AlfanNeg(alfanNeg), My_pos(my_pos), My_neg(my_neg),   // Updated: Filipe Ribeiro and Andre Barbosa
FprPos(fprPos),
FprNeg(fprNeg),
A_pinch(a_pinch),
Ls(ls),
...
ModIMKPinching::ModIMKPinching():UniaxialMaterial(0, MAT_TAG_ModIMKPinching), Ke0(0.0), AlfanPos(0.0),
...
UniaxialMaterial *ModIMKPinching::getCopy(void)
{
    ModIMKPinching *theCopy =
        new ModIMKPinching(this->getTag(), Ke0, AlfanPos, AlfanNeg, My_pos,
                           My_neg, FprPos, FprNeg, A_pinch, Ls, Ld, La, Lk,
...
int ModIMKPinching::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
...
int ModIMKPinching::recvSelf(int cTag, Channel & theChannel,
...
void ModIMKPinching::Print(OPS_Stream & s, int flag)
{
...
void ModIMKPinching::envelPosCap2(double fy, double alphaPos,
...
void ModIMKPinching::envelNegCap2(double fy, double alphaNeg,
                                  double alphaCap, double cpDsp, double &d,
                                  double &f, double &ek, double elstk,
