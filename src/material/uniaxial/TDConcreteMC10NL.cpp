...
OPS_TDConcreteMC10NL ()
{
    ...
    if (numTDConcreteMC10NL == 0)
      {
          opserr <<
              "Time-Dependent Concrete Material Model - Written by Nikola Tosic, 2019 \n";
    ...
    if (numArgs == 19)
      {
          ...
          if (OPS_GetIntInput (&numData, &iData) != 0)
            {
                opserr <<
                    "WARNING: invalid uniaxialMaterial TDConcreteMC10NL tag\n";
                return 0;
            }
          ...
          //ntosic
          theMaterial =
              new TDConcreteMC10NL (iData, dData[0], dData[1], dData[2],
          ...
          if (theMaterial == 0)
            {
                opserr <<
                    "WARNING: could not create uniaxialMaterial of type TDConcreteMC10NL \n";
                return 0;
            }
...
TDConcreteMC10NL::TDConcreteMC10NL (int tag, double _fc, double _fcu,
                                    double _epscu, double _ft, double _Ec,
                                    double _Ecm, double _beta, double _age,
                                    double _epsba, double _epsbb,
                                    double _epsda, double _epsdb,
                                    double _phiba, double _phibb,
                                    double _phida, double _phidb,
                                    double _tcast, double _cem):
...
TDConcreteMC10NL::setPhiBasic (double time, double tp)
{
    ...
    double phiBasic =
        phiba * log (pow (30.0 / tpa + 0.035, 2.0) * (tmtp / phibb) + 1.0);
    return phiBasic;
}
...
TDConcreteMC10NL::setPhiDrying (double time, double tp)
{
    ...
    double tpa = tp * pow (9.0 / (2.0 + pow (tp, 1.2)) + 1.0, cem);
    double phiDrying =
        phida / (0.1 + pow (tpa, 0.2)) * pow (tmtp,
                                              1.0 / (2.3 +
                                                     3.5 / pow (tpa,
                                                                0.5))) /
        pow (phidb + tmtp, 1.0 / (2.3 + 3.5 / pow (tpa, 0.5)));
    return phiDrying;
}
...
TDConcreteMC10NL::commitState (void)
{
    ...
    if (fabs (eps_m / sig) > Ec)
      {                         //ntosic: originaly was eps_m/sig
          E_i[count + 1] = Ec;
      }
    else
      {
...
{
    s << "TDConcreteMC10NL:(strain, stress, tangent) " << eps << " " << sig <<
        " " << e << endln;
}
