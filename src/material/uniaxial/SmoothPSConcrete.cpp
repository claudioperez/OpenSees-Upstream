...
void *OPS_SmoothPSConcrete()
{
    ...
    if (OPS_GetIntInput(&numdata, &tag) < 0) {
        opserr <<
            "WARNING invalid uniaxialMaterial SmoothPSConcrete tag\n";
        return 0;
    }
    // double fu, Ec, fc;
    double data[3];
...
SmoothPSConcrete::SmoothPSConcrete
    (int tag, double FPC, double FPCU, double EC, double EPSC0,
     double EPSCU, double ETA):UniaxialMaterial(tag,
                                                MAT_TAG_SmoothPSConcrete),
    ...
    if (Tflag == -1) {          // virgin material
        ...
        if (Tepsc >= Tepsp + eta * (Tepsp - Tepsr)) {
            ...
            Transition_p(eta * (Tepsp - Tepsr));
                 ...
                 && (Tepsc >= Tepsr + eta * (Tepsp - Tepsr))) { //flag = 1;
            Tsig = TEur * (Tepsc - Tepsp);
            TEt = TEur;
...
int SmoothPSConcrete::setTrial(double strain, double &stress,
                               ...
                               FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0) {
        opserr <<
            "SmoothPSConcrete::recvSelf() - failed to receive data\n";
        this->setTag(0);
    } else {
...
// AddingSensitivity:BEGIN ///////////////////////////////////
int SmoothPSConcrete::setParameter(const char **argv, int argc,
                                   Parameter & param)
{
...
int SmoothPSConcrete::updateParameter(int parameterID, Information & info)
{
    switch (parameterID) {
    case 1:
...
int SmoothPSConcrete::activateParameter(int passedParameterID)
{
...
double
    SmoothPSConcrete::getStressSensitivity(int gradNumber,
                                           bool conditional)
{
        ...
        if (flag == -1) {       // % virgin material
            ...
            if ((epsc > epsr) & (epsc < epsr + eta * (epsp - epsr))) {
                ...
                depsr1dh =
                    ...
                    detadh * (epsp - epsr);
                dsigr1dh =
                    dEurdh * (epsr1 - epsp) + Eur * (depsr1dh - depspdh);
                depsr2dh =
                    depsrdh - eta * (depspdh - depsrdh) -
                    detadh * (epsp - epsr);
                dsigr2dh =
...
int SmoothPSConcrete::Monotonic_Envelope(double epsc, double *sig,
                                         double *Et)
{
...
int SmoothPSConcrete::Compute_epsp(void)
{
    ...
    // +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
int SmoothPSConcrete::Transition_r(double epsc, double e1, double e2,
                                   double s1, double s2, double e1th,
                                   double e2th, double Et1, double Et2,
                                   double *sig, double *Et)
{
    ...
    double b =
        (-(Et2 + 2 * Et1) * (e2 - e1) +
         3 * (s2 - s1)) / ((e2 - e1) * (e2 - e1));
    double a =
        (Et2 - Et1 - 2 * b * (e2 - e1)) / 3. / ((e2 - e1) * (e2 - e1));
    //c = Et1;
                                              ...
                                              double dEcdh, double depsdh)
{
    ...
    if (epsilon > -eps0) {      //         % ascending branch in compression (Popovics)
        ...
        double Et =
            -(1 / eps0) * fc * K * (-1 - D * pow(Eta, r) + D * pow(Eta,
                                                                   r) *
                                    r) / (pow((1 + D * pow(Eta,
                                                           r)),
                                              2));
...
double
 SmoothPSConcrete::Compute_depspdh(double epsr, double sigr,
                                   double depsrdh, double dsigrdh,
                                   double deps0dh, double dfcdh,
                                   double dEcdh)
{
...
double
    SmoothPSConcrete::Transition_r_sens(double epsc, double e1, double e2,
                                        double s1, double s2, double e1th,
                                        double e2th, double Et1,
                                        double Et2, double depsdh,
                                        double de1dh, double de2dh,
                                        double ds1dh, double ds2dh,
                                        ...
                                        double dEcdh)
{
    ...
    double a =
        1. / 3 * (Et2 - Et1 - 2 * b * (e2 - e1)) / pow((e2 - e1), 2);
...
double
 SmoothPSConcrete::Transition_p_sens(double epsc, double epsp,
                                     double Delta, double Eur,
                                     double depsdh, double depspdh,
                                     double dDeltadh, double dEurdh)
{
    //  %Et1 = 0; Et2 = Eur; s1 = 0;
    ...
    double dsigdh = dadh * pow((epsc - e1),
                               3) + dbdh * (epsc - e1) * (epsc - e1) +
        Et * (depsdh - de1dh);
...
double
    SmoothPSConcrete::Monotonic_Envelope_Et_sens(double epsilon,
                                                 double depsdh,
                                                 double dfcdh,
                                                 double deps0dh,
                                                 double depsudh,
                                                 ...
                                                 double dEcdh)
{
    ...
    if (epsilon > -eps0) {      // % ascending branch in compression (Popovics)
        ...
        double dt2 = 2 * (1 + D * pow(Eta, r)) * pow(Eta,
                                                     r) * (dDdh +
                                                           drdh *
                                                           log(Eta) +
                                                           r / Eta *
        ...
        double C = K * (Ksig - 1) / pow((Keps - 1), 2) - 1 / Keps;
        double dCdh = dKdh * (Ksig - 1) / pow((Keps - 1),
                                              2) +
            dKsig * K / pow((Keps - 1),
                            2) - 2 * K * (Ksig -
                                          1) * dKeps / pow((Keps - 1),
 ...
 SmoothPSConcrete::compute_epsmax(double *epsmax, double *sigmax)
{
    ...
    (*sigmax) =
        -fc * K * emax / (1 + A * emax + B * emax * emax +
                          C * pow(emax, 3));
    return 0;
}
