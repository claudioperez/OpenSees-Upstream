...
void *OPS_SAWSMaterial(void)
{
    ...
    if (OPS_GetIntInput(&numData, iData) != 0) {
        opserr << "WARNING invalid uniaxialMaterial SAWSMaterial tag" <<
            endln;
        return 0;
    }
    ...
    if (OPS_GetDoubleInput(&numData, dData) != 0) {
        opserr <<
            "Invalid Args want: uniaxialMaterial SAWS tag? F0? FI? dU? S0?"
            << endln;
...
SAWSMaterial::SAWSMaterial(int tag,
                           ...
                           double A, double B):UniaxialMaterial(tag,
                                                                MAT_TAG_SAWSMaterial),
F0(f0), FI(fI), DU(dU), S0(s0), R1(r1), R2(r2), R3(r3), R4(r4), ALPHA(A),
BETA(B)
{
...
SAWSMaterial::SAWSMaterial():UniaxialMaterial(0, MAT_TAG_SAWSMaterial),
F0(0.0), FI(0.0), DU(0.0), S0(0.0), R1(0.0), R2(0.0), R3(0.0),
R4(0.0), ALPHA(0.0), BETA(0.0)
{
...
double SAWSMaterial::getStrain(void)
{
    return DISPL;
}

double SAWSMaterial::getStress(void)
{
    return FORCE;
}

double SAWSMaterial::getTangent(void)
{
    return STIFF;
}


double SAWSMaterial::getInitialTangent(void)
{
    return S0;
}

int SAWSMaterial::commitState(void)
{
    cDISPL = DISPL;
    cFORCE = FORCE;
    cSTIFF = STIFF;
...
int SAWSMaterial::revertToLastCommit(void)
{
    DISPL = cDISPL;
    FORCE = cFORCE;
    STIFF = cSTIFF;
...
int SAWSMaterial::revertToStart(void)
{
    DISPL = 0.0;
    FORCE = 0.0;
    STIFF = S0;
...
UniaxialMaterial *SAWSMaterial::getCopy(void)
{
...
int SAWSMaterial::sendSelf(int commitTag, Channel & theChannel)
{
    int res = 0;
...
int SAWSMaterial::recvSelf(int commitTag, Channel & theChannel,
...
void SAWSMaterial::Print(OPS_Stream & s, int flag)
{
