...
void *OPS_Concrete02()
{
    ...
    if (OPS_GetIntInput(&numData, iData) != 0) {
        opserr << "WARNING invalid uniaxialMaterial Concrete02 tag" <<
            endln;
        return 0;
    }
    ...
    if (numData != 7) {
        opserr << "Invalid #args, want: uniaxialMaterial Concrete02 " <<
    ...
    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type Concrete02 Material\n";
        return 0;
    }
...
Concrete02::Concrete02(int tag, double _fc, double _epsc0, double _fcu,
                       double _epscu, double _rat, double _ft,
...
Concrete02::Concrete02(void):UniaxialMaterial(0, MAT_TAG_Concrete02)
{
...
UniaxialMaterial *Concrete02::getCopy(void)
{
    Concrete02 *theCopy =
        new Concrete02(this->getTag(), fc, epsc0, fcu, epscu, rat, ft,
                       Ets);

    return theCopy;
}

double Concrete02::getInitialTangent(void)
{
    return 2.0 * fc / epsc0;
}

int Concrete02::setTrialStrain(double trialStrain, double strainRate)
{
...
double Concrete02::getStrain(void)
{
    return eps;
}

double Concrete02::getStress(void)
{
    return sig;
}

double Concrete02::getTangent(void)
{
    return e;
}

int Concrete02::commitState(void)
{
    ecminP = ecmin;
    deptP = dept;
...
int Concrete02::revertToLastCommit(void)
{
    ecmin = ecminP;;
    dept = deptP;
...
int Concrete02::revertToStart(void)
{
    ecminP = 0.0;
    deptP = 0.0;
...
int Concrete02::sendSelf(int commitTag, Channel & theChannel)
{
...
int Concrete02::recvSelf(int commitTag, Channel & theChannel,
...
void Concrete02::Print(OPS_Stream & s, int flag)
{
    if (flag == OPS_PRINT_PRINTMODEL_MATERIAL) {
        s << "Concrete02:(strain, stress, tangent) " << eps << " " << sig
            << " " << e << endln;
    }
...
void Concrete02::Tens_Envlp(double epsc, double &sigc, double &Ect)
{
/*-----------------------------------------------------------------------
...
void Concrete02::Compr_Envlp(double epsc, double &sigc, double &Ect)
{
/*-----------------------------------------------------------------------
...
int Concrete02::getVariable(const char *varName, Information & theInfo)
{
