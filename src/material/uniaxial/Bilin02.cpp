...
Bilin02::Bilin02 (int tag, double p_Ke0, double p_AsPos, double p_AsNeg,
                  double p_My_pos, double p_My_neg, double p_LamdaS,
                  double p_LamdaD, double p_LamdaA, double p_LamdaK,
                  double p_Cs, double p_Cd, double p_Ca, double p_Ck,
                  double p_Thetap_pos, double p_Thetap_neg,
                  double p_Thetapc_pos, double p_Thetapc_neg, double p_KPos,
                  double p_KNeg, double p_Thetau_pos, double p_Thetau_neg,
                  double p_PDPlus, double p_PDNeg, double p_nFactor):
UniaxialMaterial (tag, MAT_TAG_Bilin02),
Ke0 (p_Ke0),
AsPos (p_AsPos),
AsNeg (p_AsNeg),
My_pos (p_My_pos),
...
Ca (p_Ca),
Cd (p_Cd),
Thetap_pos (p_Thetap_pos),
Thetap_neg (p_Thetap_neg),
Thetapc_pos (p_Thetapc_pos),
Thetapc_neg (p_Thetapc_neg),
KPos (p_KPos),
KNeg (p_KNeg),
Thetau_pos (p_Thetau_pos),
Thetau_neg (p_Thetau_neg),
PDPlus (p_PDPlus),
PDNeg (p_PDNeg),
...
Bilin02::Bilin02 (int tag, double p_Ke0, double p_AsPos, double p_AsNeg,
                  ...
                  double p_Thetapc_pos, double p_Thetapc_neg, double p_KPos,
                  double p_KNeg, double p_Thetau_pos, double p_Thetau_neg,
                  double p_PDPlus, double p_PDNeg):
UniaxialMaterial (tag, MAT_TAG_Bilin02),
Ke0 (p_Ke0),
AsPos (p_AsPos),
AsNeg (p_AsNeg),
...
Thetau_neg (p_Thetau_neg),
PDPlus (p_PDPlus),
PDNeg (p_PDNeg)                 // Updated: Filipe Ribeiro and Andre Barbosa
{
    //initialize variables
    this->revertToStart ();
    //this->revertToLastCommit();
...
Bilin02::Bilin02 ():UniaxialMaterial (0, MAT_TAG_Bilin02), Ke0 (0), AsPos (0), AsNeg (0), My_pos (0), My_neg (0), LamdaS (0), LamdaD (0), LamdaA (0), LamdaK (0), Cs (0), Cd (0), Ca (0), Ck (0), Thetap_pos (0), Thetap_neg (0), Thetapc_pos (0), Thetapc_neg (0), KPos (0), KNeg (0), Thetau_pos (0), Thetau_neg (0), PDPlus (0), PDNeg (0), nFactor (0)
                                                                                                                                                                                                                                                                                                                                        // Updated: Filipe Ribeiro and Andre Barbosa
{
    this->revertToStart ();
}
...
Bilin02::setTrialStrain (double strain, double strainRate)
{
    ...
    if (kon == 0)
      {
          ...
          fPeakPos =
              My_pos + ekhardPos * ((Thetap_pos + My_pos / Ke) - My_pos / Ke);
          fPeakNeg =
    ...
    if (deltaD >= 0.0)
          ...
          if (kon == 2)
            {
                ...
                if ((LamdaK != 0.0) && (d < sp)
                    && fabs (capSlope * (1 + nFactor)) >= 1.0e-3
                    && (fabs (capSlopeNeg * (1 + nFactor)) >= 1.0e-3))
                  {             // Updated: Filipe Ribeiro and Andre Barbosa
                      ...
                      if (ekunload <= (0.1 / (1 + nFactor * (1 - 0.1))) * Ke)
                        {       // Updated: Filipe Ribeiro and Andre Barbosa
                      ...
                      if (cpPos <= dyPos)
                      ...
                      else if (cpPos > dyPos)
                        {
                            interPoint (xDevPos1, yDevPos1, sn, resSn,
                                        ...
                                        capSlope * Ke);
                            if (d <= sn && d <= xDevPos1)
                              { // Unloading in positive loading direction
                                  ek = ekunload;
                                  f = fP + ek * deltaD;
                            ...
                            if (flagControlResponse == 1)
                              { // Response has to be zero since post capping regions hit zero
                                  f = 1.0e-10;
                              }
    ...
    else
      {
                ...
                if ((LamdaK != 0.0) && (d > sn) && (flagstopdeg != 1)
                    && (flagdeg != 1))
                  {
                      ...
                      if (flagstopdeg != 1 && flagdeg != 1)
                        ...
                        {       // Keep the same unloading stiffness
                            ekunload = ekexcurs;
                        }
                      if (ekunload <= (0.1 / (1 + nFactor * (1 - 0.1))) * Ke)
                        {       // Updated: Filipe Ribeiro and Andre Barbosa
          ...
          else if (((iNoFneg == 1) && (iNoFpos == 1) && (d <= (-Thetau_neg)))
                   || flagControlResponse == 1)
            {
                f = -1.0e-10;
                ek = 1.0e-7;
          ...
          if (deltaD < 0.0)
            {
                ...
                if (fyNeg >= KNeg * My_neg)
                  {             // If strength drops below residual
                      fyNeg = KNeg * My_neg;
                      alphaNeg = 10 ^ (-4);
                      fCapRefNeg = fyNeg;
          ...
          else
            {
                ...
                if (fyPos <= KPos * My_pos)
                  {             //% If yield Strength Pos drops below residual
                      fyPos = KPos * My_pos;
                      alphaPos = pow (10.0, -4);
                      fCapRefPos = fyPos;
                      capSlope = -pow (10.0, -6);
    ...
    if ((interup == 1 && (ek == Ke * alphaPos)) || (ek == Ke * alphaNeg)
        || (ek == capSlope * Ke) || (ek == capSlopeNeg * Ke))
      {                         //fmk
          interup = 0;
      }
...
double
Bilin02::getStress (void)
{
    return (fP);
}

double
Bilin02::getTangent (void)
{
...
double
Bilin02::getInitialTangent (void)
{
    return (Ke);
}
...
int
Bilin02::revertToLastCommit (void)
{
...
int
Bilin02::sendSelf (int cTag, Channel & theChannel)
{
    int res = 0;
...
int
Bilin02::recvSelf (int cTag, Channel & theChannel,
...
//my functions
void
Bilin02::interPoint (double &xInt, double &yInt, double x1, double y1,
...
void
Bilin02::envelPosCap2 (double fy, double alphaPos, double alphaCap,
...
void
Bilin02::envelNegCap2 (double fy, double alphaNeg, double alphaCap,
                       double cpDsp, double &d, double &f, double &ek,
