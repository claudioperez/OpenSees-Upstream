...
void *OPS_Steel2(void)
{
    ...
    if (numData != 3 && numData != 6 && numData != 10 && numData != 11) {
        opserr << "Invalid #args, want: uniaxialMaterial Steel2 " <<
    ...
    if (numData == 3) {
        if (OPS_GetDoubleInput(&numData, dData) != 0) {
            opserr << "Invalid arggs: uniaxialMaterial Steel2 " <<
                iData[0] << " fy? E? b? <R0? cR1? cR2? <a1? a2? a3? a4?>>"
                << endln;
            return 0;
        }
        ...
        // Parsing was successful, allocate the material
        theMaterial =
            new Steel2(iData[0], dData[0], dData[1], dData[2], dData[3],
                       dData[4], dData[5]);
    ...
    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type Steel2 Material\n";
        return 0;
    }
...
Steel2::Steel2(int tag,
               ...
               double sigInit):UniaxialMaterial(tag, MAT_TAG_Steel2),
Fy(_Fy),
E0(_E0),
b(_b),
R0(_R0),
...
Steel2::Steel2(int tag,
               double _Fy, double _E0, double _b,
               double _R0, double _cR1, double _cR2):UniaxialMaterial(tag,
                                                                      MAT_TAG_Steel2),
...
Steel2::Steel2(int tag, double _Fy, double _E0,
               double _b):UniaxialMaterial(tag, MAT_TAG_Steel2), Fy(_Fy),
E0(_E0), b(_b), sigini(0.0)
{
...
Steel2::Steel2(void):UniaxialMaterial(0, MAT_TAG_Steel2)
{
    Fy = 0.0;
    E0 = 0.0;
    b = 0.0;
...
UniaxialMaterial *Steel2::getCopy(void)
{
    Steel2 *theCopy =
        new Steel2(this->getTag(), Fy, E0, b, R0, cR1, cR2, a1, a2, a3, a4,
                   sigini);

    return theCopy;
}

double Steel2::getInitialTangent(void)
{
    return E0;
}

int Steel2::setTrialStrain(double trialStrain, double strainRate)
{
    double Esh = b * E0;
    double epsy = Fy / E0;
...
double Steel2::getStrain(void)
{
    return eps;
}

double Steel2::getStress(void)
{
    return sig;
}

double Steel2::getTangent(void)
{
    return e;
}

int Steel2::commitState(void)
{
    epsminP = epsmin;
    epsmaxP = epsmax;
...
int Steel2::revertToLastCommit(void)
{
    epsmin = epsminP;
    epsmax = epsmaxP;
...
int Steel2::sendSelf(int commitTag, Channel & theChannel)
{
...
int Steel2::recvSelf(int commitTag, Channel & theChannel,
...
void Steel2::Print(OPS_Stream & s, int flag)
{
    s << "Steel2:(strain, stress, tangent) " << eps << " " << sig << " " <<
        e << endln;
}
