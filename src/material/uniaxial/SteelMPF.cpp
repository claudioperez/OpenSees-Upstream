...
// Default Constructor
SteelMPF::SteelMPF(int tag, double FYp, double FYn, double E, double Bp, double Bn, double Rr, double A1, double A2):UniaxialMaterial(tag, MAT_TAG_SteelMPF), sigyieldp(FYp), sigyieldn(FYn), E0(E), bp(Bp), bn(Bn), R0(Rr), aa1(A1), a2(A2), a3(0.0), a4(1.0), a5(0.0), a6(1.0)        // default values for strain hardening
{
...
SteelMPF::SteelMPF
    (int tag, double FYp, double FYn, double E, double Bp, double Bn,
     ...
     double A6):UniaxialMaterial(tag, MAT_TAG_SteelMPF), sigyieldp(FYp),
sigyieldn(FYn), E0(E), bp(Bp), bn(Bn), R0(Rr), aa1(A1), a2(A2), a3(A3),
a4(A4), a5(A5), a6(A6)
{
    // Sets all history and state variables to initial values
...
int SteelMPF::setTrial(double strain, double &stress, double &tangent,
                       double strainRate)
{
    // Reset history variables to last converged state
    inc = incold;
...
void SteelMPF::determineTrialState(double def)
{
    ...
    if (incold == 0) {
        ...
        if (inc == 1) {
            ...
            double sigstp =
                bp * estp + ((1.0 - bp) / pow((1.0 + pow(estp, Rp)),
                                              (1.0 / Rp))) * estp;
            ...
            double sigstn =
                bn * estn + ((1.0 - bn) / pow((1.0 + pow(estn, Rn)),
                                              (1.0 / Rn))) * estn;
        ...
        if (incold == 1) {
            ...
            if (e < eold) {
                ...
                double estn = (e - ern) / (e0n - ern);
                double sigstn =
                    bn * estn + ((1.0 - bn) / pow((1 + pow(estn, Rn)),
                                                  (1.0 / Rn))) * estn;
                ...
                double estp = (e - erp) / (e0p - erp);
                double sigstp =
                    bp * estp + ((1.0 - bp) / pow((1 + pow(estp, Rp)),
                                                  (1.0 / Rp))) * estp;
...
int SteelMPF::recvSelf(int commitTag, Channel & theChannel,
