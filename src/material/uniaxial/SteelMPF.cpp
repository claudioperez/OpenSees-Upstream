...
OPS_SteelMPF (void)
{
    ...
    if (numArgs != 9 && numArgs != 13)
      {
          opserr <<
              "Incorrect # args, Want: uniaxialMaterial SteelMPF tag? sigyieldp? sigyieldn? E0? bp? bn? R0? cR1? cR2? <a1? a2? a3? a4?>";
          return 0;
      }
    ...
    theMaterial =
        new SteelMPF (iData[0], dData[0], dData[1], dData[2], dData[3],
...
SteelMPF::SteelMPF
    (int tag, double FYp, double FYn, double E, double Bp, double Bn,
     double Rr, double A1, double A2):
UniaxialMaterial (tag, MAT_TAG_SteelMPF), sigyieldp (FYp), sigyieldn (FYn),
E0 (E), bp (Bp), bn (Bn), R0 (Rr), aa1 (A1), a2 (A2), a3 (0.0), a4 (1.0),
a5 (0.0),
a6 (1.0)                        // default values for strain hardening
{
...
SteelMPF::SteelMPF
    (int tag, double FYp, double FYn, double E, double Bp, double Bn,
     double Rr, double A1, double A2, double A3, double A4, double A5,
     double A6):
UniaxialMaterial (tag, MAT_TAG_SteelMPF), sigyieldp (FYp), sigyieldn (FYn),
E0 (E), bp (Bp), bn (Bn), R0 (Rr), aa1 (A1), a2 (A2), a3 (A3), a4 (A4),
a5 (A5),
a6 (A6)
{
    // Sets all history and state variables to initial values
...
sigyieldp (0.0), sigyieldn (0.0), E0 (0.0), bp (0.0), bn (0.0), R0 (0.0),
aa1 (0.0), a2 (0.0), a3 (0.0), a4 (0.0), a5 (0.0), a6 (0.0)
{
...
int
SteelMPF::setTrialStrain (double strain, double strainRate)
{
...
int
SteelMPF::setTrial (double strain, double &stress, double &tangent,
                    double strainRate)
{
    // Reset history variables to last converged state
    inc = incold;
...
void
SteelMPF::determineTrialState (double def)
{
          ...
          if (inc == 1)
            {
                ...
                double sigstp =
                    bp * estp +
                    ((1.0 - bp) / pow ((1.0 + pow (estp, Rp)),
                                       (1.0 / Rp))) * estp;
                double sig = sigrp + sigstp * (sig0p - sigrp);
                double Et =
                    ((sig0p - sigrp) / (e0p - erp)) * (bp +
                                                       ((1.0 -
                                                         bp) / pow ((1.0 +
                                                                     pow
                                                                     (estp,
          ...
          else
            {
                ...
                double sigstn =
                    bn * estn +
                    ((1.0 - bn) / pow ((1.0 + pow (estn, Rn)),
                                       (1.0 / Rn))) * estn;
                double sig = sigrn + sigstn * (sig0n - sigrn);
                double Et =
                    ((sig0n - sigrn) / (e0n - ern)) * (bn +
                                                       ((1.0 -
                                                         bn) / pow ((1.0 +
                                                                     pow
                                                                     (estn,
                ...
                if (e < eold)
                  {
                      ...
                      double sigstar =
                          -sigy * a3 * (abs (ernmaxmax) / eyieldp - a4);
                      ...
                      double estnlimit =
                          (e - erntwoprev) / (e0ntwoprev - erntwoprev);
                      double sigstnlimit =
                          bn * estnlimit +
                          ((1.0 - bn) / pow ((1 + pow (estnlimit, Rntwoprev)),
                                             (1.0 / Rntwoprev))) * estnlimit;

                      double siglimit =
                          sigrntwoprev + sigstnlimit * (sig0ntwoprev -
                                                        sigrntwoprev);
                      double Etlimit =
                          ((sig0ntwoprev - sigrntwoprev) / (e0ntwoprev -
                                                            erntwoprev)) *
                          (bn +
                           ((1.0 -
                             bn) / pow ((1 + pow (estnlimit, Rntwoprev)),
                                        (1.0 / Rntwoprev))) * (1.0 -
                                                               pow (estnlimit,
                                                                    Rntwoprev)
                                                               / (1 +
                      ...
                      double estcontrol =
                          (ern - erntwoprev) / (e0ntwoprev - erntwoprev);
                      double sigstcontrol =
                          bn * estcontrol +
                          ((1 - bn) / pow ((1.0 + pow (estcontrol, Rcontrol)),
                                           (1.0 / Rcontrol))) * estcontrol;
                      double sigcontrol =
                          sigrntwoprev + sigstcontrol * (sig0ntwoprev -
                                                         sigrntwoprev);
                ...
                else
                  {
                      ...
                      double estplimit =
                          (e - erptwoprev) / (e0ptwoprev - erptwoprev);
                      double sigstplimit =
                          bp * estplimit +
                          ((1.0 - bp) / pow ((1 + pow (estplimit, Rptwoprev)),
                                             (1.0 / Rptwoprev))) * estplimit;

                      double siglimit =
                          sigrptwoprev + sigstplimit * (sig0ptwoprev -
                                                        sigrptwoprev);
                      double Etlimit =
                          ((sig0ptwoprev - sigrptwoprev) / (e0ptwoprev -
                                                            erptwoprev)) *
                          (bp +
                           ((1.0 -
                             bp) / pow ((1 + pow (estplimit, Rptwoprev)),
                                        (1.0 / Rptwoprev))) * (1.0 -
                                                               pow (estplimit,
                                                                    Rptwoprev)
                                                               / (1 +
                ...
                if (e > eold)
                  {
                      ...
                      double sigstar =
                          sigy * a5 * (abs (erpmaxmax) / eyieldn - a6);
                      ...
                      double estcontrol =
                          (erp - erptwoprev) / (e0ptwoprev - erptwoprev);
                      double sigstcontrol =
                          bp * estcontrol +
                          ((1 - bp) / pow ((1.0 + pow (estcontrol, Rcontrol)),
                                           (1.0 / Rcontrol))) * estcontrol;
                      double sigcontrol =
                          sigrptwoprev + sigstcontrol * (sig0ptwoprev -
                                                         sigrptwoprev);
...
{
    SteelMPF *theCopy =
        new SteelMPF (this->getTag (), sigyieldp, sigyieldn, E0, bp, bn, R0,
                      aa1, a2, a3, a4, a5, a6);
...
int
SteelMPF::sendSelf (int commitTag, Channel & theChannel)
{
    int res = 0;
...
int
SteelMPF::recvSelf (int commitTag, Channel & theChannel,
...
void
SteelMPF::Print (OPS_Stream & s, int flag)
{
