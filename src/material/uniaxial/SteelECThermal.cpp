...
void *OPS_SteelECThermal(void)
{
    ...
    if (OPS_GetIntInput(&numData, iData) != 0) {
        opserr << "WARNING invalid uniaxialMaterial SteelECThermal tag?" <<
            endln;
        return 0;
    }
    ...
    if (OPS_GetNumRemainingInputArgs() == 3
        ...
        if (strcmp(typeChar, "EC3") == 0) {
            ...
            iData[1] = 22;
        } else if (strcmp(typeChar, "EC2X") == 0
                   || strcmp(typeChar, "EC2x") == 0) {
            iData[1] = 23;
        } else {
            opserr <<
                "WARNING invalid material type for uniaxialMaterial SteelECThermal "
                << iData[0] << endln;
            return 0;
        }
    ...
    theMaterial =
        new SteelECThermal(iData[0], iData[1], dData[0], dData[1],
                           dData[2], dData[3], dData[4], dData[5]);


    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type SteelECThermal Material\n";
        return 0;
    }
...
SteelECThermal::SteelECThermal
    (int tag, int TypeTag, double FY, double E,
     double A1, double A2, double A3, double A4):UniaxialMaterial(tag,
                                                                  MAT_TAG_SteelECThermal),
typeTag(TypeTag), fyT(FY), E0T(E), a1(A1), a2(A2), a3(A3), a4(A4)
{
...
SteelECThermal::SteelECThermal():UniaxialMaterial(0,
                 MAT_TAG_SteelECThermal),
...
int
 SteelECThermal::setTrialStrain(double strain, double FiberTemperature,
                                double strainRate)
{
...
int SteelECThermal::setTrial(double strain, double &stress,
...
//Changed on Aug 5 by Liming;
    ...
    double EpsiU = 0.2;
    double CT =
        (fy - fp) * (fy - fp) / ((EpsiYT - EpsiPT) * E0 - 2 * (fy - fp));
    ...
    if (fabsTstrain <= EpsiPT) {
        ...
        Tstress =
            ...
            (BT / AT) *
            (pow
             ((AT * AT - (EpsiYT - fabsTstrain) * (EpsiYT - fabsTstrain)),
              0.5));
        Ttangent =
            BT * (EpsiYT -
                  fabsTstrain) / (AT *
...
void SteelECThermal::detectLoadReversal(double dStrain)
{
    ...
    if (Tloading == 1 && dStrain < 0.0) {
        ...
        if (Cstrain > TmaxStrain)
            TmaxStrain = Cstrain;
        TshiftN =
            1 + a1 * pow((TmaxStrain - TminStrain) / (2.0 * a2 * epsy),
                         0.8);
    }
    ...
    if (Tloading == -1 && dStrain > 0.0) {
        ...
        if (Cstrain < TminStrain)
            TminStrain = Cstrain;
        TshiftP =
            1 + a3 * pow((TmaxStrain - TminStrain) / (2.0 * a4 * epsy),
                         0.8);
    }
}
...
double SteelECThermal::getThermalElongation(void)
{
...
double
    SteelECThermal::getElongTangent(double TempT, double &ET,
                                    double &Elong, double TempTmax)
{
    ...
    if (typeTag == 0 || typeTag == 3) {
        double FyRfEC3[12] =
            { 1.0, 1.0, 1.0, 1.0, 0.78, 0.47, 0.23, 0.11, 0.06, 0.04, 0.02,
            0.0
        };
        double FpRfEC3[12] =
            { 1.0, 0.807, 0.613, 0.420, 0.36, 0.18, 0.075, 0.050, 0.0375,
            0.025, 0.0125, 0.0
        };
        double E0RfEC3[12] =
            { 1.0, 0.9, 0.8, 0.7, 0.6, 0.31, 0.13, 0.09, 0.0675, 0.045,
            0.0225, 0.0
        };
        for (int i = 0; i < 12; i++) {
        ...
        double FpRfEC21[12] =
            { 1.0, 0.81, 0.61, 0.42, 0.36, 0.18, 0.07, 0.05, 0.04, 0.02,
            0.01, 0.0
        };
        double E0RfEC21[12] =
            { 1.0, 0.9, 0.8, 0.7, 0.6, 0.31, 0.13, 0.09, 0.07, 0.04, 0.02,
            0.0
        };
        for (int i = 0; i < 12; i++) {
        ...
        double FyRfEC22[12] =
            { 1.0, 1.0, 1.0, 0.94, 0.67, 0.40, 0.12, 0.11, 0.08, 0.05,
            0.03,
            0.0
        };
        double FpRfEC22[12] =
            { 0.96, 0.92, 0.81, 0.63, 0.44, 0.26, 0.08, 0.06, 0.05, 0.03,
            0.02, 0.0
        };
        double E0RfEC22[12] =
            { 1.0, 0.87, 0.72, 0.56, 0.40, 0.24, 0.08, 0.06, 0.05, 0.03,
            0.02, 0.0
        };
        for (int i = 0; i < 12; i++) {
        ...
        double FyRfEC23[12] =
            { 1.0, 1.0, 1.0, 0.90, 0.70, 0.47, 0.23, 0.11, 0.06, 0.04,
            0.02,
            0.0
        };
        double FpRfEC23[12] =
            { 1.00, 0.87, 0.74, 0.70, 0.51, 0.18, 0.07, 0.05, 0.04, 0.02,
            0.01, 0.0
        };
        double E0RfEC23[12] =
            { 1.0, 0.95, 0.90, 0.75, 0.60, 0.31, 0.13, 0.09, 0.07, 0.04,
            0.02, 0.0
        };
        for (int i = 0; i < 12; i++) {
        ...
        opserr << "WARNING SteelECThermal received an invalid typeTag: " <<
            typeTag << endln;
                ...
                fy = fyT * (FyRfactors[i - 1] -
                            (TempT + 20 -
                             100 * i) * (FyRfactors[i - 1] -
                                         FyRfactors[i]) / 100);
                fp = fyT * (FpRfactors[i - 1] -
                            (TempT + 20 -
                             100 * i) * (FpRfactors[i - 1] -
                                         FpRfactors[i]) / 100);
...
///*    
    if (TempT <= 1) {
        ...
        ThermalElongation =
            -2.416e-4 + 1.2e-5 * (TempT + 20) + 0.4e-8 * (TempT +
                                                          20) * (TempT +
                                                                 20);
    } else if (TempT <= 840) {
...
{
    SteelECThermal *theCopy =
        new SteelECThermal(this->getTag(), typeTag, fy, E0,
                           a1, a2, a3, a4);
...
// AddingSensitivity:BEGIN ///////////////////////////////////
int SteelECThermal::setParameter(const char **argv, int argc,
                                 Parameter & param)
{
...
int SteelECThermal::updateParameter(int parameterID, Information & info)
{
    switch (parameterID) {
    case -1:
        return -1;
...
int SteelECThermal::activateParameter(int passedParameterID)
{
...
int SteelECThermal::commitSensitivity(double TstrainSensitivity,
...
{
    opserr <<
        "SteelECThermal::setTrialStrain (double strain, double strainRate) - should never be called\n";
    return 0;
}


int SteelECThermal::getVariable(const char *variable, Information & info)
{
