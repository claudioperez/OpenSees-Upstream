...
void *OPS_MultiLinear(void)
{
    ...
    if (OPS_GetNumRemainingInputArgs() < 5) {
        opserr <<
            "Invalid #args,  want: uniaxialMaterial MultiLinear tag? e1 s1 e2 s2 ... "
            << endln;
        return 0;
    }
    ...
    if (OPS_GetIntInput(&numData, iData) != 0) {
        opserr <<
            "WARNING invalid tag or soilType uniaxialMaterial MultiLinearMaterial"
            << endln;
        return 0;
    }
    ...
    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type MultiLinear\n";
        return 0;
    }
...
MultiLinear::MultiLinear(int tag, const Vector & s,
                         const Vector & e):UniaxialMaterial(tag,
                                                            MAT_TAG_MultiLinear),
numSlope(0)
{
    numSlope = e.Size();
...
MultiLinear::MultiLinear():UniaxialMaterial(0, MAT_TAG_MultiLinear),
numSlope(0)
{
    tStrain = 0;
    tStress = 0;
 ...
 MultiLinear::setTrialStrain(double strain, double strainRate)
{
    ...
    if (tStrain >= data(0, 0) && tStrain <= data(0, 1)) {       // elastic
        ...
        if (tSlope == numSlope)
            tSlope = numSlope - 1;
        tStress =
            data(tSlope, 2) + (tStrain - data(tSlope, 0)) * data(tSlope,
                                                                 4);
        ...
        if (tSlope == numSlope)
            tSlope = numSlope - 1;
        tStress =
            data(tSlope, 3) + (tStrain - data(tSlope, 1)) * data(tSlope,
                                                                 4);
        tTangent = data(tSlope, 4);
    }
...
double MultiLinear::getStrain(void)
{
    return tStrain;

}

double MultiLinear::getStress(void)
{
    return tStress;
}

double MultiLinear::getTangent(void)
{
    return tTangent;
}

int MultiLinear::commitState(void)
{
...
int MultiLinear::revertToLastCommit(void)
{
    tStrain = cStrain;
    tTangent = cTangent;
    tStress = cStress;
...
int MultiLinear::revertToStart(void)
{
    data(0, 1) = data(0, 5);
...
UniaxialMaterial *MultiLinear::getCopy(void)
{
    MultiLinear *theCopy = new MultiLinear();
...
int MultiLinear::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
    static ID iData(2);
    iData(0) = this->getTag();
...
int MultiLinear::recvSelf(int cTag, Channel & theChannel,
...
void MultiLinear::Print(OPS_Stream & s, int flag)
{
