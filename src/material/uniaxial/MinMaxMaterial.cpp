...
void *OPS_MinMaxMaterial(void)
{
    ...
    if (argc < 2) {
        opserr <<
            "WARNING invalid uniaxialMaterial MinMaxMaterial $tag $otherTag <-min $minStrain> <-max $maxStrain>"
            << endln;
        return 0;
    }
    ...
    if (OPS_GetIntInput(&numData, iData) != 0) {
        opserr <<
            "WARNING invalid uniaxialMaterial MinMaxMaterial $tag $otherTag"
            << endln;
        return 0;
    }
    ...
    while (argc > 1) {
        ...
        if ((strcmp(argvLoc, "-min") == 0)
            ...
            if (OPS_GetDouble(&numData, &minStrain) != 0) {
                opserr <<
                    "WARNING invalid min value  uniaxialMaterial MinMax tag: "
                    << iData[0] << endln;
                return 0;
            }
    ...
    theMaterial =
        new MinMaxMaterial(iData[0], *theOtherMaterial, minStrain,
                           maxStrain);
...
MinMaxMaterial::MinMaxMaterial(int tag, UniaxialMaterial & material,
                               double min,
                               double max):UniaxialMaterial(tag,
                                                            MAT_TAG_MinMax),
theMaterial(0), minStrain(min), maxStrain(max), Tfailed(false),
Cfailed(false)
{
    ...
    if (theMaterial == 0) {
        opserr <<
            "MinMaxMaterial::MinMaxMaterial -- failed to get copy of material\n";
        exit(-1);
    }
}

MinMaxMaterial::MinMaxMaterial():UniaxialMaterial(0, MAT_TAG_MinMax), theMaterial(0),
minStrain(0.0), maxStrain(0.0), Tfailed(false),
Cfailed(false)
{
...
int MinMaxMaterial::setTrialStrain(double strain, double temp,
                                   double strainRate)
{
    if (Cfailed)
        return 0;
...
double MinMaxMaterial::getStress(void)
{
    if (Tfailed)
        return 0.0;
    else
...
double MinMaxMaterial::getTangent(void)
{
    if (Tfailed)
        //return 0.0;
...
double MinMaxMaterial::getDampTangent(void)
{
    if (Tfailed)
        return 0.0;
    else
...
double MinMaxMaterial::getStrain(void)
{
    return theMaterial->getStrain();
}

double MinMaxMaterial::getStrainRate(void)
{
    return theMaterial->getStrainRate();
}

int MinMaxMaterial::commitState(void)
{
    Cfailed = Tfailed;
...
int MinMaxMaterial::revertToLastCommit(void)
{
...
int MinMaxMaterial::revertToStart(void)
{
    Cfailed = false;
    Tfailed = false;
...
UniaxialMaterial *MinMaxMaterial::getCopy(void)
{
    MinMaxMaterial *theCopy =
        new MinMaxMaterial(this->getTag(), *theMaterial, minStrain,
                           maxStrain);
...
int MinMaxMaterial::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (theChannel.sendVector(dbTag, cTag, dataVec) < 0) {
        opserr <<
            "MinMaxMaterial::sendSelf() - failed to send the Vector\n";
        return -2;
    }

    if (theMaterial->sendSelf(cTag, theChannel) < 0) {
        opserr <<
            "MinMaxMaterial::sendSelf() - failed to send the Material\n";
        return -3;
    }
...
int MinMaxMaterial::recvSelf(int cTag, Channel & theChannel,
                             FEM_ObjectBroker & theBroker)
{
    ...
    if (theMaterial == 0) {
        ...
        if (theMaterial == 0) {
            opserr <<
                "MinMaxMaterial::recvSelf() - failed to create Material with classTag "
                << dataID(0) << endln;
            return -2;
        }
    }
    ...
    if (theChannel.recvVector(dbTag, cTag, dataVec) < 0) {
        opserr <<
            "MinMaxMaterial::recvSelf() - failed to get the Vector\n";
        return -3;
    }
    ...
    if (theMaterial->recvSelf(cTag, theChannel, theBroker) < 0) {
        opserr <<
            "MinMaxMaterial::recvSelf() - failed to get the Material\n";
        return -4;
    }
    return 0;
}

void MinMaxMaterial::Print(OPS_Stream & s, int flag)
{
...
int MinMaxMaterial::setParameter(const char **argv, int argc,
                                 Parameter & param)
{
    //
...
int MinMaxMaterial::updateParameter(int parameterID, Information & info)
{
    return 0;
}
...
double MinMaxMaterial::getInitialTangentSensitivity(int gradIndex)
{
    return theMaterial->getInitialTangentSensitivity(gradIndex);
}

double MinMaxMaterial::getDampTangentSensitivity(int gradIndex)
{
    return theMaterial->getDampTangentSensitivity(gradIndex);
}

double MinMaxMaterial::getRhoSensitivity(int gradIndex)
{
    return theMaterial->getRhoSensitivity(gradIndex);
}

int MinMaxMaterial::commitSensitivity(double strainGradient,
