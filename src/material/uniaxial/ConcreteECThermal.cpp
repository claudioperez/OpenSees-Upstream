...
void *OPS_ConcreteECThermal(void)
{
    ...
    if (OPS_GetIntInput(&numData, iData) != 0) {
        opserr << "WARNING invalid uniaxialMaterial ConcreteECThermal tag"
            << endln;
        return 0;
    }
    ...
    if (numData != 7) {
        opserr <<
            "Invalid #args, want: uniaxialMaterial ConcreteECThermal " <<
    ...
    theMaterial =
        new ConcreteECThermal(iData[0], dData[0], dData[1], dData[2],
                              dData[3], dData[4], dData[5], dData[6]);

    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type ConcreteECThermal Material\n";
        return 0;
    }
...
ConcreteECThermal::ConcreteECThermal(int tag, double _fc, double _epsc0,
                                     double _fcu, double _epscu,
                                     double _rat, double _ft,
                                     double _Ets):UniaxialMaterial(tag,
                                                                   MAT_TAG_ConcreteECThermal),
...
ConcreteECThermal::ConcreteECThermal(void):UniaxialMaterial(0,
                                                            MAT_TAG_ConcreteECThermal)
{
...
UniaxialMaterial *ConcreteECThermal::getCopy(void)
{
    ConcreteECThermal *theCopy =
        new ConcreteECThermal(this->getTag(), fc, epsc0, fcu, epscu, rat,
                              ft, Ets);

    return theCopy;
}

double ConcreteECThermal::getInitialTangent(void)
{
    return 2.0 * fc / epsc0;
}

int ConcreteECThermal::setTrialStrain(double trialStrain,
...
double ConcreteECThermal::getStrain(void)
{
    return eps;
}

double ConcreteECThermal::getStress(void)
{
    return sig;
}

double ConcreteECThermal::getTangent(void)
{
    return e;
}

double ConcreteECThermal::getThermalElongation(void)    //***JZ
{
    return ThermalElongation;
}

double ConcreteECThermal::getElongTangent(double TempT, double &ET, double &Elong, double TempTmax)     //PK add to include max temp
{
...
int ConcreteECThermal::commitState(void)
{
    ecminP = ecmin;
    deptP = dept;
...
int ConcreteECThermal::revertToLastCommit(void)
{
    ecmin = ecminP;;
    dept = deptP;
...
int ConcreteECThermal::revertToStart(void)
{
    ecminP = 0.0;
    deptP = 0.0;
...
int ConcreteECThermal::sendSelf(int commitTag, Channel & theChannel)
{
...
int ConcreteECThermal::recvSelf(int commitTag, Channel & theChannel,
...
void ConcreteECThermal::Print(OPS_Stream & s, int flag)
{
    s << "ConcreteECThermal:(strain, stress, tangent) " << eps << " " <<
        sig << " " << e << endln;
}


void ConcreteECThermal::Tens_Envlp(double epsc, double &sigc, double &Ect)
{
/*-----------------------------------------------------------------------
...
void ConcreteECThermal::Compr_Envlp(double epsc, double &sigc, double &Ect)
{
/*-----------------------------------------------------------------------
...
int ConcreteECThermal::getVariable(const char *varName,
                                   Information & theInfo)
{
...
int ConcreteECThermal::setTrialStrain(double strain, double strainRate)
{
    opserr <<
        "ConcreteECThermal::setTrialStrain(double strain, double strainRate) - should never be called\n";
    return -1;
}
