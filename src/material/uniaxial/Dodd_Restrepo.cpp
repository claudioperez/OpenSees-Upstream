...
void *OPS_Dodd_Restrepo(void)
{
    ...
    if (numArgs < 8 || numArgs > 10) {
        opserr <<
            "WARNING wrong # args: uniaxialMaterial $tag $Fy $Fsu $ESH $ESU $Youngs $ESHI $FSHI <$OmegaFac>"
            << endln;
        return 0;
    }
    ...
    if (OPS_GetIntInput(&numData, iData) != 0) {
        opserr << "WARNING invalid uniaxialMaterial ElasticPP tag" <<
            endln;
        return 0;
    }
...
Dodd_Restrepo::Dodd_Restrepo(int tag,
                             ...
                             double youngs,
                             double eSHI,
                             double fSHI, double omegaFac,
                             double conv):UniaxialMaterial(tag, 0), Fy(fy),
Fsu(fsu), ESH(eSH), ESU(eSU), Youngs(youngs), ESHI(eSHI), FSHI(fSHI),
Conv(conv), OmegaFac(omegaFac)
{
    numDoddRestrepo++;
    ...
    SHPower =
        log((FpSHI + Fpsu * (Epsu - EpSHI) - Fpsu) / C1) / log((Epsu -
                                                                EpSHI) /
                                                               (Epsu -
                                                                EpSH));
...
UniaxialMaterial *Dodd_Restrepo::getCopy(void)
{
    return new Dodd_Restrepo(this->getTag(), Fy, Fsu, ESH, ESU, Youngs,
                             ESHI, FSHI, OmegaFac, Conv);
}
...
extern "C" int steel_(double *Es, double *EpsLast, double *FpsLast,
                      double *YpTanLast, double *EpsOld, double *Fy,
                      double *Epy, double *EpSH, double *Epsu,
                      double *Fpsu, double *Youngs, double *SHPower,
                      double *Epr, double *Fpr, double *Epa, double *Fpa,
                      double *Epo, double *EpoMax, double *EpsuSh,
                      double *YoungsUn, double *Power, int *BFlag,
                      int *LMR, double *EprM, double *FprM, double *EpaM,
                      double *FpaM, double *YpTanM, double *PowerM,
                      double *Eps, double *Fps, double *Fs, double *YpTan,
                      double *YTan, double *OmegFac);
...
int Dodd_Restrepo::setTrialStrain(double strain, double strainRate)
{
    if (fabs(strain - tStrain) > DBL_EPSILON) {
        ...
        steel_(&tStrain, &EpsLast, &FpsLast, &YpTanLast,
               ...
               Epa, Fpa, Epo, &EpoMax, EpsuSh, &YoungsUn, Power, BFlag,
               &LMR, EprM, FprM, EpaM, FpaM, YpTanM, PowerM, &Eps, &Fps,
               &Fs, &YpTan, &YTan, &OmegaFac);
...
int Dodd_Restrepo::setTrial(double strain, double &stress, double &stiff,
                            double strainRate)
{
    if (fabs(strain - tStrain) > DBL_EPSILON) {
...
double Dodd_Restrepo::getStrain(void)
{
    return tStrain;
}

double Dodd_Restrepo::getStress(void)
{
    return tStress;
}

double Dodd_Restrepo::getTangent(void)
{
    return tTangent;
}

double Dodd_Restrepo::getInitialTangent(void)
{
    return Youngs;
}

int Dodd_Restrepo::commitState(void)
{
    if (cStrain != tStrain) {
        EpsOld = EpsLast;
        EpsLast = Eps;
...
int Dodd_Restrepo::revertToLastCommit(void)
{
    tStrain = cStrain;
    tStress = cStress;
    tTangent = cTangent;
...
int Dodd_Restrepo::revertToStart(void)
{
    double C1;                  // Temporary constant
    double EpSHI;               // Intermediate strain hardening curve natural strain
...
int Dodd_Restrepo::sendSelf(int commitTag, Channel & theChannel)
{
    int res = 0;
...
int Dodd_Restrepo::recvSelf(int commitTag, Channel & theChannel,
...
void Dodd_Restrepo::Print(OPS_Stream & s, int flag)
{
    s << "Dodd_Restrepo: " << this->getTag() << endln;
    s << "tStrain: " << tStrain << "  tStress: " << tStress <<
        " tTangent: " << tTangent << endln;
}
