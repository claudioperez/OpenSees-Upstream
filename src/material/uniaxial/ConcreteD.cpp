...
void *OPS_ConcreteD(void)
{
    ...
    if (numData != 7 && numData != 9) {
        opserr << "Invalid #args, want: uniaxialMaterial ConcreteD " <<
            iData[0] <<
    ...
    if (numData == 7) {
        if (OPS_GetDoubleInput(&numData, dData) != 0) {
            opserr << "Invalid #args: uniaxialMaterial ConcreteD " <<
                iData[0] <<
                "(fcr? epcr? ft? eptr? Ec? alphac? alphat? <cesp? etap?>)"
                << endln;
            return 0;
    ...
    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type ConcreteD\n";
        return 0;
    }
    // return the material
    return theMaterial;
...
//Constructor
ConcreteD::ConcreteD(int tag, double fc0, double ec0, double ft0,
                     double eptt0, double Ec0, double alphac0,
                     double alphat0, double cesp0,
                     double etap0):UniaxialMaterial(tag, 0), fcc(fc0),
epcc(ec0), ft(ft0), Ec(Ec0), alphac(alphac0), alphat(alphat0), TDtp(0.0),
cesp(cesp0), etap(etap0), CLoadState(0), CStress(0.0), CStrain(0.0),
CTangent(Ec0), CDc(0.0), CDt(0.0), CDcp(0.0), CEpp(0.0), CRc(0.0),
eptt(eptt0), CRt(0.0), TLoadState(0), TStrain(0.0), TStress(0.0),
TTangent(0.0), TDc(0.0), TDt(0.0), TDcp(0.0), TEpp(0.0), TRc(0.0),
TRt(0.0), CSecant(Ec0), TSecant(Ec0), CDtp(0.0)
{
}

ConcreteD::ConcreteD(int tag, double fc0, double ec0, double ft0,
                     double eptt0, double Ec0, double alphac0,
                     double alphat0):UniaxialMaterial(tag, 0),
fcc(fc0),
epcc(ec0),
ft(ft0),
Ec(Ec0),
...
TDt(0.0),
TDcp(0.0),
TEpp(0.0), TRc(0.0), TRt(0.0), CSecant(Ec0), TSecant(Ec0), CDtp(0.0)
{
}

//Constructor
ConcreteD::ConcreteD():UniaxialMaterial(0, 0),
fcc(0.0), epcc(0.0), ft(0.0), Ec(0.0), alphac(0.0), alphat(0.0),
TDtp(0.0), cesp(0.0), etap(0.0), CLoadState(0), CStress(0.0),
CStrain(0.0), CDtp(0.0), CTangent(0.0), CDc(0.0), CDt(0.0), CDcp(0.0),
CEpp(0.0), CRc(0.0), eptt(0.0), CRt(0.0), TLoadState(0), TStrain(0.0),
TStress(0.0), TTangent(0.0), TDc(0.0), TDt(0.0), TDcp(0.0), TEpp(0.0),
TRc(0.0), TRt(0.0), CSecant(0.0), TSecant(0.0)
{
}
...
void ConcreteD::unload()        //unloading, reloading curve
{
    if (TStrain > TEpp)         //
    {
        TStress = (1 - TDtp) * Ec * (TStrain - TEpp);
...
int ConcreteD::setTrialStrain(double strain, double strainRate)
{
...
double ConcreteD::getStrain(void)
{
    return TStrain;
}

double ConcreteD::getStress(void)
{
    return TStress;
}

double ConcreteD::getSecant(void)
{
    if (TStrain == 0)
        return Ec;
    else
...
double ConcreteD::getTangent(void)
{
    return TTangent;
}

int ConcreteD::commitState(void)
{
...
int ConcreteD::revertToLastCommit(void)
{
    TLoadState = CLoadState;
    TStress = CStress;
    TTangent = CTangent;
...
int ConcreteD::revertToStart(void)
{
    CLoadState = 0;
    CStrain = 0.0;
    CStress = 0.0;
...
UniaxialMaterial *ConcreteD::getCopy(void)
{
    ConcreteD *theCopy =
        new ConcreteD(this->getTag(), fcc, epcc, ft, eptt, Ec, alphac,
                      alphat, cesp, etap);
//History Parameters
...
int ConcreteD::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
...
int ConcreteD::recvSelf(int cTag, Channel & theChannel,
...
void ConcreteD::Print(OPS_Stream & s, int flag)
{
