...
void *OPS_ElasticBilin(void)
{
    ...
    if (argc != 4 && argc != 7) {
        opserr <<
            "WARNING incorrect num args want: uniaxialMaterial ElasticBilin tag E1P? E2P? eps2P? <E1N? E2N? eps2N?>"
            << endln;
        return 0;
    }
    ...
    if (OPS_GetIntInput(&numData, iData) != 0) {
        opserr << "WARNING invalid uniaxialMaterial ElasticBilin tag" <<
            endln;
        return 0;
    }
    ...
    if (OPS_GetDoubleInput(&numData, dData) != 0) {
        opserr <<
            "WARNING invalid double data: uniaxialMaterial ElasticBilin tag E2P eps2P <E2N? eps2N?>"
            << endln;
        return 0;
    }
    ...
    if (argc == 3)
        theMaterial =
            new ElasticBilin(iData[0], dData[0], dData[1], dData[2]);
    else
        theMaterial =
            new ElasticBilin(iData[0], dData[0], dData[1], dData[2],
...
ElasticBilin::ElasticBilin():UniaxialMaterial(0, MAT_TAG_ElasticBilin),
...
ElasticBilin::ElasticBilin(int tag, double e, double e2,
                           double eps2):UniaxialMaterial(tag,
                                                         MAT_TAG_ElasticBilin),
...
ElasticBilin::ElasticBilin(int tag, double ep, double e2p, double eps2p,
                           double en, double e2n,
                           double eps2n):UniaxialMaterial(tag,
                                                          MAT_TAG_ElasticBilin),
...
double ElasticBilin::getStrain(void)
{
    return trialStrain;
}

double ElasticBilin::getStress(void)
{
    return trialStress;
}


double ElasticBilin::getTangent(void)
{
    return trialTangent;
}

int ElasticBilin::commitState(void)
{
    return 0;
}


int ElasticBilin::revertToLastCommit(void)
{
...
int ElasticBilin::revertToStart(void)
{
    trialStrain = 0;
    trialStress = 0;
    trialTangent = 0;
...
UniaxialMaterial *ElasticBilin::getCopy(void)
{
...
int ElasticBilin::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
...
int ElasticBilin::recvSelf(int cTag, Channel & theChannel,
...
void ElasticBilin::Print(OPS_Stream & s, int flag)
{
    s << "ElasticBilin tag: " << this->getTag() << endln;
    s << "Input Parameters: E1P: " << E1P << " E2P: " << E2P << " eps2P: "
        << eps2P;
