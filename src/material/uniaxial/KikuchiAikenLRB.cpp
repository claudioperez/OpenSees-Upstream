...
OPS_KikuchiAikenLRB ()
{
    ...
    return new KikuchiAikenLRB (idata[0], idata[1], ddata[0], ddata[1],
                                ddata[2], ddata[3], ddata[4], ddata[5],
                                ddata[6], temp, ddata2[0], ddata2[1],
                                ddata3[0], ddata3[1]);
}
#endif
...
KikuchiAikenLRB::KikuchiAikenLRB (int tag, int type, double ar, double hr,
                                  double gr, double ap, double tp,
                                  double alph, double beta, double temp,
                                  double rk, double rq, double rs, double rf):
UniaxialMaterial (tag, MAT_TAG_KikuchiAikenLRB),
Type (type),
Ar (ar),
Hr (hr),
Gr (gr),
...
Rf (rf)
{
    ...
    kd100 =
        (Gr * Ar / Hr + Alph * Ap / Hr) * exp (-0.00271 * (Temp - 15.0)) * Rk;
    ku100 = Beta * kd100;
...
KikuchiAikenLRB::KikuchiAikenLRB ():UniaxialMaterial (0,
                  MAT_TAG_KikuchiAikenLRB)
{
...
KikuchiAikenLRB::setTrialStrain (double strain, double strainRate)
{
    ...
    if (fabs (trialDDeform) < DBL_EPSILON)
      {                         // if ( trialDDeform == 0.0 )
    ...
    if (trialIfElastic || fabs (trialStrain) == trialMaxStrain)
      {
          ...
          heq =
              (this->calcCHeq) (tmpStrain) * compHeq (tmpDeform, qd, kd, ku);
          u = qd / (keq * tmpDeform);
          ...
          if (trialDDeform * commitDDeformLastSign < 0)
            {
                ...
                if (trialIdxRev == 0)
                  {             // unload
                      ...
                      //b
                      double tqd =
                          (this->calcCQd) (fabs (commitStrain)) * qd100;
                      double tkd =
                          (this->calcCKd) (fabs (commitStrain)) * kd100;
                      double tku =
                          (this->calcCKd) (fabs (commitStrain)) * ku100;
                      double tkeq = compKeq (fabs (commitDeform), tqd, tkd);
                      double theq =
                          (this->calcCHeq) (fabs (commitStrain)) *
                          compHeq (fabs (commitDeform), tqd, tkd, tku);
                      ...
                      if (revB[trialIdxRev - 1] == 0)
                        ...
                        {       //reverse at different sign of "x"
                            ...
                            double tku =
                                (this->calcCKd) (fabs (commitStrain)) * ku100;
                            double tkeq =
                                compKeq (fabs (commitDeform), tqd, tkd);
                            double theq =
                                (this->calcCHeq) (fabs (commitStrain)) *
                                compHeq (fabs (commitDeform), tqd, tkd, tku);
                            double tu = tqd / (tkeq * fabs (commitDeform));
...
KikuchiAikenLRB::getCopy (void)
{
    ...
    KikuchiAikenLRB *theCopy =
        new KikuchiAikenLRB (this->getTag (), Type, Ar, Hr, Gr, Ap, Tp,
...
double
KikuchiAikenLRB::compQ2Unload (double u, double a, double b, double c,
                               double fm, double x)
{
...
double
KikuchiAikenLRB::compQ2Masing (double u, double a, double b, double c,
...
double
KikuchiAikenLRB::compAlpha (double a, double b1, double b2, double c,
...
double
KikuchiAikenLRB::compQ1Derivertive (double u, double n, double p, double keq,
                                    double x)
{
//  new F1 model
...
double
KikuchiAikenLRB::compQ2UnloadDerivertive (double u, double a, double b,
                                          double c, double keq, double x)
{
    return u * keq * (2 * a * exp (-a * (1 + x)) + b * exp (-c * (1 + x)) -
                      b * c * (1 + x) * exp (-c * (1 + x)));
}


double
KikuchiAikenLRB::compQ2MasingDerivertive (double u, double a, double b,
                                          double c, double keq, double x1,
                                          double x2, double alpha)
{
    return alpha * u * keq * (2 * a * exp (-a * (x1 - x2)) +
...
double
KikuchiAikenLRB::compABisection (double heq, double u, double min, double max,
...
double
KikuchiAikenLRB::calcNType1 (double gm)
{
    return 8.0;
}

double
KikuchiAikenLRB::calcPType1 (double gm)
{
    if (gm < 2.0)
      {
          return 0.0;
      ...
      {
          return -0.30226 + 0.15113 * gm;
      }
}

double
KikuchiAikenLRB::calcCType1 (double gm)
{
    return 6.0;
}

double
KikuchiAikenLRB::calcCQdType1 (double gm)
{
    if (gm < 0.1)
      {
          return 2.036 * pow (gm, 0.410);
      }
    else if (gm < 0.5)
      {
          return 1.106 * pow (gm, 0.145);
      }
    else
      {
          return 1.0;
...
double
KikuchiAikenLRB::calcCKdType1 (double gm)
{
    if (gm < 0.25)
      {
          return 0.779 * pow (gm, -0.43);
      }
    else if (gm < 1.0)
      {
          return pow (gm, -0.25);
      ...
      {
          return 0.0482025 * pow ((gm - 2.0), 2.0) + 0.92019;
      }
}

double
KikuchiAikenLRB::calcCHeqType1 (double gm)
{
    if (gm < 2.0)
      {
          return 1.0;
      ...
      {
          return 0.036375 * pow ((gm - 2.0), 2.0) + 1.0;
      }
}

double
KikuchiAikenLRB::calcAType1 (double gm, double heq, double u)
{
    if (gm < 2.0)
      {
          return compABisection (heq, u, 0.0, 50.0, 1e-6, 26.501472);
      }
    else
      {
          return 26.501472;
...
double
KikuchiAikenLRB::calcBType1 (double gm, double a, double c, double heq,
                             double u)
{
    if (gm < 2.0)
      {
