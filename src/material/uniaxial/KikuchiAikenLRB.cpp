...
KikuchiAikenLRB::KikuchiAikenLRB(int tag, int type, double ar, double hr,
                                 ...
                                 double rk, double rq, double rs,
                                 double rf):UniaxialMaterial(tag,
                                                             MAT_TAG_KikuchiAikenLRB),
Type(type), Ar(ar), Hr(hr), Gr(gr), Ap(ap), Tp(tp), Alph(alph), Beta(beta),
Temp(temp), Rk(rk), Rq(rq), Rs(rs), Rf(rf)
{
    ...
    kd100 =
        (Gr * Ar / Hr +
         Alph * Ap / Hr) * exp(-0.00271 * (Temp - 15.0)) * Rk;
    ku100 = Beta * kd100;
...
double KikuchiAikenLRB::getStress(void)
{
    return trialForce;
}

double KikuchiAikenLRB::getTangent(void)
{
    return trialStiff;
}

double KikuchiAikenLRB::getInitialTangent(void)
{
    return initialStiff;
}

double KikuchiAikenLRB::getStrain(void)
{
    return trialDeform;
}

int KikuchiAikenLRB::commitState(void)
{
...
int KikuchiAikenLRB::revertToLastCommit(void)
{
...
int KikuchiAikenLRB::revertToStart(void)
{
...
UniaxialMaterial *KikuchiAikenLRB::getCopy(void)
{
...
int KikuchiAikenLRB::sendSelf(int cTag, Channel & theChannel)
{
...
int KikuchiAikenLRB::recvSelf(int cTag, Channel & theChannel,
...
void KikuchiAikenLRB::Print(OPS_Stream & s, int flag)
{
...
double
    KikuchiAikenLRB::compQ1(double u, double n, double p, double fm,
                            double x)
{
//  new F1 model
...
double
    KikuchiAikenLRB::compQ1Derivertive(double u, double n, double p,
                                       double keq, double x)
{
//  new F1 model
...
double
 KikuchiAikenLRB::compABisection(double heq, double u, double min,
...
//Keq
double KikuchiAikenLRB::compKeq(double xm, double qd, double kd)
{
    return (qd + kd * xm) / xm;
}

//Heq
double KikuchiAikenLRB::compHeq(double xm, double qd, double kd, double ku)
{
    double dy, qy, fm, heq;
