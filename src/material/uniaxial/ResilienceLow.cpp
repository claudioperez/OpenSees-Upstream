...
OPS_ResilienceLow (void)
{
    ...
    if (numData != 5)
      {
          opserr << "Invalid #args, want: uniaxialMaterial ResilienceLow " <<
              iData[0] << "  PY DPmax Pmax Ke Kd" << endln;
          return 0;
      }
...
ResilienceLow::ResilienceLow (int tag, double pPY, double pDPmax,
                              double pPmax, double pKe, double pKd):
...
double
ResilienceLow::getStress (void)
{
    return stress;
};

double
ResilienceLow::getTangent ()
{
    switch (mode)
      {
      case 1:
...
double
ResilienceLow::getInitialTangent (void)
{
    return Ke;
}
...
int
ResilienceLow::revertToLastCommit (void)
{
    return 0;
}

int
ResilienceLow::revertToStart (void)
{
    return 0;
}

UniaxialMaterial *
ResilienceLow::getCopy (void)
{
    ResilienceLow *theCopy =
        new ResilienceLow (this->getTag (), PY, DPmax, Pmax, Ke, Kd);
    return theCopy;
}

int
ResilienceLow::sendSelf (int cTag, Channel & theChannel)
{
    return -1;
}

int
ResilienceLow::recvSelf (int cTag, Channel & theChannel,
...
void
ResilienceLow::Print (OPS_Stream & s, int flag)
{
          ...
          if (strain > 0.45 * Pmax / Kd + DPmax)
            {
                ...
                Kui =
                    (1.25 * Pmax - Kd * (Di - DPmax)) / (0.5 * Pmax / Ke -
                                                         0.25 * Pmax / Kri +
                                                         Di);
            }
          else
            {
                Kri =
                    (0.5 * Pmax - Kd * (Di - DPmax)) / (Di - 0.5 * Pmax / Ke);
                Kui =
                    (1.25 * Pmax - Kd * (Di - DPmax)) / (0.5 * Pmax / Ke -
                                                         0.25 * Pmax / Kri +
                                                         Di);
          ...
          if (Flag == 6)
            {
                ...
                if (dd >= 0)
                  ...
                  {
                      if (stress <
                          Kri * (strain + 0.5 * Pmax / Ke) - 0.5 * Pmax)
                        {
                            mode = 8;
                            determineState ();
                        }
                ...
                if (strain < -0.45 * Pmax / Kd - DPmax)
                  {
                      ...
                      Kui =
                          (1.25 * Pmax -
                           Kd * (Di - DPmax)) / (0.5 * Pmax / Ke -
                                                 0.25 * Pmax / Kri + Di);
                  }
                else
                  {
                      Kri =
                          (0.5 * Pmax - Kd * (Di - DPmax)) / (Di -
                                                              0.5 * Pmax /
                                                              Ke);
                      Kui =
                          (1.25 * Pmax -
