...
UniaxialMaterial (tag, MAT_TAG_Pinching)
{
    ...
    if (ecaps < 0.0 || ecapk < 0.0 || ecapa < 0.0 || ecapd < 0.0)
      {
          opserr <<
              "Error: Pinching::Pinching  : All gamma values must be >= 0\n"
              << "\a";
      }
    ...
    if (cs < 0.0 || ck < 0.0 || ca < 0.0 || cd < 0.0)
      {
          opserr <<
              "Error: Pinching::Pinching  : All 'c' values must be >= 0\n" <<
              "\a";
      }
    ...
    if (Resfac < 0.0 || Resfac > 1.0)
      {
          opserr <<
              "Error: Pinching::Pinching  : Residual must be > 0 and <= 1\n"
              << "\a";
      }
    ...
    if (a_pinch < 0.0 || a_pinch > 1.0)
      {
          opserr <<
              "Error: Pinching::Pinching  : kappad (dev. point)must be > 0 and <= 1\n"
              << "\a";
      }
    ...
    if (alpha > 0.8 || alpha < -0.8)
      {
          opserr <<
              "Error: Pinching::Pinching  : alpha must be < 0.8 and > -0.8\n"
              << "\a";
      }
    ...
    if (alpha == capSlope)
      {
          opserr <<
              "Error: Pinching::Pinching  : Error: alpha Hard. can not be equal to alphaCap\n"
              << "\a";
      }
...
Pinching::Pinching ():UniaxialMaterial (0, MAT_TAG_Pinching)
{
}
...
void
Pinching::Print (OPS_Stream & s, int flag)
{
...
double
Pinching::getInitialTangent (void)
{
    return elstk;
}
...
int
Pinching::recvSelf (int cTag, Channel & theChannel,
...
int
Pinching::sendSelf (int cTag, Channel & theChannel)
{
    return 0;
}
...
int
Pinching::setTrialStrain (double d, double strainRate)
{
          ...
          if (kon == 2)
            {
                ...
                if ((Enrgc - RSE) / (Enrgtk - (Enrgtot - RSE)) < 0.0)
                    RSE = 0.0;
...
void
Pinching::envelPosCap (double fy, double alphaPos, double alphaCap,
...
void
Pinching::envelNegCap (double fy, double alphaNeg, double alphaCap,
                       double cpDsp, double d, double *f, double *ek)
{
    double dy, Res, rcap, dres;
