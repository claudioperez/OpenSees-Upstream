...
//
//
// Clough.cpp: implementation of the Clough class from Fortran version.
...
Clough::Clough (int tag, Vector inputParam):
UniaxialMaterial (tag, MAT_TAG_SnapClough)
{
    if ((inputParam.Size ()) < 16)
        opserr << "Error: Clough(): inputParam, size <16\n" << "\a";
    ...
    if (ecaps < 0.0 || ecapk < 0.0 || ecapa < 0.0 || ecapd < 0.0)
        opserr << "Error: Clough::Clough  : All gamma values must be >= 0\n"
            << "\a";

    if (cs < 0.0 || ck < 0.0 || ca < 0.0 || cd < 0.0)
        opserr << "Error: Clough::Clough  : All 'c' values must be >= 0\n" <<
            "\a";

    if (capSlope > 0.0)
        opserr << "Error: Clough::Clough  : CapSlope must be < 0\n" << "\a";

    if (Resfac < 0.0 || Resfac > 1.0)
        opserr << "Error: Clough::Clough  : Residual must be > 0 and <= 1\n"
            << "\a";

    if (alpha > 0.8 || alpha < -0.8)
        opserr << "Error: Clough::Clough  : alpha must be < 0.8 and > -0.8\n"
            << "\a";

    if (alpha == capSlope)
        opserr <<
            "Error: Clough::Clough  : Error: alpha Hard. can not be equal to alphaCap\n"
            << "\a";
...
Clough::Clough ():UniaxialMaterial (0, MAT_TAG_SnapClough)
{
}
...
void
Clough::Print (OPS_Stream & s, int flag)
{
...
int
Clough::recvSelf (int cTag, Channel & theChannel,
...
int
Clough::sendSelf (int cTag, Channel & theChannel)
{
    int res = 0;
    static Vector inp (40);
...
int
Clough::setTrialStrain (double d, double strainRate)
{
          ...
          if (kon == 2)
            {
                ...
                if (a2 <= 0.0 && Enrgtk != 0.0)
                    opserr <<
                        "Warning: Clough::SetTrial  : Maximum energy capacity has been reached for stiffness degradation\n"
                        << "\a";
                ...
                if (ekunload <= 1.e-7)
                    opserr <<
                        "Warning: Clough::SetTrial  : Total stiffness loss\n"
                        << "\a";
    ...
    if (flagDeg == 1 || flagDeg == 2)
      {
          ...
          if (betaa >= 1.0)
            {
                opserr <<
                    "Warning: Clough::SetTrial  : Total accelerated stiffness loss\n"
                    << "\a";
                betaa = 1.0;
            }
...
void
Clough::envelPosCap (double fy, double alphaPos, double alphaCap,
...
void
Clough::envelNegCap (double fy, double alphaNeg, double alphaCap,
                     double cpDsp, double d, double *f, double *ek)
{
