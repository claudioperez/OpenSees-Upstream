...
Clough::Clough(int tag, Vector inputParam):UniaxialMaterial(tag,
                                                            MAT_TAG_SnapClough)
{
    ...
    if (ecaps < 0.0 || ecapk < 0.0 || ecapa < 0.0 || ecapd < 0.0)
        opserr <<
            "Error: Clough::Clough  : All gamma values must be >= 0\n" <<
            "\a";
    ...
    if (Resfac < 0.0 || Resfac > 1.0)
        opserr <<
            "Error: Clough::Clough  : Residual must be > 0 and <= 1\n" <<
            "\a";

    if (alpha > 0.8 || alpha < -0.8)
        opserr <<
            "Error: Clough::Clough  : alpha must be < 0.8 and > -0.8\n" <<
            "\a";
...
void Clough::recordInfo(int cond)
{

}


void Clough::envelPosCap(double fy, double alphaPos, double alphaCap,
...
void Clough::envelNegCap(double fy, double alphaNeg, double alphaCap,
                         double cpDsp, double d, double *f, double *ek)
{
