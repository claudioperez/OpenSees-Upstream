...
UniaxialMaterial *
TclModelBuilder_addSnapMaterial (ClientData clientData, Tcl_Interp * interp,
                                 int argc, TCL_Char ** argv)
{
    if (argc < 3)
      {
          opserr <<
              "WARNING insufficient number of arguments for the Snap material model\n";
    ...
    if (strcmp (argv[1], "Bilinear") == 0)
      {
          ...
          if ((int) input (9) == 0)
          ...
          else
            {
                ...
                if (strength == 0)
                  {
                      opserr <<
                          "WARNING damage model for strength deterioration not found\n";
          ...
          if ((int) input (11) == 0)
          ...
          else
            {
                ...
                if (capping == 0)
                  {
                      opserr <<
                          "WARNING damage model for capping deterioration not found\n";
          ...
          theMaterial =
              new Bilinear (tag, input, strength, stiffness, capping);
      }
          ...
          if (argc < 19)
            {
                opserr << "WARNING insufficient arguments\n";
                printCommand (argc, argv);
                opserr << "Want: uniaxialMaterial Clough tag? 17 args" <<
                    endln;
                return 0;
            }
             ...
             || strcmp (argv[1], "CloughDamage") == 0)
      {
          ...
          if ((int) input (10) == 0)
          ...
          else
            {
                ...
                if (accelerated == 0)
                  {
                      opserr <<
                          "WARNING damage model for accelerated stiffness deterioration not found\n";
                      opserr << "Damage Model: " << input (10);
                      opserr << "\nClough material: " << tag << endln;
                      exit (-1);
                  }
          ...
          theMaterial =
              new CloughDamage (tag, input, strength, stiffness, accelerated,
                                capping);
      }
             ...
             || strcmp (argv[1], "pinchingDamage") == 0)
      {
          ...
          theMaterial =
              new PinchingDamage (tag, input, strength, stiffness,
                                  accelerated, capping);
      }
