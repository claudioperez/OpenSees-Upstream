...
PinchingDamage::PinchingDamage (int tag, Vector inputParam,
...
UniaxialMaterial (tag, MAT_TAG_SnapPinchingDamage)
{
    ...
    if (capSlope > 0.0)
      {
          opserr <<
              "Error: PinchingDamage::PinchingDamage  : CapSlope must be < 0\n"
              << "\a";
      }
    ...
    if (Resfac < 0.0 || Resfac > 1.0)
      {
          opserr <<
              "Error: PinchingDamage::PinchingDamage  : Residual must be > 0 and <= 1\n"
              << "\a";
      }
    ...
    if (a_pinch < 0.0 || a_pinch > 1.0)
      {
          opserr <<
              "Error: PinchingDamage::PinchingDamage  : kappad (dev. point)must be > 0 and <= 1\n"
              << "\a";
      }
    ...
    if (alpha > 0.8 || alpha < -0.8)
      {
          opserr <<
              "Error: PinchingDamage::PinchingDamage  : alpha must be < 0.8 and > -0.8\n"
              << "\a";
      }
    ...
    if (alpha == capSlope)
      {
          opserr <<
              "Error: PinchingDamage::PinchingDamage  : Error: alpha Hard. can not be equal to alphaCap\n"
              << "\a";
      }
    ...
    if (strength != NULL)
      {
          ...
          if (StrDamage == NULL)
            {
                opserr <<
                    "Error: CloughDamage::CloughDamage  : Can not make a copy of strength damage model\n"
                    << "\a";
                exit (-1);
            }
      }
    ...
    if (stiffness != NULL)
      {
          ...
          if (StfDamage == NULL)
            {
                opserr <<
                    "Error: CloughDamage::CloughDamage  : Can not make a copy of stiffness damage model\n"
                    << "\a";
                exit (-1);
            }
      }
    ...
    if (accelerated != NULL)
      {
          ...
          if (AccDamage == NULL)
            {
                opserr <<
                    "Error: CloughDamage::CloughDamage  : Can not make a copy of accelerated stiffness degradation damage model\n"
                    << "\a";
                exit (-1);
            }
      }
    ...
    if (capping != NULL)
      {
          ...
          if (CapDamage == NULL)
            {
                opserr <<
                    "Error: CloughDamage::CloughDamage  : Can not make a copy of capping damage model\n"
                    << "\a";
                exit (-1);
            }
      }
...
PinchingDamage::PinchingDamage ():UniaxialMaterial (0,
                  MAT_TAG_SnapPinchingDamage)
{
...
void
PinchingDamage::Print (OPS_Stream & s, int flag)
{
...
int
PinchingDamage::revertToLastCommit ()
{
    for (int i = 0; i < 24; i++)
      {
          hsTrial[i] = hsCommit[i];
...
double
PinchingDamage::getTangent ()
{
    return hsTrial[2];
}

double
PinchingDamage::getInitialTangent (void)
{
    return elstk;
}

double
PinchingDamage::getStress ()
{
    return hsTrial[1];
}
...
int
PinchingDamage::recvSelf (int cTag, Channel & theChannel,
...
int
PinchingDamage::sendSelf (int cTag, Channel & theChannel)
{
    return 0;
}
...
PinchingDamage::getCopy (void)
{
    ...
    PinchingDamage *theCopy =
        new PinchingDamage (this->getTag (), inp, StrDamage, StfDamage,
                            AccDamage, CapDamage);
...
int
PinchingDamage::setTrialStrain (double d, double strainRate)
{
    int flagDeg, flgstop, Unl, kon;
    double deltaD;
    double ekP, ekunload, ekexcurs, Enrgtot, Enrgc, sp, sn, dmax, dmin, fyPos,
        fyNeg, fP, dP;
    ...
    if (deltaD >= 0.0)
      {
          if (kon == 2)
            {
                ...
                if (fP < 0.0)
                  {
                      if ((fabs (dmaxDeg - dyieldPos) >= 1.e-10)
                          && (fabs (dP - fP / ekunload) <= 1.e-10))
                        {
                            sn = 1.e-9;
                        }
                      else
          ...
          if (d >= dmaxDeg)
            ...
            {
                this->envelPosCap (fyPos, alphaPos, capSlope, cpPos, dmaxDeg,
                                   &fmax, &ekt);
    ...
    else
      {
          ...
          if (kon == 1)
            {
                ...
                if (fP > 0.0)
                  {
                      if (fabs (dminDeg - dyieldNeg) >= 1.e-10
                          && fabs (dP - fP / ekunload) <= 1.e-10)
                        {
                            sp = 1.e-9;
                        }
          ...
          if (d < dminDeg)
            ...
            {
                this->envelNegCap (fyNeg, alphaNeg, capSlope, cpNeg, dminDeg,
                                   &fmin, &ekt);
    ...
    if (f * fP < 0.0)
      {
          if (fP > 0.0 && dmaxDeg > dyieldPos)
              flagDeg = 1;      // positive half cycle
          if (fP < 0.0 && dminDeg < dyieldNeg)
          ...
          if (deltaD < 0.0)
            {
                ...
                if (AccDamage != NULL)
                    betaa = AccDamage->getNegDamage ();
                if (betaa >= 1.0)
                  {
                      opserr <<
                          "Total loss for accelerated stiffness degradation\n";
                      betaa = 1.0;
                  }
                if (CapDamage != NULL)
                    betad = CapDamage->getNegDamage ();
                if (betad >= 1.0)
                  {
                ...
                fCapRefNeg =
                    (-capSlope * elstk * capDispNeg + fyieldNeg +
                     elstk * alpha * (capDispNeg - dyieldNeg)) * (1 - betad);
          ...
          else
            {
                ...
                fCapRefPos =
                    (-capSlope * elstk * capDispPos + fyieldPos +
                     elstk * alpha * (capDispPos - dyieldPos)) * (1 - betad);
...
void
PinchingDamage::recordInfo (int cond)
{
...
void
PinchingDamage::envelPosCap (double fy, double alphaPos, double alphaCap,
...
void
PinchingDamage::envelNegCap (double fy, double alphaNeg, double alphaCap,
                             double cpDsp, double d, double *f, double *ek)
{
    double dy, Res, rcap, dres;
