...
CloughDamage::CloughDamage (int tag, Vector inputParam,
                            DamageModel * strength, DamageModel * stiffness,
                            DamageModel * accelerated, DamageModel * capping):
UniaxialMaterial (tag, MAT_TAG_SnapCloughDamage)
{
    ...
    if (capSlope > 0.0)
        opserr <<
            "Error: CloughDamage::CloughDamage  : CapSlope must be < 0\n" <<
            "\a";

    if (Resfac < 0.0 || Resfac > 1.0)
        opserr <<
            "Error: CloughDamage::CloughDamage  : Residual must be > 0 and <= 1\n"
            << "\a";

    if (alpha > 0.8 || alpha < -0.8)
        opserr <<
            "Error: CloughDamage::CloughDamage  : alpha must be < 0.8 and > -0.8\n"
            << "\a";

    if (alpha == capSlope)
        opserr <<
            "Error: CloughDamage::CloughDamage  : Error: alpha Hard. can not be equal to alphaCap\n"
            << "\a";
    ...
    if (accelerated != NULL)
      {
          ...
          if (AccDamage == NULL)
            {
                opserr <<
                    "Error: CloughDamage::CloughDamage  : Can not make a copy of accelerated stiffness damage model\n"
                    << "\a";
                exit (-1);
            }
      }
...
CloughDamage::CloughDamage ():UniaxialMaterial (0, MAT_TAG_SnapCloughDamage)
{
...
void
CloughDamage::Print (OPS_Stream & s, int flag)
{
...
int
CloughDamage::revertToLastCommit ()
{
...
double
CloughDamage::getInitialTangent (void)
{
...
int
CloughDamage::recvSelf (int cTag, Channel & theChannel,
...
int
CloughDamage::sendSelf (int cTag, Channel & theChannel)
{
...
CloughDamage::getCopy (void)
{
    ...
    CloughDamage *theCopy =
        new CloughDamage (this->getTag (), inp, StrDamage, StfDamage,
                          AccDamage, CapDamage);
...
int
CloughDamage::setTrialStrain (double d, double strainRate)
{
...
void
CloughDamage::recordInfo (int cond)
{
...
void
CloughDamage::envelPosCap (double fy, double alphaPos, double alphaCap,
...
void
CloughDamage::envelNegCap (double fy, double alphaNeg, double alphaCap,
                           double cpDsp, double d, double *f, double *ek)
{
