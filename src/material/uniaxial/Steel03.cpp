    ...
    if (argc > 9)
      {
          ...
          return new Steel03 (tag, data[0], data[1], data[2], data[3],
                              data[4], data[5], opt[0], opt[1], opt[2],
                              opt[3]);
      }
    else
        // Parsing was successful, allocate the material
...
fy (FY), E0 (E), b (B), r (R), cR1 (R1), cR2 (R2), a1 (A1), a2 (A2), a3 (A3),
a4 (A4)
{
...
fy (0.0), E0 (0.0), b (0.0), r (0.0), cR1 (0.0), cR2 (0.0), a1 (0.0),
...
int
Steel03::setTrialStrain (double strain, double strainRate)
{
...
int
Steel03::setTrial (double strain, double &stress, double &tangent,
                   double strainRate)
{
    // Reset history variables to last converged state
    TminStrain = CminStrain;
...
void
Steel03::determineTrialState (double dStrain)
{
    ...
    if (Cloading != 0)
      {
          ...
          Tstress =
              TrStress + b * c4c5 * c6 + (1 - b) * c4c5 * c6 / pow (intval,
                                                                    1 /
                                                                    TcurR);
...
int
Steel03::sendSelf (int commitTag, Channel & theChannel)
{
    int res = 0;
...
int
Steel03::recvSelf (int commitTag, Channel & theChannel,
...
void
Steel03::Print (OPS_Stream & s, int flag)
{
