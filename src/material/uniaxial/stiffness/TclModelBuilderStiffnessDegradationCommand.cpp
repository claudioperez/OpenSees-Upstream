                                            ...
                                            Domain * theDomain)
{
    ...
    if (argc < 2)
      {
          opserr <<
              "WARNING insufficient number of stiffnessDegradation arguments\n";
          opserr <<
              "Want: stiffnessDegradation type? tag? <specific stiffnessDegradation args>"
              << endln;
          return TCL_ERROR;
      }
    ...
    if (strcmp (argv[1], "Ductility") == 0)
      ...
      {
          opserr << "WARNING unknown type of stiffnessDegradation: " <<
              argv[1];
    ...
    if (theState == 0)
      {
          opserr <<
              "WARNING ran out of memory creating stiffnessDegradation\n";
          opserr << argv[1] << endln;
          return TCL_ERROR;
      }
    ...
    if (OPS_addStiffnessDegradation (theState) == false)
      {
          opserr <<
              "WARNING could not add stiffnessDegradation to the domain\n";
