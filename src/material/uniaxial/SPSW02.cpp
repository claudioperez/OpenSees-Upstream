    ...
    if (paramsSet)
        theMaterial =
            new SPSW02(tag, E0, b, Fts, Fcs, cmpUnldngEFac, sigTEFac,
                       sigTFfac, epsTFfac, R, epsPCFac, pstCapEFac, gama,
                       c, resFac);
    else
        theMaterial =
...
SPSW02::SPSW02(int tag, double _fpy, double _E0, double _b, double _t,
               ...
               double _pstCapEFac, double _gama, double _c,
               double _resFac):UniaxialMaterial(tag, MAT_TAG_SPSW02),
fpy(_fpy), E0(_E0), b(_b), t(_t), hs(_hs), l(_l), R(_R),
epsPCFac(_epsPCFac), pstcpEFac(_pstCapEFac), gama(_gama), c(_c),
resFac(_resFac)
{
    givenParams = false;
...
SPSW02::SPSW02(int tag, double _E0, double _b, double _FTS, double _FCS,
               ...
               double _pstCapEFac, double _gama, double _c,
               double _resFac):UniaxialMaterial(tag, MAT_TAG_SPSW02),
E0(_E0), b(_b), FTS(_FTS), FCS(_FCS), cmpUnldngEFac(_cmpUnldngEFac),
sigTEFac(_sigTEFac), sigTFfac(_sigTFfac), epsTFfac(_epsTFfac), R(_R),
epsPCFac(_epsPCFac), pstcpEFac(_pstCapEFac), gama(_gama), c(_c),
resFac(_resFac)
{
    givenParams = true;
    ...
    if (givenParams)
        theCopy =
            new SPSW02(this->getTag(), E0, b, FTS, FCS, cmpUnldngEFac,
                       sigTEFac, sigTFfac, epsTFfac, R, epsPCFac,
    ...
    if (kon == 11) {
        if (deps > 0) {
            if (epsP > plstr - 2. * Fcs / (cmpUnldngEFac * E0))
                ...
                sigr = sigP;
                epss0 =
                    epsr - (sigr - sigTEFac * Fcs) / (cmpUnldngEFac * E0);
                sigs0 = sigTEFac * Fcs;
            }
        } else {
            sig = -Fcs;
...
int SPSW02::recvSelf(int commitTag, Channel & theChannel,
...
{
    s << "SPSW02:(strain, stress, tangent)" << eps << " " << sig << " " <<
        e << endln;
}
...
void SPSW02::MenegottoPinto(double epsc, double E2, double R, double &sigc,
                            double &ec)
{
