...
OPS_SPSW02 ()
{
    if (numSPSWcall == 0)
      {
          opserr <<
              "------ SPSW02 unaxialMaterial, Written by SAJalali @ Amirkabir University of Technology, Tehran, 2015-------\n";
          opserr <<
              "------------------------------ Please Send Comments to: seyyed-jalali@aut.ac.ir-----------------------------\n";
          opserr << "-------Syntax:\n";
          opserr << "-------UniaxialMaterial SPSW02 tag ";
          opserr <<
              "-------E0 b <-geom Fpy t h l> <-params Fts Fcs cmpUnldngEFac sigTEFac sigTFfac epsTFfac> -R $R -Damage epsPCFac pstCapEFac gama c resFac\n\n";
          opserr <<
              "------------------------------------------------------------------------------------------------------------\n\n\n";
          numSPSWcall = 1;
      }
    ...
    if (fpy != 0 && paramsSet)
      {
          opserr <<
              "WARNING both -params and -geom options cannot be used at the same time";
          opserr << "uniaxialMaterial SPSW02: " << tag << endln;
          return 0;
      }
    R = 50;
    ...
    if (paramsSet)
        theMaterial =
            new SPSW02 (tag, E0, b, Fts, Fcs, cmpUnldngEFac, sigTEFac,
                        sigTFfac, epsTFfac, R, epsPCFac, pstCapEFac, gama, c,
                        resFac);
    else
        theMaterial =
            new SPSW02 (tag, fpy, E0, b, t, hs, l, R, epsPCFac, pstCapEFac,
                        gama, c, resFac);
    //opserr<<"ok\n";
...
SPSW02::SPSW02 (int tag, double _fpy, double _E0, double _b, double _t,
                double _hs, double _l, double _R, double _epsPCFac,
                double _pstCapEFac, double _gama, double _c, double _resFac):
UniaxialMaterial (tag, MAT_TAG_SPSW02),
fpy (_fpy),
E0 (_E0),
b (_b),
t (_t),
...
SPSW02::SPSW02 (int tag, double _E0, double _b, double _FTS, double _FCS,
                double _cmpUnldngEFac, double _sigTEFac, double _sigTFfac,
                double _epsTFfac, double _R, double _epsPCFac,
                double _pstCapEFac, double _gama, double _c, double _resFac):
UniaxialMaterial (tag, MAT_TAG_SPSW02),
E0 (_E0),
b (_b),
...
SPSW02::getCopy ()
{
    ...
    if (givenParams)
        theCopy =
            new SPSW02 (this->getTag (), E0, b, FTS, FCS, cmpUnldngEFac,
                        sigTEFac, sigTFfac, epsTFfac, R, epsPCFac, pstcpEFac,
                        gama, c, resFac);
    else
        theCopy =
            new SPSW02 (this->getTag (), fpy, E0, b, t, hs, l, R, epsPCFac,
                        pstcpEFac, gama, c, resFac);
    return theCopy;
}

double
SPSW02::getInitialTangent ()
{
    return E0;
}

int
SPSW02::setTrialStrain (double trialStrain, double strainRate)
{
          ...
          if (deps > 0)
            {
                if (epsP > plstr - 2. * Fcs / (cmpUnldngEFac * E0))
                ...
                else
                  {
                      kon = 13;
                      epsTF =
                          plstr - epsTFfac * (plstr -
                                              (epsP +
                                               Fcs / (cmpUnldngEFac * E0))) +
                          sigTF / E0;
                      epsr = epsP;
                      sigr = sigP;
                      epss0 =
                          epsr - (sigr -
                                  sigTEFac * Fcs) / (cmpUnldngEFac * E0);
                      sigs0 = sigTEFac * Fcs;
                  }
            }
          else
          ...
          if (beta > 0.999 || beta < 0)
            {
                opserr << "\nSPSW02:" << this->
                    getTag () <<
                    " WARNING! Maximum Energy Absorbance Capacity Reached\n"
                    << endln;
                beta = 0.999;
...
int
SPSW02::sendSelf (int commitTag, Channel & theChannel)
{
...
int
SPSW02::recvSelf (int commitTag, Channel & theChannel,
...
void
SPSW02::Print (OPS_Stream & s, int flag)
{
    s << "SPSW02:(strain, stress, tangent)" << eps << " " << sig << " " << e
        << endln;
}

void
SPSW02::Calc_sigcr ( /*double & _Fts, double &_Fcs */ )
{
...
void
SPSW02::MenegottoPinto (double epsc, double E2, double R, double &sigc,
                        double &ec)
{
