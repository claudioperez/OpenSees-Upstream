    ...
    if (numArgs == 13)
      {
          ...
          if (OPS_GetIntInput (&numData, &iData) != 0)
            {
                opserr <<
                    "WARNING: invalid uniaxialMaterial TDConcrete tag\n";
                return 0;
            }
          ...
          theMaterial =
              new TDConcrete (iData, dData[0], dData[1], dData[2], dData[3],
...
TDConcrete::TDConcrete (int tag, double _fc, double _ft, double _Ec,
                        double _beta, double _age, double _epsshu,
                        double _epssha, double _tcr, double _epscru,
                        double _epscra, double _epscrd, double _tcast):
...
{
    TDConcrete *theCopy =
        new TDConcrete (this->getTag (), fc, ft, Ec, beta, age, epsshu,
                        epssha, tcr, epscru, epscra, epscrd, tcast);
...
{
    s << "TDConcrete:(strain, stress, tangent) " << eps << " " << sig << " "
        << e << endln;
}
...
Response *
TDConcrete::setResponse (const char **argv, int argc, OPS_Stream & theOutput)
{
