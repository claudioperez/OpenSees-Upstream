...
Neoprene::Neoprene(int tag, double e, double gap0):UniaxialMaterial(tag,
                                                                    MAT_TAG_EPPGap),
commitStrain(0.0), trialStrain(0.0), E(e), gap(gap0),
...
Neoprene::Neoprene():UniaxialMaterial(0, MAT_TAG_EPPGap),
 ...
 Neoprene::setTrialStrain(double strain, double strainRate)
{
    ...
    if (gap >= 0) {
        if (trialStrain > gap && deps > 0) {
            ...
            trialStress =
                maxElasticYieldStrain * pow(trialStrain - gap, 2);
...
double Neoprene::getStrain(void)
{
    return trialStrain;
}

double Neoprene::getStress(void)
{
    return trialStress;

}

double Neoprene::getTangent(void)
{
    return trialTangent;
}

double Neoprene::getInitialTangent(void)
{
    if (fabs(gap) > 0.0)
        return 0.0;
    else
    ...
    if (gap > 0 && trialStrain > gap) {
        if (deps < 0)
            ...
            maxElasticYieldStrain =
                trialStress / pow(trialStrain - gap, 2);
...
int Neoprene::revertToLastCommit(void)
{
...
int Neoprene::revertToStart(void)
{
    commitStrain = 0.0;
    trialStrain = 0.0;
...
UniaxialMaterial *Neoprene::getCopy(void)
{
...
int Neoprene::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
...
int Neoprene::recvSelf(int cTag, Channel & theChannel,
...
void Neoprene::Print(OPS_Stream & s, int flag)
{
