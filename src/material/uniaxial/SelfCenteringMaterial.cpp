...
void *OPS_SelfCenteringMaterial()
{
    ...
    UniaxialMaterial *mat =
        new SelfCenteringMaterial(tag, data[0], data[1], data[2], data[3],
                                  data[4], data[5], data[6]);
    if (mat == 0) {
        opserr <<
            "WARNING: failed to create Selfcenteringmaterial material\n";
        return 0;
    }
...
SelfCenteringMaterial::SelfCenteringMaterial(int tag, double K1, double K2,
                                             ...
                                             double sd, double bd,
                                             double
                                             rb):UniaxialMaterial(tag,
                                                                  MAT_TAG_SelfCentering),
...
SelfCenteringMaterial::SelfCenteringMaterial():UniaxialMaterial(0, MAT_TAG_SelfCentering),
...
double SelfCenteringMaterial::getStress(void)
{
    return Tstress;
}

double SelfCenteringMaterial::getTangent(void)
{
    return Ttangent;
}

double SelfCenteringMaterial::getStrain(void)
{
    return Tstrain;
}

int SelfCenteringMaterial::commitState(void)
{
...
int SelfCenteringMaterial::revertToLastCommit(void)
{
    Tstrain = Cstrain;
    Tstress = Cstress;
    Ttangent = Ctangent;
...
int SelfCenteringMaterial::revertToStart(void)
{
...
UniaxialMaterial *SelfCenteringMaterial::getCopy(void)
{
...
int SelfCenteringMaterial::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
    ...
    if (res < 0)
        opserr <<
            "SelfCenteringMaterial::sendSelf() - failed to send data\n";

    return res;
}

int SelfCenteringMaterial::recvSelf(int cTag, Channel & theChannel,
                                    FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0) {
        opserr <<
            "SelfCenteringMaterial::recvSelf() - failed to receive data\n";
        this->setTag(0);
    } else {
...
void SelfCenteringMaterial::Print(OPS_Stream & s, int flag)
{
