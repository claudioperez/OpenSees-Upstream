...
OPS_Concrete04 ()
{
    ...
    if (numdata < 5)
      {
          ...
          opserr <<
              "Want: uniaxialMaterial Concrete04 tag? fpc? epsc0? epscu? Ec0?";
          opserr << " <ft? etu? <beta?> >\n";
          return 0;
      }
    ...
    if (type == 1)
      ...
      {
          mat =
              new Concrete04 (tag, data[0], data[1], data[2], data[3],
                              data2[0], data2[1]);
      }
    else if (type == 3)
      {
...
Concrete04::Concrete04
    (int tag, double FPC, double EPSC0, double EPSCU, double EC0, double FCT,
     double ETU):
UniaxialMaterial (tag, MAT_TAG_Concrete04), fpc (FPC), epsc0 (EPSC0),
epscu (EPSCU), Ec0 (EC0), fct (FCT), etu (ETU), beta (0.1), CminStrain (0.0),
CendStrain (0.0), CcompStrain (0.0), CUtenStress (FCT), Cstrain (0.0),
Cstress (0.0),
CmaxStrain (0.0)
{
    ...
    if (fpc > 0.0 || epsc0 > 0.0 || epscu > 0.0)
      {
          opserr <<
              "error: negative values required for concrete stress-strain model"
              << endln;
      }
    ...
    if (fct < 0.0)
      {
          fct = 0.0;
          opserr <<
              "warning: fct less than 0.0 so the tensile response part is being set to 0"
              << endln;
      }
...
Concrete04::Concrete04
    (int tag, double FPC, double EPSC0, double EPSCU, double EC0, double FCT,
     double ETU, double BETA):
UniaxialMaterial (tag, MAT_TAG_Concrete04), fpc (FPC), epsc0 (EPSC0),
epscu (EPSCU), Ec0 (EC0), fct (FCT), etu (ETU), beta (BETA), CminStrain (0.0),
CendStrain (0.0), CcompStrain (0.0), CUtenStress (FCT), Cstrain (0.0),
Cstress (0.0),
CmaxStrain (0.0)
{
...
fpc (FPC), epsc0 (EPSC0), epscu (EPSCU), Ec0 (EC0), fct (0.0), etu (0.0),
beta (0.0), CminStrain (0.0), CendStrain (0.0), CcompStrain (0.0),
CUtenStress (0.0), Cstrain (0.0), Cstress (0.0),
CmaxStrain (0.0)
{
    // Make all concrete parameters negative
    if (fpc > 0.0 || epsc0 > 0.0 || epscu > 0.0)
...
fpc (0.0), epsc0 (0.0), epscu (0.0), Ec0 (0.0), fct (0.0), etu (0.0),
beta (0.0), CminStrain (0.0), CunloadSlope (0.0), CendStrain (0.0),
CcompStrain (0.0), CUtenStress (0.0), CUtenSlope (0.0), Cstrain (0.0),
Cstress (0.0), CmaxStrain (0.0)
{
    // Set trial values
    this->revertToLastCommit ();
...
int
Concrete04::setTrialStrain (double strain, double strainRate)
{
    ...
    TUtenSlope = CUtenSlope;
    if (dStrain <= 0.0)
      {                         /*// Material can be either in Compression-Reloading    // or Tension-Unloading state. */
          if (Tstrain > 0.0)
            {
...
Concrete04::TensEnvelope ()
{
    ...
    if (Tstrain <= ect)
    ...
    else
      {
          ...
          Ttangent =
              fct * pow (beta,
                         (Tstrain - ect) / (etu - ect)) * log (beta) / (etu -
                                                                        ect);
      }
}

void
Concrete04::setTenUnload ()
{
    TUtenStress = Tstress;
...
double
Concrete04::getStress ()
{
    return Tstress;
}

double
Concrete04::getStrain ()
{
    return Tstrain;
}

double
Concrete04::getTangent ()
{
    return Ttangent;
}

int
Concrete04::commitState ()
{
    /*// History variables */
...
Concrete04::revertToStart ()
{
    ...
    Cstress = 0.0;
    Ctangent = Ec0;
    /*// Reset trial variables and state */ this->revertToLastCommit ();
    return 0;
}
...
Concrete04::getCopy ()
{
    ...
    /*// Converged state variables */ theCopy->Cstrain = Cstrain;
...
int
Concrete04::sendSelf (int commitTag, Channel & theChannel)
{
    int res = 0;
...
int
Concrete04::recvSelf (int commitTag, Channel & theChannel,
...
void
Concrete04::Print (OPS_Stream & s, int flag)
{
