...
OPS_DegradingPinchedBW(void)
{
    ...
    theMaterial =
        new DegradingPinchedBW(iData1[0], dData[0], dData[1], dData[2],
                               dData[3], dData[4], dData[5], dData[6],
                               dData[7], dData[8], dData[9], dData[10],
                               dData[11], dData[12], dData[13], dData[14],
                               dData[15], dData[16], dData[17], iData2[0]);

    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type DegradingPinchedBW\n";
        return 0;
    }
...
DegradingPinchedBW::DegradingPinchedBW(int tag,
                                       ...
                                       double ptolerance,
                                       int
                                       pMaxNumIter):UniaxialMaterial(tag,
                                                                     0),
xmax(0.0), xmaxp(0.0), m(p_m), Fy(p_Fy), xu(p_xu), alpha(p_alpha),
ko(p_ko), n(p_n), eta(p_eta), beta(p_beta), rhoeps(p_rhoeps), rhox(p_rhox),
phi(p_phi), deltak(p_deltak), deltaf(p_deltaf), sigma(p_sigma), u(p_u),
epsp(p_epsp), rhop(p_rhop), tolerance(ptolerance), maxNumIter(pMaxNumIter)
{
...
int DegradingPinchedBW::setTrialStrain(double strain, double strainRate)
{
    ...
    double TDamIndex, Tpk, TA, Tbetak, Tbetaf, TPow, Psi, Phi, f, Tznew,
        Tzold;
    double Te_z, TDamIndex_z, Tpk_z, TA_z, Tbetak_z, Tbetaf_z, TPow_z,
        Phi_z, f_z;
    double Te_x, TDamIndex_x, Tpk_x, TA_x, Tbetak_x, Tbetaf_x, Phi_x, f_x;
    double Te_z_x, TDamIndex_z_x, Tpk_z_x, TA_z_x, Tbetak_z_x, Tbetaf_z_x,
        Phi_z_x, f_z_x;
    double gp, feps, kp;
    ...
    while ((fabs(Tzold - Tznew) > tolerance) && count < maxNumIter) {
        ...
        TA_z =
            TA * (-deltak * TDamIndex_z * Tpk -
                  deltak * TDamIndex * Tpk_z);
        Tbetak_z = beta * TA_z;
        ...
        Phi_z =
            TA_z - (TPow_z * Tbetak * Tbetaf + TPow * Tbetak_z * Tbetaf +
                    TPow * Tbetak * Tbetaf_z) * Psi;
        ...
        if (fabs(f_z) < 1.0e-10) {
            opserr <<
                "WARNING: DegradingPinchedBW::setTrialStrain() -- zero derivative "
        ...
        if (count == maxNumIter) {
            ...
            opserr <<
                "WARNING: DegradingPinchedBW::setTrialStrain() -- did not"
        ...
        if (Tz != 0.0) {
            ...
            TA_x =
                TA * (-deltak * TDamIndex_x * Tpk -
                      deltak * TDamIndex * Tpk_x);
            Tbetak_x = beta * TA_x;
            Tbetaf_x = Tbetaf * (n * deltaf * TDamIndex_x);
            Phi_x =
                TA_x - (TPow * Tbetak_x * Tbetaf +
                        TPow * Tbetak * Tbetaf_x) * Psi;
            ...
            Tpk_z_x =
                Tpk_x * (-phi * TDamIndex_z) +
                Tpk * (-phi * TDamIndex_z_x);
            TA_z_x =
                TA_x * (-deltak * TDamIndex_z * Tpk -
                        deltak * TDamIndex * Tpk_z) -
                TA * (deltak * TDamIndex_z_x * Tpk +
                      deltak * TDamIndex_z * Tpk_x +
                      deltak * TDamIndex_x * Tpk_z +
                      deltak * TDamIndex * Tpk_z_x);
            Tbetak_z_x = beta * TA_z_x;
            Tbetaf_z_x =
                Tbetaf_x * (n * deltaf * TDamIndex_z) +
                Tbetaf * (n * deltaf * TDamIndex_z_x);
            Phi_z_x =
                TA_z_x - (TPow_z * Tbetak * Tbetaf_x +
                          TPow_z * Tbetak_x * Tbetaf +
                          TPow * Tbetaf_z_x * Tbetak +
                          TPow * Tbetaf_z * Tbetak_x +
                          TPow * Tbetaf_x * Tbetak_z +
                          TPow * Tbetaf * Tbetak_z_x) * Psi;
...
double DegradingPinchedBW::getStress(void)
{
    return Tstress;
}

double DegradingPinchedBW::getInitialTangent(void)
{
    return (alpha * ko + (1 - alpha) * ko);
}


double DegradingPinchedBW::getTangent(void)
{
    return Ttangent;
}

double DegradingPinchedBW::getStrain(void)
{
    return Tstrain;
}

int DegradingPinchedBW::commitState(void)
{
...
int DegradingPinchedBW::revertToLastCommit(void)
{
    xmax = xmaxp;
    return 0;
}

int DegradingPinchedBW::revertToStart(void)
{
    xmaxp = 0.0;
    Tstrain = 0.0;
    Cstrain = 0.0;
...
UniaxialMaterial *DegradingPinchedBW::getCopy(void)
{
    DegradingPinchedBW *theCopy =
        new DegradingPinchedBW(this->getTag(), m, Fy, xu, alpha, ko, n,
                               eta,
                               beta, rhoeps,
                               rhox, phi, deltak, deltaf, sigma, u, epsp,
                               rhop, tolerance, maxNumIter);
    theCopy->xmax = xmax;
    theCopy->xmaxp = xmaxp;
...
int DegradingPinchedBW::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
...
int DegradingPinchedBW::recvSelf(int cTag, Channel & theChannel,
...
void DegradingPinchedBW::Print(OPS_Stream & s, int flag)
{
