...
DegradingPinchedBW::DegradingPinchedBW (int tag,
                                        ...
                                        double p_epsp,
                                        double p_rhop,
                                        double ptolerance, int pMaxNumIter):
...
DegradingPinchedBW::setTrialStrain (double strain, double strainRate)
{
    ...
    double TDamIndex, Tpk, TA, Tbetak, Tbetaf, TPow, Psi, Phi, f, Tznew,
        Tzold;
    double Te_z, TDamIndex_z, Tpk_z, TA_z, Tbetak_z, Tbetaf_z, TPow_z, Phi_z,
        f_z;
    ...
    while ((fabs (Tzold - Tznew) > tolerance) && count < maxNumIter)
      {
          ...
          TA_z =
              TA * (-deltak * TDamIndex_z * Tpk - deltak * TDamIndex * Tpk_z);
          Tbetak_z = beta * TA_z;
...
DegradingPinchedBW::getCopy (void)
{
    DegradingPinchedBW *theCopy =
        new DegradingPinchedBW (this->getTag (), m, Fy, xu, alpha, ko, n, eta,
                                beta, rhoeps,
...
int
DegradingPinchedBW::recvSelf (int cTag, Channel & theChannel,
