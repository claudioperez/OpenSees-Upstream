...
ConcretewBeta::ConcretewBeta(int tag, double fpc1, double ec01,
                             ...
                             double ecc1):UniaxialMaterial(tag,
                                                           MAT_TAG_ConcretewBeta),
fpc(fpc1), ec0(ec01), fcint(fcint1), ecint(ecint1), fcres(fcres1),
ecres(ecres1), fct(fct1), ftint(ftint1), etint(etint1), ftres(ftres1),
etres(etres1), lambda(lambda1), alpha(alpha1), bint(bint1),
etbint(etbint1), bres(bres1), etbres(etbres1), M(M1), E0(E01), fcc(fcc1),
ecc(ecc1)
{
...
int ConcretewBeta::setTrialStrainwBeta(double newStrain, double et,
                                       double strainRate)
{
    //rever to last value.
...
UniaxialMaterial *ConcretewBeta::getCopy(void)
{
    ConcretewBeta *theCopy =
        new ConcretewBeta(this->getTag(), this->fpc, this->ec0,
                          this->fcint,
                          this->ecint,
                          this->fcres, this->ecres, this->fct, this->ftint,
                          this->etint, this->ftres, this->etres,
                          this->lambda,
                          this->alpha, this->bint, this->etbint,
                          this->bres,
...
int ConcretewBeta::recvSelf(int commitTag, Channel & theChannel,
...
int ConcretewBeta::setValues(double newStrain, double beta,
                             double &newStress, double &newStressPure,
                             double &newTangent)
{
    if (E0 == 0) {
    ...
    if (de > 0) {
        ...
        if ((newStrain - CMaxStrainCompr) <= DBL_EPSILON) {
            ...
            if (newStrain >= eaftc && CMaxStrainTens > 0) {
                ...
                double temp2 = (fpc - fcc) / pow(ec0 - ecc,
                                                 3) * pow(newStrain - ecc,
                                                          2);
...
Response *ConcretewBeta::setResponse(const char **argv, int argc,
...
int ConcretewBeta::getResponse(int responseID, Information & matInfo)
{
    ...
    if ((responseID > 10000) && (responseID < 20000)) {
        matInfo.setDouble(this->getStressSensitivity(responseID - 10000,
                                                     false));
        return 0;
