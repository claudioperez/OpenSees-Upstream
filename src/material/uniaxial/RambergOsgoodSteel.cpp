...
OPS_RambergOsgoodSteel(void)
{
    ...
    theMaterial =
        new RambergOsgoodSteel(iData[0], dData[0], dData[1], dData[2],
                               dData[3]);

    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type RambergOsgoodSteel\n";
        return 0;
    }
    return theMaterial;
}
...
RambergOsgoodSteel::RambergOsgoodSteel(int tag,
                                       double _Fy, double _E0,
                                       double _rezaA,
                                       double _rezaN):UniaxialMaterial(tag,
                                                                       MAT_TAG_RambergOsgoodSteel),
...
RambergOsgoodSteel::RambergOsgoodSteel(int tag, double _Fy,
                                       double _E0):UniaxialMaterial(tag,
                                                                    MAT_TAG_RambergOsgoodSteel),
Fy(_Fy), E0(_E0)
{
    konP = 0;
...
RambergOsgoodSteel::RambergOsgoodSteel(void):UniaxialMaterial(0,
                                                              MAT_TAG_RambergOsgoodSteel)
{
    konP = 0;
}
...
{
    RambergOsgoodSteel *theCopy =
        new RambergOsgoodSteel(this->getTag(), Fy, E0, rezaAA, rezaNN);

    return theCopy;
}

double RambergOsgoodSteel::getInitialTangent(void)
{
    return E0;
}

int RambergOsgoodSteel::setTrialStrain(double trialStrain,
                                       double strainRate)
{
    double epsy = Fy / E0;
    ...
    if (kon == 2 || kon == 1) {
        ...
        while (M >= 0.0001) {
            ...
            F[kk] =
                (trialSig[kk] / E0) +
                rezaAA * (pow((trialSig[kk] / sigs0), rezaNN)) -
                fabs(eps - epsr);
            dF[kk] =
                (1 / E0) +
                rezaAA * (1 / sigs0) * rezaNN *
                (pow((trialSig[kk] / sigs0), (rezaNN - 1)));
            ...
            if (kk == 1000) {
                opserr <<
                    "NewtonRaphson method does NOT converge at eps="
                    << eps << "\n";
                M = 0;
            }
        }
...
double RambergOsgoodSteel::getStrain(void)
{
    return eps;
}

double RambergOsgoodSteel::getStress(void)
{
    return sig;
}

double RambergOsgoodSteel::getTangent(void)
{
    return e;
}
...
int RambergOsgoodSteel::recvSelf(int commitTag, Channel & theChannel,
...
{
    s << "RambergOsgoodSteel:(strain, stress, tangent) " << eps << " " <<
        sig << " " << e << endln;
}
