...
void *OPS_SeriesMaterial(void)
{
    ...
    if (OPS_GetIntInput(&numArgs, iData) != 0) {
        opserr << "WARNING invalid data for uniaxialMaterial Series" <<
            endln;
        return 0;
    }
    ...
    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type Series\n";
        return 0;
    }
...
SeriesMaterial::SeriesMaterial(int tag, int num,
                               ...
                               int maxIter,
                               double tol):UniaxialMaterial(tag,
                                                            MAT_TAG_SeriesMaterial),
Tstrain(0.0), Cstrain(0.0), Tstress(0.0), Cstress(0.0), Ttangent(0.0),
Ctangent(0.0), maxIterations(maxIter), tolerance(tol), stress(0), flex(0),
strain(0), initialFlag(false), numMaterials(num), theModels(0)
{
    theModels = new UniaxialMaterial *[numMaterials];

    if (theModels == 0) {
        opserr <<
            "SeriesMaterial::SeriesMaterial -- failed to allocate material array\n";
        exit(-1);
    }
    ...
    for (i = 0; i < numMaterials; i++) {
        ...
        if (theModels[i] == 0) {
            opserr <<
                "SeriesMaterial::SeriesMaterial -- failed to get copy of material: "
                << i << endln;
            exit(-1);
        }
    }
    ...
    if (strain == 0) {
        opserr <<
            "SeriesMaterial::SeriesMaterial -- failed to allocate strain array\n";
        exit(-1);
    }
    ...
    if (stress == 0) {
        opserr <<
            "SeriesMaterial::SeriesMaterial -- failed to allocate stress array\n";
        exit(-1);
    }
    ...
    if (flex == 0) {
        opserr <<
            "SeriesMaterial::SeriesMaterial -- failed to allocate flex array\n";
        exit(-1);
    }
...
SeriesMaterial::SeriesMaterial():UniaxialMaterial(0, MAT_TAG_SeriesMaterial),
...
double SeriesMaterial::getStrain(void)
{
    return Tstrain;
}

double SeriesMaterial::getStress(void)
{
    return Tstress;
}

double SeriesMaterial::getTangent(void)
{
    return Ttangent;
}


double SeriesMaterial::getInitialTangent(void)
{
    double kf = 0.0;
    double k = 0.0;
...
int SeriesMaterial::commitState(void)
{
    int err = 0;
...
int SeriesMaterial::revertToLastCommit(void)
{
    int err = 0;
...
int SeriesMaterial::revertToStart(void)
{
    int err = 0;
...
UniaxialMaterial *SeriesMaterial::getCopy(void)
{
...
int SeriesMaterial::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
    ...
    if (res < 0) {
        opserr <<
            "SeriesMaterial::sendSelf -- failed to send data Vector\n";
        return res;
    }
    ...
    if (res < 0) {
        opserr <<
            "SeriesMaterial::sendSelf -- failed to send classTags ID\n";
        return res;
    }
    ...
    for (i = 0; i < numMaterials; i++) {
        ...
        if (res < 0) {
            opserr <<
                "SeriesMaterial::sendSelf -- failed to send UniaxialMaterial: "
                << i << endln;
            return res;
        }
    }
...
int SeriesMaterial::recvSelf(int cTag, Channel & theChannel,
                             FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0) {
        opserr <<
            "SeriesMaterial::recvSelf -- failed to receive data Vector\n";
        return res;
    }
    ...
    if (numMaterials != (int) data(1)) {
        ...
        if (theModels == 0) {
            opserr <<
                "SeriesMaterial::recvSelf -- failed to allocate UniaxialMaterial array\n";
            return -1;
        }
        ...
        if (strain == 0) {
            opserr <<
                "SeriesMaterial::recvSelf -- failed to allocate strain array\n";
            return -1;
        }
        ...
        if (stress == 0) {
            opserr <<
                "SeriesMaterial::recvSelf -- failed to allocate stress array\n";
            return -1;
        }
        ...
        if (flex == 0) {
            opserr <<
                "SeriesMaterial::recvSelf -- failed to allocate flex array\n";
            return -1;
        }
    }
    ...
    if (res < 0) {
        opserr <<
            "SeriesMaterial::recvSelf -- failed to receive classTags ID\n";
        return res;
    }
    ...
    for (i = 0; i < numMaterials; i++) {
        ...
        if (theModels[i] == 0) {
            opserr <<
                "SeriesMaterial::recvSelf -- failed to get a newUniaxialMaterial\n";
            return -1;
        }
        ...
        if (res < 0) {
            opserr <<
                "SeriesMaterial::recvSelf -- failed to receive UniaxialMaterial: "
                << i << endln;
            return res;
        }
    }
...
void SeriesMaterial::Print(OPS_Stream & s, int flag)
{
...
Response *SeriesMaterial::setResponse(const char **argv, int argc,
                                      OPS_Stream & theOutput)
{
    ...
    // strain
        ...
        theResponse =
            new MaterialResponse(this, 100, Vector(numMaterials));
        ...
        if (argc > 1) {
            ...
            if (matNum >= 0 && matNum < numMaterials)
                theResponse =
                    theModels[matNum]->setResponse(&argv[2], argc - 2,
                                                   theOutput);
        }
    }
...
int SeriesMaterial::getResponse(int responseID, Information & info)
{
