...
void *OPS_EPPGapMaterial()
{
    ...
    if (numArgs < 4) {
        opserr <<
            "Invalid #args,  want: uniaxialMaterial ElasticPPGap tag E Fy gap <eta damage>\n";
        return 0;
    }
    ...
    if (OPS_GetIntInput(&numData, &tag) != 0) {
        opserr << "WARNING invalid tag for uniaxialMaterial EPPGap" <<
            endln;
        return 0;
    }
    ...
    if (numData > 0) {
        ...
        if (strcmp(damagestr, "damage") == 0
            || strcmp(damagestr, "Damage") == 0) {
            damage = 1;
        }
    }
    // Parsing was successful, allocate the material
    theMaterial =
        new EPPGapMaterial(tag, dData[0], dData[1], dData[2], dData[3],
                           damage);
    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type EPPGap\n";
        return 0;
    }
...
EPPGapMaterial::EPPGapMaterial(int tag, double e, double fyl, double gap0,
                               double eta0,
                               int accum):UniaxialMaterial(tag,
                                                           MAT_TAG_EPPGap),
E(e), fy(fyl), gap(gap0), eta(eta0), minElasticYieldStrain(gap0),
damage(accum), parameterID(0), SHVs(0), EnergyP(0.0), trialStrain(0.0),
commitStrain(0.0)
{
    if (E == 0.0) {
        opserr <<
            "EPPGapMaterial::EPPGapMaterial -- E is zero, continuing with E = fy/0.002\n";
        if (fy != 0.0)
            E = fabs(fy) / 0.002;
        else {
            opserr <<
                "EPPGapMaterial::EPPGapMaterial -- E and fy are zero\n";
            exit(-1);
        }
    }
    ...
    if (fy * gap < 0) {
        opserr <<
            "EPPGapMaterial::EPPGapMaterial -- Alternate signs on fy and gap encountered, continuing anyway\n";
    }

    if ((eta >= 1) || (eta <= -1)) {
        opserr <<
            "EPPGapMaterial::EPPGapMaterial -- value of eta must be -1 <= eta <= 1, setting eta to 0\n";
        eta = 0;
    }
...
EPPGapMaterial::EPPGapMaterial():UniaxialMaterial(0, MAT_TAG_EPPGap),
E(0.0), fy(0.0), gap(0.0), eta(0.0), minElasticYieldStrain(0.0),
maxElasticYieldStrain(0.0), damage(0), parameterID(0), SHVs(0),
EnergyP(0.0), trialStrain(0.0), trialStress(0.0), trialTangent(0.0),
...
double EPPGapMaterial::getStrain(void)
{
    return trialStrain;
}

double EPPGapMaterial::getStress(void)
{
    return trialStress;

}

double EPPGapMaterial::getTangent(void)
{
    return trialTangent;
}

double EPPGapMaterial::getInitialTangent(void)
{
...
int EPPGapMaterial::commitState(void)
{
    if (fy >= 0) {
        if (trialStrain > maxElasticYieldStrain) {
            ...
            minElasticYieldStrain = trialStrain - trialStress / E;
                   ...
                   && damage == 0) {
            maxElasticYieldStrain =
                (trialStrain - eta * gap) / (1 - eta) + fy / E;
    ...
    EnergyP +=
        0.5 * (commitStress + trialStress) * (trialStrain - commitStrain);
...
int EPPGapMaterial::revertToLastCommit(void)
{
...
int EPPGapMaterial::revertToStart(void)
{
    trialStrain = 0.0;
    maxElasticYieldStrain = fy / E + gap;
...
UniaxialMaterial *EPPGapMaterial::getCopy(void)
{
    EPPGapMaterial *theCopy =
        new EPPGapMaterial(this->getTag(), E, fy, gap, eta, damage);
...
int EPPGapMaterial::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
...
int EPPGapMaterial::recvSelf(int cTag, Channel & theChannel,
...
void EPPGapMaterial::Print(OPS_Stream & s, int flag)
{
    if (flag == OPS_PRINT_PRINTMODEL_MATERIAL) {
        s << "EPPGap tag: " << this->getTag() << endln;
        s << "  E: " << E << ", kinematic hardening ratio: " << eta <<
            endln;
        s << "  fy: " << fy << endln;
...
int EPPGapMaterial::setParameter(const char **argv, int argc,
                                 Parameter & param)
{
...
int EPPGapMaterial::updateParameter(int parameterID, Information & info)
{
    switch (parameterID) {
    case 1:
        E = info.theDouble;
...
int EPPGapMaterial::activateParameter(int passedParameterID)
{
    ...
    EPPGapMaterial::getStressSensitivity(int gradIndex, bool conditional)
{
    ...
    if (fy >= 0) {
        if (trialStrain > maxElasticYieldStrain) {
            dsdh =
                dfydh + (-dgapdh - dfydh / E +
                         fy / (E * E) * dEdh) * eta * E + (trialStrain -
                                                           gap -
                                                           fy / E) * eta *
                dEdh;
        } else if (trialStrain < minElasticYieldStrain) {
            ...
            dsdh =
                dEdh * (trialStrain - minElasticYieldStrain) -
                E * depsyMindh;
        }
    } else {
        if (trialStrain < maxElasticYieldStrain) {
...
double EPPGapMaterial::getInitialTangentSensitivity(int gradIndex)
{
    if (parameterID == 1)
        return 1.0;
    else
...
int EPPGapMaterial::commitSensitivity(double dedh, int gradIndex,
                                      int numGrads)
{
    if (SHVs == 0) {
