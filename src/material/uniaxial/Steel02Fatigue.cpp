...
void *OPS_Steel02Fatigue()
{
    ...
    if (OPS_GetIntInput(&numData, iData) != 0) {
        opserr << "WARNING invalid uniaxialMaterial Steel02Fatigue tag" <<
            endln;
        return 0;
    }
    ...
    if (numData != 9 && numData != 12 && numData != 16 && numData != 17) {
        opserr << "Invalid #args, want: uniaxialMaterial Steel02Fatigue "
            << iData[0] <<
            " fy? E? b? Cd? Cf? alpha? beta? minStrain? maxStrain? <R0? cR1? cR2? <a1? a2? a3? a4?>>"
            << endln;
        return 0;
    }
    ...
    if (numData == 9) {
        if (OPS_GetDoubleInput(&numData, dData) != 0) {
            opserr << "Invalid double: uniaxialMaterial Steel02Fatigue "
                << iData[0] <<
                " fy? E? b? Cd? Cf? alpha? beta? minStrain? maxStrain? <R0? cR1? cR2? <a1? a2? a3? a4?>>"
                << endln;
            return 0;
        ...
        theMaterial =
            new Steel02Fatigue(iData[0], dData[0], dData[1], dData[2],
                               dData[3], dData[4], dData[5], dData[6],
                               dData[7], dData[8]);

    } else if (numData == 12) {
        if (OPS_GetDoubleInput(&numData, dData) != 0) {
            opserr << "Invalid int: uniaxialMaterial Steel02Fatigue " <<
                iData[0] <<
                " fy? E? b? Cd? Cf? alpha? beta? minStrain? maxStrain? <R0? cR1? cR2? <a1? a2? a3? a4?>>"
                << endln;
            return 0;
        ...
        if (OPS_GetDoubleInput(&numData, dData) != 0) {
            opserr << "Invalid arggs: uniaxialMaterial Steel02Fatigue " <<
                iData[0] <<
                " fy? E? b? Cd? Cf? alpha? beta? minStrain? maxStrain? <R0? cR1? cR2? <a1? a2? a3? a4?>>"
                << endln;
            return 0;
    ...
    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type Steel02Fatigue Material\n";
        return 0;
    }
...
Steel02Fatigue::Steel02Fatigue(int tag,
                               ...
                               double _a4, double _Cd, double _Cf,
                               double _Alpha, double _Beta,
                               double _minStrain, double _maxStrain,
                               double sigInit):UniaxialMaterial(tag,
                                                                MAT_TAG_Steel02Fatigue),
Fy(_Fy), E0(_E0), b(_b), R0(_R0), cR1(_cR1), cR2(_cR2), a1(_a1), a2(_a2),
a3(_a3), a4(_a4), Cd(_Cd), Cf(_Cf), Alpha(_Alpha), Beta(_Beta),
minStrain(_minStrain), maxStrain(_maxStrain), sigini(sigInit)
{
    konP = 0;
    kon = 0;
    eP = E0;
...
Steel02Fatigue::Steel02Fatigue(int tag,
                               double _Fy, double _E0, double _b,
                               double _R0, double _cR1, double _cR2,
                               double _Cd, double _Cf, double _Alpha,
                               double _Beta, double _minStrain,
                               double _maxStrain):UniaxialMaterial(tag,
                                                                   MAT_TAG_Steel02Fatigue),
Fy(_Fy), E0(_E0), b(_b), R0(_R0), cR1(_cR1), cR2(_cR2), Cd(_Cd), Cf(_Cf),
Alpha(_Alpha), Beta(_Beta), minStrain(_minStrain), maxStrain(_maxStrain),
sigini(0.0)
{
    konP = 0;
...
Steel02Fatigue::Steel02Fatigue(int tag, double _Fy, double _E0, double _b,
                               ...
                               double _maxStrain):UniaxialMaterial(tag,
                                                                   MAT_TAG_Steel02Fatigue),
Fy(_Fy), E0(_E0), b(_b), Cd(_Cd), Cf(_Cf), Alpha(_Alpha), Beta(_Beta),
minStrain(_minStrain), maxStrain(_maxStrain), sigini(0.0)
{
    konP = 0;
...
Steel02Fatigue::Steel02Fatigue(void):UniaxialMaterial(0,
                                                      MAT_TAG_Steel02Fatigue)
{
    konP = 0;
}
...
int Steel02Fatigue::commitSteel(void)
{
    epsminP = epsmin;
    epsmaxP = epsmax;
    epsplP = epspl;
...
UniaxialMaterial *Steel02Fatigue::getCopy(void) // Add Fatigue... Need to add additional variables later
{
    Steel02Fatigue *theCopy =
        new Steel02Fatigue(this->getTag(), Fy, E0, b, R0, cR1, cR2, a1, a2,
                           a3, a4, Cd, Cf, Alpha, Beta, minStrain,
                           maxStrain,
                           sigini);
...
double Steel02Fatigue::getInitialTangent(void)  // Added Fatigue
{
    return E0;
}

int Steel02Fatigue::setTrialStrain(double trialStrain, double strainRate)
{
    ...
    if (kon == 2 && deps > 0.0) {
        ...
        epss0 =
            (Fy * shft - Esh * epsy * shft - sigr + E0 * epsr) / (E0 -
                                                                  Esh);
...
double Steel02Fatigue::getStrain(void)  // Added Fatigue
{
    return eps;
}

double Steel02Fatigue::getStress(void)  // Added Fatigue
{
    double modifier = 1.0;
...
double Steel02Fatigue::getTangent(void) // Added Fatigue
{
    double modifier = 1.0;
    double damageloc = 1.0 - Fatigue_Dmax + Fatigue_DL;
    if (Fatigue_Cfailed)
...
int Steel02Fatigue::commitState(void)   // Added Fatigue
{
    //if (Fatigue_Cfailed) {
            ...
            if (Fatigue_X < Fatigue_Y) {
                ...
                if (Fatigue_PCC == 1) {
                    ...
                    Fatigue_DI =
                        Fatigue_DI +
                        1.0 /
                        fabs(pow((Fatigue_X / Fatigue_E0), 1 / Fatigue_m));
                    Lambda_SR = Zd * Fatigue_DI;
                    //added 6/9/2006
                ...
                if (Fatigue_PCC == 1) {
                    ...
                    Fatigue_DI =
                        Fatigue_DI +
                        1.0 /
                        fabs(pow((Fatigue_Y / Fatigue_E0), 1 / Fatigue_m));
                    Lambda_SR = Zd * Fatigue_DI;
                    //added 6/9/2006
    ...
    if (Fatigue_Cfailed == false) {
        ...
        if (Fatigue_B == 0 && Fatigue_C == 0 && Fatigue_D == 0) {
            ...
            if (fabs(Fatigue_A - trialStrain) >
                fabs(Fatigue_A - Fatigue_B)) {
                ...
                if (fabs(Fatigue_C - trialStrain) < 1e-10) {
                    ...
                    Fatigue_DL =
                        ...
                        1.0 /
                        fabs(pow
                             ((fabs(Fatigue_C - trialStrain) /
                               Fatigue_E0), 1 / Fatigue_m));
                    // added 6/9/2006
                    //values for recorder
...
int Steel02Fatigue::revertToLastCommit(void)    // Added Fatigue
{
    epsmin = epsminP;
    epsmax = epsmaxP;
    epspl = epsplP;
...
int Steel02Fatigue::revertToStart(void) // Add Fatigue
{
    eP = E0;
    epsP = 0.0;
    sigP = 0.0;
...
int Steel02Fatigue::sendSelf(int commitTag, Channel & theChannel)
{
...
int Steel02Fatigue::recvSelf(int commitTag, Channel & theChannel,
...
void Steel02Fatigue::Print(OPS_Stream & s, int flag)
{
