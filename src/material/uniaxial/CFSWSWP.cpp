...
OPS_CFSWSWP (void)
{
    ...
    theMaterial =
        new CFSWSWP (iData[0], dData[0], dData[1], dData[2], dData[3],
    ...
    if (theMaterial == 0)
      {
          opserr <<
              "WARNING could not create uniaxialMaterial of type CFSWSWP\n";
          return 0;
      }
...
void
CFSWSWP::lateralShearStrength (void)
{
    Precision = 100;
    E = 203000.00;
    int nstud = 0;
    ...
    a = (((width / 2) / screw_Spacing) -
         floor ((width / 2) / screw_Spacing)) * screw_Spacing;
    if ((a - dis) >= 0)
    ...
    a = (((hight / 2) / screw_Spacing) -
         floor ((hight / 2) / screw_Spacing)) * screw_Spacing;
    if ((a - dis) >= 0)
      {
          a = ((width / 2) - dis);
    ...
    a = ((deltay / screw_Spacing) -
         floor (deltay / screw_Spacing)) * screw_Spacing;
    ...
    if ((a - dis) >= 0)
      {
          D += (2 *
                sqrt ((pow (((width / 2) - dis), 2) +
                       pow ((screw_Spacing * (l + w)), 2))));
      }
    ...
    Alphab =
        pow (6 / Cu, 2) * pow (6 / (screw_Spacing / 25.4), (1.3 * nc) / Cu);
    if ((Cu > 30) && (Cu < 50))
        Alphav = Alphab = 0.06;
    As = ts * width;
    double b = width;
    Is = ts * (pow (b, 3) / 12);
    Ks = ((Gs * As) / (1.2 * hight)) * Alphav +
        ((3 * Es * Is) / pow (hight, 3)) * Alphab;
    Kf = (nstud * 3 * Ifi * E / pow (hight, 3)) +
        ((nstude * 3 * E * Ife) / pow (hight, 3));
    double r, fo;
...
int
CFSWSWP::setTrialStrain (double strain, double CstrainRate)
{
...
double
CFSWSWP::getStrain (void)
{
    return Tstrain;
}

double
CFSWSWP::getStress (void)
{
    return Tstress;
}

double
CFSWSWP::getTangent (void)
{
    return Ttangent;
}

double
CFSWSWP::getInitialTangent (void)
{
    return envlpPosStress (0) / envlpPosStrain (0);
}

int
CFSWSWP::commitState (void)
{
    Cstate = Tstate;
...
int
CFSWSWP::revertToLastCommit (void)
{
...
UniaxialMaterial *
CFSWSWP::getCopy (void)
{
    CFSWSWP *theCopy = new CFSWSWP (this->getTag (),
                                    hight, width, fuf, tf, Ife, Ifi, ts, np,
                                    ds, Vs, screw_Spacing, nc, type, A, L);
...
int
CFSWSWP::sendSelf (int commitTag, Channel & theChannel)
{
    return -1;
}

int
CFSWSWP::recvSelf (int commitTag, Channel & theChannel,
...
void
CFSWSWP::Print (OPS_Stream & s, int flag)
{
...
double
CFSWSWP::GetTangentFromCurve (double Strain)
{
    int i = 0;
    int Neg = 0;
...
double
CFSWSWP::GetStressFromCurve (double Strain)
{
    int i = 0;
    int Neg = 0;
    if (Strain < 0)
...
void
CFSWSWP::getstate (double u, double du)
{
    int cid = 0;
    int cis = 0;
    int newState = 0;
...
double
CFSWSWP::posEnvlpStress (double u)
{
    double k = 0.0;
    int i = 0;
    double f = 0.0;
...
double
CFSWSWP::posEnvlpTangent (double u)
{
    double k = 0.0;
    int i = 0;
    while (k == 0.0 && i <= 5)
...
double
CFSWSWP::negEnvlpStress (double u)
{
    double k = 0.0;
    int i = 0;
    double f = 0.0;
...
double
CFSWSWP::negEnvlpTangent (double u)
{
    double k = 0.0;
    int i = 0;
    while (k == 0.0 && i <= 5)
...
void
CFSWSWP::getState3 (Vector & state3Strain, Vector & state3Stress,
                    double kunload)
{
...
void
CFSWSWP::getState4 (Vector & state4Strain, Vector & state4Stress,
                    double kunload)
{
...
double
CFSWSWP::Envlp3Tangent (Vector s3Strain, Vector s3Stress, double u)
{
    double k = 0.0;
    k = Spline3.EvalT (u);
    if (k != 10e8)
...
double
CFSWSWP::Envlp4Tangent (Vector s4Strain, Vector s4Stress, double u)
{
    double k = 0.0;
    int i = 0;
    k = Spline4.EvalT (u);
...
double
CFSWSWP::Envlp3Stress (Vector s3Strain, Vector s3Stress, double u)
{
    double k = 0.0;
    int i = 0;
    double f = 0.0;
...
double
CFSWSWP::Envlp4Stress (Vector s4Strain, Vector s4Stress, double u)
{
    double k = 0.0;
    int i = 0;
    double f = 0.0;
...
void
CFSWSWP::updateDmg (double strain, double dstrain)
{
    double tes = 0.0;
    double umaxAbs =
