...
CFSWSWP::CFSWSWP(int tag,
                 ...
                 double sc, double nc, double type, double A,
                 double L):UniaxialMaterial(tag, MAT_TAG_Pinching4),
hight(H), width(B), fuf(fuf), tf(tf), Ife(Ife), Ifi(Ifi), ts(ts), np(np),
ds(ds), Vs(Vs), screw_Spacing(sc), nc(nc), type(type), A(A), L(L),
envlpPosStress(7), envlpPosStrain(7), envlpNegStress(7), envlpNegStrain(7),
tagMat(tag), gammaDLimit(0.0), gammaFLimit(0.0), gammaE(10.0),
rDispP(0.488), rForceP(0.183), uForceP(-0.08), rDispN(0.488),
rForceN(0.244), uForceN(-0.08), state3Stress(4), state3Strain(4),
state4Stress(4), state4Strain(4), envlpPosDamgdStress(7),
envlpNegDamgdStress(7), TnCycle(0.0), CnCycle(0.0)
{
    double fdeg, ddeg;
...
void CFSWSWP::SetEnvelope(void)
{
    ...
    double u =
        (strain1p > -strain1n) ? 1e-20 * strain1p : -1e-20 * strain1n;
    ...
    double XAdvance =
        XFit[Precision * (Size - 3) - 1] - XFit[Precision * (Size - 3) -
                                                2];
    double YAdvance =
        YFit[Precision * (Size - 3) - 1] - YFit[Precision * (Size - 3) -
                                                2];
    double Tangente =
...
void CFSWSWP::getState3(Vector & state3Strain, Vector & state3Stress,
                        double kunload)
{
...
void CFSWSWP::getState4(Vector & state4Strain, Vector & state4Stress,
                        double kunload)
{
