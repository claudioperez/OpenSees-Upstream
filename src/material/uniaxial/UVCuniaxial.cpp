...
OPS_UVCuniaxial (void)
{
    ...
    if (OPS_GetIntInput (&nInputsToRead, materialTag) != 0)
      {
          opserr << "WARNING invalid uniaxialMaterial UVCuniaxial tag" <<
              endln;
          return 0;
      }
    ...
    theMaterial = new UVCuniaxial (materialTag[0],
                                   ...
                                   updProps[0], updProps[1], cK, gammaK);
 ...
 */
UVCuniaxial::UVCuniaxial (int tag, double E, double sy0, double qInf,
 ...
 */
void
UVCuniaxial::returnMapping (double strainIncrement)
{
    ...
    while ((!converged) && (iterationNumber < MAXIMUM_ITERATIONS))
      {
          ...
          if (abs (plasticStrainIncrement) >
              abs (stressTrial / elasticModulus))
            {
                plasticStrainIncrement =
                    sgn <
                    double >(plasticStrainIncrement) * 0.95 *
                    abs (stressTrial / elasticModulus);
            }
          ...
          for (int i = 0; i < nBackstresses; ++i)
            {
                alphaKTrial[i] =
                    sgn < double >(stressRadius) * cK[i] / gammaK[i] - (sgn <
                                                                        double
                                                                        >
                                                                        (stressRadius)
                                                                        *
    ...
    if (iterationNumber == MAXIMUM_ITERATIONS - 1)
      {
          opserr <<
              "WARNING: return mapping in UVCuniaxial does not converge!" <<
              endln;
          opserr << "\tStrain increment = " << strainIncrement << endln;
          opserr << "\tExiting with phi = " << phi << " > " << RETURN_MAP_TOL
              << endln;
      }
...
void
UVCuniaxial::calculateStiffness ()
{
    ...
    if (!plasticLoading)
    ...
    else
      {
          ...
          plasticModulus = bIso * (qInf - sigmaY1) - aIso * (dInf - sigmaY2);
          for (int i = 0; i < nBackstresses; ++i)
            {
 ...
 */
int
UVCuniaxial::setTrialStrain (double strain, double strainRate)
{
 ...
 */
int
UVCuniaxial::revertToLastCommit ()
{
 ...
 */
int
UVCuniaxial::sendSelf (int commitTag, Channel & theChannel)
{
 ...
 */
int
UVCuniaxial::recvSelf (int commitTag, Channel & theChannel,
 ...
 */
void
UVCuniaxial::Print (OPS_Stream & s, int flag)
{
