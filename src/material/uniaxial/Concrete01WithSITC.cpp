    ...
    if (numdata > 0)
      {
          ...
          mat =
              new Concrete01WithSITC (tag, data[0], data[1], data[2], data[3],
                                      endStrainSITC);
      }
    else
      {
          mat =
              new Concrete01WithSITC (tag, data[0], data[1], data[2],
                                      data[3]);
      }
...
Concrete01WithSITC::Concrete01WithSITC
    (int tag, double FPC, double EPSC0, double FPCU, double EPSCU,
     double endStrainSITC):
UniaxialMaterial (tag, MAT_TAG_Concrete01WithSITC), fpc (FPC), epsc0 (EPSC0),
fpcu (FPCU), epscu (EPSCU), CminStrain (0.0), CendStrain (0.0), Cstrain (0.0),
Cstress (0.0), CmaxStrain (0.0), CslopeSITC (0.0),
CendStrainSITC (endStrainSITC), Cindex (0),
CsmallStrainIndex (0)
{
    //count++;
...
int
Concrete01WithSITC::setTrialStrain (double strain, double strainRate)
{
    // Set trial strain
    Tstrain = strain;
    ...
    if (Tstrain < 0.0)
      {                         // compression
          if (Tstrain <= CminStrain)
            ...
            {                   // anywhere in compression greater than minimum strain
                if (dStrain <= 0.0)
                      ...
                      if (Cindex == 2 || Cindex == 1)
                      ...
                      else if (Cindex == 5)
                        {
                            if (Tstrain <= CendStrainSITC
                                && Cstrain >= CendStrainSITC)
                              {
                                  Ttangent = TslopeSITC;
                                  Tstress =
                                      TslopeSITC * (Tstrain - CendStrainSITC);
                                  Tindex = 3;
                              }
                            else if (Tstrain <= TendStrain)
                              {
                                  Ttangent = TunloadSlope;
                                  Tstress =
                                      TunloadSlope * (Tstrain - TendStrain);
                                  Tindex = 2;
                              }
                            else
                              {
                      ...
                      else
                        {
                            ...
                            opserr <<
                                "something in compression is wrong!! Cstrain "
                                << endln;
                        }
                  }
                else
                      ...
                      if (Cindex == 1 || Cindex == 2)
                      ...
                      else if (Cindex == 3)
                        {
                            ...
                            if (Tstress > 0.0)
                              {
                                  opserr <<
                                      "THERE IS A PROBLEM IN UNLOADING IN COMPRESSION!!!"
                                      << endln;
                              }
                        }
                      else if (Cindex == 5)
                        ...
                        {
                            opserr <<
                                "Something is wrong in tension!!!! Cindex is "
                                << endln;
                        }
                  }
            }
...
int
Concrete01WithSITC::setTrial (double strain, double &stress, double &tangent,
                              double strainRate)
{
    TminStrain = CminStrain;
    //TmaxStrain = CmaxStrain;
          ...
          if (dStrain > 0.0)
            {                   // going toward tension
                if (Cindex == 1 || Cindex == 2 || Cindex == 5 || Cindex == 0)
                  ...
                  {
                      opserr <<
                          " something is wrong in tension loading!!! Cindex "
                          << endln;
                  }
            }
          else
            {                   // going toward compression
                if (Cindex == 5)
                  {
                      if (Tstrain <= CendStrainSITC
                          && Cstrain >= CendStrainSITC)
                  ...
                  {
                      opserr <<
                          "something is wrong in tension going to compression "
                          << endln;
                  }
            }
...
void
Concrete01WithSITC::getSITCslope ()
{
...
double
Concrete01WithSITC::getStress ()
{
    return Tstress;
}

double
Concrete01WithSITC::getStrain ()
{
    return Tstrain;
}

double
Concrete01WithSITC::getTangent ()
{
    return Ttangent;
}

void
Concrete01WithSITC::determineTrialState (double dStrain)
{
    TminStrain = CminStrain;
...
int
Concrete01WithSITC::commitState ()
{
...
int
Concrete01WithSITC::revertToLastCommit ()
{
...
int
Concrete01WithSITC::revertToStart ()
{
    double Ec0 = 2.0 * fpc / epsc0;
...
int
Concrete01WithSITC::sendSelf (int commitTag, Channel & theChannel)
{
    int res = 0;
...
int
Concrete01WithSITC::recvSelf (int commitTag, Channel & theChannel,
                              FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0)
      {
          opserr <<
              "Concrete01WithSITC::recvSelf() - failed to receive data\n";
          this->setTag (0);
      }
    else
      {
...
void
Concrete01WithSITC::Print (OPS_Stream & s, int flag)
{
