...
void *OPS_Concrete01WithSITC()
{
    ...
    if (mat == 0) {
        opserr <<
            "WARNING: failed to create Concrete01WithSITC material\n";
        return 0;
    }
...
Concrete01WithSITC::Concrete01WithSITC
    (int tag, double FPC, double EPSC0, double FPCU, double EPSCU,
     double endStrainSITC):UniaxialMaterial(tag,
                                            MAT_TAG_Concrete01WithSITC),
fpc(FPC), epsc0(EPSC0), fpcu(FPCU), epscu(EPSCU), CminStrain(0.0),
CendStrain(0.0), Cstrain(0.0), Cstress(0.0), CmaxStrain(0.0),
CslopeSITC(0.0), CendStrainSITC(endStrainSITC), Cindex(0),
CsmallStrainIndex(0)
{
    //count++;
...
int Concrete01WithSITC::setTrial(double strain, double &stress,
