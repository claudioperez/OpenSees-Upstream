...
void *OPS_ResilienceMaterialHR(void)
{
    ...
    if (OPS_GetIntInput(&numData, iData) != 0) {
        opserr <<
            "WARNING invalid uniaxialMaterial ResilienceMaterialHR tag" <<
            endln;
        return 0;
    }
    ...
    if (numData != 7) {
        opserr <<
            "Invalid #args, want: uniaxialMaterial ResilienceMaterialHR "
    ...
    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type ResilienceMaterialHR\n";
        return 0;
    }
...
ResilienceMaterialHR::ResilienceMaterialHR(int tag, double pDY, double pPY,
                                           double pDPmax, double pPmax,
                                           double pKe, double pKd,
                                           double pK):UniaxialMaterial(tag,
                                                                       MAT_TAG_ResilienceMaterialHR),
strain(0.0), stress(0.0), tangent(0.0), Cstrain(0.0), Cstress(0.0),
Ctangent(0.0)
{
    this->DY = pDY;
    this->PY = pPY;
...
double ResilienceMaterialHR::getStress(void)
{
    return stress;
};

double ResilienceMaterialHR::getTangent()
{
    switch (mode) {
    case 1:
        {
    ...
    case 2:
        {
            tangent = (Pmax - PY) / (DPmax - DY);
        }
        break;
    case 3:
        {
            tangent = (stressRFMode2 + PY) / (strainRFMode2 + DY);
        }
        break;
    case 4:
        {
    ...
    case 9:
        {
            tangent =
                (-0.85 * Pmax - stressRFMode13) / (strainP9 -
                                                   strainRFMode13);
        }
        break;
    case 10:
    ...
    case 12:
        {
            tangent =
                (0.85 * Pmax - stressRFMode6) / (strainP12 -
                                                 strainRFMode6);
        }
        break;
    case 13:
...
double ResilienceMaterialHR::getInitialTangent(void)
{
    return Ke;
}

double ResilienceMaterialHR::getStrain(void)
{
    return strain;
}

int ResilienceMaterialHR::commitState(void)
{
    Cstrain = strain;
    Cstress = stress;
    Ctangent = tangent;
...
int ResilienceMaterialHR::revertToLastCommit(void)
{
    strain = Cstrain;
    stress = Cstress;
    tangent = Ctangent;
...
int ResilienceMaterialHR::revertToStart(void)
{
    return -1;
}


UniaxialMaterial *ResilienceMaterialHR::getCopy(void)
{
    ResilienceMaterialHR *theCopy =
        new ResilienceMaterialHR(this->getTag(), DY, PY, DPmax, Pmax, Ke,
                                 Kd, coefficient);
    return theCopy;
}

int ResilienceMaterialHR::sendSelf(int cTag, Channel & theChannel)
{
    opserr << "ResilienceMaterialHR::sendSelf() - does not send self\n";
    return -1;
}

int ResilienceMaterialHR::recvSelf(int cTag, Channel & theChannel,
                                   FEM_ObjectBroker & theBroker)
{
    return -1;
}

void ResilienceMaterialHR::Print(OPS_Stream & s, int flag)
{
...
int ResilienceMaterialHR::determineState()
{
    ...
    case 3:
        if (dd > 0) {
            if (strain > strainRFMode2) {
                ...
                stress =
                    (stressRFMode2 + PY) / (strainRFMode2 +
                                            DY) * (strain -
                                                   strainRFMode2) +
                    stressRFMode2;
        } else {
    ...
    case 4:
        if (dd > 0) {
            ...
            if (strain < -DPmax) {
                ...
                stress =
                    (Pmax - PY) / (DPmax - DY) * (strain + DPmax) - Pmax;
        }
        break;
    ...
    case 5:
        if (dd >= 0) {
            if (strain > DY) {
                ...
                stress =
                    (stressRFMode4 - PY) / (strainRFMode4 -
                                            DY) * (strain - DY) + PY;
        } else {
        ...
        if (dd > 0) {
            ...
            if (strain > strainRFMode13) {
                stress =
                    (-0.85 * Pmax - stressRFMode13) / (strainP9 -
                                                       strainRFMode13) *
                    (strain - strainRFMode13) + stressRFMode13;
            } else {
                mode = 13;
                determineState();
            }
        ...
        if (dd > 0) {
            if (strain > strainRFMode6) {
                ...
                stress =
                    (0.85 * Pmax - stressRFMode6) / (strainP12 -
                                                     strainRFMode6) *
                    (strain - strainRFMode6) + stressRFMode6;
        } else {
