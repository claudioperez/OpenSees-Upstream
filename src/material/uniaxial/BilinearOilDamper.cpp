...
BilinearOilDamper::BilinearOilDamper (int tag, double k, double c, double fr,
                                      double pp, double lgap, double nm,
                                      double reltol, double abstol,
                                      double maxhalf):
UniaxialMaterial (tag, MAT_TAG_BilinearOilDamper),
K (k),
C (c),
Fr (fr),
p (pp),
...
double
BilinearOilDamper::getStress (void)
{
    return Tstress;
}

double
BilinearOilDamper::getTangent (void)
{
...
double
BilinearOilDamper::getInitialTangent (void)
{
    //return 0.0;
    return K;                   // MHS
}
...
double
BilinearOilDamper::getDampTangent (void)
{
    //return 0.0;
...
int
BilinearOilDamper::DormandPrince (double vel0, double vel1, double y0,
                                  ...
                                  double &error)
{
    ...
    k4 = f ((vel1 - vel0) * (4. / 5.) + vel0,
            y0 + (44. / 45.) * k1 + (-56. / 15.) * k2 + (32. / 9.) * k3) * h;
    ...
    yt = y0 + (35. / 384.) * k1 + (500. / 1113.) * k3 + (125. / 192.) * k4 +
...
BilinearOilDamper::getCopy (void)
{
    BilinearOilDamper *theCopy =
        new BilinearOilDamper (this->getTag (), K, C, Fr, p, LGap, NM, RelTol,
                               AbsTol, MaxHalf);
...
int
BilinearOilDamper::setParameter (const char **argv, int argc,
                                 Parameter & param)
{
