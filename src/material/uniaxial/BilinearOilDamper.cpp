...
OPS_BilinearOilDamper(void)
{
    if (numBilinearOilDamperMaterials == 0) {
        ...
        opserr <<
            "BilinearOilDamper Model by Sarven Akcelyan and Dimitrios G. Lignos, PhD, McGill University\n";
    }
    ...
    if (numData != 2 && numData != 4 && numData != 5 && numData != 9) {
        opserr <<
            "Invalid #args, want: uniaxialMaterial BilinearOilDamper " <<
            iData[0] <<
            " K? C? <Fr? p?> <LGap?> <NM? RelTol? AbsTol? MaxHalf?>" <<
            endln;
        return 0;
    }
    ...
    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type BilinearOilDamper Material\n";
        return 0;
    }
...
BilinearOilDamper::BilinearOilDamper(int tag, double k, double c,
                                     double fr, double pp, double lgap,
                                     double nm, double reltol,
                                     double abstol,
                                     double maxhalf):UniaxialMaterial(tag,
                                                                      MAT_TAG_BilinearOilDamper),
K(k), C(c), Fr(fr), p(pp), LGap(lgap), NM(nm), RelTol(reltol),
AbsTol(abstol), MaxHalf(maxhalf)
{
    if (p < 0.0) {
        opserr <<
            "BilinearOilDamper::BilinearOilDamper -- p < 0.0, setting to 0.0\n";
        p = 0.0;
    }
...
BilinearOilDamper::BilinearOilDamper():UniaxialMaterial(0, MAT_TAG_BilinearOilDamper),
K(0.0), C(0.0), Fr(0.0), p(0.0), LGap(0.0), NM(0.0), RelTol(0.0),
AbsTol(0.0), MaxHalf(0.0)
{
    this->revertToStart();
}
...
double BilinearOilDamper::getStrain(void)
{
    return Tstrain;
}

double BilinearOilDamper::getStrainRate(void)
{
    return TVel;
}

int BilinearOilDamper::commitState(void)
{
...
int BilinearOilDamper::revertToLastCommit(void)
{
    Tstrain = Cstrain;
    Tstress = Cstress;
    Ttangent = Ctangent;
...
int BilinearOilDamper::revertToStart(void)
{
...
double BilinearOilDamper::sgn(double dVariable)
{
    if (dVariable < 0.0) {
        return -1.0;
    } else {
...
int BilinearOilDamper::DormandPrince(double vel0, double vel1, double y0,
                                     double h, double &yt, double &eps,
                                     double &error)
{
    ...
    k3 = f((vel1 - vel0) * (3. / 10.) + vel0,
           y0 + (3. / 40.) * k1 + (9. / 40.) * k2) * h;

    k4 = f((vel1 - vel0) * (4. / 5.) + vel0,
           y0 + (44. / 45.) * k1 + (-56. / 15.) * k2 +
           (32. / 9.) * k3) * h;

    k5 = f((vel1 - vel0) * (8. / 9.) + vel0,
           y0 + (19372.0 / 6561.0) * k1 + (-25360.0 / 2187.0) * k2 +
           (64448.0 / 6561.0) * k3 + (-212.0 / 729.0) * k4) * h;

    k6 = f((vel1 - vel0) * (1.) + vel0,
           y0 + (9017.0 / 3168.0) * k1 + (-355.0 / 33.0) * k2 +
           (46732.0 / 5247.0) * k3 + (49.0 / 176.0) * k4 +
           (-5103.0 / 18656.0) * k5) * h;

    yt = y0 + (35. / 384.) * k1 + (500. / 1113.) * k3 +
        (125. / 192.) * k4 + (-2187. / 6784.) * k5 + (11. / 84.) * k6;

    k7 = f((vel1 - vel0) * (1.) + vel0, yt) * h;

    error =
        (71. / 57600.) * k1 + (-71. / 16695.) * k3 + (71. / 1920.) * k4 +
...
double BilinearOilDamper::f(double v, double fd)
{
...
UniaxialMaterial *BilinearOilDamper::getCopy(void)
{
    BilinearOilDamper *theCopy =
        new BilinearOilDamper(this->getTag(), K, C, Fr, p, LGap, NM,
                              RelTol,
                              AbsTol, MaxHalf);
...
int BilinearOilDamper::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
...
int BilinearOilDamper::recvSelf(int cTag, Channel & theChannel,
                                FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0) {
        opserr <<
            "BilinearOilDamper::recvSelf() - failed to receive data\n";
        this->setTag(0);
    } else {
...
void BilinearOilDamper::Print(OPS_Stream & s, int flag)
{
...
int BilinearOilDamper::setParameter(const char **argv, int argc,
                                    Parameter & param)
{
...
int BilinearOilDamper::updateParameter(int parameterID, Information & info)
{
    switch (parameterID) {
    case 1:
        K = info.theDouble;
