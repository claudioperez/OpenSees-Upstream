...
OPS_Bond_SP01 (void)
{
    ...
    if (numInput != 7 && numInput != 11)
      {
          opserr <<
              "Invalid #args,  uniaxialMaterial Bond_SP01 tag? fy? sy? fu? su? b? R?";
    ...
    if (numInput == 7)
        theMaterial =
            new Bond_SP01 (iData[0], dData[0], dData[1], dData[2], dData[3],
                           dData[4], dData[5]);
    else
        theMaterial =
            new Bond_SP01 (iData[0], dData[0], dData[1], dData[2], dData[3],
...
Bond_SP01::Bond_SP01
    (int tag, double FY, double SY, double FU, double SU, double KZ, double r,
     double CD, double DB, double FC, double LA):
UniaxialMaterial (tag, MAT_TAG_Bond_SP01), fy (FY), sy (SY), fu (FU), su (SU),
Kz (KZ), R (r), Cd (CD), db (DB), fc (FC),
lba (LA)
{
    // Check units.  Need parameters in ksi and in.
    if (fy >= 1000 || sy >= 1)
        opserr <<
            "WARNING: For the Strain-Penetration Model: input values in ksi and in."
            << endln;
...
Bond_SP01::Bond_SP01
    (int tag, double FY, double SY, double FU, double SU, double KZ,
     double r):
UniaxialMaterial (tag, MAT_TAG_Bond_SP01), fy (FY), sy (SY), fu (FU), su (SU),
Kz (KZ), R (r), Cd (0.0), db (1.0), fc (4.35),
lba (0.0)
{
    // Check units.  Need parameters in ksi and in.
    if (fy >= 1000 || sy >= 1)
        opserr <<
            "WARNING: WARNING: For the Strain-Penetration Model: input values in ksi and in."
            << endln;
...
fy (64.0), sy (0.021), fu (102.2), su (1.0), Kz (0.3), R (.8), Cd (0.0),
db (1.0), fc (4.35), lba (0.0)
{
...
int
Bond_SP01::setTrialStrain (double strain, double strainRate)
{
...
int
Bond_SP01::setTrial (double strain, double &stress, double &tangent,
                     double strainRate)
{
    // Reset history variables to last converged state
    TRSlip = CRSlip;            // Return slip
...
void
Bond_SP01::determineTrialState (double ts, double dslip)
{
                ...
                else            //%% ts < maxrs
                  {
                      if (rsvg > rslip) //%%  turn from compression to tension
                            ...
                            if (ts >= rsvg)     //%%% curve reloading (tension)
                              {
                                  ...
                                  ft = ss /
                                      pow ((pow (1 / suy, R1) + pow (ss, R1)),
                                           (1 / R1));
                                  Tload = ft * maxrl;
                                  st = pow (suy, (1 - R1)) / pow (suy - ssy,
                                                                  2) /
                                      pow ((pow (1 / suy, R1) + pow (ss, R1)),
                                           (1 / R1 + 1));
                                  Ttangent = st * Ere;
                      ...
                      else      //%%  rsvg < rslip turn from linear tension to tension
                        {
                            ...
                            ft = ss / pow ((pow (1 / suy, R1) + pow (ss, R1)),
                                           (1 / R1));
...
void
Bond_SP01::detectStressReversal (double dslip)
{
    if (Tloading > 0.0)         // (Cloading>0): was loading (tension)
      {
          if (dslip < 0.0)      //% (dslip<0): turn unloading (compression)
...
double
Bond_SP01::getEnvelopeStress (double s)
{
    double ss;                  // normalized slip
    double ft;                  // normalized load
    double st;                  // normalized slope
                ...
                if (s < su)
                  {
                      ...
                      suy = (su - sy) / sy;
                      ss = ssy / (suy - ssy);
                      ft = ss / pow ((pow (1 / suy / Kz, Cr) + pow (ss, Cr)),
                                     1 / Cr);
                      load = fy + ft * (fu - fy);
                      st = (pow (suy, (1 - Cr)) / pow (Kz, Cr) /
                            pow ((suy - ssy),
                                 2)) / pow ((pow (1 / suy / Kz, Cr) + pow (ss,
                                                                           Cr)),
                                            (1 / Cr + 1));
                      Ttangent = st * E0;
                  }
...
double
Bond_SP01::getTangent ()
{
    return Ttangent;
}

double
Bond_SP01::getInitialTangent ()
{
    return fy / sy;
}

int
Bond_SP01::commitState ()
{
    // History variables
...
{
    Bond_SP01 *theCopy =
        new Bond_SP01 (this->getTag (), fy, sy, fu, su, Kz, R, Cd, db, fc,
                       lba);
...
int
Bond_SP01::sendSelf (int cTag, Channel & theChannel)
{
    int res = 0;
...
int
Bond_SP01::recvSelf (int cTag, Channel & theChannel,
...
void
Bond_SP01::Print (OPS_Stream & s, int flag)
{
