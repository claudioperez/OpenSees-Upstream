...
Bond_SP01::Bond_SP01
    (int tag, double FY, double SY, double FU, double SU, double KZ,
     double r, double CD, double DB, double FC,
     double LA):UniaxialMaterial(tag, MAT_TAG_Bond_SP01), fy(FY), sy(SY),
fu(FU), su(SU), Kz(KZ), R(r), Cd(CD), db(DB), fc(FC), lba(LA)
{
...
Bond_SP01::Bond_SP01
    (int tag, double FY, double SY, double FU, double SU, double KZ,
     double r):UniaxialMaterial(tag, MAT_TAG_Bond_SP01), fy(FY), sy(SY),
fu(FU), su(SU), Kz(KZ), R(r), Cd(0.0), db(1.0), fc(4.35), lba(0.0)
{
    // Check units.  Need parameters in ksi and in.
    if (fy >= 1000 || sy >= 1)
        opserr <<
...
int Bond_SP01::setTrial(double strain, double &stress, double &tangent,
                        double strainRate)
{
    // Reset history variables to last converged state
    TRSlip = CRSlip;            // Return slip
