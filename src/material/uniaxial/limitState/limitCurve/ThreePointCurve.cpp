...
void *OPS_ThreePointCurve()
{
    ...
    return new ThreePointCurve(tag, eleTag, theDomain,
                               x1, y1, x2, y2, x3, y3, Kdeg, Fres, defType,
                               forType, ndI, ndJ, dof - 1, perpDirn - 1);
}
#endif
...
ThreePointCurve::ThreePointCurve(int tag, int eTag, Domain * theDom,
                                 ...
                                 double Kd, double Fr, int dType,
                                 int fType, int ni, int nj, int df,
                                 int dirn):LimitCurve(tag,
                                                      TAG_ThreePointCurve),
eleTag(eTag), theDomain(theDom), theElement(0), Kdeg(Kd), Fres(Fr),
defType(dType), forType(fType), x1(a1), y1(b1), x2(a2), y2(b2), x3(a3),
y3(b3), ndI(ni), ndJ(nj), dof(df), perpDirn(dirn)
{
    stateFlag = 0;
    count = 0;
...
LimitCurve(0, TAG_ThreePointCurve), eleTag(0), theDomain(0),
theElement(0), Kdeg(0), Fres(0), defType(0), forType(0), x1(0), y1(0),
x2(0), y2(0), x3(0), y3(0)
{
//VOID//
}
...
LimitCurve *ThreePointCurve::getCopy(void)
{
...
int ThreePointCurve::checkElementState(double springForce)
{
    DummyStream dummy;
...
//                                              "AxialCurve::AxialCurve");
    ...
    if (defType == 1)           // maximum chord rotations
    {
        ...
        if (theRotations == 0) {
            opserr <<
                "ThreePointCurve::checkElementState, defType = 1, basicDeformations not implemented in element setResponse"
                << endln;
            return -1;
        }
...
double ThreePointCurve::getDegSlope(void)
{
    return Kdeg;
}


double ThreePointCurve::getResForce(void)
{
    return Fres;
}

double ThreePointCurve::getUnbalanceForce(void)
{
    //Do nothing for this class
    return 0.0;
}

int ThreePointCurve::sendSelf(int commitTag, Channel & theChannel)
{
    return -1;
}

int ThreePointCurve::recvSelf(int commitTag, Channel & theChannel,
                              FEM_ObjectBroker & theBroker)
{
    return -1;
}

void ThreePointCurve::Print(OPS_Stream & s, int flag)
{
...
double ThreePointCurve::findLimit(double x)
{
    double y = 0.0;
