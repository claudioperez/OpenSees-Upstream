...
void *OPS_ShearCurve()
{
    ...
    if (theDomain == 0)
        return 0;
    return new ShearCurve(tag, eleTag, theDomain, rho, fc, b, h, d, Fsw, Kdeg, Fres, defType, forType,  //SDK
                          ndI, ndJ, dof - 1, perpDirn - 1, delta);
}
#endif


ShearCurve::ShearCurve(int tag, int eTag, Domain * theDom, double r, double f, double B, double H, double D, double Fsw,        //SDK
                       double Kd, double Fr,
                       int dType, int fType,
                       int ni, int nj, int df, int dirn,
                       double dd):LimitCurve(tag, TAG_ShearCurve),
eleTag(eTag), theDomain(theDom), theElement(0), Kdeg(Kd), Fres(Fr),
defType(dType), forType(fType), rho(r), fc(f), b(B), h(H), d(D), Fsw(Fsw),
...
LimitCurve *ShearCurve::getCopy(void)
{
    ShearCurve *theCopy = new ShearCurve(this->getTag(),
                                         eleTag, theDomain, rho, fc, b, h,
                                         d,
                                         Fsw,
                                         Kdeg, Fres, defType, forType, ndI,
...
int ShearCurve::checkElementState(double springForce)
{
    DummyStream dummy;
...
//                                              "AxialCurve::AxialCurve");
    ...
    if (defType == 1)           // maximum chord rotations
    {
        ...
        if (theRotations == 0) {
            opserr <<
                "ShearCurve::checkElementState, defType = 1, basicDeformations not implemented in element setResponse"
                << endln;
            return -1;
        }
...
double ShearCurve::getDegSlope(void)
{
    return Kdeg;
}


double ShearCurve::getResForce(void)
{
    return Fres;
}

double ShearCurve::getUnbalanceForce(void)
{
    //Do nothing for this class
    return 0.0;
}

int ShearCurve::sendSelf(int commitTag, Channel & theChannel)
{
    return -1;
}

int ShearCurve::recvSelf(int commitTag, Channel & theChannel,
                         FEM_ObjectBroker & theBroker)
{
    return -1;
}

void ShearCurve::Print(OPS_Stream & s, int flag)
{
...
void ShearCurve::setDegSlope(double V, double Dshear)
{
    if (Kdeg > 0.0) {
        ...
        //Fsw = Ast * fyt * dc/s;
        Daxial =
            0.04 * (1 + tan(theta) * tan(theta)) / (tan(theta) +
                                                    P / Fsw / tan(theta));
...
double ShearCurve::findLimit(double DR) //SDK
{
    ...
    if (DR < 0.01)
        ...
        V = 500 * (0.03 + delta + 4 * rho - DR -
                   0.025 * P / b / h / (fc / 1000)) * (b * d * sqrt(fc) /
                                                       1000);
...
int ShearCurve::setParameter(const char **argv, int argc,
                             Parameter & param)
{
    if (argc < 1)
        return 0;
    ...
    if (strcmp(argv[0], "fc") == 0)
        ...
        opserr << "WARNING: Could not set parameter in Shear Curve. " <<
            endln;
...
int ShearCurve::updateParameter(int parameterID, Information & info)
{
    switch (parameterID) {
    case -1:
        return -1;
