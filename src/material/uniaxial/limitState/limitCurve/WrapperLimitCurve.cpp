...
WrapperLimitCurve::WrapperLimitCurve(const char *name,
                                     limCrvObject *
                                     theLimCrv_):LimitCurve(theLimCrv_->
                                                            tag,
                                                            LIMCRV_TAG_WrapperLimitCurve),
funcName(0), theLimCrv(theLimCrv_), springForce(0.0), Kdeg(0.0), Fres(0.0),
DR(0.0)
{
    ...
    int error = 0;
    theLimCrv->limCrvFunctPtr(theLimCrv, &theModelState, &springForce,
                              &Kdeg, &Fres, &isw, &error);

    //  theLimCrv->limCrvObjectPtr = this;
}

WrapperLimitCurve::WrapperLimitCurve():LimitCurve(0, LIMCRV_TAG_WrapperLimitCurve),
funcName(0),
theLimCrv(0), springForce(0.0), Kdeg(0.0), Fres(0.0), DR(0.0)
{
...
double WrapperLimitCurve::getDegSlope(void)
{
    return Kdeg;
}

double WrapperLimitCurve::getResForce(void)
{
    return Fres;
}

double WrapperLimitCurve::getUnbalanceForce(void)
{
    return 0.0;
}

double WrapperLimitCurve::findLimit(double DR)
{
    return 0.0;
}

int WrapperLimitCurve::revertToStart(void)
{
...
LimitCurve *WrapperLimitCurve::getCopy(void)
{
    ...
    WrapperLimitCurve *theResult =
        new WrapperLimitCurve(funcName, theLimCrvObject);
    return theResult;
}

int WrapperLimitCurve::sendSelf(int commitTag, Channel & theChannel)
{
    return -1;
}

int WrapperLimitCurve::recvSelf(int commitTag, Channel & theChannel,
                                FEM_ObjectBroker & theBroker)
{
    return -1;
}

void WrapperLimitCurve::Print(OPS_Stream & s, int flag)
{
    s << "WrapperLimitCurve - wrapping function  limCrvTag: " <<
        theLimCrv->tag << endln;
}
