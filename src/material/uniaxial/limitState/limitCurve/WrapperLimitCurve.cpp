...
DR (0.0)
{
    ...
    int error = 0;
    theLimCrv->limCrvFunctPtr (theLimCrv, &theModelState, &springForce, &Kdeg,
                               &Fres, &isw, &error);
...
int
WrapperLimitCurve::recvSelf (int commitTag, Channel & theChannel,
...
{
    s << "WrapperLimitCurve - wrapping function  limCrvTag: " << theLimCrv->
        tag << endln;
}
