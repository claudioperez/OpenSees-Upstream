...
OPS_PinchingLimitState(void)
{
    ...
    if (!(argc == 32 || argc == 21)) {
        opserr <<
            "WARNING PinchingLimitStateMaterial -- insufficient arguments\n";
        ...
        opserr << "uniaxialMaterial PinchingLimitStateMaterial matTag?\n";
        opserr <<
            "nodeT? nodeB? driftAxis? Kelas? crvTyp? crvTag? eleTag?\n";
    ...
    if (OPS_GetIntInput(&numData, iTagData) != 0) {
        opserr <<
            "WARNING PinchingLimitStateMaterial -- invalid uniaxialMaterial matTag?\n"
            << endln;
        return 0;
    }
    numData = 3;
    if (OPS_GetIntInput(&numData, iNodeData) != 0) {
        opserr <<
            "WARNING PinchingLimitStateMaterial -- invalid nodeT? nodeB? driftAxis?\n"
            << endln;
        return 0;
    }
    Domain *theDomain = 0;
    ...
    if (theDomain == 0) {
        opserr <<
            "WARNING PinchingLimitStateMaterial -- Pointer to Domain was not returned\n"
            << endln;
        return 0;
    }
    Node *theNodeT = 0;
    ...
    if (iNodeData[2] < 1 || iNodeData[2] > 3) {
        opserr <<
            "WARNING PinchingLimitStateMaterial -- driftAxis is invalid\n";
    ...
    if ((dKelasData[0] < -4 || dKelasData[0] == 0) && argc == 23) {
        opserr <<
            "WARNING PinchingLimitStateMaterial -- Kelas? is invalid\n";
        opserr <<
            "Kelas = -4 -- Shear stiffness calculated assuming double curvature and shear springs top and bottom\n";
        opserr <<
            "Kelas = -3 -- Shear stiffness calculated assuming double curvature and a shear spring at the bottom\n";
        opserr <<
            "Kelas = -2 -- Shear stiffness calculated assuming single curvature and shear springs top and bottom\n";
        opserr <<
            "Kelas = -1 -- Shear stiffness calculated assuming single curvature and a shear spring at the bottom\n";
    ...
    if (OPS_GetIntInput(&numData, iCrvData) != 0) {
        opserr <<
            "WARNING PinchingLimitStateMaterial -- invalid crvTyp? crvTag?\n"
            << endln;
        return 0;
    }
    ...
    if (crvTyp == 2) {
        ...
        if (theCurve == 0 && crvTyp != 0) {
            opserr <<
                "WARNING PinchingLimitStateMaterial -- limit curve with tag "
                << crvTag << " not found for material tag " << iTagData[0]
                << endln << endln;
            return 0;
        }
    }
    if (crvTyp < 0 || crvTyp > 2) {
        opserr <<
            "WARNING PinchingLimitStateMaterial --  crvTyp? is invalid\n";
    ...
    if (crvTyp == 1) {
        opserr <<
            "WARNING PinchingLimitStateMaterial -- Axial curve has not been implemented\n"
            << endln;
        return 0;
    }
    if (argc == 32) {
        ...
        if (OPS_GetDoubleInput(&numData, dpinchPN) != 0) {
            opserr <<
                "WARNING PinchingLimitStateMaterial -- invalid YpinchUPN? YpinchRPN? XpinchRPN?\n"
                << endln;
            return 0;
        }
        numData = 3;
        if (OPS_GetDoubleInput(&numData, dpinchNP) != 0) {
            opserr <<
                "WARNING PinchingLimitStateMaterial -- invalid YpinchUNP? YpinchRNP? XpinchRNP?\n"
                << endln;
            return 0;
        }
        numData = 2;
        if (OPS_GetDoubleInput(&numData, dDmgProp) != 0) {
            opserr <<
                "WARNING PinchingLimitStateMaterial -- invalid dmgStrsLimE? dmgDispMax?\n"
                << endln;
            return 0;
        }
        ...
        if (OPS_GetDoubleInput(&numData, dDmgEdata) != 0) {
            opserr <<
                "WARNING PinchingLimitStateMaterial -- invalid dmgE1? dmgE2? dmgE3? dmgE4? dmgELim?\n"
                << endln;
            return 0;
        }
        numData = 6;
        if (OPS_GetDoubleInput(&numData, dDmgRdata) != 0) {
            opserr <<
                "WARNING PinchingLimitStateMaterial -- invalid dmgR1? dmgR2? dmgR3? dmgR4? dmgRLim? dmgRCyc?\n"
                << endln;
            return 0;
        }
        numData = 6;
        if (OPS_GetDoubleInput(&numData, dDmgSdata) != 0) {
            opserr <<
                "WARNING PinchingLimitStateMaterial -- invalid dmgS1? dmgS2? dmgS3? dmgS4? dmgSLim? dmgSCyc?\n"
                << endln;
            return 0;
        }
        ...
        if (theMaterial == 0) {
            opserr <<
                "WARNING could not create uniaxialMaterial with PinchinLimitState\n";
            return 0;
        }
        return theMaterial;
        ...
        if (OPS_GetIntInput(&numData, iEleTag) != 0) {
            opserr <<
                "WARNING PinchingLimitStateMaterial -- invalid eleTag?\n"
                << endln;
            return 0;
        }
        ...
        if (theElement == 0) {
            opserr <<
                "WARNING PinchingLimitStateMaterial -- Element with tag "
        ...
        if (OPS_GetDoubleInput(&numData, dPropData) != 0) {
            opserr <<
                "WARNING PinchingLimitStateMaterial -- invalid b? d? h? a? st? As? Acc? ld? db? rhot? f'c? fy? fyt?\n"
                << endln;
            return 0;
        }
        theMaterial = new PinchingLimitStateMaterial(iTagData[0],
...
PinchingLimitStateMaterial::PinchingLimitStateMaterial(int matTag,
                                                       ...
                                                       LimitCurve & theCrv,
                                                       Element *
                                                       theEle):UniaxialMaterial
    (matTag, MAT_TAG_PinchingLimitStateMaterial), nodeTop(nodeT),
nodeBot(nodeB), driftAxis(drftAx), E1(Kelas), curveType(crvTyp),
curveTag(crvTag), YpinchUnloadPN(YpinchUPN), YpinchReloadPN(YpinchRPN),
XpinchReloadPN(XpinchRPN), YpinchUnloadNP(YpinchUNP),
YpinchReloadNP(YpinchRNP), XpinchReloadNP(XpinchRNP),
dmgStressLimE(dmgStrsLimE), dmgDeflMax(dmgDispMax), dmgElastic1(dmgE1),
dmgElastic2(dmgE2), dmgElastic3(dmgE3), dmgElastic4(dmgE4),
dmgElasticLim(dmgELim), dmgUnload1(dmgU1), dmgUnload2(dmgU2),
dmgUnload3(dmgU3), dmgUnload4(dmgU4), dmgUnloadLim(dmgULim),
dmgReload1(dmgR1), dmgReload2(dmgR2), dmgReload3(dmgR3), dmgReload4(dmgR4),
dmgReloadLim(dmgRLim), dmgReloadCyclic(dmgRCyc), dmgStrength1(dmgS1),
dmgStrength2(dmgS2), dmgStrength3(dmgS3), dmgStrength4(dmgS4),
dmgStrengthLim(dmgSLim), dmgStrengthCyclic(dmgSCyc), eleTag(eTag), b(B),
d(D), h(H), a(A), st(S), As(AreaS), Acc(AreaCC), ld(Ldev), db(DiaBar),
rhot(rhoTrans), fc(Fc), fy(Fy), fyt(Fytrans), theDomain(theDom),
theNodeT(theNdT), theNodeB(theNdB), theCurve(0), theElement(theEle)
{
    if (curveType != 0)
        theCurve = theCrv.getCopy();
    if (theCurve == 0 && curveType != 0) {
        opserr <<
            "FATAL ERROR PinchingLimitStateMaterial -- out of memory, could not get a copy of the Limit Curve\n"
            << endln;
        exit(-1);
    }
    defineE1();
...
PinchingLimitStateMaterial::PinchingLimitStateMaterial():UniaxialMaterial(0, MAT_TAG_PinchingLimitStateMaterial),
nodeTop(0), nodeBot(0), driftAxis(0), E1(0.0), curveType(0),
curveTag(0), YpinchUnloadPN(0.0), YpinchReloadPN(0.0),
XpinchReloadPN(0.0), YpinchUnloadNP(0.0), YpinchReloadNP(0.0),
XpinchReloadNP(0.0), dmgStressLimE(0.0), dmgDeflMax(0.0),
dmgElastic1(0.0), dmgElastic2(0.0), dmgElastic3(0.0), dmgElastic4(0.0),
dmgElasticLim(0.0), dmgUnload1(0.0), dmgUnload2(0.0), dmgUnload3(0.0),
dmgUnload4(0.0), dmgUnloadLim(0.0), dmgReload1(0.0), dmgReload2(0.0),
dmgReload3(0.0), dmgReload4(0.0), dmgReloadLim(0.0), dmgReloadCyclic(0.0),
dmgStrength1(0.0), dmgStrength2(0.0), dmgStrength3(0.0),
dmgStrength4(0.0), dmgStrengthLim(0.0), dmgStrengthCyclic(0.0), eleTag(0),
b(0.0), d(0.0), h(0.0), a(0.0), st(0.0), As(0.0), Acc(0.0), ld(0.0),
db(0.0), rhot(0.0), fc(0.0), fy(0.0), fyt(0.0), theDomain(0),
theNodeT(0), theNodeB(0), theCurve(0), theElement(0)
{
    defineE1();
    revertToStart();
    revertToLastCommit();
...
int
 PinchingLimitStateMaterial::setTrialStrain(double strain,
                                            double strainRate)
{
    Tstrain = strain;
    ...
    if (CstateFlag == 0) {
        ...
        case -8:
            TpinchSlopePN =
                (TpinchStressUnloadNP - Cstress) / (TpinchStrainUnloadNP -
                                                    Cstrain);
...
double PinchingLimitStateMaterial::getStrain(void)
{
    return Tstrain;
}

double PinchingLimitStateMaterial::getStress(void)
{
    return Tstress;
}

double PinchingLimitStateMaterial::getTangent(void)
{
    return Ttangent;
}

int PinchingLimitStateMaterial::commitState(void)
{
    if ((TstateFlag == 2 || TstateFlag == -2) && countGlobalEnv < 2) {
        countGlobalEnv++;
        slopeGlobalEnv =
            (fabs(Tstress) - fabs(Cstress)) / (fabs(TstrainGlobal) -
                                               fabs(CstrainGlobal));
        interceptGlobalEnv =
            fabs(Cstress) - slopeGlobalEnv * fabs(CstrainGlobal);
        strainGlobalFresKdeg =
            (Fres - interceptGlobalEnv) / slopeGlobalEnv;
    }
    ...
    if (TstateFlag == 3 || TstateFlag == -3)
        resFlag = 1;
    if (TstateFlag == 4 || TstateFlag == -4
        || (TstateFlag == 5 && CstateFlag == -10) || (TstateFlag == -5
                                                      && CstateFlag == 10))
    {
        if (TstateFlag == 4 || TstateFlag == -4) {
            strainUn = Cstrain;
    ...
    if ((TstateFlag == 6 && CstateFlag != 6)
        || (TstateFlag == -6 && CstateFlag != -6) || (TstateFlag == 10
                                                      && CstateFlag != 10)
        || (TstateFlag == -10 && CstateFlag != -10)) {
        strainReload = Cstrain;
        stressReload = Cstress;
        Ereload = TdmgReloadE;
        if ((stressUnDmg < 0.0 && Cstress < 0.0)
            || (stressUnDmg > 0.0 && Cstress > 0.0))
            slopeFlexPred =
                (stressUnDmg - Cstress) / (strainFlexRevDmg - CstrainFlex);
        else
            slopeFlexPred =
                (-stressUnDmg - Cstress) / (-strainFlexRevDmg -
                                            CstrainFlex);
        if (resFlag == 0 && ((stressUn > 0.0 && Cstress < 0.0)
                             || (stressUn < 0.0 && Cstress > 0.0))) {
            interceptGlobalEnv =
                interceptGlobalEnv - fabs(stressUn) * dmgStrengthCyclic;
            updateDamageS();
            strainGlobalFresKdeg =
                (Fres - interceptGlobalEnv) / slopeGlobalEnv;
        }
    ...
    if (TstateFlag == 6 || TstateFlag == -6 || TstateFlag == 10
        || TstateFlag == -10) {
        double shiftFlexRe = getFlexShift();
        if (shiftFlexRe < 0.0001)
            shiftFlexRe = 0.0001;
        double slopeFlexRe =
            Tstress / fabs(Tstress) * (Tstress -
                                       stressReload) / (shiftFlexRe);
        TdmgReloadE =
            1 / (1 / slopeFlexRe + 1 / Ereload) * (1 - dmgReloadCyclic);
        if (TstateFlag == 6)
    ...
    if (((CstateFlag == 6 && TstateFlag == 2)
         || (CstateFlag == -6 && TstateFlag == -2) || (CstateFlag == 10
                                                       && TstateFlag == -2)
         || (CstateFlag == -10 && TstateFlag == 2)) && resFlag == 0) {
...
int PinchingLimitStateMaterial::revertToLastCommit(void)
{
    Tstress = Cstress;
    Tstrain = Cstrain;
    Ttangent = Ctangent;
...
int PinchingLimitStateMaterial::revertToStart(void)
{
...
{
    PinchingLimitStateMaterial *theCopy =
        new PinchingLimitStateMaterial(this->getTag(),
                                       nodeTop, nodeBot, driftAxis, E1,
                                       curveType, curveTag,
                                       YpinchUnloadPN, YpinchReloadPN,
                                       XpinchReloadPN,
                                       YpinchUnloadNP, YpinchReloadNP,
                                       XpinchReloadNP,
                                       dmgStressLimE, dmgDeflMax,
                                       dmgElastic1, dmgElastic2,
                                       dmgElastic3,
                                       dmgElastic4, dmgElasticLim,
                                       dmgUnload1, dmgUnload2, dmgUnload3,
                                       dmgUnload4, dmgUnloadLim,
                                       dmgReload1, dmgReload2, dmgReload3,
                                       dmgReload4, dmgReloadLim,
                                       dmgReloadCyclic,
                                       dmgStrength1, dmgStrength2,
                                       dmgStrength3, dmgStrength4,
                                       dmgStrengthLim, dmgStrengthCyclic,
...
int PinchingLimitStateMaterial::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
    ...
    if (res < 0)
        opserr <<
            "PinchingLimitStateMaterial::sendSelf() - failed to send data\n";

    return res;
}

int PinchingLimitStateMaterial::recvSelf(int cTag, Channel & theChannel,
                                         FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0)
        opserr <<
            "PinchingLimitStateMaterial::recvSelf() - failed to recv data\n";
    else {
        this->setTag(data(0));
        E1 = data(1);
        dmgElastic1 = data(2);
...
void PinchingLimitStateMaterial::Print(OPS_Stream & s, int flag)
{
...
void PinchingLimitStateMaterial::defineBackbone(void)
{
    Kdeg = theCurve->getDegSlope();
    Fres = theCurve->getResForce();
    if (fabs(Tstress) < Fres) {
        opserr <<
            "WARNING PinchingLimitStateMaterial::defineBackbone() - Fres must be less than shear load at failure\n";
        Fres = 0.2 * Tstress;
    ...
    InelastMonoEnergy =
        0.5 * (TstrainFresKdegDmg - Tstrain) * (Tstress - Fres) +
        (dmgDeflMax - Tstrain) * Fres;
}

void PinchingLimitStateMaterial::updateEnergy(void)
{
    if (TstateFlag == 2 || TstateFlag == 3 || TstateFlag == -2
        || TstateFlag == -3 || TstateFlag == -10 || TstateFlag == -10)
        Tenergy = Cenergy + Tdu * (Tstress + Cstress) / 2;
}

void PinchingLimitStateMaterial::updateDamageS(void)
{
    double DispRatio =
        fmax((fabs(TstrainMax) - strainShearFailure) / dmgDeflMax,
             (fabs(TstrainMin) - strainShearFailure) / dmgDeflMax);
    double EnergyRatio = Tenergy / InelastMonoEnergy;
    double Dstrength = dmgStrength1 * pow(DispRatio,
                                          dmgStrength3) +
        dmgStrength2 * pow(EnergyRatio,
                           dmgStrength4);
...
void PinchingLimitStateMaterial::updateDamageR(void)
{
    double DispRatio =
        fmax((fabs(TstrainMax) - strainShearFailure) / dmgDeflMax,
             (fabs(TstrainMin) - strainShearFailure) / dmgDeflMax);
    double EnergyRatio = Tenergy / InelastMonoEnergy;
    double Dreload = dmgReload1 * pow(DispRatio,
                                      dmgReload3) +
        dmgReload2 * pow(EnergyRatio,
                         dmgReload4);
    ...
    if (TstateFlag == 6 || TstateFlag == -6)
        TdmgReloadE =
            fmin(TdmgReloadE,
                 fmin(TdmgReloadE * (1 - Dreload),
                      fabs((stressUn - Cstress) / (strainUn - Cstrain))));
    if (TstateFlag == 9 || TstateFlag == -9)
        TdmgReloadE =
            fmin(TdmgReloadE,
                 fmin(TdmgReloadE * (1 - Dreload),
                      fabs((-stressUnDmg - Cstress) / (-strainUnDmg -
                                                       Cstrain))));
    else
        TdmgReloadE = fmin(TdmgReloadE * (1 - Dreload), TdmgReloadE);
}

int PinchingLimitStateMaterial::getStateFlag(void)
{
    if ((CstateFlag == 1 || CstateFlag == 2) && Tdu > 0.0
        && Tstrain < TstrainFresKdegDmg)
        return 2;
    else if ((CstateFlag == 2 || CstateFlag == 3) && Tdu > 0.0
             && (Tstrain >= TstrainFresKdegDmg
                 || (resFlag == 1 && Tstress >= Fres)))
        return 3;
    else if ((CstateFlag == 1 || CstateFlag == 2 || CstateFlag == 3)
             && Tdu < 0.0)
        return 4;
    else if ((CstateFlag == 4 || CstateFlag == 5 || CstateFlag == 6)
             && Tdu < 0.0 && Tstrain >= TpinchStrainUnloadPN)
        return 5;
    else if ((CstateFlag == 4 || CstateFlag == 5 || CstateFlag == 6)
             && Tdu > 0.0)
        return 6;
    else if ((CstateFlag == 5 || CstateFlag == 7) && Tdu < 0.0
             && Tstrain < TpinchStrainUnloadPN
             && Tstrain >= TpinchStrainReloadPN)
        return 7;
    else if (CstateFlag == 7 && Tdu > 0.0 && Tstrain < TpinchStrainUnloadPN
             && Tstrain >= TpinchStrainReloadPN)
        return 8;
    else if (CstateFlag == 7 && Tdu < 0.0
             && Tstrain < TpinchStrainReloadPN)
        return 9;
    else if (CstateFlag == 10 && Tdu < 0.0
             && Tstrain < TpinchStrainReloadPN)
        return 10;
    else if (CstateFlag == 10 && Tdu > 0.0
             && Cstrain < TpinchStrainReloadPN)
        return 11;

    else if ((CstateFlag == -1 || CstateFlag == -2) && Tdu < 0.0
             && Tstrain > -TstrainFresKdegDmg)
        return -2;
    else if ((CstateFlag == -2 || CstateFlag == -3) && Tdu < 0.0
             && (Tstrain <= -TstrainFresKdegDmg
        ...
        return -4;
             ...
             && Tdu > 0.0 && Tstrain <= TpinchStrainUnloadNP)
        return -5;
    else if ((CstateFlag == -4 || CstateFlag == -5 || CstateFlag == -6)
             && Tdu < 0.0)
        return -6;
    else if ((CstateFlag == -5 || CstateFlag == -7) && Tdu > 0.0
             && Tstrain > TpinchStrainUnloadNP
             && Tstrain <= TpinchStrainReloadNP)
        return -7;
    else if (CstateFlag == -7 && Tdu < 0.0
             && Tstrain > TpinchStrainUnloadNP
             && Tstrain <= TpinchStrainReloadNP)
        ...
        return -10;
             ...
             && Cstrain > TpinchStrainReloadNP)
        return -11;
    else
        return 999;
}

void PinchingLimitStateMaterial::definePinchingPN(void)
{
    TpinchStressUnloadPN = Cstress * YpinchUnloadPN;
    TpinchStrainUnloadPN =
        (TpinchStressUnloadPN - Cstress) / TdmgElasticE + Cstrain;
    ...
    TpinchSlopePN =
        (TpinchStressReloadPN -
         TpinchStressUnloadPN) / (TpinchStrainReloadPN -
                                  TpinchStrainUnloadPN);
    TpinchInterceptPN =
        TpinchStressUnloadPN - TpinchSlopePN * TpinchStrainUnloadPN;
}

void PinchingLimitStateMaterial::definePinchingNP(void)
{
    TpinchStressUnloadNP = Cstress * YpinchUnloadNP;
    TpinchStrainUnloadNP =
        (TpinchStressUnloadNP - Cstress) / TdmgElasticE + Cstrain;
    ...
    TpinchSlopeNP =
        (TpinchStressReloadNP -
         TpinchStressUnloadNP) / (TpinchStrainReloadNP -
                                  TpinchStrainUnloadNP);
    TpinchInterceptNP =
        TpinchStressUnloadNP - TpinchSlopeNP * TpinchStrainUnloadNP;
}

double PinchingLimitStateMaterial::getFlexDisp(void)
{
...
double PinchingLimitStateMaterial::getFlexShift(void)
{
...
void PinchingLimitStateMaterial::checkEnvelope(void)
{
    double shiftFlexRe = getFlexShift();
    double KdegLim =
        slopeGlobalEnv * fabs(TstrainGlobal) + interceptGlobalEnv;
    if (Tstress >= 0.0 && Tstrain >= 0.0) {
        if (Tstress >= KdegLim && TstrainGlobal < strainGlobalFresKdeg
            && resFlag == 0) {
            TstateFlag = 2;
            Ttangent = Kdeg;
            Tstress = KdegLim;
        } else if (Tstress >= Fres
                   && TstrainGlobal >= strainGlobalFresKdeg) {
            TstateFlag = 3;
            Ttangent = 0.0001;
            Tstress = Fres;
        }
...
void PinchingLimitStateMaterial::defineTargetVars(void)
{
    double Ag = b * h;
    ...
    double dmgSCyc =
        0.037133 + 0.251204 * (fy * As / (fc * Ag)) -
        0.354989 * (Acc / Ag) + 0.056569 * (a / d);
    dmgStrengthCyclic = fmax(dmgSCyc, 0);
    double YpinchU =
        -0.169113 + 0.088820 * (v * 1000 / sqrt(fc * 1000)) -
        44.375649 * (rhot) + 0.189494 * (st / d);
    YpinchUnloadPN = fmax(YpinchU, 0.0);
    YpinchUnloadNP = fmax(YpinchU, 0.0);
    double XpinchR =
        -0.589984 + 0.685461 * (P / (fc * Ag)) + 0.008966 * (ld / db) -
        0.209699 * (fy * As / (fc * Ag));
    XpinchReloadPN = XpinchR;
    XpinchReloadNP = XpinchR;
    double YpinchR =
        0.262867 + 0.761220 * (P / (fc * Ag)) -
        1.066009 * (fy * As / (fc * Ag)) + 0.005967 * (ld / db);
    YpinchReloadPN = fmax(YpinchR, 0.0);
    YpinchReloadNP = fmax(YpinchR, 0.0);
}

double PinchingLimitStateMaterial::getAxialForce(void)
{
    int trash;
    ...
    if (forceVec == 0) {
        opserr <<
            "FATAL ERROR RotationShearCurve -- unable to assign force vector\n"
            << endln;
        exit(-1);
    }
...
void PinchingLimitStateMaterial::defineE1(void)
{
    double Ag = b * h;          //in^2
