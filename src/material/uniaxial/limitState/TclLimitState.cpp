...
//start //**MRL
//Package Commands
extern LimitCurve *Tcl_addWrapperLimitCurve (limCrvObj *,
...
//////////////////////////////////////////////////////////////////////
int
Tcl_AddLimitCurveCommand (ClientData clientData, Tcl_Interp * interp,
                          int argc, TCL_Char ** argv, Domain * theDomain)
{
    ...
    if (strcmp (argv[1], "Axial") == 0)
      {
          ...
          if (defType == 2)
            {
                ...
                if (Tcl_GetInt (interp, argv[11], &dof) != TCL_OK)
                  {
                      opserr <<
                          "WARNING invalid degree of freedom for drift\n";
                      opserr << "LimitCurve Axial: " << tag << endln;
                      return TCL_ERROR;
                  }
                if (Tcl_GetInt (interp, argv[12], &perpDirn) != TCL_OK)
                  {
                      opserr <<
                          "WARNING invalid direction for column length\n";
                      opserr << "LimitCurve Axial: " << tag << endln;
                      return TCL_ERROR;
                  }
            }
          if (argc >= 14)
            {
                if (Tcl_GetDouble (interp, argv[13], &delta) != TCL_OK)
                  {
                      opserr <<
                          "WARNING invalid shift in drift surface (delta)\n";
                      opserr << "LimitCurve Axial: " << tag << endln;
                      return TCL_ERROR;
                  }
            }
          ...
          theCurve = new AxialCurve (interp, tag, eleTag, theDomain, Fsw,       //SDK
                                     Kdeg, Fres, defType, forType, ndI, ndJ,
                                     dof - 1, perpDirn - 1, delta, eleRemove);
      }
    //////////////////////////
    ...
    //////////////////////////
    ...
    else if (strcmp (argv[1], "ThreePoint") == 0)
      {
          ...
          if (Tcl_GetInt (interp, argv[2], &tag) != TCL_OK)
            {
                opserr << "WARNING invalid limitCurve ThreePoint tag" <<
                    endln;
                return TCL_ERROR;
            }
          if (Tcl_GetInt (interp, argv[3], &eleTag) != TCL_OK)
    ...
    /////////////////////////////
    else if (strcmp (argv[1], "Shear") == 0)
      {
          ...
          // Parsing was successful, allocate the material
          // Subtract one from dof and perpDirn for C indexing
          theCurve = new ShearCurve (tag, eleTag, theDomain, rho, fc, b, h, d, Fsw, Kdeg, Fres, defType, forType,       //SDK
    ...
    //***********************************************************************************************************************start package commands //**MRL
    else
      {
          ...
          if (theCurve == 0)
            {
                ...
                LimitCurvePackageCommand *limCrvCommands =
                    theLimitCurvePackageCommands;
                bool found = false;
                while (limCrvCommands != NULL && found == false)
                  {
                      if (strcmp (argv[1], limCrvCommands->funcName) == 0)
                        {
                            OPS_ResetInputNoBuilder (clientData, interp, 2,
                                                     argc, argv, theDomain);
                            theCurve =
                                (LimitCurve
                                 *) (*(limCrvCommands->funcPtr)) (argc, argv);
                            found = true;;
                        }
                      else
          ...
          if (theCurve == 0)
            {
                ...
                limCrvObj *limCrvObject =
                    OPS_GetLimitCurveType (limCrvType, strlen (limCrvType));
                ...
                if (limCrvObject != 0)
                  {
                      theCurve =
                          Tcl_addWrapperLimitCurve (limCrvObject, clientData,
                                                    interp, argc, argv);
                      if (theCurve == 0)
                          delete limCrvObject;
                  }
                ...
                if (res == 0)
                  {
                      ...
                      LimitCurvePackageCommand *theLimCrvCommand =
                          new LimitCurvePackageCommand;
          ...
          if (theCurve == 0)
            {
                opserr << "WARNING could not create LimitCurve " << argv[1] <<
                    endln;
                return TCL_ERROR;
            }
...
UniaxialMaterial *
Tcl_AddLimitStateMaterial (ClientData clientData, Tcl_Interp * interp,
                           int argc, TCL_Char ** argv)
{
    ...
    if (Tcl_GetInt (interp, argv[i++], &tag) != TCL_OK)
      {
          opserr << "WARNING invalid uniaxialMaterial LimitState tag" <<
              endln;
          return theMaterial;
      }
    ...
    if (argc == 22 || argc == 23)
      {
          ...
          theMaterial = new LimitStateMaterial (tag,
                                                ...
                                                mom3p, rot3p, mom1n, rot1n,
                                                mom2n, rot2n, mom3n, rot3n,
                                                pinchX, pinchY, damfc1,
                                                damfc2, beta, *theCurve,
                                                curveType, degrade);
      }
