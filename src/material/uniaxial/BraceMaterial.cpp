...
OPS_Cast(void)
{
    ...
    if (numRemaining != 17 || numRemaining != 13) {
        if (OPS_GetDoubleInput(&numRemaining, dData) != 0) {
            opserr <<
                "Invalid Args want: uniaxialMaterial CastFuse tag? NLegs? bo? h? Fy? E? L? b? R0? cR1? cR2? a1? a2? a3? a4?";
            return 0;
        }
...
BraceMaterial::BraceMaterial(int tag,
                             ...
                             double r2n, double m3n, double r3n, double px,
                             double py, double d1, double d2,
                             double b):UniaxialMaterial(tag,
                                                        MAT_TAG_BraceMaterial),
...
rot1n(r1n), mom2n(m2n), rot2n(r2n), mom3n(m3n), rot3n(r3n)
{
    ...
    if (error) {
        opserr <<
            "BraceMaterial::BraceMaterial -- input backbone is not unique (one-to-one)\n";
        exit(-1);
    }
    energyA =
        0.5 * (rot1p * mom1p + (rot2p - rot1p) * (mom2p + mom1p) +
               (rot3p - rot2p) * (mom3p + mom2p) + rot1n * mom1n + (rot3n -
                                                                    rot1n)
               * (mom3n + mom1n));
...
BraceMaterial::BraceMaterial(int tag,
                             ...
                             double m2n, double r2n, double px, double py,
                             double d1, double d2,
                             double b):UniaxialMaterial(tag,
                                                        MAT_TAG_BraceMaterial),
pinchX(px), pinchY(py), damfc1(d1), damfc2(d2), beta(b), mom1p(m1p),
...
double BraceMaterial::getStrain(void)
{
    return Tstrain;
}

double BraceMaterial::getStress(void)
{
    return Tstress;
}

double BraceMaterial::getTangent(void)
{
    return Ttangent;
}

void BraceMaterial::positiveIncrement(double dStrain)
{
    ...
    double rotmp2 =
        TrotMax - (1.0 - pinchY) * (maxmom - TmomNu) / (E1p * kp);
    ...
    if (Tstrain < rotch) {
        ...
        tmpmo1 = Cstress + E1p * kp * dStrain;
        tmpmo2 =
            TmomNu + pinchY * (maxmom - TmomNu) + (Tstrain -
                                                   rotch) * Ttangent;
        if (tmpmo1 < tmpmo2) {
            Tstress = tmpmo1;
            Ttangent = E1p * kp;
...
void BraceMaterial::negativeIncrement(double dStrain)
{
    // double kn = pow(CrotMin/rot1n,beta);
    double kn = 1.0;
    // kn = (kn < 1.0) ? 1.0 : 1.0/kn;
...
int BraceMaterial::commitState(void)
{
    CrotMax = TrotMax;
    CrotMin = TrotMin;
    CrotPu = TrotPu;
...
int BraceMaterial::revertToLastCommit(void)
{
    TrotMax = CrotMax;
    TrotMin = CrotMin;
    TrotPu = CrotPu;
...
int BraceMaterial::revertToStart(void)
{
    CrotMax = 0.0;
    CrotMin = 0.0;
    CrotPu = 0.0;
...
UniaxialMaterial *BraceMaterial::getCopy(void)
{
...
int BraceMaterial::sendSelf(int commitTag, Channel & theChannel)
{
    int res = 0;
...
int BraceMaterial::recvSelf(int commitTag, Channel & theChannel,
...
void BraceMaterial::Print(OPS_Stream & s, int flag)
{
...
void BraceMaterial::setEnvelope(void)
{
    E1p = mom1p / rot1p;
...
double BraceMaterial::posEnvlpStress(double strain)
{
    if (strain <= 0.0)
        return 0.0;
...
double BraceMaterial::negEnvlpStress(double strain)
{
    if (strain >= 0.0)
        return 0.0;
...
double BraceMaterial::posEnvlpTangent(double strain)
{
    if (strain < 0.0)
        return E1p * 1.0e-9;
    else if (strain <= rot1p)
...
double BraceMaterial::negEnvlpTangent(double strain)
{
    if (strain > 0.0)
        return E1n * 1.0e-9;
    else if (strain >= rot1n)
...
double BraceMaterial::posEnvlpRotlim(double strain)
{
...
double BraceMaterial::negEnvlpRotlim(double strain)
{
