...
void *OPS_BWBN(void)
{
    ...
    theMaterial =
        new BWBN(iData1[0], dData[0], dData[1], dData[2], dData[3],
                 dData[4], dData[5], dData[6], dData[7], dData[8],
                 dData[9], dData[10], dData[11], dData[12], iData2[0]);

    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type BWBN\n";
        return 0;
    }
...
BWBN::BWBN(int tag,
           ...
           double p_Shi,
           double p_deltaShi,
           double p_lamda, double ptolerance,
           int pMaxNumIter):UniaxialMaterial(tag, MAT_TAG_BWBN),
alpha(p_alpha), ko(p_ko), n(p_n), gamma(p_gamma), beta(p_beta), Ao(p_Ao),
q(p_q), zetas(p_zetas), p(p_p), Shi(p_Shi), deltaShi(p_deltaShi),
lamda(p_lamda), tolerance(ptolerance), maxNumIter(pMaxNumIter)
{
...
int BWBN::setTrialStrain(double strain, double strainRate)
{
    ...
    while ((fabs(Tzold - Tznew) > tolerance) && count < maxNumIter) {
        ...
        h = 1.0 -
            Tzeta1 * exp(-pow((Tz * signum(dStrain) - q * zu), 2) /
                         (Tzeta2 * Tzeta2));
        Psi = gamma + beta * sign;
        ...
        Tzeta2_ =
            Shi * Tzeta1_ + lamda * deltaShi * Te_ +
            deltaShi * Te * Tzeta1_ + deltaShi * Te_ * Tzeta1;
        ...
        h_ = -exp(-pow(Tz * signum(dStrain) - q * zu, 2) /
                  (Tzeta2 * Tzeta2)) * (Tzeta1_ -
                                        Tzeta1 * 2 * (Tz *
                                                      signum(dStrain) -
                                                      q * zu) *
                                        signum(dStrain) / (Tzeta2 *
                                                           Tzeta2) +
                                        Tzeta2_ * Tzeta1 * 2 *
                                        pow((Tz * signum(dStrain) -
                                             q * zu),
                                            2) / (Tzeta2 * Tzeta2 *
                                                  Tzeta2));
        sign = signum(Tz);
        ...
        if (fabs(f_) < 1.0e-10) {
            opserr <<
                "WARNING: BWBN::setTrialStrain() -- zero derivative " <<
        ...
        if (count == maxNumIter) {
            ...
            opserr << "WARNING: BWBN::setTrialStrain() -- did not" <<
        ...
        if (Tz != 0.0) {
            ...
            b3 = Shi * b2 + lamda * deltaShi * b1 + deltaShi * Te * b2 +
                deltaShi * b1 * Tzeta1;
            b4 = -exp(-pow((Tz * signum(dStrain) - q * zu), 2) /
                      (Tzeta2 * Tzeta2)) * (b2 +
                                            b3 * Tzeta1 * 2 *
                                            pow((Tz * signum(dStrain) -
                                                 q * zu),
            ...
            b7 = Shi * b6 + lamda * deltaShi * b5 + deltaShi * Te * b6 +
                deltaShi * b5 * Tzeta1;
            ...
            b8 = -exp(-pow((Tz * signum(dStrain) - q * zu), 2) /
                      (Tzeta2 * Tzeta2)) * (b6 -
                                            Tzeta1 * 2 * (Tz *
                                                          signum
                                                          (dStrain) -
...
double BWBN::getStress(void)
{
    return Tstress;
}

double BWBN::getInitialTangent(void)
{
    return (alpha * ko + (1 - alpha) * ko * Ao);
}


double BWBN::getTangent(void)
{
    return Ttangent;
}

double BWBN::getStrain(void)
{
    return Tstrain;
}

int BWBN::commitState(void)
{
...
int BWBN::revertToLastCommit(void)
{
    // Nothing to do here
    return 0;
}

int BWBN::revertToStart(void)
{
    Tstrain = 0.0;
    Cstrain = 0.0;
    Tz = 0.0;
...
UniaxialMaterial *BWBN::getCopy(void)
{
    BWBN *theCopy = new BWBN(this->getTag(), alpha, ko, n, gamma,
                             beta, Ao, q, zetas, p, Shi, deltaShi, lamda,
...
int BWBN::sendSelf(int cTag, Channel & theChannel)
{
    return 0;
}

int BWBN::recvSelf(int cTag, Channel & theChannel,
                   FEM_ObjectBroker & theBroker)
{
    return 0;
}

void BWBN::Print(OPS_Stream & s, int flag)
{
