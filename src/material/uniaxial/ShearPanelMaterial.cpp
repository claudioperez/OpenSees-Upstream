...
void *OPS_ShearPanelMaterial()
{
    ...
    if (argc == 42) {
        return new ShearPanelMaterial(tag,
                                      ...
                                      datap[6], datap[7], datan[0],
                                      datan[1], datan[2], datan[3],
                                      datan[4], datan[5], datan[6],
                                      datan[7], dataP[0], dataP[1],
                                      dataP[2], dataN[0], dataN[1],
                                      dataN[2], data[0], data[0], data[0],
    ...
    if (argc == 31) {
        return new ShearPanelMaterial(tag,
                                      datap[0], datap[1], datap[2],
                                      datap[3], datap[4], datap[5],
                                      datap[6], datap[7], dataP[0],
                                      dataP[1], dataP[2], data[0], data[0],
                                      data[0], data[0], data[0], data[0],
                                      data[0], data[0], data[0], data[0],
                                      data[0], data[0], data[0], data[0],
...
ShearPanelMaterial::ShearPanelMaterial(int tag,
                                       double f1p, double d1p, double f2p,
                                       double d2p, double f3p, double d3p,
                                       double f4p, double d4p, double f1n,
                                       double d1n, double f2n, double d2n,
                                       double f3n, double d3n, double f4n,
                                       double d4n, double mdp, double mfp,
                                       double msp, double mdn, double mfn,
                                       double msn, double gk1, double gk2,
                                       double gk3, double gk4,
                                       double gklim, double gd1,
                                       double gd2, double gd3, double gd4,
                                       double gdlim, double gf1,
                                       double gf2, double gf3, double gf4,
                                       double gflim, double ge,
                                       double ystr):UniaxialMaterial(tag,
                                                                     MAT_TAG_ShearPanelMaterial),
stress1p(f1p), strain1p(d1p), stress2p(f2p), strain2p(d2p), stress3p(f3p),
strain3p(d3p), stress4p(f4p), strain4p(d4p), stress1n(f1n), strain1n(d1n),
stress2n(f2n), strain2n(d2n), stress3n(f3n), strain3n(d3n), stress4n(f4n),
strain4n(d4n), yldStress(ystr), yldStrain(0.0), envlpPosStress(6),
envlpPosStrain(6), envlpNegStress(6), envlpNegStrain(6), tagMat(tag),
...
gammaE(ge), rDispP(mdp), rForceP(mfp), uForceP(msp), rDispN(mdn),
rForceN(mfn), uForceN(msn), state3Stress(4), state3Strain(4),
state4Stress(4), state4Strain(4), envlpPosDamgdStress(6),
envlpNegDamgdStress(6)
{
    bool error = false;
    ...
    if (error) {
        opserr <<
            "ERROR: -- input backbone is not unique (one-to-one) , ShearPanelMaterial::ShearPanelMaterial"
            << "\a";
    }
...
ShearPanelMaterial::ShearPanelMaterial(int tag,
                                       double f1p, double d1p, double f2p,
                                       double d2p, double f3p, double d3p,
                                       double f4p, double d4p, double mdp,
                                       double mfp, double msp, double gk1,
                                       double gk2, double gk3, double gk4,
                                       double gklim, double gd1,
                                       double gd2, double gd3, double gd4,
                                       double gdlim, double gf1,
                                       double gf2, double gf3, double gf4,
                                       ...
                                       double ystr):UniaxialMaterial(tag,
                                                                     MAT_TAG_ShearPanelMaterial),
stress1p(f1p), strain1p(d1p), stress2p(f2p), strain2p(d2p), stress3p(f3p),
strain3p(d3p), stress4p(f4p), strain4p(d4p), yldStress(ystr),
yldStrain(0.0), envlpPosStress(6), envlpPosStrain(6), envlpNegStress(6),
envlpNegStrain(6), tagMat(tag), gammaK1(gk1), gammaK2(gk2), gammaK3(gk3),
gammaK4(gk4), gammaKLimit(gklim), gammaD1(gd1), gammaD2(gd2), gammaD3(gd3),
gammaD4(gd4), gammaDLimit(gdlim), gammaF1(gf1), gammaF2(gf2), gammaF3(gf3),
gammaF4(gf4), gammaFLimit(gflim), gammaE(ge), rDispP(mdp), rForceP(mfp),
uForceP(msp), state3Stress(4), state3Strain(4), state4Stress(4),
state4Strain(4), envlpPosDamgdStress(6), envlpNegDamgdStress(6)
{
    bool error = false;
...
int
 ShearPanelMaterial::setTrialStrain(double strain, double CstrainRate)
{
    ...
    case 3:
        kunload =
            (hghTstateStrain < 0.0) ? kElasticNegDamgd : kElasticPosDamgd;
    ...
    case 4:
        kunload =
            (lowTstateStrain < 0.0) ? kElasticNegDamgd : kElasticPosDamgd;
    ...
    elasticStrainEnergy =
        (Tstrain >
         0.0) ? 0.5 * Tstress / kElasticPosDamgd * Tstress : 0.5 *
        Tstress / kElasticNegDamgd * Tstress;
...
void ShearPanelMaterial::SetEnvelope(void)
{
    ...
    envlpPosStress(5) =
        (k1 >
         0.0) ? stress4p + k1 * (envlpPosStrain(5) -
                                 strain4p) : stress4p * 1.1;
    envlpNegStrain(5) = 1e+6 * strain4n;
    envlpNegStress(5) =
        (k2 >
         0.0) ? stress4n + k2 * (envlpNegStrain(5) -
                                 strain4n) : stress4n * 1.1;
    ...
    for (int jt = 0; jt < 4; jt++) {
        energypos +=
            0.5 * (envlpPosStress(jt) +
                   envlpPosStress(jt + 1)) * (envlpPosStrain(jt + 1) -
                                              envlpPosStrain(jt));
    }
    ...
    for (int jy = 0; jy < 4; jy++) {
        energyneg +=
            0.5 * (envlpNegStress(jy) +
                   envlpNegStress(jy + 1)) * (envlpNegStrain(jy + 1) -
                                              envlpNegStrain(jy));
    }
    ...
    if (yldStress < envlpPosStress(2) && yldStress > envlpPosStress(1)) {
        double slope =
            (envlpPosStress(2) -
             envlpPosStress(1)) / (envlpPosStrain(2) - envlpPosStrain(1));
        yldStrain =
            envlpPosStrain(1) + (yldStress - envlpPosStress(1)) / slope;
    } else if (yldStress <= envlpPosStress(3)
               && yldStress >= envlpPosStress(2)) {
        double slope =
            (envlpPosStress(3) -
             envlpPosStress(2)) / (envlpPosStrain(3) - envlpPosStrain(2));
        yldStrain =
            envlpPosStrain(2) + (yldStress - envlpPosStress(2)) / slope;
    } else if (yldStress > envlpPosStress(3)) {
        double slope = 0.0;
        yldStrain = 0.0;
    }
    ...
    if (u < lowTstateStrain || u > hghTstateStrain || cid) {
        if (Tstate == 0) {
            ...
            if (u < uMinDamgd) {
                ...
                gammaFUsed = CgammaF;
                for (int i = 0; i <= 5; i++) {
                    envlpNegDamgdStress(i) =
                        envlpNegStress(i) * (1.0 - gammaFUsed);
                }
                lowTstateStrain = envlpNegStrain(5);
                lowTstateStress = envlpNegStress(5);
                hghTstateStrain = envlpNegStrain(0);
            ...
            if (u > uMaxDamgd) {
                ...
                gammaFUsed = CgammaF;
                for (int i = 0; i <= 5; i++) {
                    envlpPosDamgdStress(i) =
                        envlpPosStress(i) * (1.0 - gammaFUsed);
                }
                lowTstateStrain = envlpPosStrain(0);
                lowTstateStress = envlpPosStress(0);
                hghTstateStrain = envlpPosStrain(5);
...
double ShearPanelMaterial::posEnvlpStress(double u)
{
    ...
    while (k == 0.0 && i <= 4) {
        ...
        if (u <= envlpPosStrain(i + 1)) {
            k = (envlpPosDamgdStress(i + 1) -
                 envlpPosDamgdStress(i)) / (envlpPosStrain(i + 1) -
                                            envlpPosStrain(i));
    ...
    if (k == 0.0) {
        k = (envlpPosDamgdStress(5) -
             envlpPosDamgdStress(4)) / (envlpPosStrain(5) -
                                        envlpPosStrain(4));
...
double ShearPanelMaterial::negEnvlpStress(double u)
{
    ...
    while (k == 0.0 && i <= 4) {
        if (u >= envlpNegStrain(i + 1)) {
            k = (envlpNegDamgdStress(i) -
                 envlpNegDamgdStress(i + 1)) / (envlpNegStrain(i) -
                                                envlpNegStrain(i + 1));
            f = envlpNegDamgdStress(i + 1) + (u -
                                              envlpNegStrain(i + 1)) * k;
        }
        i++;
    }

    if (k == 0.0) {
        k = (envlpNegDamgdStress(4) -
             envlpNegDamgdStress(5)) / (envlpNegStrain(4) -
                                        envlpNegStrain(5));
...
void ShearPanelMaterial::getState3(Vector & state3Strain,
                                   Vector & state3Stress, double kunload)
{

    double kmax =
        (kunload > kElasticNegDamgd) ? kunload : kElasticNegDamgd;
    ...
    if (state3Strain(0) * state3Strain(3) < 0.0) {
        ...
        if (rForceN - uForceN > 1e-8) {
            ...
            if (TminStrainDmnd < envlpNegStrain(3)) {
                ...
                double st1 =
                    envlpNegDamgdStress(3) * uForceN * (1.0 + 1e-6);
                double st2 = envlpNegDamgdStress(4) * (1.0 + 1e-6);
                state3Stress(1) = (st1 < st2) ? st1 : st2;
            }
        }
        // if reload stiffness exceeds unload stiffness, reduce reload stiffness to make it equal to unload stiffness
        if ((state3Stress(1) - state3Stress(0)) / (state3Strain(1) -
                                                   state3Strain(0)) >
            kElasticNegDamgd) {
            state3Strain(1) =
                lowTstateStrain + (state3Stress(1) -
                                   state3Stress(0)) / kElasticNegDamgd;
        }
        ...
        if (state3Strain(1) > state3Strain(3)) {
            ...
            state3Strain(2) =
                hghTstateStrain - (hghTstateStress -
            ...
            if (state3Strain(2) > state3Strain(3)) {
            ...
            } else if ((state3Stress(2) -
                        state3Stress(1)) / (state3Strain(2) -
                                            state3Strain(1)) > kmax) {
                ...
                state3Stress(1) = state3Stress(0) + 0.33 * df;
                state3Stress(2) = state3Stress(0) + 0.67 * df;
            } else if ((state3Strain(2) < state3Strain(1))
                       || ((state3Stress(2) - state3Stress(1)) /
                           (state3Strain(2) - state3Strain(1)) < 0)) {
                if (state3Strain(2) < 0.0) {
                    ...
                    double avgforce =
                        0.5 * (state3Stress(2) + state3Stress(1));
                    double dfr = 0.0;
                    if (avgforce < 0.0) {
                        dfr = -avgforce / 100;
                    } else {
                    ...
                    double slope12 =
                        (state3Stress(1) -
                         state3Stress(0)) / (state3Strain(1) -
                                             state3Strain(0));
                    double slope34 =
                        (state3Stress(3) -
                         state3Stress(2)) / (state3Strain(3) -
                                             state3Strain(2));
                    state3Stress(1) = avgforce - dfr;
                    state3Stress(2) = avgforce + dfr;
                    state3Strain(1) =
                        state3Strain(0) + (state3Stress(1) -
                                           state3Stress(0)) / slope12;
                    state3Strain(2) =
                        state3Strain(3) - (state3Stress(3) -
...
void ShearPanelMaterial::getState4(Vector & state4Strain,
                                   Vector & state4Stress, double kunload)
{

    double kmax =
        (kunload > kElasticPosDamgd) ? kunload : kElasticPosDamgd;
    ...
    if (state4Strain(0) * state4Strain(3) < 0.0) {
        ...
        if (uForceP == 0.0) {
            ...
            if (TmaxStrainDmnd > envlpPosStrain(3)) {
                ...
                double st1 =
                    envlpPosDamgdStress(3) * uForceP * (1.0 + 1e-6);
                double st2 = envlpPosDamgdStress(4) * (1.0 + 1e-6);
                state4Stress(2) = (st1 > st2) ? st1 : st2;
            }
        }
        // if reload stiffness exceeds unload stiffness, reduce reload stiffness to make it equal to unload stiffness
        if ((state4Stress(3) - state4Stress(2)) / (state4Strain(3) -
                                                   state4Strain(2)) >
            kElasticPosDamgd) {
            state4Strain(2) =
                hghTstateStrain - (state4Stress(3) -
                                   state4Stress(2)) / kElasticPosDamgd;
        }
        ...
        if (state4Strain(2) < state4Strain(0)) {
            ...
            state4Strain(1) =
                lowTstateStrain + (-lowTstateStress +
                                   state4Stress(1)) / kunload;
            ...
            if (state4Strain(1) < state4Strain(0)) {
            ...
            } else if ((state4Stress(2) -
                        state4Stress(1)) / (state4Strain(2) -
                                            state4Strain(1)) > kmax) {
                // linear unload-reload path expected
                double du = state4Strain(3) - state4Strain(0);
                double df = state4Stress(3) - state4Stress(0);
                ...
                state4Stress(1) = state4Stress(0) + 0.33 * df;
                state4Stress(2) = state4Stress(0) + 0.67 * df;
            } else if ((state4Strain(2) < state4Strain(1))
                       || ((state4Stress(2) - state4Stress(1)) /
                           (state4Strain(2) - state4Strain(1)) < 0)) {
                if (state4Strain(1) > 0.0) {
                    ...
                    double avgforce =
                        0.5 * (state4Stress(2) + state4Stress(1));
                    double dfr = 0.0;
                    if (avgforce < 0.0) {
                        dfr = -avgforce / 100;
                    } else {
                    ...
                    double slope12 =
                        (state4Stress(1) -
                         state4Stress(0)) / (state4Strain(1) -
                                             state4Strain(0));
                    double slope34 =
                        (state4Stress(3) -
                         state4Stress(2)) / (state4Strain(3) -
                                             state4Strain(2));
                    state4Stress(1) = avgforce - dfr;
                    state4Stress(2) = avgforce + dfr;
                    state4Strain(1) =
                        state4Strain(0) + (state4Stress(1) -
                                           state4Stress(0)) / slope12;
                    state4Strain(2) =
                        state4Strain(3) - (state4Stress(3) -
...
double
    ShearPanelMaterial::Envlp3Tangent(Vector s3Strain, Vector s3Stress,
                                      double u)
{
    double k = 0.0;
    int i = 0;
    while ((k == 0.0 || i <= 2) && (i <= 2)) {
        if (u >= s3Strain(i)) {
            k = (s3Stress(i + 1) - s3Stress(i)) / (s3Strain(i + 1) -
                                                   s3Strain(i));
        }
        i++;
    }
...
double
 ShearPanelMaterial::Envlp4Tangent(Vector s4Strain, Vector s4Stress,
                                   double u)
{
    double k = 0.0;
    int i = 0;
    while ((k == 0.0 || i <= 2) && (i <= 2)) {
        if (u >= s4Strain(i)) {
            k = (s4Stress(i + 1) - s4Stress(i)) / (s4Strain(i + 1) -
                                                   s4Strain(i));
        }
        i++;
    }
...
double
    ShearPanelMaterial::Envlp3Stress(Vector s3Strain, Vector s3Stress,
                                     double u)
{
    double k = 0.0;
    int i = 0;
...
double
 ShearPanelMaterial::Envlp4Stress(Vector s4Strain, Vector s4Stress,
                                  double u)
{
    double k = 0.0;
    int i = 0;
...
void ShearPanelMaterial::updateDmg(double strain, double dstrain)
{
    ...
    double umaxAbs =
        (TmaxStrainDmnd >
         -TminStrainDmnd) ? TmaxStrainDmnd : -TminStrainDmnd;
    double uultAbs =
        (envlpPosStrain(4) >
         -envlpNegStrain(4)) ? envlpPosStrain(4) : -envlpNegStrain(4);


    if ((strain < uultAbs && strain > -uultAbs)
        && Tenergy < energyCapacity) {
        ...
        if ((umaxAbs >= yldStrain) && (yldStrain != 0.0)) {
            ...
            double b =
                -gammaFLimit * yldStrain * uultAbs / (uultAbs - yldStrain);
        ...
        double kmin =
            ((kminP / kElasticPos) >
             (kminN / kElasticNeg)) ? (kminP / kElasticPos) : (kminN /
                                                               kElasticNeg);
        ...
        TgammaK =
            (gammaKLimit < gammaKLimEnv) ? gammaKLimit : gammaKLimEnv;
        TgammaD = gammaDLimit;
        TgammaF = gammaFLimit;
    }
