...
OPS_ModIMKPeakOriented()
{
    ...
    if (OPS_GetIntInput(&numData, iData) != 0) {
        opserr <<
            "WARNING invalid uniaxialMaterial  ModIMKPeakOriented tag" <<
            endln;
        return 0;
    }
    ...
    if (numData != 23 && numData != 24) {
        opserr <<
            "Invalid Args want: uniaxialMaterial ModIMKPeakOriented tag? Ke?, alfaPos?, alfaNeg?, My_pos?, My_neg?";
        opserr <<
    ...
    if (numData == 23) {
        ...
        theMaterial = new ModIMKPeakOriented(iData[0], dData[0], dData[1], dData[2], dData[3], dData[4], dData[5], dData[6], dData[7], dData[8], dData[9], dData[10], dData[11], dData[12], dData[13], dData[14], dData[15], dData[16], dData[17], dData[18], dData[19], dData[20], dData[21], dData[22]);      // Updated: Filipe Ribeiro and Andre Barbosa
        ...
        theMaterial = new ModIMKPeakOriented(iData[0], dData[0], dData[1], dData[2], dData[3], dData[4], dData[5], dData[6], dData[7], dData[8], dData[9], dData[10], dData[11], dData[12], dData[13], dData[14], dData[15], dData[16], dData[17], dData[18], dData[19], dData[20], dData[21], dData[22], dData[23]);   // Updated: Filipe Ribeiro and Andre Barbosa
    ...
    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type ModIMKPeakOriented Material\n";
        return 0;
    }
...
ModIMKPeakOriented::ModIMKPeakOriented(int tag, double Ke0, double alfanPos, double alfanNeg, double my_pos, double my_neg,     // Updated: Filipe Ribeiro and Andre Barbosa
                                       double ls, double ld, double la, double lk, double cs, double cd, double ca, double ck, double thetaPpos, double thetaPneg, double thetaPCpos, double thetaPCneg, double resfacPos, double resfacNeg, double fracDispPos, double fracDispNeg, double dPos, double dNeg):UniaxialMaterial(tag, MAT_TAG_ModIMKPeakOriented), Ke0(Ke0), AlfanPos(alfanPos), AlfanNeg(alfanNeg), My_pos(my_pos), My_neg(my_neg), Ls(ls), Ld(ld), La(la), Lk(lk), Cs(cs), Cd(cd), Ca(ca), Ck(ck), ThetaPpos(thetaPpos), ThetaPneg(thetaPneg), ThetaPCpos(thetaPCpos), ThetaPCneg(thetaPCneg), ResfacPos(resfacPos), ResfacNeg(resfacNeg), FracDispPos(fracDispPos), FracDispNeg(fracDispNeg), DPos(dPos), DNeg(dNeg)  // Updated: Filipe Ribeiro and Andre Barbosa
{
    // Initialize Variables by Calling revertToStart function
    this->revertToStart();
...
ModIMKPeakOriented::ModIMKPeakOriented():UniaxialMaterial(0, MAT_TAG_ModIMKPeakOriented), Ke0(0.0), AlfanPos(0.0),
AlfanNeg(0.0), My_pos(0.0), My_neg(0.0), Ls(0.0), Ld(0.0), La(0.0),
Lk(0.0),
    // Updated: Filipe Ribeiro and Andre Barbosa
    Cs(0.0), Cd(0.0), Ca(0.0), Ck(0.0), ThetaPpos(0.0), ThetaPneg(0.0), ThetaPCpos(0.0), ThetaPCneg(0.0), ResfacPos(0.0), ResfacNeg(0.0), FracDispPos(0.0), FracDispNeg(0.0), DPos(0.0), DNeg(0.0), nFactor(0.0)        // Updated: Filipe Ribeiro and Andre Barbosa
{
    // Initialize Variables by Calling revertToStart function
    this->revertToStart();
    Tangent = Ke;
...
UniaxialMaterial *ModIMKPeakOriented::getCopy(void)
{
    ModIMKPeakOriented *theCopy =
        new ModIMKPeakOriented(this->getTag(), Ke0, AlfanPos, AlfanNeg,
...
int ModIMKPeakOriented::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
...
int ModIMKPeakOriented::recvSelf(int cTag, Channel & theChannel,
                                 FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0) {
        opserr <<
            "ModIMKPeakOriented::recvSelf() - failed to receive data\n";
        this->setTag(0);
    } else {
...
void ModIMKPeakOriented::Print(OPS_Stream & s, int flag)
{
...
void ModIMKPeakOriented::envelPosCap2(double fy, double alphaPos,
...
void ModIMKPeakOriented::envelNegCap2(double fy, double alphaNeg,
                                      double alphaCap, double cpDsp,
                                      double &d, double &f, double &ek,
...
Response *ModIMKPeakOriented::setResponse(const char **argv, int argc,
...
int ModIMKPeakOriented::getResponse(int responseID,
