...
Elastic2Material::Elastic2Material(int tag, double e,
                                   double et):UniaxialMaterial(tag,
                                                               MAT_TAG_Elastic2Material),
trialStrain(0.0), trialStrainRate(0.0), E(e), eta(et),
initialStrain(99999.99)
{

}

Elastic2Material::Elastic2Material():UniaxialMaterial(0, MAT_TAG_Elastic2Material),
...
int Elastic2Material::setTrial(double strain, double &stress,
...
double Elastic2Material::getStress(void)
{
    if (zeroE == 1)
        return eta * trialStrainRate;
    return E * trialStrain + eta * trialStrainRate;
}


int Elastic2Material::commitState(void)
{
    return 0;
}


int Elastic2Material::revertToLastCommit(void)
{
    return 0;
}


int Elastic2Material::revertToStart(void)
{
    initialStrain = 99999.99;
    trialStrain = 0.0;
    trialStrainRate = 0.0;
...
UniaxialMaterial *Elastic2Material::getCopy(void)
{
    Elastic2Material *theCopy =
        new Elastic2Material(this->getTag(), E, eta);
...
int Elastic2Material::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
...
int Elastic2Material::recvSelf(int cTag, Channel & theChannel,
                               FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0) {
        opserr <<
            "Elastic2Material::recvSelf() - failed to receive data\n";
        E = 0;
        this->setTag(0);
    } else {
        this->setTag(data(0));
...
void Elastic2Material::Print(OPS_Stream & s, int flag)
{
    s << "Elastic tag: " << this->getTag() << endln;
    s << "  E: " << E << " eta: " << eta << endln;
}

int Elastic2Material::setParameter(const char **argv, int argc,
                                   Parameter & param)
{
...
int Elastic2Material::updateParameter(int parameterID, Information & info)
{
    switch (parameterID) {
    case -1:
        return -1;
