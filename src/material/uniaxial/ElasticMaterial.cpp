...
void *OPS_ElasticMaterial(void)
{
    ...
    if (OPS_GetNumRemainingInputArgs() < 2) {
        opserr <<
            "Invalid #args,  want: uniaxialMaterial Elastic tag? E? <eta?> <Eneg?> ... "
            << endln;
        return 0;
    }
    ...
    if (numData >= 3) {
        ...
        numData = 1;
        if (OPS_GetDoubleInput(&numData, dData) != 0) {
            opserr << "Invalid data for uniaxialMaterial Elastic " <<
                iData[0] << endln;
            return 0;
        }
        dData[1] = 0.0;
    ...
    theMaterial =
        new ElasticMaterial(iData[0], dData[0], dData[1], dData[2]);
    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type ElasticMaterial\n";
        return 0;
    }
...
ElasticMaterial::ElasticMaterial(int tag, double e,
                                 double et):UniaxialMaterial(tag,
                                                             MAT_TAG_ElasticMaterial),
trialStrain(0.0), trialStrainRate(0.0), committedStrain(0.0),
committedStrainRate(0.0), Epos(e), Eneg(e), eta(et), parameterID(0)
{

}


ElasticMaterial::ElasticMaterial(int tag, double ep, double et,
                                 double en):UniaxialMaterial(tag,
                                                             MAT_TAG_ElasticMaterial),
trialStrain(0.0), trialStrainRate(0.0), committedStrain(0.0),
committedStrainRate(0.0), Epos(ep), Eneg(en), eta(et), parameterID(0)
{

}


ElasticMaterial::ElasticMaterial():UniaxialMaterial(0, MAT_TAG_ElasticMaterial),
...
int ElasticMaterial::setTrial(double strain, double &stress,
...
double ElasticMaterial::getStress(void)
{
    if (trialStrain >= 0.0)
...
double ElasticMaterial::getTangent(void)
{
    if (trialStrain > 0.0)
        return Epos;
...
double ElasticMaterial::getInitialTangent(void)
{
    return (Epos > Eneg) ? Epos : Eneg;
}


int ElasticMaterial::commitState(void)
{
...
int ElasticMaterial::revertToLastCommit(void)
{
...
int ElasticMaterial::revertToStart(void)
{
    trialStrain = 0.0;
    trialStrainRate = 0.0;
    return 0;
...
UniaxialMaterial *ElasticMaterial::getCopy(void)
{
    ElasticMaterial *theCopy =
        new ElasticMaterial(this->getTag(), Epos, eta, Eneg);
...
int ElasticMaterial::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
...
int ElasticMaterial::recvSelf(int cTag, Channel & theChannel,
...
void ElasticMaterial::Print(OPS_Stream & s, int flag)
{
...
int ElasticMaterial::setParameter(const char **argv, int argc,
                                  Parameter & param)
{
...
int ElasticMaterial::updateParameter(int parameterID, Information & info)
{
    switch (parameterID) {
    case 1:
        Epos = info.theDouble;
...
int ElasticMaterial::activateParameter(int paramID)
{
    parameterID = paramID;
...
double ElasticMaterial::getInitialTangentSensitivity(int gradIndex)
{
    if (parameterID == 1)
        return 1.0;
    if (parameterID == 2)
