...
TzLiq1::TzLiq1 (int tag, int classtag, int tz_type, double t_ult, double z_50,
                double dash_pot, int solid_elem1, int solid_elem2,
                Domain * the_Domain):
TzSimple1 (tag, classtag, tz_type, t_ult, z_50, dash_pot),
solidElem1 (solid_elem1),
solidElem2 (solid_elem2),
...
TzLiq1::TzLiq1 (int tag, int classtag, int tz_type, double t_ult, double z_50,
                double dash_pot, Domain * the_Domain,
                TimeSeries * the_Series):
TzSimple1 (tag, classtag, tz_type, t_ult, z_50, dash_pot),
theDomain (the_Domain),
theSeries (the_Series)
{
    // Initialize TzSimple variables and history variables
    //
    this->revertToStart ();
                ...
                for (int i = 0; i < 4; i++)
                  {
                      ...
                      if (NDM->getClassTag () !=
                          ND_TAG_FluidSolidPorousMaterial)
                        {
                            opserr << "Material: " << NDM->
                                getTag () <<
                                " cannot be used to read effective stress for a TzLiq1 material."
                      ...
                      if (NDM->getClassTag () ==
                          ND_TAG_InitialStateAnalysisWrapper)
                        {
                            InitialStateAnalysisWrapper *NDM =
                                (InitialStateAnalysisWrapper *) (theElement1->
                                                                 theMaterial);
                ...
                for (int i = 0; i < 4; i++)
                  {
                      ...
                      if (NDM->getClassTag () ==
                        ...
                        {
                            InitialStateAnalysisWrapper *NDM =
                                (InitialStateAnalysisWrapper *) (theElement2->
                                                                 theMaterial);
                            if (NDM->getMainClassTag () !=
                                ND_TAG_PressureDependMultiYield
                                && NDM->getMainClassTag () !=
          ...
          if (theElement1->getClassTag () == ELE_TAG_Nine_Four_Node_QuadUP)
            {
                ...
                NineFourNodeQuadUP *theElement1 =
                    (NineFourNodeQuadUP *) (theDomain->
                                            getElement (solidElem1));
                meanStress = 0.0;
...
int
TzLiq1::setParameter (const char **argv, int argc, Parameter & param)
{
    if (argc >= 2)
