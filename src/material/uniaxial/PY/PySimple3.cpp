...
void *OPS_PySimple3()
{
    ...
    if (numdata < 5) {
        ...
        opserr <<
            "Want: uniaxialMaterial PySimple3 tag? pult? pyield? ke? C? dashpot? "
            << endln;
        return 0;
    }
    ...
    theMaterial =
        new PySimple3(idata[0], MAT_TAG_PySimple3, ddata[0], ddata[1],
...
PySimple3::PySimple3(int tag, int classtag, double p_ult, double p_yield,
                     double k_e, double C_,
                     double dashpot_):UniaxialMaterial(tag, classtag),
...
PySimple3::PySimple3():UniaxialMaterial(0, 0),
...
double
 PySimple3::getResidual(double ke, double Cp, double Tp, double dy,
                        double pu, double C, double Tpin, double dashpot,
                        double tstepCurrent, double dyELast,
                        double CyeTotal, double tstepLast, double Pveguess,
                        double bump)
{
    signdy = sign(dy);
    ...
    if (pu > Tp * signdy) {
        if (tstep != 0.0) {
            return (C * ke *
                    (dy -
                     ((Tp - Cp +
                       (dashpot * (dyELast / tstepLast) - bump)) / (ke +
                                                                    (dashpot
                                                                     /
                                                                     tstepCurrent)))))
                + (Tp - Cp) + (Tpin -
                               pu * signVEguess) * (log(pu -
                                                        Cp *
                                                        signVEguess) -
                                                    log(pu -
                                                        Tp * signVEguess));
            ...
            return ((Tp - Cp) * (1.0 - 1.0 / C) +
                    ((Tpin - pu * signdy) * (log(pu - Tp * signdy) -
                                             log(pu -
                                                 Cp * signdy))) / C -
                    ke * dy);
        }
...
/////////////////////////////////////////////////////////////////////
// Main function
int PySimple3::setTrialStrain(double y, double yRate)
{
    ...
    if (yRate == 0.0) {
        ...
        P1 = Cp + ke * dy + ((dashpot / tstep1) * dy) -
            ((dashpot / tstep2) * dyELast);
    }
    ...
    if (yRate == 0.0) {
        ...
        Ttangent =
            ke + dashpot * ((1.0 / tstep1) - (dyELast / (tstep2 * dy)));
    ...
    if (f <= 0.0) {
        TyeTotal =
            (Tp - Cp + ke * CyeTotal + dashpot * CyeTotal / tstep1 +
             dashpot * dyELast / tstep2) / (ke + dashpot / tstep1);
    ...
    if (f > 0) {
        ...
        if ((fabs(Cp) > 0.99 * pult && fabs(P1) > fabs(pult)
             && signdy == signdyLast) || (fabs(Cp) > 0.99 * pult
                                          && fabs(P1) > fabs(pult)
                                          && signdy == 0)) {
            ...
            TyeTotal =
                (Tp - CpUse + ke * CyeTotalUse +
                 dashpot * CyeTotalUse / tstep1 +
                 dashpot * dyELastUse / tstep2) / (ke + dashpot / tstep1);
        ...
        if (signP != signdy && signP != 0 && dy == 0.0
            && signP != CLastYieldDir)
        ...
        //Compute residuals       
        R1 = getResidual(ke, CpUse, pn1_a, dy, pult, C, TpinUse, dashpot,
                         tstep1, dyELastUse, CyeTotalUse, tstep2, P1,
                         bump);
        R2 = getResidual(ke, CpUse, pn1_b, dy, pult, C, TpinUse, dashpot,
                         tstep1, dyELastUse, CyeTotalUse, tstep2, P1,
                         bump);
        ...
        if (sign(R1) == sign(R2)) {
            ...
            pn1_b =
                (1.0 - 0.0000000001 * PYtolerance) * TLastYieldDir * pult;
            R2 = getResidual(ke, CpUse, pn1_b, dy, pult, C, TpinUse,
                             dashpot, tstep1, dyELastUse, CyeTotalUse,
                             tstep2, P1, bump);
            if (fabs(R2) < PYtolerance * pult) {
        ...
        double S = 0.0;
        int i = 0;
        while ((fabs(R1) > PYtolerance * pult)
               && (fabs(R2) > PYtolerance * pult)) {
            pn1_3 = (pn1_a + pn1_b) / 2.0;
            R3 = getResidual(ke, CpUse, pn1_3, dy, pult, C, TpinUse,
                             dashpot, tstep1, dyELastUse, CyeTotalUse,
                             tstep2, P1, bump);
            ...
            pn1_4 =
                pn1_3 + (pn1_3 -
                         pn1_a) * ((sign(R1 - R2) * R3) /
                                   (sqrt(R3 * R3 - R1 * R2)));
            R4 = getResidual(ke, CpUse, pn1_4, dy, pult, C, TpinUse,
                             dashpot, tstep1, dyELastUse, CyeTotalUse,
                             tstep2, P1, bump);
            ...
            if (i == PYmaxIterations) {
                ...
                if (fabs(pn1_a) > 0.995 * pult
                    && fabs(pn1_b) > 0.995 * pult) {
                    Tp = (pn1_a + pn1_b) / 2.0;
                    if (fabs(Tp) >= pult) {
                        Tp = (pult - PYtolerance) * signdy;
                    }
                    ...
                    opserr << "Ridder's method for material tag " <<
                        this->getTag() <<
                        " failed to find a working value for P in " <<
                        PYmaxIterations << " iterations." << endln;
                    Tp = Cp;
                    Ttangent = Ctangent;
                    Tpalpha = Cpalpha;
                    TdyE = dy;
...
/////////////////////////////////////////////////////////////////////
double PySimple3::getStress(void)
{
    return this->Tp;
}

/////////////////////////////////////////////////////////////////////
double PySimple3::getTangent(void)
{
...
/////////////////////////////////////////////////////////////////////
double PySimple3::getInitialTangent(void)
{
    return this->initialTangent;
}

/////////////////////////////////////////////////////////////////////
double PySimple3::getDampTangent(void)
{
...
/////////////////////////////////////////////////////////////////////
double PySimple3::getStrain(void)
{
    return this->Ty;
}

/////////////////////////////////////////////////////////////////////
double PySimple3::getStrainRate(void)
{
    return this->TyRate;
}

/////////////////////////////////////////////////////////////////////
int PySimple3::commitState(void)
{
...
/////////////////////////////////////////////////////////////////////
int PySimple3::revertToLastCommit(void)
{
    Tp = Cp;
    Ty = Cy;
    TpinF = CpinF;
...
/////////////////////////////////////////////////////////////////////
int PySimple3::revertToStart(void)
{
    dy = 0.0;
    CLastYieldDir = 0;
    TLastYieldDir = 0;
...
/////////////////////////////////////////////////////////////////////
UniaxialMaterial *PySimple3::getCopy(void)
{
...
/////////////////////////////////////////////////////////////////////
int PySimple3::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
...
/////////////////////////////////////////////////////////////////////
int PySimple3::recvSelf(int cTag, Channel & theChannel,
...
/////////////////////////////////////////////////////////////////////
void PySimple3::Print(OPS_Stream & s, int flag)
{
