...
ConfinedConcrete01::ConfinedConcrete01(int tag, std::vector < double >*eps,
                                       std::vector <
                                       double
                                       >*sigmac):UniaxialMaterial(tag,
                                                                  MAT_TAG_ConfinedConcrete01),
CminStrain(0.0), CendStrain(0.0), Cstrain(0.0), Cstress(0.0)
{
    this->sigmac = sigmac;
    this->eps = eps;
}
...
ConfinedConcrete01::ConfinedConcrete01(int tag, int secType, int dim,
                                       ...
                                       double rhos, double fpc,
                                       double stRatio, double Ec,
                                       ...
                                       double tol,
                                       int
                                       maxNumIter):UniaxialMaterial(tag,
                                                                    MAT_TAG_ConfinedConcrete01),
CminStrain(0.0), CendStrain(0.0), Cstrain(0.0), Cstress(0.0)
{
    int ii;
    ...
    opserr << fpc << " " << stRatio << " " << Ec << " " << aggrType << " "
        << concrType << " " << epsc << " " << fc << " " << epsic << " " <<
        fic << " " << ft << " " << fpl << " " << alpha << Eti << endln;

    bglModel(semiLength, epscu, epscuOption, epscuLimit, nuOption, nuc,
             epsc, fc, epsic, fic, ft, fpl, alpha, Eti, phis, As, Is, S,
             fyh, mueps, Es0, haRatio, phiLon, secType, dim, tol,
             maxNumIter);
...
void
 ConfinedConcrete01::setupAttardSetunge(double fpc, double stRatio,
                                        double Ec, double aggrType,
                                        double concrType, double &epsc,
                                        double &fc, double &epsic,
                                        double &fic, double &ft,
                                        double &fpl, double &alpha,
                                        double &Eti)
{
...
void ConfinedConcrete01::bglModel(std::vector < double >semiLength,
                                  double &epscu, int epscuOption,
                                  double epscuLimit, int nuOption,
                                  double nuc, double epsc, double fc,
                                  double epsic, double fic, double ft,
                                  ...
                                  std::vector < double >Es0,
                                  std::vector < double >haRatio,
                                  double phiLon, int secType, int dim,
                                  double tol, double maxNumIter)
{
    ...
    for (i = 0;
        ...
        do {                    // Beginning of the iteration loop
            ...
            if (nuOption == 0)
                ...
                double nuTemp =
                    nu0 * (1 + 0.2 * rn - pow(rn, 2) + 1.55 * pow(rn, 3));
            ...
            if (secType == 5) { //S5 without wrapping
                airyConSqSec(semiLength[0], Ec[i], nu[i], As[0], Is[0],
                             Es[i][0], S[0], eps->at(i), A[i][0], B[i][0]);
                B[i][1] =
            ...
            if (secType == 5) { // S5 without wrapping     
                ...
                for (j = 0; j < dim; j++) {
                    frm[i][j] =
                        confPressSqSec(semiLength[j], B[i][j], ksl[j]);
                }
            }
            ...
            if (iter > 0 && ((prevDiff < 0 && difference > 0)
...
double
    ConfinedConcrete01::confAlongCol(double semiLength, double phis,
                                     double S, int dim, double phiLon)
{
...
void ConfinedConcrete01::trReinfModCircSec(double syh, double duc,
                                           double Eel, double Atr,
                                           double DimSec, double hard,
                                           double CB, double &etr,
...
void ConfinedConcrete01::airyConSqSec(double semiLength, double Ecz,
                                      double nuz, double As, double Is,
...
void ConfinedConcrete01::superPosConfPress(std::vector < std::vector <
                                           double > >B,
                                           std::vector <
                                           double >semiLength,
                                           std::vector < std::vector <
                                           double > >&frm, int secType,
                                           int dim, int i)
{
...
void ConfinedConcrete01::attSet(double epsc, double fc, double epsic,
                                double fic, double ft, double fpl,
                                double alpha, double Eti, double epsz,
                                double fr, double &sigmaci, double &Eczi,
                                double fcu, double &epscu,
...
int ConfinedConcrete01::setTrial(double strain, double &stress,
...
// AddingSensitivity:BEGIN ///////////////////////////////////
int ConfinedConcrete01::setParameter(const char **argv, int argc,
                                     Parameter & param)
{
...
int ConfinedConcrete01::updateParameter(int parameterID,
                                        Information & info)
{
    switch (parameterID) {
    case 1:
...
int ConfinedConcrete01::activateParameter(int passedParameterID)
{
...
double
    ConfinedConcrete01::getStressSensitivity(int gradNumber,
                                             bool conditional)
{
...
int ConfinedConcrete01::getVariable(const char *varName,
                                    Information & theInfo)
{
...
OPS_Export void *OPS_ConfinedConcrete01Material()
{
    ...
    int secType;
    int dim = 1;
    std::vector < double >semiLength, phis, S, fyh, Es0, haRatio, mueps,
        As, Is;
    double rhos;
    double fpc;
    double stRatio = 1.0;
