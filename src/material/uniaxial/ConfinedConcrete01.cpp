...
ConfinedConcrete01::ConfinedConcrete01 ():UniaxialMaterial (0, MAT_TAG_ConfinedConcrete01),
...
ConfinedConcrete01::ConfinedConcrete01 (int tag, std::vector < double >*eps,
...
ConfinedConcrete01::ConfinedConcrete01 (int tag, int secType, int dim,
                                        std::vector < double >semiLength,
                                        std::vector < double >phis,
                                        std::vector < double >S,
                                        std::vector < double >fyh,
                                        std::vector < double >Es0,
                                        std::vector < double >haRatio,
                                        std::vector < double >mueps,
                                        std::vector < double >As,
                                        std::vector < double >Is, double rhos,
                                        double fpc, double stRatio, double Ec,
                                        int epscuOption, double epscu,
                                        double epscuLimit, int nuOption,
                                        double nuc, double phiLon,
                                        int concrType, int aggrType,
                                        double tol, int maxNumIter):
UniaxialMaterial (tag, MAT_TAG_ConfinedConcrete01),
CminStrain (0.0),
CendStrain (0.0),
Cstrain (0.0),
Cstress (0.0)
{
    ...
    opserr << fpc << " " << stRatio << " " << Ec << " " << aggrType << " " <<
        concrType << " " << epsc << " " << fc << " " << epsic << " " << fic <<
        " " << ft << " " << fpl << " " << alpha << Eti << endln;

    bglModel (semiLength, epscu, epscuOption, epscuLimit, nuOption, nuc, epsc,
              fc, epsic, fic, ft, fpl, alpha, Eti, phis, As, Is, S, fyh,
              mueps, Es0, haRatio, phiLon, secType, dim, tol, maxNumIter);
...
void
ConfinedConcrete01::setupAttardSetunge (double fpc, double stRatio, double Ec,
                                        double aggrType, double concrType,
                                        double &epsc, double &fc,
                                        double &epsic, double &fic,
                                        double &ft, double &fpl,
...
void
ConfinedConcrete01::bglModel (std::vector < double >semiLength, double &epscu,
                              int epscuOption, double epscuLimit,
                              int nuOption, double nuc, double epsc,
                              double fc, double epsic, double fic, double ft,
                              double fpl, double alpha, double Eti,
                              std::vector < double >phis,
                              std::vector < double >As,
                              ...
                              std::vector < double >mueps,
                              std::vector < double >Es0,
                              std::vector < double >haRatio, double phiLon,
                              int secType, int dim, double tol,
                              double maxNumIter)
{
    ...
    std::vector < std::vector < double >>sigmaTr (1,
                                                  std::vector < double >(dim,
                                                                         0.0));
    std::vector < std::vector < double >>epsTr (1,
                                                std::vector < double >(dim,
                                                                       0.0));
    ...
    std::vector < std::vector < double >>frm (1,
...
// ----Setting of initial value of some parameters
    ...
    for (i = 0;
         eps->at (i) <= epscu + dEpsZ / 10.0 && (epscuOption != 2
                                                 || eps->at (i) <=
                                                 epscuLimit + dEpsZ / 10.0);
         i++)
      {
          int iter = 0;
          nu.push_back (0.0);
          ...
          do
            {                   // Beginning of the iteration loop
                ...
                if (secType == 5)
                  {             // S5 without wrapping  
                      trReinfModSqSec (fyh[0], mueps[0], Es0[0], As[0], S[0],
                                       semiLength[0], haRatio[0], A[i][0],
                                       B[i][0], epsTr[i][0], sigmaTr[i][0],
                                       Es[i][0]);
                      trReinfModCircSec (fyh[1], mueps[1], Es0[1], As[1],
                                         semiLength[1], haRatio[1], B[i][1],
                                         epsTr[i][1], sigmaTr[i][1],
                                         Es[i][1]);

                      if (dim == 3)
                        {       //S5 with Wrapping
                            trReinfModSqSec (fyh[2], mueps[2], Es0[2], As[2],
                                             S[2], semiLength[2], haRatio[2],
                                             A[i][2], B[i][2], epsTr[i][2],
                                             sigmaTr[i][2], Es[i][2]);
                        }
                  }
                else if (secType == 6)
                  {             //S6
                      for (j = 0; j < dim; j++)
                        {
                            trReinfModCircSec (fyh[j], mueps[j], Es0[j],
                                               As[j], semiLength[j],
                                               haRatio[j], B[i][j],
                                               epsTr[i][j], sigmaTr[i][j],
                                               Es[i][j]);
                        }
                  }
                else
                  {             //Other Sections
                      for (j = 0; j < dim; j++)
                        {
                            trReinfModSqSec (fyh[j], mueps[j], Es0[j], As[j],
                                             S[j], semiLength[j], haRatio[j],
                                             A[i][j], B[i][j], epsTr[i][j],
                                             sigmaTr[i][j], Es[i][j]);
                        }
                  }
                // ----------------------------- Airy's Constants -----------------------
                if (secType == 5)
                  {             //S5 without wrapping
                      airyConSqSec (semiLength[0], Ec[i], nu[i], As[0], Is[0],
                                    Es[i][0], S[0], eps->at (i), A[i][0],
                                    B[i][0]);
                      B[i][1] =
                          airyConCircSec (semiLength[1], Ec[i], nu[i], As[1],
                                          Es[i][1], S[1], eps->at (i));

                      if (dim == 3)     //S5 with wrapping
                          airyConSqSec (semiLength[2], Ec[i], nu[i], As[2],
                                        Is[2], Es[i][2], S[2], eps->at (i),
                                        A[i][2], B[i][2]);
                  }
                else if (secType == 6)
                  {             //S6
                      for (j = 0; j < dim; j++)
                        {
                            B[i][j] =
                                airyConCircSec (semiLength[j], Ec[i], nu[i],
                                                As[j], Es[i][j], S[j],
                                                eps->at (i));
                        }
                  }
                  ...
                  {             //Other sections
                      for (j = 0; j < dim; j++)
                        {
                            airyConSqSec (semiLength[j], Ec[i], nu[i], As[j],
                                          Is[j], Es[i][j], S[j], eps->at (i),
                                          A[i][j], B[i][j]);
                        }
                  }
                ...
                if (secType == 5)
                  {             // S5 without wrapping     
                      frm[i][0] =
                          confPressSqSec (semiLength[0], B[i][0], ksl[0]);
                      frm[i][1] = confPressCircSec (S[1], B[i][1], ksl[1]);

                      if (dim == 3)     //S5 with wrapping
                          frm[i][2] =
                              confPressSqSec (semiLength[2], B[i][2], ksl[2]);
                  }
                else if (secType == 6)
                  {             //S6
                      for (j = 0; j < dim; j++)
                        {
                            frm[i][j] =
                                confPressCircSec (S[j], B[i][j], ksl[j]);
                        }
                  }
                else
                  {             //Other sections
                      for (j = 0; j < dim; j++)
                        {
                            frm[i][j] =
                                confPressSqSec (semiLength[j], B[i][j],
                                                ksl[j]);
                        }
                  }
                ...
                attSet (epsc, fc, epsic, fic, ft, fpl, alpha, Eti,
                        eps->at (i), frm[i][dim], (*sigmac)[i], Ec[i], fcu,
                        epscu, epscuOption, epscuLimit);
          ...
          if (iter == maxNumIter)
            {
                printf
                    ("WARNING: Does not converge with specified maximum number of iterations.\n");
            }
          totalIter += iter;
...
// --- Calculation of Confined Concrete Peak Stress 
          ...
          if (eps->at (i) + dEpsZ <= epscu + dEpsZ / 10.0
              && (epscuOption != 2
                  || eps->at (i) + dEpsZ <= epscuLimit + dEpsZ / 10.0))
            {
                eps->push_back (0.0);
                sigmac->push_back (0.0);
                Ec.push_back (0.0);
...
double
ConfinedConcrete01::confAlongCol (double semiLength, double phis, double S,
                                  int dim, double phiLon)
{
...
void
ConfinedConcrete01::trReinfModSqSec (double syh, double duc, double Eel,
                                     double Atr, double Str, double DimSec,
                                     double hard, double CA, double CB,
                                     double &etr, double &sgtr, double &Essec)
{
...
void
ConfinedConcrete01::trReinfModCircSec (double syh, double duc, double Eel,
                                       double Atr, double DimSec, double hard,
                                       double CB, double &etr, double &sgtr,
                                       double &Essec)
{
    double etry, etru;
...
void
ConfinedConcrete01::airyConSqSec (double semiLength, double Ecz, double nuz,
                                  double As, double Is, double Esz, double S,
                                  double epsz, double &Az, double &Bz)
{
    ...
    DCA2 =
        6 * S * Ecz * Esz * semiLength * (315 * Is * (nuz + 1) +
                                          2 * pow (semiLength,
                                                   2) * As * (2 * nuz + 5));
    ...
    NCB =
        18 * Ecz * Esz * As * nuz * (S * Ecz * pow (semiLength, 3) +
                                     105 * Esz * Is * (nuz + 1));
...
double
ConfinedConcrete01::airyConCircSec (double R, double Ecz, double nuz,
                                    double As, double Esz, double S,
                                    double epsz)
{
...
double
ConfinedConcrete01::confPressSqSec (double semiLength, double Bz, double k)
{
...
double
ConfinedConcrete01::confPressCircSec (double S, double q, double k)
{
...
void
ConfinedConcrete01::superPosConfPress (std::vector < std::vector <
                                       double > >B,
                                       std::vector < double >semiLength,
                                       std::vector < std::vector <
                                       double > >&frm, int secType, int dim,
                                       int i)
{

    double fr1, fr2;

    if ((secType == 1) || (secType == 6))
      {                         //S1 and C
          if (dim == 1)
              frm[i][dim] = frm[i][0];
          else
              frm[i][dim] =
                  pow (semiLength[0] / semiLength[1],
                       2) * frm[i][0] + frm[i][1];
      }
      ...
      {                         //S2 or S3 
          if (dim == 2)         // Without wrapping
              ...
              frm[i][dim] =
                  (frm[i][0] +
                   0.5 * frm[i][1]) * pow (semiLength[0] / semiLength[2],
                                           2) + frm[i][2];
      }
    else if (secType == 41)
      {                         // 2l(1)=2l(3)=L, 2l(2)=a, 2l(4)=b
          fr1 =
              frm[i][1] * pow (semiLength[1],
                               2) + frm[i][0] * (semiLength[0] -
                                                 semiLength[1]) *
              semiLength[1];
          fr1 = fr1 / (semiLength[0] * semiLength[1]);

          fr2 =
              frm[i][3] * pow (semiLength[3],
                               2) + frm[i][2] * (semiLength[2] -
                                                 semiLength[3]) *
              semiLength[3];
          ...
          if (dim == 4)         //Without wrapping 
              frm[i][dim] =
                  (1 + semiLength[3] / semiLength[0]) * fr1 +
                  semiLength[3] / semiLength[0] * fr2;
          else                  // With wrapping dim=5
              frm[i][dim] =
                  fr1 * semiLength[0] * (semiLength[0] +
                                         semiLength[3]) / pow (semiLength[4],
                                                               2) +
                  fr2 * semiLength[3] * semiLength[0] / pow (semiLength[4],
                                                             2) + frm[i][4];
      }
    else if (secType == 42)
      {                         // 2l(1)=2l(2)=2l(3)=L, 2l(4)=b
          fr1 = frm[i][0];
          fr2 =
              (frm[i][2] * pow (semiLength[2], 2) +
               frm[i][1] * (semiLength[1] - semiLength[2]) * semiLength[2]);
          ...
          if (dim == 3)
              ...
              frm[i][dim] =
                  fr1 * pow (semiLength[0] / semiLength[3],
                             2) +
                  2 * (semiLength[2] * semiLength[0] /
                       pow (semiLength[3], 2)) * fr2 + frm[i][3];
      }
    else if (secType == 5)
      {                         // S5
          if (dim == 2)         // Without wrapping
              ...
              frm[i][dim] =
                  frm[i][0] * pow (semiLength[0] / semiLength[2],
                                   2) +
                  frm[i][1] * (PI / 4.0) * pow (semiLength[0] / semiLength[2],
                                                2) + frm[i][2];
      }
    else if (secType == 7)
      {                         // l(1)=d, l(2)=c, l(3)=l(1)+cover, l(4)=l(2)+cover 
          if (dim == 2)
            {                   // Without wrapping
                frm[i][dim] =
                    (frm[i][1] * pow (semiLength[1], 2) +
                     frm[i][0] * (semiLength[0] -
                                  semiLength[1]) * semiLength[1]) /
                    (semiLength[0] * semiLength[1]);
            }
          else
            {                   // With wrapping
                fr1 =
...
void
ConfinedConcrete01::attSet (double epsc, double fc, double epsic, double fic,
                            double ft, double fpl, double alpha, double Eti,
                            double epsz, double fr, double &sigmaci,
                            double &Eczi, double fcu, double &epscu,
                            double epscuOption, double epscuLimit)
{
//     Attard&Setunge's variables 
    ...
    if (fr == 0)
      {                         // If the confining pressure is equal zero
// ------------------------------Sargin's constant------------------------         
...
// -----Stress and Strain at Point of Inflection
          fi = ((fic / fc - 1.0) / (5.06 * pow ((fr / fc), 0.57) + 1.0) +
                1.0) * f0;
          epsi =
              ((epsic / epsc - 2.0) / (1.12 * pow (fr / fc, 0.26) + 1.0) +
               2.0) * eps0;

// -----Stress and Strain at Second Point         
          f2ic = (1.45 - 0.25 * log (fc)) * fc;
          f2i =
              ((f2ic / fc - 1.0) / (6.35 * pow (fr / fc, 0.62) + 1.0) +
               1.0) * f0;
          eps2i = 2.0 * epsi - eps0;
...
//      Descending branch      
          ...
          Ad = (eps2i - epsi) / eps0 * (eps2i * Ei / (f0 - fi) -
                                        4 * epsi * E2i / (f0 - f2i));
...
int
ConfinedConcrete01::setTrialStrain (double strain, double strainRate)
{
...
int
ConfinedConcrete01::setTrial (double strain, double &stress, double &tangent,
                              double strainRate)
{
    // Reset trial history variables to last committed state
    TminStrain = CminStrain;
...
void
ConfinedConcrete01::determineTrialState (double dStrain)
{
    TminStrain = CminStrain;
    TendStrain = CendStrain;
    TunloadSlope = CunloadSlope;
        ...
        for (int i = 0; i < (int) eps->size (); i++)
          {
              if (Tstrain > eps->at (i))
                {
                    Ttangent =
                        (sigmac->at (i) - sigmac->at (i - 1)) / (eps->at (i) -
                                                                 eps->at (i -
                                                                          1));
                    Tstress =
                        (Tstrain - eps->at (i - 1)) * Ttangent +
                        sigmac->at (i - 1);
                    break;
                }
          }
...
double
ConfinedConcrete01::getStress ()
{
    return Tstress;
}

double
ConfinedConcrete01::getStrain ()
{
    return Tstrain;
}

double
ConfinedConcrete01::getTangent ()
{
    return Ttangent;
}

int
ConfinedConcrete01::commitState ()
{
    // History variables
...
int
ConfinedConcrete01::revertToLastCommit ()
{
    // Reset trial history variables to last committed state
    TminStrain = CminStrain;
    TendStrain = CendStrain;
...
int
ConfinedConcrete01::revertToStart ()
{
//      double Ec0 = 2.0*fpc/epsc0;
...
UniaxialMaterial *
ConfinedConcrete01::getCopy ()
{
    ConfinedConcrete01 *theCopy =
        new ConfinedConcrete01 (this->getTag (), eps, sigmac);
...
int
ConfinedConcrete01::sendSelf (int commitTag, Channel & theChannel)
{
    int res = 0;
...
int
ConfinedConcrete01::recvSelf (int commitTag, Channel & theChannel,
                              FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0)
      {
          opserr <<
              "ConfinedConcrete01::recvSelf() - failed to receive data\n";
          this->setTag (0);
      }
    else
      {
...
void
ConfinedConcrete01::Print (OPS_Stream & s, int flag)
{
...
// AddingSensitivity:BEGIN ///////////////////////////////////
int
ConfinedConcrete01::setParameter (const char **argv, int argc,
                                  Parameter & param)
{
    ...
    else if (strcmp (argv[0], "fcu") == 0)
      {                         // Crushing strength
          return param.addObject (3, this);
      }
    else if (strcmp (argv[0], "epscu") == 0)
...
int
ConfinedConcrete01::commitSensitivity (double TstrainSensitivity,
...
OPS_ConfinedConcrete01Material ()
{
    ...
    int secType;
    int dim = 1;
    std::vector < double >semiLength, phis, S, fyh, Es0, haRatio, mueps, As,
        Is;
    double rhos;
    double fpc;
    double stRatio = 1.0;
    ...
    if (strcmp (argvS, "S1") == 0 || strcmp (argvS, "s1") == 0)
      ...
      {
          opserr <<
              "WARNING invalid section type, should be:(S1-S3, S4a, S4b, S5, R, C)\n";
    ...
    if (strcmp (argvEPSCU, "-scott") == 0
        || strcmp (argvEPSCU, "-Scott") == 0)
      {
          epscuOption = 1;
      }
          ...
          if (OPS_GetDouble (&numData, &epscu) != 0)
            {
                ...
                opserr << "uniaxialMaterial ConfinedConcrete01: " << tag <<
                    endln;
                return 0;
            }
          else
...
// --Parse transverse reinforcement properties---------------------------------
    ...
    while (argc > 0)
      {
          ...
          // internal transverse
                   ...
                   || (strcmp (argvLoc, "-internal") == 0))
            {
                ...
                if (!
                    ((2 <= secType && secType <= 5) || secType == 41
                     || secType == 42))
                  {
                      opserr <<
                          "WARNING Stirrups only valid for section types S2, S3, S4a, S4b, S5\n";
                      opserr << "uniaxialMaterial ConfinedConcrete01: " << tag
                          << endln;
                      return 0;
                  }
          ...
          else if (strcmp (argvLoc, "-wrap") == 0)
            {
                if (argc < 6)
                  {
                      opserr <<
                          "WARNING insufficient arguments with -wrap option\n";
                      return 0;
                  }
                ...
                if (secType == 7)
                  {
                      dim++;
                      semiLength.push_back (semiLength[1] + phis[1] / 2 +
                                            temp);
                  }
    ...
    if (secType == 1)
      ...
      {                         //S5
          rhos =
              2 * (As[0] +
                   (1 + sqrt (2.0)) / 3.0 * As[1]) / (S[0] * semiLength[0]);
      }
    else if (secType == 6)
      {                         //S6
    ...
    theMaterial =
        new ConfinedConcrete01 (tag, secType, dim, semiLength, phis, S, fyh,
                                Es0, haRatio, mueps, As, Is, rhos, fpc,
                                stRatio, Ec, epscuOption, epscu, epscuLimit,
                                nuOption, nuc, phiLon, concrType, aggrType,
                                tol, maxNumIter);
