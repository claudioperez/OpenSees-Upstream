...
void *OPS_ElasticMaterialThermal(void)
{
    ...
    if (OPS_GetNumRemainingInputArgs() < 3) {
        opserr <<
            "Invalid #args,  want: uniaxialMaterial Elastic tag? E? alpha?<eta?> ... "
            << endln;
        return 0;
    }
    ...
    if (OPS_GetIntInput(&numData, iData) != 0) {
        opserr << "WARNING invalid tag for uniaxialMaterial Elastic" <<
            endln;
        return 0;
    }
    ...
    if (OPS_GetDoubleInput(&numData, dData1) != 0) {
        opserr << "Invalid data for uniaxial Elastic " << iData[0] <<
            endln;
        return 0;
    }
    ...
    theMaterial =
        new ElasticMaterialThermal(iData[0], dData1[0], dData1[1],
                                   dData2[0], dData2[1], softindex);
    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type ElasticMaterialThermal\n";
        return 0;
    }
...
double
    ElasticMaterialThermal::SteelRedFactors[12] =
    { 1.0, 0.9, 0.8, 0.7, 0.6, 0.31, 0.13, 0.09, 0.0675, 0.045, 0.0225,
0.0 };
double
 ElasticMaterialThermal::ConcRedFactors[12] =
    { 0.625, 0.4318, 0.3036, 0.1875, 0.1, 0.045, 0.03, 0.015, 0.008,
    0.004, 0.001, 0.0
};

ElasticMaterialThermal::ElasticMaterialThermal(int tag, double e,
                                               double alpha, double et,
                                               double eneg,
                                               int
                                               softindex):UniaxialMaterial
    (tag, MAT_TAG_ElasticMaterialThermal), trialStrain(0.0),
trialStrainRate(0.0), committedStrain(0.0), committedStrainRate(0.0),
Epos(e), eta(et), Alpha(alpha), E0(e), softIndex(softindex),
ThermalElongation(0), Temp(0), parameterID(0)
{
    ...
    if (softIndex < 0 || softIndex > 2) {
        opserr << "ElasticMaterialThermal " << this->getTag() <<
            " receives an invalid softening index, setting softening index to 0"
            << endln;
        softIndex = 0;
    }
}

ElasticMaterialThermal::ElasticMaterialThermal():UniaxialMaterial(0, MAT_TAG_ElasticMaterialThermal),
trialStrain(0.0), trialStrainRate(0.0), committedStrain(0.0),
committedStrainRate(0.0), Epos(0.0), Eneg(0.0), eta(0.0), Alpha(0.0),
E0(0.0), Eneg0(0.0), softIndex(0), ThermalElongation(0), Temp(0),
parameterID(0)
{
...
int ElasticMaterialThermal::setTrial(double strain, double &stress,
...
double ElasticMaterialThermal::getStress(void)
{
    if (trialStrain >= 0.0)
...
double ElasticMaterialThermal::getTangent(void)
{
    if (trialStrain > 0.0)
        return Epos;
...
double ElasticMaterialThermal::getInitialTangent(void)
{
    return (Epos > Eneg) ? Epos : Eneg;
}


int ElasticMaterialThermal::commitState(void)
{
...
int ElasticMaterialThermal::revertToLastCommit(void)
{
...
int ElasticMaterialThermal::revertToStart(void)
{
    trialStrain = 0.0;
    trialStrainRate = 0.0;
    return 0;
...
UniaxialMaterial *ElasticMaterialThermal::getCopy(void)
{
    ElasticMaterialThermal *theCopy =
        new ElasticMaterialThermal(this->getTag(), Epos, Alpha, eta, Eneg,
                                   softIndex);
...
int ElasticMaterialThermal::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
    ...
    if (res < 0)
        opserr <<
            "ElasticMaterialThermal::sendSelf() - failed to send data\n";

    return res;
}


int ElasticMaterialThermal::recvSelf(int cTag, Channel & theChannel,
                                     FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0) {
        opserr <<
            "ElasticMaterialThermal::recvSelf() - failed to receive data\n";
        Epos = Eneg = 0;
        this->setTag(0);
    } else {
...
void ElasticMaterialThermal::Print(OPS_Stream & s, int flag)
{
...
int ElasticMaterialThermal::setParameter(const char **argv, int argc,
                                         Parameter & param)
{
...
int ElasticMaterialThermal::updateParameter(int parameterID,
                                            Information & info)
{
    switch (parameterID) {
    case 1:
...
int ElasticMaterialThermal::activateParameter(int paramID)
{
    parameterID = paramID;
...
double
    ElasticMaterialThermal::getStressSensitivity(int gradIndex,
                                                 bool conditional)
{
    if (parameterID == 1)
        return trialStrain;
...
double ElasticMaterialThermal::getInitialTangentSensitivity(int gradIndex)
{
    if (parameterID == 1)
        return 1.0;
    if (parameterID == 2)
...
int ElasticMaterialThermal::commitSensitivity(double strainGradient,
...
double
    ElasticMaterialThermal::getElongTangent(double TempT, double &ET,
    ...
    if (softIndex != 0) {
        ...
        for (int i = 0; i < 13; i++) {
            if (Temp <= 80 + 100 * i) {
                if (i == 0) {
                    ...
                    Eneg =
                        Eneg0 * (1.0 - Temp * (1.0 - redfactors[0]) / 80);
                } else if (i == 12) {
                    opserr << "Warning:The temperature " << Temp <<
                        " for SteelECthermal is out of range\n";
                    return -1;
                } else {
                    Epos =
                        E0 * (redfactors[i - 1] -
                              (Temp + 20 -
                               100 * i) * (redfactors[i - 1] -
                                           redfactors[i]) / 100);
                    Eneg =
                        Eneg0 * (redfactors[i - 1] -
                                 (Temp + 20 -
                                  100 * i) * (redfactors[i - 1] -
                                              redfactors[i]) / 100);
                }
        ...
        if (softIndex == 1) {
            if (Temp <= 1) {
                ...
                ThermalElongation =
                    -2.416e-4 + 1.2e-5 * (Temp + 20) + 0.4e-8 * (Temp +
                                                                 20) *
                    (Temp + 20);
            } else if (Temp <= 840) {
            ...
            if (Temp <= 1) {
                ...
                ThermalElongation =
                    -1.8e-4 + 9e-6 * (Temp + 20) + 2.3e-11 * (Temp +
                                                              20) *
                    (Temp + 20) * (Temp + 20);
            } else if (Temp <= 1180) {
...
int ElasticMaterialThermal::getVariable(const char *variable,
                                        Information & info)
{
