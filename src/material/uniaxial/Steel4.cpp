...
void *OPS_Steel4(void)
{
    ...
    int argc = 1,
        numBasic = 2,
        numKin = 4, numIso = 5, numUlt = 2, numMem = 1, numInit = 1;
    ...
    if (argc < numBasic) {
        opserr <<
            "Invalid #args, want at least two args for Steel4 in the following format:\n"
            << "uniaxialMaterial Steel4" << iData[0] << " E0? fy?" <<
            endln;
        return 0;
    }
    ...
    while (argc > 1) {
        ...
        //kinematic hardening
            ...
            if (OPS_GetDouble(&numKin, kinData) != 0) {
                opserr << "WARNING invalid -kin args for Steel4 (tag: "
                    << iData[0] << ")\n" << endln;
                return 0;
            }
        ...
        //isotropic hardening with MP characteristics
            ...
            if (OPS_GetDouble(&numIso, isoData) != 0) {
                opserr << "WARNING invalid -iso args for Steel4 (tag: "
                    << iData[0] << ")\n" << endln;
                return 0;
            }
            //define the unspecified values in case of symmetric hardening
        ...
        //ultimate strength limit
            ...
            if (OPS_GetDouble(&numUlt, ultData) != 0) {
                opserr << "WARNING invalid -ult args for Steel4 (tag: "
                    << iData[0] << ")\n" << endln;
                return 0;
            }
            //define the unspecified values in case of symmetric hardening
        ...
        //load history memory
            ...
            if (OPS_GetInt(&numMem, memData) != 0) {
                opserr << "WARNING invalid -mem args for Steel4 (tag: "
                    << iData[0] << ")\n" << endln;
                return 0;
            }
        }
        //initial stress
            ...
            if (OPS_GetDouble(&numInit, initData) != 0) {
                opserr << "WARNING invalid -init args for Steel4 (tag: "
                    << iData[0] << ")\n" << endln;
                return 0;
            }
        }
    ...
    theMaterial = new Steel4(iData[0],
                             ...
                             kinData[0], kinData[1], kinData[2],
                             kinData[3], kinData[4], kinData[5],
                             kinData[6], kinData[7], isoData[0],
                             isoData[1], isoData[2], isoData[3],
                             isoData[4], isoData[5], isoData[6],
                             isoData[7], isoData[8], ultData[0],
                             ultData[1], ultData[2], ultData[3],
                             memData[0], initData[0]);

    //Just in case there was a problem with material creation
    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type Steel4\n";
        return 0;
    }
...
Steel4::Steel4(int tag,
               ...
               //isotropic hardening
               double _b_i, double _rho_i, double _b_l, double _R_i,
               double _l_yp, double _b_ic, double _rho_ic, double _b_lc,
               double _R_ic,
               //ultimate strength limit
               ...
               int _cycNum,
               //initial stress
               double _sig_init):UniaxialMaterial(tag, MAT_TAG_Steel4),
f_y(_f_y),
E_0(_E_0),
b_k(_b_k),
R_0(_R_0),
...
R_ic(_R_ic),
f_u(_f_u),
R_u(_R_u), f_uc(_f_uc), R_uc(_R_uc), cycNum(_cycNum), sig_init(_sig_init)
{
...
Steel4::Steel4(void):UniaxialMaterial(0, MAT_TAG_Steel4)
{
    dir_P = 0;
}
...
UniaxialMaterial *Steel4::getCopy(void)
{
...
double Steel4::getInitialTangent(void)
{
    return E_0;
}

double
    Steel4::isoHardening(double eps_pl_tot, double b_1, double b_2,
                         double rho, double R)
{
    ...
    if (eps_pl_tot / eps_y0 > l_yp) {
        ...
        if (mu > 10.0 * DBL_EPSILON) {
            return mu * (b_2 +
                         (b_1 - b_2) / pow(1.0 + pow(b_1 * mu / rho, R),
                                           1 / R));
        }
    }
    return 0.0;
 ...
 Steel4::loadReversal(int loadDir)
{
    ...
    if ((loadDir == 2 && eps_P >= eps_lP)
        || (loadDir == 1 && eps_P <= eps_lP)) {
    ...
    if (loadDir == 1) {
        df_yi =
            df_yiP + (isoHardening(eps_plTot, b_ic, b_lc, rho_ic, R_ic) -
                      isoHardening(eps_plTot, b_i, b_l, rho_i, R_i)) * f_y;
        df_yk =
            E_0 * (eps_0B - sig_0B / E_0 - deps_O) * (b_k -
                                                      b_kc) / ((1 -
                                                                b_k) * (1 -
                                                                        b_kc));
    } else if (loadDir == 2) {
        df_yi =
            df_yiP + (isoHardening(eps_plTot, b_i, b_l, rho_i, R_i) -
                      isoHardening(eps_plTot, b_ic, b_lc, rho_ic,
                                   R_ic)) * f_y;
        df_yk =
            E_0 * (eps_0B - sig_0B / E_0 - deps_O) * (b_k -
                                                      b_kc) / ((1 -
...
void Steel4::calcBreakpoints(int loadDir, double eps_0BC, double sig_0BC,
                             double df_yC, double df_ykC,
                             double eps_pl_tot, double &eps_yC,
                             double &sig_yC, double &eps_lC)
{
    //calculate the total accumulated plastic strain dependent isotropic hardening
    shft = 1.0;
    if (loadDir == 1) {
        shft += isoHardening(eps_pl_tot, b_i, b_l, rho_i, R_i);
    } else if (loadDir == 2) {
        shft += isoHardening(eps_pl_tot, b_ic, b_lc, rho_ic, R_ic);
    }
...
double
    Steel4::calcStress(int loadDir, double eps_C, double eps_0C,
                       double sig_0C, double eps_0BC, double sig_0BC,
                       bool saveProps, double df_yC, double df_ykC)
{
    //calculate the plastic strain at eps_C strain
    if (dir == 1)
        eps_plD = std::max(eps_C - eps_0Y, 0.0);
    else if (dir == 2)
        eps_plD = std::max(-(eps_C - eps_0Y), 0.0);
    //calculate the intersection point for elastic and hardening behavior
    calcBreakpoints(dir, eps_0BC, sig_0BC, df_yC, df_ykC,
                    eps_plTot + eps_plD, eps_yD, sig_yD, eps_lD);
    ...
    if (loadDir == 1) {         //tension
        sig_D = (sig_yD - sig_0C) * eps_ratY *
            (b_k / pow(1.0 + pow(fabs(eps_ratU), R_uy), (1 / R_uy)) +
             (1.0 - b_k) / pow(1.0 + pow(fabs(eps_ratY), R_y),
                               (1 / R_y))) + sig_0C;
    } else if (loadDir == 2) {  //compression
        sig_D = (sig_yD - sig_0C) * eps_ratY *
            (b_kc / pow(1.0 + pow(fabs(eps_ratU), R_uy), (1 / R_uy)) +
             (1.0 - b_kc) / pow(1.0 + pow(fabs(eps_ratY), R_y),
                                (1 / R_y))) + sig_0C;
    }
 ...
 Steel4::setTrialStrain(double trialStrain, double strainRate)
{
    ...
    if (sig_init != 0.0)
        eps = trialStrain + sig_init / E_0;
    else
        eps = trialStrain;      // source: Steel02 mod. by C-P. Lamarche 2006
    ...
    if (dir == 2 && deltaEps > 0.0) {
        ...
        calcBreakpoints(dir, eps_0B, sig_0B, df_yi, df_yk, eps_plTot,
                        eps_0Y, sig_yD, eps_lD);
    ...
    sig =
        calcStress(dir, eps, eps_0, sig_0, eps_0B, sig_0B, true, df_yi,
                   df_yk);
    //calculate the stress increase over eps_inc for stiffness assessment
    sig_inc =
        sig - calcStress(dir, eps - eps_inc, eps_0, sig_0, eps_0B, sig_0B,
                         false, df_yi, df_yk);
    E = sig_inc / eps_inc;
    ...
    if (cycNum != 0 && dir == 1) {      //tension
        ...
        while (i > 0) {
            if (dir_Par[i] == 1) {
                if ((eps_01Par[i] -
                     (eps_0 - (sig_0 - sig_01Par[i]) / E_0)) <
                    -10 * DBL_EPSILON) {
                    //calculate the stress corresponding to eps on the parent loading curve
                    sig_Par =
                        calcStress(dir, eps, eps_01Par[i],
                                   sig_01Par[i], eps_01BPar[i],
                                   sig_01BPar[i], false, df_yiPar[i],
                                   df_ykPar[i]);
                    if (sig_Par < sig) {
                        eps_0 = eps_01Par[i];
                        sig_0 = sig_01Par[i];
                        ...
                        sig =
                            calcStress(dir, eps, eps_0, sig_0,
                                       eps_0B, sig_0B, true, df_yi, df_yk);
                        //calculate the stress increase over eps_inc for stiffness assessment
                        sig_inc =
                            sig - calcStress(dir, eps - eps_inc,
                                             eps_0, sig_0, eps_0B,
                                             sig_0B, true, df_yi, df_yk);
                        E = sig_inc / eps_inc;
                        parentCount = i;
                    }
                }
        ...
        while (i > 0) {
            if (dir_Par[i] == 2) {
                if (((eps_0 + (sig_02Par[i] - sig_0) / E_0) -
                     eps_02Par[i]) < -10 * DBL_EPSILON) {
                    //calculate the stress corresponding to eps on the parent loading curve
                    sig_Par =
                        calcStress(dir, eps, eps_02Par[i],
                                   sig_02Par[i], eps_02BPar[i],
                                   sig_02BPar[i], false, df_yiPar[i],
                                   df_ykPar[i]);
                    if (sig_Par > sig) {
                        eps_0 = eps_02Par[i];
                        sig_0 = sig_02Par[i];
...
double Steel4::getStrain(void)
{
    return eps;
}

double Steel4::getStress(void)
{
    return sig;
}

double Steel4::getTangent(void)
{
    return E;
}

int Steel4::commitState(void)
{
...
int Steel4::revertToLastCommit(void)
{
...
void Steel4::Print(OPS_Stream & s, int flag)
{
