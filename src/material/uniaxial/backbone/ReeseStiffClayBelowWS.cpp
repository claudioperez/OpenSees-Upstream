...
ReeseStiffClayBelowWS::ReeseStiffClayBelowWS(int tag, double esi, double y,
                                             double as,
                                             double
                                             pc):HystereticBackbone(tag,
                                                                    BACKBONE_TAG_ReeseStiffClayBelowWS),
 ...
 ReeseStiffClayBelowWS::getTangent(double strain)
{
    ...
    if (strain <= y1)
        ...
        return 0.25 * Pc / y50 * pow(strain / y50,
                                     -0.5) -
...
double ReeseStiffClayBelowWS::getStress(double strain)
{
    ...
    if (strain <= y1)
        ...
        stress =
            0.5 * Pc * pow(strain / y50,
                           0.5) - 0.055 * Pc * pow((strain - y2) / y2,
                                                   1.25);
...
double ReeseStiffClayBelowWS::getEnergy(double strain)
{
    return 0.0;
}

double ReeseStiffClayBelowWS::getYieldStrain(void)
{
    return 0.0;
}
...
int ReeseStiffClayBelowWS::setVariable(char *argv)
{
    return -1;
}
...
int ReeseStiffClayBelowWS::sendSelf(int commitTag, Channel & theChannel)
{
    ...
    if (res < 0) {
        opserr <<
            "ReeseStiffClayBelowWS::sendSelf -- could not send Vector" <<
            endln;
                                    ...
                                    FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0) {
        opserr <<
            "ReeseStiffClayBelowWS::recvSelf -- could not receive Vector"
            << endln;
