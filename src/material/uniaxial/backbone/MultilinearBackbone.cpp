...
numPoints (num)
{
    ...
    if (E == 0)
        opserr <<
            "MultilinearBackbone::MultilinearBackbone -- failed to allocate tangent array"
            << endln;

    e = new double[numPoints + 1];
    if (e == 0)
        opserr <<
            "MultilinearBackbone::MultilinearBackbone -- failed to allocate strain array"
            << endln;

    s = new double[numPoints + 1];
    if (s == 0)
        opserr <<
            "MultilinearBackbone::MultilinearBackbone -- failed to allocate stress array"
            << endln;

    c = new double[numPoints + 1];
    if (c == 0)
        opserr <<
            "MultilinearBackbone::MultilinearBackbone -- failed to allocate energy array"
            << endln;
