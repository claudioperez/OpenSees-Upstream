                                         ...
                                         Domain * theDomain)
{
    if (argc < 3) {
        opserr <<
            "WARNING insufficient number of hystereticBackbone arguments\n";
        opserr <<
            "Want: hystereticBackbone type? tag? <specific hystereticBackbone args>"
            << endln;
        return TCL_ERROR;
    }
    ...
    if (strcmp(argv[1], "Bilinear") == 0) {
        ...
        if (argc < 6) {
            ...
            opserr <<
                "Want: hystereticBackbone ReeseSoftClay tag? pu? y50? n?"
                << endln;
            return TCL_ERROR;
        }
        ...
        if (Tcl_GetInt(interp, argv[2], &tag) != TCL_OK) {
            opserr <<
                "WARNING invalid hystereticBackbone ReeseSoftClay tag" <<
                endln;
            return TCL_ERROR;
        }
        ...
        if (Tcl_GetDouble(interp, argv[3], &pu) != TCL_OK) {
            opserr <<
                "WARNING invalid hystereticBackbone ReeseSoftClay pu" <<
                endln;
            return TCL_ERROR;
        }
        ...
        if (Tcl_GetDouble(interp, argv[4], &y50) != TCL_OK) {
            opserr <<
                "WARNING invalid hystereticBackbone ReeseSoftClay y50" <<
                endln;
            return TCL_ERROR;
        }

        if (Tcl_GetDouble(interp, argv[5], &n) != TCL_OK) {
            opserr << "WARNING invalid hystereticBackbone ReeseSoftClay n"
                << endln;
            return TCL_ERROR;
        }
        ...
        if (argc < 8) {
            ...
            printCommand(argc, argv);
            opserr <<
                "Want: hystereticBackbone ReeseSand tag? kx? ym? pm? yu? pu?"
                << endln;
            return TCL_ERROR;
        }
        ...
        if (Tcl_GetInt(interp, argv[2], &tag) != TCL_OK) {
            opserr << "WARNING invalid hystereticBackbone ReeseSand tag"
                << endln;
            return TCL_ERROR;
        }

        if (Tcl_GetDouble(interp, argv[3], &kx) != TCL_OK) {
            opserr << "WARNING invalid hystereticBackbone ReeseSand kx" <<
                endln;
            return TCL_ERROR;
        }
        if (Tcl_GetDouble(interp, argv[4], &ym) != TCL_OK) {
            opserr << "WARNING invalid hystereticBackbone ReeseSand ym" <<
                endln;
            return TCL_ERROR;
        }
        if (Tcl_GetDouble(interp, argv[5], &pm) != TCL_OK) {
            opserr << "WARNING invalid hystereticBackbone ReeseSand pm" <<
                endln;
            return TCL_ERROR;
        }
        if (Tcl_GetDouble(interp, argv[6], &yu) != TCL_OK) {
            opserr << "WARNING invalid hystereticBackbone ReeseSand yu" <<
                endln;
            return TCL_ERROR;
        }
        if (Tcl_GetDouble(interp, argv[7], &pu) != TCL_OK) {
            opserr << "WARNING invalid hystereticBackbone ReeseSand pu" <<
                endln;
            return TCL_ERROR;
        }
        ...
        if (argc < 7) {
            ...
            printCommand(argc, argv);
            opserr <<
                "Want: hystereticBackbone ReeseStiffClayBelowWS tag? Esi? y50? As?  Pc?"
                << endln;
            return TCL_ERROR;
        }
        ...
        if (Tcl_GetInt(interp, argv[2], &tag) != TCL_OK) {
            opserr <<
                "WARNING invalid hystereticBackbone ReeseStiffClayBelowWS tag"
                << endln;
            return TCL_ERROR;
        }
        if (Tcl_GetDouble(interp, argv[3], &Esi) != TCL_OK) {
            opserr <<
                "WARNING invalid hystereticBackbone ReeseStiffClayBelowWS Esi"
                << endln;
            return TCL_ERROR;
        }
        if (Tcl_GetDouble(interp, argv[4], &y50) != TCL_OK) {
            opserr <<
                "WARNING invalid hystereticBackbone ReeseStiffClayBelowWS y50"
                << endln;
            return TCL_ERROR;
        }
        if (Tcl_GetDouble(interp, argv[5], &As) != TCL_OK) {
            opserr <<
                "WARNING invalid hystereticBackbone ReeseStiffClayBelowWS As"
                << endln;
            return TCL_ERROR;
        }
        if (Tcl_GetDouble(interp, argv[6], &Pc) != TCL_OK) {
            opserr <<
                "WARNING invalid hystereticBackbone ReeseStiffClayBelowWS Pc"
                << endln;
            return TCL_ERROR;
        }
        ...
        if (argc < 10) {
            ...
            printCommand(argc, argv);
            opserr <<
                "Want: hystereticBackbone Raynor tag? Es? fy? fsu? Epsilonsh? Epsilonsm? C1? Ey?"
                << endln;
            return TCL_ERROR;
        }
        ...
        if (Tcl_GetInt(interp, argv[2], &tag) != TCL_OK) {
            opserr << "WARNING invalid hystereticBackbone Raynor tag" <<
                endln;
            return TCL_ERROR;
        }
        if (Tcl_GetDouble(interp, argv[3], &Es) != TCL_OK) {
            opserr << "WARNING invalid hystereticBackbone Raynor Es" <<
                endln;
            return TCL_ERROR;
        }
        if (Tcl_GetDouble(interp, argv[4], &fy) != TCL_OK) {
            opserr << "WARNING invalid hystereticBackbone Raynor fy" <<
                endln;
            return TCL_ERROR;
        }
        if (Tcl_GetDouble(interp, argv[5], &fsu) != TCL_OK) {
            opserr << "WARNING invalid hystereticBackbone Raynor fsu" <<
                endln;
            return TCL_ERROR;
        }
        if (Tcl_GetDouble(interp, argv[6], &Epsilonsh) != TCL_OK) {
            opserr <<
                "WARNING invalid hystereticBackbone Raynor Epsilonsh" <<
                endln;
            return TCL_ERROR;
        }
        if (Tcl_GetDouble(interp, argv[7], &Epsilonsm) != TCL_OK) {
            opserr <<
                "WARNING invalid hystereticBackbone Raynor Epsilonsm" <<
                endln;
            return TCL_ERROR;
        }
        ...
        theBackbone =
            new RaynorBackbone(tag, Es, fy, fsu, Epsilonsh, Epsilonsm, C1,
                               Ey);
        ...
        if (argc < 5) {
            ...
            printCommand(argc, argv);
            opserr <<
                "Want: hystereticBackbone Capped tag? hystereticBackboneTag? capTag?"
                << endln;
            return TCL_ERROR;
        }
        ...
        if (Tcl_GetInt(interp, argv[2], &tag) != TCL_OK) {
            opserr << "WARNING invalid hystereticBackbone Capped tag" <<
                endln;
            return TCL_ERROR;
        }
        ...
        if (Tcl_GetInt(interp, argv[4], &cTag) != TCL_OK) {
            opserr << "WARNING invalid hystereticBackbone Capped capTag"
                << endln;
            return TCL_ERROR;
        }
        ...
        if (argc < 7) {
            ...
            printCommand(argc, argv);
            opserr <<
                "Want: hystereticBackbone LinearCapped tag? backboneTag? eCap? E? sRes?"
                << endln;
            return TCL_ERROR;
        }
        ...
        if (Tcl_GetInt(interp, argv[2], &tag) != TCL_OK) {
            opserr <<
                "WARNING invalid hystereticBackbone LinearCapped tag" <<
                endln;
            return TCL_ERROR;
        }
        ...
        if (Tcl_GetInt(interp, argv[3], &bTag) != TCL_OK) {
            opserr <<
                "WARNING invalid hystereticBackbone LinearCapped backboneTag"
                << endln;
            return TCL_ERROR;
        }
        ...
        if (Tcl_GetDouble(interp, argv[4], &e) != TCL_OK) {
            opserr <<
                "WARNING invalid hystereticBackbone LinearCapped eCap" <<
                endln;
            return TCL_ERROR;
        }

        if (Tcl_GetDouble(interp, argv[5], &E) != TCL_OK) {
            opserr << "WARNING invalid hystereticBackbone LinearCapped E"
                << endln;
            return TCL_ERROR;
        }
        ...
        if (Tcl_GetDouble(interp, argv[6], &s) != TCL_OK) {
            opserr <<
                "WARNING invalid hystereticBackbone LinearCapped sRes" <<
                endln;
            return TCL_ERROR;
        }
        ...
        if (backbone == 0) {
            opserr << "WARNING hystereticBackbone does not exist\n";
            opserr << "hystereticBackbone: " << bTag;
            opserr << "\nhystereticBackbone LinearCapped: " << tag <<
                endln;
            return TCL_ERROR;
        }
        ...
        if (argc < 4) {
            opserr << "WARNING insufficient arguments\n";
            printCommand(argc, argv);
            opserr << "Want: hystereticBackbone Material tag? matTag?" <<
                endln;
            return TCL_ERROR;
        }
        ...
        opserr << "WARNING unknown type of hystereticBackbone: " <<
            argv[1];
        opserr << "\nValid types: Bilinear, Trilinear, Arctangent," <<
            endln;
    ...
    if (theBackbone == 0) {
        opserr <<
            "WARNING ran out of memory creating hystereticBackbone\n";
        opserr << argv[1] << endln;
        return TCL_ERROR;
    }
    ...
    if (OPS_addHystereticBackbone(theBackbone) == false) {
        opserr <<
            "WARNING could not add hystereticBackbone to the domain\n";
