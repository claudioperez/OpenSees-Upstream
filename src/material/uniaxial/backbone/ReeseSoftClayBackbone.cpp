...
ReeseSoftClayBackbone::ReeseSoftClayBackbone(int tag, double p, double y,
                                             double
                                             nn):HystereticBackbone(tag,
                                                                    BACKBONE_TAG_ReeseSoftClay),
pu(p), y50(y), n(nn)
{
...
double ReeseSoftClayBackbone::getStress(double strain)
{
    int signStrain = (strain > 0.0) ? 1 : -1;
    strain = signStrain * strain;
...
double ReeseSoftClayBackbone::getEnergy(double strain)
{
    return 0.0;
}

double ReeseSoftClayBackbone::getYieldStrain(void)
{
    return 0.0;
}

HystereticBackbone *ReeseSoftClayBackbone::getCopy(void)
{
...
void ReeseSoftClayBackbone::Print(OPS_Stream & s, int flag)
{
...
int ReeseSoftClayBackbone::setVariable(char *argv)
{
    return -1;
}

int ReeseSoftClayBackbone::getVariable(int varID, double &theValue)
{
    return -1;
}

int ReeseSoftClayBackbone::sendSelf(int commitTag, Channel & theChannel)
{
    int res = 0;
    ...
    if (res < 0) {
        opserr <<
            "ReeseSoftClayBackbone::sendSelf -- could not send Vector" <<
            endln;
...
int ReeseSoftClayBackbone::recvSelf(int commitTag, Channel & theChannel,
                                    FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0) {
        opserr <<
            "ReeseSoftClayBackbone::recvSelf -- could not receive Vector"
            << endln;
