...
LinearCappedBackbone::LinearCappedBackbone(int tag,
                                           HystereticBackbone & backbone,
                                           double def, double slope,
                                           double
                                           res):HystereticBackbone(tag,
                                                                   BACKBONE_TAG_LinearCapped),
eCap(def), E(slope), eRes(0.0), sRes(res)
{
    ...
    if (theBackbone == 0)
        opserr <<
            "LinearCappedBackbone::LinearCappedBackbone -- failed to get copy of backbone"
            << endln;
...
double LinearCappedBackbone::getStress(double strain)
{
    if (strain < eCap)
...
double LinearCappedBackbone::getEnergy(double strain)
{
    if (strain < eCap)
...
double LinearCappedBackbone::getYieldStrain(void)
{
...
HystereticBackbone *LinearCappedBackbone::getCopy(void)
{
    LinearCappedBackbone *theCopy =
        new LinearCappedBackbone(this->getTag(), *theBackbone, eCap, E,
                                 sRes);

    return theCopy;
}

void LinearCappedBackbone::Print(OPS_Stream & s, int flag)
{
...
int LinearCappedBackbone::setVariable(char *argv)
{
    return -1;
}

int LinearCappedBackbone::getVariable(int varID, double &theValue)
{
    return -1;
}

int LinearCappedBackbone::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
    ...
    if (res < 0) {
        opserr << "LinearCappedBackbone::sendSelf -- could not send Vector"
            << endln;
        return res;
    }
    ...
    if (res < 0) {
        opserr << "LinearCappedBackbone::sendSelf -- could not send ID" <<
            endln;
        return res;
    }
    ...
    if (res < 0) {
        opserr <<
            "LinearCappedBackbone::sendSelf -- could not send HystereticBackbone"
            << endln;
        return res;
    }
...
int LinearCappedBackbone::recvSelf(int cTag, Channel & theChannel,
                                   FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0) {
        opserr <<
            "LinearCappedBackbone::recvSelf -- could not receive Vector" <<
            endln;
        return res;
    }
    ...
    if (res < 0) {
        opserr << "LinearCappedBackbone::recvSelf -- could not receive ID"
            << endln;
        return res;
    }
    ...
    if (theBackbone == 0) {
        ...
        if (theBackbone == 0) {
            opserr <<
                "LinearCappedBackbone::recvSelf -- could not get a HystereticBackbone"
                << endln;
            return -1;
        }
    }
    ...
    if (res < 0) {
        opserr <<
            "LinearCappedBackbone::recvSelf -- could not receive HystereticBackbone"
            << endln;
        return res;
    }
