...
void
CubicSpline::Fit (double *x, int xl, double *y, int yl)
{
    ////printf("jaja?\n");
    xs = new double[xl];
    ys = new double[xl];
    ...
    for (int i = 0; i < dxsL - 1; i++)
      {
          ...
          if (m * mNext <= 0)
          ...
          else
            {
                ...
                c1s[c1sc] =
                    (3 * common /
                     ((common + dxNext) / m + (common + dx) / mNext));
                c1sc++;
            }
      }
    c1s[c1sc] = (ms[msL - 1]);
    ...
    for (int i = 0; i < c1sL - 1; i++)
      {
          double c1 = c1s[i], m = ms[i], invDx = 1 / dxs[i], common =
              c1 + c1s[i + 1] - m - m;
          c2s[c2sc] = ((m - c1 - common) * invDx);
          c3s[c3sc] = (common * invDx * invDx);
          c2sc++;
          c3sc++;
      }
    c2sL = c2sc;
