...
** ****************************************************************** */
 ...
 //----------------------------------------------------------------------------------------------------------------------------
 // Description: This file contains the source code of TDConcreteEXP. 
 // TDConcreteEXP is a time-dependent concrete material model that calculates
...
#include "TDConcreteEXP.h"      //Changed by AMK
#include <OPS_Globals.h>
#include <float.h>
#include <Channel.h>
#include <base/Information.h>
#include <iostream>             //Added by AMK for debugging
// #include <elementAPI.h> // cmp //Added by AMK to use methods for parsing data line;
...
OPS_TDConcreteEXP ()
{
    ...
    if (numTDConcrete == 0)
      {
          opserr <<
              "Time-Dependent Concrete Material Model - Written by Adam Knaack, University of Notre Dame, 2012 \n";
          numTDConcrete = 1;
      }
    ...
    if (numArgs == 14)
      {
          ...
          if (OPS_GetIntInput (&numData, &iData) != 0)
            {
                opserr <<
                    "WARNING: invalid uniaxialMaterial TDConcreteEXP tag\n";
                return 0;
            }
          ...
          theMaterial =
              new TDConcreteEXP (iData, dData[0], dData[1], dData[2],
          ...
          if (theMaterial == 0)
            {
                opserr <<
                    "WARNING: could not create uniaxialMaterial of type TDConcreteEXP \n";
                return 0;
            }
...
TDConcreteEXP::TDConcreteEXP (int tag, double _fc, double _ft, double _Ec,
                              double _beta, double _age, double _epsshu,
                              double _epssha, double _tcr, double _epscru,
                              double _sigCr, double _epscra, double _epscrd,
                              double _tcast):
...
UniaxialMaterial *
TDConcreteEXP::getCopy (void)
{
    TDConcreteEXP *theCopy =
        new TDConcreteEXP (this->getTag (), fc, ft, Ec, beta, age, epsshu,
                           epssha, tcr, epscru, sigCr, epscra, epscrd, tcast);

    return theCopy;
}

double
TDConcreteEXP::getInitialTangent (void)
{
...
double
TDConcreteEXP::setCreepStrain (double time, double stress)
{
    double creep;
    double runSum = 0.0;
    ...
    for (int i = 1; i <= count; i++)
      {
          ...
          runSum += PHI_i[i] * DSIG_i[i] / sigCr;       //CONSTANT STRESS
      }
...
double
TDConcreteEXP::setPhi (double time, double tp)
{
    // ACI Equation:
    double tmtp = time - tp;
    ...
    double f3 =
        (1.25 * pow ((tp - tcast), -0.118)) / (1.25 * pow (tcr, -0.118));
    double phi = f2 * f3;
...
double
TDConcreteEXP::setShrink (double time)
{
...
int
TDConcreteEXP::setTrialStrain (double trialStrain, double strainRate)
{
    ...
    if (t - tcast < (2.0 - 0.0001))
      {
          //Assumed that concrete can only carry load
          //once hardened at 2 days following casting
          eps_cr = 0.0;
          eps_sh = 0.0;
          eps_m = 0.0;
          eps_total = 0.0;
      ...
      {                         // Concrete has hardened and is ready to accept load
          ...
          // Calculate creep and mechanical strain, 
          // assuming stress remains constant in a time step:
          if (ops_Creep == 1)
            {
            ...
            {                   //Static Analysis using previously converged TD strains
                eps_cr = epsP_cr;
                eps_sh = epsP_sh;
                eps_m = eps_total - eps_cr - eps_sh;
                sig = setStress (eps_m, e);
...
double
TDConcreteEXP::setStress (double strain, double &stiff)
{
            ...
            {                   // either Tens Load, Tens Unload, or Tens reload
                ...
                if (strain >= ecmax)
                  {             //Tens Load or reload if strain is larger than before
                      ...
                      if (strain >= et0)
                        {       //cracking has occured, set cracking flag
                            crack_flag = 1;
                        }
                  }
                else
                  {             //Tens Unload or Tens Reload
                      if (strain <= et0 && ecmax <= et0)
...
double
TDConcreteEXP::getPHI_i (void)
{
    return phi_i;
}

double
TDConcreteEXP::getStress (void)
{
    return sig;
}

double
TDConcreteEXP::getTangent (void)
{
    return e;
}

double
TDConcreteEXP::getCreep (void)
{
    return eps_cr;
}

double
TDConcreteEXP::getShrink (void)
{
    return eps_sh;
}

double
TDConcreteEXP::getMech (void)
{
    return eps_m;
}

int
TDConcreteEXP::commitState (void)
{
    iter = 0;
    ...
    if (eps_m < 0 && fabs (eps_m) > 0.50 * fabs (fc / Ec))
      {
          ...
          cout << "\n          Strain Compression Limit Exceeded: " << s <<
              "fc'";
      }
...
int
TDConcreteEXP::revertToLastCommit (void)
{
    ecmin = ecminP;;
    dept = deptP;
...
int
TDConcreteEXP::revertToStart (void)
{
    ecminP = 0.0;
    deptP = 0.0;
...
int
TDConcreteEXP::recvSelf (int commitTag, Channel & theChannel,
...
void
TDConcreteEXP::Print (OPS_Stream & s, int flag)
{
    s << "TDConcreteEXP:(strain, stress, tangent) " << eps << " " << sig <<
        " " << e << endln;
}
...
void
TDConcreteEXP::Tens_Envlp (double epsc, double &sigc, double &Ect)
{
    double Ec0 = Ec;
    double eps0 = ft / Ec0;
...
void
TDConcreteEXP::Compr_Envlp (double epsc, double &sigc, double &Ect)
{
//Linear
    Ect = Ec;
    sigc = Ect * epsc;
...
int
TDConcreteEXP::getVariable (const char *varName, Information & theInfo)
{
...
Response *
TDConcreteEXP::setResponse (const char **argv, int argc,
...
int
TDConcreteEXP::getResponse (int responseID, Information & matInfo)
{
