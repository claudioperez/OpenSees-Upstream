...
OPS_KikuchiAikenHDR ()
{
    ...
    return new KikuchiAikenHDR (tag, tp, ddata[0], ddata[1], ddata2[0],
                                ddata2[1], ddata2[2], ddata3[0], ddata3[1]);
}
#endif
...
KikuchiAikenHDR::KikuchiAikenHDR (int tag, int tp, double ar, double hr,
                                  double cg, double ch, double cu, double rs,
                                  double rf):
UniaxialMaterial (tag, MAT_TAG_KikuchiAikenHDR),
Tp (tp),
Ar (ar),
Hr (hr),
Cg (cg),
...
KikuchiAikenHDR::KikuchiAikenHDR ():UniaxialMaterial (0,
                  MAT_TAG_KikuchiAikenHDR)
{
...
KikuchiAikenHDR::setTrialStrain (double strain, double strainRate)
{
    ...
    if (fabs (trialDStrain) < DBL_EPSILON)
      {                         // if ( trialDStrain == 0.0 )
    ...
    if (!trialIfElastic)
      {
          ...
          if (trialDStrain * commitDStrainLastSign < 0)
            {
                ...
                if (trialIdxRev == 0)
                  {             // unload
                      ...
                      //b
                      b = (this->calcB) (fabs (commitStrain), a, c,
                                         (this->
                                          calcHeq) (fabs (commitStrain)) * Ch,
                                         (this->calcU) (fabs (commitStrain)) *
                                         Cu);
                  ...
                  {             //reverse
                      ...
                      //b
                      if (revB[trialIdxRev - 1] == 0)
                        {       // after reversal point with b=0
                            b = 0;
                        }
                      else if (revXEnd[trialIdxRev] * revXBgn[trialIdxRev] >
                               0)
                        {       //consecutive reverse at same sign of "x"
                            b = 0;
                        }
                      else
                        {       // reverse at different sign of "x"
                            b = (this->calcB) (fabs (commitStrain), a, c,
                                               (this->
                                                calcHeq) (fabs (commitStrain))
                                               * Ch,
                                               (this->
                                                calcU) (fabs (commitStrain)) *
                                               Cu);
                        }
                      ...
                      if (trialDStrain > 0)
                        {
                            alpha =
                                (this->compAlpha) (a, revB[trialIdxRev - 1],
                                                   b, c, revXEnd[trialIdxRev],
                                                   revXBgn[trialIdxRev],
                                                   revAlpha[trialIdxRev - 1]);
                        }
                      else
                        {
                            alpha =
          ...
          if (fabs (trialStrain) == trialMaxStrain)
            {                   //if reach skeleton curve
                trialIdxRev = 0;
            }
          else if (trialIdxRev >= 2)
            {                   //if pass through reversal point
                while (trialIdxRev >= 2
                       && (x - revXBgn[trialIdxRev]) * (x -
                                                        revXEnd[trialIdxRev])
                       > 0)
                  {
                      trialIdxRev--;
    ...
    if (trialIdxRev == 0)
      {                         // skeleton curve
          ...
          if (trialDStrain > 0)
            {
                trialQ2 =
                    (this->compQ2Unload) (u, a, revB[trialIdxRev], c, -fm,
                                          -x);
                q2Tan =
                    (this->compQ2UnloadDerivertive) (u, a, revB[trialIdxRev],
                                                     c, geq, x);
            }
          else
            {
                trialQ2 =
                    (this->compQ2Unload) (u, a, revB[trialIdxRev], c, fm, x);
                q2Tan =
                    (this->compQ2UnloadDerivertive) (u, a, revB[trialIdxRev],
                                                     c, geq, -x);
            }
      ...
      {                         //reverse
          ...
          if (trialDStrain > 0)
            {
                trialQ2 =
                    (this->compQ2Masing) (u, a, revB[trialIdxRev], c, fm, x,
                                          revXBgn[trialIdxRev],
                                          revQ2Bgn[trialIdxRev],
                                          revAlpha[trialIdxRev]);
                q2Tan =
                    (this->compQ2MasingDerivertive) (u, a, revB[trialIdxRev],
                                                     c, geq, x,
                                                     revXBgn[trialIdxRev],
                                                     revAlpha[trialIdxRev]);
            }
...
KikuchiAikenHDR::getCopy (void)
{
    ...
    KikuchiAikenHDR *theCopy =
        new KikuchiAikenHDR (this->getTag (), Tp, Ar, Hr,
                             Cg, Ch, Cu, Rs, Rf);
...
double
KikuchiAikenHDR::compQ2Unload (double u, double a, double b, double c,
                               double fm, double x)
{
    return u * fm * (1 - 2 * exp (-a * (1 + x)) +
                     b * (1 + x) * exp (-c * (1 + x)));
}

double
KikuchiAikenHDR::compQ2Masing (double u, double a, double b, double c,
                               double fm, double x1, double x2, double q2i,
                               double alpha)
{
    return q2i + alpha * u * fm * (2 - 2 * exp (-a * (x1 - x2)) +
                                   b * (x1 - x2) * exp (-c * (x1 - x2)));
}


double
KikuchiAikenHDR::compAlpha (double a, double b1, double b2, double c,
                            double x1, double x2, double alpha0)
{
    return alpha0 * (2 - 2 * exp (-a * (x1 - x2)) +
                     b1 * (x1 - x2) * exp (-c * (x1 - x2))) / (2 -
                                                               2 * exp (-a *
                                                                        (x1 -
                                                                         x2))
                                                               + b2 * (x1 -
...
double
KikuchiAikenHDR::compQ2UnloadDerivertive (double u, double a, double b,
                                          double c, double geq, double x)
{
    return u * geq * (2 * a * exp (-a * (1 + x)) + b * exp (-c * (1 + x)) -
                      b * c * (1 + x) * exp (-c * (1 + x)));
}


double
KikuchiAikenHDR::compQ2MasingDerivertive (double u, double a, double b,
                                          double c, double geq, double x1,
                                          double x2, double alpha)
{
    return alpha * u * geq * (2 * a * exp (-a * (x1 - x2)) +
                              b * exp (-c * (x1 - x2)) - b * c * (x1 -
                                                                  x2) *
                              exp (-c * (x1 - x2)));
}
...
double
KikuchiAikenHDR::compABisection (double heq, double u, double min, double max,
                                 double tol, double lim)
{
...
double
KikuchiAikenHDR::calcGeqTp1 (double gm)
{
    if (gm < 1.15)
      {
          return (0.68358 * pow (gm, -0.39964)) * 1e6;
      }
    else
      {
          return (1.10103 - 0.61946 * gm + 0.22047 * gm * gm -
                  0.022191 * gm * gm * gm) * 1e6;
      }
}

double
KikuchiAikenHDR::calcHeqTp1 (double gm)
{
    return 0.23834 - 0.028108 * gm + 0.000087664 * gm * gm;
}

double
KikuchiAikenHDR::calcUTp1 (double gm)
{
    return 0.44236 - 0.062943 * gm - 0.0037571 * gm * gm;
}

double
KikuchiAikenHDR::calcNTp1 (double gm)
{
    if (gm < 1.5)
      {
          return 1.0;
      ...
      {
          return 0.91173 - 0.59184 * gm + 0.43379 * gm * gm;
      }
}

double
KikuchiAikenHDR::calcATp1 (double gm, double heq, double u)
{
    if (gm < 1.4)
      {
          return compABisection (heq, u, 0.0, 20.0, 1e-6, 10.19171);
      }
    else
      {
          return 10.19171;
...
double
KikuchiAikenHDR::calcBTp1 (double gm, double a, double c, double heq,
                           double u)
{
    if (gm < 1.4)
      {
      ...
      {
          return c * c * (M_PI * heq / u - (2 + 2 / a * (exp (-2 * a) - 1)));
      }
}
...
double
KikuchiAikenHDR::calcGeqTp2 (double gm)
{
    if (gm < 1.1)
      {
          return (0.67148 * pow (gm, -0.41264)) * 1e6;
      }
    else
      {
          return (0.99225 - 0.45820 * gm + 0.14236 * gm * gm -
                  0.011238 * gm * gm * gm) * 1e6;
      }
}

double
KikuchiAikenHDR::calcHeqTp2 (double gm)
{
    return 0.21907 - 0.021302 * gm - 0.0013313 * gm * gm;
}

double
KikuchiAikenHDR::calcUTp2 (double gm)
{
    return 0.44226 - 0.091218 * gm + 0.0030028 * gm * gm;
}

double
KikuchiAikenHDR::calcNTp2 (double gm)
{
    if (gm < 1.5)
      {
          return 1.0;
      ...
      {
          return 2.10973 - 1.6166 * gm + 0.58452 * gm * gm;
      }
}

double
KikuchiAikenHDR::calcATp2 (double gm, double heq, double u)
{
    if (gm < 1.3)
      {
          return compABisection (heq, u, 0.0, 20.0, 1e-6, 10.38035);
      }
    else
      {
          return 10.38035;
...
double
KikuchiAikenHDR::calcBTp2 (double gm, double a, double c, double heq,
                           double u)
{
    if (gm < 1.3)
      {
...
double
KikuchiAikenHDR::calcGeqTp3 (double gm)
{
    if (gm < 2.0)
      {
          return (0.38073 * pow (gm, -0.42052)) * 1e6;
      }
    else
      {
          return (0.46499 - 0.12723 * gm + 0.0184823 * gm * gm) * 1e6;
      }
}

double
KikuchiAikenHDR::calcHeqTp3 (double gm)
{
    return 0.22184 + 0.0059574 * gm - 0.0072693 * gm * gm +
        0.0014267 * gm * gm * gm;
}

double
KikuchiAikenHDR::calcUTp3 (double gm)
{
    return 0.42715 - 0.041644 * gm + 0.0070867 * gm * gm -
        0.0019158 * gm * gm * gm;
}

double
KikuchiAikenHDR::calcNTp3 (double gm)
{
    if (gm < 2.0)
      {
          return 1.0;
      ...
      {
          return 0.20924 + 0.39538 * gm;
      }
}

double
KikuchiAikenHDR::calcATp3 (double gm, double heq, double u)
{
    if (gm < 1.5)
      {
          return compABisection (heq, u, 0.0, 20.0, 1e-6, 12.5603);
      }
    else
      {
          return 12.5603;
...
double
KikuchiAikenHDR::calcBTp3 (double gm, double a, double c, double heq,
                           double u)
{
    if (gm < 1.5)
      {
...
double
KikuchiAikenHDR::calcGeqTp4 (double gm)
{
    if (gm < 2.0)
      {
          return (0.40132 * pow (gm, -0.39224)) * 1e6;
      }
    else
      {
          return (0.53123 - 0.18673 * gm + 0.037003 * gm * gm) * 1e6;
      }
}

double
KikuchiAikenHDR::calcHeqTp4 (double gm)
{
    return 0.18932 + 0.010906 * gm - 0.007593 * gm * gm;
}

double
KikuchiAikenHDR::calcUTp4 (double gm)
{
    return 0.36558 - 0.0021387 * gm - 0.013006 * gm * gm;
}

double
KikuchiAikenHDR::calcNTp4 (double gm)
{
    if (gm < 2.0)
      {
          return 1.0;
      ...
      {
          return -0.842 + 0.921 * gm;
      }
}

double
KikuchiAikenHDR::calcATp4 (double gm, double heq, double u)
{
    if (gm < 1.2)
      {
          return compABisection (heq, u, 0.0, 20.0, 1e-6, 15.7279);
      }
    else
      {
          return 15.7279;
...
double
KikuchiAikenHDR::calcBTp4 (double gm, double a, double c, double heq,
                           double u)
{
    if (gm < 1.2)
      {
...
double
KikuchiAikenHDR::calcGeqTp5 (double gm)
{
    if (gm < 2.0)
      {
          return (0.32669 * pow (gm, -0.34317)) * 1e6;
      }
    else
      {
          return (0.50315 - 0.23474 * gm + 0.069144 * gm * gm -
                  0.0065894 * gm * gm * gm) * 1e6;
      }
}

double
KikuchiAikenHDR::calcHeqTp5 (double gm)
{
    return 0.18872 - 0.028833 * gm + 0.0070127 * gm * gm -
        0.00073321 * gm * gm * gm;
}

double
KikuchiAikenHDR::calcUTp5 (double gm)
{
    return 0.40097 - 0.12992 * gm + 0.038851 * gm * gm -
        0.0052934 * gm * gm * gm;
}

double
KikuchiAikenHDR::calcNTp5 (double gm)
{
    if (gm < 2.0)
      {
          return 1.0;
      ...
      {
          return 0.26386 + 0.19679 * gm + 0.085685 * gm * gm;
      }
}

double
KikuchiAikenHDR::calcATp5 (double gm, double heq, double u)
{
    if (gm < 1.5)
      {
          return compABisection (heq, u, 0.0, 20.0, 1e-6, 10.5057);
      }
    else
      {
          return 10.5057;
...
double
KikuchiAikenHDR::calcBTp5 (double gm, double a, double c, double heq,
                           double u)
{
    if (gm < 1.5)
      {
...
double
KikuchiAikenHDR::calcGeqTp6 (double gm)
{
    if (gm < 2.0)
      {
          return (0.33433 * pow (gm, -0.34538)) * 1e6;
      }
    else
      {
          return (0.49000 - 0.21014 * gm + 0.057652 * gm * gm -
                  0.0046475 * gm * gm * gm) * 1e6;
      }
}

double
KikuchiAikenHDR::calcHeqTp6 (double gm)
{
    return 0.18956 - 0.05284 * gm + 0.019156 * gm * gm -
        0.0028118 * gm * gm * gm;
}

double
KikuchiAikenHDR::calcUTp6 (double gm)
{
    return 0.38928 - 0.15678 * gm + 0.05432 * gm * gm -
        0.0077259 * gm * gm * gm;
}

double
KikuchiAikenHDR::calcNTp6 (double gm)
{
    if (gm < 2.0)
      {
          return 1.0;
      ...
      {
          return 1.86072 - 1.135 * gm + 0.35232 * gm * gm;
      }
}

double
KikuchiAikenHDR::calcATp6 (double gm, double heq, double u)
{
    if (gm < 1.5)
      {
          return compABisection (heq, u, 0.0, 20.0, 1e-6, 10.3411);
      }
    else
      {
          return 10.3411;
...
double
KikuchiAikenHDR::calcBTp6 (double gm, double a, double c, double heq,
                           double u)
{
    if (gm < 1.5)
      {
