...
void *OPS_PathIndependentMaterial()
{
    ...
    if (theMat == 0) {
        ...
        opserr << "\nuniaxialMaterial PathIndependent: " << tag[0] <<
            endln;
        return 0;
    }
    ...
    if (mat == 0) {
        opserr <<
            "WARNING: failed to create PathIndependentmaterial material\n";
        return 0;
    }
...
PathIndependentMaterial::PathIndependentMaterial(int tag,
    ...
    (tag, MAT_TAG_PathIndependent), theMaterial(0)
{
    theMaterial = material.getCopy();

    if (theMaterial == 0) {
        opserr <<
            "PathIndependentMaterial::PathIndependentMaterial -- failed to get copy of material\n";
        exit(-1);
    }
}

PathIndependentMaterial::PathIndependentMaterial():UniaxialMaterial(0, MAT_TAG_PathIndependent),
theMaterial(0)
{
...
double PathIndependentMaterial::getStress(void)
{
    return theMaterial->getStress();
}


double PathIndependentMaterial::getTangent(void)
{
    return theMaterial->getTangent();
}

double PathIndependentMaterial::getDampTangent(void)
{
    return theMaterial->getDampTangent();
}

double PathIndependentMaterial::getInitialTangent(void)
{
    return theMaterial->getInitialTangent();
}

double PathIndependentMaterial::getStrain(void)
{
    return theMaterial->getStrain();
}

double PathIndependentMaterial::getStrainRate(void)
{
    return theMaterial->getStrainRate();
}

int PathIndependentMaterial::commitState(void)
{
    return 0;                   // commit nothing, path independent
}

int PathIndependentMaterial::revertToLastCommit(void)
{
    return 0;
}

int PathIndependentMaterial::revertToStart(void)
{
    return theMaterial->revertToStart();
}

UniaxialMaterial *PathIndependentMaterial::getCopy(void)
{
...
int PathIndependentMaterial::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
    ...
    if (res < 0) {
        opserr <<
            "PathIndependentMaterial::sendSelf -- could not send ID\n";
        return res;
    }
    ...
    if (res < 0) {
        opserr <<
            "PathIndependentMaterial::sendSelf -- could not send UniaxialMaterial\n";
        return res;
    }
...
int PathIndependentMaterial::recvSelf(int cTag, Channel & theChannel,
                                      FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0) {
        opserr <<
            "PathIndependentMaterial::recvSelf -- could not receive ID\n";
        return res;
    }
    ...
    if (theMaterial == 0) {
        ...
        if (theMaterial == 0) {
            opserr <<
                " PathIndependentMaterial::recvSelf -- could not get a UniaxialMaterial\n";
            return -1;
        }
    }
    ...
    if (res < 0) {
        opserr <<
            "PathIndependentMaterial::recvSelf -- could not receive UniaxialMaterial\n";
        return res;
    }
...
void PathIndependentMaterial::Print(OPS_Stream & s, int flag)
{
    s << "PathIndependentMaterial tag: " << this->getTag() << endln;
    s << "\tmaterial: " << theMaterial->getTag() << endln;
}

int PathIndependentMaterial::setParameter(const char **argv, int argc,
                                          Parameter & param)
{
    return theMaterial->setParameter(argv, argc, param);
}

int PathIndependentMaterial::updateParameter(int parameterID,
                                             Information & info)
{
    return 0;
}
...
double
    PathIndependentMaterial::getStressSensitivity(int gradIndex,
                                                  bool conditional)
{
...
double PathIndependentMaterial::getInitialTangentSensitivity(int gradIndex)
{
    return theMaterial->getInitialTangentSensitivity(gradIndex);
}

double PathIndependentMaterial::getDampTangentSensitivity(int gradIndex)
{
    return theMaterial->getDampTangentSensitivity(gradIndex);
}

double PathIndependentMaterial::getRhoSensitivity(int gradIndex)
{
    return theMaterial->getRhoSensitivity(gradIndex);
}

int PathIndependentMaterial::commitSensitivity(double strainGradient,
