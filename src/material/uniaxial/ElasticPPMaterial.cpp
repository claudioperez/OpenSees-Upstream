...
void *OPS_ElasticPPMaterial(void)
{
    ...
    if (numArgs < 3 || numArgs > 5) {
        opserr <<
            "Invalid #args,  want: uniaxialMaterial ElasticPP $tag $E $epsP <$epsN $eps0>\n";
        return 0;
    }
    ...
    theMaterial =
        new ElasticPPMaterial(iData[0], dData[0], dData[1], dData[2],
                              dData[3]);
    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type ElasticPP\n";
        return 0;
    }
...
ElasticPPMaterial::ElasticPPMaterial(int tag, double e,
                                     double eyp):UniaxialMaterial(tag,
                                                                  MAT_TAG_ElasticPPMaterial),
ezero(0.0), E(e), ep(0.0), trialStrain(0.0), trialStress(0.0),
...
ElasticPPMaterial::ElasticPPMaterial(int tag, double e, double eyp,
                                     double eyn,
                                     double ez):UniaxialMaterial(tag,
                                                                 MAT_TAG_ElasticPPMaterial),
ezero(ez), E(e), ep(0.0), trialStrain(0.0), trialStress(0.0),
trialTangent(E), commitStrain(0.0), commitStress(0.0), commitTangent(E)
{
    if (eyp < 0) {
        opserr <<
            "ElasticPPMaterial::ElasticPPMaterial() - eyp < 0, setting > 0\n";
        eyp *= -1.;
    }
    if (eyn > 0) {
        opserr <<
...
ElasticPPMaterial::ElasticPPMaterial():UniaxialMaterial(0, MAT_TAG_ElasticPPMaterial),
...
double ElasticPPMaterial::getStrain(void)
{
    return trialStrain;
}

double ElasticPPMaterial::getStress(void)
{
    return trialStress;
}


double ElasticPPMaterial::getTangent(void)
{
    return trialTangent;
}

int ElasticPPMaterial::commitState(void)
{
    double sigtrial;            // trial stress
    double f;                   // yield function
...
int ElasticPPMaterial::revertToLastCommit(void)
{
...
int ElasticPPMaterial::revertToStart(void)
{
...
UniaxialMaterial *ElasticPPMaterial::getCopy(void)
{
...
int ElasticPPMaterial::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
...
int ElasticPPMaterial::recvSelf(int cTag, Channel & theChannel,
...
void ElasticPPMaterial::Print(OPS_Stream & s, int flag)
{
    if (flag == OPS_PRINT_PRINTMODEL_MATERIAL) {
        s << "ElasticPPMaterial tag: " << this->getTag() << endln;
        s << "  E: " << E << endln;
        s << "  ep: " << ep << endln;
        s << "  stress: " << trialStress << " tangent: " << trialTangent <<
            endln;
    }
...
int ElasticPPMaterial::setParameter(const char **argv, int argc,
                                    Parameter & param)
{
...
int ElasticPPMaterial::updateParameter(int parameterID, Information & info)
{
    switch (parameterID) {
    case -1:
        return -1;
