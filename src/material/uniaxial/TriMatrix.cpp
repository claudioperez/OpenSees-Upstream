...
TriDiagonalMatrixF::N ()
{
    return (A != 0 ? Lenght : 0);
                ...
                /// </summary>
double
TriDiagonalMatrixF::GetMat (int row, int col)
{
...
void
TriDiagonalMatrixF::SetMat (int row, int col, double value)
{
    int di = row - col;
...
TriDiagonalMatrixF::Solve (double *d, int dLength)
{
    ...
    for (int i = 1; i < n; i++)
      {
          dPrime[i] =
              (d[i] - dPrime[i - 1] * A[i]) / (B[i] - cPrime[i - 1] * A[i]);
      }
