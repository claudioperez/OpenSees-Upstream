...
OPS_IMKPinching()
{
    if (numIMKPinchingMaterials == 0) {
        ...
        OPS_Error
            ("IMK Model with Pinched Response - Code by A. ELKADY & H. ELJISR (July 2020)\n",
             1);
    }
    ...
    if (OPS_GetIntInput(&numData, iData) != 0) {
        opserr << "WARNING invalid uniaxialMaterial IMKPinching tag" <<
            endln;
        return 0;
    }
    ...
    if (OPS_GetDoubleInput(&numData, dData) != 0) {
        opserr <<
            "Invalid Args want: uniaxialMaterial IMKPinching tag? Ke? ";
        opserr <<
            "Up_pos? Upc_pos? Uu_pos? Fy_pos? FmaxFy_pos? ResF_pos? ";
        opserr <<
            "Up_neg? Upc_neg? Uu_neg? Fy_neg? FmaxFy_neg? ResF_neg? ";
        opserr <<
            "LamdaS? LamdaC? LamdaA? LamdaK? Cs? Cc? Ca? Ck? D_pos? D_neg? kappaF? kappaD? ";
        return 0;
    }
    ...
    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type IMKPinching Material\n";
        return 0;
    }
...
IMKPinching::IMKPinching(int tag, double p_Ke,
                         double p_Up_pos, double p_Upc_pos,
                         double p_Uu_pos, double p_Fy_pos,
                         double p_FmaxFy_pos, double p_ResF_pos,
                         double p_Up_neg, double p_Upc_neg,
                         double p_Uu_neg, double p_Fy_neg,
                         double p_FmaxFy_neg, double p_ResF_neg,
                         double p_LAMBDA_S, double p_LAMBDA_C,
                         double p_LAMBDA_A, double p_LAMBDA_K,
                         double p_c_S, double p_c_C, double p_c_A,
                         double p_c_K, double p_D_pos, double p_D_neg,
                         double p_kappaF,
                         double p_kappaD):UniaxialMaterial(tag, 0),
Ke(p_Ke), Up_pos(p_Up_pos), Upc_pos(p_Upc_pos), Uu_pos(p_Uu_pos),
Fy_pos(p_Fy_pos), FmaxFy_pos(p_FmaxFy_pos), ResF_pos(p_ResF_pos),
Up_neg(p_Up_neg), Upc_neg(p_Upc_neg), Uu_neg(p_Uu_neg), Fy_neg(p_Fy_neg),
FmaxFy_neg(p_FmaxFy_neg), ResF_neg(p_ResF_neg), LAMBDA_S(p_LAMBDA_S),
LAMBDA_C(p_LAMBDA_C), LAMBDA_A(p_LAMBDA_A), LAMBDA_K(p_LAMBDA_K),
c_S(p_c_S), c_C(p_c_C), c_A(p_c_A), c_K(p_c_K), D_pos(p_D_pos),
D_neg(p_D_neg), kappaF(p_kappaF), kappaD(p_kappaD)
{
    this->revertToStart();
}

IMKPinching::IMKPinching():UniaxialMaterial(0, 0), Ke(0),
Up_pos(0), Upc_pos(0), Uu_pos(0), Fy_pos(0), FmaxFy_pos(0), ResF_pos(0),
Up_neg(0), Upc_neg(0), Uu_neg(0), Fy_neg(0), FmaxFy_neg(0), ResF_neg(0),
LAMBDA_S(0), LAMBDA_C(0), LAMBDA_A(0), LAMBDA_K(0), c_S(0), c_C(0),
c_A(0), c_K(0), D_pos(0), D_neg(0), kappaF(0), kappaD(0)
{
    this->revertToStart();
}
...
int
 IMKPinching::setTrialStrain(double strain, double strainRate)
{
    ...
    if (Failure_Flag != 1) {
        ...
        if ((fi_1 > 0) && (du <= 0) && (du * du_i_1 <= 0)) {
            ...
            Reloading_Flag = 0;
            K_check =
                (FLastPeak_pos_j_1 - fi_1) / (ULastPeak_pos_j_1 - ui_1);
            if ((K_check >= 1.05 * Kul_j_1)
                || (K_check <= 0.95 * Kul_j_1)) {       // a tailored criteria to avoid registering last peak points during small unload/reload excursions on the unloading branch 
            ...
            Reloading_Flag = 0;
            K_check =
                (FLastPeak_neg_j_1 - fi_1) / (ULastPeak_neg_j_1 - ui_1);
            if ((K_check >= 1.01 * Kul_j_1)
                || (K_check <= 0.99 * Kul_j_1)) {
        ...
        if (Excursion_Flag == 1) {
            ...
            if ((fi_1 < 0) && (Yield_Flag == 1)) {
                ...
                Uy_pos_j_1 =
                    std::max(Uy_pos_j_1 -
                             Fy_pos_j_1 * betaS * D_pos / Ke,
                             Fres_pos_j_1 / Ke);
                Fy_pos_j_1 =
                    std::max(Fy_pos_j_1 * (1 - betaS * D_pos),
                             Fres_pos_j_1);
                ...
                sPCsp =
                    (Fy_pos_j_1 - Uy_pos_j_1 * Kp_pos_j_1 -
                     Fmax_pos_j_1 +
                     Kpc_pos_j_1 * Umax_pos_j_1) / (Kpc_pos_j_1 -
                                                    Kp_pos_j_1);
                Fmax_pos_j_1 =
                    Fmax_pos_j_1 + (sPCsp - Umax_pos_j_1) * Kpc_pos_j_1;
                Umax_pos_j_1 = sPCsp;
                // Post-capping strength deterioration: Capping point
                sPCpcp =
                    max(Umax_pos_j_1 +
                        betaC * D_pos * (Fmax_pos_j_1 -
                                         Kpc_pos_j_1 * Umax_pos_j_1) /
                        (Kpc_pos_j_1 - Kp_pos_j_1), Uy_pos_j_1);
                Fmax_pos_j_1 =
                    Fmax_pos_j_1 + (sPCpcp - Umax_pos_j_1) * Kp_pos_j_1;
                Umax_pos_j_1 = sPCpcp;
                ...
                if (Upeak_pos_j_1 <= Uy_pos_j_1) {
                    ...
                    Fpeak_pos_j_1 =
                        Kp_pos_j_1 * (Upeak_pos_j_1 - Uy_pos_j_1) +
                        Fy_pos_j_1;
                    // Target peak deformation in post-capping branch of the updated backbone
                } else {
                    Fpeak_pos_j_1 =
                        max(Kpc_pos_j_1 *
                            (Upeak_pos_j_1 - Umax_pos_j_1) +
                            Fmax_pos_j_1, Fres_pos_j_1);
                }
            } else if ((fi_1 >= 0) && (Yield_Flag == 1)) {
                // Update Negative Backbone and Target Peak Point
                // Basic strength deterioration: Yield point
                Uy_neg_j_1 =
                    min(Uy_neg_j_1 - Fy_neg_j_1 * betaS * D_neg / Ke,
                        Fres_neg_j_1 / Ke);
                Fy_neg_j_1 =
                    min(Fy_neg_j_1 * (1 - betaS * D_neg), Fres_neg_j_1);
                // Basic strength deterioration: Post-yield stiffness
                ...
                // Basic strength deterioration: Capping point
                sPCsn =
                    (Fy_neg_j_1 - Uy_neg_j_1 * Kp_neg_j_1 -
                     Fmax_neg_j_1 +
                     Kpc_neg_j_1 * Umax_neg_j_1) / (Kpc_neg_j_1 -
                                                    Kp_neg_j_1);
                Fmax_neg_j_1 =
                    Fmax_neg_j_1 + (sPCsn - Umax_neg_j_1) * Kpc_neg_j_1;
                Umax_neg_j_1 = sPCsn;
                // Post-capping strength deterioration: Capping point
                sPCpcn =
                    min(Umax_neg_j_1 +
                        betaC * D_neg * (Fmax_neg_j_1 -
                                         Kpc_neg_j_1 * Umax_neg_j_1) /
                        (Kpc_neg_j_1 - Kp_neg_j_1), Uy_neg_j_1);
                Fmax_neg_j_1 =
                    Fmax_neg_j_1 + (sPCpcn - Umax_neg_j_1) * Kp_neg_j_1;
                Umax_neg_j_1 = sPCpcn;
                // Accelerated reloading stiffness deterioration: Target peak deformation point
                ...
                if (Upeak_neg_j_1 >= Uy_neg_j_1) {
                    ...
                    Fpeak_neg_j_1 =
                        Kp_neg_j_1 * (Upeak_neg_j_1 - Uy_neg_j_1) +
                        Fy_neg_j_1;
                    // Target peak deformation in post-capping branch of the updated backbone
                } else {
                    Fpeak_neg_j_1 =
                        min(Kpc_neg_j_1 *
                            (Upeak_neg_j_1 - Umax_neg_j_1) +
                            Fmax_neg_j_1, Fres_neg_j_1);
                }
            }
        }
        // Update Deformation at Residual Points
        Ures_pos_j_1 =
            (Fres_pos_j_1 - Fmax_pos_j_1 +
             Kpc_pos_j_1 * Umax_pos_j_1) / Kpc_pos_j_1;
        Ures_neg_j_1 =
            (Fres_neg_j_1 - Fmax_neg_j_1 +
             Kpc_neg_j_1 * Umax_neg_j_1) / Kpc_neg_j_1;
        ...
        if (Excursion_Flag == 1) {
            if (du >= 0) {
                Krel_LastPeak =
                    FLastPeak_pos_j_1 / (ULastPeak_pos_j_1 - u0);
                Krel_GlobalPeak = Fpeak_pos_j_1 / (Upeak_pos_j_1 - u0);
            } else {
                Krel_LastPeak =
                    FLastPeak_neg_j_1 / (ULastPeak_neg_j_1 - u0);
            ...
            if ((du >= 0) && (FLastPeak_pos_j_1 >= Fpeak_pos_j_1)) {
                ...
                TargetPeak_Flag = 0;
                       ...
                       (abs
                        ((ULastPeak_pos_j_1 -
                          Upeak_pos_j_1) / Upeak_pos_j_1) < 0.05)
                       && (abs(Krel_LastPeak) <=
                           1.05 * abs(Krel_GlobalPeak))) {
                TargetPeak_Flag = 0;
            } else if ((du <= 0)
                       &&
                       (abs
                        ((ULastPeak_neg_j_1 -
                          Upeak_neg_j_1) / Upeak_neg_j_1) < 0.05)
                       && (abs(Krel_LastPeak) <=
                           1.05 * abs(Krel_GlobalPeak))) {
                TargetPeak_Flag = 0;
            } else {
        ...
        if (Excursion_Flag == 1) {
            if (du > 0) {
                ...
                Ubp = (1 - kappaD) * Upl;
                Fbp =
                    kappaF * Fpeak_pos_j_1 * abs((Ubp - u0) /
                                                 (Upeak_pos_j_1 - u0));
            } else if (du < 0) {
                Upl = Upeak_neg_j_1 - (Fpeak_neg_j_1 / Kul_j_1);
                Ubp = (1 - kappaD) * Upl;
                Fbp =
                    kappaF * Fpeak_neg_j_1 * abs((Ubp - u0) /
                                                 (Upeak_neg_j_1 - u0));
            }
        }
        ...
        if (fi_1 + du * Kul_j_1 >= 0) {
            ...
            if ((ui >= 0) && (Upeak_pos_j_1 <= Uy_pos_j_1)
                && (Yield_Flag == 0)) {
                if (ui >= Uy_pos_j_1) {
                    df = Ke * (Uy_pos_j_1 - ui_1) + Kp_pos_j_1 * (ui -
                                                                  Uy_pos_j_1);
                } else {
                    df = du * Ke;
                }
                ...
                // CASE 3: WHEN RELOADING BUT BETWEEN LAST CYCLE PEAK POINT AND GLOBAL PEAK POINT
                       ...
                       && (ui <= Upeak_pos_j_1)
                       && (FLastPeak_pos_j_1 <= Fpeak_pos_j_1)) {
                if (TargetPeak_Flag == 1) {
                    if ((FLastPeak_pos_j_1 <= Fbp) && (ui <= Ubp)) {
                        Krel_j_1 =
                            (Fbp - FLastPeak_pos_j_1) / (Ubp -
                                                         ULastPeak_pos_j_1);
                    } else if ((FLastPeak_pos_j_1 <= Fbp)
                               && (ui >= Ubp)) {
                        Krel_j_1 =
                            (Fpeak_pos_j_1 - Fbp) / (Upeak_pos_j_1 - Ubp);
                    } else {
                        Krel_j_1 =
                            (Fpeak_pos_j_1 -
                             FLastPeak_pos_j_1) / (Upeak_pos_j_1 -
                                                   ULastPeak_pos_j_1);
                    }
                    df = du * Krel_j_1;
                    ...
                    if (ui_1 <= ULastPeak_pos_j_1) {
                        df = Kul_j_1 * (ULastPeak_pos_j_1 - ui_1) +
                            Krel_j_1 * (ui - ULastPeak_pos_j_1);
                    } else if (ui <= Ubp) {
                        df = du * Krel_j_1;
                    } else if (ui >= Ubp) {
                        Krel_j_1 =
                ...
                if ((TargetPeak_Flag == 0) && (ui >= Ubp)) {
                    Krel_j_1 =
                        (Fpeak_pos_j_1 - fi_1) / (Upeak_pos_j_1 - ui_1);
                } else if ((TargetPeak_Flag == 0) && (ui <= Ubp)) {
                    Krel_j_1 = (Fbp) / (Ubp - u0);
                } else if ((TargetPeak_Flag == 1)
                           && (ui <= ULastPeak_pos_j_1)) {
                    Krel_j_1 =
                        (FLastPeak_pos_j_1) / (ULastPeak_pos_j_1 - u0);
                } else if ((TargetPeak_Flag == 1)
                           && (ui >= ULastPeak_pos_j_1)) {
                ...
                if ((ui_1 <= Umax_pos_j_1) && (ui >= Umax_pos_j_1)) {
                    df = Kp_pos_j_1 * (Umax_pos_j_1 - ui_1) +
                        Kpc_pos_j_1 * (ui - Umax_pos_j_1);
                } else {
                    df = du * Kpc_pos_j_1;
                }
                //cout << "  Case = 7+" << endln;
        ...
        if (fi_1 + du * Kul_j_1 <= 0) {
            ...
            // CASE 0: At THE ELASTIC SLOPE
            if ((ui <= 0) && (Upeak_neg_j_1 >= Uy_neg_j_1)
                && (Yield_Flag == 0)) {
                if (ui <= Uy_neg_j_1) {
                    df = Ke * (Uy_neg_j_1 - ui_1) + Kp_neg_j_1 * (ui -
                                                                  Uy_neg_j_1);
                } else {
                    df = du * Ke;
                }
                ...
                // CASE 3: WHEN RELOADING BUT BETWEEN LAST CYCLE PEAK POINT AND GLOBAL PEAK POINT
                       ...
                       && (ui >= Upeak_neg_j_1)
                       && (FLastPeak_neg_j_1 >= Fpeak_neg_j_1)) {
                if (TargetPeak_Flag == 1) {
                    if ((FLastPeak_neg_j_1 >= Fbp) && (ui <= Ubp)) {
                        Krel_j_1 =
                            (Fbp - FLastPeak_neg_j_1) / (Ubp -
                                                         ULastPeak_neg_j_1);
                    } else if ((FLastPeak_neg_j_1 >= Fbp)
                               && (ui <= Ubp)) {
                        Krel_j_1 =
                            (Fpeak_neg_j_1 - Fbp) / (Upeak_neg_j_1 - Ubp);
                    } else {
                        Krel_j_1 =
                            (Fpeak_neg_j_1 -
                             FLastPeak_neg_j_1) / (Upeak_neg_j_1 -
                                                   ULastPeak_neg_j_1);
                    }
                    df = du * Krel_j_1;
                } else if (TargetPeak_Flag == 0) {
                    ...
                    if (ui_1 >= ULastPeak_neg_j_1) {
                        df = Kul_j_1 * (ULastPeak_neg_j_1 - ui_1) +
                            Krel_j_1 * (ui - ULastPeak_neg_j_1);
                    } else if (ui >= Ubp) {
                        df = du * Krel_j_1;
                    } else if (ui <= Ubp) {
                        Krel_j_1 =
                ...
                if ((TargetPeak_Flag == 0) && (ui <= Ubp)) {
                    Krel_j_1 =
                        (Fpeak_neg_j_1 - fi_1) / (Upeak_neg_j_1 - ui_1);
                } else if ((TargetPeak_Flag == 0) && (ui >= Ubp)) {
                    Krel_j_1 = (Fbp) / (Ubp - u0);
                } else if ((TargetPeak_Flag == 1)
                           && (ui >= ULastPeak_neg_j_1)) {
                    Krel_j_1 =
                        (FLastPeak_neg_j_1) / (ULastPeak_neg_j_1 - u0);
                } else if ((TargetPeak_Flag == 1)
                           && (ui <= ULastPeak_neg_j_1)) {
                ...
                if ((ui_1 >= Umax_neg_j_1) && (ui <= Umax_neg_j_1)) {
                    df = Kp_neg_j_1 * (Umax_neg_j_1 - ui_1) +
                        Kpc_neg_j_1 * (ui - Umax_neg_j_1);
                } else {
                    df = du * Kpc_neg_j_1;
                }
                //cout << "  Case = 7-" << endln;
...
int IMKPinching::revertToStart(void)
{
    ...
    Ures_pos_j_1 =
        (Fres_pos_j_1 - Fmax_pos_j_1) / Kpc_pos_j_1 + Umax_pos_j_1;
    Ures_neg_j_1 =
        (Fres_neg_j_1 - Fmax_neg_j_1) / Kpc_neg_j_1 + Umax_neg_j_1;
...
UniaxialMaterial *IMKPinching::getCopy(void)
{
    IMKPinching *theCopy = new IMKPinching(this->getTag(), Ke,
                                           Uy_pos, Umax_pos, Uu_pos,
                                           Fy_pos,
                                           FmaxFy_pos, ResF_pos,
                                           Uy_neg, Umax_neg, Uu_neg,
                                           Fy_neg,
                                           FmaxFy_neg, ResF_neg,
                                           LAMBDA_S, LAMBDA_C, LAMBDA_A,
                                           LAMBDA_K, c_S, c_C, c_A, c_K,
                                           D_pos, D_neg, kappaF, kappaD);
...
int IMKPinching::recvSelf(int cTag, Channel & theChannel,
