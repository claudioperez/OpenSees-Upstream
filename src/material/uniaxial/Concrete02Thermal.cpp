...
void *OPS_Concrete02Thermal()
{
    ...
    if (OPS_GetIntInput(&numData, iData) != 0) {
        opserr << "WARNING invalid uniaxialMaterial Concrete02Thermal tag"
            << endln;
        return 0;
    }
    ...
    if (numData != 7) {
        opserr <<
            "Invalid #args, want: uniaxialMaterial Concrete02Thermal " <<
    ...
    theMaterial =
        new Concrete02Thermal(iData[0], dData[0], dData[1], dData[2],
                              dData[3], dData[4], dData[5], dData[6]);

    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type Concrete02Thermal Material\n";
        return 0;
    }
...
Concrete02Thermal::Concrete02Thermal(int tag, double _fc, double _epsc0,
                                     double _fcu, double _epscu,
                                     double _rat, double _ft,
                                     double _Ets):UniaxialMaterial(tag,
                                                                   MAT_TAG_Concrete02Thermal),
...
Concrete02Thermal::Concrete02Thermal(void):UniaxialMaterial(0,
                                                            MAT_TAG_Concrete02Thermal)
{
...
UniaxialMaterial *Concrete02Thermal::getCopy(void)
{
    Concrete02Thermal *theCopy =
        new Concrete02Thermal(this->getTag(), fc, epsc0, fcu, epscu, rat,
                              ft, Ets);

    return theCopy;
}

double Concrete02Thermal::getInitialTangent(void)
{
    return 2.0 * fc / epsc0;
}

int Concrete02Thermal::setTrialStrain(double trialStrain,
...
double Concrete02Thermal::getStrain(void)
{
    return eps;
}

double Concrete02Thermal::getStress(void)
{
    return sig;
}

double Concrete02Thermal::getTangent(void)
{
    return e;
}

double Concrete02Thermal::getThermalElongation(void)    //***JZ
{
    return ThermalElongation;
}

double Concrete02Thermal::getElongTangent(double TempT, double &ET, double &Elong, double TempTmax)     //PK add to include max temp
{
...
//jz assign a miner to the valuables
    ...
    if (Temp <= 1) {
        ...
        ThermalElongation =
            -1.8e-4 + 9e-6 * (Temp + 20) + 2.3e-11 * (Temp + 20) * (Temp +
                                                                    20) *
            (Temp + 20);
    } else if (Temp <= 1180) {
...
//opserr << "cooling " << Temp << " " << TempP << endln;
        ...
        if (Tempmax < 0) {
            ...
            epsc0max =
                -(0.0025 + (0.004 - 0.0025) * (Tempmax - 0) / (80 - 0));
            epscumax =
                -(0.0200 + (0.0225 - 0.0200) * (Tempmax - 0) / (80 - 0));
        } else if (Tempmax <= 180) {
            epsc0max =
                -(0.0040 + (0.0055 - 0.0040) * (Tempmax - 80) / 100);
            epscumax =
                -(0.0225 + (0.0225 - 0.0200) * (Tempmax - 80) / 100);
        } else if (Tempmax <= 280) {
            epsc0max =
                -(0.0055 + (0.0070 - 0.0055) * (Tempmax - 180) / 100);
            epscumax = -(0.0250 + 0.0025 * (Tempmax - 180) / 100);
        } else if (Tempmax <= 380) {
            epsc0max =
                -(0.0070 + (0.0100 - 0.0070) * (Tempmax - 280) / 100);
            epscumax = -(0.0275 + 0.0025 * (Tempmax - 280) / 100);
        } else if (Tempmax <= 480) {
            epsc0max =
                -(0.0100 + (0.0150 - 0.0100) * (Tempmax - 380) / 100);
            epscumax = -(0.03 + 0.0025 * (Tempmax - 380) / 100);
        } else if (Tempmax <= 580) {
            epsc0max =
                -(0.0150 + (0.0250 - 0.0150) * (Tempmax - 480) / 100);
...
int Concrete02Thermal::commitState(void)
{
    ecminP = ecmin;
    deptP = dept;
...
int Concrete02Thermal::revertToLastCommit(void)
{
    ecmin = ecminP;;
    dept = deptP;
...
int Concrete02Thermal::revertToStart(void)
{
    ecminP = 0.0;
    deptP = 0.0;
...
int Concrete02Thermal::sendSelf(int commitTag, Channel & theChannel)
{
...
int Concrete02Thermal::recvSelf(int commitTag, Channel & theChannel,
...
void Concrete02Thermal::Print(OPS_Stream & s, int flag)
{
    s << "Concrete02Thermal:(strain, stress, tangent) " << eps << " " <<
        sig << " " << e << endln;
}


void Concrete02Thermal::Tens_Envlp(double epsc, double &sigc, double &Ect)
{
/*-----------------------------------------------------------------------
...
void Concrete02Thermal::Compr_Envlp(double epsc, double &sigc, double &Ect)
{
/*-----------------------------------------------------------------------
...
int Concrete02Thermal::getVariable(const char *varName,
                                   Information & theInfo)
{
...
int Concrete02Thermal::setTrialStrain(double strain, double strainRate)
{
    opserr <<
        "Concrete02Thermal::setTrialStrain(double strain, double strainRate) - should never be called\n";
    return -1;
}
