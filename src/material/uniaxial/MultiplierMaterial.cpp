...
void *OPS_MultiplierMaterial(void)
{
    ...
    if (argc < 2) {
        opserr <<
            "WARNING insufficient args, uniaxialMaterial Multiplier $tag $otherTag $multiplier"
            << endln;
        return 0;
    }
    ...
    if (OPS_GetIntInput(&numData, iData) < 0) {
        opserr <<
            "WARNING invalid uniaxialMaterial Multiplier $tag $otherTag $multiplier"
            << endln;
        return 0;
    }
    ...
    if (theOtherMaterial == 0) {
        opserr <<
            "WARNING invalid otherTag uniaxialMaterial Multiplier tag: " <<
            iData[0] << endln;
        return 0;
    }
    ...
    theMaterial =
        new MultiplierMaterial(iData[0], *theOtherMaterial, multiplier);

    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type MultiplierMaterial\n";
        return 0;
    }
...
MultiplierMaterial::MultiplierMaterial(int tag,
                                       UniaxialMaterial & material,
                                       double mult):UniaxialMaterial(tag,
                                                                     MAT_TAG_Multiplier),
theMaterial(0), multiplier(mult), parameterID(0)
{
    theMaterial = material.getCopy();

    if (theMaterial == 0) {
        opserr <<
            "MultiplierMaterial::MultiplierMaterial -- failed to get copy of material\n";
        exit(-1);
    }
}

MultiplierMaterial::MultiplierMaterial():UniaxialMaterial(0, MAT_TAG_Multiplier), theMaterial(0), multiplier(1.0),
parameterID(0)
{
...
int MultiplierMaterial::setTrialStrain(double strain, double temp,
                                       double strainRate)
{
...
double MultiplierMaterial::getStress(void)
{
    return multiplier * theMaterial->getStress();
}

double MultiplierMaterial::getTangent(void)
{
    return multiplier * theMaterial->getTangent();
}

double MultiplierMaterial::getDampTangent(void)
{
...
double MultiplierMaterial::getStrain(void)
{
    return theMaterial->getStrain();
}

double MultiplierMaterial::getStrainRate(void)
{
    return theMaterial->getStrainRate();
}

int MultiplierMaterial::commitState(void)
{
    return theMaterial->commitState();
}

int MultiplierMaterial::revertToLastCommit(void)
{
    return theMaterial->revertToLastCommit();
}

int MultiplierMaterial::revertToStart(void)
{
    return theMaterial->revertToStart();
}

UniaxialMaterial *MultiplierMaterial::getCopy(void)
{
...
int MultiplierMaterial::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (theChannel.sendID(dbTag, cTag, dataID) < 0) {
        opserr <<
            "MultiplierMaterial::sendSelf() - failed to send the ID\n";
        return -1;
    }
    ...
    if (theChannel.sendVector(dbTag, cTag, dataVec) < 0) {
        opserr <<
            "MultiplierMaterial::sendSelf() - failed to send the Vector\n";
        return -2;
    }

    if (theMaterial->sendSelf(cTag, theChannel) < 0) {
        opserr <<
            "MultiplierMaterial::sendSelf() - failed to send the Material\n";
        return -3;
    }
...
int MultiplierMaterial::recvSelf(int cTag, Channel & theChannel,
                                 FEM_ObjectBroker & theBroker)
{
    ...
    if (theChannel.recvID(dbTag, cTag, dataID) < 0) {
        opserr <<
            "MultiplierMaterial::recvSelf() - failed to get the ID\n";
        return -1;
    }
    ...
    if (theMaterial == 0) {
        ...
        if (theMaterial == 0) {
            opserr <<
                "MultiplierMaterial::recvSelf() - failed to create Material with classTag "
                << dataID(0) << endln;
            return -2;
        }
    }
    ...
    if (theChannel.recvVector(dbTag, cTag, dataVec) < 0) {
        opserr <<
            "MultiplierMaterial::recvSelf() - failed to get the Vector\n";
        return -3;
    }
    ...
    if (theMaterial->recvSelf(cTag, theChannel, theBroker) < 0) {
        opserr <<
            "MultiplierMaterial::recvSelf() - failed to get the Material\n";
        return -4;
    }
    return 0;
}

void MultiplierMaterial::Print(OPS_Stream & s, int flag)
{
...
int MultiplierMaterial::setParameter(const char **argv, int argc,
                                     Parameter & param)
{
...
int MultiplierMaterial::updateParameter(int parameterID,
                                        Information & info)
{
    switch (parameterID) {
    case -1:
...
int MultiplierMaterial::activateParameter(int paramID)
{
    parameterID = paramID;
...
double
    MultiplierMaterial::getStressSensitivity(int gradIndex,
                                             bool conditional)
{
    ...
    if (parameterID == 1)
        ...
        return multiplier * theMaterial->getStressSensitivity(gradIndex,
                                                              conditional);
}
...
double MultiplierMaterial::getInitialTangentSensitivity(int gradIndex)
{
    if (parameterID == 1)
        return theMaterial->getInitialTangent();
    else
        return multiplier *
            theMaterial->getInitialTangentSensitivity(gradIndex);
}

double MultiplierMaterial::getDampTangentSensitivity(int gradIndex)
{
    if (parameterID == 1)
...
double MultiplierMaterial::getRhoSensitivity(int gradIndex)
{
    return theMaterial->getRhoSensitivity(gradIndex);
}

int MultiplierMaterial::commitSensitivity(double strainGradient,
