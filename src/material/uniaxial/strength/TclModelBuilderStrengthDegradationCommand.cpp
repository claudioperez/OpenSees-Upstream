                                           ...
                                           Domain * theDomain)
{
    ...
    if (argc < 2)
      {
          opserr <<
              "WARNING insufficient number of strengthDegradation arguments\n";
          opserr <<
              "Want: strengthDegradation type? tag? <specific strengthDegradation args>"
              << endln;
          return TCL_ERROR;
      }
    ...
    if (strcmp (argv[1], "Section") == 0)
      {
          if (argc < 7)
            {
                ...
                opserr <<
                    "Want: strengthDegradation Section tag? code e1? V2? e2? <-yield ey?>"
                    << endln;
                return TCL_ERROR;
            }
          ...
          if (Tcl_GetInt (interp, argv[2], &tag) != TCL_OK)
            {
                opserr << "WARNING invalid strengthDegradation Section tag" <<
                    endln;
                return TCL_ERROR;
            }
                ...
                if (Tcl_GetDouble (interp, argv[8], &ey) != TCL_OK)
                  {
                      ...
                      opserr << "strengthDegradation Section: " << tag <<
                          endln;
                      return TCL_ERROR;
                  }
                isDuctility = true;
          ...
          if (isDuctility)
              theState =
                  new SectionStrengthDegradation (tag, ey, e1, V2, e2, code);
          else
              theState =
                  new SectionStrengthDegradation (tag, e1, V2, e2, code);
      }
      ...
      {
          opserr << "WARNING unknown type of strengthDegradation: " <<
              argv[1];
          opserr <<
    ...
    if (theState == 0)
      {
          opserr <<
              "WARNING ran out of memory creating strengthDegradation\n";
          opserr << argv[1] << endln;
          return TCL_ERROR;
      }
    ...
    if (OPS_addStrengthDegradation (theState) == false)
      {
          opserr <<
              "WARNING could not add strengthDegradation to the domain\n";
