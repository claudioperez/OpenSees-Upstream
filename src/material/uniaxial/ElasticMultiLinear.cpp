...
void *OPS_ElasticMultiLinear()
{
    ...
    theMaterial =
        new ElasticMultiLinear(tag[0], strainPts, stressPts, eta);
...
ElasticMultiLinear::ElasticMultiLinear(int tag,
                                       ...
                                       const Vector & stressPts,
                                       double et):UniaxialMaterial(tag,
                                                                   MAT_TAG_ElasticMultiLinear),
strainPoints(strainPts), stressPoints(stressPts), eta(et), trialID(0),
trialIDmin(0), trialIDmax(0), numDataPoints(2), initTangent(0.0),
trialStrain(0.0), trialStrainRate(0.0), trialStress(0.0), trialTangent(0.0)
{
...
int ElasticMultiLinear::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (res < 0)
        opserr <<
            "ElasticMultiLinear::sendSelf() - failed to send data.\n";
                                 ...
                                 FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0)
        opserr <<
            "ElasticMultiLinear::recvSelf() - failed to recv data.\n";
    else {
