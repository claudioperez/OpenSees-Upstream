...
OPS_ElasticMultiLinear ()
{
    ...
    if (OPS_GetIntInput (&numData, tag) != 0)
      {
          opserr <<
              "WARNING invalid uniaxialMaterial ElasticMultiLinear tag\n";
          return 0;
      }
          ...
          if (OPS_GetDoubleInput (&numData, &eta) != 0)
            {
                ...
                opserr << "uniaxialMaterial ElasticMultiLinear: " << tag[0] <<
                    endln;
                return 0;
            }
          argc--;
...
ElasticMultiLinear::ElasticMultiLinear (int tag,
                                        const Vector & strainPts,
                                        const Vector & stressPts, double et):
UniaxialMaterial (tag, MAT_TAG_ElasticMultiLinear),
strainPoints (strainPts),
...
ElasticMultiLinear::ElasticMultiLinear ():UniaxialMaterial (0, MAT_TAG_ElasticMultiLinear),
...
int
ElasticMultiLinear::setTrialStrain (double strain, double strainRate)
{
    trialStrain = strain;
    ...
    trialStress =
        sig1 + trialTangent * (trialStrain - eps1) + eta * trialStrainRate;
...
int
ElasticMultiLinear::commitState ()
{
    return 0;
}
...
int
ElasticMultiLinear::revertToLastCommit ()
{
    return 0;
}
...
int
ElasticMultiLinear::revertToStart ()
{
    trialID = 0;
    trialStrain = 0.0;
    trialStrainRate = 0.0;
...
int
ElasticMultiLinear::sendSelf (int cTag, Channel & theChannel)
{
    int res = 0;
...
int
ElasticMultiLinear::recvSelf (int cTag, Channel & theChannel,
                              FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0)
    ...
    else
      {
          ...
          res +=
              theChannel.recvVector (this->getDbTag (), cTag, strainPoints);
          res +=
              theChannel.recvVector (this->getDbTag (), cTag, stressPoints);
          if (res < 0)
              opserr <<
                  "ElasticMultiLinear::recvSelf() - failed to recv arrays.\n";
      }
