...
OPS_HystereticPoly()
{
    ...
    if (numArgs < 6) {
        ...
        opserr <<
            "Want: uniaxialMaterial HystereticPoly tag? ka? kb? a? b1? b2? <tol?>"
            << endln;
        return 0;
    }
    ...
    if (OPS_GetDoubleInput(&numData, dData) != 0) {
        opserr << "Invalid data for uniaxial HystereticPoly " << iData[0]
            << endln;
        return 0;
    }
    if (dData[0] <= 0.0) {
        opserr << "uniaxialMaterial HystereticPoly ka must be positive" <<
            endln;
        return 0;
    }
    if (dData[1] >= dData[0]) {
        opserr << "uniaxialMaterial HystereticPoly kb must be < ka" <<
            endln;
        return 0;
    }
    if (dData[2] <= 0.0) {
        opserr <<
            "uniaxialMaterial HystereticPoly a must be positive and <> 1"
            << endln;
        return 0;
    }
    if (dData[2] == 1.0) {
    ...
    theMaterial =
        new HystereticPoly(iData[0], dData[0], dData[1], dData[2],
...
HystereticPoly::HystereticPoly(int tag, double K1, double K2, double A,
                               double C, double D,
                               double TOL):UniaxialMaterial(tag,
                                                            MAT_TAG_HystereticPoly),
...
k1(0.0), k2(0.0), a(0.0), c(0.0), d(0.0),
tol(0.0)
{
...
int
 HystereticPoly::setTrialStrain(double strain, double strainRate)
{
    ...
    uj = Cstrain + st * Uo -
        st * pow(st * a1 / k12 *
                 (Cstress - c * pow(Cstrain, 3) - d * pow(Cstrain, 5) -
                  k2 * Cstrain - st * Fbar + k12 * am1 * Uoa / st), am1);


    Tstress =
        c * pow(Tstrain, 3) + d * pow(Tstrain,
                                      5) + k2 * Tstrain + k12 * (pow(1 +
                                                                     st *
                                                                     Tstrain
                                                                     -
                                                                     st *
    ...
    Ttangent =
        3 * c * pow(Tstrain, 2) + 5 * d * pow(Tstrain,
                                              4) + k2 + k12 * pow(1 +
                                                                  st *
                                                                  Tstrain -
                                                                  st * uj +
...
{
    HystereticPoly *theCopy =
        new HystereticPoly(this->getTag(), k1, k2, a, c, d, tol);
...
// AddingSensitivity:BEGIN ///////////////////////////////////
int HystereticPoly::setParameter(const char **argv, int argc,
                                 Parameter & param)
{
...
int HystereticPoly::updateParameter(int parameterID, Information & info)
{
    switch (parameterID) {
    case -1:
        return -1;
...
int HystereticPoly::activateParameter(int passedParameterID)
{
...
double HystereticPoly::getStrainSensitivity(int gradNumber)
{
    if (SHVs == 0) {
        opserr << "warning:HystereticPoly::getStrainsSensitivity, SHVs =0 "
            << endln;
        return 0.0;
    }
    ...
    HystereticPoly::getStressSensitivity(int gradIndex, bool conditional)
{
    ...
    double Delta2 = Cstress - c * pow(Cstrain, 3) - d * pow(Cstrain,
                                                            5) -
        k2 * Cstrain - st * Fbar + Delta3;
    uj = Cstrain + st * (1 + 2 * uo) - st * pow(Delta1 * Delta2,
                                                1 / (1 - a));
    double Delta4 =
        pow(1 + st * Tstrain - st * uj + 2 * uo, 1 - a) / st / (1 - a);
    ...
    double Duo =
        0.5 * uo / a * (tol / (k1 - k2) *
                        ((Dk1 - Dk2) / tol - (k1 - k2) / pow(tol,
                                                             2) * Dtol) -
                        Da / a * log((k1 - k2) / tol));
    double DFbar =
        0.5 * (Dk1 - Dk2) * (pow(1 + 2 * uo, 1 - a) - 1) / (1 - a) +
        0.5 * (k1 - k2) * (pow(1 + 2 * uo,
                               1 - a) / (1 - a) * (2 * Duo * (1 - a) / (1 +
                                                                        2 *
                                                                        uo)
                                                   - Da * log(1 +
                                                              2 * uo)) +
                           Da * (pow(1 + 2 * uo, 1 - a) - 1) / pow(1 - a,
                                                                   2));

    double DD1 =
        (st * Da * (k1 - k2) + st * (1 - a) * (Dk1 - Dk2)) / pow(k1 - k2,
                                                                 2);
    double DD3 = (Dk1 - Dk2) * pow(1 + 2 * uo,
                                   1 - a) / st / (1 - a) + (k1 -
                                                            k2) * pow(1 +
                                                                      2 *
                                                                      uo,
    ...
    double DD2 = Dfc - Dc * pow(Cstrain, 3) - Dd * pow(Cstrain,
                                                       5) - Dk2 * Cstrain -
        (3 * c * pow(Cstrain, 2) + 5 * d * pow(Cstrain, 4) + k2) * Duc -
        st * DFbar + DD3;

    double Duj = Duc + 2 * st * Duo - st * pow(Delta1 * Delta2,
                                               1 / (1 -
                                                    a)) * (Da *
                                                           log(Delta1 *
                                                               Delta2) /
    ...
    double DD4 = pow(1 + st * Tstrain - st * uj + 2 * uo, 1 - a) / pow(st,
                                                                       2) /
        pow(1 - a,
            2) * Da + Delta4 * (1 - a) * (st * Dut - st * Duj +
                                          2 * Duo) / (1 + st * Tstrain -
                                                      st * uj + 2 * uo);
    double DD5 =
        (pow(1 + 2 * uo, 1 - a)) / pow(1 - a, 2) * Da + pow(1 + 2 * uo,
                                                            1 - a) / (1 -
                                                                      a) *
        (2 * Duo * (1 - a) / (1 + 2 * uo) - Da * log(1 + 2 * uo));


    gradient =
        Dc * pow(Tstrain, 3) + Dd * pow(Tstrain,
                                        5) + Dk2 * Tstrain +
        (3 * c * pow(Tstrain, 2) + 5 * d * pow(Tstrain, 4) + k2) * Dut +
        st * DFbar + (Dk1 - Dk2) * (Delta4 - Delta5) + (k1 - k2) * (DD4 -
                                                                    DD5);
 ...
 HystereticPoly::getInitialTangentSensitivity(int gradIndex)
{
    ...
    gradient =
        (st * (Dk1 - Dk2) * (1 - a) - st * Da * (k1 - k2)) * pow(1 -
                                                                 st * uj +
                                                                 2 * uo,
                                                                 -1 * a) -
        st * (k1 - k2) * (1 - a) * pow(1 - st * uj + 2 * uo,
                                       -1 * a) * (Da * log(1 - st * uj +
                                                           2 * uo) +
                                                  a / (1 - st * uj +
                                                       2 * uo) * (2 * Duo -
...
int HystereticPoly::commitSensitivity(double TstrainSensitivity,
...
double HystereticPoly::signum(double value)
{
    if (value > 0.0) {
        return 1.0;
    } else if (value == 0) {
