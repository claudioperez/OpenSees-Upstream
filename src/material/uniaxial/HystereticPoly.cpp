...
OPS_HystereticPoly ()
{
    ...
    theMaterial =
        new HystereticPoly (iData[0], dData[0], dData[1], dData[2], dData[3],
                            dData[4], dData[5]);
    if (theMaterial == 0)
      {
          opserr <<
...
double
HystereticPoly::getStrain ()
{
    return Tstrain;
}

double
HystereticPoly::getStress ()
{
    return Tstress;
}

double
HystereticPoly::getTangent ()
{
    return Ttangent;
}

int
HystereticPoly::commitState ()
{
    // State variables
    Cstrain = Tstrain;
    Cstress = Tstress;
...
int
HystereticPoly::revertToLastCommit ()
{
...
int
HystereticPoly::sendSelf (int commitTag, Channel & theChannel)
{
    int res = 0;
...
int
HystereticPoly::recvSelf (int commitTag, Channel & theChannel,
...
void
HystereticPoly::Print (OPS_Stream & s, int flag)
{
...
HystereticPoly::getStressSensitivity (int gradIndex, bool conditional)
{
    ...
    double Delta2 =
        Cstress - c * pow (Cstrain, 3) - d * pow (Cstrain,
                                                  5) - k2 * Cstrain -
        st * Fbar + Delta3;
        ...
        0.5 * (k1 -
               k2) * (pow (1 + 2 * uo,
                           1 - a) / (1 - a) * (2 * Duo * (1 - a) / (1 +
                                                                    2 * uo) -
                                               Da * log (1 + 2 * uo)) +
                      Da * (pow (1 + 2 * uo, 1 - a) - 1) / pow (1 - a, 2));

    double DD1 =
        (st * Da * (k1 - k2) + st * (1 - a) * (Dk1 - Dk2)) / pow (k1 - k2, 2);
    double DD3 =
        (Dk1 - Dk2) * pow (1 + 2 * uo,
                           1 - a) / st / (1 - a) + (k1 - k2) * pow (1 +
                                                                    2 * uo,
                                                                    1 -
                                                                    a) /
        pow (st, 2) / pow (1 - a,
                           2) * Da + Delta3 * (2 * Duo * (1 - a) / (1 +
                                                                    2 * uo) -
                                               Da * log (1 + 2 * uo));

    double DD2 =
        Dfc - Dc * pow (Cstrain, 3) - Dd * pow (Cstrain,
                                                5) - Dk2 * Cstrain -
        (3 * c * pow (Cstrain, 2) + 5 * d * pow (Cstrain, 4) + k2) * Duc -
        st * DFbar + DD3;

    double Duj =
        Duc + 2 * st * Duo - st * pow (Delta1 * Delta2,
                                       1 / (1 -
                                            a)) * (Da * log (Delta1 *
                                                             Delta2) /
                                                   pow (1 - a,
    ...
    double DD4 =
        pow (1 + st * Tstrain - st * uj + 2 * uo, 1 - a) / pow (st,
                                                                2) / pow (1 -
                                                                          a,
                                                                          2) *
        Da + Delta4 * (1 - a) * (st * Dut - st * Duj + 2 * Duo) / (1 +
                                                                   st *
                                                                   Tstrain -
                                                                   st * uj +
                                                                   2 * uo);
...
int
HystereticPoly::commitSensitivity (double TstrainSensitivity, int gradIndex,
                                   int numGrads)
{
    if (SHVs == 0)
      {
