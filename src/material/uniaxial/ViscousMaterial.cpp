...
OPS_ViscousMaterial (void)
{
    ...
    if (numArgs < 3 || numArgs > 4)
      {
          opserr <<
              "Invalid #args,  want: uniaxialMaterial Viscous tag? C? alpha? <minVel?> ... "
              << endln;
          return 0;
      }
    ...
    theMaterial =
        new ViscousMaterial (iData[0], dData[0], dData[1], dData[2]);
    if (theMaterial == 0)
      {
          opserr <<
              "WARNING could not create uniaxialMaterial of type Viscous\n";
          return 0;
      }
...
commitRate (0.0)
{
    if (Alpha < 0.0)
      {
          opserr <<
              "ViscousMaterial::ViscousMaterial -- Alpha < 0.0, setting to 1.0\n";
          Alpha = 1.0;
      }
    ...
    if (minVel == 0.0)
      {
          opserr <<
              "ViscousMaterial::ViscousMaterial -- minVel == 0.0, setting to 1.0e-21\n";
          minVel = 1.0e-21;
      }
}

ViscousMaterial::ViscousMaterial ():UniaxialMaterial (0, MAT_TAG_Viscous),
trialRate (0.0), trialStrain (0.0), C (0.0), Alpha (0.0), minVel (1e-11),
commitStrain (0.0), commitRate (0.0)
{
...
{
    ViscousMaterial *theCopy =
        new ViscousMaterial (this->getTag (), C, Alpha, minVel);
