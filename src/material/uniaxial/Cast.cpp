...
OPS_Cast(void)
{
    ...
    if (numData < 14) {
        opserr <<
            "WARNING insufficient number of args want  uniaxialMaterial CastFuse tag? NLegs? bo? h? Fy? E? L? b? R0? cR1? cR2? a1? a2? a3? a4\n";
        return 0;
    }
...
Cast::Cast(int tag, double NLegs, double _BO, double H, double _Fy,
           ...
           double _a4):UniaxialMaterial(tag, MAT_TAG_Cast), nLegs(NLegs),
bo(_BO), h(H), fy(_Fy), eo(E), l(L), b(_b), R0(_R0), cR1(_cR1), cR2(_cR2),
...
Cast::Cast(int tag, double NLegs, double _BO, double H,
           double _Fy, double E, double L, double _b):UniaxialMaterial(tag,
                                                                       MAT_TAG_Cast),
...
Cast::Cast(void):UniaxialMaterial(0, MAT_TAG_Cast)
{
    konP = 0;
}
...
int Cast::setTrialStrain(double trialStrain, double strainRate)
{
    ...
    if (kon == 2 && deps > 0.0) {
        ...
        epss0 =
            (Pp * shft - Esh * epsy * shft - sigr + kp * epsr) / (kp -
                                                                  Esh);
...
double Cast::getStrain(void)
{
    return eps;
}

double Cast::getStress(void)
{
    return sig;
}

double Cast::getTangent(void)
{
    return e;
}

int Cast::commitState(void)
{
    epsminP = epsmin;
    epsmaxP = epsmax;
    epsplP = epspl;
...
int Cast::revertToLastCommit(void)
{
    epsmin = epsminP;
    epsmax = epsmaxP;
    epspl = epsplP;
...
UniaxialMaterial *Cast::getCopy(void)
{
    Cast *theCopy =
        new Cast(this->getTag(), nLegs, bo, h, fy, eo, l, b, R0, cR1, cR2,
                 a1, a2, a3, a4);

    return theCopy;
}

int Cast::sendSelf(int commitTag, Channel & theChannel)
{
    int res = 0;
...
int Cast::recvSelf(int commitTag, Channel & theChannel,
...
void Cast::Print(OPS_Stream & s, int flag)
{
