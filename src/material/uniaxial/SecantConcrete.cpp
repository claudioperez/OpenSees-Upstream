...
SecantConcrete::SecantConcrete(int tag, double f, double ec,
                               double eu):UniaxialMaterial(tag,
                                                           MAT_TAG_SecantConcrete),
fc(f), epsc(ec), epsu(eu), parameterID(0), SHVs(0)
{
    // Initialize variables
    this->revertToStart();
}

SecantConcrete::SecantConcrete():UniaxialMaterial(0, MAT_TAG_SecantConcrete),
...
int SecantConcrete::setTrialStrain(double strain, double strainRate)
{
    // Set total strain
    Tstrain = strain;
...
double SecantConcrete::getStress(void)
{
    return Tstress;
}

double SecantConcrete::getTangent(void)
{
    return Ttangent;
}

double SecantConcrete::getStrain(void)
{
    return Tstrain;
}

int SecantConcrete::commitState(void)
{
...
int SecantConcrete::revertToLastCommit(void)
{
    // Nothing to do here
    return 0;
}

int SecantConcrete::revertToStart(void)
{
...
UniaxialMaterial *SecantConcrete::getCopy(void)
{
    SecantConcrete *theCopy =
        new SecantConcrete(this->getTag(), fc, epsc, epsu);
...
int SecantConcrete::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
...
int SecantConcrete::recvSelf(int cTag, Channel & theChannel,
...
void SecantConcrete::Print(OPS_Stream & s, int flag)
{
...
int SecantConcrete::setParameter(const char **argv, int argc,
                                 Parameter & param)
{
...
int SecantConcrete::updateParameter(int parameterID, Information & info)
{
    switch (parameterID) {
    case -1:
        return -1;
...
int SecantConcrete::activateParameter(int paramID)
{
    parameterID = paramID;
...
int SecantConcrete::commitSensitivity(double strainGradient,
...
double SecantConcrete::backboneCondDeriv(double strain, int gradIndex)
{
    double dfcdh = 0.0;
    double decdh = 0.0;
    double deudh = 0.0;
    ...
    if (strain > 0.0 || strain < epsu)
        ...
        double xi = strain / epsc;
        return dfcdh * (2.0 * xi - xi * xi) +
            decdh * 2.0 * fc / epsc * (xi * xi - xi);
    } else {
        double E = -fc / (epsu - epsc);
        double dEdh =
            -dfcdh / (epsu - epsc) +
            fc / ((epsu - epsc) * (epsu - epsc)) * (deudh - decdh);
        return dEdh * (strain - epsu) - E * deudh;
    }
}

double SecantConcrete::getStressGradient(int gradIndex)
{
    double depsdh = 0.0;
    double dsigdh = 0.0;
    ...
    if (Tstrain > 0.0 || Tstrain < epsu)
        ...
        double sigmaMin, dummy;
        this->backbone(CminStrain, sigmaMin, dummy);
        return Tstrain * (CminStrain * dsigdh -
                          sigmaMin * depsdh) / (CminStrain * CminStrain);
    } else
 ...
 SecantConcrete::setStrainGradient(int gradIndex, double depsilondh)
{
    if (SHVs != 0) {
        ...
        if (TminStrain < CminStrain) {
            depsdh = depsilondh;
            dsigdh =
                this->backboneUncondDeriv(TminStrain, gradIndex, depsdh);
        }
    }
