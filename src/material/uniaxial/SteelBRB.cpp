...
SteelBRB::setTrialStrain (double pStrain, double pStrainRate)
{
    ...
    if (strainInc == 0)
      ...
      {                         //  % (CStress* strainInc(i)<0) assume elastic unloading 
          if (fabs (strainInc) <= fabs (CStress / E))
            {                   // % strainInc is not big enough, no special treatment needed.
                plastStrainInc = 0;
                plastStrain = CPlastStrain;
            ...
            {                   // % strainInc is too big enough, need special treatment.
                ...
                if (CStress < 0)
                  {
                      ...
                      cumPlastStrain =
                          CCumPlastStrain + fabs (plastStrainInc);
                      yieldStress =
                          sigmaY0 + (sigmaY_T - sigmaY0) * (1 -
                                                            exp
                                                            (-cumPlastStrain /
...
double
SteelBRB::PlastStrainIncRes (double CStress, double beta, double CPlastStrain,
...
//      %temp3 = (sigmaY-sigmaY0)*(plastStrainInc/(delta*abs(plastStrainInc)))*exp(-(cumPlastStrain+abs(plastStrainInc))/delta)
    ...
    double f =
        1.0 - strainInc * alpha * pow ((fabs (temp1 / temp2)),
                                       (alpha -
                                        2)) * (temp1 / temp2) * (((-E -
                                                                   beta * E) *
                                                                  temp2 -
...
SteelBRB::Newton_BRB (double CStress, double beta, double CPlastStrain,
                      double sigmaY, double cumPlastStrain, double delta,
                      double alpha, double strainInc, double x0, double Tol,
                      int N0)
{
    ...
    double F_low =
        PlastStrainIncRes (CStress, beta, CPlastStrain, sigmaY,
    ...
    double F =
        PlastStrainIncRes (CStress, beta, CPlastStrain, sigmaY,
                           cumPlastStrain, delta, alpha, strainInc, x0);
    double F_dev = 0.0;
    double x1 = x0;
...
double
SteelBRB::getStrainSensitivity (int gradNumber)
{
    if (SHVs == 0)
      {
          ...
          if (fabs (strainInc) <= fabs (CStress / E))
            ...
            {                   // % strainInc is too big enough, need special treatment.
                ...
                double strain_unloadingSensitivity =
                    -(E * CStressSensitivity -
                      ESensitivity * CStress) / E / E;
                ...
                if (CStress < 0)
                  {
                      ...
                      double G =
                          alpha_T * pow (fabs (A),
                                         (alpha_T -
                                          2)) * A * strain_loading * (C / v -
                                                                      u * D /
                                                                      v / v) +
                          strain_loadingSensitivity * pow (fabs (A),
                                                           alpha_T) +
                          pow (fabs (A),
                               alpha_T) * log (fabs (A)) * strain_loading *
                          alpha_TSensitivity;
                      double H =
                          alpha_T * pow (fabs (A),
                                         (alpha_T -
                  ...
                  {             //%stress(i-1) > 0
                      ...
                      double G =
                          ...
                          strain_loadingSensitivity * pow (fabs (A),
                                                           alpha_C) +
                          pow (fabs (A),
                               alpha_C) * log (fabs (A)) * strain_loading *
                          alpha_CSensitivity;
                      double H =
                          alpha_C * pow (fabs (A),
                                         (alpha_C -
...
int
SteelBRB::commitSensitivity (double TstrainSensitivity, int gradNumber,
                             int numGrads)
{
    if (SHVs == 0)
      {
