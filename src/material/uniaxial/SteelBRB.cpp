...
void *OPS_SteelBRB(void)
{
    ...
    int tag;
    double E, sigmaY0, sigmaY_T, alpha_T, beta_T, delta_T, sigmaY_C,
        alpha_C, beta_C, delta_C, Tol;
    ...
    if (numArgs != 11 && numArgs != 12) {       // ---- correct!!
        opserr <<
            "Warning Insufficient args: unixialMaterial SteelBRB tag E sigmaY0 sigmaY_T alpha_T beta_T delta_T sigmaY_C alpha_C beta_C delta_C <Tol> \n";
        return 0;
    }
    ...
    if (OPS_GetInt(&numData, iData) != 0) {
        opserr <<
            "WARNING invalid integer values: nDMaterial SimplifiedJ2 \n";
        return 0;
    }
    tag = iData[0];
    ...
    if (OPS_GetDouble(&numData, dData) != 0) {
        opserr << "WARNING invalid double values: nDMaterial SimplifiedJ2 "
            << tag << endln;
        return 0;
    }
...
SteelBRB::SteelBRB(int pTag, double pE, double pSigmaY0, double pSigmaY_T,
                   double pAlpha_T, double pAlpha_C, double pSigmaY_C,
                   double pBeta_T, double pBeta_C, double pDelta_T,
                   double pDelta_C, double pTol):UniaxialMaterial(pTag,
                                                                  MAT_TAG_SteelBRB),
strain(0.0), stress(0.0), tangent(0.0), CStress(0.0), CPlastStrain(0.0),
CCumPlastStrain(0.0), CYieldStress(0.0), CStrain(0.0)
{
    E = pE;
 ...
 SteelBRB::setTrialStrain(double pStrain, double pStrainRate)
{
    ...
    if (strainInc == 0) {
        ...
        if (CStress >= 0) {
            plastStrainInc =
                Newton_BRB(CStress, beta_T, CPlastStrain, sigmaY_T,
                           CCumPlastStrain, delta_T, alpha_T, strainInc,
                           0, Tol, N0);
            ...
            yieldStress =
                sigmaY0 + (sigmaY_T - sigmaY0) * (1.0 -
                                                  exp(-cumPlastStrain /
                                                      delta_T));

            // ----------- tangent ---------
            double u =
                CStress + E * (strainInc - plastStrainInc) -
                beta_T * E * (CPlastStrain + plastStrainInc);
            double v =
                sigmaY0 + (sigmaY_T - sigmaY0) * (1 -
                                                  exp(-
                                                      (CCumPlastStrain +
                                                       fabs
                                                       (plastStrainInc))
/ delta_T));
            ...
            double D = alpha_T * pow(fabs(A),
                                     (alpha_T -
                                      2)) * A * strainInc * u / v / v *
                (sigmaY_T -
                 sigmaY0) / delta_T * sign * exp(-(CCumPlastStrain +
                                                   fabs(plastStrainInc))
                                                 / delta_T);
            double tmp =
                (C + pow(fabs(A), alpha_T)) / (1 + C * (1 + beta_T) + D);
            tangent = E * (1 - tmp);
            dissipatedEnergy =
                CDissipatedEnergy + 0.5 * (stress + CStress -
                                           beta_T * E * (CPlastStrain +
                                                         plastStrain)) *
                plastStrainInc;
            ...
            plastStrainInc =
                Newton_BRB(CStress, beta_C, CPlastStrain, sigmaY_C,
                           CCumPlastStrain, delta_C, alpha_C, strainInc,
                           0, Tol, N0);
            plastStrain = CPlastStrain + plastStrainInc;
            stress = CStress + E * (strainInc - plastStrainInc);
            cumPlastStrain = CCumPlastStrain + fabs(plastStrainInc);
            yieldStress =
                sigmaY0 + (sigmaY_C - sigmaY0) * (1 -
                                                  exp(-cumPlastStrain /
                                                      delta_C));
            ...
            double u =
                CStress + E * (strainInc - plastStrainInc) -
                beta_C * E * (CPlastStrain + plastStrainInc);
            double v =
                sigmaY0 + (sigmaY_C - sigmaY0) * (1 -
                                                  exp(-
                                                      (CCumPlastStrain +
...
/ delta_C));
            ...
            double D = alpha_C * pow(fabs(A),
                                     (alpha_C -
                                      2)) * A * strainInc * u / v / v *
                (sigmaY_C -
                 sigmaY0) / delta_C * sign * exp(-(CCumPlastStrain +
                                                   fabs(plastStrainInc))
                                                 / delta_C);
            double tmp =
                (C + pow(fabs(A), alpha_C)) / (1 + C * (1 + beta_C) + D);
            tangent = E * (1 - tmp);
            dissipatedEnergy =
                CDissipatedEnergy + 0.5 * (stress + CStress -
                                           beta_C * E * (CPlastStrain +
                                                         plastStrain)) *
                plastStrainInc;
        ...
        if (fabs(strainInc) <= fabs(CStress / E)) {     // % strainInc is not big enough, no special treatment needed.
            ...
            if (CStress < 0) {
                plastStrainInc =
                    Newton_BRB(0, beta_T, CPlastStrain, sigmaY_T,
                               CCumPlastStrain, delta_T, alpha_T,
                               strain_loading, 0, Tol, N0);
                plastStrain = CPlastStrain + plastStrainInc;
                ...
                // ------------ tangent ---
                double u =
                    0 + E * (strain_loading - plastStrainInc) -
                    beta_T * E * (CPlastStrain + plastStrainInc);
                double v =
                    sigmaY0 + (sigmaY_T - sigmaY0) * (1 -
                                                      exp(-
...
/ delta_T));
                ...
                double D = alpha_T * pow(fabs(A),
                                         (alpha_T -
                                          2)) * A * strain_loading * u /
                    v / v * (sigmaY_T -
                             sigmaY0) / delta_T * sign *
                    exp(-(CCumPlastStrain + fabs(plastStrainInc)) /
                        delta_T);
                double tmp =
                    (C + pow(fabs(A), alpha_T)) / (1 + C * (1 + beta_T) +
                                                   D);
                tangent = E * (1 - tmp);
                dissipatedEnergy =
                    CDissipatedEnergy + 0.5 * (stress + 0 -
                                               beta_T * E *
                                               (CPlastStrain +
                                                plastStrain)) *
                    plastStrainInc;
                ...
                plastStrainInc =
                    Newton_BRB(0, beta_C, CPlastStrain, sigmaY_C,
                               CCumPlastStrain, delta_C, alpha_C,
                               strain_loading, 0, Tol, N0);
                plastStrain = CPlastStrain + plastStrainInc;
                stress = E * (strain_loading - plastStrainInc);
                cumPlastStrain = CCumPlastStrain + fabs(plastStrainInc);
...
/ delta_C));
                ...
                double D = alpha_C * pow(fabs(A),
                                         (alpha_C -
                                          2)) * A * strain_loading * u /
                    v / v * (sigmaY_C -
                             sigmaY0) / delta_C * sign *
                    exp(-(CCumPlastStrain + fabs(plastStrainInc)) /
                        delta_C);
                double tmp =
                    (C + pow(fabs(A), alpha_C)) / (1 + C * (1 + beta_C) +
                                                   D);
                tangent = E * (1 - tmp);
                dissipatedEnergy =
                    CDissipatedEnergy + 0.5 * (stress + 0 -
...
double SteelBRB::getStress(void)
{
    return stress;
}

double SteelBRB::getTangent(void)
{
    return tangent;
}

double SteelBRB::getInitialTangent(void)
{
    // return the initial tangent
    return E;
}

double SteelBRB::getStrain(void)
{
    return strain;
}

int SteelBRB::commitState(void)
{
    CStress = stress;
...
int SteelBRB::revertToLastCommit(void)
{
    return 0;
}

int SteelBRB::revertToStart(void)
{
...
UniaxialMaterial *SteelBRB::getCopy(void)
{
    SteelBRB *theCopy =
        new SteelBRB(this->getTag(), E, sigmaY0, sigmaY_T, alpha_T,
                     alpha_C, sigmaY_C, beta_T, beta_C, delta_T, delta_C,
                     Tol);
...
int SteelBRB::sendSelf(int cTag, Channel & theChannel)
{
    return -1;
}

int SteelBRB::recvSelf(int cTag, Channel & theChannel,
                       FEM_ObjectBroker & theBroker)
{
    return -1;
}

void SteelBRB::Print(OPS_Stream & s, int flag)
{
    return;
}
...
double
    SteelBRB::PlastStrainIncRes(double CStress, double beta,
                                double CPlastStrain, double sigmaY,
                                double cumPlastStrain, double delta,
                                double alpha, double strainInc,
                                double plastStrainInc)
{


    double temp1 =
        CStress + E * (strainInc - plastStrainInc) -
        beta * E * (CPlastStrain + plastStrainInc);
    double temp2 =
        sigmaY0 + (sigmaY - sigmaY0) * (1.0 -
                                        exp(-
                                            (cumPlastStrain +
                                             fabs(plastStrainInc)) /
                                            delta));
    ...
    double f =
        plastStrainInc - pow(fabs(temp1 / temp2), alpha) * strainInc;
...
double
 SteelBRB::PlastStrainIncResDev(double CStress, double beta,
                                double CPlastStrain, double sigmaY,
                                double cumPlastStrain, double delta,
                                double alpha, double strainInc,
                                double plastStrainInc)
...
//      %temp3 = (sigmaY-sigmaY0)*(plastStrainInc/(delta*abs(plastStrainInc)))*exp(-(cumPlastStrain+abs(plastStrainInc))/delta)
    ...
    double temp3 =
        (sigmaY -
         sigmaY0) / delta * sign * exp(-(cumPlastStrain +
                                         fabs(plastStrainInc)) / delta);
    double f = 1.0 - strainInc * alpha * pow((fabs(temp1 / temp2)),
                                             (alpha -
                                              2)) * (temp1 / temp2) *
        (((-E - beta * E) * temp2 - temp1 * temp3) / temp2 / temp2);
    return f;
...
double
    SteelBRB::Newton_BRB(double CStress, double beta, double CPlastStrain,
                         double sigmaY, double cumPlastStrain,
                         double delta, double alpha, double strainInc,
                         double x0, double Tol, int N0)
{
    ...
    double F_low = PlastStrainIncRes(CStress, beta, CPlastStrain, sigmaY,
                                     cumPlastStrain, delta, alpha,
                                     strainInc,
                                     lowerbound);
    double F_upp = PlastStrainIncRes(CStress, beta, CPlastStrain, sigmaY,
                                     cumPlastStrain, delta, alpha,
                                     strainInc,
                                     upperbound);
    ...
    if (F_low * F_upp > 0)
        opserr <<
            "In SteelBRB::Newton_BRB, lower bound and upper bound have the same sign!\n";


    double F = PlastStrainIncRes(CStress, beta, CPlastStrain, sigmaY,
                                 cumPlastStrain, delta, alpha, strainInc,
                                 x0);
    double F_dev = 0.0;
    double x1 = x0;
    ...
    while ((i <= N0) && (fabs(F) > Tol)) {
        ...
        F_dev =
            PlastStrainIncResDev(CStress, beta, CPlastStrain, sigmaY,
                                 cumPlastStrain, delta, alpha, strainInc,
                                 x0);
        x1 = x0 - F / F_dev;
        ...
        F = PlastStrainIncRes(CStress, beta, CPlastStrain, sigmaY,
                              cumPlastStrain, delta, alpha, strainInc, x1);
    ...
    if (fabs(F) > Tol) {
        opserr <<
            "Fatal error: SteelBRB::Newton_BRB does not converge ===============\n";
        exit(-1);
    }
 ...
 SteelBRB::setParameter(const char **argv, int argc, Information & info)
{
    ...
    if (strcmp(argv[0], "delta_C") == 0) {
        ...
        opserr << "WARNING: Could not set parameter in SteelBRB. " <<
            endln;
...
int SteelBRB::updateParameter(int parameterID, Information & info)
{
    switch (parameterID) {
    case -1:
        return -1;
...
int SteelBRB::activateParameter(int passedParameterID)
{
...
{
    if (SHVs == 0) {
        opserr << "warning:SteelBRB::getStrainsSensitivity, SHVs =0 " <<
            endln;
        return 0.0;
    }
...
double SteelBRB::getStressSensitivity(int gradNumber, bool conditional)
{
...
//                 stressSensitivity = CStressSensitivity;                      
        stressSensitivity =
            CStressSensitivity + ESensitivity * strainInc +
            E * strainIncSensitivity;
...
/ delta_T));
            ...
            double C =
                CStressSensitivity + (strainInc - plastStrainInc -
                                      beta_T * CPlastStrain -
                                      beta_T * plastStrainInc) *
                ESensitivity + E * strainIncSensitivity -
                E * (CPlastStrain + plastStrainInc) * beta_TSensitivity -
                beta_T * E * CPlastStrainSensitivity;
            double D =
                sigmaY0Sensitivity + (sigmaY_TSensitivity -
                                      sigmaY0Sensitivity) * (1.0 -
                                                             exp(-
                                                                 (CCumPlastStrain
                                                                  +
...
/ delta_T))
                - (sigmaY_T -
                   sigmaY0) * exp(-(CCumPlastStrain +
                                    fabs(plastStrainInc)) / delta_T) *
                (delta_TSensitivity *
                 (CCumPlastStrain +
                  fabs(plastStrainInc)) / delta_T / delta_T -
                 CCumPlastStrainSensitivity / delta_T);
            ...
            double G = alpha_T * pow(fabs(A),
                                     (alpha_T -
                                      2)) * A * strainInc * (C / v -
                                                             u * D / v /
                                                             v) +
                strainIncSensitivity * pow(fabs(A),
                                           alpha_T) + pow(fabs(A),
                                                          alpha_T) *
                log(fabs(A)) * strainInc * alpha_TSensitivity;
            double H = alpha_T * pow(fabs(A),
                                     (alpha_T - 2)) * A * strainInc * ((E +
                                                                        E *
                                                                        beta_T)
                                                                       /
                                                                       v +
            ...
            plastStrainSensitivity =
                CPlastStrainSensitivity + plastStrainIncSensitivity;
            stressSensitivity =
                CStressSensitivity + ESensitivity * (strainInc -
                                                     plastStrainInc) +
                E * (strainIncSensitivity - plastStrainIncSensitivity);
            cumPlastStrainSensitivity =
                CCumPlastStrainSensitivity +
                sign * plastStrainIncSensitivity;
            yieldStressSensitivity =
                sigmaY0Sensitivity + (sigmaY_TSensitivity -
                                      sigmaY0Sensitivity) * (1.0 -
                                                             exp
                                                             (-cumPlastStrain
                                                              ...
                                                              delta_T)) +
                (sigmaY_T -
                 sigmaY0) * exp(-cumPlastStrain / delta_T) *
                (cumPlastStrainSensitivity * delta_T -
                 cumPlastStrain * delta_TSensitivity) / delta_T / delta_T;
...
/ delta_C));
            ...
            double C =
                CStressSensitivity + (strainInc - plastStrainInc -
                                      beta_C * CPlastStrain -
                                      beta_C * plastStrainInc) *
                ESensitivity + E * strainIncSensitivity -
                E * (CPlastStrain + plastStrainInc) * beta_CSensitivity -
                beta_C * E * CPlastStrainSensitivity;
            double D =
                sigmaY0Sensitivity + (sigmaY_CSensitivity -
                                      sigmaY0Sensitivity) * (1.0 -
                                                             exp(-
                                                                 (CCumPlastStrain
                                                                  +
...
/ delta_C))
                - (sigmaY_C -
                   sigmaY0) * exp(-(CCumPlastStrain +
                                    fabs(plastStrainInc)) / delta_C) *
                (delta_CSensitivity *
                 (CCumPlastStrain +
                  fabs(plastStrainInc)) / delta_C / delta_C -
                 CCumPlastStrainSensitivity / delta_C);
            ...
            double G = alpha_C * pow(fabs(A),
                ...
                strainIncSensitivity * pow(fabs(A),
                                           alpha_C) + pow(fabs(A),
                                                          alpha_C) *
                log(fabs(A)) * strainInc * alpha_CSensitivity;
            double H = alpha_C * pow(fabs(A),
                                     (alpha_C - 2)) * A * strainInc * ((E +
                                                                        E *
                                                                        beta_C)
                                                                       /
                                                                       v +
                ...
                sigmaY0Sensitivity + (sigmaY_CSensitivity -
                                      sigmaY0Sensitivity) * (1.0 -
                                                             ...
                                                             (-cumPlastStrain
                                                              ...
                                                              delta_C)) +
                (sigmaY_C -
                 sigmaY0) * exp(-cumPlastStrain / delta_C) *
                (cumPlastStrainSensitivity * delta_C -
                 cumPlastStrain * delta_CSensitivity) / delta_C / delta_C;
        }                       // %stress(i-1) < 0
    }
        ...
        if (fabs(strainInc) <= fabs(CStress / E)) {     // % strainInc is not big enough, no special treatment needed.
            ...
            double strain_unloadingSensitivity =
                -(E * CStressSensitivity - ESensitivity * CStress) / E / E;
            double strain_loadingSensitivity =
                strainIncSensitivity - strain_unloadingSensitivity;
            ...
            if (CStress < 0) {
                ...
                if (plastStrainInc > 0)
                    sign = 1.0;
                double u =
                    0.0 + E * (strain_loading - plastStrainInc) -
                    beta_T * E * (CPlastStrain + plastStrainInc);
                double v =
                    sigmaY0 + (sigmaY_T - sigmaY0) * (1 -
                                                      exp(-
...
/ delta_T));
                ...
                double C =
                    0.0 + (strain_loading - plastStrainInc -
                           beta_T * CPlastStrain -
                           beta_T * plastStrainInc) * ESensitivity +
                    E * strain_loadingSensitivity - E * (CPlastStrain +
                                                         plastStrainInc)
                    * beta_TSensitivity -
                    beta_T * E * CPlastStrainSensitivity;
                double D =
...
/ delta_T))
                ...
                double G = alpha_T * pow(fabs(A),
                                         (alpha_T -
                                          2)) * A * strain_loading * (C /
                                                                      v -
                                                                      u *
                                                                      D /
                                                                      v /
                                                                      v) +
                    strain_loadingSensitivity * pow(fabs(A),
                                                    alpha_T) + pow(fabs(A),
                                                                   alpha_T)
                    * log(fabs(A)) * strain_loading * alpha_TSensitivity;
                double H = alpha_T * pow(fabs(A),
                                         (alpha_T -
                                          2)) * A * strain_loading * ((E +
                                                                       E *
                                                                       beta_T)
                                                                      / v +
                                                                      u *
                ...
                plastStrainSensitivity =
                    CPlastStrainSensitivity + plastStrainIncSensitivity;
                stressSensitivity =
                    0.0 + ESensitivity * (strain_loading -
                                          plastStrainInc) +
                    E * (strain_loadingSensitivity -
                         plastStrainIncSensitivity);
                cumPlastStrainSensitivity =
                    CCumPlastStrainSensitivity +
                    sign * plastStrainIncSensitivity;
                yieldStressSensitivity =
                    ...
                    + (sigmaY_T -
                       sigmaY0) * exp(-cumPlastStrain / delta_T) *
                    (cumPlastStrainSensitivity * delta_T -
                     cumPlastStrain * delta_TSensitivity) / delta_T /
                    delta_T;
            } else {            //%stress(i-1) > 0
                plastStrainInc = plastStrain - CPlastStrain;    //Newton_BRB(CStress, beta_C, CPlastStrain, sigmaY_C,CCumPlastStrain, delta_C, alpha_C, strainInc,0, Tol, N0);
                int sign = -1.0;
                if (plastStrainInc > 0)
                    sign = 1.0;
                double u =
                    E * (strain_loading - plastStrainInc) -
                    beta_C * E * (CPlastStrain + plastStrainInc);
                double v =
                    sigmaY0 + (sigmaY_C - sigmaY0) * (1 -
                                                      exp(-
...
/ delta_C));
                ...
                double C =
                    0.0 + (strain_loading - plastStrainInc -
                           beta_C * CPlastStrain -
                           beta_C * plastStrainInc) * ESensitivity +
                    E * strain_loadingSensitivity - E * (CPlastStrain +
                                                         plastStrainInc)
                    * beta_CSensitivity -
                    beta_C * E * CPlastStrainSensitivity;
...
/ delta_C))
                ...
                double G = alpha_C * pow(fabs(A),
                    ...
                    strain_loadingSensitivity * pow(fabs(A),
                                                    alpha_C) + pow(fabs(A),
                                                                   alpha_C)
                    * log(fabs(A)) * strain_loading * alpha_CSensitivity;
                double H = alpha_C * pow(fabs(A),
                                         (alpha_C -
                                          2)) * A * strain_loading * ((E +
                                                                       E *
                ...
                yieldStressSensitivity =
                    ...
                    + (sigmaY_C -
                       sigmaY0) * exp(-cumPlastStrain / delta_C) *
                    (cumPlastStrainSensitivity * delta_C -
                     cumPlastStrain * delta_CSensitivity) / delta_C /
                    delta_C;
            }                   // %stress(i-1) < 0 
        }                       // % if (abs(strainInc(i))<=CStress/E)
...
int SteelBRB::commitSensitivity(double TstrainSensitivity, int gradNumber,
                                int numGrads)
{
    if (SHVs == 0) {
...
/ delta_T))
            ...
            dissipatedEnergySensitivity =
                CDissipatedEnergySensitivity + 0.5 * (stress + CStress -
                                                      beta_T * E *
                                                      (CPlastStrain +
                                                       plastStrain)) *
                plastStrainIncSensitivity + 0.5 * (stressSensitivity +
                                                   CStressSensitivity -
                                                   (beta_TSensitivity *
                                                    E +
                                                    beta_T *
...
//                        dissipatedEnergy += 0.5*(stress+0-beta_T*E*(CPlastStrain+plastStrain))*plastStrainInc;
                dissipatedEnergySensitivity =
                    CDissipatedEnergySensitivity + 0.5 * (stress + 0 -
                                                          beta_T * E *
                                                          (CPlastStrain
                                                           + plastStrain))
                    * plastStrainIncSensitivity +
                    0.5 * (stressSensitivity + 0 -
                           (beta_TSensitivity * E +
                            beta_T * ESensitivity) * (CPlastStrain +
                                                      plastStrain) -
                           beta_T * E * (CPlastStrainSensitivity +
                                         plastStrainSensitivity)) *
                    plastStrainInc;
...
//                   dissipatedEnergy += 0.5*(stress+0-beta_C*E*(CPlastStrain+plastStrain))*plastStrainInc;                      
                dissipatedEnergySensitivity =
                    ...
                    * plastStrainIncSensitivity +
                    0.5 * (stressSensitivity + 0 -
                           (beta_CSensitivity * E +
                            beta_C * ESensitivity) * (CPlastStrain +
                                                      plastStrain) -
                           beta_C * E * (CPlastStrainSensitivity +
                                         plastStrainSensitivity)) *
                    plastStrainInc;
...
double SteelBRB::getInitialTangentSensitivity(int gradNumber)
{
    if (parameterID == 1) {
        return 1.0;
    } else {
...
Response *SteelBRB::setResponse(const char **argv, int argc,
...
int SteelBRB::getResponse(int responseID, Information & matInfo)
{
