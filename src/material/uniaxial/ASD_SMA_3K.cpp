...
ASD_SMA_3K::ASD_SMA_3K(int tag, double K1, double K2, double K3,
                       double fa, double b):UniaxialMaterial(tag,
                                                             MAT_TAG_ASD_SMA_3K),
...
ASD_SMA_3K::ASD_SMA_3K():UniaxialMaterial(0, MAT_TAG_ASD_SMA_3K),
    ...
    if (fabs(Tstrain) <= ((1 - beta) * ActF / k1)) {
        ...
        if (Tstrain >= 0) {
            ...
            if ((Tstrain >= ClowerStrainPos) &&
                ...
                if (diffStrain < DBL_EPSILON) {
                    if (No_Y_Pos == 1) {
                        ...
                        TupperStrainPos =
                            (k1 * Tstrain - Tstress - k2 * ActDef +
                             ActF) / (k1 - k2);
                        TupperStressPos =
                            k2 * TupperStrainPos - k2 * ActDef + ActF;
                    } else {
                        Tstress =
                            fmin(Cstress + diffStrain * k1, Tstrain * k1);
                        Ttangent = k1;
                    }
                    ...
                    if (No_Y_Pos == 1) {
                        ...
                        Tstress =
                            fmin((Tstrain - CactivStrainPos) * k1,
                                 Tstrain * k1);
                        Ttangent = k1;
                        No_k2_Pos = 0;
                    }
            ...
            // Upper Activation 
                ...
                TupperStressPos =
                    CupperStressPos + (Tstrain - CupperStrainPos) * k2;
                TupperStrainPos = Tstrain;

                X = (-k3 * TupperStrainPos + TupperStressPos) / (k1 - k3);
                Y = k1 * ((-k3 * TupperStrainPos + TupperStressPos) /
                          (k1 - k3));

                TlowerStrainPos =
                    (TupperStressPos - TupperStrainPos * k3 -
                     ActF * (1 - beta) * (1 - (k2 / k1))) / (k2 - k3);
                TlowerStressPos =
                    k2 *
                    ((TupperStressPos - TupperStrainPos * k3 -
                      ActF * (1 - beta) * (1 - (k2 / k1))) / (k2 -
                                                              k3)) +
                    ActF * (1 - beta) * (1 - (k2 / k1));
                No_k2_Pos = 0;
        ...
        // Negative Quadrant (Bottom Left) where strain < 0
            ...
            if ((Tstrain <= ClowerStrainNeg) &&
                ...
                if (diffStrain > DBL_EPSILON) {
                    if (No_Y_Neg == 1) {
                        ...
                        TupperStrainNeg =
                            (k1 * Tstrain - Tstress + k2 * ActDef -
                             ActF) / (k1 - k2);
                        TupperStressNeg =
                            k2 * TupperStrainNeg + k2 * ActDef - ActF;
                    } else {
                        Tstress =
                            fmax(Cstress + diffStrain * k1, Tstrain * k1);
                        Ttangent = k1;
                    }
                } else if (diffStrain < DBL_EPSILON) {
                    ...
                    if (No_Y_Neg == 1) {
                        ...
                        Tstress =
                            fmax((Tstrain - CactivStrainNeg) * k1,
                                 Tstrain * k1);
                        Ttangent = k1;
                        No_k2_Neg = 0;
            ...
            // Upper Activation
                ...
                TupperStressNeg =
                    CupperStressNeg + (Tstrain - CupperStrainNeg) * k2;
                TupperStrainNeg = Tstrain;

                X = (-k3 * TupperStrainNeg + TupperStressNeg) / (k1 - k3);
                Y = k1 * ((-k3 * TupperStrainNeg + TupperStressNeg) /
                          (k1 - k3));

                TlowerStrainNeg =
                    (TupperStressNeg - TupperStrainNeg * k3 +
                     ActF * (1 - beta) * (1 - (k2 / k1))) / (k2 - k3);
                TlowerStressNeg =
                    k2 *
                    ((TupperStressNeg - TupperStrainNeg * k3 +
            ...
            // Lower Activation
                ...
                if (k1 * Tstrain >= ClowerStressNeg && No_k2_Neg == 1) {
                    ...
                    TlowerStressNeg =
                        ClowerStressNeg + (Tstrain - ClowerStrainNeg) * k2;
...
double ASD_SMA_3K::getStress(void)
{
    return Tstress;
}

double ASD_SMA_3K::getTangent(void)
{
    return Ttangent;
}

double ASD_SMA_3K::getStrain(void)
{
    return Tstrain;
}

int ASD_SMA_3K::commitState(void)
{
...
int ASD_SMA_3K::revertToLastCommit(void)
{
    Tstrain = Cstrain;
    Tstress = Cstress;
    Ttangent = Ctangent;
...
int ASD_SMA_3K::revertToStart(void)
{
...
UniaxialMaterial *ASD_SMA_3K::getCopy(void)
{
    ASD_SMA_3K *theCopy =
...
int ASD_SMA_3K::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
...
int ASD_SMA_3K::recvSelf(int cTag, Channel & theChannel,
...
void ASD_SMA_3K::Print(OPS_Stream & s, int flag)
{
