...
OPS_Bilin()
{
    ...
    if (numData != 23 && numData != 24) {       // Updated: Filipe Ribeiro and Andre Barbosa
        opserr <<
            "Invalid Args want: uniaxialMaterial Bilin tag? Ke? AsPos? AsNeg? My_pos? My_neg? LamdaS? ";
        opserr <<
    ...
    if (numData == 23) {
        ...
        theMaterial = new Bilin(iData[0], dData[0], dData[1], dData[2], dData[3], dData[4], dData[5], dData[6], dData[7], dData[8], dData[9], dData[10], dData[11], dData[12], dData[13], dData[14], dData[15], dData[16], dData[17], dData[18], dData[19], dData[20], dData[21], dData[22]);   // Updated: Filipe Ribeiro and Andre Barbosa
        ...
        // Parsing was successful, allocate the material
        theMaterial = new Bilin(iData[0], dData[0], dData[1], dData[2], dData[3], dData[4], dData[5], dData[6], dData[7], dData[8], dData[9], dData[10], dData[11], dData[12], dData[13], dData[14], dData[15], dData[16], dData[17], dData[18], dData[19], dData[20], dData[21], dData[22], dData[23]);        // Updated: Filipe Ribeiro and Andre Barbosa

    }

    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type Bilin Material\n";
        return 0;
    }
...
// if nFactor is assigned                                                       
// Updated: Filipe Ribeiro and Andre Barbosa 
Bilin::Bilin(int tag, double p_Ke0, double p_AsPos, double p_AsNeg, double p_My_pos, double p_My_neg, double p_LamdaS, double p_LamdaD, double p_LamdaA, double p_LamdaK, double p_Cs, double p_Cd, double p_Ca, double p_Ck, double p_Thetap_pos, double p_Thetap_neg, double p_Thetapc_pos, double p_Thetapc_neg, double p_KPos, double p_KNeg, double p_Thetau_pos, double p_Thetau_neg, double p_PDPlus, double p_PDNeg, double p_nFactor):UniaxialMaterial(tag, MAT_TAG_Bilin), Ke0(p_Ke0), AsPos(p_AsPos), AsNeg(p_AsNeg), My_pos(p_My_pos), My_neg(p_My_neg), LamdaS(p_LamdaS), LamdaK(p_LamdaK), LamdaA(p_LamdaA), LamdaD(p_LamdaD), Cs(p_Cs), Ck(p_Ck), Ca(p_Ca), Cd(p_Cd), Thetap_pos(p_Thetap_pos), Thetap_neg(p_Thetap_neg), Thetapc_pos(p_Thetapc_pos), Thetapc_neg(p_Thetapc_neg), KPos(p_KPos), KNeg(p_KNeg), Thetau_pos(p_Thetau_pos), Thetau_neg(p_Thetau_neg), PDPlus(p_PDPlus), PDNeg(p_PDNeg), nFactor(p_nFactor)   // Updated: Filipe Ribeiro and Andre Barbosa 
{
...
Bilin::Bilin(int tag, double p_Ke0, double p_AsPos, double p_AsNeg, double p_My_pos, double p_My_neg, double p_LamdaS, double p_LamdaD, double p_LamdaA, double p_LamdaK, double p_Cs, double p_Cd, double p_Ca, double p_Ck, double p_Thetap_pos, double p_Thetap_neg, double p_Thetapc_pos, double p_Thetapc_neg, double p_KPos, double p_KNeg, double p_Thetau_pos, double p_Thetau_neg, double p_PDPlus, double p_PDNeg):UniaxialMaterial(tag, MAT_TAG_Bilin), Ke0(p_Ke0), AsPos(p_AsPos), AsNeg(p_AsNeg), My_pos(p_My_pos), My_neg(p_My_neg), LamdaS(p_LamdaS), LamdaK(p_LamdaK), LamdaA(p_LamdaA), LamdaD(p_LamdaD), Cs(p_Cs), Ck(p_Ck), Ca(p_Ca), Cd(p_Cd), Thetap_pos(p_Thetap_pos), Thetap_neg(p_Thetap_neg), Thetapc_pos(p_Thetapc_pos), Thetapc_neg(p_Thetapc_neg), KPos(p_KPos), KNeg(p_KNeg), Thetau_pos(p_Thetau_pos), Thetau_neg(p_Thetau_neg), PDPlus(p_PDPlus), PDNeg(p_PDNeg) // Updated: Filipe Ribeiro and Andre Barbosa 
{
    //initialize variables
    this->revertToStart();
    //this->revertToLastCommit();
...
Bilin::Bilin():UniaxialMaterial(0, MAT_TAG_Bilin), Ke0(0), AsPos(0), AsNeg(0), My_pos(0),
My_neg(0), LamdaS(0), LamdaD(0), LamdaA(0), LamdaK(0), Cs(0), Cd(0), Ca(0),
Ck(0), Thetap_pos(0), Thetap_neg(0), Thetapc_pos(0), Thetapc_neg(0),
        ...
        if (deltaD < 0.0) {
            ...
            if (fyNeg >= KNeg * My_neg) {       // If strength drops below residual
                ...
                capSlopeNeg =
                    capSlopeNeg *
                    (fabs
                     ((KNeg * My_neg - fyNeg) / (KNeg * My_neg - My_neg)));
            ...
            if (fyPos <= KPos * My_pos) {       //% If yield Strength Pos drops below residual
                ...
                capSlope =
                    capSlope *
                    (fabs
                     ((KPos * My_pos - fyPos) / (KPos * My_pos - My_pos)));
                //3rd - recompute the post-capping ratio (updated)     
...
double Bilin::getStress(void)
{
    return (fP);
}

double Bilin::getTangent(void)
{
    return (Tangent);
}

double Bilin::getInitialTangent(void)
{
    return (Ke);
}
...
int Bilin::revertToLastCommit(void)
{
...
UniaxialMaterial *Bilin::getCopy(void)
{
    Bilin *theCopy =
        new Bilin(this->getTag(), Ke0, AsPos, AsNeg, My_pos, My_neg,
                  LamdaS,
                  LamdaD,
...
int Bilin::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
...
int Bilin::recvSelf(int cTag, Channel & theChannel,
...
void Bilin::Print(OPS_Stream & s, int flag)
{
...
//my functions
void Bilin::interPoint(double &xInt, double &yInt, double x1, double y1,
...
void Bilin::envelPosCap2(double fy, double alphaPos, double alphaCap,
...
void Bilin::envHitsZero(double &f)
{
    if (fP > 0) {
        if ((f * fP) < 0) {
            f = 0;
...
void Bilin::envelNegCap2(double fy, double alphaNeg, double alphaCap,
                         double cpDsp, double &d, double &f, double &ek,
