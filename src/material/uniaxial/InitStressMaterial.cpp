...
void *OPS_InitStressMaterial(void)
{
    ...
    theMaterial =
        new InitStressMaterial(iData[0], *theOtherMaterial, dData[0]);

    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type InitStressMaterial\n";
        return 0;
    }
...
int InitStressMaterial::findInitialStrain(void)
{
    ...
    if ((fabs(tStress - sigInit) < tol))
        ...
        opserr <<
            "WARNING: InitStressMaterial - could not find initStrain to within tol for material: "
            << theMaterial->getTag();
        opserr << " wanted sigInit: " << sigInit << " using tStress: " <<
            theMaterial->getStress() << endln;
        return -1;
    }
...
InitStressMaterial::InitStressMaterial(int tag,
                                       ...
                                       double sigini):UniaxialMaterial(tag,
                                                                       MAT_TAG_InitStress),
theMaterial(0), epsInit(0.0), sigInit(sigini)
{
    theMaterial = material.getCopy();

    if (theMaterial == 0) {
        opserr <<
            "InitStressMaterial::InitStressMaterial -- failed to get copy of material\n";
        exit(-1);
    }
...
InitStressMaterial::InitStressMaterial():UniaxialMaterial(0, MAT_TAG_InitStress), theMaterial(0),
epsInit(0.0)
{
...
double InitStressMaterial::getStress(void)
{
    return theMaterial->getStress();
}

double InitStressMaterial::getTangent(void)
{
    return theMaterial->getTangent();
}

double InitStressMaterial::getDampTangent(void)
{
    return theMaterial->getDampTangent();
}

double InitStressMaterial::getStrain(void)
{
    return theMaterial->getStrain();
}

double InitStressMaterial::getStrainRate(void)
{
    return theMaterial->getStrainRate();
}

int InitStressMaterial::commitState(void)
{
    return theMaterial->commitState();
}

int InitStressMaterial::revertToLastCommit(void)
{
    return theMaterial->revertToLastCommit();
}

int InitStressMaterial::revertToStart(void)
{
    int res = 0;
...
UniaxialMaterial *InitStressMaterial::getCopy(void)
{
...
int InitStressMaterial::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (theChannel.sendID(dbTag, cTag, dataID) < 0) {
        opserr <<
            "InitStressMaterial::sendSelf() - failed to send the ID\n";
        return -1;
    }
    ...
    if (theChannel.sendVector(dbTag, cTag, dataVec) < 0) {
        opserr <<
            "InitStressMaterial::sendSelf() - failed to send the Vector\n";
        return -2;
    }

    if (theMaterial->sendSelf(cTag, theChannel) < 0) {
        opserr <<
            "InitStressMaterial::sendSelf() - failed to send the Material\n";
        return -3;
    }
...
int InitStressMaterial::recvSelf(int cTag, Channel & theChannel,
                                 FEM_ObjectBroker & theBroker)
{
    ...
    if (theChannel.recvID(dbTag, cTag, dataID) < 0) {
        opserr <<
            "InitStressMaterial::recvSelf() - failed to get the ID\n";
        return -1;
    }
    ...
    if (theMaterial == 0) {
        ...
        if (theMaterial == 0) {
            opserr <<
                "InitStressMaterial::recvSelf() - failed to create Material with classTag "
                << dataID(0) << endln;
            return -2;
        }
    }
    ...
    if (theChannel.recvVector(dbTag, cTag, dataVec) < 0) {
        opserr <<
            "InitStressMaterial::recvSelf() - failed to get the Vector\n";
        return -3;
    }
    ...
    if (theMaterial->recvSelf(cTag, theChannel, theBroker) < 0) {
        opserr <<
            "InitStressMaterial::recvSelf() - failed to get the Material\n";
        return -4;
    }
    return 0;
}

void InitStressMaterial::Print(OPS_Stream & s, int flag)
{
...
int InitStressMaterial::setParameter(const char **argv, int argc,
                                     Parameter & param)
{
    if (strcmp(argv[0], "sig0") == 0 || strcmp(argv[0], "f0") == 0
        || strcmp(argv[0], "F0") == 0) {
...
int InitStressMaterial::updateParameter(int parameterID,
                                        Information & info)
{
    switch (parameterID) {
    case -1:
...
double
    InitStressMaterial::getStressSensitivity(int gradIndex,
                                             bool conditional)
{
...
int InitStressMaterial::commitSensitivity(double strainGradient,
