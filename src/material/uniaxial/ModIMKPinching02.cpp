...
// $Source: /usr/local/cvs/OpenSees/SRC/material/uniaxial/ModIMKPinching02.cpp,v $
...
//**********************************************************************
//
// Description: This file contains the class implementation for ModIMKPinching02 Model
...
static int numModIMKPinching02Materials = 0;
...
void *OPS_ModIMKPinching02(void)
{
    ...
    if (OPS_GetIntInput(&numData, iData) != 0) {
        opserr << "WARNING invalid uniaxialMaterial  ModIMKPinching02 tag"
            << endln;
        return 0;
    }
    ...
    if (numData != 27 && numData != 26) {
        opserr <<
            "Invalid Args want: uniaxialMaterial ModIMKPinching02 tag? Ke?, alfaPos?, alfaNeg?, My_pos?, My_neg?";
        opserr <<
    ...
    if (numData == 26) {        // Updated: Filipe Ribeiro and Andre Barbosa
        ...
        theMaterial = new ModIMKPinching02(iData[0],
                                           ...
                                           dData[18], dData[19], dData[20],
                                           dData[21], dData[22], dData[23],
                                           dData[24], dData[25]);
        ...
        // Parsing was successful, allocate the material with zero index
        theMaterial = new ModIMKPinching02(iData[0], dData[0], dData[1], dData[2], dData[3], dData[4], dData[5], dData[6], dData[7], dData[8], dData[9], dData[10], dData[11], dData[12], dData[13], dData[14], dData[15], dData[16], dData[17], dData[18], dData[19], dData[20], dData[21], dData[22], dData[23], dData[24], dData[25], dData[26]);    // Updated: Filipe Ribeiro and Andre Barbosa

    }

    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type ModIMKPinching02 Material\n";
        return 0;
    }
...
ModIMKPinching02::ModIMKPinching02(int tag, double ke0, double alfanPos, double alfanNeg, double my_pos, double my_neg, // Updated: Filipe Ribeiro and Andre Barbosa
                                   double fprPos, double fprNeg, double a_pinch, double ls, double ld, double la, double lk, double cs, double cd, double ca, double ck, double thetaPpos, double thetaPneg, double thetaPCpos, double thetaPCneg, double resfacPos, double resfacNeg, double fracDispPos, double fracDispNeg, double dPos, double dNeg, double NFactor)        // Updated: Filipe Ribeiro and Andre Barbosa
:UniaxialMaterial(tag, MAT_TAG_ModIMKPinching02), Ke0(ke0), AlfanPos(alfanPos), AlfanNeg(alfanNeg), My_pos(my_pos), My_neg(my_neg),     // Updated: Filipe Ribeiro and Andre Barbosa
FprPos(fprPos), FprNeg(fprNeg), A_pinch(a_pinch), Ls(ls), Ld(ld), La(la), Lk(lk), Cs(cs), Cd(cd), Ca(ca), Ck(ck), ThetaPpos(thetaPpos), ThetaPneg(thetaPneg), ThetaPCpos(thetaPCpos), ThetaPCneg(thetaPCneg), ResfacPos(resfacPos), ResfacNeg(resfacNeg), FracDispPos(fracDispPos), FracDispNeg(fracDispNeg), DPos(dPos), DNeg(dNeg), nFactor(NFactor)  // Updated: Filipe Ribeiro and Andre Barbosa
{
...
ModIMKPinching02::ModIMKPinching02(int tag, double ke0, double alfanPos, double alfanNeg, double my_pos, double my_neg, // Updated: Filipe Ribeiro and Andre Barbosa
                                   double fprPos, double fprNeg, double a_pinch, double ls, double ld, double la, double lk, double cs, double cd, double ca, double ck, double thetaPpos, double thetaPneg, double thetaPCpos, double thetaPCneg, double resfacPos, double resfacNeg, double fracDispPos, double fracDispNeg, double dPos, double dNeg):UniaxialMaterial(tag, MAT_TAG_ModIMKPinching02), Ke0(ke0), AlfanPos(alfanPos), AlfanNeg(alfanNeg), My_pos(my_pos), My_neg(my_neg),     // Updated: Filipe Ribeiro and Andre Barbosa
FprPos(fprPos),
FprNeg(fprNeg),
A_pinch(a_pinch),
Ls(ls),
...
ThetaPpos(thetaPpos),
ThetaPneg(thetaPneg),
ThetaPCpos(thetaPCpos),
ThetaPCneg(thetaPCneg),
ResfacPos(resfacPos),
ResfacNeg(resfacNeg),
...
ModIMKPinching02::ModIMKPinching02():UniaxialMaterial(0, MAT_TAG_ModIMKPinching02), Ke0(0.0), AlfanPos(0.0),
AlfanNeg(0.0), My_pos(0.0), My_neg(0.0), FprPos(0.0), FprNeg(0.0),
A_pinch(0.0),
    // Updated: Filipe Ribeiro and Andre Barbosa
    Ls(0.0), Ld(0.0), La(0.0), Lk(0.0), Cs(0.0), Cd(0.0), Ca(0.0), Ck(0.0), ThetaPpos(0.0), ThetaPneg(0.0), ThetaPCpos(0.0), ThetaPCneg(0.0), ResfacPos(0.0), ResfacNeg(0.0), FracDispPos(0.0), FracDispNeg(0.0), DPos(0.0), DNeg(0.0), nFactor(0.0)    // Updated: Filipe Ribeiro and Andre Barbosa
{
    // Initialize Variables by Calling revertToStart function
    this->revertToStart();
    Tangent = Ke;
...
ModIMKPinching02::~ModIMKPinching02()
{
    // does nothing
}

int
 ModIMKPinching02::setTrialStrain(double strain, double strainRate)
{
    ...
    if (kon == 0) {
        ...
        fCapRefPos =
            -capSlopePos * Ke * cpPos + (My_pos + ekhardPos * (ThetaPpos));
        fCapRefNeg =
            -capSlopeNeg * Ke * cpNeg + (My_neg +
                                         ekhardNeg * (-ThetaPneg));
...
double ModIMKPinching02::getStress(void)
{

    return (fP);
}

double ModIMKPinching02::getTangent(void)
{
    return (Tangent);
}

double ModIMKPinching02::getInitialTangent(void)
{
    return (Ke);
}

double ModIMKPinching02::getDampTangent(void)
{
    double DTangent = ekP;
    return DTangent;
}
...
double ModIMKPinching02::getStrain(void)
{
    return dP;
}

double ModIMKPinching02::getStrainRate(void)
{
    return 0;
}

int ModIMKPinching02::commitState(void)
{
    commitCalledOnce = 1;
...
int ModIMKPinching02::revertToLastCommit(void)
{
    dP = Cstrain;
    fP = Cstress;
    Tangent = Ctangent;
...
int ModIMKPinching02::revertToStart(void)
{
    commitCalledOnce = 0;
    ...
    fCapRefPos =
        -capSlopePos * Ke * (ThetaPpos + My_pos / Ke) + (My_pos +
                                                         ekhardPos *
                                                         (ThetaPpos));
    fCapRefNeg =
        -capSlopeNeg * Ke * (-ThetaPneg + My_neg / Ke) + (My_neg +
                                                          ekhardNeg *
                                                          (-ThetaPneg));
    ...
    CfCapRefPos =
        -capSlopePos * Ke * cpPos + (My_pos + ekhardPos * (ThetaPpos));
    CfCapRefNeg =
        -capSlopeNeg * Ke * cpNeg + (My_neg + ekhardNeg * (-ThetaPneg));
...
UniaxialMaterial *ModIMKPinching02::getCopy(void)
{
    ModIMKPinching02 *theCopy =
        new ModIMKPinching02(this->getTag(), Ke0, AlfanPos, AlfanNeg,
                             My_pos, My_neg, FprPos, FprNeg, A_pinch, Ls,
                             Ld,
                             La, Lk,
...
int ModIMKPinching02::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
    ...
    if (res < 0)
        opserr << "ModIMKPinching02::sendSelf() - failed to send data\n";

    return res;
}

int ModIMKPinching02::recvSelf(int cTag, Channel & theChannel,
                               FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0) {
        opserr <<
            "ModIMKPinching02::recvSelf() - failed to receive data\n";
        this->setTag(0);
    } else {
...
void ModIMKPinching02::Print(OPS_Stream & s, int flag)
{
...
void ModIMKPinching02::envelPosCap2(double fy, double alphaPos,
...
void ModIMKPinching02::envelNegCap2(double fy, double alphaNeg,
                                    double alphaCap, double cpDsp,
                                    double &d, double &f, double &ek,
