...
ModIMKPinching02::ModIMKPinching02 (int tag, double ke0, double alfanPos, double alfanNeg, double my_pos, double my_neg,        // Updated: Filipe Ribeiro and Andre Barbosa
    ...
    :
UniaxialMaterial (tag, MAT_TAG_ModIMKPinching02),
Ke0 (ke0),
AlfanPos (alfanPos),
AlfanNeg (alfanNeg),
My_pos (my_pos),
My_neg (my_neg),                // Updated: Filipe Ribeiro and Andre Barbosa
FprPos (fprPos),
FprNeg (fprNeg),
A_pinch (a_pinch),
Ls (ls),
...
ResfacPos (resfacPos),
ResfacNeg (resfacNeg),
FracDispPos (fracDispPos),
FracDispNeg (fracDispNeg),
DPos (dPos),
DNeg (dNeg),
nFactor (NFactor)               // Updated: Filipe Ribeiro and Andre Barbosa
{
...
ModIMKPinching02::ModIMKPinching02 (int tag, double ke0, double alfanPos, double alfanNeg, double my_pos, double my_neg,        // Updated: Filipe Ribeiro and Andre Barbosa
                                    double fprPos, double fprNeg,
                                    double a_pinch, double ls, double ld,
                                    double la, double lk, double cs,
                                    double cd, double ca, double ck,
                                    double thetaPpos, double thetaPneg,
                                    double thetaPCpos, double thetaPCneg,
                                    double resfacPos, double resfacNeg,
                                    double fracDispPos, double fracDispNeg,
...
ModIMKPinching02::ModIMKPinching02 ():UniaxialMaterial (0, MAT_TAG_ModIMKPinching02), Ke0 (0.0), AlfanPos (0.0), AlfanNeg (0.0), My_pos (0.0), My_neg (0.0), FprPos (0.0), FprNeg (0.0), A_pinch (0.0),
                                                                                                                                                                        // Updated: Filipe Ribeiro and Andre Barbosa
                ...
                else if (fabs (sn) > 1.e-10)
                  {
                      ...
                      if (Ke > Ke0)
                        {       // Updated: Filipe Ribeiro and Andre Barbosa    
                            ...
                            ekpinch =
                                fpDegPos / (dmaxOrig - fpDegPos / Ke0 - sn);
                ...
                else if (fabs (sp) > 1.e-10)
                  {
                      ...
                      if (Ke > Ke0)
                        {       // Updated: Filipe Ribeiro and Andre Barbosa    
                            ...
                            ekpinch =
                                fpDegNeg / (dminOrig - fpDegNeg / Ke0 - sp);
                            A_pinch_times_dch = A_pinch * dch - fpinch / Ke0;
                        }
                      else
                        {
...
double
ModIMKPinching02::getStress (void)
{
...
double
ModIMKPinching02::getTangent (void)
{
    return (Tangent);
}

double
ModIMKPinching02::getInitialTangent (void)
{
    return (Ke);
}

double
ModIMKPinching02::getDampTangent (void)
{
    double DTangent = ekP;
    return DTangent;
}
...
double
ModIMKPinching02::getStrain (void)
{
    return dP;
}

double
ModIMKPinching02::getStrainRate (void)
{
    return 0;
}

int
ModIMKPinching02::commitState (void)
{
    commitCalledOnce = 1;
...
int
ModIMKPinching02::revertToLastCommit (void)
{
    dP = Cstrain;
    fP = Cstress;
    Tangent = Ctangent;
...
int
ModIMKPinching02::revertToStart (void)
{
    commitCalledOnce = 0;
...
int
ModIMKPinching02::sendSelf (int cTag, Channel & theChannel)
{
    int res = 0;
...
int
ModIMKPinching02::recvSelf (int cTag, Channel & theChannel,
                            FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0)
      {
          opserr << "ModIMKPinching02::recvSelf() - failed to receive data\n";
          this->setTag (0);
      }
    else
      {
...
void
ModIMKPinching02::envelPosCap2 (double fy, double alphaPos, double alphaCap,
                                double cpDsp, double &d, double &f,
                                double &ek, double elstk, double fyieldPos,
                                double Resfac, double fracDisp, int &flagStop)
{
...
void
ModIMKPinching02::envelNegCap2 (double fy, double alphaNeg, double alphaCap,
                                double cpDsp, double &d, double &f,
                                double &ek, double elstk, double fyieldNeg,
                                double Resfac, double fracDisp, int &flagStop)
{
