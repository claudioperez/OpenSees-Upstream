...
HardeningMaterial2::HardeningMaterial2(int tag, double e, double s,
                                       ...
                                       double n):UniaxialMaterial(tag,
                                                                  MAT_TAG_Hardening),
E(e), sigmaY(s), Hiso(hi), Hkin(hk), eta(n), parameterID(0), SHVs(0)
{
    // Initialize variables
    this->revertToStart();
}

HardeningMaterial2::HardeningMaterial2():UniaxialMaterial(0, MAT_TAG_Hardening),
...
double HardeningMaterial2::getStress(void)
{
    return Tstress;
}

double HardeningMaterial2::getTangent(void)
{
    return Ttangent;
}

double HardeningMaterial2::getStrain(void)
{
    return Tstrain;
}

int HardeningMaterial2::commitState(void)
{
...
int HardeningMaterial2::revertToLastCommit(void)
{
    return 0;
}

int HardeningMaterial2::revertToStart(void)
{
...
UniaxialMaterial *HardeningMaterial2::getCopy(void)
{
...
int HardeningMaterial2::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
...
int HardeningMaterial2::recvSelf(int cTag, Channel & theChannel,
                                 FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0) {
        opserr <<
            "HardeningMaterial2::recvSelf() - failed to receive data\n";
        E = 0;
        this->setTag(0);
    } else {
...
int HardeningMaterial2::setParameter(const char **argv, int argc,
                                     Parameter & param)
{
    if (argc < 1)
        return 0;

    if (strcmp(argv[0], "E") == 0 || strcmp(argv[0], "EA") == 0
        || strcmp(argv[0], "EI") == 0) {
        param.setValue(E);
...
int HardeningMaterial2::updateParameter(int parameterID,
                                        Information & info)
{
    switch (parameterID) {
    case -1:
...
int HardeningMaterial2::activateParameter(int paramID)
{
    parameterID = paramID;
...
double HardeningMaterial2::getStressGradient(int gradIndex)
{
    ...
    if (f <= -DBL_EPSILON * E)
        ...
        dsigmadh =
            -E * dPlasticStraindh + dEdh * (Tstrain - CplasticStrain);
    ...
    // Plastic step
        ...
        double dfdh =
            dxidh * n - dsigmaYdh - Hiso * dHardeningdh -
            dHisodh * Chardening;
        ...
        double ddgdh =
            -dg * invEHiHk * (dEdh + dHisodh + dHkindh) + dfdh * invEHiHk;

        // CONDITIONAL derivative of stress
        dsigmadh =
            -E * (dPlasticStraindh + ddgdh * n) + dEdh * (Tstrain -
                                                          CplasticStrain -
                                                          dg * n);
    }
...
int HardeningMaterial2::setStrainGradient(int gradIndex, double depsilondh)
{
    if (SHVs == 0)
        return 0;
...
void HardeningMaterial2::Print(OPS_Stream & s, int flag)
{
...
double
    HardeningMaterial2::getStressSensitivity(int gradIndex,
                                             bool conditional)
{
...
int HardeningMaterial2::commitSensitivity(double strainGradient,
