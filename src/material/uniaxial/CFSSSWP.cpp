...
OPS_CFSSSWP (void)
{
    ...
    theMaterial =
        new CFSSSWP (iData[0], dData[0], dData[1], dData[2], dData[3],
    ...
    if (theMaterial == 0)
      {
          opserr <<
              "WARNING could not create uniaxialMaterial of type CFSSSWP\n";
          return 0;
      }
...
CFSSSWP::CFSSSWP (int tag, double H, int B, double fuf, double fyf,
                  ...
                  double np, double ds, double Vs, double sc, double A,
                  double L):
...
state4Stress (4),
state4Strain (4),
envlpPosDamgdStress (7),
envlpNegDamgdStress (7)
{
...
void
CFSSSWP::lateralShearStrength (void)
{
    Precision = 100;
    double Alpha, Alpha1, Alpha2, Beta, Beta1, Beta2, Beta3, Lambda, Wmax,
        Pns, Pns1, Pns2, Pns3, Pnsed, We, rho, V, V1, V2, Gs, Omega1, Omega2,
        Omega3, Omega4, Delta1, Delta2, Delta3, Delta4, DeltaV, MinPns,
        MinPns1, MinPns2, N, Pn;
    Pns = 0;
    MinPns = 0;
    double mu = 0.3;
    E = 203000.00;
    ...
    Lambda =
        1.736 * (Alpha1 * Alpha2) / (Beta1 * Beta2 * pow (Beta3, 2) * Alpha);
    ...
    V1 = (((We / (2 * screw_Spacing)) * Pn) +
          ((We * width) / (2 * screw_Spacing * hight) * Pn) +
          Vs * (width / (sqrt (pow (hight, 2) + (width * width)))));
    ...
    rho = 0.075 * (ts / 0.457);
    Delta1 =
        (2 * (stress3p / (width * np)) * pow (hight, 3) /
         (3 * E * Af * width));
    Omega1 = screw_Spacing / 152.4;
    Omega2 = 0.838 / tf;
    Delta2 =
        Omega1 * Omega2 * ((stress3p / (width * np)) * hight) / (rho * Gs *
                                                                 ts);
    Omega3 = sqrt (hight / (2 * width));
    Delta3 =
        pow (Omega1,
             (5 / 4)) * Omega2 * Omega3 * Omega4 * pow ((stress3p /
                                                         (width * np) /
                                                         (0.0029 * Beta)), 2);
    ...
    Dy = (stress2p / ke);
    strain2p =
        (stress2p * (strain3p + Dy - 2 * strain4p - strain1p) +
         stress3p * strain4p + stress4p * (strain4p -
                                           strain3p)) / (0.6 * stress3p);
    stress1p = stress1p;
    stress2p = stress2p;
    stress3p = stress3p;
    stress4p = stress4p;
...
int
CFSSSWP::setTrialStrain (double strain, double CstrainRate)
{
    Tstate = Cstate;
    Tenergy = Cenergy;
    Tstrain = strain;
    ...
    elasticStrainEnergy =
        (Tstrain >
         0.0) ? 0.5 * Tstress / kElasticPos * Tstress : 0.5 * Tstress /
        kElasticNeg * Tstress;
...
double
CFSSSWP::getStrain (void)
{
    return Tstrain;
}

double
CFSSSWP::getStress (void)
{
    return Tstress;
}

double
CFSSSWP::getTangent (void)
{
    return Ttangent;
}

double
CFSSSWP::getInitialTangent (void)
{
    return envlpPosStress (0) / envlpPosStrain (0);
}

int
CFSSSWP::commitState (void)
{
    Cstate = Tstate;
...
int
CFSSSWP::revertToLastCommit (void)
{
...
int
CFSSSWP::sendSelf (int commitTag, Channel & theChannel)
{
    return -1;
}

int
CFSSSWP::recvSelf (int commitTag, Channel & theChannel,
...
void
CFSSSWP::Print (OPS_Stream & s, int flag)
{
...
CFSSSWP::SetEnvelope (void)
{
    ...
    envlpPosStress (6) =
        (k1 >
         0.0) ? envlpPosStress (5) + k1 * (envlpPosStrain (6) -
                                           envlpPosStrain (5)) :
        envlpPosStress (5) * 1.1;
    envlpNegStrain (6) = 1e+6 * strain4n;
    envlpNegStress (6) =
        (k2 >
         0.0) ? envlpNegStress (5) + k1 * (envlpNegStrain (6) -
                                           envlpNegStrain (5)) :
        envlpNegStress (5) * 1.1;
    ...
    double *XFit = new double[(Size - 3) * Precision + 2], *YFit =
        new double[(Size - 3) * Precision + 2];
          ...
          for (int j = 0; j < Precision; j++)
            {
                ...
                XFit[i * Precision + j] =
                    ((a[2] + t * (a[1] + t * a[0])) * t + a[3]);
                YFit[i * Precision + j] =
                    ((b[2] + t * (b[1] + t * b[0])) * t + b[3]);
            }
      }

    double XAdvance =
        XFit[Precision * (Size - 3) - 1] - XFit[Precision * (Size - 3) - 2];
    double YAdvance =
        YFit[Precision * (Size - 3) - 1] - YFit[Precision * (Size - 3) - 2];
    double Tangente =
        (YFit[Precision * (Size - 3) - 1] -
         YFit[Precision * (Size - 3) - 2]) / (XFit[Precision * (Size - 3) -
                                                   1] -
                                              XFit[Precision * (Size - 3) -
                                                   2]);
    ...
    XFit[Precision * (Size - 3)] =
        XFit[Precision * (Size - 3) - 1] + (Epsilon -
                                            YFit[Precision * (Size - 3) -
                                                 1]) / Tangente;
...
double
CFSSSWP::GetTangentFromCurve (double Strain)
{
    int i = 0;
    int Neg = 0;
    ...
    if (BSplineXs[i] == Strain)
      {
          return (BSplineYs[i + 1] - BSplineYs[i - 1]) / (BSplineXs[i + 1] -
                                                          BSplineXs[i - 1]);
      }
    else if (i < BSplineXLength - 2 && BSplineXs[i + 1] == Strain)
      {
          return (BSplineYs[i + 2] - BSplineYs[i]) / (BSplineXs[i + 2] -
                                                      BSplineXs[i]);
      }
    else
      {
          return (BSplineYs[i] - BSplineYs[i - 1]) / (BSplineXs[i] -
                                                      BSplineXs[i - 1]);
      }
}

double
CFSSSWP::GetStressFromCurve (double Strain)
{
    int i = 0;
    int Neg = 0;
    if (Strain < 0)
    ...
    if (BSplineXs[i] == Strain)
      ...
      {
          double Stress =
              BSplineYs[i - 1] + (BSplineYs[i] -
                                  BSplineYs[i - 1]) / (BSplineXs[i] -
                                                       BSplineXs[i -
                                                                 1]) *
              (Strain - BSplineXs[i - 1]);
          if (Neg == 1)
              return -Stress;
          return Stress;
      }
...
void
CFSSSWP::getstate (double u, double du)
{
    int cid = 0;
    int cis = 0;
    int newState = 0;
...
double
CFSSSWP::posEnvlpStress (double u)
{
    double k = 0.0;
    int i = 0;
    double f = 0.0;
    ...
    if (k == 0.0)
      {
          k = (envlpPosDamgdStress (6) -
               envlpPosDamgdStress (5)) / (envlpPosStrain (6) -
                                           envlpPosStrain (5));
...
double
CFSSSWP::posEnvlpTangent (double u)
{
    double k = 0.0;
    int i = 0;
    while (k == 0.0 && i <= 5)
...
double
CFSSSWP::negEnvlpStress (double u)
{
    double k = 0.0;
    int i = 0;
    double f = 0.0;
    ...
    if (k == 0.0)
      {
          k = (envlpNegDamgdStress (5) -
               envlpNegDamgdStress (6)) / (envlpNegStrain (5) -
                                           envlpNegStrain (6));
...
double
CFSSSWP::negEnvlpTangent (double u)
{
    double k = 0.0;
    int i = 0;
    while (k == 0.0 && i <= 5)
...
void
CFSSSWP::getState3 (Vector & state3Strain, Vector & state3Stress,
                    double kunload)
{
...
void
CFSSSWP::getState4 (Vector & state4Strain, Vector & state4Stress,
                    double kunload)
{
...
double
CFSSSWP::Envlp3Tangent (Vector s3Strain, Vector s3Stress, double u)
{
    double k = 0.0;
    k = Spline3.EvalT (u);
    if (k != 10e8)
...
double
CFSSSWP::Envlp4Tangent (Vector s4Strain, Vector s4Stress, double u)
{
    double k = 0.0;
    int i = 0;
    k = Spline4.EvalT (u);
...
double
CFSSSWP::Envlp3Stress (Vector s3Strain, Vector s3Stress, double u)
{
    double k = 0.0;
    int i = 0;
    double f = 0.0;
...
double
CFSSSWP::Envlp4Stress (Vector s4Strain, Vector s4Stress, double u)
{
    double k = 0.0;
    int i = 0;
    double f = 0.0;
...
void
CFSSSWP::updateDmg (double strain, double dstrain)
{
    double tes = 0.0;
    double umaxAbs =
    ...
    double uultAbs =
        (envlpPosStrain (1) >
         -envlpNegStrain (1)) ? envlpPosStrain (1) : -envlpNegStrain (1);
    TnCycle = CnCycle;
    if ((strain < uultAbs && strain > -uultAbs)
        && Tenergy < elasticStrainEnergy)
      {
