...
state3Strain(4),
state4Stress(4),
state4Strain(4), envlpPosDamgdStress(7), envlpNegDamgdStress(7)
{
 ...
 CFSSSWP::lateralShearStrength(void)
{
    ...
    double Alpha, Alpha1, Alpha2, Beta, Beta1, Beta2, Beta3, Lambda, Wmax,
        Pns, Pns1, Pns2, Pns3, Pnsed, We, rho, V, V1, V2, Gs, Omega1,
        Omega2, Omega3, Omega4, Delta1, Delta2, Delta3, Delta4, DeltaV,
        MinPns, MinPns1, MinPns2, N, Pn;
    Pns = 0;
    MinPns = 0;
    double mu = 0.3;
    E = 203000.00;
    ...
    Lambda =
        1.736 * (Alpha1 * Alpha2) / (Beta1 * Beta2 * pow(Beta3, 2) *
                                     Alpha);
...
UniaxialMaterial *CFSSSWP::getCopy(void)
{
    CFSSSWP *theCopy = new CFSSSWP(this->getTag(),
                                   ...
                                   fys, ts, np, ds, Vs, screw_Spacing, A,
                                   L);
...
void CFSSSWP::getState3(Vector & state3Strain, Vector & state3Stress,
                        double kunload)
{
...
void CFSSSWP::getState4(Vector & state4Strain, Vector & state4Stress,
                        double kunload)
{
