...
void *OPS_OOHystereticMaterial(void)
{
    ...
    if (OPS_GetNumRemainingInputArgs() < 5) {
        ...
        opserr <<
            "Want: uniaxialMaterial OOHysteretic tag? bTag+? unlRulTag+? stfDegTag+? strDegTag+? "
            <<
    ...
    if (argc > 8) {
        ...
        StiffnessDegradation *negStf =
            OPS_getStiffnessDegradation(stfTagNeg);
        ...
        StrengthDegradation *negStr =
            OPS_getStrengthDegradation(strTagNeg);
        ...
        theMaterial =
            new OOHystereticMaterial(tag, *posBB, *posUnl, *posStf,
    ...
    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type OOHystereticMaterial\n";
        return 0;
    }
...
OOHystereticMaterial::OOHystereticMaterial(int tag,
                                           ...
                                           double px,
                                           double py):UniaxialMaterial(tag,
                                                                       MAT_TAG_OOHysteretic),
theSection(0), pinchX(px), pinchY(py), posEnvelope(0), negEnvelope(0),
E1p(0.0), E1n(0.0), posUnlRule(0), negUnlRule(0), posStfDegr(0),
negStfDegr(0), posStrDegr(0), negStrDegr(0), firstIter(true)
{
    posEnvelope = bb.getCopy();

    if (posEnvelope == 0)
        opserr <<
            "OOHystereticMaterial::OOHystereticMaterial -- failed to get copy of positive backbone"
            << endln;
    ...
    if (negEnvelope == 0)
        opserr <<
            "OOHystereticMaterial::OOHystereticMaterial -- failed to get copy of negative backbone"
            << endln;
    ...
    if (posUnlRule == 0)
        opserr <<
            "OOHystereticMaterial::OOHystereticMaterial -- failed to get copy of unloading rule"
            << endln;
    ...
    if (posStfDegr == 0)
        opserr <<
            "OOHystereticMaterial::OOHystereticMaterial -- failed to get copy of stiffness degradation"
            << endln;
    ...
    if (posStrDegr == 0)
        opserr <<
            "OOHystereticMaterial::OOHystereticMaterial -- failed to get copy of strength degradation"
            << endln;
...
posEnvelope(0), negEnvelope(0), E1p(0.0), E1n(0.0), rot1p(0.0),
rot1n(0.0), posUnlRule(0), negUnlRule(0), posStfDegr(0), negStfDegr(0),
...
OOHystereticMaterial::OOHystereticMaterial(int tag,
                                           ...
                                           s):UniaxialMaterial(tag,
                                                               MAT_TAG_OOHysteretic),
theSection(s), pinchX(px), pinchY(py), posEnvelope(0), negEnvelope(0),
E1p(0.0), E1n(0.0), posUnlRule(0), negUnlRule(0), posStfDegr(0),
negStfDegr(0), posStrDegr(0), negStrDegr(0), firstIter(true)
{
    posEnvelope = posBB.getCopy();
...
double OOHystereticMaterial::getStrain(void)
{
    return Tstrain;
}

double OOHystereticMaterial::getStress(void)
{
    return Tstress;
}

double OOHystereticMaterial::getTangent(void)
{
    return Ttangent;
}

double OOHystereticMaterial::getInitialTangent(void)
{
    return E1p;
}

void OOHystereticMaterial::positiveIncrement(double dStrain)
{
    ...
    double rotlim =
        (E <
         0) ? CrotMin +
        negEnvelope->getStress(-CrotMin) / E : NEG_INF_STRAIN;
...
void OOHystereticMaterial::negativeIncrement(double dStrain)
{
    // Get current degradation values
    double vup = posUnlRule->getValue();
    ...
    double rotlim =
        (E <
         0) ? CrotMax -
        posEnvelope->getStress(CrotMax) / E : POS_INF_STRAIN;
...
int OOHystereticMaterial::commitState(void)
{
    CrotMax = TrotMax;
    CrotMin = TrotMin;
    CtargMax = TtargMax;
...
int OOHystereticMaterial::revertToLastCommit(void)
{
    TrotMax = CrotMax;
    TrotMin = CrotMin;
    TtargMax = CtargMax;
...
int OOHystereticMaterial::revertToStart(void)
{
    CrotMax = 0.0;
    CrotMin = 0.0;
    CtargMax = 0.0;
...
UniaxialMaterial *OOHystereticMaterial::getCopy(void)
{
...
UniaxialMaterial *OOHystereticMaterial::getCopy(SectionForceDeformation *
                                                s)
{
    OOHystereticMaterial *theCopy =
        new OOHystereticMaterial(this->getTag(),
...
int OOHystereticMaterial::setVariable(const char *argv, Information & info)
{
...
int OOHystereticMaterial::getVariable(int varID, Information & info)
{
    switch (varID) {
    case 1:
        info = (Cstrain / rot1p);
    ...
    case 4:
        info =
            posEnvelope->getEnergy(rot1p) + negEnvelope->getEnergy(-rot1n);
        return 0;
    default:
        if (varID >= 100 && theSection != 0) {
            ...
            opserr <<
                "OOHysteretic -- Not calling theSection->getVariable";
            return 0;
        } else
            return -1;
    }
...
int OOHystereticMaterial::sendSelf(int commitTag, Channel & theChannel)
{
    return -1;
}

int OOHystereticMaterial::recvSelf(int commitTag, Channel & theChannel,
                                   FEM_ObjectBroker & theBroker)
{
    return -1;
}

void OOHystereticMaterial::Print(OPS_Stream & s, int flag)
{
    ...
    s << "pos stiffness degradation, tag: " << posStfDegr->
        getTag() << endln;
    s << "neg stiffness degradation, tag: " << negStfDegr->
        getTag() << endln;
    s << "pos strength degradation, tag: " << posStrDegr->
        getTag() << endln;
    s << "neg strength degradation, tag: " << negStrDegr->
        getTag() << endln;
}
