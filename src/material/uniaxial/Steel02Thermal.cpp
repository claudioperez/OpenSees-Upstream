...
void *OPS_Steel02Thermal()
{
    ...
    if (OPS_GetIntInput(&numData, iData) != 0) {
        opserr << "WARNING invalid uniaxialMaterial Steel02Thermal tag" <<
            endln;
        return 0;
    }
    ...
    if (numData == 3) {
        if (OPS_GetDoubleInput(&numData, dData) != 0) {
            opserr << "Invalid arggs: uniaxialMaterial Steel02Thermal " <<
                iData[0] << " fy? E? b? <R0? cR1? cR2? <a1? a2? a3? a4?>>"
                << endln;
            return 0;
        }
        ...
        theMaterial =
            new Steel02Thermal(iData[0], dData[0], dData[1], dData[2]);
    ...
    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type Steel02Thermal Material\n";
        return 0;
    }
...
Steel02Thermal::Steel02Thermal(int tag,
                               ...
                               double sigInit):UniaxialMaterial(tag,
                                                                MAT_TAG_Steel02Thermal),
FyT(_Fy), E0T(_E0), b(_b), R0(_R0), cR1(_cR1), cR2(_cR2), a1(_a1), a2(_a2),
a3(_a3), a4(_a4), sigini(sigInit)
{
...
Steel02Thermal::Steel02Thermal(int tag,
                               double _Fy, double _E0, double _b,
                               double _R0, double _cR1,
                               double _cR2):UniaxialMaterial(tag,
                                                             MAT_TAG_Steel02Thermal),
...
Steel02Thermal::Steel02Thermal(int tag, double _Fy, double _E0,
                               double _b):UniaxialMaterial(tag,
                                                           MAT_TAG_Steel02Thermal),
...
Steel02Thermal::Steel02Thermal(void):UniaxialMaterial(0,
                                                      MAT_TAG_Steel02Thermal)
{
    konP = 0;
}
...
UniaxialMaterial *Steel02Thermal::getCopy(void)
{
    Steel02Thermal *theCopy =
        new Steel02Thermal(this->getTag(), Fy, E0, b, R0, cR1, cR2, a1, a2,
                           a3, a4, sigini);

    return theCopy;
}

double Steel02Thermal::getInitialTangent(void)
{
    return E0;
}

int Steel02Thermal::setTrialStrain(double trialStrain,
...
double Steel02Thermal::getStrain(void)
{
    return eps;
}

double Steel02Thermal::getStress(void)
{
    return sig;
}

double Steel02Thermal::getTangent(void)
{
    return e;
}
...
//JZ 07/10 /////////////////////////////////////////////////////////////start
double Steel02Thermal::getElongTangent(double TempT, double &ET, double &Elong, double TempTmax)        //PK add to include max temp
{
...
int Steel02Thermal::commitState(void)
{
    epsminP = epsmin;
    epsmaxP = epsmax;
    epsplP = epspl;
...
int Steel02Thermal::revertToLastCommit(void)
{
    epsmin = epsminP;
    epsmax = epsmaxP;
    epspl = epsplP;
...
int Steel02Thermal::revertToStart(void)
{
    eP = E0;
    epsP = 0.0;
    sigP = 0.0;
...
int Steel02Thermal::sendSelf(int commitTag, Channel & theChannel)
{
...
int Steel02Thermal::recvSelf(int commitTag, Channel & theChannel,
...
void Steel02Thermal::Print(OPS_Stream & s, int flag)
{
    s << "Steel02Thermal:(strain, stress, tangent) " << eps << " " << sig
        << " " << e << endln;
}


//this function is no use, just for the definiation of pure virtual function.
int Steel02Thermal::setTrialStrain(double strain, double strainRate)
{
    opserr <<
        "Steel02Thermal::setTrialStrain(double strain, strainRate) - should never be called\n";
    return 0;
}


int Steel02Thermal::getVariable(const char *variable, Information & info)
{
