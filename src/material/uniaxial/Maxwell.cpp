...
void *OPS_Maxwell()
{
    ...
    if (OPS_GetDoubleInput(&numData, dData) != 0) {
        opserr <<
            "Invalid Args want: uniaxialMaterial Maxwell tag? K? C? Alpha? Length L?"
            << endln;
        return 0;
    }
    ...
    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type Maxwell Material\n";
        return 0;
    }
...
Maxwell::Maxwell(int tag, double k, double c, double a, double l,
                 int retD):UniaxialMaterial(tag, MAT_TAG_Maxwell), K(k),
C(c), Alpha(a), L(l), returnD(retD)
{
    if (Alpha < 0.0) {
...
Maxwell::Maxwell():UniaxialMaterial(0, MAT_TAG_Maxwell),
...
double Maxwell::getStress(void)
{

    return Tstress;
}

double Maxwell::getTangent(void)
{
    return K;
}

double Maxwell::getInitialTangent(void)
{
    return K;
}

double Maxwell::getDampTangent(void)
{
    if (returnD == 1) {
        double DTangent = Tstress / Tstrain;
        return DTangent;
...
double Maxwell::getStrain(void)
{
    return Tstrain;
}

double Maxwell::getStrainRate(void)
{
    return 0;
}

int Maxwell::commitState(void)
{
...
int Maxwell::revertToLastCommit(void)
{
    Tstrain = Cstrain;
    Tstress = Cstress;
    Ttangent = Ctangent;
...
int Maxwell::revertToStart(void)
{
...
UniaxialMaterial *Maxwell::getCopy(void)
{
    Maxwell *theCopy =
        new Maxwell(this->getTag(), K, C, Alpha, L, returnD);
...
int Maxwell::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
...
int Maxwell::recvSelf(int cTag, Channel & theChannel,
...
void Maxwell::Print(OPS_Stream & s, int flag)
{
