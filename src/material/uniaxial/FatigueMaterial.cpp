...
FatigueMaterial::FatigueMaterial(int tag, UniaxialMaterial & material,
                                 ...
                                 double epsmin,
                                 double epsmax):UniaxialMaterial(tag,
                                                                 MAT_TAG_Fatigue),
theMaterial(0), Cfailed(false), trialStrain(0)
{
    DI = 0;                     //Damage index
...
FatigueMaterial::FatigueMaterial():UniaxialMaterial(0, MAT_TAG_Fatigue), theMaterial(0),
...
int FatigueMaterial::setTrialStrain(double strain, double strainRate)
{
    if (Cfailed) {
        trialStrain = strain;
        // return 0;
...
double FatigueMaterial::getStress(void)
{
    double modifier = 1.0;
...
double FatigueMaterial::getTangent(void)
{
    double modifier = 1.0;
    double damageloc = 1.0 - Dmax + DL;
    if (Cfailed)
...
double FatigueMaterial::getDampTangent(void)
{
    if (Cfailed)
        return 0.0;
    else
...
double FatigueMaterial::getStrain(void)
{
    return theMaterial->getStrain();
}

double FatigueMaterial::getStrainRate(void)
{
    return theMaterial->getStrainRate();
}

int FatigueMaterial::commitState(void)
{
        ...
        if (B == 0 && C == 0 && D == 0) {
            ...
            if (fabs(A - trialStrain) > fabs(A - B)) {
                ...
                if (fabs(C - trialStrain) < 1e-10) {
                    ...
                    DL = DI +
                        1.0 /
                        fabs(pow((fabs(C - trialStrain) / E0), 1 / m));
                    // added 6/9/2006
                    //values for recorder
...
int FatigueMaterial::revertToLastCommit(void)
{
...
int FatigueMaterial::revertToStart(void)
{
...
UniaxialMaterial *FatigueMaterial::getCopy(void)
{
    FatigueMaterial *theCopy =
        new FatigueMaterial(this->getTag(), *theMaterial, Dmax, E0, m,
                            minStrain, maxStrain);
...
int FatigueMaterial::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (theChannel.sendVector(dbTag, cTag, dataVec) < 0) {
        opserr <<
            "FatigueMaterial::sendSelf() - failed to send the Vector\n";
        return -2;
    }

    if (theMaterial->sendSelf(cTag, theChannel) < 0) {
        opserr <<
            "FatigueMaterial::sendSelf() - failed to send the Material\n";
        return -3;
    }
...
int FatigueMaterial::recvSelf(int cTag, Channel & theChannel,
                              FEM_ObjectBroker & theBroker)
{
    ...
    if (theMaterial == 0) {
        ...
        if (theMaterial == 0) {
            opserr <<
                "FatigueMaterial::recvSelf() - failed to create Material with classTag "
                << dataID(0) << endln;
            return -2;
        }
    }
    ...
    if (theChannel.recvVector(dbTag, cTag, dataVec) < 0) {
        opserr <<
            "FatigueMaterial::recvSelf() - failed to get the Vector\n";
        return -3;
    }
    ...
    if (theMaterial->recvSelf(cTag, theChannel, theBroker) < 0) {
        opserr <<
            "FatigueMaterial::recvSelf() - failed to get the Material\n";
        return -4;
    }
    return 0;
}
...
void FatigueMaterial::Print(OPS_Stream & s, int flag)
{
    if (flag == 100) {
        s << DL << endln;
    }
...
Response *FatigueMaterial::setResponse(const char **argv, int argc,
...
int FatigueMaterial::getResponse(int responseID, Information & matInfo)
{
...
bool FatigueMaterial::hasFailed(void)
{
    return Cfailed;
}
