...
OPS_ViscousDamper (void)
{
    ...
    if (theMaterial == 0)
      {
          opserr <<
              "WARNING could not create uniaxialMaterial of type ViscousDamper Material\n";
          return 0;
      }
...
ViscousDamper::ViscousDamper (int tag, double k, double c, double a,
                              double lgap, double nm, double reltol,
                              double abstol, double maxhalf):
UniaxialMaterial (tag, MAT_TAG_ViscousDamper),
K (k),
C (c),
Alpha (a),
LGap (lgap),
...
MaxHalf (maxhalf)
{
    if (Alpha < 0.0)
      {
          opserr <<
              "ViscousDamper::ViscousDamper -- Alpha < 0.0, setting to 1.0\n";
          Alpha = 1.0;
      }
...
ViscousDamper::ViscousDamper ():UniaxialMaterial (0, MAT_TAG_ViscousDamper),
K (0.0), C (0.0), Alpha (0.0), LGap (0.0), NM (0.0), RelTol (0.0),
...
double
ViscousDamper::getStress (void)
{
    return Tstress;
}

double
ViscousDamper::getTangent (void)
{
    return 0;
}

double
ViscousDamper::getInitialTangent (void)
{
    return 0;
}

double
ViscousDamper::getDampTangent (void)
{
...
int
ViscousDamper::DormandPrince (double vel0, double vel1, double y0, double h,
                              double &yt, double &eps, double &error)
{
...
int
ViscousDamper::ABM6 (double vel0, double vel1, double y0, double h,
                     double &y6, double &eps, double &error)
{
...
int
ViscousDamper::ROS (double vel0, double vel1, double y0, double h, double &y2,
                    double &eps, double &error)
{
...
{
    ViscousDamper *theCopy =
        new ViscousDamper (this->getTag (), K, C, Alpha, LGap, NM, RelTol,
                           AbsTol, MaxHalf);
