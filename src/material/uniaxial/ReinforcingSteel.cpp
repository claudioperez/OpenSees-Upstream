...
void *OPS_ReinforcingSteel()
{
    ...
    while (OPS_GetNumRemainingInputArgs() > 0) {
        ...
        if (strcmp(type, "-GABuck") == 0) {
            ...
            if (numdata < 4) {
                opserr <<
                    "WARNING insufficient optional arguments for -GABuck\n";
            ...
            if (numdata < 2) {
                opserr <<
                    "WARNING insufficient optional arguments for -DMBuck\n";
            ...
            if (dmbuckdata[1] < 0.75 || dmbuckdata[1] > 1.0) {
                opserr <<
                    "WARNING alpha usually is between 0.75 and 1.0\n";
                return 0;
            }
            ...
            if (numdata < 3) {
                opserr <<
                    "WARNING insufficient optional arguments for -CMFatigue\n";
            ...
            if (numdata < 2) {
                opserr <<
                    "WARNING insufficient optional arguments for -IsoHard\n";
    ...
    theMaterial =
        new ReinforcingSteel(tag, data[0], data[1], data[2], data[3],
                             data[4], data[5], buckModel, slen, beta,
                             gabuckdata[2], gabuckdata[3], fatiguedata[0],
                             fatiguedata[1], fatiguedata[2], mpdata[0],
                             mpdata[1], mpdata[2], isodata[0], isodata[1]);

    if (theMaterial == 0) {
        opserr <<
            "WARNING could not create uniaxialMaterial of type ReinforcingSteel\n";
        return 0;
    }
...
ReinforcingSteel::ReinforcingSteel(int tag, double fy_, double fsu_,
                                   double Es_, double Esh_, double esh_,
                                   double esu_, int buckModel,
                                   double slenderness, double alpha,
                                   double r, double gama, double Fatigue1,
                                   double Fatigue2, double Degrade,
                                   double rc1, double rc2, double rc3,
                                   double A1,
                                   double HardLim):UniaxialMaterial(tag,
                                                                    MAT_TAG_ReinforcingSteel),
fy(fy_), fsu(fsu_), Es(Es_), Esh(Esh_), esh(esh_), esu(esu_), a1(A1),
hardLim(HardLim), BuckleModel(buckModel), LDratio(slenderness),
beta(alpha), fsu_fraction(gama), Fat1(Fatigue1), RC1(rc1), RC2(rc2),
RC3(rc3)
{
    if ((r >= 0.0) & (r <= 1.0))
        reduction = r;
...
void ReinforcingSteel::updateHardeningLoactionParams()
{
...
ReinforcingSteel::ReinforcingSteel(int tag):UniaxialMaterial(tag,
                                                             MAT_TAG_ReinforcingSteel)
{
#ifdef HelpDebugMat
...
#ifdef _WIN32
    if (_fpclass(Tfch) < 8 || _fpclass(Tfch) == 512 || _fpclass(Tfch) < 8
        || _fpclass(Tfch) == 512) {
...
#ifdef _WIN32
    if (_fpclass(TStress) < 8 || _fpclass(TStress) == 512
        || _fpclass(TTangent) < 8 || _fpclass(TTangent) == 512) {
        opserr << "bad stress or tangent\n";
        return -1;
    }
#endif
...
double ReinforcingSteel::getStrain(void)
{
    return exp(TStrain) - 1.0;
}

double ReinforcingSteel::getStress(void)
{
    if (theBarFailed)
        return 0.0;
    ...
    switch (BuckleModel) {
    case 1:
        tempstr = Buckled_stress_Gomes(TStrain, TStress);
        break;
    case 2:
        tempstr = Buckled_stress_Dhakal(TStrain, TStress);
        break;
    }
...
double ReinforcingSteel::getTangent(void)
{
    double taTan = TTangent;
    switch (BuckleModel) {
    case 1:
        taTan = Buckled_mod_Gomes(TStrain, TStress, TTangent);
        break;
    case 2:
        taTan = Buckled_mod_Dhakal(TStrain, TStress, TTangent);
        break;
    }
...
double ReinforcingSteel::getInitialTangent(void)
{
    return Esp;
}

/***************** path dependent behavior methods ***********/
int ReinforcingSteel::commitState(void)
{
#ifdef HelpDebugMat
    thisClassCommit++;
    thisClassStep = 0;
...
int ReinforcingSteel::revertToLastCommit(void)
{
    for (int i = 0; i <= LastRule_RS / 2; i++)
...
int ReinforcingSteel::revertToStart(void)
{
...
UniaxialMaterial *ReinforcingSteel::getCopy(void)
{
    ReinforcingSteel *theCopy = new ReinforcingSteel(this->getTag());
...
int ReinforcingSteel::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
    int index = 0;
...
int ReinforcingSteel::recvSelf(int cTag, Channel & theChannel,
                               FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0) {
        opserr <<
            "ReinforcingSteel::recvSelf() - failed to receive data\n";
        this->setTag(0);
    } else {
...
void ReinforcingSteel::Print(OPS_Stream & s, int flag)
{
...
int ReinforcingSteel::Sign(double x)
{
    if (x < 0.0)
        return -1;
    else
...
/*****************************************************************************************/
double ReinforcingSteel::MPfunc(double a)
{
    if (a >= 1.0)
    ...
    return TEb - TEsec * (1 - pow(a, TR + 1)) / (1 - a) + TEa * a * (1 -
                                                                     pow(a,
                                                                         TR))
        / (1 - a);
}

int ReinforcingSteel::SetMP()
{
    double Rmin;
    double a = 0.01;
    double ao, ao_last;
    ...
    if (TEb - TEsec == 0.0) {
        ...
        if (TEsec != TEa) {
            ...
            if (numIter >= maxIter) {
                opserr <<
                    "WARNING: ReinforcingSteel::SetMP() - did not converge finding a\n";
                return -1;
            }
            ...
            if (numIter >= maxIter) {
                opserr <<
                    "WARNING: ReinforcingSteel::SetMP() - did not converge finding ao\n";
                return -2;
            }
            if (ao >= 1.0)
                ao = 0.999999;
            ...
            if (numIter >= maxIter) {
                ...
                da = da / 100.0;
                ao = ao_last;
                ao = ao - 2 * MPfunc(ao) * da / (MPfunc(ao + da) -
                                                 MPfunc(ao - da));
                return -3;
            }
            if (ao > 0.99999999)
...
double ReinforcingSteel::MP_f(double e)
{
    return Tfa + TEa * (e - Tea) * (TQ -
                                    (TQ -
                                     1.0) /
                                    pow(pow
                                        (fabs
...
double ReinforcingSteel::MP_E(double e)
{
    if (TR > 100.0 || e == Tea) {
        return TEa;
    } else {
        ...
        return Esec - (Esec -
                       TQ * TEa) /
            (pow(fabs(TEa * (e - Tea) / (Tfch - Tfa)), -TR) + 1.0);
    }
}

void ReinforcingSteel::SetTRp(void)
{
    TR = pow(fyp / Esp, RC1) * RC2 * (1.0 - RC3 * (Teb - Tea));
}

void ReinforcingSteel::SetTRn(void)
{
    TR = pow(fyp / Esp, RC1) * RC2 * (1.0 - RC3 * (Tea - Teb));
}

void ReinforcingSteel::SetTRp1(void)
{
    TR = pow(fyp / Esp, RC1) * RC2 * (1.0 - RC3 * (Teb - Tea));
}

void ReinforcingSteel::SetTRn1(void)
{
    TR = pow(fyp / Esp, RC1) * RC2 * (1.0 - RC3 * (Tea - Teb));
}

void ReinforcingSteel::SetPastCurve(int branch)
{
    if (branch == 1)
        TBranchMem = 0;
    else
...
/*****************************************************************************************/
double ReinforcingSteel::Backbone_f(double e)
{
    if (e < 0.0)
...
double ReinforcingSteel::Backbone_fNat(double essp)
{
    if (essp > eshpa) {
        if (essp > esup)
            ...
            if (essp < eshp + 0.0002)
                return (Eshpb - Eypp) * pow(essp - eshpa,
                                            2.0) / (2 * (eshp + 0.0002 -
                                                         eshpa)) +
                    essp * Eypp + fint;
            else
                return fshp + (essp - eshp) * Esup + (fsup -
                                                      fshp) * (1.0 -
                                                               pow((esup
                                                                    - essp)
                                                                   /
        ...
        return essp * ((Esp - Eypp) /
                       pow(1 + pow((Esp - Eypp) * essp / fint, 10.0),
                           0.1) + Eypp);
}

double ReinforcingSteel::Backbone_E(double e)
{
    double essp = fabs(e);

    if (essp <= eshpa)
        return (Esp - Eypp) / pow(1.0 +
                                  pow((Esp - Eypp) * essp / fint, 10.0),
                                  1.1) + Eypp;
    else {
        if (essp > esup)
            return Esup;
            ...
            if (essp < eshp + 0.0002)
                return (Eshpb - Eypp) * (essp - eshpa) / (eshp + 0.0002 -
                                                          eshpa) + Eypp;
            else {
...
double ReinforcingSteel::Buckled_stress_Dhakal(double ess, double fss)
{
    if (LDratio <= 0.0)
        return fss;
    ...
    if (e < -eyp) {
        ...
        double fStar =
            fStarL * beta * (1.1 -
                             0.016 * sqrt(fyp / Esp * 2000) * LDratio);
        if (fStar > -0.2 * fyp)
            fStar = -0.2 * fyp;
        if (TBranchNum % 4 > 1) {
            if ((e < -eyp) && (e >= eStar)) {
                aveStress =
                    fss * (1.0 -
                           (1.0 - fStar / fStarL) * (e + eyp) / (eStar +
                                                                 eyp));
            } else if (e < eStar) {
                aveStress =
                    fss * (fStar - 0.02 * Esp * (e - eStar)) / fStarL;
            ...
            if ((e < -eyp) && (e >= eStar)) {
                ...
                aveStress =
                    Tfa * (fStar - 0.02 * Esp * (e - eStar)) / fStarL;
                if (aveStress > -0.2 * fyp)
                    aveStress = -0.2 * fyp;
            }
            return BackStress - (BackStress - fss) * (BackStress -
                                                      aveStress) /
                (BackStress - Tfa);
...
double ReinforcingSteel::Buckled_stress_Gomes(double ess, double fss)
{
    if (LDratio <= 0.0)
        return fss;
    ...
    double fs_buck =
        beta * sqrt(32.0 / (e_cross - ess)) / (9.42477796076938 * LDratio);
    double stress_diff = fabs(fs_buck - 1.0);
    if (stress_diff <= Dft)
        beta = 1 - gama * (Dft - stress_diff) / Dft;

    //double factor = ((1.0>fs_buck)?fs_buck:1.0)*beta + reduction)/(1.0+reduction);
    double factor =
        ((1.0 >
          fs_buck) ? fs_buck : 1.0) * beta * (1 - reduction) + reduction;

    double t_s_out =
        fsup * fsu_fraction - (factor +
                               fsu_fraction) * (fsup * fsu_fraction -
                                                fss) / (1.0 +
                                                        fsu_fraction);
    return t_s_out;
}
...
{
    double Etmp =
        Ess + (Buckled_stress_Gomes(ess + 0.00005, fss) -
               Buckled_stress_Gomes(ess - 0.00005, fss)) / 0.0001;
    return Etmp;
}
...
{
    double Etmp =
        Ess + (Buckled_stress_Dhakal(ess + 0.00005, fss) -
               Buckled_stress_Dhakal(ess - 0.00005, fss)) / 0.0001;
    return Etmp;
}
...
int ReinforcingSteel::BranchDriver(int res)
{
    switch (TBranchNum) {
    case 1:
        res += Rule1(res);
...
int ReinforcingSteel::Rule1(int res)
{
    ...
    if (TStrain - CStrain < 0.0) {
        if (strain - eshp > -ZeroTol) {
            ...
            updateHardeningLoaction(TeCumPlastic + Tea - emin -
                                    (Tfa - Backbone_f(emin)) / Esp);
            Tfb = Backbone_f(emin);
            TEb = Backbone_E(emin);
            ...
            if (TEsec < TEb) {
                ...
                TEsec = (Tfb - Tfa) / (Teb - Tea);
                opserr <<
                    "Adjusted Compressive Curve anchor in ReinforcingSteel::Rule1()\n";
            }
        ...
        T_ePlastic[0] =
            getPlasticStrain(TStrain - TeAbsMin, TStress - Cfa[1]);
        TFatDamage += damage(T_ePlastic[0]);
        TeCumPlastic += T_ePlastic[0];
        //}
    }
...
int ReinforcingSteel::Rule2(int res)
{
    ...
    if (TStrain - CStrain > 0.0) {
        if (strain + eshp < ZeroTol) {
            ...
            updateHardeningLoaction(TeCumPlastic + emax - Tea -
                                    (Backbone_f(emax) - Tfa) / Esp);
            Tfb = Backbone_f(emax);
            TEb = Backbone_E(emax);
        ...
        T_ePlastic[1] =
            getPlasticStrain(TeAbsMax - TStrain, Cfa[0] - TStress);
        TFatDamage += damage(T_ePlastic[1]);
        TeCumPlastic += T_ePlastic[1];
        //}
    }
...
int ReinforcingSteel::Rule3(int res)
{
    if (TStrain - CStrain > 0.0) {      // reversal from branch 
        if (Temin > CStrain - Teo_n)
            Temin = CStrain - Teo_n;
        ...
        updateHardeningLoaction(TeCumPlastic + Teb - Tea -
                                (Backbone_f(Teb - Teo_p) - Tfa) / Esp);
        ...
        if (TStrain - Teb <= ZeroTol) {
            ...
            T_ePlastic[2] =
                getPlasticStrain(TeAbsMax - TStrain, Tfa - TStress);
            TFatDamage += damage(T_ePlastic[2]);
            TeCumPlastic += T_ePlastic[2];
        }
    }
...
int ReinforcingSteel::Rule4(int res)
{
    if (TStrain - CStrain < 0.0) {
        if (Temax < CStrain - Teo_p)
            Temax = CStrain - Teo_p;
        ...
        updateHardeningLoaction(TeCumPlastic + Tea - Teb -
                                (Tfa - Backbone_f(Teb - Teo_n)) / Esp);
        ...
        if (TStrain - Teb >= -ZeroTol) {
            ...
            TeCumPlastic -= T_ePlastic[2];
            T_ePlastic[2] =
                getPlasticStrain(TStrain - TeAbsMin, TStress - Tfa);
            TFatDamage += damage(T_ePlastic[2]);
            TeCumPlastic += T_ePlastic[2];
        }
    }
...
int ReinforcingSteel::Rule5(int res)
{
    if (TStrain - CStrain < 0.0) {
        rE1 = 0.0;
        rE2 = 0.0;
        ...
        Tea =
            Ceb[3] * (CStrain - Cea[3]) / (Ceb[3] - Cea[3]) +
            Cea[2] * (Ceb[3] - CStrain) / (Ceb[3] - Cea[3]);
        Teb = Ceb[2];

        updateHardeningLoaction(TeCumPlastic + CStrain - Tea +
                                (Backbone_f(Tea - Teo_p) - CStress) / Esp);
        Tfa = Backbone_f(Tea - Teo_p);
        TEa = CEa[2];

        updateHardeningLoaction(TeCumPlastic + CStrain - Teb -
                                (CStress - Backbone_f(Teb - Teo_n)) / Esp);
        Tfb = Backbone_f(Teb - Teo_n);
        TEb = Backbone_E(Teb - Teo_n);
...
int ReinforcingSteel::Rule6(int res)
{
    if (TStrain - CStrain > 0.0) {
        rE1 = 0.0;
        rE2 = 0.0;
        ...
        updateHardeningLoaction(TeCumPlastic + Tea - CStrain +
                                (CStress - Backbone_f(Tea - Teo_n)) / Esp);
        Tfa = Backbone_f(Tea - Teo_n);
        TEa = CEa[2];

        updateHardeningLoaction(TeCumPlastic + Teb - CStrain -
                                (Backbone_f(Teb - Teo_p) - CStress) / Esp);
        Tfb = Backbone_f(Teb - Teo_p);
        TEb = Backbone_E(Teb - Teo_p);
...
int ReinforcingSteel::Rule7(int res)
{
    if (TStrain - CStrain > 0.0) {
        ...
        if (TStrain - Teb <= ZeroTol) {
            ...
            Tea =
                Ceb[3] * (Tea - Cea[3]) / (Ceb[3] - Cea[3]) +
                Cea[2] * (Ceb[3] - Tea) / (Ceb[3] - Cea[3]);
            Teb = Ceb[2];

            updateHardeningLoaction(TeCumPlastic + tempTeb - Tea +
                                    (Backbone_f(Tea - Teo_p) - Tfb) / Esp);
            Tfa = Backbone_f(Tea - Teo_p);
            TEa = CEa[2];

            updateHardeningLoaction(TeCumPlastic + tempTeb - Teb -
                                    (Tfb - Backbone_f(Teb - Teo_n)) / Esp);
            Tfb = Backbone_f(Teb - Teo_n);
            TEb = Backbone_E(Teb - Teo_n);
...
int ReinforcingSteel::Rule8(int res)
{
    if (TStrain - CStrain < 0.0) {
        SetPastCurve(TBranchNum - 2);
        ...
        if (TStrain - Teb >= -ZeroTol) {
            ...
            updateHardeningLoaction(TeCumPlastic + Tea - tempTeb +
                                    (Tfb - Backbone_f(Tea - Teo_n)) / Esp);
            Tfa = Backbone_f(Tea - Teo_n);
            TEa = CEa[2];

            updateHardeningLoaction(TeCumPlastic + Teb - tempTeb -
                                    (Backbone_f(Teb - Teo_p) - Tfb) / Esp);
            Tfb = Backbone_f(Teb - Teo_p);
            TEb = Backbone_E(Teb - Teo_p);
...
int ReinforcingSteel::Rule9(int res)
{
    if (TStrain - CStrain < 0.0) {
        double eb = Tea;
        ...
        if (TStrain - Teb >= -ZeroTol) {
            ...
            TFatDamage -= damage(T_ePlastic[TBranchMem]);
            TeCumPlastic -= T_ePlastic[TBranchMem];
            T_ePlastic[TBranchMem] =
                getPlasticStrain(TStrain - Tea, TStress - Tfa);
            TFatDamage += damage(T_ePlastic[TBranchMem]);
            TeCumPlastic += T_ePlastic[TBranchMem];
        }
    }
...
int ReinforcingSteel::Rule10(int res)
{
    if (TStrain - CStrain > 0.0) {
        double eb = Tea;
        if (TBranchNum + 4 <= LastRule_RS)
        ...
        if (TStrain - Teb <= ZeroTol) {
            ...
            TeCumPlastic -= T_ePlastic[TBranchMem];
            T_ePlastic[TBranchMem] =
                getPlasticStrain(Tea - TStrain, Tfa - TStress);
            TFatDamage += damage(T_ePlastic[TBranchMem]);
            TeCumPlastic += T_ePlastic[TBranchMem];
        }
    }
...
int ReinforcingSteel::Rule11(int res)
{
    if (TStrain - CStrain > 0.0) {
        // reset past curve
        double eb = Tea;
...
int ReinforcingSteel::Rule12(int res)
{
    if (TStrain - CStrain < 0.0) {
        // reset past curve
        double eb = Tea;
...
/*****************************************************************************************/
double ReinforcingSteel::getPlasticStrain(double ehalf, double stressAmp)
{
...
double ReinforcingSteel::damage(double ehalfPlastic)
{
    return pow(ehalfPlastic / Fat1, Fat2);
}

double ReinforcingSteel::scalefactor()
{
    if (theBarFailed)
        return 0.0;
...
double ReinforcingSteel::ReturnSlope(double dea)
{
