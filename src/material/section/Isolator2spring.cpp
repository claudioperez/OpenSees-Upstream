...
Isolator2spring::Isolator2spring
    (int tag, double tol_in, double k1_in, double Fy_in, double kb_in,
     double kvo_in, double hb_in, double Pe_in,
     double po_in):SectionForceDeformation(tag, SEC_TAG_Isolator2spring),
tol(tol_in), k1(k1_in), Fyo(Fy_in), kbo(kb_in), kvo(kvo_in), h(hb_in),
Pe(Pe_in), po(po_in), x0(5), ks(3, 3)
{
    this->revertToStart();
...
tol(1.0e-12), k1(0.0), Fyo(0.0), kbo(0.0), kvo(0.0), h(0.0), Pe(0.0),
po(0.0)
{
...
const Matrix & Isolator2spring::getSectionTangent(void)
{
...
const Matrix & Isolator2spring::getInitialTangent(void)
{
...
const Vector & Isolator2spring::getStressResultant(void)
{
    ...
    while (normf0 > tol) {
        ...
        if (iter > 19) {
            opserr <<
                "WARNING! Iso2spring: Newton iteration failed. Norm Resid: "
                << normf0 << endln;
            break;
        }
    }
...
const Vector & Isolator2spring::getSectionDeformation(void)
{
...
int Isolator2spring::commitState(void)
{
    sP_n = sP_n1;
    q_n = q_n1;
...
int Isolator2spring::revertToLastCommit(void)
{
    return 0;
}

int Isolator2spring::revertToStart(void)
{
    for (int i = 0; i < 2; i++) {
        utpt[i] = 0.0;
    }
...
SectionForceDeformation *Isolator2spring::getCopy(void)
{
    Isolator2spring *theCopy =
        new Isolator2spring(this->getTag(), tol, k1, Fyo, kbo, kvo, h, Pe,
                            po);
...
const ID & Isolator2spring::getType(void)
{
    return code;
}

int Isolator2spring::getOrder(void) const
{
    return 3;
}

int Isolator2spring::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
...
int Isolator2spring::recvSelf(int cTag, Channel & theChannel,
...
void Isolator2spring::Print(OPS_Stream & s, int flag)
{
