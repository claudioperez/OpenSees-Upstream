...
OPS_ElasticWarpingShearSection2d ()
{
    ...
    if (OPS_GetIntInput (&numdata, &tag) < 0)
      {
          opserr << "WARNING invalid section ElasticWarpingShearSection2d tag"
              << endln;
          return 0;
      }
...
Vector
ElasticWarpingShearSection2d::s (5);
Matrix
ElasticWarpingShearSection2d::ks (5, 5);
ID
ElasticWarpingShearSection2d::code (5);
...
ElasticWarpingShearSection2d::ElasticWarpingShearSection2d
    (int tag, double E_in, double A_in, double I_in, double G_in,
     double alpha_in, double J_in, double B_in, double C_in):
SectionForceDeformation (tag, SEC_TAG_ElasticWarpingShear2d), E (E_in),
A (A_in), I (I_in), G (G_in), alpha (alpha_in), J (J_in), B (B_in), C (C_in),
e (5), eCommit (5),
parameterID (0)
{
...
ElasticWarpingShearSection2d::Print (OPS_Stream & s, int flag)
{
    if (flag == OPS_PRINT_PRINTMODEL_SECTION)
      {
          s << "ElasticWarpingShearSection2d, tag: " << this->
              getTag () << endln;
          s << "\tE: " << E << endln;
          s << "\tA: " << A << endln;
          s << "\tI: " << I << endln;
...
int
ElasticWarpingShearSection2d::updateParameter (int paramID,
                                               Information & info)
{
    if (paramID == 1)
        E = info.theDouble;
