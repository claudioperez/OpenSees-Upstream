...
WFFiberSection2d::WFFiberSection2d(int tag, UniaxialMaterial & theMat,
                                   double D, double Tw, double Bf,
                                   double Tf, int Nfdw,
                                   int Nftf):FiberSection2d(), d(D),
tw(Tw), bf(Bf), tf(Tf), nfdw(Nfdw), nftf(Nftf)
{
    ...
    for (int i = 0; i < numFibers; i++) {
        ...
        if (theMaterials[i] == 0)
            opserr <<
                "WFFiberSection2d::WFFiberSection2d -- failed to get copy of beam fiber"
                << endln;
    }
...
FiberSection2d(), d(0.0), tw(0.0), bf(0.0), tf(0.0), nfdw(0), nftf(0)
{
...
SectionForceDeformation *WFFiberSection2d::getCopy(void)
{
...
int WFFiberSection2d::sendSelf(int commitTag, Channel & theChannel)
{
    return -1;
}

int WFFiberSection2d::recvSelf(int commitTag, Channel & theChannel,
                               FEM_ObjectBroker & theBroker)
{
    return -1;
}

void WFFiberSection2d::Print(OPS_Stream & s, int flag)
{
                                                    ...
                                                    bool conditional)
{
    ...
    ds = FiberSection2d::getStressResultantSensitivity(gradIndex,
                                                       conditional);
