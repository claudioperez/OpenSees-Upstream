...
double
 ParallelSection::workArea[2 * maxOrder * (maxOrder + 1)];
int
 ParallelSection::codeArea[maxOrder];
...
void *OPS_ParallelSection()
{
    ...
    SectionForceDeformation **theMats =
        new SectionForceDeformation *[numMaterials];
    ...
    for (int i = 0; i < numMaterials; i++) {
        ...
        SectionForceDeformation *theMat =
            OPS_getSectionForceDeformation(tagI);
    ...
    SectionForceDeformation *theSection =
        new ParallelSection(tag, numMaterials, theMats);
...
ParallelSection::ParallelSection(int tag, int numSecs,
                                 ...
                                 theSecs):SectionForceDeformation(tag,
                                                                  SEC_TAG_Parallel),
numSections(numSecs), theSections(0), e(0), s(0), ks(0), fs(0), order(0),
theCode(0), otherDbTag(0)
{
    if (theSecs == 0) {
        opserr <<
            "ParallelSection::ParallelSection -- null section array passed\n";
        exit(-1);
    }
    ...
    if (theSections == 0) {
        opserr <<
            "ParallelSection::ParallelSection -- failed to allocate pointers\n";
        exit(-1);
    }
    ...
    for (int i = 0; i < numSections; i++) {
        if (theSecs[i] == 0) {
            opserr <<
                "ParallelSection::ParallelSection -- null section pointer passed\n";
            exit(-1);
        }
        ...
        if (theSections[i] == 0) {
            opserr <<
                "ParallelSection::ParallelSection -- failed to copy section\n";
            exit(-1);
        }
    }
    ...
    if (order > maxOrder) {
        opserr <<
            "ParallelSection::ParallelSection -- order too big, need to modify the #define in ParallelSection.cpp to "
            << order << endln;
        exit(-1);
    }
...
e(0), s(0), ks(0), fs(0), order(0), theCode(0), otherDbTag(0)
{
...
const Vector & ParallelSection::getSectionDeformation(void)
{
    return *e;
}

const Matrix & ParallelSection::getSectionTangent(void)
{
    ks->Zero();
...
const Matrix & ParallelSection::getInitialTangent(void)
{
    ks->Zero();
...
const Vector & ParallelSection::getStressResultant(void)
{
    s->Zero();
...
SectionForceDeformation *ParallelSection::getCopy(void)
{
    ParallelSection *theCopy = 0;

    theCopy =
        new ParallelSection(this->getTag(), numSections, theSections);
...
const ID & ParallelSection::getType()
{
    return *theCode;
}

int ParallelSection::getOrder() const
{
    return order;
}

int ParallelSection::commitState(void)
{
    int err = 0;
...
int ParallelSection::revertToLastCommit(void)
{
    int err = 0;
...
int ParallelSection::revertToStart(void)
{
    int err = 0;
...
int ParallelSection::sendSelf(int cTag, Channel & theChannel)
{
    return 0;
}


int ParallelSection::recvSelf(int cTag, Channel & theChannel,
                              FEM_ObjectBroker & theBroker)
{
    return 0;
}

void ParallelSection::Print(OPS_Stream & s, int flag)
{
    if (flag == OPS_PRINT_PRINTMODEL_SECTION
        ...
        if (flag == OPS_PRINT_PRINTMODEL_MATERIAL) {
            ...
            for (int i = 0; i < numSections; i++)
                s << "\t\tSection, tag: " << theSections[i]->getTag() <<
                    endln;
        }
    }
...
Response *ParallelSection::setResponse(const char **argv, int argc,
                                       OPS_Stream & output)
{
...
int ParallelSection::getResponse(int responseID, Information & info)
{
...
int ParallelSection::setParameter(const char **argv, int argc,
                                  Parameter & param)
{
    if (argc < 1)
        return -1;
                                                   ...
                                                   bool conditional)
{
    ...
    for (int i = 0; i < numSections; i++) {
        ...
        const ID & code = theSections[i]->getType();
        const Vector & si =
            theSections[i]->getStressResultantSensitivity(gradIndex,
                                                          conditional);
        for (int j = 0; j < orderi; j++)
            for (int k = 0; k < order; k++)
                if (code(j) == (*theCode) (k))
...
const Matrix & ParallelSection::getSectionTangentSensitivity(int gradIndex)
{
    ks->Zero();
...
int ParallelSection::commitSensitivity(const Vector & defSens,
                                       int gradIndex, int numGrads)
{
    ...
    for (int i = 0; i < numSections; i++) {
        ...
        ret +=
            theSections[i]->commitSensitivity(defi, gradIndex, numGrads);
    }
...
const Vector & ParallelSection::getdedh(void)
{
    return dedh;
}
