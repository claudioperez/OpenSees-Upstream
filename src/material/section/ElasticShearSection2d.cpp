...
Vector
ElasticShearSection2d::s (3);
Matrix
ElasticShearSection2d::ks (3, 3);
ID
ElasticShearSection2d::code (3);

#ifdef OPS_API_COMMANDLINE
void *
OPS_ElasticShearSection2d ()
{
    ...
    return new ElasticShearSection2d (tag, data[0], data[1], data[2], data[3],
                                      data[4]);
}
#endif
...
ElasticShearSection2d::ElasticShearSection2d
    (int tag, double E_in, double A_in, double I_in, double G_in,
     double alpha_in):
SectionForceDeformation (tag, SEC_TAG_ElasticShear2d), E (E_in), A (A_in),
I (I_in), G (G_in), alpha (alpha_in), e (3),
parameterID (0)
{
    if (E <= 0.0)
      {
