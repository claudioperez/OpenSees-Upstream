...
// constructors:
NDFiberSection2d::NDFiberSection2d(int tag, int num, Fiber ** fibers,
                                   double a,
                                   bool
                                   compCentroid):SectionForceDeformation
    (tag, SEC_TAG_NDFiberSection2d), numFibers(num), sizeFibers(num),
theMaterials(0), matData(0), QzBar(0.0), Abar(0.0), yBar(0.0),
computeCentroid(compCentroid), alpha(a), sectionIntegr(0), e(3), s(0),
ks(0), parameterID(0), dedh(3)
{
    if (numFibers != 0) {
        ...
        if (theMaterials == 0) {
            opserr <<
                "NDFiberSection2d::NDFiberSection2d -- failed to allocate Material pointers";
            exit(-1);
        }
        ...
        if (matData == 0) {
            opserr <<
                "NDFiberSection2d::NDFiberSection2d -- failed to allocate double array for material data\n";
            exit(-1);
        }
        ...
        for (int i = 0; i < numFibers; i++) {
            ...
            if (theMaterials[i] == 0) {
                opserr <<
                    "NDFiberSection2d::NDFiberSection2d -- failed to get copy of a Material\n";
                exit(-1);
            }
        }
...
NDFiberSection2d::NDFiberSection2d(int tag, int num, double a,
                                   bool
                                   compCentroid):SectionForceDeformation
    (tag, SEC_TAG_NDFiberSection2d), numFibers(0), sizeFibers(num),
theMaterials(0), matData(0), QzBar(0.0), Abar(0.0), yBar(0.0),
computeCentroid(compCentroid), alpha(a), sectionIntegr(0), e(3), s(0),
ks(0), parameterID(0), dedh(3)
{
...
ks(0), parameterID(0), dedh(3)
{
    ...
    if (sectionIntegr == 0) {
        opserr <<
            "Error: NDFiberSection2d::NDFiberSection2d: could not create copy of section integration object"
            << endln;
        exit(-1);
    }
...
alpha(1.0), sectionIntegr(0), e(3), s(0), ks(0), parameterID(0), dedh(3)
{
    s = new Vector(sData, 3);
    ks = new Matrix(kData, 3, 3);
...
int NDFiberSection2d::addFiber(Fiber & newFiber)
{
    ...
    if (numFibers == sizeFibers) {
        ...
        if (newArray == 0 || newMatData == 0) {
            opserr <<
                "NDFiberSection2d::addFiber -- failed to allocate Fiber pointers\n";
            return -1;
        }
    ...
    if (theMaterials[numFibers] == 0) {
        opserr <<
            "NDFiberSection2d::addFiber -- failed to get copy of a Material\n";
        return -1;
    }
...
const Vector & NDFiberSection2d::getSectionDeformation(void)
{
    return e;
}

const Matrix & NDFiberSection2d::getInitialTangent(void)
{
...
const Matrix & NDFiberSection2d::getSectionTangent(void)
{
    //opserr << *ks << endln;
    return *ks;
}

const Vector & NDFiberSection2d::getStressResultant(void)
{
    return *s;
}

SectionForceDeformation *NDFiberSection2d::getCopy(void)
{
    ...
    if (numFibers != 0) {
        ...
        if (theCopy->theMaterials == 0) {
            opserr <<
                "NDFiberSection2d::getCopy -- failed to allocate Material pointers\n";
            exit(-1);
        }
        ...
        if (theCopy->matData == 0) {
            opserr <<
                "NDFiberSection2d::getCopy -- failed to allocate double array for material data\n";
            exit(-1);
        }
        ...
        for (int i = 0; i < numFibers; i++) {
            ...
            if (theCopy->theMaterials[i] == 0) {
                opserr <<
                    "NDFiberSection2d::getCopy -- failed to get copy of a Material";
                exit(-1);
            }
        }
    }
...
const ID & NDFiberSection2d::getType()
{
    return code;
}

int NDFiberSection2d::getOrder() const
{
    return 3;
}

int NDFiberSection2d::commitState(void)
{
    int err = 0;
...
int NDFiberSection2d::revertToLastCommit(void)
{
    int err = 0;
...
int NDFiberSection2d::revertToStart(void)
{
...
int NDFiberSection2d::sendSelf(int commitTag, Channel & theChannel)
{
    int res = 0;
    ...
    if (numFibers != 0) {
        ...
        if (res < 0) {
            opserr <<
                "NDFiberSection2d::sendSelf - failed to send material data\n";
            return res;
        }
...
int NDFiberSection2d::recvSelf(int commitTag, Channel & theChannel,
                               FEM_ObjectBroker & theBroker)
{
    ...
    if (data(1) != 0) {
        ...
        if (res < 0) {
            opserr <<
                "NDFiberSection2d::recvSelf - failed to recv material data\n";
            return res;
        }
        ...
        if (theMaterials == 0 || numFibers != data(1)) {
            ...
            if (numFibers != 0) {
                ...
                if (theMaterials == 0) {
                    opserr <<
                        "NDFiberSection2d::recvSelf -- failed to allocate Material pointers\n";
                    exit(-1);
                }
                ...
                if (matData == 0) {
                    opserr <<
                        "NDFiberSection2d::recvSelf  -- failed to allocate double array for material data\n";
                    exit(-1);
                }
            }
        }
...
void NDFiberSection2d::Print(OPS_Stream & s, int flag)
{
...
Response *NDFiberSection2d::setResponse(const char **argv, int argc,
                                        OPS_Stream & output)
{
...
int NDFiberSection2d::getResponse(int responseID, Information & sectInfo)
{
...
int NDFiberSection2d::setParameter(const char **argv, int argc,
                                   Parameter & param)
{
    if (argc < 1)
        return -1;
...
int NDFiberSection2d::updateParameter(int paramID, Information & info)
{
    switch (paramID) {
    case 1:
...
int NDFiberSection2d::activateParameter(int paramID)
{
    parameterID = paramID;
...
const Vector &
    NDFiberSection2d::getStressResultantSensitivity(int gradIndex,
                                                    bool conditional)
{
    static Vector ds(3);
