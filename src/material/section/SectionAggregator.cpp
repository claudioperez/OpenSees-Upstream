...
void *OPS_SectionAggregator()
{
    ...
    if (OPS_GetNumRemainingInputArgs() > 1) {
        ...
        if (strcmp(flag, "-section") == 0) {
            if (OPS_GetIntInput(&numdata, &secTag) < 0) {
                opserr << "WARNING invalid Aggregator section tag" <<
                    endln;
                return 0;
            }
    ...
    if (theSec) {
        return new SectionAggregator(tag, *theSec, nMats, &theMats[0],
                                     codes);
    } else {
...
double
 SectionAggregator::workArea[2 * maxOrder * (maxOrder + 1)];
int
 SectionAggregator::codeArea[maxOrder];
...
SectionAggregator::SectionAggregator(int tag,
                                     ...
                                     const ID &
                                     addCodes):SectionForceDeformation(tag,
                                                                       SEC_TAG_Aggregator),
theSection(0), theAdditions(0), matCodes(0), numMats(numAdds), e(0), s(0),
ks(0), fs(0), theCode(0), otherDbTag(0)
{
    theSection = theSec.getCopy();

    if (!theSection) {
        opserr << "SectionAggregator::SectionAggregator " << tag <<
    ...
    if (!theAdds) {
        opserr << "SectionAggregator::SectionAggregator " << tag <<
            " -- null uniaxial material array passed\n";
        exit(-1);
    }
    ...
    if (order > maxOrder) {
        opserr << "SectionAggregator::SectionAggregator   " << tag <<
            "  -- order too big, need to modify the #define in SectionAggregator.cpp to "
            << order << endln;
        exit(-1);
    }
...
ks(0), fs(0), theCode(0), otherDbTag(0)
{
    ...
    if (order > maxOrder) {
        opserr << "SectionAggregator::SectionAggregator   " << tag <<
            " -- order too big, need to modify the #define in SectionAggregator.cpp to %d\n";
        exit(-1);
    }
...
SectionAggregator::SectionAggregator(int tag,
                                     ...
                                     int c):SectionForceDeformation(tag,
                                                                    SEC_TAG_Aggregator),
theSection(0), theAdditions(0), matCodes(0), numMats(1), e(0), s(0), ks(0),
fs(0), theCode(0), otherDbTag(0)
{
    theSection = theSec.getCopy();
...
e(0), s(0), ks(0), fs(0), theCode(0), otherDbTag(0)
{
...
int
 SectionAggregator::setTrialSectionDeformation(const Vector & def)
{
    int ret = 0;
    int i = 0;
...
const Vector & SectionAggregator::getSectionDeformation(void)
{
    int i = 0;
...
const Matrix & SectionAggregator::getSectionTangent(void)
{
    int i = 0;
...
const Matrix & SectionAggregator::getInitialTangent(void)
{
    int i = 0;
    ...
    for (; i < order; i++)
        (*ks) (i, i) =
            theAdditions[i - theSectionOrder]->getInitialTangent();

    return *ks;
}

const Matrix & SectionAggregator::getSectionFlexibility(void)
{
    int i = 0;
...
const Matrix & SectionAggregator::getInitialFlexibility(void)
{
    int i = 0;
...
const Vector & SectionAggregator::getStressResultant(void)
{
    int i = 0;
...
SectionForceDeformation *SectionAggregator::getCopy(void)
{
    ...
    if (theCopy == 0) {
        opserr <<
            "SectionAggregator::getCopy -- failed to allocate copy\n";
        exit(-1);
    }
...
const ID & SectionAggregator::getType()
{
    int i = 0;
...
int SectionAggregator::commitState(void)
{
    int err = 0;
...
int SectionAggregator::revertToLastCommit(void)
{
    int err = 0;
...
int SectionAggregator::revertToStart(void)
{
    int err = 0;
...
int SectionAggregator::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
    ...
    if (res < 0) {
        opserr <<
            "SectionAggregator::sendSelf -- could not send data ID\n";
    ...
    if (res < 0) {
        opserr <<
            "SectionAggregator::sendSelf -- could not send classTags ID\n";
        return res;
    }
    ...
    for (i = 0; i < numMats; i++) {
        ...
        if (res < 0) {
            opserr <<
                "SectionAggregator::sendSelf -- could not send UniaxialMaterial, i = "
                << i << endln;
            return res;
        }
    }
    ...
    if (theSection != 0) {
        ...
        if (res < 0) {
            opserr <<
                "SectionAggregator::sendSelf -- could not send SectionForceDeformation\n";
            return res;
        }
    }
...
int
 SectionAggregator::recvSelf(int cTag, Channel & theChannel,
                             FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0) {
        opserr <<
            "SectionAggregator::recvSelf -- could not receive data ID\n";
        return res;
    }
    ...
    if (order > 0) {
        if (e == 0 || e->Size() != order) {
            ...
            s = new Vector(&workArea[maxOrder], order);
            ks = new Matrix(&workArea[2 * maxOrder], order, order);
            fs = new Matrix(&workArea[maxOrder * (maxOrder + 2)], order,
                            order);
            theCode = new ID(codeArea, order);
        }
    }
    ...
    if (res < 0) {
        opserr <<
            "SectionAggregator::recvSelf -- could not receive classTags ID\n";
        return res;
    }
    ...
    if (theAdditions == 0) {
        ...
        if (theAdditions == 0) {
            opserr <<
                "SectionAggregator::recvSelf -- could not allocate UniaxialMaterial array\n";
            return -1;
        }
    ...
    for (i = 0; i < numMats; i++) {
        ...
        if (theAdditions[i] == 0) {
            opserr <<
                "SectionAggregator::recvSelf -- could not get UniaxialMaterial, i = "
                << i << endln;
            return -1;
        }
        ...
        if (res < 0) {
            opserr <<
                "SectionAggregator::recvSelf -- could not receive UniaxialMaterial, i = "
                << i << endln;
            return res;
        }
    }
    ...
    if (theSectionOrder != 0) {
        ...
        if (res < 0) {
            opserr <<
                "SectionAggregator::recvSelf -- could not receive SectionForceDeformation\n";
            return res;
        }
    }
...
Response *SectionAggregator::setResponse(const char **argv, int argc,
                                         OPS_Stream & output)
{
    ...
    if ((strcmp(argv[0], "deformations") == 0)
        ...
        return this->SectionForceDeformation::setResponse(argv, argc,
                                                          output);
    }
    // by SAJalali
    int num = numMats;
...
void SectionAggregator::Print(OPS_Stream & s, int flag)
{
    if (flag == OPS_PRINT_PRINTMODEL_SECTION) {
        ...
        for (int i = 0; i < numMats; i++)
            s << "\t\tUniaxial Material, tag: " <<
                theAdditions[i]->getTag() << endln;
...
int SectionAggregator::setParameter(const char **argv, int argc,
                                    Parameter & param)
{
    if (argc < 1)
        return -1;
    ...
    if (strstr(argv[0], "addition") != 0
        ...
        for (int i = 0; i < numMats; i++)
            if (materialTag == theAdditions[i]->getTag()) {
                ok = theAdditions[i]->setParameter(&argv[2], argc - 2,
                                                   param);
                if (ok != -1)
                    result = ok;
            }
    ...
    // Check if the parameter belongs to only the section
        ...
        if (argc < 2) {
            opserr <<
                "SectionAggregator::setParameter() - insufficient argc < 2 for section option. "
                << endln;
            return -1;
        }
                                                     ...
                                                     bool conditional)
{
    ...
    if (theSection) {
        const Vector & dsdh =
            theSection->getStressResultantSensitivity(gradIndex,
                                                      conditional);
        theSectionOrder = theSection->getOrder();
    ...
    for (; i < order; i++)
        (*s) (i) =
            theAdditions[i -
                         theSectionOrder]->getStressSensitivity(gradIndex,
                                                                conditional);
    ...
    SectionAggregator::getSectionTangentSensitivity(int gradIndex)
{
    ...
    if (theSection) {
        const Matrix & kSec =
            theSection->getSectionTangentSensitivity(gradIndex);
        theSectionOrder = theSection->getOrder();
    ...
    for (; i < order; i++)
        (*ks) (i, i) =
            theAdditions[i -
                         theSectionOrder]->
            getTangentSensitivity(gradIndex);
    ...
    SectionAggregator::getInitialTangentSensitivity(int gradIndex)
{
    ...
    if (theSection) {
        const Matrix & kSec =
            theSection->getInitialTangentSensitivity(gradIndex);
        theSectionOrder = theSection->getOrder();
    ...
    for (; i < order; i++)
        (*ks) (i, i) =
            theAdditions[i -
                         theSectionOrder]->getInitialTangentSensitivity
            (gradIndex);
                                      ...
                                      int gradIndex, int numGrads)
{
    ...
    for (; i < order; i++)
        ret +=
            theAdditions[i -
                         theSectionOrder]->commitSensitivity(defSens(i),
                                                             gradIndex,
                                                             numGrads);
