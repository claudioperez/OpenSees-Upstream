...
Response *
SectionAggregator::setResponse (const char **argv, int argc,
                                OPS_Stream & output)
{
    ...
    if (flag == OPS_PRINT_PRINTMODEL_SECTION)
      {
          ...
          for (int i = 0; i < numMats; i++)
              s << "\t\tUniaxial Material, tag: " << theAdditions[i]->
                  getTag () << endln;
...
int
SectionAggregator::setParameter (const char **argv, int argc,
                                 Parameter & param)
{
    if (argc < 1)
        return -1;
...
SectionAggregator::getSectionTangentSensitivity (int gradIndex)
{
    ...
    for (; i < order; i++)
        (*ks) (i, i) =
            theAdditions[i -
                         theSectionOrder]->getTangentSensitivity (gradIndex);
...
SectionAggregator::getInitialTangentSensitivity (int gradIndex)
{
    ...
    for (; i < order; i++)
        (*ks) (i, i) =
            theAdditions[i -
                         theSectionOrder]->
            getInitialTangentSensitivity (gradIndex);
                                      ...
                                      int gradIndex, int numGrads)
{
    ...
    for (; i < order; i++)
        ret +=
            theAdditions[i - theSectionOrder]->commitSensitivity (defSens (i),
                                                                  gradIndex,
                                                                  numGrads);
