...
GenericSection1d::GenericSection1d(int tag, UniaxialMaterial & m,
                                   int type):SectionForceDeformation(tag,
                                                                     SEC_TAG_Generic1d),
code(type)
{
    theModel = m.getCopy();
    ...
    if (!theModel) {
        opserr <<
            "GenericSection1d::GenericSection1d  -- failed to get copy of material model\n";
        exit(-1);
    }
}

GenericSection1d::GenericSection1d():SectionForceDeformation(0, SEC_TAG_Generic1d), theModel(0),
code(0)
{
...
const Vector & GenericSection1d::getSectionDeformation()
{
...
const Vector & GenericSection1d::getStressResultant()
{
...
const Matrix & GenericSection1d::getSectionTangent()
{
...
const Matrix & GenericSection1d::getInitialTangent()
{
...
const Matrix & GenericSection1d::getSectionFlexibility()
{
...
const Matrix & GenericSection1d::getInitialFlexibility()
{
...
int GenericSection1d::commitState()
{
    return theModel->commitState();
}

int GenericSection1d::revertToLastCommit()
{
    return theModel->revertToLastCommit();
}

int GenericSection1d::revertToStart()
{
    return theModel->revertToStart();
}

const ID & GenericSection1d::getType()
{
    c(0) = code;
...
int GenericSection1d::getOrder() const
{
    return 1;
}

SectionForceDeformation *GenericSection1d::getCopy()
{
...
int GenericSection1d::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
    ...
    if (res < 0) {
        opserr <<
            "GenericSection1d::sendSelf -- could not send UniaxialMaterial\n";
        return res;
    }
...
int GenericSection1d::recvSelf(int cTag, Channel & theChannel,
                               FEM_ObjectBroker & theBroker)
{
    ...
    if (theModel == 0) {
        opserr <<
            "GenericSection1d::recvSelf -- could not get a UniaxialMaterial\n";
        return -1;
    }
    ...
    if (res < 0) {
        opserr <<
            "GenericSection1d::recvSelf -- could not receive UniaxialMaterial\n";
        return res;
    }
...
void GenericSection1d::Print(OPS_Stream & s, int flag)
{
...
int GenericSection1d::setParameter(const char **argv, int argc,
                                   Parameter & param)
{
