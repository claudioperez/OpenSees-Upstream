...
{
    TaggedObject *obj =
        theSectionForceDeformationObjects.removeComponent(tag);
    if (obj != 0) {
        delete obj;
        return true;
    }
...
SectionForceDeformation *OPS_getSectionForceDeformation(int tag)
{
    ...
    TaggedObject *theResult =
        theSectionForceDeformationObjects.getComponentPtr(tag);
    if (theResult == 0) {
        opserr <<
            "SectionForceDeformation *getSectionForceDeformation(int tag) - none found with tag: "
            << tag << endln;
        return 0;
    }
    SectionForceDeformation *theMat =
        (SectionForceDeformation *) theResult;
...
void OPS_printSectionForceDeformation(OPS_Stream & s, int flag)
{
    ...
    if (flag == OPS_PRINT_PRINTMODEL_JSON) {
        s << "\t\t\"sections\": [\n";
        MapOfTaggedObjectsIter theObjects =
            theSectionForceDeformationObjects.getIter();
        theObjects.reset();
        TaggedObject *theObject;
        int count = 0;
        int numComponents =
            theSectionForceDeformationObjects.getNumComponents();
        while ((theObject = theObjects()) != 0) {
            SectionForceDeformation *theSection =
                (SectionForceDeformation *) theObject;
...
SectionForceDeformation::SectionForceDeformation(int tag,
                                                 int
                                                 classTag):Material(tag,
                                                                    classTag),
fDefault(0), sDefault(0)
{

}

SectionForceDeformation::SectionForceDeformation():Material(0, 0), fDefault(0),
sDefault(0)
{
...
const Matrix & SectionForceDeformation::getSectionFlexibility()
{
    ...
    if (fDefault == 0) {
        ...
        if (fDefault == 0) {
            opserr <<
                "SectionForceDeformation::getSectionFlexibility -- failed to allocate flexibility matrix\n";
            exit(-1);
        }
    }
...
const Matrix & SectionForceDeformation::getInitialFlexibility()
{
    ...
    if (fDefault == 0) {
        ...
        if (fDefault == 0) {
            opserr <<
                "SectionForceDeformation::getInitialFlexibility -- failed to allocate flexibility matrix\n";
            exit(-1);
        }
    }
...
double SectionForceDeformation::getRho(void)
{
    return 0.0;
}

Response *SectionForceDeformation::setResponse(const char **argv, int argc,
                                               OPS_Stream & output)
{
    ...
    if (strcmp(argv[0], "deformations") == 0
        ...
        theResponse =
            new MaterialResponse(this, 12, this->getSectionTangent());
    }

    else if (strcmp(argv[0], "flexibility") == 0) {
        theResponse =
            new MaterialResponse(this, 13, this->getSectionFlexibility());
    }
    ...
    case 2:
        {
            ...
            const Vector & dedh =
                this->getSectionDeformationSensitivity(gradIndex);
            const Vector & dsdh =
                this->getStressResultantSensitivity(gradIndex, true);
...
const Vector &
    SectionForceDeformation::getStressResultantSensitivity(int gradIndex,
                                                           bool
                                                           conditional)
{
    if (sDefault == 0)
    ...
    SectionForceDeformation::getSectionTangentSensitivity(int gradIndex)
{
    ...
    if (fDefault == 0) {
        ...
        if (fDefault == 0) {
            opserr <<
                "SectionForceDeformation::getSectionTangentSensitivity -- failed to allocate matrix\n";
            exit(-1);
        }
    }
    ...
    SectionForceDeformation::getInitialTangentSensitivity(int gradIndex)
{
    ...
    if (fDefault == 0) {
        ...
        if (fDefault == 0) {
            opserr <<
                "SectionForceDeformation::getInitialTangentSensitivity -- failed to allocate matrix\n";
            exit(-1);
        }
    }
...
const Matrix &
    SectionForceDeformation::
getSectionFlexibilitySensitivity(int gradIndex)
{
    int order = this->getOrder();
    ...
    if (fDefault == 0) {
        ...
        if (fDefault == 0) {
            opserr <<
                "SectionForceDeformation::getSectionFlexibilitySensitivity -- failed to allocate matrix\n";
            exit(-1);
        }
    }
...
const Matrix &
    SectionForceDeformation::
getInitialFlexibilitySensitivity(int gradIndex)
{
    int order = this->getOrder();
    ...
    if (fDefault == 0) {
        ...
        if (fDefault == 0) {
            opserr <<
                "SectionForceDeformation::getInitialFlexibilitySensitivity -- failed to allocate matrix\n";
            exit(-1);
        }
    }
...
int SectionForceDeformation::commitSensitivity(const Vector & defSens,
...
int SectionForceDeformation::setTrialSectionDeformation(const Vector &) //JZ
{
    opserr <<
        "SectionForceDeformation::setTrialSectionDeformation(strain) - should not be called\n";
    return -1;
}

int SectionForceDeformation::setTrialSectionDeformation(const Vector & nouse, const Vector & data)      //JZ
{
    opserr <<
        "SectionForceDeformation::setTrialSectionDeformationTemperature (strain, tData) - should not be called\n";
    return -1;
}

static Vector errRes(3);

const Vector & SectionForceDeformation::getTemperatureStress(const Vector & tData)      //PK
{
    opserr <<
