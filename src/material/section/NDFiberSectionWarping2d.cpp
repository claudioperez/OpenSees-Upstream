...
// constructors:
NDFiberSectionWarping2d::NDFiberSectionWarping2d(int tag, int num,
                                                 Fiber ** fibers,
                                                 double
                                                 a):SectionForceDeformation
    (tag, SEC_TAG_NDFiberSectionWarping2d), numFibers(num),
theMaterials(0), matData(0), yBar(0.0), alpha(a), sectionIntegr(0), e(5),
eCommit(5), s(0), ks(0), parameterID(0), dedh(5), yBarZero(0.0)
{
    if (numFibers != 0) {
        ...
        if (theMaterials == 0) {
            opserr <<
                "NDFiberSectionWarping2d::NDFiberSectionWarping2d -- failed to allocate Material pointers";
            exit(-1);
        }
        ...
        if (matData == 0) {
            opserr <<
                "NDFiberSectionWarping2d::NDFiberSectionWarping2d -- failed to allocate double array for material data\n";
            exit(-1);
        }
        ...
        for (int i = 0; i < numFibers; i++) {
            ...
            if (theMaterials[i] == 0) {
                opserr <<
                    "NDFiberSectionWarping2d::NDFiberSectionWarping2d -- failed to get copy of a Material\n";
                exit(-1);
            }
        }
...
eCommit(5), s(0), ks(0), parameterID(0), dedh(5), yBarZero(0.0)
{
    ...
    if (sectionIntegr == 0) {
        opserr <<
            "Error: NDFiberSectionWarping2d::NDFiberSectionWarping2d: could not create copy of section integration object"
            << endln;
        exit(-1);
    }
...
sectionIntegr(0), e(5), eCommit(5), s(0), ks(0), parameterID(0),
dedh(5), yBarZero(0.0)
{
    s = new Vector(sData, 5);
    ks = new Matrix(kData, 5, 5);
...
int NDFiberSectionWarping2d::addFiber(Fiber & newFiber)
{
    ...
    if (newArray == 0 || newMatData == 0) {
        opserr <<
            "NDFiberSectionWarping2d::addFiber -- failed to allocate Fiber pointers\n";
        return -1;
    }
    ...
    if (newArray[numFibers] == 0) {
        opserr <<
            "NDFiberSectionWarping2d::addFiber -- failed to get copy of a Material\n";
        delete[]newMatData;
        return -1;
    }
...
setTrialSectionDeformation(const Vector & deforms)
{
    int res = 0;
    ...
    double maxLoc(fiberLocs[1] - yBarZero),
        minLoc(fiberLocs[1] - yBarZero);
    for (int i = 0; i < numFibers; i++) {
    ...
    for (int i = 0; i < numFibers; i++) {
        ...
        eps(0) =
            d0 - y * d1 + (y * y * y / (h * h * h) - 0.6 * y / h) * d4;
...
const Vector & NDFiberSectionWarping2d::getSectionDeformation(void)
{
    return e;
}

double NDFiberSectionWarping2d::getRho(void)
{
    return 0.0;
}

const Matrix & NDFiberSectionWarping2d::getInitialTangent(void)
{
...
const Matrix & NDFiberSectionWarping2d::getSectionTangent(void)
{
    return *ks;
}

const Vector & NDFiberSectionWarping2d::getStressResultant(void)
{
    return *s;
}

SectionForceDeformation *NDFiberSectionWarping2d::getCopy(void)
{
    ...
    if (numFibers != 0) {
        ...
        if (theCopy->theMaterials == 0) {
            opserr <<
                "NDFiberSectionWarping2d::getCopy -- failed to allocate Material pointers\n";
            exit(-1);
        }
        ...
        if (theCopy->matData == 0) {
            opserr <<
                "NDFiberSectionWarping2d::getCopy -- failed to allocate double array for material data\n";
            exit(-1);
        }
        ...
        for (int i = 0; i < numFibers; i++) {
            ...
            theCopy->theMaterials[i] =
                theMaterials[i]->getCopy("BeamFiber2d");

            if (theCopy->theMaterials[i] == 0) {
                opserr <<
                    "NDFiberSectionWarping2d::getCopy -- failed to get copy of a Material";
                exit(-1);
            }
        }
    }
...
const ID & NDFiberSectionWarping2d::getType()
{
    return code;
}

int NDFiberSectionWarping2d::getOrder() const
{
    return 5;
}

int NDFiberSectionWarping2d::commitState(void)
{
    int err = 0;
...
int NDFiberSectionWarping2d::revertToLastCommit(void)
{
    int err = 0;
...
int NDFiberSectionWarping2d::revertToStart(void)
{
...
int NDFiberSectionWarping2d::sendSelf(int commitTag, Channel & theChannel)
{
    int res = 0;
    ...
    if (res < 0) {
        opserr <<
            "NDFiberSectionWarping2d::sendSelf - failed to send ID data\n";
        return res;
    }
    ...
    if (numFibers != 0) {
        ...
        if (res < 0) {
            opserr <<
                "NDFiberSectionWarping2d::sendSelf - failed to send material data\n";
            return res;
        }
...
int NDFiberSectionWarping2d::recvSelf(int commitTag, Channel & theChannel,
                                      FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0) {
        opserr <<
            "NDFiberSectionWarping2d::recvSelf - failed to recv ID data\n";
        return res;
    }
    this->setTag(data(0));
    ...
    if (data(1) != 0) {
        ...
        if (res < 0) {
            opserr <<
                "NDFiberSectionWarping2d::recvSelf - failed to recv material data\n";
            return res;
        }
        ...
        if (theMaterials == 0 || numFibers != data(1)) {
            ...
            if (numFibers != 0) {
                ...
                if (theMaterials == 0) {
                    opserr <<
                        "NDFiberSectionWarping2d::recvSelf -- failed to allocate Material pointers\n";
                    exit(-1);
                }
                ...
                if (matData == 0) {
                    opserr <<
                        "NDFiberSectionWarping2d::recvSelf  -- failed to allocate double array for material data\n";
                    exit(-1);
                }
            }
        }
...
void NDFiberSectionWarping2d::Print(OPS_Stream & s, int flag)
{
...
Response *NDFiberSectionWarping2d::setResponse(const char **argv, int argc,
                                               OPS_Stream & output)
{
...
int NDFiberSectionWarping2d::getResponse(int responseID,
...
int NDFiberSectionWarping2d::setParameter(const char **argv, int argc,
                                          Parameter & param)
{
    if (argc < 1)
        return -1;
...
int NDFiberSectionWarping2d::updateParameter(int paramID,
                                             Information & info)
{
    switch (paramID) {
    case 1:
...
int NDFiberSectionWarping2d::activateParameter(int paramID)
{
    parameterID = paramID;
...
const Vector &
    NDFiberSectionWarping2d::
getSectionDeformationSensitivity(int gradIndex)
{
    return dedh;
}

const Vector &
    NDFiberSectionWarping2d::getStressResultantSensitivity(int gradIndex,
                                                           bool
                                                           conditional)
{
    static Vector ds(5);
    ...
    for (int i = 0; i < numFibers; i++) {
        ...
        if (locsDeriv[i] != 0.0) {
            ...
            ds(2) +=
                rootAlpha * (-locsDeriv[i] * tangent(1, 0) * e(1)) * A;
        }
