...
//parameters
const double
    ElasticMembranePlateSection::five6 = 5.0 / 6.0;     //shear correction

//static vector and matrices
Vector
ElasticMembranePlateSection::stress (8);
Matrix
ElasticMembranePlateSection::tangent (8, 8);
ID
ElasticMembranePlateSection::array (8);

#ifdef OPS_API_COMMANDLINE
void *
OPS_ElasticMembranePlateSection ()
{
    if (OPS_GetNumRemainingInputArgs () < 4)
      {
          opserr << "WARNING insufficient arguments\n";
          opserr <<
              "Want: section ElasticMembranePlateSection tag? E? nu? h? <rho?> <Ep_modifer?>\n";
          return 0;
      }
    ...
    return new ElasticMembranePlateSection (tag, data[0], data[1], data[2],
                                            data[3], data[4]);
}
#endif
...
SectionForceDeformation (0, SEC_TAG_ElasticMembranePlateSection), strain (8)
{
...
//get the strain 
int
ElasticMembranePlateSection::setTrialSectionDeformation (const Vector &
                                                         strain_from_element)
{
    ...
    if (flag == OPS_PRINT_PRINTMODEL_SECTION)
      {
          ...
          s << "  Young's Modulus for Membrane (in-plane) Action, Em = " << Em
              << endln;
          s << "  Young's Modulus for Plate (out-of-plane) Action, Ep = " <<
              Ep << endln;
...
ElasticMembranePlateSection::sendSelf (int cTag, Channel & theChannel)
{
    ...
    if (res < 0)
        opserr <<
            "ElasticMembranePlateSection::sendSelf() - failed to send data\n";
                                       ...
                                       FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0)
        opserr <<
            "ElasticMembranePlateSection::recvSelf() - failed to recv data\n";
    else
      {
          this->setTag (data (0));
          Em = data (1);
