...
// constructors:
NDFiberSection3d::NDFiberSection3d(int tag, int num, Fiber ** fibers,
                                   double a,
                                   bool
                                   compCentroid):SectionForceDeformation
    (tag, SEC_TAG_NDFiberSection3d), numFibers(num), sizeFibers(num),
theMaterials(0), matData(0), Abar(0.0), QyBar(0.0), QzBar(0.0), yBar(0.0),
zBar(0.0), computeCentroid(compCentroid), alpha(a), sectionIntegr(0), e(6),
s(0), ks(0), parameterID(0), dedh(6)
{
    if (numFibers != 0) {
        ...
        if (theMaterials == 0) {
            opserr <<
                "NDFiberSection3d::NDFiberSection3d -- failed to allocate Material pointers";
            exit(-1);
        }
        ...
        if (matData == 0) {
            opserr <<
                "NDFiberSection3d::NDFiberSection3d -- failed to allocate double array for material data\n";
            exit(-1);
        }
        ...
        for (int i = 0; i < numFibers; i++) {
            ...
            if (theMaterials[i] == 0) {
                opserr <<
                    "NDFiberSection3d::NDFiberSection3d -- failed to get copy of a Material\n";
                exit(-1);
            }
        }
...
NDFiberSection3d::NDFiberSection3d(int tag, int num, double a,
                                   bool
                                   compCentroid):SectionForceDeformation
    (tag, SEC_TAG_NDFiberSection3d), numFibers(0), sizeFibers(num),
theMaterials(0), matData(0), Abar(0.0), QyBar(0.0), QzBar(0.0), yBar(0.0),
zBar(0.0), computeCentroid(compCentroid), alpha(a), sectionIntegr(0), e(6),
s(0), ks(0), parameterID(0), dedh(6)
{
...
s(0), ks(0), parameterID(0), dedh(6)
{
    ...
    if (sectionIntegr == 0) {
        opserr <<
            "Error: NDFiberSection3d::NDFiberSection3d: could not create copy of section integration object"
            << endln;
        exit(-1);
    }
...
computeCentroid(true), alpha(1.0), sectionIntegr(0), e(6), s(0), ks(0),
...
int NDFiberSection3d::addFiber(Fiber & newFiber)
{
    ...
    if (numFibers == sizeFibers) {
        ...
        if (newArray == 0 || newMatData == 0) {
            opserr <<
                "NDFiberSection3d::addFiber -- failed to allocate Fiber pointers\n";
            return -1;
        }
    ...
    if (theMaterials[numFibers] == 0) {
        opserr <<
            "NDFiberSection3d::addFiber -- failed to get copy of a Material\n";
        return -1;
    }
...
const Vector & NDFiberSection3d::getSectionDeformation(void)
{
    return e;
}

const Matrix & NDFiberSection3d::getInitialTangent(void)
{
...
const Matrix & NDFiberSection3d::getSectionTangent(void)
{
    return *ks;
}

const Vector & NDFiberSection3d::getStressResultant(void)
{
    return *s;
}

SectionForceDeformation *NDFiberSection3d::getCopy(void)
{
    ...
    if (numFibers != 0) {
        ...
        if (theCopy->theMaterials == 0) {
            opserr <<
                "NDFiberSection3d::getCopy -- failed to allocate Material pointers\n";
            exit(-1);
        }
        ...
        if (theCopy->matData == 0) {
            opserr <<
                "NDFiberSection3d::getCopy -- failed to allocate double array for material data\n";
            exit(-1);
        }
        ...
        for (int i = 0; i < numFibers; i++) {
            ...
            theCopy->theMaterials[i] =
                theMaterials[i]->getCopy("BeamFiber");

            if (theCopy->theMaterials[i] == 0) {
                opserr <<
                    "NDFiberSection3d::getCopy -- failed to get copy of a Material";
                exit(-1);
            }
        }
    }
...
const ID & NDFiberSection3d::getType()
{
    return code;
}

int NDFiberSection3d::getOrder() const
{
    return 6;
}

int NDFiberSection3d::commitState(void)
{
    int err = 0;
...
int NDFiberSection3d::revertToLastCommit(void)
{
    int err = 0;
...
int NDFiberSection3d::revertToStart(void)
{
...
int NDFiberSection3d::sendSelf(int commitTag, Channel & theChannel)
{
    int res = 0;
    ...
    if (numFibers != 0) {
        ...
        if (res < 0) {
            opserr <<
                "NDFiberSection3d::sendSelf - failed to send material data\n";
            return res;
        }
...
int NDFiberSection3d::recvSelf(int commitTag, Channel & theChannel,
                               FEM_ObjectBroker & theBroker)
{
    ...
    if (data(1) != 0) {
        ...
        if (res < 0) {
            opserr <<
                "NDFiberSection3d::recvSelf - failed to recv material data\n";
            return res;
        }
        ...
        if (theMaterials == 0 || numFibers != data(1)) {
            ...
            if (numFibers != 0) {
                ...
                if (theMaterials == 0) {
                    opserr <<
                        "NDFiberSection3d::recvSelf -- failed to allocate Material pointers\n";
                    exit(-1);
                }
                ...
                if (matData == 0) {
                    opserr <<
                        "NDFiberSection3d::recvSelf  -- failed to allocate double array for material data\n";
                    exit(-1);
                }
            }
        }
...
void NDFiberSection3d::Print(OPS_Stream & s, int flag)
{
...
Response *NDFiberSection3d::setResponse(const char **argv, int argc,
                                        OPS_Stream & output)
{
...
int NDFiberSection3d::getResponse(int responseID, Information & sectInfo)
{
...
int NDFiberSection3d::setParameter(const char **argv, int argc,
                                   Parameter & param)
{
    if (argc < 1)
        return -1;
...
int NDFiberSection3d::updateParameter(int paramID, Information & info)
{
    switch (paramID) {
    case 1:
...
int NDFiberSection3d::activateParameter(int paramID)
{
    parameterID = paramID;
...
const Vector &
    NDFiberSection3d::getStressResultantSensitivity(int gradIndex,
                                                    bool conditional)
{
    static Vector ds(6);
                                     ...
                                     int gradIndex, int numGrads)
{
    ...
    for (int i = 0; i < numFibers; i++) {
        ...
        depsdh(1) =
            rootAlpha * d3 - z * d5 + drootAlphadh * e(3) - dzdh[i] * e(5);
        depsdh(2) =
            rootAlpha * d4 + y * d5 + drootAlphadh * e(4) + dydh[i] * e(5);
