...
// constructors:
FiberSectionGJThermal::FiberSectionGJThermal(int tag, int num,
                                             Fiber ** fibers,
                                             double
                                             gj):SectionForceDeformation
    (tag, SEC_TAG_FiberSectionGJThermal), numFibers(num), sizeFibers(num),
theMaterials(0), matData(0), dataMixed(25), QzBar(0.0), QyBar(0.0),
ABar(0.0), yBar(0.0), zBar(0.0), e(4), eCommit(4), GJ(gj),
AverageThermalElong(3)
{
    if (numFibers != 0) {
        ...
        if (theMaterials == 0) {
            opserr <<
                "FiberSectionGJThermal::FiberSectionGJThermal -- failed to allocate Material pointers\n";
            exit(-1);
        }
        ...
        if (matData == 0) {
            opserr <<
                "FiberSectionGJThermal::FiberSectionGJThermal -- failed to allocate double array for material data\n";
            exit(-1);
        }
        double Qz = 0.0;
        double Qy = 0.0;
        ...
        for (int i = 0; i < numFibers; i++) {
            ...
            if (theMaterials[i] == 0) {
                opserr <<
                    "FiberSectionGJThermal::FiberSectionGJThermal -- failed to get copy of a Material\n";
                exit(-1);
            }
        }
...
FiberSectionGJThermal::FiberSectionGJThermal(int tag, int num,
                                             double
                                             gj):SectionForceDeformation
    (tag, SEC_TAG_FiberSectionGJThermal), numFibers(0), sizeFibers(num),
theMaterials(0), matData(0), dataMixed(25), QzBar(0.0), QyBar(0.0),
ABar(0.0), yBar(0.0), zBar(0.0), e(4), eCommit(4), GJ(gj),
AverageThermalElong(3)
{
...
int FiberSectionGJThermal::addFiber(Fiber & newFiber)
{
 ...
 FiberSectionGJThermal::setTrialSectionDeformation(const Vector & deforms)
{
    ...
    for (int i = 0; i < numFibers; i++) {
        ...
        FiberTemperature =
            this->determineFiberTemperature(dataMixed, -yi - yBar,
                                            zi + zBar);
...
const Matrix & FiberSectionGJThermal::getInitialTangent(void)
{
    kData[0] = 0.0;
    kData[1] = 0.0;
    kData[2] = 0.0;
...
const Vector & FiberSectionGJThermal::getSectionDeformation(void)
{
    return e;
}

const Matrix & FiberSectionGJThermal::getSectionTangent(void)
{
    ks(0, 0) = kData[0];
    ks(0, 1) = ks(1, 0) = kData[1];
    ks(0, 2) = ks(2, 0) = kData[2];
...
const Vector & FiberSectionGJThermal::getStressResultant(void)
{
    s(0) = sData[0];
    s(1) = sData[1];
    s(2) = sData[2];
    ...
    FiberSectionGJThermal::getTemperatureStress(const Vector & DataMixed)
{
    ...
    for (int i = 0; i < numFibers; i++) {
        FiberForce =
            ThermalTangent[i] * matData[3 * i + 2] * DeltaThermalElong[i];
        SectionArea += matData[3 * i + 2];

        SectionMomofArea1 +=
            (matData[3 * i + 2] * (matData[3 * i] - yBar));
        SectionMomofArea2 +=
            (matData[3 * i + 2] * (matData[3 * i + 1] - zBar));
        ThermalForce += Fiber_ElongP[i] * matData[3 * i + 2];
        ThermalMoment1 +=
            Fiber_ElongP[i] * matData[3 * i + 2] * (matData[3 * i] - yBar);
        ThermalMoment2 +=
            Fiber_ElongP[i] * matData[3 * i + 2] * (matData[3 * i + 1] -
                                                    zBar);
...
const Vector & FiberSectionGJThermal::getThermalElong(void)
{
...
SectionForceDeformation *FiberSectionGJThermal::getCopy(void)
{
    ...
    if (numFibers != 0) {
        ...
        for (int i = 0; i < numFibers; i++) {
            ...
            if (theCopy->theMaterials[i] == 0) {
                opserr <<
                    "FiberSectionGJThermal::getCopy -- failed to get copy of a Material\n";
                exit(-1);
            }
        }
    }
...
const ID & FiberSectionGJThermal::getType()
{
    return code;
}

int FiberSectionGJThermal::getOrder() const
{
    return 4;
}

int FiberSectionGJThermal::commitState(void)
{
    int err = 0;
...
int FiberSectionGJThermal::revertToLastCommit(void)
{
    int err = 0;
...
int FiberSectionGJThermal::revertToStart(void)
{
...
int FiberSectionGJThermal::sendSelf(int commitTag, Channel & theChannel)
{
    int res = 0;
...
int FiberSectionGJThermal::recvSelf(int commitTag, Channel & theChannel,
                                    FEM_ObjectBroker & theBroker)
{
    ...
    if (numFibers != 0) {
        ...
        if (res < 0) {
            opserr <<
                "FiberSection2d::recvSelf - failed to send material data\n";
            return res;
        }
...
void FiberSectionGJThermal::Print(OPS_Stream & s, int flag)
{
...
Response *FiberSectionGJThermal::setResponse(const char **argv, int argc,
                                             OPS_Stream & output)
{

    // See if the response is one of the defaults
    Response *theResponse =
        SectionForceDeformation::setResponse(argv, argc, output);
    if (theResponse != 0)
        return theResponse;
    ...
    if (key < numFibers && key >= 0) {
        ...
        theResponse =
            theMaterials[key]->setResponse(&argv[passarg], argc - passarg,
                                           output);
...
int FiberSectionGJThermal::getResponse(int responseID,
...
int FiberSectionGJThermal::setParameter(const char **argv, int argc,
                                        Parameter & param)
{
    if (argc < 3)
        return 0;
                          ...
                          double fiberLocz)
{
    ...
    if (DataMixed.Size() == 18) {
        ...
        if (fiberLocy <= dataTempe[1]) {
            opserr << "FiberSectionGJThermal " << this->getTag() <<
                ":: fiber locy " << fiberLocy <<
                " is out of the section below " << dataTempe[1] << endln;
            ...
            opserr << "FiberSectionGJThermal " << this->getTag() <<
                " :: fiber loc " << fiberLocy <<
                " is out of the section over" << dataTempe[17] << endln;
        }
        ...
        if (fiberLocy <= dataTempe[1]) {
            if (fiberLocz <= dataTempe[12]) {
                opserr <<
                    "WARNING: FiberSectionGJThermal failed to find the fiber with locy: "
