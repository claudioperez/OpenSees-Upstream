...
FiberSection::FiberSection(int tag, int num,
                           Fiber ** fibers):SectionForceDeformation(tag,
                                                                    SEC_TAG_Fiber),
numFibers(num), theFibers(0), sizeFibers(num), e(0), eCommit(0), s(0),
ks(0), order(0), code(0), otherDbTag(0)
{
    theFibers = new Fiber *[numFibers];

    if (theFibers == 0) {
        opserr <<
            "FiberSection::FiberSection -- failed to allocate Fiber pointers";
        exit(-1);
    }
    ...
    for (int i = 0; i < numFibers; i++) {
        ...
        if (theFibers[i] == 0) {
            opserr <<
                "FiberSection::FiberSection -- failed to get copy of Fiber";
            exit(-1);
        }
...
FiberSection::FiberSection(int tag, int num):SectionForceDeformation(tag,
                                                                     SEC_TAG_Fiber),
numFibers(0), theFibers(0), sizeFibers(num), e(0), eCommit(0), s(0), ks(0),
order(0), code(0), otherDbTag(0)
{
 ...
 FiberSection::addFiber(Fiber & newFiber)
{
    ...
    if (numFibers < sizeFibers) {
        ...
        if (newArray == 0) {
            opserr <<
                "FiberSection::addFiber -- failed to allocate Fiber pointers";
            exit(-1);
...
int
 FiberSection::setTrialSectionDeformation(const Vector & deforms)
{
    int res = 0;
...
const Vector & FiberSection::getSectionDeformation(void)
{
    return *e;
}

const Matrix & FiberSection::getSectionTangent(void)
{
    ks->Zero();
...
const Matrix & FiberSection::getInitialTangent(void)
{
    ks->Zero();
...
const Vector & FiberSection::getStressResultant(void)
{
    s->Zero();
...
SectionForceDeformation *FiberSection::getCopy(void)
{
    FiberSection *theCopy =
        new FiberSection(this->getTag(), numFibers, theFibers);
...
const ID & FiberSection::getType()
{
    return *code;
}

int FiberSection::getOrder() const
{
    return order;
}

int FiberSection::commitState(void)
{
...
int FiberSection::revertToLastCommit(void)
{
    int err = 0;
...
int FiberSection::revertToStart(void)
{
...
int FiberSection::sendSelf(int commitTag, Channel & theChannel)
{
    int res = 0;
    ...
    if (order > 0) {
        ...
        res +=
            theChannel.sendVector(this->getDbTag(), commitTag, *eCommit);
        if (res < 0) {
            opserr <<
                "FiberSection::sendSelf -- failed to send section deformations";
            return res;
        }
    }
    ...
    if (numFibers > 0) {
        ...
        for (i = 0; i < numFibers; i++) {
            ...
            if (res < 0) {
                opserr <<
                    "FiberSection::sendSelf -- failed to send Fiber %d";
                return res;
            }
        }
    }
...
int FiberSection::recvSelf(int commitTag, Channel & theChannel,
                           FEM_ObjectBroker & theBroker)
{
    ...
    if (order > 0) {
        ...
        res +=
            theChannel.recvVector(this->getDbTag(), commitTag, *eCommit);
        if (res < 0) {
            opserr <<
                "FiberSection::recvSelf -- failed to receive section deformations";
            return res;
        }
    ...
    if (numFibers > 0) {
        ...
        if (res < 0) {
            opserr <<
                "FiberSection::recvSelf -- failed to receive dbTags ID";
            return res;
        }
        ...
        if (theFibers == 0) {
            ...
            if (theFibers == 0) {
                opserr <<
                    "FiberSection::recvSelf -- failed to allocate Fiber pointers";
                return -1;
            }
        ...
        for (i = 0; i < numFibers; i++) {
            ...
            if (theFibers[i] == 0) {
                opserr <<
                    "FiberSection::recvSelf -- could not get Fiber %d";
                return -1;
            }
            ...
            if (res < 0) {
                opserr <<
                    "FiberSection::recvSelf -- could not receive Fiber %d";
                return res;
            }
        }
...
void FiberSection::Print(OPS_Stream & s, int flag)
{
...
Response *FiberSection::setResponse(const char **argv, int argc,
                                    OPS_Stream & output)
{
    ...
    if (strcmp(argv[0], "fiber") == 0) {
        ...
        if (key < numFibers && key >= 0) {
            ...
            theResponse =
                theFibers[key]->setResponse(&argv[passarg],
                                            argc - passarg, output);
            output.endTag();
        }
    }
...
int FiberSection::getResponse(int responseID, Information & sectInfo)
{
