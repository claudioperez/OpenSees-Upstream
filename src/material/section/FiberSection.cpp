    ...
    for (int i = 0; i < numFibers; i++)
      {
          ...
          s->addVector (1.0, theFibers[i]->getFiberStressResultants (), 1.0);
          ks->addMatrix (1.0, theFibers[i]->getFiberTangentStiffContr (),
                         1.0);
      }
    ...
    if (numFibers > 0)
      {
          ...
          if (res < 0)
            {
                opserr <<
                    "FiberSection::sendSelf -- failed to send dbTags ID";
                return res;
            }
...
int
FiberSection::recvSelf (int commitTag, Channel & theChannel,
