...
ABar(0.0), yBar(0.0), computeCentroid(compCentroid), sectionIntegr(0),
e(2), s(0), ks(0), dedh(2)
{
    if (numFibers > 0) {
...
e(2), s(0), ks(0), dedh(2)
{
    ...
    if (sectionIntegr == 0) {
        opserr <<
            "Error: FiberSection2d::FiberSection2d: could not create copy of section integration object"
            << endln;
        exit(-1);
    }
...
int FiberSection2d::addFiber(Fiber & newFiber)
{
...
const Vector & FiberSection2d::getSectionDeformation(void)
{
    return e;
}

const Matrix & FiberSection2d::getInitialTangent(void)
{
...
const Matrix & FiberSection2d::getSectionTangent(void)
{
    return *ks;
}

const Vector & FiberSection2d::getStressResultant(void)
{
    return *s;
}

SectionForceDeformation *FiberSection2d::getCopy(void)
{
    ...
    if (numFibers != 0) {
        ...
        if (theCopy->theMaterials == 0) {
            opserr <<
                "FiberSection2d::getCopy -- failed to allocate Material pointers\n";
            exit(-1);
        }
        ...
        if (theCopy->matData == 0) {
            opserr <<
                "FiberSection2d::getCopy -- failed to allocate double array for material data\n";
            exit(-1);
        }
        ...
        for (int i = 0; i < numFibers; i++) {
            ...
            if (theCopy->theMaterials[i] == 0) {
                opserr <<
                    "FiberSection2d::getCopy -- failed to get copy of a Material";
                exit(-1);
            }
        }
    }
...
int FiberSection2d::getOrder() const
{
    return 2;
}

int FiberSection2d::commitState(void)
{
    int err = 0;
...
int FiberSection2d::revertToLastCommit(void)
{
    int err = 0;
...
int FiberSection2d::revertToStart(void)
{
...
int FiberSection2d::sendSelf(int commitTag, Channel & theChannel)
{
    int res = 0;
...
int FiberSection2d::recvSelf(int commitTag, Channel & theChannel,
                             FEM_ObjectBroker & theBroker)
{
    ...
    if (data(1) != 0) {
        ...
        if (res < 0) {
            opserr <<
                "FiberSection2d::recvSelf - failed to recv material data\n";
            return res;
        }
...
void FiberSection2d::Print(OPS_Stream & s, int flag)
{
...
Response *FiberSection2d::setResponse(const char **argv, int argc,
                                      OPS_Stream & output)
{
    ...
    if (argc > 2 || strcmp(argv[0], "fiber") == 0) {
        ...
        return theResponse =
            new MaterialResponse(this, 5, theResponseData);
...
int FiberSection2d::getResponse(int responseID, Information & sectInfo)
{
    if (responseID == 5) {
...
int FiberSection2d::setParameter(const char **argv, int argc,
                                 Parameter & param)
{
    if (argc < 1)
        return -1;
    ...
    if (strstr(argv[0], "fiber") != 0) {
        ...
        if (key >= 0 && key < numFibers)
            return theMaterials[key]->setParameter(&argv[passarg],
                                                   argc - passarg, param);
    }
...
const Vector &
    FiberSection2d::getStressResultantSensitivity(int gradIndex,
                                                  bool conditional)
{
    static Vector ds(2);
...
const Matrix & FiberSection2d::getInitialTangentSensitivity(int gradIndex)
{
...
int FiberSection2d::commitSensitivity(const Vector & defSens,
