...
void *OPS_TubeSection()
{
    if (OPS_GetNumRemainingInputArgs() < 6) {
        ...
        opserr <<
            "Want: section Tube tag? matTag? D? t? nfw? nfr? <-nd shape?>"
            << endln;
        return 0;
    }
    ...
    if (OPS_GetNumRemainingInputArgs() > 0) {
        ...
        if (OPS_GetNumRemainingInputArgs() > 0) {
            ...
            if (strcmp(flag, "-nd") == 0) {
                ...
                opserr <<
                    "TubeSection -- not implemented yet for fiber warping section"
                    << endln;
                theSection = 0;
        ...
        // Parsing was successful, allocate the section
        theSection =
            new FiberSection3d(tag, numFibers, theMats, tubesect,
                               *torsion);
...
TubeSectionIntegration::TubeSectionIntegration(double DIAM,
                                               double T, int NFW,
                                               int
                                               NFR):SectionIntegration
    (SECTION_INTEGRATION_TAG_Tube), D(DIAM), t(T), Nfwedge(NFW),
Nfring(NFR), parameterID(0)
{
...
int TubeSectionIntegration::arrangeFibers(UniaxialMaterial ** theMaterials,
...
int TubeSectionIntegration::arrangeFibers(NDMaterial ** theMaterials,
                                          NDMaterial * theSteel)
{
    int numFibers = this->getNumFibers();
...
void TubeSectionIntegration::getFiberLocations(int nFibers, double *yi,
                                               double *zi)
{
...
void TubeSectionIntegration::getFiberWeights(int nFibers, double *wt)
{
    static const double pi = 3.141592653589793;
...
SectionIntegration *TubeSectionIntegration::getCopy(void)
{
    return new TubeSectionIntegration(D, t, Nfwedge, Nfring);
}

int TubeSectionIntegration::setParameter(const char **argv, int argc,
                                         Parameter & param)
{
    if (argc < 1)
        return -1;
...
int TubeSectionIntegration::updateParameter(int parameterID,
                                            Information & info)
{
    switch (parameterID) {
    case 1:
...
int TubeSectionIntegration::activateParameter(int paramID)
{
    parameterID = paramID;
...
void TubeSectionIntegration::getLocationsDeriv(int nFibers, double *dyidh,
                                               double *dzidh)
{
...
void TubeSectionIntegration::getWeightsDeriv(int nFibers, double *dwtdh)
{
...
void TubeSectionIntegration::Print(OPS_Stream & s, int flag)
{
    s << "Tube" << endln;
    s << " D = " << D;
    s << " t = " << t;
...
int TubeSectionIntegration::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (theChannel.sendVector(dbTag, cTag, data) < 0) {
        opserr <<
            "TubeSectionIntegration::sendSelf() - failed to send Vector data\n";
        return -1;
    }
...
int TubeSectionIntegration::recvSelf(int cTag, Channel & theChannel,
                                     FEM_ObjectBroker & theBroker)
{
    ...
    if (theChannel.recvVector(dbTag, cTag, data) < 0) {
        opserr <<
            "TubeSectionIntegration::recvSelf() - failed to receive Vector data\n";
        return -1;
    }
