...
void *OPS_WFSection2d()
{
    if (OPS_GetNumRemainingInputArgs() < 8) {
        ...
        opserr <<
            "Want: section WFSection2d tag? matTag? d? tw? bf? tf? nfdw? nftf? <-nd shape?>"
            << endln;
        return 0;
    }
    ...
    if (OPS_GetNumRemainingInputArgs() > 0) {
        ...
        if (OPS_GetNumRemainingInputArgs() > 0) {
            ...
            if (strcmp(flag, "-nd") == 0) {
                theSection =
                    new NDFiberSection2d(tag, numFibers, theMats,
                                         wfsect, shape);
            } else if (strcmp(flag, "-ndWarping") == 0) {
                theSection =
                    new NDFiberSectionWarping2d(tag, numFibers,
                                                theMats, wfsect, shape);
            }
        }
        delete[]theMats;
    } else {
...
WideFlangeSectionIntegration::WideFlangeSectionIntegration(double D,
                                                           ...
                                                           int NFDW,
                                                           int
                                                           NFTF):SectionIntegration
    (SECTION_INTEGRATION_TAG_WideFlange), d(D), tw(TW), bf(BF), tf(TF),
Nfdw(NFDW), Nftf(NFTF), parameterID(0)
{
...
int WideFlangeSectionIntegration::arrangeFibers(UniaxialMaterial **
                                                theMaterials,
                                                UniaxialMaterial *
                                                theSteel)
{
...
int WideFlangeSectionIntegration::arrangeFibers(NDMaterial ** theMaterials,
                                                NDMaterial * theSteel)
{
    int numFibers = this->getNumFibers();
...
void WideFlangeSectionIntegration::getFiberWeights(int nFibers, double *wt)
{
    double dw = d - 2 * tf;
...
SectionIntegration *WideFlangeSectionIntegration::getCopy(void)
{
    return new WideFlangeSectionIntegration(d, tw, bf, tf, Nfdw, Nftf);
}

int WideFlangeSectionIntegration::setParameter(const char **argv, int argc,
                                               Parameter & param)
{
    if (argc < 1)
        return -1;
...
int WideFlangeSectionIntegration::activateParameter(int paramID)
{
    parameterID = paramID;
...
void WideFlangeSectionIntegration::Print(OPS_Stream & s, int flag)
{
    s << "WideFlange" << endln;
    s << " d = " << d;
    s << " tw = " << tw;
...
int WideFlangeSectionIntegration::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (theChannel.sendVector(dbTag, cTag, data) < 0) {
        opserr <<
            "WideFlangeSectionIntegration::sendSelf() - failed to send Vector data\n";
        return -1;
    }
...
int WideFlangeSectionIntegration::recvSelf(int cTag, Channel & theChannel,
                                           FEM_ObjectBroker & theBroker)
{
    ...
    if (theChannel.recvVector(dbTag, cTag, data) < 0) {
        opserr <<
            "WideFlangeSectionIntegration::recvSelf() - failed to receive Vector data\n";
        return -1;
    }
