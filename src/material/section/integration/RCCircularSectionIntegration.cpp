...
void *OPS_RCCircularSection()
{
    if (OPS_GetNumRemainingInputArgs() < 13) {
        ...
        opserr <<
            "Want: section RCCircularSection tag? coreTag? coverTag? steelTag? d? cover? As? NringsCore? NringsCover? Nwedges? Nsteel? -GJ GJ <or> -torsion matTag\n";
        return 0;
    }
    ...
    int tag = idata[0], coreTag = idata[1], coverTag = idata[2], steelTag =
        idata[3];
    double d = ddata[0], cover = ddata[1], As = ddata[2];
    int ncore = idata[4], ncover = idata[5], nwedge = idata[6], nsteel =
        idata[7];
    ...
    RCCircularSectionIntegration rcsect(d, As, cover, ncore, ncover,
                                        nwedge, nsteel);
...
RCCircularSectionIntegration::RCCircularSectionIntegration(double D,
    ...
    (SECTION_INTEGRATION_TAG_RCCIRCULAR), d(D), As(AS), cover(COVER),
...
NringsCore(1), NringsCover(1), Nwedges(2), Nsteel(1), parameterID(0)
{
...
int RCCircularSectionIntegration::arrangeFibers(UniaxialMaterial **
                                                theMaterials,
...
void RCCircularSectionIntegration::getFiberLocations(int nFibers,
                                                     double *yi,
                                                     double *zi)
{
...
void RCCircularSectionIntegration::getFiberWeights(int nFibers, double *wt)
{
    static const double pi = 3.141592653589793;
...
SectionIntegration *RCCircularSectionIntegration::getCopy(void)
{
    return new RCCircularSectionIntegration(d, As, cover, NringsCore,
                                            NringsCover, Nwedges, Nsteel);
}

int RCCircularSectionIntegration::setParameter(const char **argv, int argc,
                                               Parameter & param)
{
    if (argc < 1)
        return -1;
...
int RCCircularSectionIntegration::updateParameter(int parameterID,
                                                  Information & info)
{
    switch (parameterID) {
    case 1:
...
int RCCircularSectionIntegration::activateParameter(int paramID)
{
    parameterID = paramID;
...
void RCCircularSectionIntegration::getLocationsDeriv(int nFibers,
                                                     double *dyidh,
                                                     double *dzidh)
{
...
void RCCircularSectionIntegration::getWeightsDeriv(int nFibers,
                                                   double *dwtsdh)
{
    double dddh = 0.0;
    double dAsdh = 0.0;
...
void RCCircularSectionIntegration::Print(OPS_Stream & s, int flag)
{
...
int RCCircularSectionIntegration::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (theChannel.sendVector(dbTag, cTag, data) < 0) {
        opserr <<
            "RCCircularSectionIntegration::sendSelf() - failed to send Vector data\n";
        return -1;
    }
...
int RCCircularSectionIntegration::recvSelf(int cTag, Channel & theChannel,
                                           FEM_ObjectBroker & theBroker)
{
    ...
    if (theChannel.recvVector(dbTag, cTag, data) < 0) {
        opserr <<
            "RCCircularSectionIntegration::recvSelf() - failed to receive Vector data\n";
        return -1;
    }
