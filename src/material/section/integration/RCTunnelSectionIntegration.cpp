...
void *OPS_RCTunnelSection()
{
    if (OPS_GetNumRemainingInputArgs() < 13) {
        ...
        opserr <<
            "Want: section RCTunnelSection tag? concreteTag? steelTag? d? h? coverinner? coverouter? Asinner? Asouter? Nrings? Nwedges? Nbarsinner? Nbarsouter?\n";
        return 0;
    }
    ...
    double d = ddata[0], h = ddata[1], coverinner = ddata[2], coverouter =
        ddata[3], Asinner = ddata[4], Asouter = ddata[5];
    int nring = idata[3], nwedge = idata[4], nbarsinner =
        idata[5], nbarsouter = idata[6];
    ...
    RCTunnelSectionIntegration rcsect(d, h, Asinner, Asouter, coverinner,
                                      coverouter, nring, nwedge,
                                      nbarsinner, nbarsouter);
    ...
    SectionForceDeformation *theSection =
        new FiberSection3d(tag, numFibers, theMats, rcsect, *torsion);
...
RCTunnelSectionIntegration::RCTunnelSectionIntegration(double D, double H,
                                                       ...
                                                       int NBARin,
                                                       int
                                                       NBARout):SectionIntegration
    (SECTION_INTEGRATION_TAG_RCTUNNEL), d(D), h(H), Asinner(ASin),
Asouter(ASout), coverinner(COVERin), coverouter(COVERout), Nrings(NRINGS),
Nwedges(NWEDGES), Nbarsinner(NBARin), Nbarsouter(NBARout), parameterID(0)
{
    /*
       if (Nrings < 1)
       Nrings = 1;
...
d(0.0), h(0.0), Asinner(0.0), Asouter(0.0), coverinner(0.0),
coverouter(0.0), Nrings(1), Nwedges(2), Nbarsinner(1), Nbarsouter(1),
parameterID(0)
{
...
int RCTunnelSectionIntegration::arrangeFibers(UniaxialMaterial **
                                              theMaterials,
                                              UniaxialMaterial *
                                              theConcrete,
...
void RCTunnelSectionIntegration::getFiberLocations(int nFibers, double *yi,
                                                   double *zi)
{
...
void RCTunnelSectionIntegration::getFiberWeights(int nFibers, double *wt)
{
    static const double pi = 3.141592653589793;
...
SectionIntegration *RCTunnelSectionIntegration::getCopy(void)
{
    return new RCTunnelSectionIntegration(d, h, Asinner, Asouter,
                                          coverinner, coverouter, Nrings,
                                          Nwedges, Nbarsinner, Nbarsouter);
}

int RCTunnelSectionIntegration::setParameter(const char **argv, int argc,
                                             Parameter & param)
{
    if (argc < 1)
        return -1;
...
int RCTunnelSectionIntegration::updateParameter(int parameterID,
                                                Information & info)
{
    switch (parameterID) {
    case 1:
...
int RCTunnelSectionIntegration::activateParameter(int paramID)
{
    parameterID = paramID;
...
void RCTunnelSectionIntegration::getLocationsDeriv(int nFibers,
                                                   double *dyidh,
                                                   double *dzidh)
{
    double dddh = 0.0;
    ...
    for (int i = 0; i < Nrings; i++) {
        ...
        double dxbardh =
            (area *
             (xouter * dAouterdh + dxouterdh * Aouter -
              xinner * dAinnerdh - dxinnerdh * Ainner) - (xouter * Aouter -
                                                          xinner *
                                                          Ainner) *
             dareadh) / (area * area);
        double angle = theta;
...
void RCTunnelSectionIntegration::getWeightsDeriv(int nFibers,
                                                 double *dwtsdh)
{
    double dddh = 0.0;
    double dhdh = 0.0;
...
void RCTunnelSectionIntegration::Print(OPS_Stream & s, int flag)
{
...
int RCTunnelSectionIntegration::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (theChannel.sendVector(dbTag, cTag, data) < 0) {
        opserr <<
            "RCTunnelSectionIntegration::sendSelf() - failed to send Vector data\n";
        return -1;
    }
...
int RCTunnelSectionIntegration::recvSelf(int cTag, Channel & theChannel,
                                         FEM_ObjectBroker & theBroker)
{
    ...
    if (theChannel.recvVector(dbTag, cTag, data) < 0) {
        opserr <<
            "RCTunnelSectionIntegration::recvSelf() - failed to receive Vector data\n";
        return -1;
    }
