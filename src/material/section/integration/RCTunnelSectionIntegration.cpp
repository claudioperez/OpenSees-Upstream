...
OPS_RCTunnelSection ()
{
    ...
    RCTunnelSectionIntegration rcsect (d, h, Asinner, Asouter, coverinner,
                                       coverouter, nring, nwedge, nbarsinner,
                                       nbarsouter);
...
void
RCTunnelSectionIntegration::getFiberLocations (int nFibers, double *yi,
                                               double *zi)
{
...
{
    return new RCTunnelSectionIntegration (d, h, Asinner, Asouter, coverinner,
                                           coverouter, Nrings, Nwedges,
...
void
RCTunnelSectionIntegration::getLocationsDeriv (int nFibers, double *dyidh,
                                               double *dzidh)
{
    double dddh = 0.0;
    double dhdh = 0.0;
    ...
    for (int i = 0; i < Nrings; i++)
      {
          ...
          double dxbardh =
              (area *
               (xouter * dAouterdh + dxouterdh * Aouter - xinner * dAinnerdh -
                dxinnerdh * Ainner) - (xouter * Aouter -
                                       xinner * Ainner) * dareadh) / (area *
                                                                      area);
          double angle = theta;
          for (int j = 0; j < Nwedges; j++)
            {
