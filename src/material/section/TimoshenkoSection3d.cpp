...
// constructors:
TimoshenkoSection3d::TimoshenkoSection3d(int tag, int num,
                                         NDMaterial ** fibers, double *y,
                                         double *z,
                                         double
                                         *A):SectionForceDeformation(tag,
                                                                     SEC_TAG_TimoshenkoSection3d),
numFibers(num), theMaterials(0), matData(0), yBar(0.0), zBar(0.0), e(6),
s(0), ks(0)
{
    if (numFibers != 0) {
        ...
        if (theMaterials == 0) {
            opserr <<
                "TimoshenkoSection3d::TimoshenkoSection3d -- failed to allocate Material pointers\n";
            exit(-1);
        }
        ...
        if (matData == 0) {
            opserr <<
                "TimoshenkoSection3d::TimoshenkoSection3d -- failed to allocate double array for material data\n";
            exit(-1);
        }
        ...
        for (int i = 0; i < numFibers; i++) {
            ...
            if (theMaterials[i] == 0) {
                opserr <<
                    "TimoshenkoSection3d::TimoshenkoSection3d -- failed to get copy of a Material\n";
                exit(-1);
            }
        }
...
const Matrix & TimoshenkoSection3d::getInitialTangent(void)
{
    kData[0] = 0.0;
    kData[1] = 0.0;
    kData[2] = 0.0;
...
const Vector & TimoshenkoSection3d::getSectionDeformation(void)
{
    return e;
}

const Matrix & TimoshenkoSection3d::getSectionTangent(void)
{
    return *ks;
}

const Vector & TimoshenkoSection3d::getStressResultant(void)
{
    return *s;
}

SectionForceDeformation *TimoshenkoSection3d::getCopy(void)
{
    ...
    if (numFibers != 0) {
        ...
        for (int i = 0; i < numFibers; i++) {
            ...
            if (theCopy->theMaterials[i] == 0) {
                opserr <<
                    "TimoshenkoSection3d::getCopy -- failed to get copy of a Material\n";
                exit(-1);
            }
        }
    }
...
const ID & TimoshenkoSection3d::getType()
{
    return code;
}

int TimoshenkoSection3d::getOrder() const
{
    return 6;
}

int TimoshenkoSection3d::commitState(void)
{
    int err = 0;
...
int TimoshenkoSection3d::revertToLastCommit(void)
{
    int err = 0;
...
int TimoshenkoSection3d::revertToStart(void)
{
...
int TimoshenkoSection3d::sendSelf(int commitTag, Channel & theChannel)
{
    int res = 0;
    ...
    if (res < 0) {
        opserr <<
            "TimoshenkoSection2d::sendSelf - failed to send ID data\n";
        return res;
    }
    ...
    if (numFibers != 0) {
        ...
        if (res < 0) {
            opserr <<
                "TimoshenkoSection2d::sendSelf - failed to send material data\n";
            return res;
        }
...
int TimoshenkoSection3d::recvSelf(int commitTag, Channel & theChannel,
                                  FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0) {
        opserr <<
            "TimoshenkoSection2d::sendSelf - failed to recv ID data\n";
        return res;
    }
    ...
    if (data(1) != 0) {
        ...
        if (theMaterials == 0 || numFibers != data(1)) {
            ...
            if (numFibers != 0) {
                ...
                if (theMaterials == 0) {
                    opserr <<
                        "TimoshenkoSection2d::recvSelf -- failed to allocate Material pointers\n";
                    exit(-1);
                }
                ...
                if (matData == 0) {
                    opserr <<
                        "TimoshenkoSection2d::recvSelf  -- failed to allocate double array for material data\n";
                    exit(-1);
                }
            }
        }
...
void TimoshenkoSection3d::Print(OPS_Stream & s, int flag)
{
    if (flag == 2) {
        int loc = 0;
        for (int i = 0; i < numFibers; i++) {
            s << -matData[loc] << " " << matData[loc +
                                                 1] << " " << matData[loc
                                                                      + 2]
                << " ";
            s << theMaterials[i]->
...
Response *TimoshenkoSection3d::setResponse(const char **argv, int argc,
                                           OPS_Stream & output)
{
    ...
    // Check if fiber response is requested
        ...
        if (argc <= 3) {        // fiber number was input directly
            ...
            if (key < numFibers && key >= 0)
                return theMaterials[key]->setResponse(&argv[passarg],
                                                      argc - passarg,
                                                      output);
            else
                return 0;
...
int TimoshenkoSection3d::getResponse(int responseID,
...
int TimoshenkoSection3d::setParameter(const char **argv, int argc,
                                      Parameter & param)
{
    if (argc < 1)
        return -1;
