...
void *OPS_FiberSectionWarping3d()
{
    ...
    SectionForceDeformation *section =
        new FiberSectionWarping3d(tag, num, *torsion);
    if (deleteTorsion)
        delete torsion;
    return section;
}
...
// constructors:
FiberSectionWarping3d::FiberSectionWarping3d(int tag, int num,
                                             Fiber ** fibers,
                                             UniaxialMaterial &
                                             torsion):SectionForceDeformation
    (tag, SEC_TAG_FiberSectionWarping3d), numFibers(num), sizeFibers(num),
theMaterials(0), matData(0), yBar(0.0), zBar(0.0), e(8), eCommit(8), s(0),
ks(0), theTorsion(0)
{
    if (numFibers != 0) {
        ...
        if (theMaterials == 0) {
            opserr <<
                "FiberSectionWarping3d::FiberSectionWarping3d -- failed to allocate Material pointers\n";
            exit(-1);
        }
        ...
        if (matData == 0) {
            opserr <<
                "FiberSectionWarping3d::FiberSectionWarping3d -- failed to allocate double array for material data\n";
            exit(-1);
        }
        ...
        for (int i = 0; i < numFibers; i++) {
            ...
            if (theMaterials[i] == 0) {
                opserr <<
                    "FiberSectionWarping3d::FiberSectionWarping3d -- failed to get copy of a Material\n";
                exit(-1);
            }
        }
    ...
    if (theTorsion == 0)
        opserr <<
            "FiberSectionWarping3d::FiberSectionWarping3d -- failed to get copy of torsion material\n";
...
FiberSectionWarping3d::FiberSectionWarping3d(int tag, int num,
                                             UniaxialMaterial &
                                             torsion):SectionForceDeformation
    (tag, SEC_TAG_FiberSectionWarping3d), numFibers(0), sizeFibers(num),
theMaterials(0), matData(0), yBar(0.0), zBar(0.0), e(8), eCommit(8), s(0),
ks(0), theTorsion(0)
{
    if (sizeFibers != 0) {
        ...
        if (theMaterials == 0) {
            opserr <<
                "FiberSection3d::FiberSection3d -- failed to allocate Material pointers\n";
            exit(-1);
        }
    ...
    if (theTorsion == 0)
        opserr <<
            "FiberSection3d::FiberSection3d -- failed to get copy of torsion material\n";
...
int FiberSectionWarping3d::addFiber(Fiber & newFiber)
{
    ...
    if (newArray == 0 || newMatData == 0) {
        opserr <<
            "FiberSectionWarping3d::addFiber -- failed to allocate Fiber pointers\n";
        exit(-1);
    }
    ...
    if (newArray[numFibers] == 0) {
        opserr <<
            "FiberSectionWarping3d::addFiber -- failed to get copy of a Material\n";
        exit(-1);
 ...
 FiberSectionWarping3d::setTrialSectionDeformation(const Vector & deforms)
{
    ...
    for (int i = 0; i < numFibers; i++) {
        ...
        double strain =
            d0 - y * d1 - z * d2 - omig * d3 + 0.5 * d5 * d5 +
            0.5 * d6 * d6 + 0.5 * (y * y + z * z) * d4 * d4 - y * d7 * d2 +
            z * d7 * d1;
...
const Matrix & FiberSectionWarping3d::getInitialTangent(void)
{
...
const Vector & FiberSectionWarping3d::getSectionDeformation(void)
{
    return e;
}

const Matrix & FiberSectionWarping3d::getSectionTangent(void)
{
    return *ks;
}

const Vector & FiberSectionWarping3d::getStressResultant(void)
{
    return *s;
}

SectionForceDeformation *FiberSectionWarping3d::getCopy(void)
{
    ...
    if (numFibers != 0) {
        ...
        for (int i = 0; i < numFibers; i++) {
            ...
            if (theCopy->theMaterials[i] == 0) {
                opserr <<
                    "FiberSectionWarping3d::getCopy -- failed to get copy of a Material\n";
                exit(-1);
            }
        }
    }
...
const ID & FiberSectionWarping3d::getType()
{
    return code;
}

int FiberSectionWarping3d::getOrder() const
{
    return 5;
}

int FiberSectionWarping3d::commitState(void)
{
    int err = 0;
...
int FiberSectionWarping3d::revertToLastCommit(void)
{
    int err = 0;
...
int FiberSectionWarping3d::revertToStart(void)
{
...
int FiberSectionWarping3d::sendSelf(int commitTag, Channel & theChannel)
{
    int res = 0;
    ...
    if (res < 0) {
        opserr <<
            "FiberSectionWarping3d::sendSelf - failed to send ID data\n";
        return res;
    }
    ...
    if (numFibers != 0) {
        ...
        if (res < 0) {
            opserr <<
                "FiberSectionWarping3d::sendSelf - failed to send material data\n";
            return res;
        }
...
int FiberSectionWarping3d::recvSelf(int commitTag, Channel & theChannel,
                                    FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0) {
        opserr <<
            "FiberSectionWarping3d::sendSelf - failed to recv ID data\n";
        return res;
    }
    ...
    if (data(2) == 1 && theTorsion == 0) {
        ...
        if (theTorsion == 0) {
            opserr <<
                "FiberSectionWarping3d::recvSelf - failed to get torsion material \n";
            return -1;
        }
        theTorsion->setDbTag(dbTag);
    }
    ...
    if (theTorsion->recvSelf(commitTag, theChannel, theBroker) < 0) {
        opserr <<
            "FiberSectionWarping3d::recvSelf - torsion failed to recvSelf \n";
        return -2;
    }
    ...
    if (data(1) != 0) {
        ...
        if (theMaterials == 0 || numFibers != data(1)) {
            ...
            if (numFibers != 0) {
                ...
                if (theMaterials == 0) {
                    opserr <<
                        "FiberSectionWarping3d::recvSelf -- failed to allocate Material pointers\n";
                    exit(-1);
                }
                ...
                if (matData == 0) {
                    opserr <<
                        "FiberSectionWarping3d::recvSelf  -- failed to allocate double array for material data\n";
                    exit(-1);
                }
            }
        }
...
void FiberSectionWarping3d::Print(OPS_Stream & s, int flag)
{
    if (flag == 2) {
        for (int i = 0; i < numFibers; i++) {
            s << -matData[4 * i] << " " << matData[4 * i +
                                                   1] << " " << matData[4
                                                                        *
                                                                        i
                                                                        +
...
Response *FiberSectionWarping3d::setResponse(const char **argv, int argc,
                                             OPS_Stream & output)
{
...
int FiberSectionWarping3d::getResponse(int responseID,
...
int FiberSectionWarping3d::setParameter(const char **argv, int argc,
                                        Parameter & param)
{
    if (argc < 3)
        return -1;
...
const Vector &
    FiberSectionWarping3d::getStressResultantSensitivity(int gradIndex,
                                                         bool conditional)
{
...
int
 FiberSectionWarping3d::commitSensitivity(const Vector & defSens,
