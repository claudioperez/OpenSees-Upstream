...
LayeredShellFiberSectionThermal::LayeredShellFiberSectionThermal(int tag,
                                                                 ...
                                                                 *thickness,
                                                                 NDMaterial
                                                                 **
                                                                 fibers):SectionForceDeformation
    (tag, SEC_TAG_LayeredShellFiberSectionThermal), strainResultant(8),
countnGauss(0), AverageThermalMomentP(0), AverageThermalForceP(0), sT(0),
ThermalElongation(0)
{
...
//mass per unit area
double LayeredShellFiberSectionThermal::getRho()
{
...
Response *LayeredShellFiberSectionThermal::setResponse(const char **argv,
                                                       int argc,
                                                       OPS_Stream & output)
{
...
int LayeredShellFiberSectionThermal::getResponse(int responseID,
                                                 Information & secInfo)
{
    switch (responseID) {
    case 1:
...
int LayeredShellFiberSectionThermal::sendSelf(int commitTag,
                                              Channel & theChannel)
{
    int res = 0;
    ...
    if (nLayers > 0) {
        ...
        for (i = 0; i < nLayers; i++) {
            ...
            if (res < 0) {
                opserr <<
                    "WARNING LayeredShellFiberSectionThermal::sendSelf() - "
                    << this->getTag() << " failed to send its Material" <<
                    endln;
                return res;
            }
        }
...
int LayeredShellFiberSectionThermal::recvSelf(int commitTag,
                                              Channel & theChannel,
