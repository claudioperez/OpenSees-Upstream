...
Vector
LayeredShellFiberSectionThermal::stressResultant (8);
Matrix
LayeredShellFiberSectionThermal::tangent (8, 8);
ID
LayeredShellFiberSectionThermal::array (8);
//const double  LayeredShellFiberSectionThermal::root56 = sqrt(5.0/6.0) ; //shear correction
const double
    LayeredShellFiberSectionThermal::root56 = 1;        //shear correction
//null constructor
...
strainResultant (8), nLayers (0), countnGauss (0), AverageThermalMomentP (0),
AverageThermalForceP (0), sT (0), ThermalElongation (0)
{

}

//full constructor
LayeredShellFiberSectionThermal::LayeredShellFiberSectionThermal (int tag,
                                                                  int iLayers,
                                                                  double
                                                                  *thickness,
                                                                  NDMaterial
...
strainResultant (8),
countnGauss (0),
AverageThermalMomentP (0),
AverageThermalForceP (0),
sT (0),
ThermalElongation (0)
{
...
LayeredShellFiberSectionThermal::getCopy ()
{
    ...
    if (thickness != 0)
      {
          ...
          clone = new LayeredShellFiberSectionThermal (this->getTag (), nLayers, thickness, theFibers); //make the copy
          delete[]thickness;
      }
    return clone;
}
...
int
LayeredShellFiberSectionThermal::getOrder () const
{
    return 8;
}
...
//send back order of strainResultant in vector form
const ID &
LayeredShellFiberSectionThermal::getType ()
{
    return array;
}
...
int
LayeredShellFiberSectionThermal::commitState ()
{
    int success = 0;
...
int
LayeredShellFiberSectionThermal::revertToLastCommit ()
{
    int success = 0;
...
//revert to start
int
LayeredShellFiberSectionThermal::revertToStart ()
{
    int success = 0;
    ...
    if (strcmp (argv[0], "deformations") == 0
    ...
    else if (strcmp (argv[0], "fiber") == 0 || strcmp (argv[0], "Fiber") == 0)
      {
          if (argc < 3)
            {
                opserr <<
                    "LayeredShellFiberSectionThermal::setResponse() - need to specify more data\n";
                return 0;
            }
          ...
          if (pointNum > 0 && pointNum <= nLayers)
            {
                ...
                theResponse =
                    theFibers[pointNum - 1]->setResponse (&argv[2], argc - 2,
                                                          output);
...
int
LayeredShellFiberSectionThermal::getResponse (int responseID,
                                              Information & secInfo)
{
    switch (responseID)
      {
...
int
LayeredShellFiberSectionThermal::setTrialSectionDeformation (const Vector &
                                                             strainResultant_from_element)
{
    ...
    for (i = 0; i < nLayers; i++)
      {
          ...
          strain (0) =
              strainResultant (0) - z * strainResultant (3) -
              ThermalElongation[i];

          strain (1) =
              strainResultant (1) - z * strainResultant (4) -
              ThermalElongation[i];
          ...
          if (strainResultant (8) == 111 && i == 0)
            {
                opserr << "LayeredShellSection z : " << z << "strain  " <<
                    strain << endln;
                strain (5) = 1110;
            }
          else
...
//#endif
          ...
          if (strainResultant (8) == 111 && i == 0)
            {
                opserr << "LayeredShellSection z : " << z << "stress  " <<
                    theFibers[i]->getStress () << endln;
...
const Vector &
LayeredShellFiberSectionThermal::getSectionDeformation ()
{
...
getTemperatureStress (const Vector & dataMixed)
{
    this->countnGauss = 0;
    ...
    for (int i = 0; i < nLayers; i++)
      {
          ...
          theFibers[i]->getThermalTangentAndElongation (FiberTemperature,
                                                        tangent, elongation);
...
const Vector &
LayeredShellFiberSectionThermal::getStressResultant ()
{
...
LayeredShellFiberSectionThermal::Print (OPS_Stream & s, int flag)
{
    ...
    for (int i = 0; i < nLayers; i++)
      {
          s << "Layer " << i +
              1 << ", thickness h = " << 0.5 * wg[i] * h << endln;
...
int
LayeredShellFiberSectionThermal::sendSelf (int commitTag,
                                           Channel & theChannel)
{
    int res = 0;
    ...
    if (res < 0)
      {
          opserr << "WARNING LayeredShellFiberSectionThermal::sendSelf() - "
              << this->getTag () << " failed to send data" << endln;
          return res;
      }
    ...
    if (nLayers > 0)
      {
          ...
          if (res < 0)
            {
                opserr <<
                    "WARNING LayeredShellFiberSectionThermal::sendSelf() - "
                    << this->getTag () << " failed to send data" << endln;
                return res;
            }
          ...
          if (res < 0)
            {
                opserr <<
                    "WARNING LayeredShellFiberSectionThermal::sendSelf() - "
                    << this->getTag () << " failed to send ID" << endln;
                return res;
            }
                ...
                if (res < 0)
                  {
                      opserr <<
                          "WARNING LayeredShellFiberSectionThermal::sendSelf() - "
                          << this->
                          getTag () << " failed to send its Material" <<
                          endln;
                      return res;
                  }
            }
...
int
LayeredShellFiberSectionThermal::recvSelf (int commitTag,
                                           Channel & theChannel,
                                           FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0)
      {
          opserr << "WARNING LayeredShellFiberSectionThermal::recvSelf() - "
              << this->getTag () << " failed to receive data" << endln;
          return res;
      }
    ...
    if (nLayers > 0)
      {
          ...
          if (res < 0)
            {
                opserr <<
                    "WARNING LayeredShellFiberSectionThermal::recvSelf() - "
                    << this->getTag () << " failed to receive data" << endln;
                return res;
            }
          for (i = 0; i < nLayers; i++)
          ...
          if (res < 0)
            {
                opserr <<
                    "WARNING LayeredShellFiberSectionThermal::recvSelf() - "
                    << this->getTag () << " failed to receive ID" << endln;
                return res;
            }
          ...
          for (i = 0; i < nLayers; i++)
            {
                ...
                if (theFibers[i]->getClassTag () != matClassTag)
                  {
                      ...
                      if (theFibers[i] == 0)
                        {
                            opserr <<
                                "LayeredShellFiberSectionThermal::recvSelf() - "
                                <<
                                "Broker could not create NDMaterial of class type"
                                << matClassTag << endln;
                            return -1;
                        }
                  }
                ...
                res +=
                    theFibers[i]->recvSelf (commitTag, theChannel, theBroker);
                if (res < 0)
                  {
                      opserr <<
                          "LayeredShellFiberSectionThermal::recvSelf() - material "
