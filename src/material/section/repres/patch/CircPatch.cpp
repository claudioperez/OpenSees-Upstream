...
CircPatch::CircPatch(int materialID, int numSubdivCircunf,
                     int numSubdivRadial, const Vector & centerPosition,
                     double internRadius, double externRadius,
                     double initialAngle,
                     double finalAngle):matID(materialID),
...
Cell **CircPatch::getCells(void) const
{
    double pi = acos(-1.0);
    ...
    if (nDivRad > 0 && nDivCirc > 0) {
        ...
        for (j = 0; j < nDivRad; j++) {
            ...
            for (i = 0; i < nDivCirc; i++) {
                ...
                cells[k] =
                    new CircSectionCell(rad_j, rad_j1, deltaTheta,
                                        theta_i1, centerPosit(0),
                                        centerPosit(1));
...
Patch *CircPatch::getCopy(void) const
{
...
void CircPatch::Print(OPS_Stream & s, int flag) const
{
    ...
    s << "\nNumber of subdivisions in the circunferential direction: " <<
        nDivCirc;
    s << "\nCenter Position: " << centerPosit;
    s << "\nInternal Radius: " << intRad << "\tExternal Radius: " <<
        extRad;
