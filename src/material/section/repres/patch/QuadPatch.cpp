...
QuadPatch::QuadPatch(int materialID, int numSubdivIJ, int numSubdivJK,
                     const Matrix & vertexCoords):matID(materialID),
nDivIJ(numSubdivIJ), nDivJK(numSubdivJK), vertCoord(vertexCoords)
{
...
Cell **QuadPatch::getCells(void) const
{
    double deltaXi;
    double deltaEta;
...
Patch *QuadPatch::getCopy(void) const
{
