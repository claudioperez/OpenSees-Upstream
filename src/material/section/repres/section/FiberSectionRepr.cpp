...
FiberSectionRepr::FiberSectionRepr(int sectionID, int maxNumPatches,
                                   int
                                   maxNumReinfLayers):SectionRepres
    (sectionID), sectID(sectionID), maxNPatches(maxNumPatches),
maxNReinfLayers(maxNumReinfLayers), patch(0), reinfLayer(0), nPatches(0),
nReinfLayers(0), numFibers(0), theFibers(0), sizeFibers(32), numHFibers(0),
theHFibers(0), sizeHFibers(32)
{
    ...
    if (theFibers == 0) {
        opserr <<
            "FiberSectionRepr::FiberSectionRepr -- failed to allocate Fiber pointers\n";
    ...
    if (!patch) {
        opserr <<
            "FATAL ERROR: FiberSectionRepr - not enough memory to allocate "
            << maxNumPatches << " patches";
        exit(-1);
    }
    int i;
    ...
    if (!reinfLayer) {
        opserr <<
            "FATAL ERROR: FiberSectionRepr - not enough memory to allocate "
            << maxNumReinfLayers << "reinforcing layers";
        exit(-1);
    }
...
FiberSectionRepr::FiberSectionRepr(int sectionID):SectionRepres(sectionID),
sectID(sectionID),
maxNPatches(0),
maxNReinfLayers(0), patch(0), reinfLayer(0), nPatches(0), nReinfLayers(0)
{
    opserr <<
        "Function FiberSectionRepr::FiberSectionRepr not implemented yet";
    exit(-1);
}
...
FiberSectionRepr::FiberSectionRepr(int sectionID,
                                   ...
                                   int numReinfLayers,
                                   ReinfLayer **
                                   reinfLayers):SectionRepres(sectionID),
sectID(sectionID), maxNPatches(numPatches),
maxNReinfLayers(numReinfLayers), patch(patches), reinfLayer(reinfLayers),
nPatches(numPatches), nReinfLayers(numReinfLayers)
{
    opserr <<
        "Function FiberSectionRepr::FiberSectionRepr not implemented yet";
    exit(-1);
...
void
 FiberSectionRepr::setNumPatches(int numPatches)
{
    opserr <<
        "Function FiberSectionRepr::setnumPatches not implemented yet";
    exit(-1);
}
...
{
    opserr <<
        "Function FiberSectionRepr::setnumReinfLayers not implemented yet";
    exit(-1);
}
...
{
    opserr <<
        "Function FiberSectionRepr::setReinfLayers not implemented yet";
    exit(-1);
...
int FiberSectionRepr::addFiber(Fiber & newFiber)
{
...
int FiberSectionRepr::addHFiber(Fiber & newFiber)
{
...
int FiberSectionRepr::getNumHFibers(void) const
{
    return numHFibers;
}

Fiber **FiberSectionRepr::getHFibers(void) const
{
    return theHFibers;
}
