...
const double
    MembranePlateFiberSectionThermal::root56 = sqrt (5.0 / 6.0);        //shear correction

//static vector and matrices
Vector
MembranePlateFiberSectionThermal::stressResultant (8);
Matrix
MembranePlateFiberSectionThermal::tangent (8, 8);
ID
MembranePlateFiberSectionThermal::array (8);


const double
    MembranePlateFiberSectionThermal::sg[] = { -1,
    -0.65465367,
    0,
    0.65465367,
    1
...
const double
    MembranePlateFiberSectionThermal::wg[] = { 0.1,
    0.5444444444,
    0.7111111111,
    0.5444444444,
    0.1
...
//full constructor
MembranePlateFiberSectionThermal::MembranePlateFiberSectionThermal (int tag,
                                                                    double
                                                                    thickness,
                                                                    NDMaterial
                                                                    & Afiber):
...
MembranePlateFiberSectionThermal::getCopy ()
{
    ...
    clone = new MembranePlateFiberSectionThermal (this->getTag (), this->h, *theFibers[0]);     //make the copy
    return clone;
}
...
int
MembranePlateFiberSectionThermal::getOrder () const
{
    return 8;
}
...
//send back order of strainResultant in vector form
const ID &
MembranePlateFiberSectionThermal::getType ()
{
    return array;
}
...
int
MembranePlateFiberSectionThermal::commitState ()
{
    int success = 0;
...
int
MembranePlateFiberSectionThermal::setTrialSectionDeformation (const Vector &
...
const Vector &
MembranePlateFiberSectionThermal::getSectionDeformation ()
{
...
const Vector &
MembranePlateFiberSectionThermal::getStressResultant ()
{
...
#ifdef _SDEBUG
          opserr << "MembranePlateFiberSection:resultantstress " << endln <<
              stress;
#endif
...
getTemperatureStress (const Vector & dataMixed)
{
          ...
          if (fabs (dataTempe[1]) <= 1e-10 && fabs (dataTempe[17]) <= 1e-10)
          ...
          else
            {
                ...
                if (yi < dataTempe[1])
                  {
                      opserr <<
                          "MembranePlateFiberSectionThermal::setTrialSectionDeformationup -- fiber loc is out of the section";
                  }
                else if (yi <= dataTempe[3])
                  {
                      FiberTemperature =
                          dataTempe[0] - (dataTempe[1] - yi) * (dataTempe[0] -
                                                                dataTempe[2])
                  ...
                  {
                      FiberTemperature =
                          dataTempe[2] - (dataTempe[3] - yi) * (dataTempe[2] -
                                                                dataTempe[4])
                  ...
                  {
                      FiberTemperature =
                          dataTempe[4] - (dataTempe[5] - yi) * (dataTempe[4] -
                                                                dataTempe[6])
                  ...
                  {
                      FiberTemperature =
                          dataTempe[6] - (dataTempe[7] - yi) * (dataTempe[6] -
                                                                dataTempe[8])
                  ...
                  {
                      FiberTemperature =
                          dataTempe[8] - (dataTempe[9] - yi) * (dataTempe[8] -
                                                                dataTempe[10])
    ...
    sTData[0] =
        fabs (ThermalTangent[0]) * h * 12e-6 * fabs (dataTempe[0] +
                                                     dataTempe[16]) / 2;
    ...
    sTData[1] =
        fabs (dataTempe[0] -
              dataTempe[16]) * 12e-6 * fabs (ThermalTangent[0]) * h * h / 12 /
        0.7;
    return *sT;
}
...
int
MembranePlateFiberSectionThermal::sendSelf (int commitTag,
                                            Channel & theChannel)
{
    int res = 0;
    ...
    if (res < 0)
      {
          opserr << "WARNING MembranePlateFiberSectionThermal::sendSelf() - "
              << this->getTag () << " failed to send ID\n";
    ...
    for (i = 0; i < 5; i++)
      {
          ...
          if (res < 0)
            {
                opserr <<
                    "WARNING MembranePlateFiberSectionThermal::sendSelf() - "
...
int
MembranePlateFiberSectionThermal::recvSelf (int commitTag,
                                            Channel & theChannel,
                                            FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0)
      {
          opserr << "WARNING MembranePlateFiberSectionThermal::recvSelf() - "
          ...
          for (i = 0; i < 5; i++)
            {
                ...
                if (theFibers[i] == 0)
                  {
                      opserr <<
                          "MembranePlateFiberSectionThermal::recvSelf() - " <<
                ...
                if (res < 0)
                  {
                      opserr << "MembranePlateFiber::recvSelf() - material "
          ...
          for (i = 0; i < 5; i++)
            {
                ...
                if (res < 0)
                  {
                      opserr <<
                          "MembranePlateFiberSectionThermal::recvSelf() - material "
                          << i << ", failed to recv itself\n";
                      return res;
                  }
            }
    ...
    if (strcmp (argv[0], "deformations") == 0
    ...
    else if (strcmp (argv[0], "fiber") == 0 || strcmp (argv[0], "Fiber") == 0)
      {
          if (argc < 3)
            {
                opserr <<
                    "MembranePlateFiberSectionThermal::setResponse() - need to specify more data\n";
                return 0;
            }
...
int
MembranePlateFiberSectionThermal::getResponse (int responseID,
                                               Information & secInfo)
{
    switch (responseID)
      {
