...
FiberSection3dThermal::FiberSection3dThermal(int tag, int num,
    ...
    (tag, SEC_TAG_FiberSection3dThermal), numFibers(num), sizeFibers(num),
theMaterials(0), matData(0), QzBar(0.0), QyBar(0.0), ABar(0.0), yBar(0.0),
zBar(0.0), computeCentroid(compCentroid), e(3), eCommit(3), s(0), ks(0),
sT(0), Fiber_T(0), Fiber_TMax(0), parameterID(0), SHVs(0)
{
    if (numFibers != 0) {
        ...
        if (theMaterials == 0) {
            opserr <<
                "FiberSection3dThermal::FiberSection3dThermal -- failed to allocate Material pointers\n";
            exit(-1);
        }
        ...
        if (matData == 0) {
            opserr <<
                "FiberSection3dThermal::FiberSection3dThermal -- failed to allocate double array for material data\n";
            exit(-1);
        }
        ...
        for (int i = 0; i < numFibers; i++) {
            ...
            if (theMaterials[i] == 0) {
                opserr <<
                    "FiberSection3dThermal::FiberSection3dThermal -- failed to get copy of a Material\n";
                exit(-1);
            }
        }
...
FiberSection3dThermal::FiberSection3dThermal(int tag, int num,
                                             bool
                                             compCentroid):SectionForceDeformation
    (tag, SEC_TAG_FiberSection3dThermal), numFibers(0), sizeFibers(num),
theMaterials(0), matData(0), QzBar(0.0), QyBar(0.0), ABar(0.0), yBar(0.0),
zBar(0.0), computeCentroid(compCentroid), e(3), eCommit(3), s(0), ks(0),
sT(0), Fiber_T(0), Fiber_TMax(0), parameterID(0), SHVs(0)
{
...
computeCentroid(true), e(3), eCommit(3), s(0), ks(0), sT(0),
Fiber_T(0), Fiber_TMax(0), parameterID(0), SHVs(0)
{
    s = new Vector(sData, 3);
    ks = new Matrix(kData, 3, 3);
...
int FiberSection3dThermal::addFiber(Fiber & newFiber)
{
    ...
    if (numFibers == sizeFibers) {
        ...
        if (newArray == 0 || newMatData == 0) {
            opserr <<
                "FiberSection3d::addFiber -- failed to allocate Fiber pointers\n";
            exit(-1);
        }
    ...
    if (theMaterials[numFibers] == 0) {
        opserr <<
            "FiberSection3d::addFiber -- failed to get copy of a Material\n";
        return -1;
    }
 ...
 FiberSection3dThermal::setTrialSectionDeformation(const Vector & deforms)
{
    ...
    for (int i = 0; i < numFibers; i++) {
        ...
        res +=
            theMat->setTrial(strain, FiberTemperature, stress, tangent,
                             ThermalElongation);
...
const Matrix & FiberSection3dThermal::getInitialTangent(void)
{
...
const Vector & FiberSection3dThermal::getSectionDeformation(void)
{
    return e;
}

const Matrix & FiberSection3dThermal::getSectionTangent(void)
{
    return *ks;
}

const Vector & FiberSection3dThermal::getStressResultant(void)
{
    return *s;
}
    ...
    FiberSection3dThermal::getTemperatureStress(const Vector & dataMixed)
{
    ...
    for (int i = 0; i < numFibers; i++) {
        ...
        FiberTemperature =
            this->determineFiberTemperature(dataMixed, -yi, zi);
    ...
    for (int i = 0; i < numFibers; i++) {
        FiberForce =
            ThermalTangent[i] * matData[3 * i + 2] * ThermalElong[i];
        sTData[0] += FiberForce;
...
SectionForceDeformation *FiberSection3dThermal::getCopy(void)
{
    ...
    if (numFibers != 0) {
        ...
        for (int i = 0; i < numFibers; i++) {
            ...
            if (theCopy->theMaterials[i] == 0) {
                opserr <<
                    "FiberSection3dThermal::getCopy -- failed to get copy of a Material\n";
                exit(-1);
            }
        }
    }
...
const ID & FiberSection3dThermal::getType()
{
    return code;
}

int FiberSection3dThermal::getOrder() const
{
    return 3;
}

int FiberSection3dThermal::commitState(void)
{
    int err = 0;
...
int FiberSection3dThermal::revertToLastCommit(void)
{
    int err = 0;
...
int FiberSection3dThermal::revertToStart(void)
{
...
int FiberSection3dThermal::sendSelf(int commitTag, Channel & theChannel)
{
    int res = 0;
...
int FiberSection3dThermal::recvSelf(int commitTag, Channel & theChannel,
...
void FiberSection3dThermal::Print(OPS_Stream & s, int flag)
{
    if (flag == 2) {
        for (int i = 0; i < numFibers; i++) {
            s << -matData[3 * i] << " " << matData[3 * i +
                                                   1] << " " << matData[3
                                                                        *
                                                                        i
                                                                        +
                ...
                << " ";
            s << theMaterials[i]->
                getStress() << " " << theMaterials[i]->getStrain() <<
                endln;
        }
    } else {
...
Response *FiberSection3dThermal::setResponse(const char **argv, int argc,
                                             OPS_Stream & output)
{
...
int FiberSection3dThermal::getResponse(int responseID,
...
int FiberSection3dThermal::setParameter(const char **argv, int argc,
                                        Parameter & param)
{
    if (argc < 3)
        return -1;
...
const Vector &
    FiberSection3dThermal::getStressResultantSensitivity(int gradIndex,
                                                         bool conditional)
{
    ...
    for (int i = 0; i < numFibers; i++) {
        ...
        double A = matData[loc++];
        stressGradient =
            theMaterials[i]->getStressSensitivity(gradIndex, conditional);
        stressGradient *= A;
        ds(0) += stressGradient;
...
int
 FiberSection3dThermal::commitSensitivity(const Vector & defSens,
                                          int gradIndex, int numGrads)
{
    ...
    for (int i = 0; i < numFibers; i++) {
        ...
        theMaterials[i]->commitSensitivity(strainSens, gradIndex,
                                           numGrads);
    }
...
determineFiberTemperature(const Vector & DataMixed, double fiberLocy,
                          double fiberLocz)
{
    ...
    if (DataMixed.Size() == 18) {
        ...
        if (fiberLocy <= dataTempe[1]) {
            ...
            FiberTemperature =
                dataTempe[0] - (dataTempe[1] -
                                fiberLocy) * (dataTempe[0] -
                                              dataTempe[2]) /
                (dataTempe[1] - dataTempe[3]);
        } else if (fiberLocy <= dataTempe[5]) {
            FiberTemperature =
                dataTempe[2] - (dataTempe[3] -
                                fiberLocy) * (dataTempe[2] -
                                              dataTempe[4]) /
                (dataTempe[3] - dataTempe[5]);
        } else if (fiberLocy <= dataTempe[7]) {
            FiberTemperature =
                dataTempe[4] - (dataTempe[5] -
                                fiberLocy) * (dataTempe[4] -
                                              dataTempe[6]) /
                (dataTempe[5] - dataTempe[7]);
        } else if (fiberLocy <= dataTempe[9]) {
            FiberTemperature =
                dataTempe[6] - (dataTempe[7] -
                                fiberLocy) * (dataTempe[6] -
                                              dataTempe[8]) /
                (dataTempe[7] - dataTempe[9]);
        } else if (fiberLocy <= dataTempe[11]) {
            FiberTemperature =
                dataTempe[8] - (dataTempe[9] -
                                fiberLocy) * (dataTempe[8] -
                                              dataTempe[10]) /
                (dataTempe[9] - dataTempe[11]);
        } else if (fiberLocy <= dataTempe[13]) {
            FiberTemperature =
                dataTempe[10] - (dataTempe[11] -
                                 fiberLocy) * (dataTempe[10] -
                                               dataTempe[12]) /
                (dataTempe[11] - dataTempe[13]);
        } else if (fiberLocy <= dataTempe[15]) {
            FiberTemperature =
                dataTempe[12] - (dataTempe[13] -
                                 fiberLocy) * (dataTempe[12] -
                                               dataTempe[14]) /
                (dataTempe[13] - dataTempe[15]);
        } else if (fiberLocy <= dataTempe[17]) {
            FiberTemperature =
                dataTempe[14] - (dataTempe[15] -
                                 fiberLocy) * (dataTempe[14] -
                                               dataTempe[16]) /
                (dataTempe[15] - dataTempe[17]);
        } else {
            opserr <<
                "FiberSection3dThermal::setTrialSectionDeformation -- fiber loc "
                << fiberLocy << " is out of the section" << endln;
        }
        ...
        if (fiberLocy <= dataTempe[1]) {
            if (fiberLocz <= dataTempe[12]) {
                opserr <<
                    "WARNING: FiberSection3dThermal failed to find the fiber with locy: "
                    << fiberLocy << " , locZ: " << fiberLocz << endln;
            } else if (fiberLocz <= dataTempe[15]) {
                FiberTemperature =
                    dataTempe[10] - (dataTempe[10] -
                                     dataTempe[13]) * (dataTempe[12] -
                                                       fiberLocz) /
                    (dataTempe[12] - dataTempe[15]);
            } else if (fiberLocz <= dataTempe[18]) {
                FiberTemperature =
                    dataTempe[13] - (dataTempe[13] -
                                     dataTempe[16]) * (dataTempe[15] -
                                                       fiberLocz) /
                    (dataTempe[15] - dataTempe[18]);
            } else if (fiberLocz <= dataTempe[21]) {
                FiberTemperature =
                    dataTempe[16] - (dataTempe[16] -
                                     dataTempe[19]) * (dataTempe[18] -
                                                       fiberLocz) /
                    (dataTempe[18] - dataTempe[21]);
            } else if (fiberLocz <= dataTempe[24]) {
                FiberTemperature =
                    dataTempe[19] - (dataTempe[19] -
                                     dataTempe[22]) * (dataTempe[21] -
                                                       fiberLocz) /
                    (dataTempe[21] - dataTempe[24]);
            } else {
                opserr <<
                    "WARNING: FiberSection3dThermal failed to find the fiber with locy: "
                    << fiberLocy << " , locZ: " << fiberLocz << endln;
            ...
            if (fiberLocz <= dataTempe[12]) {
                ...
                FiberTemperature =
                    dataTempe[11] - (dataTempe[11] -
                                     dataTempe[14]) * (dataTempe[12] -
                                                       fiberLocz) /
                    (dataTempe[12] - dataTempe[15]);
            } else if (fiberLocz <= dataTempe[18]) {
                FiberTemperature =
                    dataTempe[14] - (dataTempe[14] -
                                     dataTempe[17]) * (dataTempe[15] -
                                                       fiberLocz) /
                    (dataTempe[15] - dataTempe[18]);
            } else if (fiberLocz <= dataTempe[21]) {
                FiberTemperature =
                    dataTempe[17] - (dataTempe[17] -
                                     dataTempe[20]) * (dataTempe[18] -
                                                       fiberLocz) /
                    (dataTempe[18] - dataTempe[21]);
            } else if (fiberLocz <= dataTempe[24]) {
                FiberTemperature =
                    dataTempe[20] - (dataTempe[20] -
                                     dataTempe[23]) * (dataTempe[21] -
                                                       fiberLocz) /
                    (dataTempe[21] - dataTempe[24]);
            } else {
                opserr <<
