...
//parameters
const double
    MembranePlateFiberSection::root56 = sqrt (5.0 / 6.0);       //shear correction

//static vector and matrices
Vector
MembranePlateFiberSection::stressResultant (8);
Matrix
MembranePlateFiberSection::tangent (8, 8);
ID
MembranePlateFiberSection::array (8);


const double
    MembranePlateFiberSection::sg[] = { -1,
    -0.65465367,
    0,
    0.65465367,
    1
...
const double
    MembranePlateFiberSection::wg[] = { 0.1,
    0.5444444444,
    0.7111111111,
    0.5444444444,
    0.1
...
MembranePlateFiberSection::getCopy ()
{
    ...
    clone = new MembranePlateFiberSection (this->getTag (), this->h, *theFibers[0]);    //make the copy
    return clone;
}
...
int
MembranePlateFiberSection::setTrialSectionDeformation (const Vector &
...
MembranePlateFiberSection::sendSelf (int commitTag, Channel & theChannel)
{
    ...
    if (res < 0)
      {
          opserr << "WARNING MembranePlateFiberSection::sendSelf() - " <<
...
int
MembranePlateFiberSection::recvSelf (int commitTag, Channel & theChannel,
                                     FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0)
      {
          opserr << "WARNING MembranePlateFiberSection::recvSelf() - " <<
          ...
          for (i = 0; i < numFibers; i++)
            {
                ...
                if (theFibers[i]->getClassTag () != matClassTag)
                  {
                      ...
                      if (theFibers[i] == 0)
                        {
                            opserr <<
                                "MembranePlateFiberSection::recvSelf() - " <<
                                "Broker could not create NDMaterial of class type"
                                << matClassTag << endln;
                            exit (-1);
                        }
                ...
                if (res < 0)
                  {
                      opserr <<
                          "MembranePlateFiberSection::recvSelf() - material "
                          << i << ", failed to recv itself\n";
                      return res;
                  }
            }
    ...
    if (argc > 2 || strcmp (argv[0], "fiber") == 0)
      {
          ...
          if (key > 0 && key <= numFibers)
            {
                theResponse =
                    theFibers[key - 1]->setResponse (&argv[passarg],
                                                     argc - passarg, output);
            }
...
int
MembranePlateFiberSection::getResponse (int responseID,
