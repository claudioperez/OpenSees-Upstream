...
// constructors:
FiberSectionGJ::FiberSectionGJ(int tag, int num, Fiber ** fibers,
                               double gj):SectionForceDeformation(tag,
                                                                  SEC_TAG_FiberSectionGJ),
numFibers(num), theMaterials(0), matData(0), yBar(0.0), zBar(0.0), e(4),
GJ(gj)
{
    if (numFibers != 0) {
        ...
        if (theMaterials == 0) {
            opserr <<
                "FiberSectionGJ::FiberSectionGJ -- failed to allocate Material pointers\n";
            exit(-1);
        }
        ...
        if (matData == 0) {
            opserr <<
                "FiberSectionGJ::FiberSectionGJ -- failed to allocate double array for material data\n";
            exit(-1);
        }
        double Qz = 0.0;
        double Qy = 0.0;
        ...
        for (int i = 0; i < numFibers; i++) {
            ...
            if (theMaterials[i] == 0) {
                opserr <<
                    "FiberSectionGJ::FiberSectionGJ -- failed to get copy of a Material\n";
                exit(-1);
            }
        }
...
int FiberSectionGJ::addFiber(Fiber & newFiber)
{
    ...
    if (newArray[numFibers] == 0) {
        opserr <<
            "FiberSectionGJ::addFiber -- failed to get copy of a Material\n";
...
const Matrix & FiberSectionGJ::getInitialTangent(void)
{
    kData[0] = 0.0;
    kData[1] = 0.0;
    kData[2] = 0.0;
...
const Vector & FiberSectionGJ::getSectionDeformation(void)
{
    return e;
}

const Matrix & FiberSectionGJ::getSectionTangent(void)
{
    ks(0, 0) = kData[0];
    ks(0, 1) = ks(1, 0) = kData[1];
    ks(0, 2) = ks(2, 0) = kData[2];
...
const Vector & FiberSectionGJ::getStressResultant(void)
{
    s(0) = sData[0];
    s(1) = sData[1];
    s(2) = sData[2];
...
SectionForceDeformation *FiberSectionGJ::getCopy(void)
{
    ...
    if (numFibers != 0) {
        ...
        for (int i = 0; i < numFibers; i++) {
            ...
            if (theCopy->theMaterials[i] == 0) {
                opserr <<
                    "FiberSectionGJ::getCopy -- failed to get copy of a Material\n";
                exit(-1);
            }
        }
    }
...
const ID & FiberSectionGJ::getType()
{
    return code;
}

int FiberSectionGJ::getOrder() const
{
    return 4;
}

int FiberSectionGJ::commitState(void)
{
    int err = 0;
...
int FiberSectionGJ::revertToLastCommit(void)
{
    int err = 0;
...
int FiberSectionGJ::revertToStart(void)
{
...
int FiberSectionGJ::sendSelf(int commitTag, Channel & theChannel)
{
    int res = 0;
...
int FiberSectionGJ::recvSelf(int commitTag, Channel & theChannel,
...
void FiberSectionGJ::Print(OPS_Stream & s, int flag)
{
...
Response *FiberSectionGJ::setResponse(const char **argv, int argc,
                                      OPS_Stream & output)
{
...
int FiberSectionGJ::getResponse(int responseID, Information & sectInfo)
{
...
int FiberSectionGJ::setParameter(const char **argv, int argc,
                                 Parameter & param)
{
    if (argc < 3)
        return 0;
