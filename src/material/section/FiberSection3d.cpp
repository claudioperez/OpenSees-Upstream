...
void *OPS_FiberSection3d()
{
    ...
    SectionForceDeformation *section =
        new FiberSection3d(tag, num, *torsion, computeCentroid);
    if (deleteTorsion)
        delete torsion;
    return section;
}
...
FiberSection3d::FiberSection3d(int tag, int num, Fiber ** fibers,
                               ...
                               bool
                               compCentroid):SectionForceDeformation(tag,
                                                                     SEC_TAG_FiberSection3d),
numFibers(num), sizeFibers(num), theMaterials(0), matData(0), QzBar(0.0),
QyBar(0.0), Abar(0.0), yBar(0.0), zBar(0.0), computeCentroid(compCentroid),
...
FiberSection3d::FiberSection3d(int tag, int num,
                               ...
                               compCentroid):SectionForceDeformation(tag,
                                                                     SEC_TAG_FiberSection3d),
numFibers(0), sizeFibers(num), theMaterials(0), matData(0), QzBar(0.0),
QyBar(0.0), Abar(0.0), yBar(0.0), zBar(0.0), computeCentroid(compCentroid),
sectionIntegr(0), e(4), s(0), ks(0), theTorsion(0)
{
    if (sizeFibers != 0) {
...
sectionIntegr(0), e(4), s(0), ks(0), theTorsion(0)
{
    ...
    if (sectionIntegr == 0) {
        opserr <<
            "Error: FiberSection3d::FiberSection3d: could not create copy of section integration object"
            << endln;
        exit(-1);
    }
...
computeCentroid(true), sectionIntegr(0), e(4), s(0), ks(0), theTorsion(0)
{
    s = new Vector(sData, 4);
    ks = new Matrix(kData, 4, 4);
...
int FiberSection3d::addFiber(Fiber & newFiber)
{
...
const Matrix & FiberSection3d::getInitialTangent(void)
{
...
const Vector & FiberSection3d::getSectionDeformation(void)
{
    return e;
}

const Matrix & FiberSection3d::getSectionTangent(void)
{
    return *ks;
}

const Vector & FiberSection3d::getStressResultant(void)
{
    return *s;
}

SectionForceDeformation *FiberSection3d::getCopy(void)
{
    ...
    if (numFibers != 0) {
        ...
        for (int i = 0; i < numFibers; i++) {
            ...
            if (theCopy->theMaterials[i] == 0) {
                opserr <<
                    "FiberSection3d::getCopy -- failed to get copy of a Material\n";
                exit(-1);
            }
        }
    }
...
const ID & FiberSection3d::getType()
{
    return code;
}

int FiberSection3d::getOrder() const
{
    return 4;
}

int FiberSection3d::commitState(void)
{
    int err = 0;
...
int FiberSection3d::revertToLastCommit(void)
{
    int err = 0;
...
int FiberSection3d::revertToStart(void)
{
...
int FiberSection3d::sendSelf(int commitTag, Channel & theChannel)
{
    int res = 0;
    ...
    if (numFibers != 0) {
        ...
        if (res < 0) {
            opserr <<
                "FiberSection3d::sendSelf - failed to send fiber data\n";
            return res;
        }
...
int FiberSection3d::recvSelf(int commitTag, Channel & theChannel,
                             FEM_ObjectBroker & theBroker)
{
    ...
    if (data(2) == 1 && theTorsion == 0) {
        ...
        if (theTorsion == 0) {
            opserr <<
                "FiberSection3d::recvSelf - failed to get torsion material \n";
            return -1;
        }
        theTorsion->setDbTag(dbTag);
    }
    ...
    if (theTorsion->recvSelf(commitTag, theChannel, theBroker) < 0) {
        opserr <<
            "FiberSection3d::recvSelf - torsion failed to recvSelf \n";
        return -2;
    }
    ...
    if (data(1) != 0) {
        ...
        if (res < 0) {
            opserr <<
                "FiberSection3d::recvSelf - failed to recv material data\n";
            return res;
        }
        ...
        if (theMaterials == 0 || numFibers != data(1)) {
            ...
            if (numFibers != 0) {
                ...
                if (matData == 0) {
                    opserr <<
                        "FiberSection3d::recvSelf  -- failed to allocate double array for material data\n";
                    exit(-1);
                }
            }
        }
        ...
        if (res < 0) {
            opserr <<
                "FiberSection3d::recvSelf - failed to recv fiber data\n";
            return res;
        }
...
void FiberSection3d::Print(OPS_Stream & s, int flag)
{
    if (flag == OPS_PRINT_PRINTMODEL_SECTION
        || flag == OPS_PRINT_PRINTMODEL_MATERIAL) {
    ...
    if (flag == 3) {
        for (int i = 0; i < numFibers; i++) {
            s << theMaterials[i]->getTag() << " " << matData[3 *
                                                             i] << " " <<
                matData[3 * i + 1] << " " << matData[3 * i + 2] << " ";
            s << theMaterials[i]->
    ...
    if (flag == 4) {
        for (int i = 0; i < numFibers; i++) {
            s << "add fiber # " << i +
                1 << " using material # " << theMaterials[i]->getTag() <<
                " to section # 1\n";
            s << "fiber_cross_section = " << matData[3 * i +
                                                     2] << "*m^2\n";
            s << "fiber_location = (" << matData[3 *
                                                 i] << "*m, " << matData[3
                                                                         *
                                                                         i
                                                                         +
...
Response *FiberSection3d::setResponse(const char **argv, int argc,
                                      OPS_Stream & output)
{
...
int FiberSection3d::getResponse(int responseID, Information & sectInfo)
{
...
int FiberSection3d::setParameter(const char **argv, int argc,
                                 Parameter & param)
{
    if (argc < 1)
        return -1;
...
const Vector &
    FiberSection3d::getStressResultantSensitivity(int gradIndex,
                                                  bool conditional)
{
    static Vector ds(4);
    ...
    for (int i = 0; i < numFibers; i++) {
        ...
        ds(0) +=
            (tmpMatrix(0, 0) * e(0) + tmpMatrix(0, 1) * e(1) +
             tmpMatrix(0, 2) * e(2)) * A;
        ds(1) +=
            (tmpMatrix(1, 0) * e(0) + tmpMatrix(1, 1) * e(1) +
             tmpMatrix(1, 2) * e(2)) * A;
        ds(2) +=
            (tmpMatrix(2, 0) * e(0) + tmpMatrix(2, 1) * e(1) +
             tmpMatrix(2, 2) * e(2)) * A;
    }
...
const Matrix & FiberSection3d::getSectionTangentSensitivity(int gradIndex)
{
...
int FiberSection3d::commitSensitivity(const Vector & defSens,
