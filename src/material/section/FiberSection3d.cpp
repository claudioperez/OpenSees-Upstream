...
computeCentroid (true), sectionIntegr (0), e (4), s (0), ks (0),
theTorsion (0)
{
    s = new Vector (sData, 4);
    ks = new Matrix (kData, 4, 4);
    ...
    if (flag == 4)
      {
          for (int i = 0; i < numFibers; i++)
            {
                s << "add fiber # " << i +
                    1 << " using material # " << theMaterials[i]->
                    getTag () << " to section # 1\n";
...
int
FiberSection3d::commitSensitivity (const Vector & defSens, int gradIndex,
                                   int numGrads)
{
...
//by SAJalali
double
FiberSection3d::getEnergy () const
{
    static double fiberArea[10000];
