...
ID
FiberSection2dThermal::code (2);

#ifdef OPS_API_COMMANDLINE
void *
OPS_FiberSection2dThermal ()
{
...
int
FiberSection2dThermal::setParameter (const char **argv, int argc,
                                     Parameter & param)
{
    if (argc < 1)
        return -1;
    ...
    if (strstr (argv[0], "material") != 0)
      {
          ...
          for (int i = 0; i < numFibers; i++)
              if (materialTag == theMaterials[i]->getTag ())
                {
                    int ok =
                        theMaterials[i]->setParameter (&argv[2], argc - 2,
                                                       param);
                    if (ok != -1)
                        result = ok;
                }
...
const Vector &
FiberSection2dThermal::determineFiberTemperature (const Vector & DataMixed,
                                                  double fiberLoc)
{
    double FiberTemperature = 0;
    double FiberTempMax = 0;
