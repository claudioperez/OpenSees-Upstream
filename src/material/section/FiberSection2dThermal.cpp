...
// constructors:
FiberSection2dThermal::FiberSection2dThermal(int tag, int num, Fiber ** fibers, bool compCentroid):SectionForceDeformation(tag, SEC_TAG_FiberSection2dThermal), numFibers(num), sizeFibers(num), theMaterials(0), matData(0), QzBar(0.0), ABar(0.0), yBar(0.0), computeCentroid(compCentroid), sectionIntegr(0), e(2), eCommit(2), s(0), ks(0), DataMixed(27), sT(0), Fiber_Tangent(0), Fiber_ElongP(0), AverageThermalElong(2), dedh(2)        //,theTemperatures(temperatures),theTemperatureFactor(0)
{
    if (numFibers != 0) {
        ...
        if (theMaterials == 0) {
            opserr <<
                "FiberSection2dThermal::FiberSection2dThermal -- failed to allocate Material pointers";
            exit(-1);
        }
        ...
        if (matData == 0) {
            opserr <<
                "FiberSection2dThermal::FiberSection2dThermal -- failed to allocate double array for material data\n";
            exit(-1);
        }
        ...
        for (int i = 0; i < numFibers; i++) {
            ...
            if (theMaterials[i] == 0) {
                opserr <<
                    "FiberSection2dThermal::FiberSection2dThermal -- failed to get copy of a Material\n";
                exit(-1);
            }
        }
...
FiberSection2dThermal::FiberSection2dThermal(int tag, int num,
                                             bool
                                             compCentroid):SectionForceDeformation
    (tag, SEC_TAG_FiberSection2dThermal), numFibers(0), sizeFibers(num),
theMaterials(0), matData(0), QzBar(0.0), ABar(0.0), yBar(0.0),
computeCentroid(compCentroid), sectionIntegr(0), e(2), eCommit(2), s(0),
ks(0), DataMixed(27), sT(0), Fiber_Tangent(0), Fiber_ElongP(0),
AverageThermalElong(2), dedh(2)
{
    if (sizeFibers > 0) {
        ...
        if (matData == 0) {
            opserr <<
                "FiberSection2d::FiberSection2d -- failed to allocate double array for material data\n";
            exit(-1);
        }
...
FiberSection2dThermal::FiberSection2dThermal(int tag, int num, UniaxialMaterial ** mats, SectionIntegration & si, bool compCentroid):SectionForceDeformation(tag, SEC_TAG_FiberSection2dThermal), numFibers(num), sizeFibers(num), theMaterials(0), matData(0), QzBar(0.0), ABar(0.0), yBar(0.0), computeCentroid(compCentroid), sectionIntegr(0), e(2), eCommit(2), s(0), ks(0), DataMixed(27), sT(0), Fiber_Tangent(0), Fiber_ElongP(0), AverageThermalElong(2), dedh(2)      //,theTemperature(0)
{
    if (numFibers != 0) {
        theMaterials = new UniaxialMaterial *[numFibers];
    ...
    if (sectionIntegr == 0) {
        opserr <<
            "Error: FiberSection2dThermal::FiberSection2dThermal: could not create copy of section integration object"
            << endln;
        exit(-1);
    }
...
FiberSection2dThermal::FiberSection2dThermal():
SectionForceDeformation(0, SEC_TAG_FiberSection2dThermal), numFibers(0), sizeFibers(0), theMaterials(0), matData(0), QzBar(0.0), ABar(0.0), yBar(0.0), computeCentroid(true), sectionIntegr(0), e(2), eCommit(2), s(0), ks(0), DataMixed(27), Fiber_Tangent(0), Fiber_ElongP(0), AverageThermalElong(2), dedh(2)        //, theTemperatures(0),theTemperatureFactor(0)
{
    s = new Vector(sData, 2);
    ks = new Matrix(kData, 2, 2);
...
int FiberSection2dThermal::addFiber(Fiber & newFiber)
{
    ...
    if (numFibers == sizeFibers) {
        ...
        if (newArray == 0 || newMatData == 0) {
            opserr <<
                "FiberSection2d::addFiber -- failed to allocate Fiber pointers\n";
            return -1;
        }
    ...
    if (theMaterials[numFibers] == 0) {
        opserr <<
            "FiberSection2d::addFiber -- failed to get copy of a Material\n";
        return -1;
    }
 ...
 FiberSection2dThermal::setTrialSectionDeformation(const Vector & deforms)
{
    ...
    for (int i = 0; i < numFibers; i++) {
        ...
        if (fabs(DataMixed(1)) <= 1e-10 && fabs(DataMixed(17)) <= 1e-10)        //no tempe load
        {
            ...
            Vector TempV =
                this->determineFiberTemperature(DataMixed, fiberLocs[i]);
...
const Vector & FiberSection2dThermal::getSectionDeformation(void)
{
    return e;
}

const Matrix & FiberSection2dThermal::getInitialTangent(void)
{
...
const Matrix & FiberSection2dThermal::getSectionTangent(void)
{
    return *ks;
}

const Vector & FiberSection2dThermal::getStressResultant(void)
{
    return *s;
}
    ...
    FiberSection2dThermal::getTemperatureStress(const Vector & dataMixed)
{
    ...
    for (int i = 0; i < numFibers; i++) {
        FiberForce =
            Fiber_Tangent[i] * fiberArea[i] * DeltaThermalElong[i];
        SectionArea += fiberArea[i];
        SectionMomofArea +=
            (fiberArea[i] * (fiberLocs[i] - yBar) * (fiberLocs[i] - yBar));
        ThermalForce += Fiber_ElongP[i] * fiberArea[i];
        ThermalMoment +=
            Fiber_ElongP[i] * fiberArea[i] * (fiberLocs[i] - yBar);
        sTData[0] += FiberForce;
...
const Vector & FiberSection2dThermal::getThermalElong(void)
{
...
SectionForceDeformation *FiberSection2dThermal::getCopy(void)
{
    ...
    if (numFibers != 0) {
        ...
        if (theCopy->theMaterials == 0) {
            opserr <<
                "FiberSection2dThermal::getCopy -- failed to allocate Material pointers\n";
            exit(-1);
        }
        ...
        if (theCopy->matData == 0) {
            opserr <<
                "FiberSection2dThermal::getCopy -- failed to allocate double array for material data\n";
            exit(-1);
        }
        ...
        for (int i = 0; i < numFibers; i++) {
            ...
            if (theCopy->theMaterials[i] == 0) {
                opserr <<
                    "FiberSection2dThermal::getCopy -- failed to get copy of a Material";
                exit(-1);
            }
        }
...
const ID & FiberSection2dThermal::getType()
{
    return code;
}

int FiberSection2dThermal::getOrder() const
{
    return 2;
}

int FiberSection2dThermal::commitState(void)
{
    int err = 0;
...
int FiberSection2dThermal::revertToLastCommit(void)
{
    int err = 0;
...
int FiberSection2dThermal::revertToStart(void)
{
...
int FiberSection2dThermal::sendSelf(int commitTag, Channel & theChannel)
{
    int res = 0;
    ...
    if (res < 0) {
        opserr <<
            "FiberSection2dThermal::sendSelf - failed to send ID data\n";
        return res;
    }
    ...
    if (numFibers != 0) {
        ...
        if (res < 0) {
            opserr <<
                "FiberSection2dThermal::sendSelf - failed to send material data\n";
            return res;
        }
                                 ...
                                 FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0) {
        opserr <<
            "FiberSection2dThermal::recvSelf - failed to recv ID data\n";
        return res;
    }
    this->setTag(data(0));
    ...
    if (data(1) != 0) {
        ...
        if (res < 0) {
            opserr <<
                "FiberSection2dThermal::recvSelf - failed to recv material data\n";
            return res;
        }
        ...
        if (theMaterials == 0 || numFibers != data(1)) {
            ...
            if (numFibers != 0) {
                ...
                if (theMaterials == 0) {
                    opserr <<
                        "FiberSection2dThermal::recvSelf -- failed to allocate Material pointers\n";
                    exit(-1);
                }
                ...
                if (matData == 0) {
                    opserr <<
                        "FiberSection2dThermal::recvSelf  -- failed to allocate double array for material data\n";
                    exit(-1);
                }
            }
        }
...
Response *FiberSection2dThermal::setResponse(const char **argv, int argc,
                                             OPS_Stream & output)
{
    ...
    if (strcmp(argv[0], "deformations") == 0
        ...
        theResponse =
            new MaterialResponse(this, 1, this->getSectionDeformation());
        ...
        theResponse =
            new MaterialResponse(this, 4, Vector(2 * this->getOrder()));
...
int FiberSection2dThermal::setParameter(const char **argv, int argc,
                                        Parameter & param)
{
    if (argc < 1)
        return -1;
    ...
    if (strstr(argv[0], "material") != 0) {
        ...
        for (int i = 0; i < numFibers; i++)
            if (materialTag == theMaterials[i]->getTag()) {
                int ok = theMaterials[i]->setParameter(&argv[2], argc - 2,
                                                       param);
                if (ok != -1)
                    result = ok;
            }
...
const Vector &
    FiberSection2dThermal::getStressResultantSensitivity(int gradIndex,
                                                         bool conditional)
{
    static Vector ds(2);
    ...
    for (int i = 0; i < numFibers; i++) {
        ...
        stressGradient =
            theMaterials[i]->getStressSensitivity(gradIndex, true);
    ...
    FiberSection2dThermal::getInitialTangentSensitivity(int gradIndex)
{
    ...
    for (int i = 0; i < numFibers; i++) {
        ...
        dtangentdh =
            theMaterials[i]->getInitialTangentSensitivity(gradIndex);

        dksdh(0, 0) += dtangentdh * A + tangent * dAdh;

        dksdh(0, 1) +=
            -y * (dtangentdh * A + tangent * dAdh) - dydh * (tangent * A);

        dksdh(1, 1) +=
            2 * (y * dydh * tangent * A) + y * y * (dtangentdh * A +
                                                    tangent * dAdh);
    }
...
determineFiberTemperature(const Vector & DataMixed, double fiberLoc)
{
    double FiberTemperature = 0;
    double FiberTempMax = 0;
    ...
    if (fiberLoc <= dataTempe[1]) {
        opserr <<
            "FiberSection2dThermal::setTrialSectionDeformationTemperature -- fiber loc is out of the section";
    } else if (fiberLoc <= dataTempe[3]) {
        FiberTemperature =
            dataTempe[0] - (dataTempe[1] - fiberLoc) * (dataTempe[0] -
                                                        dataTempe[2]) /
            (dataTempe[1] - dataTempe[3]);
        ...
        FiberTemperature =
            dataTempe[2] - (dataTempe[3] - fiberLoc) * (dataTempe[2] -
                                                        dataTempe[4]) /
            (dataTempe[3] - dataTempe[5]);
        ...
        FiberTemperature =
            dataTempe[4] - (dataTempe[5] - fiberLoc) * (dataTempe[4] -
                                                        dataTempe[6]) /
            (dataTempe[5] - dataTempe[7]);
        ...
        FiberTemperature =
            dataTempe[6] - (dataTempe[7] - fiberLoc) * (dataTempe[6] -
                                                        dataTempe[8]) /
            (dataTempe[7] - dataTempe[9]);
        ...
        FiberTemperature =
            dataTempe[8] - (dataTempe[9] - fiberLoc) * (dataTempe[8] -
                                                        dataTempe[10]) /
            (dataTempe[9] - dataTempe[11]);
        ...
        FiberTemperature =
            dataTempe[10] - (dataTempe[11] - fiberLoc) * (dataTempe[10] -
                                                          dataTempe[12]) /
            (dataTempe[11] - dataTempe[13]);
        ...
        FiberTemperature =
            dataTempe[12] - (dataTempe[13] - fiberLoc) * (dataTempe[12] -
                                                          dataTempe[14]) /
            (dataTempe[13] - dataTempe[15]);
        ...
        FiberTemperature =
            dataTempe[14] - (dataTempe[15] - fiberLoc) * (dataTempe[14] -
                                                          dataTempe[16]) /
            (dataTempe[15] - dataTempe[17]);
        //FiberTempMax = dataTempe[25] - (dataTempe[15] - fiberLoc) * (dataTempe[25] - dataTempe[26])/(dataTempe[15] - dataTempe[17]);
    } else {
        opserr <<
            "FiberSection2dThermal::setTrialSectionDeformation -- fiber loc is out of the section";
    }
