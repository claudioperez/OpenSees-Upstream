...
YieldSurfaceSection2d(void):SectionForceDeformation(0,
                                                    SEC_TAG_YieldSurface2d),
use_Kr_orig(true), ys(0), e(2), s(2), eCommit(2), sCommit(2), ks(2, 2),
...
YieldSurfaceSection2d::YieldSurfaceSection2d
    (int tag, int classtag, YieldSurface_BC * ptrys,
     bool use_kr):SectionForceDeformation(tag, classtag),
use_Kr_orig(use_kr), ys(0), e(2), s(2), eCommit(2), sCommit(2), ks(2, 2),
...
int YieldSurfaceSection2d::revertToLastCommit(void)
{
    e = eCommit;
    s = sCommit;
...
int YieldSurfaceSection2d::revertToStart(void)
{
    eCommit.Zero();
    sCommit.Zero();
    //    ys->revertToStart();
...
int YieldSurfaceSection2d::setTrialSectionDeformation(const Vector & def)
{
    ys->update();               // important
    use_Kr = use_Kr_orig;
    //    split_step = false;
    ...
    {
        opserr <<
            "WARNING: YieldSurfaceSection2d::setTrialSectionDeformation, driftOld outside ["
            << getTag() << "]\n";
    }
    ...
    if (fabs(lamda) < 1e-8)
        lamda = 0.0;            // to get rid of -1e-15 etc
...
const Vector & YieldSurfaceSection2d::getSectionDeformation(void)
{
    return e;
}

const Vector & YieldSurfaceSection2d::getStressResultant(void)
{
    return s;
}

const Matrix & YieldSurfaceSection2d::getSectionTangent(void)
{
    return ks;
}

const Matrix & YieldSurfaceSection2d::getSectionFlexibility(void)
{
...
const ID & YieldSurfaceSection2d::getType()
{
    return code;
}

int YieldSurfaceSection2d::getOrder() const
{
    return 2;
}

int YieldSurfaceSection2d::sendSelf(int commitTag, Channel & theChannel)
{
    return -1;
}

int YieldSurfaceSection2d::recvSelf(int commitTag, Channel & theChannel,
                                    FEM_ObjectBroker & theBroker)
{
    return -1;
}

void YieldSurfaceSection2d::Print(OPS_Stream & s, int flag)
{
