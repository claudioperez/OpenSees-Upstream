...
YS_Section2D02::YS_Section2D02(void):YieldSurfaceSection2d(0,
                                                           SEC_TAG_YieldSurface2D02,
                                                           0, true), E(0),
A(0), I(0), maxPlstkRot(0.0), peakPlstkRot(0.0), iFactor(1.0)
{
}

YS_Section2D02::YS_Section2D02
    (int tag, double E_in, double A_in, double I_in, double theta_p_max,
     YieldSurface_BC * ptrys, bool use_kr):YieldSurfaceSection2d(tag,
                                                                 SEC_TAG_YieldSurface2D02,
                                                                 ptrys,
                                                                 use_kr),
E(E_in), A(A_in), I(I_in), maxPlstkRot(theta_p_max), peakPlstkRot(0.0),
iFactor(1.0)
{
    if (E <= 0.0) {
        opserr <<
            "YS_Section2D02::YS_Section2D02s -- Input E <= 0.0 ... setting E to 1.0\n";
        E = 1.0;
    }

    if (A <= 0.0) {
        opserr <<
            "YS_Section2D02::YS_Section2D02s -- Input A <= 0.0 ... setting A to 1.0\n";
        A = 1.0;
    }

    if (I <= 0.0) {
        opserr <<
            "YS_Section2D02::YS_Section2D02s -- Input I <= 0.0 ... setting I to 1.0\n";
        I = 1.0;
    }

    if (maxPlstkRot <= 0.0) {
        opserr <<
            "YS_Section2D02::YS_Section2D02s -- Input maxPlstkRot <= 0.0 ... setting to 0.0\n";
        maxPlstkRot = 0.0;
    }
...
int
 YS_Section2D02::commitState(void)
{
...
const Matrix & YS_Section2D02::getInitialTangent(void)
{
    ks(0, 0) = E * A;
    ks(0, 1) = 0.0;
    ks(1, 0) = 0.0;
...
SectionForceDeformation *YS_Section2D02::getCopy()
{
    ...
    YS_Section2D02 *theCopy =
        new YS_Section2D02(this->getTag(), E, A, I, maxPlstkRot, ys,
                           use_Kr_orig);
