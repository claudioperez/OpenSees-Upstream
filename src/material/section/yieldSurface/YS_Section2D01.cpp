...
YS_Section2D01::YS_Section2D01(void):YieldSurfaceSection2d(0,
                                                           SEC_TAG_YieldSurface2D01,
                                                           0, true), E(0),
A(0), I(0)
{
}
...
E(E_in), A(A_in), I(I_in)
{
    if (E <= 0.0) {
        opserr <<
            "YS_Section2D01::YS_Section2D01 -- Input E <= 0.0 ... setting E to 1.0\n";
        E = 1.0;
    }

    if (A <= 0.0) {
        opserr <<
            "YS_Section2D01::YS_Section2D01 -- Input A <= 0.0 ... setting A to 1.0\n";
        A = 1.0;
    }

    if (I <= 0.0) {
        opserr <<
            "YS_Section2D01::YS_Section2D01 -- Input I <= 0.0 ... setting I to 1.0",
            I = 1.0;
    }
...
void
 YS_Section2D01::getSectionStiffness(Matrix & Ks)
{
    Ks(0, 0) = E * A;
    Ks(0, 1) = 0.0;
    Ks(1, 0) = 0.0;
...
const Matrix & YS_Section2D01::getInitialTangent(void)
{
    ks(0, 0) = E * A;
    ks(0, 1) = 0.0;
    ks(1, 0) = 0.0;
