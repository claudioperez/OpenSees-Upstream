...
SoilFootingSection2d::SoilFootingSection2d(void):SectionForceDeformation(0,
                                                                         SEC_TAG_SoilFooting2d),
e(3), s(3), eCommit(3), sCommit(3), deModel(3), ks(3, 3), ksE(3, 3),
ini_size(3)
{
...
SoilFootingSection2d::SoilFootingSection2d
    (int tag, double fs, double vult, double l, double kv, double kh,
     double rv, double deltaL):SectionForceDeformation(tag,
                                                       SEC_TAG_SoilFooting2d),
e(3), s(3), eCommit(3), sCommit(3), deModel(3), ks(3, 3), ksE(3, 3),
ini_size(3), FS(fs), Vult(vult), L(l), Kv(kv), Kh(kh), Rv(rv), dL(deltaL)
{
    if (FS <= 1.0) {
...
void SoilFootingSection2d::initializeInternalVariables(void)
{
...
void SoilFootingSection2d::tempFunction(void)
{
...
int SoilFootingSection2d::commitState(void)
{
...
int SoilFootingSection2d::revertToLastCommit(void)
{
...
int SoilFootingSection2d::revertToStart(void)
{
...
int SoilFootingSection2d::setTrialSectionDeformation(const Vector & def)
{
...
int SoilFootingSection2d::applyLoading(Vector de)
{
...
//        hCurr = dMt / dHt;
        ...
        if (dTheta > 0.0) {
            ...
            do                  // check for Vinit
            {
                ...
                e_2 = eCommit(2);
                n_load =
                    9.5 * pow((thetaPlus - e_2) / thetaRange, 1.0) + 0.5;
                n_unload =
                    9.5 * pow((e_2 - thetaMinus) / thetaRange, 1.0) + 0.5;
...
/////////////////////////////////////////////////////////////////////////////
            ...
            dist1 =
                pow(pow(ptFm1 - Fm1, 2.0) + pow(ptFh1 - Fh1, 2.0), 0.5);
            dist2 =
                pow(pow(ptFm1 - Fm2, 2.0) + pow(ptFh1 - Fh2, 2.0), 0.5);
...
*/
            ...
            if (delta / deltaIn > 0.05) {
                ...
                uM = (2.0 * L * (dTheta) * (L / hCurr) *
                      pow(B / A, 2.0)) * pow(1.0 - delta / deltaIn, 2.0);
//         uM = 0.0;
                uH = de(1) - uM;
...
const Vector & SoilFootingSection2d::getSectionDeformation(void)
{
    return e;
}

const Vector & SoilFootingSection2d::getStressResultant(void)
{
    return s;
}

const Matrix & SoilFootingSection2d::getSectionTangent(void)
{
    return ks;
}

const Matrix & SoilFootingSection2d::getSectionFlexibility(void)
{
...
const ID & SoilFootingSection2d::getType()
{
    return code;
}

int SoilFootingSection2d::getOrder() const
{
    return 3;
}


const Matrix & SoilFootingSection2d::getInitialTangent()
{
...
int SoilFootingSection2d::sendSelf(int commitTag, Channel & theChannel)
{
    return -1;
}

int SoilFootingSection2d::recvSelf(int commitTag, Channel & theChannel,
                                   FEM_ObjectBroker & theBroker)
{
    return -1;
}

void SoilFootingSection2d::Print(OPS_Stream & s, int flag)
{
