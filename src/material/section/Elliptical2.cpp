...
void *OPS_Elliptical2()
{
    if (OPS_GetNumRemainingInputArgs() < 8) {
        ...
        opserr <<
            "Want: section Elliptical tag? E1? E2? sigY1? sigY2? Hiso? Hkin1? Hkin2? <code1? code2?>"
            << endln;
        return 0;
    }
...
Elliptical2::Elliptical2
    (int tag, double e1, double e2, double sy1, double sy2, double Hi,
     double Hk1, double Hk2, int c1, int c2):SectionForceDeformation(tag,
                                                                     SEC_TAG_Elliptical2),
...
const Matrix & Elliptical2::getSectionTangent(void)
{
    static Vector str(2);
...
const Matrix & Elliptical2::getInitialTangent(void)
{
    ks(0, 0) = E[0];
    ks(1, 1) = E[1];
    ks(0, 1) = ks(1, 0) = 0.0;
...
const Vector & Elliptical2::getStressResultant(void)
{
    // Compute trial stress using elastic tangent
...
const Vector & Elliptical2::getSectionDeformation(void)
{
    static Vector e(2);
...
int Elliptical2::commitState(void)
{
    eP_n[0] = eP_n1[0];
    eP_n[1] = eP_n1[1];
...
int Elliptical2::revertToLastCommit(void)
{
    return 0;
}

int Elliptical2::revertToStart(void)
{
    for (int i = 0; i < 2; i++) {
        eP_n[i] = 0.0;
        eP_n1[i] = 0.0;
...
SectionForceDeformation *Elliptical2::getCopy(void)
{
    Elliptical2 *theCopy =
...
const ID & Elliptical2::getType(void)
{
    code(0) = code1;
    code(1) = code2;
...
int Elliptical2::getOrder(void) const
{
    return 2;
}

int Elliptical2::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
...
int Elliptical2::recvSelf(int cTag, Channel & theChannel,
...
void Elliptical2::Print(OPS_Stream & s, int flag)
{
...
Response *Elliptical2::setResponse(const char **argv, int argc,
                                   OPS_Stream & output)
{
...
int Elliptical2::getResponse(int responseID, Information & eleInformation)
{
    if (responseID == 123) {
...
int Elliptical2::setParameter(const char **argv, int argc,
                              Parameter & param)
{
...
int Elliptical2::updateParameter(int parameterID, Information & info)
{
    switch (parameterID) {
    case 1:
...
int Elliptical2::activateParameter(int paramID)
{
    parameterID = paramID;
                                               ...
                                               bool conditional)
{
    ...
    if (F >= -10 * DBL_EPSILON) {
        ...
        c(1) =
            dzdh[1] - (E[1] +
                       Hkin[1]) * dg / q * (B(1,
                                            ...
                                            dQ[1] * xsi[1]);
        c(2) =
            Hiso * dalphadh + dHisodh * alpha_n1 -
            0.5 / q * (xsi[0] * dQ[0] * xsi[0] + xsi[1] * dQ[1] * xsi[1]);
        ...
        double dndh[2];
        dndh[0] =
            (Q[0] - n[0] * n[0]) / q * dx(0) - (n[0] * n[1]) / q * dx(1) +
            B(0, 0) / q * dQ[0] * xsi[0] + B(0, 1) / q * dQ[1] * xsi[1];
        dndh[1] =
            (Q[1] - n[1] * n[1]) / q * dx(1) - (n[1] * n[0]) / q * dx(0) +
            B(1, 0) / q * dQ[0] * xsi[0] + B(1, 1) / q * dQ[1] * xsi[1];
...
int
 Elliptical2::commitSensitivity(const Vector & dedh, int gradIndex,
                                int numGrads)
{
    if (SHVs == 0) {
    ...
    double dzdh[2];
    dzdh[0] =
        dEdh[0] * e_n1[0] + E[0] * dedh(0) - (dEdh[0] +
                                              dHkindh[0]) * eP_n1[0] -
        (E[0] + Hkin[0]) * dePdh[0];
    dzdh[1] =
        dEdh[1] * e_n1[1] + E[1] * dedh(1) - (dEdh[1] +
                                              dHkindh[1]) * eP_n1[1] -
        (E[1] + Hkin[1]) * dePdh[1];
