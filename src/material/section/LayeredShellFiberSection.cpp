...
Vector
LayeredShellFiberSection::stressResultant (8);
Matrix
LayeredShellFiberSection::tangent (8, 8);
ID
LayeredShellFiberSection::array (8);
...
//full constructor
LayeredShellFiberSection::LayeredShellFiberSection (int tag,
                                                    int iLayers,
                                                    double *thickness,
                                                    NDMaterial ** fibers):
    ...
    for (i = 0; i < iLayers; i++)
      {
          ...
          if (theFibers[i] == 0)
            {
                opserr <<
                    "LayeredShellFiberSection::ERROR: Could Not return a PlateFiber Material: ";
...
LayeredShellFiberSection::getCopy ()
{
    ...
    if (thickness != 0)
      {
          ...
          clone = new LayeredShellFiberSection (this->getTag (), nLayers, thickness, theFibers);        //make the copy
          delete[]thickness;
      }
...
int
LayeredShellFiberSection::getOrder () const
{
    return 8;
}
...
//send back order of strainResultant in vector form
const ID &
LayeredShellFiberSection::getType ()
{
    return array;
}
    ...
    if (strcmp (argv[0], "deformations") == 0
    ...
    else if (strcmp (argv[0], "fiber") == 0 || strcmp (argv[0], "Fiber") == 0)
      {
          if (argc < 3)
            {
                opserr <<
                    "LayeredShellFiberSection::setResponse() - need to specify more data\n";
                return 0;
            }
...
int
LayeredShellFiberSection::setTrialSectionDeformation (const Vector &
...
const Vector &
LayeredShellFiberSection::getSectionDeformation ()
{
...
const Vector &
LayeredShellFiberSection::getStressResultant ()
{
...
LayeredShellFiberSection::sendSelf (int commitTag, Channel & theChannel)
{
    ...
    if (res < 0)
      {
          opserr << "WARNING LayeredShellFiberSection::sendSelf() - " <<
          ...
          for (i = 0; i < nLayers; i++)
            {
                ...
                if (res < 0)
                  {
                      opserr <<
                          "WARNING LayeredShellFiberSection::sendSelf() - " <<
                          this->
...
int
LayeredShellFiberSection::recvSelf (int commitTag, Channel & theChannel,
                                    FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0)
      {
          opserr << "WARNING LayeredShellFiberSection::recvSelf() - " <<
          ...
          for (i = 0; i < nLayers; i++)
            {
                ...
                if (theFibers[i]->getClassTag () != matClassTag)
                  {
                      ...
                      if (theFibers[i] == 0)
                        {
                            opserr <<
                                "LayeredShellFiberSection::recvSelf() - " <<
                ...
                if (res < 0)
                  {
                      opserr <<
                          "LayeredShellFiberSection::recvSelf() - material "
