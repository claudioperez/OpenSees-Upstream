...
void *OPS_UniaxialFiber2d()
{
    ...
    return new UniaxialFiber2d(numUniaxialFiber2d++, *theMat, data[2],
                               data[0]);
}
#endif
...
UniaxialFiber2d::UniaxialFiber2d(int tag,
                                 ...
                                 double Area, double position):Fiber(tag,
                                                                     FIBER_TAG_Uniaxial2d),
theMaterial(0), area(Area), y(-position)
{
    ...
    if (theMaterial == 0) {
        opserr <<
            "UniaxialFiber2d::UniaxialFiber2d  -- failed to get copy of UniaxialMaterial\n";
        exit(-1);
    }
...
Vector & UniaxialFiber2d::getFiberStressResultants(void)
{
...
Matrix & UniaxialFiber2d::getFiberTangentStiffContr(void)
{
    // Use the section kinematic matrix to get the fiber 
...
Fiber *UniaxialFiber2d::getCopy(void)
{
...
int UniaxialFiber2d::getOrder(void)
{
    return 2;
}

const ID & UniaxialFiber2d::getType(void)
{
    return code;
}

int UniaxialFiber2d::commitState(void)
{
    return theMaterial->commitState();
}


int UniaxialFiber2d::revertToLastCommit(void)
{
    return theMaterial->revertToLastCommit();
}


int UniaxialFiber2d::revertToStart(void)
{
    return theMaterial->revertToStart();
}


int UniaxialFiber2d::sendSelf(int commitTag, Channel & theChannel)
{
    // 
    ...
    if (res < 0) {
        opserr <<
            "UniaxialFiber2d::sendSelf - failed to send Vector data\n";
        return res;
    }
    ...
    if (res < 0) {
        opserr <<
            "UniaxialFiber2d::sendSelf - failed to send UniaxialMaterial\n";
        return res;
    }
...
int UniaxialFiber2d::recvSelf(int commitTag, Channel & theChannel,
                              FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0) {
        opserr <<
            "UniaxialFiber2d::recvSelf - failed to receive Vector data\n";
        return res;
    }
    ...
    if (res < 0) {
        opserr <<
            "UniaxialFiber2d::recvSelf() - the material failed in recvSelf()\n";
        return res;
    }
...
Response *UniaxialFiber2d::setResponse(const char **argv, int argc,
                                       OPS_Stream & s)
{
    if (argc == 0)
        return 0;
...
int UniaxialFiber2d::getResponse(int responseID, Information & fibInfo)
{
    switch (responseID) {
    case 1:
...
void UniaxialFiber2d::getFiberLocation(double &yLoc, double &zLoc)
{
    yLoc = -y;
    zLoc = 0.0;
}

int UniaxialFiber2d::setParameter(const char **argv, int argc,
                                  Parameter & param)
{
...
int UniaxialFiber2d::updateParameter(int parameterID, Information & info)
{
    switch (parameterID) {
    case 1:
        area = info.theDouble;
...
int UniaxialFiber2d::activateParameter(int parameterID)
{
    return -1;
}
