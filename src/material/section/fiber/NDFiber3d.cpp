...
void *OPS_NDFiber3d()
{
    ...
    return new NDFiber3d(numNDFiber3d++, *theMat, data[2], data[0],
                         data[1]);
}
#endif
...
NDFiber3d::NDFiber3d(int tag, NDMaterial & theMat,
                     ...
                     double d):Fiber(tag, FIBER_TAG_ND3d), theMaterial(0),
area(Area), y(yy), z(zz), dValue(d)
{
 ...
 NDFiber3d::setTrialFiberStrain(const Vector & vs)
{
    ...
    opserr << "NDFiber3d::setTrialFiberStrain() -- not implemented" <<
        endln;
...
int NDFiber3d::getOrder(void)
{
    return 6;
}

const ID & NDFiber3d::getType(void)
{
    return code;
}

int NDFiber3d::commitState(void)
{
    return theMaterial->commitState();
}


int NDFiber3d::revertToLastCommit(void)
{
    return theMaterial->revertToLastCommit();
}


int NDFiber3d::revertToStart(void)
{
    return theMaterial->revertToStart();
}


int NDFiber3d::sendSelf(int commitTag, Channel & theChannel)
{
    // 
    ...
    if (res < 0) {
        opserr <<
            "NDFiber3d::sendSelf - failed to send UniaxialMaterial\n";
        return res;
    }
...
int NDFiber3d::recvSelf(int commitTag, Channel & theChannel,
...
Response *NDFiber3d::setResponse(const char **argv, int argc,
                                 OPS_Stream & s)
{
    if (argc == 0)
        return 0;
...
int NDFiber3d::getResponse(int responseID, Information & fibInfo)
{
    switch (responseID) {
    case 1:
...
void NDFiber3d::getFiberLocation(double &yLoc, double &zLoc)
{
    yLoc = y;
    zLoc = z;
}

int NDFiber3d::setParameter(const char **argv, int argc, Parameter & param)
{
...
int NDFiber3d::updateParameter(int parameterID, Information & info)
{
    switch (parameterID) {
    case 1:
        area = info.theDouble;
...
int NDFiber3d::activateParameter(int parameterID)
{
    return -1;
}

const Vector & NDFiber3d::getFiberSensitivity(int gradNumber, bool cond)
{
    return Fiber::getFiberSensitivity(gradNumber, cond);
}

int NDFiber3d::commitSensitivity(const Vector & dedh, int gradNumber,
                                 int numGrads)
{
    return -1;
}
