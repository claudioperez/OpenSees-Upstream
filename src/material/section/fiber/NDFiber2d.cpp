...
NDFiber2d::NDFiber2d(int tag,
                     NDMaterial & theMat, double Area,
                     double position):Fiber(tag, FIBER_TAG_ND2d),
theMaterial(0), area(Area), y(-position)
{
    ...
    if (theMaterial == 0) {
        opserr <<
            "NDFiber2d::NDFiber2d -- failed to get copy of NDMaterial\n";
        exit(-1);
    }
 ...
 NDFiber2d::setTrialFiberStrain(const Vector & vs)
{
    ...
    opserr << "NDFiber2d::setTrialFiberStrain() -- not implemented" <<
        endln;
...
Vector & NDFiber2d::getFiberStressResultants(void)
{
    fs.Zero();

    opserr << "NDFiber2d::getFiberStressResultants() -- not implemented" <<
        endln;
...
Matrix & NDFiber2d::getFiberTangentStiffContr(void)
{
    ks.Zero();

    opserr << "NDFiber2d::getFiberTangentStiffContr() -- not implemented"
        << endln;

    return ks;
}

Fiber *NDFiber2d::getCopy(void)
{
...
int NDFiber2d::getOrder(void)
{
    return 3;
}

const ID & NDFiber2d::getType(void)
{
    return code;
}

int NDFiber2d::commitState(void)
{
    return theMaterial->commitState();
}


int NDFiber2d::revertToLastCommit(void)
{
    return theMaterial->revertToLastCommit();
}


int NDFiber2d::revertToStart(void)
{
    return theMaterial->revertToStart();
}


int NDFiber2d::sendSelf(int commitTag, Channel & theChannel)
{
    // 
    ...
    if (res < 0) {
        opserr <<
            "NDFiber2d::sendSelf - failed to send UniaxialMaterial\n";
        return res;
    }
...
int NDFiber2d::recvSelf(int commitTag, Channel & theChannel,
                        FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0) {
        opserr <<
            "NDFiber2d::recvSelf() - the material failed in recvSelf()\n";
        return res;
    }
...
Response *NDFiber2d::setResponse(const char **argv, int argc,
                                 OPS_Stream & s)
{
    if (argc == 0)
        return 0;
...
int NDFiber2d::getResponse(int responseID, Information & fibInfo)
{
    switch (responseID) {
    case 1:
...
void NDFiber2d::getFiberLocation(double &yLoc, double &zLoc)
{
    yLoc = -y;
    zLoc = 0.0;
}

int NDFiber2d::setParameter(const char **argv, int argc, Parameter & param)
{
...
int NDFiber2d::updateParameter(int parameterID, Information & info)
{
    switch (parameterID) {
    case 1:
        area = info.theDouble;
...
int NDFiber2d::activateParameter(int parameterID)
{
    return -1;
}

const Vector & NDFiber2d::getFiberSensitivity(int gradNumber, bool cond)
{
    return Fiber::getFiberSensitivity(gradNumber, cond);
}

int NDFiber2d::commitSensitivity(const Vector & dedh, int gradNumber,
                                 int numGrads)
{
    return -1;
}
