...
void *OPS_UniaxialFiber3d()
{
    ...
    fiberPos(0) = data[0];
    fiberPos(1) = data[1];
    return new UniaxialFiber3d(numUniaxialFiber3d++, *theMat, data[2],
                               fiberPos);
}
#endif
...
// constructor:
UniaxialFiber3d::UniaxialFiber3d():Fiber(0, FIBER_TAG_Uniaxial3d),
...
theMaterial(0), area(Area), dValue(d)
{
    ...
    if (theMaterial == 0) {
        opserr <<
            "UniaxialFiber3d::UniaxialFiber2d -- failed to get copy of UniaxialMaterial\n";
        exit(-1);
    }
 ...
 UniaxialFiber3d::setTrialFiberStrain(const Vector & vs)
{
    ...
    if (theMaterial != 0)
        ...
        opserr <<
            "UniaxialFiber3d::setTrialFiberStrain() - no material!\n";
...
Vector & UniaxialFiber3d::getFiberStressResultants(void)
{
...
Matrix & UniaxialFiber3d::getFiberTangentStiffContr(void)
{
    // ks = (as^as) * area * Et;
...
Fiber *UniaxialFiber3d::getCopy(void)
{
...
int UniaxialFiber3d::getOrder(void)
{
    return 3;
}

const ID & UniaxialFiber3d::getType(void)
{
    return code;
}

int UniaxialFiber3d::commitState(void)
{
    return theMaterial->commitState();
}


int UniaxialFiber3d::revertToLastCommit(void)
{
    return theMaterial->revertToLastCommit();
}

int UniaxialFiber3d::revertToStart(void)
{
    return theMaterial->revertToStart();
}


int UniaxialFiber3d::sendSelf(int commitTag, Channel & theChannel)
{
    // 
    ...
    if (theChannel.sendID(dbTag, commitTag, idData) < 0) {
        opserr <<
            "UniaxialFiber3d::sendSelf() -  failed to send ID data\n";
        return -1;
    }
    // 
    ...
    if (theChannel.sendVector(dbTag, commitTag, dData) < 0) {
        opserr <<
            "UniaxialFiber3d::sendSelf() -  failed to send Vector data\n";
        return -2;
    }
    // now invoke sendSelf on the material
    if (theMaterial->sendSelf(commitTag, theChannel) < 0) {
        opserr <<
            "UniaxialFiber3d::sendSelf() -  the material failed in sendSelf()\n";
        return -3;
    }
...
int UniaxialFiber3d::recvSelf(int commitTag, Channel & theChannel,
                              FEM_ObjectBroker & theBroker)
{
    ...
    if (theChannel.recvID(dbTag, commitTag, idData) < 0) {
        opserr <<
            "UniaxialFiber3d::recvSelf() -  failed to recv ID data\n";
        return -1;
    }
    ...
    if (theChannel.recvVector(dbTag, commitTag, dData) < 0) {
        opserr <<
            "UniaxialFiber3d::recvSelf() -  failed to recv Vector data\n";
        return -2;
    }
    area = dData(0);
    as[0] = dData(1);
    ...
    if (theMaterial->recvSelf(commitTag, theChannel, theBroker) < 0) {
        opserr <<
            "UniaxialFiber3d::recvSelf() -  the material failed in recvSelf()\n";
        return -4;
    }
...
Response *UniaxialFiber3d::setResponse(const char **argv, int argc,
                                       OPS_Stream & s)
{
    if (argc == 0)
        return 0;
...
int UniaxialFiber3d::getResponse(int responseID, Information & fibInfo)
{
    switch (responseID) {
    case 1:
...
void UniaxialFiber3d::getFiberLocation(double &yLoc, double &zLoc)
{
    yLoc = -as[0];
    zLoc = as[1];
}
