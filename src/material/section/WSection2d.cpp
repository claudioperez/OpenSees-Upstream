...
WSection2d::WSection2d(int tag, NDMaterial & theMat,
                       ...
                       int Nfdw, int Nftf, double shape,
                       double flag):SectionForceDeformation(tag,
                                                            SEC_TAG_WSection2d),
theFibers(0), yFibers(0), AFibers(0), e(6), d(D), tw(Tw), bf(Bf), tf(Tf),
nfdw(Nfdw), nftf(Nftf), shapeFactor(shape)
{
    ...
    for (int i = 0; i < numFibers; i++) {
        ...
        if (theFibers[i] == 0)
            opserr <<
                "WSection2d::WSection2d -- failed to get copy of beam fiber"
                << endln;
    }
...
d(0.0), tw(0.0), bf(0.0), tf(0.0), nfdw(0), nftf(0), shapeFactor(0.0)
{
    code(0) = SECTION_RESPONSE_P;
    code(1) = SECTION_RESPONSE_MZ;
    code(2) = SECTION_RESPONSE_MY;
...
//        0  0 0         0 sqrt(5/6)  y]
int
 WSection2d::setTrialSectionDeformation(const Vector & deforms)
{
    int res = 0;
...
const Vector & WSection2d::getSectionDeformation(void)
{
    return e;
}
...
//        0  0 0 sqrt(5/6)         0 -z
//        0  0 0         0 sqrt(5/6)  y]
const Matrix & WSection2d::getSectionTangent(void)
{
    ks.Zero();
...
//      0  0 0 sqrt(5/6)         0 -z
//      0  0 0         0 sqrt(5/6)  y]
const Vector & WSection2d::getStressResultant(void)
{
    s.Zero();
...
SectionForceDeformation *WSection2d::getCopy(void)
{
    WSection2d *theCopy = new WSection2d(this->getTag(), *theFibers[0],
...
const ID & WSection2d::getType(void)
{
    return code;
}

int WSection2d::getOrder(void) const
{
    return 6;
}

int WSection2d::commitState(void)
{
    int err = 0;
...
int WSection2d::revertToLastCommit(void)
{
    int err = 0;
...
int WSection2d::revertToStart(void)
{
    int err = 0;
...
int WSection2d::sendSelf(int commitTag, Channel & theChannel)
{
    return -1;
}

int WSection2d::recvSelf(int commitTag, Channel & theChannel,
                         FEM_ObjectBroker & theBroker)
{
    return -1;
}

void WSection2d::Print(OPS_Stream & s, int flag)
{
