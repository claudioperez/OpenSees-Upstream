...
void *OPS_Bidirectional()
{
    if (OPS_GetNumRemainingInputArgs() < 5) {
        ...
        opserr << "Want: section Bidirectional tag? E? sigY? Hiso? Hkin?"
            << endln;
        return 0;
    }
...
Bidirectional::Bidirectional
    (int tag, double e, double sy, double Hi, double Hk, int c1,
     int c2):SectionForceDeformation(tag, SEC_TAG_Bidirectional), E(e),
sigY(sy), Hiso(Hi), Hkin(Hk), code1(c1), code2(c2)
{
    for (int i = 0; i < 2; i++) {
        eP_n[i] = 0.0;
        eP_n1[i] = 0.0;
...
const Matrix & Bidirectional::getSectionTangent(void)
{
...
const Matrix & Bidirectional::getInitialTangent(void)
{
    ks(0, 0) = ks(1, 1) = E;
    ks(0, 1) = ks(1, 0) = 0.0;
...
const Vector & Bidirectional::getStressResultant(void)
{
    // Compute trial stress using elastic tangent
    s(0) = E * (e_n1[0] - eP_n[0]);
    s(1) = E * (e_n1[1] - eP_n[1]);
...
const Vector & Bidirectional::getSectionDeformation(void)
{
...
int Bidirectional::commitState(void)
{
    eP_n[0] = eP_n1[0];
    eP_n[1] = eP_n1[1];
...
int Bidirectional::revertToLastCommit(void)
{
    return 0;
}

int Bidirectional::revertToStart(void)
{
    for (int i = 0; i < 2; i++) {
        eP_n[i] = 0.0;
        eP_n1[i] = 0.0;
...
SectionForceDeformation *Bidirectional::getCopy(void)
{
    Bidirectional *theCopy =
        new Bidirectional(this->getTag(), E, sigY, Hiso, Hkin, code1,
                          code2);
...
const ID & Bidirectional::getType(void)
{
    code(0) = code1;
    code(1) = code2;
...
int Bidirectional::getOrder(void) const
{
    return 2;
}

int Bidirectional::sendSelf(int cTag, Channel & theChannel)
{
    int res = 0;
...
int Bidirectional::recvSelf(int cTag, Channel & theChannel,
...
void Bidirectional::Print(OPS_Stream & s, int flag)
{
