...
extern "C"
    int METIS_PartGraphRecursive(int *, int *, int *, int *, int *, int *,
                                 int *, int *, int *, int *, int *);

extern "C"
    int METIS_PartGraphKway(int *, int *, int *, int *, int *, int *,
                            int *, int *, int *, int *, int *);

extern "C"
    int METIS_PartMeshDual(int *, int *, int *, int *, int *, int *, int *,
                           int *, int *);
//int METIS_PartMeshDual(&ne, &nn, elmnts, &etype, &numflag, &nparts, &edgecut, epart, npart);

extern "C"
    int METIS_PartMeshNodal(int *, int *, int *, int *, int *, int *,
                            int *, int *, int *);
//int METIS_PartMeshNodal(&ne, &nn, elmnts, &etype, &numflag, &nparts, &edgecut, epart, npart);

#endif

Metis::Metis(int numParts):GraphNumberer(GraphNUMBERER_TAG_Metis),
myPtype(0),
myMtype(0),
myCoarsenTo(0),
myRtype(0),
myIPtype(0), defaultOptions(true), numPartitions(numParts), theRefResult(0)
{

}

Metis::Metis(int Ptype,
             int Mtype, int coarsenTo, int Rtype, int IPtype,
             int numParts):GraphNumberer(GraphNUMBERER_TAG_Metis),
myPtype(Ptype), myMtype(Mtype), myCoarsenTo(coarsenTo), myRtype(Rtype),
myIPtype(IPtype), defaultOptions(false), numPartitions(numParts),
theRefResult(0)
{
...
int Metis::partition(Graph & theGraph, int numPart)
{
    ...
    if (myPtype == 1) {
        ...
        opserr <<
            " Metis::partition PartGraphRecursive not yet implemented for METIS 5.1 \n";
#else
        METIS_PartGraphRecursive(&numVertex, xadj, adjncy, vwgts, ewgts,
                                 &weightflag, &numbering, &numPart,
        ...
        int errorflag =
            METIS_PartGraphKway(&numVertex, &ncon, xadj, adjncy, vwgts,
                                null_vsize, null_adjwgt, &numPart,
                                null_tpwgts, null_ubvec, NULL, &edgecut,
                                partition);

        if (errorflag != METIS_OK) {
            opserr << "METIS_PartGraphKway failed with flag: " <<
                errorflag << endln;
            if (errorflag == METIS_ERROR_INPUT)
                opserr << "Indicates an input error." << endln;
            else if (errorflag == METIS_ERROR_MEMORY)
                opserr <<
                    "Indicates that it could not allocate the required memory."
                    << endln;
            else if (errorflag == METIS_ERROR)
                opserr << "Indicates some other type of error." << endln;
            return -1;
        }
#else
        METIS_PartGraphKway(&numVertex, xadj, adjncy, vwgts, ewgts,
                            &weightflag, &numbering, &numPart, options,
                            &edgecut, partition);
#endif
    }
    //
...
int Metis::partitionGraph(int *nvtxs, int *xadj, int *adjncy, int *vwgt,
                          int *adjwgt, int *wgtflag, int *numflag,
                          int *nparts, int *options, int *edgecut,
                          int *part, bool whichToUse)
{
    ...
    if (whichToUse) {
#ifdef _USE_METIS_5p1
        opserr <<
            "METIS_PartGraphRecursive not implemented for METIS 5.1\n";
        return -1;
#else
        METIS_PartGraphRecursive(nvtxs, xadj, adjncy, vwgt, adjwgt,
        ...
        real_t *null_ubvec = NULL;
        int errorflag =
            METIS_PartGraphKway(nvtxs, &ncon, xadj, adjncy, vwgt,
                                null_vsize, null_adjwgt, nparts,
                                null_tpwgts, null_ubvec, NULL, edgecut,
                                part);
...
#else
        METIS_PartGraphKway(nvtxs, xadj, adjncy, vwgt, adjwgt, wgtflag,
...
int Metis::partitionHexMesh(int *elmnts, int *epart, int *npart, int ne,
                            int nn, int nparts, bool whichToUse)
{
    // which to use -> if true use edge cut partitinioning, else uses communication
    // based partition, standard aplications , uniformly mesh regions need use the edge-cut
    // first one works fine second is weird
...
#ifdef _USE_METIS_5p1
    opserr <<
        "METIS_PartMeshNodal or METIS_PartMeshDual not implemented for METIS 5.1\n";
    return -1;
#else
    if (whichToUse)
...
const ID & Metis::number(Graph & theGraph, int lastVertex)
{
    // first we check that the options are valid
    ...
    if (myPtype == 1) {
...
#else
        METIS_PartGraphRecursive(&numVertex, xadj, adjncy, vwgts, ewgts,
                                 &weightflag, &numbering, &numPartitions,
                                 options, &edgecut, partition);
#endif
    } else {
#ifdef _USE_METIS_5p1
        ...
        int errorflag =
            METIS_PartGraphKway(&numVertex, &ncon, xadj, adjncy, vwgts,
                                null_vsize, null_adjwgt, &numPartitions,
                                null_tpwgts, null_ubvec, options, &edgecut,
                                partition);
#else
        METIS_PartGraphKway(&numVertex, xadj, adjncy, vwgts, ewgts,
                            &weightflag, &numbering, &numPartitions,
...
const ID & Metis::number(Graph & theGraph, const ID & lastVertices)
{
    return this->number(theGraph);
}

int Metis::sendSelf(int cTag, Channel & theChannel)
{
    return 0;
}

int Metis::recvSelf(int cTag, Channel & theChannel,
