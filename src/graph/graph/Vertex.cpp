...
Vertex::Vertex(int tag, int ref, double weight,
               int color):TaggedObject(tag), myRef(ref), myWeight(weight),
myColor(color), myDegree(0), myTmp(0), myAdjacency(0, 8)
{

}


Vertex::Vertex(const Vertex & other):TaggedObject(other.getTag()),
myRef(other.myRef),
...
void Vertex::setTmp(int newTmp)
{
    myTmp = newTmp;
}

int Vertex::getRef(void) const
{
    return myRef;
}

double Vertex::getWeight(void) const
{
    return myWeight;
}
...
int Vertex::getTmp(void) const
{
    return myTmp;
}

int Vertex::addEdge(int otherTag)
{
...
int Vertex::getDegree(void) const
{
    return myDegree;
}

const ID & Vertex::getAdjacency(void) const
{
    return myAdjacency;
}

void Vertex::Print(OPS_Stream & s, int flag)
{
    s << this->getTag() << " ";
    s << myRef << " ";
    if (flag == 1)
        ...
        s << " weight: " << myWeight << " color: " << myColor << " tmp: "
            << myTmp << " ";
...
int Vertex::sendSelf(int commitTag, Channel & theChannel)
{
    ...
    if (theChannel.sendID(0, commitTag, idData) < 0) {
        opserr <<
            "Graph::sendSelf() - failed to receive the initial data\n";
        return -1;
    }
    ...
    if (theChannel.sendID(0, commitTag, myAdjacency) < 0) {
        opserr <<
            "Graph::sendSelf() - failed to receive the adjacency data\n";
        return -1;
    }
...
int Vertex::recvSelf(int commitTag, Channel & theChannel,
                     FEM_ObjectBroker & theBroker)
{
    ...
    if (theChannel.recvID(0, commitTag, idData) < 0) {
        opserr <<
            "Graph::recvSelf() - failed to receive the initial data\n";
        return -1;
    }
