...
DOF_Graph::DOF_Graph (AnalysisModel & theModel):Graph (theModel.getNumEqn ()),
myModel (theModel)
{
          ...
          for (int i = 0; i < size; i++)
            {
                ...
                if (dofTag >= START_EQN_NUM)
                  {
                      Vertex *
                          vertexPtr = this->getVertexPtr (dofTag);
                      if (vertexPtr == 0)
                        {
                            Vertex *
                            ...
                            if (this->addVertex (vertexPtr, false) == false)
                              {
                                  opserr <<
                                      "WARNING DOF_Graph::DOF_Graph - error adding vertex\n";
                              }
                        }
                  }
            }
                      ...
                      for (int j = i + 1; j < size; j++)
                        {
                            ...
                            if (eqn2 >= START_EQN_NUM)
                                this->addEdge (eqn1 - START_EQN_NUM +
                                               START_VERTEX_NUM,
                                               eqn2 - START_EQN_NUM +
                                               START_VERTEX_NUM);
                        }
