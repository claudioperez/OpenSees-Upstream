...
Graph::Graph():myVertices(0), theVertexIter(0), numEdge(0),
nextFreeTag(START_VERTEX_NUM), vertices()
{
...
Graph::Graph(int numVertices):myVertices(0),
theVertexIter(0), numEdge(0), nextFreeTag(START_VERTEX_NUM), vertices()
{
    myVertices = new MapOfTaggedObjects();
    theVertexIter = new VertexIter(myVertices);
}
...
Graph::Graph(TaggedObjectStorage & theVerticesStorage):myVertices(&theVerticesStorage), theVertexIter(0), numEdge(0),
...
Graph::Graph(Graph & other):myVertices(0), theVertexIter(0), numEdge(0),
nextFreeTag(START_VERTEX_NUM), vertices()
{
    myVertices = new MapOfTaggedObjects();
    theVertexIter = new VertexIter(myVertices);
 ...
 Graph::addVertex(Vertex * vertexPtr, bool checkAdjacency)
{
    ...
    if (result == false) {
        ...
        opserr << "WARNING Graph::addVertex";
        opserr <<
            " - vertex could not be stored in TaggedObjectStorage object\n";
    }
...
int Graph::addEdge(int vertexTag, int otherVertexTag)
{
    ...
    if ((vertex1 == 0) || (vertex2 == 0)) {
        opserr <<
            "WARNING Graph::addEdge() - one or both of the vertices ";
...
int Graph::addEdgeFast(int vertexTag, int otherVertexTag)
{
    // get pointers to the vertices, if one does not exist return
    if (vertices.size() <= vertexTag || vertices.size() <= otherVertexTag) {
...
Vertex *Graph::getVertexPtr(int vertexTag)
{
...
VertexIter & Graph::getVertices(void)
{
...
int Graph::getNumVertex(void) const
{
    return myVertices->getNumComponents();
}

int Graph::getNumEdge(void) const
{
    return numEdge;
}

int Graph::getFreeTag(void)
{
    return nextFreeTag;
}

Vertex *Graph::removeVertex(int tag, bool flag)
{
...
void Graph::Print(OPS_Stream & s, int flag)
{
...
int Graph::sendSelf(int commitTag, Channel & theChannel)
{
    // check not a datastore .. 
    if (theChannel.isDatastore() != 0) {
        opserr <<
            "Graph::sendSelf() - does not at present send to a database\n";
        return -1;
    }
...
int Graph::recvSelf(int commitTag, Channel & theChannel,
                    FEM_ObjectBroker & theBroker)
{
    // check not from a datastore
    if (theChannel.isDatastore() != 0) {
        opserr <<
            "Graph::recvSelf() - at present does not receive from a database\n";
        return -1;
    }
    // check blank
    if (this->getNumVertex() != 0) {
        opserr <<
            "Graph::recvSelf() - can only receive to an empty graph at present\n";
    ...
    if (numVertex != 0) {
        ...
        if (vertexData != 0) {
            ...
            if (theChannel.recvVector(0, commitTag, vertexWeights) < 0) {
                opserr <<
                    "Graph::recvSelf() - failed to receive the weights\n";
                return -3;
            }
