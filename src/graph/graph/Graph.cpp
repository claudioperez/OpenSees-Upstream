...
nextFreeTag (START_VERTEX_NUM), vertices ()
{
    ...
    while ((vertexPtr = otherVertices ()) != 0)
      {
          int
              vertexTag = vertexPtr->getTag ();
          int
              vertexRef = vertexPtr->getRef ();
    ...
    while ((vertexPtr = otherVertices2 ()) != 0)
      {
          ...
          const
              ID &
              adjacency = vertexPtr->getAdjacency ();
          ...
          if (vertexData != 0)
            {
                ...
                ID verticesData (vertexData, 5 * numVertex + 2 * numEdge,
                                 true);
...
int
Graph::recvSelf (int commitTag, Channel & theChannel,
          ...
          if (vertexData != 0)
            {
                ...
                if (theChannel.recvID (0, commitTag, verticesData) < 0)
                  {
                      opserr <<
                          "Graph::recvSelf() - failed to receive the id\n";
                      return -3;
                  }
                Vector vertexWeights (numVertex);
