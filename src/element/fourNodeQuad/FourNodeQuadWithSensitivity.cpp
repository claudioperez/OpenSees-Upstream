...
Vector FourNodeQuadWithSensitivity::P(8);
double FourNodeQuadWithSensitivity::shp[3][4];
double FourNodeQuadWithSensitivity::pts[4][2];
double FourNodeQuadWithSensitivity::wts[4];
...
int
FourNodeQuadWithSensitivity::getNumExternalNodes() const
{
    return 4;
}

const ID&
FourNodeQuadWithSensitivity::getExternalNodes()
{
...
Node **
FourNodeQuadWithSensitivity::getNodePtrs(void)
{
  return theNodes;
}

int
FourNodeQuadWithSensitivity::getNumDOF()
{
    return 8;
}

void
FourNodeQuadWithSensitivity::setDomain(Domain *theDomain)
{
	...
	//opserr << "FATAL ERROR FourNodeQuadWithSensitivity (tag: %d), node not found in domain",
	//	this->getTag());
	...
	//opserr << "FATAL ERROR FourNodeQuadWithSensitivity (tag: %d), has differing number of DOFs at its nodes",
	//	this->getTag());
...
int
FourNodeQuadWithSensitivity::commitState()
{
    int retVal = 0;

    // call element commitState to do any base class stuff
    if ((retVal = this->Element::commitState()) != 0) {
      opserr << "FourNodeQuadWithSensitivity::commitState () - failed in base class";
    }
...
int
FourNodeQuadWithSensitivity::revertToLastCommit()
{
    int retVal = 0;
...
int
FourNodeQuadWithSensitivity::revertToStart()
{
    int retVal = 0;
...
int
FourNodeQuadWithSensitivity::update()
{
...
const Matrix&
FourNodeQuadWithSensitivity::getTangentStiff()
{
...
const Matrix&
FourNodeQuadWithSensitivity::getInitialStiff()
{
  if (Ki != 0)
    return *Ki;
...
const Matrix&
FourNodeQuadWithSensitivity::getMass()
{
	K.Zero();
...
const Vector&
FourNodeQuadWithSensitivity::getResistingForce()
{
	P.Zero();
...
const Vector&
FourNodeQuadWithSensitivity::getResistingForceIncInertia()
{
	int i;
	static double rhoi[4];
	double sum = this->rho;
...
int
FourNodeQuadWithSensitivity::sendSelf(int commitTag, Channel &theChannel)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING FourNodeQuadWithSensitivity::sendSelf() - " << this->getTag() << " failed to send Vector\n";
    return res;
  }
  ...
  if (res < 0) {
    opserr << "WARNING FourNodeQuadWithSensitivity::sendSelf() - " << this->getTag() << " failed to send ID\n";
    return res;
  }
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (res < 0) {
      opserr << "WARNING FourNodeQuadWithSensitivity::sendSelf() - " << this->getTag() << " failed to send its Material\n";
      return res;
    }
  }
...
int
FourNodeQuadWithSensitivity::recvSelf(int commitTag, Channel &theChannel,
						FEM_ObjectBroker &theBroker)
{
  ...
  if (res < 0) {
    opserr << "WARNING FourNodeQuadWithSensitivity::recvSelf() - failed to receive Vector\n";
    return res;
  }
  ...
  if (res < 0) {
    opserr << "WARNING FourNodeQuadWithSensitivity::recvSelf() - " << this->getTag() << " failed to receive ID\n";
    return res;
  }
  ...
  if (theMaterial == 0) {
    ...
    theMaterial = new NDMaterial *[4];
    if (theMaterial == 0) {
      opserr << "FourNodeQuadWithSensitivity::recvSelf() - Could not allocate NDMaterial* array\n";
      return -1;
    }
    for (int i = 0; i < 4; i++) {
      ...
      if (theMaterial[i] == 0) {
	opserr << "FourNodeQuadWithSensitivity::recvSelf() - Broker could not create NDMaterial of class type " << matClassTag << endln;
	return -1;
      }
...
Response*
FourNodeQuadWithSensitivity::setResponse(const char **argv, int argc, 
			  OPS_Stream &output)
{
	...
	return theMaterial[pointNum-1]->setResponse(&argv[2], argc-2, output);
      else 
	return 0;
...
int
FourNodeQuadWithSensitivity::setParameter(const char **argv, int argc, Parameter &param)
{
  int res = -1;
  ...
  // a material parameter
	  ...
	  for (int i=0; i<4; i++) {
			res = theMaterial[i]->setParameter(&argv[1], argc-1, param);
			if (res<0) {
				opserr<<"FourNodeQuadWithSensitivity::setParameter() can not set parameter! \n";
				return -1;
			}
	  }
  }
...
int
FourNodeQuadWithSensitivity::updateParameter(int parameterID, Information &info)
{
  switch (parameterID) {
    case -1:
      return -1;
...
void 
FourNodeQuadWithSensitivity::setPressureLoadAtNodes(void)
{
        pressureLoad.Zero();
...
int
FourNodeQuadWithSensitivity::activateParameter(int passedParameterID)
{
...
const Matrix &
FourNodeQuadWithSensitivity::getKiSensitivity(int gradNumber)
{
	K.Zero();
	return K;
}
...
const Matrix &
FourNodeQuadWithSensitivity::getMassSensitivity(int gradNumber)
{
	K.Zero();
	return K;
}
...
const Vector &
FourNodeQuadWithSensitivity::getResistingForceSensitivity(int gradNumber)
{
	P.Zero();
...
int
FourNodeQuadWithSensitivity::commitSensitivity(int gradNumber, int numGrads)
{
	
	static double u[2][4];

	u[0][0] = theNodes[0]->getDispSensitivity(1,gradNumber);
	u[1][0] = theNodes[0]->getDispSensitivity(2,gradNumber);
	u[0][1] = theNodes[1]->getDispSensitivity(1,gradNumber);
	u[1][1] = theNodes[1]->getDispSensitivity(2,gradNumber);
	u[0][2] = theNodes[2]->getDispSensitivity(1,gradNumber);
	u[1][2] = theNodes[2]->getDispSensitivity(2,gradNumber);
	u[0][3] = theNodes[3]->getDispSensitivity(1,gradNumber);
	u[1][3] = theNodes[3]->getDispSensitivity(2,gradNumber);
