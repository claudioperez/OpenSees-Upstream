...
peerSearchNGA (const char *Eq,
               ...
               const char *LatNE,
               const char *LngSW,
               const char *LngNE, StringContainer & ngaRecordNames)
{
    char *resHTML = 0;
    char *ngaHTML = 0;
    ...
    if (strlen (latNE) == 0)
        sprintf (peerPage,
                 "/nga/search?qid=&fault=%s&mag_lo=%s&mag_hi=%s&dist_lo=%s&dist_hi=%s&vs30_lo=%s&vs30_hi=%s&pga_lo=%s&pga_hi=%s&loc=&format=&latSW=&latNE=&lngSW=&lngNE=",
                 fault, magLo, magHi, distLo, distHi, vsLo, vsHi, pgaLo,
                 pgaHi);
    else
        sprintf (peerPage,
                 "/nga/search?qid=&fault=%s&mag_lo=%s&mag_hi=%s&dist_lo=%s&dist_hi=%s&vs30_lo=%s&vs30_hi=%s&pga_lo=%s&pga_hi=%s&loc=&format=&latSW=%s&latNE=%s&lngSW=%s&lngNE=%s",
                 fault, magLo, magHi, distLo, distHi, vsLo, vsHi, pgaLo,
                 pgaHi, latSW, latNE, lngSW, lngNE);
                            ...
                            if (ok == 0)
                              {
                                  ...
                                  if (recordLoc != 0)
                                    {
                                        recordLoc += 14;
                                        char *recordEndLoc =
                                            strstr (recordLoc, ".AT2");
                                        int recordLength =
                                            recordEndLoc - recordLoc;
                                        char *recordName =
                                            new char[recordLength + 1];
                                        strncpy (recordName, recordLoc,
                                                 recordLength);
                                  ...
                                  recordLoc =
                                      strstr (recordLoc, "/nga_files/ath/");
                                  if (recordLoc != 0)
                                    {
                                        recordLoc += 14;
                                        char *recordEndLoc =
