...
NormDispOrUnbalance::NormDispOrUnbalance(double theTolDisp,
    ...
    (CONVERGENCE_TEST_NormDispOrUnbalance), theSOE(0), tolDisp(theTolDisp),
tolUnbalance(theTolUnbalance), maxNumIter(maxIter), currentIter(0),
printFlag(printIt), norms(2 * maxIter), nType(normType), maxIncr(maxincr),
numIncr(0)
{
    if (maxIncr < 0)
        maxIncr = maxNumIter;
...
int NormDispOrUnbalance::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (res < 0)
        opserr <<
            "NormDispOrUnbalance::sendSelf() - failed to send data\n";

    return res;
}

int NormDispOrUnbalance::recvSelf(int cTag, Channel & theChannel,
