...
NormDispOrUnbalance::NormDispOrUnbalance ():ConvergenceTest (CONVERGENCE_TEST_NormDispOrUnbalance),
...
NormDispOrUnbalance::NormDispOrUnbalance (double theTolDisp,
                                          double theTolUnbalance, int maxIter,
                                          int printIt, int normType,
                                          int maxincr):
ConvergenceTest (CONVERGENCE_TEST_NormDispOrUnbalance),
theSOE (0),
tolDisp (theTolDisp),
tolUnbalance (theTolUnbalance),
maxNumIter (maxIter),
currentIter (0),
printFlag (printIt),
norms (2 * maxIter),
...
ConvergenceTest *
NormDispOrUnbalance::getCopy (int iterations)
{
...
void
NormDispOrUnbalance::setTolerance (double newTolDisp)
{
    tolDisp = newTolDisp;
}
...
int
NormDispOrUnbalance::setEquiSolnAlgo (EquiSolnAlgo & theAlgo)
{
...
NormDispOrUnbalance::test (void)
{
    ...
    if (currentIter == 0)
      {
          opserr <<
              "WARNING: NormDispOrUnbalance::test() - start() was never invoked.\n";
          return -2;
      }
    ...
    if (printFlag == 1)
      {
          opserr << "NormDispOrUnbalance::test() - iteration: " <<
              currentIter;
    ...
    // algo failed to converged after specified number of iterations - but RETURN OK
      ...
      {
          opserr <<
              "WARNING: NormDispOrUnbalance::test() - failed to converge but going on - ";
          opserr << " current NormX: " << normX;
          opserr << ", NormB: " << normB << ", NormIncr: " << numIncr << "\n";
          return currentIter;
      }
    ...
    else if (currentIter >= maxNumIter || numIncr > maxIncr)
      {                         // failes to converge
          opserr <<
              "WARNING: NormDispOrUnbalance::test() - failed to converge \n";
...
int
NormDispOrUnbalance::start (void)
{
    if (theSOE == 0)
      {
          opserr <<
              "WARNING: NormDispOrUnbalance::test() - no SOE returning true\n";
          return -1;
      }
...
int
NormDispOrUnbalance::getNumTests ()
{
    return currentIter;
}
...
int
NormDispOrUnbalance::getMaxNumTests (void)
{
    return maxNumIter;
}
...
double
NormDispOrUnbalance::getRatioNumToMax (void)
{
