...
CTestPFEM::CTestPFEM():ConvergenceTest(CONVERGENCE_TEST_CTestPFEM),
theSOE(0), tolv(0), tolp(0), tolv2(0), tolp2(0), tolvrel(0),
tolprel(0), maxNumIter(0), currentIter(0), printFlag(0), nType(2),
maxIncr(-1), numIncr(0), normsv(), normsp(), normsresv(), normsresp(),
...
CTestPFEM::CTestPFEM(double tv, double tp, double tv2, double tp2,
                     double tvrel, double tprel, int maxIter, int maxincr,
                     int printIt,
                     int
                     normType):ConvergenceTest(CONVERGENCE_TEST_CTestPFEM),
theSOE(0), tolv(tv), tolp(tp), tolv2(tv2), tolp2(tp2), tolvrel(tvrel),
tolprel(tprel), maxNumIter(maxIter), currentIter(0), printFlag(printIt),
nType(normType), maxIncr(maxincr), numIncr(0), normsv(), normsp(),
normsresv(), normsresp(), normv0(0), normp0(0), norms()
{
...
ConvergenceTest *CTestPFEM::getCopy(int iterations)
{
    CTestPFEM *theCopy;
    theCopy =
        new CTestPFEM(this->tolv, this->tolp, this->tolv2, this->tolp2,
                      this->tolvrel, this->tolprel, iterations,
                      this->maxIncr, this->printFlag, this->nType);
...
int CTestPFEM::setEquiSolnAlgo(EquiSolnAlgo & theAlgo)
{
    try {
        ...
        opserr <<
            "WARNING: system is not PFEMLinSOE -- CTestPFEM::setEquiSolnAlgo\n";
        return -1;
    }
    if (theSOE == 0) {
...
int CTestPFEM::test(void)
{
    ...
    if (currentIter == 0) {
        opserr <<
            "WARNING: CTestPFEM::test() - start() was never invoked.\n";
        return -2;
    }
    ...
    if (dofType.Size() != x.Size()) {
        opserr <<
            "WARNING: x and dofType have different size -- CTestPFEM::test()\n";
        return -2;
    }
    ...
    if (MPI_Bcast(&allnorms, 6, MPI_DOUBLE, 0, MPI_COMM_WORLD) !=
        MPI_SUCCESS) {
    ...
    if (stage == 0 && currentIter > 1 && maxIncr > 0) {
        if (normv > 10 * normsv[currentIter - 2]
            || normp > 10 * normsp[currentIter - 2]
            || normresv > 10 * normsresv[currentIter - 2]
            || normresp > 10 * normsresp[currentIter - 2]) {
            numIncr++;
        }
    }
    ...
    if ((normv <= tolv && normp <= tolp)
        || (normresv <= tolv2 && normresp <= tolp2) || (normvrel <= tolvrel
                                                        && normprel <=
                                                        tolprel)) {
    ...
    // algo failed to converged after specified number of iterations - but RETURN OK
             ...
             && (currentIter >= maxNumIter || numIncr > maxIncr
                 || badnorm)) {
