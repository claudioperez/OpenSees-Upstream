...
ConvergenceTest *
CTestPFEM::getCopy (int iterations)
{
    CTestPFEM *theCopy;
...
void
CTestPFEM::setTolerance (double newTol)
{
...
int
CTestPFEM::setEquiSolnAlgo (EquiSolnAlgo & theAlgo)
{
    try
    {
...
CTestPFEM::test (void)
{
    ...
    else if (currentIter >= maxNumIter || numIncr > maxIncr || badnorm)
      {                         // failes to converge
...
double
CTestPFEM::getRatioNumToMax (void)
{
...
int
CTestPFEM::sendSelf (int cTag, Channel & theChannel)
{
    int res = 0;
    Vector x (10);
    x (0) = tolv;
...
int
CTestPFEM::recvSelf (int cTag, Channel & theChannel,
