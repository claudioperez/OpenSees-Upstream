...
CTestNormUnbalance::CTestNormUnbalance():ConvergenceTest(CONVERGENCE_TEST_CTestNormUnbalance),
theSOE(0), tol(0.0), maxTol(OPS_MAXTOL), maxNumIter(0), currentIter(0),
printFlag(0), norms(1), nType(2), maxIncr(0), numIncr(0)
{

}


CTestNormUnbalance::CTestNormUnbalance(double theTol, int maxIter,
                                       int printIt, int normType,
                                       int maxincr,
                                       double
                                       max):ConvergenceTest
    (CONVERGENCE_TEST_CTestNormUnbalance), theSOE(0), tol(theTol),
...
ConvergenceTest *CTestNormUnbalance::getCopy(int iterations)
{
    ...
    theCopy =
        new CTestNormUnbalance(this->tol, iterations, this->printFlag,
...
int CTestNormUnbalance::test(void)
{
    ...
    if (currentIter == 0) {
        opserr <<
            "WARNING: CTestNormUnbalance::test() - start() was never invoked.\n";
        return -2;
    }
    ...
    if (printFlag == 1) {
        opserr << "CTestNormUnbalance::test() - iteration: " <<
            currentIter;
...
int CTestNormUnbalance::start(void)
{
    if (theSOE == 0) {
        opserr <<
            "WARNING: CTestNormUnbalance::test() - no SOE returning true\n";
        return -1;
    }
