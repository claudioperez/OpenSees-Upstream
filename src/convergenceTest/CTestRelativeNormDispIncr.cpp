...
CTestRelativeNormDispIncr::CTestRelativeNormDispIncr():ConvergenceTest(CONVERGENCE_TEST_CTestRelativeNormDispIncr),
...
CTestRelativeNormDispIncr::CTestRelativeNormDispIncr(double theTol,
                                                     int maxIter,
                                                     int printIt,
                                                     int
                                                     normType):ConvergenceTest
    (CONVERGENCE_TEST_CTestRelativeNormDispIncr), theSOE(0), tol(theTol),
maxNumIter(maxIter), currentIter(0), printFlag(printIt), norms(maxIter),
...
ConvergenceTest *CTestRelativeNormDispIncr::getCopy(int iterations)
{
    ...
    theCopy =
        new CTestRelativeNormDispIncr(this->tol, iterations,
...
int CTestRelativeNormDispIncr::test(void)
{
    ...
    if (theSOE == 0) {
        opserr <<
            "WARNING: CTestRelativeNormDispIncr::test() - no SOE set.\n";
        return -1;
    }
    ...
    if (printFlag == 1) {
        opserr << "CTestRelativeNormDispIncr::test() - iteration: " <<
            currentIter;
        opserr << " current Ratio (|dR|/|dR1|): " << norm << " (max: " <<
            tol << ")\n";
    }
    if (printFlag == 4) {
        opserr << "CTestRelativeNormDispIncr::test() - iteration: " <<
    ...
    if (norm <= tol) {
        ...
        if (printFlag != 0) {
            if (printFlag == 1 || printFlag == 4)
                ...
                opserr <<
                    "CTestRelativeNormDispIncr::test() - iteration: " <<
                    currentIter;
                opserr << " current Ratio (|dR|/|dR1|): " << norm <<
                    " (max: " << tol << ")\n";
            }
        }
    ...
    // algo failed to converged after specified number of iterations - but RETURN OK
        ...
        opserr <<
            "WARNING: CTestRelativeNormDispIncr::test() - failed to converge but going on -";
        opserr << " current Ratio (|dR|/|dR1|): " << norm << " (max: " <<
            tol;
        opserr << ", Norm deltaR: " << theSOE->
            getB().pNorm(nType) << ")\n";
        return currentIter;
    }
...
int CTestRelativeNormDispIncr::start(void)
{
    if (theSOE == 0) {
        opserr <<
            "WARNING: CTestRelativeNormDispIncr::test() - no SOE returning true\n";
        return -1;
    }
...
int CTestRelativeNormDispIncr::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (res < 0)
        opserr <<
            "CTestRelativeNormDispIncr::sendSelf() - failed to send data\n";
