...
CTestEnergyIncr::CTestEnergyIncr():ConvergenceTest(CONVERGENCE_TEST_CTestEnergyIncr),
theSOE(0), tol(0), maxTol(OPS_MAXTOL), maxNumIter(0), currentIter(0),
printFlag(0), nType(2), norms(20)
{
...
CTestEnergyIncr::CTestEnergyIncr(double theTol, int maxIter, int printIt,
                                 ...
                                 double
                                 max):ConvergenceTest
    (CONVERGENCE_TEST_CTestEnergyIncr), theSOE(0), tol(theTol),
...
ConvergenceTest *CTestEnergyIncr::getCopy(int iterations)
{
    ...
    theCopy =
        new CTestEnergyIncr(this->tol, iterations, this->printFlag,
                            this->nType, this->maxTol);
...
int CTestEnergyIncr::test(void)
{
    ...
    if (currentIter == 0) {
        opserr <<
            "WARNING: CTestEnergyIncr::test() - start() was never invoked.\n";
        return -2;
    }
    ...
    if (product <= tol) {
        ...
        if (printFlag != 0) {
            if (printFlag == 1 || printFlag == 4)
                ...
                opserr << "CTestEnergyIncr::test() - iteration: " <<
                    currentIter;
                opserr << " last EnergyIncr: " << product << " (max: "
                    << tol << ")\n";
            }
        }
    ...
    // algo failed to converged after specified number of iterations - but RETURN OK
        ...
        opserr <<
            "WARNING: CTestEnergyIncr::test() - failed to converge but goin on -";
        opserr << " current EnergyIncr: " << product << " (max: " << tol <<
            ")\n";
        opserr << "\tNorm deltaX: " << x.pNorm(nType) << ", Norm deltaR: "
            << b.pNorm(nType) << endln;
...
int CTestEnergyIncr::start(void)
{
    if (theSOE == 0) {
        opserr <<
            "WARNING: CTestEnergyIncr::test() - no SOE returning true\n";
        return -1;
    }
