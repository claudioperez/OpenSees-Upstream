...
CTestFixedNumIter::CTestFixedNumIter ():ConvergenceTest (CONVERGENCE_TEST_CTestFixedNumIter),
...
CTestFixedNumIter::getCopy (int iterations)
{
    ...
    theCopy =
        new CTestFixedNumIter (iterations, this->printFlag, this->nType);
...
int
CTestFixedNumIter::setEquiSolnAlgo (EquiSolnAlgo & theAlgo)
{
...
CTestFixedNumIter::test (void)
{
    ...
    if (currentIter == 0)
      {
          opserr <<
              "WARNING: CTestFixedNumIter::test() - start() was never invoked.\n";
          return -2;
      }
    ...
    if (printFlag == 1)
      {
          ...
          opserr << " (Norm deltaX: " << x.
              pNorm (nType) << ", Norm deltaR: " << b.pNorm (nType) << ")\n";
      }
    if (printFlag == 4)
      {
          opserr << "CTestFixedNumIter::test() - iteration: " << currentIter;
          ...
          if (printFlag != 0)
            {
                if (printFlag == 1 || printFlag == 4)
                  ...
                  {
                      opserr << "CTestFixedNumIter::test() - iteration: " <<
                          currentIter;
                      opserr << " last EnergyIncr: " << product;
                      opserr << " (Norm deltaX: " << x.
                          pNorm (nType) << ", Norm deltaR: " << b.
                          pNorm (nType) << ")\n";
                  }
            }
...
int
CTestFixedNumIter::start (void)
{
    if (theSOE == 0)
      {
          opserr <<
              "WARNING: CTestFixedNumIter::test() - no SOE returning true\n";
          return -1;
      }
...
int
CTestFixedNumIter::getMaxNumTests (void)
{
    return maxNumIter;
}
...
double
CTestFixedNumIter::getRatioNumToMax (void)
{
...
int
CTestFixedNumIter::sendSelf (int cTag, Channel & theChannel)
{
    int res = 0;
    Vector x (3);
    x (0) = maxNumIter;
...
int
CTestFixedNumIter::recvSelf (int cTag, Channel & theChannel,
