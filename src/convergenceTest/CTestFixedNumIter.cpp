...
CTestFixedNumIter::CTestFixedNumIter(int maxIter, int printIt,
                                     int
                                     normType):ConvergenceTest
    (CONVERGENCE_TEST_CTestFixedNumIter), theSOE(0), maxNumIter(maxIter),
currentIter(0), printFlag(printIt), norms(maxNumIter), nType(normType)
{
    ...
    if (currentIter == maxNumIter) {
        ...
        if (printFlag != 0) {
            if (printFlag == 1 || printFlag == 4)
                ...
                opserr << " (Norm deltaX: " << x.pNorm(nType) <<
                    ", Norm deltaR: " << b.pNorm(nType) << ")\n";
            }
        }
