...
NormDispAndUnbalance::NormDispAndUnbalance ():ConvergenceTest (CONVERGENCE_TEST_NormDispAndUnbalance),
...
NormDispAndUnbalance::NormDispAndUnbalance (double theTolDisp,
                                            double theTolUnbalance,
...
int
NormDispAndUnbalance::setEquiSolnAlgo (EquiSolnAlgo & theAlgo)
{
...
int
NormDispAndUnbalance::test (void)
{
    ...
    if (currentIter == 0)
      {
          opserr <<
              "WARNING: NormDispAndUnbalance::test() - start() was never invoked.\n";
          return -2;
      }
    ...
    if (printFlag == 1)
      {
          opserr << "NormDispAndUnbalance::test() - iteration: " <<
              currentIter;
    ...
    // algo failed to converged after specified number of iterations - but RETURN OK
      ...
      {
          opserr <<
              "WARNING: NormDispAndUnbalance::test() - failed to converge but going on - ";
          opserr << " current NormX: " << normX;
          opserr << ", NormB: " << normB << ", NormIncr: " << numIncr << "\n";
          return currentIter;
      }
    ...
    // algo failed to converged after specified number of iterations - return FAILURE -2
      ...
      {                         // failes to converge
          opserr <<
              "WARNING: NormDispAndUnbalance::test() - failed to converge \n";
...
int
NormDispAndUnbalance::start (void)
{
    if (theSOE == 0)
      {
          opserr <<
              "WARNING: NormDispAndUnbalance::test() - no SOE returning true\n";
          return -1;
      }
...
int
NormDispAndUnbalance::getMaxNumTests (void)
{
    return maxNumIter;
}
...
double
NormDispAndUnbalance::getRatioNumToMax (void)
{
...
int
NormDispAndUnbalance::sendSelf (int cTag, Channel & theChannel)
{
    int res = 0;
    Vector x (6);
    x (0) = tolDisp;
                                ...
                                FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0)
      {
          opserr <<
              "NormDispAndUnbalance::sendSelf() - failed to send data\n";
          tolDisp = 1.0e-8;
          maxNumIter = 25;
          printFlag = 0;
          nType = 2;
