...
NormDispAndUnbalance::NormDispAndUnbalance(double theTolDisp,
                                           ...
                                           int normType,
                                           int
                                           maxincr):ConvergenceTest
    (CONVERGENCE_TEST_NormDispAndUnbalance), theSOE(0),
tolDisp(theTolDisp), tolUnbalance(theTolUnbalance), maxNumIter(maxIter),
currentIter(0), printFlag(printIt), norms(2 * maxIter), nType(normType),
maxIncr(maxincr), numIncr(0)
{
    if (maxIncr < 0)
        maxIncr = maxNumIter;
}
...
int NormDispAndUnbalance::test(void)
{
    ...
    if ((currentIter > 1 && norms(currentIter - 2) < normX)
        || (currentIter > 1
            && norms(maxNumIter + currentIter - 2) < normB)) {
        numIncr++;
    }
...
int NormDispAndUnbalance::recvSelf(int cTag, Channel & theChannel,
