...
CTestRelativeTotalNormDispIncr(double theTol, int maxIter, int printIt,
                               int
                               normType):ConvergenceTest
(CONVERGENCE_TEST_CTestRelativeTotalNormDispIncr), theSOE(0), tol(theTol), maxNumIter(maxIter),
currentIter(0), printFlag(printIt), norms(maxIter), totNorm(0.0), nType(normType)
{
...
int CTestRelativeTotalNormDispIncr::test(void)
{
    ...
    if (printFlag == 4) {
        ...
        opserr << " current ratio (|dR|/|dRtot|): " << norm << " (max: " <<
            tol << ")\n";
        opserr << "\tNorm deltaX: " << norm << ", Norm deltaR: " <<
            theSOE->getB().pNorm(nType) << endln;
...
int CTestRelativeTotalNormDispIncr::sendSelf(int cTag,
                                             Channel & theChannel)
{
    int res = 0;
    static Vector x(4);
...
int CTestRelativeTotalNormDispIncr::recvSelf(int cTag,
                                             Channel & theChannel,
