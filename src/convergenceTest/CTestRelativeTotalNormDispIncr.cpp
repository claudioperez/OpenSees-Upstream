...
CTestRelativeTotalNormDispIncr::CTestRelativeTotalNormDispIncr ():ConvergenceTest (CONVERGENCE_TEST_CTestRelativeTotalNormDispIncr),
...
CTestRelativeTotalNormDispIncr::CTestRelativeTotalNormDispIncr (double theTol,
                                                                int maxIter,
                                                                int printIt,
                                                                int normType):
...
CTestRelativeTotalNormDispIncr::getCopy (int iterations)
{
    ...
    theCopy =
        new CTestRelativeTotalNormDispIncr (this->tol, iterations,
...
int
CTestRelativeTotalNormDispIncr::setEquiSolnAlgo (EquiSolnAlgo & theAlgo)
{
...
int
CTestRelativeTotalNormDispIncr::test (void)
{
    ...
    if (theSOE == 0)
      {
          opserr <<
              "WARNING: CTestRelativeTotalNormDispIncr::test() - no SOE set.\n";
          return -1;
      }
    ...
    if (currentIter == 0)
      {
          opserr <<
              "WARNING: CTestRelativeTotalNormDispIncr::test() - start() was never invoked.\n";
          return -2;
      }
    ...
    if (printFlag == 1)
      {
          opserr << "CTestRelativeTotalNormDispIncr::test() - iteration: " <<
              currentIter;
          opserr << " current ratio (|dR|/|dRtot|): " << norm << " (max: " <<
              tol << ")\n";
      }
    if (printFlag == 4)
      {
          ...
          opserr << " current ratio (|dR|/|dRtot|): " << norm << " (max: " <<
              tol << ")\n";
          opserr << "\tNorm deltaX: " << norm << ", Norm deltaR: " << theSOE->
              getB ().pNorm (nType) << endln;
          ...
          if (printFlag != 0)
            {
                if (printFlag == 1 || printFlag == 4)
                  ...
                  {
                      opserr <<
                          "CTestRelativeTotalNormDispIncr::test() - iteration: "
                          << currentIter;
                      opserr << " current ratio (|dR|/|dRtot|): " << norm <<
                          " (max: " << tol << ")\n";
                  }
            }
    ...
    // algo failed to converged after specified number of iterations - but RETURN OK
      ...
      {
          opserr <<
              "WARNING: CTestRelativeTotalNormDispIncr::test() - failed to converge but going on -";
          opserr << " current ratio (|dR|/|dRtot|): " << norm << " (max: " <<
              tol << ")\n";
          opserr << "\tNorm deltaX: " << norm << ", Norm deltaR: " << theSOE->
              getB ().pNorm (nType) << endln;
    ...
    // algo failed to converged after specified number of iterations - return FAILURE -2
      ...
      {                         // failes to converge
          opserr <<
              "WARNING: CTestRelativeTotalNormDispIncr::test() - failed to converge \n";
...
int
CTestRelativeTotalNormDispIncr::start (void)
{
    if (theSOE == 0)
      {
          opserr <<
              "WARNING: CTestRelativeTotalNormDispIncr::test() - no SOE returning true\n";
          return -1;
      }
...
int
CTestRelativeTotalNormDispIncr::getMaxNumTests (void)
{
    return maxNumIter;
}
...
double
CTestRelativeTotalNormDispIncr::getRatioNumToMax (void)
{
...
int
CTestRelativeTotalNormDispIncr::sendSelf (int cTag, Channel & theChannel)
{
    int res = 0;
    static Vector x (4);
    x (0) = tol;
    ...
    if (res < 0)
        opserr <<
            "CTestRelativeTotalNormDispIncr::sendSelf() - failed to send data\n";
...
int
CTestRelativeTotalNormDispIncr::recvSelf (int cTag, Channel & theChannel,
