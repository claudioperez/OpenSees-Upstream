...
CTestNormDispIncr::CTestNormDispIncr():ConvergenceTest(CONVERGENCE_TEST_CTestNormDispIncr),
theSOE(0), tol(0), maxTol(OPS_MAXTOL), maxNumIter(0), currentIter(0),
printFlag(0), norms(25), nType(2)
{

}


CTestNormDispIncr::CTestNormDispIncr(double theTol, int maxIter,
                                     int printIt, int normType,
                                     double
                                     max):ConvergenceTest
    (CONVERGENCE_TEST_CTestNormDispIncr), theSOE(0), tol(theTol),
maxTol(max), maxNumIter(maxIter), currentIter(0), printFlag(printIt),
...
ConvergenceTest *CTestNormDispIncr::getCopy(int iterations)
{
    ...
    theCopy =
        new CTestNormDispIncr(this->tol, iterations, 0, this->nType,
                              this->maxTol);
...
int CTestNormDispIncr::test(void)
{
    ...
    if (currentIter == 0) {
        opserr <<
            "WARNING: CTestNormDispIncr::test() - start() was never invoked.\n";
        return -2;
    }
    ...
    if (norm <= tol) {
        ...
        if (printFlag != 0) {
            if (printFlag == 1 || printFlag == 4)
                ...
                opserr << "CTestNormDispIncr::test() - iteration: " <<
                    currentIter;
                opserr << " current Norm: " << norm << " (max: " << tol;
                opserr << ", Norm deltaR: " << theSOE->
                    getB().pNorm(nType) << ")\n";
    ...
    // algo failed to converged after specified number of iterations - but RETURN OK
        ...
        opserr <<
            "WARNING: CTestNormDispIncr::test() - failed to converge but going on - ";
        opserr << " current Norm: " << norm << " (max: " << tol;
        opserr << ", Norm deltaR: " << theSOE->
            getB().pNorm(nType) << ")\n";
        return currentIter;
...
int CTestNormDispIncr::start(void)
{
    if (theSOE == 0) {
        opserr <<
            "WARNING: CTestNormDispIncr::test() - no SOE returning true\n";
        return -1;
    }
