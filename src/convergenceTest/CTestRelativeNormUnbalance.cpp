...
CTestRelativeNormUnbalance::CTestRelativeNormUnbalance():ConvergenceTest(CONVERGENCE_TEST_CTestRelativeNormUnbalance),
...
CTestRelativeNormUnbalance::CTestRelativeNormUnbalance(double theTol,
                                                       int maxIter,
                                                       int printIt,
                                                       int
                                                       normType):ConvergenceTest
    (CONVERGENCE_TEST_CTestRelativeNormUnbalance), theSOE(0), tol(theTol),
...
ConvergenceTest *CTestRelativeNormUnbalance::getCopy(int iterations)
{
    ...
    theCopy =
        new CTestRelativeNormUnbalance(this->tol, iterations,
...
int CTestRelativeNormUnbalance::test(void)
{
    ...
    if (theSOE == 0) {
        opserr <<
            "WARNING: CTestRelativeNormUnbalance::test() - no SOE set.\n";
        return -1;
    }
    ...
    if (currentIter == 0) {
        opserr <<
            "WARNING: CTestRelativeNormUnbalance::test() - start() was never invoked.\n";
        return -2;
    }
    ...
    if (printFlag == 1) {
        opserr << "CTestRelativeNormUnbalance::test() - iteration: " <<
            currentIter;
        opserr << " current Ratio (|dR|/|dR0|): " << norm << " (max: " <<
            tol << ")\n";
    }
    if (printFlag == 4) {
        opserr << "CTestRelativeNormUnbalance::test() - iteration: " <<
        ...
        opserr << " current Ratio (|dR|/|dR0|): " << norm << " (max: " <<
            tol << ")\n";
        opserr << "\tNorm deltaX: " << theSOE->
            getX().pNorm(nType) << ", Norm deltaR: " << norm << endln;
    ...
    if (norm <= tol) {          // the algorithm converged
        ...
        if (printFlag != 0) {
            if (printFlag == 1 || printFlag == 4)
                ...
                opserr <<
                    "CTestRelativeNormUnbalance::test() - iteration: "
                    << currentIter;
                opserr << " current Ratio (|dR|/|dR0|): " << norm <<
                    " (max: " << tol << ")\n";
            }
        }
    ...
    // algo failed to converged after specified number of iterations - but RETURN OK
        ...
        opserr <<
            "WARNING: CTestRelativeNormUnbalance::test() - failed to converge but going on -";
...
int CTestRelativeNormUnbalance::start(void)
{
    if (theSOE == 0) {
        opserr <<
            "WARNING: CTestRelativeNormUnbalance::test() - no SOE returning true\n";
        return -1;
    }
...
int CTestRelativeNormUnbalance::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (res < 0)
        opserr <<
            "CTestRelativeNormUnbalance::sendSelf() - failed to send data\n";
