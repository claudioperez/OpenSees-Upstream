...
HystereticEnergy::HystereticEnergy(int tag, double Etot,
                                   double Cpow):DamageModel(tag,
                                                            DMG_TAG_HystereticEnergy),
Etotal(Etot), Cpower(Cpow)
{
    if (Etot <= 0 || Cpow <= 0)
        opserr <<
            "DamageModel::DamageModel : Incorrect arguments for the damage model";

    this->revertToStart();
}

HystereticEnergy::HystereticEnergy():DamageModel(0,
            DMG_TAG_HystereticEnergy)
{
    // Does nothing
}
...
{
    double TDisp, TForce, TKunload, TEnrgTot, TEnrgc, TExcurDmg,
        TCyclicDmg;
    double CDisp, CForce, CKunload, CEnrgTot, CEnrgc, CExcurDmg,
        CCyclicDmg;
    ...
    if (trialVector.Size() < 3) {
        opserr <<
            "WARNING: HystereticEnergy::setTrial Wrong vector size for trial data"
            << endln;
        return -1;
    }
    ...
    if (TKunload < 0.0) {
        opserr <<
            "WARNING: HystereticEnergy::setTrial negative unloading stiffness specified"
            << endln;
        return -1;
    }
    ...
    TExcurDmg =
        pow((TEnrgc - RSE) / ((Etotal - RSE) - (TEnrgTot - RSE)), Cpower);
...
int HystereticEnergy::setTrial()
{
    opserr << "WARNING: HystereticEnergy::setTrial Wrong Method called" <<
        endln;
    opserr << "HystereticEnergy Model uses vector based setTrial method" <<
        endln;
    return -1;
}


double HystereticEnergy::getDamage(void)
{
    TrialInfo[7] =
        CommitInfo[6] + TrialInfo[5] - TrialInfo[5] * CommitInfo[6];
    if (TrialInfo[7] < CommitInfo[7])
        TrialInfo[7] = CommitInfo[7];
    return TrialInfo[7];
}
...
int HystereticEnergy::commitState(void)
{
    for (int i = 0; i < 8; i++) {
...
int HystereticEnergy::revertToLastCommit(void)
{
    for (int i = 0; i < 8; i++) {
...
int HystereticEnergy::revertToStart(void)
{
    for (int i = 0; i < 8; i++) {
        TrialInfo[i] = 0.0;
        CommitInfo[i] = 0.0;
...
DamageModel *HystereticEnergy::getCopy(void)
{
    HystereticEnergy *theCopy =
        new HystereticEnergy(this->getTag(), Etotal, Cpower);
...
Response *HystereticEnergy::setResponse(const char **argv, int argc,
                                        OPS_Stream & info)
{
//
...
int HystereticEnergy::getResponse(int responseID, Information & info)
{
    switch (responseID) {
    case -1:
        return -1;
...
int HystereticEnergy::sendSelf(int commitTag, Channel & theChannel)
{
    return -1;
}


int HystereticEnergy::recvSelf(int commitTag, Channel & theChannel,
...
void HystereticEnergy::Print(OPS_Stream & s, int flag)
{
...
int HystereticEnergy::setInputResponse(Element * elem, const char **argv,
                                       int argc, int ndof)
{
    return -1;
}
