...
int
TclModelBuilderDamageModelCommand(ClientData clientData,
                                  ...
                                  TCL_Char ** argv)
{
    ...
    if (argc < 3) {
        opserr <<
            "WARNING insufficient number of damage model arguments\n";
    ...
    if (strcmp(argv[1], "Mehanny") == 0) {
        if (argc != 8 && argc != 10 && argc != 12) {
            ...
            opserr <<
                "Want: damageModel Mehanny tag? Alpha? Beta? Gamma? ultimatePosDisp? ultimateNegDisp? AbsTol? RelTol?"
                << endln;
            return TCL_ERROR;
        }

        int tag;
        double alpha, beta, gamma, ultimatePosDisp, ultimateNegDisp,
            abstol, reltol, posmodifier, negmodifier;
        ...
        if (argc > 8) {
            ...
            if (argc == 12) {
                ...
                if (Tcl_GetDouble(interp, argv[10], &posmodifier) !=
                    TCL_OK) {
                ...
                if (Tcl_GetDouble(interp, argv[11], &negmodifier) !=
                    TCL_OK) {
        ...
        theDamage =
            new Mehanny(tag, alpha, beta, gamma, ultimatePosDisp,
                        ultimateNegDisp, abstol, reltol, posmodifier,
                        negmodifier);
    }
    ...
    // Kratzig damage model
        ...
        if (argc != 7 && argc != 8) {
            ...
            printCommand(argc, argv);
            opserr <<
                "Want: damageModel Kratzig tag? Alpha? Beta? <Gamma?> ultimatePosDisp? ultimateNegDisp?"
                << endln;
            return TCL_ERROR;
        }
    ...
    // NormalizedPeak damage model
        ...
        if (argc != 6) {
            ...
            printCommand(argc, argv);
            opserr <<
                "Want: damageModel NormalizedPeak tag? maxValue? minValue? response?"
                << endln;
            return TCL_ERROR;
        }
    ...
    // HystereticEnergy damage model
        ...
        if (argc != 5) {
            ...
            printCommand(argc, argv);
            opserr <<
                "Want: damageModel HystereticEnergy tag?  Etotal? Cpower?"
                << endln;
            return TCL_ERROR;
        }
    ...
    // ParkAng damage model
        ...
        if (argc != 6) {
            ...
            printCommand(argc, argv);
            opserr <<
                "Want: damageModel ParkAng tag?  deltaU?  beta?  sigmaY?"
                << endln;
            return TCL_ERROR;
        }
    ...
    // end of NormalizedPeak damage model
        ...
        opserr << "WARNING the damage model specified does not exist " <<
            argv[1] << endln;
        return TCL_ERROR;
    }
    ...
    if (theDamage == 0) {
        opserr << "WARNING could not create DamageModel, out of memory " <<
            argv[1] << endln;
        return TCL_ERROR;
    }
