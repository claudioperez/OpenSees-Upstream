...
NormalizedPeak::NormalizedPeak (int tag, double maxVal, double minVal,
                                const char *argv):
DamageModel (tag, DMG_TAG_NormalizedPeak),
damagetype (NotSpecified),
MaxValue (maxVal),
MinValue (minVal),
TrialDmg (0.0),
CommitDmg (0.0),
    ...
    switch (damagetype)
      ...
      case PlasticDefo:
          if (TrialVector (2) != 0.0)
            {
                TrialScalar =
                    TrialVector (0) - TrialVector (1) / TrialVector (2);
            }
          else
            {
                TrialScalar = TrialVector (0);
...
double
NormalizedPeak::getPosDamage (void)
{
    return TrialDmg;
}
...
double
NormalizedPeak::getNegDamage (void)
{
    return TrialDmg;
}
