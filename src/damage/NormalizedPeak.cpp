...
NormalizedPeak::NormalizedPeak(int tag, double maxVal, double minVal,
                               const char *argv):DamageModel(tag,
                                                             DMG_TAG_NormalizedPeak),
damagetype(NotSpecified), MaxValue(maxVal), MinValue(minVal),
TrialDmg(0.0), CommitDmg(0.0), LCommitDmg(0.0), TrialVector(3),
CommitVector(3), LCommitVector(3)
{
    if (MaxValue < 0.0 || MinValue > 0.0 || argv == NULL) {
        opserr <<
            "NormalizedPeak::NormalizedPeak : Incorrect arguments for the damage model";
        exit(-1);
    }
    ...
    if (strcmp(damagename, "force") == 0
        || strcmp(damagename, "Force") == 0) {
        damagetype = Force;
    } else if (strcmp(damagename, "strain") == 0
               || strcmp(damagename, "Strain") == 0
               || strcmp(damagename, "defo") == 0
               || strcmp(damagename, "deformation") == 0
               || strcmp(damagename, "Deformation") == 0) {
        damagetype = Deformation;
    } else if (strcmp(damagename, "plasticDefo") == 0
               || strcmp(damagename, "PlasticDefo") == 0
               || strcmp(damagename, "plasticStrain") == 0
               || strcmp(damagename, "PlasticStrain") == 0
               || strcmp(damagename, "plasticDeformation") == 0
               || strcmp(damagename, "PlasticDeformation") == 0) {
        damagetype = PlasticDefo;
    } else if (strcmp(damagename, "energy") == 0
               || strcmp(damagename, "Energy") == 0
               || strcmp(damagename, "totalEnergy") == 0
               || strcmp(damagename, "TotalEnergy") == 0) {
        ...
        opserr <<
            "NormalizedPeak::NormalizedPeak : The damage type specified is not supported";
        exit(-1);
    }
...
NormalizedPeak::NormalizedPeak():DamageModel(0, DMG_TAG_NormalizedPeak)
{
    // Does nothing
}
...
DamageModel *NormalizedPeak::getCopy(void)
{
    NormalizedPeak *theCopy =
        new NormalizedPeak(this->getTag(), MaxValue, MinValue, damagename);
...
int NormalizedPeak::commitState(void)
{
...
int NormalizedPeak::revertToLastCommit(void)
{
...
int NormalizedPeak::revertToStart(void)
{
...
int NormalizedPeak::setTrial(const Vector & trialVector)
{
    if (trialVector.Size() < 3) {
        opserr <<
            "WARNING: NormalizedPeak::setTrial Wrong vector size for trial data"
            << endln;
        return -1;
    }
    ...
    case TotalEnergy:
        TrialScalar =
            CommitScalar + 0.5 * (TrialVector(1) +
                                  CommitVector(1)) * (TrialVector(0) -
                                                      CommitVector(0));
        break;
    case PlasticEnergy:
        if (TrialVector(2) > 0.0)
...
double NormalizedPeak::getDamage(void)
{
    return TrialDmg;
}
...
int NormalizedPeak::sendSelf(int commitTag, Channel & theChannel)
{
    return 0;
}


int NormalizedPeak::recvSelf(int commitTag, Channel & theChannel,
...
void NormalizedPeak::Print(OPS_Stream & s, int flag)
{
    s << "NormalizedPeak tag: " << this->getTag() << endln;
    s << "  MaximumValue: " << MaxValue << " MinimumValue: " << MinValue <<
        endln;
