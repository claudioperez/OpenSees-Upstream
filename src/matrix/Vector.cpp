...
double
    Vector::VECTOR_NOT_VALID_ENTRY = 0.0;

// Vector():
//      Standard constructor, sets size = 0;

Vector::Vector ():sz (0), theData (0), fromFree (0)
{
...
fromFree (0)
{
    ...
    if (sz < 0)
      {
          opserr << "Vector::Vector(int) - size " << size <<
              " specified < 0\n";
          sz = 1;
      }
#endif
    ...
    if (size > 0)
      {
          ...
          if (theData == 0)
            {
                opserr <<
                    "Vector::Vector(int) - out of memory creating vector of size "
                    << size << endln;
...
fromFree (1)
{
    ...
    if (sz <= 0)
      {
          opserr << "Vector::Vector(double *, size) - size " << size <<
              " specified <= 0\n";
          sz = 0;
      }
#endif
...
//  Move constructor
#ifdef USE_CXX11
Vector::Vector (Vector && other):sz (other.sz), theData (other.theData),
fromFree (0)
{
    //opserr << "move ctor!\n";
    other.theData = 0;
...
Vector::setData (double *newData, int size)
{
    ...
    if (sz <= 0)
      {
          opserr << " Vector::Vector(double *, size) - size specified: " <<
              size << " <= 0\n";
          sz = 0;
      }
...
Vector::resize (int newSize)
{
    ...
    if (newSize < 0)
      {
          opserr << "Vector::resize) - size specified " << newSize <<
              " <= 0\n";
          return -1;
      }
    ...
    // otherwise if newSize is gretaer than oldSize free old space and get new space
    else if (newSize > sz)
      {
          ...
          if (theData == 0)
            {
                opserr << "Vector::resize() - out of memory for size " <<
                    newSize << endln;
                sz = 0;
                return -2;
            }
    ...
    if (sz != other.sz)
      {
          ...
          opserr <<
              "WARNING Vector::addVector() - incompatable Vector sizes\n";
          return -1;
      }
#endif
          ...
          if (otherFact == 1.0)
            ...
            {                   // no point doing a multiplication if otherFact == 1.0
              ...
              for (int i = 0; i < sz; i++)
                {
                    double value =
                        *dataPtr * thisFact + *otherDataPtr++ * otherFact;
                    *dataPtr++ = value;
                }
      }
...
int
Vector::addMatrixVector (double thisFact, const Matrix & m, const Vector & v,
                         double otherFact)
{
    // see if quick return
    ...
    if ((sz != m.noRows ()) && (m.noRows () != v.sz))
      {
          // otherwise incompatable sizes
          opserr <<
              "Vector::addMatrixTransposeVector() - incompatable sizes\n";
          return -1;
      }
#endif
...
Vector::operator[] (int x)
{
    ...
    if (x < 0)
      {
       opserr << "Vector::() - x " << x << " outside range 0 - << " << sz -
       1 << endln; return VECTOR_NOT_VALID_ENTRY;}
#endif
       ...
       dataNew[i] = theData[i];
       for (int j = sz; j < x; j++)
       dataNew[j] = 0.0; if (fromFree == 0) if (theData != 0)
       {
       delete[]theData; theData = 0;}
       theData = dataNew; sz = x + 1;}
...
//      current vector located in positions given by the ID rows.
       ...
       int pos; for (int i = 0; i < rows.Size (); i++)
       {
       pos = rows (i); if (pos < 0 || pos >= sz)
       {
       opserr << "Vector::()(ID) - invalid location " << pos <<
...
//
       ...
       if (this->theData != 0)
       {
       delete[]this->theData; this->theData = 0;}
       this->sz = V.sz;
       ...
       //         copy the data
       for (int i = 0; i < sz; i++) theData[i] = V.theData[i];}
...
// Move assignment operator.  
       ...
       {
       delete[]this->theData; this->theData = 0;}
       theData = V.theData; this->sz = V.sz; V.theData = 0; V.sz = 0;}
       return *this;}
#endif
...
//      The += operator adds fact to each element of the vector, data[i] = data[i]+fact.
       ...
       {
       if (fact != 0.0)
       for (int i = 0; i < sz; i++) theData[i] += fact; return *this;}
...
//      Program exits if divide-by-zero would occur with warning message.
       ...
       for (int i = 0; i < sz; i++) theData[i] = VECTOR_VERY_LARGE_VALUE;}
       else
       {
       for (int i = 0; i < sz; i++) theData[i] /= fact;}
...
//      are return(i) = theData[i]+fact;
       ...
       if (result.Size () != sz)
       opserr <<
       "Vector::operator+(double) - ran out of memory for new Vector\n";
       result += fact; return result;}
...
//      vectors are of same size is performed if VECTOR_CHECK is defined.
       ...
       {
       opserr <<
       "WARNING Vector::operator+=(Vector):Vectors not of same sizes: " << sz
       << " != " << other.sz << endln; return *this;}
#endif

       for (int i = 0; i < sz; i++)
       theData[i] += other.theData[i]; return *this;}
...
//      Then returns a Vector whose components are the vector sum of current and V's data.
       ...
       {
       opserr <<
       "Vector::operator-(Vector): new Vector not of correct size \n";
       return result;}
...
//      Method to perform (Vector)transposed * vector.
       ...
       for (int i = 0; i < sz; i++)
       result += *dataThis++ * *dataV++; return result;}
...
//      Method to return inv(M)*this
       ...
       {
       Vector res (M.noRows ()); if (M.noRows () != M.noCols ())
       {                        // if not square do least squares solution
       Matrix A (M ^ M); A.Solve (*this, res);}
       else
       {
       M.Solve (*this, res);}
       return res;}
...
//      Then returns 1 if all the components of the two vectors are equal and 0 otherwise.
       ...
       double *dataV = V.theData;
       for (int i = 0; i < sz; i++)
       if (*dataThis++ != *dataV++) return 0; return 1;}

       int Vector::operator== (double value) const
       {
       double *dataThis = theData;
       for (int i = 0; i < sz; i++)
       if (*dataThis++ != value) return 0; return 1;}
...
//      Then returns 1 if any of the components of the two vectors are unequal and 0 otherwise.
       ...
       double *dataV = V.theData;
       for (int i = 0; i < sz; i++)
       if (*dataThis++ != *dataV++) return 1; return 0;}

       int Vector::operator!= (double value) const
       {
       double *dataThis = theData;
       for (int i = 0; i < sz; i++)
       if (*dataThis++ != value) return 1; return 0;}
...
*/
       ...
       int Vector::Assemble (const Vector & V, int init_pos, double fact)
       {
       int res = 0;
       ...
       {
       for (int j = 0; j < V.sz; j++) (*this) (cur_pos++) += V (j) * fact;}
       else
       {
       opserr <<
       "WARNING: Vector::Assemble(const Vector &V, int init_pos, double fact): ";
       opserr << "position outside bounds \n"; res = -1;}

       return res;}

       int Vector::Extract (const Vector & V, int init_pos, double fact)
       {
       int res = 0;
       int cur_pos = init_pos;
       ...
       {
       for (int j = 0; j < sz; j++) (*this) (j) = V (cur_pos++) * fact;}
       else
       {
       opserr <<
       "WARNING: Vector::Assemble(const Vector &V, int init_pos, double fact): ";
       ...
       // else sizes are incompatable, do nothing but warning
       opserr <<
       "WARNING Vector::tensor multiplication operator % - incompatable Vector sizes\n";
       return -1;}
#endif
       ...
       for (int i = 0; i < sz; i++)
       for (int j = 0; j < sz; j++)
       result (i, j) = theData[i] * V.theData[j]; return result;}
