...
int
    ID::ID_NOT_VALID_ENTRY = 0;

// ID():
//      Standard constructor, sets size = 0;

ID::ID ():sz (0), data (0), arraySize (0), fromFree (0)
{
    ...
    if (size > 0)
      {
          ...
          if (data == 0)
            {
                opserr << "ID::ID(int): ran out of memory with size " << size
                    << endln;
                exit (-1);
            }
...
fromFree (0)
{
    ...
    if (sz < 0)
      {
          opserr << "ID::ID(size, arraySize) - size " << size <<
              " specified < 0\n";
          sz = 0;
      }
    if (arraySz <= 0)
      {
          opserr << "ID::ID(size, arraySize) - arraySize " << arraySz <<
              " specified < 0\n";
          if (sz != 0)
              arraySz = sz;
          else
    ...
    if (data == 0)
      {
          opserr << "ID::ID(int, int): ran out of memory with arraySize: " <<
              arraySize << endln;
          exit (-1);
      }
...
{
    if (d == 0)
      {                         // OOPS must have been other constructor we wanted
          sz = 0;
          data = 0;
          arraySize = size;
          fromFree = 0;
          ...
          if (arraySize != 0)
            {
                ...
                if (data == 0)
                  {
                      opserr <<
                          "ID::ID(int, int): ran out of memory with arraySize "
                          << arraySize << endln;
                      exit (-1);
                  }
            }
...
fromFree (0)
{
    ...
    if (data == 0)
      {
          opserr << "ID::ID(ID): ran out of memory with arraySize " <<
              arraySize << endln, exit (-1);
      }
...
ID::setData (int *newData, int size, bool cleanIt)
{
    ...
    if (sz <= 0)
      {
          opserr << "ID::ID(int *, size) - size " << size <<
              " specified <= 0\n";
          sz = 0;
      }
...
ID::unique (void)
{
    ...
    for (std::list < int >::iterator pos = uniquesl.begin ();
         pos != uniquesl.end (); pos++)
        newdata[count++] = *pos;
...
ID::operator[] (int x)
{
    ...
    if (x < 0)
      {
       ...
       if (x < arraySize)
       {
       for (int i = sz; i < x; i++) data[i] = 0; sz = x + 1; return data[x];}

       // otherwise we go get more space
       if (x >= arraySize)
       {
       int newArraySize = arraySize * 2;
       if (newArraySize <= x) newArraySize = x + 1;
       //    int *newData = (int *)malloc(newArraySize*sizeof(int));    
       int *newData = new (nothrow) int[newArraySize]; if (newData != 0)
       {

       // copy the old
       for (int i = 0; i < sz; i++) newData[i] = data[i];
       // zero the new
       for (int j = sz; j < newArraySize; j++) newData[j] = 0; sz = x + 1;
       // release the memory held by the old
       //      free((void *)data);           
       if (fromFree == 0 && data != 0)
       delete[]data;
       data = newData; arraySize = newArraySize; return newData[x];}
       else
       {
       // we could not allocate more mem .. leave the current size
       opserr << "ID::[]): ran out of memory with arraySize " << arraySize <<
       endln; return ID_NOT_VALID_ENTRY;}
       }
       ...
       int ID::resize (int newSize, int fill_value)
       {
       ...
       for (int i = sz; i < newSize; i++) data[i] = fill_value; sz = newSize;}
       else
       if (newSize > arraySize)
       {
    ...
    // otherwise we go get more space
       ...
       int *newData = new (nothrow) int[newSize]; if (newData != 0)
       {
       // copy the old
       for (int i = 0; i < sz; i++) newData[i] = data[i];
       // zero the new
       for (int j = sz; j < newSize; j++)
       newData[j] = fill_value; sz = newSize;
       // release the memory held by the old
       //      free((void *)data);           
       delete[]data; data = newData; arraySize = newSize;}
       else
       {
       opserr << "ID::resize() - out of memory creating ID of size " <<
       newSize << "\n"; return -1;}
       }
       ...
       int ID::fill (int fill_value)
       {
       for (int i = 0; i < sz; i++) data[i] = fill_value; return 0;}
...
//      Then returns 1 if all the components of the two IDs are equal and 0 otherwise.
       ...
       int ID::operator== (const ID & V) const
       {
       if (sz != V.sz)
       return 0;
       int *dataThis = data;
...
*/
       ...
       int ID::insert (int x)
       {
       int middle = 0; int left = 0; int right = sz - 1; if (sz != 0)
       {
       while (left <= right)
       {
       middle = (left + right) / 2; double dataMiddle = data[middle]; if (x == dataMiddle) return 1;    // already there
       else
       if (x > dataMiddle) left = middle + 1;
       else
       right = middle - 1;}
       }
       ...
       int i = sz; while (i > middle)
       {
       data[i] = data[i - 1]; i--;}
       sz++; data[i] = x; return 0;}
       else
       {
       ...
       sz++;
       if (data != 0 && fromFree == 0)
       delete[]data; data = newData; arraySize = newArraySize; return 0;}
       }
       return -1;}
