...
int
    Matrix::sizeDoubleWork = MATRIX_WORK_AREA;
int
    Matrix::sizeIntWork = INT_WORK_AREA;
double
    Matrix::MATRIX_NOT_VALID_ENTRY = 0.0;
double *
    Matrix::matrixWork = 0;
int *
    Matrix::intWork = 0;
...
Matrix::Matrix ():numRows (0), numCols (0), dataSize (0), data (0),
fromFree (0)
{
    ...
    if (matrixWork == 0)
      {
          ...
          if (matrixWork == 0 || intWork == 0)
            {
                opserr <<
                    "WARNING: Matrix::Matrix() - out of memory creating work area's\n";
                exit (-1);
            }
      }
}
    ...
    if (dataSize > 0)
      {
          ...
          if (data == 0)
            {
                opserr <<
                    "WARNING:Matrix::Matrix(int,int): Ran out of memory on init ";
...
fromFree (1)
{
    ...
    if (row < 0)
      {
          opserr <<
              "WARNING: Matrix::Matrix(int,int): tried to init matrix with numRows: ";
          opserr << row << " <0\n";
          numRows = 0;
          numCols = 0;
          dataSize = 0;
    ...
    if (col < 0)
      {
          opserr <<
              "WARNING: Matrix::Matrix(int,int): tried to init matrix with numCols: ";
          opserr << col << " <0\n";
          numRows = 0;
          numCols = 0;
          dataSize = 0;
          ...
          if (data == 0)
            {
                ...
                opserr << "Ran out of memory on init of size " << dataSize <<
                    endln;
                numRows = 0;
                numCols = 0;
                dataSize = 0;
...
// Move ctor
#ifdef USE_CXX11
Matrix::Matrix (Matrix && other):numRows (other.numRows), numCols (other.numCols), dataSize (other.dataSize),
data (other.data), fromFree (other.fromFree)
{
    other.numRows = 0;
    other.numCols = 0;
    other.dataSize = 0;
...
Matrix::setData (double *theData, int row, int col)
{
    ...
    if (row < 0)
      {
          opserr <<
              "WARNING: Matrix::setSize(): tried to init matrix with numRows: ";
          opserr << row << " <0\n";
          numRows = 0;
          numCols = 0;
          dataSize = 0;
    ...
    if (col < 0)
      {
          opserr <<
              "WARNING: Matrix::setSize(): tried to init matrix with numCols: ";
          opserr << col << " <0\n";
          numRows = 0;
          numCols = 0;
          dataSize = 0;
...
Matrix::resize (int rows, int cols)
{
    ...
    if (newSize < 0)
      {
          opserr << "Matrix::resize) - rows " << rows << " or cols " << cols
              << " specified <= 0\n";
          return -1;
      }
    ...
    else if (newSize > dataSize)
      {
          ...
          if (data == 0)
            {
                opserr << "Matrix::resize(" << rows << "," << cols <<
                    ") - out of memory\n";
                numRows = 0;
                numCols = 0;
                dataSize = 0;
...
int
Matrix::Assemble (const Matrix & V, const ID & rows, const ID & cols,
                  double fact)
{
          ...
          for (int j = 0; j < rows.Size (); j++)
            {
                ...
                if ((pos_Cols >= 0) && (pos_Rows >= 0) && (pos_Rows < numRows)
                    && (pos_Cols < numCols) && (i < V.numCols)
                    && (j < V.numRows))
                    (*this) (pos_Rows, pos_Cols) += V (j, i) * fact;
                else
                  {
                      opserr <<
                          "WARNING: Matrix::Assemble(const Matrix &V, const ID &l): ";
                      opserr << " - position (" << pos_Rows << "," << pos_Cols
                          << ") outside bounds \n";
                      res = -1;
                  }
            }
...
Matrix::Solve (const Vector & b, Vector & x) const
{
    ...
    if (n != x.Size ())
      {
          opserr << "Matrix::Solve(b,x) - dimension of x, " << numRows <<
              "is not same as matrix " << x.Size () << endln;
          return -2;
      }
    ...
    if (n != b.Size ())
      {
          opserr << "Matrix::Solve(b,x) - dimension of x, " << numRows <<
              "is not same as matrix " << b.Size () << endln;
          return -2;
      }
#endif
    ...
    if (dataSize > sizeDoubleWork)
      {
          ...
          if (matrixWork == 0)
            {
                opserr <<
                    "WARNING: Matrix::Solve() - out of memory creating work area's\n";
                sizeDoubleWork = 0;
                return -3;
            }
      }
...
Matrix::Solve (const Matrix & b, Matrix & x) const
{
    ...
    if (n != x.numRows)
      {
          opserr << "Matrix::Solve(B,X) - #rows of X, " << x.
              numRows << " is not same as the matrices: " << numRows << endln;
          return -2;
      }

    if (n != b.numRows)
      {
          opserr << "Matrix::Solve(B,X) - #rows of B, " << b.
              numRows << " is not same as the matrices: " << numRows << endln;
          return -2;
      }
    ...
    if (x.numCols != b.numCols)
      {
          opserr << "Matrix::Solve(B,X) - #cols of B, " << b.
              numCols << " , is not same as that of X, b " << x.
              numCols << endln;
          return -3;
      }
#endif
...
Matrix::Invert (Matrix & theInverse) const
{
    ...
    if (n != theInverse.numRows)
      {
          opserr << "Matrix::Solve(B,X) - #rows of X, " << numRows <<
              ", is not same as matrix " << theInverse.numRows << endln;
          return -2;
      }
#endif
          ...
          if (factOther == 1.0)
          ...
          else
            {
                ...
                for (int i = 0; i < dataSize; i++)
                  {
                      double value =
                          *dataPtr * factThis + *otherDataPtr++ * factOther;
                      *dataPtr++ = value;
                  }
            }
      }
...
int
Matrix::addMatrixTranspose (double factThis, const Matrix & other,
                            double factOther)
{
    if (factThis == 1.0 && factOther == 0.0)
        return 0;
                ...
                for (int j = 0; j < numCols; j++)
                  {
                      for (int i = 0; i < numRows; i++)
                          *dataPtr++ +=
                              (other.data)[j + i * numCols] * factOther;
                  }
            }
      }
...
Matrix::addMatrixProduct (double thisFact,
                          const Matrix & B,
                          const Matrix & C, double otherFact)
{
    if (thisFact == 1.0 && otherFact == 0.0)
        return 0;
#ifdef _G3DEBUG
    if ((B.numRows != numRows) || (C.numCols != numCols)
        || (B.numCols != C.numRows))
      {
          opserr <<
              "Matrix::addMatrixProduct(): incompatable matrices, this\n";
          return -1;
      }
#endif
...
#ifdef _G3DEBUG
    if ((B.numCols != numRows) || (C.numCols != numCols)
        || (B.numRows != C.numRows))
      {
          opserr <<
              "Matrix::addMatrixProduct(): incompatable matrices, this\n";
...
Matrix::addMatrixTripleProduct (double thisFact,
                                const Matrix & T,
                                const Matrix & B, double otherFact)
{
    if (thisFact == 1.0 && otherFact == 0.0)
        return 0;
#ifdef _G3DEBUG
    if ((numCols != numRows) || (B.numCols != B.numRows)
        || (T.numCols != numRows) || (T.numRows != B.numCols))
      {
          opserr <<
              "Matrix::addMatrixTripleProduct() - incompatable matrices\n";
          return -1;
      }
#endif
                                ...
                                const Matrix & C, double otherFact)
{
    if (thisFact == 1.0 && otherFact == 0.0)
        return 0;
#ifdef _G3DEBUG
    if ((numRows != A.numRows) || (A.numCols != B.numRows)
        || (B.numCols != C.numRows) || (C.numCols != numCols))
      {
          opserr <<
              "Matrix::addMatrixTripleProduct() - incompatable matrices\n";
          return -1;
...
Matrix Matrix::operator ()(const ID & rows, const ID & cols)
     const
     {
         int
             nRows,
...
Matrix & Matrix::operator/= (double fact)
{
    ...
    if (fact != 0.0)
    ...
    else
      {
          ...
          opserr <<
              "WARNING:Matrix::operator/= - 0 factor specified all values in Matrix set to ";
...
Matrix Matrix::operator/ (double fact) const
{
    if (fact == 0.0)
      {
          opserr <<
              "Matrix::operator/(const double &fact): ERROR divide-by-zero\n";
          exit (0);
      }
    Matrix
    result (*this);
    ...
    for (int i = 0; i < nCols; i++)
      {
          double *
              aDataPtr = data;
          double *
              bStartColDataPtr = &(M.data[i * innerDim]);
          for (int j = 0; j < numCols; j++)
            {
                double *
                    bDataPtr = bStartColDataPtr;
                double
                    sum = 0.0;
                for (int k = 0; k < innerDim; k++)
                  {
...
Matrix & Matrix::operator+= (const Matrix & M)
{
    ...
    if (numRows != M.numRows || numCols != M.numCols)
      {
          opserr <<
              "Matrix::operator+=(const Matrix &M) - matrices incompatable\n";
          return *this;
      }
#endif
...
Matrix::Assemble (const Matrix & V, int init_row, int init_col, double fact)
{
    ...
    if ((init_row >= 0) && (final_row < numRows) && (init_col >= 0)
        && (final_col < numCols))
      {
      ...
      {
          opserr <<
              "WARNING: Matrix::Assemble(const Matrix &V, int init_row, int init_col, double fact): ";
...
int
Matrix::AssembleTranspose (const Matrix & V, int init_row, int init_col,
                           double fact)
{
    int pos_Rows, pos_Cols;
    int res = 0;
    ...
    if ((init_row >= 0) && (final_row < numRows) && (init_col >= 0)
      ...
      {
          opserr <<
              "WARNING: Matrix::AssembleTranspose(const Matrix &V, int init_row, int init_col, double fact): ";
          opserr << "position outside bounds \n";
          res = -1;
      }
...
int
Matrix::AssembleTranspose (const Vector & V, int init_row, int init_col,
                           double fact)
{
    int pos_Rows, pos_Cols;
    int res = 0;
...
Matrix::Extract (const Matrix & V, int init_row, int init_col, double fact)
{
    ...
    if ((init_row >= 0) && (final_row < VnumRows) && (init_col >= 0)
        && (final_col < VnumCols))
      {
          for (int i = 0; i < numCols; i++)
            {
      ...
      {
          opserr <<
              "WARNING: Matrix::Extract(const Matrix &V, int init_row, int init_col, double fact): ";
          opserr << "position outside bounds \n";
          res = -1;
      }
...
Matrix::diagonal () const
{
    ...
    if (numRows != numCols)
      {
          opserr << "Matrix::diagonal() - Matrix is not square numRows = " <<
              numRows << " numCols = " << numCols <<
              " returning truncated diagonal." << endln;
      }
