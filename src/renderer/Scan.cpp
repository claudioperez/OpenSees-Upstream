...
ScanLineConverter::ScanLineConverter ():zBuffer (0), sizeBuffer (0)
{
    scanMode = WINDING;
    fillMode = 1;
}
...
//  float R,G,B,r1,g1,b1,r2,g2,b2,ka,kd,r,g,b,Iar,Iag,Iab,DotLN,N;
    ...
    FOR_EACH_PAIR (Point1, Point2, face.pointList,
                   {
                   y1 = (int) Point1->Y ();
                   y2 = (int) Point2->Y (); if (y1 > y2)
                   {
                   if ((y1 - 1) >= y2)
                   {            // only add if it will be drawn
                   ...
                   FaceEdges.AddSorted (*Edge, &Edge->Y_Top);
                   if (y2 > Ymax) Ymax = y2; if (y1 < Ymin) Ymin = y1;}
                   }
    );
    ...
    for (int ScanLine = Ymax; ScanLine >= Ymin; ScanLine--)
      {
          ...
          FOR_EACH_PAIR (Edge1, Edge2, ActiveEdges,
                         ...
                         slope =
                         slope + Edge1->edge_Slope () + Edge2->edge_Slope ();
                         if (xleft < xright)
                         {

                         cnt++;
                         if (((cnt % 2) == 1)
                             || (scanMode == WINDING && slope != 0))
                         {
                         ...
                         Zcur += dZ_dX;
                         Rcur += dR_dX; Gcur += dG_dX; Bcur += dB_dX;}
                         theDevice->ENDPOINT ();}
                         }

                         else
                         if (xleft == xright)
                         {      // crossing edges or a vertex
