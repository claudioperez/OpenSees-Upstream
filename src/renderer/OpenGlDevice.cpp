...
OpenGlDevice::OpenGlDevice ():winOpen (1), width (0), height (0),
windowTitle (0)
{
...
void
OpenGlDevice::WINOPEN (const char *_title, int _xLoc, int _yLoc, int _width,
                       int _height)
{
    if (windowTitle != 0)
        delete[]windowTitle;
    ...
    theWND =
        oglCreateWindow (windowTitle, _xLoc, _yLoc, _width, _height, &theHRC,
                         &theHDC);
    if (theWND == NULL)
        exit (1);
    winOpen = 0;
    ...
    /* create a window */
    swa.colormap =
        XCreateColormap (theDisplay, RootWindow (theDisplay, visual->screen),
                         visual->visual, AllocNone);
    ...
    theWindow =
        XCreateWindow (theDisplay, RootWindow (theDisplay, visual->screen),
                       _xLoc, _yLoc, _width, _height, 0, visual->depth,
                       InputOutput, visual->visual, mask, &swa);
    ...
    XSetStandardProperties (theDisplay, theWindow, windowTitle, windowTitle,
                            None, 0, 0, &hints);
    ...
    if (cx == 0)
      {
          cerr <<
              "OpenGlDevice::WINOPEN() - could not create a glx context\n";
          exit (-1);
      }
                        ...
                        char horizontalJustify, char verticalJustify)
{
    ...
    glCallLists (strlen (text), GL_UNSIGNED_BYTE, text);
#elif _AGL
    theDevice->drawText (x, y, z, text, length, horizontalJustify,
                         verticalJustify);
...
OpenGlDevice::saveImageAsBMP (const char *fileName)
{
    ...
    if (newFileName == 0)
      {
          cerr << "OpenGlDevice::saveImageAsBMP() failed to open file: " <<
              fileName << "\n";
          return -1;
      }
    ...
    if ((fp = fopen (newFileName, "wb")) == NULL)
      {
          cerr << "OpenGLDevice::saveBmpImage() - could not open file named"
              << newFileName << "\n";
          return -1;
      }
...
//    if (fwrite(&header, 1, sizeof(BITMAPFILEHEADER), fp) < sizeof(BITMAPFILEHEADER)) {
          cerr <<
              "OpenGLDevice::saveBmpImage() - failed to write BITMAPHEADER\n";
          fclose (fp);
          return -4;
      }
...
//    if (fwrite(&info, 1, sizeof(BITMAPINFOHEADER), fp) < sizeof(BITMAPINFOHEADER)) {
          cerr <<
              "OpenGLDevice::saveBmpImage() - failed to write BITMAPINFOHEADER\n";
          fclose (fp);
          return -5;
      }
...
OpenGlDevice::saveImageAsPNG (const char *fileName)
{
    ...
    if (newFileName == 0)
      {
          cerr << "OpenGlDevice::saveImageAsPNG() failed to open file: " <<
              fileName << "\n";
          delete[]image;
          return -1;
      }
    ...
    if (!png_ptr)
      {
          ...
          fclose (fp);
          cerr <<
              "OpenGlDevice::saveImageAsPNG() - out of memery creating write structure\n";
          return -1;
      }
    ...
    if (!info_ptr)
      {
          ...
          fclose (fp);
          cerr <<
              "OpenGlDevice::saveImageAsPNG() - out of memery creating info structure\n";
    ...
    if (row_pointers == 0)
      {
          ...
          fclose (fp);
          cerr <<
              "OpenGlDevice::failed to allocate memory for row pointers\n";
          return (-1);
      }
