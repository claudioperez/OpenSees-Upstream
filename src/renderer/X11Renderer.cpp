...
X11Renderer::X11Renderer (const char *title, int xLoc, int yLoc, int width,
                          int height, ColorMap & _theMap):
Renderer (_theMap),
theFileName (0)
{
    theView = new View;
    ...
    if (fileName != 0)
      {
          ...
          if (theFileName == 0)
            {
                opserr <<
                    "WARNING - X11Renderer::X11Renderer() - out of memory ccopying file name: "
                    << *fileName << endln;
                exit (-1);
            }
          else
            {
                ...
                if (!theFile)
                  {
                      opserr <<
                          "WARNING - X11Renderer::X11Renderer() - could not open file: "
                          << fileName << endln;;
                      aFile = 0;
                  }
                else
...
X11Renderer::startImage (void)
{
    ...
    if (aFile == 1)
      {
          theFile << "StartImage\n";
          theFile << "VRP " << theView->vrp[0] << " " << theView->
              vrp[1] << " " << theView->vrp[2] << " " << endln;
          theFile << "VPN " << theView->vpn[0] << " " << theView->
              vpn[1] << " " << theView->vpn[2] << " " << endln;
          theFile << "VUV " << theView->vuv[0] << " " << theView->
              vuv[1] << " " << theView->vuv[2] << " " << endln;
          theFile << "COP " << theProjection->cop[0] << " " << theProjection->
              cop[1] << " " << theProjection->cop[2] << " " << endln;
          theFile << "PROJECTIONMODE " << theProjection->
              projection_mode << endln;
          theFile << "VPWINDOW " << theProjection->
              vpwindow[0] << " " << theProjection->
              vpwindow[1] << " " << theProjection->
              vpwindow[2] << " " << theProjection->
              vpwindow[3] << " " << endln;
          theFile << "PLANES " << theProjection->
              planedist[0] << " " << theProjection->
              planedist[2] << " " << endln;
          theFile << "PORTWINDOW " << theViewport->
              portwindow[0] << " " << theViewport->
              portwindow[1] << " " << theViewport->
              portwindow[2] << " " << theViewport->
              portwindow[3] << " " << endln;
...
int
X11Renderer::drawPoint (const Vector & pos1, float V1, int tag, int mode,
                        int numPixels)
{
    float r, g, b;
    r = theMap->getRed (V1);
...
int
X11Renderer::drawPoint (const Vector & pos1, const Vector & rgb, int tag,
                        int mode, int numPixels)
{
    float r, g, b;
    r = rgb (0);
    g = rgb (1);
...
X11Renderer::drawLine (const Vector & pos1, const Vector & pos2,
                       float V1, float V2, int tag, int mode, int width,
                       int style)
{
...
int
X11Renderer::drawPolygon (const Matrix & pos, const Vector & data, int tag,
                          int mode)
{
#ifdef _G3DEBUG
    if (pos.noCols () != 3)
    ...
    if (pos.noRows () != data.Size ())
      {
          opserr <<
              "X11Renderer::drawPolygon - matrix & vector incompatable\n";
          return -1;
      }
#endif
...
int
X11Renderer::drawPolygon (const Matrix & pos, const Matrix & data, int tag,
                          int mode)
{
#ifdef _G3DEBUG
...
X11Renderer::setProjectionMode (const char *newMode)
{
    ...
    if ((strcmp (newMode, "parallel") == 0)
        || (strcmp (newMode, "Parallel") == 0))
        projectionMode = PARALLEL_MODE;
    else if ((strcmp (newMode, "perspective") == 0)
             || (strcmp (newMode, "Perspective") == 0))
...
X11Renderer::setFillMode (const char *newMode)
{
    ...
    if ((strcmp (newMode, "wire") == 0) || (strcmp (newMode, "Wire") == 0))
        fillMode = WIRE_MODE;
    else if ((strcmp (newMode, "fill") == 0)
             || (strcmp (newMode, "Fill") == 0))
        fillMode = FILL_MODE;
...
int
X11Renderer::setPortWindow (float left, float right, float bottom, float top)
{
