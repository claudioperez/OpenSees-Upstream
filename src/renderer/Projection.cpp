...
Projection::update (void)
{
    ...
    if (projection_mode == PARALLEL_MODE)
      ...
      {                         // perspective projection
          ...
          if (dopN > 0)
              dopN = -dopN;     // WARNING MESSAGE// WARNING MESSAGE
          ...
          TMat.Set (a, 0.0, 0.0, 0.0, 0.0, b, 0.0, 0.0, a * shU, b * shV, e * c, -c, -a * g, -b * h, e * c * dopN + f, -c * dopN);      // dopN = -cop[2]!

      }
    return 0;
}

FACE & Projection::transform (FACE & input)
{
    MYPOINT *
        point;
        ...
        if (projection_mode == PERSPECTIVE_MODE)
          {
              ...
              if (wVal == 0)
                {
                    opserr <<
                        "Some Point in local coord sys at same level as eye\n";
                    return input;
                }
              else
                {               // we will homo x and y coord, but leave z as was befor we came in
    ...
    if (projection_mode == PERSPECTIVE_MODE)
      {
          ...
          if (wVal == 0)
            {                   // point at eye position ignore delete it
                delete input;
                opserr << "HELP\n";
                return 0;
            }
