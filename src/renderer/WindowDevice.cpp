...
int
WindowDevice::numWindowDevice (0);

#ifdef _UNIX
Display *
    WindowDevice::theDisplay;
Colormap
    WindowDevice::cmap;
int
    WindowDevice::theScreen;
unsigned long
    WindowDevice::pixels[X11_MAX_COLORS];
XColor
    WindowDevice::colors[X11_MAX_COLORS];
int
    WindowDevice::colorFlag;
unsigned long
WindowDevice::foreground (0);
unsigned long
WindowDevice::background (0);
 ...
 */
int
oglSetPixelFormat (HDC hDC, BYTE type, DWORD flags)
{
    int pf;
 ...
 */
HWND
oglCreateWindow (char *title, int x, int y, int width, int height,
                 HGLRC * hRC, HDC * hDC)
{
    WNDCLASS wc;
    HWND hWnd;
 ...
 */
int
oglDestroyWindow (char *title, HWND hWnd, HGLRC hRC, HDC hDC)
{
    HINSTANCE hInstance;
 ...
 */
int
oglCreateBitmap (int width, int height, HGLRC * hRC, HDC * hDC,
...
//      void **theBitsPtr = &theBits;
    ...
    *theBitmap =
        CreateDIBSection (*hDC, info, DIB_RGB_COLORS, &theBits, NULL, 0);
 ...
 */
int
oglDestroyBitmap (HBITMAP * theBitmap, HGLRC hRC, HDC hDC)
{
    /*
     * now release the device context, destroy the rendering context
...
WindowDevice::WindowDevice ():winOpen (1), height (0), width (0), numPoints (0),
drawingPolygon (0)
{
#ifdef _UNIX
    hints.x = 50;
...
void
WindowDevice::WINOPEN (const char *_title, int _xLoc, int _yLoc, int _width,
                       int _height)
{
    ...
    if (background == 0)
      {
          if (XMatchVisualInfo
              (theDisplay, theScreen, depth, PseudoColor, &visual) == 0)
            {
    ...
    XSetStandardProperties (theDisplay, theWindow, title, title, None, 0, 0,
                            &hints);
          ...
          if (numWindowDevice == 1)
            {
                ...
                if (XMatchVisualInfo
                  ...
                  {
                      opserr <<
                          "WindowDevice::WINOPEN have created our own colormap, \n";
                      opserr <<
                          "windows may change color as move mouse from one window to\n";
                      opserr <<
                          "another - depends on your video card to use another colormap\n\n";
                ...
                if (cmap == 0)
                  {
                      opserr <<
                          "WindowDevice::initX11() - could not get a new color table\n";
                      exit (-1);
                  }
                ...
                if (depth < 8)
                  {
                      opserr <<
                          "WindowDevice::initX11() - needed at least 8 planes\n";
                      exit (-1);
                  }
                if (fail == false)
                  {
                            ...
                            for (int green = 0; green < 8; green++)
                              {
                                  for (int blue = 0; blue < 4; blue++)
                                    {
                                        pixels[32 * red + 4 * green + blue] =
                                            cnt;
                                        colors[cnt].pixel =
                                            pixels[32 * red + 4 * green +
                                                   blue];
    ...
    theWND =
        oglCreateWindow (title, xLoc, yLoc, width, height, &theHRC, &theHDC);
    if (theWND == NULL)
        exit (1);
    winOpen = 0;
...
WindowDevice::C3F (float r, float g, float b)
{
    ...
    if (colorFlag == 0 || colorFlag == 3)
      {
          val =
              (((int) ((r * 7.0) + .5)) * 32 + ((int) ((g * 7.0) + .5)) * 4 +
               ((int) ((b * 3.0) + .5)));
      }
    else if (colorFlag == 1)
      {
          val =
              (((int) ((r * 7.0) + .5)) * 24 + ((int) ((g * 5.0) + .5)) * 4 +
               ((int) ((b * 3.0) + .5)));
      }
    else if (colorFlag == 2)
      {
    ...
    if (drawingPolygon)
      {
          if (numPoints == MAX_NUM_POINTS_FOR_POLYGON)
            {
                opserr << "ERROR: Maximum number of points has been exceeded"
                    << endln;
                return;
            }
...
#ifdef _UNIX
    y = height - y;
    XDrawString (theDisplay, theWindow, theGC, (int) x, (int) y, text,
                 length);
#else
...
WindowDevice::initWindow (void)
{
    ...
    else if (XAllocColorCells (theDisplay, cmap, false, NULL, 0, pixels, 192)
             != 0)
      {
    ...
    else if (XAllocColorCells (theDisplay, cmap, false, NULL, 0, pixels, 64)
             != 0)
      {
          colorFlag = 2;
          int cnt = 0;
