...
OpenGLRenderer::OpenGLRenderer (const char *_title, int _xLoc, int _yLoc,
                                int _width, int _height, ColorMap & _theMap):
...
portWindow (4)
{
    ...
    if (bitmapFileName != 0)
      {
          opserr <<
              "OpenGLRenderer:;OpenGlRenderer - feature to save image only to BMP removed\n";
      }
    theDevice->WINOPEN (_title, _xLoc, _yLoc, _width, _height);;
    ...
    if (outputFileName != 0)
      {
          ...
          if (theFile.bad ())
            {
                opserr <<
                    "WARNING - OpenGLRenderer::OpenGLRenderer() - could not open file: "
                    << outputFileName << endln;
                theOutputFileName = 0;
            }
          else
            {
                theFile << windowTitle << endln;
                theFile << 1.0 * xLoc << " " << 1.0 * yLoc << " " << 1.0 *
                    width << " " << 1.0 * height << endln;
            }
      }
...
OpenGLRenderer::startImage (void)
{
    ...
    float PRPu =
        u (0) * cop (0) + u (1) * cop (1) + u (2) * cop (2) + ViewMat (0, 3);
    float PRPv =
        v (0) * cop (0) + v (1) * cop (1) + v (2) * cop (2) + ViewMat (1, 3);
    float PRPn =
        n (0) * cop (0) + n (1) * cop (1) + n (2) * cop (2) + ViewMat (2, 3);
    ...
    if (projectionMode == PARALLEL_MODE)
      {
          ...
          ProjMat (0, 0) = 2.0 / diffU;
          ProjMat (0, 1) = 0.0;
          ProjMat (0, 2) = 2.0 * shU / diffU;
          ProjMat (0, 3) = -2 * midU / diffU, ProjMat (1, 0) = 0.0;
          ProjMat (1, 1) = 2 / diffV;
          ProjMat (1, 2) = 2 * shV / diffV;
	...
	******* so in the meantime use the following - NO SHEARING  **************/
          ...
          glMatrixMode (GL_MODELVIEW);
          glLoadIdentity ();
          gluLookAt (cop (0), cop (1), cop (2), vrp (0), vrp (1), vrp (2),
                     vuv (0), vuv (1), vuv (2));
    ...
    glViewport ((GLsizei) xMin, (GLsizei) yMin, (GLsizei) xDiff,
                (GLsizei) yDiff);
    ...
    if (theOutputFileName != 0)
      {
          ...
          theFile << "PLANES " << clippingPlanes[0] << " " <<
              clippingPlanes[1] << "\n";
          theFile << "PORTWINDOW " << portWindow (0) << " " << portWindow (1)
              << " " << portWindow (2) << " " << portWindow (3) << " " <<
              endln;
      }
...
int
OpenGLRenderer::drawPoint (const Vector & pos1, float V1, int tag, int mode,
                           int numPixels)
{
...
int
OpenGLRenderer::drawPoint (const Vector & pos1, const Vector & rgb, int tag,
...
int
OpenGLRenderer::drawPolygon (const Matrix & pos, const Vector & data, int tag,
                             int mode)
{
#ifdef _G3DEBUG
    if (pos.noCols () != 3)
    ...
    if (pos.noRows () != data.Size ())
      {
          opserr <<
              "OpenGLRenderer::drawPolygon - matrix & vector incompatable\n";
          return -1;
      }
#endif
