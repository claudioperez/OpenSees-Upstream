...
// vector (4th coordinate is 1).
{
    if (v[3] == 0)
      {
          opserr << "ERROR: w coordinate of vector is 0, cannot unhomogeneize"
              << endln;
          return 0;
      }
    v[0] = v[0] / v[3];
...
// Implements VECTOR * MATRIX multiplications
{
    VECTOR *
        V = new VECTOR;
    V[0] =
        v[0] * M.m[0][0] + v[1] * M.m[1][0] + v[2] * M.m[2][0] +
        v[3] * M.m[3][0];
    V[1] =
    ...
    V[2] =
        ...
        v[3] * M.m[3][2];
    V[3] =
        v[0] * M.m[0][3] + v[1] * M.m[1][3] + v[2] * M.m[2][3] +
        v[3] * M.m[3][3];
    return *V;
}
...
{
    float a, b, c, d;
    a = v[0] * M.m[0][0] + v[1] * M.m[0][1] + v[2] * M.m[0][2] +
        v[3] * M.m[0][3];
    b = v[0] * M.m[1][0] + v[1] * M.m[1][1] + v[2] * M.m[1][2] +
        v[3] * M.m[1][3];
    c = v[0] * M.m[2][0] + v[1] * M.m[2][1] + v[2] * M.m[2][2] +
        v[3] * M.m[2][3];
    d = v[0] * M.m[3][0] + v[1] * M.m[3][1] + v[2] * M.m[3][2] +
        v[3] * M.m[3][3];
    v[0] = a;
    v[1] = b;
    v[2] = c;
...
OPS_Stream & operator<< (OPS_Stream & os, VECTOR & v)
{
    os << "[ " << v[0] << ' ' << v[1] << ' ' << v[2] << ' ' << v[3] << " ]";
    return os;
}

OPS_Stream & operator<< (OPS_Stream & os, MATRIX & M)
{
    os << "{ " << M.m[0][0] << ' ' << M.m[0][1] << ' ' << M.
        m[0][2] << ' ' << M.m[0][3] << " }" << endln;
    os << "{ " << M.m[1][0] << ' ' << M.m[1][1] << ' ' << M.
        m[1][2] << ' ' << M.m[1][3] << " }" << endln;
    os << "{ " << M.m[2][0] << ' ' << M.m[2][1] << ' ' << M.
        m[2][2] << ' ' << M.m[2][3] << " }" << endln;
    os << "{ " << M.m[3][0] << ' ' << M.m[3][1] << ' ' << M.
        m[3][2] << ' ' << M.m[3][3] << " }" << endln;
    return os;
}
