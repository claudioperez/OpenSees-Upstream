...
int
X11Device::numX11Device (0);
Display *
    X11Device::theDisplay;
Colormap
    X11Device::cmap;
int
    X11Device::theScreen;
unsigned long
    X11Device::pixels[X11_MAX_COLORS];
XColor
    X11Device::colors[X11_MAX_COLORS];
int
    X11Device::colorFlag;


X11Device::X11Device ():winOpen (1)
{
    hints.x = 50;
    hints.y = 50;
    hints.width = 0;
...
X11Device::WINOPEN (int _width, int _height)
{
    ...
    XSetStandardProperties (theDisplay, theWindow, "g3", "g3", None, 0, 0,
                            &hints);
    ...
    if (colorFlag == 3)
      {
          ...
          if (cmap == 0)
            {
                opserr <<
                    "X11Device::initX11() - could not get a new color table\n";
                exit (-1);
            }
...
X11Device::initX11 (void)
{
    ...
    XVisualInfo vinfo;
    if (XMatchVisualInfo (theDisplay, theScreen, 8, PseudoColor, &vinfo) ==
        false)
    ...
    if (XAllocColorCells (theDisplay, cmap, false, NULL, 0, pixels, 256) != 0)
    ...
    else
      {
          ...
          opserr <<
              "X11Device::initX11() - could not any colors from the default\n";
          opserr <<
              "colormap - have to create our own for the app - windows will\n";
          opserr <<
              "windows will change color as move mouse from one window to another\n\n";
      }
}
