...
DataFileStream::
DataFileStream(int indent):OPS_Stream(OPS_STREAM_TAGS_DataFileStream),
fileOpen(0), fileName(0), indentSize(indent), sendSelfCount(0),
theChannels(0), numDataRows(0), mapping(0), maxCount(0), sizeColumns(0),
theColumns(0), theData(0), theRemoteData(0), doCSV(0), commonColumns(0)
{
    if (indentSize < 1)
        indentSize = 1;
...
DataFileStream::DataFileStream(const char *file, openMode mode, int indent,
                               int csv, bool closeWrite, int prec,
                               bool
                               scientific):OPS_Stream
    (OPS_STREAM_TAGS_DataFileStream), fileOpen(0), fileName(0),
indentSize(indent), sendSelfCount(0), theChannels(0), numDataRows(0),
mapping(0), maxCount(0), sizeColumns(0), theColumns(0), theData(0),
theRemoteData(0), doCSV(csv), closeOnWrite(closeWrite), commonColumns(0)
{
    thePrecision = prec;
 ...
 DataFileStream::setFile(const char *name, openMode mode)
{
    ...
    if (fileName == 0) {
        ...
        if (fileName == 0) {
            std::cerr <<
                "DataFileStream::setFile() - out of memory copying name: "
                << name << std::endl;
            return -1;
        }
        // copy the strings
...
int DataFileStream::open(void)
{
    ...
    if (fileName == 0) {
        std::cerr <<
            "DataFileStream::open(void) - no file name has been set\n";
        return -1;
    }
...
int DataFileStream::setPrecision(int prec)
{
    if (fileOpen == 0)
        this->open();
...
int DataFileStream::setFloatField(floatField field)
{
    if (fileOpen == 0)
        this->open();
...
int DataFileStream::tag(const char *tagName)
{
    return 0;
}

int DataFileStream::tag(const char *tagName, const char *value)
{
    return 0;
}


int DataFileStream::endTag()
{
    return 0;
}

int DataFileStream::attr(const char *name, int value)
{
    return 0;
}

int DataFileStream::attr(const char *name, double value)
{
    return 0;
}

int DataFileStream::attr(const char *name, const char *value)
{
    return 0;
}

int DataFileStream::write(Vector & data)
{
        ...
        if (i == 0) {
            ...
            if (numColumns != 0) {
                ...
                if (theChannels[i - 1]->recvVector(0, 0, *theV) < 0) {
                    opserr <<
                        "DataFileStream::write - failed to recv data\n";
                }
            }
        }
    }
...
OPS_Stream & DataFileStream::write(const char *s, int n)
{
    if (fileOpen == 0)
        this->open();
...
OPS_Stream & DataFileStream::write(const unsigned char *s, int n)
{
    if (fileOpen == 0)
        this->open();
...
OPS_Stream & DataFileStream::write(const signed char *s, int n)
{
    if (fileOpen == 0)
        this->open();
...
OPS_Stream & DataFileStream::write(const void *s, int n)
{
    if (fileOpen == 0)
        this->open();
...
OPS_Stream & DataFileStream::write(const double *s, int n)
{
    numDataRows++;
...
OPS_Stream & DataFileStream::operator<<(char c)
{
    if (fileOpen == 0)
        this->open();
...
OPS_Stream & DataFileStream::operator<<(unsigned char c)
{
    if (fileOpen == 0)
        this->open();
...
OPS_Stream & DataFileStream::operator<<(signed char c)
{
    if (fileOpen == 0)
        this->open();
...
OPS_Stream & DataFileStream::operator<<(const char *s)
{
    if (fileOpen == 0)
        this->open();
...
OPS_Stream & DataFileStream::operator<<(const unsigned char *s)
{
    if (fileOpen == 0)
        this->open();
...
OPS_Stream & DataFileStream::operator<<(const signed char *s)
{
    if (fileOpen == 0)
        this->open();
...
OPS_Stream & DataFileStream::operator<<(const void *p)
{
    if (fileOpen == 0)
        this->open();
...
OPS_Stream & DataFileStream::operator<<(int n)
{
    if (fileOpen == 0)
        this->open();
...
OPS_Stream & DataFileStream::operator<<(unsigned int n)
{
    if (fileOpen == 0)
        this->open();
...
OPS_Stream & DataFileStream::operator<<(long n)
{
    if (fileOpen == 0)
        this->open();
...
OPS_Stream & DataFileStream::operator<<(unsigned long n)
{
    if (fileOpen == 0)
        this->open();
...
OPS_Stream & DataFileStream::operator<<(short n)
{
    if (fileOpen == 0)
        this->open();
...
OPS_Stream & DataFileStream::operator<<(unsigned short n)
{
    if (fileOpen == 0)
        this->open();
...
OPS_Stream & DataFileStream::operator<<(bool b)
{
    if (fileOpen == 0)
        this->open();
...
OPS_Stream & DataFileStream::operator<<(double n)
{
    if (fileOpen == 0)
        this->open();
...
OPS_Stream & DataFileStream::operator<<(float n)
{
    if (fileOpen == 0)
        this->open();
...
int DataFileStream::sendSelf(int commitTag, Channel & theChannel)
{
    sendSelfCount++;
    ...
    if (fileNameLength != 0) {
        ...
        if (theChannel.sendMsg(0, commitTag, theMessage) < 0) {
            opserr <<
                "DataFileStream::sendSelf() - failed to send message\n";
            return -1;
        }
    }
...
int DataFileStream::recvSelf(int commitTag, Channel & theChannel,
                             FEM_ObjectBroker & theBroker)
{
    ...
    if (fileNameLength != 0) {
        ...
        if (theChannel.recvMsg(0, commitTag, theMessage) < 0) {
            opserr <<
                "DataFileStream::recvSelf() - failed to recv message\n";
            return -1;
        }
...
void DataFileStream::indent(void)
{
    if (fileOpen != 0)
...
int DataFileStream::setOrder(const ID & orderData)
{
    if (sendSelfCount == 0)
        return 0;
    ...
    if (sendSelfCount > 0) {
        ...
        for (int i = 0; i < sendSelfCount; i++) {
            ...
            if (theChannels[i]->recvID(0, 0, numColumnID) < 0) {
                opserr <<
                    "DataFileStream::setOrder - failed to recv column size for process: "
                    << i + 1 << endln;
                return -1;
            }
            ...
            if (numColumns != 0) {
                ...
                if (theChannels[i]->recvID(0, 0, *theColumns[i + 1]) < 0) {
                    opserr <<
                        "DataFileStream::setOrder - failed to recv column data for process: "
                        << i + 1 << endln;
                    return -1;
                }
