...
DataOutputFileHandler::DataOutputFileHandler(const char *theFileName,
                                             echoMode theEMode,
                                             openMode
                                             theOMode):DataOutputHandler
    (DATAHANDLER_TAGS_DataOutputFileHandler), fileName(0),
theEchoMode(theEMode), theOpenMode(theOMode), numColumns(-1)
{
    if (theFileName != 0) {
        fileName = new char[strlen(theFileName) + 1];
        if (fileName == 0) {
            opserr <<
                "DataOutputFileHandler::DataOutputFileHandler() - out of memory\n";
        } else
    ...
    if (fileName != 0 && outputFile.setFile(fileName, theOpenMode) < 0) {
        opserr <<
            "DataOutputFileHandler::DataOutputFileHandler() - setFile() failed\n";
        if (fileName != 0) {
            delete[]fileName;
            fileName = 0;
        }
 ...
 DataOutputFileHandler::open(char **dataDescription, int numData)
{
    if (fileName == 0) {
        opserr <<
            "DataOutputFileHandler::open() - no filename passed in constructor\n";
        return -1;
    }

    if (dataDescription == 0) {
        opserr <<
            "DataOutputFileHandler::open() - no column description data passed\n";
        return -1;
    }

    if (numData < 0) {
        opserr << "DataOutputFileHandler::open() - numColumns (" << numData
            << ") < 0\n";
        return -1;
    } else
        numColumns = numData;
    ...
    if (theEchoMode == DATA_FILE) {
        ...
        if (xmlFileName == 0) {
            opserr <<
                "DataOutputFileHandler::open - out of memory creating copy of string "
                << xmlFileName << endln;
            return -1;
        }
        ...
        if (fileNameLength > 4
            && strcmp(".out", &name[fileNameLength - 4]) == 0) {
        ...
        if (xmlFile.setFile(xmlFileName, OVERWRITE) == 0) {
            ...
            xmlFile << "\t<DataFile>\n";
            xmlFile << "\t\t<DataFileName> " << name <<
                "</DataFileName>\n";
            xmlFile << "\t\t<NumberDataColumns> " << numData <<
                "</NumberDataColumns>\n";
            xmlFile << "\t</DataFile>\n";
            for (int i = 0; i < numData; i++) {
                ...
                xmlFile << "\t\t<ColumnLocation> " << i +
                    1 << "</ColumnLocation>\n";
                xmlFile << "\t\t<Description> " << dataDescription[i] <<
                    "</Description>\n";
                xmlFile << "\t</DataColumnDescription>\n";
            }
            xmlFile << "</NumericalFileDataDescription>\n";
            ...
            opserr <<
                "DataOutputFileHandler::open - failed to open cml file: "
                << xmlFileName << endln;
            delete[]xmlFileName;
            res = -1;
        }
...
int DataOutputFileHandler::write(Vector & data)
{
    if (fileName == 0 || numColumns < 0) {
        opserr <<
            "DataOutputFileHandler::write() - no filename or data description has been set\n";
        return -1;
    }
...
int DataOutputFileHandler::sendSelf(int commitTag, Channel & theChannel)
{
    static ID idData(3);
    ...
    if (theChannel.sendID(0, commitTag, idData) < 0) {
        opserr <<
            "DataOutputFileHandler::sendSelf() - failed to send id data\n";
        return -1;
    }
    ...
    if (fileNameLength != 0) {
        ...
        if (theChannel.sendMsg(0, commitTag, theMessage) < 0) {
            opserr <<
                "DataOutputFileHandler::sendSelf() - failed to send message\n";
            return -1;
        }
    }
...
int DataOutputFileHandler::recvSelf(int commitTag, Channel & theChannel,
                                    FEM_ObjectBroker & theBroker)
{
    ...
    if (theChannel.recvID(0, commitTag, idData) < 0) {
        opserr <<
            "DataOutputFileHandler::recvSelf() - failed to recv id data\n";
        return -1;
    }
    ...
    if (fileNameLength != 0) {
        ...
        if (fileName == 0) {
            opserr <<
                "DataOutputFileHandler::recvSelf() - out of memory\n";
            return -1;
        }
        ...
        if (theChannel.recvMsg(0, commitTag, theMessage) < 0) {
            opserr <<
                "DataOutputFileHandler::recvSelf() - failed to recv message\n";
            return -1;
        }
