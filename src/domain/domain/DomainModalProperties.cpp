                  ...
                  while ((nodePtr = theNodes ()) != 0)
                    {
                        ...
                        // map
                        pos.emplace (std::
                                     make_pair (nodePtr->getTag (), counter));
...
DomainModalProperties::compute (Domain * domain)
{
  ...
  for (auto & iV:V)
    ...
    auto addM =
        [&M, &ML, &MLfree, &aux_MD, &aux_ML, &aux_MC, &aux_C,
            ...
            for (int j = 0; j < x.noRows (); ++j)
              {
                  ...
                  for (int i = 0; i < x.noCols (); ++i)
                      out << DMP_OUT_FLOAT (scale *
                                            cleanFloat (x (j, i), tol));
                  out << "\n";
              }
        };
        auto print_svec =
            [&out] (const std::vector < std::string > &x, const char *fchar =
                    DMP_OUT_BLANK)
        {
            out << fchar;
            for (int i = 0; i < x.size (); ++i)
...
void
DomainModalProperties::print (const std::string & file_name)
{
