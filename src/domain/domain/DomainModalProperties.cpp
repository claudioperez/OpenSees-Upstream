...
namespace {
    ...
    struct triplet_t {
        ...
        double val;
         triplet_t() = default;
         triplet_t(int r, int c, double v):i(r), j(c), val(v) {
    ...
    int domainSize(Domain * domain) {
        ...
        if ((ndm != 2) && (ndm != 3))
            DMP_ERR
                ("DomainModalProperties can be calculated only when NDM is 2 or 3, not "
                 << ndm << "\n");
        return ndm;
    }
         ...
         node_map_t(Domain * domain, int ndm, int ndf) {
            ...
            if (n > 0) {
                ...
                while ((nodePtr = theNodes()) != 0) {
                    ...
                    // map
                    pos.emplace(std::make_pair
                                (nodePtr->getTag(), counter));
                    // id (original), needed to see if this dof is fixed
                    const ID & id_source =
                        nodePtr->getDOF_GroupPtr()->getID();
                    // id (remapped)
                    ...
                    for (int j = 0; j < node_ndf; ++j)
                        flags[static_cast < size_t >(j)] =
                            id(j) == DOF_EXCLUDED ? -1 : j;
                    // go on
                    ++counter;
                }
            }
    ...
    struct ele_map_t {
        ...
        // for each element, save a list of flags. true if the dof is translational, false otherwise
         ...
         ele_map_t(Domain * domain, const node_map_t & nm) {
            size_t count =
                static_cast < size_t >(domain->getNumElements());
             ...
             count = 0;
            {
                ...
                while ((elePtr = theEles()) != 0) {
                    ...
                    for (int i = 0; i < ele_nodes.Size(); ++i) {
                        size_t pos = nm.getPosition(ele_nodes(i));
                        const ID & node_id = nm.node_ids[pos];
                        const std::vector < int >&node_uf =
                            nm.node_u_flags[pos];
                        if (static_cast <
                            int >(dof_count) + node_id.Size() > id.Size())
                            DMP_ERR
                                ("FE_Element::getID() Size < sum(size(mapped node IDs))");
                        for (int j = 0; j < node_id.Size(); ++j) {
                            id(dof_count) = node_id(j);
                            uf[dof_count] =
                                node_uf[static_cast < size_t >(j)];
                    ...
                    for (size_t i = 0; i < ele_nodes.Size(); ++i) {
                        ...
                        opserr << " ) {";
                        for (size_t j = 0;
                             j < static_cast < size_t >(node_id.Size());
                             ++j)
    ...
    while (loc < numArgs) {
        ...
        if (strcmp(iarg, "-unorm") == 0) {
            ...
            if (loc < numArgs - 1) {
                ...
                opserr <<
                    "Error in modalProperties <-print> <-file $fileName> <-unorm>.\n"
...
DomainModalProperties::DomainModalProperties(bool unorm):m_unorm(unorm)
{
}

bool
 DomainModalProperties::compute(Domain * domain)
{
    /*
       Notes:
  ...
  for (auto & iV:V) {
    ...
    auto addM =[&M, &ML, &MLfree, &aux_MD, &aux_ML, &aux_MC, &aux_C,
                &aux_Lumped] (const Matrix & iM, const ID & iD,
    ...
    auto addV =[&V, num_eigen] (const Matrix & iV, const ID & iD) {
        int n = iD.Size();
        if (iV.noRows() != n
            || iV.noCols() != static_cast < int >(V.size()))
  ...
  for (const auto & it:M.triplets) {
    ...
    auto compute_extra_rotary_mass =
        [&nodemap, num_nodes, ndf, this] (Matrix & iML) {
        for (int i = 0; i < num_nodes; ++i) {
            Node *node = nodemap.nodes[static_cast < size_t >(i)];
            const ID & ids = nodemap.node_ids[static_cast < size_t >(i)];
            const Vector & pos = node->getCrds();
      ...
      for (const auto & it:M.triplets)
        ...
        for (int i = 0; i < ndf; ++i) {
            ...
            for (int inode = 0; inode < num_nodes; ++inode) {
                ...
                if (i >= ndm) {
                    const Vector & pos =
                        nodemap.nodes[static_cast <
                                      size_t >(inode)]->getCrds();
                    double dx = pos(0) - m_center_of_mass(0);
                    double dy = pos(1) - m_center_of_mass(1);
                    if (ndf == 3) {     // 2D case
                        if (i == 2) {
    ...
    for (int j = 0; j < ndf; ++j) {
        ...
        double inv_tot_mass =
            tot_mass == 0.0 ? DMP_DBL_LARGE : 1.0 / tot_mass;
        double accum_1 = 0.0;
        double accum_2 = 0.0;
...
#define DMP_OUT_REL_TOL 1.0e-12
    ...
    template < class TStream >
        void print_internal(TStream & out,
                            const DomainModalProperties & dmp) {
        // utils
        auto print_vec =[&out] (const Vector & x, const char *fchar =
                                DMP_OUT_BLANK) {
        ...
        auto print_mat =[&out] (const Matrix & x, double scale =
                                1.0, const char *fchar = DMP_OUT_BLANK) {
            double tol = getTolerance(x);
        ...
        auto print_svec =[&out] (const std::vector < std::string > &x,
                                 const char *fchar = DMP_OUT_BLANK) {
            out << fchar;
            for (int i = 0; i < x.size(); ++i)
                out << DMP_OUT_GEN(x[i]);
            out << "\n";
        ...
        // labels
        static std::vector < std::string > lab_freq =
            { "MODE", "LAMBDA", "OMEGA", "FREQUENCY", "PERIOD" };
        static std::vector < std::string > lab_mass_2d =
            { "MX", "MY", "RMZ" };
        static std::vector < std::string > lab_mass_3d =
            { "MX", "MY", "MZ", "RMX", "RMY", "RMZ" };
        static std::vector < std::string > lab_efmass_2d =
            { "MODE", "MX", "MY", "RMZ" };
        static std::vector < std::string > lab_efmass_3d =
            { "MODE", "MX", "MY", "MZ", "RMX", "RMY", "RMZ" };
        static std::vector < std::string > lab_pos_2d = { "X", "Y" };
        static std::vector < std::string > lab_pos_3d = { "X", "Y", "Z" };
        static std::vector < std::string > lab_sep_2 =
            { DMP_OUT_HLINE, DMP_OUT_HLINE };
        static std::vector < std::string > lab_sep_3 =
            { DMP_OUT_HLINE, DMP_OUT_HLINE, DMP_OUT_HLINE };
        static std::vector < std::string > lab_sep_4 =
            { DMP_OUT_HLINE, DMP_OUT_HLINE, DMP_OUT_HLINE, DMP_OUT_HLINE };
        static std::vector < std::string > lab_sep_5 =
            { DMP_OUT_HLINE, DMP_OUT_HLINE, DMP_OUT_HLINE, DMP_OUT_HLINE,
            DMP_OUT_HLINE
        };
        static std::vector < std::string > lab_sep_6 =
            { DMP_OUT_HLINE, DMP_OUT_HLINE, DMP_OUT_HLINE, DMP_OUT_HLINE,
            DMP_OUT_HLINE, DMP_OUT_HLINE
        };
        static std::vector < std::string > lab_sep_7 =
            { DMP_OUT_HLINE, DMP_OUT_HLINE, DMP_OUT_HLINE, DMP_OUT_HLINE,
            DMP_OUT_HLINE, DMP_OUT_HLINE, DMP_OUT_HLINE
        };
        ...
        for (int i = 0; i < dmp.eigenvalues().Size(); ++i) {
            ...
            out << DMP_OUT_BLANK << DMP_OUT_GEN(i + 1)
                ...
                << DMP_OUT_FLOAT(omega)
                << DMP_OUT_FLOAT(freq) << DMP_OUT_FLOAT(period) << "\n";
        }
        out << "\n\n";
        ...
        print_svec(ndm == 2 ? lab_efmass_2d : lab_efmass_3d,
                   DMP_OUT_COMMENT);
