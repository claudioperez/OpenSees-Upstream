...
PartitionedDomain::PartitionedDomain (int numNodes, int numElements,
                                      ...
                                      int numLoadPatterns, int numSubdomains,
                ...
                while ((theSub = theSubdomains2 ()) != 0
                       && retainedNodePtr == 0)
                  {
                      ...
                      if (hasRestrained == true)
                        {
                            retainedNodePtr =
                                theSub->getNode (retainedNodeTag);
...
int
PartitionedDomain::removeSP_Constraint (int nodeTag, int dof,
                                        int loadPatternTag)
{
          ...
          while ((theObject = theSubsIter ()) != 0)
            {
                Subdomain *theSub = (Subdomain *) theObject;
                result +=
                    theSub->removeSP_Constraint (nodeTag, dof,
                                                 loadPatternTag);
            }
      }
...
int
PartitionedDomain::setRayleighDampingFactors (double alphaM, double betaK,
          ...
          if (theSubdomains != 0)
            {
                ...
                while (((theObject = theSubsIter ()) != 0)
                  ...
                  {
                      Subdomain *theSub = (Subdomain *) theObject;
                      domainChangedAnySubdomain =
                          theSub->getDomainChangeFlag ();
                  }
            }
      }
          ...
          while ((theObject = theSubsIter ()) != 0)
            {
                Subdomain *theSub = (Subdomain *) theObject;
                res +=
                    theSub->eigenAnalysis (numModes, generalized,
                                           findSmallest);
                if (res != 0)
                    opserr << "PartitionedDomain::step - subdomain " <<
                        theSub->getTag () << " failed in step\n";
...
void
PartitionedDomain::Print (OPS_Stream & s, ID * nodeTags, ID * eleTags,
                          int flag)
{
    if (nodeTags != 0)
...
int
PartitionedDomain::partition (int numPartitions, bool usingMain,
                              int mainPartitionID, int specialElementTag)
{
    int result = 0;
                      ...
                      if (res != 0)
                        {
                            ...
                            opserr <<
                                " - failed to add Recorder to subdomain\n";
                            return res;
                        }
                  }
            }
                      ...
                      if (res != 0)
                        {
                            opserr << "PartitionedDomain::partition(void)";
                            opserr <<
                                " - failed to add Parameter to subdomain\n";
                            return res;
                        }
                  }
            }
...
int
PartitionedDomain::repartition (int numPartitions, bool usingMain,
                                int mainPartitionID, int specialElementTag)
{
...
Graph & PartitionedDomain::getSubdomainGraph (void)
{
    ...
    int
        numVertex = theSubdomains->getNumComponents () + 1;     // Use P0 as a vertex toooo
    ...
    int
        subDTag = 1;
    double
        myCostP0 = 0.0;         //this->getUpdateTime();

    Vertex *
        selfvertexPtr = new Vertex (subDTag, subDTag, myCostP0);
    ...
    while ((subDPtr = theSubdomains ()) != 0)
      {
          int
              subDTag = subDPtr->getTag ();

          //Vertex for current subD
          Vertex *
              vertexPtr = new Vertex (subDTag, subDTag, subDPtr->getCost ());
    ...
    while ((nodPtr = niter ()) != 0)
      {
          int
              nodeTag = nodPtr->getTag ();
          Vertex *
              vertexPtr = new Vertex (count++, nodeTag);
          vertexPtr->addEdge (1);
    ...
    while ((subDPtr = theSubdomains2 ()) != 0)
      {
          ...
          const
              ID &
              subdEdgeNodes = subDPtr->getExternalNodes ();
          int
              numberExternalNodes = subdEdgeNodes.Size ();
          for (int i = 0; i < numberExternalNodes; i++)
            {
                int
                    currentNodeTag = subdEdgeNodes (i);

                MAP_VERTEX_ITERATOR
                    mpvtx = nodeTagToVtx.find (currentNodeTag);
                if (mpvtx != nodeTagToVtx.end ())       // If we find current node Tag....
                  ...
                  {
                      Vertex *
                          newvtxptr = new Vertex (count++, currentNodeTag);
                      newvtxptr->addEdge (subdTag);
                      nodeTagToVtx.
                          insert (MAP_VERTEX_TYPE
    ...
    for (auto it = nodeTagToVtx.begin (); it != nodeTagToVtx.end (); ++it)
      {
          ...
          const
              ID &
              connectedSubdomains = vertexPtr->getAdjacency ();

          int
              numberOfConnectedSubdomains = connectedSubdomains.Size ();
          for (int i = 0; i < numberOfConnectedSubdomains; i++)
            {
                int
                    subdomainTag1 = connectedSubdomains (i);
                int
                    vertexTag1 = subDTagToVtxTag.find (subdomainTag1)->first;

                for (int j = 0; j < numberOfConnectedSubdomains; j++)
                    if (i != j)
                      {
                          int
                              subdomainTag2 = connectedSubdomains (j);
                          ...
                          if (vertexTag1 > vertexTag2)
                            {
                                mySubdomainGraph->addEdge (vertexTag1,
                                                           vertexTag2);
                                mySubdomainGraph->addEdge (vertexTag2,
                                                           vertexTag1);
                            }
                      }
            }
...
const Vector *
PartitionedDomain::getElementResponse (int eleTag, const char **argv,
                                       int argc)
{
...
TaggedObjectStorage *
PartitionedDomain::getElementsStorage ()
{
    return elements;
}
...
GraphPartitioner *
PartitionedDomain::getGraphPartitioner (void)
{
