...
typedef
 MAP_INT::value_type MAP_INT_TYPE;
typedef
 MAP_INT::iterator MAP_INT_ITERATOR;

typedef
 map < int, ID * >MAP_ID;
typedef
 MAP_ID::value_type MAP_ID_TYPE;
typedef
 MAP_ID::iterator MAP_ID_ITERATOR;

typedef
 map < int, Vertex * >MAP_VERTEX;
typedef
 MAP_VERTEX::value_type MAP_VERTEX_TYPE;
typedef
 MAP_VERTEX::iterator MAP_VERTEX_ITERATOR;

PartitionedDomain::PartitionedDomain():Domain(),
...
theSubdomainIter(0), mySubdomainGraph(0), has_sent_yet(false)
{
    ...
    if (theSubdomains == 0 || elements == 0 ||
        theSubdomainIter == 0 || theEleIter == 0 || mainEleIter == 0) {
...
PartitionedDomain::PartitionedDomain(DomainPartitioner & thePartitioner):Domain(),
...
PartitionedDomain::PartitionedDomain(int numNodes, int numElements,
                                     int numSPs, int numMPs,
                                     int numLoadPatterns,
                                     int numSubdomains,
                                     DomainPartitioner &
                                     thePartitioner):Domain(numNodes, 0,
                                                            numSPs, numMPs,
                                                            numLoadPatterns),
theSubdomains(0), theDomainPartitioner(&thePartitioner),
theSubdomainIter(0), mySubdomainGraph(0), has_sent_yet(false)
...
bool PartitionedDomain::addElement(Element * elePtr)
{
    ...
    for (int i = 0; i < nodes.Size(); i++) {
        ...
        if (nodePtr == 0) {
            opserr << "PartitionedDomain::addElement - In element " <<
                eleTag;
...
bool PartitionedDomain::addSP_Constraint(SP_Constraint * load)
{
    ...
    while ((theSub = theSubdomains()) != 0) {
        ...
        if (res == true) {
            ...
            if (ok == false) {
                opserr <<
                    "PartitiondDomain::addSP - failed to add to remote subdomain\n";
                load->Print(opserr);
                return ok;
            }
        }
    ...
    if (ok == false) {
        opserr <<
            "PartitionedDomain::addSP_Constraint - cannot add as node with tag"
...
int PartitionedDomain::addSP_Constraint(int axisDirn, double axisValue,
                                        const ID & fixityCodes, double tol)
{
    int numAdded = 0;

    numAdded =
        this->Domain::addSP_Constraint(axisDirn, axisValue, fixityCodes,
                                       tol);
    ...
    while ((theSub = theSubdomains()) != 0) {
        numAdded +=
            theSub->addSP_Constraint(axisDirn, axisValue, fixityCodes,
                                     tol);
    }
...
bool PartitionedDomain::addSP_Constraint(SP_Constraint * load, int pattern)
{
    int nodeTag = load->getNodeTag();
    bool ok = false;
...
bool PartitionedDomain::addMP_Constraint(MP_Constraint * load)
{
    bool res = false;
    ...
    if (constrainedNodePtr != 0) {
        if (retainedNodePtr != 0) {
            ...
            if (res == false) {
                opserr <<
                    "PartitionedDomain::addMP_Constraint - problems adding to main domain\n";
                return res;
            } else {
                addedMain = true;
            }
    ...
    while ((theSub = theSubdomains()) != 0) {
        ...
        if (hasConstrained == true) {
            ...
            if (hasRestrained == true) {
                ...
                if (res == false) {
                    opserr <<
                        "PartitionedDomain::addMP_Constraint - problems adding to subdomain with retained\n";
                    return res;
                } else {
                    ;
                }
    ...
    if (getRetained == true) {
        ...
        if (retainedNodePtr == 0) {
            ...
            while ((theSub = theSubdomains2()) != 0
                ...
                if (hasRestrained == true) {
                    ...
                    Matrix mass(retainedNodePtr->getNumberDOF(),
                                retainedNodePtr->getNumberDOF());
                    mass.Zero();
        ...
        if (retainedNodePtr == 0) {
            opserr <<
                "partitionedDomain::addMP_Constraint - can't find retained node anywhere!\n";
            return false;
        } else {
            ;
        }
        ...
        if (constrainedNodePtr != 0 && addedMain == false) {
            ...
            if (res == false) {
                opserr <<
                    "PartitionedDomain::addMP_Constraint - problems adding retained to main domain\n";
                return res;
            }
            res = this->Domain::addMP_Constraint(load);
            ...
            if (res == false) {
                opserr <<
                    "PartitionedDomain::addMP_Constraint - problems adding constraint to main domain after adding node\n";
                return res;
            }
        }
        //
        ...
        while ((theSub = theSubdomains3()) != 0) {
            ...
            if (hasConstrained == true) {
                ...
                if (hasRestrained == false) {
                    ...
                    if (res == false) {
                        opserr <<
                            "PartitionedDomain::addMP_Constraint - problems adding retained to subdomain\n";
                        return res;
                    }
                    ...
                    if (res == false) {
                        opserr <<
                            "PartitionedDomain::addMP_Constraint - problems adding constraint to subdomain after adding node\n";
                        return res;
                    }
                }
            }
...
bool PartitionedDomain::addLoadPattern(LoadPattern * loadPattern)
{
    bool result = true;
    ...
    if (this->getLoadPattern(tag) != 0) {
        opserr <<
            "PartitionedDomain::addLoadPattern - cannot add as LoadPattern with tag"
...
bool PartitionedDomain::addNodalLoad(NodalLoad * load, int pattern)
{
    int nodeTag = load->getNodeTag();
    ...
    opserr <<
        "PartitionedDomain::addNodalLoad - cannot add as node with tag" <<
        nodeTag << "does not exist in model\n";
    return false;
}


bool PartitionedDomain::addElementalLoad(ElementalLoad * load, int pattern)
{
    ...
    // if no subdomain .. node not in model
    opserr <<
        "PartitionedDomain::addElementalLoad - cannot add as element with tag"
...
Element *PartitionedDomain::removeElement(int tag)
{
...
Node *PartitionedDomain::removeNode(int tag)
{
...
SP_Constraint *PartitionedDomain::removeSP_Constraint(int tag)
{
    // we first see if its in the original domain
...
int PartitionedDomain::removeSP_Constraint(int nodeTag, int dof,
                                           int loadPatternTag)
{

    // we first see if its in the original domain
    int result =
        this->Domain::removeSP_Constraint(nodeTag, dof, loadPatternTag);
    if (!has_sent_yet) {
        return result;
    }
    // if not there we must check all the other subdomains
    if (theSubdomains != 0) {
        ...
        while ((theObject = theSubsIter()) != 0) {
            Subdomain *theSub = (Subdomain *) theObject;
            result +=
                theSub->removeSP_Constraint(nodeTag, dof, loadPatternTag);
        }
    }
...
MP_Constraint *PartitionedDomain::removeMP_Constraint(int tag)
{
    // we first see if its in the original domain
...
int PartitionedDomain::removeMP_Constraints(int tag)
{
    // we first see if its in the original domain
...
LoadPattern *PartitionedDomain::removeLoadPattern(int tag)
{
    // we first see if its in the original domain
...
ElementIter & PartitionedDomain::getElements()
{
    theEleIter->reset();
    return *theEleIter;
}
...
Element *PartitionedDomain::getElement(int tag)
{
    // we first see if its in the original domain
...
int PartitionedDomain::getNumElements(void) const
{
...
void PartitionedDomain::applyLoad(double timeStep)
{
...
void PartitionedDomain::setCommitTag(int newTag)
{
...
void PartitionedDomain::setCurrentTime(double newTime)
{
...
void PartitionedDomain::setCommittedTime(double newTime)
{
...
void PartitionedDomain::setLoadConstant(void)
{
...
int PartitionedDomain::setRayleighDampingFactors(double alphaM,
                                                 double betaK,
                                                 double betaK0,
                                                 double betaKc)
{
    ...
    if (theSubdomains != 0) {
        ...
        while ((theObject = theSubsIter()) != 0) {
            Subdomain *theSub = (Subdomain *) theObject;
            theSub->setRayleighDampingFactors(alphaM, betaK, betaK0,
                                              betaKc);
        }
    }
    return 0;
...
int PartitionedDomain::update(void)
{
...
#ifdef _PARALLEL_PROCESSING
int PartitionedDomain::barrierCheck(int res)
{
    int result = res;
...
int PartitionedDomain::update(double newTime, double dT)
{
...
int PartitionedDomain::hasDomainChanged(void)
{
    return this->Domain::hasDomainChanged();
}

int PartitionedDomain::analysisStep(double dT)
{
    ...
    if (domainChangedAnySubdomain == false) {
        ...
        if (theSubdomains != 0) {
            ...
            TaggedObject *theObject;
            while (((theObject = theSubsIter()) != 0)
                   && (domainChangedAnySubdomain == false)) {
                Subdomain *theSub = (Subdomain *) theObject;
    ...
    if (theSubdomains != 0) {
        ...
        while ((theObject = theSubsIter()) != 0) {
            ...
            if (res != 0)
                opserr << "PartitionedDomain::step - subdomain " <<
                    theSub->getTag() << " failed in step\n";
        }
    }
    return res;
}
...
int PartitionedDomain::eigenAnalysis(int numModes, bool generalized,
                                     bool findSmallest)
{
    // first we need to see if any subdomain has changed & mark the change in domain
    bool domainChangedAnySubdomain = this->getDomainChangeFlag();
    ...
    if (theSubdomains != 0) {
        ...
        while ((theObject = theSubsIter()) != 0) {
            Subdomain *theSub = (Subdomain *) theObject;
            res +=
                theSub->eigenAnalysis(numModes, generalized, findSmallest);
            if (res != 0)
                opserr << "PartitionedDomain::step - subdomain " <<
                    theSub->getTag() << " failed in step\n";
        }
...
int PartitionedDomain::record(bool fromAnalysis)
{
    int result = 0;
...
int PartitionedDomain::commit(void)
{
    ...
    if (result < 0) {
        opserr <<
            "PartitionedDomain::commit(void) - failed in Domain::commit()\n";
        return result;
    }
    // do the same for all the subdomains
    if (theSubdomains != 0) {
...
int PartitionedDomain::revertToLastCommit(void)
{
    int result = this->Domain::revertToLastCommit();
    if (result < 0) {
        opserr <<
            "PartitionedDomain::revertToLastCommit(void) - failed in Domain::revertToLastCommit()\n";
        return result;
    }
    // do the same for all the subdomains
    if (theSubdomains != 0) {
...
int PartitionedDomain::revertToStart(void)
{
...
int PartitionedDomain::addRecorder(Recorder & theRecorder)
{
...
int PartitionedDomain::removeRecorders(void)
{
    // do the same for all the subdomains
    if (theSubdomains != 0) {
        ArrayOfTaggedObjectsIter theSubsIter(*theSubdomains);
...
int PartitionedDomain::removeRecorder(int tag)
{
...
void PartitionedDomain::Print(OPS_Stream & s, int flag)
{
...
void PartitionedDomain::Print(OPS_Stream & s, ID * nodeTags, ID * eleTags,
                              int flag)
{
    if (nodeTags != 0)
...
int PartitionedDomain::setPartitioner(DomainPartitioner * thePartitioner)
{
...
int PartitionedDomain::partition(int numPartitions, bool usingMain,
                                 int mainPartitionID,
                                 int specialElementTag)
{
    int result = 0;
    ...
    if (thePartitioner != 0) {
        ...
        result =
            thePartitioner->partition(numPartitions, usingMain,
                                      mainPartitionID, specialElementTag);
    } else {
        opserr <<
            "PartitionedDomain::partition(int numPartitions) - no associated partitioner\n";
        return -1;
    }
...
int PartitionedDomain::repartition(int numPartitions, bool usingMain,
                                   int mainPartitionID,
                                   int specialElementTag)
{
...
bool PartitionedDomain::addSubdomain(Subdomain * theSubdomain)
{
...
int PartitionedDomain::getNumSubdomains(void)
{
    return theSubdomains->getNumComponents();
}

Subdomain *PartitionedDomain::getSubdomainPtr(int tag)
{
...
SubdomainIter & PartitionedDomain::getSubdomains(void)
{
...
DomainPartitioner *PartitionedDomain::getPartitioner(void) const
{
...
int PartitionedDomain::buildEleGraph(Graph * theEleGraph)
{
    // see if quick return
...
Node *PartitionedDomain::removeExternalNode(int tag)
{
    return (this->Domain::removeNode(tag));
}

Graph & PartitionedDomain::getSubdomainGraph(void)
{
    ...
    while ((subDPtr = theSubdomains()) != 0) {
        ...
        Vertex *vertexPtr =
            new Vertex(subDTag, subDTag, subDPtr->getCost());
    ...
    while ((subDPtr = theSubdomains2()) != 0) {
        ...
        for (int i = 0; i < numberExternalNodes; i++) {
            ...
            } else              //If not found
            {
                ...
                nodeTagToVtx.insert(MAP_VERTEX_TYPE
                                    (currentNodeTag, newvtxptr));
            }
        }
    }
        ...
        for (int i = 0; i < numberOfConnectedSubdomains; i++) {
            ...
            for (int j = 0; j < numberOfConnectedSubdomains; j++)
                if (i != j) {
                    ...
                    int vertexTag2 =
                        subDTagToVtxTag.find(subdomainTag2)->second;
...
double PartitionedDomain::getNodeDisp(int nodeTag, int dof, int &errorFlag)
{
...
int PartitionedDomain::setMass(const Matrix & mass, int nodeTag)
{
...
const Vector *PartitionedDomain::getNodeResponse(int nodeTag,
                                                 NodeResponseType response)
{
    ...
    if (theSubdomains != 0) {
        ...
        while ((theObject = theSubsIter()) != 0) {
            Subdomain *theSub = (Subdomain *) theObject;
            const Vector *result =
                theSub->getNodeResponse(nodeTag, response);
            if (result != 0)
                return result;
            // else
...
const Vector *PartitionedDomain::getElementResponse(int eleTag,
                                                    const char **argv,
                                                    int argc)
{

    const Vector *res =
        this->Domain::getElementResponse(eleTag, argv, argc);
    if (res != 0) {
        return res;
    }
    // do the same for all the subdomains
    if (theSubdomains != 0) {
        ...
        while ((theObject = theSubsIter()) != 0) {
            Subdomain *theSub = (Subdomain *) theObject;
            const Vector *result =
                theSub->getElementResponse(eleTag, argv, argc);
            if (result != 0) {
                return result;
            }
        }
...
int PartitionedDomain::calculateNodalReactions(bool inclInertia)
{
...
bool PartitionedDomain::addParameter(Parameter * param)
{
...
Parameter *PartitionedDomain::removeParameter(int tag)
{
...
int PartitionedDomain::updateParameter(int tag, int value)
{
    int res = 0;
...
int PartitionedDomain::updateParameter(int tag, double value)
{
...
int PartitionedDomain::activateElements(const ID & elementList)
{
    int res = 0;
