...
Element::Element(int tag, int cTag):DomainComponent(tag, cTag),
alphaM(0.0),
betaK(0.0),
betaK0(0.0),
betaKc(0.0),
Kc(0),
previousK(0),
numPreviousK(0), index(-1), nodeIndex(-1), is_this_element_active(true)
{
    // does nothing
...
int Element::addInertiaLoadToUnbalance(const Vector & accel)
{
    return -1;
}


int Element::setRayleighDampingFactors(double alpham, double betak,
                                       double betak0, double betakc)
{
    ...
    if (betaKc != 0.0) {
        ...
        if (Kc == 0) {
            opserr <<
                "WARNING - ELEMENT::setRayleighDampingFactors - out of memory\n";
            betaKc = 0.0;
        }
...
const Matrix & Element::getDamp(void)
{
    if (index == -1) {
...
const Matrix & Element::getMass(void)
{
    if (index == -1) {
        this->setRayleighDampingFactors(alphaM, betaK, betaK0, betaKc);
    }
...
const Vector & Element::getResistingForceIncInertia(void)
{
    if (index == -1) {
        this->setRayleighDampingFactors(alphaM, betaK, betaK0, betaKc);
    }
...
const Vector & Element::getRayleighDampingForces(void)
{
...
int Element::addLoad(ElementalLoad * theLoad, double loadFactor)
{
    return 0;
}

int Element::addLoad(ElementalLoad * theLoad, const Vector & loadFactors)
{
    return 0;
}
...
bool Element::isSubdomain(void)
{
    return false;
}

Response *Element::setResponse(const char **argv, int argc,
                               OPS_Stream & output)
{
    ...
    if (strcmp(argv[0], "force") == 0 || strcmp(argv[0], "forces") == 0 ||
        ...
        theResponse =
            new ElementResponse(this, 111111, this->getResistingForce());
    }
        ...
        theResponse =
            new ElementResponse(this, 222222, this->getResistingForce());
    }

    else if (strcmp(argv[0], "dynamicForce") == 0
             || strcmp(argv[0], "dynamicForces") == 0) {
        const Vector & force = this->getResistingForce();
        int size = force.Size();
        for (int i = 0; i < size; i++) {
            sprintf(nodeData, "P%d", i + 1);
        ...
        theResponse =
            new ElementResponse(this, 333333, this->getResistingForce());
    }

    else if (strcmp(argv[0], "inertialForce") == 0
             || strcmp(argv[0], "inertialForces") == 0) {
        const Vector & force = this->getResistingForce();
        int size = force.Size();
        for (int i = 0; i < size; i++) {
            sprintf(nodeData, "P%d", i + 1);
        ...
        theResponse =
            new ElementResponse(this, 444444, this->getResistingForce());
    }
...
int Element::getResponseSensitivity(int responseID, int gradIndex,
                                    Information & eleInfo)
{
    return -1;
}
...
const Vector & Element::getResistingForceSensitivity(int gradIndex)
{
    if (index == -1) {
        this->setRayleighDampingFactors(alphaM, betaK, betaK0, betaKc);
    }
...
const Matrix & Element::getTangentStiffSensitivity(int gradIndex)
{
    if (index == -1) {
        this->setRayleighDampingFactors(alphaM, betaK, betaK0, betaKc);
    }
    ...
    if (!warningShown) {
        opserr <<
            "Rayleigh damping with non-zero betaCurrentTangent is not implemented for DDM sensitivity analysis with this element"
            << endln;
        warningShown = true;
    }
...
const Matrix & Element::getInitialStiffSensitivity(int gradIndex)
{
    if (index == -1) {
        this->setRayleighDampingFactors(alphaM, betaK, betaK0, betaKc);
    }
    ...
    if (!warningShown) {
        opserr <<
            "Rayleigh damping with non-zero betaInitialTangent is not implemented for DDM sensitivity analysis with this element"
            << endln;
        warningShown = true;
    }
...
const Matrix & Element::getCommittedStiffSensitivity(int gradIndex)
{
    if (index == -1) {
        this->setRayleighDampingFactors(alphaM, betaK, betaK0, betaKc);
    }
    ...
    if (!warningShown) {
        opserr <<
            "Rayleigh damping with non-zero betaCommittedTangent is not implemented for DDM sensitivity analysis with this element"
            << endln;
        warningShown = true;
    }
...
const Matrix & Element::getMassSensitivity(int gradIndex)
{
    if (index == -1) {
        this->setRayleighDampingFactors(alphaM, betaK, betaK0, betaKc);
    }
...
int Element::commitSensitivity(int gradIndex, int numGrads)
{
    return -1;
}
...
const Matrix & Element::getDampSensitivity(int gradIndex)
{
    if (index == -1) {
        this->setRayleighDampingFactors(alphaM, betaK, betaK0, betaKc);
    }
    ...
    theMatrix->Zero();
    if (alphaM != 0.0) {
        theMatrix->addMatrix(0.0, this->getMassSensitivity(gradIndex),
                             alphaM);
    }
    if (betaK != 0.0) {
        theMatrix->addMatrix(1.0,
                             this->getTangentStiffSensitivity(gradIndex),
                             betaK);
    }
    if (betaK0 != 0.0) {
        theMatrix->addMatrix(1.0,
                             this->getInitialStiffSensitivity(gradIndex),
                             betaK0);
    }
    if (betaKc != 0.0) {
        theMatrix->addMatrix(1.0,
                             this->getCommittedStiffSensitivity(gradIndex),
                             betaKc);
    }
    // return the computed matrix
    return *theMatrix;
...
int Element::addResistingForceToNodalReaction(int flag)
{
    int result = 0;
    int numNodes = this->getNumExternalNodes();
    Node **theNodes = this->getNodePtrs();
...
int Element::storePreviousK(int numK)
{
...
const Matrix *Element::getPreviousK(int num)
{
    if (num < numPreviousK)
...
int Element::displaySelf(Renderer &, int mode, float fact,
...
const Matrix & Element::getGeometricTangentStiff()
{
    if (index == -1) {
        this->setRayleighDampingFactors(alphaM, betaK, betaK0, betaKc);
    }
...
void Element::onActivate()
{
    ...
    if (report_once) {
        opserr <<
            "onActivate not implemented for this element. classTag = " <<
...
void Element::onDeactivate()
{
    ...
    if (report_once) {
        opserr <<
            "onDeactivate not implemented for this element. classTag = " <<
            this->getClassTag() << endln;
        report_once = false;
    }
}
