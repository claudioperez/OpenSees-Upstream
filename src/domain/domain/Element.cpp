...
int
Element::setRayleighDampingFactors (double alpham, double betak,
    ...
    if (betaKc != 0.0)
      {
          theMatrix->addMatrix (1.0,
                                this->
                                getCommittedStiffSensitivity (gradIndex),
                                betaKc);
      }
...
double
Element::getCharacteristicLength (void)
{
    int numNodes = this->getNumExternalNodes ();
    Node **theNodes = this->getNodePtrs ();
    double cLength = 0.0;
...
int
Element::displaySelf (Renderer &, int mode, float fact,
...
Element::onActivate ()
{
    ...
    if (report_once)
      {
          opserr << "onActivate not implemented for this element. classTag = "
