...
Domain::Domain():theRecorders(0), numRecorders(0),
...
resetBounds(false), theBounds(6), theEigenvalues(0),
theEigenvalueSetTime(0), theModalProperties(0), theModalDampingFactors(0),
inclModalMatrix(false), lastChannel(0), paramIndex(0), paramSize(0),
numParameters(0)
{
...
Domain::Domain(int numNodes, int numElements, int numSPs, int numMPs,
               int numLoadPatterns):theRecorders(0),
numRecorders(0),
currentTime(0.0),
committedTime(0.0),
dT(0.0),
...
Domain::Domain(TaggedObjectStorage & theNodesStorage, TaggedObjectStorage & theElementsStorage, TaggedObjectStorage & theMPsStorage, TaggedObjectStorage & theSPsStorage, TaggedObjectStorage & theLoadPatternsStorage):theRecorders(0), numRecorders(0),
currentTime(0.0), committedTime(0.0), dT(0.0), currentGeoTag(0),
hasDomainChangedFlag(false), theDbTag(0), lastGeoSendTag(-1),
dbEle(0), dbNod(0), dbSPs(0), dbPCs(0), dbMPs(0), dbLPs(0), dbParam(0),
eleGraphBuiltFlag(false), nodeGraphBuiltFlag(false), theNodeGraph(0),
...
Domain::Domain(TaggedObjectStorage & theStorage):theRecorders(0), numRecorders(0),
currentTime(0.0), committedTime(0.0), dT(0.0), currentGeoTag(0),
hasDomainChangedFlag(false), theDbTag(0), lastGeoSendTag(-1),
dbEle(0), dbNod(0), dbSPs(0), dbPCs(0), dbMPs(0), dbLPs(0), dbParam(0),
eleGraphBuiltFlag(false), nodeGraphBuiltFlag(false), theNodeGraph(0),
 ...
 Domain::addElement(Element * element)
{
    ...
    if (other != 0) {
        opserr << "Domain::addElement - element with tag " << eleTag <<
...
bool Domain::addSP_Constraint(SP_Constraint * spConstraint)
{
    ...
    if (nodePtr == 0) {
        opserr <<
            "Domain::addSP_Constraint - cannot add as node node with tag"
    ...
    if (other != 0) {
        opserr <<
            "Domain::addSP_Constraint - cannot add as constraint with tag "
    ...
    if (result == false) {
        opserr <<
            "Domain::addSP_Constraint - cannot add constraint with tag" <<
...
bool Domain::addPressure_Constraint(Pressure_Constraint * pConstraint)
{
#ifdef _G3DEBUG
    // check the Node exists in the Domain
    ...
    if (nodePtr == 0) {
        opserr <<
            "Domain::addPressure_Constraint - cannot add as node with tag";
    ...
    if (other != 0) {
        opserr <<
            "Domain::addPressure_Constraint - cannot add as constraint with tag";
    ...
    if (result == false) {
        opserr <<
            "Domain::addPressure_Constraint - cannot add constraint with tag";
...
int Domain::addSP_Constraint(int axisDirn, double axisValue,
            ...
            if (fabs(nodeCrdDirn - axisValue) <= tol) {
                ...
                for (int i = 0; i < fixityConditions.Size(); i++) {
                    if ((i < numDOF) && (fixityConditions(i) == 1)) {
                        ...
                        // check if an existing SP_COnstraint exists for that dof at the node
                        bool found = false;
                        SP_ConstraintIter & theExistingSPs =
                            this->getSPs();
                        SP_Constraint *theExistingSP = 0;
                        while ((found == false)
                               &&
                               ((theExistingSP = theExistingSPs()) != 0)) {
                            int spNodeTag = theExistingSP->getNodeTag();
                        ...
                        if (found == false) {
                            ...
                            SP_Constraint *theSP =
                                new SP_Constraint(nodeTag, i,
                                                  0.0, true);

                            if (this->addSP_Constraint(theSP) == false) {
                                opserr <<
                                    "WARNING could not add SP_Constraint to domain for node "
                                    << theNode->getTag();
                                delete theSP;
                            } else {
                                numAddedSPs++;
...
bool Domain::addMP_Constraint(MP_Constraint * mpConstraint)
{
//#ifdef _G3DEBUG
    // perform the checks
    ...
    if (nodePtr == 0) {
        opserr <<
            "Domain::addMP_Constraint -cannot add as constrained node with tag"
    ...
    if (nodePtr == 0) {
        opserr <<
            "Domain::addMP_Constraint - cannot add as retained node with tag"
    ...
    if (other != 0) {
        opserr <<
            "Domain::addMP_Constraint - cannot add as constraint with tag"
            << tag << "already exists in model";
    ...
    if (result == true) {
        ...
        opserr <<
            "Domain::addMP_Constraint - cannot add constraint with tag" <<
            tag << "to the container\n";

    return result;
}

bool Domain::addLoadPattern(LoadPattern * load)
{
    ...
    if (other != 0) {
        opserr <<
            "Domain::addLoadPattern - cannot add as LoadPattern with tag"
            << tag << "already exists in model\n";
    ...
    if (result == true) {
        ...
        opserr <<
            "Domain::addLoadPattern - cannot add LoadPattern with tag" <<
            tag << "to the container\n";

    return result;
}

bool Domain::addParameter(Parameter * theParam)
{
    ...
    if (other != 0) {
        opserr << "Domain::addParameter - parameter with tag " << paramTag
            << "already exists in model\n";
        return false;
    }
    ...
    if (result == false) {
        opserr << "Domain::addParameter - parameter " << paramTag <<
            "could not be added to container\n";
        theParam->setDomain(this);
        return result;
    }
...
bool Domain::addSP_Constraint(SP_Constraint * spConstraint, int pattern)
{
#ifdef _G3DEBUG
    // check the Node exists in the Domain
    int nodeTag = spConstraint->getNodeTag();
    ...
    if (thePattern == 0) {
        opserr <<
            "Domain::addSP_Constraint - cannot add as pattern with tag" <<
    ...
    if (result == false) {
        opserr << "Domain::addSP_Constraint - " << pattern <<
...
bool Domain::addNodalLoad(NodalLoad * load, int pattern)
{
    ...
    if (thePattern == 0) {
        opserr << "Domain::addNodalLoad() - no pattern with tag " <<
            pattern << " in the model, not adding the nodal load " << *load
            << endln;
...
bool Domain::addElementalLoad(ElementalLoad * load, int pattern)
{
    // now add it to the pattern
    TaggedObject *thePattern = theLoadPatterns->getComponentPtr(pattern);
    if (thePattern == 0) {
        opserr << "Domain::addElementalLoad() - no pattern with tag " <<
            pattern << "exits in  the model, not adding the ele load " <<
            *load << endln;
...
Element *Domain::removeElement(int tag)
{
...
Node *Domain::removeNode(int tag)
{
    // remove the object from the container
...
int Domain::removeSP_Constraint(int theNode, int theDOF,
                                int loadPatternTag)
{
...
Pressure_Constraint *Domain::removePressure_Constraint(int tag)
{
    // remove the object from the container    
...
MP_Constraint *Domain::removeMP_Constraint(int tag)
{
    // remove the object from the container        
...
int Domain::removeMP_Constraints(int nodeTag)
{
    ID tagsToRemove(0);
    int sizeTags = 0;
...
Parameter *Domain::removeParameter(int tag)
{
    Parameter *theParam =
        (Parameter *) theParameters->getComponentPtr(tag);
...
LoadPattern *Domain::removeLoadPattern(int tag)
{
    // remove the object from the container            
...
NodalLoad *Domain::removeNodalLoad(int tag, int loadPattern)
{
    // remove the object from the container            
...
ElementalLoad *Domain::removeElementalLoad(int tag, int loadPattern)
{
    // remove the object from the container            
    LoadPattern *theLoadPattern = this->getLoadPattern(loadPattern);
...
SP_Constraint *Domain::removeSP_Constraint(int tag, int loadPattern)
{
    // remove the object from the container            
    LoadPattern *theLoadPattern = this->getLoadPattern(loadPattern);
...
Pressure_ConstraintIter & Domain::getPCs()
{
    thePC_Iter->reset();
    return *thePC_Iter;
}
...
ParameterIter & Domain::getParameters()
{
    theParamIter->reset();
    return *theParamIter;
}
...
Element *Domain::getElement(int tag)
{
...
SP_Constraint *Domain::getSP_Constraint(int tag)
{
...
Pressure_Constraint *Domain::getPressure_Constraint(int tag)
{
...
MP_Constraint *Domain::getMP_Constraint(int tag)
{
...
Parameter *Domain::getParameter(int tag)
{
...
Parameter *Domain::getParameterFromIndex(int index)
{
    if (index >= 0 && index < numParameters)
        return this->getParameter(paramIndex[index]);

    else {
        opserr << "Domain::getParameterFromIndex -- index " << index <<
            " out of bounds 0 ... " << numParameters - 1 << endln;
        return 0;
    }

}

int Domain::getParameterIndex(int tag)
{
    int index;
    ...
    if (index == numParameters) {
        opserr << "Domain::getParameterIndex -- parameter with tag " << tag
            << " not found" << endln;
        return -1;
    }
...
LoadPattern *Domain::getLoadPattern(int tag)
{
...
double Domain::getCurrentTime(void) const
{
    return currentTime;
}

int Domain::getCommitTag(void) const
{
    return commitTag;
}


int Domain::getNumElements(void) const
{
    return theElements->getNumComponents();
}

int Domain::getNumNodes(void) const
{
    return theNodes->getNumComponents();
}

int Domain::getNumSPs(void) const
{
    return theSPs->getNumComponents();
}

int Domain::getNumPCs(void) const
{
    return thePCs->getNumComponents();
}

int Domain::getNumMPs(void) const
{
    return theMPs->getNumComponents();
}

int Domain::getNumLoadPatterns(void) const
{
    return theLoadPatterns->getNumComponents();
}

int Domain::getNumParameters(void) const
{
    return theParameters->getNumComponents();
}

const Vector & Domain::getPhysicalBounds(void)
{
...
const Vector *Domain::getNodeResponse(int nodeTag,
...
const Vector *Domain::getElementResponse(int eleTag, const char **argv,
                                         int argc)
{
...
Graph & Domain::getElementGraph(void)
{
    if (eleGraphBuiltFlag == false) {
        ...
        theElementGraph =
            new Graph(this->getNumElements() + START_VERTEX_NUM);
        ...
        if (this->buildEleGraph(theElementGraph) == 0)
            ...
            opserr <<
                "Domain::getElementGraph() - failed to build the element graph\n";
    }
    // return the Graph
...
Graph & Domain::getNodeGraph(void)
{
    if (nodeGraphBuiltFlag == false) {
        ...
        if (this->buildNodeGraph(theNodeGraph) == 0)
            ...
            opserr <<
                "Domain::getNodeGraph() - failed to build the node graph\n";
    }
    // return the Graph
    return *theNodeGraph;
}

void Domain::clearElementGraph(void)
{
    if (theElementGraph != 0)
        delete theElementGraph;
    theElementGraph = 0;
...
void Domain::clearNodeGraph(void)
{
    if (theNodeGraph != 0)
        delete theNodeGraph;
    theNodeGraph = 0;
...
void Domain::setCommitTag(int newTag)
{
    commitTag = newTag;
}

void Domain::setCurrentTime(double newTime)
{
    currentTime = newTime;
    dT = currentTime - committedTime;
}

void Domain::setCommittedTime(double newTime)
{
    committedTime = newTime;
    dT = currentTime - committedTime;
}

void Domain::setCreep(int newCreep)
{
    ops_Creep = newCreep;
}

int Domain::getCreep(void) const
{
    return ops_Creep;
}

void Domain::applyLoad(double timeStep)
{
...
void Domain::setLoadConstant(void)
{
...
void Domain::unsetLoadConstant(void)
{
    // loop over all the load patterns that are currently added to the domain
    // getting them to set their loads as now constant
    LoadPattern *thePattern;
...
int Domain::setRayleighDampingFactors(double alphaM, double betaK,
                                      double betaK0, double betaKc)
{
    ...
    while ((elePtr = theElemIter()) != 0)
        result +=
            elePtr->setRayleighDampingFactors(alphaM, betaK, betaK0,
                                              betaKc);
...
int Domain::record(bool fromAnalysis)
{
    int res = 0;
...
int Domain::revertToLastCommit(void)
{
    // 
...
int Domain::update(double newTime, double dT)
{
...
int Domain::updateParameter(int tag, int value)
{
...
int Domain::updateParameter(int tag, double value)
{
    // remove the object from the container    
    TaggedObject *mc = theParameters->getComponentPtr(tag);
    ...
    if (mc == 0) {
        opserr <<
            "Domain::updateParameter(int tag, double value) - parameter with tag not present\n";
        return 0;
    }
...
int Domain::analysisStep(double dT)
{
    return 0;
}

int Domain::eigenAnalysis(int nuMode, bool generalized, bool findSmallest)
{
    return 0;
}
...
int Domain::setEigenvalues(const Vector & theValues)
{
...
const Vector & Domain::getEigenvalues(void)
{
...
double Domain::getTimeEigenvaluesSet(void)
{
...
const DomainModalProperties & Domain::getModalProperties(void) const
{
    if (theModalProperties == 0) {
        opserr <<
            "Domain::getModalProperties - DomainModalProperties were never set\n";
        exit(-1);
    }
    return *theModalProperties;
}

int Domain::setModalDampingFactors(Vector * theValues, bool inclMatrix)
{
    ...
    // make sure the eigen value vector is large enough
    if (theModalDampingFactors == 0
        || theModalDampingFactors->Size() != theValues->Size()) {
        if (theModalDampingFactors != 0)
            delete theModalDampingFactors;
...
const Vector *Domain::getModalDampingFactors(void)
{
    return theModalDampingFactors;
}

bool Domain::inclModalDampingMatrix(void)
{
    return inclModalMatrix;
}

void Domain::setDomainChangeStamp(int newStamp)
{
    currentGeoTag = newStamp;
}


void Domain::domainChange(void)
{
    hasDomainChangedFlag = true;
}


bool Domain::getDomainChangeFlag(void)
{
    return hasDomainChangedFlag;
}


int Domain::hasDomainChanged(void)
{
...
void Domain::Print(OPS_Stream & s, int flag)
{
    ...
    s << "\nLOAD PATTERNS: numPatterns: " <<
        theLoadPatterns->getNumComponents() << "\n\n";
    theLoadPatterns->Print(s, flag);

    s << "\nPARAMETERS: numParameters: " <<
        theParameters->getNumComponents() << "\n\n";
...
int Domain::addRecorder(Recorder & theRecorder)
{
    ...
    if (newRecorders == 0) {
        opserr <<
            "Domain::addRecorder() - could not add ran out of memory\n";
        return -1;
    }
...
int Domain::removeRecorders(void)
{
    for (int i = 0; i < numRecorders; i++)
        if (theRecorders[i] != 0)
            delete theRecorders[i];
...
int Domain::removeRecorder(int tag)
{
    for (int i = 0; i < numRecorders; i++) {
        if (theRecorders[i] != 0) {
...
int Domain::addRegion(MeshRegion & theRegion)
{
    MeshRegion **newRegions = new MeshRegion *[numRegions + 1];
    if (newRegions == 0) {
        opserr <<
            "Domain::addRegion() - could not add ran out of memory\n";
        return -1;
    }
...
MeshRegion *Domain::getRegion(int tag)
{
    for (int i = 0; i < numRegions; i++)
...
void Domain::getRegionTags(ID & rtags) const
{
...
int Domain::buildEleGraph(Graph * theEleGraph)
{
    // see if quick return
    ...
    while ((theEle = theElements()) != 0) {
        ...
        if (vertexPtr == 0) {
            opserr <<
                "WARNING Domain::buildEleGraph - Not Enough Memory to create the "
                << count << " vertex\n";
            return -1;
        }
        ...
        if (theEleToVertexMapEle == theEleToVertexMap.end()) {
            ...
            if (theEleToVertexMapEle == theEleToVertexMap.end()) {
                opserr <<
                    "Domain::buildEleGraph - map STL failed to add object with tag : "
                    << eleTag << endln;
                return false;
            }
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (theEleToVertexMapEle != theEleToVertexMap.end()) {
                ...
                for (int j = 0; j < size; j++)
                    if (i != j) {
                        int eleTag2 = (*id) (j);
                        theEleToVertexMapEle =
                            theEleToVertexMap.find(eleTag2);
                        if (theEleToVertexMapEle !=
                            theEleToVertexMap.end()) {
                            int vertexTag2 =
                                (*theEleToVertexMapEle).second;

                            // addEdge() adds for both vertices - do only once

                            if (vertexTag1 > vertexTag2) {
                                theEleGraph->addEdge(vertexTag1,
                                                     vertexTag2);
                                theEleGraph->addEdge(vertexTag2,
                                                     vertexTag1);
                            }
                        }
                    }
...
int Domain::buildNodeGraph(Graph * theNodeGraph)
{
...
int Domain::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (theChannel.sendVector(theDbTag, commitTag, domainTime) < 0) {
        opserr <<
            "Domain::send - channel failed to send the time Vector\n";
        return -2;
    }
    ...
    if (lastChannel != theChannel.getTag()
        ...
        if (numNod != 0) {
            ...
            if (theChannel.sendID(dbNod, currentGeoTag, nodeData) < 0) {
                opserr <<
                    "Domain::send - channel failed to send the node ID\n";
                return -2;
            }
        }
        ...
        if (numEle != 0) {
            ...
            if (theChannel.sendID(dbEle, currentGeoTag, elementData) < 0) {
                opserr <<
                    "Domain::send - channel failed to send the element ID\n";
                return -3;
            }
        }
        ...
        if (numSPs != 0) {
            ...
            if (theChannel.sendID(dbSPs, currentGeoTag, spData) < 0) {
                opserr <<
                    "Domain::send - channel failed to send the SP_Constraint ID\n";
                return -4;
            }
        }
        ...
        if (numPCs != 0) {
            ...
            if (theChannel.sendID(dbPCs, currentGeoTag, pData) < 0) {
                opserr <<
                    "Domain::send - channel failed to send the Pressure_Constraint ID\n";
                return -4;
            }
        }
        ...
        if (numMPs != 0) {
            ...
            if (theChannel.sendID(dbMPs, currentGeoTag, mpData) < 0) {
                opserr <<
                    "Domain::send - channel failed to send the MP_Constraint ID\n";
                return -5;
            }
        }
        ...
        if (numLPs != 0) {
            ...
            if (theChannel.sendID(dbLPs, currentGeoTag, lpData) < 0) {
                opserr <<
                    "Domain::send - channel failed to send the LoadPattern ID\n";
                return -6;
            }
        }
    ...
    while ((theNode = theNodes()) != 0) {
        if (theNode->sendSelf(commitTag, theChannel) < 0) {
            opserr << "Domain::send - node with tag " << theNode->getTag()
                << " failed in sendSelf\n";
            return -7;
        }
    }
    ...
    while ((theEle = theElements()) != 0) {
        if (theEle->sendSelf(commitTag, theChannel) < 0) {
            opserr << "Domain::send - element with tag " <<
                theEle->getTag() << " failed in sendSelf\n";
            return -8;
        }
    }
    ...
    while ((theSP = theSPs()) != 0) {
        if (theSP->sendSelf(commitTag, theChannel) < 0) {
            opserr << "Domain::send - SP_Constraint with tag " <<
                theSP->getTag() << " failed in sendSelf\n";
            return -9;
        }
    }
    ...
    while ((thePC = thePCs()) != 0) {
        if (thePC->sendSelf(commitTag, theChannel) < 0) {
            opserr << "Domain::send - Pressure_Constraint with tag " <<
                thePC->getTag() << " failed in sendSelf\n";
            return -9;
        }
    }
    ...
    while ((theMP = theMPs()) != 0) {
        if (theMP->sendSelf(commitTag, theChannel) < 0) {
            opserr << "Domain::send - MP_Constraint with tag " <<
                theMP->getTag() << " failed in sendSelf\n";
            return -10;
        }
    }
    ...
    while ((theLP = theLPs()) != 0) {
        if (theLP->sendSelf(commitTag, theChannel) < 0) {
            opserr << "Domain::send - LoadPattern with tag " <<
                theLP->getTag() << " failed in sendSelf\n";
            return -11;
        }
    }
    ...
    while ((theParam = theParams()) != 0) {
        if (theParam->sendSelf(commitTag, theChannel) < 0) {
            opserr << "Domain::send - Parameter with tag " <<
                theParam->getTag() << " failed in sendSelf\n";
            return -12;
        }
    }
...
int Domain::recvSelf(int cTag, Channel & theChannel,
                     FEM_ObjectBroker & theBroker)
{
    ...
    if (currentGeoTag == 0 || lastChannel != theChannel.getTag()
        || domainData(0) != currentGeoTag) {
        ...
        if (numNod != 0) {
            ...
            if (theChannel.recvID(dbNod, geoTag, nodeData) < 0) {
                opserr <<
                    "Domain::recv - channel failed to recv the node ID\n";
                return -2;
            }
            ...
            for (i = 0; i < numNod; i++) {
                ...
                if (theNode == 0) {
                    opserr <<
                        "Domain::recv - cannot create node with classTag "
                        << classTag << endln;
                    return -2;
                }
                ...
                if (theNode->recvSelf(commitTag, theChannel, theBroker) <
                    0) {
                    opserr << "Domain::recv - node with dbTag " << dbTag <<
                        " failed in recvSelf\n";
                    return -2;
                }
                ...
                if (this->addNode(theNode) == false) {
                    opserr <<
                        "Domain::recv - could not add node with tag "
                        << theNode->getTag() << " into domain\n!";
                    return -3;
                }
        ...
        if (numEle != 0) {
            ...
            if (theChannel.recvID(dbEle, geoTag, eleData) < 0) {
                opserr <<
                    "Domain::recv - channel failed to recv the Ele ID\n";
                return -2;
            }
            ...
            for (i = 0; i < numEle; i++) {
                ...
                if (theEle == 0) {
                    opserr <<
                        "Domain::recv - cannot create element with classTag "
                        << classTag << endln;
                    return -2;
                }
                theEle->setDbTag(dbTag);
                ...
                if (theEle->recvSelf(commitTag, theChannel, theBroker) < 0) {
                    opserr << "Domain::recv - Ele with dbTag " <<
                        dbTag << " failed in recvSelf()\n";
                    return -2;
                }

                if (this->addElement(theEle) == false) {
                    opserr <<
                        "Domain::recv - could not add Ele with tag "
                        << theEle->getTag() << " into domain!\n";
                    return -3;
                }
        ...
        if (numSPs != 0) {
            ...
            if (theChannel.recvID(dbSPs, geoTag, spData) < 0) {
                opserr <<
                    "Domain::recv - channel failed to recv the SP_Constraints ID\n";
                return -2;
            }
            ...
            for (i = 0; i < numSPs; i++) {
                ...
                if (theSP == 0) {
                    opserr <<
                        "Domain::recv - cannot create SP_Constraint with classTag "
                        << classTag << endln;
                    return -2;
                }
                theSP->setDbTag(dbTag);
                ...
                if (theSP->recvSelf(commitTag, theChannel, theBroker) < 0) {
                    opserr <<
                        "Domain::recv - SP_Constraint with dbTag " <<
                        dbTag << " failed in recvSelf\n";
                    return -2;
                }
                ...
                if (this->addSP_Constraint(theSP) == false) {
                    opserr <<
                        "Domain::recv - could not add SP_Constraint with tag "
                        << theSP->getTag() << " into domain!\n";
                    return -3;
                }
        ...
        if (numPCs != 0) {
            ...
            if (theChannel.recvID(dbPCs, geoTag, pData) < 0) {
                opserr <<
                    "Domain::recv - channel failed to recv the Pressure_Constraints ID\n";
                return -2;
            }
            ...
            for (i = 0; i < numPCs; i++) {
                ...
                if (thePC == 0) {
                    opserr <<
                        "Domain::recv - cannot create Pressure_Constraint with classTag "
                        << classTag << endln;
                    return -2;
                }
                thePC->setDbTag(dbTag);
                ...
                if (thePC->recvSelf(commitTag, theChannel, theBroker) < 0) {
                    opserr <<
                        "Domain::recv - Pressure_Constraint with dbTag "
                        << dbTag << " failed in recvSelf\n";
                    return -2;
                }
                ...
                if (this->addPressure_Constraint(thePC) == false) {
                    opserr <<
                        "Domain::recv - could not add Pressure_Constraint with tag "
                        << thePC->getTag() << " into domain!\n";
                    return -3;
                }
        ...
        if (numMPs != 0) {
            ...
            if (theChannel.recvID(dbMPs, geoTag, mpData) < 0) {
                opserr <<
                    "Domain::recv - channel failed to recv the MP_Constraints ID\n";
                return -2;
            }
            ...
            for (i = 0; i < numMPs; i++) {
                ...
                if (theMP == 0) {
                    opserr <<
                        "Domain::recv - cannot create MP_Constraint with classTag "
                        << classTag << endln;
                    return -2;
                }
                theMP->setDbTag(dbTag);
                ...
                if (theMP->recvSelf(commitTag, theChannel, theBroker) < 0) {
                    opserr <<
                        "Domain::recv - MP_Constraint with dbTag " <<
                        dbTag << " failed in recvSelf\n";
                    return -2;
                }
                ...
                if (this->addMP_Constraint(theMP) == false) {
                    opserr <<
                        "Domain::recv - could not add MP_Constraint with tag "
                        << theMP->getTag() << " into domain!\n";
                    return -3;
                }
        ...
        if (numLPs != 0) {
            ...
            for (i = 0; i < numLPs; i++) {
                ...
                if (theLP->recvSelf(commitTag, theChannel, theBroker) < 0) {
                    opserr << "Domain::recv - LoadPattern with dbTag "
                        << dbTag << " failed in recvSelf\n";
                    return -2;
                }
                ...
                if (this->addLoadPattern(theLP) == false) {
                    opserr <<
                        "Domain::recv - could not add LoadPattern with tag "
                        << theLP->getTag() << " into the Domain\n";
                    return -3;
                }
        ...
        if (numParameters != 0) {
            ...
            for (i = 0; i < numParameters; i++) {
                ...
                if (theParameter->recvSelf
                    (commitTag, theChannel, theBroker) < 0) {
                    opserr << "Domain::recv - Parameter with dbTag " <<
                        dbTag << " failed in recvSelf\n";
                    return -2;
                }
                ...
                if (this->addParameter(theParameter) == false) {
                    opserr <<
                        "Domain::recv - could not add LoadPattern with tag "
                        << theParameter->getTag() << " into the Domain\n";
                    return -3;
                }
        ...
        while ((theNode = theNodes()) != 0) {
            if (theNode->recvSelf(commitTag, theChannel, theBroker) < 0) {
                opserr << "Domain::recv - node with tag " <<
                    theNode->getTag() << " failed in recvSelf\n";
                return -7;
            }
        }
        ...
        while ((theEle = theElements()) != 0) {
            if (theEle->recvSelf(commitTag, theChannel, theBroker) < 0) {
                opserr << "Domain::recv - element with tag " <<
        ...
        while ((theSP = theSPs()) != 0) {
            if (theSP->recvSelf(commitTag, theChannel, theBroker) < 0) {
                opserr << "Domain::recv - SP_Constraint with tag " <<
                    theSP->getTag() << " failed in recvSelf\n";
                return -9;
            }
        }
        ...
        while ((thePC = thePCs()) != 0) {
            if (thePC->recvSelf(commitTag, theChannel, theBroker) < 0) {
                opserr << "Domain::recv - Pressure_Constraint with tag "
                    << thePC->getTag() << " failed in recvSelf\n";
                return -9;
            }
        }
        ...
        while ((theMP = theMPs()) != 0) {
            if (theMP->recvSelf(commitTag, theChannel, theBroker) < 0) {
                opserr << "Domain::recv - MP_Constraint with tag " <<
                    theMP->getTag() << " failed in recvSelf\n";
                return -10;
            }
        }
        ...
        while ((theLP = theLPs()) != 0) {
            if (theLP->recvSelf(commitTag, theChannel, theBroker) < 0) {
                opserr << "Domain::recv - LoadPattern with tag" <<
                    theLP->getTag() << " failed in recvSelf";
                return -11;
            }
        }
        ...
        while ((theParam = theParams()) != 0) {
            if (theParam->recvSelf(commitTag, theChannel, theBroker) < 0) {
                opserr << "Domain::recv - Parameter with tag" <<
                    theParam->getTag() << " failed in recvSelf";
                return -12;
            }
        }
    }
...
double Domain::getNodeDisp(int nodeTag, int dof, int &errorFlag)
{
    double result = 0.0;
    errorFlag = 0;
    Node *theNode = this->getNode(nodeTag);
...
int Domain::setMass(const Matrix & mass, int nodeTag)
{
    Node *theNode = this->getNode(nodeTag);
    if (theNode == 0) {
        return -1;
...
int Domain::calculateNodalReactions(int flag)
{
...
//added by SAJalali
Recorder *Domain::getRecorder(int tag)
{
    Recorder *res = 0;
