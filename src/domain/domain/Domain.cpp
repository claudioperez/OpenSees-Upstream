...
numParameters (0)
{
    ...
    if (theElements == 0 || theNodes == 0 ||
        ...
        theLoadPatterns == 0 || theLoadPatternIter == 0 || theParameters == 0)
      {
    ...
    for (int i = 0; i < nodes.Size (); i++)
      {
          ...
          if (nodePtr == 0)
            {
                opserr << "WARNING Domain::addElement - In element " <<
                    eleTag;
                opserr << "\n no Node " << nodeTag <<
                            ...
                            if ((i < numDOF) && (fixityConditions (i) == 1))
                              {
                                  ...
                                  while ((found == false)
                                    ...
                                    {
                                        int spNodeTag =
                                            theExistingSP->getNodeTag ();
                                        int dof =
                                            theExistingSP->getDOF_Number ();
                                  ...
                                  if (found == false)
                                    {
                                        ...
                                        if (this->addSP_Constraint (theSP) ==
                                            false)
                                          {
                                              opserr <<
    ...
    if (eleGraphBuiltFlag == false)
      {
          ...
          if (theElementGraph == 0)
            {                   // if still 0 try a smaller one
                theElementGraph = new Graph ();

                if (theElementGraph == 0)
                  {             // if still 0 out of memory
                                   ...
                                   double betaKc)
{
    ...
    while ((elePtr = theElemIter ()) != 0)
        result +=
            elePtr->setRayleighDampingFactors (alphaM, betaK, betaK0, betaKc);
...
void
Domain::setModalProperties (const DomainModalProperties & dmp)
{
...
void
Domain::unsetModalProperties (void)
{
    if (theModalProperties)
      {
          delete theModalProperties;
...
const DomainModalProperties &
Domain::getModalProperties (void) const
{
    if (theModalProperties == 0)
      {
          opserr <<
...
Domain::Print (OPS_Stream & s, int flag)
{
    ...
    s << "\nSP_Constraints: numConstraints: " << theSPs->
        getNumComponents () << "\n";
    theSPs->Print (s, flag);

    s << "\nPressure_Constraints: numConstraints: " << thePCs->
        getNumComponents () << "\n";
    thePCs->Print (s, flag);

    s << "\nMP_Constraints: numConstraints: " << theMPs->
        getNumComponents () << "\n";
    theMPs->Print (s, flag);

    s << "\nLOAD PATTERNS: numPatterns: " << theLoadPatterns->
        getNumComponents () << "\n\n";
    theLoadPatterns->Print (s, flag);

    s << "\nPARAMETERS: numParameters: " << theParameters->
        getNumComponents () << "\n\n";
...
void
Domain::Print (OPS_Stream & s, ID * nodeTags, ID * eleTags, int flag)
{
    if (nodeTags != 0)
      {
                ...
                while ((theNode = theNodes ()) != 0)
                  {
                      ...
                      if (dbTag == 0)
                        {       // go get a new tag and setDbTag in ele if this not 0 
    ...
    while ((theNode = theNodes ()) != 0)
      {
          if (theNode->sendSelf (commitTag, theChannel) < 0)
            {
                opserr << "Domain::send - node with tag " << theNode->
                    getTag () << " failed in sendSelf\n";
                return -7;
            }
      }
    ...
    while ((theEle = theElements ()) != 0)
      {
          if (theEle->sendSelf (commitTag, theChannel) < 0)
            {
                opserr << "Domain::send - element with tag " << theEle->
                    getTag () << " failed in sendSelf\n";
                return -8;
            }
      }
    ...
    while ((theSP = theSPs ()) != 0)
      {
          if (theSP->sendSelf (commitTag, theChannel) < 0)
            {
                opserr << "Domain::send - SP_Constraint with tag " << theSP->
                    getTag () << " failed in sendSelf\n";
                return -9;
            }
      }
    ...
    while ((theMP = theMPs ()) != 0)
      {
          if (theMP->sendSelf (commitTag, theChannel) < 0)
            {
                opserr << "Domain::send - MP_Constraint with tag " << theMP->
                    getTag () << " failed in sendSelf\n";
                return -10;
            }
      }
    ...
    while ((theLP = theLPs ()) != 0)
      {
          if (theLP->sendSelf (commitTag, theChannel) < 0)
            {
                opserr << "Domain::send - LoadPattern with tag " << theLP->
                    getTag () << " failed in sendSelf\n";
                return -11;
            }
      }
    ...
    while ((theParam = theParams ()) != 0)
      {
          if (theParam->sendSelf (commitTag, theChannel) < 0)
            {
                opserr << "Domain::send - Parameter with tag " << theParam->
                    getTag () << " failed in sendSelf\n";
                return -12;
            }
      }
...
int
Domain::recvSelf (int cTag, Channel & theChannel,
                ...
                for (i = 0; i < numNod; i++)
                  {
                      ...
                      if (theNode->
                        ...
                        {
                            opserr << "Domain::recv - node with dbTag " <<
                ...
                for (i = 0; i < numSPs; i++)
                  {
                      ...
                      if (theSP->recvSelf (commitTag, theChannel, theBroker) <
                          0)
                        {
                            opserr <<
                ...
                for (i = 0; i < numPCs; i++)
                  {
                      ...
                      if (thePC->recvSelf (commitTag, theChannel, theBroker) <
                          0)
                        {
                            opserr <<
                ...
                for (i = 0; i < numMPs; i++)
                  {
                      ...
                      if (theMP->recvSelf (commitTag, theChannel, theBroker) <
                          0)
                        {
                            opserr <<
                ...
                for (i = 0; i < numLPs; i++)
                  {
                      ...
                      LoadPattern *theLP =
                          theBroker.getNewLoadPattern (classTag);
                      if (theLP == 0)
                        {
                            opserr <<
                                "Domain::recv - cannot create MP_Constraint with classTag  "
                      ...
                      if (theLP->recvSelf (commitTag, theChannel, theBroker) <
                          0)
                        {
                ...
                for (i = 0; i < numParameters; i++)
                  {
                      ...
                      Parameter *theParameter =
                          theBroker.getParameter (classTag);
                      if (theParameter == 0)
                        {
                            opserr <<
                                "Domain::recv - cannot create MP_Constraint with classTag  "
          ...
          while ((theNode = theNodes ()) != 0)
            {
                if (theNode->recvSelf (commitTag, theChannel, theBroker) < 0)
                  {
                      opserr << "Domain::recv - node with tag " << theNode->
                          getTag () << " failed in recvSelf\n";
                      return -7;
                  }
            }
          ...
          while ((theEle = theElements ()) != 0)
            {
                if (theEle->recvSelf (commitTag, theChannel, theBroker) < 0)
                  {
                      opserr << "Domain::recv - element with tag " << theEle->
                          getTag () << " failed in recvSelf\n";
                      return -8;
                  }
                theEle->update ();
...
int
Domain::activateElements (const ID & elementList)
{
...
int
Domain::deactivateElements (const ID & elementList)
{
    // ElementIter& iter = getElements();
    Element *theElement;
    for (int i = 0; i < elementList.Size (); ++i)
