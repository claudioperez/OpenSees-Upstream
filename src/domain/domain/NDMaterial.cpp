...
NDMaterial::NDMaterial(int tag, int classTag):Material(tag, classTag)
{

}

NDMaterial::NDMaterial():Material(0, 0)
{
...
NDMaterial *NDMaterial::getCopy(const char *type)
{
    if (strcmp(type, "PlaneStress") == 0 ||
        strcmp(type, "PlaneStress2D") == 0) {
        NDMaterial *copy = this->getCopy("ThreeDimensional");
        PlaneStressMaterial *clone =
            new PlaneStressMaterial(this->getTag(), *copy);
        return clone;
    } else if (strcmp(type, "BeamFiber") == 0 ||
               strcmp(type, "TimoshenkoFiber") == 0) {
        NDMaterial *copy = this->getCopy("ThreeDimensional");
        BeamFiberMaterial *clone =
            new BeamFiberMaterial(this->getTag(), *copy);
        return clone;
    } else if (strcmp(type, "BeamFiber2d") == 0 ||
               strcmp(type, "TimoshenkoFiber2d") == 0) {
        NDMaterial *copy = this->getCopy("ThreeDimensional");
        BeamFiberMaterial2d *clone =
            new BeamFiberMaterial2d(this->getTag(), *copy);
        return clone;
    } else if (strcmp(type, "PlateFiber") == 0) {
        NDMaterial *copy = this->getCopy("ThreeDimensional");
        PlateFiberMaterial *clone =
            new PlateFiberMaterial(this->getTag(), *copy);
        return clone;
    } else
        return 0;
}

double NDMaterial::getRho(void)
{
    return 0.0;
}
...
int NDMaterial::setTrialStrain(const Vector & v, const Vector & r)
{
    opserr << "NDMaterial::setTrialStrain -- subclass responsibility\n";
    return -1;
}
...
{
    opserr <<
        "NDMaterial::setTrialStrainIncr -- subclass responsibility\n";
    return -1;
}

int NDMaterial::setTrialStrainIncr(const Vector & v, const Vector & r)
{
    opserr <<
        "NDMaterial::setTrialStrainIncr -- subclass responsibility\n";
    return -1;
...
const Matrix & NDMaterial::getTangent(void)
{
    opserr << "NDMaterial::getTangent -- subclass responsibility\n";
    return errMatrix;
}

const Vector & NDMaterial::getStress(void)
{
    opserr << "NDMaterial::getStress -- subclass responsibility\n";
    return errVector;
}

const Vector & NDMaterial::getStrain(void)
{
...
double
    NDMaterial::getThermalTangentAndElongation(double &TempT, double &ET,
                                               double &Elong)
{
    opserr <<
        "NDMaterial::getThermalTangentAndElongation -- subclass responsibility\n";
    return -1;
}

double
 NDMaterial::setThermalTangentAndElongation(double &TempT, double &ET,
                                            double &Elong)
{
    opserr <<
        "NDMaterial::setThermalTangentAndElongation -- subclass responsibility\n";
    return -1;
}

const Vector & NDMaterial::getTempAndElong()
{
...
Response *NDMaterial::setResponse(const char **argv, int argc,
                                  OPS_Stream & output)
{
    ...
    //Adding temperature and thermal expansion output,L.Jiang [SIF]
        ...
        theResponse =
            new MaterialResponse(this, 3, this->getTempAndElong());
    }
...
int NDMaterial::getResponse(int responseID, Information & matInfo)
{
    switch (responseID) {
    case 1:
...
const Vector & NDMaterial::getStrainSensitivity(int gradIndex)
{
    static Vector dummy(1);
    return dummy;
}

double NDMaterial::getRhoSensitivity(int gradIndex)
{
    return 0.0;
}

const Matrix & NDMaterial::getDampTangentSensitivity(int gradIndex)
{
    static Matrix dummy(1, 1);
    return dummy;
}

const Matrix & NDMaterial::getTangentSensitivity(int gradIndex)
{
    static Matrix dummy(1, 1);
    return dummy;
}

const Matrix & NDMaterial::getInitialTangentSensitivity(int gradIndex)
{
    static Matrix dummy(1, 1);
    return dummy;
}

int NDMaterial::commitSensitivity(const Vector & strainSensitivity,
