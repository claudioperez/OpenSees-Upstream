...
UniaxialMaterial *OPS_getUniaxialMaterial(int tag)
{
    ...
    TaggedObject *theResult =
        theUniaxialMaterialObjects.getComponentPtr(tag);
    if (theResult == 0) {
        opserr <<
            "UniaxialMaterial *getUniaxialMaterial(int tag) - none found with tag: "
            << tag << endln;
        return 0;
    }
...
void OPS_printUniaxialMaterial(OPS_Stream & s, int flag)
{
    if (flag == OPS_PRINT_PRINTMODEL_JSON) {
        ...
        MapOfTaggedObjectsIter theObjects =
            theUniaxialMaterialObjects.getIter();
        theObjects.reset();
...
UniaxialMaterial::UniaxialMaterial(int tag, int clasTag):Material(tag,
                                                                  clasTag)
{
...
UniaxialMaterial::UniaxialMaterial():Material(0, 0)
{
...
int
 UniaxialMaterial::setTrialStrain(double strain, double temperature,
                                  double strainRate)
{
...
int UniaxialMaterial::setTrial(double strain, double &stress,
...
int UniaxialMaterial::setTrial(double strain, double temperature,
...
double UniaxialMaterial::getStrainRate(void)
{
    return 0.0;
}
...
double UniaxialMaterial::getDampTangent(void)
{
    return 0.0;
}
...
double UniaxialMaterial::getRho(void)
{
    return 0.0;
}

UniaxialMaterial *UniaxialMaterial::getCopy(SectionForceDeformation * s)
{
    return this->getCopy();
}

Response *UniaxialMaterial::setResponse(const char **argv, int argc,
                                        OPS_Stream & theOutput)
{
    ...
    if ((strcmp(argv[0], "stress") == 0) ||
        ...
        // tangent
            ...
            theResponse =
                new MaterialResponse(this, 2, this->getTangent());
        }
        // strain
...
int UniaxialMaterial::getResponse(int responseID, Information & matInfo)
{
    ...
    case 7:
        if ((this->getVariable("TempAndElong", infoData)) != 0) {
            opserr <<
                "Warning: invalid tag in uniaxialMaterial:getVariable" <<
                endln;
            return -1;
        }
...
double UniaxialMaterial::getTangentSensitivity(int gradIndex)
{
    return 0.0;
}

double UniaxialMaterial::getInitialTangentSensitivity(int gradIndex)
{
    return 0.0;
}

double UniaxialMaterial::getRhoSensitivity(int gradIndex)
{
    return 0.0;
}

double UniaxialMaterial::getDampTangentSensitivity(int gradIndex)
{
    return 0.0;
}

int UniaxialMaterial::commitSensitivity(double strainSensitivity,
                                        int gradIndex, int numGrads)
{
    return -1;
}

double UniaxialMaterial::getInitialTangent(void)
{
    opserr << "UniaxialMaterial::getInitialTangent() -- this method " <<
        endln << " is not implemented for the selected material. " <<
        endln;
    return 0.0;
}
