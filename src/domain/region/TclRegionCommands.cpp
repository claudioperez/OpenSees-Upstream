    ...
    while (loc < argc)
      {
          ...
          if (strcmp (argv[loc], "-ele") == 0
              || strcmp (argv[loc], "-eleOnly") == 0)
            {
                ...
                while (loc < argc
                       && Tcl_GetInt (interp, argv[loc++], &eleTag) == TCL_OK)
                  {
                   ...
                   || strcmp (argv[loc], "-eleOnlyRange") == 0)
            {
                ...
                if (argc < loc + 3)
                  {
                      opserr <<
                          "WARNING region tag? .. -eleRange start? end?  .. - no ele tags specified\n";
                      return TCL_ERROR;
                  }
                ...
                if (Tcl_GetInt (interp, argv[loc + 1], &start) != TCL_OK)
                  {
                      opserr <<
                          "WARNING region tag? -eleRange start? end? - invalid start "
                          << argv[loc + 1] << endln;
                      return TCL_ERROR;
                  }
                if (Tcl_GetInt (interp, argv[loc + 2], &end) != TCL_OK)
                  {
                      opserr <<
                          "WARNING region tag? -eleRange start? end? - invalid end "
                          << argv[loc + 2] << endln;
                      return TCL_ERROR;
                  }
                if (start > end)
                   ...
                   || strcmp (argv[loc], "-nodeOnly") == 0)
            {
                ...
                if (argc < loc + 2)
                  {
                      opserr <<
                          "WARNING region tag? .. -node tag1? .. - no node tags specified\n";
                      return TCL_ERROR;
                  }
                ...
                int nodTag;
                while (loc < argc
                       && Tcl_GetInt (interp, argv[loc++], &nodTag) == TCL_OK)
                  {
                   ...
                   || strcmp (argv[loc], "-nodeOnlyRange") == 0)
            {
                ...
                if (argc < loc + 3)
                  {
                      opserr <<
                          "WARNING region tag? .. -nodeRange start? end?  .. - no node tags specified\n";
                      return TCL_ERROR;
                  }
          ...
          else if (strcmp (argv[loc], "-rayleigh") == 0)
            {
                ...
                if (argc < loc + 5)
                  {
                      opserr <<
                          "WARNING region tag? .. -rayleigh aM? bK? bK0?  .. - not enough factors\n";
                      return TCL_ERROR;
                  }
                ...
                if (Tcl_GetDouble (interp, argv[loc + 1], &alphaM) != TCL_OK)
                  {
                      opserr <<
                          "WARNING region tag? .. -rayleigh aM bK bK0 - invalid aM "
                          << argv[loc + 1] << endln;
                      return TCL_ERROR;
                  }
                if (Tcl_GetDouble (interp, argv[loc + 2], &betaK) != TCL_OK)
                  {
                      opserr <<
                          "WARNING region tag? .. -rayleigh aM bK bK0 - invalid bK "
                          << argv[loc + 2] << endln;
                      return TCL_ERROR;
                  }
                if (Tcl_GetDouble (interp, argv[loc + 3], &betaK0) != TCL_OK)
                  {
                      opserr <<
                          "WARNING region tag? .. -rayleigh aM bK bK0 - invalid bK0 "
                          << argv[loc + 3] << endln;
                      return TCL_ERROR;
                  }
                if (Tcl_GetDouble (interp, argv[loc + 4], &betaKc) != TCL_OK)
                  {
                      opserr <<
                          "WARNING region tag? .. -rayleigh aM bK bK0 - invalid bKc "
                          << argv[loc + 4] << endln;
                      return TCL_ERROR;
                  }
                loc += 5;
          ...
          if (nodeOnly == false)
            {
                if (theElements == 0)
                    ...
                    opserr <<
                        "WARNING region - both elements & nodes set, ONLY set using elements\n";
            }
          else
            {
