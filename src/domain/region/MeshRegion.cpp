...
MeshRegion::MeshRegion(int tag):DomainComponent(tag,
                                                REGION_TAG_MeshRegion),
alphaM(0), betaK(0), betaK0(0), betaKc(0), theNodes(0), theElements(0),
xEles(), currentGeoTag(0), lastGeoSendTag(-1), dbNod(0), dbEle(0)
{
    // does nothing
}

MeshRegion::MeshRegion(int tag, int cTag):DomainComponent(tag, cTag),
alphaM(0),
betaK(0),
betaK0(0),
betaKc(0),
...
int MeshRegion::setNodesOnly(const ID & theNods)
{
...
int MeshRegion::setElements(const ID & theEles)
{
    // destroy the old lists
    if (theNodes != 0)
        delete theNodes;
...
int MeshRegion::setElementsOnly(const ID & theEles)
{
...
const ID & MeshRegion::getNodes(void)
{
    if (theNodes == 0)
...
const ID & MeshRegion::getElements(void)
{
    if (theElements == 0)
        opserr <<
            "FATAL::MeshRegion::getElements(void) - no elements yet set\n";

    return *theElements;
}

int MeshRegion::setRayleighDampingFactors(double alpham, double betak,
                                          double betak0, double betakc)
{
    ...
    if (theDomain == 0) {
        opserr <<
            "MeshRegion::setRayleighDampingFactors() - no domain yet set\n";
        return -1;
    }
    if (theElements != 0) {
...
int MeshRegion::sendSelf(int commitTag, Channel & theChannel)
{
    ...
    if (theChannel.sendID(myDbTag, commitTag, lpData) < 0) {
        opserr <<
            "MeshRegion::sendSelf - channel failed to send the initial ID\n";
        return -1;
    }
    ...
    if (lastGeoSendTag != currentGeoTag) {
        if (numNod != 0)
            if (theChannel.sendID(dbNod, currentGeoTag, *theNodes) < 0) {
                opserr <<
                    "MeshRegion::sendSelf - channel failed to send the nodes\n";
                return -1;
            }
        if (numEle != 0)
            if (theChannel.sendID(dbEle, currentGeoTag, *theElements) < 0) {
                opserr <<
                    "MeshRegion::sendSelf - channel failed to send the elements\n";
                return -1;
            }
...
int MeshRegion::recvSelf(int commitTag, Channel & theChannel,
                         FEM_ObjectBroker & theBroker)
{
    ...
    if (theChannel.recvID(myDbTag, commitTag, lpData) < 0) {
        opserr <<
            "MeshRegion::recvSelf - channel failed to recv the initial ID\n";
        return -1;
    }
    ...
    if (currentGeoTag != lpData(0)) {
        ...
        if (numNod != 0)
            if (theChannel.recvID(dbNod, currentGeoTag, *theNodes) < 0) {
                opserr <<
                    "MeshRegion::sendSelf - channel failed to recv the nodes\n";
                return -1;
            }
        if (numEle != 0)
            if (theChannel.recvID(dbEle, currentGeoTag, *theElements) < 0) {
                opserr <<
                    "MeshRegion::sendSelf - channel failed to recv the elements\n";
                return -1;
            }
...
void MeshRegion::Print(OPS_Stream & s, int flag)
{
