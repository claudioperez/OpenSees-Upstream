...
ActorSubdomain::ActorSubdomain(Channel & theChannel, FEM_ObjectBroker & theBroker):Subdomain(0), Actor(theChannel, theBroker, 0),
msgData(4),
lastResponse(0)
{
    // does nothing
 ...
 ActorSubdomain::run(void)
{
    ...
    while (exitYet == false) {
        ...
        case ShadowActorSubdomain_addSP_ConstraintAXIS:
            ...
            msgData(0) = 0;
            numSP =
                this->addSP_Constraint(axisDirn, (*theV) (0), *theI,
                                       (*theV) (1));
        ...
        case ShadowActorSubdomain_addNodalLoadToPattern:
            ...
            if (theNodalLoad != 0) {
                ...
                bool result =
                    this->addNodalLoad(theNodalLoad, loadPatternTag);
                if (result == true)
                    msgData(0) = 0;
                else
                    msgData(0) = -1;
        ...
        case ShadowActorSubdomain_addSP_ConstraintToPattern:
            ...
            if (theSP != 0) {
                ...
                this->recvObject(*theSP);
                bool result =
                    this->addSP_Constraint(theSP, loadPatternTag);
        ...
        case ShadowActorSubdomain_removeSP_ConstraintNoTag:
            ...
            loadPatternTag = msgData(3);
            msgData(0) =
                this->removeSP_Constraint(tag, dof, loadPatternTag);
            this->sendID(msgData);
        ...
        case ShadowActorSubdomain_setAnalysisIntegrator:
            theType = msgData(1);
            theIntegrator =
                theBroker->getNewIncrementalIntegrator(theType);
            if (theIntegrator != 0) {
...
//          this->clearAnalysis();
        ...
        case ShadowActorSubdomain_getElementResponse:
            ...
            if (this->recvMessage(theMessage) < 0) {
                opserr <<
                    "ElementRecorder::recvSelf() - failed to recv message\n";
                return -1;
            }
            //
            ...
            theVector =
                this->getElementResponse(tag, (const char **) argv, argc);
        ...
        case ShadowActorSubdomain_setRayleighDampingFactors:
            ...
            this->recvVector(*theV);
            intRes =
                this->Subdomain::setRayleighDampingFactors((*theV) (0),
                                                           (*theV) (1),
                                                           (*theV) (2),
                                                           (*theV) (3));
            delete theV;
        ...
        case ShadowActorSubdomain_updateParameterDOUBLE:
            ...
            msgData(0) =
                this->Domain::updateParameter(theType, theVect1(0));
        ...
        default:
            opserr << "ActorSubdomain::invalid action " << action <<
                "received\n";
            msgData(0) = -1;
...
const Vector & ActorSubdomain::getLastExternalSysResponse(void)
{
...
int ActorSubdomain::update(void)
{
...
int ActorSubdomain::updateTimeDt(void)
{
...
int ActorSubdomain::barrierCheck(int myResult)
{
    static ID data(1);
    data(0) = myResult;
    this->sendID(data);
