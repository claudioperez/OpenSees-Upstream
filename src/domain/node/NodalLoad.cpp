...
NodalLoad::NodalLoad(int theClasTag):Load(0, theClasTag),
...
NodalLoad::NodalLoad(int tag, int node, int theClassTag):Load(tag,
                                                              theClassTag),
...
NodalLoad::NodalLoad(int tag, int node, const Vector & theLoad,
                     bool isLoadConstant):Load(tag, LOAD_TAG_NodalLoad),
myNode(node), myNodePtr(0), load(0), konstant(isLoadConstant)
{
    ...
    if (load == 0) {
        opserr <<
            "FATAL NodalLoad::NodalLoad(int node, const Vector &theLoad) -";
...
int NodalLoad::getNodeTag(void) const
{
    return myNode;
}


void NodalLoad::applyLoad(double loadFactor)
{
    if (myNodePtr == 0) {
        ...
        if ((theDomain == 0) ||
            ...
            opserr <<
                "WARNING NodalLoad::applyLoad() - No associated Node node ";
            opserr << " for NodalLoad " << *this;
            return;
        }
    }
...
void NodalLoad::applyLoadSensitivity(double loadFactor)
{
    if (myNodePtr == 0) {
        Domain *theDomain = this->getDomain();
        if ((theDomain == 0) ||
            (myNodePtr = theDomain->getNode(myNode)) == 0) {
            opserr <<
                "WARNING NodalLoad::applyLoadSensitivity() - No associated Node node ";
            opserr << " for NodalLoad " << *this;
            return;
        }
    }
...
int NodalLoad::sendSelf(int cTag, Channel & theChannel)
{
...
int NodalLoad::recvSelf(int cTag, Channel & theChannel,
...
void NodalLoad::Print(OPS_Stream & s, int flag)
{
    s << "Nodal Load: " << myNode;
    if (load != 0)
        s << " load : " << *load;
...
// AddingSensitivity:BEGIN /////////////////////////////////////
int NodalLoad::setParameter(const char **argv, int argc, Parameter & param)
{
    if (argc < 1)
        return -1;
...
int NodalLoad::updateParameter(int parameterID, Information & info)
{
    switch (parameterID) {
    case -1:
        return -1;
...
int NodalLoad::activateParameter(int passedParameterID)
{
...
const Vector & NodalLoad::getExternalForceSensitivity(int gradNumber)
{
...
void NodalLoad::applyLoad(Vector & loadFactors)
{

}

const Vector & NodalLoad::getData(int &type)
{
