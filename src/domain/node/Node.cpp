...
Node::Node(int theClassTag):DomainComponent(0, theClassTag),
numberDOF(0),
theDOF_GroupPtr(0),
Crd(0),
commitDisp(0),
...
alphaM(0.0), theEigenvectors(0), index(-1), reaction(0), displayLocation(0)
{
...
Node::Node(int tag, int theClassTag):DomainComponent(tag, theClassTag),
numberDOF(0),
theDOF_GroupPtr(0),
Crd(0),
commitDisp(0),
...
Node::Node(int tag, int ndof, double Crd1,
           Vector * dLoc):DomainComponent(tag, NOD_TAG_Node),
numberDOF(ndof), theDOF_GroupPtr(0), Crd(0), commitDisp(0), commitVel(0),
commitAccel(0), trialDisp(0), trialVel(0), trialAccel(0), unbalLoad(0),
incrDisp(0), incrDeltaDisp(0), disp(0), vel(0), accel(0), dbTag1(0),
dbTag2(0), dbTag3(0), dbTag4(0), R(0), mass(0), unbalLoadWithInertia(0),
alphaM(0.0), theEigenvectors(0), index(-1), reaction(0), displayLocation(0)
{
    // AddingSensitivity:BEGIN /////////////////////////////////////////
    dispSensitivity = 0;
...
incrDisp(0), incrDeltaDisp(0), disp(0), vel(0), accel(0), dbTag1(0),
dbTag2(0), dbTag3(0), dbTag4(0), R(0), mass(0), unbalLoadWithInertia(0),
alphaM(0.0), theEigenvectors(0), reaction(0), displayLocation(0)
{
    // AddingSensitivity:BEGIN /////////////////////////////////////////
    dispSensitivity = 0;
    velSensitivity = 0;
...
Node::Node(const Node & otherNode,
           bool copyMass):DomainComponent(otherNode.getTag(),
                                          otherNode.getClassTag()),
numberDOF(otherNode.numberDOF), theDOF_GroupPtr(0), Crd(0), commitDisp(0),
commitVel(0), commitAccel(0), trialDisp(0), trialVel(0), trialAccel(0),
unbalLoad(0), incrDisp(0), incrDeltaDisp(0), disp(0), vel(0), accel(0),
dbTag1(0), dbTag2(0), dbTag3(0), dbTag4(0), R(0), mass(0),
unbalLoadWithInertia(0), alphaM(0.0), theEigenvectors(0), reaction(0),
displayLocation(0)
{
    // AddingSensitivity:BEGIN /////////////////////////////////////////
    dispSensitivity = 0;
    ...
    if (Crd == 0) {
        opserr <<
            " FATAL Node::Node(node *) - ran out of memory for Crd\n";
        exit(-1);
    }
    ...
    if (otherNode.commitDisp != 0) {
        if (this->createDisp() < 0) {
            opserr <<
                " FATAL Node::Node(node *) - ran out of memory for displacement\n";
            exit(-1);
        }
    ...
    if (otherNode.commitVel != 0) {
        if (this->createVel() < 0) {
            opserr <<
                " FATAL Node::Node(node *) - ran out of memory for velocity\n";
            exit(-1);
        }
    ...
    if (otherNode.commitAccel != 0) {
        if (this->createAccel() < 0) {
            opserr <<
                " FATAL Node::Node(node *) - ran out of memory for acceleration\n";
            exit(-1);
        }
        for (int i = 0; i < 2 * numberDOF; i++)
    ...
    if (otherNode.unbalLoad != 0) {
        ...
        if (unbalLoad == 0) {
            opserr <<
                " FATAL Node::Node(node *) - ran out of memory for Load\n";
            exit(-1);
        }
        unbalLoad->Zero();
    }
    ...
    if (otherNode.mass != 0 && copyMass == true) {
        ...
        if (mass == 0) {
            opserr <<
                " FATAL Node::Node(node *) - ran out of memory for mass\n";
            exit(-1);
        }
    }
    ...
    if (otherNode.R != 0) {
        ...
        if (R == 0) {
            opserr <<
                " FATAL Node::Node(node *) - ran out of memory for R\n";
            exit(-1);
        }
    }
...
void Node::setDOF_GroupPtr(DOF_Group * theDOF_Grp)
{
...
DOF_Group *Node::getDOF_GroupPtr(void)
{
...
const Vector & Node::getCrds() const
{
...
const Vector & Node::getDisp(void)
{
...
const Vector & Node::getAccel(void)
{
    // construct memory and Vectors for trial and committed
...
const Vector & Node::getTrialDisp(void)
{
    if (trialDisp == 0) {
        if (this->createDisp() < 0) {
...
const Vector & Node::getTrialVel(void)
{
    if (trialVel == 0) {
        if (this->createVel() < 0) {
...
const Vector & Node::getTrialAccel(void)
{
    if (trialAccel == 0) {
        if (this->createAccel() < 0) {
...
const Vector & Node::getIncrDisp(void)
{
    if (incrDisp == 0) {
        if (this->createDisp() < 0) {
            opserr << "FATAL Node::getTrialDisp() -- ran out of memory\n";
...
const Vector & Node::getIncrDeltaDisp(void)
{
    if (incrDeltaDisp == 0) {
        if (this->createDisp() < 0) {
            opserr << "FATAL Node::getTrialDisp() -- ran out of memory\n";
...
int Node::setTrialDisp(double value, int dof)
{
...
int Node::setTrialDisp(const Vector & newTrialDisp)
{
    // check vector arg is of correct size
...
int Node::setTrialVel(const Vector & newTrialVel)
{
    // check vector arg is of correct size
...
int Node::setTrialAccel(const Vector & newTrialAccel)
{
    // check vector arg is of correct size
...
int Node::incrTrialDisp(const Vector & incrDispl)
{
    // check vector arg is of correct size
...
int Node::incrTrialVel(const Vector & incrVel)
{
    // check vector arg is of correct size
...
void Node::zeroUnbalancedLoad(void)
{
    if (unbalLoad != 0)
        unbalLoad->Zero();
}

int Node::addUnbalancedLoad(const Vector & add, double fact)
{
    // check vector arg is of correct size
...
int Node::addInertiaLoadToUnbalance(const Vector & accelG, double fact)
{
    ...
    if (accelG.Size() != R->noCols()) {
        opserr <<
            "Node::addInertiaLoadToUnbalance - accelG not of correct dimension";
        return -1;
    }
    // if no load yet create it and assign
    if (unbalLoad == 0) {
...
int Node::addInertiaLoadSensitivityToUnbalance(const Vector & accelG,
                                               double fact,
                                               bool
...
const Vector & Node::getUnbalancedLoad(void)
{
...
const Vector & Node::getUnbalancedLoadIncInertia(void)
{
    // make sure it was created before we return it
    if (unbalLoadWithInertia == 0) {
        ...
        if (unbalLoadWithInertia == 0) {
            opserr <<
                "FATAL Node::getunbalLoadWithInertia -- ran out of memory\n";
            exit(-1);
        }
    } else
    ...
    if (mass != 0) {
        ...
        if (alphaM != 0.0) {
            ...
            unbalLoadWithInertia->addMatrixVector(1.0, *mass, theVel,
                                                  -alphaM);
        }
    }
...
const Matrix & Node::getMass(void)
{
    if (index == -1) {
        setGlobalMatrices();
    }
...
int Node::setRayleighDampingFactor(double alpham)
{
    alphaM = alpham;
    return 0;
}
...
const Matrix & Node::getDamp(void)
{
    if (index == -1) {
        setGlobalMatrices();
    }
...
const Matrix & Node::getDampSensitivity(void)
{
    if (index == -1) {
        setGlobalMatrices();
    }
...
int Node::setMass(const Matrix & newMass)
{
    // check right size
...
int Node::setNumColR(int numCol)
{
    if (R != 0) {
        if (R->noCols() != numCol) {
            delete R;
...
int Node::setR(int row, int col, double Value)
{
...
const Vector & Node::getRV(const Vector & V)
{
    ...
    if (unbalLoadWithInertia == 0) {
        ...
        if (unbalLoadWithInertia == 0) {
            opserr <<
                "Node::getunbalLoadWithInertia -- ran out of memory\n";
            exit(-1);
        }
    }
    ...
    if (R->noCols() != V.Size()) {
        opserr <<
            "WARNING Node::getRV() - R and V of incompatible dimesions\n";
        opserr << "R: " << *R << "V: " << V;
        unbalLoadWithInertia->Zero();
        return *unbalLoadWithInertia;
    }
...
int Node::setNumEigenvectors(int numVectorsToStore)
{
    // ensure a positive number of vectors
    if (numVectorsToStore <= 0) {
        opserr << "Node::setNumEigenvectors() - " << numVectorsToStore <<
            " < 0\n";
        return -1;
    }
    // if matrix not yet assigned or not of correct size delete old and create new
    if (theEigenvectors == 0
        || theEigenvectors->noCols() != numVectorsToStore) {
        if (theEigenvectors != 0)
            delete theEigenvectors;
...
int Node::setEigenvector(int mode, const Vector & eigenVector)
{
    if (theEigenvectors == 0 || theEigenvectors->noCols() < mode) {
        opserr << "Node::setEigenvectors() - mode " << mode <<
            " invalid\n";
        return -1;
    }
    ...
    if (eigenVector.Size() != numberDOF) {
        opserr <<
            "Node::setEigenvectors() - eigenvector of incorrect size\n";
        return -2;
    }
    // set the values
    for (int i = 0; i < numberDOF; i++)
...
const Matrix & Node::getEigenvectors(void)
{
...
int Node::sendSelf(int cTag, Channel & theChannel)
{
...
int Node::recvSelf(int cTag, Channel & theChannel,
                   FEM_ObjectBroker & theBroker)
{
    ...
    if (Crd == 0) {
        opserr <<
            "Node::recvSelf() - out of memory creating Coordinate vector\n";
        return -1;
    }

    if (theChannel.recvVector(dataTag, cTag, *Crd) < 0) {
        opserr <<
            "Node::recvSelf() - failed to receive the Coordinate vector\n";
        return -2;
    }
    ...
    if (data(4) == 0) {
        ...
        if (theChannel.recvVector(dbTag3, cTag, *commitAccel) < 0) {
            opserr <<
                "Node::recvSelf - failed to receive Acceleration data\n";
            return -4;
        }
...
void Node::Print(OPS_Stream & s, int flag)
{
    if (flag == OPS_PRINT_CURRENTSTATE) {       // print out everything
        ...
        if (mass != 0) {
            ...
            s << "\t Rayleigh Forces: " <<
                *this->getResponse(RayleighForces);
        }
        if (theEigenvectors != 0)
...
int Node::displaySelf(Renderer & theRenderer, int theEleMode,
                      int theNodeMode, float fact)
{
...
// display coordinates should be based on eleMode, not on nodeMode - ambaker1
    ...
    if (theNodeMode == -1) {
        ...
        return theRenderer.drawText(position, theText,
                                    (int) strlen(theText));

    } else if (theNodeMode > 0) {
        // draw a point - pixel size equals displayMode tag
        return theRenderer.drawPoint(position, 0.0, this->getTag(), 0,
                                     theNodeMode);
    }
...
int Node::createDisp(void)
{
    ...
    if (disp == 0) {
        opserr <<
            "WARNING - Node::createDisp() ran out of memory for array of size "
            << 2 * numberDOF << endln;
    ...
    if (commitDisp == 0 || trialDisp == 0 || incrDisp == 0
        || incrDeltaDisp == 0) {
...
int Node::createVel(void)
{
    vel = new double[2 * numberDOF];

    if (vel == 0) {
        opserr <<
            "WARNING - Node::createVel() ran out of memory for array of size "
            << 2 * numberDOF << endln;
        return -1;
    }
    for (int i = 0; i < 2 * numberDOF; i++)
...
int Node::createAccel(void)
{
    accel = new double[2 * numberDOF];

    if (accel == 0) {
        opserr <<
            "WARNING - Node::createAccel() ran out of memory for array of size "
            << 2 * numberDOF << endln;
        return -1;
    }
    for (int i = 0; i < 2 * numberDOF; i++)
    ...
    if (commitAccel == 0 || trialAccel == 0) {
        opserr <<
            "WARNING - Node::createAccel() ran out of memory creating Vectors(double *,int)\n";
        return -2;
    }
...
int Node::getCrdsSensitivity(void)
{
...
int Node::setParameter(const char **argv, int argc, Parameter & param)
{
    ...
    if ((strstr(argv[0], "mass") != 0) || (strstr(argv[0], "-mass") != 0)) {
        ...
        if ((direction >= 1 && direction <= 3) || direction == 7
            || direction == 8)
...
int Node::updateParameter(int pparameterID, Information & info)
{
...
int Node::activateParameter(int passedParameterID)
{
...
int Node::saveDispSensitivity(const Vector & v, int gradIndex,
                              int numGrads)
{
...
int Node::saveVelSensitivity(const Vector & vdot, int gradIndex,
                             int numGrads)
{
    // If the sensitivity matrices are not already created:
    if (velSensitivity == 0) {
...
int Node::saveAccelSensitivity(const Vector & vdotdot, int gradIndex,
                               int numGrads)
{
    // If the sensitivity matrices are not already created:
    if (accSensitivity == 0) {
...
double Node::getDispSensitivity(int dof, int gradIndex)
{
    if (dispSensitivity != 0)
...
double Node::getVelSensitivity(int dof, int gradIndex)
{
    if (velSensitivity != 0)
...
double Node::getAccSensitivity(int dof, int gradIndex)
{
    if (accSensitivity != 0)
...
int Node::addReactionForce(const Vector & add, double factor)
{
    ...
    if (add.Size() != numberDOF) {
        opserr <<
            "WARNING Node::addReactionForce() - vector not of correct size\n";
        return -1;
    }
...
int Node::resetReactionForce(int flag)
{
...
const Vector *Node::getResponse(NodeResponseType responseType)
{
    ...
    if (responseType == Disp)
        ...
        if (alphaM != 0.0 && mass != 0) {
            const Vector & theVel = this->getTrialVel();        // in case vel not created
            unbalLoadWithInertia->addMatrixVector(0.0, *mass, theVel,
                                                  -alphaM);
        } else
            unbalLoadWithInertia->Zero();
...
void Node::setCrds(double Crd1, double Crd2)
{
    if (Crd != 0 && Crd->Size() >= 2) {
        (*Crd) (0) = Crd1;
        (*Crd) (1) = Crd2;
...
void Node::setCrds(double Crd1, double Crd2, double Crd3)
{
    if (Crd != 0 && Crd->Size() >= 3) {
        (*Crd) (0) = Crd1;
        (*Crd) (1) = Crd2;
...
void Node::setCrds(const Vector & newCrds)
{
...
int Node::getDisplayRots(Vector & res, double fact, int mode)
{
    int ndm = Crd->Size();
...
int Node::getDisplayCrds(Vector & res, double fact, int mode)
{
    // Get all DOFs
    int ndm = Crd->Size();
    int resSize = res.Size();
    ...
    if (mode < 0) {
        int eigenMode = -mode;
        if ((theEigenvectors != 0)
            && ((*theEigenvectors).noCols() >= eigenMode)) {
            if (displayLocation != 0)
                for (int i = 0; i < ndm; i++)
                    res(i) =
                        (*displayLocation) (i) + (*theEigenvectors) (i,
                                                                     eigenMode
                                                                     -
                                                                     1) *
                        fact;
        ...
        if (commitDisp != 0) {
            if (displayLocation != 0)
                for (int i = 0; i < ndm; i++)
                    res(i) =
                        (*displayLocation) (i) + (*commitDisp) (i) * fact;
            else
                for (int i = 0; i < ndm; i++)
...
int Node::setDisplayCrds(const Vector & theCrds)
{
...
NodalThermalAction *Node::getNodalThermalActionPtr(void)
{
    return theNodalThermalActionPtr;
}

void Node::setNodalThermalActionPtr(NodalThermalAction * theAction)
{
