    ...
    if (trialAccel == 0)
      {
          if (this->createAccel () < 0)
            {
                opserr <<
                    "FATAL Node::incrTrialAccel() - ran out of memory\n";
                exit (-1);
            }
          for (int i = 0; i < numberDOF; i++)
              accel[i] = incrAccel (i);
...
int
Node::addInertiaLoadSensitivityToUnbalance (const Vector & accelG,
                                            double fact,
...
int
Node::recvSelf (int cTag, Channel & theChannel, FEM_ObjectBroker & theBroker)
{
    int res = 0;
    int dataTag = this->getDbTag ();
    ...
    if (data (3) == 0)
      {
          ...
          if (theChannel.recvVector (dbTag2, cTag, *commitVel) < 0)
            {
                opserr <<
                    "Node::recvSelf - failed to receive Velocity data\n";
                return -3;
            }
...
Node::Print (OPS_Stream & s, int flag)
{
    if (flag == OPS_PRINT_CURRENTSTATE)
          ...
          if (mass != 0)
            {
                ...
                s << "\t Rayleigh Forces: " << *this->
                    getResponse (RayleighForces);
            }
          if (theEigenvectors != 0)
      ...
      {                         // print out: nodeId displacements
...
int
Node::displaySelf (Renderer & theRenderer, int theEleMode, int theNodeMode,
                   float fact)
{
...
Matrix Node::getMassSensitivity (void)
{
    ...
    if (mass == 0)
      ...
      {
          Matrix
          massSens (mass->noRows (), mass->noCols ());
...
int
Node::saveAccelSensitivity (const Vector & vdotdot, int gradIndex,
                            int numGrads)
{
    // If the sensitivity matrices are not already created:
    if (accSensitivity == 0)
    ...
    if (reaction == 0)
      {
          ...
          if (reaction == 0)
            {
                opserr <<
                    "WARNING Node::addReactionForce() - out of memory\n";
                return -1;
            }
      }
...
Node::getResponse (NodeResponseType responseType)
{
    ...
    if (responseType == Disp)
    ...
    else if (responseType == RayleighForces)
      {
          if (unbalLoadWithInertia == 0)
            {
                unbalLoadWithInertia =
                    new Vector (this->getUnbalancedLoad ());
            }
