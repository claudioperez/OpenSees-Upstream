...
GroundMotion::GroundMotion(TimeSeries * dispSeries,
                           ...
                           double
                           factor):MovableObject
    (GROUND_MOTION_TAG_GroundMotion), theAccelSeries(accelSeries),
theVelSeries(velSeries), theDispSeries(dispSeries),
theIntegrator(theIntegratr), data(3), delta(dTintegration), fact(factor)
{
...
GroundMotion::GroundMotion(int theClassTag):MovableObject(theClassTag),
theAccelSeries(0),
theVelSeries(0),
theDispSeries(0), theIntegrator(0), data(3), delta(0.0), fact(1.0)
{
...
TimeSeries *GroundMotion::integrate(TimeSeries * theSeries, double delta)
{
    ...
    if (theIntegrator == 0) {
        ...
        if (theIntegrator == 0) {
            opserr <<
                "WARNING:GroundMotion::integrate() - no TimeSeriesIntegrator provided - failed to create a Trapezoidal .. memory problems! \n";
            return 0;
        }
    }
    ...
    if (theNewSeries == 0) {
        opserr <<
            "GroundMotion::integrate - no TimeSeriesIntegrator failed to integrate\n";
        return 0;
    }
...
double GroundMotion::getDuration(void)
{
    if (theAccelSeries != 0)
...
double GroundMotion::getPeakAccel(void)
{
    if (theAccelSeries != 0)
...
double GroundMotion::getPeakVel(void)
{
    if (theVelSeries != 0)
...
double GroundMotion::getPeakDisp(void)
{
    if (theDispSeries != 0)
...
double GroundMotion::getAccel(double time)
{
    if (time < 0.0)
        return 0.0;
...
double GroundMotion::getAccelSensitivity(double time)
{
    if (time < 0.0)
        return 0.0;
...
double GroundMotion::getVel(double time)
{
    if (time < 0.0)
        return 0.0;
    ...
    // if theAccel is not 0, integrate accel series to get a vel series
        ...
        if (theVelSeries != 0) {
            ...
            opserr <<
                " WARNING: GroundMotion::getVel(double time) - failed to integrate\n";
            return 0.0;
        }
    }
...
double GroundMotion::getDisp(double time)
{
    if (time < 0.0)
        return 0.0;
    ...
    // if theAccel is not 0, integrate vel series to get disp series
        ...
        opserr <<
            " WARNING: GroundMotion::getDisp(double time) - default integration required to get the ground displacements from the ground velocities via the ground accelerations\n";
        theVelSeries = this->integrate(theAccelSeries, delta);
        if (theVelSeries != 0) {
            theDispSeries = this->integrate(theVelSeries, delta);
            if (theDispSeries != 0)
...
const Vector & GroundMotion::getDispVelAccel(double time)
{
    if (time < 0.0) {
        data(0) = 0.0;
        data(1) = 0.0;
...
int GroundMotion::sendSelf(int commitTag, Channel & theChannel)
{
    ...
    if (res < 0) {
        opserr <<
            "GroundMotion::sendSelf() - channel failed to send data\n";
        return res;
    }
    // now send the series
    if (theAccelSeries != 0) {
        ...
        if (res < 0) {
            opserr <<
                "GroundMotion::sendSelf - failed to send accel series\n";
            return res;
        }
    }
    ...
    if (theVelSeries != 0) {
        ...
        if (res < 0) {
            opserr <<
                "GroundMotion::sendSelf - failed to send velocity series\n";
            return res;
        }
    }
    ...
    if (theDispSeries != 0) {
        ...
        if (res < 0) {
            opserr <<
                "GroundMotion::sendSelf - failed to send disp series\n";
            return res;
        }
    }
...
int GroundMotion::recvSelf(int commitTag, Channel & theChannel,
                           FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0) {
        opserr <<
            "GroundMotion::recvSelf() - channel failed to send data\n";
        return res;
    }
    ...
    if (seriesClassTag != -1) {
        ...
        if (theAccelSeries == 0
            || theAccelSeries->getClassTag() != seriesClassTag) {
            if (theAccelSeries != 0)
                delete theAccelSeries;
            theAccelSeries = theBroker.getNewTimeSeries(seriesClassTag);
            if (theAccelSeries == 0) {
                opserr <<
                    "GroundMotion::recvSelf - could not create a Series object\n";
                return -2;
            }
        }
        theAccelSeries->setDbTag(seriesDbTag);
        ...
        if (res < 0) {
            opserr <<
                "GroundMotion::recvSelf() - accel series failed to send data\n";
            return res;
        }
    }
    ...
    if (seriesClassTag != -1) {
        ...
        if (theVelSeries == 0
            || theVelSeries->getClassTag() != seriesClassTag) {
            if (theVelSeries != 0)
                delete theVelSeries;
    ...
    if (seriesClassTag != -1) {
        ...
        if (theDispSeries == 0
            || theDispSeries->getClassTag() != seriesClassTag) {
            if (theDispSeries != 0)
                delete theDispSeries;
    ...
    if (seriesClassTag != -1) {
        ...
        if (theIntegrator == 0
            || theIntegrator->getClassTag() != seriesClassTag) {
            if (theIntegrator != 0)
                delete theIntegrator;
            theIntegrator =
                theBroker.getNewTimeSeriesIntegrator(seriesClassTag);
            if (theIntegrator == 0) {
                opserr <<
                    "GroundMotion::recvSelf - could not create an Integrator object\n";
                return -2;
            }
        }
        theIntegrator->setDbTag(seriesDbTag);
...
int GroundMotion::setParameter(const char **argv, int argc,
                               Parameter & param)
{
