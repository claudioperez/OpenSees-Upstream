...
GroundMotionRecord::GroundMotionRecord():GroundMotion(GROUND_MOTION_TAG_GroundMotionRecord),
...
GroundMotionRecord::GroundMotionRecord(const char *fileNameAccel,
                                       ...
                                       double
                                       dT):GroundMotion
    (GROUND_MOTION_TAG_GroundMotionRecord), theAccelTimeSeries(0),
theVelTimeSeries(0), theDispTimeSeries(0), data(3), delta(dT)
{

    theAccelTimeSeries =
        new PathSeries(0, fileNameAccel, timeStep, theFactor);

    if (theAccelTimeSeries == 0) {
        opserr <<
            "GroundMotionRecord::GroundMotionRecord() - unable to create PathSeries\n";
    }
...
theVelTimeSeries(0), theDispTimeSeries(0), data(3), delta(dT)
{
    ...
    theAccelTimeSeries =
        new PathTimeSeries(0, fileNameAccel, fileNameTime, theFactor);
...
double GroundMotionRecord::getPeakAccel(void)
{
    if (theAccelTimeSeries != 0)
...
double GroundMotionRecord::getPeakVel(void)
{
    if (theVelTimeSeries != 0)
...
double GroundMotionRecord::getPeakDisp(void)
{
    if (theDispTimeSeries != 0)
...
double GroundMotionRecord::getAccel(double time)
{
    if (time < 0.0)
        return 0.0;
...
double GroundMotionRecord::getVel(double time)
{
    if (time < 0.0)
        return 0.0;
...
double GroundMotionRecord::getDisp(double time)
{
    if (time < 0.0)
        return 0.0;
...
const Vector & GroundMotionRecord::getDispVelAccel(double time)
{
    if (time < 0.0) {
        data(0) = 0.0;
        data(1) = 0.0;
    ...
    if (theAccelTimeSeries != 0 && theVelTimeSeries != 0
        && theDispTimeSeries != 0) {
...
int GroundMotionRecord::sendSelf(int commitTag, Channel & theChannel)
{
    ...
    if (res < 0) {
        opserr <<
            "GroundMotionRecord::sendSelf() - channel failed to send data\n";
        return res;
    }
    // now send the series
    if (theAccelTimeSeries != 0) {
        ...
        if (res < 0) {
            opserr <<
                "GroundMotionRecord::sendSelf - failed to send accel series\n";
            return res;
        }
    }
    ...
    if (theVelTimeSeries != 0) {
        ...
        if (res < 0) {
            opserr <<
                "GroundMotionRecord::sendSelf - failed to send velocity series\n";
            return res;
        }
    }
    ...
    if (theDispTimeSeries != 0) {
        ...
        if (res < 0) {
            opserr <<
                "GroundMotionRecord::sendSelf - failed to send disp series\n";
            return res;
        }
    }
...
int GroundMotionRecord::recvSelf(int commitTag, Channel & theChannel,
                                 FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0) {
        opserr <<
            "GroundMotionRecord::recvSelf() - channel failed to send data\n";
        return res;
    }
    ...
    if (seriesClassTag != -1) {
        ...
        if (theAccelTimeSeries == 0
            || theAccelTimeSeries->getClassTag() != seriesClassTag) {
            if (theAccelTimeSeries != 0)
                delete theAccelTimeSeries;
            theAccelTimeSeries =
                theBroker.getNewTimeSeries(seriesClassTag);
            if (theAccelTimeSeries == 0) {
                opserr <<
                    "GroundMotionRecord::recvSelf - could not create a TimeSeries object\n";
                return -2;
            }
        }
        theAccelTimeSeries->setDbTag(seriesDbTag);
        res =
            theAccelTimeSeries->recvSelf(commitTag, theChannel, theBroker);
        if (res < 0) {
            opserr <<
                "GroundMotionRecord::recvSelf() - accel series failed to send data\n";
            return res;
        }
    }
    ...
    if (seriesClassTag != -1) {
        ...
        if (theVelTimeSeries == 0
            || theVelTimeSeries->getClassTag() != seriesClassTag) {
            if (theVelTimeSeries != 0)
                delete theVelTimeSeries;
    ...
    if (seriesClassTag != -1) {
        ...
        if (theDispTimeSeries == 0
            || theDispTimeSeries->getClassTag() != seriesClassTag) {
            if (theDispTimeSeries != 0)
                delete theDispTimeSeries;
        ...
        theDispTimeSeries->setDbTag(seriesDbTag);
        res =
            theDispTimeSeries->recvSelf(commitTag, theChannel, theBroker);
        if (res < 0) {
            opserr <<
                "GroundMotionRecord::recvSelf() - accel series failed to send data\n";
            return res;
