                        ...
                        MultiSupportPattern * thePattern)
{
    ...
    if (argc < 4)
      {
          opserr <<
              "WARNING invalid command - want: groundMotion tag type <args>\n";
    ...
    if ((strcmp (argv[startArg], "Series") == 0) ||
        (strcmp (argv[startArg], "Plain") == 0))
      {
          ...
          while (currentArg < argc - 1)
            {
                if ((strcmp (argv[currentArg], "-accel") == 0) ||
                      ...
                      if (accelSeries == 0)
                        {
                            opserr << "WARNING invalid accel series: " <<
                                argv[currentArg];
                            opserr <<
                                " groundMotion tag Series -accel {series}\n";
                            return TCL_ERROR;
                        }
                      currentArg++;
                      ...
                      if (velSeries == 0)
                        {
                            opserr << "WARNING invalid vel series: " <<
                                argv[currentArg];
                            opserr <<
                                " groundMotion tag Series -vel {series}\n";
                            return TCL_ERROR;
                        }
                      currentArg++;
                      ...
                      if (dispSeries == 0)
                        {
                            opserr << "WARNING invalid disp series: " <<
                                argv[currentArg];
                            opserr <<
                                " groundMotion tag Series -disp {series}\n";
                            return TCL_ERROR;
                        }
                      currentArg++;
                      ...
                      if (seriesIntegrator == 0)
                        {
                            opserr << "WARNING invalid series integrator: " <<
                                argv[currentArg];
                            opserr <<
                                " - groundMotion tag Series -int {Series Integrator}\n";
                            return TCL_ERROR;
                        }
                         ...
                         (strcmp (argv[currentArg], "-deltaT") == 0))
                  {
                      ...
                      currentArg++;
                      if (Tcl_GetDouble (interp, argv[currentArg], &dtInt) !=
                          TCL_OK)
                        {
                            opserr << "WARNING invalid dtInt: " <<
                                argv[currentArg];
                            opserr <<
                                " - groundMotion tag Series -dtInt dt\n";
                            return TCL_ERROR;
                        }
                         ...
                         (strcmp (argv[currentArg], "-factor") == 0))
                  {
                      ...
                      currentArg++;
                      if (Tcl_GetDouble (interp, argv[currentArg], &fact) !=
                          TCL_OK)
                        {
                            opserr << "WARNING invalid factor: " <<
                                argv[currentArg];
                            opserr <<
                                " - groundMotion tag Series -fact factor\n";
                            return TCL_ERROR;
                        }
          ...
          theMotion = new GroundMotion (dispSeries, velSeries,
                                        accelSeries, seriesIntegrator, dtInt,
                                        fact);
    ...
    else if (strcmp (argv[startArg], "Interpolated") == 0)
      {
                ...
                for (int i = 3; i < endMotionIDs; i++)
                  {
                      ...
                      if (theMotion1 == 0)
                        {
                            opserr << "WARNING no groundMotion with tag " <<
                                motionID << " :";
                            opserr <<
                                " pattern MultiSupport gMotion1? gMotion? .. ";
          ...
          for (int i = 0; i < numMotions; i++)
            {
                double fact;
                if (Tcl_GetDouble (interp, argv[i + endMotionIDs + 1], &fact)
                    != TCL_OK)
                    return TCL_ERROR;
                facts[i] = fact;
            }
    ...
    if (theMotion != 0)
      {
          if (thePattern->addMotion (*theMotion, gMotionTag) < 0)
            {
                opserr << "WARNING could not add ground motion with tag " <<
                    gMotionTag;
                opserr << " to pattern\n ";
