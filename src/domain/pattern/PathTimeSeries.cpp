...
PathTimeSeries::PathTimeSeries():TimeSeries(TSERIES_TAG_PathTimeSeries),
...
PathTimeSeries::PathTimeSeries(int tag,
                               ...
                               double theFactor, bool last):TimeSeries(tag,
                                                                       TSERIES_TAG_PathTimeSeries),
thePath(0), time(0), currentTimeLoc(0), cFactor(theFactor), dbTag1(0),
dbTag2(0), lastSendCommitTag(-1), lastChannel(0), useLast(last)
{
    // check vectors are of same size
    if (theLoadPath.Size() != theTimePath.Size()) {
        opserr <<
            "WARNING PathTimeSeries::PathTimeSeries() - vector containing data ";
        ...
        if (thePath == 0 || thePath->Size() == 0 ||
            ...
            opserr <<
                "WARNING PathTimeSeries::PathTimeSeries() - out of memory\n ";
            if (thePath != 0)
                delete thePath;
            if (time != 0)
                delete time;
...
dbTag2(0), lastSendCommitTag(-1), lastChannel(0), useLast(last)
{
    ...
    if (numDataPoints1 != numDataPoints2) {
        opserr <<
            "WARNING PathTimeSeries::PathTimeSeries() - files containing data ";
        opserr <<
            "points for path and time do not contain same number of points\n";
    } else {
        ...
        if (numDataPoints1 != 0) {
            ...
            if (theFile2.bad() || !theFile2.is_open()) {
                opserr << "WARNING - PathTimeSeries::PathTimeSeries()";
                opserr << " - could not open file " << filePathName <<
                    endln;
                delete thePath;
                delete time;
                thePath = 0;
...
PathTimeSeries::PathTimeSeries(int tag,
                               ...
                               double theFactor, bool last):TimeSeries(tag,
                                                                       TSERIES_TAG_PathTimeSeries),
thePath(0), time(0), currentTimeLoc(0), cFactor(theFactor), dbTag1(0),
dbTag2(0), lastChannel(0), useLast(last)
{
    // determine the number of data points
    int numDataPoints = 0;
    double dataPoint;
    ...
    if ((numDataPoints % 2) != 0) {
        opserr << "WARNING - PathTimeSeries::PathTimeSeries()";
        opserr << " - num data entries in file NOT EVEN! " << fileName <<
            endln;
        numDataPoints--;
    }
    ...
    if (numDataPoints != 0) {
        ...
        // ensure did not run out of memory creating copies
        if (thePath == 0 || thePath->Size() == 0 || time == 0
            || time->Size() == 0) {
...
TimeSeries *PathTimeSeries::getCopy(void)
{
    return new PathTimeSeries(this->getTag(), *thePath, *time, cFactor,
                              useLast);
}

double PathTimeSeries::getTimeIncr(double pseudoTime)
{
    // NEED TO FILL IN, FOR NOW return 1.0
    return 1.0;
}

double PathTimeSeries::getFactor(double pseudoTime)
{
...
double PathTimeSeries::getDuration()
{
    if (thePath == 0) {
        opserr <<
            "WARNING -- PathTimeSeries::getDuration() on empty Vector" <<
            endln;
        return 0.0;
    }
...
double PathTimeSeries::getPeakFactor()
{
    if (thePath == 0) {
        opserr <<
            "WARNING -- PathTimeSeries::getPeakFactor() on empty Vector" <<
            endln;
        return 0.0;
    }
...
int PathTimeSeries::sendSelf(int commitTag, Channel & theChannel)
{
    ...
    if (result < 0) {
        opserr <<
            "PathTimeSeries::sendSelf() - channel failed to send data\n";
        return result;
    }
...
int PathTimeSeries::recvSelf(int commitTag, Channel & theChannel,
                             FEM_ObjectBroker & theBroker)
{
    ...
    if (result < 0) {
        opserr <<
            "PathTimeSeries::sendSelf() - channel failed to receive data\n";
        cFactor = 1.0;
        return result;
    }
    cFactor = data(0);
    ...
    if (thePath == 0 && size > 0) {
        ...
        result =
            theChannel.recvVector(dbTag1, lastSendCommitTag, *thePath);
        if (result < 0) {
...
void PathTimeSeries::Print(OPS_Stream & s, int flag)
{
