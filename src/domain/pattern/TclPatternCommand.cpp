...
extern TimeSeriesIntegrator *TclSeriesIntegratorCommand (ClientData
                                                         clientData,
                                                         Tcl_Interp * interp,
                                                         TCL_Char * arg);
                   ...
                   int argc, TCL_Char ** argv, Domain * theDomain)
{
    ...
    if (argc < 4)
      {
          ...
          opserr <<
              " <type args> {list of load and sp constraints commands}\n";
          opserr <<
          ...
          if (thePattern == 0 || theSeries == 0)
            {
                ...
                if (thePattern == 0)
                  {
                      opserr <<
                          "WARNING - out of memory creating LoadPattern ";
                      opserr << patternID << endln;
                  }
                else
                  {
                      opserr <<
                          "WARNING - problem creating TimeSeries for LoadPattern ";
                      opserr << patternID << endln;
                  }
    ...
    //--Adding FireLoadPattern:[BEGIN] by UoE OpenSees Group--//
    else if (strcmp (argv[1], "Fire") == 0)
      {
          ...
          TimeSeries *theSeries1 =
              TclSeriesCommand (clientData, interp, argv[3]);
          TimeSeries *theSeries2 =
              TclSeriesCommand (clientData, interp, argv[4]);
          TimeSeries *theSeries3 =
              TclSeriesCommand (clientData, interp, argv[5]);
          TimeSeries *theSeries4 =
              TclSeriesCommand (clientData, interp, argv[6]);
          TimeSeries *theSeries5 =
              TclSeriesCommand (clientData, interp, argv[7]);
          TimeSeries *theSeries6 =
              TclSeriesCommand (clientData, interp, argv[8]);
          TimeSeries *theSeries7 =
              TclSeriesCommand (clientData, interp, argv[9]);
          TimeSeries *theSeries8 =
              TclSeriesCommand (clientData, interp, argv[10]);
          TimeSeries *theSeries9 =
              TclSeriesCommand (clientData, interp, argv[11]);
          ...
          if (thePattern == 0)
            ...
            {
                opserr <<
                    "WARNING - problem creating TimeSeries1 for LoadPattern ";
                opserr << patternID << endln;
            }
          else if (theSeries2 == 0)
            {
                opserr <<
                    "WARNING - problem creating TimeSeries2 for LoadPattern ";
                opserr << patternID << endln;
            }
          else if (theSeries3 == 0)
            {
                opserr <<
                    "WARNING - problem creating TimeSeries3 for LoadPattern ";
                opserr << patternID << endln;
            }
          else if (theSeries4 == 0)
            {
                opserr <<
                    "WARNING - problem creating TimeSeries4 for LoadPattern ";
                opserr << patternID << endln;
            }
          else if (theSeries5 == 0)
            {
                opserr <<
                    "WARNING - problem creating TimeSeries5 for LoadPattern ";
                opserr << patternID << endln;
            }
          else if (theSeries6 == 0)
            {
                opserr <<
                    "WARNING - problem creating TimeSeries6 for LoadPattern ";
                opserr << patternID << endln;
            }
          else if (theSeries7 == 0)
            {
                opserr <<
                    "WARNING - problem creating TimeSeries7 for LoadPattern ";
                opserr << patternID << endln;
            }
          else if (theSeries8 == 0)
            {
                opserr <<
                    "WARNING - problem creating TimeSeries8 for LoadPattern ";
                opserr << patternID << endln;
            }
          else if (theSeries9 == 0)
            {
                opserr <<
                    "WARNING - problem creating TimeSeries9 for LoadPattern ";
                opserr << patternID << endln;
            }
          ...
          theFirePattern->setFireTimeSeries (theSeries1, theSeries2,
                                             theSeries3, theSeries4,
                                             theSeries5, theSeries6,
                                             theSeries7, theSeries8,
                                             theSeries9);
    ...
    //--Adding FireLoadPattern:[END] by UoE OpenSees Group--//
    ...
    else if (strcmp (argv[1], "UniformExcitation") == 0)
      {
          ...
          if (Tcl_GetInt (interp, argv[3], &dir) != TCL_OK)
            {
                opserr << "WARNING invalid patternID: pattern type " <<
                    argv[2] << "<type args>\n";
                return TCL_ERROR;
            }
          ...
          while (currentArg < argc - 1 && doneSeries == false)
            {
                ...
                if ((strcmp (argv[currentArg], "-vel0") == 0) ||
                    (strcmp (argv[currentArg], "-initialVel") == 0))
                  {
                      ...
                      currentArg++;
                      if ((currentArg < argc) &&
                          (Tcl_GetDouble (interp, argv[currentArg], &vel0) !=
                           TCL_OK))
                        {
                            opserr <<
                                "WARNING invalid vel0: pattern type UniformExcitation\n";
                            return TCL_ERROR;
                        }
                         ...
                         || (strcmp (argv[currentArg], "-factor") == 0))
                  {
                      ...
                      currentArg++;
                      if ((currentArg < argc) &&
                          (Tcl_GetDouble (interp, argv[currentArg], &fact) !=
                           TCL_OK))
                        {
                            opserr <<
                                "WARNING invalid fact: pattern type UniformExcitation\n";
                            return TCL_ERROR;
                        }
                         ...
                         (strcmp (argv[currentArg], "-acceleration") == 0))
                  {
                      ...
                      currentArg++;
                      accelSeries =
                          TclSeriesCommand (clientData, interp,
                                            argv[currentArg]);

                      if (accelSeries == 0)
                        {
                            opserr << "WARNING invalid accel series: " <<
                                argv[currentArg];
                            opserr <<
                         ...
                         (strcmp (argv[currentArg], "-velocity") == 0))
                  {
                      ...
                      if (velSeries == 0)
                        {
                            opserr << "WARNING invalid vel series: " <<
                                argv[currentArg];
                            opserr <<
                                " pattern UniformExcitation -vel {series}\n";
                            return TCL_ERROR;
                        }
                      currentArg++;
                         ...
                         (strcmp (argv[currentArg], "-displacement") == 0))
                  {
                      ...
                      if (dispSeries == 0)
                        {
                            opserr << "WARNING invalid disp series: " <<
                                argv[currentArg];
                            opserr <<
                                " pattern UniformExcitation -disp {series}\n";
                            return TCL_ERROR;
                        }
                      currentArg++;
                         ...
                         (strcmp (argv[currentArg], "-integrator") == 0))
                  {
                      ...
                      currentArg++;
                      seriesIntegrator =
                          TclSeriesIntegratorCommand (clientData, interp,
                                                      argv[currentArg]);
                      if (seriesIntegrator == 0)
                        {
                            opserr << "WARNING invalid series integrator: " <<
                                argv[currentArg];
                            opserr <<
                                " - pattern UniformExcitation -int {Series Integrator}\n";
                            return TCL_ERROR;
                        }
                      currentArg++;
                  }
          ...
          if (dispSeries == 0 && velSeries == 0 && accelSeries == 0)
            {
                opserr <<
                    "WARNING invalid series, want - pattern UniformExcitation";
                opserr <<
                    "-disp {dispSeries} -vel {velSeries} -accel {accelSeries} ";
          ...
          if (theMotion == 0)
            {
                opserr <<
                    "WARNING ran out of memory creating ground motion - pattern UniformExcitation ";
                opserr << patternID << endln;
          ...
          thePattern =
              new UniformExcitation (*theMotion, dir, patternID, vel0, fact);

          if (thePattern == 0)
            {
                opserr <<
                    "WARNING ran out of memory creating load pattern - pattern UniformExcitation ";
                opserr << patternID << endln;
    ...
    else if (strcmp (argv[1], "Uniform") == 0)
      {
          ...
          if (numInputs < 5)
            {
                opserr <<
                    "WARNING insufficient number of arguments - want: pattern ";
                opserr << "UniformExcitation " << patternID <<
                    " dir factor\n";
                return TCL_ERROR;
            }
          ...
          if (Tcl_GetInt (interp, argv[3], &dir) != TCL_OK)
            {
                ...
                switch (inputDir)
                  ...
                  default:
                      opserr <<
                          "WARNING cannot read direction for excitation \n";
                      opserr << "UniformExcitation " << patternID <<
                          " dir factor" << endln;
                      return TCL_ERROR;
                      break;
             ...
             (strcmp (argv[1], "MultiSupport") == 0))
      {
          ...
          if (thePattern == 0)
            {
                opserr <<
                    "WARNING ran out of memory creating load pattern - pattern MultipleSupportExcitation ";
                opserr << patternID << endln;
            }
          commandEndMarker = 2;
      }
             ...
             (strcmp (argv[1], "h5drm") == 0))
      {
          ...
          opserr << "Creating H5DRM tag = " << tag << " filename = " <<
              filename.c_str () << " factor = " << factor << endln;

          thePattern = new H5DRM (tag, filename, factor);

          opserr << "Done! Creating H5DRM tag = " << tag << " filename = " <<
              filename.c_str () << " factor = " << factor << endln;
    ...
    else if (strcmp (argv[1], "DRMLoadPattern") == 0)
      {
          ...
          if ((strcmp (argv[2], "-inputdata") == 0)
              || (strcmp (argv[2], "-InputData") == 0))
            {
                ...
                PlaneDRMInputHandler *patternhandler =
                    new PlaneDRMInputHandler (1.0, files, nf, dt, 0,
                                              num_steps, f_d, 15, n1, n2,
                                              drm_box_crds, drm_box_crds,
                                              ele_d,
                                              myMesher, steps_cached,
                                              theDomain);
                DRMLoadPattern *ptr =
          ...
          else
            {
                ...
                while (c_arg < end)
                  {
                      ...
                      if ((strcmp (argv[c_arg], "-dt") == 0)
                        ...
                        {
                            c_arg++;
                            if (Tcl_GetDouble (interp, argv[c_arg], &dt) !=
                                TCL_OK)
                              {
                                  opserr <<
                                      " Error reading deltaT for DRMLoadPattern \n";
                                  exit (-1);
                              }
                            c_arg++;
                        }
                      ...
                      if ((strcmp (argv[c_arg], "-numSteps") == 0)
                        ...
                        {
                            c_arg++;
                            if (Tcl_GetInt (interp, argv[c_arg], &num_steps)
                                != TCL_OK)
                              {
                                  opserr <<
                                      " Error reading number of steps for DRMLoadPattern \n";
                                  exit (-1);
                              }
                            c_arg++;
                        }
                        ...
                        {
                            c_arg++;
                            if (Tcl_GetDouble (interp, argv[c_arg], &ele_d[0])
                                != TCL_OK)
                              {
                                  opserr <<
                                      " Error reading deltaT for DRMLoadPattern \n";
                            ...
                            c_arg++;
                            if (Tcl_GetDouble (interp, argv[c_arg], &ele_d[1])
                                != TCL_OK)
                              {
                                  opserr <<
                                      " Error reading deltaT for DRMLoadPattern \n";
                            ...
                            c_arg++;
                            if (Tcl_GetDouble (interp, argv[c_arg], &ele_d[2])
                                != TCL_OK)
                              {
                                  opserr <<
                                      " Error reading deltaT for DRMLoadPattern \n";
                        ...
                        {
                            c_arg++;
                            if (Tcl_GetInt (interp, argv[c_arg], &f_d[1]) !=
                                TCL_OK)
                              {
                                  opserr <<
                                      " Error reading grid data f1 for DRMLoadPattern \n";
                                  exit (-1);
                              }
                            c_arg++;
                            if (Tcl_GetInt (interp, argv[c_arg], &f_d[2]) !=
                                TCL_OK)
                              {
                                  opserr <<
                                      " Error reading grid data f1 for DRMLoadPattern \n";
                        ...
                        {
                            c_arg++;
                            if (Tcl_GetInt (interp, argv[c_arg], &f_d[4]) !=
                                TCL_OK)
                              {
                                  opserr <<
                                      " Error reading grid data f2 for DRMLoadPattern \n";
                                  exit (-1);
                              }
                            c_arg++;
                            if (Tcl_GetInt (interp, argv[c_arg], &f_d[5]) !=
                                TCL_OK)
                              {
                                  opserr <<
                                      " Error reading grid data f2 for DRMLoadPattern \n";
                        ...
                        {
                            c_arg++;
                            if (Tcl_GetInt (interp, argv[c_arg], &f_d[7]) !=
                                TCL_OK)
                              {
                                  opserr <<
                                      " Error reading grid data f3 for DRMLoadPattern \n";
                                  exit (-1);
                              }
                            c_arg++;
                            if (Tcl_GetInt (interp, argv[c_arg], &f_d[8]) !=
                                TCL_OK)
                              {
                                  opserr <<
                                      " Error reading grid data f3 for DRMLoadPattern \n";
                        ...
                        {
                            c_arg++;
                            if (Tcl_GetInt (interp, argv[c_arg], &f_d[10]) !=
                                TCL_OK)
                              {
                                  opserr <<
                                      " Error reading grid data f4 for DRMLoadPattern \n";
                                  exit (-1);
                              }
                            c_arg++;
                            if (Tcl_GetInt (interp, argv[c_arg], &f_d[11]) !=
                                TCL_OK)
                              {
                                  opserr <<
                                      " Error reading grid data f4 for DRMLoadPattern \n";
                        ...
                        {
                            c_arg++;
                            if (Tcl_GetInt (interp, argv[c_arg], &f_d[13]) !=
                                TCL_OK)
                              {
                                  opserr <<
                                      " Error reading grid data f5 for DRMLoadPattern \n";
                                  exit (-1);
                              }
                            c_arg++;
                            if (Tcl_GetInt (interp, argv[c_arg], &f_d[14]) !=
                                TCL_OK)
                              {
                                  opserr <<
                                      " Error reading grid data f5 for DRMLoadPattern \n";
                      ...
                      else if ((strcmp (argv[c_arg], "-fileFace5aGridPoints")
                                == 0))
                        {
                            c_arg++;
                            if (Tcl_GetInt (interp, argv[c_arg], &n1) !=
                                TCL_OK)
                              {
                                  opserr <<
                                      " Error reading grid data f5 for DRMLoadPattern \n";
                      ...
                      else if ((strcmp (argv[c_arg], "-fileFace5bGridPoints")
                                == 0))
                        {
                            c_arg++;
                            if (Tcl_GetInt (interp, argv[c_arg], &n2) !=
                                TCL_OK)
                              {
                                  opserr <<
                                      " Error reading grid data f5 for DRMLoadPattern \n";
                        ...
                        {
                            c_arg++;
                            if (Tcl_GetDouble (interp, argv[c_arg], &factor)
                                != TCL_OK)
                              {
                                  opserr <<
                                      " Error reading number of steps for DRMLoadPattern \n";
                      ...
                      else if ((strcmp (argv[c_arg], "-DRMBoxCrds") == 0))
                        {
                            ...
                            if (Tcl_GetDouble
                                (interp, argv[c_arg],
                                 &drm_box_crds[0]) != TCL_OK)
                              {
                                  opserr <<
                                      " Error reading DRM box Crds, xmin for DRMLoadPattern \n";
                                  exit (-1);
                              }
                            c_arg++;
                            if (Tcl_GetDouble
                                (interp, argv[c_arg],
                                 &drm_box_crds[1]) != TCL_OK)
                              {
                                  opserr <<
                                      " Error reading DRM box Crds, xmax for DRMLoadPattern \n";
                                  exit (-1);
                              }
                            c_arg++;
                            if (Tcl_GetDouble
                                (interp, argv[c_arg],
                                 &drm_box_crds[2]) != TCL_OK)
                              {
                                  opserr <<
                                      " Error reading DRM box Crds, ymin for DRMLoadPattern \n";
                                  exit (-1);
                              }
                            c_arg++;
                            if (Tcl_GetDouble
                                (interp, argv[c_arg],
                                 &drm_box_crds[3]) != TCL_OK)
                              {
                                  opserr <<
                                      " Error reading DRM box Crds, ymax for DRMLoadPattern \n";
                                  exit (-1);
                              }
                            c_arg++;
                            if (Tcl_GetDouble
                                (interp, argv[c_arg],
                                 &drm_box_crds[4]) != TCL_OK)
                              {
                                  opserr <<
                                      " Error reading DRM box Crds, zmin for DRMLoadPattern \n";
                                  exit (-1);
                              }
                            c_arg++;
                            if (Tcl_GetDouble
                                (interp, argv[c_arg],
                                 &drm_box_crds[5]) != TCL_OK)
                              {
                                  opserr <<
                                      " Error reading DRM box Crds, zmax for DRMLoadPattern \n";
                                  exit (-1);
                              }
                            c_arg++;
                        }
                ...
                thePattern =
                    new DRMLoadPatternWrapper (patternID, factor, files, nf,
                                               dt, num_steps, f_d, 15, n1, n2,
                                               drm_box_crds, ele_d,
                                               steps_cached);
                theTclMultiSupportPattern = 0;
    ...
    ///////// ///////// ///////// //////// //////  // DRMLoadPattern add END
    ...
    else
      {
          ...
          opserr <<
              " \t valid types: Plain, UniformExcitation, MultiSupportExciatation \n";
          return TCL_ERROR;
      }
