...
MultiSupportPattern::MultiSupportPattern(int tag):LoadPattern(tag,
                                                              PATTERN_TAG_MultiSupportPattern),
theMotions(0), theMotionTags(0, 16), numMotions(0), dbMotions(0)
{
...
MultiSupportPattern::MultiSupportPattern():LoadPattern(0, PATTERN_TAG_MultiSupportPattern),
...
int MultiSupportPattern::addMotion(GroundMotion & theMotion, int tag)
{
    // ensure no motion with given tag already added
    if (theMotionTags.getLocation(tag) >= 0) {
        opserr <<
            "MultiSupportPattern::addMotion - could not add new, motion with same tag exists\n";
        return -1;
    }
    // make space for new
    ...
    if (newMotions == 0) {
        opserr <<
            "MultiSupportPattern::addMotion - could not add new, out of mem\n";
        return -1;
    }
    // copy old
    for (int i = 0; i < numMotions; i++)
...
bool MultiSupportPattern::addNodalLoad(NodalLoad *)
{
    opserr <<
        "MultiSupportPattern::addNodalLoad() - cannot add NodalLoad to EQ pattern\n";
    return false;
}

bool MultiSupportPattern::addElementalLoad(ElementalLoad *)
{
    opserr <<
        "MultiSupportPattern::addElementalLoad() - cannot add ElementalLoad to EQ pattern\n";
    return false;
}



int MultiSupportPattern::sendSelf(int commitTag, Channel & theChannel)
{
    ...
    if (this->LoadPattern::sendSelf(commitTag, theChannel) < 0) {
        opserr <<
            "MultiSupportPattern::sendSelf() - LoadPattern class failed in sendSelf()";
        return -1;
    }
    ...
    if (theChannel.sendID(myDbTag, commitTag, myData) < 0) {
        opserr <<
            "MultiSupportPattern::sendSelf - channel failed to send the initial ID\n";
        return -1;
    }
    ...
    if (numMotions != 0) {
        ...
        if (theChannel.sendID(dbMotions, commitTag, motionData) < 0) {
            opserr <<
                "MultiSupportPattern::sendSelf - channel failed to send the NodalLoads ID\n";
            return -4;
        }
        ...
        for (int j = 0; j < numMotions; j++) {
            ...
            if (theMotion->sendSelf(commitTag, theChannel) < 0) {
                opserr <<
                    "MultiSupportPattern::sendSelf - ground motion  failed in sendSelf\n";
                return -7;
            }
        }
    }
...
int MultiSupportPattern::recvSelf(int commitTag, Channel & theChannel,
                                  FEM_ObjectBroker & theBroker)
{
    ...
    if (this->LoadPattern::recvSelf(commitTag, theChannel, theBroker) < 0) {
        opserr <<
            "MultiSupportPattern::recvSelf() - LoadPattern class failed in sendSelf()";
        return -1;
    }
    ...
    if (numMotions != 0) {
        ...
        for (int i = 0; i < numMotions; i++) {
            ...
            if (theMotion == 0) {
                opserr <<
                    "MultiSupportPattern::sendSelf - failed to get a ground motion from object broker\n";
                return -1;
            }
            ...
            if (theMotion->recvSelf(commitTag, theChannel, theBroker) < 0) {
                opserr <<
                    "MultiSupportPattern::sendSelf - ground motion failed in recvSelf\n";
                return -7;
            }
...
void MultiSupportPattern::Print(OPS_Stream & s, int flag)
{
...
LoadPattern *MultiSupportPattern::getCopy(void)
{
