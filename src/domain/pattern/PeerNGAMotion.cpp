...
void *OPS_PeerNGAMotion(void)
{
    ...
    if (numRemainingArgs < 2) {
        opserr <<
            "WARNING: invalid num args PeerNGAMotion <tag?> $eqMotion $factor\n";
        return 0;
    }
    ...
    if (OPS_GetDouble(&numData, &factor) != 0) {
        opserr << "WARNING invalid shift in peerNGAMotion with tag?" << tag
            << endln;
        return 0;
    }
    ...
    if (theSeries == 0) {
        opserr <<
            "WARNING ran out of memory creating PeerNGAMotion with tag: "
            << tag << "\n";
        return 0;
    }
...
PeerNGAMotion::PeerNGAMotion():TimeSeries(TSERIES_TAG_PeerNGAMotion),
thePath(0), dT(0.0),
...
lastSendCommitTag(-1), lastChannel(0)
{
    ...
    if ((strcmp(type, "ACCEL") == 0) || (strcmp(type, "-accel") == 0)
        || (strcmp(type, "-ACCEL") == 0) || (strcmp(type, "accel") == 0)
        || (strcmp(type, "ATH") == 0) || (strcmp(type, "-ATH") == 0)) {
        sprintf(peerPage, "/nga_files/ath/%s/%s.AT2", earthquake, station);
    } else if ((strcmp(type, "DISP") == 0) || (strcmp(type, "-disp") == 0)
               || (strcmp(type, "-DISP") == 0)
               || (strcmp(type, "adisp") == 0)
               || (strcmp(type, "DTH") == 0)
               || (strcmp(type, "-DTH") == 0)) {
        opserr << "PeerNGAMotion::PeerNGAMotion() - not a valid type:" <<
            type << " (-ACCEL requiured)\n";
    } else {
        opserr << "PeerNGAMotion::PeerNGAMotion() - not a valid type:" <<
            type << " (-ACCEL requiured)\n";
        return;
    ...
    if (nextData != 0) {
        opserr <<
            "PeerNGAMotion::PeerNGAMotion() - could not get Data for record from Database, ";
    ...
    if (nextData == NULL) {
        opserr <<
            "PeerNGAMotion::PeerNGAMotion() - could not find nPts in record, send email opensees-support@berkeley.edu";
        free(eqData);
        return;
    }
    ...
    if (nextData == NULL) {
        ...
        if (nextData == NULL) {
            opserr <<
                "PeerNGAMotion::PeerNGAMotion() - could not find dt in record, send email opensees-support@berkeley.edu";
            free(eqData);
            return;
        }
    }
...
lastSendCommitTag(-1), lastChannel(0)
{
    ...
    if (thePath->Size() == 0) {
        delete thePath;
        thePath = 0;
        opserr << "PeerNGAMotion - nodata for record from url: " <<
            peerPage << endln;
    }
...
TimeSeries *PeerNGAMotion::getCopy(void)
{
...
double PeerNGAMotion::getFactor(double pseudoTime)
{
...
double PeerNGAMotion::getDuration()
{
    if (thePath == 0) {
        opserr << "WARNING -- PeerNGAMotion::getDuration() on empty Vector"
            << endln;
        return 0.0;
    }
...
double PeerNGAMotion::getPeakFactor()
{
    if (thePath == 0) {
        opserr <<
            "WARNING -- PeerNGAMotion::getPeakFactor() on empty Vector" <<
            endln;
        return 0.0;
    }
...
double PeerNGAMotion::getDt()
{
    return dT;
}

int PeerNGAMotion::getNPts()
{
    if (thePath == 0)
        return 0;
    else
...
int PeerNGAMotion::sendSelf(int commitTag, Channel & theChannel)
{
    ...
    if (result < 0) {
        opserr <<
            "PeerNGAMotion::sendSelf() - channel failed to send data\n";
        return result;
    }
...
int PeerNGAMotion::recvSelf(int commitTag, Channel & theChannel,
                            FEM_ObjectBroker & theBroker)
{
    ...
    if (result < 0) {
        opserr <<
            "PeerNGAMotion::sendSelf() - channel failed to receive data\n";
        cFactor = 1.0;
        return result;
    }
...
void PeerNGAMotion::Print(OPS_Stream & s, int flag)
{
