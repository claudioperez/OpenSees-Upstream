...
DRMLoadPatternWrapper::DRMLoadPatternWrapper (int ttag, double cfact,
                                              char **on_files, int sfiles,
                                              double ddt, int numsteps,
                                              int *filedata, int fileDatasize,
                                              int nd_1, int nd_2,
                                              double *drmboxcrds,
                                              double *ele_D,
                                              int stepstocache):
...
DRMLoadPatternWrapper::DRMLoadPatternWrapper ():LoadPattern (0,
...
void
DRMLoadPatternWrapper::initialize ()
{
    Mesh3DSubdomain *my_mesher = new Mesh3DSubdomain (this->getDomain ());

    PlaneDRMInputHandler *patternhandler =
        new PlaneDRMInputHandler (1, infiles, files, dt, 0,
                                  num_steps, file_data, fileData_size,
                                  nd1, nd2,
                                  drm_box_crds, drm_box_crds, eleD,
                                  my_mesher, steps_to_cache,
                                  this->getDomain ());

    DRMLoadPattern *ptr =
        new DRMLoadPattern (1, factor, patternhandler, this->getDomain ());
...
void
DRMLoadPatternWrapper::applyLoad (double time)
{
    if (!initialized)
        this->initialize ();
...
int
DRMLoadPatternWrapper::sendSelf (int commitTag, Channel & theChannel)
{
    ...
    if (theChannel.sendID (dbTag, commitTag, i_Data) < 0)
      {
          opserr <<
              "DRMLoadPatternWrapper::sendSelf L.121 failed to sendID \n";
          return -1;
      }
    ...
    if (theChannel.sendVector (dbTag, commitTag, d_Data) < 0)
      {
          opserr <<
              "DRMLoadPatternWrapper::sendSelf L.135 failed to sendVector \n";
          return -1;
      }
...
//  int pos =0;
    ...
    if (theChannel.sendID (dbTag, commitTag, c_Data_sz) < 0)
      {
          opserr <<
              "DRMLoadPatternWrapper::sendSelf L.156 failed to sendID2 \n";
          return -1;
      }
    ...
    if (theChannel.sendMsg (dbTag, commitTag, c_Data))
      {
          opserr <<
              "DRMLoadPatternWrapper::sendSelf L.161 failed to sendMsg \n";
          return -1;
      }
...
int
DRMLoadPatternWrapper::recvSelf (int commitTag, Channel & theChannel,
                                 FEM_ObjectBroker & theBroker)
{
    ...
    if (theChannel.recvID (dbTag, commitTag, i_Data) < 0)
      {
          opserr <<
              "DRMLoadPatternWrapper::recvSelf L.176 failed to recvID \n";
          return -1;
      }
    ...
    if (theChannel.recvVector (dbTag, commitTag, d_Data) < 0)
      {
          opserr <<
              "DRMLoadPatternWrapper::recvSelf L.200 failed to recvVector \n";
          return -1;
      }
    ...
    if (theChannel.recvID (dbTag, commitTag, c_Data_sz) < 0)
      {
          opserr <<
              "DRMLoadPatternWrapper::recvSelf L.217 failed to recvID2 \n";
          return -1;
      }
    ...
    if (theChannel.recvMsg (dbTag, commitTag, c_Data) < 0)
      {
          opserr <<
              "DRMLoadPatternWrapper::recvSelf L.227 failed to recvMsg \n";
          return -1;
      }
...
LoadPattern *
DRMLoadPatternWrapper::getCopy ()
{
    return new DRMLoadPatternWrapper (this->getTag (), this->factor,
                                      this->infiles, this->files, this->dt,
                                      this->num_steps, this->file_data,
                                      this->fileData_size, this->nd1,
                                      this->nd2, this->drm_box_crds,
                                      this->eleD, this->steps_to_cache);
}


void
DRMLoadPatternWrapper::Print (OPS_Stream & s, int flag)
{
    opserr << "DRMLoadPattern::Print() - not yet implemented\n";
    for (int i = 0; i < this->files; i++)
        printf ("infiles @i %d %s \n", i, this->infiles[i]);
    printf (" %d %f %d %f %d %d %d %d \n", fileData_size, factor, files, dt,
            num_steps, nd1, nd2, steps_to_cache);
    for (int i = 0; i < 6; i++)
