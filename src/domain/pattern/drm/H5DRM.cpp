...
H5DRM::H5DRM (int tag,
              ...
              double cFactor_, double crd_scale_, double distance_tolerance_):
...
H5DRM::intitialize ()
{
    ...
    read_scalar_double_dataset_into_double (id_drm_file, "DRM_Metadata/tend",
                                            tend);
    ...
    H5DRMout << "Found and connected " << n_nodes_found << " nodes (d_err = "
        << d_err << ")\n";
    ...
    H5DRMout << "Found  " << Elements.
        Size () << " elements on DRM boundary. \n";
    ...
    if (found_it)
      {
          ...
          if (id_velocity < 0)
            {
                H5DRMerror <<
                    "Could not open \"DRM_Data/velocity\" array!\n ";
                return;
            }
...
bool
H5DRM::drm_direct_read (double t)
{
    ...
    H5DRMout << "t = " << t << " dt = " << dt << " i1 = " << i1 << " i2 = " <<
        i2 << " t1 = " << t1 << " t2 = " << t2 << " dtau = " << dtau << endln;
...
H5DRM::drm_differentiate_displacements (double t)
{
    ...
    i_first =
        i_first >
        (hsize_t) number_of_timesteps - 1 ? number_of_timesteps - 1 : i_first;

    i_last = i_last < 0 ? 0 : i_last;
    i_last =
        i_last >
        (hsize_t) number_of_timesteps - 1 ? number_of_timesteps - 1 : i_last;
                ...
                if (DEBUG_DRM_INTEGRATION)
                  {
                      ...
                      fprintf (fptr,
                               ...
                               (int) i, dtau, tau_1, tau_2, dt, local_pos,
                               dir);
                      fprintf (fptr,
...
void
H5DRM::node_matching_UsePlaneInfo (double d_tol, const ID & internal,
                                   const Matrix & xyz,
...
void
H5DRM::node_matching_OctTree (double d_tol, const ID & internal,
                              const Matrix & xyz, const Vector & drmbox_x0,
...
void
H5DRM::node_matching_BruteForce (double d_tol, const ID & internal,
                                 const Matrix & xyz, const Vector & drmbox_x0,
                                 double &d_err, int &n_nodes_found)
{
...
Vector
H5DRM_calculate_cross_product (const Vector & a, const Vector & b)
{
                          ...
                          std::string dataset_name, ID & result)
{
    ...
    if (ndims != 1)
      {
          opserr << "failure trying to open dataset: " << dataset_name.
              c_str () << endln;
          opserr <<
...
bool
Plane::get_ij_coordinates_to_point (double x1, double x2, int &i, int &j) const
{
    int N1 = xi1.Size ();
    int N2 = xi2.Size ();
...
int
Plane::getStationNumber (int i, int j) const
{
    int N1 = xi1.Size ();
    int N2 = xi2.Size ();
...
void
convert_h5drmcrd_to_ops_crd (Vector & v)
{
...
void
convert_h5drmcrd_to_ops_crd (Matrix & xyz)
{
