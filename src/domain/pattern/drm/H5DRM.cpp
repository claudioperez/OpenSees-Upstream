...
bool read_int_dataset_into_id(const hid_t & h5drm_dataset,
                              std::string dataset_name, ID & result);
bool read_scalar_double_dataset_into_double(const hid_t & h5drm_dataset,
                                            std::string dataset_name,
                                            double &result);
bool read_double_dataset_into_vector(const hid_t & h5drm_dataset,
                                     std::string dataset_name,
                                     Vector & result);
bool read_double_dataset_into_matrix(const hid_t & h5drm_dataset,
                                     std::string dataset_name,
                                     Matrix & result);
bool read_int_dataset_into_array(const hid_t & h5drm_dataset,
                                 std::string dataset_name, int *&result);
...
H5DRM::H5DRM():LoadPattern(0, PATTERN_TAG_H5DRM),
DRMForces(100),
DRMDisplacements(100),
DRMAccelerations(100),
last_integration_time(0),
crd_scale(0),
distance_tolerance(0), maxnodetag(0), station_id2data_pos(100)
{
    ...
    id_velocity_dataspace = id_displacement_dataspace =
        id_acceleration_dataspace = 0;
...
H5DRM::H5DRM(int tag,
             ...
             double cFactor_, double crd_scale_,
             double distance_tolerance_):LoadPattern(tag,
                                                     PATTERN_TAG_H5DRM),
HDF5filename(HDF5filename_), DRMForces(100), DRMDisplacements(100),
DRMAccelerations(100), last_integration_time(0), crd_scale(crd_scale_),
distance_tolerance(distance_tolerance_), maxnodetag(0),
 ...
 H5DRM::intitialize()
{
    ...
    Matrix xyz;
    Vector drmbox_x0;
    read_double_dataset_into_vector(id_drm_file, "DRM_Metadata/drmbox_x0",
                                    drmbox_x0);
    ...
    read_int_dataset_into_id(id_drm_file, "DRM_Data/data_location",
                             station_id2data_pos);
    read_scalar_double_dataset_into_double(id_drm_file,
    ...
    read_scalar_double_dataset_into_double(id_drm_file,
                                           ...
                                           drmbox_zmin);
    read_scalar_double_dataset_into_double(id_drm_file, "DRM_Metadata/dt",
                                           dt);
    read_scalar_double_dataset_into_double(id_drm_file,
                                           "DRM_Metadata/tstart", tstart);
    read_scalar_double_dataset_into_double(id_drm_file,
                                           "DRM_Metadata/tend", tend);
    ...
    node_matching_BruteForce(d_tol, internal, xyz, drmbox_x0, d_err,
                             n_nodes_found);
    ...
    H5DRMout << "Found and connected " << n_nodes_found <<
        " nodes (d_err = " << d_err << ")\n";
    ...
    for (int i = 0; i < Nodes.Size(); ++i) {
        fprintf(fptr, "%d %d\n", Nodes(i),
                1 * IsBoundary(i) + 2 * (1 - IsBoundary(i)));
    }
    ...
    H5DRMout << "Found  " << Elements.Size() <<
        " elements on DRM boundary. \n";
    ...
    if (found_it) {
        ...
        id_velocity =
            H5Dopen(id_drm_file, "DRM_Data/velocity", H5P_DEFAULT);
        ...
        hsize_t one_node_data_dims[2] =
            { 3, H5DRM_NUM_OF_PRECOMPUTED_TIMESTEPS };
        hsize_t one_node_data_maxdims[2] =
            { 3, H5DRM_NUM_OF_PRECOMPUTED_TIMESTEPS };
        id_one_node_memspace = H5Screate_simple(rank_two_array, one_node_data_dims, one_node_data_maxdims);     // create dataspace of memory
    }

    found_it =
        H5Lexists(id_drm_file, "DRM_Data/acceleration", H5P_DEFAULT);
    if (found_it) {
        H5DRMout << "Opening acceleration dataset.\n";


        id_acceleration =
            H5Dopen(id_drm_file, "DRM_Data/acceleration", H5P_DEFAULT);

        if (id_acceleration < 0) {
            H5DRMerror <<
                "Could not open \"DRM_Data/acceleration\" array!\n ";
            return;
        }
    ...
    found_it =
        H5Lexists(id_drm_file, "DRM_Data/displacement", H5P_DEFAULT);
    if (found_it) {
        H5DRMout << "Opening displacement dataset.\n";


        id_displacement =
            H5Dopen(id_drm_file, "DRM_Data/displacement", H5P_DEFAULT);

        if (id_displacement < 0) {
            H5DRMerror <<
                "Could not open \"DRM_Data/displacement\" array!\n ";
            return;
        }
 ...
 H5DRM::clean_all_data()
{
    ...
    while (id_drm_file > 0 and n_obj_open > 0) {
        ...
        n_obj_open =
            H5Fget_obj_count(id_drm_file,
                             H5F_OBJ_DATASET | H5F_OBJ_GROUP | H5F_OBJ_ATTR
                             | H5F_OBJ_LOCAL);
        ...
        //Close datasets
        number_of_open_objects =
            H5Fget_obj_ids(id_drm_file, H5F_OBJ_DATASET | H5F_OBJ_LOCAL,
                           H5DRM_MAX_RETURN_OPEN_OBJS, obj_id_list);
        if (number_of_open_objects > 0) {
            cout << "                                   - Closing " <<
                number_of_open_objects << " datasets.\n";
        }
        for (int i = 0;
             i < std::min(number_of_open_objects,
                          H5DRM_MAX_RETURN_OPEN_OBJS); i++) {
        ...
        //Close groups
        number_of_open_objects =
            H5Fget_obj_ids(id_drm_file, H5F_OBJ_GROUP | H5F_OBJ_LOCAL,
                           H5DRM_MAX_RETURN_OPEN_OBJS, obj_id_list);
        if (number_of_open_objects > 0) {
            cout << "                                   - Closing " <<
                number_of_open_objects << " groups.\n";
        }
        for (int i = 0;
             i < std::min(number_of_open_objects,
                          H5DRM_MAX_RETURN_OPEN_OBJS); i++) {
        ...
        //Close groups
        number_of_open_objects =
            H5Fget_obj_ids(id_drm_file, H5F_OBJ_ATTR | H5F_OBJ_LOCAL,
                           H5DRM_MAX_RETURN_OPEN_OBJS, obj_id_list);
        if (number_of_open_objects > 0) {
            cout << "                                   - Closing " <<
                number_of_open_objects << " attributes.\n";
        }
        for (int i = 0;
             i < std::min(number_of_open_objects,
                          H5DRM_MAX_RETURN_OPEN_OBJS); i++) {
...
void H5DRM::applyLoad(double time)
{
    ...
    if (number_of_local_elements <= 0) {
        ...
        if (theDomain == 0 || Elements.Size() == 0 || !computedLoads) {
            opserr <<
                "H5DRM::applyLoad -- Error! Failed to compute DRM loads at time = "
                << time << endln;
            return;
        }
...
bool
 H5DRM::ComputeDRMMotions(double next_integration_time)
{
...
bool H5DRM::drm_direct_read(double t)
{
    ...
    H5DRMout << "t = " << t << " dt = " << dt << " i1 = " << i1 << " i2 = "
        << i2 << " t1 = " << t1 << " t2 = " << t2 << " dtau = " << dtau <<
        endln;
    ...
    for (int n = 0; n < Nodes.Size(); ++n) {
        ...
        H5Sselect_hyperslab(id_displacement_dataspace,
                            H5S_SELECT_SET, start, stride, count, block);
        H5Sselect_hyperslab(id_acceleration_dataspace,
                            H5S_SELECT_SET, start, stride, count, block);
        ...
        H5Sselect_hyperslab(memspace,
                            H5S_SELECT_SET, mem_start, mem_stride,
                            mem_count, mem_block);

        //Read data
        herr_t errorflag1 =
            H5Dread(id_displacement, H5T_NATIVE_DOUBLE, memspace,
                    id_displacement_dataspace, id_xfer_plist, d1);
        herr_t errorflag2 =
            H5Dread(id_acceleration, H5T_NATIVE_DOUBLE, memspace,
        ...
        for (int i = 0; i < 3; ++i) {
            if (isnan(d1[i]) ||
                isnan(a1[i]) || isnan(d2[i]) || isnan(a2[i])) {
                nanfound = true;
            }
        ...
        if (errorflag1 < 0 || errorflag2 < 0 || errorflag3 < 0
            || errorflag4 < 0 || nanfound) {
            H5DRMerror <<
                "H5DRM::drm_direct_read - Failed to read displacement or acceleration array!!\n"
                << " n = " << n << endln << " nodeTag = " << nodeTag <<
                endln << " station_id = " << station_id << endln <<
                " i1 = " << i1 << endln << " data_pos = " << data_pos <<
                endln << " local_pos = " << local_pos << endln << " Nt = "
                << 1 << endln << " last_integration_time = " <<
                last_integration_time << endln << " start   = [" <<
                start[0] << ", " << start[1] << "]" << endln <<
                " mem_start  = [" << mem_start[0] << "]" << endln <<
                " stride  = [" << stride[0] << ", " << stride[1] << "]" <<
                endln << " count   = [" << count[0] << ", " << count[1] <<
                "]" << endln << " block   = [" << block[0] << ", " <<
                block[1] << "]" << endln << " errorflag1 = [" <<
                errorflag1 << ", " << block[1] << "]" << endln <<
                " errorflag2 = [" << errorflag2 << ", " << block[1] << "]"
                << endln << " errorflag3 = [" << errorflag3 << ", " <<
                block[1] << "]" << endln << " errorflag4 = [" <<
                errorflag4 << ", " << block[1] << "]" << endln <<
                " nanfound   = [" << nanfound << ", " << block[1] << "]"
                << endln;
        ...
        DRMDisplacements(3 * local_pos + 0) =
            d1[0] * (1 - dtau) + d2[0] * (dtau);
        DRMDisplacements(3 * local_pos + 1) =
            d1[1] * (1 - dtau) + d2[1] * (dtau);
        DRMDisplacements(3 * local_pos + 2) =
            d1[2] * (1 - dtau) + d2[2] * (dtau);

        DRMAccelerations(3 * local_pos + 0) =
            a1[0] * (1 - dtau) + a2[0] * (dtau);
        DRMAccelerations(3 * local_pos + 1) =
            a1[1] * (1 - dtau) + a2[1] * (dtau);
        DRMAccelerations(3 * local_pos + 2) =
            a1[2] * (1 - dtau) + a2[2] * (dtau);
    }

    H5DRMout << "t = " << t << " u = (" << umin << ", " << umax <<
        ") a = (" << amin << ", " << amax << ")" << endln;
...
bool
 H5DRM::drm_differentiate_displacements(double t)
{
    ...
    i_first =
        i_first >
        (hsize_t) number_of_timesteps - 1 ? number_of_timesteps -
        1 : i_first;
    ...
    for (int n = 0; n < Nodes.Size(); ++n) {
        ...
        for (int i = 0; i < 3; ++i) {
            <<<<<<<HEAD
                ...
                == == == =
                    if (isnan(d1[i]) ||
                        isnan(a1[i]) || isnan(d2[i]) || isnan(a2[i]))
                    >>>>>>>732044f 5 b1846fa0047e4516e0717dc0d22bffc3 {
                    nanfound = true;
                    }
            umax = d1[i] > umax ? d1[i] : umax;
            umax = d2[i] > umax ? d2[i] : umax;
        ...
        if (errorflag1 < 0 ||
            ...
            nanfound) {
            H5DRMerror <<
                "Failed to read displacement or acceleration array!!\n" <<
                " n = " << n << endln << " nodeTag = " << nodeTag << endln
                << " station_id = " << station_id << endln << " i1 = " <<
                i1 << endln << " data_pos = " << data_pos << endln <<
                " local_pos = " << local_pos << endln << " Nt = " << 1 <<
                endln << " last_integration_time = " <<
                last_integration_time << endln << " start   = [" <<
                start[0] << ", " << start[1] << "]" << endln <<
                " mem_start  = [" << mem_start[0] << ", " << mem_start[1]
                << "]" << endln << " stride  = [" << stride[0] << ", " <<
                stride[1] << "]" << endln << " count   = [" << count[0] <<
                ", " << count[1] << "]" << endln << " block   = [" <<
                block[0] << ", " << block[1] << "]" << endln;
            exit(-1);
        }
...
bool
 H5DRM::drm_integrate_velocity(double next_integration_time)
{
    ...
    for (int n = 0; n < Nodes.Size(); ++n) {
        ...
        //Selection in dataspace
        H5Sselect_hyperslab(id_velocity_dataspace,
                            H5S_SELECT_SET, start, stride, count, block);
        ...
        hsize_t mem_start[2] = { 0, 0 };
        H5Sselect_hyperslab(memspace,
                            H5S_SELECT_SET, mem_start, stride, count,
                            block);

        //Read data
        herr_t errorflag =
            H5Dread(id_velocity, H5T_NATIVE_DOUBLE, memspace,
        ...
        if (errorflag < 0) {
            H5DRMerror << "Failed to read velocity array!!\n" <<
                ...
                endln << " start   = [" << start[0] << ", " << start[1] <<
                "]" << endln << " mem_start  = [" << mem_start[0] << ", "
                << mem_start[1] << "]" << endln << " stride  = [" <<
                stride[0] << ", " << stride[1] << "]" << endln <<
                " count   = [" << count[0] << ", " << count[1] << "]" <<
                endln << " block   = [" << block[0] << ", " << block[1] <<
                "]" << endln;
            exit(-1);
        }
        ...
        <<<<<<<HEAD for (hsize_t i = 0; i < Nt; ++i) {
            ...
            if (DEBUG_DRM_INTEGRATION) {
                /* FMK
                   ...
                   ***** */
                fprintf(fptr,
                        "i = %d dtau=%f tau_1=%f tau_2=%f dt=%f local_pos=%d dir=%d\n",
                        (int) i, dtau, tau_1, tau_2, dt, local_pos, dir);
                fprintf(fptr,
                        "    DRMDisplacements(3 * local_pos + 0) = %f -->  v[0][i] = %f  v[0][i+1] = %f\n",
                        DRMDisplacements(3 * local_pos + 0),
                        v[0 + i * Nt], v[0 + (i + 1) * 3]);
                fprintf(fptr,
                        "    DRMDisplacements(3 * local_pos + 1) = %f -->  v[1][i] = %f  v[1][i+1] = %f\n",
                        DRMDisplacements(3 * local_pos + 1),
                        v[1 + i * Nt], v[1 + (i + 1) * 3]);
                fprintf(fptr,
                        "    DRMDisplacements(3 * local_pos + 2) = %f -->  v[2][i] = %f  v[2][i+1] = %f\n",
                        DRMDisplacements(3 * local_pos + 2),
                        v[2 + i * Nt], v[2 + (i + 1) * 3]);
            ...
            /* FMK
               ...
               ***** */
            double du1 =
                (v[0 + i * 3] + v[0 + (i + 1) * 3]) * (dir * dtau / 2);
            double du2 =
                (v[1 + i * 3] + v[1 + (i + 1) * 3]) * (dir * dtau / 2);
            double du3 =
                (v[2 + i * 3] + v[2 + (i + 1) * 3]) * (dir * dtau / 2);
            ...
            if (DEBUG_DRM_INTEGRATION) {
                fprintf(fptr,
                        "    DRMDisplacements(3 * local_pos + 0) = %f \n",
                        DRMDisplacements(3 * local_pos + 0));
                fprintf(fptr,
                        "    DRMDisplacements(3 * local_pos + 1) = %f \n",
                        DRMDisplacements(3 * local_pos + 1));
                fprintf(fptr,
                        "    DRMDisplacements(3 * local_pos + 2) = %f \n",
                        DRMDisplacements(3 * local_pos + 2));
            }
            ...
            if (isnan(u1) || isnan(du1) ||
                isnan(u2) || isnan(du2) ||
                isnan(u3) || isnan(du3) || isnan(dt) || isnan(dtau)) {
                ...
                printf
                    ("        DRMDisplacements(3 * local_pos + 0) = %f -->  v[0][i] = %f  v[0][i+1] = %f\n",
                     DRMDisplacements(3 * local_pos + 0), v[0 + i * 3],
                     v[0 + (i + 1) * 3]);
                printf
                    ("        DRMDisplacements(3 * local_pos + 1) = %f -->  v[1][i] = %f  v[1][i+1] = %f\n",
                     DRMDisplacements(3 * local_pos + 1), v[1 + i * 3],
                     v[1 + (i + 1) * 3]);
                printf
                    ("        DRMDisplacements(3 * local_pos + 2) = %f -->  v[2][i] = %f  v[2][i+1] = %f\n",
                     DRMDisplacements(3 * local_pos + 2), v[2 + i * 3],
                     v[2 + (i + 1) * 3]);
                exit(-1);
            }
        ...
        == == == = for (hsize_t i = 0; i < Nt; ++i) {
            ...
            if (DEBUG_DRM_INTEGRATION) {
                ...
                fprintf(fptr,
                        "    DRMDisplacements(3 * local_pos + 0) = %f -->  v[0][i] = %f  v[0][i+1] = %f\n",
                        DRMDisplacements(3 * local_pos + 0), v[0][i],
                        v[0][i + 1]);
                fprintf(fptr,
                        "    DRMDisplacements(3 * local_pos + 1) = %f -->  v[1][i] = %f  v[1][i+1] = %f\n",
                        DRMDisplacements(3 * local_pos + 1), v[1][i],
                        v[1][i + 1]);
                fprintf(fptr,
                        "    DRMDisplacements(3 * local_pos + 2) = %f -->  v[2][i] = %f  v[2][i+1] = %f\n",
                        DRMDisplacements(3 * local_pos + 2), v[2][i],
                        v[2][i + 1]);
            }
        ...
        DRMAccelerations(3 * local_pos + 0) =
            (v[0][eval_i] - v[0][eval_i + 1]) / dt;
        DRMAccelerations(3 * local_pos + 1) =
            (v[1][eval_i] - v[1][eval_i + 1]) / dt;
        DRMAccelerations(3 * local_pos + 2) =
            (v[2][eval_i] - v[2][eval_i + 1]) / dt;

        if (DEBUG_DRM_INTEGRATION) {
            fprintf(fptr, "eval_i = %d \n", eval_i);
            fprintf(fptr,
                    "     v[0][eval_i] = %f  v[0][eval_i+1] = %f\n",
                    v[0][eval_i], v[0][eval_i + 1]);
            fprintf(fptr,
                    "     v[1][eval_i] = %f  v[1][eval_i+1] = %f\n",
                    v[1][eval_i], v[1][eval_i + 1]);
            fprintf(fptr,
                    "     v[2][eval_i] = %f  v[2][eval_i+1] = %f\n",
                    v[2][eval_i], v[2][eval_i + 1]);
            fprintf(fptr,
                    "     DRMAccelerations(3 * local_pos + 0) = %f\n",
                    DRMAccelerations(3 * local_pos + 0));
            fprintf(fptr,
                    "     DRMAccelerations(3 * local_pos + 1) = %f\n",
                    DRMAccelerations(3 * local_pos + 1));
            fprintf(fptr,
                    "     DRMAccelerations(3 * local_pos + 2) = %f\n",
                    DRMAccelerations(3 * local_pos + 2));
 ...
 H5DRM::ComputeDRMLoads(double t)
{
    ...
    int NDOF = 3;
    int NIEMAX = 8;
    static Vector Fm(NIEMAX * NDOF);
    Fm.Zero();
    static Vector Fk(NIEMAX * NDOF);
    Fk.Zero();
    static Vector u_e(NIEMAX * NDOF);
    u_e.Zero();
    static Vector udd_e(NIEMAX * NDOF);
    udd_e.Zero();
    static Vector v1(NDOF);
    v1.Zero();
    static Vector v2(NDOF);
    v2.Zero();
    static ID B_node(NIEMAX);
    B_node.Zero();
    static ID E_node(NIEMAX);
    E_node.Zero();
        ...
        for (int e = 0; e < Elements.Size(); e++) {
            ...
            if (nB != 0 and nE != 0) {
                ...
                for (int k = 0; k < NIE; k++) {
                    int nodeTag = elementNodes(k);
                    int local_pos = nodetag2local_pos[nodeTag];
                    DRMForces(3 * local_pos + 0) +=
                        Fk(3 * k + 0) + Fm(3 * k + 0);
                    DRMForces(3 * local_pos + 1) +=
                        Fk(3 * k + 1) + Fm(3 * k + 1);
                    DRMForces(3 * local_pos + 2) +=
                        Fk(3 * k + 2) + Fm(3 * k + 2);
...
LoadPattern *H5DRM::getCopy(void)
{
...
Vector *H5DRM::getNodalLoad(int nodeTag, double time)
{
...
void H5DRM::node_matching_UsePlaneInfo(double d_tol, const ID & internal,
                                       const Matrix & xyz,
                                       const Vector & drmbox_x0,
                                       double &d_err, int &n_nodes_found)
{
...
void H5DRM::node_matching_OctTree(double d_tol, const ID & internal,
                                  const Matrix & xyz,
                                  const Vector & drmbox_x0, double &d_err,
                                  int &n_nodes_found)
{
    return;
}
...
void H5DRM::node_matching_BruteForce(double d_tol, const ID & internal,
                                     const Matrix & xyz,
                                     const Vector & drmbox_x0,
                                     double &d_err, int &n_nodes_found)
{
    ...
    while ((node_ptr = node_iter()) != 0) {
        ...
        if (DEBUG_NODE_MATCHING)
            fprintf(fptrdrm, "%d %f %f %f\n", ++drmtag,
                    node_xyz[0] + drmbox_x0[0], node_xyz[1] + drmbox_x0[1],
                    node_xyz[2] + drmbox_x0[2]);
        ...
        if (fabs(dmin) < d_tol) {
            ...
            for (int dir = 0; dir < 3; ++dir)
                station_xyz(dir) = xyz(station_id, dir);
            double this_d_err =
                (station_xyz - node_xyz - drmbox_x0).Norm();
            if (this_d_err > d_err)
                d_err = this_d_err;
            nodetag2station_id.insert(std::pair < int,
                                      int >(tag, station_id));
            nodetag2local_pos.insert(std::pair < int,
                                     int >(tag, local_pos));
            Nodes[local_pos] = tag;
            ...
            if (DEBUG_NODE_MATCHING)
                fprintf(fptrdrm,
                        "Node # %05d @ (%4.2f, %4.2f, %4.2f) rejected \n",
                        tag, node_xyz(0) + drmbox_x0(0),
                        node_xyz(1) + drmbox_x0(1),
                        node_xyz(2) + drmbox_x0(2));
        }
    }
    if (DEBUG_NODE_MATCHING)
...
Vector H5DRM_calculate_cross_product(const Vector & a, const Vector & b)
{
    ...
    if ((a.Size() != 3) || (b.Size() != 3)) {
        opserr <<
            "Error: H5DRM_calculate_cross_product only defined for 3x1 vectors.\n";
        exit(-1);
    }
...
bool
read_int_dataset_into_id(const hid_t & h5drm_dataset,
                         std::string dataset_name, ID & result)
{
    hid_t id_dataset =
        H5Dopen(h5drm_dataset, dataset_name.c_str(), H5P_DEFAULT);
    hid_t id_dataspace = H5Dget_space(id_dataset);
    int ndims = H5Sget_simple_extent_ndims(id_dataspace);
    if (ndims != 1) {
        opserr << "failure trying to open dataset: " <<
            dataset_name.c_str() << endln;
        opserr <<
            "read_int_dataset_into_id - array dimension should be 1\n";
        Vector error(-1);
        return false;
    }
    hsize_t dim;
    ...
    H5Dread(id_dataset, H5T_NATIVE_INT, id_memspace, id_dataspace,
            id_xfer_plist, d);
    result.resize(dim);
...
bool
read_double_dataset_into_vector(const hid_t & h5drm_dataset,
                                std::string dataset_name, Vector & result)
{
    hid_t id_dataset =
        H5Dopen(h5drm_dataset, dataset_name.c_str(), H5P_DEFAULT);
    hid_t id_dataspace = H5Dget_space(id_dataset);
    ...
    if (ndims != 1) {
        opserr << "failure trying to open dataset: " <<
            dataset_name.c_str() << endln;
        opserr <<
            "read_double_dataset_into_vector - array dimension should be 1.\n";
        Vector error(-1);
        return false;
    }
    hsize_t dim;
    ...
    H5Dread(id_dataset, H5T_NATIVE_DOUBLE, id_memspace, id_dataspace,
            id_xfer_plist, d);
    result.resize(dim);
    for (hsize_t i = 0; i < dim; ++i) {
        result(i) = d[i];
...
bool
read_scalar_double_dataset_into_double(const hid_t & h5drm_dataset,
                                       std::string dataset_name,
                                       double &result)
{
    hid_t id_dataset =
    ...
    if (ndims != 0) {
        opserr << "failure trying to open dataset: " <<
            dataset_name.c_str() << endln;
        opserr <<
            "read_scalar_double_dataset_into_double - array dimension should be 0\n";
        Vector error(-1);
        return false;
    }
    H5Sselect_all(id_dataspace);
...
bool
read_double_dataset_into_matrix(const hid_t & h5drm_dataset,
                                std::string dataset_name, Matrix & result)
{
    hid_t id_dataset =
        H5Dopen(h5drm_dataset, dataset_name.c_str(), H5P_DEFAULT);
    hid_t id_dataspace = H5Dget_space(id_dataset);
    ...
    if (ndims != 2) {
        opserr << "failure trying to open dataset: " <<
            dataset_name.c_str() << endln;
        opserr <<
            "read_double_dataset_into_matrix - array dimension should be 2\n";
        Vector error(-1);
        return false;
    }
    hsize_t dim[2];
...
bool
read_int_dataset_into_array(const hid_t & h5drm_dataset,
                            std::string dataset_name, int *&result)
{
    hid_t id_dataset =
        H5Dopen(h5drm_dataset, dataset_name.c_str(), H5P_DEFAULT);
    ...
    if (ndims != 2) {
        opserr << "failure trying to open dataset: " <<
            dataset_name.c_str() << endln;
        opserr <<
            "read_int_dataset_into_array - array dimension should be 2\n";
        Vector error(-1);
        return false;
    }
    hsize_t dim[2];
...
Plane::Plane(const hid_t & id_h5drm_file, int plane_number,
             double crd_scale):number(plane_number),
internal(false), stations(0), v0(3), v1(3), v2(3)
{
    char plane_group_name[H5DRM_MAX_STRINGSIZE];
    sprintf(plane_group_name, "/DRM_Planes/plane_%02d", plane_number);
    hid_t id_plane_grp =
        H5Gopen(id_h5drm_file, plane_group_name, H5P_DEFAULT);
...
bool
 Plane::locate_point(const Vector & x, double &xi1_, double &xi2_,
...
bool Plane::get_ij_coordinates_to_point(double x1, double x2, int &i,
                                        int &j) const
{
    int N1 = xi1.Size();
    int N2 = xi2.Size();
...
int Plane::getStationNumber(int i, int j) const
{
    ...
    if (stations == 0) {
        opserr <<
            "Plane::getStationNumber - stations uninitialized at Plane # "
            << number << "\n";
        return -1;
    } else {
...
void
 Plane::print(FILE * fptr) const
{
    fprintf(fptr,
            "    Plane # %d v0 = (%5.3f, %5.3f, %5.3f) v1 = (%5.3f, %5.3f, %5.3f) v2 = (%5.3f, %5.3f, %5.3f)\n",
            number, v0(0), v0(1), v0(2), v1(0), v1(1), v1(2), v2(0),
            v2(1), v2(2));
}
