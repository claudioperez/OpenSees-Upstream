...
void GeometricBrickDecorator::setDomain(Domain * theDomain)
{
    this->myDomain = theDomain;
}

void GeometricBrickDecorator::clearBrick()
{
    this->myBrick = NULL;
}

void GeometricBrickDecorator::clearDomain()
{
    this->myDomain = NULL;
}

void GeometricBrickDecorator::clearAll()
{
    this->clearBrick();
    this->clearDomain();
}

bool GeometricBrickDecorator::isClear()
{
    return ((this->myBrick == NULL) && (this->myDomain == NULL));
}

bool GeometricBrickDecorator::isEmpty()
{
    return ((this->myBrick == NULL) || (this->myDomain == NULL));
}

bool GeometricBrickDecorator::isPointInVolume(const Vector & pP)
{
    ...
    // must be less than 0
    double bFSV =
        this->evalSignedVol(this->myDomain->getNode(extNodes(0))->
                            getCrds(),
                            this->myDomain->getNode(extNodes(1))->
                            getCrds(),
                            this->myDomain->getNode(extNodes(2))->
                            getCrds(),
                            pP);
    // must be less than 0
    double tFSV =
        this->evalSignedVol(this->myDomain->getNode(extNodes(6))->
                            getCrds(),
                            this->myDomain->getNode(extNodes(5))->
                            getCrds(),
                            this->myDomain->getNode(extNodes(4))->
                            getCrds(),
                            pP);
    // must be less than 0
    double lFSV =
        this->evalSignedVol(this->myDomain->getNode(extNodes(3))->
                            getCrds(),
                            this->myDomain->getNode(extNodes(7))->
                            getCrds(),
                            this->myDomain->getNode(extNodes(4))->
                            getCrds(),
                            pP);
    // must be less than 0 
    double rFSV =
        this->evalSignedVol(this->myDomain->getNode(extNodes(2))->
                            getCrds(),
                            this->myDomain->getNode(extNodes(1))->
                            getCrds(),
                            this->myDomain->getNode(extNodes(5))->
                            getCrds(),
                            pP);
    if ((bFSV < 0) && (tFSV < 0) && (lFSV < 0) && (rFSV < 0)) {
        return true;
    }
...
double
    GeometricBrickDecorator::evalSignedVol(const Vector & pA,
                                           ...
                                           const Vector & pP)
{
    return (1.0 / 6.0 * (-this->eval3OrderDet(pB, pC, pP)
...
double
 GeometricBrickDecorator::eval3OrderDet(const Vector & pA,
                                        ...
                                        const Vector & pC)
{
    return (pA(0) * pB(1) * pC(2) +
            pA(1) * pB(2) * pC(0) +
            pA(2) * pB(0) * pC(1) -
            pA(0) * pB(2) * pC(1) -
            pA(1) * pB(0) * pC(2) - pA(2) * pB(1) * pC(0));
}

void GeometricBrickDecorator::getFace(int which, ID & face, ID & faceID)
{
    const ID extNodes = this->myBrick->getExternalNodes();
    /*
       which == 1 -> bottom
...
double GeometricBrickDecorator::getMinMaxCrds(int which, int whichtoret)
{
    /* 
       which goes from 1-> 3
     */
    ...
    default:
        std::cout <<
            " ERROR ERROR ERROR in geometric brick decorator L.252 \n";
        ret = 0.0;
    }
...
bool GeometricBrickDecorator::compareFaceToFace(int which, ID & faceOther)
{
    ID faceID(4);
    ID myFace(4);
    ...
    return ((myFace(0) == faceOther(0)) &&
            ...
            (myFace(2) == faceOther(2)) && (myFace(3) == faceOther(3)));
}

bool GeometricBrickDecorator::isFaceinPlane(int which, const Vector & pP)
{
    /*
...
bool GeometricBrickDecorator::isFaceinVertPlane(int which, double xy,
...
bool GeometricBrickDecorator::isLeftBoundary(double xMin, double xMax,
                                             double yMin, double yMax,
                                             double zMin, double zMax)
{
    return (isFaceinVertPlane(3, xMin, zMin, zMax, 3) &&
            isFaceinVertPlane(3, xMin, yMin, yMax, 2));
}

bool GeometricBrickDecorator::isRightBoundary(double xMin, double xMax,
                                              double yMin, double yMax,
                                              double zMin, double zMax)
{
    return (isFaceinVertPlane(4, xMax, zMin, zMax, 3) &&
            isFaceinVertPlane(4, xMax, yMin, yMax, 2));
}

bool GeometricBrickDecorator::isFrontBoundary(double xMin, double xMax,
                                              double yMin, double yMax,
                                              double zMin, double zMax)
{
    return (isFaceinVertPlane(5, yMin, zMin, zMax, 3) &&
            isFaceinVertPlane(5, yMin, xMin, xMax, 1));
}

bool GeometricBrickDecorator::isRearBoundary(double xMin, double xMax,
                                             double yMin, double yMax,
                                             double zMin, double zMax)
{
    return (isFaceinVertPlane(6, yMax, zMin, zMax, 3) &&
            isFaceinVertPlane(6, yMax, xMin, xMax, 1));
}

bool GeometricBrickDecorator::isBottomBoundary(double xMin, double xMax,
                                               double yMin, double yMax,
                                               double zMin, double zMax)
{
    return (isFaceinVertPlane(1, zMin, xMin, xMax, 1) &&
            isFaceinVertPlane(1, zMin, yMin, yMax, 2));
}

bool GeometricBrickDecorator::isBoundaryLayerEle(double xMin, double xMax,
                                                 double yMin, double yMax,
                                                 double zMin, double zMax)
{
