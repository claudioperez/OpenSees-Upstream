...
Vector
PlaneDRMInputHandler::Vtm2 (24);
Vector
PlaneDRMInputHandler::Vtm1 (24);
Vector
PlaneDRMInputHandler::Vtp1 (24);
Vector
PlaneDRMInputHandler::Vtp2 (24);


Vector
PlaneDRMInputHandler::Vtm2_n1 (3);
Vector
PlaneDRMInputHandler::Vtm2_n2 (3);
Vector
PlaneDRMInputHandler::Vtm2_n3 (3);
Vector
PlaneDRMInputHandler::Vtm2_n4 (3);

Vector
PlaneDRMInputHandler::Vtm1_n1 (3);
Vector
PlaneDRMInputHandler::Vtm1_n2 (3);
Vector
PlaneDRMInputHandler::Vtm1_n3 (3);
Vector
PlaneDRMInputHandler::Vtm1_n4 (3);

Vector
PlaneDRMInputHandler::Vtp1_n1 (3);
Vector
PlaneDRMInputHandler::Vtp1_n2 (3);
Vector
PlaneDRMInputHandler::Vtp1_n3 (3);
Vector
PlaneDRMInputHandler::Vtp1_n4 (3);

Vector
PlaneDRMInputHandler::Vtp2_n1 (3);
Vector
PlaneDRMInputHandler::Vtp2_n2 (3);
Vector
PlaneDRMInputHandler::Vtp2_n3 (3);
Vector
PlaneDRMInputHandler::Vtp2_n4 (3);

Vector
PlaneDRMInputHandler::Vtempm2 (3);
Vector
PlaneDRMInputHandler::Vtempm1 (3);
Vector
PlaneDRMInputHandler::Vtempp1 (3);
Vector
PlaneDRMInputHandler::Vtempp2 (3);





PlaneDRMInputHandler::PlaneDRMInputHandler (int tag, char **in_files,
                                            int files, double dt,
                                            double *time_array, int num_steps,
                                            ...
                                            int steps_to_cache,
                                            Domain * domain):
DRMInputHandler (tag, in_files, files, dt, time_array, num_steps, file_data,
                 fileData_size, domain_crds, drm_box_crds, my_mesher)
{
    /*
     *  Current implementations assumes num_steps +1 = total actual time steps,
    ...
    this->f1buffer =
        new double[3 * (this->cacheValue + 4) * this->fileData[0]];
    this->f2buffer =
        new double[3 * (this->cacheValue + 4) * this->fileData[3]];
    this->f3buffer =
        new double[3 * (this->cacheValue + 4) * this->fileData[6]];
    this->f4buffer =
        new double[3 * (this->cacheValue + 4) * this->fileData[9]];
    this->f5buffer =
        new double[3 * (this->cacheValue + 4) * this->fileData[12]];
    ...
    if (f1buffer == 0 || f2buffer == 0 || f3buffer == 0 || f4buffer == 0
        || f5buffer == 0)
      {
          opserr <<
              "Error in memory allocations for DRM Load pattern, try smaller caching "
              << endln;
          if (f1buffer != 0)
              delete[]f1buffer;
          if (f2buffer != 0)
...
void
PlaneDRMInputHandler::populateBuffers ()
{
    int pid;
    bool debug = false;
    ...
    if (this->initial)
      {
          ...
          //    for (int i=3*temp; i<3*(cacheValue + 3)*temp; i++) { 
          for (int k = 1; k < (cacheValue + 4); k++)
            {
                for (int i = 0; i < 3 * 15000; i++)
                  {
                      ifile5a >> dataIn;
                      f5buffer[index++] = dataIn;
                  }
                for (int i = 0; i < 3 * 16626; i++)
                  {
                      ifile5b >> dataIn;
                      f5buffer[index++] = dataIn;
                  }
          ...
          for (int i = 0; i < 3 * temp; i++)
            {
                ...
                f1buffer[i + 3 * temp] =
                    f1buffer[i + 3 * (cacheValue + 2) * temp];
                f1buffer[i + 6 * temp] =
                    f1buffer[i + 3 * (cacheValue + 3) * temp];
            }
          double dataIn = 0.0;
          ...
          for (int i = 0; i < 3 * temp; i++)
            {
                ...
                f2buffer[i + 3 * temp] =
                    f2buffer[i + 3 * (cacheValue + 2) * temp];
                f2buffer[i + 6 * temp] =
                    f2buffer[i + 3 * (cacheValue + 3) * temp];
            }
          dataIn = 0.0;
          for (int i = 9 * temp; i < 3 * (rem + 4) * temp; i++)
            {
          ...
          for (int i = 0; i < 3 * temp; i++)
            {
                ...
                f3buffer[i + 3 * temp] =
                    f3buffer[i + 3 * (cacheValue + 2) * temp];
                f3buffer[i + 6 * temp] =
                    f3buffer[i + 3 * (cacheValue + 3) * temp];
            }
          dataIn = 0.0;
          for (int i = 9 * temp; i < 3 * (rem + 4) * temp; i++)
            {
          ...
          for (int i = 0; i < 3 * temp; i++)
            {
                ...
                f4buffer[i + 3 * temp] =
                    f4buffer[i + 3 * (cacheValue + 2) * temp];
                f4buffer[i + 6 * temp] =
                    f4buffer[i + 3 * (cacheValue + 3) * temp];
            }
          dataIn = 0.0;
          for (int i = 9 * temp; i < 3 * (rem + 4) * temp; i++)
            {
          ...
          for (int i = 0; i < 3 * temp; i++)
            {
                ...
                f5buffer[i + 3 * temp] =
                    f5buffer[i + 3 * (cacheValue + 2) * temp];
                f5buffer[i + 6 * temp] =
                    f5buffer[i + 3 * (cacheValue + 3) * temp];
            }
          dataIn = 0.0;
          int index = 9 * temp;
          //need some change here for two files
          //for (int i=6*temp; i<3*(rem + 3)*temp; i++) {
          for (int k = 2; k < (cacheValue + 4); k++)
            {
                for (int i = 0; i < 3 * 15000; i++)
                  {
                      ifile5a >> dataIn;
...
void
PlaneDRMInputHandler::getMotions (Element * eletag, double time, Vector & U,
                                  Vector & Ud, Vector & Udd)
{
    ...
    int result = 0;
    if (this->myDecorator->
        isLeftBoundary (xMin, xMax, yMin, yMax, zMin, zMax))
        result = 3;
    if (this->myDecorator->
...
void
PlaneDRMInputHandler::computeHistory (Element * eletag, double time,
                                      Vector & U, Vector & Ud, Vector & Udd,
                                      bool updateDm1)
{
    // account for units of input versus model units cm -> m
    double scaleFactor = 0.01;
    double 1 o2dt = 0.5 / deltaT;
    ...
    Udd = (1.0 - mt) * 1 o2dt * (Vtp1 - Vtm2) + mt * 1 o2dt * (Vtp2 - Vtm1);

    // trapezoidal rule for the displacements
    Vector & Dm1 = ((FLBrick *) eletag)->getDtm1 ();
    if (updateDm1)
    ...
    /////////////////////////////////////////////////////////////////////////////////// up to here fixed fine!!!

void
PlaneDRMInputHandler::handle_elementAtface5 (Element * eletag, double time,
                                             Vector & U, Vector & Ud,
                                             Vector & Udd)
{
    ...
    int lastIndex = ((FLBrick *) eletag)->getLastDRMIndex ();
    if (index != lastIndex)
      {
          updateDm1 = true;
          ((FLBrick *) eletag)->setLastDRMIndex (index);
      }
...
void
PlaneDRMInputHandler::handle_elementAtface1 (Element * eletag, double time,
                                             Vector & U, Vector & Ud,
                                             Vector & Udd)
{
    bool updateDm1 = false;
...
void
PlaneDRMInputHandler::handle_elementAtface2 (Element * eletag, double time,
                                             Vector & U, Vector & Ud,
                                             Vector & Udd)
{
    bool updateDm1 = false;
...
void
PlaneDRMInputHandler::handle_elementAtface3 (Element * eletag, double time,
                                             Vector & U, Vector & Ud,
                                             Vector & Udd)
{
    bool updateDm1 = false;
...
void
PlaneDRMInputHandler::handle_elementAtface4 (Element * eletag, double time,
                                             Vector & U, Vector & Ud,
                                             Vector & Udd)
{
    bool updateDm1 = false;
...
void
PlaneDRMInputHandler::getLocations (double x, double y, double dx, double dy,
                                    int *xloc, int *yloc)
{
    *xloc = floor (x / dx);
    *yloc = floor (y / dy);
...
void
PlaneDRMInputHandler::getTemporal (double time, int *tloc)
{
    *tloc = floor (time / deltaT);
}

int
PlaneDRMInputHandler::getIndex (double time)
{
    // kill roundoffs
    double tmptime = time + 0.00000001;
    ...
    if ((index < 1) || (index > cacheValue + 1))
      {
          opserr << " Severe error aborting tasks index is: " << index <<
              " gc is: " << globalCounter << "\n";
          return -1;
      }
    return index - 1;
...
void
PlaneDRMInputHandler::getf5pointer (Node * node_tag, int local_tag, int index)
{
...
void
PlaneDRMInputHandler::getf1pointer (Node * node_tag, int local_tag, int index)
{
...
void
PlaneDRMInputHandler::getf2pointer (Node * node_tag, int local_tag, int index)
{
    const Vector & crd = node_tag->getCrds ();
    double x = crd (0);
    double y = crd (1);
...
void
PlaneDRMInputHandler::getf3pointer (Node * node_tag, int local_tag, int index)
{
    const Vector & crd = node_tag->getCrds ();
    double x = crd (0);
    double y = crd (1);
...
void
PlaneDRMInputHandler::getf4pointer (Node * node_tag, int local_tag, int index)
{
    bool debug = false;
    const Vector & crd = node_tag->getCrds ();
    double x = crd (0);
...
void
PlaneDRMInputHandler::pointerCopy (int node_from, int node_to)
{
...
void
PlaneDRMInputHandler::populateTempBuffers (int index, int fileptr, double ksi,
                                           double eta)
{
    //num nodes in file
    ...
    //finally perform spatial interpolation @ the node for the three time steps
    Vtempm2 =
        0.25 * (1 - ksi) * (1 - eta) * Vtm2_n1 + 0.25 * (1 + ksi) * (1 -
                                                                     eta) *
        Vtm2_n2 + 0.25 * (1 + ksi) * (1 + eta) * Vtm2_n3 + 0.25 * (1 -
                                                                   ksi) * (1 +
                                                                           eta)
        * Vtm2_n4;
    Vtempm1 =
        0.25 * (1 - ksi) * (1 - eta) * Vtm1_n1 + 0.25 * (1 + ksi) * (1 -
                                                                     eta) *
        Vtm1_n2 + 0.25 * (1 + ksi) * (1 + eta) * Vtm1_n3 + 0.25 * (1 -
                                                                   ksi) * (1 +
                                                                           eta)
        * Vtm1_n4;
    Vtempp1 =
        0.25 * (1 - ksi) * (1 - eta) * Vtp1_n1 + 0.25 * (1 + ksi) * (1 -
                                                                     eta) *
        Vtp1_n2 + 0.25 * (1 + ksi) * (1 + eta) * Vtp1_n3 + 0.25 * (1 -
                                                                   ksi) * (1 +
                                                                           eta)
        * Vtp1_n4;
    Vtempp2 =
        0.25 * (1 - ksi) * (1 - eta) * Vtp2_n1 + 0.25 * (1 + ksi) * (1 -
                                                                     eta) *
        Vtp2_n2 + 0.25 * (1 + ksi) * (1 + eta) * Vtp2_n3 + 0.25 * (1 -
                                                                   ksi) * (1 +
                                                                           eta)
        * Vtp2_n4;
