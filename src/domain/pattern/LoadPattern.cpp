...
LoadPattern::LoadPattern(int tag, int clasTag,
                         double fact):DomainComponent(tag, clasTag),
isConstant(1), loadFactor(0), scaleFactor(fact), theSeries(0),
currentGeoTag(0), lastGeoSendTag(-1), theNodalLoads(0),
theElementalLoads(0), theSPs(0), theNodIter(0), theEleIter(0),
theSpIter(0), lastChannel(0)
{
...
LoadPattern::LoadPattern():DomainComponent(0, PATTERN_TAG_LoadPattern),
...
LoadPattern::LoadPattern(int tag, double fact):DomainComponent(tag,
                                                               PATTERN_TAG_LoadPattern),
isConstant(1), loadFactor(0.), scaleFactor(fact), theSeries(0),
currentGeoTag(0), lastGeoSendTag(-1), dbSPs(0), dbNod(0), dbEle(0),
...
bool LoadPattern::addSP_Constraint(SP_Constraint * theSp)
{
    ...
    if (result == true) {
        ...
        opserr <<
            "WARNING: LoadPattern::addSP_Constraint() - load could not be added\n";
    return result;
}
...
void LoadPattern::setLoadConstant(void)
{
    isConstant = 0;
}


void LoadPattern::unsetLoadConstant(void)
{
    isConstant = 1;
}
...
int LoadPattern::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (theChannel.sendID(myDbTag, cTag, lpData) < 0) {
        opserr <<
            "LoadPattern::sendSelf - channel failed to send the initial ID\n";
        return -1;
    }
    ...
    if (theChannel.sendVector(myDbTag, cTag, data) < 0) {
        opserr <<
            "LoadPattern::sendSelf - channel failed to send the Vector\n";
        return -2;
    ...
    if (theSeries != 0)
        if (theSeries->sendSelf(cTag, theChannel) < 0) {
            opserr <<
                "LoadPattern::sendSelf - the TimeSeries failed to send\n";
            return -3;
        }
    ...
    if (lastChannel != theChannel.getTag()
        ...
        if (numNodLd != 0) {
            ...
            if (theChannel.sendID(dbNod, currentGeoTag, nodeData) < 0) {
                opserr <<
                    "LoadPattern::sendSelf - channel failed to send the NodalLoads ID\n";
                return -4;
            }
        }
    ...
    while ((theNode = theNodes()) != 0) {
        if (theNode->sendSelf(cTag, theChannel) < 0) {
            opserr << "LoadPattern::sendSelf - node with tag " <<
                theNode->getTag() << " failed in sendSelf\n";
            return -7;
        }
    }
    ...
    while ((theEle = theElements()) != 0) {
        if (theEle->sendSelf(cTag, theChannel) < 0) {
            opserr << "LoadPattern::sendSelf - element with tag " <<
                          ...
                          FEM_ObjectBroker & theBroker)
{
    ...
    if (theChannel.recvID(myDbTag, cTag, lpData) < 0) {
        opserr <<
            "LoadPattern::recvSelf - channel failed to recv the initial ID\n";
        return -1;
    }
    ...
    if (theChannel.recvVector(myDbTag, cTag, data) < 0) {
        opserr <<
            "LoadPattern::recvSelf - channel failed to recv the Vector\n";
        return -2;
    }
    loadFactor = data(0);
    scaleFactor = data(1);
    ...
    if (lpData(8) != -1) {
        ...
        if (theSeries == 0) {
            opserr <<
                "LoadPattern::recvSelf - failed to create TimeSeries\n";
            return -3;
        }
        ...
        if (theSeries->recvSelf(cTag, theChannel, theBroker) < 0) {
            opserr <<
                "LoadPattern::recvSelf - the TimeSeries failed to recv\n";
            return -3;
        }
    }
    ...
    if (lastChannel != theChannel.getTag() || currentGeoTag != lpData(0)
        ...
        if (numNod != 0) {
            ...
            if (theChannel.recvID(dbNod, currentGeoTag, nodeData) < 0) {
                opserr <<
                    "LoadPAttern::recvSelf - channel failed to recv the NodalLoad ID\n";
                return -2;
            }
            ...
            for (int i = 0; i < numNod; i++) {
                ...
                if (theNode == 0) {
                    opserr <<
                        "LoadPattern::recv - cannot create NodalLoad with classTag "
                        << classTag << endln;
                    return -2;
                }
                ...
                if (theNode->recvSelf(cTag, theChannel, theBroker) < 0) {
                    opserr <<
                        "LoadPattern::recvSelf - NodalLoad with dbTag "
                ...
                if (this->addNodalLoad(theNode) == false) {
                    opserr <<
                        "LoadPattern::recvSelf - failed adding NodalLoad tagged "
        ...
        if (numEle != 0) {
            ...
            for (int i = 0; i < numEle; i++) {
                ...
                ElementalLoad *theEle =
                    theBroker.getNewElementalLoad(classTag);
                if (theEle == 0) {
                    opserr <<
                        "LoadPattern::recv - cannot create ElementalLoad with classTag "
                        << classTag << endln;
                    return -2;
                }
                ...
                if (theEle->recvSelf(cTag, theChannel, theBroker) < 0) {
                    opserr <<
                        "LoadPattern::recvSelf - Ele with dbTag " <<
                        dbTag << " failed in recvSelf\n";
                    return -2;
                }
                ...
                if (this->addElementalLoad(theEle) == false) {
                    opserr <<
                        "LoadPattern::recvSelf - could not add Ele with tag "
        ...
        if (numSPs != 0) {
            ...
            if (theChannel.recvID(dbSPs, currentGeoTag, spData) < 0) {
                opserr <<
                    "LoadPattern::recvSelf - channel failed to recv the SP_Constraints ID\n";
                return -2;
            }
            ...
            for (int i = 0; i < numSPs; i++) {
                ...
                if (theSP == 0) {
                    opserr <<
                        "LoadPattern::recv - cannot create SP_Constraint with classTag "
                        << classTag << endln;
                    return -2;
                }
                theSP->setDbTag(dbTag);
                ...
                if (theSP->recvSelf(cTag, theChannel, theBroker) < 0) {
                    opserr <<
                        "LoadPattern::recvSelf - SP_Constraint with dbTag "
                        << dbTag << " failed in recvSelf\n";
                    return -2;
                }
        ...
        while ((theNode = theNodes()) != 0) {
            if (theNode->recvSelf(cTag, theChannel, theBroker) < 0) {
                opserr << "LoadPattern::recvSelf - node with tag " <<
        ...
        while ((theSP = theSPs()) != 0) {
            if (theSP->recvSelf(cTag, theChannel, theBroker) < 0) {
                opserr <<
                    "LoadPattern::recvSelf - SP_Constraint tagged " <<
...
{
    opserr <<
        "LoadPattern::addMotion() - cannot add GroundMotion - use MultiSupport Pattern instead\n";
    return -1;
}

GroundMotion *LoadPattern::getMotion(int tag)
{
    return 0;
}
...
int LoadPattern::setParameter(const char **argv, int argc,
                              Parameter & param)
{
    if (theSeries == 0) {
        opserr <<
            "set/update/activate parameter is illegaly called in LoadPattern "
            << endln;
        return 0;
    }
...
int LoadPattern::updateParameter(int parameterID, Information & info)
{
    ...
    opserr <<
        "LoadPattern::updateParameter -- no parameters defined, this method should not be called"
        << endln;
...
int LoadPattern::activateParameter(int parameterID)
{
    ...
    opserr <<
        "LoadPattern::activateParameter -- no parameters defined, this method should not be called"
        << endln;
