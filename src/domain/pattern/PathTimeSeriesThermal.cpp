...
PathTimeSeriesThermal::PathTimeSeriesThermal():TimeSeries(TSERIES_TAG_PathTimeSeriesThermal), CurrentFactors(0),
...
PathTimeSeriesThermal::PathTimeSeriesThermal(int tag,
                                             ...
                                             double
                                             theFactor):TimeSeries(tag,
                                                                   TSERIES_TAG_PathTimeSeriesThermal),
CurrentFactors(0), thePath(0), time(0), currentTimeLoc(0),
numCols(DataNum), numRows(0), cFactor(theFactor), dbTag1(0), dbTag2(0),
lastChannel(0), TempOut(tempOut)
{
    ...
    if (theFile.bad() || !theFile.is_open()) {
        opserr <<
            "WARNING - PathTimeSeriesThermal::PathTimeSeriesThermal()";
    ...
    if ((numDataPoints % (numCols + 1)) != 0) {
        opserr <<
            "WARNING - PathTimeSeriesThermal::PathTimeSeriesThermal()";
        opserr << " - num data entries in file NOT COMPATIBLE! " <<
            fileName << endln;
    }
    ...
    if (numRows != 0) {
        ...
        if (thePath == 0 || thePath->noCols() == 0
            || thePath->noRows() == 0 || time == 0 || time->Size() == 0) {

            opserr <<
                "WARNING PathTimeSeriesThermal::PathTimeSeriesThermal() - out of memory\n ";
            if (thePath != 0)
                delete thePath;
            if (time != 0)
                delete time;
...
int
 PathTimeSeriesThermal::WriteResults(double currentTime,
                                     const Vector & newData)
{
    numRows++;
    ...
    if (newData == 0 || newData.Size() < numCols) {
        opserr <<
            "WARNING::PathTimeSeriesThermal received incompatible data when attempring to write the results"
            << endln;
        return -1;
    }
...
TimeSeries *PathTimeSeriesThermal::getCopy(void)
{
    opserr << "PathTimeSeriesThermal::getCopy() should not be called" <<
        endln;
    return new PathTimeSeriesThermal();
}


double PathTimeSeriesThermal::getTimeIncr(double pseudoTime)
{
    // NEED TO FILL IN, FOR NOW return 1.0
    return 1.0;
}

const Vector & PathTimeSeriesThermal::getFactors(double pseudoTime)
{
    ...
    if (pseudoTime == time1) {
        for (int j = 0; j < numCols; j++) {
            (*CurrentFactors) (j) =
                cFactor * (*thePath) (currentTimeLoc, j);
        }
    ...
    if (pseudoTime < time1 && currentTimeLoc == 0) {
        for (int j = 0; j < numCols; j++) {
            (*CurrentFactors) (j) =
                (*thePath) (currentTimeLoc, j) * pseudoTime / time1;
        }
        return *CurrentFactors;
    }
    ...
    for (int j = 0; j < numCols; j++) {
        ...
        (*CurrentFactors) (j) =
            cFactor * (value1 +
                       (value2 - value1) * (pseudoTime - time1) / (time2 -
                                                                   time1));
    }
    return *CurrentFactors;
}

double PathTimeSeriesThermal::getDuration()
{
    if (thePath == 0) {
        opserr <<
            "WARNING -- PathTimeSeriesThermal::getDuration() on empty Vector"
            << endln;
        return 0.0;
    }
...
double PathTimeSeriesThermal::getPeakFactor()
{
    if (thePath == 0) {
        opserr <<
            "WARNING -- PathTimeSeriesThermal::getPeakFactor() on empty Vector"
            << endln;
        return 0.0;
    }
...
int PathTimeSeriesThermal::sendSelf(int commitTag, Channel & theChannel)
{
    ...
    if (result < 0) {
        opserr <<
            "PathTimeSeriesThermal::sendSelf() - channel failed to send data\n";
        return result;
    }
    ...
    if (lastChannel != &theChannel || (lastSendCommitTag == commitTag)
        ...
        if (thePath != 0) {
            opserr <<
                "Remote access to PathTimeSeriesThermal(sendSelf) is deactivated by UoE Group"
                << endln;
            /*     
...
int PathTimeSeriesThermal::recvSelf(int commitTag, Channel & theChannel,
                                    FEM_ObjectBroker & theBroker)
{
    ...
    if (result < 0) {
        opserr <<
            "PathTimeSeriesThermal::sendSelf() - channel failed to receive data\n";
        cFactor = 1.0;
        return result;
    }
    cFactor = data(0);
...
void PathTimeSeriesThermal::Print(OPS_Stream & s, int flag)
{
