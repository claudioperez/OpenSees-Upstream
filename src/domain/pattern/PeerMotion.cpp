...
void *OPS_PeerMotion(void)
{
    ...
    if (numRemainingArgs < 4) {
        opserr <<
            "WARNING: invalid num args PeerMotion <tag?> $eqMotion $station $type $factor\n";
        return 0;
    }
    ...
    // get tag if provided
    if (numRemainingArgs == 5 || numRemainingArgs == 7
        || numRemainingArgs == 9) {
        numData = 1;
        if (OPS_GetIntInput(&numData, &tag) != 0) {
            opserr << "WARNING invalid series tag in Constant tag?" <<
                endln;
            return 0;
        }
        numRemainingArgs -= 1;
    ...
    if ((OPS_GetStringCopy(&eqMotion) != 0) || eqMotion == 0) {
        opserr << "WARNING invalid eqMotion for PeerMotion with tag: " <<
            tag << endln;
        return 0;
    }

    if ((OPS_GetStringCopy(&station) != 0) || station == 0) {
        opserr << "WARNING invalid station for PeerMotion with tag: " <<
            tag << endln;
        return 0;
    }

    if ((OPS_GetStringCopy(&type) != 0) || type == 0) {
        opserr << "WARNING invalid type  for PeerMotion with tag: " << tag
            << endln;
        return 0;
    }
    ...
    if (OPS_GetDouble(&numData, &factor) != 0) {
        opserr << "WARNING invalid facor in PeerMotion Series with tag?" <<
            tag << endln;
        return 0;
    }
    ...
    if (theSeries == 0) {
        opserr <<
            "WARNING ran out of memory creating PeerMotion with tag: " <<
            tag << "\n";
        return 0;
    }
...
PeerMotion::PeerMotion():TimeSeries(TSERIES_TAG_PeerMotion),
thePath(0), dT(0.0),
...
PeerMotion::PeerMotion(int tag,
                       ...
                       const char *station,
                       const char *type, double theFactor):TimeSeries(tag,
                                                                      TSERIES_TAG_PeerMotion),
thePath(0), dT(0.0), cFactor(theFactor), dbTag1(0), dbTag2(0),
lastSendCommitTag(-1), lastChannel(0)
{
    char peerPage[124];
    ...
    if (earthquake != 0 && station != 0 && type != 0) {
        ...
        if ((strcmp(type, "ACCEL") == 0) || (strcmp(type, "-accel") == 0)
            || (strcmp(type, "-ACCEL") == 0)
            || (strcmp(type, "accel") == 0) || (strcmp(type, "ATH") == 0)
            || (strcmp(type, "-ATH") == 0)) {
            sprintf(peerPage, "/smcat/data/ath/%s/%s.AT2", earthquake,
                    station);
                   ...
                   || (strcmp(type, "DTH") == 0)
                   || (strcmp(type, "-DTH") == 0)) {
            sprintf(peerPage, "/smcat/data/dth/%s/%s.DT2", earthquake,
                    station);
        } else {
            opserr << "PeerMotion::PeerMotion() - not a valid type:" <<
                type << " (-DISP or -ACCEL requiured)\n";
            return;
        }
        ...
        if (nextData != 0) {
            opserr <<
                "PeerMotion::PeerMotion() - could not get Data for record from Database, ";
        ...
        if (nextData == NULL) {
            ...
            if (nextData == NULL) {
                opserr <<
                    "PeerMotion::PeerMotion() - could not find dt in record, send email opensees-support@berkeley.edu";
                free(eqData);
                return;
            }
        }
...
PeerMotion::PeerMotion(int tag,
                       ...
                       double theTimeStep,
                       double theFactor):TimeSeries(tag,
                                                    TSERIES_TAG_PeerMotion),
thePath(0), dT(theTimeStep), cFactor(theFactor), dbTag1(0), dbTag2(0),
lastSendCommitTag(-1), lastChannel(0)
{
    if (theDataPoints != 0)
...
TimeSeries *PeerMotion::getCopy(void)
{
...
double PeerMotion::getFactor(double pseudoTime)
{
    ...
    return cFactor * (value1 +
                      (value2 - value1) * (pseudoTime / dT - incr1));
}

double PeerMotion::getDuration()
{
    if (thePath == 0) {
        opserr << "WARNING -- PeerMotion::getDuration() on empty Vector" <<
            endln;
        return 0.0;
    }
...
double PeerMotion::getPeakFactor()
{
    if (thePath == 0) {
        opserr << "WARNING -- PeerMotion::getPeakFactor() on empty Vector"
            << endln;
        return 0.0;
    }
...
double PeerMotion::getDt()
{
    return dT;
}

int PeerMotion::getNPts()
{
    if (thePath == 0)
        return 0;
    else
...
int PeerMotion::sendSelf(int commitTag, Channel & theChannel)
{
    ...
    if ((lastSendCommitTag == commitTag)
        || (theChannel.isDatastore() == 0)) {
        if (thePath != 0) {
            result =
                theChannel.sendVector(otherDbTag, commitTag, *thePath);
            if (result < 0) {
...
int PeerMotion::recvSelf(int commitTag, Channel & theChannel,
                         FEM_ObjectBroker & theBroker)
{
    ...
    if (result < 0) {
        opserr <<
            "PeerMotion::sendSelf() - channel failed to receive data\n";
        cFactor = 1.0;
        return result;
    }
    ...
    if (thePath == 0 && size > 0) {
        ...
        result =
            theChannel.recvVector(otherDbTag, lastSendCommitTag, *thePath);
        if (result < 0) {
...
void PeerMotion::Print(OPS_Stream & s, int flag)
{
