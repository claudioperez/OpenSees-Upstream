             ...
             (strcmp (argv[0], "Path") == 0))
      {
          ...
          if (filePathName != 0 && fileTimeName == 0 && timeIncr != 0.0)
          ...
          else if (fileName != 0)
            {
                ...
                theSeries =
                    new PathTimeSeries (tag, argv[fileName], cFactor,
                                        useLast);
                ...
                while (argCount + 1 < argc)
                  {
                      if ((strcmp (argv[argCount], "-dT") == 0)
                          ...
                          || (strcmp (argv[argCount], "-DT") == 0))
                        {
                            ...
                            char string[30];
                            sprintf (string, "set %s %.18e", variableName,
                                     dT);
          ...
          for (int i = 0; i < numModFuncs; i++)
            {
                ...
                theModFunc = 0;
                theModFunc =
                    theReliabilityDomain->getModulatingFunction (tagI);

                if (theModFunc == 0)
                  {
                      opserr << "WARNING modulating function number " <<
                          argv[i +
                               argsBeforeModList] << "does not exist...\n";
                      delete[]theModFUNCS;
                      return 0;
                  }
                else
