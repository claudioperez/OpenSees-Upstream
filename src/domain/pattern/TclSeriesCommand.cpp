...
TimeSeries *TclTimeSeriesCommand(ClientData clientData,
                                 Tcl_Interp * interp,
                                 ...
                                 Domain * theDomain)
{
    ...
    if ((strcmp(argv[0], "Constant") == 0)
        ...
        while (endMarker < argc && done == false) {
            ...
            if (strcmp(argv[endMarker], "-dt") == 0) {
                ...
                endMarker++;
                if (endMarker == argc ||
                    Tcl_GetDouble(interp, argv[endMarker],
                                  &timeIncr) != TCL_OK) {
                ...
                if (endMarker != argc) {
                    ...
                    if (Tcl_SplitList(interp, argv[endMarker],
                        ...
                        opserr <<
                            "WARNING problem splitting path list "
                            << argv[endMarker] << " - ";
                    ...
                    for (int i = 0; i < pathSize; i++) {
                        double value;
                        if (Tcl_GetDouble
                            (interp, pathStrings[i], &value) != TCL_OK) {
                            opserr <<
                                "WARNING problem reading path data value "
                                << pathStrings[i] << " - ";
                            opserr << " Series -values {path} ... \n";
                            cleanup(pathStrings);
                            return 0;
                ...
                if (endMarker != argc) {
                    ...
                    if (Tcl_SplitList(interp, argv[endMarker],
                        ...
                        opserr <<
                            "WARNING problem spltting time path " <<
                            argv[endMarker] << " - ";
                    ...
                    for (int i = 0; i < pathSize; i++) {
                        ...
                        if (Tcl_GetDouble
                            (interp, pathStrings[i], &value) != TCL_OK) {
                            opserr <<
                                "WARNING problem reading time path value "
                                << pathStrings[i] << " - ";
                            opserr << " Series -values {path} ... \n";
        ...
        if (filePathName != 0 && fileTimeName == 0 && timeIncr != 0.0) {
            ...
            theSeries =
                new PathSeries(tag, argv[filePathName], timeIncr,
                               cFactor, useLast, prependZero, startTime);
        }
            ...
            theSeries =
                new PathTimeSeries(tag, argv[fileName], cFactor, useLast);
            ...
            theSeries =
                new PathTimeSeries(tag, argv[filePathName],
            ...
            theSeries =
                new PathTimeSeries(tag, *dataPath, *dataTime, cFactor,
                                   useLast);
            delete dataPath;
            delete dataTime;
            ...
            opserr <<
                "WARNING choice of options for Path Series invalid - valid options for ";
            opserr << " Path are\n";
            opserr << " \t -fileT fileTimeName -fileP filePathName \n";
            opserr << " \t -dt constTimeIncr -file filePathName\n";
            opserr <<
                " \t -dt constTimeIncr -values {list of points on path}\n";
            opserr <<
                " \t -time {list of time points} -values {list of points on path}\n";
            return 0;
        }
        ...
        ModulatingFunction **theModFUNCS =
            new ModulatingFunction *[numModFuncs];

        // For each modulating function, get the tag and ensure it exists
        int tagI;
        for (int i = 0; i < numModFuncs; i++) {
            if (Tcl_GetInt(interp, argv[i + argsBeforeModList], &tagI) !=
                TCL_OK) {
                opserr << "WARNING invalid modulating function tag. " <<
                    endln;
                return 0;
            }
        ...
        theSeries =
            new DiscretizedRandomProcessSeries(0, numModFuncs, theModFUNCS,
                                               mean, maxStdv);
    }
    ///// added by K Fujimura /////
    /*FMK RELIABILITY
     ...
     */
        ...
        if (Tcl_GetInt(interp, argv[1], &spectrumTag) != TCL_OK) {
            opserr <<
                "WARNING invalid input to SimulatedRandomProcess: spectrumTag"
                << endln;
            return 0;
        }
        ...
        if (Tcl_GetDouble(interp, argv[2], &mean) != TCL_OK) {
            opserr <<
                "WARNING invalid input to SimulatedRandomProcess: mean" <<
                endln;
            return 0;
        }
        ...
        if (theRandomNumberGenerator == 0) {
            opserr <<
                "WARNING: A random number generator must be instantiated before SimulatedRandomProcess."
                << endln;
            return 0;
        }
        ...
        if (theSpectrum == 0) {
            opserr <<
                "WARNING: Could not find the spectrum for the SimulatedRandomProcess."
                << endln;
            return 0;
        }
        // Parsing was successful, create the random process series object
        theSeries =
            new SimulatedRandomProcessSeries(0, theRandomNumberGenerator,
                                             theSpectrum, numFreqIntervals,
                                             mean);
    }
#endif
        ...
        opserr <<
            " valid types: Linear, Rectangular, Path, Constant, Trig, Sine\n";
        return 0;
    }
...
TimeSeries *TclSeriesCommand(ClientData clientData, Tcl_Interp * interp,
                             TCL_Char * arg)
{
    int argc;
    TCL_Char **argv;
