...
DiscretizedRandomProcessSeries::DiscretizedRandomProcessSeries(int tag,
                                                               ...
                                                               **
                                                               ...
                                                               p_mean,
                                                               double
                                                               p_maxStdv):TimeSeries
    (tag, TSERIES_TAG_DiscretizedRandomProcessSeries)
{
    randomVariables = 0;
    kickInTimes = 0;
...
TimeSeries *DiscretizedRandomProcessSeries::getCopy(void)
{
    opserr <<
        "DiscretizedRandomProcessSeries::getCopy() - not yet implemented\n";
    return 0;
}
 ...
 DiscretizedRandomProcessSeries::getFactor(double time)
{
    if (time == 0.0) {
        ...
        opserr << "ERROR in DiscretizedRandomProcessSeries::getFactor(): "
            << endln <<
            " random variables or kick-in times vector(s) do not exist. "
            << endln;
        return 0.0;
    } else if (kickInTimes->Size() != randomVariables->Size()) {
        opserr << "ERROR in DiscretizedRandomProcessSeries::getFactor(): "
            << endln <<
            " number of random variables is not the same as kick-in times. "
            << endln;
        return 0.0;
    } else {
        double sum1;
        ...
        for (int k = 0; k < numModFuncs; k++) {
            ...
            modFuncAmplitude =
                theModulatingFunctions[k]->getAmplitude(time);
            ...
            for (int i = 0; i < nrv; i++) {
                ...
                filterAmplitude =
                    theFilter->getAmplitude(time - (*kickInTimes) (i));
...
double DiscretizedRandomProcessSeries::getFactorSensitivity(double time)
{
    ...
    if (time == 0.0) {
        ...
        opserr <<
            "ERROR in DiscretizedRandomProcessSeries::getFactorSensitivity(): "
            << endln <<
            " random variables or kick-in times vector(s) do not exist. "
            << endln;
        return 0.0;
        ...
        for (int k = 0; k < numModFuncs; k++) {
            ...
            sum2 =
                theFilter->getAmplitude(time -
                                        (*kickInTimes) (parameterID - 1));
            sum1 += sum2 * modFuncAmplitude;
        }
...
int DiscretizedRandomProcessSeries::sendSelf(int commitTag,
                                             Channel & theChannel)
{
    return 0;
}
...
int DiscretizedRandomProcessSeries::recvSelf(int commitTag,
                                             Channel & theChannel,
...
void DiscretizedRandomProcessSeries::Print(OPS_Stream & s, int flag)
{
}

int DiscretizedRandomProcessSeries::setParameter(const char **argv,
                                                 int argc,
                                                 Parameter & param)
{
    if (argc < 1)
    ...
    // In case the vector isn't big enough
        ...
        for (int t = 1; t <= nrv; t++) {
            ...
            for (int k = 0; k < numModFuncs; k++) {
                ...
                // Get value of modulating function number k at time t
                Filter *theFilter_k =
                    theModulatingFunctions[k]->getFilter();
                double modFuncAmplitude_k =
                    theModulatingFunctions[k]->getAmplitude((*kickInTimes)
                                                            (t - 1));
                ...
                for (int l = 0; l < numModFuncs; l++) {
                    ...
                    Filter *theFilter_l =
                        theModulatingFunctions[l]->getFilter();
                    double modFuncAmplitude_l =
                        theModulatingFunctions[l]->
                        getAmplitude((*kickInTimes) (t - 1));
                    ...
                    for (int i = 0; i < nrv; i++) {
                        ...
                        double filterAmplitude_k =
                            theFilter_k->getAmplitude(((*kickInTimes)
                                                       (t - 1)) -
                                                      (*kickInTimes) (i));
                        double filterAmplitude_l =
                            theFilter_l->getAmplitude(((*kickInTimes)
                                                       (t - 1)) -
                                                      (*kickInTimes) (i));

                        // Add contribution 'ui * hi'
                        denominator +=
                            filterAmplitude_k * filterAmplitude_l *
...
int DiscretizedRandomProcessSeries::updateParameter(int parameterID,
                                                    Information & info)
{
    // In case the vector doesn't exist
    if (randomVariables == 0) {
...
int DiscretizedRandomProcessSeries::
activateParameter(int passedParameterID)
{
