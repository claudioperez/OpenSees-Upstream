...
zeroShift (zeroshift)
{
    if (period == 0.0)
      {
          opserr <<
              "TriangleSeries::TriangleSeries -- input period is zero, setting period to 1\n";
          period = 1;
      }
}


TriangleSeries::TriangleSeries ():TimeSeries (TSERIES_TAG_TriangleSeries),
...
double
TriangleSeries::getFactor (double pseudoTime)
{
    if (tStart <= pseudoTime && pseudoTime <= tFinish)
      {
          ...
          double k =
              (pseudoTime + phi - tStart) / period -
              floor ((pseudoTime + phi - tStart) / period);
          if (k < 0.25)
...
int
TriangleSeries::sendSelf (int commitTag, Channel & theChannel)
{
    ...
    if (result < 0)
      {
          opserr <<
              "TriangleSeries::sendSelf() - channel failed to send data\n";
          return result;
      }
...
int
TriangleSeries::recvSelf (int commitTag, Channel & theChannel,
                          FEM_ObjectBroker & theBroker)
{
    ...
    if (result < 0)
      {
          opserr <<
              "TriangleSeries::recvSelf() - channel failed to receive data\n";
          cFactor = 1.0;
          tStart = 0.0;
          tFinish = 0.0;
          period = 1.0;
