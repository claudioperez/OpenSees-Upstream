...
void *OPS_RectangularSeries(void)
{
    ...
    if (numRemainingArgs == 3 || numRemainingArgs == 5) {
        ...
        if (OPS_GetIntInput(&numData, &tag) != 0) {
            opserr <<
                "WARNING invalid series tag in Rectangular tag? tStart tFinish <-factor cFactor>\n";
            return 0;
        }
        numRemainingArgs -= 1;
    }
    ...
    if (OPS_GetDouble(&numData, dData) != 0) {
        opserr <<
            "WARNING invalid double data for RectangularSeries with tag: "
            << tag << endln;
        return 0;
    }
    numRemainingArgs -= 2;
    ...
    while (numRemainingArgs > 1) {
        ...
        if (strcmp(argvS, "-factor") == 0) {
            ...
            if (OPS_GetDouble(&numData, &dData[2]) != 0) {
                opserr <<
                    "WARNING invalid shift in Trig Series with tag?" <<
                    tag << endln;
                return 0;
            }
        } else {
            opserr << "WARNING unknown option: " << argvS <<
                "  in Rectangular Series with tag?" << tag << endln;
            return 0;
        }
        numRemainingArgs -= 2;
    }
    ...
    if (theSeries == 0) {
        opserr <<
            "WARNING ran out of memory creating RectangularSeries with tag: "
            << tag << "\n";
        return 0;
    }
...
RectangularSeries::RectangularSeries():TimeSeries(TSERIES_TAG_RectangularSeries),
...
TimeSeries *RectangularSeries::getCopy(void)
{
    return new RectangularSeries(this->getTag(), tStart, tFinish, cFactor);
}

double RectangularSeries::getFactor(double pseudoTime)
{
...
int RectangularSeries::sendSelf(int commitTag, Channel & theChannel)
{
    ...
    if (result < 0) {
        opserr <<
            "RectangularSeries::sendSelf() - channel failed to send data\n";
        return result;
    }
    return 0;
}
...
int RectangularSeries::recvSelf(int commitTag, Channel & theChannel,
                                FEM_ObjectBroker & theBroker)
{
    ...
    if (result < 0) {
        opserr <<
            "RectangularSeries::sendSelf() - channel failed to receive data\n";
        cFactor = 1.0;
        tStart = 0.0;
        tFinish = 0.0;
        return result;
...
void RectangularSeries::Print(OPS_Stream & s, int flag)
{
