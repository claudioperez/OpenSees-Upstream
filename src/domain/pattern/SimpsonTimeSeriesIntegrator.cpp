...
void *
OPS_SimpsonTimeSeriesIntegrator ()
{
    return new SimpsonTimeSeriesIntegrator ();
}
#endif

SimpsonTimeSeriesIntegrator::SimpsonTimeSeriesIntegrator ():TimeSeriesIntegrator
    (TIMESERIES_INTEGRATOR_TAG_Simpson)
{
...
SimpsonTimeSeriesIntegrator::integrate (TimeSeries * theSeries, double delta)
{
    ...
    if (delta <= 0.0)
      {
          opserr <<
              "SimpsonTimeSeriesIntegrator::integrate() - attempting to integrate time step "
              << delta << "<= 0.0.\n";
          return 0;
      }
    ...
    if (theSeries == 0)
      {
          opserr <<
              "SimpsonTimeSeriesIntegrator::integrate() - no TimeSeries passed.\n";
          return 0;
      }
    ...
    if (theInt == 0 || theInt->Size () == 0)
      {
          opserr <<
              "SimpsonTimeSeriesIntegrator::integrate() - ran out of memory allocating Vector of size "
              << numSteps << endln;
    ...
    for (int i = 2; i < numSteps - 1; i++)
      {
          ...
          (*theInt)[i] =
              (*theInt)[i - 2] + delta / 3.0 * (fi + 4.0 * fj + fk);
    ...
    (*theInt)[numSteps - 1] =
        (*theInt)[numSteps - 3] + delta / 3.0 * (fi + 4.0 * fj + fk);
    ...
    if (returnSeries == 0)
      {
          opserr <<
              "SimpsonTimeSeriesIntegrator::integrate() - ran out of memory creating PathSeries.\n";
          return 0;
      }
...
int
SimpsonTimeSeriesIntegrator::sendSelf (int commitTag, Channel & theChannel)
{
    return 0;
}
...
int
SimpsonTimeSeriesIntegrator::recvSelf (int commitTag, Channel & theChannel,
...
void
SimpsonTimeSeriesIntegrator::Print (OPS_Stream & s, int flag)
{
    return;
}
