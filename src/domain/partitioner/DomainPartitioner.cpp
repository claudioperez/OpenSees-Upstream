...
{
    s << "NodeLocations tag: " << this->
        getTag () << " partitions: " << nodePartitions;
}
...
int
DomainPartitioner::partition (int numParts, bool usingMain,
                              int mainPartitionTag, int specialElementTag)
{
          ...
          if (i != mainPartition)
            {
                ...
                if (subdomainPtr == 0)
                  {
                      opserr <<
                          "DomainPartitioner::partition - No Subdomain: ";
                      opserr << i << " exists\n";
                      return -1;
                  }
            }
                            ...
                            if (numPartitions == 1)
                              {
                                  Subdomain *theSubdomain =
                                      myDomain->getSubdomainPtr (partition);
                                  theLoadPattern->
                                      removeNodalLoad (theNodalLoad->
                                                       getTag ());
                                  if ((theSubdomain->
                                       addNodalLoad (theNodalLoad,
                                                     loadPatternTag)) != true)
                                      opserr <<
                      ...
                      if (partition != mainPartition)
                        {
                            Subdomain *theSubdomain =
                                myDomain->getSubdomainPtr (partition);
                            if (numPartitions == 1)
                                theLoadPattern->removeSP_Constraint (spPtr->
                                                                     getTag
                                                                     ());
                            int res =
                ...
                while (((theSub = theSubdomains ()) != 0) && (added == false))
                  {
                      ...
                      if (res == true)
                        {
                            theLoadPattern->removeElementalLoad (theLoad->
                                                                 getTag ());
                            theSub->addElementalLoad (theLoad,
                                                      loadPatternTag);
                            if (res < 0)
                                opserr <<
...
int
DomainPartitioner::swapBoundary (int from, int to,
                                 bool adjacentVertexNotInOther)
{
    opserr << "DomainPartitioner::swapBoundary: from " << from << " to " << to
        << endln;
  /***********************************************************************************
                                  ...
                                  bool adjacentVertexNotInOther)
{
    ...
    if (mustReleaseToLighter == false)
      ...
      {                         // check the other partition has a lighter load
...
GraphPartitioner *
DomainPartitioner::getGraphPartitioner ()
{
    std::
        cout <<
