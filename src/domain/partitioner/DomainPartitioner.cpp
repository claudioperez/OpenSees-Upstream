...
// This helper class maps each node tag to the partitions it belongs to.... useful to figure out 
// the boundaries between subdomains....
...
NodeLocations::NodeLocations(int tag):TaggedObject(tag),
nodePartitions(0, 1), numPartitions(0)
{
...
{
    s << "NodeLocations tag: " << this->getTag() << " partitions: " <<
        nodePartitions;
}
...
DomainPartitioner::DomainPartitioner(GraphPartitioner & theGraphPartitioner):myDomain(0), thePartitioner(theGraphPartitioner), theBalancer(0),
theElementGraph(0), theBoundaryElements(0),
theNodeLocations(0), elementPlace(0), numPartitions(0),
partitionFlag(false), usingMainDomain(false)
{

}

DomainPartitioner::DomainPartitioner(GraphPartitioner & theGraphPartitioner, LoadBalancer & theLoadBalancer):myDomain(0), thePartitioner(theGraphPartitioner),
theBalancer(&theLoadBalancer), theElementGraph(0), theBoundaryElements(0),
theNodeLocations(0), elementPlace(0), numPartitions(0),
partitionFlag(false), usingMainDomain(false)
{
...
int DomainPartitioner::partition(int numParts, bool usingMain,
                                 int mainPartitionTag,
                                 int specialElementTag)
{

    opserr << "DomainPartitioner::partition() - Start\n";
    ...
    /* print graph */
    opserr << "  * Identifying components to transfer.\n";
    ...
    if (specialElementTag != 0) {
        ...
        while ((found == false)
               && ((vertexPtr = theVerticesSpecial()) != 0)) {
    ...
    while ((nodePtr = theNodes()) != 0) {
        NodeLocations *theNodeLocation =
            new NodeLocations(nodePtr->getTag());
        if (theNodeLocation == 0) {
            opserr << "DomainPartitioner::partition(int numParts)";
            opserr <<
                " - ran out of memory creating NodeLocation for node: " <<
                nodePtr->getTag() << endln;
            numPartitions = 0;
            return -1;
        }
        if (theNodeLocations->addComponent(theNodeLocation) == false) {
            opserr << "DomainPartitioner::partition(int numParts)";
            opserr << " - failed to add NodeLocation to Map for Node: " <<
                nodePtr->getTag() << endln;
            numPartitions = 0;
            return -1;
        }
    ...
    while ((vertexPtr = theVertexIter()) != 0) {
        ...
        // For each element, transverse its neighboring element
        const ID & adjacency = vertexPtr->getAdjacency();
        int size = adjacency.Size();
        for (int i = 0; i < size; i++) {
            Vertex *otherVertex =
                theElementGraph->getVertexPtr(adjacency(i));

            //If a neighbor has a different color (partition)
            if (otherVertex->getColor() != vertexColor) {

                //Add it into the boundary elements graph
                theBoundaryElements[vertexColor -
                                    1]->addVertex(vertexPtr, false);
                i = size;
            }
        }

        //Also for each element, transverse its connected nodes
        Element *elePtr = myDomain->getElement(eleTag);
        const ID & nodes = elePtr->getExternalNodes();
        size = nodes.Size();
        for (int j = 0; j < size; j++) {
            int nodeTag = nodes(j);
            TaggedObject *theTaggedObject =
                theNodeLocations->getComponentPtr(nodeTag);
            ...
            // Add current partition as a location into current node's location map...
            NodeLocations *theNodeLocation =
                (NodeLocations *) theTaggedObject;
    ...
    while ((mpPtr = theMPs()) != 0) {
        ...
        TaggedObject *theRetainedObject =
            theNodeLocations->getComponentPtr(retained);
        TaggedObject *theConstrainedObject =
            theNodeLocations->getComponentPtr(constrained);
        ...
        NodeLocations *theRetainedLocation =
            (NodeLocations *) theRetainedObject;
        NodeLocations *theConstrainedLocation =
            (NodeLocations *) theConstrainedObject;

        ID & theConstrainedNodesPartitions =
            theConstrainedLocation->nodePartitions;
        int numPartitions = theConstrainedNodesPartitions.Size();
        for (int i = 0; i < numPartitions; i++) {
            int part = theConstrainedNodesPartitions(i);
    ...
    MP_ConstraintIter & theMPs2 = myDomain->getMPs();
    ...
    // opserr << "DomainPartitioner::partition() - Sending Nodes\n";

    //Transverse the node locations.....
    opserr << "  * Sending nodes.\n";
    TaggedObjectIter & theNodeLocationIter =
        theNodeLocations->getComponents();
    ...
    while ((theNodeObject = theNodeLocationIter()) != 0) {
        ...
        //Cycle the partitions that this node belongs to
        for (int i = 0; i < numPartitions; i++) {
            int partition = nodePartitions(i);

            //If current partition is not the main_partition... its a remote subdomain
            if (partition != mainPartition) {
                Subdomain *theSubdomain =
                    myDomain->getSubdomainPtr(partition);

                // opserr << "Adding node # " << nodeTag << " to partition # " << partition;
                //If the node belongs to only one partition
                if (numPartitions == 1) {
                    // opserr << " ... internal\n";
                    // Remove it from current and add it as internal node in the remote partition
                    Node *nodePtr = myDomain->removeNode(nodeTag);
                    theSubdomain->addNode(nodePtr);
                } else {
                    // opserr << " ... external\n";
                    // Otherwise, the node belongs to several partitions and is, therefore, a boundary node
                    // Add it to the external nodes in the remote partition
    ...
    // opserr << "DomainPartitioner::partition() - Sending Elements\n";
    ...
    while ((vertexPtr = theVertices()) != 0) {
        ...
        if (partition != mainPartition) {
            ...
            if (elePtr != 0) {
                ...
                opserr <<
                    "DomainPartitioner::partitioner - element GONE! - eleTag "
                    << eleTag << endln;
            }
        }
    }
    ...
    while ((theLoadPattern = theLoadPatterns()) != 0) {
        ...
        for (int i = 1; i <= numParts; i++) {
            if (i != mainPartition) {
                ...
                LoadPattern *loadPatternCopy =
                    theSubdomain->getLoadPattern(loadPatternTag);
                if (loadPatternCopy == 0) {
                    LoadPattern *newLoadPattern =
                        theLoadPattern->getCopy();
                    if (newLoadPattern == 0) {
                        opserr <<
                            "DomaiPartitioner::partition - out of memory creating LoadPatterns\n";
                        return -1;
                    }
            ...
            for (int i = 0; i < numPartitions; i++) {
                ...
                if (partition != mainPartition) {
                    if (numPartitions == 1) {
                        Subdomain *theSubdomain =
                            myDomain->getSubdomainPtr(partition);
                        theLoadPattern->removeNodalLoad
                            (theNodalLoad->getTag());
                        if ((theSubdomain->addNodalLoad
                             (theNodalLoad, loadPatternTag)) != true)
                            opserr <<
                                "DomainPartitioner::partition() - failed to add Nodal Load\n";
                    }
                }
            }
        }
        ...
        while ((spPtr = theSPs()) != 0) {
            ...
            for (int i = 0; i < numPartitions; i++) {
                ...
                if (partition != mainPartition) {
                    ...
                    if (res < 0)
                        opserr <<
                            "DomainPartitioner::partition() - failed to add SP Constraint\n";
                }
            }
        }
        ...
        while ((theLoad = theLoads()) != 0) {
            ...
            while (((theSub = theSubdomains()) != 0) && (added == false)) {
                ...
                if (res == true) {
                    ...
                    if (res < 0)
                        opserr <<
                            "DomainPartitioner::partition() - failed to add ElementalLoad\n";
                    added = true;
                }
            }
        }
    ...
    while ((spPtr = theDomainSP()) != 0) {
        ...
        if (theTaggedObject == 0) {
            opserr << "DomainPartitioner::partition(int numParts)";
            opserr << " - failed to find NodeLocation in Map for Node: "
                << nodeTag << " -- A BUG!!\n";
            numPartitions = 0;
            return -1;
        }
    ...
    while ((mpPtr = moreMPs()) != 0) {
        ...
        // if(retained == 5)
        // {
        // NodeLocations *theRetainedLocation = (NodeLocations *)theNodeLocations->getComponentPtr(retained);
        // opserr << "  > Retained: " << *theRetainedLocation;
        // opserr << "  > Constrained: " << *theConstrainedLocation;
        // }
        for (int i = 0; i < numPartitions; i++) {
            ...
            if (partition != mainPartition) {
                ...
                if (res < 0)
                    opserr <<
                        "DomainPartitioner::partition() - failed to add MP Constraint\n";
                // else
    ...
    opserr << "DomainPartitioner::partition() - Done\n";


    // myDomain->Print(opserr);

    return 0;
}


#define DOMAINPARTITIONER_REPARTITION_DEBUG_DISABLED_LV_1 0
#define DEBUGSTREAM1 \
    if (DOMAINPARTITIONER_REPARTITION_DEBUG_DISABLED_LV_1) {} \
    else opserr

#define DOMAINPARTITIONER_REPARTITION_DEBUG_DISABLED_LV_2 0
#define DEBUGSTREAM2 \
    if (DOMAINPARTITIONER_REPARTITION_DEBUG_DISABLED_LV_2) {} \
    else opserr
...
int DomainPartitioner::balance(Graph & theWeightedPGraph)
{
    ...
    if (theBalancer != 0) {
        ...
        // timer.start();
        // If we have a balancer, then call the balance function
        ...
        // All domains are informed that there has been a domain change
        SubdomainIter & theSubDomains = myDomain->getSubdomains();
        Subdomain *theSubDomain;
        ...
        // if(res > 0)
        // {
        // fid << myDomain->getCurrentTime() << " " << timer.getReal() << endln;
        // }
        // }
    } else {
...
int DomainPartitioner::getNumPartitions(void) const
{
...
Graph & DomainPartitioner::getPartitionGraph(void)
{
    if (myDomain == 0) {
...
Graph & DomainPartitioner::getColoredGraph(void)
{
    if (myDomain == 0) {
        opserr << "ERROR: DomainPartitioner::getPartitionGraph(void)";
        opserr << " - No domain has been set";
...
int DomainPartitioner::swapVertex(int from, int to, int vertexTag,
                                  bool adjacentVertexNotInOther)
{
    opserr << "DomainPartitioner::swapVertex() " << from << " " << to <<
        " " << vertexTag << endln;
...
int DomainPartitioner::swapBoundary(int from, int to,
                                    bool adjacentVertexNotInOther)
{
    opserr << "DomainPartitioner::swapBoundary: from " << from << " to " <<
        to << endln;
  /***********************************************************************************
                                     ...
                                     bool adjacentVertexNotInOther)
{
    ...
    if (mustReleaseToLighter == false)
        return swapVertex(from, partition, vertexTag,
        ...
        Vertex *toVertex =
            theWeightedPartitionGraph.getVertexPtr(partition);
                                       ...
                                       bool adjacentVertexNotInOther)
{
    ...
    while ((vertexPtr = verticesToSwap()) != 0)
        releaseVertex(from,
                      ...
                      factorGreater, adjacentVertexNotInOther);
...
GraphPartitioner *DomainPartitioner::getGraphPartitioner()
{
    std::cout <<
        "DomainPartitioner::getGraphPartitioner() - thePartitioner is @ "
        << static_cast < void *>(&thePartitioner) << "\n" << std::endl;
    return &thePartitioner;
}
