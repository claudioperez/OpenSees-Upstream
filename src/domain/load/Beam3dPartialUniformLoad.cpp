...
Beam3dPartialUniformLoad::Beam3dPartialUniformLoad(int tag, double wy,
                                                   double wz, double wa,
                                                   double aL, double bL,
                                                   int
                                                   theElementTag):ElementalLoad
    (tag, LOAD_TAG_Beam3dPartialUniformLoad, theElementTag), wTransy(wy),
wTransz(wz), wAxial(wa), aOverL(aL), bOverL(bL), parameterID(0)
{

}

Beam3dPartialUniformLoad::Beam3dPartialUniformLoad():ElementalLoad(LOAD_TAG_Beam3dPartialUniformLoad),
wTransy(0.0), wTransz(0.0), wAxial(0.0), aOverL(0.0), bOverL(0.0),
parameterID(0)
{
 ...
 Beam3dPartialUniformLoad::sendSelf(int commitTag, Channel & theChannel)
{
    ...
    if (result < 0) {
        opserr <<
            "Beam3dPartialUniformLoad::sendSelf - failed to send data\n";
        return result;
    }
...
int Beam3dPartialUniformLoad::recvSelf(int commitTag, Channel & theChannel,
                                       FEM_ObjectBroker & theBroker)
{
    ...
    if (result < 0) {
        opserr <<
            "Beam3dPartialUniformLoad::recvSelf - failed to recv data\n";
        return result;
    }
...
void Beam3dPartialUniformLoad::Print(OPS_Stream & s, int flag)
{
...
int Beam3dPartialUniformLoad::setParameter(const char **argv, int argc,
                                           Parameter & param)
{
    if (argc < 1)
        return -1;
...
int Beam3dPartialUniformLoad::updateParameter(int parameterID,
                                              Information & info)
{
    switch (parameterID) {
    case 1:
...
int Beam3dPartialUniformLoad::activateParameter(int paramID)
{
    parameterID = paramID;
...
const Vector & Beam3dPartialUniformLoad::getSensitivityData(int gradNumber)
{
    data.Zero();
