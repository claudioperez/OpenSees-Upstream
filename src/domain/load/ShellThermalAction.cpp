...
ShellThermalAction::ShellThermalAction(int tag,
                                       ...
                                       double locY2, double t3,
                                       double locY3, double t4,
                                       double locY4, double t5,
                                       double locY5, double t6,
                                       double locY6, double t7,
                                       double locY7, double t8,
                                       double locY8, double t9,
                                       double locY9,
                                       int
                                       theElementTag):ElementalLoad(tag,
                                                                    LOAD_TAG_ShellThermalAction,
                                                                    theElementTag),
...
ShellThermalAction::ShellThermalAction(int tag,
                                       ...
                                       theElementTag):ElementalLoad(tag,
                                                                    LOAD_TAG_ShellThermalAction,
                                                                    theElementTag),
theSeries(theSeries), ThermalActionType(LOAD_TAG_ShellThermalAction)
{
    Loc[0] = locY1;
    Loc[8] = locY2;
...
const Vector & ShellThermalAction::getData(int &type, double loadFactor)
{
...
void ShellThermalAction::applyLoad(const Vector & factors)
{
    for (int i = 0; i < 9; i++) {
...
void ShellThermalAction::applyLoad(double loadfactor)
{
...
int ShellThermalAction::sendSelf(int commitTag, Channel & theChannel)
{
    return -1;
}

int ShellThermalAction::recvSelf(int commitTag, Channel & theChannel,
...
// do it later
void ShellThermalAction::Print(OPS_Stream & s, int flag)
{
    s << "ShellThermalAction - reference load : " << Temp[0] <<
        " change  temp of bot\n";
