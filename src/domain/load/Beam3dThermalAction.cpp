...
Beam3dThermalAction::Beam3dThermalAction(int tag,
                                         double t1, double locY1,
                                         double t2, double locY2,
                                         double t3, double locY3,
                                         double t4, double locY4,
                                         double t5, double locY5,
                                         double t6, double t7,
                                         double locZ1, double t8,
                                         double t9, double locZ2,
                                         double t10, double t11,
                                         double locZ3, double t12,
                                         double t13, double locZ4,
                                         double t14, double t15,
                                         double locZ5,
                                         int
                                         theElementTag):ElementalLoad(tag,
                                                                      LOAD_TAG_Beam3dThermalAction,
                                                                      theElementTag),
...
Beam3dThermalAction::Beam3dThermalAction(int tag,
                                         ...
                                         double t4, double locY4,
                                         double t5, double locY5,
                                         double t6, double locY6,
                                         double t7, double locY7,
                                         double t8, double locY8,
...
Beam3dThermalAction::Beam3dThermalAction(int tag,
                                         ...
                                         theElementTag):ElementalLoad(tag,
                                                                      LOAD_TAG_Beam3dThermalAction,
                                                                      theElementTag),
theSeries(theSeries), ThermalActionType(LOAD_TAG_Beam3dThermalAction)
{
    Loc[0] = locY1;
    Loc[4] = locY2;
    Loc[5] = locZ1;
...
theSeries(theSeries), ThermalActionType(LOAD_TAG_Beam3dThermalAction)
{
    if (locs.Size() != 9) {
        opserr <<
            " WARNING::Beam3DThermalAction constructor failed to get 9 loc values"
            << endln;
...
Beam3dThermalAction::Beam3dThermalAction(int tag,
                                         ...
                                         theElementTag):ElementalLoad(tag,
                                                                      LOAD_TAG_Beam3dThermalAction,
                                                                      theElementTag),
ThermalActionType(LOAD_TAG_NodalThermalAction)
{
    Factors.Zero();
    for (int i = 0; i < 15; i++) {
        Temp[i] = 0;
...
const Vector & Beam3dThermalAction::getData(int &type, double loadFactor)
{
...
void Beam3dThermalAction::applyLoad(const Vector & factors)
{
...
void Beam3dThermalAction::applyLoad(double loadfactor)
{
    ...
    if (indicator == 2) {
        ...
        Factors =
            ((PathTimeSeriesThermal *) theSeries)->getFactors(loadfactor);
        for (int i = 0; i < 15; i++) {
...
int Beam3dThermalAction::sendSelf(int commitTag, Channel & theChannel)
{
    return -1;
}

int Beam3dThermalAction::recvSelf(int commitTag, Channel & theChannel,
...
// do it later
void Beam3dThermalAction::Print(OPS_Stream & s, int flag)
{
    if (indicator == 4 || indicator == 5) {
        s << "Beam3dThermalAction - reference load : " << TempApp[0] <<
            " at bot\n";
        s << TempApp[8] << " at top\n";
