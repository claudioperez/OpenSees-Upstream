...
ThermalActionWrapper::ThermalActionWrapper(int tag, int EleTag,
                                           ...
                                           theNodalTA1,
                                           NodalThermalAction *
                                           theNodalTA2):ElementalLoad(tag,
                                                                      LOAD_TAG_ThermalActionWrapper,
                                                                      EleTag),
ThermalActionType(LOAD_TAG_ThermalActionWrapper), theRatios(0), NumData(0),
IntData(0), ConstLoc(0), Transpoint(0)
{
    ...
    if ((theNodalTA[0]->getThermalActionType()) !=
        (theNodalTA[1]->getThermalActionType())) {
        opserr <<
            "WARNING::ThermalActionWrapper: Thermal action type derived from NodalThermalAction is not consistent"
            << endln;
    } else {
        if (theNodalTA[0]->getThermalActionType() == 1) {
            ...
            opserr <<
                "WARNING::ThermalActionWrapper: Thermal action type derived from NodalThermalAction is unable to be identified"
                << endln;
        }
    }
...
IntData(0), ConstLoc(0), Transpoint(0)
{
    ...
    if ((theNodalTA[0]->getThermalActionType()) !=
        (theNodalTA[2]->getThermalActionType())) {
        opserr <<
            "WARNING::ThermalActionWrapper: Thermal action type derived from NodalThermalAction is not consistent"
            << endln;
    } else {
...
ThermalActionWrapper::ThermalActionWrapper(int tag, int EleTag,
                                           ...
                                           theNodalTA3,
                                           NodalThermalAction *
                                           theNodalTA4):ElementalLoad(tag,
                                                                      LOAD_TAG_ThermalActionWrapper,
                                                                      EleTag),
ThermalActionType(LOAD_TAG_ThermalActionWrapper), theRatios(0), NumData(0),
ConstLoc(0), Transpoint(0)
{
    ...
    if ((theNodalTA[0]->getThermalActionType()) !=
        (theNodalTA[3]->getThermalActionType())) {
        opserr <<
            "WARNING::ThermalActionWrapper: Thermal action type derived from NodalThermalAction is not consistent"
            << endln;
    } else {
...
ConstLoc(0), Transpoint(0)
{
    ...
    if ((theNodalTA[0]->getThermalActionType()) !=
        (theNodalTA[4]->getThermalActionType())) {
        opserr <<
            "WARNING::ThermalActionWrapper: Thermal action type derived from NodalThermalAction is not consistent"
            << endln;
    } else {
...
ConstLoc(0), Transpoint(0)
{
    ...
    if ((theNodalTA[0]->getThermalActionType()) !=
        (theNodalTA[5]->getThermalActionType())) {
        opserr <<
            "WARNING::ThermalActionWrapper: Thermal action type derived from NodalThermalAction is not consistent"
            << endln;
    } else {
 ...
 ThermalActionWrapper::setRatios(const Vector & theRatio)
{
    ...
    if (theRatio != 0) {
        ...
        opserr << "WARNIGN::ThermalActionWrapper received invalid ratios"
            << endln;
        return -1;
    }
    ...
    if (theRatios.Size() != NodalLocs.noRows()) {
        opserr <<
            "WARNIGN::ThermalActionWrapper received an incompatible ratio"
            << endln;
        return -2;
    }
    ...
    if (theRatios(NumNodTA - 1) > 2) {
        opserr <<
            "WARNING::ThermalActionWrapper received a ration vector ends up with "
            << theRatios(NumNodTA -
                         1) << " , which should be 1.0 or 2.0" << endln;
        return -2;
    } else if (theRatios(0) < 0) {
        opserr <<
            "WARNING::ThermalActionWrapper received a ration vector ends up with "
            << theRatios(NumNodTA -
                         1) << " , which should be 0 or greater" << endln;
        return -2;
    } else {
        if (theRatios(0) > 0)
        ...
        for (int i = 1; i < NumNodTA - 1; i++) {
            if (theRatios(i) < -1e-6) {
                if (Transpoint != 0)
                    opserr <<
                        "WARNING::ThermalActionWrapper over defined the transition point"
                        << endln;
                else
...
const Vector & ThermalActionWrapper::getData(int &type, double loadFactor)
{
...
const Vector & ThermalActionWrapper::getIntData(const Vector & locs)
{
    ...
    for (int i = 0; i < ndm; i++) {
        sQdistInt +=
            (locs(i) - NodalLocs(0, i)) * (locs(i) - NodalLocs(0, i));
        sQr +=
            (NodalLocs(NumNodalTA - 1, i) -
             NodalLocs(0, i)) * (NodalLocs(NumNodalTA - 1,
                                           i) - NodalLocs(0, i));
    ...
    if (NumNodalTA == 2) {
        ...
        if (NumData == 9) {
            ...
            for (int i = 0; i < 9; i++) {
                if (fabs(data0(2 * i + 1) - data1(2 * i + 1)) > 1e-6) {
                    opserr <<
                        "Warning:The NodalThermalAction in dispBeamColumn2dThermalNUT "
                        << this->getTag() <<
                        "incompatiable loc input for datapoint " << i
                        << endln;
                } else {
        ...
        //end of numData==9;
            ...
            for (int i = 0; i < 5; i++) {
                if (fabs(data0(2 * i + 1) - data1(2 * i + 1)) > 1e-6
                    && fabs(data0(3 * i + 12) - data1(3 * i + 12)) >
                    1e-6) {
                    opserr <<
                        "Warning:The NodalThermalAction in dispBeamColumn2dThermalNUT "
                        << this->getTag() <<
        ...
        r1 = theRatios(1) * r;
        ratio[0] =
            (distInt - r1) * (distInt - r) / (constStart -
                                              r1) / (constStart - r);
        ratio[1] =
            (distInt - constStart) * (distInt - r) / (r1 -
                                                      constStart) / (r1 -
                                                                     r);
        ratio[2] =
            (distInt - constStart) * (distInt - r1) / (r -
                                                       constStart) / (r -
                                                                      r1);
        ...
        r2 = theRatios(2) * r;
        ratio[0] =
            (distInt - r1) * (distInt - r) * (distInt - r2) / (constStart -
                                                               r1) /
            (constStart - r) / (constStart - r2);
        ratio[1] =
            (distInt - constStart) * (distInt - r) * (distInt - r2) / (r1 -
                                                                       constStart)
            / (r1 - r) / (r1 - r2);
        ratio[2] =
            (distInt - constStart) * (distInt - r) * (distInt - r1) / (r2 -
                                                                       constStart)
            / (r2 - r) / (r2 - r1);
        ratio[3] =
            (distInt - constStart) * (distInt - r1) * (distInt - r2) / (r -
                                                                        constStart)
            / (r - r1) / (r - r2);
    ...
    //for num nodes =4
        ...
        if (distInt > constStart && distInt < Transpoint) {
            ratio[0] =
                (distInt - r1) * (distInt - Transpoint) / (constStart -
                                                           r1) /
                (constStart - Transpoint);
            ratio[1] =
                (distInt - constStart) * (distInt - Transpoint) / (r1 -
                                                                   constStart)
                / (r1 - Transpoint);
            ratio[2] =
                (distInt - constStart) * (distInt - r1) / (Transpoint -
                                                           constStart) /
                (Transpoint - r1);
        } else if (distInt > Transpoint && distInt < r) {
            ratio[0] =
                (distInt - r3) * (distInt - r) / (Transpoint -
                                                  r3) / (Transpoint - r);
            ratio[1] =
                (distInt - Transpoint) * (distInt - r) / (r3 -
                                                          Transpoint) /
                (r3 - r);
            ratio[2] =
                (distInt - Transpoint) * (distInt - r3) / (r -
                                                           Transpoint) /
                (r - r3);
        } else {
            opserr << "WARNIGN ThermalActionWrapper received a int Value "
                << distInt << " out of range" << endln;
        }
...
int ThermalActionWrapper::sendSelf(int commitTag, Channel & theChannel)
{
    return -1;
}

int ThermalActionWrapper::recvSelf(int commitTag, Channel & theChannel,
...
// do it later
void ThermalActionWrapper::Print(OPS_Stream & s, int flag)
{
