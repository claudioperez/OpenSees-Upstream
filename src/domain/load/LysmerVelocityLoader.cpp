...
Vector
LysmerVelocityLoader::data (3);

LysmerVelocityLoader::LysmerVelocityLoader (int tag, int theElementTag,
                                            int dir_):
ElementalLoad (tag, LOAD_TAG_LysmerVelocityLoader, theElementTag),
dir (dir_)
{
}
...
LysmerVelocityLoader::sendSelf (int commitTag, Channel & theChannel)
{
    ...
    if (res < 0)
      {
          opserr << "WARNING LysmerVelocityLoader::sendSelf() - " << this->
...
int
LysmerVelocityLoader::recvSelf (int commitTag, Channel & theChannel,
                                FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0)
      {
          opserr << "WARNING LysmerVelocityLoader::recvSelf() - " << this->
