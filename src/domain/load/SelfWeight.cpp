...
SelfWeight::SelfWeight(int tag, double xf, double yf, double zf,
...
SelfWeight::SelfWeight():ElementalLoad(LOAD_TAG_SelfWeight),
...
const Vector & SelfWeight::getData(int &type, double loadFactor)
{
...
int SelfWeight::sendSelf(int commitTag, Channel & theChannel)
{
...
int SelfWeight::recvSelf(int commitTag, Channel & theChannel,
...
void SelfWeight::Print(OPS_Stream & s, int flag)
{
    s << "SelfWeight...";
    s << "  element acted on: " << eleTag << endln;;
    s << "  (xFact, yFact, zFact) = (" << xFact << ", " << yFact << ", " <<
        zFact << ") " << endln;;
}
