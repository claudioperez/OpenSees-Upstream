...
ImposedMotionSP::ImposedMotionSP():SP_Constraint(CNSTRNT_TAG_ImposedMotionSP),
groundMotionTag(0), patternTag(0),
theGroundMotion(0), theNode(0), theNodeResponse(0),
theGroundMotionResponse(3)
{
    // does nothing else
}

// constructor for a subclass to use
ImposedMotionSP::ImposedMotionSP(int node, int ndof, int pattern,
                                 int motion):SP_Constraint(node, ndof,
                                                           CNSTRNT_TAG_ImposedMotionSP),
groundMotionTag(motion), patternTag(pattern), theGroundMotion(0),
theNode(0), theNodeResponse(0), theGroundMotionResponse(3)
{
...
int ImposedMotionSP::applyConstraint(double time)
{
    // on first 
    if (theGroundMotion == 0 || theNode == 0 || theNodeResponse == 0) {
        ...
        if (theNode == 0) {
            opserr << "ImposedMotionSP::applyConstraint() - node " <<
                nodeTag << " does not exist\n";
            return -1;
        }
        ...
        if (dofNumber < 0 || numNodeDOF <= dofNumber) {
            opserr << "ImposedMotionSP::applyConstraint() - dof number "
                << dofNumber++ << " at node " << nodeTag << " not valid\n";
            return -2;
        }
        ...
        if (theNodeResponse == 0) {
            opserr <<
                "ImposedMotionSP::applyConstraint() - out of memory\n";
            return -2;
        }

        LoadPattern *theLoadPattern =
            theDomain->getLoadPattern(patternTag);
        if (theLoadPattern == 0) {
            opserr <<
                "ImposedMotionSP::applyConstraint() - no load pattern\n";
            return -3;
        }
        ...
        if (theGroundMotion == 0) {
            opserr <<
                "ImposedMotionSP::applyConstraint() - no ground motion\n";
            return -4;
        }
    }
...
bool ImposedMotionSP::isHomogeneous(void) const
{
    return false;
}


int ImposedMotionSP::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (result < 0) {
        opserr <<
            "ImposedMotionSP::sendSelf() - base SP_Constraint class failed\n";
        return -1;
    }
    ...
    if (theChannel.sendID(dbTag, cTag, myExtraData) < 0) {
        opserr <<
            "ImposedMotionSP::sendSelf() - failed to send extra data\n";
        return -1;
    }
...
int ImposedMotionSP::recvSelf(int cTag, Channel & theChannel,
                              FEM_ObjectBroker & theBroker)
{
    ...
    if (result < 0) {
        opserr <<
            "ImposedMotionSP::recvSelf() - base SP_Constraint class failed\n";
        return -1;
    }
...
void ImposedMotionSP::Print(OPS_Stream & s, int flag)
{
