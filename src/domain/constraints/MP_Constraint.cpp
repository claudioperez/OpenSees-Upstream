...
MP_Constraint::MP_Constraint(int clasTag):DomainComponent(nextTag++,
                                                          clasTag),
nodeRetained(0), nodeConstrained(0), constraint(0), constrDOF(0),
retainDOF(0), dbTag1(0), dbTag2(0)
{
    numMPs++;
}
...
MP_Constraint::MP_Constraint(int nodeRetain, int nodeConstr,
                             ID & constrainedDOF,
                             ID & retainedDOF,
                             int clasTag):DomainComponent(nextTag++,
                                                          clasTag),
nodeRetained(nodeRetain), nodeConstrained(nodeConstr), constraint(0),
constrDOF(0), retainDOF(0), dbTag1(0), dbTag2(0)
{
    numMPs++;
...
MP_Constraint::MP_Constraint(int nodeRetain, int nodeConstr,
                             Matrix & constr, ID & constrainedDOF,
                             ID & retainedDOF):DomainComponent(nextTag++,
                                                               CNSTRNT_TAG_MP_Constraint),
nodeRetained(nodeRetain), nodeConstrained(nodeConstr), constraint(0),
constrDOF(0), retainDOF(0), dbTag1(0), dbTag2(0)
{
    numMPs++;
...
int MP_Constraint::getNodeConstrained(void) const
{
...
const ID & MP_Constraint::getConstrainedDOFs(void) const
{
    if (constrDOF == 0) {
...
const ID & MP_Constraint::getRetainedDOFs(void) const
{
    if (retainDOF == 0) {
...
int MP_Constraint::applyConstraint(double timeStamp)
{
    // does nothing MP_Constraint objects are time invariant
    return 0;
}

bool MP_Constraint::isTimeVarying(void) const
{
    return false;
}


const Matrix & MP_Constraint::getConstraint(void)
{
    if (constraint == 0) {
...
int MP_Constraint::sendSelf(int cTag, Channel & theChannel)
{
    static ID data(10);
    ...
    if (constraint == 0)
        ...
        data(3) = constraint->noRows();
    if (constraint == 0)
        data(4) = 0;
    else
        data(4) = constraint->noCols();
    if (constrDOF == 0)
        data(5) = 0;
    else
        data(5) = constrDOF->Size();
    if (retainDOF == 0)
        data(6) = 0;
    else
        data(6) = retainDOF->Size();
    ...
    if (result < 0) {
        opserr <<
            "WARNING MP_Constraint::sendSelf - error sending ID data\n";
        return result;
    }
...
int MP_Constraint::recvSelf(int cTag, Channel & theChannel,
                            FEM_ObjectBroker & theBroker)
{
    ...
    if (result < 0) {
        opserr <<
            "WARNING MP_Constraint::recvSelf - error receiving ID data\n";
        return result;
    }
...
void MP_Constraint::Print(OPS_Stream & s, int flag)
{
