...
ImposedMotionSP1::ImposedMotionSP1():SP_Constraint(CNSTRNT_TAG_ImposedMotionSP1),
...
ImposedMotionSP1::ImposedMotionSP1(int node, int ndof, int pattern,
                                   int motion):SP_Constraint(node, ndof,
                                                             CNSTRNT_TAG_ImposedMotionSP1),
groundMotionTag(motion), patternTag(pattern), theGroundMotion(0),
theNode(0), theGroundMotionResponse(3)
{
...
int ImposedMotionSP1::applyConstraint(double time)
{
    // on first 
...
bool ImposedMotionSP1::isHomogeneous(void) const
{
    return false;
}


int ImposedMotionSP1::sendSelf(int cTag, Channel & theChannel)
{
    ...
    if (result < 0) {
        opserr <<
            "ImposedMotionSP1::sendSelf() - base SP_Constraint class failed\n";
        return -1;
    }
    ...
    if (theChannel.sendID(dbTag, cTag, myExtraData) < 0) {
        opserr <<
            "ImposedMotionSP1::sendSelf() - failed to send extra data\n";
        return -1;
    }
...
int ImposedMotionSP1::recvSelf(int cTag, Channel & theChannel,
                               FEM_ObjectBroker & theBroker)
{
    ...
    if (result < 0) {
        opserr <<
            "ImposedMotionSP1::recvSelf() - base SP_Constraint class failed\n";
        return -1;
    }
...
void ImposedMotionSP1::Print(OPS_Stream & s, int flag)
{
