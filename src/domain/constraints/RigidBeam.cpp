...
RigidBeam::RigidBeam (Domain & theDomain, int nR, int nC)
{
    ...
    if (nodeR == 0)
      {
          opserr << "RigidBeam::RigidBeam - retained Node" << nR <<
              "not in domain\n";
          return;
      }
    ...
    if (nodeR == 0)
      {
          opserr << "RigidBeam::RigidBeam - constrained Node" << nC <<
              "not in domain\n";
          return;
      }
    ...
    if (newC == 0)
      {
          opserr << "RigidBeam::RigidBeam - for nodes " << nC << " and " << nR
              << ", out of memory\n";
      }
    else
      {
          ...
          if (theDomain.addMP_Constraint (newC) == false)
            {
                opserr << "RigidBeam::RigidBeam - for nodes " << nC << " and "
