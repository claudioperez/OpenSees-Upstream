...
Pressure_Constraint::Pressure_Constraint(int classTag):DomainComponent(0,
                                                                       classTag),
pTag(0), fluidEleTags(), otherEleTags(), pval(0), freesurf(false)
{
}

Pressure_Constraint::Pressure_Constraint(int nodeId,
                                         int ptag):DomainComponent(nodeId,
                                                                   CNSTRNT_TAG_Pressure_Constraint),
...
Pressure_Constraint::Pressure_Constraint(int nodeId,
                                         double
                                         val):DomainComponent(nodeId,
                                                              CNSTRNT_TAG_Pressure_Constraint),
pTag(nodeId), fluidEleTags(), otherEleTags(), pval(0), freesurf(false)
{
    pval = new double[2];
    pval[0] = val;
    pval[1] = 0.0;
...
Node *Pressure_Constraint::getPressureNode()
{
    if (pval != 0)
        return 0;
...
void Pressure_Constraint::setPressure(double p)
{
    if (pval != 0) {
        pval[0] = p;
        return;
...
void Pressure_Constraint::setPdot(double pdot)
{
    if (pval != 0) {
        pval[1] = pdot;
        return;
...
double Pressure_Constraint::getPressure(int last)
{
    if (pval != 0) {
        return pval[0];
    }
...
double Pressure_Constraint::getPdot(int last)
{
    if (pval != 0) {
        return pval[1];
    }
...
const ID & Pressure_Constraint::getFluidElements()
{
    Domain *theDomain = this->getDomain();
    if (theDomain == 0) {
        opserr << "WARNING: domain has not been set";
...
const ID & Pressure_Constraint::getOtherElements()
{
    Domain *theDomain = this->getDomain();
    if (theDomain == 0) {
        opserr << "WARNING: domain has not been set";
...
void Pressure_Constraint::connect(int eleId, bool fluid)
{
    Domain *theDomain = this->getDomain();
    if (theDomain == 0) {
        opserr << "WARNING: domain has not been set";
...
void Pressure_Constraint::disconnect(int eleId)
{
    fluidEleTags.removeValue(eleId);
}

void Pressure_Constraint::disconnect()
{
    otherEleTags = ID();
}
...
bool Pressure_Constraint::isInterface() const
{
    return fluidEleTags.Size() > 0 && otherEleTags.Size() > 0;
}

bool Pressure_Constraint::isStructure() const
{
    return fluidEleTags.Size() == 0 && otherEleTags.Size() > 0;
}

bool Pressure_Constraint::isIsolated() const
{
    return fluidEleTags.Size() == 0 && otherEleTags.Size() == 0;
}
