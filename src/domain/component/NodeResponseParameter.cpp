...
NodeResponseParameter::NodeResponseParameter(int passedTag, Node * theNode,
                                             ...
                                             int
                                             theDOF):Parameter(passedTag,
                                                               1976),
myNode(theNode), myType(theType), myDOF(theDOF), currentValue(0.0)
{
...
int NodeResponseParameter::update(double newValue)
{
...
double NodeResponseParameter::getValue(void)
{
    return currentValue;
}

void NodeResponseParameter::setValue(double newValue)
{
    currentValue = newValue;
}

bool NodeResponseParameter::isImplicit(void)
{
    return true;
}

double NodeResponseParameter::getSensitivity(int index)
{
...
double NodeResponseParameter::getPerturbation(void)
{
    return 0.005;
}

int NodeResponseParameter::getPointerTag(void)
{
    return myNode->getTag();
}

void NodeResponseParameter::Print(OPS_Stream & s, int flag)
{
    s << "NodeResponseParameter, tag = " << this->getTag() << endln;
    myNode->Print(s, flag);
}

void NodeResponseParameter::setDomain(Domain * theDomain)
{
    return;
}

int NodeResponseParameter::sendSelf(int commitTag, Channel & theChannel)
{
    return 0;
}

int NodeResponseParameter::recvSelf(int commitTag, Channel & theChannel,
