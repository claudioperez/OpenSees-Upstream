...
Parameter::Parameter(int passedTag,
                     ...
                     const char **argv, int argc):TaggedObject(passedTag),
MovableObject(PARAMETER_TAG_Parameter),
parameterID(0),
theObjects(0),
numObjects(0),
maxNumObjects(0),
theComponents(0), numComponents(0), maxNumComponents(0), gradIndex(-1)
{
    ...
    if (ok < 0) {
        opserr << "Parameter::Parameter " << this->getTag() <<
...
Parameter::Parameter(const Parameter & param):TaggedObject(param.getTag()),
MovableObject(PARAMETER_TAG_Parameter),
...
Parameter::Parameter(int tag, int classTag):TaggedObject(tag),
...
Parameter::Parameter():TaggedObject(0), MovableObject(PARAMETER_TAG_Parameter),
theObjects(0),
theComponents(0), numComponents(0), maxNumComponents(0),
...
int Parameter::addComponent(DomainComponent * parentObject,
                            const char **argv, int argc)
{
    if (numComponents == maxNumComponents) {
        maxNumComponents += expandSize;
        DomainComponent **newComponents =
            new DomainComponent *[maxNumComponents];
    ...
    if (numObjects == oldNumObjects || ok < 0) {
        opserr << "Parameter::addComponent " << this->getTag() <<
            " -- no objects were able to identify parameter" << endln;
        for (int i = 0; i < argc; i++)
            opserr << argv[i] << ' ';
        opserr << endln;
        return -1;
...
int Parameter::activate(bool active)
{
    ...
    for (int i = 0; i < numObjects; i++) {
        ok +=
            theObjects[i]->activateParameter(active ? parameterID[i] : 0);
    }

    return ok;
}

double Parameter::getSensitivity(int index)
{
    //return 1.0;
    return (index == gradIndex) ? 1.0 : 0.0;
}

void Parameter::Print(OPS_Stream & s, int flag)
{
    s << "Parameter, tag = " << this->getTag() << " " << this->
        getValue() << endln;
    //s << "\tparameterID = " << parameterID << endln;
}


int Parameter::addObject(int paramID, MovableObject * object)
{
...
void Parameter::setDomain(Domain * theDomain)
{
    return;
}

int Parameter::sendSelf(int commitTag, Channel & theChannel)
{
...
int Parameter::recvSelf(int commitTag, Channel & theChannel,
                        FEM_ObjectBroker & theBroker)
{
    return 0;
}

int Parameter::clean(void)
{
    for (int i = 0; i < numObjects; i++) {
        theObjects[i] = 0;
    }
