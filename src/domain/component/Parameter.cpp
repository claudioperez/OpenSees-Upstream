...
gradIndex (-1)
{
    ...
    if (ok < 0)
      {
          opserr << "Parameter::Parameter " << this->
              getTag () <<
                         ...
                         const char **argv, int argc)
{
    ...
    if (numObjects == oldNumObjects || ok < 0)
      {
          opserr << "Parameter::addComponent " << this->
              getTag () << " -- no objects were able to identify parameter" <<
              endln;
          for (int i = 0; i < argc; i++)
              opserr << argv[i] << ' ';
          opserr << endln;
...
int
Parameter::recvSelf (int commitTag, Channel & theChannel,
