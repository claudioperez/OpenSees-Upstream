...
MatParameter::MatParameter(int theTag, int materialTag, const char
                           *materialParameterName):Parameter(theTag,
                                                             PARAMETER_TAG_MatParameter),
theParameterName(0), theMaterialTag(materialTag), theParameterID(-1)
{
    if (materialParameterName != 0) {
        ...
        if (theParameterName == 0) {
            opserr <<
                "MatParameter::MatParameter - out of memory for parameter: ";
...
MatParameter::MatParameter():Parameter(),
...
void MatParameter::setDomain(Domain * theDomain)
{
    Element *theEle;
...
int MatParameter::sendSelf(int commitTag, Channel & theChannel)
{
    static ID theData(3);
...
int MatParameter::recvSelf(int commitTag, Channel & theChannel,
