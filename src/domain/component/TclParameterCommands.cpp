    ...
    if (argc < 2)
      {
          ...
          opserr <<
              "Want: parameter tag <specific parameter args> .. see manual for valid parameter types and arguments\n";
          return TCL_ERROR;
      }
                ...
                if (theReliabilityDomain == 0)
                  {
                      opserr << "ERROR parameter " << paramTag <<
                          " -- reliability domain has not been created" <<
                          endln;
                  }
          ...
          if (argc > argStart && strstr (argv[argStart], "element") != 0)
            {
                ...
                if (argc < 4)
                  {
                      opserr <<
                          "WARNING parameter -- insufficient number of arguments for parameter with tag "
                          << paramTag << '\n';
                      return TCL_ERROR;
                  }

                if (Tcl_GetInt (interp, argv[argStart + 1], &eleTag) !=
                    TCL_OK)
                  {
                ...
                theObject =
                    (DomainComponent *) theTclDomain->getElement (eleTag);
          ...
          else if (argc > argStart && strstr (argv[argStart], "node") != 0)
            {
                ...
                int nodeTag;
                if (Tcl_GetInt (interp, argv[argStart + 1], &nodeTag) !=
                    TCL_OK)
                  {
                ...
                // Retrieve element from domain
                theObject =
                    (DomainComponent *) theTclDomain->getNode (nodeTag);

                argStart = (theRV) ? 6 : 4;
            }
          else if (argc > argStart
                   && strstr (argv[argStart], "loadPattern") != 0)
            {
                if (argc < 4)
                  {
                      opserr <<
                ...
                int loadTag;
                if (Tcl_GetInt (interp, argv[argStart + 1], &loadTag) !=
                    TCL_OK)
                  {
                      opserr <<
                          "WARNING parameter -- invalid load pattern tag\n";
                      return TCL_ERROR;
                  }

                // Retrieve element from domain
                theObject =
                    (DomainComponent *) theTclDomain->
                    getLoadPattern (loadTag);
            ...
            {
                opserr <<
                    "WARNING - unable to assign parameter to object of type "
                    << argv[2] << '\n';
                return TCL_ERROR;
            }
          ...
          if (strcmp (argv[0], "parameter") == 0)
            {
                ...
                if (theParameter != 0)
                  {
                      opserr << "WARNING parameter -- parameter with tag " <<
                          paramTag << " already exists in domain\n";
                      return TCL_ERROR;
                  }
                ...
                if (argc > argStart)
                  {
                      if (isele == false)
                        ...
                        {
                            newParameter =
                                new ElementParameter (paramTag, eleTag,
                                                      (const char **)
                                                      &argv[argStart],
                                                      argc - argStart);
                        }
          ...
          if (strcmp (argv[0], "addToParameter") == 0)
            {
                ...
                if (theParameter == 0)
                  {
                      opserr <<
                          "WARNING addToParameter -- parameter with tag " <<
                          paramTag << " not found in domain\n";
                      return TCL_ERROR;
                  }
                else
                  {
                      if (isele == false)
                          theParameter->addComponent (theObject,
                                                      (const char **)
                                                      &argv[argStart],
                                                      argc - argStart);
                      else
    ...
    if (strcmp (argv[0], "updateParameter") == 0)
      {
          ...
          if (theParameter == 0)
            {
                opserr << "WARNING updateParameter -- parameter with tag " <<
                    paramTag << " not found in domain\n";
                //  return TCL_ERROR;
            }
          ...
          if (Tcl_GetDouble (interp, argv[2], &newValue) != TCL_OK)
            {
                opserr <<
                    "WARNING updateParameter -- invalid parameter value\n";
                return TCL_ERROR;
            }
