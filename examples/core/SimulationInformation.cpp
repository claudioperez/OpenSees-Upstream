...
SimulationInformation::SimulationInformation ():title (0), description (0), contactName (0),
          ...
          if (numStrings > 0)
            {
                ...
                for (int i = 0; i < numStrings - 1; i++)
                    s << "\t\t\"type\": \"" << modelTypes.
                        getString (i) << "\",\n";
                s << "\t\t\"type\": \"" << modelTypes.getString (numStrings -
                                                                 1) << "\"\n";
                s << "\t},\n";
            }
          ...
          if (numStrings > 0)
            {
                s << "\t\"analysisTypes\": {\n";
                for (int i = 0; i < numStrings - 1; i++)
                    s << "\t\t\"type\": \"" << analysisTypes.
                        getString (i) << "\",\n";
                s << "\t\t\"type\": \"" << analysisTypes.
                    getString (numStrings - 1) << "\"\n";
                s << "\t},\n";
            }
...
int
UploadFiles (const char *cookie, int projID, int expID, int trialID,
             File * theFile, const char *path, int basePath)
{
    int res = 0;
    ...
    if (theFile->isDir () == true)
      {
          ...
          res =
              neesADD_TrialAnalysisDir (cookie, projID, expID, trialID,
          ...
          if (res < 0)
            {
                opserr <<
                    "ERROR: SimulationInformation.cpp - uploadFiles() - failed to add directory: "
                    << fileName << " to path: " << &path[basePath] << endln;
                return res;
            }
          ...
          while ((theDirFile = theDirFiles ()) != 0)
            {
                res =
                    UploadFiles (cookie, projID, expID, trialID, theDirFile,
                                 theFile->getDescription (), basePath);
                if (res < 0)
                    return res;
            }
    ...
    else
      {
          ...
          if (res < 0)
            {
                opserr <<
                    "ERROR: SimulationInformation.cpp - uploadFiles() - failed to add file: "
                    << fileName << " to path: " << &path[basePath] << endln;
                return res;
            }
      }
...
SimulationInformation::neesUpload (const char *username,
                                   const char *passwd, int projID, int expID)
{
#ifdef _HTTPS
    ...
    if (username == 0 || passwd == 0)
      {
          opserr <<
              "ERROR: SimulationInformation::neesUpload()  could not login to nees central no username or passwd provided\n";
          return -1;
      }
    ...
    if (res != 0)
      {
          ...
          opserr <<
              "ERROR: SimulationInformation::neesUpload() - failed to login check username: "
    ...
    if (cookie != 0)
        opserr <<
            "SimulationInformation::neesUpload() - information about cookie: "
            << cookie << endln;
    ...
    trialID =
        neesADD_Trial (cookie, projID, expID, title, title, " ", description);
    ...
    if (trialID < 0)
      {
          ...
          opserr <<
              "SimulationInformation::neesUpload() - failed to create new Trial in project: "
    ...
    while (foundRoot == false)
      {
          if (startLoadingDir->isDir () == true
              && startLoadingDir->getNumFiles () == 1)
            {
    ...
    while ((theDirFile = theDirFiles ()) != 0)
        res +=
            UploadFiles (cookie, projID, expID, trialID, theDirFile,
                         startLoadingDir->getDescription (), pathLength);
...
#else
    opserr <<
        "ERROR: SimulationInformation::neesUpload() - not available in this build\n";
    return -1;
