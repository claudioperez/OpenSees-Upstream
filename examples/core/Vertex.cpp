    ...
    if (myWeight != 0.0)
      {
          ...
          if (theChannel.sendVector (0, commitTag, vectData) < 0)
            {
                opserr <<
                    "Graph::rendSelf() - failed to receive the weight\n";
                return -2;
            }
      }
...
int
Vertex::recvSelf (int commitTag, Channel & theChannel,
    ...
    if (idData (5) == 1)
      {
          ...
          if (theChannel.recvVector (0, commitTag, vectData) < 0)
            {
                opserr <<
                    "Graph::recvSelf() - failed to receive the weight\n";
                return -2;
            }
          myWeight = vectData (0);
      }
