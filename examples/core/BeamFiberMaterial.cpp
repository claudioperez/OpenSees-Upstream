...
Vector
BeamFiberMaterial::stress (3);
Matrix
BeamFiberMaterial::tangent (3, 3);
...
strain (3)
{
    ...
    if (theMaterial == 0)
      {
          opserr <<
              "BeamFiberMaterial::BeamFiberMaterial -- failed to get copy of material\n";
          exit (-1);
      }
}
    ...
    do
      {
          ...
          if (theMaterial->setTrialStrain (threeDstrain) < 0)
            {
                opserr <<
                    "BeamFiberMaterial::setTrialStrain - setStrain failed in material with strain "
                    << threeDstrain;
                return -1;
            }
...
BeamFiberMaterial::sendSelf (int commitTag, Channel & theChannel)
{
    ...
    if (res < 0)
      {
          opserr <<
              "BeamFiberMaterial::sendSelf() - failed to send id data\n";
          return res;
      }
    ...
    if (res < 0)
      {
          opserr <<
              "BeamFiberMaterial::sendSelf() - failed to send vector data\n";
          return res;
      }
    ...
    if (res < 0)
        opserr <<
            "BeamFiberMaterial::sendSelf() - failed to send vector material\n";

    return res;
}

int
BeamFiberMaterial::recvSelf (int commitTag, Channel & theChannel,
    ...
    if (theMaterial == 0 || theMaterial->getClassTag () != matClassTag)
      {
          ...
          if (theMaterial == 0)
            {
                opserr <<
                    "BeamFiberMaterial::recvSelf() - failed to get a material of type: "
                    << matClassTag << endln;
                return -1;
            }
      }
