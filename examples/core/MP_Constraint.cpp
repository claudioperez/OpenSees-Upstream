...
OPS_EqualDOF ()
{
    ...
    if (OPS_GetNumRemainingInputArgs () < 3)
      {
          opserr <<
              "WARNING: invalid # of args: equalDOF rNodeTag cNodeTag dof1 ...\n";
          return -1;
      }
    ...
    MP_Constraint *theMP =
        new MP_Constraint (data (0), data (1), Ccr, rcDOF, rcDOF);
    if (theMP == 0)
      {
...
OPS_EqualDOF_Mixed ()
{
    ...
    if (OPS_GetNumRemainingInputArgs () < numDOF * 2)
      {
          opserr <<
              "WARNING insufficient args - want: equalDOFmixed RnodeID? CnodeID? numDOF? RDOF1? CDOF1? ... ...";
          return -1;
      }
    ...
    for (int k = 0; k < numDOF; k++)
      {
          ...
          if (dofIDC < 0 || dofIDR < 0)
            {
                opserr << "WARNING invalid dofID: " << " must be >= 1";
                return -1;
            }
          rDOF (k) = dofIDR;
          cDOF (k) = dofIDC;
...
DomainComponent (nextTag++, clasTag),
nodeRetained (nodeRetain),
nodeConstrained (nodeConstr),
constraint (0),
constrDOF (0),
retainDOF (0),
dbTag1 (0),
