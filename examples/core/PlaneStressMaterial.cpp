...
Vector
PlaneStressMaterial::stress (3);
Matrix
PlaneStressMaterial::tangent (3, 3);
...
NDMaterial (0, ND_TAG_PlaneStressMaterial), strain (3)
{
}
...
PlaneStressMaterial::getCopy ()
{
    ...
    clone = new PlaneStressMaterial (this->getTag (), *theMaterial);    //make the copy
    ...
    do
      {
          ...
          if (theMaterial->setTrialStrain (threeDstrain) < 0)
            {
                opserr <<
                    "PlaneStressMaterial::setTrialStrain() - setTrialStrain in material failed with strain "
                    << threeDstrain;
                return -1;
            }
...
PlaneStressMaterial::sendSelf (int commitTag, Channel & theChannel)
{
    ...
    if (res < 0)
      {
          opserr <<
              "PlaneStressMaterial::sendSelf() - failed to send id data\n";
          return res;
      }
    ...
    if (res < 0)
      {
          opserr <<
              "PlaneStressMaterial::sendSelf() - failed to send vector data\n";
          return res;
      }
    ...
    if (res < 0)
        opserr <<
            "PlaneStressMaterial::sendSelf() - failed to send vector material\n";

    return res;
}

int
PlaneStressMaterial::recvSelf (int commitTag, Channel & theChannel,
    ...
    if (theMaterial == 0 || theMaterial->getClassTag () != matClassTag)
      {
          ...
          if (theMaterial == 0)
            {
                opserr <<
                    "PlaneStressMaterial::recvSelf() - failed to get a material of type: "
                    << matClassTag << endln;
                return -1;
            }
      }
