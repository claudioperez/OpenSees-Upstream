...
Matrix
DOF_Group::errMatrix (1, 1);
Vector
DOF_Group::errVect (1);
Matrix **
    DOF_Group::theMatrices;     // array of pointers to class wide matrices
Vector **
    DOF_Group::theVectors;      // array of pointers to class widde vectors
int
DOF_Group::numDOFs (0);         // number of objects
...
DOF_Group::addPIncInertiaToUnbalance (double fact)
{
    if (myNode != 0)
      {
          if (unbalance->
              addVector (1.0, myNode->getUnbalancedLoadIncInertia (),
                         fact) < 0)
            {
...
DOF_Group::addLocalM_Force (const Vector & accel, double fact)
{
    if (myNode != 0)
      {
          if (unbalance->
              addMatrixVector (1.0, myNode->getMass (), accel, fact) < 0)
            {
...
DOF_Group::addM_ForceSensitivity (const Vector & Udotdot, double fact)
{
    ...
    if (unbalance->
        addMatrixVector (1.0, myNode->getMassSensitivity (), accel, fact) < 0)
      {
          opserr << "DOF_Group::addM_Force() ";
          opserr << " invoking addMatrixVector() on the unbalance failed\n";
      }
...
DOF_Group::addD_ForceSensitivity (const Vector & Udot, double fact)
{
    ...
    if (unbalance->
        addMatrixVector (1.0, myNode->getDampSensitivity (), vel, fact) < 0)
      {
