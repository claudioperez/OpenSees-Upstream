...
Vector
BeamFiberMaterial2d::stress (2);
Matrix
BeamFiberMaterial2d::tangent (2, 2);
...
strain (2)
{
    ...
    if (theMaterial == 0)
      {
          opserr <<
              "BeamFiberMaterial2d::BeamFiberMaterial2d -- failed to get copy of material\n";
          exit (-1);
      }
}
    ...
    do
      {
          ...
          if (theMaterial->setTrialStrain (threeDstrain) < 0)
            {
                opserr <<
                    "BeamFiberMaterial2d::setTrialStrain - setStrain failed in material with strain "
                    << threeDstrain;
                return -1;
            }
...
BeamFiberMaterial2d::sendSelf (int commitTag, Channel & theChannel)
{
    ...
    if (res < 0)
      {
          opserr <<
              "BeamFiberMaterial2d::sendSelf() - failed to send id data\n";
          return res;
      }
    ...
    if (res < 0)
      {
          opserr <<
              "BeamFiberMaterial2d::sendSelf() - failed to send vector data\n";
          return res;
      }
    ...
    if (res < 0)
        opserr <<
            "BeamFiberMaterial2d::sendSelf() - failed to send vector material\n";

    return res;
}

int
BeamFiberMaterial2d::recvSelf (int commitTag, Channel & theChannel,
    ...
    if (theMaterial == 0 || theMaterial->getClassTag () != matClassTag)
      {
          ...
          if (theMaterial == 0)
            {
                opserr <<
                    "BeamFiberMaterial2d::recvSelf() - failed to get a material of type: "
                    << matClassTag << endln;
                return -1;
            }
      }
