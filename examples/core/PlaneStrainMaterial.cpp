...
Vector
PlaneStrainMaterial::stress (3);
Matrix
PlaneStrainMaterial::tangent (3, 3);

//null constructor
PlaneStrainMaterial::PlaneStrainMaterial ():
NDMaterial (0, ND_TAG_PlaneStrainMaterial), strain (3)
{
}


//full constructor
PlaneStrainMaterial::PlaneStrainMaterial (int tag,
...
PlaneStrainMaterial::getCopy ()
{
    ...
    clone = new PlaneStrainMaterial (this->getTag (), *theMaterial);    //make the copy
...
PlaneStrainMaterial::setTrialStrain (const Vector & strainFromElement)
{
    ...
    if (theMaterial->setTrialStrain (threeDstrain) < 0)
      {
          opserr <<
              "PlaneStrainMaterial::setTrialStrain() - setTrialStrain in material failed with strain "
              << threeDstrain;
          return -1;
      }
...
PlaneStrainMaterial::getInitialTangent ()
{
    ...
    static Matrix
    threeDtangentCopy (6, 6);

    //three dimensional tangent 
    const
        Matrix &
        threeDtangent = theMaterial->getInitialTangent ();
...
PlaneStrainMaterial::sendSelf (int commitTag, Channel & theChannel)
{
    ...
    if (res < 0)
      {
          opserr <<
              "PlaneStrainMaterial::sendSelf() - failed to send id data\n";
          return res;
      }
    ...
    if (res < 0)
        opserr <<
            "PlaneStrainMaterial::sendSelf() - failed to send vector material\n";

    return res;
}

int
PlaneStrainMaterial::recvSelf (int commitTag, Channel & theChannel,
    ...
    if (theMaterial == 0 || theMaterial->getClassTag () != matClassTag)
      {
          ...
          if (theMaterial == 0)
            {
                opserr <<
                    "PlaneStrainMaterial::recvSelf() - failed to get a material of type: "
                    << matClassTag << endln;
                return -1;
            }
      }
