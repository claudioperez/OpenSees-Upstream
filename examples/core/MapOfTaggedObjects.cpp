...
MapOfTaggedObjects::MapOfTaggedObjects ():myIter (*this)
{
...
MapOfTaggedObjects::setSize (int newSize)
{
    ...
    if (newSize > maxSize)
      {
          opserr <<
              "MapOfTaggedObjects::setSize - failed as map stl has a max size of "
              << maxSize << "\n";
          return -1;
      }
...
MapOfTaggedObjects::addComponent (TaggedObject * newComponent)
{
    ...
    if (theEle == theMap.end ())
      {
          ...
          if (theEle == theMap.end ())
            {
                opserr <<
                    "MapOfTaggedObjects::addComponent - map STL failed to add object with tag : "
                    << newComponent->getTag () << "\n";
                return false;
            }
      }
    ...
    // as the map template does not allow multiple entries wih the same tag
      ...
      {
          opserr <<
              "MapOfTaggedObjects::addComponent - not adding as one with similar tag exists, tag: "
              << newComponent->getTag () << "\n";
          return false;
      }
      ...
      {                         // the object exists so we remove it
          ...
          if (ok != 1)
            {                   // ensure the map did remove the object
                opserr <<
                    "MapOfTaggedObjects::removeComponent - map STL failed to remove object with tag "
                    << tag << "\n";
                return 0;
            }
      }
...
TaggedObjectIter & MapOfTaggedObjects::getComponents ()
{
    myIter.reset ();
    return myIter;
}
