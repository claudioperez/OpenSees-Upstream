    ...
    for (int i = 0; i <= sendSelfCount; i++)
      {
          ...
          if (i == 0)
          ...
          else
            {
                if (numColumns != 0)
                  {
                      theChannels[i - 1]->recvVector (0, 0,
                                                      *(theRemoteData[i]));
                  }
            }
      }
...
int
BinaryFileStream::recvSelf (int commitTag, Channel & theChannel,
...
binaryToText (const char *inputFilename, const char *outputFilename)
{
    ...
    if (input.bad ())
      {
          ...
          std::cerr << " - could not open file " << inputFilename << std::
              endl;
          output.close ();
          return -1;
      }
    if (output.bad ())
      {
          std::cerr << "WARNING - BinaryFileStream - binaryToText()";
          std::cerr << " - could not open file " << outputFilename << std::
              endl;
          output.close ();
          return -1;
      }
    ...
    while (!input.eof ())
      {
          ...
          if ((*c == '\n') && (intervalLF % 8 == 0))
            {
                aveIntervalLF =
                    (aveIntervalLF * nLF + intervalLF) / (nLF + 1);
                nLF++;
                intervalLF = 0;
            }
          else
