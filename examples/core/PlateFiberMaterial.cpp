...
Vector
PlateFiberMaterial::stress (5);
Matrix
PlateFiberMaterial::tangent (5, 5);
...
NDMaterial (0, ND_TAG_PlateFiberMaterial), strain (5)
{
...
PlateFiberMaterial::getCopy ()
{
    ...
    clone = new PlateFiberMaterial (this->getTag (), *theMaterial);     //make the copy
...
PlateFiberMaterial::sendSelf (int commitTag, Channel & theChannel)
{
    ...
    if (res < 0)
      {
          opserr <<
              "PlateFiberMaterial::sendSelf() - failed to send id data\n";
          return res;
      }
    ...
    if (res < 0)
      {
          opserr <<
              "PlateFiberMaterial::sendSelf() - failed to send vector data\n";
          return res;
      }
    ...
    if (res < 0)
        opserr <<
            "PlateFiberMaterial::sendSelf() - failed to send vector material\n";

    return res;
}

int
PlateFiberMaterial::recvSelf (int commitTag, Channel & theChannel,
    ...
    if (theMaterial == 0 || theMaterial->getClassTag () != matClassTag)
      {
          ...
          if (theMaterial == 0)
            {
                opserr <<
                    "PlateFiberMaterial::recvSelf() - failed to get a material of type: "
                    << matClassTag << endln;
                return -1;
            }
      }
