...
SumElementForcesRecorder::SumElementForcesRecorder ():Recorder (-1),
numEle (0), theElements (0), eleID (0),
theDomain (0), theOutput (0), echoTimeFlag (true), data (0)
{
...
SumElementForcesRecorder::record (int commitTag, double timeStamp)
{
    ...
    if (data == 0)
      {
          opserr <<
              "SumElementForcesRecorder::record() - setDomain() has not been called\n";
          return -1;
      }
...
SumElementForcesRecorder::setDomain (Domain & theDom)
{
    ...
    if (numEle == 0)
      {
          opserr <<
              "WARNING SumElementForcesRecorder::initialize() - no elements tags passed in input!\n";
          return 0;
      }
    ...
    if (theElements == 0)
      {
          opserr <<
              "WARNING SumElementForcesRecorder::initialize() - out of memory\n";
          ...
          if (theEle != 0)
            {
                ...
                if (sizeArray == -1)
                  ...
                  {
                      opserr << "WARNING: forces mismatch - element: " <<
                          eleTag << " will not be included\n";
                  }
                else
                  {
                      theElements[i] = theEle;
...
SumElementForcesRecorder::sendSelf (int commitTag, Channel & theChannel)
{
    ...
    if (theChannel.sendID (0, commitTag, eleID) < 0)
      {
          opserr <<
              "SumElementForcesRecorder::sendSelf() - failed to send idData\n";
          return -1;
      }
                                    ...
                                    FEM_ObjectBroker & theBroker)
{
    ...
    if (theOutput == 0)
      {
          opserr <<
              "SumElementForcesRecorder::recvSelf() - failed to get Output of correct type\n";
          return -1;
      }
    ...
    if (theChannel.recvID (0, commitTag, eleID) < 0)
      {
          opserr <<
              "SumElementForcesRecorder::recvSelf() - failed to recv eleID\n";
          return -1;
      }
