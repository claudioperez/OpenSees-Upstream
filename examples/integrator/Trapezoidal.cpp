...
Trapezoidal::Trapezoidal ():TransientIntegrator (0),
...
int
Trapezoidal::newStep (double deltaT)
{
    // check the vectors have been created
    if (U == 0)
      {
          opserr <<
              "Trapezoidal::newStep() - domainChange() failed or hasn't been called\n";
          return -3;
      }
...
int
Trapezoidal::formEleTangent (FE_Element * theEle)
{
    theEle->zeroTangent ();
...
int
Trapezoidal::formNodTangent (DOF_Group * theDof)
{
    theDof->zeroTangent ();
...
int
Trapezoidal::update (const Vector & deltaU)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel == 0)
      {
    ...
    if (Ut == 0)
      {
          opserr <<
              "WARNING Trapezoidal::update() - domainChange() failed or not called\n";
          return -2;
      }
    ...
    if (deltaU.Size () != U->Size ())
      {
          opserr <<
              "WARNING Trapezoidal::update() - Vectors of incompatible size ";
...
int
Trapezoidal::sendSelf (int cTag, Channel & theChannel)
{
    return 0;
}
...
int
Trapezoidal::recvSelf (int cTag, Channel & theChannel,
...
void
Trapezoidal::Print (OPS_Stream & s, int flag)
{
    AnalysisModel *theModel = this->getAnalysisModel ();
    if (theModel != 0)
      {
