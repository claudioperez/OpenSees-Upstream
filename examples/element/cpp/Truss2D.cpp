...
OPS_Truss2D ()
{
    ...
    if (numMyTruss == 0)
      {
          opserr <<
              "Truss2D element - Written by fmk UC Berkeley Copyright 2008 - Use at your Own Peril\n";
          numMyTruss++;
      }
    ...
    if (numRemainingArgs != 5)
      {
          opserr <<
              "ERROR - Truss2D not enough args provided, want: element Truss2D tag? iNode? jNode? Area? matTag?\n";
          numMyTruss++;
      }
    ...
    if (OPS_GetIntInput (&numData, &iData[3]) != 0)
      {
          opserr << "WARNING error reading element material tag for element "
              << eleTag << endln;
          return 0;
      }
    ...
    if (theMaterial == 0)
      {
          opserr << "WARNING material with tag " << matID <<
    ...
    theTruss =
        new Truss2D (eleTag, iData[1], iData[2], *theMaterial, dData[0]);

    if (theTruss == 0)
      {
          opserr << "WARNING ran out of memory creating element with tag " <<
              eleTag << endln;
          delete theMaterial;
          return 0;
      }
...
Truss2D::Truss2D (int tag,
                  int Nd1, int Nd2, UniaxialMaterial & theMat, double a):
Element (tag, 0),
externalNodes (2),
trans (1, 4),
L (0.0),
A (a)
{
    ...
    if (theMaterial == 0)
      {
          opserr <<
              "FATAL Truss2D::Truss2D() - out of memory, could not get a copy of the Material\n";
          exit (-1);
      }
    ...
    if (externalNodes.Size () != 2)
      {
          opserr <<
              "FATAL Truss2D::Truss2D() - out of memory, could not create an ID of size 2\n";
          exit (-1);
      }
...
Truss2D::Truss2D ():Element (0, 0),
theMaterial (0),
externalNodes (2), trans (1, 4), L (0.0), A (0.0)
{
    theNodes[0] = 0;
    theNodes[1] = 0;
}
...
Truss2D::setDomain (Domain * theDomain)
{
    ...
    if (end1Ptr == 0)
      {
          opserr << "WARNING Truss2D::setDomain() - at truss " << this->
              getTag () << " node " << Nd1 << "  does not exist in domain\n";
    ...
    if (end2Ptr == 0)
      {
          opserr << "WARNING Truss2D::setDomain() - at truss " << this->
              getTag () << " node " << Nd2 << "  does not exist in domain\n";
    ...
    if (L == 0.0)
      {
          opserr << "WARNING Truss2D::setDomain() - Truss2D " << this->
              getTag () << " has zero length\n";
...
{
    if (L == 0.0)
      {                         // length = zero - problem in setDomain() warning message already printed
          trussK.Zero ();
          return trussK;
      }
...
{
    if (L == 0.0)
      {                         // if length == 0, problem in setDomain()
          trussR.Zero ();
          return trussR;
      }
...
Truss2D::sendSelf (int commitTag, Channel & theChannel)
{
    ...
    if (res < 0)
      {
          opserr <<
              "WARNING Truss2D::sendSelf() - failed to send the Material\n";
          return -3;
      }
...
int
Truss2D::recvSelf (int commitTag, Channel & theChannel,
                   FEM_ObjectBroker & theBroker)
{
    ...
    if (res < 0)
      {
          opserr <<
              "WARNING Truss2D::recvSelf() - failed to receive Vector\n";
          return -1;
      }
    ...
    if (theMaterial == 0)
      {
          opserr <<
              "WARNING Truss2D::recvSelf() - failed to create a Material\n";
          return -3;
      }
    ...
    if (res < 0)
      {
          opserr <<
              "WARNING Truss2D::recvSelf() - failed to receive the Material\n";
          return -3;
      }
...
Truss2D::setResponse (const char **argv, int argc, OPS_Stream & output)
{
    ...
    if (strcmp (argv[0], "force") == 0 || strcmp (argv[0], "forces") == 0 ||
        ...
        || strcmp (argv[0], "globalForces") == 0)
      {
          ...
          theResponse =
              new ElementResponse (this, 1, this->getResistingForce ());
      }
             ...
             || strcmp (argv[0], "dampingForces") == 0)
      {
          ...
          theResponse =
              new ElementResponse (this, 2, this->getResistingForce ());
      }
