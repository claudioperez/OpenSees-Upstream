...
```{.cpp}
int Solve(const Matrix &b, G3MAtrix &x);
```
...
*int addMatrixProduct(double factThis, const Matrix &A, const Matrix &B,
double factOther);*

*int addMatrixTripleProduct(double factThis, const Matrix &A, const
Matrix &B, double factOther);*

*int addMatrixTripleProduct(double factThis, const Matrix &T, const
Matrix &B, double factOther);*
...
returned, `Zero()`{.cpp} is called on the Matrix.\
...
current value plus *fact* times the value of the matrix `M`{.cpp} at location
...
current Matrix and Vector `V`{.cpp} are not compatible, i.e. V.Size() is not
...
Vector *V*. If the current Matrix and Vector `V`{.cpp} are not compatible,
...
as an argument to the constructor. The `addMatrix()`{.cpp} method is then
...
an argument to the constructor. The `addMatrix()`{.cpp} method is then invoked
...
is returned. The method tests for the type of the current Matrix, to see
whether the performance can be improved by avoiding having to call the
overloaded (i,j) operators, if `M`{.cpp} is of type genMatrix.
...
avoiding having to call the overloaded (i,j) operators, if `M`{.cpp} is of
type genMatrix.\
...
A function to print out the contents of the Matrix `M`{.cpp} to the output
stream *s*. Does this by invoking `Output()`{.cpp} on the Matrix *M*.\
