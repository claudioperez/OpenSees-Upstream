...
```{.cpp}
double operator $\hat{ }$(const Vector &V) const;
```
...
returned. The `Zero()`{.cpp} method is invoked on the new Vector before it is
returned.\
...
plus *fact* times the value of the Vector `V`{.cpp} at location (*i*). returns
$0$ if successful. A warning message is printed for each invalid
location in the current Vector or `V`{.cpp} and a $-1$ is returned.\

```{.cpp}
void addVector(const Vector &other, double fact = 1.0);
```
...
Returns the 2 norm of the Vector. Returns the `sqrt()`{.cpp} of the result of
...
A method to add the contents of the Vector `V`{.cpp} to the current Vector. If
...
A method to subtract the contents of the Vector `V`{.cpp} from the current
...
current Vector and `V`{.cpp} are not of the same size, a warning message is
...
*V*. If the current Vector and `V`{.cpp} are not of the same size, a warning
...
*M.noRows()* x `M.noRows()`{.cpp} and set equal to `M`{.cpp} if `M`{.cpp} is square, or
$M^tM$ if `M`{.cpp} is not square. The new Vector is then set equal to the
...
A function to print out the contents of the Vector `V`{.cpp} to the output
...
A function to read the contents of the Vector `V`{.cpp} from the input stream
*s*. Sets the components of `V`{.cpp} equal to the next `V.Size()`{.cpp} entries in
the stream.\
