...
the domain invokes *setDomain(0)* on the element and
...
is to be removed the domain invokes *setDomain(0)* on the node and
...
pointer to the load case that was removed. Invokes *setDomain(0)* on the
...
domain invokes *setDomain(0)* on the constraint and
