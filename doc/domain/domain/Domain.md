...
Otherwise the domain invokes *setDomain(0)* on the element (using a cast
...
node is to be removed the domain invokes *setDomain(0)* on the node and
...
was not in the domain, otherwise the domain invokes *setDomain(0)* on
...
invoking *setDomain(0)* on these objects. Returns $0$ if the load was
not in the domain, otherwise returns a pointer to the load that was
removed. Invokes *setDomain(0)* on the load case before it is returned.\
\
\
...
which have been added to the domain.\
\
To print the state of the domain. The domain invokes *Print(s,flag)* on
