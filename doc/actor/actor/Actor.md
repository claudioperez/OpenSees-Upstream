...
processes through a channel object, `theChannel`{.cpp} and which uses
*theBroker* to receive movable objects sent from other processes. The
subclass will be able to add `numMethods`{.cpp} actor methods using
*addMethod()* call.\
The base classes constructor invokes `setUpActor()`{.cpp} on *theChannel*
object. It then sets the Address of the remote shadow object which
created the actor process by invoking `getLastSendersAddress()`{.cpp} on
*theChannel*.
...
invoked by the actor on invocation of `processMethod()`{.cpp} with `tag` as
...
less than `numActorMethods`{.cpp} the function is added and $0$ is returned,
...
\*theAddress =0);*\
A method which will send `theObject`{.cpp} through the actors channel either
to the address given by `theAddress`{.cpp} or to the address of the shadow
...
\*theAddress =0);*\
A method which will receive `theObject`{.cpp} from the actors channel either
from the address given by `theAddress`{.cpp} or from the address of the shadow
object that created the actor if no address is specified.
...
\*theAddress =0);*\
A method which will send the data in the message `theMessage`{.cpp} through
the actors channel either to the address given by `theAddress`{.cpp} or to the
...
A method which will receive the data in the message `theMessage`{.cpp} from
the actors channel either from the address given by `theAddress`{.cpp} or from
...
=0);*\
A method which will send `theMatrix`{.cpp} through the actors channel either
to the address given by `theAddress`{.cpp} or to the address of the shadow
object that created the actor if no address is specified.
...
A method which will receive `theMatrix`{.cpp} from the actors channel either
from the address given by `theAddress`{.cpp} or from the address of the shadow
object that created the actor if no address is specified.
...
=0);*\
A method which will send `theVector`{.cpp} through the actors channel either
to the address given by `theAddress`{.cpp} or to the address of the shadow
object that created the actor if no address is specified.
...
A method which will receive `theVector`{.cpp} from the actors channel either
from the address given by `theAddress`{.cpp} or from the address of the shadow
object that created the actor if no address is specified.
...
A method which will send `theID`{.cpp} through the actors channel either to
the address given by `theAddress`{.cpp} or to the address of the shadow object
...
A method which will receive `theID`{.cpp} from the actors channel either from
the address given by `theAddress`{.cpp} or from the address of the shadow
object that created the actor if no address is specified.
