...
The method `setTrialStrain()`{.cpp} takes one or two arguments, an updated
...
and strain rate of this UniaxialMaterial. The method `getStrain()`{.cpp} is
pure virtual, while `getStrainRate()`{.cpp} is only virtual; by default it
...
The `getStress()`{.cpp} method is pure virtual and must be implemented by
...
The `getTangent()`{.cpp} is also pure virtual and must be implemented in all
UniaxialMaterial subclasses.

The `getDampTangent()`{.cpp} method is next, and is to return the current
...
Finally, the `getSecant()`{.cpp} method is provided to return the material
...
variables. First, the method `commitState()`{.cpp} is invoked to inform a
...
accordingly. Next, the method `revertToLastCommit()`{.cpp} is provided to let
a UniaxialMaterial object know that it should return to its last
committed state at. Finally, `revertToStart()`{.cpp} informs the
...
The `getCopy()`{.cpp} method is declared so a calling object, be it an
...
The final two methods, `setResponse()`{.cpp} and *getResponse()*, are declared
...
HardeningMaterial object. The method `commitState()`{.cpp} sets the committed
...
The `getCopy()`{.cpp} method is defined so this HardeningMaterial object can
...
Calls are made to the Tcl routines `Tcl_GetInt`{.cpp} and `Tcl_GetDouble`{.cpp} to
...
determination. First, `setTrialStrain()`{.cpp} stores the trial strain, then
...
*getStrain()*, `getStress()*, and *getTangent()`{.cpp} return the strain,
...
FedeasMaterial object. The `commitState()`{.cpp} method copies the trial
...
set to $0.0$ in the `revertToStart()`{.cpp} method.
...
The `Print()`{.cpp} method outputs the name of this FedeasMaterial object to
...
The method `invokeSubroutine()`{.cpp} calls the appropriate subroutine based
...
$0.0$ in `revertToStart()*, the *getCopy()`{.cpp} method must also be
overridden to return a pointer to the subclass. If `getCopy()`{.cpp} is not
...
to the `getNewUniaxialMaterial()`{.cpp} method in FEM_ObjectBroker in order
