...
reference="eq:e=av"}, the method `setTrialSectionDeformation()`{.cpp} may be
...
method `getStressResultant()`{.cpp} must be invoked. To perform the
transformation from basic to global resisting force
(equation [\[eq:p=p(q,u)\]](#eq:p=p(q,u)){reference-type="ref"
reference="eq:p=p(q,u)"}), the method `getGlobalResistingForce()`{.cpp} should
be invoked.
...
stiffness, ${\bf k}_b$, the method `getGlobalStiffMatrix()`{.cpp} should be
